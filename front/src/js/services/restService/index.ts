...
import onRequestStart from 'src/js/services/restService/onRequestStart';
import onRequestEnd from 'src/js/services/restService/onRequestEnd';

export type FetchOptions = NonNullable<
  ConstructorParameters<typeof RestClient>[0]['fetchOptions']
>;
...
export class RestService extends WithRUMInteractionName {
  ...
  constructor(config: RestClientConfig, name: string) {
    super(name);
    this.config = config;
    this.config.authType = RestClient.AuthConstants.SIMPLE_AUTH;
    const { sandbox } = this.config;
    const { performance } = sandbox;
    this.config.apiKey = (this.config.apiKey ||
      sandbox?.pluginConfig?.extendedProperties?.API_KEY) as string;

    if (!this.config.onRequestStart) {
      this.config.onRequestStart = onRequestStart(performance).bind(this);
    }

    if (!this.config.onRequestEnd) {
      this.config.onRequestEnd = onRequestEnd(performance).bind(this);
    }
  }
