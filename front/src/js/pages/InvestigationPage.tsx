...
const InvestigationPage: React.FC<InvestigationPageProps> = ({
  ...
  const api = useMemo(() => new GAIAService(sandbox, false), [sandbox]);
  ...
  const [selectedInputType, setSelectedInputType] = useState<'userId' | 'deviceId'>('userId');
  ...
  const updateAgentDetails = async (
    ...
    if (stepId === InvestigationStepId.NETWORK) {
      ...
      hasValidRiskAssessment =
        ...
        Object.keys(deviceResponse.device_signal_risk_assessment).length > 0;
  ...
  const handleInvestigatorSend = async (text: string) => {
    ...
    setInvestigatorComments((prev) => [
      ...prev,
      { ...message, investigationId: investigationIdState, entityId, entityType },
    ]);
    try {
  ...
  return (
    <div className="h-full bg-gray-50 relative flex flex-row min-h-0">
      <CommentSidebar
        ...
        messages={
          selectedCommentRole === 'Investigator' ? investigatorComments : policyComments
        }
        onSend={
          selectedCommentRole === 'Investigator' ? handleInvestigatorSend : handlePolicySend
        }
          ...
          <div className="flex gap-2 h-full min-h-0">
            <div className="flex-1 transition-all duration-300 h-full min-h-0">
              <div className="bg-white rounded-lg h-full shadow-md p-6 flex flex-col min-h-0">
                ...
                {/* Splitter layout for risk scores and steps */}
                <div
                  style={{
                    display: 'flex',
