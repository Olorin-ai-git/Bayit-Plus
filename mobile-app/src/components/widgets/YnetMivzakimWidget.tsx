/**
 * YnetMivzakimWidget Component (Mobile)
 *
 * Displays Ynet breaking news (mivzakim) with auto-refresh.
 * Mobile adaptation of the web YnetMivzakimWidget.
 */
...
const moduleLogger = logger.scope('YnetMivzakimWidget');
...
export function YnetMivzakimWidget({
  ...
  const fetchNews = useCallback(async () => {
    try {
      setError(null);
      const response = await api.get(`/news/mivzakim?limit=${maxItems}`);
      const data = response.data as { items: NewsItem[] };
      setNews(data.items || []);
      setLastUpdate(new Date());
    } catch (err) {
      moduleLogger.error('Failed to fetch Ynet mivzakim', { error: err, maxItems });
      setError(t('widget.newsLoadError', 'Failed to load news'));
    } finally {
      setLoading(false);
    }
  }, [maxItems, t]);
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <View style={styles.headerTitleContainer}>
          <Text style={styles.headerTitle}>{t('widget.ynetBreakingNews', 'Ynet Breaking News')}</Text>
          {lastUpdate && (
