...
const FlowCard: React.FC<FlowCardProps> = ({ flow, onPress, getLocalizedName }) => {
  const { t } = useTranslation();
  const { isRTL, textAlign } = useDirection();
  const iconConfig = FLOW_ICONS[flow.name] || { icon: '▶️', colors: [colors.primary, colors.primaryDark] };
      ...
      <View style={[styles.flowCardContent, { flexDirection: isRTL ? 'row' : 'row-reverse' }]}>
        ...
        <View style={[styles.flowInfo, { alignItems: isRTL ? 'flex-start' : 'flex-end' }]}>
          <View style={[styles.flowHeader, { flexDirection: isRTL ? 'row' : 'row-reverse' }]}>
            <Text style={[styles.flowTitle, { textAlign }]}>{getLocalizedName(flow)}</Text>
            {isSystem && (
          ...
          {flow.description && (
            <Text style={[styles.flowDescription, { textAlign }]} numberOfLines={2}>
              {flow.description}
            </Text>
          )}

          <View style={[styles.flowFeatures, { flexDirection: isRTL ? 'row' : 'row-reverse' }]}>
            {flow.ai_enabled && (
            ...
            {flow.auto_play && (
              <View style={styles.featureBadge}>
                <Text style={styles.featureText}>▶️ {t('flows.autoPlay', 'Auto')}</Text>
              </View>
            )}
            {flow.items.length > 0 && (
              <View style={styles.featureBadge}>
...
export const FlowsScreenMobile: React.FC = () => {
  ...
  const getLocalizedName = useCallback((flow: Flow): string => {
      ...
      >
        ...
        <View style={[styles.header, { flexDirection: isRTL ? 'row' : 'row-reverse' }]}>
          ...
          <View style={styles.headerTextContainer}>
            <Text style={[styles.title, { textAlign }]}>{t('flows.title', 'Flows')}</Text>
        ...
        {activeFlow && (
          <Animated.View style={[styles.activeBanner, { transform: [{ scale: pulseAnim }] }]}>
            ...
            >
              <View style={[styles.activeBannerContent, { flexDirection: isRTL ? 'row' : 'row-reverse' }]}>
                <View style={styles.activeFlowIconContainer}>
                  <Text style={styles.activeFlowIcon}>
                    {FLOW_ICONS[activeFlow.name]?.icon || '▶️'}
                  </Text>
                </View>
                <View style={[styles.activeFlowInfo, { alignItems: isRTL ? 'flex-start' : 'flex-end' }]}>
            ...
            {selectedFlow && (
              <>
                ...
                <View style={styles.modalInfo}>
                  ...
                  <View style={styles.modalInfoRow}>
                    <Text style={styles.modalInfoLabel}>{t('flows.items', 'Items')}</Text>
                    <Text style={styles.modalInfoValue}>
