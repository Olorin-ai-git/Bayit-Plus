"""
Bayit+ Content Service.

Extracted Cloud Run service for content catalog, live TV, radio, podcasts,
audiobooks, EPG, subtitles, audio tracks, and trending routes. Connects to
the same MongoDB cluster and Redis instance as the monolith, but handles only
content endpoints with independent scaling and deployment.
...
SERVICE_TITLE = "Bayit+ Content Service"
...
async def lifespan(service_app: FastAPI):
    ...
    # Initialize Redis client (content caching, rate limiting)
    try:
