...
entity_type = "user_id"
parallel_execution = True  # Set to False for sequential execution
...
async def websocket_listener(investigation_id: str):
    ...
    # Try multiple investigation IDs - the real one will be generated by the system
    # We'll listen to a few potential IDs and see which one gets messages
    potential_ids = [investigation_id]

    # Since we don't know the real investigation ID yet, we'll try to connect to multiple
    # WebSocket endpoints or use a wildcard approach. For now, let's try the provided ID
    # and capture the real one from the message content.
...
print(f"\n🚀 Starting Autonomous Investigation for {entity_type}: {entity_id}")
print("=" * 60)

# Start autonomous investigation
print(f"\n1️⃣ Starting autonomous investigation...")
start_url = f"{BASE_URL}/agent/start/{entity_id}"
...
if not os.path.exists("DejaVuSans.ttf"):
    ...
    pdf.cell(0, 15, "Autonomous Investigation Report", ln=1, align="C")
    pdf.ln(5)
    ...
    pdf.cell(
        0,
        5,
        "Autonomous Investigation System - Real-time Fraud Detection",
        ln=1,
        align="C",
    )

    pdf_filename = f"autonomous_investigation_user_{entity_id}.pdf"
