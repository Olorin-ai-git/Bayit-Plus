...
async def execute_custom_prompt():
    async with httpx.AsyncClient() as client:
        response = await client.post(
            "http://localhost:3001/prompts/custom",
            json={
...
1. **Start Enhanced Server**:
   ```bash
   python app/mcp/enhanced_mcp_server.py
   ```
...
3. **Check Server Status**:
   ```bash
   curl http://localhost:3001/status/enhanced
   ```
...
4. Enhanced server runs on port 3001 (different from original)
...
1. **Server Not Responding**
   - Check if enhanced server is running on port 3001
