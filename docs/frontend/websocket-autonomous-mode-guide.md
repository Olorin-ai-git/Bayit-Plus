...
export function useAutonomousInvestigation(investigationId: string, parallel: boolean = true) {
  const messages = ref<WebSocketMessage[]>([]);
  const connectionStatus = ref<'connecting' | 'connected' | 'disconnected' | 'error'>('connecting');
  const ws = ref<WebSocket | null>(null);

  const progress = computed(() => {
    const progressMessages = messages.value.filter(m => m.type === 'investigation_progress');
    const completedMessage = messages.value.find(m => m.type === 'investigation_completed');
    
    if (completedMessage) {
      return {
        completedAgents: completedMessage.summary?.successful_agents || 5,
...
export const MessageLog: React.FC<MessageLogProps> = ({ 
  messages, 
  showTypes = ['agent_started', 'agent_completed', 'investigation_progress', 'error'] 
}) => {
  const filteredMessages = messages.filter(m => showTypes.includes(m.type));
...
### 1. Message Buffering
For high-frequency messages, implement buffering to avoid overwhelming the UI:

```typescript
const useBufferedMessages = (messages: WebSocketMessage[], bufferTime: number = 100) => {
...
### 2. Message Filtering
Filter messages on the frontend to reduce processing load:

```typescript
const useFilteredMessages = (messages: WebSocketMessage[], filters: string[]) => {
  return useMemo(() => {
    return messages.filter(message => filters.includes(message.type));
  }, [messages, filters]);
};
```
...
1. **WebSocket Connection Failed**
   - Check if the investigation ID is valid
   - Verify the WebSocket URL format
   - Ensure the server is running and accessible

2. **Missing Messages**
   - Check if WebSocket connected before investigation started
   - Verify the investigation is in autonomous mode with websocket_updates=true

3. **Duplicate Messages**
   - Implement message deduplication using timestamps or message IDs
