"""
Bayit+ Search Service.

Extracted Cloud Run service for search routes. Connects to the same
MongoDB cluster and Redis instance as the monolith, but handles only
search endpoints (~17 routes) with independent scaling and deployment.
...
SERVICE_TITLE = "Bayit+ Search Service"
...
async def lifespan(service_app: FastAPI):
    ...
    # Initialize Redis client (search caching, rate limiting)
    try:
