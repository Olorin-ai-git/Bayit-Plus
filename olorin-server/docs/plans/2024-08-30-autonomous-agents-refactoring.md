# Autonomous Agents Refactoring Plan

**Author**: Gil Klainert  
**Date**: 2024-08-30  
**Objective**: Refactor `app/service/agent/autonomous_agents.py` (943 lines) to comply with 200-line rule  
**Diagram**: [Link to Mermaid diagram](/docs/diagrams/autonomous-agents-refactoring-architecture.mermaid)

## Analysis of Current File Structure

The current `autonomous_agents.py` file contains:
- **Lines 45-375**: `AutonomousInvestigationAgent` base class (330 lines)
- **Lines 378-501**: `autonomous_network_agent` function (123 lines)  
- **Lines 504-627**: `autonomous_device_agent` function (123 lines)
- **Lines 630-753**: `autonomous_location_agent` function (123 lines)
- **Lines 756-879**: `autonomous_logs_agent` function (123 lines)
- **Lines 882-942**: Helper functions and context management (60 lines)

## Refactoring Strategy

### 1. **Base Agent Classes** (`base_agents.py` - ~180 lines)
- Core `AutonomousInvestigationAgent` class
...
- Concrete domain-specific autonomous agent functions:
...
- `AutonomousInvestigationAgent` class (core functionality)
...
- `autonomous_investigate()` - Core investigation workflow  
- `_create_investigation_prompt()` - Prompt generation
- `_parse_autonomous_result()` - Result parsing
...
```
app/service/agent/
├── autonomous_agents.py (REFACTORED - imports only, ~20 lines)
...
- All existing imports will continue to work through `autonomous_agents.py`
...
- Maintain all autonomous agent functionality
...
1. **Create base_agents.py** with core `AutonomousInvestigationAgent` class
...
5. **Refactor autonomous_agents.py** to import and re-export from new modules
