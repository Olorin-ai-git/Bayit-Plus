...
try:
    from app.service.agent.orchestration.hybrid.migration.feature_flags.flag_manager import (
        FeatureFlags,
        GraphType,
    )
    from app.service.agent.orchestration.hybrid.migration.graph_selection.graph_selector import (
        GraphSelector,
    )
    from app.service.agent.orchestration.hybrid.migration.rollback.rollback_triggers import (
        RollbackTriggers,
    )
...
class TestDeterministicGraphSelection:
    ...
    @pytest.mark.skipif(
        not graph_selection_imports_available,
        reason="Graph selection imports not available",
    )
    @pytest.mark.asyncio
    async def test_same_inputs_same_graph_determinism(self):
        ...
        with patch(
            "app.service.agent.orchestration.hybrid.migration.graph_selection.graph_selector.GraphBuilders"
        ) as mock_builders:
            # Setup mock
            ...
            test_cases = [
                {"investigation_id": "test-001", "entity_type": "ip"},
                {"investigation_id": "test-002", "entity_type": "user"},
                {"investigation_id": "test-003", "entity_type": "device"},
            ]
            ...
            for test_case in test_cases:
                ...
                for attempt in range(5):  # Test 5 identical runs
                    ...
                    selection_record = selector.get_selection_history(
                        test_case["investigation_id"]
                    )
                    graph_type = selection_record[test_case["investigation_id"]][
                        "graph_type"
                    ]
                    selection_reason = selection_record[test_case["investigation_id"]][
                        "selection_reason"
                    ]
                ...
                for i, result in enumerate(results[1:], 1):
                    assert (
                        result == first_result
                    ), f"Selection {i+1} differs from first: {result} vs {first_result}"

                print(
                    f"✅ Deterministic selection for {test_case['investigation_id']}: {first_result}"
                )
    ...
    async def test_rollback_trigger_determinism(self):
        ...
        with patch(
            ...
            for scenario in test_scenarios:
                ...
                with patch.object(
                    selector.rollback_triggers,
                    "should_rollback",
                    return_value=scenario["trigger_rollback"],
                ):
                    ...
                    for attempt in range(3):
                        ...
                        selection_reason = selection_record["test-rollback-001"][
                            "selection_reason"
                        ]
                    ...
                    assert all(
                        reason == scenario["expected_reason"] for reason in results
                    ), f"Inconsistent rollback behavior: {results}"

                    print(
                        f"✅ Rollback determinism: trigger={scenario['trigger_rollback']} → reason={scenario['expected_reason']}"
                    )
    ...
    async def test_feature_flag_determinism(self):
        ...
        with patch(
            ...
            for scenario in flag_scenarios:
                ...
                with patch.object(
                    selector.feature_flags,
                    "is_enabled",
                    return_value=scenario["hybrid_enabled"],
                ):
                    ...
                    with patch.object(
                        ...
                        with patch.object(
                            ...
                            for attempt in range(3):
                                ...
                                graph_type = selection_record["test-flag-001"][
                                    "graph_type"
                                ]
                            ...
                            assert all(
                                graph_type == expected_graph for graph_type in results
                            ), f"Inconsistent feature flag behavior: {results}, expected: {expected_graph}"

                            print(
                                f"✅ Feature flag determinism: enabled={scenario['hybrid_enabled']} → graph={expected_graph}"
                            )
    ...
    async def test_ab_test_assignment_consistency(self):
        ...
        with patch(
            ...
            with patch.object(
                selector.rollback_triggers, "should_rollback", return_value=False
            ):
                ...
                for investigation_id in test_investigation_ids:
                    ...
                    with patch.object(
                        ...
                        for attempt in range(3):
                            ...
                            selection_reason = selection_record[investigation_id][
                                "selection_reason"
                            ]
                            graph_type = selection_record[investigation_id][
                                "graph_type"
                            ]
                        ...
                        assert (
                            first_result[1] == "ab_test"
                        ), f"Expected ab_test reason, got {first_result[1]}"

                        print(
                            f"✅ A/B test consistency for {investigation_id}: {first_result}"
                        )
    ...
    async def test_priority_order_determinism(self):
        ...
        with patch(
            ...
            selector = GraphSelector()
            graph = await selector.get_investigation_graph(
                "test-001", force_graph_type=GraphType.ORCHESTRATOR
            )
            ...
            selector = GraphSelector()
            with patch.object(
                selector.rollback_triggers, "should_rollback", return_value=True
            ):
                with patch.object(
                    selector.ab_test_manager,
                    "get_ab_test_assignment",
                    return_value=GraphType.HYBRID,
                ):
                    with patch.object(
                        selector.feature_flags, "is_enabled", return_value=True
                    ):
            ...
            with patch.object(
                ...
                with patch.object(
                    selector.ab_test_manager,
                    "get_ab_test_assignment",
                    return_value=GraphType.ORCHESTRATOR,
                ):
                    with patch.object(
                        selector.feature_flags, "is_enabled", return_value=True
                    ):
            ...
            with patch.object(selector.feature_flags, "is_enabled", return_value=True):
                with patch.object(
                    ...
                    with patch.object(
                        ...
                        record = selector.get_selection_history("test-error-001")[
                            "test-error-001"
                        ]
                        ...
                        assert record["graph_type"] == "CLEAN"
                        print(
                            "✅ Error scenarios deterministically fall back to clean graph"
                        )
                ...
                with patch.object(
                    ...
                    with patch.object(
                        ...
                        with patch.object(
                            ...
                            record = selector.get_selection_history(investigation_id)[
                                investigation_id
                            ]
                            results.append(
                                (record["graph_type"], record["selection_reason"])
                            )
        ...
        with patch(
            ...
            with patch.object(
                ...
                for i in range(3):
                    ...
                    record = selector.get_selection_history(f"rollback-test-{i}")[
                        f"rollback-test-{i}"
                    ]
