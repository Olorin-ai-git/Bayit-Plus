        ...
        with get_db_session() as session:
            ...
            if state.progress_json:
                try:
                    ...
                    print(f"  📈 Key Fields:")
                    print(f"    percent_complete: {progress.get('percent_complete', 'N/A')}")
                    print(f"    current_phase: {progress.get('current_phase', 'N/A')}")
                    print(f"    status: {progress.get('status', 'N/A')}")
                    print(f"    tool_executions count: {len(progress.get('tool_executions', []))}")
                ...
                if state.status == InvestigationStatus.IN_PROGRESS or state.lifecycle_stage == LifecycleStage.IN_PROGRESS:
                    issues_found.append("progress_json is NULL but status is IN_PROGRESS - should be initialized")
            ...
            if state.status == InvestigationStatus.IN_PROGRESS and state.lifecycle_stage != LifecycleStage.IN_PROGRESS:
                issues_found.append(f"status is IN_PROGRESS but lifecycle_stage is {state.lifecycle_stage}")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python verify_and_fix_investigation.py <investigation_id> [--no-auto-fix]")
        print("Example: python verify_and_fix_investigation.py inv-1762520013758-c80qozw")
        sys.exit(1)
