...
def test_evidence_strength_function():
    ...
    try:
        ...
        def calculate_evidence_strength_simulation():
            ...
            class MockResult:
                def __init__(self):
                    ...
                    self.knowledge_retrieved = ['item1', 'item2', 'item3', 'item4', 'item5']
            ...
            if result.knowledge_retrieved:
                evidence_factors.append(len(result.knowledge_retrieved) * 0.1)  # 5 * 0.1 = 0.5
            
            # Entity evidence
            if result.related_entities:
                evidence_factors.append(len(result.related_entities) * 0.05)  # 4 * 0.05 = 0.2
        ...
        if result.evidence_strength == 0.4:
            ...
            print(f"❌ Evidence strength not capped correctly: {result.evidence_strength}")
            return False
    ...
    try:
        ...
        for computed, llm, description in test_cases:
            ...
            if discordance > 0.3:
                ...
                if expected_range[0] <= result <= expected_range[1] + 0.1:  # Allow for weighted average
