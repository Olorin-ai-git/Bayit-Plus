...
async def main():
    ...
    try:
        ...
        if dist_results:
            ...
            print(f"   Earliest transaction: {row.get('EARLIEST_TX') or row.get('earliest_tx')}")
            print(f"   Latest transaction: {row.get('LATEST_TX') or row.get('latest_tx')}")
    ...
    try:
        ...
        if recent_results:
            ...
            for i, row in enumerate(recent_results[:10]):
                ...
                print(f"   {i+1}. TX_ID={str(tx_id)[:20]}..., Date={tx_datetime}, IS_FRAUD_TX={status}, Decision={decision}, Email={str(email)[:30] if email else 'N/A'}...")
