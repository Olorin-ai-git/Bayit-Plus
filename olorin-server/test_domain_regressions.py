...
from app.service.agent.orchestration.domain.aggregator import (
    aggregate_domains,
    summarize_investigation,
)
from app.service.agent.orchestration.domain.domain_result import (
    DomainResult,
    validate_domain,
)
from app.service.agent.orchestration.domain.linter import (
    lint_investigation,
    lint_specific_user_regressions,
)
...
def test_logs_single_clean_transaction():
    ...
    assert (
        result.score <= 0.25
    ), f"Single clean transaction should cap at 0.25, got {result.score}"
...
def test_network_no_signals_scores_none():
    ...
    ), f"No network signals should result in score=None, got {result.score}"
...
def test_network_with_signals_has_score():
    ...
    print(
        f"✅ Network with signals: score={result.score}, signals={len(result.signals)}"
    )
...
def test_aggregation_hard_evidence_floors():
    ...
    logs = DomainResult(
        name="logs", score=0.20, status="OK", signals=["clean transaction"]
    )
    network = DomainResult(
        name="network", score=None, status="INSUFFICIENT_EVIDENCE", signals=[]
    )
    ...
    assert (
        final_risk >= 0.60
    ), f"Hard evidence should apply fraud floor >=0.60, got {final_risk}"
...
def test_aggregation_insufficient_evidence_blocks():
    ...
    assert (
        final_risk is None
    ), f"Insufficient evidence should block publication, got {final_risk}"
    assert "Insufficient corroborating evidence" in reason
    print(
        f"✅ Insufficient evidence blocking: final_risk={final_risk}, gating={gating}"
    )
...
def test_is_public_deterministic():
    ...
    print(
        f"✅ is_public deterministic: public={result1.is_public}, private={result2.is_public}, loopback={result3.is_public}"
    )
...
def test_domain_validation_catches_contradictions():
    ...
    assert (
        result.score is None
    ), "Validation should clear score when status=INSUFFICIENT_EVIDENCE"
...
def test_full_corrected_flow():
    ...
    # Generate summary
    summary = summarize_investigation(
        final_risk, gating, reason, [logs_result, network_result]
    )

    # Validate final results
    assert (
        logs_result.score is None or logs_result.score <= 0.25
    ), f"Logs score should be capped or None: {logs_result.score}"
    assert (
        network_result.score is None
    ), f"Network should have no score: {network_result.score}"
    ...
    critical_errors = [
        e
        for e in errors
        if any(p in e for p in ["float(None)", "out of bounds", "contradiction"])
    ]
    assert (
        len(critical_errors) == 0
    ), f"No critical errors should remain: {critical_errors}"

    print(
        f"✅ Full corrected flow: logs={logs_result.score}, network={network_result.score}, final={final_risk}"
    )
