...
def classify_transaction(predicted_risk: float, actual_outcome: Any, risk_threshold: float) -> str:
    """
...
async def analyze_investigation_confusion_matrix(investigation_id: str, risk_threshold: float):
    ...
    window_start = inv_dict.get('from_date') or inv_dict.get('start_time') or inv_dict.get('start')
    window_end = inv_dict.get('to_date') or inv_dict.get('end_time') or inv_dict.get('end')
        ...
        if settings_json:
            ...
            try:
                ...
                if isinstance(time_range, dict):
                    window_start = window_start or time_range.get('start_time') or time_range.get('from') or time_range.get('start')
                    window_end = window_end or time_range.get('end_time') or time_range.get('to') or time_range.get('end')
            except Exception as e:
                logger.debug(f"Error parsing settings_json: {e}")
    
    if not window_start or not window_end:
        print(f"⚠️ Investigation {investigation_id} has no time window (from_date/to_date)")
    ...
    for tx in transactions:
        ...
        classification = classify_transaction(predicted_risk, actual_outcome, risk_threshold)
    ...
    print("-" * 40)
    total_classified = sum(len(v) for k, v in classifications.items() if k != 'EXCLUDED')
    ...
    risk_scores = [tx.get('predicted_risk') for tx in transactions if tx.get('predicted_risk') is not None]
    if risk_scores:
        ...
        print(f"   Above threshold (>= {risk_threshold}): {above_threshold} transactions")
        print(f"   Below threshold (< {risk_threshold}): {below_threshold} transactions")
    
    # Analyze actual outcomes
    actual_outcomes = [tx.get('actual_outcome') for tx in transactions if tx.get('actual_outcome') is not None]
    ...
    if classifications['TP']:
        ...
        print(f"   → Investigation risk_score ({investigation_risk_score:.3f}) >= threshold ({risk_threshold})")
    ...
    if classifications['FP']:
        ...
        print(f"   → Investigation predicted fraud, but transactions were NOT actually fraud")
        print(f"   → Investigation risk_score ({investigation_risk_score:.3f}) >= threshold ({risk_threshold})")
        ...
        print(f"      - Investigation identified suspicious patterns that didn't result in confirmed fraud")
    ...
    for class_type in ['TP', 'FP', 'TN', 'FN']:
        if classifications[class_type]:
            ...
            print(f"{'TX ID':<20} {'Predicted Risk':<20} {'Actual Outcome':<20} {'Predicted Label':<20} {'Reason'}")
            print("-" * 100)
            ...
            for tx_info in classifications[class_type][:10]:  # Show first 10
                ...
                print(f"{str(tx_id):<20} {str(predicted_risk):<20} {str(actual_outcome):<20} {predicted_label:<20} {reason}")
            
            if len(classifications[class_type]) > 10:
                print(f"... and {len(classifications[class_type]) - 10} more {class_type} transactions")
    ...
    if classifications['EXCLUDED']:
        ...
        for tx_info in classifications['EXCLUDED'][:5]:  # Show first 5
            ...
            print(f"{str(tx_id):<20} {str(predicted_risk):<20} {str(actual_outcome):<20} {reason}")
...
async def analyze_recent_investigations(limit: int = 3):
    ...
    completed = [inv for inv in investigations 
                 if (inv.get('status') if isinstance(inv, dict) else getattr(inv, 'status', None)) == 'completed']
    ...
    completed.sort(key=lambda x: (
        x.get('completion_time') if isinstance(x, dict) else getattr(x, 'completion_time', None)
    ) or '', reverse=True)
    ...
    risk_threshold = float(os.getenv('RISK_THRESHOLD_DEFAULT', EVAL_DEFAULTS.get('RISK_THRESHOLD_DEFAULT', 0.3)))
    
    results = []
    for inv in recent:
        inv_id = inv.get('id') if isinstance(inv, dict) else getattr(inv, 'id', None)
        if inv_id:
            result = await analyze_investigation_confusion_matrix(inv_id, risk_threshold)
            if result:
    ...
    if results:
        ...
        if total_classified > 0:
            print(f"{'TP (True Positives)':<20} {total_tp:<15} {total_tp/total_classified*100:.1f}%")
            print(f"{'FP (False Positives)':<20} {total_fp:<15} {total_fp/total_classified*100:.1f}%")
            print(f"{'TN (True Negatives)':<20} {total_tn:<15} {total_tn/total_classified*100:.1f}%")
            print(f"{'FN (False Negatives)':<20} {total_fn:<15} {total_fn/total_classified*100:.1f}%")
