...
async def test_tools():
    ...
    print("="*80)
    device_tool = next((t for t in tools if t.name == "device_fingerprint_analysis"), None)
    if device_tool:
        try:
    ...
    # AbuseIPDB
    abuseipdb_tool = next((t for t in tools if "abuseipdb" in t.name.lower() and "ip_reputation" in t.name.lower()), None)
    if abuseipdb_tool:
        try:
    ...
    vt_ip_tool = next((t for t in tools if "virustotal" in t.name.lower() and "ip" in t.name.lower() and "domain" not in t.name.lower()), None)
    if vt_ip_tool:
        try:
    ...
    vt_domain_tool = next((t for t in tools if "virustotal" in t.name.lower() and "domain" in t.name.lower()), None)
    if vt_domain_tool:
        try:
            print(f"🔍 Testing {vt_domain_tool.name}...")
            result = await vt_domain_tool.ainvoke({
                "domain": "example.com"
            })
            print(f"✅ Result: {result[:200]}...")
        except Exception as e:
    ...
    shodan_tool = next((t for t in tools if "shodan" in t.name.lower() and "infrastructure" in t.name.lower()), None)
    if shodan_tool:
        try:
