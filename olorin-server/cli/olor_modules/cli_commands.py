...
class CLICommands:
    ...
    def add_file_to_investigation(
        self, investigation_id: str, file_path: Path, file_kind: Optional[str]
    ) -> None:
    ...
    def generate_report(
        ...
        if not output_path:
            output_path = (
                self.workspace_root / "reports" / f"{investigation_id}_report.html"
            )
    ...
    def compare_investigations(
        ...
        if not output_path:
            output_path = (
                self.workspace_root
                / "comparisons"
                / f"{left_investigation}_vs_{right_investigation}.html"
            )
    ...
    def show_resource(
        self, investigation_id: Optional[str], comparison_id: Optional[str]
    ) -> None:
    ...
    def cleanup_workspace(self, yes: bool, files: bool, registry_only: bool) -> None:
