...
class CriticalIssuesValidator:
    ...
    def validate_authoritative_override_removal(self):
        ...
        base_file = (
            project_root / "app/service/agent/orchestration/domain_agents/base.py"
        )
    ...
    def validate_risk_fusion_logic(self):
        ...
        for file_path in files_to_check:
            ...
            if full_path.exists():
                ...
                if "0.4" in content and (
                    "cap" in content.lower() or "min(" in content or "max(" in content
                ):
            ...
            if full_path.exists():
                ...
                for line_num, line in enumerate(lines, 1):
                    if "float(" in line:
                        ...
                        if (
                            "try:" not in line
                            and "except" not in line_context
                            and "if" not in line_context
                            and "or 0" not in line_context
                        ):
                            # Look for potentially unsafe patterns
                            if (
                                "response_data[" in line
                                or "get(" in line
                                or "None" in line
                            ):
                                unsafe_conversions.append(
                                    (file_path, line_num, line_context)
                                )
