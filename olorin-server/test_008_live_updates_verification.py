...
from app.service.event_feed_models import (
    EventsFeedResponse, InvestigationEvent, EventFilterParams, AuditTrailSummary
)
...
def create_test_investigation_state(db: Session, investigation_id: str = "test-inv-001"):
...
def test_01_investigation_progress_service_real_data():
    ...
    assert progress.completed_tools == 1, f"Expected 1 completed, got {progress.completed_tools}"
    assert progress.running_tools == 1, f"Expected 1 running, got {progress.running_tools}"
    assert progress.queued_tools == 1, f"Expected 1 queued, got {progress.queued_tools}"
    print(f"✅ Tool counts correct: total={progress.total_tools}, completed={progress.completed_tools}")
    
    # VERIFY: Tool executions have REAL data
    assert len(progress.tool_executions) == 3, f"Expected 3 executions, got {len(progress.tool_executions)}"
    tool_exec = progress.tool_executions[0]
    assert tool_exec.tool_name == "device_analysis", f"Expected device_analysis, got {tool_exec.tool_name}"
    assert tool_exec.agent_type == "device", f"Expected device agent, got {tool_exec.agent_type}"
    assert tool_exec.status == "completed", f"Expected completed, got {tool_exec.status}"
    assert tool_exec.result is not None, "Tool result should not be None"
    assert tool_exec.result.risk_score == 45.5, f"Expected risk 45.5, got {tool_exec.result.risk_score}"
    assert len(tool_exec.result.findings) == 2, f"Expected 2 findings, got {len(tool_exec.result.findings)}"
    print(f"✅ Tool execution 1 data verified: {tool_exec.tool_name} ({tool_exec.status})")
    ...
    assert len(progress.entities) == 3, f"Expected 3 entities, got {len(progress.entities)}"
    ...
    print(f"✅ Entities verified: {len(progress.entities)} entities extracted from settings_json")
    
    # VERIFY: Completion percent is REAL data
    assert progress.completion_percent == 45, f"Expected 45%, got {progress.completion_percent}%"
...
def test_02_etag_generation_and_304():
    ...
    assert len(etag) == 34, f"ETag length invalid (expected 34 with quotes): {len(etag)}"
    ...
    etag_from_hash = hashlib.md5((progress_json + str(state.version)).encode()).hexdigest()
...
def test_03_event_pagination_cursor():
    ...
    for event in result.items:
        assert isinstance(event, InvestigationEvent), f"Event not InvestigationEvent: {type(event)}"
    ...
    if result.has_more:
        assert result.next_cursor is not None, "next_cursor should be set if has_more=True"
...
def test_04_event_filtering():
    ...
    assert result.items[0].op == "CREATED", f"Expected CREATED, got {result.items[0].op}"
...
def test_05_end_to_end_flow():
    ...
    progress = InvestigationProgressService.build_progress_from_state(state)
    progress_json = progress.model_dump_json()
    etag1 = f'"{hashlib.md5((progress_json + str(state.version)).encode()).hexdigest()}"'
    print(f"✅ Step 2: Got progress, ETag={etag1}")
    print(f"   - Status: {progress.status}")
    print(f"   - Completion: {progress.completion_percent}%")
    print(f"   - Tools: {progress.total_tools} (completed={progress.completed_tools}, running={progress.running_tools})")
    ...
    etag2 = f'"{hashlib.md5((progress_json2 + str(state.version)).encode()).hexdigest()}"'
...
def test_06_data_integrity():
    ...
    completed_count = sum(1 for te in progress.tool_executions if te.status == "completed")
    assert completed_count == progress.completed_tools, "Completed tool count mismatch"
    print(f"✅ Check 1: Tool statistics consistent (completed={progress.completed_tools})")
...
def run_all_tests():
    ...
    print("╔" + "="*78 + "╗")
    print("║" + " "*78 + "║")
    print("║" + "  🧪 COMPREHENSIVE TEST SUITE: 008-live-investigation-updates  ".center(78) + "║")
    print("║" + " "*78 + "║")
    print("╚" + "="*78 + "╝")
    
    tests = [
        ("Investigation Progress Service - REAL Data", test_01_investigation_progress_service_real_data),
