    ...
    try:
        with get_db_session() as session:
            ...
            if not investigations:
                ...
                print(f"{'='*80}\n")
                print(f"❌ No investigations found" + (f" with status '{status_filter}'" if status_filter else ""))
                return
            
            print(f"\n{'='*80}")
            print(f"📋 INVESTIGATIONS" + (f" (status: {status_filter})" if status_filter else ""))
            print(f"{'='*80}\n")
            ...
            for i, inv in enumerate(investigations, 1):
                ...
                print(f"   JSON: Settings={has_settings} Progress={has_progress} Results={has_results}")
                print()
