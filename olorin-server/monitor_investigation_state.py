...
def monitor_investigation_state(investigation_id: str, interval: int = 5, max_iterations: int = 12):
    ...
    for iteration in range(max_iterations):
        try:
            with get_db_session() as session:
                ...
                # Check for changes
                version_changed = previous_version is not None and state.version != previous_version
                ...
                print(f"  Version: {state.version}", "🔄 CHANGED" if version_changed else "")
                ...
                if state.progress_json:
                    try:
                        ...
                        print(f"    - tool_executions: {len(progress.get('tool_executions', []))}")
                        if progress.get('tool_executions'):
                            print(f"    - Tools:")
                            for te in progress.get('tool_executions', [])[:3]:
                                print(f"      • {te.get('tool_name', 'unknown')} ({te.get('status', 'unknown')})")
                        if progress_changed:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python monitor_investigation_state.py <investigation_id> [interval] [max_iterations]")
