...
async def run_investigation_with_health_check(investigation_id: str):
    ...
    try:
        ...
        if health_status.get("should_block_investigations"):
            ...
            error_msg = (
                f"Critical services down: {[f['service'] for f in critical_failures]}"
            )
        ...
        if health_status["status"] == "unhealthy":
            logger.warning(
                f"Some services have issues but investigation can proceed: {health_status['message']}"
            )
        else:
            logger.info(
                "✅ All critical services are healthy - investigation can proceed"
            )
...
async def run_comprehensive_service_validation():
    ...
    try:
        ...
        logger.info(
            f"Smoke test completed: {report.passed} passed, {report.failed} failed, {report.skipped} skipped"
        )
        ...
        for service in report.services:
            if service.overall_status.value == "passed":
                logger.info(
                    f"✅ {service.service_name}: All tests passed ({service.total_response_time_ms}ms)"
                )
            elif service.overall_status.value == "failed":
                logger.error(
                    f"❌ {service.service_name}: {len(service.failed_tests)} tests failed"
                )
        ...
        if report.should_block_investigations:
            logger.error(
                "🚫 CRITICAL: Investigations should be blocked due to service failures"
            )
            return False
        elif report.has_critical_failures:
            logger.warning(
                "⚠️  Some services have issues but investigations can proceed"
            )
            return True
        else:
    ...
    while True:
        try:
            ...
            if health_status["status"] == "healthy":
                ...
                logger.warning(
                    f"Service issues detected (failure #{consecutive_failures}): {health_status['message']}"
                )

                # Alert after 3 consecutive failures
                if consecutive_failures >= 3:
                    logger.error(
                        "🚨 ALERT: Multiple consecutive service failures detected"
                    )
...
async def validate_configuration_before_deployment():
    ...
    if os.getenv("USE_SNOWFLAKE", "false").lower() == "true":
        ...
        missing_fields = [
            field for field in required_fields if not snowflake_config.get(field)
        ]

        if missing_fields:
            validation_errors.append(
                f"Missing Snowflake configuration: {missing_fields}"
            )
        else:
    ...
    for service_name, api_key_var, enable_var in threat_intel_services:
        if os.getenv(enable_var, "false").lower() == "true":
            ...
            if not api_key:
                validation_errors.append(
                    f"Missing {service_name} API key: {api_key_var}"
                )
