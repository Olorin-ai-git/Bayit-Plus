...
async def test_mcp_tools():
        ...
        try:
            ...
            if isinstance(result_dict, dict):
                if result_dict.get("success"):
                    print(f"✅ Search successful - {result_dict.get('num_results', 0)} results")
                    print(f"   First result: {str(result_dict.get('results', [])[0])[:100] if result_dict.get('results') else 'N/A'}...")
                else:
                    print(f"❌ Search failed: {result_dict.get('error', 'Unknown error')}")
            else:
    ...
    print("="*80)
    crawl_tool = next((t for t in composio_tools if t.name == "composio_webcrawl"), None)
    if crawl_tool:
        try:
            ...
            if isinstance(result_dict, dict):
                if result_dict.get("success"):
                    ...
                    print(f"❌ WebCrawl failed: {result_dict.get('error', 'Unknown error')}")
            else:
                print(f"✅ Result: {str(result)[:200]}...")
        except Exception as e:
            print(f"❌ Error: {e}")
