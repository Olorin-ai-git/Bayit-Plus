...
def test_bulletproof_timer():
    ...
    from app.service.agent.orchestration.timing import run_timer, domain_timer
    ...
    assert state.total_duration_ms > 0, f"total_duration_ms is {state.total_duration_ms}"
    assert state.total_duration_ms >= 10, f"Duration too small: {state.total_duration_ms}ms"
    ...
    assert "domain_durations_ms" in state.performance_metrics, "domain_durations_ms not created"
    assert "test_domain" in state.performance_metrics["domain_durations_ms"], "test_domain duration not recorded"
...
def test_timing_fields():
    ...
    assert isinstance(state.total_duration_ms, int), f"total_duration_ms should be int, got {type(state.total_duration_ms)}"
    assert state.total_duration_ms > 0, f"total_duration_ms should be > 0, got {state.total_duration_ms}"
    
    # Verify timestamps make sense
    assert end_time > start_time, "end_time should be after start_time"
    
    print(f"   ✅ Timing fields: start={state.start_time[:19]}, end={state.end_time[:19]}, duration={state.total_duration_ms}ms")
