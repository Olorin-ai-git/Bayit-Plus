    ...
    try:
        ...
        try:
            ...
            mock_investigation_result = {
                ...
                'findings': {
                    'risk_score': 0.35,
                    'evidence': ['pattern_detection', 'anomaly_detection', 'behavioral_analysis'],
                    'confidence': 0.7
                },
                'domain_results': {
                    'network': {'risk_score': 0.3, 'evidence': ['ip_reputation'], 'confidence': 0.8},
                    'device': {'risk_score': 0.4, 'evidence': ['device_fingerprint'], 'confidence': 0.75},
                    'location': {'risk_score': 0.2, 'evidence': ['geo_analysis'], 'confidence': 0.9}
                },
                'metadata': {
                    'duration': 32.5,
            ...
            validation_result = await validator.validate_investigation_result(
                mock_investigation_result['investigation_id'],
                mock_investigation_result
            )
            
            print(f"✅ Validation completed: {validation_result.validation_status.value}")
            
            if validation_result.critical_issues:
                print(f"⚠️  Critical issues found: {len(validation_result.critical_issues)}")
        ...
        try:
            ...
            test_cases = [
                {"computed": 0.8, "llm": 0.2, "expected_discordance": True, "description": "High discordance"},
                {"computed": 0.3, "llm": 0.35, "expected_discordance": False, "description": "Low discordance"},
                {"computed": 0.9, "llm": 0.1, "expected_discordance": True, "description": "Very high discordance"}
            ]
            ...
            for case in test_cases:
                ...
                print(f"✅ {case['description']}: Computed={computed_score}, LLM={llm_score}")
