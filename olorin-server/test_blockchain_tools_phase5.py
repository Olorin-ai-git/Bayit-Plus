...
async def test_blockchain_tools_registration():
    ...
    try:
        from app.service.agent.tools.tool_registry import initialize_tools, get_blockchain_tools
...
async def test_blockchain_wallet_analysis():
    ...
    try:
        from app.service.agent.tools.blockchain_tools.blockchain_wallet_analysis import BlockchainWalletAnalysisTool
        ...
        if result['clustering_results']:
            logger.info(f"  Cluster Size: {result['clustering_results']['cluster_size']}")
...
async def test_cryptocurrency_tracing():
    ...
    try:
        from app.service.agent.tools.blockchain_tools.cryptocurrency_tracing import CryptocurrencyTracingTool
        ...
        logger.info(f"  Exchanges Identified: {result['summary']['exchanges_identified']}")
...
async def test_defi_protocol_analysis():
    ...
    try:
        from app.service.agent.tools.blockchain_tools.defi_protocol_analysis import DeFiProtocolAnalysisTool
        ...
        if result['attack_detection']['attacks_detected']:
            logger.info(f"  ⚠️  Attacks Detected: {len(result['attack_detection']['attacks_detected'])}")
...
async def test_nft_fraud_detection():
    ...
    try:
        from app.service.agent.tools.blockchain_tools.nft_fraud_detection import NFTFraudDetectionTool
        ...
        if result['authenticity_check']:
            ...
            logger.info(f"  Authenticity Score: {result['authenticity_check']['authenticity_score']}")
        
        if result['fraud_assessment']['detected_fraud_types']:
            logger.info(f"  Fraud Types: {result['fraud_assessment']['detected_fraud_types']}")
...
async def test_blockchain_forensics():
    ...
    try:
        from app.service.agent.tools.blockchain_tools.blockchain_forensics import BlockchainForensicsTool
        ...
        logger.info(f"  Evidence Items: {result['evidence_collection']['evidence_items']}")
        logger.info(f"  Forensics Standard: {result['forensics_standard']}")
        logger.info(f"  Chain of Custody ID: {result['chain_of_custody']['custody_id']}")
...
async def test_domain_objectives_blockchain_integration():
    ...
    try:
        ...
        blockchain_keywords = [
            "blockchain", "cryptocurrency", "crypto", "defi", "nft", 
            "forensics", "compliance", "darkweb", "tracing"
        ]
        ...
        for domain in domains:
            ...
            blockchain_mentions = [kw for kw in blockchain_keywords if kw in objectives_text]
        ...
        logger.info(f"  Domains with blockchain tools: {integrated_domains}/{len(domains)}")
        logger.info(f"  Integration coverage: {(integrated_domains/len(domains)*100):.1f}%")
        
        return integrated_domains >= 3  # At least 60% of domains should have blockchain integration
...
async def main():
    ...
    test_functions = [
        ...
        ("Domain Objectives Integration", test_domain_objectives_blockchain_integration)
    ]
    ...
    if failed == 0:
        logger.info("🎉 ALL PHASE 5 TESTS PASSED! Blockchain tools successfully integrated.")
        logger.info("✅ Phase 5: Blockchain & Cryptocurrency Intelligence Tools - COMPLETED")
    else:
        logger.warning(f"⚠️ {failed} Phase 5 tests failed. Please review the issues above.")
