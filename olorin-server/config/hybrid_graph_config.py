...
class HybridGraphConfig(BaseModel):
    ...
    max_duration_minutes: int = Field(
        ..., gt=0, le=60, description="Maximum investigation duration in minutes"
    )
    max_concurrent_per_user: int = Field(
        ..., gt=0, le=10, description="Maximum concurrent investigations per user"
    )
    history_retention_days: int = Field(
        ..., gt=0, le=365, description="Investigation history retention period in days"
    )

    # Polling configuration
    status_cache_ttl_seconds: int = Field(
        ..., gt=0, le=10, description="Status response cache TTL in seconds"
    )
    ...
    export_max_size_mb: int = Field(
        ..., gt=0, le=500, description="Maximum export file size in megabytes"
    )
    export_url_expiry_hours: int = Field(
        ..., gt=0, le=168, description="Export download URL expiry time in hours"
    )
    ...
    try:
        config = HybridGraphConfig(
            feature_enabled=os.environ.get(
                "FEATURE_ENABLE_HYBRID_GRAPH_POLLING", ""
            ).lower()
            == "true",
            feature_multi_entity_enabled=os.environ.get(
                "FEATURE_ENABLE_MULTI_ENTITY_INVESTIGATIONS", ""
            ).lower()
            == "true",
            max_duration_minutes=int(os.environ["INVESTIGATION_MAX_DURATION_MINUTES"]),
            max_concurrent_per_user=int(
                os.environ["INVESTIGATION_MAX_CONCURRENT_PER_USER"]
            ),
            history_retention_days=int(
                os.environ["INVESTIGATION_HISTORY_RETENTION_DAYS"]
            ),
            status_cache_ttl_seconds=int(
                os.environ["INVESTIGATION_POLLING_STATUS_CACHE_TTL_SECONDS"]
            ),
            status_query_timeout_ms=int(
                os.environ["INVESTIGATION_STATUS_QUERY_TIMEOUT_MS"]
            ),
            results_query_timeout_ms=int(
                os.environ["INVESTIGATION_RESULTS_QUERY_TIMEOUT_MS"]
            ),
            export_max_size_mb=int(os.environ["INVESTIGATION_EXPORT_MAX_SIZE_MB"]),
            export_url_expiry_hours=int(
                os.environ["INVESTIGATION_EXPORT_URL_EXPIRY_HOURS"]
            ),
