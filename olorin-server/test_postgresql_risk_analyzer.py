...
async def test_risk_analyzer():
    ...
    try:
        ...
        logging.getLogger(
            "app.service.agent.tools.database_tool.postgres_client"
        ).setLevel(logging.DEBUG)
        logging.getLogger(
            "app.service.agent.tools.database_tool.query_translator"
        ).setLevel(logging.DEBUG)

        results = await analyzer.get_top_risk_entities(
            time_window="24h", group_by=IP, top_percentage=10, force_refresh=True
        )
        ...
        if results.get("status") == "success":
            ...
            if entities:
                ...
                for i, entity in enumerate(entities[:5], 1):
                    print(
                        f'   {i}. IP: {entity.get("entity")}, Risk Score: {entity.get("risk_score"):.4f}'
                    )
            else:
