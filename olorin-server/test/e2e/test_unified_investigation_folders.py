...
def test_folder_creation():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert folder_name.startswith("MOCK_"), f"Folder should start with MOCK_, got: {folder_name}"
        assert investigation_id in folder_name, f"Folder should contain investigation_id, got: {folder_name}"
...
>>>>>>> 001-modify-analyzer-method
        ...
        assert folder_path.exists(), "Investigation folder should exist"
        folder_name = folder_path.name
        assert folder_name.startswith("LIVE_"), f"Folder should start with LIVE_, got: {folder_name}"
        ...
        interaction_id = logger.log_llm_interaction(
            ...
            tokens_used={"total_tokens": 100, "prompt_tokens": 50, "completion_tokens": 50},
...
def test_folder_listing_and_cleanup():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert len(all_investigations) == 3, f"Should have 3 investigations, got {len(all_investigations)}"
        
        # Test filtering by mode
        live_investigations = folder_manager.list_investigations(InvestigationMode.LIVE)
        assert len(live_investigations) == 1, f"Should have 1 LIVE investigation, got {len(live_investigations)}"
        ...
        assert len(mock_investigations) == 1, f"Should have 1 MOCK investigation, got {len(mock_investigations)}"
