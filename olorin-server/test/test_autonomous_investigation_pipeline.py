    ...
    try:
        ...
        if results:
            ...
            if "MODEL_SCORE" in sample_result:
                ...
                logger.info(
                    f"   🎯 Sample MODEL_SCORE: {model_score} (type: {type(model_score)})"
                )
                ...
                try:
                    ...
                    if float_score >= 0.8:
                        logger.info(
                            "   🚨 High-risk MODEL_SCORE detected - should trigger high risk score"
                        )
                        return True
                    else:
                        logger.warning(
                            f"   ⚠️ MODEL_SCORE {float_score} is not high-risk"
                        )
                        return False
...
async def test_domain_agents_processing():
    ...
    try:
        ...
        test_state = create_initial_state(
            investigation_id="test_001", entity_id="192.168.1.100", entity_type="ip"
        )
        ...
        logger.info(
            f"   📊 Test Snowflake data type: {type(test_state['snowflake_data'])}"
        )
        logger.info(
            f"   📋 Test results count: {len(test_state['snowflake_data']['results'])}"
        )
        ...
        if "domain_findings" in result:
            ...
            model_score_indicators = [
                ind for ind in risk_indicators if "Model fraud score" in ind
            ]
            if model_score_indicators:
                ...
                if risk_score >= 0.85:
                    ...
                    logger.error(
                        f"   ❌ RISK SCORE TOO LOW: Expected >= 0.85, got {risk_score}"
                    )
                    return False
            else:
...
async def test_final_risk_calculation():
    ...
    try:
        ...
        logger.info(
            f"   📊 Domain risk scores: network={test_state['domain_findings']['network']['risk_score']}, device={test_state['domain_findings']['device']['risk_score']}, location={test_state['domain_findings']['location']['risk_score']}"
        )
        ...
        if final_risk >= 0.8:
            ...
            logger.error(
                f"   ❌ FINAL RISK SCORE TOO LOW: Expected >= 0.8, got {final_risk}"
            )
            return False
...
async def test_json_parsing_in_orchestrator():
    ...
    try:
        ...
        logger.info(
            f"   📏 Mock content length: {len(mock_tool_message_content)} chars"
        )

        # Test the fix logic (from orchestrator_agent.py line 404)
        parsed_data = (
            json.loads(mock_tool_message_content)
            if isinstance(mock_tool_message_content, str)
            else mock_tool_message_content
        )
        ...
        if isinstance(parsed_data, dict) and "results" in parsed_data:
            ...
            if results and "MODEL_SCORE" in results[0]:
                ...
                logger.info(
                    f"   🎯 Parsed MODEL_SCORE: {model_score} (type: {type(model_score)})"
                )

                if isinstance(model_score, (int, float)) and model_score >= 0.9:
                    logger.info(
                        "   ✅ JSON PARSING SUCCESSFUL - High-risk data preserved"
                    )
                    return True
                else:
                    logger.error(
                        f"   ❌ MODEL_SCORE not preserved correctly: {model_score}"
                    )
                    return False
            else:
...
async def run_comprehensive_pipeline_test():
    ...
    if all_passed:
        ...
        failed_tests = [
            name for name, result in zip(test_names, test_results) if not result
        ]
