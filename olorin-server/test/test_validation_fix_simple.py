...
def test_original_bug_scenario():
    ...
    assert metrics.scenarios_passed == 0, f"❌ Expected 0 passed scenarios, got {metrics.scenarios_passed}"
    assert metrics.scenarios_failed == 1, f"❌ Expected 1 failed scenario, got {metrics.scenarios_failed}"
    ...
    if metrics.scenarios_failed == 0:
        ...
        final_status = f"Some tests failed: {metrics.scenarios_failed}/{metrics.scenarios_tested}"
        exit_code = 1
    
    assert exit_code == 1, f"❌ Expected exit code 1 (failure), got {exit_code}"
    assert "failed" in final_status.lower(), f"❌ Expected failure message, got '{final_status}'"
...
def test_low_quality_score_fix():
    ...
    # Verify the fix
    assert result.status == "failed", f"❌ Low quality investigation should be failed, got '{result.status}'"
    assert len(result.errors) > 0, "❌ Expected error message for low quality"
    assert "below acceptable threshold" in result.errors[0], "❌ Expected quality threshold error"
...
def test_high_quality_remains_completed():
    ...
    assert result.status == "completed", f"❌ High quality investigation should remain completed, got '{result.status}'"
    assert len(result.errors) == 0, "❌ High quality investigation should have no errors"
...
def test_comprehensive_fix_validation():
    ...
    # Verify results
    expected_statuses = ["failed", "failed", "completed", "failed"]  # Expected final statuses
    for i, (result, expected) in enumerate(zip(results, expected_statuses)):
        assert result.status == expected, f"❌ Result {i} expected '{expected}', got '{result.status}'"
    
    assert metrics.scenarios_passed == 1, f"❌ Expected 1 passed, got {metrics.scenarios_passed}"
    assert metrics.scenarios_failed == 3, f"❌ Expected 3 failed, got {metrics.scenarios_failed}"
    ...
    if metrics.scenarios_failed == 0:
        ...
        final_report = f"Some tests failed: {metrics.scenarios_failed}/{metrics.scenarios_tested}"
        exit_code = 1
    
    assert exit_code == 1, f"❌ Expected exit code 1, got {exit_code}"
    assert "failed" in final_report.lower(), f"❌ Expected failure in report: {final_report}"
    
    print(f"✅ Processed {metrics.scenarios_tested} scenarios")
    print(f"✅ Correctly identified {metrics.scenarios_passed} passed and {metrics.scenarios_failed} failed")
