...
import pytest
from app.service.agent.orchestration.risk.policy import derive_confirmed_fraud, fuse_risk, dedupe_recommendations
...
def test_canonical_is_public_overrides():
    ...
    result = canonical_is_public("8.8.8.8", vt_is_malicious=False, abuse_is_public=False)
...
def test_exculpatory_weight_limits():
    ...
    assert risk >= 0.6, f"High model score floor should resist exculpatory weight, got {risk}"
    
    risk = fuse_risk(0.99, [0.8], 1.0, True)  # Confirmed fraud + max exculpatory
    assert risk >= 0.9, f"Confirmed fraud floor should resist exculpatory weight, got {risk}"
...
def test_safe_comparison_helpers():
    ...
    from app.service.agent.orchestration.metrics.safe import safe_gt, coerce_float
...
def test_canonical_is_public_conflicts():
    ...
    result = canonical_is_public("192.168.1.1", vt_is_malicious=True, abuse_is_public=True)
...
def test_uncertainty_uplift_with_coverage():
    ...
    from app.service.agent.orchestration.risk.policy import coverage_score, uncertainty_uplift
    ...
    poor_domains = {
        "network": {"risk_score": None}  # All missing
    }
...
def test_anti_flap_guard_functionality():
    ...
    state_new_evidence = {
        "domain_findings": {"network": {"evidence": ["Initial evidence", "NEW evidence"]}},
        "tool_results": {"abuseipdb": {"data": "result"}, "virustotal": {"data": "new result"}},
        "snowflake_data": {"results": [{"MODEL_SCORE": 0.99}]}
    }
...
def test_comprehensive_regression_prevention():
    ...
    from app.service.agent.orchestration.risk.policy import (
        fuse_risk, risk_from_location, has_minimum_evidence, action_plan, derive_confirmed_fraud
    )
    from app.service.agent.orchestration.metrics.safe import safe_gt, fmt_num
    
    # Test 1: Risk freefall prevention
    high_model_low_domains = fuse_risk(0.99, [0.1, 0.2], 0.0, False)
    assert high_model_low_domains >= 0.6, f"Risk freefall detected: {high_model_low_domains}"
    
    # Test 2: Missing location data handling
    empty_location = risk_from_location({})
    assert empty_location is None, f"Missing location should be None, got {empty_location}"
    ...
    assert has_minimum_evidence(snowflake_only) == False, "Evidence gate should block Snowflake-only"
