...
from app.service.agent.orchestration.consistency_guardrails import uses_model_score
...
def test_comprehensive_validation_scenario():
    ...
    assert (
        result["final"] <= 0.40
    ), f"Discordant signals should cap at 0.40, got {result['final']}"
    assert result["status"] == "capped_for_low_evidence"

    # Evidence strength should be low (0.2-0.4 range)
    assert (
        0.2 <= result["evidence_strength"] <= 0.4
    ), f"Evidence strength should be 0.2-0.4, got {result['evidence_strength']}"
