...
def test_entity_consistency_validation():
    ...
    context = create_query_context(
        investigation_id="test_drift_001", entity_id="203.0.113.45", entity_type="ip"
    )
...
def test_query_formatting():
    ...
    context_with_quotes = create_query_context(
        investigation_id="test_quotes_001", entity_id="user's email", entity_type="text"
    )
    ...
    if sql_escaped == expected_escaped:
        ...
        print(
            f"   ❌ SQL escaping failed: expected '{expected_escaped}', got '{sql_escaped}'"
        )
        return False
