...
from app.service.agent.flow_continuity import ContinuityStrategy, FlowContinuityManager
...
class TestStructuredInvestigationOrchestrator:
    ...
    @pytest.mark.asyncio
    async def test_orchestrate_investigation_success(
        self, orchestrator, sample_investigation_data, mock_ai_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_ai_decision_making(
        self, orchestrator, sample_investigation_data, mock_ai_client
    ):
        ...
        with patch.object(orchestrator, "ai_client", mock_ai_client):
            ...
            decision = await orchestrator._make_ai_decision(
    ...
    @pytest.mark.asyncio
    async def test_orchestrator_resilience_on_ai_failure(
        self, orchestrator, sample_investigation_data
    ):
        ...
        with patch.object(orchestrator, "ai_client", mock_failing_client):
            ...
            assert any(
                "fallback" in decision.get("reasoning", "").lower()
                for decision in result.get("decisions", [])
            )

    @pytest.mark.asyncio
    async def test_investigation_phase_progression(
        self, orchestrator, sample_investigation_data, mock_ai_client
    ):
    ...
    async def test_agent_coordination_integration(
        ...
        mock_coordination_result = {
            ...
            "data_shared": {
                "risk_indicators": ["suspicious_login", "unusual_location"]
            },
        }
...
class TestAgentCoordination:
    ...
    async def test_agent_handoff_with_data_transfer(self, agent_coordinator):
        ...
        handoff_data = {
            ...
            "metadata": {"confidence": 0.8, "quality_score": 0.9},
        }
...
class TestFlowContinuity:
    ...
    @pytest.mark.asyncio
    async def test_investigation_recovery_from_checkpoint(
        self, flow_continuity_manager
    ):
    ...
    async def test_continuity_strategy_selection(self, flow_continuity_manager):
        ...
        strategy = await flow_continuity_manager.select_continuity_strategy(
            failure_context
        )
    ...
    async def test_partial_result_synthesis(self, flow_continuity_manager):
        ...
        assert (
            synthesized_result["completion_percentage"] < 100
        )  # Not complete due to failures
...
class TestOrchestratorResilience:
    ...
    async def test_universal_exception_transformation(self, exception_handler):
        ...
        transformed = await exception_handler.transform_exception(
            network_error, "network_operation", {"service": "device_analysis"}
        )
        ...
        transformed = await exception_handler.transform_exception(
            auth_error, "api_call", {"service": "ai_client"}
        )
    ...
    async def test_circuit_breaker_functionality(self, exception_handler):
        ...
        with pytest.raises(Exception) as exc_info:
            await exception_handler.execute_with_circuit_breaker(
                service_name, lambda: asyncio.sleep(0.1)  # Mock operation
            )
    ...
    async def test_exception_handling_with_fallback(self, exception_handler):
        ...
        result = await exception_handler.execute_with_fallback(
            primary_operation, fallback_operation, "test_operation"
        )
...
class TestOrchestratorMonitoring:
    ...
    async def test_metric_recording(self, monitoring_system):
        ...
        metrics_history = monitoring_system.metrics_history[
            MonitoringMetric.DECISION_LATENCY
        ]
...
class TestOrchestratorIntegration:
    ...
    @pytest.mark.asyncio
    async def test_end_to_end_investigation_orchestration(
        self, sample_investigation_data
    ):
        ...
        with patch.object(orchestrator, "ai_client") as mock_ai:
            ...
            assert (
                ...
                == sample_investigation_data["investigation_id"]
            )
    ...
    async def test_investigation_with_agent_failures(self, sample_investigation_data):
        ...
        with patch.object(orchestrator, "ai_client") as mock_ai:
            mock_ai.messages.create = AsyncMock(
                return_value=Mock(content=[Mock(text=json.dumps(mock_ai_response))])
            )
