...
class TestRAGEnhancedInvestigationAgent:
    ...
    def test_init_with_rag_enabled(self, mock_tools, mock_rag_orchestrator):
        ...
        with patch(
            "app.service.agent.rag_enhanced_agent.create_context_augmentor"
        ) as mock_create:
    ...
    def test_init_with_rag_disabled(self, mock_tools):
        ...
        agent = RAGEnhancedInvestigationAgent(
            domain="network", tools=mock_tools, enable_rag=False
        )
    ...
    def test_init_with_rag_failure(self, mock_tools):
        ...
        with patch(
            "app.service.agent.rag_enhanced_agent.get_rag_orchestrator",
            side_effect=Exception("RAG failed"),
        ):
            agent = RAGEnhancedInvestigationAgent(
                domain="network", tools=mock_tools, enable_rag=True
            )
    ...
    async def test_structured_investigate_rag_enhanced(
        self, mock_tools, mock_rag_orchestrator, mock_investigation_context
    ):
    ...
    async def test_structured_investigate_rag_failure_fallback(
        ...
        with patch("app.service.agent.rag_enhanced_agent.create_context_augmentor"):
            ...
            agent._rag_enhanced_investigation = AsyncMock(
                side_effect=Exception("RAG failed")
            )
            ...
            with patch.object(
                ...
                result = await agent.structured_investigate(
                    mock_investigation_context, config, None
                )
    ...
    async def test_structured_investigate_rag_disabled(
        self, mock_tools, mock_investigation_context
    ):
    ...
    async def test_rag_enhanced_investigation_full_flow(
        ...
        with patch(
            ...
            mock_augmentor = Mock(spec=ContextAugmentor)
            mock_augmentor.augment_investigation_context = AsyncMock(
            ...
            # Mock the methods
            agent._create_rag_enhanced_prompt = AsyncMock(
    ...
    async def test_create_rag_enhanced_prompt(
        ...
        with patch(
            ...
            mock_augmentor = Mock(spec=ContextAugmentor)
            mock_augmentor.inject_context_into_prompt = AsyncMock(
                return_value="enhanced prompt with context"
            )
            mock_create.return_value = mock_augmentor
            ...
            with patch(
                "app.service.agent.rag_enhanced_agent.create_investigation_prompt"
            ) as mock_create_prompt:
                mock_create_prompt.return_value = "base prompt"

                result = await agent._create_rag_enhanced_prompt(
                    mock_investigation_context, mock_knowledge_context, ["objective1"]
                )
    ...
    def test_augment_findings_with_rag_metadata(
        self, mock_tools, mock_rag_orchestrator, mock_knowledge_context
    ):
        ...
        with patch("app.service.agent.rag_enhanced_agent.create_context_augmentor"):
            ...
            result = agent._augment_findings_with_rag_metadata(
                findings, mock_knowledge_context
            )

            assert len(result.recommended_actions) > 0
            assert any(
                "enhanced with" in action for action in result.recommended_actions
            )
    ...
    def test_update_rag_stats(self, mock_tools, mock_rag_orchestrator):
        ...
        with patch("app.service.agent.rag_enhanced_agent.create_context_augmentor"):
            ...
            assert (
                agent.rag_stats["avg_context_retrieval_time_ms"] == 125.0
            )  # (150 + 100) / 2
    ...
    def test_get_rag_performance_stats(self, mock_tools, mock_rag_orchestrator):
        ...
        with patch("app.service.agent.rag_enhanced_agent.create_context_augmentor"):
            ...
            assert (
                stats["performance_metrics"]["avg_chunks_per_investigation"] == 5.0
            )  # 40/8
...
class TestRAGEnhancedAgentIntegration:
    ...
    def test_backward_compatibility_with_base_agent(self):
        ...
        agent = RAGEnhancedInvestigationAgent(
            domain="logs", tools=mock_tools, enable_rag=False
        )
    ...
    @pytest.mark.asyncio
    async def test_enhanced_logging_integration(
        self, mock_tools, mock_rag_orchestrator
    ):
...
class TestRAGEnhancedAgentErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_context_augmentor_failure_handling(
        self, mock_tools, mock_rag_orchestrator
    ):
