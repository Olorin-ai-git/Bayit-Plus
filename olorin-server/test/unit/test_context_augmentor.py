...
class TestContextAugmentor:
    ...
    @pytest.mark.asyncio
    async def test_generate_domain_queries(self, mock_rag_orchestrator, mock_investigation_context):
    ...
    @pytest.mark.asyncio
    async def test_retrieve_domain_knowledge(self, mock_rag_orchestrator, mock_investigation_context, sample_chunks):
        """Test domain knowledge retrieval"""
        mock_rag_orchestrator.knowledge_base.search_chunks = AsyncMock(return_value=sample_chunks)
    ...
    @pytest.mark.asyncio
    async def test_calculate_chunk_relevance(self, mock_rag_orchestrator, mock_investigation_context):
    ...
    @pytest.mark.asyncio
    async def test_categorize_knowledge_by_relevance(self, mock_rag_orchestrator, mock_investigation_context, sample_chunks):
        ...
        with patch.object(augmentor, '_calculate_chunk_relevance') as mock_calc:
            mock_calc.side_effect = [0.95, 0.85, 0.75, 0.65, 0.45]  # Critical, high, supporting, supporting, background
        ...
        assert len(knowledge_context.supporting_knowledge) == 3  # 0.85, 0.75, 0.65 >= 0.7
        assert len(knowledge_context.background_knowledge) == 0  # 0.45 < 0.5
        assert knowledge_context.total_chunks == 4
    
    @pytest.mark.asyncio
    async def test_inject_context_into_prompt_empty_context(self, mock_rag_orchestrator):
    ...
    @pytest.mark.asyncio
    async def test_inject_context_into_prompt_with_knowledge(self, mock_rag_orchestrator, sample_chunks):
        ...
        result = await augmentor.inject_context_into_prompt(base_prompt, knowledge_context)
    ...
    @pytest.mark.asyncio
    async def test_augment_investigation_context_cached(self, mock_rag_orchestrator, mock_investigation_context):
        ...
        cache_key = augmentor._create_context_cache_key("test-investigation-123", "network", None)
    ...
    @pytest.mark.asyncio
    async def test_augment_investigation_context_full_flow(self, mock_rag_orchestrator, mock_investigation_context, sample_chunks):
...
class TestContextAugmentorIntegration:
    ...
    @pytest.mark.asyncio
    async def test_error_handling_in_augmentation(self, mock_rag_orchestrator, mock_investigation_context):
        """Test error handling during context augmentation"""
        # Mock knowledge base to raise exception
        mock_rag_orchestrator.knowledge_base.search_chunks = AsyncMock(side_effect=Exception("Search failed"))
