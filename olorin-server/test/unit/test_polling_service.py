...
def test_poll_state_returns_none_for_304(
    service: PollingService, create_state, db_session: Session
):
    ...
    result2 = service.poll_state(
        investigation_id="inv-004", user_id="user-123", if_none_match=etag
    )
    assert result2 is None


def test_poll_changes_returns_delta(
    service: PollingService, create_state, db_session: Session
):
    ...
    result = service.poll_changes(
        investigation_id="inv-005", user_id="user-123", since_version=1
    )
...
def test_poll_active_investigations_filtered(service: PollingService, create_state):
    ...
    result = service.poll_active_investigations(
        user_id="user-123", status_filter=InvestigationStatus.IN_PROGRESS
    )
...
def test_poll_active_investigations_paginated(service: PollingService, create_state):
    ...
    result = service.poll_active_investigations(user_id="user-123", limit=2, offset=0)
