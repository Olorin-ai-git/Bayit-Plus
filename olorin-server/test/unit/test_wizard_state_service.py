...
def test_get_state_updates_last_accessed(service: InvestigationStateService, db_session: Session):
    ...
    state_before = db_session.query(InvestigationState).filter_by(investigation_id="inv-004").first()
    initial_accessed = state_before.last_accessed

    service.get_state(investigation_id="inv-004", user_id="user-123")

    state_after = db_session.query(InvestigationState).filter_by(investigation_id="inv-004").first()
...
def test_update_state_success(service: InvestigationStateService):
    ...
    result = service.update_state(investigation_id="inv-005", user_id="user-123", data=update)
...
def test_update_state_version_conflict(service: InvestigationStateService):
    ...
    with pytest.raises(HTTPException) as exc:
        service.update_state(investigation_id="inv-006", user_id="user-123", data=update)
    assert exc.value.status_code == 409
...
def test_delete_state_success(service: InvestigationStateService, db_session: Session):
    ...
    state = db_session.query(InvestigationState).filter_by(investigation_id="inv-007").first()
...
def test_get_history_pagination(service: InvestigationStateService):
    ...
    history_page1 = service.get_history(investigation_id="inv-009", user_id="user-123", limit=1, offset=0)
