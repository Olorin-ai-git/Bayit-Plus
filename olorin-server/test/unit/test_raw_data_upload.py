...
async def create_investigation(client: httpx.AsyncClient) -> str:
    ...
    try:
        response = await client.post(
            f"{API_BASE_URL}/api/investigations/", json=investigation_data
        )
...
async def upload_csv_file(
    client: httpx.AsyncClient, investigation_id: str, csv_path: str
):
    ...
    with open(csv_file, "rb") as f:
        ...
        try:
            ...
            logger.info(
                f"🚀 Sending request to {API_BASE_URL}/api/investigation/raw-data"
            )
            response = await client.post(
...
def display_results(result: dict):
    ...
    if "processing_result" in result and result["processing_result"]:
        ...
        logger.info(
            f"  • Processing Time: {pr.get('processing_time_seconds', 0):.2f} seconds"
        )
        ...
        if "quality_metrics" in pr and pr["quality_metrics"]:
            ...
            logger.info(
                f"  • Processing Time: {qm.get('processing_time', 0):.2f} seconds"
            )
            ...
            if "anomalies_detected" in qm and qm["anomalies_detected"]:
                ...
                for i, anomaly in enumerate(
                    qm["anomalies_detected"][:5], 1
                ):  # Show first 5
                    logger.info(f"  {i}. Type: {anomaly.get('type', 'N/A')}")
                    logger.info(
                        f"     Transaction: {anomaly.get('transaction_id', 'N/A')}"
                    )
                    logger.info(
                ...
                if len(qm["anomalies_detected"]) > 5:
                    logger.info(
                        f"  ... and {len(qm['anomalies_detected']) - 5} more anomalies"
                    )
