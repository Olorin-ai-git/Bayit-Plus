...
async def test_basic_langfuse():
    ...
    trace = langfuse.trace(
        ...
        user_id="test_user",
        metadata={"test_type": "basic", "timestamp": datetime.now().isoformat()},
    ...
    # Create a span
    span = langfuse.span(
        trace_id=trace.id, name="analyze_transaction", metadata={"agent": "test_agent"}
    )
    ...
    # Update span
    span.update(output={"result": "transaction_analyzed"}, level="INFO")
    span.end()
    ...
    # Update trace
    trace.update(output={"status": "completed", "fraud_detected": True})
    ...
    logger.info(
        f"View trace at: https://us.cloud.langfuse.com/project/pk-lf-0c1b17de-e7c1-43a1-bc05-e15231c5d00a/traces/{trace.id}"
    )
...
async def test_error_handling():
    ...
    trace = langfuse.trace(
        name="error_handling_test", metadata={"test_type": "error_handling"}
    )

    try:
        # Create a span that will fail
        span = langfuse.span(trace_id=trace.id, name="failing_operation")
        ...
        trace.update(output={"status": "error", "error_message": str(e)}, level="ERROR")
