...
class TestMCPInputValidator:
    ...
    def test_string_validation_xss_detection(self, validator):
        ...
        assert any(keyword in result.errors[0] for keyword in ["XSS", "command characters", "dangerous"])
    ...
    def test_sql_injection_detection(self, validator):
        ...
        assert any(keyword in result.errors[0] for keyword in ["SQL injection", "command characters", "dangerous"])
        assert result.risk_score > 0.5
    ...
    def test_command_injection_detection(self, validator):
        ...
        assert any(keyword in result.errors[0] for keyword in ["command characters", "dangerous"])
        assert result.risk_score > 0.5
    ...
    def test_custom_validation_rules(self, validator):
        ...
        result = validator.validate(42, InputType.INTEGER, additional_rules=[custom_rule])
        assert result.is_valid
        
        # Test failed validation
        result = validator.validate(-5, InputType.INTEGER, additional_rules=[custom_rule])
        assert not result.is_valid
    ...
    def test_mcp_tool_input_validation(self, validator):
        ...
        for field_name, result in results.items():
            if field_name == "tool_name":
                ...
                assert result.is_valid, f"Field {field_name} validation failed: {result.errors}"
    ...
    def test_validation_summary(self, validator):
        ...
        results = {
            ...
            "field2": ValidationResult(is_valid=False, errors=["Error 1"], risk_score=0.8),
            "field3": ValidationResult(is_valid=True, sanitized_value="value3", warnings=["Warning 1"])
        }
...
class TestInputTypeDetection:
    ...
    def test_determine_input_type_by_field_name(self):
        ...
        assert validator._determine_input_type("user_id", "test", None) == InputType.USER_ID
        assert validator._determine_input_type("transaction_id", "test", None) == InputType.TRANSACTION_ID
    ...
    def test_determine_input_type_by_value(self):
        ...
        assert validator._determine_input_type("unknown", True, None) == InputType.BOOLEAN
        ...
        assert validator._determine_input_type("unknown", {"key": "value"}, None) == InputType.JSON
        assert validator._determine_input_type("unknown", ["item1", "item2"], None) == InputType.JSON
        assert validator._determine_input_type("unknown", "default", None) == InputType.STRING
    ...
    def test_determine_input_type_by_schema(self):
        ...
        assert validator._determine_input_type("field1", "123", schema) == InputType.INTEGER
        assert validator._determine_input_type("field2", "true", schema) == InputType.BOOLEAN
