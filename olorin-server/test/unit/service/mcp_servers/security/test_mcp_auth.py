...
class TestMCPAuthenticationService:
    ...
    async def test_validate_mcp_token_success(
        self, mcp_auth_service, mock_base_auth, sample_user
    ):
        ...
        context, result = await mcp_auth_service.validate_mcp_token(
            token="valid_token", ip="192.168.1.1", user_agent="TestAgent/1.0"
        )
    ...
    async def test_validate_mcp_token_with_permissions(
        self, mcp_auth_service, mock_base_auth, sample_user
    ):
    ...
    async def test_validate_mcp_token_invalid_base_token(
        self, mcp_auth_service, mock_base_auth
    ):
        ...
        # Test
        context, result = await mcp_auth_service.validate_mcp_token(
            token="invalid_token", ip="192.168.1.1", user_agent="TestAgent/1.0"
        )
    ...
    async def test_ip_lockout(self, mcp_auth_service, mock_base_auth):
        ...
        for i in range(mcp_auth_service.config.max_failed_attempts):
            context, result = await mcp_auth_service.validate_mcp_token(
                token="invalid_token", ip=ip, user_agent="TestAgent/1.0"
            )
            assert context is None

        # Next attempt should be locked out
        context, result = await mcp_auth_service.validate_mcp_token(
            token="another_invalid_token", ip=ip, user_agent="TestAgent/1.0"
        )
    ...
    def test_permission_checking(self, mcp_auth_service):
        ...
        context = MCPSecurityContext(
            ...
            permissions=[
                MCPPermission.FRAUD_QUERY_DATABASE.value,
                MCPPermission.TOOL_EXECUTE.value,
            ],
            session_id="test_session",
            ip="192.168.1.1",
            user_agent="TestAgent/1.0",
    ...
    def test_role_permission_mapping(self):
        ...
        investigator_perms = MCPAuthenticationService.ROLE_PERMISSIONS[
