...
class TestVerificationConfig:
    ...
    @patch.dict(os.environ, {
        'LLM_VERIFICATION_CONFIDENCE_THRESHOLD': 'invalid'
    })
...
def clean_environment():
    ...
    original_env = {}
    verification_vars = [key for key in os.environ.keys() if key.startswith('LLM_VERIFICATION_')]
...
class TestVerificationConfigurationIntegration:
    ...
    def test_model_priority_ordering(self):
        ...
        assert 'gpt' in primary_model.value.lower() and 'turbo' in primary_model.value.lower()