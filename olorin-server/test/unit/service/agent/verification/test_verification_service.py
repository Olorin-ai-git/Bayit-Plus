...
class TestLLMVerificationService:
    ...
    def verification_service(self, mock_config):
        ...
        with (
            patch(
                "app.service.agent.verification.verification_service.VerificationModels"
            ),
            patch(
                "app.service.agent.verification.verification_service.VerificationCache"
            ),
            patch(
                "app.service.agent.verification.verification_service.VerificationLogger"
            ),
            patch(
                "app.service.agent.verification.verification_service.VerificationMetrics"
            ),
            patch(
                "app.service.agent.verification.verification_service.IterativeImprover"
            ),
        ):
    ...
    async def test_successful_verification(self, verification_service):
        ...
        result = await verification_service.verify_response(
            original_request=messages, response=response, context={}
        )
    ...
    async def test_verification_with_retry(self, verification_service):
        ...
        verification_service.iterative_improver.analyze_verification_failure = (
            MagicMock(return_value=mock_suggestions)
        )
    ...
    async def test_verification_with_context(self, verification_service):
        ...
        result = await verification_service.verify_response(
            original_request=messages, response=response, context=context
        )
    ...
    def test_get_service_status(self, verification_service):
        ...
        verification_service.models.health_check = MagicMock(
            return_value={"healthy": True}
        )
        verification_service.cache.health_check = AsyncMock(
            return_value={"healthy": True}
        )
        verification_service.metrics.health_check = MagicMock(
            return_value={"healthy": True}
        )
...
class TestVerificationServiceIntegration:
    ...
    async def test_full_verification_pipeline(self):
        ...
        with (
            ...
            mock_models_instance.verify_response = AsyncMock(
                return_value={
                    "is_valid": True,
                    "confidence_score": 0.85,
            ...
            result = await service.verify_response(
                original_request=messages, response=response, context={"topic": "AI/ML"}
            )
