...
class TestValidationStatusBugFix:
    ...
    @pytest.mark.asyncio
    async def test_validation_failure_sets_failed_status(self, test_runner, mock_context, mock_result):
        ...
        with patch.object(test_runner, '_validate_investigation_results', side_effect=mock_validation):
            with patch.object(test_runner, '_collect_performance_metrics', return_value={}):
                with patch.object(test_runner, '_extract_final_risk_score', return_value=0.0):
                    with patch.object(test_runner, '_extract_confidence_score', return_value=0.0):
                        ...
                        validation_results = await test_runner._validate_investigation_results(mock_context, mock_result)
                        mock_result.validation_results = validation_results
                        
                        # After the fix, status should remain "failed" (not overwritten)
                        assert mock_result.status == "failed", f"Expected status 'failed', got '{mock_result.status}'"
                        assert len(mock_result.errors) > 0, "Expected errors to be present for failed validation"
                        assert "Data extraction failed" in mock_result.errors, "Expected specific error message"
        ...
        for res in test_runner.results:
            if res.status == "completed" and res.validation_results:
                ...
                if overall_score < 70:
                    ...
                    if not res.errors:
                        res.errors = []
                    res.errors.append(f"Investigation quality score {overall_score:.1f}/100 is below acceptable threshold of 70/100")
        
        # Recalculate counts
        test_runner.metrics.scenarios_passed = sum(1 for r in test_runner.results if r.status == "completed")
        test_runner.metrics.scenarios_failed = sum(1 for r in test_runner.results if r.status == "failed")
        
        # Verify the fix worked
        assert result.status == "failed", f"Expected low-score investigation to be marked as failed, got {result.status}"
        assert test_runner.metrics.scenarios_passed == 0, f"Expected 0 passed scenarios, got {test_runner.metrics.scenarios_passed}"
        assert test_runner.metrics.scenarios_failed == 1, f"Expected 1 failed scenario, got {test_runner.metrics.scenarios_failed}"
        assert len(result.errors) > 0, "Expected error message for low quality score"
        assert "below acceptable threshold" in result.errors[0], "Expected quality threshold error message"
    ...
    async def test_high_score_completed_remains_completed(self, test_runner):
        ...
        assert result.status == "completed", f"Expected high-score investigation to remain completed, got {result.status}"
        assert test_runner.metrics.scenarios_passed == 1, f"Expected 1 passed scenario, got {test_runner.metrics.scenarios_passed}"
        assert test_runner.metrics.scenarios_failed == 0, f"Expected 0 failed scenarios, got {test_runner.metrics.scenarios_failed}"
    ...
    async def test_invalid_status_becomes_failed(self, test_runner):
        ...
        assert result.status == "failed", f"Expected invalid status to become failed, got {result.status}"
        assert test_runner.metrics.scenarios_passed == 0, f"Expected 0 passed scenarios, got {test_runner.metrics.scenarios_passed}"
        assert test_runner.metrics.scenarios_failed == 1, f"Expected 1 failed scenario, got {test_runner.metrics.scenarios_failed}"
    ...
    def test_original_bug_scenario_reproduction(self, test_runner):
        ...
        assert result.status == "failed", "Original bug: failed investigation should remain failed"
        assert test_runner.metrics.scenarios_passed == 0, "Should have 0 passed scenarios"
        assert test_runner.metrics.scenarios_failed == 1, "Should have 1 failed scenario"
        ...
        assert scenarios_failed > 0, "Bug fix verification: failed investigations should be counted as failed"
