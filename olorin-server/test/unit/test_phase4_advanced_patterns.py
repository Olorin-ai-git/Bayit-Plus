...
from app.service.agent.orchestration.multi_agent_coordination import (
    ...
    CoordinationStrategy, AgentPool, ParallelStrategy, CommitteeStrategy
)
...
class TestMultiAgentCoordination:
    ...
    async def test_parallel_strategy(self):
        ...
        agents = [
            AgentCapabilities(name=f"agent_{i}", 
    ...
    async def test_committee_strategy(self):
        ...
        agents = [
            AgentCapabilities(name=f"voter_{i}", 
                            specializations=["decision"],
                            success_rate=0.95)
            for i in range(5)
        ]
...
class TestCustomToolBuilder:
    ...
    def test_create_tool_from_template(self, tool_builder):
        ...
        tool = tool_builder.create_from_template(
            "device_analyzer",
            implementation=impl
        )
