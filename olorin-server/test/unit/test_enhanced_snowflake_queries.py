...
class TestEnhancedSnowflakeQueries:
    ...
    def test_comprehensive_field_coverage(self):
        ...
        missing_fields = [
            field for field in critical_fields if field not in REAL_COLUMNS
        ]
        ...
        assert (
            len(device_fields) >= 5
        ), f"Insufficient device fields: {len(device_fields)}"
        assert len(risk_fields) >= 3, f"Insufficient risk fields: {len(risk_fields)}"
        assert (
            len(payment_fields) >= 3
        ), f"Insufficient payment fields: {len(payment_fields)}"
    ...
    def test_priority_fields_selection(self):
        ...
        missing_priority = [
            field
            for field in must_have_priority
            if field not in PRIORITY_EVIDENCE_FIELDS
        ]

        assert (
            len(missing_priority) == 0
        ), f"Missing priority fields: {missing_priority}"
        assert len(PRIORITY_EVIDENCE_FIELDS) < len(
            REAL_COLUMNS
        ), "Priority fields should be subset of all fields"

        print(
            f"   ✅ Priority fields: {len(PRIORITY_EVIDENCE_FIELDS)}/{len(REAL_COLUMNS)}"
        )
    ...
    def test_query_builder_comprehensive_query(self):
        ...
        critical_fields_in_query = sum(
            1
            for field in [TX_ID_KEY, MODEL_SCORE, DEVICE_ID, USER_AGENT]
            if field in query
        )
        assert (
            critical_fields_in_query >= 3
        ), f"Not enough critical fields in query: {critical_fields_in_query}"
        ...
        assert (
            metadata["field_count"] > 20
        ), f"Comprehensive query should have many fields: {metadata['field_count']}"

        # Verify validation
        assert (
            validation["evidence_coverage_score"] > 0.7
        ), f"Low evidence coverage: {validation['evidence_coverage_score']}"

        print(f"   ✅ Query includes {metadata['field_count']} evidence fields")
        print(f"   ✅ Evidence coverage: {validation['evidence_coverage_score']}")
        print(
            f"   ✅ Performance tier: {metadata['performance_estimate']['performance_tier']}"
        )
    ...
    def test_query_builder_different_focuses(self):
        ...
        for focus in focuses:
            ...
            if focus == "minimal":
                assert (
                    field_count < 15
                ), f"Minimal should have few fields: {field_count}"
            elif focus == "comprehensive":
                assert (
                    field_count > 25
                ), f"Comprehensive should have many fields: {field_count}"

            print(
                f"   ✅ {focus}: {field_count} fields, {metadata['query_complexity']} complexity"
            )
    ...
    def test_snowflake_tool_query_validation(self):
        ...
        ), f"Comprehensive query should be valid: {validation}"
        assert (
            len(validation["missing_critical_fields"]) == 0
        ), f"Should have no missing fields: {validation['missing_critical_fields']}"
        assert (
            validation["total_evidence_score"] > 0.8
        ), f"Should have high evidence score: {validation['total_evidence_score']}"

        # Test minimal query validation
        minimal_query = (
            f"SELECT {TX_ID_KEY} FROM TRANSACTIONS_ENRICHED WHERE IP = '1.1.1.1'"
        )

        minimal_validation = tool.validate_query_fields(minimal_query)

        assert (
            minimal_validation["valid"] == False
        ), "Minimal query should be flagged as incomplete"
        assert (
            len(minimal_validation["missing_critical_fields"]) > 0
        ), "Should identify missing critical fields"
        assert (
            len(minimal_validation["recommendations"]) > 0
        ), "Should provide recommendations"

        print(
            f"   ✅ Comprehensive query validation: {validation['total_evidence_score']}"
        )
        print(f"   ✅ Missing field detection working")
        print(
            f"   ✅ Recommendations generated: {len(minimal_validation['recommendations'])}"
        )
    ...
    def test_get_recommended_query_convenience_function(self):
        ...
        # Verify comprehensive field inclusion
        evidence_fields_count = sum(
            1
            for field in [TX_ID_KEY, MODEL_SCORE, DEVICE_ID, USER_AGENT, CARD_BRAND]
            if field in query
        )
        assert (
            evidence_fields_count >= 4
        ), f"Should include multiple evidence fields: {evidence_fields_count}"
    ...
    def test_performance_optimization_suggestions(self):
        ...
        assert (
            fast_perf["performance_tier"] == "fast"
        ), f"Should be fast: {fast_perf['performance_tier']}"
        ...
        assert (
            len(slow_perf["optimization_suggestions"]) > 0
