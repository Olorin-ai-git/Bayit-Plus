...
class TestUserAgentValidation:
    ...
    def test_validate_user_agent_valid(self, network_validator, valid_user_agent):
        ...
        is_valid, error, analysis = network_validator.validate_user_agent(
            valid_user_agent
        )
        assert (
            is_valid is True
        ), f"User agent should be valid: {valid_user_agent[:50]}..."
        assert error is None
        assert isinstance(analysis, dict)
    ...
    def test_validate_user_agent_suspicious(
        self, network_validator, suspicious_user_agent
    ):
    ...
    def test_user_agent_browser_detection(self, network_validator):
        ...
        for user_agent, expected_browser in browser_tests:
            ...
            if "browser" in analysis:
                ...
                assert (
                    expected_browser.lower() in detected_browser.lower()
                ), f"Should detect {expected_browser} in {user_agent[:50]}..."
    ...
    def test_user_agent_os_detection(self, network_validator):
        ...
        for user_agent, expected_os in os_tests:
            ...
            if "operating_system" in analysis:
                ...
                assert (
                    expected_os.lower() in detected_os.lower()
                ), f"Should detect {expected_os} in {user_agent[:50]}..."
    ...
    def test_user_agent_device_type_detection(self, network_validator):
        ...
        for user_agent, expected_device in device_tests:
            ...
            if "device_type" in analysis:
                ...
                assert (
                    expected_device in detected_device
                ), f"Should detect {expected_device} in {user_agent[:50]}..."
...
class TestDeviceFingerprintValidation:
    ...
    def test_validate_device_fingerprint_valid(self, network_validator):
        ...
        for fingerprint in valid_fingerprints:
            is_valid, error, analysis = network_validator.validate_device_fingerprint(
                fingerprint
            )
            assert (
                is_valid is True
            ), f"Device fingerprint should be valid: {fingerprint}"
            assert error is None
            assert isinstance(analysis, dict)
    ...
    def test_validate_device_fingerprint_invalid(self, network_validator):
        ...
        for fingerprint in invalid_fingerprints:
            ...
            assert (
                is_valid is False
            ), f"Device fingerprint should be invalid: {fingerprint}"
            assert error is not None
    ...
    def test_fingerprint_entropy_analysis(self, network_validator):
        ...
        high_is_valid, high_error, high_analysis = (
            network_validator.validate_device_fingerprint(high_entropy_fp)
        )
        low_is_valid, low_error, low_analysis = (
            network_validator.validate_device_fingerprint(low_entropy_fp)
        )

        if "entropy" in high_analysis and "entropy" in low_analysis:
            assert (
                high_analysis["entropy"] > low_analysis["entropy"]
            ), "High entropy fingerprint should have higher entropy score"
...
class TestNetworkSecurityValidation:
    ...
    def test_detect_proxy_networks(self, network_validator):
        ...
        for ip in proxy_ips:
            ...
            if "proxy_risk" in analysis:
                assert (
                    analysis["proxy_risk"] > 0.5
                ), f"Should detect proxy risk for {ip}"
    ...
    def test_detect_tor_networks(self, network_validator):
        ...
        with patch.object(network_validator, "_is_tor_exit_node", return_value=True):
            is_valid, error, analysis = network_validator.validate_ip_address(
                "185.220.100.1"
            )
    ...
    def test_detect_datacenter_ips(self, network_validator):
        ...
        for ip in datacenter_ips:
            ...
            if "is_datacenter" in analysis:
                assert (
                    analysis["is_datacenter"] is True
                ), f"Should detect datacenter IP: {ip}"
    ...
    def test_detect_malicious_ips(self, network_validator):
        ...
        with patch.object(
            network_validator, "_check_malicious_ip_lists", return_value=True
        ):
            for ip in malicious_ips:
                is_valid, error, analysis = network_validator.validate_ip_address(ip)
                if "is_malicious" in analysis:
                    assert (
                        analysis["is_malicious"] is True
                    ), f"Should detect malicious IP: {ip}"
    ...
    def test_rate_limiting_detection(self, network_validator):
        ...
        is_suspicious, rate_msg, rate_analysis = (
            network_validator.validate_request_patterns(rate_limit_data)
        )

        if not is_suspicious:
            # High rates should trigger suspicion
            assert (
                rate_analysis["rate_limit_violations"] > 0
    ...
    def test_geographic_anomaly_detection(self, network_validator):
        ...
        is_normal, geo_msg, geo_analysis = (
            network_validator.validate_geographic_consistency(geographic_data)
        )

        if "geographic_anomalies" in geo_analysis:
            assert (
                geo_analysis["geographic_anomalies"] > 0
...
class TestPerformanceRequirements:
    ...
    def test_ip_validation_performance(self, network_validator):
        ...
        ), f"IP validation took {execution_time_ms}ms, should be <50ms"
    ...
    def test_user_agent_validation_performance(self, network_validator):
        ...
        assert (
            execution_time_ms < 50
        ), f"User agent validation took {execution_time_ms}ms, should be <50ms"
    ...
    def test_device_fingerprint_performance(self, network_validator):
        ...
        assert (
            execution_time_ms < 50
        ), f"Device fingerprint validation took {execution_time_ms}ms, should be <50ms"
    ...
    def test_batch_validation_performance(self, network_validator):
        ...
        for _ in range(25):
            ...
            network_validator.validate_device_fingerprint(
                network_data["device_fingerprint"]
            )
        end_time = time.time()
        ...
        assert (
            execution_time_ms < 50
        ), f"Batch network validation took {execution_time_ms}ms, should be <50ms"
...
class TestEdgeCasesAndErrorHandling:
    ...
    def test_unicode_input_handling(self, network_validator):
        ...
        for unicode_input in unicode_inputs:
            ...
            try:
                ...
                is_valid, error, analysis = (
                    network_validator.validate_device_fingerprint(unicode_input)
                )
                assert isinstance(is_valid, bool)
                assert isinstance(analysis, dict)
    ...
    def test_malicious_input_handling(self, network_validator):
        ...
        for malicious_input in malicious_inputs:
            ...
            if is_valid:
                assert (
                    analysis.get("is_suspicious", False) is True
                ), f"Should flag as suspicious: {malicious_input}"
            ...
            assert (
                is_valid is False
            ), f"Should reject malicious fingerprint: {malicious_input}"
