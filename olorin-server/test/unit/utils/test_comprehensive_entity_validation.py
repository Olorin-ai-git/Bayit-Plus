...
class TestEmailValidation:
    ...
    def test_validate_email_valid_formats(self, validator, valid_email):
        ...
        is_valid, error = validator.validate_entity(
            EntityType.EMAIL, "email", valid_email
        )
    ...
    def test_validate_email_invalid_formats(
        self, validator, invalid_email, expected_error
    ):
        """Test validation of invalid email formats"""
        is_valid, error = validator.validate_entity(
            EntityType.EMAIL, "email", invalid_email
        )
    ...
    def test_email_security_validation(self, validator):
        ...
        for malicious_email in malicious_emails:
            is_valid, error = validator.validate_entity(
                EntityType.EMAIL, "email", malicious_email
            )
            assert (
                is_valid is False
            ), f"Malicious email should be rejected: {malicious_email}"
    ...
    def test_email_performance_requirement(self, validator):
        ...
        ), f"Email validation took {execution_time_ms}ms, should be <100ms"
...
class TestPhoneValidation:
    ...
    def test_validate_phone_invalid_formats(
        self, validator, invalid_phone, expected_error
    ):
        """Test validation of invalid phone formats"""
        is_valid, error = validator.validate_entity(
            EntityType.PHONE, "phone", invalid_phone
        )
...
class TestIPAddressValidation:
    ...
    @pytest.mark.parametrize(
        "invalid_ip",
        ["256.256.256.256", "192.168.1", "192.168.1.1.1", "not_an_ip", "192.168.1.-1"],
    )
...
class TestFinancialValidation:
    ...
    def test_validate_financial_amount_invalid(
        self, validator, invalid_amount, expected_error
    ):
        """Test validation of invalid financial amounts"""
        is_valid, error = validator.validate_entity(
            EntityType.AMOUNT, "amount", invalid_amount
        )
    ...
    def test_validate_currency_code_valid(self, validator, valid_currency):
        ...
        is_valid, error = validator.validate_entity(
            EntityType.CURRENCY, "currency", valid_currency
        )
    ...
    def test_validate_currency_code_invalid(self, validator, invalid_currency):
        ...
        is_valid, error = validator.validate_entity(
            EntityType.CURRENCY, "currency", invalid_currency
        )
...
class TestTimestampValidation:
    ...
    def test_validate_timestamp_valid_formats(self, validator, valid_timestamp):
        ...
        is_valid, error = validator.validate_entity(
            EntityType.TX_TIMESTAMP, "timestamp", valid_timestamp
        )
    ...
    def test_validate_timestamp_invalid_formats(self, validator, invalid_timestamp):
        ...
        is_valid, error = validator.validate_entity(
            EntityType.TX_TIMESTAMP, "timestamp", invalid_timestamp
        )
...
class TestSecurityValidation:
    ...
    def test_prevent_xss_attacks(self, validator, xss_payload):
        ...
        is_valid, error = validator.validate_entity(
            EntityType.USER, "user_input", xss_payload
        )
    ...
    def test_prevent_sql_injection(self, validator, sql_payload):
        ...
        is_valid, error = validator.validate_entity(
            EntityType.USER, "user_input", sql_payload
        )
        assert (
            is_valid is False
        ), f"SQL injection payload should be rejected: {sql_payload}"
        assert "malicious content" in error
    ...
    def test_excessive_html_tags_rejected(self, validator):
        ...
        is_valid, error = validator.validate_entity(
            EntityType.USER, "description", excessive_html
        )
    ...
    def test_safe_content_allowed(self, validator):
        ...
        for safe_input in safe_inputs:
            is_valid, error = validator.validate_entity(
                EntityType.USER, "name", safe_input
            )
...
class TestNullValueValidation:
    ...
    def test_required_fields_reject_null(self, validator, required_field):
        ...
        for null_value in [None, "", "   ", "\t", "\n"]:
            is_valid, error = validator.validate_entity(
                EntityType.TRANSACTION, required_field, null_value
            )
            assert (
                is_valid is False
            ), f"Required field {required_field} should reject null value: {null_value}"
...
class TestStringValidation:
    ...
    def test_string_length_limits(self, validator):
        ...
        valid_string = "a" * 1000
        is_valid, error = validator.validate_entity(
            EntityType.USER, "description", valid_string
        )
        ...
        is_valid, error = validator.validate_entity(
            EntityType.USER, "description", over_limit_string
        )
    ...
    def test_control_characters_rejected(self, validator):
        ...
        for text in control_chars:
            is_valid, error = validator.validate_entity(
                EntityType.USER, "description", text
            )
            assert (
                is_valid is False
            ), f"Control character should be rejected: {repr(text)}"
    ...
    def test_allowed_control_characters(self, validator):
        ...
        allowed_chars = ["line1\nline2", "tab\ttext", "carriage\rreturn"]
        ...
        for text in allowed_chars:
            ...
            assert (
                is_valid is True
            ), f"Allowed control character should pass: {repr(text)}"
            assert error is None
...
class TestMultiEntityValidation:
    ...
    def test_validate_multiple_entities_all_invalid(self, validator):
        ...
        assert all(
            entity_type in errors
            for entity_type in ["email", "amount", "currency", "ip"]
        )
...
class TestPerformanceRequirements:
    ...
    def test_single_validation_performance(self, validator):
        ...
        for entity_type, field_name, value in test_data:
            ...
            assert (
                execution_time_ms < 100
            ), f"Validation took {execution_time_ms}ms, should be <100ms"
    ...
    def test_batch_validation_performance(self, validator):
        ...
        assert (
            execution_time_ms < 100
        ), f"Batch validation took {execution_time_ms}ms, should be <100ms"
...
class TestEdgeCasesAndErrorHandling:
    ...
    def test_unicode_handling(self, validator):
        ...
        for unicode_input in unicode_inputs:
            ...
            is_valid, error = validator.validate_entity(
                EntityType.EMAIL, "email", unicode_input
            )
    ...
    def test_very_large_inputs(self, validator):
        ...
        is_valid, error = validator.validate_entity(
            EntityType.USER, "description", very_large_input
        )
    ...
    def test_validation_statistics(self, validator):
        ...
        for field in required_fields:
            ...
            assert isinstance(
                stats[field], int
            ), f"Statistics field {field} should be integer"
...
class TestConvenienceFunctions:
    ...
    def test_validate_entity_field_function(self):
        ...
        is_valid, error = validate_entity_field(
            EntityType.EMAIL, "email", "user@example.com"
        )
    ...
    def test_global_validator_consistency(self):
        ...
        is_valid, error = validate_entity_field(
            EntityType.EMAIL, "email", "test@example.com"
        )
        assert is_valid is True
        assert error is None
