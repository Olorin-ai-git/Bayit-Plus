...
async def test_multi_agent_investigation():
    ...
    agent_tasks = [
        agent.analyze(test_data, trace.id) 
        for agent in agents
    ]
    ...
    langfuse.score(
        ...
        value=1.0 - (suspicious_count / len(agents) - 0.5) * 2 if suspicious_count != len(agents) / 2 else 0,
...
async def test_error_recovery():
    ...
    trace.update(
        output={"status": "completed_with_recovery"},
        level="DEFAULT"
    )
...
async def test_performance_monitoring():
    ...
    trace.update(
        output={"status": "performance_analysis_complete"},
        level="DEFAULT"
    )
