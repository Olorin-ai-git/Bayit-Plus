...
class PerformanceBenchmark:
    ...
    def _benchmark_latency(self) -> Dict[str, float]:
        ...
        latency_results['standard_p95_ms'] = sorted(latencies)[int(0.95 * len(latencies))]
        ...
        for i in range(1000):
            start_time = time.perf_counter()
            structured_logger.info(f"Structured log message {i}", 
                                 iteration=i, 
        ...
        latency_results['structured_p95_ms'] = sorted(latencies)[int(0.95 * len(latencies))]
        ...
        print(f"  ✅ Structured logging: {latency_results['structured_avg_ms']:.3f}ms avg")
    ...
    def _benchmark_throughput(self) -> Dict[str, float]:
        ...
        structured_logger = get_unified_logger("benchmark.throughput.structured", structured=True)
        
        start_time = time.perf_counter()
        
        for i in range(num_logs):
            structured_logger.info(f"Structured throughput test {i}",
                                 iteration=i,
        ...
        print(f"  ✅ Standard throughput: {throughput_results['standard_logs_per_second']:.0f} logs/sec")
        print(f"  ✅ Structured throughput: {throughput_results['structured_logs_per_second']:.0f} logs/sec")
    ...
    def _benchmark_memory_usage(self) -> Dict[str, float]:
        ...
        memory_results['hundred_loggers_mb'] = logger_creation_end - logger_creation_start
        memory_results['per_logger_kb'] = (memory_results['hundred_loggers_mb'] * 1024) / 100
        ...
        print(f"  ✅ Memory under load increase: {memory_results['under_load_increase_mb']:.2f} MB")
    ...
    def _benchmark_concurrency(self) -> Dict[str, float]:
        ...
        for num_threads in [1, 5, 10, 20]:
            ...
            with ThreadPoolExecutor(max_workers=num_threads) as executor:
                futures = [executor.submit(logging_worker, i, logs_per_thread) 
            ...
            concurrency_results[f'{num_threads}_threads_logs_per_second'] = total_logs / total_duration
            concurrency_results[f'{num_threads}_threads_avg_thread_time'] = statistics.mean(thread_times)
        
        print(f"  ✅ 1 thread: {concurrency_results['1_threads_logs_per_second']:.0f} logs/sec")
        print(f"  ✅ 10 threads: {concurrency_results['10_threads_logs_per_second']:.0f} logs/sec")
        print(f"  ✅ 20 threads: {concurrency_results['20_threads_logs_per_second']:.0f} logs/sec")
    ...
    def _benchmark_format_switching(self) -> Dict[str, float]:
        ...
        for i in range(num_switches):
            ...
            logger = get_unified_logger(f"benchmark.format_switch.{format_to_use.value}")
        ...
        format_results['total_switches_per_second'] = num_switches / (total_end_time - total_start_time)
        
        print(f"  ✅ Avg format switch time: {format_results['avg_switch_time_ms']:.3f} ms")
        print(f"  ✅ Switches per second: {format_results['total_switches_per_second']:.1f}")
    ...
    def _system_health_check(self) -> Dict[str, Any]:
        ...
        health_results['memory_leak_detected'] = memory_growth > 50  # Arbitrary threshold
        ...
        health_results['latency_consistency_cv'] = statistics.stdev(latencies) / statistics.mean(latencies)
        health_results['performance_consistent'] = health_results['latency_consistency_cv'] < 0.5
        
        print(f"  ✅ Memory growth: {memory_growth:.2f} MB")
        print(f"  ✅ Performance consistent: {health_results['performance_consistent']}")
    ...
    def _print_benchmark_summary(self):
        ...
        print(f"  Standard Logging Target (<1ms):    {standard_avg:.3f}ms {'✅ PASS' if standard_avg < 1.0 else '❌ FAIL'}")
        print(f"  JSON Logging Target (<5ms):        {json_avg:.3f}ms {'✅ PASS' if json_avg < 5.0 else '❌ FAIL'}")
        print(f"  Structured Logging Target (<5ms):  {structured_avg:.3f}ms {'✅ PASS' if structured_avg < 5.0 else '❌ FAIL'}")
        ...
        print(f"  Standard Throughput Target (>10k):  {standard_throughput:.0f}/s {'✅ PASS' if standard_throughput > 10000 else '❌ FAIL'}")
        print(f"  Structured Throughput Target (>2k): {structured_throughput:.0f}/s {'✅ PASS' if structured_throughput > 2000 else '❌ FAIL'}")
        
        # Memory targets
        base_overhead = self.results['memory']['base_overhead_mb']
        per_logger = self.results['memory']['per_logger_kb']
        
        print(f"  Base Memory Target (<10MB):        {base_overhead:.2f}MB {'✅ PASS' if base_overhead < 10.0 else '❌ FAIL'}")
        print(f"  Per-Logger Target (<1KB):          {per_logger:.3f}KB {'✅ PASS' if per_logger < 1.0 else '❌ FAIL'}")
        
        # Overall system health
        health = self.results['system_health']
        print(f"  Memory Leak Check:                  {'✅ PASS' if not health['memory_leak_detected'] else '❌ FAIL'}")
        print(f"  Performance Consistency:            {'✅ PASS' if health['performance_consistent'] else '❌ FAIL'}")
        
        # Concurrency performance
        concurrent_10 = self.results['concurrency']['10_threads_logs_per_second']
        print(f"  Concurrency (10 threads):           {concurrent_10:.0f}/s {'✅ GOOD' if concurrent_10 > 5000 else '⚠️  OK'}")
        ...
        print(f"  Overall Grade: {'🏆 EXCELLENT' if passed_tests >= 7 else '✅ GOOD' if passed_tests >= 6 else '⚠️  NEEDS IMPROVEMENT'}")
