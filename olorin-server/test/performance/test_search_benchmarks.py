...
class TestBooleanSearchPerformance:
    ...
    def test_boolean_parser_performance_simple(self):
        ...
        print(
            f"Simple Boolean parsing - Avg: {avg_parse_time*1000:.3f}ms, Max: {max_parse_time*1000:.3f}ms"
        )
    ...
    def test_boolean_parser_performance_complex(self):
        ...
        print(
            f"Complex Boolean parsing - Avg: {avg_parse_time*1000:.3f}ms, Max: {max_parse_time*1000:.3f}ms"
        )
    ...
    def test_boolean_evaluation_performance(self):
        ...
        print(
            f"Boolean evaluation - Avg: {avg_eval_time*1000:.3f}ms, Max: {max_eval_time*1000:.3f}ms"
        )
    ...
    def test_large_entity_set_performance(self):
        ...
        parser = BooleanQueryParser(
            expression=expression, entity_mapping=entity_mapping
        )
        ...
        print(
            f"Large entity set (50 entities) - Parse: {parse_time*1000:.3f}ms, Eval Avg: {avg_eval_time*1000:.3f}ms"
        )
...
class TestMultiEntityInvestigationPerformance:
    ...
    async def test_investigation_startup_performance(self):
        ...
        with patch(
            "app.service.agent.multi_entity.investigation_orchestrator.MultiEntityInvestigationCoordinator"
        ) as mock_coordinator:
            ...
            print(
                f"Investigation startup - Avg: {avg_startup_time*1000:.3f}ms, Max: {max_startup_time*1000:.3f}ms"
            )
    ...
    def test_concurrent_investigation_performance(self):
        ...
        async def run_concurrent_tests():
            ...
            results = await asyncio.gather(
                *[run_investigation(req) for req in requests]
            )
            ...
            assert (
                total_time < 2.0
            )  # Total time should be reasonable due to concurrency
            assert success_count == 10  # All should succeed
            assert avg_individual_time < 0.2  # Individual times should be reasonable

            print(
                f"Concurrent investigations (10) - Total: {total_time*1000:.3f}ms, Avg Individual: {avg_individual_time*1000:.3f}ms"
            )
...
class TestMemoryAndResourcePerformance:
    ...
    def test_memory_usage_large_expressions(self):
        ...
        expression = " AND ".join(
            [f"({entity} OR {entity}_alt)" for entity in entities]
        )
        entity_mapping = {}
        ...
        for _ in range(10):
            parser = BooleanQueryParser(
                expression=expression, entity_mapping=entity_mapping.copy()
            )
            parse_result = parser.parse()
            assert parse_result["valid"] is True
        ...
        print(
            f"Memory increase for large expressions: {memory_increase / (1024 * 1024):.2f}MB"
        )
    ...
    def test_parser_cleanup_performance(self):
        ...
        for i in range(100):
            ...
            parser = BooleanQueryParser(
                ...
                entity_mapping={
                    f"entity_{i}": f"entity_{i}",
                    f"entity_{i+1}": f"entity_{i+1}",
                },
            )
            parse_result = parser.parse()
            assert parse_result["valid"] is True
        ...
        print(
            f"Parser creation/cleanup - Avg: {avg_creation_time*1000:.3f}ms, Max: {max_creation_time*1000:.3f}ms"
        )
...
class TestScalabilityBenchmarks:
    ...
    def test_expression_complexity_scaling(self):
        ...
        for depth in range(1, 11):  # Test nesting depths 1-10
            ...
            entity_results = {
                entity: i % 2 == 0 for i, entity in enumerate(entity_mapping.keys())
            }
        ...
        for result in complexity_results:
            print(
                f"  Depth {result['depth']}: Parse {result['parse_time']*1000:.3f}ms, "
    ...
    def test_entity_count_scaling(self):
        ...
        for result in scaling_results:
            print(
                f"  {result['entity_count']} entities: Parse {result['parse_time']*1000:.3f}ms, "
