...
async def test_tool_registry_fixes():
    ...
    try:
        ...
        logger.info(
            f"✅ Key Investigation Tools: Found {len(found_tools)}/{len(key_tools)}"
        )
...
async def test_enhanced_tool_executor():
    ...
    try:
        ...
        assert hasattr(
            enhanced_node, "health_manager"
        ), "Health manager not initialized"
        assert isinstance(
            enhanced_node.health_manager, ToolHealthManager
        ), "Health manager wrong type"

        # Check that tool metrics are initialized
        assert hasattr(enhanced_node, "tool_metrics"), "Tool metrics not initialized"
        assert (
            mock_tool.name in enhanced_node.health_manager.health_checks
        ), "Tool not added to health manager"
...
async def test_state_management():
    ...
    try:
        ...
        assert (
            initial_state["current_phase"] == "initialization"
        ), "should start in initialization phase"
...
async def main():
    ...
    if passed_tests == total_tests:
        ...
        logger.error(
            f"💥 {total_tests - passed_tests} tests failed. Some issues remain."
        )
        return 1
