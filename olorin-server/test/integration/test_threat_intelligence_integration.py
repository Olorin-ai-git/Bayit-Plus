...
async def demonstrate_agent_with_threat_intel():
    ...
    logger.info(
        "  The LLM will decide which tools to use based on investigation needs."
    )
...
async def main():
    ...
    try:
        ...
        logger.info("=" * 60)
        logger.info(
            f"  Tool Availability Test: {'✅ PASSED' if test1_passed else '❌ FAILED'}"
        )
        logger.info(
            f"  Individual Tools Test: {'✅ PASSED' if test2_passed else '❌ FAILED'}"
        )
        logger.info(
            f"  Agent Integration Demo: {'✅ PASSED' if test3_passed else '❌ FAILED'}"
        )
        ...
        if all([test1_passed, test2_passed, test3_passed]):
            ...
            logger.info(
                "  • Agents can structuredly select threat intel tools based on needs"
            )
