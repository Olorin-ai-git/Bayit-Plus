...
class TestInvestigationAutoTrigger:
    ...
    @pytest.fixture
    def investigation_create_request(self, investigation_settings) -> InvestigationStateCreate:
    ...
    @pytest.mark.asyncio
    async def test_create_state_without_background_tasks(self, db: Session, investigation_create_request):
        ...
        state = db.query(InvestigationState).filter(
            InvestigationState.investigation_id == "inv_test_001"
        ).first()
    ...
    async def test_create_state_with_background_tasks_queues_execution(
        ...
        with patch('app.router.controllers.investigation_executor.execute_structured_investigation'):
            response = await service.create_state(
                user_id="test_user",
                data=investigation_create_request,
    ...
    async def test_trigger_service_builds_investigation_context(
        ...
        assert context["entity_type"] == "ip_address"  # Original settings value, not converted
    ...
    async def test_trigger_service_updates_state_to_in_progress(
        ...
        audit_entries = db.query(InvestigationAuditLog).filter(
    ...
    async def test_end_to_end_auto_trigger_flow(
        ...
        state_change_entries = db.query(InvestigationAuditLog).filter(
            InvestigationAuditLog.investigation_id == "inv_test_001",
            InvestigationAuditLog.action_type == "STATE_CHANGE"
        ).all()
    ...
    @pytest.mark.asyncio
    async def test_investigation_without_settings_not_triggered(
        self,
        db: Session
    ):
        ...
        state = db.query(InvestigationState).filter(
            InvestigationState.investigation_id == "inv_no_settings"
        ).first()
        assert state.status == InvestigationStatus.CREATED
    ...
    async def test_audit_trail_completeness(
        ...
        audit_entries = db.query(InvestigationAuditLog).filter(
            InvestigationAuditLog.investigation_id == "inv_test_001"
        ).order_by(InvestigationAuditLog.timestamp).all()
        ...
        state_change_entry = [e for e in audit_entries if e.action_type == "STATE_CHANGE"][0]
