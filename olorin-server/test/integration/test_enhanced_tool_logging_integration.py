...
from app.service.agent.tools.threat_intelligence_tool.unified_threat_intelligence_tool import UnifiedThreatIntelligenceTool
...
class TestToolErrorCategorization:
    ...
    def test_database_error_categorization(self):
        ...
        category = ToolErrorCategorizer.categorize_error(warehouse_error, "snowflake_query")
...
class TestEnhancedToolExecutionLogger:
    ...
    async def test_tool_execution_success_logging(self, tool_logger):
        ...
        # Log success
        result_data = [{"tx_id": "123", "amount": 100.0}, {"tx_id": "456", "amount": 250.0}]
    ...
    async def test_empty_result_logging(self, tool_logger):
        ...
        await tool_logger.log_empty_result(
            ...
            context={
                "query": "SELECT * FROM transactions WHERE false",
                "limit": 100
            }
        )
        
        # Verify WebSocket event would be emitted (we can't test actual emission without setup)
        assert len(tool_logger._websocket_handlers) == 0  # No handlers registered in test
...
class TestSnowflakeToolIntegration:
    ...
    def mock_snowflake_client(self):
        ...
        with patch('app.service.agent.tools.snowflake_tool.snowflake_tool.SnowflakeClient') as mock:
            yield mock
    
    @pytest.mark.asyncio
    async def test_snowflake_successful_execution(self, mock_snowflake_tool, mock_snowflake_client, tool_logger):
    ...
    @pytest.mark.asyncio
    async def test_snowflake_connection_failure(self, mock_snowflake_tool, mock_snowflake_client, tool_logger):
        """Test Snowflake connection failure with comprehensive error logging."""
        # Setup mock to raise connection error
        mock_client_instance = Mock()
        mock_client_instance.connect = AsyncMock(side_effect=ConnectionError("Authentication failed"))
        mock_client_instance.disconnect = AsyncMock()
        mock_snowflake_client.return_value = mock_client_instance
    ...
    @pytest.mark.asyncio  
    async def test_snowflake_empty_results(self, mock_snowflake_tool, mock_snowflake_client, tool_logger):
...
class TestThreatIntelligenceToolIntegration:
    ...
    @pytest.mark.asyncio
    async def test_threat_intel_provider_initialization_failure(self, mock_threat_intel_tool, tool_logger):
        """Test threat intelligence provider initialization failure."""
        # Mock the initialization to fail
        with patch.object(mock_threat_intel_tool, '_initialize_providers', side_effect=Exception("Provider API keys not found")):
    ...
    @pytest.mark.asyncio
    async def test_threat_intel_all_providers_failed(self, mock_threat_intel_tool, tool_logger):
        ...
        with patch.object(mock_threat_intel_tool, '_initialize_providers', return_value=None):
            with patch.object(mock_threat_intel_tool, '_query_provider', side_effect=Exception("API timeout")):
                result = await mock_threat_intel_tool._arun(
                    target="192.168.1.1",
