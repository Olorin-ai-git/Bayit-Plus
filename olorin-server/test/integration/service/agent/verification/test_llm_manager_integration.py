...
class TestLLMManagerVerificationIntegration:
    ...
    def llm_manager_with_verification(self, mock_verification_service):
        ...
        with patch.dict(os.environ, {'LLM_VERIFICATION_ENABLED': 'true'}), \
             patch('app.service.llm_manager.get_verification_config'), \
             patch('app.service.llm_manager.LLMVerificationService', return_value=mock_verification_service):
            
            manager = LLMManager()
            manager.selected_model = MagicMock()
            manager.selected_model.ainvoke = AsyncMock(return_value=MagicMock(content="Test response"))
            return manager
    ...
    def test_verification_disabled_initialization(self, llm_manager_without_verification):
    ...
    @pytest.mark.asyncio
    async def test_mandatory_verification_with_verification_enabled(self, llm_manager_with_verification):
        ...
        llm_manager_with_verification.verification_service.verify_response_with_retry.return_value = (
            "Verified response", mock_verification_response
        )
    ...
    @pytest.mark.asyncio
    async def test_mandatory_verification_with_verification_disabled(self, llm_manager_without_verification):
        """Test mandatory verification falls back to direct invocation when disabled."""
        messages = [HumanMessage(content="What is Python?")]
        
        result = await llm_manager_without_verification.invoke_with_mandatory_verification(
            messages=messages
        )
        ...
        assert result['model_used'] == llm_manager_without_verification.selected_model_id
        
        # Verify direct model invocation was called
        llm_manager_without_verification.selected_model.ainvoke.assert_called_once_with(messages)
        
    @pytest.mark.asyncio
    async def test_verification_service_failure_fallback(self, llm_manager_with_verification):
        """Test fallback to direct invocation when verification service fails."""
        # Mock verification service failure
        llm_manager_with_verification.verification_service.verify_response_with_retry.side_effect = \
            Exception("Verification service error")
    ...
    async def test_direct_invoke_method(self, llm_manager_with_verification):
        ...
        llm_manager_with_verification.selected_model.ainvoke.assert_called_once_with(messages)
    ...
    async def test_direct_invoke_error_handling(self, llm_manager_with_verification):
        ...
        # Mock model error
        llm_manager_with_verification.selected_model.ainvoke.side_effect = \
    ...
    def test_verification_status_without_service(self, llm_manager_without_verification):
    ...
    async def test_context_passing_to_verification(self, llm_manager_with_verification):
        ...
        call_args = llm_manager_with_verification.verification_service.verify_response_with_retry.call_args
    ...
    async def test_max_retries_parameter_passing(self, llm_manager_with_verification):
        ...
        mock_verification_response = MagicMock()
        llm_manager_with_verification.verification_service.verify_response_with_retry.return_value = (
            "Response", mock_verification_response
        )
    ...
    @pytest.mark.asyncio
    async def test_legacy_verification_method_deprecation_warning(self, llm_manager_with_verification):
    ...
    def test_model_configuration_with_verification(self):
        ...
        with patch.dict(os.environ, {'LLM_VERIFICATION_ENABLED': 'true'}), \
            ...
            assert manager.selected_model_id != manager.verification_model_id  # Should be different models
            
            # Verification model should be cost-effective
            assert "gemini" in manager.verification_model_id.lower() or "flash" in manager.verification_model_id.lower()
...
class TestLLMManagerVerificationRealScenarios:
    ...
    async def test_investigation_analysis_with_verification(self):
        ...
        with patch.dict(os.environ, {'LLM_VERIFICATION_ENABLED': 'true'}), \
             patch('app.service.llm_manager.get_verification_config'), \
             patch('app.service.llm_manager.LLMVerificationService') as mock_service_class:
            ...
            manager = LLMManager()
            manager.selected_model = MagicMock()
            manager.selected_model.ainvoke = AsyncMock(return_value=MagicMock(content="Initial analysis"))
            
            # Simulate investigation analysis request
            messages = [
                SystemMessage(content="You are a fraud detection specialist. Analyze the provided data for suspicious patterns."),
                HumanMessage(content="Analyze user ID 12345: Multiple logins from different countries within 2 hours, unusual transaction patterns, new payment methods added recently.")
            ]
            ...
            assert "Risk assessment" in result['response'] or "analysis" in result['response'].lower()
            assert result['verification_enabled'] is True
    ...
    async def test_entity_validation_with_verification(self):
        ...
        with patch.dict(os.environ, {'LLM_VERIFICATION_ENABLED': 'true'}), \
            ...
            manager = LLMManager()
            manager.selected_model = MagicMock()
            manager.selected_model.ainvoke = AsyncMock(return_value=MagicMock(content="Validation result"))
            
            messages = [
                SystemMessage(content="Validate the legitimacy of the provided entity based on available data."),
                HumanMessage(content="Validate email: john.doe@example.com, Phone: +1-555-0123, Address: 123 Main St, Anytown, ST 12345")
            ]
            ...
            context = {
                ...
                'entity_type': 'person',
                'validation_rules': ['email_format', 'phone_format', 'address_structure']
            }
    ...
    async def test_performance_under_load(self):
        ...
        with patch.dict(os.environ, {'LLM_VERIFICATION_ENABLED': 'true'}), \
            ...
            manager = LLMManager()
            manager.selected_model = MagicMock()
            manager.selected_model.ainvoke = AsyncMock(return_value=MagicMock(content="Response"))
