...
class TestMultiEntityInvestigationAPI:
    ...
    def test_start_multi_entity_investigation_success(self, client, sample_multi_entity_request):
        """Test successful multi-entity investigation start"""
        with patch("app.service.agent.multi_entity.investigation_orchestrator.get_multi_entity_orchestrator") as mock_orchestrator:
            ...
            assert data["boolean_logic"] == "(user_12345 AND transaction_67890) OR store_555"
...
class TestMultiEntityAPIPerformance:
    ...
    def test_large_entity_set_performance(self, client):
        ...
        entities = [
            {"entity_id": f"entity_{i}", "entity_type": "user"} 
            for i in range(10)
        ]
    ...
    def test_complex_boolean_logic_performance(self, client):
        ...
        entities = [{"entity_id": f"entity_{i}", "entity_type": "user"} for i in range(8)]
