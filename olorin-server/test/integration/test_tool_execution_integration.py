...
def test_complete_tool_execution_flow(test_db: Session, sample_investigation):
    ...
    device_exec = next((t for t in progress.tool_executions if t.tool_name == "fingerprint_analyzer"), None)
    ...
    failed_exec = next((t for t in progress.tool_executions if t.tool_name == "splunk_search"), None)
