...
async def test_raw_data_node():
    ...
    assert len(result_state["messages"]) > len(
        test_state["messages"]
    ), "Should add response message"
    ...
    if (
        ...
        and response_message.additional_kwargs
    ):
        ...
        if raw_results:
            ...
            logger.info(
                f"✓ Quality score: {raw_results.get('quality_report', {}).get('quality_score', 0):.2f}"
            )
            logger.info(f"✓ Records processed: {len(raw_results.get('data', []))}")
            logger.info(
                f"✓ Anomalies detected: {raw_results.get('anomalies_count', 0)}"
            )
        else:
...
async def main():
    ...
    try:
        ...
        logger.info(
            "✅ Phase 3: Raw Data Node LangGraph Integration - Core functionality working"
        )
