...
from app.utils.validation_engine import (
    ...
    validate_entity_field as engine_validate_field,
...
class TestCompleteValidationWorkflow:
    ...
    def test_transaction_validation_complete_workflow(self, validation_engine):
        ...
        assert execution_time_ms < 100, f"Complete transaction validation took {execution_time_ms}ms, should be <100ms"
        ...
        for expected in expected_validations:
            ...
            assert results[expected].is_valid is True, f"{expected} should be valid for good transaction"
    ...
    def test_fraudulent_transaction_detection(self, validation_engine):
        ...
        assert fraud_count >= 8, f"Should detect at least 8 fraud indicators, found {fraud_count}"
        
        # Check specific fraud detections
        assert not results['transaction_id.transaction_id'].is_valid, "Should detect XSS in transaction ID"
    ...
    def test_high_volume_validation_performance(self, comprehensive_validator):
        ...
        execution_time_ms = (end_time - start_time) * 1000
        assert execution_time_ms < 100, f"High volume validation took {execution_time_ms}ms, should be <100ms"
    ...
    def test_concurrent_validation_safety(self, validation_engine):
        ...
        assert len(invalid_results) == 0, f"All concurrent validations should be valid: {invalid_results}"
...
class TestSecurityIntegration:
    ...
    def test_comprehensive_xss_prevention(self, validation_engine):
        ...
        for payload in xss_payloads:
            for entity_type in entity_types_to_test:
                ...
                assert result.is_valid is False, f"XSS payload should be blocked for {entity_type}: {payload}"
    ...
    def test_comprehensive_sql_injection_prevention(self, validation_engine):
        ...
        for payload in sql_payloads:
            for entity_type in entity_types_to_test:
                ...
                assert result.is_valid is False, f"SQL injection should be blocked for {entity_type}: {payload}"
                assert result.severity in [ValidationSeverity.ERROR, ValidationSeverity.CRITICAL]
                assert "malicious" in result.error_message.lower()
    ...
    def test_comprehensive_fraud_pattern_detection(self, validation_engine):
        ...
        assert high_risk_count >= 3, f"Should detect at least 3 high-risk patterns, found {high_risk_count}"
        
        # Check specific fraud indicators
        email_result = results.get('email.email')
        if email_result:
            assert email_result.risk_score > 0.5, "Disposable email should have high risk"
        ...
        fraud_recommendations = [r for r in recommendations if "risk" in r.lower() or "fraud" in r.lower()]
        assert len(fraud_recommendations) >= 2, "Should provide fraud-related recommendations"
    ...
    def test_security_analysis_aggregation(self, validation_engine):
        ...
        found_metrics = [metric for metric in expected_metrics if metric in total_security_analysis]
        assert len(found_metrics) >= 2, f"Should have security metrics, found: {found_metrics}"
...
class TestRealWorldScenarios:
    ...
    def test_card_testing_fraud_scenario(self, validation_engine):
        ...
        execution_time_ms = (end_time - start_time) * 1000
        assert execution_time_ms < 500, f"Card testing scenario validation took {execution_time_ms}ms, should be <500ms"
            ...
            for result in results.values():
                ...
                if hasattr(result, 'financial_analysis') and result.financial_analysis:
                    if 'suspicious_amount_pattern' in result.financial_analysis:
                        assert result.financial_analysis['suspicious_amount_pattern'] is True
    ...
    def test_account_takeover_scenario(self, validation_engine):
        ...
        assert avg_suspicious_risk > avg_legitimate_risk + 0.3, "Suspicious transaction should have significantly higher risk"
        
        # Check for specific takeover indicators
        high_risk_suspicious = [r for r in suspicious_results.values() if r.risk_score > 0.7]
        assert len(high_risk_suspicious) >= 3, f"Should have multiple high-risk indicators for takeover, got {len(high_risk_suspicious)}"
    ...
    def test_money_laundering_scenario(self, validation_engine):
        ...
        high_value_transactions = [r for r in all_ml_results if 'amount' in r.field_name]
        
        # Should flag high-value transactions
        flagged_amounts = [r for r in high_value_transactions if r.risk_score > 0.5]
        assert len(flagged_amounts) >= 4, f"Should flag multiple high-value amounts, got {len(flagged_amounts)}"
        ...
        assert len(ml_indicators) >= 3, f"Should detect money laundering indicators, got {ml_indicators}"
    ...
    def test_synthetic_identity_fraud_scenario(self, validation_engine):
        ...
        assert len(suspicious_results) >= 2, f"Should detect synthetic identity patterns, got {len(suspicious_results)}"
        
        # Check specific indicators
        phone_result = results.get('phone_number.phone') or results.get('phone.phone')
        if phone_result:
            # Test phone numbers should be flagged
            assert phone_result.risk_score > 0.5, "Test phone number should be high risk"
        ...
        identity_recommendations = [r for r in all_recommendations if "identity" in r.lower() or "verification" in r.lower()]
        assert len(identity_recommendations) >= 1, "Should recommend additional identity verification"
...
class TestPerformanceAndScalability:
    ...
    def test_sustained_high_load_performance(self, validation_engine):
        ...
        assert actual_duration < test_duration_seconds * 1.5, f"Should complete within 1.5x target time, took {actual_duration}s"
        assert successful_validations >= total_transactions * 0.95, f"Should successfully validate 95%+ transactions, got {successful_validations}/{total_transactions}"
        
        # Calculate throughput
        throughput = successful_validations / actual_duration
        assert throughput >= 15, f"Should maintain >15 validations/second, got {throughput:.2f}"
    ...
    def test_memory_usage_stability(self, comprehensive_validator):
        ...
        assert object_growth < max_allowed_growth, f"Memory grew too much: {object_growth} objects (baseline: {baseline_objects})"
    ...
    def test_cache_effectiveness(self, comprehensive_validator):
        ...
        start_time = time.time()
        for _ in range(50):
            comprehensive_validator.validate_entity(EntityType.EMAIL, 'email', test_email)
        ...
        cache_improvement = cold_cache_time / warm_cache_time if warm_cache_time > 0 else 1
        assert cache_improvement >= 0.8, f"Cache should improve performance, ratio: {cache_improvement:.2f}"
        
        # Both should be fast regardless
        assert cold_cache_time < 0.5, f"Cold cache time should be <500ms, got {cold_cache_time * 1000:.2f}ms"
        assert warm_cache_time < 0.5, f"Warm cache time should be <500ms, got {warm_cache_time * 1000:.2f}ms"
...
class TestSystemIntegration:
    ...
    def test_validation_with_mocked_external_services(self):
        ...
        with patch('app.utils.validation_rules.network_rules.NetworkValidationRules._check_malicious_ip_lists') as mock_ip_check:
            with patch('app.utils.validation_rules.financial_rules.FinancialValidationRules._check_card_bin_database') as mock_bin_check:
                ...
                assert all(r.is_valid for r in results.values()), "All validations should pass with mocked services"
    ...
    async def test_async_validation_compatibility(self):
        ...
        async def async_validation_worker():
            ...
            result = engine.validate_entity(EntityType.EMAIL, 'email', 'async@example.com')
