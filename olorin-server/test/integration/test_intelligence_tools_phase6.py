...
async def test_social_media_profiling():
    ...
    try:
        ...
        logger.info(
            f"  Identity Confidence: {result['identity_verification']['confidence']}"
        )
        logger.info(
            f"  Verification Score: {result['identity_verification']['identity_score']}"
        )

        if result["behavior_analysis"]:
            logger.info(
                f"  Behavioral Profile: {result['behavior_analysis']['behavioral_profile']['posting_frequency']}"
            )
...
async def test_social_network_analysis():
    ...
    try:
        from app.service.agent.tools.intelligence_tools.social_network_analysis import (
        ...
        logger.info(
            f"  Network Density: {result['network_metrics']['network_density']}"
        )

        if result["community_detection"]:
            logger.info(
                f"  Communities Found: {result['community_detection']['total_communities']}"
            )
            suspicious = [
                c
                for c in result["community_detection"]["communities"]
                if c.get("type") == "suspicious"
            ]
            if suspicious:
...
async def test_social_media_monitoring():
    ...
    try:
        from app.service.agent.tools.intelligence_tools.social_media_monitoring import (
...
async def test_osint_data_aggregator():
    ...
    try:
        from app.service.agent.tools.intelligence_tools.osint_data_aggregator import (
        ...
        if result["data_correlation"]:
            logger.info(
                f"  Correlation Score: {result['data_correlation']['correlation_score']}"
            )
            logger.info(
                f"  Verified Connections: {result['data_correlation']['verified_connections']}"
            )

        if result["timeline_analysis"]:
            logger.info(
                f"  Timeline Events: {result['timeline_analysis']['total_events']}"
            )
...
async def test_people_search():
    ...
    try:
        ...
        # Test people search
        result = tool._run(
            person_name="John Doe", additional_identifiers={"email": "john@example.com"}
        )

        logger.info(f"✅ People Search Result:")
        logger.info(f"  Person: {result['person_name']}")
        logger.info(
            f"  Verification Score: {result['identity_verification']['verification_score']}"
        )
        logger.info(
            f"  Aliases Found: {len(result['identity_verification']['aliases'])}"
        )
...
async def test_darkweb_monitoring():
    ...
    try:
        from app.service.agent.tools.intelligence_tools.darkweb_monitoring import (
        ...
        for threat in threats[:2]:  # Show first 2 threats
            logger.info(
                f"    - {threat['type']}: {threat['severity']} (confidence: {threat['confidence']})"
            )
    ...
    try:
        ...
        for domain in domains:
            ...
            intelligence_mentions = [
                kw for kw in intelligence_keywords if kw in objectives_text
            ]
        ...
        logger.info(
            f"  Domains with intelligence tools: {integrated_domains}/{len(domains)}"
        )
        logger.info(
        ...
        return (
            integrated_domains >= 4
        )  # At least 80% of domains should have intelligence integration
...
async def main():
    ...
    test_functions = [
        ...
        (
            ...
            test_domain_objectives_intelligence_integration,
        ),
    ]
    ...
    if failed == 0:
        logger.info(
            "🎉 ALL PHASE 6 TESTS PASSED! Intelligence tools successfully integrated."
        )
        logger.info("✅ Phase 6: Advanced Intelligence Gathering Tools - COMPLETED")
    else:
        logger.warning(
            f"⚠️ {failed} Phase 6 tests failed. Please review the issues above."
        )
