...
def create_test_node(name: str, status: str):
    ...
    async def test_node(state: TestState) -> TestState:
        ...
        new_message = AIMessage(content=f"Completed {name} analysis with status: {status}")
        
        return {
            "messages": [new_message],
            "redis_status": f"{name} completed"
        }
...
async def test_redis_with_simple_graph():
    ...
    try:
        ...
        # Add test nodes
        builder.add_node("device_analysis", create_test_node("Device Analysis", "suspicious"))
        builder.add_node("location_check", create_test_node("Location Check", "high_risk"))
        builder.add_node("final_assessment", create_test_node("Final Assessment", "fraud_detected"))
        ...
        initial_state = TestState(
            messages=[HumanMessage(content=f"Investigate suspicious transaction: ${test_data['transaction_amount']}")],
