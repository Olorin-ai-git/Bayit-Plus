...
class StructuredAPITester:
    ...
    async def test_fraud_investigation_scenario(self):
        ...
        try:
            ...
            logger.info(
                f"   Entity: {investigation_data['entity_type']} - {investigation_data['entity_id']}"
            )
            logger.info(
                f"   Amount: ${investigation_data['context']['transaction']['amount']}"
            )
            logger.info(
                f"   Location: {investigation_data['context']['network']['city']}, {investigation_data['context']['network']['country']}"
            )
            logger.info(
                f"   Risk Signals: {len(investigation_data['context']['triggers'])} triggers detected"
            )
    ...
    async def _analyze_investigation_results(self, result: dict):
        ...
        if "successful_agents" in result:
            logger.info(
                f"✅ Successful Agents: {', '.join(result['successful_agents'])}"
            )
...
async def main():
    ...
    try:
        ...
        if response.status_code != 200:
            logger.error(
                "❌ Backend server is not responding. Please start the backend server first."
            )
            logger.error(
                "   Run: cd olorin-server && poetry run python -m app.local_server"
            )
            return False
    except requests.exceptions.RequestException:
        logger.error(
            "❌ Cannot connect to backend server. Please ensure it's running on localhost:8090"
        )
        logger.error(
            "   Run: cd olorin-server && poetry run python -m app.local_server"
        )
    ...
    if success:
        ...
        logger.info(
            "   The structured investigation orchestrator is working correctly!"
        )
    else:
