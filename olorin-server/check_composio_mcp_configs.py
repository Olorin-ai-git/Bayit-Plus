...
print("-" * 80)
print(
    f"{'COMPOSIO_API_KEY':35} {'✅ Configured' if COMPOSIO_API_KEY else '❌ Not configured':20}"
)
print(
    f"{'COMPOSIO_ENCRYPTION_KEY':35} {'✅ Configured' if COMPOSIO_ENCRYPTION_KEY else '❌ Not configured':20}"
)
                    ...
                    if line.startswith("data: "):
                        ...
                        try:
                            ...
                            if "result" in result:
                                ...
                                if (
                                    isinstance(tools_list, dict)
                                    and "tools" in tools_list
                                ):
...
for key, url in MCP_CONFIGS.items():
    if url:
        ...
        if result["status"] == "success":
            print(
                f"  ✅ Connected - Tools available: {result.get('tools_count', 'unknown')}"
            )
