...
for pattern_name, field in patterns.items():
    ...
    if fraud_stats and clean_stats:
        ...
        print(
            f"   Fraud: mean={fraud_mean:.2f}, median={fraud_stats['median']:.2f}, min={fraud_stats['min']:.2f}, max={fraud_stats['max']:.2f}"
        )
        print(
            f"   Clean: mean={clean_mean:.2f}, median={clean_stats['median']:.2f}, min={clean_stats['min']:.2f}, max={clean_stats['max']:.2f}"
        )
        print(
            f"   Difference: {diff_pct:+.1f}% {'(FRAUD HIGHER)' if diff_pct > 0 else '(CLEAN HIGHER)'}"
        )
        print(f"")
...
for i, entity in enumerate(fraud_data[:5], 1):
    ...
    print(
        f"   Amounts: avg=${entity['AVG_AMOUNT']:.2f}, total=${entity['TOTAL_AMOUNT']:.2f}"
    )
    print(
        f"   MODEL_SCORE: avg={entity['AVG_MODEL_SCORE']:.3f}, max={entity['MAX_MODEL_SCORE']:.3f}"
    )
    print(
        f"   Risk Weighted: total={entity['TOTAL_RISK_WEIGHTED']:.2f}, avg={entity['AVG_RISK_WEIGHTED']:.2f}"
    )
    print(f"   Velocity: {entity['TX_VELOCITY_PER_HOUR']:.2f} txs/hour")
    print(
        f"   Diversity: {entity['UNIQUE_DEVICES']} devices, {entity['UNIQUE_IPS']} IPs, {entity['UNIQUE_MERCHANTS']} merchants"
    )
    print(f"")
...
distinguishing = [
    (k, v["diff_pct"]) for k, v in comparison_results.items() if abs(v["diff_pct"]) > 30
]
...
fraud_avg_model = (
    comparison_results.get("avg_model_score", {}).get("fraud", {}).get("mean", 0)
)
clean_avg_model = (
    comparison_results.get("avg_model_score", {}).get("clean", {}).get("mean", 0)
)
...
fraud_velocity = (
    comparison_results.get("tx_velocity_per_hour", {}).get("fraud", {}).get("mean", 0)
)
clean_velocity = (
    comparison_results.get("tx_velocity_per_hour", {}).get("clean", {}).get("mean", 0)
)
print(f"2. VELOCITY-BASED (transaction rate):")
print(f"   ORDER BY (COUNT(*) / time_span_hours) DESC")
print(
    f"   Fraud velocity: {fraud_velocity:.2f} txs/hr, Clean: {clean_velocity:.2f} txs/hr"
)
print(f"")
