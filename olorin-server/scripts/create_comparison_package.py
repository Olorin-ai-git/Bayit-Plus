...
from app.service.investigation.auto_comparison import package_comparison_results, generate_summary_html
...
async def find_last_comparisons() -> List[Dict[str, Any]]:
    ...
    for search_path in search_paths:
        if search_path.exists():
            ...
            for html_file in search_path.glob("comparison_*.html"):
                ...
                match = re.match(r"comparison_([^_]+)_(.+?)_(\d{8}_\d{6})\.html", html_file.name)
                if match:
    ...
    for i, report in enumerate(top_3, 1):
        print(f"  {i}. {report['entity_type']}: {report['entity_value']} ({report['timestamp']})")
                        ...
                        with open(metadata_file) as f:
                            ...
                            if isinstance(config, dict):
                                ...
                                if isinstance(entities, list) and len(entities) > 0:
                                    entity_config = entities[0] if isinstance(entities[0], dict) else {}
                                    if entity_config.get("entity_value") == entity_value or entity_config.get("entity_id") == entity_value:
    ...
    with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
        ...
        for i, result in enumerate(comparison_results, 1):
            ...
            if investigation_folder and investigation_folder.exists():
                folder_name_in_zip = f"investigations/investigation_{i}_{investigation_id or 'unknown'}"
                ...
                for root, dirs, files in os.walk(investigation_folder):
                    ...
                    for file in files:
                        ...
                        arcname = folder_name_in_zip / file_path.relative_to(investigation_folder)
