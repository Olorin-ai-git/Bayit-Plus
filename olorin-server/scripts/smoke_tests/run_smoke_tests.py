...
def print_report_summary(report, show_details: bool = True):
    ...
    for service in report.services:
        ...
        if service.enabled and show_details:
            ...
            print(f"   Tests: {len(service.passed_tests)} passed, {len(service.failed_tests)} failed, {len(service.skipped_tests)} skipped")
            
            # Show critical failures
            critical_failures = [t for t in service.failed_tests if t.is_critical_failure]
        ...
        if args.quick:
            ...
            if args.output:
                with open(args.output, 'w') as f:
                    f.write(output if args.json else json.dumps(health_status, indent=2, default=str))
                if not args.silent:
            ...
            if not args.silent:
                if args.services:
                    print(f"Running smoke tests for services: {', '.join(args.services)}")
                else:
            ...
            if args.output:
                with open(args.output, 'w') as f:
                    f.write(output if args.json else json.dumps(report.to_dict(), indent=2, default=str))
                if not args.silent:
                    print(f"\nResults written to {args.output}")
            elif args.json:
                print(output)
