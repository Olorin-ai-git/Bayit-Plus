...
async def main():
    ...
    windows_to_try = [
        (
            recent_end - timedelta(days=14),
            recent_end,
            recent_end - timedelta(days=28),
            recent_end - timedelta(days=14),
        ),  # Recent 14d vs Previous 14d
        (
            recent_end - timedelta(days=14),
            recent_end,
            recent_end - timedelta(days=42),
            recent_end - timedelta(days=28),
        ),  # Recent 14d vs 2-4 weeks ago
        (
            recent_end - timedelta(days=30),
            recent_end,
            recent_end - timedelta(days=60),
            recent_end - timedelta(days=30),
    ...
    for window_a_start, window_a_end, window_b_start, window_b_end in windows_to_try:
        ...
        logger.info(
            f"  Window A ({window_a_start.date()} to {window_a_end.date()}): {availability['window_a']['count']} transactions"
        )
        logger.info(
            f"  Window B ({window_b_start.date()} to {window_b_end.date()}): {availability['window_b']['count']} transactions"
        )
    ...
    if not found_valid_windows:
        logger.error(
            "❌ Could not find time windows with data in both periods for this entity."
        )
