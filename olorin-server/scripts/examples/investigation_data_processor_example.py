...
def print_processing_summary(result: ProcessedInvestigationData):
    ...
    print(
        f"  Agents Used: {len(result.agents_used)} ({', '.join(result.agents_used[:3])}{'...' if len(result.agents_used) > 3 else ''})"
    )
    print(
        f"  Tools Used: {len(result.tools_used)} ({', '.join(result.tools_used[:3])}{'...' if len(result.tools_used) > 3 else ''})"
    )
...
def print_detailed_data_samples(result: ProcessedInvestigationData):
    ...
    if result.tool_executions:
        ...
        print(
            f"  Input Parameters: {json.dumps(execution.input_parameters, indent=2)[:200]}..."
        )
        print()
