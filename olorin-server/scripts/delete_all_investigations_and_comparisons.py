    ...
    try:
        with registry._get_connection() as conn:
            ...
            if inv_count > 0 or file_count > 0:
                ...
                cursor.execute("DELETE FROM audit_log WHERE resource_type IN ('investigation', 'file')")
            ...
            if delete_files and comparisons:
                ...
                for comp_id, canonical_path in comparisons:
                    try:
                        ...
                        logger.warning(f"Failed to delete comparison file {canonical_path}: {e}")
                
                print(f"   ✅ Deleted {deleted_files_count} comparison file(s)/folder(s)")
...
This is irreversible!
    ...
    # Confirmation prompt
    if not args.yes:
        print("⚠️  WARNING: This will permanently delete ALL investigations and comparisons!")
        if args.files:
