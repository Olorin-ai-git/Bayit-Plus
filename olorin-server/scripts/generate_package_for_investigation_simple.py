    ...
    try:
        ...
        with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
            ...
            manifest = {
                ...
                "status": state.status,
                "completed_at": state.updated_at.isoformat() if state.updated_at else None,
...
async def main():
    """Main entry point."""
    if len(sys.argv) < 2:
        print("Usage: python scripts/generate_package_for_investigation_simple.py <investigation_id>")
        print("\nExample:")
        print("  python scripts/generate_package_for_investigation_simple.py auto-comp-ee88621fd85b")
        sys.exit(1)
