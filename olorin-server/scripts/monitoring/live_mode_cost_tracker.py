...
class LiveModeCostTracker:
    ...
    def analyze_investigation_cost(self, investigation_dir: Path) -> CostBreakdown:
        ...
        if report_files:
            with open(report_files[0]) as f:
                ...
                cost.snowflake_cost = cost.duration_seconds * self.cost_estimates['snowflake_per_second']
        ...
        if server_logs_file.exists():
            with open(server_logs_file) as f:
                ...
                cost.external_apis_cost += virustotal_calls * self.cost_estimates['virustotal_per_query']
            ...
            with open(thought_file) as f:
                try:
                    ...
                    if 'claude' in thought_file.name.lower():
                        cost.llm_cost += (estimated_tokens / 1000) * self.cost_estimates['claude_per_1k_tokens']
                    else:
                        cost.llm_cost += (estimated_tokens / 1000) * self.cost_estimates['openai_per_1k_tokens']
    ...
    def check_budget_status(self, daily_budget: float = 500.0, monthly_budget: float = 15000.0) -> Dict:
        ...
        if daily_percentage > 80:
            alerts.append(f"🚨 Daily budget at {daily_percentage:.1f}% (${daily_usage:.2f}/${daily_budget:.2f})")
        if monthly_percentage > 80:
            alerts.append(f"🚨 Monthly budget at {monthly_percentage:.1f}% (${monthly_usage:.2f}/${monthly_budget:.2f})")
    ...
    def print_cost_report(self, days_back: int = 7):
        ...
        print(f"   Average Cost/Investigation: ${summary['average_cost_per_investigation']:.2f}")
        ...
        print(f"\n📈 BUDGET STATUS:")
        print(f"   Daily Usage: ${budget_status['daily_usage']:.2f} ({budget_status['daily_percentage']:.1f}%)")
        print(f"   Monthly Usage: ${budget_status['monthly_usage']:.2f} ({budget_status['monthly_percentage']:.1f}%)")
        ...
        for inv in report['investigations'][:5]:  # Show last 5
            print(f"   {inv['id'][:30]}... | ${inv['total_cost']:.2f} | {inv['duration_minutes']:.1f}min")
...
def main():
    ...
    parser.add_argument('--logs-dir', default='logs/investigations', help='Investigations directory')
