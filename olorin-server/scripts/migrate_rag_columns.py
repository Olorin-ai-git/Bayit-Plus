...
async def main():
    ...
    if verification["all_migrated"]:
        ...
        print("\n📋 Summary:")
        print(f"   Documents table: {'✅ Migrated' if verification['documents_table']['migrated'] else '❌ Failed'}")
        print(f"   Document chunks table: {'✅ Migrated' if verification['document_chunks_table']['migrated'] else '❌ Failed'}")
        return 0
    else:
