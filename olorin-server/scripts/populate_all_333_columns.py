...
class ComprehensiveDataGenerator:
    ...
    def _generate_text(
        self, field_name: str, is_high_risk: bool, is_fraud: bool
    ) -> str:
...
async def populate_all_columns():
    ...
    try:
        ...
        fields_to_populate = {
            k: v for k, v in null_fields.items() if k not in exclude_fields
        }
        ...
        for i in range(0, len(records), batch_size):
            ...
            async with conn.transaction():
                for rec in batch:
                    ...
                    context = {"is_high_risk": risk_score > 0.7, "is_fraud": is_fraud}
        ...
        logger.info(
            f"Avg Fields/Record:      {total_fields_populated // updated_count if updated_count > 0 else 0}"
        )
        logger.info("=" * 80)
