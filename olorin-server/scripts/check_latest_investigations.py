            ...
            for i, inv in enumerate(invs, 1):
                ...
                if inv.progress_json:
                    try:
                        ...
                        risk_score = progress.get('overall_risk_score') or progress.get('risk_score')
                        transaction_scores = progress.get('transaction_scores')
                        domain_findings = progress.get('domain_findings', {})
                        
                        print(f"   Risk Score: {risk_score}")
                        print(f"   Has Transaction Scores: {transaction_scores is not None}")
                        ...
                        if inv.settings_json:
                            try:
                                ...
                                if entities:
                                    ...
                                    entity_id = entity.get('entity_value') or entity.get('entity_id', 'unknown')
