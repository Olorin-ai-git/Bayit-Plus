    ...
    try:
        with zipfile.ZipFile(package_path, 'r') as zip_ref:
            ...
            for html_file in html_files:
                ...
                confusion_pattern = r'(TP|FP|TN|FN|Precision|Recall|F1|Accuracy)[:\s]*([0-9.]+)'
...
def main():
    ...
    packages = sorted(comparison_dir.glob("*.zip"), key=lambda p: p.stat().st_mtime, reverse=True)
    ...
    if packages:
        ...
        if analysis['risk_scores']:
            ...
            print(f"  - Avg: {sum(analysis['risk_scores']) / len(analysis['risk_scores']):.3f}")
        else:
        ...
        if analysis['has_confusion_data']:
            for metric, values in analysis['confusion_metrics'].items():
                if values:
                    print(f"  - {metric.upper()}: {values[:5]} (found {len(values)} values)")
        else:
            print("  - No confusion matrix data found in HTML")
        print()
    
    # Check for confusion table HTML files
    confusion_files = list(Path("artifacts/comparisons/auto_startup").glob("confusion_table_*.html"))
    ...
    if confusion_files:
        ...
        for cf in sorted(confusion_files, key=lambda p: p.stat().st_mtime, reverse=True)[:5]:
            print(f"  - {cf.name} (modified: {datetime.fromtimestamp(cf.stat().st_mtime)})")
    else:
