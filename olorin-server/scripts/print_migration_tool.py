...
class PrintMigrationTool:
    ...
    def migrate_file(self, file_path: Path, dry_run: bool = True) -> Dict:
        ...
        # Perform migration
        migrated_content, migration_info = self._migrate_content(original_content, file_path)
    ...
    def _migrate_content(self, content: str, file_path: Path) -> Tuple[str, Dict]:
        ...
        for line_no, line in enumerate(lines, 1):
            if 'print(' in line:
                migrated_line, migration_info = self._migrate_line(line, line_no, file_path)
                if migration_info:
    ...
    def _migrate_line(self, line: str, line_no: int, file_path: Path) -> Tuple[str, Optional[Dict]]:
    ...
    def _should_skip_line(self, line: str, file_path: Path) -> bool:
        ...
        skip_patterns = [
            ...
            'print(' in line and ('join(' in line or 'format(' in line) and len(line) > 120
        ]
        
        return any(skip_patterns)
    
    def _apply_migration_rule(self, line: str, rule: MigrationRule, match: re.Match) -> str:
    ...
    def migrate_by_priority(self, priority: str, dry_run: bool = True) -> Dict:
        ...
        analysis_categories = self.analysis_data.get('analysis_result', {}).get('categories', {})
    ...
    def generate_migration_report(self, results: Dict) -> str:
        ...
        for file_result in results['file_results']:
            if file_result['success'] and file_result['content_changed']:
                report.append(f"  ✅ {file_result['file_path']}: {file_result['migrations_made']} migrations")
                
                for detail in file_result['migration_details'][:3]:  # Show first 3
                    report.append(f"    Line {detail['line_no']}: {detail['log_level']}")
                ...
                report.append(f"  ❌ {file_result['file_path']}: {file_result.get('error', 'Unknown error')}")
...
def main():
    parser = argparse.ArgumentParser(description='Migrate print statements to unified logging')
    parser.add_argument('--analysis-file', default='print_analysis_results.json', 
