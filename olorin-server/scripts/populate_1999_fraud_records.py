...
FRAUD_IPS = [
    "185.220.101.42",
    "185.220.101.43",
    "185.220.101.44",  # Russia
    "117.22.69.113",
    "117.22.69.114",
    "117.22.69.115",  # China
    "41.17.184.175",
    "41.17.184.176",
    "41.17.184.177",  # Nigeria
    "123.251.74.248",
    "123.251.74.249",
    "123.251.74.250",  # Vietnam
]
...
async def populate_1999_records():
    ...
    try:
                ...
                for i in range(batch_start, batch_end):
                    ...
                    for col_info in columns_info:
                        ...
                        if col_name == "tx_id_key":
                            ...
                            if is_fraud:
                                record[col_name] = (
                                    f"user{i}@{random.choice(['temp-mail.org', '10minutemail.com', 'fraud-example.com'])}"
                                )
                            else:
                                record[col_name] = f"legitimate{i}@gmail.com"
                        elif col_name == "first_name":
                            record[col_name] = random.choice(
                                ["John", "Jane", "Bob", "Alice", "Charlie", "Diana"]
                            )
                        elif col_name == "last_name":
                            record[col_name] = random.choice(
                                ["Smith", "Jones", "Brown", "Wilson", "Taylor", "Moore"]
                            )
                            ...
                            record[col_name] = (
                                f"{record.get('first_name', 'John')} {record.get('last_name', 'Doe')}"
                            )
                            ...
                            record[col_name] = base_time - timedelta(
                                hours=i % 48, minutes=i % 60
                            )
                        elif col_name == "ip":
                            ...
                            if is_fraud:
                                ...
                                record[col_name] = generator.generate_value(
                                    col_name, data_type, context
                                )
                        else:
        ...
        logger.info(
            f"  • IP Clustering: {len(FRAUD_IPS)} suspicious IPs reused across multiple transactions"
        )
