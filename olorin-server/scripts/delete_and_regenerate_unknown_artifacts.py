...
from app.router.models.investigation_comparison_models import ComparisonRequest, WindowPreset
...
async def regenerate_comparison_for_entity(entity_type: str, entity_id: str):
    ...
    try:
        ...
        if result.get("status") == "success":
            logger.info(f"✅ Successfully regenerated comparison for {entity_type}:{entity_id}")
            logger.info(f"   Investigation ID: {result.get('investigation_id', 'N/A')}")
            logger.info(f"   Report: {result.get('report_path', 'N/A')}")
            return True
        else:
            logger.error(f"❌ Failed to regenerate comparison: {result.get('error', 'Unknown error')}")
            return False
            
    except Exception as e:
        logger.error(f"❌ Error regenerating comparison for {entity_type}:{entity_id}: {e}", exc_info=True)
        return False
