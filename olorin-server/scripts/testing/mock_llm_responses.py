...
class MockLLMResponseGenerator:
    ...
    def generate_network_response(
        ...
        self.logger.warning(
            "⚠️ ⚠️ ⚠️  USING MOCK LLM - NOT REAL CLAUDE/GPT ANALYSIS ⚠️ ⚠️ ⚠️"
        )
        self.logger.warning(
            "    This is a MOCK response for testing only - NO actual LLM reasoning"
        )
        ...
        if entity_risk_score is not None:
            ...
            self.logger.warning(
                f"    Using entity risk score from Snowflake: {entity_risk_score:.4f} -> Network risk: {risk_score:.4f}"
            )
        else:
...
- Recommended action timeline: {'Immediate' if risk_score > 0.7 else 'Within 24 hours'}"""
        ...
        self.logger.info(
            f"Generated mock network response for {scenario.value}: risk={risk_score:.2f}"
        )
        return response

    def generate_device_response(
        self,
        scenario: ScenarioType,
        investigation_id: str,
        entity_risk_score: Optional[float] = None,
        ...
        if entity_risk_score is not None:
            ...
            self.logger.warning(
                f"    Device risk adjusted for entity risk {entity_risk_score:.4f} -> {risk_score:.4f}"
            )
        else:
...
- Device trust level: {self._get_trust_level(risk_score)}"""
        ...
        self.logger.info(
            f"Generated mock device response for {scenario.value}: risk={risk_score:.2f}"
        )
        return response
    ...
    def generate_location_response(
        ...
        if entity_risk_score is not None:
            ...
            self.logger.warning(
                f"    Location risk adjusted for entity risk {entity_risk_score:.4f} -> {risk_score:.4f}"
            )
        else:
...
- Location verification required: {'Yes' if risk_score > 0.5 else 'Recommended'}"""
        ...
        self.logger.info(
            f"Generated mock location response for {scenario.value}: risk={risk_score:.2f}"
        )
        return response

    def generate_logs_response(
        self,
        scenario: ScenarioType,
        investigation_id: str,
        entity_risk_score: Optional[float] = None,
        ...
        if entity_risk_score is not None:
            ...
            self.logger.warning(
                f"    Logs risk adjusted for entity risk {entity_risk_score:.4f} -> {risk_score:.4f}"
            )
        else:
...
- Monitoring priority: {profile.threat_level}"""
        ...
        self.logger.info(
            f"Generated mock logs response for {scenario.value}: risk={risk_score:.2f}"
        )
        return response
...
confirming the validity of the risk assessment."""
        ...
        self.logger.info(
            f"Generated mock risk assessment for {scenario.value}: overall_risk={risk_score:.2f}"
        )
        return response
...
def generate_mock_response(
    ...
    logger.warning(
        "    This is MOCK data for testing - NO chain of thought, NO tool reasoning"
    )
    ...
    if entity_risk_score is not None:
        logger.warning(
            f"📊 Entity has REAL risk score from Snowflake: {entity_risk_score:.4f}"
        )
    ...
    if agent_type == "network":
        return mock_response_generator.generate_network_response(
            scenario_type, investigation_id, entity_risk_score
        )
    elif agent_type == "device":
        return mock_response_generator.generate_device_response(
            scenario_type, investigation_id, entity_risk_score
        )
    elif agent_type == "location":
        return mock_response_generator.generate_location_response(
            scenario_type, investigation_id, entity_risk_score
        )
    elif agent_type == "logs":
        return mock_response_generator.generate_logs_response(
            scenario_type, investigation_id, entity_risk_score
        )
        ...
        if context_data:
            ...
            logger.warning(
                "Risk assessment requested without context data, using minimal response"
            )
            return mock_response_generator.generate_risk_assessment_response(
                scenario_type,
                investigation_id,
        ...
        verification_score = (
            0.75 if entity_risk_score and entity_risk_score > 0.5 else 0.65
        )
        return f"""
