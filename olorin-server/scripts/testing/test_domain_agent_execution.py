...
async def test_investigation_with_domain_agents():
    ...
    try:
        ...
        messages = [HumanMessage(content=f"Investigate potential fraud for IP address {entity_value}")]
        ...
        try:
            ...
            if result:
                ...
                if executed_domains:
                    ...
                    investigation_metadata = getattr(result, 'investigation_metadata', {})
                    quality_score = investigation_metadata.get('quality_score', 'unknown')
