...
def test_with_variation():
    ...
    print(
        f"✅ All responses contain risk analysis: {all('risk' in r.lower() or 'score' in r.lower() for r in responses)}"
    )
    print(
        f"✅ Response lengths vary: {len(set(len(r) for r in responses))} different lengths"
    )
