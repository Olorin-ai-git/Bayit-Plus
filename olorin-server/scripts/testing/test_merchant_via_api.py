...
def test_merchant_agent_via_api():
    ...
    try:
        ...
        print("   Please start backend: cd olorin-server && poetry run uvicorn app.main:app --host 0.0.0.0 --port 8090")
        return False
    ...
    try:
        ...
        while time.time() - start_time < max_wait:
            ...
            if status_response.status_code == 200:
                ...
                progress = investigation.get("progress_json", {}).get("progress_percentage", 0)
        ...
        if logs_dir.exists():
            ...
            if investigation_folders:
                ...
                if validation_file.exists():
                    ...
                    print(f"   Validation Complete: {validation_data.get('validation_complete', False)}")
                    if validation_data.get('validation_complete'):
                        print(f"   Predicted Risk: {validation_data.get('predicted_risk_score', 'N/A')}")
                        print(f"   Actual Fraud Rate: {validation_data.get('actual_fraud_rate', 'N/A')}")
                        print(f"   Prediction Correct: {validation_data.get('prediction_correct', 'N/A')}")
                else:
        ...
        report_has_validation = 'report_file' in locals() and report_file.exists() and 'Merchant Agent Validation' in report_content
