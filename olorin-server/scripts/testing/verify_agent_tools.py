...
def main():
    ...
    for tool in tools:
        ...
        elif any(x in tool_name.lower() for x in ['blockchain', 'crypto', 'nft', 'defi']):
            categories["Blockchain"].append(tool_name)
        elif any(x in tool_name.lower() for x in ['ml', 'anomaly', 'pattern', 'behavioral', 'risk_scoring']):
        ...
        elif any(x in tool_name.lower() for x in ['osint', 'social', 'people', 'dark', 'deep', 'business_intel']):
