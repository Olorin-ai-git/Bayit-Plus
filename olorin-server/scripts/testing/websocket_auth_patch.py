    ...
    def websocket_monitor():
        try:
            ...
            try:
                ...
                self.log_monitoring_warning(
                    "WebSocket",
                    "WebSocket auth fix not available, using basic connection",
                )
            ...
            if auth_available:
                ...
                self.log_monitoring_success(
                    "WebSocket", "Using authenticated WebSocket connection"
                )
            else:
                ...
                self.log_monitoring_warning(
                    "WebSocket", "Using basic WebSocket connection (may fail with 403)"
                )
                ...
                if close_status_code:
                    if close_status_code == 1008:  # Policy violation (auth failure)
                        self.log_monitoring_error(
                            "WebSocket",
                            f"Authentication failed (403 Forbidden) - check JWT token",
                        )
                    else:
                        self.log_monitoring_warning(
                            "WebSocket",
...
def apply_websocket_auth_patch():
    ...
    method_pattern = (
        r"(    def start_websocket_monitoring\(self\):.*?)(\n    def \w+|$)"
    )
    ...
    # Replace the method
    new_content = re.sub(
        method_pattern, replacement_method + r"\\2", content, flags=re.DOTALL
    )
