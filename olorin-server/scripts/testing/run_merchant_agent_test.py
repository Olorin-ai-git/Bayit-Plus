    ...
    try:
        ...
        while time.time() - start_time < max_wait:
            ...
            if status_response.status_code == 200:
                ...
                merchant_agent = next(
                    (a for a in agent_statuses if a.get("agent_name") == "merchant"),
                    None,
                )
                ...
                if progress > last_progress:
                    ...
                    if merchant_agent:
                        print(
                            f"   ✅ Merchant agent: {merchant_agent.get('status', 'unknown')}"
                        )
        ...
        if final_response.status_code == 200:
            ...
            merchant_agent = next(
                (a for a in agent_statuses if a.get("agent_name") == "merchant"), None
            )
            if merchant_agent:
        ...
        if logs_dir.exists():
            ...
            if investigation_folders:
                ...
                if validation_file.exists():
                    ...
                    print(
                        f"      Validation Complete: {validation.get('validation_complete', False)}"
                    )
                    if validation.get("validation_complete"):
                        print(
                            f"      Predicted Risk: {validation.get('predicted_risk_score', 'N/A')}"
                        )
                        print(
                            f"      Actual Fraud Rate: {validation.get('actual_fraud_rate', 'N/A')}"
                        )
                        print(
                            f"      Prediction Correct: {validation.get('prediction_correct', 'N/A')}"
                        )
                        print(
                            f"      Validation Quality: {validation.get('validation_quality', 'N/A')}"
                        )
                else:
