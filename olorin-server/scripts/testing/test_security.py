...
from app.models.validation import ValidatedInvestigationRequest, ValidatedUserID
...
class SecurityTester:
    ...
    def test_password_security(self):
        ...
        if not verify_password("wrong_password", hashed):
            self.log_test(
                "Password Rejection", True, "Wrong passwords correctly rejected"
            )
        else:
    ...
    def test_jwt_tokens(self):
        ...
        try:
            # Create token
            token = create_access_token(data={"sub": "testuser", "scopes": ["read"]})
    ...
    def test_encryption(self):
        ...
        try:
            ...
            if decrypted == test_data:
                self.log_test(
                    "String Encryption", True, "String encryption/decryption works"
                )
            else:
            ...
            if decrypted_dict == test_dict:
                self.log_test(
                    "Dict Encryption", True, "Dictionary encryption/decryption works"
                )
            else:
                self.log_test(
                    "Dict Encryption", False, "Dictionary decryption mismatch"
                )
    ...
    def test_field_encryption(self):
        ...
        try:
            ...
            has_encrypted_fields = any(
                key.startswith("enc_") for key in encrypted_data.keys()
            )
        ...
        async with httpx.AsyncClient() as client:
            ...
            try:
                ...
                if response.status_code == 401:
                    self.log_test(
                        "API Authentication", True, "Unauthorized access blocked"
                    )
                else:
                    self.log_test(
                        "API Authentication",
