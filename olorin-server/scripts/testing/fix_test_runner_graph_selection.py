...
async def test_minimal_hybrid_execution():
    ...
    try:
        ...
        from app.service.agent.orchestration.hybrid.graph.graph_builder import HybridGraphBuilder as ModularBuilder
        ...
        if tools:
            ...
            async def enhanced_tool_node(state):
                ...
                for msg in reversed(messages[-3:]):  # Check recent messages
                    if hasattr(msg, 'name') and hasattr(msg, 'content') and hasattr(msg, 'tool_call_id'):
                        ...
                        if "snowflake" in tool_name.lower():
                            ...
                            import json
                            try:
                                snowflake_data = json.loads(tool_content) if isinstance(tool_content, str) else tool_content
                            except json.JSONDecodeError:
                                snowflake_data = tool_content  # Keep original if not JSON
                ...
                result["tool_execution_attempts"] = state.get("tool_execution_attempts", 0) + 1
            ...
            graph_builder.add_conditional_edges("investigation", should_continue, {"tools": "tools", "__end__": END})
        ...
        async for chunk in graph.astream(initial_state, config):
            ...
            for node_name in node_names:
                if chunk[node_name]:
                    ...
                    tool_call_messages = [msg for msg in messages if hasattr(msg, 'tool_calls') and msg.tool_calls]
                    tool_result_messages = [msg for msg in messages if hasattr(msg, 'name')]
                    
                    if tool_call_messages:
                        print(f"      Tool calls found: {len(tool_call_messages)} messages with tool calls")
                    if tool_result_messages:
                        print(f"      Tool results found: {len(tool_result_messages)} result messages")
        ...
        if last_state:
            ...
            if tool_call_count > 0 and tool_result_count == 0:
                ...
                print("   🚨 ISSUE: Tool calls and results exist but tools_used not updated")
...
async def main():
    ...
    if result["success"]:
        ...
        print(f"   Hybrid graph: {result['tools_used']} tools, {result['tool_results']} results, {result['tool_attempts']} attempts")
        ...
        if result["tool_attempts"] == 0:
            ...
            print(f"   Tool calls: {result['tool_call_count']}, Results: {result['tool_result_count']}")
