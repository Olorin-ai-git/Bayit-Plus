...
async def test_real_api():
    ...
    try:
        ...
        validations = [
            ...
            ("Not mock data", "Mock" not in str(response) and "mock" not in str(response).lower()),
            ("Variable length", len(response.content) > 10),
            ("Contains analysis", any(word in response.content.lower() for word in ["risk", "fraud", "suspicious", "score"]))
        ]
