...
async def test_validation_service():
    ...
    try:
        ...
        investigation_id = f"test-merchant-validation-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
        ...
        if validation_results.get("validation_complete"):
            print(f"\n📊 Validation Results:")
            print(f"   Predicted Risk Score: {validation_results.get('predicted_risk_score', 'N/A')}")
            ...
            print(f"   Risk Correlation Error: {validation_results.get('risk_correlation_error', 'N/A')}")
            print(f"   Validation Quality: {validation_results.get('validation_quality', 'N/A')}")
            print(f"   Historical Transactions: {validation_results.get('historical_transactions', 0)}")
            print(f"   Actual Fraud Count: {validation_results.get('actual_fraud_count', 0)}")
            print(f"   Actual Total Transactions: {validation_results.get('actual_total_transactions', 0)}")
        else:
        ...
        if investigation_folder:
            ...
            if validation_file.exists():
                ...
                print(f"   File contains validation_complete: {saved_data.get('validation_complete', False)}")
            else:
        ...
        print("="*80)
        print(f"✅ Validation service initialized")
        print(f"{'✅' if validation_results.get('validation_complete') else '⚠️'} Validation executed")
        print(f"{'✅' if investigation_folder and (investigation_folder / 'merchant_validation_results.json').exists() else '⚠️'} Results saved")
        print("="*80)
...
async def main():
    ...
    if results.get("success"):
        ...
        if results.get("validation_results", {}).get("validation_complete"):
            ...
            print("   This is expected if historical data is not available for test entity")
    else:
