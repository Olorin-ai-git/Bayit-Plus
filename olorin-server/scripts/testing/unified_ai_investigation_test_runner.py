...
if "--mode" in sys.argv and "demo" in sys.argv[sys.argv.index("--mode") + 1]:
    ...
    print("🎭 TEST_MODE=demo detected in arguments - MockLLM will be used 🎭")
...
class UnifiedStructuredTestRunner:
    ...
    def __init__(self, config: TestConfiguration):
        ...
        self.investigation_log_manager = InvestigationLogManager(get_unified_logging_core())
    ...
    def _setup_logging(self):
        ...
        bridge_logger = get_bridge_logger('scripts.testing.unified_structured_test_runner')
        if self.config.mode == TestMode.DEMO:
            bridge_logger.debug("[Step 1.1.1] Command line argument parsing - Demo mode detected in sys.argv")
            bridge_logger.debug("[Step 1.1.1] Setting os.environ['TEST_MODE'] = 'demo' BEFORE agent imports")
    ...
    def _update_log_file_location(self, investigation_folder: str):
        ...
        log_filename = os.path.join(investigation_folder, f'structured_investigation_{timestamp}.log')
    ...
    async def load_snowflake_data(self) -> bool:
        ...
        try:
            ...
            self.logger.info(f"📊 Using entity grouping from .env: {default_group_by} (column: {group_by_column})")
            ...
            if results.get('status') == 'success':
                ...
                if entities:
                    ...
                    self.snowflake_entities = []  # Keep variable name for backward compatibility
                    ...
                    self.logger.info(f"✅ Loaded {len(self.snowflake_entities)} high-risk {entity_type_name} entities from {db_provider}")
                    for i, entity in enumerate(self.snowflake_entities[:5], 1):
                        entity_id = entity.get('ip') or entity.get('email') or entity.get('entity', 'N/A')
                        self.logger.info(f"  {i}. {default_group_by}: {entity_id}, Risk Score: {entity['risk_score']:.4f}")

                    return True
                else:
                    self.logger.warning(f"⚠️  No high-risk entities found in {db_provider}")
                    return False
            else:
                self.logger.error(f"❌ Failed to fetch from {db_provider}: {results.get('error')}")
                return False

        except Exception as e:
            self.logger.error(f"❌ Error loading {db_provider} data: {e}")
            return False
    ...
    async def get_available_scenarios(self) -> List[str]:
        ...
        try:
            async with self.session.get(f"{self.config.server_url}/v1/structured/scenarios") as response:
                if response.status == 200:
    ...
    async def run_single_scenario_test(self, scenario_name: str) -> InvestigationResult:
        ...
        try:
            ...
            entity_type_str = context.entity_type.value if hasattr(context.entity_type, 'value') else str(context.entity_type)
            ...
            metadata = {
                "investigation_id": investigation_id,
                "entity_id": context.entity_id,
                "entity_type": context.entity_type.value if hasattr(context.entity_type, 'value') else str(context.entity_type),
                "scenario": scenario_name,
                "test_mode": self.config.mode.value,
            ...
            investigation_log_handler = self.investigation_log_manager.start_investigation_logging(
                investigation_id=investigation_id,
                metadata=metadata,
            ...
            if investigation_log_handler:
                self._active_investigation_handlers[investigation_id] = investigation_log_handler
                self.logger.info(f"✅ Investigation log handler started: investigation.log will be created")
            else:
                self.logger.warning(f"⚠️ Failed to start investigation log handler")
            ...
            try:
                ...
                with get_db_session() as db:
                    ...
                    existing_state = db.query(InvestigationState).filter(
                    ...
                    if not existing_state:
                        ...
                        db.add(state)
                        db.commit()
                        self.logger.info(f"✅ Created InvestigationState entry for {investigation_id}")
                    else:
                        self.logger.info(f"ℹ️  InvestigationState entry already exists for {investigation_id}")
            except Exception as e:
                self.logger.warning(f"⚠️  Failed to create InvestigationState entry: {str(e)}")
            ...
            if investigation_id in self._active_investigation_handlers:
                ...
                self.logger.info(f"✅ Investigation log handler stopped: investigation.log saved")
            ...
            try:
                if 'investigation_id' in locals():
                    ...
                    if investigation_id in self._active_investigation_handlers:
                        ...
                        del self._active_investigation_handlers[investigation_id]
                        self.logger.info(f"✅ Investigation log handler stopped (failed): investigation.log saved")
    ...
    async def _create_test_context(self, investigation_id: str, scenario_name: str, entity_index: int = 0) -> StructuredInvestigationContext:
        ...
        if self.snowflake_entities:
            ...
            entity_id = snowflake_entity.get('ip') or snowflake_entity.get('email') or snowflake_entity.get('entity')
            ...
            elif ':' in str(entity_id) or ('.' in str(entity_id) and str(entity_id).count('.') == 3):
            ...
            self.logger.info(f"Using Snowflake {entity_type} entity: {entity_id} (Risk Score: {snowflake_entity['risk_score']:.4f})")
        elif self.csv_users:
        ...
        if '@' in str(entity_id_value):
            ...
            self.logger.info(f"🔍 CRITICAL FIX: Overriding entity_type to 'email' based on entity_id format: {entity_id_value}")
        elif ':' in str(entity_id_value) or ('.' in str(entity_id_value) and str(entity_id_value).count('.') == 3):
        ...
        self.logger.info(f"🔍 Context creation: entity_id={entity_id_value}, entity_type_str={entity_type_str}, EntityType={entity_type}")
    ...
    async def _run_comprehensive_investigation(
        ...
        if self.config.mode == TestMode.LIVE:
            ...
            self.logger.info(f"🔌 LIVE mode: Using real {db_provider.upper()} database + real LLM (Claude/GPT)")
        else:
            # DEMO mode: Uses MockLLM for AI analysis, but REAL database for data
            os.environ["TEST_MODE"] = "demo"
            self.logger.warning("🎭 TEST_MODE=demo set - will use MockLLM instead of real Claude/GPT 🎭")
            self.logger.info(f"📊 DEMO mode: Using REAL {db_provider.upper()} database with actual data")
        ...
        try:
            ...
            self.logger.debug(f"[Step 8.1.1]   Recursion limit: {recursion_limit} ({'LIVE: 120' if self.config.mode == TestMode.LIVE else 'DEMO: 150'})")
            self.logger.debug(f"[Step 8.1.1]   Rationale: {'Complex investigations need higher limits' if self.config.mode == TestMode.LIVE else 'Demo mode needs higher limit to allow graph safety mechanisms to complete'}")
    ...
    async def _generate_fallback_results(self, context: StructuredInvestigationContext, result: InvestigationResult) -> Dict[str, Any]:
        ...
        try:
            ...
            async with self.session.get(url) as response:
                if response.status == 200:
                    ...
                    self.logger.error(f"Failed to list anomalies: {response.status} - {error_text}")
                    return []
        except Exception as e:
            ...
            async with self.session.post(login_url, json=login_data) as response:
                if response.status == 200:
                    ...
                    if token:
                        ...
                        self.logger.warning("⚠️  No access_token in login response")
                        return None
                else:
                    error_text = await response.text()
                    self.logger.warning(f"⚠️  Login failed: {response.status} - {error_text}")
                    return None
        except Exception as e:
            self.logger.warning(f"⚠️  Authentication error: {e}")
            return None
    
    async def trigger_anomaly_investigation(self, anomaly_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            async with self.session.post(url, headers=headers) as response:
                if response.status == 201:
                    ...
                    self.logger.error("❌ Authentication failed. The endpoint requires authentication.")
                    self.logger.info("💡 Set API_USER and API_PASSWORD environment variables")
                    error_text = await response.text()
                    self.logger.error(f"Error: {error_text}")
                    return None
                elif response.status == 404:
                    error_text = await response.text()
                    self.logger.error(f"❌ Anomaly not found: {anomaly_id}")
                    self.logger.error(f"Error: {error_text}")
                    return None
                else:
                    error_text = await response.text()
                    self.logger.error(f"Failed to trigger investigation: {response.status} - {error_text}")
                    return None
        except Exception as e:
    ...
    async def monitor_investigation_status(self, investigation_id: str, timeout: int = 600) -> Dict[str, Any]:
        """Monitor investigation until completion"""
        start_time = time.time()
        check_interval = 5  # Check every 5 seconds
        
        self.logger.info(f"🔍 Monitoring investigation {investigation_id}...")
            ...
            try:
                ...
                async with self.session.get(url, headers=headers) as response:
                    if response.status == 200:
                        ...
                        self.logger.info(f"  Status: {status}, Stage: {lifecycle_stage}")
                        ...
                        async with self.session.get(legacy_url, headers=headers) as legacy_response:
                            if legacy_response.status == 200:
                                ...
                                self.logger.warning(f"Investigation {investigation_id} not found yet, waiting...")
                            else:
                                self.logger.warning(f"Unexpected response from legacy endpoint: {legacy_response.status}")
                    else:
        ...
        self.logger.warning(f"⚠️  Investigation monitoring timed out after {timeout}s")
        return {"status": "TIMEOUT", "investigation_id": investigation_id}
    
    async def run_all_tests(self) -> Dict[str, Any]:
        """Run complete test suite based on configuration"""
        self.logger.info("🚀 Starting Unified Structured Investigation Test Suite")
        self.logger.info("=" * 80)
        ...
        if self.config.list_anomalies:
            self.logger.info("📋 Listing available anomalies...")
            ...
            if anomalies:
                ...
                self.logger.warning("⚠️  No anomalies found")
            return {"summary": {"total_scenarios": 0, "scenarios_passed": 0, "scenarios_failed": 0}}
        
        # Handle anomaly-based investigation
        if self.config.anomaly_id:
            if self.config.mode != TestMode.LIVE:
                self.logger.error("❌ Anomaly-based investigations only work in LIVE mode")
                return {"error": "Anomaly investigations require --mode live"}
            
            self.logger.info(f"🔍 Triggering investigation from anomaly: {self.config.anomaly_id}")
            investigation_data = await self.trigger_anomaly_investigation(self.config.anomaly_id)
            ...
            self.logger.info(f"✅ Investigation triggered: {investigation_id}")
            
            # Monitor investigation
            final_status = await self.monitor_investigation_status(investigation_id, timeout=self.config.timeout)
        ...
        if self.config.entity_id and self.config.entity_type:
            ...
            entity_id = first_entity.get('ip') or first_entity.get('email') or first_entity.get('entity')
            ...
            scenarios = [f"real_investigation_{self.config.entity_type}"]
            self.logger.info(f"🎯 No scenario specified, using first Snowflake {entity_type} entity for investigation: {entity_id} (Risk Score: {first_entity['risk_score']:.4f})")
        else:
...
Examples:
  ...
  python unified_structured_test_runner.py --all --output-format html --output-dir ./reports
    ...
    parser.add_argument(
        "--anomaly-id",
        help="Anomaly ID to trigger investigation from (UUID format)"
    )
    parser.add_argument(
        "--list-anomalies",
        action="store_true",
...
async def main():
    ...
    if args.mode == "demo":
        os.environ["TEST_MODE"] = "demo"
        print("🎭 TEST_MODE=demo set at startup - MockLLM will be used 🎭")
    ...
    if sum([scenario_specified, all_scenarios, entity_specified, anomaly_specified]) > 1:
        print("❌ Error: Cannot combine --scenario, --all, --entity-id, and --anomaly-id options")
        return 1
    
    if anomaly_specified and config.mode != TestMode.LIVE:
        print("❌ Error: Anomaly-based investigations only work in LIVE mode. Use --mode live")
        return 1
