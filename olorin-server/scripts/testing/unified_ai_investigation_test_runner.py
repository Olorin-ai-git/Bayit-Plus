...
class UnifiedStructuredTestRunner:
    ...
    async def load_snowflake_data(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"📊 Using entity grouping from .env: {default_group_by} (column: {group_by_column})"
            )
            ...
            if results.get("status") == "success":
                ...
                if entities:
                    ...
                    self.logger.info(
                        f"✅ Loaded {len(self.snowflake_entities)} high-risk {entity_type_name} entities from {db_provider}"
                    )
                    ...
                    self.logger.warning(
                        f"⚠️  No high-risk entities found in {db_provider}"
                    )
                    return False
            else:
                self.logger.error(
                    f"❌ Failed to fetch from {db_provider}: {results.get('error')}"
                )
                return False
    ...
    async def run_single_scenario_test(self, scenario_name: str) -> InvestigationResult:
        ...
        try:
            ...
            if investigation_log_handler:
                ...
                self.logger.info(
                    f"✅ Investigation log handler started: investigation.log will be created"
                )
            else:
            ...
            try:
                ...
                with get_db_session() as db:
                    ...
                    if not existing_state:
                        ...
                        db.commit()
                        self.logger.info(
                            f"✅ Created InvestigationState entry for {investigation_id}"
                        )
                    else:
                        self.logger.info(
                            f"ℹ️  InvestigationState entry already exists for {investigation_id}"
                        )
            except Exception as e:
                self.logger.warning(
                    f"⚠️  Failed to create InvestigationState entry: {str(e)}"
                )
            ...
            if investigation_id in self._active_investigation_handlers:
                ...
                self.logger.info(
                    f"✅ Investigation log handler stopped: investigation.log saved"
                )
            ...
            try:
                if "investigation_id" in locals():
                    ...
                    if investigation_id in self._active_investigation_handlers:
                        ...
                        del self._active_investigation_handlers[investigation_id]
                        self.logger.info(
                            f"✅ Investigation log handler stopped (failed): investigation.log saved"
                        )
    ...
    async def _create_test_context(
        ...
        if "@" in str(entity_id_value):
            ...
            self.logger.info(
                f"🔍 CRITICAL FIX: Overriding entity_type to 'email' based on entity_id format: {entity_id_value}"
            )
        ...
        self.logger.info(
            f"🔍 Context creation: entity_id={entity_id_value}, entity_type_str={entity_type_str}, EntityType={entity_type}"
        )
    ...
    async def _run_comprehensive_investigation(
        ...
        if self.config.mode == TestMode.LIVE:
            ...
            self.logger.info(
                f"🔌 LIVE mode: Using real {db_provider.upper()} database + real LLM (Claude/GPT)"
            )
        else:
            # DEMO mode: Uses MockLLM for AI analysis, but REAL database for data
            os.environ["TEST_MODE"] = "demo"
            self.logger.warning(
                "🎭 TEST_MODE=demo set - will use MockLLM instead of real Claude/GPT 🎭"
            )
            self.logger.info(
                f"📊 DEMO mode: Using REAL {db_provider.upper()} database with actual data"
            )
        ...
        try:
            ...
            async with self.session.post(login_url, json=login_data) as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    self.logger.warning(
                        f"⚠️  Login failed: {response.status} - {error_text}"
                    )
                    return None
        except Exception as e:
        ...
        try:
            ...
            async with self.session.post(url, headers=headers) as response:
                if response.status == 201:
                    ...
                    self.logger.error(
                        "❌ Authentication failed. The endpoint requires authentication."
                    )
                    self.logger.info(
                        "💡 Set API_USER and API_PASSWORD environment variables"
                    )
                    error_text = await response.text()
    ...
    async def run_all_tests(self) -> Dict[str, Any]:
        ...
        if self.config.anomaly_id:
            if self.config.mode != TestMode.LIVE:
                self.logger.error(
                    "❌ Anomaly-based investigations only work in LIVE mode"
                )
                return {"error": "Anomaly investigations require --mode live"}

            self.logger.info(
                f"🔍 Triggering investigation from anomaly: {self.config.anomaly_id}"
            )
        ...
        if self.config.entity_id and self.config.entity_type:
            ...
            scenarios = [f"real_investigation_{self.config.entity_type}"]
            self.logger.info(
                f"🎯 No scenario specified, using first Snowflake {entity_type} entity for investigation: {entity_id} (Risk Score: {first_entity['risk_score']:.4f})"
            )
        else:
            return {
...
async def main():
    ...
    if (
        ...
        print(
            "❌ Error: Cannot combine --scenario, --all, --entity-id, and --anomaly-id options"
        )
        return 1

    if anomaly_specified and config.mode != TestMode.LIVE:
        print(
            "❌ Error: Anomaly-based investigations only work in LIVE mode. Use --mode live"
        )
        return 1
