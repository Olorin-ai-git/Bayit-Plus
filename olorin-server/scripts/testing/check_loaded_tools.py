...
def main():
        ...
        try:
            ...
            if tools:
                for tool in tools:
                    ...
                    print(f"     • {tool_name}")
                    if tool_name not in [t.name if hasattr(t, 'name') else str(t) for t in total_tools]:
    ...
    try:
        ...
        # Count by type
        threat_count = len([t for t in all_tools if 'threat' in t.name.lower() or 
                           'virus' in t.name.lower() or 'abuse' in t.name.lower() or
        ...
        for tool in all_tools:
            tool_name = tool.name if hasattr(tool, 'name') else str(tool)
            tool_desc = tool.description[:50] + "..." if hasattr(tool, 'description') and len(tool.description) > 50 else (tool.description if hasattr(tool, 'description') else "")
