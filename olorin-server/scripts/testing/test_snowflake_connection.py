...
async def test_snowflake_connection():
    ...
    if token:
        print(f"   OAuth Token: {'*' * 20}...{token[-10:] if len(token) > 10 else '***'}")
    else:
    ...
    try:
        ...
        if table_results and table_results[0]['TABLE_EXISTS'] > 0:
            ...
            print(f"   ⚠️  Table '{os.getenv('SNOWFLAKE_TRANSACTIONS_TABLE')}' not found in schema '{os.getenv('SNOWFLAKE_SCHEMA')}'")
...
async def test_snowflake_query():
    ...
    try:
        ...
        print("   Parameters: time_window_hours=24, group_by='EMAIL', top_percentage=0.10")
        ...
        if results:
            ...
            for i, result in enumerate(results[:5], 1):
                ...
                print(f"      Risk-Weighted Value: ${result.get('RISK_WEIGHTED_VALUE', 0):,.2f}")
...
if __name__ == "__main__":
    ...
    print(f"   Query Test: {'✅ PASSED' if query_success else '❌ FAILED' if connection_success else '⏭️  SKIPPED'}")
    print("="*80 + "\n")
