...
class ComprehensiveFraudDataGenerator:
    ...
    def _generate_users(self, count: int) -> List[Dict]:
        ...
        for i in range(count):
            ...
            dob = self.start_date - timedelta(days=random.randint(6570, 25550))  # 18-70 years old
    ...
    def _generate_devices(self, count: int) -> List[Dict]:
        ...
        os_versions = ['iOS 17.0', 'iOS 16.5', 'Android 14', 'Android 13', 'Windows 11', 'macOS 14']
    ...
    def _generate_value_for_column(self, col_name: str, col_type: str, tx_context: Dict) -> Any:
        ...
        if col_type in ['OBJECT', 'VARIANT', 'ARRAY']:
            ...
            if 'email' in col_lower and ('recipient' in col_lower or 'all_' in col_lower):
                return [f"recipient{i}@example.com" for i in range(random.randint(1, 3))]
                ...
                return [{"item_id": f"ITEM_{i}", "quantity": random.randint(1, 5)}
        ...
        if 'TIMESTAMP' in col_type:
            if random.random() < 0.3:
                return None
            return tx_context.get('tx_datetime') + timedelta(seconds=random.randint(-300, 300))
        ...
        elif 'gmv' in col_lower or 'amount_value' in col_lower or 'fee_value' in col_lower:
        ...
        if col_type in ['VARCHAR(16777216)', 'TEXT']:
            ...
            if 'segment' in col_lower or 'partner' in col_lower:
                ...
                reasons = ['High risk score', 'Velocity check', 'Manual review', 'Approved', 'Rejected']
    ...
    def generate_transaction(self, index: int) -> Dict[str, Any]:
        ...
        # Transaction amount
        if is_fraud:
            gmv = round(random.choice([random.uniform(0.01, 5.00), random.uniform(500, 5000)]), 2)
        else:
            gmv = round(random.uniform(10.00, 500.00), 2)

        # Model score
        model_score = random.uniform(0.70, 0.99) if is_fraud else random.uniform(0.01, 0.30)
        ...
        for col_def in self.schema:
            ...
            record[col_name] = self._generate_value_for_column(col_name, col_type, tx_context)
    ...
    def generate_all(self):
        ...
        verify_cmd = [
            ...
            '-d', 'olorin_db',
            '-t',
            '-c', 'SELECT COUNT(*) FROM transactions_enriched;'
        ]
