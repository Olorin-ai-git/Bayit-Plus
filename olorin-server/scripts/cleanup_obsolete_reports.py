...
def cleanup_obsolete_reports():
    ...
    if auto_startup_dir.exists():
        for report_file in auto_startup_dir.glob("comparison_*.html"):
            ...
            if latest_timestamp_prefix not in report_file.name or "152435" not in report_file.name and "152727" not in report_file.name:
    ...
    if regenerated_dir.exists():
        ...
        if regenerated_reports:
            print(f"  ✅ Found {len(regenerated_reports)} regenerated reports (keeping all)")
    ...
    # Show what's kept
    print("\n📊 Remaining reports:")
    print(f"   - Comparison reports in auto_startup: {len(list(auto_startup_dir.glob('*.html'))) if auto_startup_dir.exists() else 0}")
    print(f"   - Startup analysis reports: {len(list(reports_dir.glob('startup_analysis_*.html'))) if reports_dir.exists() else 0}")
    print(f"   - Zip packages: {len(list(comparisons_dir.glob('comparison_package_*.zip'))) if comparisons_dir.exists() else 0}")
    print(f"   - Regenerated reports: {len(list(regenerated_dir.glob('*.html'))) if regenerated_dir.exists() else 0}")
