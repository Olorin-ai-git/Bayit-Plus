...
async def regenerate_comparison(entity_id: str, entity_type: str = "email"):
    ...
    try:
        ...
        if result.get("status") == "success":
            ...
            print(f"   Window A: {result.get('metrics', {}).get('window_a_transactions', 0)} transactions")
            print(f"   Window B: {result.get('metrics', {}).get('window_b_transactions', 0)} transactions")
            return result
        else:
