...
def clear_anomaly_tables(clear_detectors: bool = False):
    ...
    try:
        ...
        logger.info(f"Found {anomaly_count} anomaly events, {run_count} detection runs, {detector_count} detectors")
        
        if anomaly_count == 0 and run_count == 0 and (not clear_detectors or detector_count == 0):
        ...
        if clear_detectors:
            ...
            logger.info(f"Keeping {detector_count} detectors (use --clear-detectors to delete them)")
...
if __name__ == "__main__":
    ...
    if not args.confirm:
        ...
        print(f"  python {sys.argv[0]} --confirm" + (" --clear-detectors" if args.clear_detectors else ""))
        sys.exit(1)
