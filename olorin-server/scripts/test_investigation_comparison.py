...
def get_investigations_with_entities() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.info(f"Found {len(valid_investigations)} investigations with entity and time window")
...
def find_comparable_investigations(investigations: List[Dict[str, Any]]) -> Optional[tuple]:
...
async def test_comparison(inv_a: Dict[str, Any], inv_b: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        request = ComparisonRequest(
            entity={
                "type": mapped_type,
                "value": inv_a.get('entity_id', '')
            } if mapped_type and inv_a.get('entity_id') else None,
            windowA=WindowSpec(
                preset='custom',
        ...
        assert hasattr(metrics_a, 'total_transactions'), "Window A missing total_transactions"
        ...
        assert hasattr(metrics_b, 'total_transactions'), "Window B missing total_transactions"
        ...
        logger.info(f"  Window A: {metrics_a.total_transactions} transactions, "
                   f"precision={metrics_a.precision:.3f}, recall={metrics_a.recall:.3f}")
        logger.info(f"  Window B: {metrics_b.total_transactions} transactions, "
                   f"precision={metrics_b.precision:.3f}, recall={metrics_b.recall:.3f}")
        logger.info(f"  Delta: precision={delta.precision:+.3f}, recall={delta.recall:+.3f}")
...
async def main():
    ...
    if len(investigations) < 2:
        logger.error(f"❌ Need at least 2 investigations with entities and time windows. Found: {len(investigations)}")
