...
class LoggingMigrationTool:
    ...
    LOGGER_INIT_PATTERNS = {
        ...
        "structlog_getlogger": re.compile(
            r"logger\s*=\s*structlog\.get_logger\(__name__\)"
        ),
    }
    ...
    def __init__(
        self, target_dir: str = "app", dry_run: bool = True, verbose: bool = False
    ):
    ...
    def _is_already_using_unified_logging(self, content: str) -> bool:
        ...
        return any(
            indicator in content for indicator in self.ALREADY_UNIFIED_INDICATORS
        )
    ...
    def _has_other_logging_usage(self, content: str) -> Dict[str, bool]:
        ...
        logging_usage = {
            "has_logging_constants": bool(
                re.search(r"logging\.(DEBUG|INFO|WARNING|ERROR|CRITICAL)", temp_content)
            ),
            "has_logging_handlers": bool(
                re.search(r"logging\.(StreamHandler|FileHandler|Handler)", temp_content)
            ),
            "has_logging_config": bool(
                re.search(r"logging\.(basicConfig|config|dictConfig)", temp_content)
            ),
            "has_structlog_config": bool(
                re.search(r"structlog\.(configure|get_config)", temp_content)
            ),
            "has_other_logging_calls": bool(
                re.search(r"logging\.(?!getLogger)", temp_content)
            ),
            "has_other_structlog_calls": bool(
                re.search(r"structlog\.(?!get_logger)", temp_content)
            ),
        }
    ...
    def _migrate_file_content(
        self, content: str, patterns: List[LoggingPattern]
    ) -> str:
        ...
        for pattern in sorted(patterns, key=lambda p: p.line_number, reverse=True):
            if pattern.pattern_type == "logger_init":
                ...
                new_content = new_content.replace(
                    pattern.original_text, "logger = get_bridge_logger(__name__)"
                )
                ...
                if pattern.original_text == "import logging" and not any(
                    [
                        usage["has_logging_constants"],
                        usage["has_logging_handlers"],
                        usage["has_logging_config"],
                        usage["has_other_logging_calls"],
                    ]
                ):
                    imports_to_remove.add(pattern.original_text)

                elif pattern.original_text == "import structlog" and not any(
                    [usage["has_structlog_config"], usage["has_other_structlog_calls"]]
                ):
                    imports_to_remove.add(pattern.original_text)
        ...
        if (
            needs_bridge_import
            and "from app.service.logging import get_bridge_logger" not in new_content
        ):
            ...
            if (
                last_import_found
                and import_insert_index < len(lines)
                and lines[import_insert_index].strip()
            ):
        ...
        try:
            ...
            if self.verbose:
                for pattern in patterns:
                    self._log(
                        f"  Line {pattern.line_number}: {pattern.original_text} ({pattern.pattern_type})",
                        "DEBUG",
                    )
    ...
    def _print_summary(self, summary: MigrationSummary):
        ...
        print(
            f"  Files already using unified logging: {len(summary.files_already_unified)}"
        )
            ...
            for result in self.results:
                if result.was_migrated and shown < 5:
                    ...
                    for pattern in result.patterns_found[:3]:  # Show first 3 patterns
                        print(
                            f"    Line {pattern.line_number}: '{pattern.original_text}' -> unified logging"
                        )
                    if len(result.patterns_found) > 3:
                        print(
                            f"    ... and {len(result.patterns_found) - 3} more patterns"
                        )
                    shown += 1
            print()
        ...
        if self.dry_run:
            ...
            if summary.total_files_migrated > 0:
                print(
                    "IMPORTANT: Test your application to ensure all functionality works correctly."
                )
  ...
  python scripts/complete_logging_migration.py --execute --target-dir app/service/
    ...
    try:
        ...
        tool = LoggingMigrationTool(
            target_dir=args.target_dir, dry_run=not args.execute, verbose=args.verbose
        )
