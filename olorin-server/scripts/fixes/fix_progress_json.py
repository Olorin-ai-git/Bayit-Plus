    ...
    try:
        with get_db_session() as session:
            if fix_all:
                ...
                print(
                    f"📊 Found {len(investigations)} investigations with null progress_json"
                )
                ...
                if state.progress_json:
                    print(
                        f"⚠️  Investigation '{investigation_id}' already has progress_json"
                    )
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage:")
        print(
            "  python fix_progress_json.py <investigation_id>  # Fix specific investigation"
        )
        print(
            "  python fix_progress_json.py --all               # Fix all IN_PROGRESS investigations with null progress_json"
        )
        print("\nExample:")
