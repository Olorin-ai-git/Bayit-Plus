    ...
    with get_db_session() as db:
        ...
        if not state.progress_json:
            ...
            try:
                ...
                if domain_findings:
                    print(f"✅ domain_findings found in progress_json: {len(domain_findings)} domains")
                    ...
                    for domain, findings in domain_findings.items():
                        ...
                        if isinstance(findings, dict):
                            print(f"      - Risk Score: {findings.get('risk_score', 'N/A')}")
                            print(f"      - Confidence: {findings.get('confidence', 'N/A')}")
                            print(f"      - Evidence Count: {len(findings.get('evidence', []))}")
                            print(f"      - Risk Indicators: {len(findings.get('risk_indicators', []))}")
                            
                            # Check for LLM analysis
                            llm_analysis = findings.get('llm_analysis', {})
                            if llm_analysis:
                                print(f"      - ✅ Has LLM Analysis:")
                                print(f"         * LLM Confidence: {llm_analysis.get('confidence', 'N/A')}")
                                print(f"         * Analysis Duration: {llm_analysis.get('analysis_duration', 'N/A')}s")
                                print(f"         * Analysis Type: {llm_analysis.get('analysis_type', 'N/A')}")
                            ...
                            print(f"      - ⚠️  Findings is not a dict (type: {type(findings).__name__})")
                else:
                    print(f"⚠️  No domain_findings found in progress_json")
                    print(f"   Available keys in progress_json: {list(progress_data.keys())[:10]}")
        ...
        if not state.results_json:
            ...
            try:
                ...
                if domain_findings:
                    print(f"✅ domain_findings found in results_json: {len(domain_findings)} domains")
                    print(f"\n   Domains: {', '.join(domain_findings.keys())}")
                    ...
                    print(f"   Available keys in results_json: {list(results_data.keys())[:10]}")
                    
                    # Check if risk_score exists (to confirm it's a valid results structure)
                    if 'risk_score' in results_data:
                        print(f"   ✅ results_json has risk_score: {results_data['risk_score']}")
        ...
        if state.progress_json:
            try:
                progress_data = json.loads(state.progress_json)
                progress_has_domain_findings = bool(progress_data.get('domain_findings'))
            except:
                pass
...
def main():
    """Main entry point."""
    if len(sys.argv) < 2:
        print("Usage: python scripts/verify_domain_findings_persistence.py <investigation_id>")
        print("\nExample:")
        print("  python scripts/verify_domain_findings_persistence.py inv-1762538988633")
        sys.exit(1)
