...
try:
    ...
    HAS_FAKER = False
    print(
        "⚠️  Faker not installed. Using basic random data. Install with: poetry add faker"
    )
...
class FraudTestDataGenerator:
    ...
    def generate_transaction(self, index: int) -> Dict[str, Any]:
        ...
        for _ in range(num_items):
            ...
            cart_brands.append(
                f"Brand {random.randint(1, 50)}" if not HAS_FAKER else fake.company()
            )
        ...
        record = {
            ...
            "tx_received_datetime": (
                tx_datetime + timedelta(milliseconds=random.randint(10, 500))
            ).isoformat(),
            "tx_uploaded_to_snowflake": (
                tx_datetime + timedelta(minutes=random.randint(1, 10))
            ).isoformat(),
            ...
            "personal_info_additional_data": json.dumps(
            ...
            "billing_address": json.dumps(
                {
                    ...
                    "country": (
                        ip_data["country"]
                        if not is_fraud
                        else random.choice(["US", "CN", "NG", "RU"])
                    ),
                }
            ),
            ...
            "failure_reason": (
                None
                if nsure_decision == "APPROVED"
                else random.choice(
                    ["insufficient_funds", "suspected_fraud", "invalid_card"]
                )
            ),
            ...
            "session_info_language": random.choice(
                ["en-US", "en-GB", "fr-FR", "de-DE"]
            ),
            "session_info_additional_data": json.dumps({}),
            # Recipient info
            "first_recipient_info": json.dumps(
            ...
            "calculated_props_uploaded_to_snowflake": (
                tx_datetime + timedelta(minutes=1)
            ).isoformat(),
            ...
            "email_data_third_party_risk_score": (
                random.uniform(0.0, 1.0) if random.random() < 0.5 else None
            ),
            # IP and location
            ...
            "is_processor_reported_stolen_card": (
                1 if is_fraud and random.random() < 0.2 else 0
            ),
            "is_processor_rejected_due_to_fraud": (
                1 if is_fraud and random.random() < 0.3 else 0
            ),
            # IP address info
            ...
            "nsure_first_decision_datetime": (
                tx_datetime + timedelta(seconds=random.randint(1, 30))
            ).isoformat(),
            ...
            "nsure_last_decision_uploaded_to_snowflake": (
                tx_datetime + timedelta(minutes=2)
            ).isoformat(),
            "is_reassessed_tx": 0,
            # Rules
            ...
            "rejection_reason": (
                None if nsure_decision == "APPROVED" else "HIGH_RISK_SCORE"
            ),
            ...
            "merchant_last_decision_uploaded_to_snowflake": (
                tx_datetime + timedelta(minutes=3)
            ).isoformat(),
            ...
            "app_install_datetime": (
                user["first_seen"] - timedelta(days=random.randint(0, 30))
            ).isoformat(),
            "device_type": random.choice(["mobile", "desktop", "tablet"]),
            "device_model": random.choice(
                ["iPhone 13", "Samsung Galaxy", "iPad", "Desktop"]
            ),
            ...
            "card_issuer_normalized": payment_method["issuer"]
            .upper()
            .replace(" ", "_"),
            ...
            "maxmind_ip_risk_score": (
                random.uniform(0.0, 1.0) if is_fraud else random.uniform(0.0, 0.3)
            ),
            "maxmind_risk_score": (
                random.uniform(0.0, 1.0) if is_fraud else random.uniform(0.0, 0.3)
            ),
            ...
            "is_freemail": (
                1
                if "@gmail.com" in user["email"] or "@yahoo.com" in user["email"]
                else 0
            ),
            "is_personal_email": 1,
            "is_valid_email": 1,
            ...
            "disputes": (
                json.dumps([])
                if not (is_fraud and random.random() < 0.5)
                else json.dumps(
                    [
                        {
                            "datetime": (
                                tx_datetime + timedelta(days=random.randint(30, 90))
                            ).isoformat(),
                            "status": "lost",
                            "reason": "fraud",
                        }
            ...
            "first_dispute_datetime": (
                (tx_datetime + timedelta(days=random.randint(30, 90))).isoformat()
            ...
            "last_dispute_amount": (
                json.dumps({"amount": gmv, "currency": "USD"})
                if (is_fraud and random.random() < 0.5)
                else None
            ),
            "last_dispute_status": (
                "lost" if (is_fraud and random.random() < 0.5) else None
            ),
            "last_dispute_decision": (
                "merchant_lost" if (is_fraud and random.random() < 0.5) else None
            ),
            "is_last_dispute_fraud_related_reason": (
                1 if (is_fraud and random.random() < 0.5) else 0
            ),
            "last_dispute_reason": (
                "fraudulent" if (is_fraud and random.random() < 0.5) else None
            ),
            "last_dispute_source": (
                json.dumps({"source": "customer"})
                if (is_fraud and random.random() < 0.5)
                else None
            ),
            ...
            "is_user_first_tx_event": (
                1 if (tx_datetime - user["first_seen"]).days < 1 else 0
            ),
            "first_user_account_activity_date": user["first_seen"].isoformat(),
            "first_sign_in_identity_provider": random.choice(
            ...
            "first_fraud_status_datetime": (
                (tx_datetime + timedelta(days=60)).isoformat() if is_fraud else None
            ),
            ...
            "buyer_name_to_pipl_name_distance": (
                random.uniform(0.0, 0.5) if not is_fraud else random.uniform(0.5, 1.0)
            ),
            ...
            "bin_risky_gmv": (
                random.uniform(0, 5000) if is_fraud else random.uniform(0, 500)
            ),
            # Demographics
            ...
            # User classification
            "is_recurring_user": (
                1 if (tx_datetime - user["first_seen"]).days > 30 else 0
            ),
            "is_new_buyer__model_feature": (
                1 if (tx_datetime - user["first_seen"]).days < 7 else 0
            ),
            ...
            "buyer_email_domain": (
                user["email"].split("@")[1] if "@" in user["email"] else None
            ),
            "refusal_reason": None,
            ...
            "product_type": random.choice(
                ["digital_game", "physical_goods", "subscription"]
            ),
            ...
            "failure_category": (
                None
                if nsure_decision == "APPROVED"
                else random.choice(["fraud", "payment_failure"])
            ),
            ...
            "distance_ip_product__model_feature": (
                random.uniform(0, 10000) if is_fraud else random.uniform(0, 1000)
            ),
            ...
            # Product details
            "most_expensive_cart_usd_item": json.dumps(
                max(cart_items, key=lambda x: x["price"]) if cart_items else {}
            ),
            "product_regions": json.dumps(["NA"]),
            "product_platform": random.choice(["PC", "Xbox", "PlayStation", "Mobile"]),
            "product_gametitle": (
                random.choice(["Game1", "Game2", "Game3"])
                if merchant["industry"] == "gaming"
                else None
            ),
            # Attack reporting
            ...
            # Partner details
            "partner_main_product_type": random.choice(
                ["digital_goods", "gaming", "subscription"]
            ),
    ...
    async def insert_batch(self, pg_provider, records: List[Dict], batch_num: int):
        ...
        for i, record in enumerate(records):
            try:
                ...
                print(
                    f"   ❌ Error inserting record {i+1} in batch {batch_num}: {str(e)[:100]}"
                )
    ...
    async def generate_and_insert(self):
        ...
        result = await pg._execute_query_async(
            "SELECT COUNT(*) as cnt FROM transactions_enriched"
        )
                ...
                for i, record in enumerate(batch_records):
                    ...
                    try:
                        ...
                        if i == 0:  # Only print first error
                            print(
                                f"      ⚠️  Insert error (continuing): {str(e)[:80]}..."
                            )

                print(
                    f"   ✅ Batch {batch_num+1}/{total_batches}: Inserted {len(batch_records)} records"
                )
        ...
        print(
            f'   Fraudulent transactions: {stats["fraud_count"]:,} ({stats["fraud_count"]/final_count*100:.1f}%)'
        )
        print(
            f'   High risk (score >= 0.70): {stats["high_risk_count"]:,} ({stats["high_risk_count"]/final_count*100:.1f}%)'
        )
