...
async def run_analyzer_on_window(window_start_offset_days: int) -> Dict[str, Any]:
    ...
    logger.info(f"{'='*80}")
    logger.info(
        f"📅 WINDOW: {target_date.strftime('%Y-%m-%d')} (T-{window_start_offset_days} days)"
    )
    logger.info(f"{'='*80}")
...
async def investigate_all_entities(
    window_data: Dict[str, Any], max_entities: int = None
) -> List[Dict[str, Any]]:
    """
    ...
    for i, entity_data in enumerate(entities, 1):
        ...
        logger.info(f"")
        logger.info(
            f"[{i}/{len(entities)}] Investigating: {entity_type}={entity_value}"
        )
        ...
        try:
            ...
            if metrics.get("window_a_transactions", 0) > 0:
                logger.info(
                    f"   📊 Window A: {metrics['window_a_transactions']} transactions"
                )
...
async def check_for_fraud(
    ...
    for result in investigation_results:
        ...
        confusion_tables = list(
            confusion_tables_dir.glob(f"**/confusion_table_{investigation_id}_*.html")
        )
...
async def systematic_window_test(
    ...
    for window_idx in range(num_windows):
        ...
        investigation_results = await investigate_all_entities(
            window_data, max_entities=max_entities_per_window
        )
    ...
    overall_precision = (
        total_fraud_found / total_entities_investigated
        if total_entities_investigated > 0
        else 0
    )
    ...
    logger.info(
        f"   Entities Investigated: {final_summary['total_entities_investigated']}"
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Systematic 24-hour moving window test"
    )
    parser.add_argument(
        "--start-offset",
        type=int,
    ...
    parser.add_argument(
        "--no-stop", action="store_true", help="Continue even after finding fraud"
    )
