...
async def run_remediation_for_entity(entity_id: str, entity_type: str = "email"):
    ...
    all_investigations = db.query(InvestigationState).filter(
    ...
    for inv in all_investigations:
        ...
        if entities and len(entities) > 0:
            ...
            if entity_data.get("entity_value") == entity_id and entity_data.get("entity_type") == entity_type:
                matching_investigations.append(inv)
                continue
        
        # Fallback: check direct fields
        if settings.get("entity_id") == entity_id and settings.get("entity_type") == entity_type:
            matching_investigations.append(inv)
        elif progress.get("entity_id") == entity_id and progress.get("entity_type") == entity_type:
            matching_investigations.append(inv)
    ...
    if overall_risk_score is None:
        overall_risk_score = progress.get("risk_score") or progress.get("overall_risk_score") or settings.get("overall_risk_score")
