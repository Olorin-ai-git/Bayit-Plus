...
def run_coverage_check():
    ...
    try:
        result = subprocess.run(
            coverage_args, capture_output=True, text=True, check=False
        )
        ...
        if coverage_json.exists():
            ...
            with open(coverage_json) as f:
                ...
                if total_coverage >= TARGET_COVERAGE:
                    print(
                        f"✅ Coverage target met! ({total_coverage:.2f}% >= {TARGET_COVERAGE}%)"
                    )
                    return 0
                else:
                    print(
                        f"❌ Coverage below target! ({total_coverage:.2f}% < {TARGET_COVERAGE}%)"
                    )
                    print(
                        f"Need to increase coverage by {TARGET_COVERAGE - total_coverage:.2f}%"
                    )
                    return 1
