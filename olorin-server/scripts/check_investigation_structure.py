                        ...
                        if 'transactions' in sf_data:
                            ...
                            if isinstance(tx_data, list):
                                ...
                                if tx_data and isinstance(tx_data[0], dict):
                                    print(f'    First transaction keys: {list(tx_data[0].keys())}')
                                    print(f'    Sample transaction: {json.dumps(tx_data[0], indent=2, default=str)[:500]}')
                                ...
                                if isinstance(result, dict):
                                    ...
                                    if 'transactions' in result:
                                        ...
                                        if isinstance(tx_data, list):
                                            print(f'  Transaction count: {len(tx_data)}')
                                            if tx_data:
                                                print(f'  First transaction keys: {list(tx_data[0].keys()) if isinstance(tx_data[0], dict) else "N/A"}')
                                    ...
                                    if 'snowflake_data' in result:
                                        ...
                                        if isinstance(sf_data, dict) and 'transactions' in sf_data:
                                            print(f'  Transactions in snowflake_data: {len(sf_data["transactions"])}')
                                    
                                    # Show sample of result data
                                    if len(str(result)) < 500:
                                        print(f'  Result sample: {json.dumps(result, indent=2, default=str)[:500]}')
            except Exception as e:
