...
async def analyze_investigation_scores():
    ...
    for i, tx in enumerate(fraud_results, 1):
        ...
        if ip:
            fraud_ips.add(ip)
        if device:
            fraud_devices.add(device)
        if merchant:
            fraud_merchants.add(merchant)

        print(f"Fraud TX {i}: {str(date)[:19]}")
        print(f"  Amount: ${amount:.2f}")
        print(
            f"  MODEL_SCORE: {score:.4f} {'⚠️ VERY LOW' if score < 0.1 else '⚠️ LOW' if score < 0.4 else ''}"
        )
        print(f"  IP: {ip}")
    ...
    if legit_results:
        ...
        for tx in legit_results:
            ...
            if ip:
                legit_ips.add(ip)
            if device:
                legit_devices.add(device)
            if merchant:
                legit_merchants.add(merchant)

        avg_legit_amount = (
            sum(legit_amounts) / len(legit_amounts) if legit_amounts else 0
        )
    ...
    if avg_fraud_amount < avg_legit_amount:
        print(
            f"  ⚠️ Fraud amounts are {((avg_legit_amount - avg_fraud_amount) / avg_legit_amount * 100):.1f}% lower"
        )
    print()
    ...
    # Time concentration
    if fraud_results:
        first = fraud_results[0].get("TX_DATETIME") or fraud_results[0].get(
            "tx_datetime"
        )
        last = fraud_results[-1].get("TX_DATETIME") or fraud_results[-1].get(
            "tx_datetime"
        )
    ...
    print(
        f"   - This critical signal was excluded (correctly, for unbiased evaluation)"
    )
