    ...
    with get_db_session() as session:
        ...
        if not force_all:
            ...
            if db_config.is_postgresql:
                ...
                with db_config.session() as vec_session:
                    ...
                    """)
                    vec_result = vec_session.execute(query_text, {
            ...
            if not force_all:
                to_index = [inv for inv in investigations if inv.investigation_id not in indexed_ids]
        ...
        for investigation in to_index:
            try:
                ...
                success = await indexer.index_investigation(investigation.investigation_id)
                if success:
                    indexed_count += 1
