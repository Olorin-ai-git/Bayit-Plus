...
from scripts.enrichment.graph_analytics_export import graph_analytics_export, load_graph_features_to_postgres
...
def get_transactions_to_enrich(limit: Optional[int] = None, min_age_days: int = 14) -> List[Dict[str, Any]]:
    ...
    with get_db_session() as db:
        ...
        """)
        results = db.execute(query, {"min_age_days": min_age_days, "limit": limit or 10000}).fetchall()
        transactions = [{
            "txn_id": r.txn_id, "txn_ts": r.txn_ts, "merchant_id": r.merchant_id, "card_id": r.card_id,
            "amount": float(r.amount) if r.amount else None, "currency": r.currency, "country": r.country,
            "mcc": r.mcc, "region": r.region, "ip_address": getattr(r, "ip_address", None),
            "ip": getattr(r, "ip_address", None), "email": getattr(r, "email", None),
            "billing_email": getattr(r, "email", None), "phone": getattr(r, "phone_number", None),
            "customer_phone": getattr(r, "phone_number", None), "phone_number": getattr(r, "phone_number", None)
        } for r in results]
    ...
    try:
        ...
        if "graph" in steps:
            ...
            if export_stats.get("status") != "skipped":
                feature_stats = load_graph_features_to_postgres(neo4j_client, limit=limit)
        ...
        track_pipeline_metrics("Enrichment Pipeline", {
...
def main():
    ...
    parser.add_argument("--limit", type=int, help="Maximum number of transactions to process")
    parser.add_argument("--steps", nargs="+", choices=["bin", "ip", "email_phone", "graph"], 
    ...
    print(f"Enrichment complete: {stats['enriched']}/{stats['total']} transactions enriched")
