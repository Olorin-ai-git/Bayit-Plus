...
def enrich_transactions_email_phone(transactions: List[Dict[str, Any]]) -> None:
    ...
    with get_db_session() as db:
        for txn in transactions:
            ...
            phone = txn.get("phone") or txn.get("customer_phone") or txn.get("phone_number")
        ...
        db.commit()
        logger.info(f"Email/phone enrichment complete: {enriched_count}/{len(transactions)} transactions enriched")
