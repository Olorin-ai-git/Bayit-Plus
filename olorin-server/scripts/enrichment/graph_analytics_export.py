...
from app.service.precision_detection.performance_monitor import (
...
from scripts.enrichment.graph_helpers import (
    ...
    create_graph_edges,
    create_graph_nodes,
    load_graph_features_batch,
    query_graph_features,
    query_snowflake_transactions,
    run_gds_algorithms,
)
...
def graph_analytics_export(
    ...
    if not neo4j_client.use_neo4j or not neo4j_client.is_available():
        reason = (
            "Neo4j disabled via USE_NEO4J=false"
            if not neo4j_client.use_neo4j
    ...
    try:
        ...
        batch_size = 1000
        cards, merchants, fraud_cards, fraud_merchants = collect_nodes_and_edges(
            transactions
        )
...
def load_graph_features_to_postgres(
    neo4j_client: Neo4jClient, limit: Optional[int] = None
) -> Dict[str, Any]:
    """
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Export graph analytics from Snowflake to Neo4j"
    )
    parser.add_argument(
        "--limit", type=int, help="Maximum number of transactions to export"
    )
    parser.add_argument(
        "--export-only",
        action="store_true",
        help="Only export graph, skip feature computation",
    )
    parser.add_argument(
        "--features-only",
        action="store_true",
        help="Only compute features, skip graph export",
    )
    ...
    if not args.features_only:
        export_stats = graph_analytics_export(
            sf_service, neo4j_client, limit=args.limit
        )
        print(
            f"Graph export: {export_stats.get('cards_created', 0)} cards, "
    ...
    if not args.export_only:
        ...
        print(
            f"Graph features loaded: {feature_stats.get('features_loaded', 0)} transactions"
        )
