...
class SnowflakePOCTester:
    ...
    def test_environment_config(self):
        ...
        self.print_test(
            "Snowflake Enable Flag", True, f"USE_SNOWFLAKE={snowflake_enabled}"
        )
        ...
        self.print_test(
            "Other Tools Disabled", all_disabled, f"Tool statuses: {tools_status}"
        )
        ...
        self.print_test("Analytics Configuration", True, f"Config: {analytics_config}")
    ...
    def test_snowflake_config(self):
        ...
        self.print_test(
            "Optional Configuration Fields", True, f"Present: {present_optional}"
        )

        # Display configuration (hide password)
        safe_config = {
            k: ("***" if "password" in k.lower() else v) for k, v in config.items()
        }
    ...
    async def test_snowflake_connection(self):
        ...
        if not is_real:
            print(
                "  ⚠️  Mock client active - set USE_SNOWFLAKE=true for real connection"
            )
            return True
        ...
        try:
            ...
            self.print_test("Connection Test", False, str(e))
            return False
    ...
    async def test_risk_analyzer(self):
        ...
        try:
            ...
            results = await analyzer.get_top_risk_entities(
                time_window="24h", group_by="email", top_percentage=10
            )
            ...
            if "summary" in results:
                ...
                print(
                    f"    Total Risk Value: ${summary.get('total_risk_value', 0):,.2f}"
                )
            ...
            self.print_test("Risk Analysis Execution", False, str(e))
            return False
    ...
    async def test_entity_analysis(self):
        ...
        try:
            ...
            results = await analyzer.analyze_entity(
                entity_value=test_entity, entity_type="email", time_window="30d"
            )

            success = results.get("status") == "success"

            self.print_test(
                "Entity Analysis Execution", success, f"Status: {results.get('status')}"
            )
            ...
            self.print_test("Entity Analysis Execution", False, str(e))
            return False
    ...
    async def test_api_endpoints(self):
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                self.print_test(
                    "Config Endpoint", config_ok, f"Status: {response.status_code}"
                )
            ...
            print(
                "  ⚠️  Server not running. Start with: poetry run python -m app.local_server"
            )
            return False
        except Exception as e:
