...
async def check_for_fraud_in_window(
    start_offset_months: int, time_window_hours: int = 24
    ...
    logger.info(f"{'='*70}")
    logger.info(
        f"🔍 Checking window: {start_offset_months} months ago, {time_window_hours}H duration"
    )
    logger.info(f"{'='*70}")
        ...
        if result:
            for row in result:
                entity_data = {
                    ...
                    "non_fraud_count": row.get("NON_FRAUD_COUNT")
                    or row.get("non_fraud_count"),
                }
            ...
            for i, entity in enumerate(entities_with_fraud[:5], 1):
                logger.info(
                    f"   {i}. {entity['email']}: {entity['fraud_count']} fraud, {entity['non_fraud_count']} clean"
                )
...
async def run_analyzer_on_window(
    start_offset_months: int, time_window_hours: int = 24
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Find fraudulent transactions by sliding analyzer window backwards"
    )
    parser.add_argument(
        "--start-offset-months",
        type=int,
        default=6,
        help="Starting offset in months (default: 6)",
    )
    parser.add_argument(
        "--max-iterations",
        type=int,
        default=30,
        help="Maximum days to search backwards (default: 30)",
    )
    parser.add_argument(
        "--time-window-hours",
        type=int,
        default=24,
        help="Analyzer window duration in hours (default: 24)",
    )
    parser.add_argument(
        "--entities-per-window",
        type=int,
        default=5,
        help="Entities to investigate per window (default: 5)",
    )
    parser.add_argument(
        "--run-investigations",
        action="store_true",
        help="Run full investigations when fraud found",
    )
    ...
    logger.info(f"╔{'═'*68}╗")
    logger.info(
        f"║  ITERATIVE FRAUD FINDER                                            ║"
    )
    logger.info(f"╠{'═'*68}╣")
    logger.info(
        f"║  Strategy: Slide 24H window backwards until fraud is found        ║"
    )
    logger.info(
        f"║  Start: {args.start_offset_months} months ago                                             ║"
    )
    logger.info(
    ...
    current_offset_days = (
        current_offset_months * 30
    )  # Convert to days for finer granularity
    ...
    while iteration < args.max_iterations and not fraud_found:
        ...
        logger.info(
            f"Checking window: {current_offset_days} days ago (~{months_offset:.1f} months)"
        )
        logger.info(f"{'─'*70}")
        ...
        if has_fraud:
            ...
            logger.info(f"")
            logger.info(
                f"🎉 SUCCESS! Found {fraud_count} fraudulent transactions at {months_offset:.1f} months ago"
            )
            break
        else:
    ...
    if not fraud_found:
        ...
        logger.warning(
            f"   Searched from {args.start_offset_months} to {current_offset_days} days ago"
        )
    ...
    logger.info(f"╔{'═'*68}╗")
    logger.info(
        f"║  FRAUD SEARCH COMPLETE                                             ║"
    )
    logger.info(f"╠{'═'*68}╣")
    logger.info(
        f"║  Window Found: {fraud_window_offset} months ago (~{fraud_window_offset*30} days)                     ║"
    )
    logger.info(
        f"║  Entities with Fraud: {len(entities_with_fraud)}                                           ║"
    )
    logger.info(
    ...
    if args.run_investigations:
        logger.info(f"")
        logger.info(
            f"🚀 Running investigations on top {args.entities_per_window} entities..."
        )
        logger.info(
            f"⚠️ This will take approximately {args.entities_per_window * 5} minutes"
        )
        logger.info(f"")
