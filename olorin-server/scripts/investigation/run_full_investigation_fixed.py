    ...
    def run_investigation(self, entity_id: str) -> Dict:
        ...
        for agent in agents:
            ...
            if findings["success"]:
                ...
                logger.info(
                    f"   Tokens: Input {findings.get('tokens', {}).get('input_tokens', 0)}, Output {findings.get('tokens', {}).get('output_tokens', 0)}"
                )
        ...
        if all_findings:
            ...
            final_analysis = self.make_api_call(
                "Senior Fraud Investigator", summary_prompt
            )
            ...
            if final_analysis["success"]:
                ...
                logger.info(
                    f"✅ Final assessment completed - Risk Score: {results['overall_risk']}/100"
                )

        results["end_time"] = datetime.now().isoformat()
        results["total_api_calls"] = self.api_calls
        results["total_cost"] = self.total_cost
        results["investigation_duration"] = (
            f"{time.time() - time.mktime(datetime.fromisoformat(results['start_time']).timetuple()):.2f}s"
        )
...
def print_investigation_results(results: Dict):
    ...
    logger.info(
        f"   Successful Agents: {len([f for f in results['agent_findings'].values()])}"
    )
    ...
    for agent_name, findings in results["agent_findings"].items():
        ...
        logger.info(
            f"      Tokens: {findings.get('tokens_used', {}).get('input_tokens', 0)} in, {findings.get('tokens_used', {}).get('output_tokens', 0)} out"
        )
    ...
    logger.info(
        f"   Average Cost per Agent: ${results['total_cost']/len(results['agent_findings']) if results['agent_findings'] else 0:.4f}"
    )
