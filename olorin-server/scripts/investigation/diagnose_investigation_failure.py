...
async def test_risk_analyzer():
    ...
    try:
        ...
        print(
            f"❌ RiskAnalyzer test failed with exception: {type(e).__name__}: {str(e)}"
        )
        import traceback
...
async def check_investigation_state(investigation_id: str):
    ...
    with get_db_session() as db:
        ...
        if state.settings_json:
            ...
            if entities:
                ...
                for i, e in enumerate(entities):
                    print(
                        f"      [{i}] type={e.get('entityType')}, value={e.get('entityValue')}"
                    )
        ...
        if state.progress_json:
            ...
            print(
                f"   Percent Complete: {progress.get('percent_complete', progress.get('progress_percentage', 0))}"
            )
...
async def simulate_execution_flow(investigation_id: str):
    ...
    is_risk_based = settings.get("investigationMode") == "risk" or settings.get(
        "autoSelectEntities"
    )
