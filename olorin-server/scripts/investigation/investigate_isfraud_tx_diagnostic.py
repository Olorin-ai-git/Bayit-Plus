...
async def check_column_exists_and_type(db_provider, table_name: str) -> dict:
    ...
    is_async = hasattr(db_provider.execute_query, "__code__") and "async" in str(
        db_provider.execute_query
    )
...
async def check_overall_distribution(db_provider, table_name: str) -> dict:
    ...
    try:
        ...
        if results:
            ...
            logger.info(
                f"   Total transactions: {row.get('TOTAL_TRANSACTIONS') or row.get('total_transactions', 0)}"
            )
            logger.info(
                f"   Non-NULL IS_FRAUD_TX: {row.get('NON_NULL_COUNT') or row.get('non_null_count', 0)}"
            )
            logger.info(
                f"   NULL IS_FRAUD_TX: {row.get('NULL_COUNT') or row.get('null_count', 0)}"
            )
            logger.info(
                f"   Fraud (IS_FRAUD_TX=1): {row.get('FRAUD_COUNT') or row.get('fraud_count', 0)}"
            )
            logger.info(
                f"   Not Fraud (IS_FRAUD_TX=0): {row.get('NOT_FRAUD_COUNT') or row.get('not_fraud_count', 0)}"
            )
            logger.info(
...
async def check_recent_transactions(
    db_provider, table_name: str, days: int = 30
) -> list:
    ...
    try:
        ...
        if results:
            ...
            for i, row in enumerate(results[:10]):
                ...
                logger.info(
                    f"   {i+1}. TX_ID={tx_id[:20]}..., Date={tx_datetime}, IS_FRAUD_TX={status}, Decision={decision}, Email={email[:30] if email else 'N/A'}..."
                )
...
async def check_approved_transactions(db_provider, table_name: str) -> dict:
    ...
    try:
        ...
        if results:
            ...
            logger.info(
                f"   Total APPROVED: {row.get('TOTAL_APPROVED') or row.get('total_approved', 0)}"
            )
            logger.info(
                f"   Non-NULL IS_FRAUD_TX: {row.get('NON_NULL_COUNT') or row.get('non_null_count', 0)}"
            )
...
async def main():
    ...
    if column_info:
        logger.info(
            f"✅ Column exists: {column_info.get('DATA_TYPE') or column_info.get('data_type')}"
        )
        logger.info(
            f"   Nullable: {column_info.get('IS_NULLABLE') or column_info.get('is_nullable')}"
        )

    if overall_dist:
        total = overall_dist.get("TOTAL_TRANSACTIONS") or overall_dist.get(
            "total_transactions", 0
        )
        null_count = overall_dist.get("NULL_COUNT") or overall_dist.get("null_count", 0)
        non_null = overall_dist.get("NON_NULL_COUNT") or overall_dist.get(
            "non_null_count", 0
        )
        ...
        if total > 0:
            ...
            logger.info(
                f"📊 Overall: {null_count}/{total} ({null_pct:.1f}%) have NULL IS_FRAUD_TX"
            )
            logger.info(
                f"   {non_null}/{total} ({100-null_pct:.1f}%) have non-NULL values"
            )

    if approved_dist:
        total_approved = approved_dist.get("TOTAL_APPROVED") or approved_dist.get(
            "total_approved", 0
        )
        null_approved = approved_dist.get("NULL_COUNT") or approved_dist.get(
            "null_count", 0
        )
        ...
        if total_approved > 0:
            ...
            logger.info(
                f"📊 APPROVED: {null_approved}/{total_approved} ({null_pct:.1f}%) have NULL IS_FRAUD_TX"
            )
