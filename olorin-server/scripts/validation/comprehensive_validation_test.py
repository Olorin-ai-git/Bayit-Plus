...
class ComprehensiveValidationTest:
    ...
    async def test_evidence_strength_capping(self):
        ...
        try:
            ...
            from app.service.agent.integration_system import (
                IntegratedInvestigationService,
            )
            ...
            successful_patterns = [
                pr for pr in test_result.pattern_results.values() if pr.success
            ]
            ...
            if test_result.knowledge_retrieved:
                evidence_factors.append(
                    len(test_result.knowledge_retrieved) * 0.1
                )  # 5 * 0.1 = 0.5

            # Entity evidence
            if test_result.related_entities:
                evidence_factors.append(
                    len(test_result.related_entities) * 0.05
                )  # 4 * 0.05 = 0.2

            # This would sum to 1.7, which should be capped at 0.4
            uncapped_strength = sum(evidence_factors)
            test_result.evidence_strength = min(
                1.0, uncapped_strength
            ...
            self.add_test_result(
                "evidence_strength_tests",
                f"Evidence strength test failed: {str(e)}",
                False,
            )
    ...
    async def test_authoritative_override_removal(self):
        ...
        try:
            ...
            self.add_test_result(
                "authoritative_override_tests",
                f"Authoritative override test failed: {str(e)}",
                False,
            )
    ...
    async def test_null_safe_formatting(self):
        ...
        try:
            ...
            validation_file = (
                Path(project_root) / "app/service/agent/enhanced_validation.py"
            )
            ...
            if validation_file.exists():
                ...
                if risk_formatting_lines:
                    ...
                    try:
                        ...
                        if (
                            test_initial_risk is not None
                            and test_final_risk is not None
                        ):
            ...
            self.add_test_result(
                "formatting_tests", f"Null-safe formatting test failed: {str(e)}", False
            )
    ...
    async def test_risk_fusion_logic(self):
        ...
        try:
            ...
            self.add_test_result(
                "risk_fusion_tests", f"Risk fusion test failed: {str(e)}", False
            )
    ...
    async def test_float_conversion_safety(self):
        ...
        try:
            ...
            self.add_test_result(
                "null_safety_tests", f"Float conversion test failed: {str(e)}", False
            )
    ...
    async def test_end_to_end_investigation(self):
        ...
        try:
            ...
            validation_result = await validator.validate_investigation_result(
                mock_result["investigation_id"], mock_result
            )
            ...
            self.add_test_result(
                "end_to_end_tests", f"End-to-end test failed: {str(e)}", False
            )

    def add_test_result(
        self, category: str, description: str, passed: bool, details: str = ""
    ):
        """Add test result"""
        result = {"description": description, "passed": passed, "details": details}
