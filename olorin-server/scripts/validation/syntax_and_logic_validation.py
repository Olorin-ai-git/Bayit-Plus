        ...
        test_cases = [
            {
                ...
                "llm": 0.2,
                "expected_max": 0.4,
                "description": "High discordance should cap at 0.4",
            },
            {
                "computed": 0.9,
                "llm": 0.1,
                "expected_max": 0.4,
                "description": "Very high discordance should cap at 0.4",
            },
            {
                "computed": 0.3,
                "llm": 0.35,
                "expected_range": (0.3, 0.35),
                "description": "Low discordance should blend",
            },
        ]
        ...
        for case in test_cases:
            ...
            if "expected_max" in case:
                if result <= case["expected_max"]:
                    results.append(
                        f"✅ {case['description']}: {result:.2f} <= {case['expected_max']}"
                    )
                else:
                    results.append(
                        f"❌ {case['description']}: {result:.2f} > {case['expected_max']}"
                ...
                if min_val <= result <= max_val:
                    results.append(
                        f"✅ {case['description']}: {result:.2f} in range [{min_val}, {max_val}]"
                    )
                else:
                    results.append(
                        f"⚠️  {case['description']}: {result:.2f} outside range [{min_val}, {max_val}] (but may be valid weighted average)"
                    )
...
def validate_evidence_strength_logic() -> tuple[bool, str]:
    ...
    try:
        ...
        def calculate_evidence_strength(
            pattern_count: int, knowledge_count: int, entity_count: int
        ) -> float:
            evidence_factors = []
        ...
        test_cases = [
            {
                ...
                "entities": 4,
                "expected_cap": True,
                "description": "High evidence should cap at 0.4",
            },
            {
                "patterns": 1,
                "knowledge": 2,
                "entities": 1,
                "expected_cap": False,
                "description": "Low evidence should not cap",
            },
            {
                "patterns": 10,
                "knowledge": 10,
                "entities": 10,
                "expected_cap": True,
                "description": "Extreme evidence should cap at 0.4",
            },
        ]
        ...
        for case in test_cases:
            result = calculate_evidence_strength(
                case["patterns"], case["knowledge"], case["entities"]
            )
            ...
            if case["expected_cap"]:
                if result == 0.4:
                    ...
                    results.append(
                        f"❌ {case['description']}: Should cap at 0.4, got {result}"
                    )
            else:
                if result < 0.4:
                    results.append(
                        f"✅ {case['description']}: No capping needed, got {result}"
                    )
                else:
                    results.append(
                        f"⚠️  {case['description']}: Unexpectedly capped at {result}"
                    )
