...
class MerchantAgentBacktester:
    ...
    async def fetch_actual_outcomes(
        ...
        logger.info(f"   Outcome window: {outcome_start.isoformat()} to {outcome_end.isoformat()}")
    ...
    async def run_merchant_agent_on_historical(
        ...
        historical_data = await self.fetch_historical_data(entity_type, entity_id, historical_date)
        
        if historical_data["row_count"] == 0:
            logger.warning(f"⚠️ No historical data found for {entity_type}={entity_id} on {historical_date.date()}")
            return {
    ...
    async def validate_predictions(
        ...
        risk_correlation = abs(predicted_risk - actual_fraud_rate) if predicted_risk is not None else None
        ...
        logger.info(f"   Predicted Risk: {predicted_risk:.3f} (High Risk: {predicted_high_risk})")
        logger.info(f"   Actual Fraud Rate: {actual_fraud_rate:.3f} (High Fraud: {actual_high_fraud})")
        logger.info(f"   Prediction Correct: {prediction_correct}")
        logger.info(f"   Risk Correlation Error: {risk_correlation:.3f}" if risk_correlation else "   Risk Correlation Error: N/A")
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Backtest merchant agent on historical data")
    parser.add_argument("--entity-type", required=True, help="Entity type (user_id, email, ip, device_id)")
    parser.add_argument("--entity-id", required=True, help="Entity identifier")
    parser.add_argument("--historical-days", type=int, default=180, help="Days to look back (default: 180 = 6 months)")
    parser.add_argument("--historical-date", help="Specific historical date (YYYY-MM-DD), overrides historical-days")
    ...
    if args.historical_date:
        try:
            ...
            logger.error(f"❌ Invalid historical date format: {args.historical_date}. Use YYYY-MM-DD")
            return
    ...
    try:
        ...
        if validation.get("validation_complete"):
            ...
            print(f"Risk Correlation Error: {validation.get('risk_correlation_error', 'N/A')}")
            print("="*60)
