...
async def check_txs_dates():
    ...
    try:
        ...
        datetime_col = (
            "TX_DATETIME" if db_provider.lower() == "snowflake" else "tx_datetime"
        )
        ...
        if results:
            ...
            print(f"\n✅ Results:")
            print(
                f"   Total records: {r.get('TOTAL_RECORDS') or r.get('total_records', 0):,}"
            )
            print(
                f"   Earliest date: {r.get('EARLIEST_DATE') or r.get('earliest_date', 'N/A')}"
            )
            print(
                f"   Latest date: {r.get('LATEST_DATE') or r.get('latest_date', 'N/A')}"
            )
        ...
        if results_24h:
            count_24h = results_24h[0].get("RECORDS_24H") or results_24h[0].get(
                "records_24h", 0
            )
        ...
        if results_7d:
            count_7d = results_7d[0].get("RECORDS_7D") or results_7d[0].get(
                "records_7d", 0
            )
        ...
        if results_sample:
            for i, row in enumerate(results_sample, 1):
                ...
                amount = row.get("PAID_AMOUNT_VALUE_IN_CURRENCY") or row.get(
