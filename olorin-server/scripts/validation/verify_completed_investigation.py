    ...
    try:
        with get_db_session() as session:
            ...
            is_completed = (
                ...
                or state.lifecycle_stage == LifecycleStage.COMPLETED
            )
            ...
            if not is_completed:
                ...
                warnings.append(
                    f"Investigation is not COMPLETED (status: {state.status})"
                )
            ...
            if state.settings_json:
                try:
                    ...
                    print(
                        f"    investigation_type: {settings_data.get('investigation_type', 'N/A')}"
                    )
                    print(
                        f"    investigation_mode: {settings_data.get('investigation_mode', 'N/A')}"
                    )
                    print(
                        f"    entities count: {len(settings_data.get('entities', []))}"
                    )
            ...
            if state.progress_json:
                try:
                    ...
                    has_tool_data = (
                        "tool_executions" in progress_data
                        or "tools_executed" in progress_data
                    )
                    has_percent = (
                        "percent_complete" in progress_data
                        or "progress_percentage" in progress_data
                    )

                    if not has_tool_data:
                        issues_found.append(
                            "progress.tool_executions or tools_executed is missing"
                        )
                    if not has_percent:
                        issues_found.append(
                            "progress.percent_complete or progress_percentage is missing"
                        )
                    ...
                    percent_complete = progress_data.get(
                        "percent_complete"
                    ) or progress_data.get("progress_percentage", "N/A")
                    print(
                        f"    percent_complete/progress_percentage: {percent_complete}"
                    )
                    print(
                        f"    current_phase: {progress_data.get('current_phase', 'N/A')}"
                    )
                    print(f"    status: {progress_data.get('status', 'N/A')}")
                    print(
                        f"    lifecycle_stage: {progress_data.get('lifecycle_stage', 'N/A')}"
                    )
                    ...
                    if tool_executions:
                        ...
                        if tool_executions:
                            ...
                            if len(tool_executions) > 5:
                                print(
                                    f"        ... and {len(tool_executions) - 5} more"
                                )
                    elif tools_executed:
                        ...
                        if is_completed:
                            warnings.append(
                                "No tool_executions or tools_executed found in progress_json for completed investigation"
                            )
                    ...
                    if "phase_progress" in progress_data:
                        ...
                        if phase_progress:
                            print(
                                f"    phase_progress: {len(phase_progress)} phases tracked"
                            )
            ...
            if state.results_json:
                try:
                    ...
                    if isinstance(results_data, dict):
                        ...
                        if "risk_score" in results_data:
                            print(
                                f"    risk_score: {results_data.get('risk_score', 'N/A')}"
                            )
                ...
                if is_completed:
                    warnings.append(
                        "results_json is NULL for completed investigation (may be expected if no results)"
                    )
                print(f"  ⚠️  NULL")
            ...
            if (
                state.status == InvestigationStatus.COMPLETED
                and state.lifecycle_stage != LifecycleStage.COMPLETED
            ):
                issues_found.append(
                    f"status is COMPLETED but lifecycle_stage is {state.lifecycle_stage}"
                )
            ...
            if issues_found:
                ...
                if is_completed:
                    ...
                    print(
                        f"   - Progress: {'✅' if progress_data else '❌'} ({len(progress_data.get('tool_executions', [])) if progress_data else 0} tools)"
                    )
                    print(
                        f"   - Results: {'✅' if results_data else '⚠️  (may be null)'}"
                    )
                return True
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print(
            "Example: python verify_completed_investigation.py inv-1762533252070-vc3xdqf"
        )
        sys.exit(1)
