...
async def main():
    ...
    try:
        ...
        precision = (entities_with_fraud / total_entities * 100) if total_entities > 0 else 0
        ...
        if abs(precision - expected_precision) <= precision_tolerance:
            checks.append(("✅", f"Precision: {precision:.2f}% (target: {expected_precision}% ±{precision_tolerance}%)"))
        elif precision > expected_precision - precision_tolerance:
            checks.append(("⚠️ ", f"Precision: {precision:.2f}% (slightly below target: {expected_precision}%)"))
        else:
            checks.append(("❌", f"Precision: {precision:.2f}% (expected: {expected_precision}% ±{precision_tolerance}%)"))
        
        # Check 2: Lift
        if abs(lift - expected_lift) <= lift_tolerance:
            checks.append(("✅", f"Lift: {lift:.1f}x (target: {expected_lift}x ±{lift_tolerance}x)"))
        elif lift > expected_lift - lift_tolerance:
            checks.append(("⚠️ ", f"Lift: {lift:.1f}x (slightly below target: {expected_lift}x)"))
        else:
            checks.append(("❌", f"Lift: {lift:.1f}x (expected: {expected_lift}x ±{lift_tolerance}x)"))
        
        # Check 3: Fraud found
        if entities_with_fraud > 0:
            checks.append(("✅", f"Fraud detected: {entities_with_fraud} entities, {total_fraud_txs} transactions"))
        else:
        ...
        if len(results) >= 2:
            ...
            sorted_by_volume = all(
                results[i].get('transaction_count', 0) >= results[i+1].get('transaction_count', 0)
        ...
        if len(low_score_entities) == 0:
            ...
            checks.append(("❌", f"{len(low_score_entities)} entities have MODEL_SCORE ≤ 0.4 (filter not applied)"))
        ...
        for i, entity in enumerate(results[:10], 1):
            ...
            avg_score = entity.get('risk_score', 0)  # Changed from avg_risk_score to risk_score
            ...
            print(f"{i:<6} {entity_id:<35} {tx_count:<8} {avg_score:<12.3f} {fraud_count:<8} {fraud_marker}")
