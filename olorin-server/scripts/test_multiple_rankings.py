...
def main():
    ...
    print(f"╠{'═'*78}╣")
    print(f"║  Testing {len(RANKINGS)} different ranking strategies                              ║")
    print(f"║  Windows: {args.num_windows} consecutive days                                       ║")
    print(f"╚{'═'*78}╝\n")
    ...
    try:
        for i in range(args.num_windows):
            ...
            for ranking_key in RANKINGS.keys():
                ...
                if result['entities_with_fraud'] > 0:
                    print(f"  {ranking_key}: {result['entities_with_fraud']}/50 fraud ({result['precision']*100:.1f}%), {result['total_fraud_txs']} fraud txs")
        ...
        for ranking_key, window_results in results.items():
            ...
            avg_precision = sum(w['precision'] for w in window_results) / len(window_results)
        ...
        print(f"{'Rank':<6} {'Strategy':<30} {'Precision':<12} {'Entities':<12} {'Windows':<10}")
        print(f"{'─'*78}")
        
        for i, stats in enumerate(ranking_stats, 1):
            print(f"{i:<6} {stats['name']:<30} {stats['overall_precision']*100:>6.1f}%     {stats['total_fraud_entities']:>4}/{1500:<6} {stats['windows_with_fraud']:>2}/30")
