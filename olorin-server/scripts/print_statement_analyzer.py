...
class PrintStatementAnalyzer:
    ...
    def analyze_codebase(self) -> Dict:
        ...
        for i, file_path in enumerate(python_files, 1):
            ...
            try:
                ...
                if prints_in_file:
                    files_with_prints += 1
                    self.file_stats[str(file_path.relative_to(self.base_path))] = len(
                        prints_in_file
                    )
    ...
    def _analyze_file(self, file_path: Path) -> List[Dict]:
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.Call):
                    if isinstance(node.func, ast.Name) and node.func.id == "print":
                        print_info = self._analyze_print_statement(
            ...
            return self._fallback_regex_analysis(
                file_path, content if "content" in locals() else None
            )
    ...
    def _determine_context(self, file_path: Path, line_content: str, args: List) -> str:
        ...
        # Debug prints
        if any(
            keyword in line_lower or keyword in args_str
            for keyword in ["debug", "debugging", "dbg", "trace", "inspect"]
        ):
            return "debug"
        ...
        if any(
            keyword in line_lower or keyword in args_str
            for keyword in ["error", "exception", "fail", "traceback", "err"]
        ):
            return "error"
        ...
        if any(
            keyword in line_lower or keyword in args_str
            for keyword in ["warning", "warn", "caution", "alert"]
        ):
            return "warning"
        ...
        if any(
            keyword in line_lower or keyword in args_str
            for keyword in ["todo", "fixme", "temp", "tmp", "hack"]
        ):
            return "development"
        ...
        migration_plan = {
            ...
            "priority_levels": {
                ...
                "high": {
                    ...
                    "count": len(self.categories["warning"])
                    + len(self.categories["debug"]),
                ...
                "low": {
                    ...
                    "count": (
                        len(self.categories["test_output"])
            ...
            "top_files_by_prints": sorted(
                analysis_result["file_stats"].items(), key=lambda x: x[1], reverse=True
            )[:20],
        }
    ...
    def save_analysis(
        self, analysis_result: Dict, migration_plan: Dict, output_file: str = None
    ):
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze print statements for unified logging migration"
    )
    parser.add_argument(
        "--base-path",
        default=".",
        help="Base path to analyze (default: current directory)",
    )
    parser.add_argument(
        "--output",
        default="print_statement_analysis.json",
        help="Output file for analysis results",
    )
    parser.add_argument(
        "--summary-only",
        action="store_true",
        help="Show only summary, no detailed analysis",
    )
    ...
    if args.summary_only:
        ...
        print(
            f"Quick analysis: {total_prints} print statements found in {len(python_files)} Python files"
        )
        return
