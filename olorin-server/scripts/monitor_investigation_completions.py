...
def monitor_completions(check_interval_seconds: int = 30, max_checks: int = 20):
    ...
    print(
        f"   Max checks: {max_checks} (total time: {max_checks * check_interval_seconds / 60:.1f} minutes)\n"
    )
    ...
    print(
        f"📊 Initial state: {len(seen_investigation_ids)} completed investigations already seen\n"
    )

    for check_num in range(1, max_checks + 1):
        print(
            f"[Check {check_num}/{max_checks}] {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        )
            ...
            for inv in investigations:
                ...
                if status.upper() in ("COMPLETED", "COMPLETE"):
                    if inv_id not in seen_investigation_ids:
                        ...
                        entity_id = inv.get("entity_id") or inv.get(
            ...
            if new_completions:
                ...
                if new_with_scores:
                    ...
                    for inv in new_with_scores:
                        print(
                            f"      • {inv['id'][:12]}... | {inv['entity']} | Risk: {inv['risk_score']:.3f}"
                        )
                ...
                if new_without_scores:
                    ...
                    for inv in new_without_scores:
                        print(
                            f"      • {inv['id'][:12]}... | {inv['entity']} | Risk: MISSING"
                        )
                ...
                running_count = sum(
                    1
                    for inv in investigations
                    if inv.get("status", "").upper()
                    in ("RUNNING", "PENDING", "IN_PROGRESS")
                )
    ...
    completed_with_score = sum(
        1
        for inv in final_investigations
        if inv.get("status", "").upper() in ("COMPLETED", "COMPLETE")
...
if __name__ == "__main__":
    ...
    monitor_completions(
        check_interval_seconds=args.interval, max_checks=args.max_checks
    )
