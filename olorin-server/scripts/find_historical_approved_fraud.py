...
async def find_historical_fraud():
    ...
    print(f"📊 Searching period: {search_start.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')} (90 days)")
    print()
    ...
    try:
        ...
        if not results:
            ...
            print(f"   Searched: {search_start.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}")
            print()
            ...
            if earlier_results:
                print(f"✅ Found {len(earlier_results)} days with approved fraud in earlier period")
                print(f"   Period: {earlier_start.strftime('%Y-%m-%d')} to {earlier_end.strftime('%Y-%m-%d')}")
                print()
        ...
        print(f"✅ Found {len(results)} days with APPROVED fraud in the 6-month lookback period")
        print()
        ...
        if results:
            ...
            print(f"Most recent day with approved fraud (before 6 months ago): {str(date)[:10]}")
            print(f"   Approved fraud transactions: {count}")
            print(f"   Unique fraud entities: {entities}")
            print()
            ...
            print()
            print(f"This will investigate the 24-hour window ending on {fraud_date.strftime('%Y-%m-%d')}")
            print(f"All {entities} fraud entities from that day will be investigated over a 2-year lookback")
