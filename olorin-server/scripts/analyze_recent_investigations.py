...
def analyze_investigation(investigation: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if progress_json:
        try:
            ...
            domain_count = (
                len(domain_findings) if isinstance(domain_findings, dict) else 0
            )
    ...
    return {
        ...
        "transaction_scores_count": (
            len(transaction_scores) if transaction_scores else 0
        ),
...
def main():
    ...
    for i, inv in enumerate(recent_invs, 1):
        ...
        print(
            f"   Has Transaction Scores: {analysis['has_transaction_scores']} ({analysis['transaction_scores_count']} transactions)"
        )
    ...
    with_scores = [
        ...
        if analyze_investigation(inv)["has_transaction_scores"]
    ]
    ...
    risk_scores = [
        inv.get("overall_risk_score")
        for inv in investigations
        if inv.get("overall_risk_score") is not None
    ]
    if risk_scores:
