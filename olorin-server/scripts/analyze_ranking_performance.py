...
# Analyze each window
for i, (current_window, fraud_aware_window) in enumerate(zip(results['current'], results['fraud_aware']), 1):
    
    if current_window['entities_with_fraud'] == 0 and fraud_aware_window['entities_with_fraud'] == 0:
        continue  # Skip windows with no fraud in either
    
    print(f"\nWindow {i}: {current_window['window_start'][:10]}")
    print(f"  Current: {current_window['entities_with_fraud']} entities, {current_window['total_fraud_txs']} fraud txs")
    print(f"  Fraud-Aware: {fraud_aware_window['entities_with_fraud']} entities, {fraud_aware_window['total_fraud_txs']} fraud txs")
    
    # Get fraud entities from each
    current_fraud = {e['email']: e for e in current_window['entities'] if e['fraud_count'] > 0}
    fraud_aware_fraud = {e['email']: e for e in fraud_aware_window['entities'] if e['fraud_count'] > 0}
    ...
    if only_current:
        ...
        for email in only_current:
            ...
            print(f"     - {email}: {e['fraud_count']} fraud, {e['tx_count']} total, ${e['avg_amount']:.0f} avg, {e['unique_ips']} IPs")
...
def analyze_entities(entities, label):
    ...
    avg_fraud_rate = sum(e['fraud_count'] / e['tx_count'] for e in entities) / len(entities)
...
# Compare amounts
current_avg_amount = sum(e['avg_amount'] for e in current_fraud_entities) / len(current_fraud_entities)
fraud_aware_avg_amount = sum(e['avg_amount'] for e in fraud_aware_fraud_entities) / len(fraud_aware_fraud_entities)

print(f"Amount difference: Current catches fraud at ${current_avg_amount:.0f} avg, Fraud-Aware at ${fraud_aware_avg_amount:.0f} avg")
print(f"Fraud-Aware is {fraud_aware_avg_amount/current_avg_amount:.1f}x higher amount fraud")
print()

# IP diversity
current_avg_ips = sum(e['unique_ips'] for e in current_fraud_entities) / len(current_fraud_entities)
fraud_aware_avg_ips = sum(e['unique_ips'] for e in fraud_aware_fraud_entities) / len(fraud_aware_fraud_entities)

print(f"IP diversity: Current catches fraud with {current_avg_ips:.1f} IPs, Fraud-Aware with {fraud_aware_avg_ips:.1f} IPs")
print()

# Transaction count
current_avg_txs = sum(e['tx_count'] for e in current_fraud_entities) / len(current_fraud_entities)
fraud_aware_avg_txs = sum(e['tx_count'] for e in fraud_aware_fraud_entities) / len(fraud_aware_fraud_entities)

print(f"Transaction count: Current catches fraud with {current_avg_txs:.1f} txs, Fraud-Aware with {fraud_aware_avg_txs:.1f} txs")
print()
