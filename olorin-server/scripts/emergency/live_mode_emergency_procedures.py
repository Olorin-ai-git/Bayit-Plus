...
from app.service.agent.orchestration.hybrid.live_mode_safety_manager import (
    LiveModeSafetyManager, EmergencyStopReason, LiveModeSafetyLevel
)
    ...
    async def emergency_stop(self, reason: str = "Manual emergency stop"):
        ...
        try:
            ...
            await self.safety_manager.activate_emergency_stop(
                EmergencyStopReason.MANUAL_KILL_SWITCH,
                reason
            )
            ...
            print(f"Emergency Report: {self.emergency_dir / f'emergency_report_{self.timestamp}.json'}")
...
>>>>>>> 001-modify-analyzer-method
        ...
        try:
            ...
            emergency_cost_report = {
                ...
                "budget_status": {
                    ...
                    "remaining_budget": max(0, self.safety_manager.cost_limits.per_session - cost_report.total_cost)
                },
                "recommendations": self.cost_tracker.get_cost_optimization_recommendations()
            }
            
            # Save cost report
            cost_file = self.emergency_dir / f"emergency_cost_report_{self.timestamp}.json"
    ...
    async def _save_emergency_report(self, emergency_report: Dict[str, Any]):
        ...
        try:
            ...
            summary_file = self.emergency_dir / f"emergency_summary_{self.timestamp}.txt"
            with open(summary_file, "w") as f:
                ...
                f.write(f"Cost report saved in: emergency_cost_report_{self.timestamp}.json\n")
    ...
    async def rollback_to_safe_configuration(self):
        ...
        try:
            ...
            rollback_file = self.emergency_dir / f"rollback_report_{self.timestamp}.json"
    ...
    async def health_check(self):
        ...
        try:
            ...
            all_checks = [env_check, process_check, safety_check, filesystem_check, network_check]
        ...
        try:
            ...
            if safety_status.requires_immediate_termination:
                return {
                    ...
                    "active_breakers": [k for k, v in self.safety_manager.circuit_breakers.items() if v]
                }
    ...
    def _check_filesystem(self) -> Dict[str, Any]:
        ...
        try:
            ...
            temp_dirs = [d for d in [Path("temp"), Path("tmp"), Path(".investigation_cache")] if d.exists()]
...
Examples:
  ...
  python live_mode_emergency_procedures.py health-check
    ...
    parser.add_argument(
        'command',
        choices=['emergency-stop', 'cost-report', 'rollback', 'health-check', 'reset-circuits', 'backup-state'],
    ...
    async def run_command():
        ...
        try:
            if args.command == 'emergency-stop':
                if not args.force:
                    response = input("🚨 EMERGENCY STOP - This will terminate all investigations and incur costs. Continue? (yes/no): ")
