...
class SelectiveMigrationTool:
    ...
    def _migrate_print_line(self, line: str, line_no: int) -> str:
        ...
        patterns = [
            # Error patterns
            (r'print\s*\(\s*([f]?["\'][^"\']*[Ee]rror[^"\']*["\'].*?)\)', r'logger.error(\1)'),
            (r'print\s*\(\s*([f]?["\'][^"\']*[Ff]ail[^"\']*["\'].*?)\)', r'logger.error(\1)'),
            (r'print\s*\(\s*([f]?["\'][^"\']*[Ee]xception[^"\']*["\'].*?)\)', r'logger.error(\1)'),
            
            # Warning patterns
            (r'print\s*\(\s*([f]?["\'][^"\']*[Ww]arn[^"\']*["\'].*?)\)', r'logger.warning(\1)'),
            
            # Debug patterns
            (r'print\s*\(\s*([f]?["\'][^"\']*[Dd]ebug[^"\']*["\'].*?)\)', r'logger.debug(\1)'),
        ...
        for file_path in files:
            ...
            if result['success']:
                if result['migrations_count'] > 0:
                    ...
                    logger.info(f"  ✅ {file_path}: {result['migrations_count']} migrations")
                else:
...
def main():
    parser = argparse.ArgumentParser(description='Selective migration of project print statements')
