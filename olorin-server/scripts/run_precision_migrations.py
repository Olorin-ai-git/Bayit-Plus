...
def main():
    ...
    try:
        with get_db_session() as db:
            ...
            logger.info(
                f"Found {len(precision_migrations)} precision detection migration file(s)"
            )
            ...
            for migration_file in precision_migrations:
                ...
                if "_sqlite.sql" in filename_lower and db_type != "sqlite":
                    logger.debug(
                        f"Skipping SQLite-specific migration: {migration_file.name}"
                    )
                    continue
                # For PostgreSQL-specific files on SQLite, check for SQLite version
                if (
                    runner._is_postgresql_specific(migration_file)
                    ...
                    if sqlite_version.exists():
                        logger.info(
                            f"Using SQLite-compatible version: {sqlite_version.name}"
                        )
                        migration_file = sqlite_version
                    else:
                        logger.warning(
                            f"Skipping PostgreSQL-specific migration (no SQLite version): {migration_file.name}"
                        )
                        continue
