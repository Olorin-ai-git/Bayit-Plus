        ...
        if folder_path and folder_path.exists():
            ...
            for log_type, log_path in file_paths.items():
                if log_path.exists():
                    ...
                    print(
                        f"  ✅ {log_type}: {log_path.name} ({size if size == 'dir' else f'{size:,} bytes'})"
                    )
                else:
            ...
            if journey_file and journey_file.exists():
                ...
                print(
                    f"Node Executions: {len(journey_data.get('node_executions', []))}"
                )
                print(
                    f"State Transitions: {len(journey_data.get('state_transitions', []))}"
                )
                print(
                    f"Agent Coordinations: {len(journey_data.get('agent_coordinations', []))}"
                )
                print()
