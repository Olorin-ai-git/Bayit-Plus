...
async def test_may_21_22():
    ...
    if features.get('max_repeated_amount_ratio', 0) > 0.5:
        print(f"  🚨 REPEATED AMOUNTS: {features.get('max_repeated_amount_count', 0)} identical")
    
    if features.get('rapid_succession', 0) > 0:
        print(f"  🚨 RAPID: Min interval {features.get('min_interval_minutes', 0):.1f} minutes")
    ...
    if risk_assessment['anomalies']:
        ...
        for anomaly in risk_assessment['anomalies']:
            ...
            print(f"     Severity: {anomaly['severity']}, Risk: {anomaly['risk_contribution']:.1%}")
        print()
    ...
    if len(fraud_txs) > 0:
        ...
        print(f"RECALL:    {recall:6.1%} ({caught_fraud}/{len(fraud_txs)} fraud detected)")
    ...
    if caught_fraud == len(fraud_txs):
        ...
        print(f"✅ SUCCESS! Caught {caught_fraud}/{len(fraud_txs)} fraud ({recall:.1%} recall)")
    else:
