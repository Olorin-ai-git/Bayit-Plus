...
def reconstruct_app_state(entities, investigations):
    ...
    class MockAppState:
        def __init__(self, entities_list, comparison_results):
            ...
            self.risk_entities_loaded_at = datetime(
                2025, 11, 14, 16, 20, 52
            ).isoformat()
            self.top_risk_entities = {
                "status": "success",
                "entities": entities_list[:10],
                "summary": {"group_by": "email", "total_entities": len(entities_list)},
...
def main():
    ...
    reports_dir = Path(
        "artifacts/comparisons/auto_startup/comparison_package_20251114_162724/comparison_reports"
    )
    ...
    investigations = get_investigations_from_time_window(
    ...
    # Generate report
    report_path = Path(
        "artifacts/comparisons/auto_startup/comparison_package_20251114_162724/startup_analysis_report.html"
    )

    print(f"\nRegenerating report with reconstructed data...")
    generated_path = generate_startup_report(
        app_state=mock_state, output_path=report_path, startup_duration_seconds=45.0
    )
