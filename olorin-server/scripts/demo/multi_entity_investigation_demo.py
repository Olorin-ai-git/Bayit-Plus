...
async def demo_multi_entity_investigation():
    ...
    request = MultiEntityInvestigationRequest(
        entities=[
            ...
            {
                "entity_id": "EMAIL_DEMO_user@test.com",
                "entity_type": "email_normalized",
            },
        ],
        relationships=[
            EntityRelationship(
    ...
    try:
        ...
        if status:
            ...
            print(
                "⚠️ Investigation not found in active investigations (may have completed)"
            )
        ...
        for entity_id, results in final_result.entity_results.items():
            ...
            for result in results:
                print(
                    f"     * {result.agent_type}: Risk={result.risk_score:.3f}, Confidence={result.confidence_score:.3f}"
                )
        ...
        if final_result.relationship_insights:
            ...
            for insight in final_result.relationship_insights:
                ...
                print(
                    f"     Risk Impact: {insight.risk_impact:.3f}, Confidence: {insight.confidence_level:.3f}"
                )
    ...
    print(
        f"   - Successful Investigations: {orchestrator_metrics['successful_investigations']}"
    )
    print(
        f"   - Failed Investigations: {orchestrator_metrics['failed_investigations']}"
    )
    print(f"   - Success Rate: {orchestrator_metrics['success_rate']:.1%}")
    print(
        f"   - Average Execution Time: {orchestrator_metrics['avg_execution_time_ms']:.0f}ms"
    )
    print(f"   - Entities Processed: {orchestrator_metrics['entities_processed']}")
    print(
        f"   - Active Investigations: {orchestrator_metrics['active_investigations']}"
    )
...
def demo_api_request_format():
    ...
    sample_api_request = {
        ...
        "enable_verbose_logging": True,
        "enable_cross_entity_analysis": True,
        "metadata": {"case_id": "CASE_DEMO_001", "analyst": "demo_user"},
    }
