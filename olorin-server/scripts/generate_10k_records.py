...
class DataGenerator:
    ...
    def __init__(self):
        ...
        self.safe_domains = ['gmail.com', 'yahoo.com', 'outlook.com', 'hotmail.com', 
                            'company.com', 'business.com', 'corporate.net', 'enterprise.org']
        self.risky_domains = ['tempmail.com', 'guerrillamail.com', 'mailinator.com', 
                             'throwaway.email', 'temporary.net']
        self.fraud_domains = ['darkweb.onion', 'stolen.cc', 'fraud.net', 'scam.org', 
                             'phishing.com', 'hackers.io']
        ...
        self.merchant_categories = {
            'low_risk': ['Groceries', 'Gas Station', 'Restaurant', 'Coffee Shop', 'Pharmacy'],
            'medium_risk': ['Electronics', 'Clothing', 'Department Store', 'Online Marketplace'],
            'high_risk': ['Cryptocurrency', 'Gambling', 'Adult', 'Money Transfer', 'Gift Cards']
        }
    ...
    def generate_location_data(self, risk_level: str) -> dict:
        ...
        city = random.choice(cities.get(country, ['City_' + str(random.randint(1, 100))]))
    ...
    for user_id in range(num_users):
        ...
        for _ in range(num_transactions):
            ...
            transaction = (
                ...
                tx_datetime - timedelta(days=random.randint(30, 1000)),  # ACCOUNT_CREATED_DATE
                random.randint(30, 1000),  # ACCOUNT_AGE_DAYS
                'VERIFIED' if risk_level == 'low' else random.choice(['VERIFIED', 'PENDING', 'UNVERIFIED']),  # ACCOUNT_VERIFICATION_STATUS
                'COMPLETED' if risk_level == 'low' else random.choice(['COMPLETED', 'PARTIAL', 'NONE']),  # KYC_STATUS
                str(random.randint(1, 3)),  # KYC_LEVEL
                
                # Device fields
                device_id,  # DEVICE_ID
                random.choice(generator.device_types),  # DEVICE_TYPE
                random.choice(['iOS', 'Android', 'Windows', 'MacOS', 'Linux']),  # DEVICE_OS
                ...
                hashlib.md5(f"{device_id}_{user_id}".encode()).hexdigest(),  # DEVICE_FINGERPRINT
                ...
                'RESIDENTIAL' if risk_level == 'low' else random.choice(['RESIDENTIAL', 'BUSINESS', 'DATACENTER', 'VPN']),  # IP_TYPE
                ...
                'CARD',  # PAYMENT_METHOD
                random.choice(['Stripe', 'PayPal', 'Square', 'Adyen']),  # PAYMENT_PROCESSOR
                ...
                # Third-party scores
                random.uniform(0, 1) if random.random() > 0.5 else None,  # MAXMIND_RISK_SCORE
                random.uniform(0, 100) if random.random() > 0.5 else None,  # SIFT_SCORE
                random.uniform(0, 1000) if random.random() > 0.5 else None,  # EMAILAGE_SCORE
                random.uniform(0, 1) if random.random() > 0.5 else None,  # IPQUALITYSCORE
                random.uniform(0, 100) if random.random() > 0.5 else None,  # THREATMETRIX_SCORE
                ...
                f'AGENT_{random.randint(1, 10):03d}' if is_fraud else None,  # MANUAL_REVIEW_AGENT
                tx_datetime + timedelta(minutes=random.randint(5, 60)) if is_fraud else None,  # MANUAL_REVIEW_DATETIME
                
                # Decision fields
                'REJECT' if is_fraud else 'APPROVE',  # NSURE_LAST_DECISION
                'Risk threshold exceeded' if risk_score > 0.7 else None,  # DECISION_REASON
...
def insert_to_snowflake(transactions: List[Tuple]):
    ...
    try:
        ...
        print(f"{'Rank':<5} {'Email':<35} {'Risk Value':<15} {'Txns':<6} {'Avg Risk':<10} {'Frauds'}")
        print("-"*90)
        
        for i, row in enumerate(cursor.fetchall(), 1):
            email = row[0][:33] + '..' if len(row[0]) > 35 else row[0]
            print(f"{i:<5} {email:<35} ${row[2]:<14,.2f} {row[1]:<6} {row[3]:<10.4f} {row[5]}")
    ...
    print("  poetry run python scripts/get_top_risk_entities.py --time-window 30d --top 5")
