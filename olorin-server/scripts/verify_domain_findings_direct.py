...
from app.persistence.database import get_db_session, init_database, get_engine
...
def run_sql_query(investigation_id: str):
    ...
    try:
        with get_db_session() as session:
            ...
            progress_domain_findings = progress_data.get('domain_findings', {}) if progress_data else {}
            results_domain_findings = results_data.get('domain_findings', {}) if results_data else {}
            ...
            if progress_domain_findings:
                ...
                for domain_name in sorted(domains_in_progress):
                    ...
                    print(f"      📊 {domain_name}:")
                    print(f"         - Risk Score: {domain_data.get('risk_score', 'N/A')}")
                    print(f"         - Confidence: {domain_data.get('confidence', 'N/A')}")
                    ...
                    if has_llm:
                        ...
                        print(f"         - LLM Duration: {llm_analysis.get('analysis_duration', 'N/A')}s")
            ...
            print(f"✅ Progress JSON has domain_findings: {has_progress} ({len(progress_domain_findings)} domains)")
            print(f"✅ Results JSON has domain_findings: {has_results} ({len(results_domain_findings)} domains)")
...
def main():
    """Main entry point."""
    if len(sys.argv) < 2:
        print("Usage: python scripts/verify_domain_findings_direct.py <investigation_id>")
        print("\nExample:")
        print("  python scripts/verify_domain_findings_direct.py inv-1762553138822-ej74xpa")
        sys.exit(1)
