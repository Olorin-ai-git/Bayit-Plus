...
FILTERS = {
    ...
    'f6_high_model': {
        'name': 'MODEL_SCORE > 0.4',
        'having': 'AVG(MODEL_SCORE) > 0.4'
    },
...
def test_filter(conn, window_start, window_end, filter_key):
    ...
    entities = [dict(zip(['EMAIL', 'TX_COUNT', 'FRAUD_COUNT', 'UNIQUE_IPS', 'AVG_MODEL_SCORE', 'AVG_AMOUNT'], row)) for row in results]
...
def main():
    ...
    print(f"╠{'═'*78}╣")
    print(f"║  Testing {len(FILTERS)} different filter strategies                           ║")
    ...
    try:
        for i in range(args.num_windows):
            ...
            for filter_key in FILTERS.keys():
                ...
                if result['total_entities'] > 0 and result['entities_with_fraud'] > 0:
                    print(f"  {filter_key}: {result['entities_with_fraud']}/{result['total_entities']} fraud ({result['precision']*100:.1f}%)")
        ...
        print(f"{'Rank':<6} {'Filter Strategy':<35} {'Precision':<12} {'Fraud/Total':<15} {'Fraud Txs':<10}")
        print(f"{'─'*78}")
        
        for i, s in enumerate(stats, 1):
            print(f"{i:<6} {s['name']:<35} {s['overall_precision']*100:>6.1f}%     {s['total_fraud_entities']:>3}/{s['total_entities']:<8} {s['total_fraud_txs']:>6}")
