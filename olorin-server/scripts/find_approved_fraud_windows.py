    ...
    try:
        ...
        if not results:
            ...
            if any_fraud_results:
                print(
                    f"✅ Found {len(any_fraud_results)} days with fraud (any decision status)"
                )
                print()
                print(
                    "Day                  | Total | Approved | Rejected | Other | Entities"
                )
                print("-" * 80)
                for row in any_fraud_results[:10]:
                    ...
                    print(
                        f"{str(date)[:10]:20} | {total:5} | {approved:8} | {rejected:8} | {other:5} | {entities:8}"
                    )
            else:
