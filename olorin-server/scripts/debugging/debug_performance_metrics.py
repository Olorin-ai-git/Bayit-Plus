...
class PerformanceMetrics:
    ...
    async def benchmark_api_response(self, endpoint: str, method: str = "GET", data: Dict = None):
    ...
    def analyze_performance(self):
        ...
        if self.metrics['system']:
            ...
            analysis['system_performance'] = {
                ...
                'memory_status': 'Good' if system.get('memory_percent', 0) < 80 else 'High',
                'disk_status': 'Good' if system.get('disk_percent', 0) < 80 else 'Low Space',
                'recommendations': []
            }
            
            if system.get('cpu_percent', 0) > 70:
                analysis['system_performance']['recommendations'].append('Consider CPU optimization')
            if system.get('memory_percent', 0) > 80:
                analysis['system_performance']['recommendations'].append('Consider memory optimization')
            if system.get('disk_percent', 0) > 80:
                analysis['system_performance']['recommendations'].append('Free up disk space')
        
        # API performance analysis
        if self.metrics['api_responses']:
            response_times = [r['response_time_ms'] for r in self.metrics['api_responses'] if 'response_time_ms' in r]
            if response_times:
                analysis['api_performance'] = {
                    ...
                    'success_rate': len([r for r in self.metrics['api_responses'] if r.get('success', False)]) / len(self.metrics['api_responses'])
                }
        
        # Tool performance analysis
        if self.metrics['tool_executions']:
            tool_times = [t['avg_execution_time_ms'] for t in self.metrics['tool_executions'] if 'avg_execution_time_ms' in t]
            if tool_times:
                analysis['tool_performance'] = {
                    'avg_tool_execution_ms': round(sum(tool_times) / len(tool_times), 2),
...
async def run_performance_analysis():
    ...
    try:
        ...
        for tool_name in tools_to_test:
            ...
            if 'error' not in result:
                print(f"      Avg execution time: {result.get('avg_execution_time_ms', 0):.2f}ms")
            else:
                print(f"      Error: {result.get('error', 'Unknown')}")
    ...
    if 'api_performance' in analysis:
        ...
        print(f"      Avg Response Time: {api_perf.get('avg_response_time_ms', 0):.2f}ms")
    ...
    if 'tool_performance' in analysis:
        ...
        print(f"      Avg Execution Time: {tool_perf.get('avg_tool_execution_ms', 0):.2f}ms")
