...
def main():
    ...
    parser.add_argument('--time-window', default=None, help='Time window (e.g., 7d, 30d, 90d)')
    parser.add_argument('--group-by', default=None, help='Group by field (email, ip, device_id)')
    parser.add_argument('--top-percentage', type=float, default=None, help='Top percentage (e.g., 10)')
    ...
    top_percentage = args.top_percentage or float(os.getenv('ANALYTICS_DEFAULT_TOP_PERCENTAGE', '10'))
    ...
    try:
        ...
        if results:
            ...
            print(f"\n📊 Processing results...")
            analysis = analyzer._process_results(results, time_window, group_by, top_percentage)
