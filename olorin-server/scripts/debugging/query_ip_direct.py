...
def main():
    ...
    try:
        ...
        if db_provider_name == 'snowflake':
            ...
            count_query = f"SELECT COUNT(*) as total_count FROM {table_name} WHERE ip = $1"
        ...
        if db_provider_name == 'snowflake':
            ...
            count_results = db_provider.execute_query(count_query, params={"ip": ip_address})
        ...
        if db_provider_name == 'snowflake':
            ...
            transactions = db_provider.execute_query(transaction_query, params={"ip": ip_address})
        ...
        if transactions:
            ...
            for i, tx in enumerate(transactions[:5], 1):  # Show first 5
                ...
                print(f"      TX_DATETIME: {tx.get('TX_DATETIME') or tx.get('tx_datetime')}")
                amount = tx.get('PAID_AMOUNT_VALUE_IN_CURRENCY') or tx.get('paid_amount_value_in_currency')
                currency = tx.get('PAID_AMOUNT_CURRENCY') or tx.get('paid_amount_currency')
                print(f"      AMOUNT: {amount} {currency}")
                print(f"      MODEL_SCORE: {tx.get('MODEL_SCORE') or tx.get('model_score')}")
                print(f"      IS_FRAUD_TX: {tx.get('IS_FRAUD_TX') or tx.get('is_fraud_tx')}")
                print(f"      DECISION: {tx.get('MODEL_DECISION') or tx.get('model_decision')}")
        ...
        if db_provider_name == 'snowflake':
            ...
            similar_results = db_provider.execute_query(similar_ip_query, params={"ip": ip_address})
        ...
        if similar_results:
            ...
            for result in similar_results:
                ...
                print(f"      IP: {ip} | Count: {count} | First: {first_tx} | Last: {last_tx}")
        else:
