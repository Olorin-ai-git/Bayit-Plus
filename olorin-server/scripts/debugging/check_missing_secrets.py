...
def check_firebase_secrets():
    ...
    if missing_secrets:
        ...
        logger.info("Operation status update")
    ...
    if env_found:
        logger.info("Configuration check completed")
        logger.info(
            f"💡 {len(env_found)} missing secrets have environment variable fallbacks"
        )
    ...
    try:
        ...
        for attr, env_name in secret_attrs:
            if hasattr(config, attr):
                ...
                if (
                    value and value != env_name
                ):  # Has actual value, not just the env var name
