...
class HybridToolExecutionDebugger:
    ...
    async def _create_debug_state(
        ...
        logger.info(f"   Initial tools_used: {len(initial_state.get('tools_used', []))}")
        logger.info(f"   Initial tool_results: {len(initial_state.get('tool_results', {}))}")
    ...
    async def _analyze_execution_results(
        ...
        logger.info(f"   Tool execution attempts: {final_state.get('tool_execution_attempts', 0)}")
    ...
    def print_debug_report(self):
        ...
        print(f"   Graph Build: {'✅ SUCCESS' if self.debug_results.get('graph_build_success') else '❌ FAILED'}")
        print(f"   Execution: {'✅ COMPLETED' if self.debug_results.get('execution_completed') else '❌ FAILED'}")
        ...
        print(f"   Execution attempts: {tool_tracking.get('tool_execution_attempts', 0)}")
        
        # Data extraction summary
        data_analysis = self.debug_results.get("data_extraction_analysis", {})
        print(f"\n📊 Data Extraction Analysis:")
        print(f"   Snowflake completed: {'✅' if data_analysis.get('snowflake_completed') else '❌'}")
        print(f"   Network data: {'✅' if data_analysis.get('network_data') else '❌'}")
        print(f"   Device data: {'✅' if data_analysis.get('device_data') else '❌'}")
        print(f"   Location data: {'✅' if data_analysis.get('location_data') else '❌'}")
        ...
        print(f"   Reached tool execution: {'✅' if phase_progression.get('reached_tool_execution') else '❌'}")
...
async def main():
    ...
    parser.add_argument("--investigation-id", default="TEST_DEBUG_001",
