...
from app.models.agent_context import AdditionalMetadata, AgentContext, AgentMetadata
...
async def debug_feature_flags():
    ...
    try:
        ...
        for flag_name, flag_data in feature_flags.flags.items():
            ...
            print(
                f"   {status_icon} {flag_name}: {enabled} ({rollout}% rollout, {mode})"
            )

        # Check if hybrid graph is enabled
        hybrid_enabled = feature_flags.is_enabled(
            "hybrid_graph_v1", "test-investigation-123"
        )
        print(
        ...
        for graph_type in [GraphType.CLEAN, GraphType.HYBRID, GraphType.ORCHESTRATOR]:
            try:
                ...
                if graph:
                    ...
                    print(
                        f"   ✅ {graph_type.value} graph created successfully ({duration:.2f}s)"
                    )
                    ...
                    domain_agents = [
                        ...
                        if any(
                            domain in node
                            for domain in ["network", "device", "location", "logs"]
                        )
                    ]
...
async def debug_domain_agent_direct():
    ...
    try:
        ...
        domain_agents = [
            (
                "network_agent",
                "app.service.agent.orchestration.domain_agents.network_agent",
            ),
            (
                "device_agent",
                "app.service.agent.orchestration.domain_agents.device_agent",
            ),
            (
                "location_agent",
                "app.service.agent.orchestration.domain_agents.location_agent",
            ),
            ...
            try:
                ...
                if hasattr(module, node_function_name):
                    ...
                    test_state = HybridInvestigationState(
                        messages=[
                            HumanMessage(content="Test message for domain agent")
                        ],
...
async def debug_investigation_execution():
    ...
    try:
        ...
        metadata = AgentMetadata(
            username="debug_user", additional_metadata=additional_metadata
        )
