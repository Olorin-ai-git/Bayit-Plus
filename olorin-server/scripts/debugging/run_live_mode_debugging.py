...
from app.service.agent.orchestration.hybrid.live_mode_safety_manager import (
    LiveModeSafetyManager, LiveModeSafetyLevel, EmergencyStopReason
)
...
class LiveModeDebuggingOrchestrator:
    ...
    async def run_full_debugging_session(self) -> Dict[str, Any]:
        ...
        try:
            ...
            session_report["duration_hours"] = (datetime.now() - self.session_start_time).total_seconds() / 3600
    ...
    async def run_phase(self, phase_number: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if phase_number == 1:
                phase_result = await self._run_phase_1_environment_verification(phase_result)
            elif phase_number == 2:
                phase_result = await self._run_phase_2_live_data_integration(phase_result)
            elif phase_number == 3:
                phase_result = await self._run_phase_3_external_api_validation(phase_result)
            elif phase_number == 4:
                phase_result = await self._run_phase_4_cost_management(phase_result)
            elif phase_number == 5:
                phase_result = await self._run_phase_5_live_investigation_testing(phase_result)
            elif phase_number == 6:
                phase_result = await self._run_phase_6_performance_analysis(phase_result)
            else:
            ...
            phase_result["duration_minutes"] = (phase_end_time - phase_start_time).total_seconds() / 60
            ...
            logger.info(f"✅ Phase {phase_number} completed - Success: {phase_result['success']}")
            return phase_result
    ...
    async def _run_phase_1_environment_verification(self, phase_result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _run_phase_2_live_data_integration(self, phase_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            await self.cost_tracker.track_snowflake_usage(1.0, {"task": "schema_validation"})
            ...
            await self.cost_tracker.track_snowflake_usage(1.5, {"task": "pipeline_testing"})
    ...
    async def _run_phase_3_external_api_validation(self, phase_result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _run_phase_4_cost_management(self, phase_result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _run_phase_5_live_investigation_testing(self, phase_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            await self.cost_tracker.track_claude_usage(
                "claude-opus-4-1", 5000, 2000, investigation_id="test_001", phase="investigation"
            )
    ...
    async def _run_phase_6_performance_analysis(self, phase_result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _handle_emergency_stop(self, reason, message, emergency_record):
        ...
        emergency_file = self.results_dir / f"emergency_stop_{datetime.now().strftime('%H%M%S')}.json"
    ...
    async def _handle_cost_alert(self, alert):
        ...
        alert_file = self.results_dir / f"cost_alert_{datetime.now().strftime('%H%M%S')}.json"
    ...
    async def _generate_session_report(self, session_report):
        ...
        session_report["safety_summary"] = self.safety_manager.get_safety_status().__dict__
        session_report["cost_summary"] = self.cost_tracker.generate_cost_report().__dict__
        ...
        with open(summary_file, "w") as f:
            ...
            for phase_name, phase_result in session_report["phases"].items():
                f.write(f"  {phase_name}: {'✅' if phase_result['success'] else '❌'}\n")
            
            if session_report.get("cost_summary"):
                f.write(f"\nCOST BREAKDOWN:\n")
                cost_summary = session_report["cost_summary"]
                for category, amount in cost_summary.get("cost_by_category", {}).items():
...
def main():
    ...
    parser.add_argument(
        '--full-run',
        action='store_true',
        help='Run complete 6-phase debugging session'
    )
    
    parser.add_argument(
        '--safety-level',
        choices=['component_test', 'single_investigation', 'limited_batch', 'operational'],
