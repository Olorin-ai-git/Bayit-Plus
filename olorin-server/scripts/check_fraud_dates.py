...
async def check_fraud_dates():
    ...
    investigation_start = datetime.now() - timedelta(days=int(2.5*365))  # 2.5 years ago
    ...
    if results:
        ...
        analyzer_fraud = [r for r in results 
                         if str(r.get('TX_DATETIME') or r.get('tx_datetime'))[:10] == analyzer_date.strftime('%Y-%m-%d')]
        
        if analyzer_fraud:
            print(f"✅ Fraud found on analyzer date: {sum(r.get('COUNT') or r.get('count') for r in analyzer_fraud)} transactions")
        else:
