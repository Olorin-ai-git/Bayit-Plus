...
def main():
    ...
    if not XGBOOST_AVAILABLE or not SKLEARN_AVAILABLE:
        logger.error(
            "Required dependencies not available. Install xgboost and scikit-learn."
        )
        return
    try:
        import pandas as pd
        ...
        calibrated_model = calibrate_model(
            model, X_val, y_val, method=calibration_method
        )
        scores = score_all_transactions(calibrated_model, X_val)
        model_version = f"precision_v1_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        txn_ids = features.attrs.get(
            "txn_ids", pd.Series(features.index, index=features.index)
        )
