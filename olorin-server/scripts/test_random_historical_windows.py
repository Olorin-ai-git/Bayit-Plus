...
async def get_entity_transactions(
    provider, email: str, window_start: datetime, context_days: int = 60
):
...
async def test_window(window_start: datetime, window_end: datetime):
    ...
    print("=" * 80)
    print(
        f"🔍 TESTING WINDOW: {window_start.strftime('%Y-%m-%d')} ({window_start.strftime('%A')})"
    )
    print("=" * 80)
    print()
    ...
    for i, entity_info in enumerate(fraud_entities[:3], 1):
        ...
        print(
            f"  Total transactions: {len(transactions)} ({len(fraud_txs)} fraud, {len(legit_txs)} legitimate)"
        )
        ...
        print(
            f"  {result_status} Risk Score: {risk_assessment['overall_risk_score']:.3f} (threshold: {threshold:.3f})"
        )
        print(
            f"  {result_status} Recall: {recall:.1%} ({tp}/{len(fraud_txs)} fraud caught)"
        )
...
async def test_random_historical_windows(
    num_windows: int = 10, min_months_ago: int = 7
):
    ...
    if all_results:
        ...
        if total_fraud > 0:
            ...
            avg_precision_list = [
                r["precision"] for r in all_results if r["precision"] > 0
            ]
            avg_precision = (
                sum(avg_precision_list) / len(avg_precision_list)
                if avg_precision_list
                else 0
            )
            ...
            print(
                f"  High (≥80%):     {high_recall:3d} entities ({high_recall/len(all_results)*100:.1f}%)"
            )
            print(
                f"  Medium (50-80%): {medium_recall:3d} entities ({medium_recall/len(all_results)*100:.1f}%)"
            )
            print(
                f"  Low (<50%):      {low_recall:3d} entities ({low_recall/len(all_results)*100:.1f}%)"
            )
            print()
            ...
            if overall_recall < 1.0:
                ...
                print(
                    f"⚠️  Missing {missed_percentage:.1f}% of fraud ({total_fn} transactions)"
                )
...
async def main():
    ...
    parser.add_argument(
        "--windows", type=int, default=10, help="Number of random windows to test"
    )
    parser.add_argument(
        "--min-months", type=int, default=7, help="Minimum months back (default: 7)"
    )
