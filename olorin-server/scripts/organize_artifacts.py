...
def organize_artifacts():
    ...
    for file_path in list(artifacts_dir.glob("investigation_*.html")) + list(artifacts_dir.glob("investigation_*.json")):
        ...
        if info:
            ...
            if not target_path.exists():
                ...
                print(f"  ✅ Moved: {file_path.name} → investigations/{info['entity_type']}/{info['entity_id']}/")
                moved_count += 1
            else:
