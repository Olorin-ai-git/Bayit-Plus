...
class TestWebSocketEndpoints:
    ...
    async def test_websocket_connection_basic(self, auth_headers, real_test_data):
        ...
        try:
            ...
            async with websockets.connect(
                ...
                try:
                    ...
                    try:
                        ...
                        logger.info(
                            f"WebSocket response type: {response_data.get('type', 'unknown')}"
                        )
                        ...
                        if response_data.get("type") == "ack":
                            ...
                            logger.warning(
                                f"WebSocket error response: {response_data.get('message', 'Unknown error')}"
                            )
                        else:
                            logger.info(f"WebSocket response data: {response_data}")
                    except json.JSONDecodeError:
                        logger.info(
                            "WebSocket response is not JSON (may be plain text)"
                        )
                        if len(response) < 200:
                    ...
                    logger.info(
                        "No WebSocket response received (timeout) - connection may be listen-only"
                    )
            ...
            pytest.fail(
                "WebSocket connection refused - server may not support WebSockets"
            )
    ...
    @pytest.mark.asyncio
    async def test_websocket_investigation_updates(
        self, endpoint_client, auth_headers, real_test_data
    ):
            ...
            async with websockets.connect(ws_uri, timeout=10) as websocket:
                ...
                async def message_listener():
                    try:
                        while True:
                            ...
                            logger.info(
                                f"Received WebSocket message: {len(message)} chars"
                            )
                ...
                if update_response.status_code == 200:
                    ...
                    if received_messages:
                        logger.info(
                            f"Received {len(received_messages)} WebSocket messages"
                        )
                            ...
                            try:
                                ...
                                if "investigation_id" in msg_data:
                                    if msg_data["investigation_id"] == investigation_id:
                                        logger.info(
                                            f"  ✓ Message for correct investigation"
                                        )
                                    else:
                                        logger.warning(
                                            f"  ✗ Message for wrong investigation: {msg_data['investigation_id']}"
                                        )
                                ...
                                if "status" in msg_data:
                                    logger.info(
                                        f"  Investigation status: {msg_data['status']}"
                                    )
                        ...
                        logger.info(
                            "No WebSocket messages received (investigation updates may not trigger WebSocket messages)"
                        )
                else:
                    logger.warning(
                        "Investigation update failed - cannot test WebSocket messages"
                    )
    ...
    async def test_websocket_authentication_scenarios(self, real_test_data):
        ...
        for scenario in auth_scenarios:
            ...
            try:
                async with websockets.connect(
                    scenario["uri"], timeout=5, extra_headers={"Origin": base_url}
                ) as websocket:
                    # If connection succeeds, it's unexpected for auth failure scenarios
                    if scenario["expected_status"] == 401:
                        logger.warning(
                            f"  ✗ Expected auth failure but connection succeeded"
                        )
                    else:
                ...
                if e.status_code == scenario["expected_status"]:
                    ...
                    logger.warning(
                        f"  ✗ Expected {scenario['expected_status']}, got {e.status_code}"
                    )

            except ConnectionRefusedError:
                logger.info(
                    f"  Connection refused - WebSocket server may not be running"
                )
    ...
    async def test_websocket_connection_limits(self, auth_headers, real_test_data):
        ...
        if not auth_headers:
            pytest.skip(
                "No authentication headers available - skipping WebSocket limits test"
            )
        ...
        try:
            ...
            for i in range(max_connections):
                try:
                    websocket = await websockets.connect(
                        ws_uri, timeout=5, extra_headers={"Origin": base_url}
                    )
            ...
            if connections:
                logger.info(
                    f"Successfully established {len(connections)} concurrent WebSocket connections"
                )
                ...
                logger.info(
                    f"{alive_connections}/{len(connections)} connections are alive"
                )
...
async def test_websocket_endpoints_summary(auth_headers, real_test_data):
    ...
    try:
        ...
        if e.status_code == 401:
            ...
            results["Authentication Test"] = {"status": e.status_code, "success": False}
