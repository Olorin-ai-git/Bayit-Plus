...
class TestAnalysisEndpoints:
    ...
    @pytest.mark.asyncio
    async def test_device_analysis(
        ...
        response, metrics = await endpoint_client.get(
            f"/api/device/{entity_id}", headers=auth_headers, params=params
        )
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                if found_fields:
                    ...
                    if "device_signals" in data:
                        ...
                        if isinstance(signals, list) and signals:
                            ...
                            logger.info(
                                f"Sample signal fields: {list(sample_signal.keys())}"
                            )
                        else:
                    ...
                    if "risk_assessment" in data:
                        ...
                        if isinstance(risk_data, dict):
                            ...
                            logger.info(
                                f"Device risk level: {risk_level}, confidence: {confidence}"
                            )
                ...
                if result.response_time_ms > 30000:  # 30 seconds
                    logger.warning(
                        f"Device analysis is very slow: {result.response_time_ms:.1f}ms"
                    )
            ...
            logger.warning(
                "Device analysis service unavailable - external dependencies may be down"
            )

        else:
            pytest.fail(
                f"Unexpected status code for device analysis: {response.status_code}"
            )

        # Fail if validation errors
        if result.errors:
            pytest.fail(
                f"Device analysis validation failed: {'; '.join(result.errors)}"
            )
    ...
    async def test_network_analysis(
        ...
        params = {"investigation_id": investigation_id, "time_range": "30d"}

        response, metrics = await endpoint_client.get(
            f"/api/network/{entity_id}", headers=auth_headers, params=params
        )
        ...
        if response.status_code == 200:
            ...
            pytest.fail(
                f"Unexpected status code for network analysis: {response.status_code}"
            )

        # Fail if validation errors
        if result.errors:
            pytest.fail(
                f"Network analysis validation failed: {'; '.join(result.errors)}"
            )

    @pytest.mark.asyncio
    async def test_oii_location_source(
        self, endpoint_client, endpoint_validator, auth_headers, real_test_data
    ):
        """Test GET /api/location/source/oii/{user_id} - OII location."""
        logger.info(
            "Testing OII location source: GET /api/location/source/oii/{user_id}"
        )
        ...
        # Test OII location
        response, metrics = await endpoint_client.get(
            f"/api/location/source/oii/{user_id}", headers=auth_headers
        )
        ...
        if response.status_code == 200:
            ...
            logger.warning(
                "OII location service error (500) - external service may be down"
            )
    ...
    @pytest.mark.asyncio
    async def test_business_location_source(
        self, endpoint_client, endpoint_validator, auth_headers, real_test_data
    ):
        """Test GET /api/location/source/business/{user_id} - Business location."""
        logger.info(
            "Testing business location source: GET /api/location/source/business/{user_id}"
        )
        ...
        response, metrics = await endpoint_client.get(
            f"/api/location/source/business/{user_id}", headers=auth_headers
        )

        # Validate response
        result = endpoint_validator.validate_response(
            response, metrics, expected_status=[200, 404, 500], endpoint_type="analysis"
        )
            ...
            try:
                ...
                if isinstance(data, list):
                    ...
                    if data:
                        ...
                        logger.info(
                            f"Sample business location fields: {list(sample_location.keys())}"
                        )
                elif isinstance(data, dict):
                    logger.info(f"Business location fields: {list(data.keys())}")
                else:
                    logger.warning(
                        f"Unexpected business location data type: {type(data)}"
                    )
        ...
        if result.errors:
            pytest.fail(
                f"Business location validation failed: {'; '.join(result.errors)}"
            )

    @pytest.mark.asyncio
    async def test_phone_location_source(
        self, endpoint_client, endpoint_validator, auth_headers, real_test_data
    ):
        """Test GET /api/location/source/phone/{user_id} - Phone location."""
        logger.info(
            "Testing phone location source: GET /api/location/source/phone/{user_id}"
        )
        ...
        # Test phone location
        response, metrics = await endpoint_client.get(
            f"/api/location/source/phone/{user_id}", headers=auth_headers
        )
    ...
    @pytest.mark.asyncio
    async def test_location_risk_analysis(
        self, endpoint_client, endpoint_validator, auth_headers, real_test_data
    ):
        """Test GET /api/location/risk-analysis/{user_id} - Location risk analysis."""
        logger.info(
            "Testing location risk analysis: GET /api/location/risk-analysis/{user_id}"
        )
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                found_fields = [
                    field for field in location_risk_fields if field in data
                ]
                    ...
                    if "overall_location_risk_assessment" in data:
                        ...
                        if risk_assessment and isinstance(risk_assessment, dict):
                            ...
                            if isinstance(risk_data, dict):
                                ...
                                logger.info(
                                    f"Location risk: {risk_level}, confidence: {confidence}"
                                )
                                if summary:
                                    logger.info(
                                        f"Location risk summary: {summary[:100]}..."
                                    )
                ...
                if result.response_time_ms > 45000:  # 45 seconds
                    logger.warning(
                        f"Location risk analysis is very slow: {result.response_time_ms:.1f}ms"
                    )
            ...
            pytest.fail(
                f"Unexpected status code for location risk analysis: {response.status_code}"
            )

        # Fail if validation errors
        if result.errors:
            pytest.fail(
                f"Location risk analysis validation failed: {'; '.join(result.errors)}"
            )
    ...
    async def test_log_analysis(
        ...
        # Test log analysis
        params = {"investigation_id": investigation_id, "time_range": "7d"}

        response, metrics = await endpoint_client.get(
            f"/api/logs/{user_id}", headers=auth_headers, params=params
        )
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                log_fields = ["risk_assessment", "splunk_data", "warning"]
                ...
                if found_fields:
                    ...
                    if "risk_assessment" in data:
                        ...
                        if isinstance(risk_assessment, dict):
                            ...
                            logger.info(
                                f"Log risk level: {risk_level}, confidence: {confidence}"
                            )
                ...
                if result.response_time_ms > 60000:  # 60 seconds
                    logger.warning(
                        f"Log analysis is very slow: {result.response_time_ms:.1f}ms"
                    )
            ...
            pytest.fail(
                f"Unexpected status code for log analysis: {response.status_code}"
            )
    ...
    @pytest.mark.asyncio
    async def test_online_identity_info(
        self, endpoint_client, endpoint_validator, auth_headers, real_test_data
    ):
        ...
        # Test OII
        response, metrics = await endpoint_client.get(
            f"/api/oii/{user_id}", headers=auth_headers
        )
...
@pytest.mark.asyncio
async def test_analysis_endpoints_summary(
    endpoint_client, auth_headers, real_test_data
):
    ...
    analysis_tests = [
        (
            ...
            {"investigation_id": investigation_id, "entity_type": "user_id"},
        ),
        (
            "Network Analysis",
            "GET",
        ...
        (
            ...
            "GET",
            f"/api/logs/{user_id}",
            {"investigation_id": investigation_id, "time_range": "7d"},
        ),
    ...
    for test_name, method, endpoint, params in analysis_tests:
        try:
            ...
            if method == "GET":
                response, metrics = await endpoint_client.get(
                    endpoint, headers=auth_headers, params=params
                )
            ...
            if response.status_code == 200:
                ...
                404,
                503,
            ]:  # Data not found or service unavailable
            ...
            results[test_name] = {"status": "ERROR", "error": str(e), "success": False}
