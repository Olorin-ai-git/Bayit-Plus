...
class TestErrorHandling:
    ...
    async def test_authentication_failures(self, endpoint_client, endpoint_validator):
        ...
        protected_endpoints = [
            ...
            ("GET", "/api/device/test_user", {"investigation_id": "test_inv", "entity_type": "user_id"}),
        ...
        for scenario in auth_scenarios:
            ...
            successful_endpoints = sum(1 for r in scenario_results if r.get("success", False))
            ...
            logger.info(f"  {scenario['name']}: {successful_endpoints}/{total_endpoints} endpoints handled auth correctly")
        ...
        for scenario_name, results in auth_results.items():
            ...
            logger.info(f"{scenario_name}: {success_rate:.1f}% correct ({results['successful']}/{results['total']})")
        
        # Overall authentication error handling should be consistent
        overall_success_rate = sum(r["successful"] for r in auth_results.values()) / sum(r["total"] for r in auth_results.values())
        logger.info(f"Overall authentication error handling: {overall_success_rate*100:.1f}%")
        
        # Assert that authentication is properly enforced
        assert overall_success_rate > 0.7, "Authentication error handling should be consistent across endpoints"

    @pytest.mark.asyncio
    async def test_invalid_parameter_handling(self, endpoint_client, endpoint_validator, auth_headers):
        """Test handling of invalid parameters."""
        logger.info("Testing invalid parameter handling")
        
        if not auth_headers:
            pytest.skip("No authentication headers available - skipping parameter validation tests")
        ...
        parameter_scenarios = [
            {
                ...
                "params": {"investigation_id": "test_inv", "entity_type": "user_id", "time_range": "invalid_range"},
            ...
            },
            {
                ...
                "params": {"investigation_id": "test_inv", "time_range": "not_a_time_range"},
                "expected_status": [400, 422],
        ...
        for scenario in parameter_scenarios:
            ...
            try:
                ...
                if success:
                    ...
                    logger.warning(f"  ✗ {scenario['name']}: {response.status_code} (expected {expected_statuses})")
        ...
        logger.info(f"Parameter validation success rate: {validation_rate*100:.1f}% ({successful_validations}/{total_validations})")
        
        # Assert that parameter validation is working
        assert validation_rate > 0.6, "Parameter validation should catch most invalid inputs"

    @pytest.mark.asyncio
    async def test_resource_not_found_handling(self, endpoint_client, endpoint_validator, auth_headers):
        ...
        not_found_scenarios = [
            ...
            },
            {
                ...
                "expected_status": [404, 200],  # May return empty results instead of 404
        ...
        logger.info(f"404 handling success rate: {not_found_rate*100:.1f}% ({successful_404s}/{total_404_tests})")

    @pytest.mark.asyncio
    async def test_malformed_request_handling(self, endpoint_client, endpoint_validator, auth_headers):
        """Test handling of malformed requests."""
        logger.info("Testing malformed request handling")
        
        if not auth_headers:
            pytest.skip("No authentication headers available - skipping malformed request tests")
        ...
        malformed_rate = successful_malformed / total_malformed_tests if total_malformed_tests > 0 else 0
        logger.info(f"Malformed request handling: {malformed_rate*100:.1f}% ({successful_malformed}/{total_malformed_tests})")

    @pytest.mark.asyncio
    async def test_rate_limiting_behavior(self, endpoint_client, endpoint_validator, auth_headers):
        """Test rate limiting behavior (if implemented)."""
        logger.info("Testing rate limiting behavior")
        
        if not auth_headers:
            pytest.skip("No authentication headers available - skipping rate limiting tests")
        ...
        if rate_limited:
            ...
            rate_limit_results.append({"test": "Rate Limiting", "success": False, "note": "Not implemented"})
        ...
        if len(response_times) > 10:
            ...
            if late_avg > early_avg * 1.5:  # 50% slower
                logger.info("Response times increased under load (possible rate limiting/throttling)")
            else:
        ...
        logger.info(f"Average response time: {sum(response_times)/len(response_times):.1f}ms")

    @pytest.mark.asyncio
    async def test_timeout_handling(self, endpoint_client, endpoint_validator, auth_headers):
        ...
        try:
            ...
            try:
                ...
                logger.info(f"Request completed despite short timeout: {response.status_code}")
            ...
            for endpoint in slow_endpoints:
                try:
                    ...
                    response, metrics = await endpoint_client.get(endpoint, headers=auth_headers)
                    
                    if metrics["response_time_ms"] > 30000:  # 30 seconds
                        logger.info(f"Endpoint is slow but completed: {metrics['response_time_ms']:.1f}ms")
                    else:
                        logger.info(f"Endpoint completed normally: {metrics['response_time_ms']:.1f}ms")
                
                except Exception as e:
                    if "timeout" in str(e).lower():
                        logger.info(f"✓ Proper timeout handling for slow endpoint: {endpoint}")
                    else:
                        logger.warning(f"Error testing slow endpoint {endpoint}: {str(e)}")
    ...
    @pytest.mark.asyncio
    async def test_boundary_conditions(self, endpoint_client, endpoint_validator, auth_headers):
        ...
        for scenario in boundary_scenarios:
            ...
            try:
                ...
                logger.info(f"  ✓ {scenario['name']}: Error (acceptable for boundary condition)")
...
async def test_error_handling_comprehensive_summary(endpoint_client, auth_headers):
    ...
    critical_success_rate = sum(1 for e in critical_errors if e["success"]) / len(critical_errors) if critical_errors else 0
    logger.info(f"Critical error handling: {critical_success_rate*100:.1f}% ({sum(1 for e in critical_errors if e['success'])}/{len(critical_errors)})")
