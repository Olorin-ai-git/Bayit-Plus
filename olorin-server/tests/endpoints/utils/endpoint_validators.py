...
class EndpointValidator:
    ...
    def validate_response(
        ...
        threshold = self.performance_thresholds.get(endpoint_type, self.performance_thresholds["default"])
    ...
    def _validate_timestamp(self, value: Any, field_name: str, result: ValidationResult):
        """Validate timestamp format and reasonableness."""
        if not isinstance(value, str):
            result.add_error(f"Timestamp field {field_name} should be string, got {type(value)}")
            return
    ...
    def _validate_id_field(self, value: Any, field_name: str, result: ValidationResult):
        ...
        if not isinstance(value, str):
            result.add_error(f"ID field {field_name} should be string, got {type(value)}")
            return
        ...
        if len(value) > 255:
            result.add_warning(f"ID field {field_name} is very long: {len(value)} chars")
    
    def _validate_error_response(self, response: httpx.Response, result: ValidationResult):
    ...
    def _validate_jwt_token(self, data: Dict[str, Any], result: ValidationResult):
        ...
        parts = token.split('.')
        if len(parts) != 3:
            result.add_error(f"Invalid JWT token format: expected 3 parts, got {len(parts)}")
    ...
    def _validate_investigation_status(self, data: Dict[str, Any], result: ValidationResult):
        """Business validator for investigation status."""
        status = data.get("status")
        valid_statuses = ["IN_PROGRESS", "COMPLETED", "PENDING", "UNDER_REVIEW", "CLOSED"]
    ...
    def _validate_network_analysis(self, data: Dict[str, Any], result: ValidationResult):
    ...
    def _validate_location_analysis(self, data: Dict[str, Any], result: ValidationResult):
        ...
        if "location_sources" in data or "oii_location_info" in data:
            ...
            for field in ["oii_location_info", "business_location_info", "phone_location_info"]:
                if data.get(field) is not None:
                    has_location_data = True
                    break
            
            if not has_location_data:
                result.add_warning("No location data found in location analysis response")
    ...
    def _validate_agent_response_structure(self, data: Dict[str, Any], result: ValidationResult):
    ...
    def validate_websocket_compatible_response(
        ...
        return self.validate_response(
            response,
            metrics,
            expected_status=[200, 101, 426],  # 101 for upgrade, 426 for upgrade required
            endpoint_type="default"
        )