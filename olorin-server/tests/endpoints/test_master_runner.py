...
class MasterTestRunner:
    ...
    def record_phase_end(self, phase_name: str, results: Dict[str, Any]):
        ...
        logger.info(
            f"Success Rate: {phase_data.get('successful', 0)}/{phase_data.get('endpoints_tested', 0)}"
        )
        logger.info("=" * 80)
            ...
            self.overall_metrics.update(
                {
                    ...
                    "average_response_time": (
                        performance_metrics["total_response_time"] or 0
                    )
                    / max(1, performance_metrics["total_requests"] or 1),
    ...
    def generate_comprehensive_report(self) -> Dict[str, Any]:
        ...
        success_rate = (
            (total_successful / total_endpoints * 100) if total_endpoints > 0 else 0
        )
        ...
        report = {
            ...
            "performance_analysis": {
                "average_response_time_ms": self.overall_metrics[
                    "average_response_time"
                ],
                "slowest_endpoint": self.overall_metrics["slowest_endpoint"],
                "fastest_endpoint": self.overall_metrics["fastest_endpoint"],
                "requests_over_threshold": len(
                    [
                        r
                        for r in performance_metrics["requests"]
        ...
        for phase_name, phase_data in self.phase_results.items():
            report["phase_breakdown"][phase_name] = {
                ...
                "success_rate": (
                    (
                        phase_data.get("successful", 0)
                        / phase_data.get("endpoints_tested", 1)
                    )
                    * 100
                    if phase_data.get("endpoints_tested", 0) > 0
                    else 0
                ),
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if self.overall_metrics["average_response_time"] > 5000:
            recommendations.append(
                "Consider optimizing API performance - average response time exceeds 5 seconds"
            )

        # Success rate recommendations
        success_rate = (
            (
                self.overall_metrics["total_successful"]
                / self.overall_metrics["total_endpoints_tested"]
                * 100
            )
            if self.overall_metrics["total_endpoints_tested"] > 0
            else 0
        )
        if success_rate < 90:
            recommendations.append(
                "API reliability is below 90% - investigate failing endpoints"
            )
        ...
        if performance_metrics["requests"]:
            slow_requests = [
                r
                for r in performance_metrics["requests"]
                if r["response_time_ms"] > 30000
            ]
            if slow_requests:
                recommendations.append(
                    f"Found {len(slow_requests)} requests taking over 30 seconds - consider timeout optimization"
                )
        ...
        for phase_name, phase_data in self.phase_results.items():
            if phase_data.get("status") != "completed":
                recommendations.append(
                    f"Phase '{phase_name}' did not complete successfully - investigate failures"
                )
            elif phase_data.get("errors"):
                recommendations.append(
                    f"Phase '{phase_name}' has {len(phase_data['errors'])} errors - review error logs"
                )

        if not recommendations:
            recommendations.append(
                "All endpoint tests completed successfully - API is functioning well"
            )
    ...
    def print_executive_summary(self):
        ...
        logger.info(
            f"Test Execution Duration: {exec_summary['total_duration_seconds']:.1f} seconds"
        )
        logger.info(f"Total Endpoints Tested: {exec_summary['total_endpoints_tested']}")
        logger.info(
            f"Overall Success Rate: {exec_summary['success_rate_percentage']:.1f}%"
        )
        ...
        if perf["slowest_endpoint"]:
            ...
            logger.info(
                f"Slowest Endpoint: {slowest['method']} {slowest['endpoint']} ({slowest['response_time_ms']:.1f}ms)"
            )

        if perf["fastest_endpoint"]:
            fastest = perf["fastest_endpoint"]
            logger.info(
                f"Fastest Endpoint: {fastest['method']} {fastest['endpoint']} ({fastest['response_time_ms']:.1f}ms)"
            )
        ...
        for phase_name, phase_data in report["phase_breakdown"].items():
            ...
            logger.info(
                f"{status_icon} {phase_name}: {phase_data['success_rate']:.1f}% success ({phase_data['duration_seconds']:.1f}s)"
            )
...
async def test_comprehensive_endpoint_suite():
    ...
    try:
        ...
        logger.info(
            f"Performance Threshold: {ENDPOINT_TEST_CONFIG['performance_threshold_ms']}ms"
        )
...
def master_test_session_coordinator(request):
    ...
    def finalize_master_test_run():
        ...
        if performance_metrics["total_requests"] > 0:
            logger.info(f"\nFinal Statistics:")
            logger.info(
                f"Total API requests made: {performance_metrics['total_requests']}"
            )
            logger.info(
                f"Total response time: {performance_metrics['total_response_time']:.1f}ms"
            )
            logger.info(
                f"Average response time: {(performance_metrics['total_response_time'] or 0)/max(1, performance_metrics['total_requests'] or 1):.1f}ms"
            )
