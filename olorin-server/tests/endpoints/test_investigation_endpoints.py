...
class TestInvestigationEndpoints:
    ...
    async def test_create_investigation(
        self, endpoint_client, endpoint_validator, auth_headers, real_test_data
    ):
        ...
        response, metrics = await endpoint_client.post(
            "/api/investigation", headers=auth_headers, json_data=investigation_payload
        )
        ...
        if response.status_code == 201 or response.status_code == 200:
            ...
            try:
                ...
                for field in required_fields:
                    assert (
                        field in data
                    ), f"Missing required field in created investigation: {field}"

                # Verify data matches payload
                assert (
                    data["id"] == investigation_payload["id"]
                ), "Investigation ID mismatch"
                assert (
                    data["entity_id"] == investigation_payload["entity_id"]
                ), "Entity ID mismatch"
                assert (
                    data["entity_type"] == investigation_payload["entity_type"]
                ), "Entity type mismatch"
                assert (
                    data["status"] == investigation_payload["status"]
                ), "Status mismatch"
            ...
            pytest.fail(
                f"Unexpected status code for investigation creation: {response.status_code}"
            )

        # Fail if validation errors
        if result.errors:
            pytest.fail(
                f"Investigation creation validation failed: {'; '.join(result.errors)}"
            )
    ...
    async def test_get_investigation(
        ...
        create_response, _ = await endpoint_client.post(
            "/api/investigation", headers=auth_headers, json_data=create_payload
        )
        ...
        response, metrics = await endpoint_client.get(
            f"/api/investigation/{investigation_id}", headers=auth_headers
        )
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                ), "Retrieved investigation ID mismatch"
                assert (
                    data["entity_id"] == create_payload["entity_id"]
                ), "Entity ID mismatch"
                assert data["status"] == create_payload["status"], "Status mismatch"
                assert (
                    data["risk_score"] == create_payload["risk_score"]
                ), "Risk score mismatch"

                logger.info(f"Retrieved investigation: {investigation_id}")
                logger.info(
                    f"Investigation details: status={data['status']}, risk={data['risk_score']}"
                )
            ...
            pytest.fail(
                f"Investigation not found: {investigation_id} - creation may have failed"
            )

        else:
            pytest.fail(
                f"Unexpected status code for get investigation: {response.status_code}"
            )

        # Fail if validation errors
        if result.errors:
            pytest.fail(
                f"Get investigation validation failed: {'; '.join(result.errors)}"
            )
    ...
    async def test_update_investigation(
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                assert (
                    data["id"] == investigation_id
                ), "Updated investigation ID mismatch"
                assert data["status"] == update_payload["status"], "Status not updated"
                assert (
                    data["risk_score"] == update_payload["risk_score"]
                ), "Risk score not updated"
            ...
            pytest.fail(
                f"Unexpected status code for update investigation: {response.status_code}"
            )

        # Fail if validation errors
        if result.errors:
            pytest.fail(
                f"Update investigation validation failed: {'; '.join(result.errors)}"
            )
    ...
    async def test_list_investigations(
        ...
        response, metrics = await endpoint_client.get(
            "/api/investigations", headers=auth_headers
        )
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                assert isinstance(
                    investigations_list, list
                ), "Investigations should be returned as a list"

                logger.info(f"Found {len(investigations_list)} investigations")

                # Should include our created investigations
                returned_ids = [
                    inv.get("id") for inv in investigations_list if inv.get("id")
                ]
                found_created = [
                    inv_id for inv_id in created_ids if inv_id in returned_ids
                ]

                logger.info(
                    f"Found {len(found_created)} of our created investigations in list"
                )
                ...
                if investigations_list:
                    ...
                    found_fields = [
                        field for field in required_fields if field in sample_inv
                    ]
                    logger.info(
                        f"Sample investigation fields: {list(sample_inv.keys())}"
                    )
                ...
                if isinstance(data, dict):
                    ...
                    found_pagination = [
                        field for field in pagination_fields if field in data
                    ]
                    if found_pagination:
            ...
            pytest.fail(
                f"Unexpected status code for list investigations: {response.status_code}"
            )

        # Fail if validation errors
        if result.errors:
            pytest.fail(
                f"List investigations validation failed: {'; '.join(result.errors)}"
            )
    ...
    async def test_delete_investigation(
        ...
        if response.status_code in [200, 204]:
            ...
            if get_response.status_code == 404:
                ...
                logger.warning(
                    f"Investigation may still exist after delete: {get_response.status_code}"
                )

        elif response.status_code == 404:
            logger.warning(
                "Investigation not found for deletion - may not have been created"
            )

        else:
            pytest.fail(
                f"Unexpected status code for delete investigation: {response.status_code}"
            )
        ...
        if result.errors:
            pytest.fail(
                f"Delete investigation validation failed: {'; '.join(result.errors)}"
            )

    @pytest.mark.asyncio
    async def test_bulk_delete_investigations(
        self, endpoint_client, endpoint_validator, auth_headers, real_test_data
    ):
        ...
        response, metrics = await endpoint_client.delete(
            "/api/investigation", headers=auth_headers, json_data=bulk_delete_payload
        )
    ...
    @pytest.mark.asyncio
    async def test_delete_all_investigations_admin(
        self, endpoint_client, endpoint_validator, auth_headers
    ):
        """Test DELETE /api/investigations/delete_all - Delete all (admin)."""
        logger.info(
            "Testing delete all investigations: DELETE /api/investigations/delete_all"
        )
        ...
        response, metrics = await endpoint_client.delete(
            "/api/investigations/delete_all", headers=auth_headers
        )
        ...
        if response.status_code in [200, 204]:
            ...
            logger.info(
                "Delete all forbidden (403) - insufficient permissions (expected for non-admin)"
            )
    ...
    def cleanup_investigations(self, request, endpoint_client, auth_headers):
        ...
        if (
            hasattr(self, "created_investigations")
            and self.created_investigations
            and auth_headers
        ):
            logger.info(
                f"Cleaning up {len(self.created_investigations)} test investigations"
            )

            async def cleanup():
                for inv_id in self.created_investigations[
                    :
                ]:  # Copy to avoid modification during iteration
                    try:
                        delete_response, _ = await endpoint_client.delete(
                            f"/api/investigation/{inv_id}", headers=auth_headers
                        )
                        ...
                        if delete_response.status_code in [200, 204, 404]:
                            ...
                            logger.warning(
                                f"Failed to cleanup investigation {inv_id}: {delete_response.status_code}"
                            )
...
async def test_investigation_endpoints_summary(endpoint_client, auth_headers):
    ...
    if not auth_headers:
        logger.warning(
            "No authentication headers - cannot test investigation endpoints"
        )
        return
    ...
    try:
        response, _ = await endpoint_client.post(
            "/api/investigation", headers=auth_headers, json_data=test_payload
        )
        results["Create"] = {
            "status": response.status_code,
        ...
        if results["Create"]["success"]:
            ...
            try:
                ...
                response, _ = await endpoint_client.put(
                    f"/api/investigation/{investigation_id}",
                    headers=auth_headers,
                    json_data=update_payload,
                )
            ...
            # Test Delete
            try:
                response, _ = await endpoint_client.delete(
                    f"/api/investigation/{investigation_id}", headers=auth_headers
                )
                results["Delete"] = {
                    "status": response.status_code,
    ...
    try:
        ...
        results["Delete All"] = {
            "status": response.status_code,
            "success": response.status_code in [200, 204, 403, 404, 501],
        }
    except Exception as e:
    ...
    for operation, result in results.items():
        if result["success"]:
            ...
            logger.warning(
                f"✗ {operation}: {result.get('status', 'ERROR')} - {result.get('error', 'Failed')}"
            )
    ...
    assert results.get("List", {}).get(
        "success", False
    ), "List investigations endpoint should be accessible"
