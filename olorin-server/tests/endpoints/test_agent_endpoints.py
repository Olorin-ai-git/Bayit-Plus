...
class TestAgentEndpoints:
    ...
    @pytest.mark.asyncio
    async def test_agent_invoke_general(
        ...
        response, metrics = await endpoint_client.post(
            "/v1/agent/invoke", headers=agent_headers, json_data=agent_payload
        )
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                found_fields = [
                    field for field in agent_response_fields if field in data
                ]
                ...
                if found_fields:
                    ...
                    if "result" in data:
                        ...
                        if isinstance(result_data, str):
                            logger.info(
                                f"Agent result length: {len(result_data)} chars"
                            )
                            # Log first part of result (avoid logging sensitive data)
                            if len(result_data) > 0:
                                logger.info(
                                    f"Agent result preview: {result_data[:100]}..."
                                )
                        elif isinstance(result_data, dict):
                            logger.info(
                                f"Agent result fields: {list(result_data.keys())}"
                            )
                    ...
                    if "risk_assessment" in data:
                        ...
                        if isinstance(risk_data, dict):
                            ...
                            logger.info(
                                f"Agent risk assessment: level={risk_level}, confidence={confidence}"
                            )
                else:
                ...
                if result.response_time_ms > 120000:  # 2 minutes
                    logger.warning(
                        f"Agent invocation is very slow: {result.response_time_ms:.1f}ms"
                    )
                elif result.response_time_ms > 30000:  # 30 seconds
                    logger.info(
                        f"Agent invocation took: {result.response_time_ms:.1f}ms (normal for AI processing)"
                    )
            ...
            pytest.fail(
                f"Unexpected status code for agent invocation: {response.status_code}"
            )

        # Fail if validation errors
        if result.errors:
            pytest.fail(
                f"Agent invocation validation failed: {'; '.join(result.errors)}"
            )

    @pytest.mark.asyncio
    async def test_agent_start_structured_investigation(
        self, endpoint_client, endpoint_validator, auth_headers, real_test_data
    ):
        """Test POST /v1/agent/start/{entity_id} - Start structured investigation."""
        logger.info(
            "Testing structured investigation: POST /v1/agent/start/{entity_id}"
        )
        ...
        if response.status_code == 200:
            ...
            logger.warning(
                "Investigation conflict (409) - investigation may already be running"
            )
            try:
                error_data = response.json()
            ...
            logger.warning(
                "Agent service unavailable (503) - structured system may be down"
            )

        else:
            pytest.fail(
                f"Unexpected status code for structured investigation: {response.status_code}"
            )

        # Fail if validation errors (except for conflicts which can be expected)
        if result.errors and response.status_code != 409:
            pytest.fail(
                f"Structured investigation validation failed: {'; '.join(result.errors)}"
            )

    @pytest.mark.asyncio
    async def test_agent_capabilities_exploration(
        self, endpoint_client, endpoint_validator, auth_headers, real_test_data
    ):
        ...
        if not auth_headers:
            pytest.skip(
                "No authentication headers available - skipping agent capabilities test"
            )
        ...
        for config in agent_configurations:
            ...
            try:
                ...
                if success:
                    logger.info(
                        f"✓ {test_name}: {response.status_code} ({metrics['response_time_ms']:.1f}ms)"
                    )
        ...
        successful_agents = sum(
            1 for r in capabilities_results.values() if r["success"]
        )
        total_agents = len(capabilities_results)

        logger.info(
            f"Agent capabilities: {successful_agents}/{total_agents} successful"
        )
        ...
        if capabilities_results:
            ...
            times = [
                r.get("time_ms", 0)
                for r in capabilities_results.values()
                if "time_ms" in r
            ]
            if times:
    ...
    async def test_agent_error_handling(
        ...
        if not auth_headers:
            pytest.skip(
                "No authentication headers available - skipping agent error handling test"
            )
        ...
        error_test_cases = [
            {
                ...
                "payload": {"agent": {}, "input": "Test input"},  # Missing name
                "expected_status": 422,
            },
            {
                "name": "Missing Input",
