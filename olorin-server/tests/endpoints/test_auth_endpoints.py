...
from .utils.auth_helper import authenticate_with_credentials, AuthenticationError
...
class TestAuthEndpoints:
    ...
    @pytest.mark.asyncio
    async def test_oauth2_form_login(self, endpoint_client, endpoint_validator, firebase_secrets_setup):
        ...
        if response.status_code == 200:
            ...
            logger.warning("OAuth2 form login failed with validation error - may be expected")
            try:
                data = response.json()
            ...
            logger.warning("OAuth2 form login failed with auth error - credentials may not exist")
        
        # Fail if validation errors
        if result.errors:
            pytest.fail(f"OAuth2 form login validation failed: {'; '.join(result.errors)}")

    @pytest.mark.asyncio 
    async def test_json_login(self, endpoint_client, endpoint_validator, firebase_secrets_setup):
        ...
        result = endpoint_validator.validate_response(
            ...
            expected_status=[200, 422, 401],  # May fail if credentials don't exist
            endpoint_type="auth",
            business_validators=[self._validate_json_login_response] if response.status_code == 200 else None
        )
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                # Should have token fields
                assert "access_token" in data, "Missing access_token in JSON login response"
                ...
                if len(token_parts) == 3:
                    ...
                    logger.warning(f"Token doesn't appear to be JWT: {len(token_parts)} parts")
            ...
            logger.warning("JSON login failed with auth error - test credentials may not exist")
    ...
    @pytest.mark.asyncio
    async def test_get_current_user(self, endpoint_client, endpoint_validator, auth_headers):
        ...
        result = endpoint_validator.validate_response(
            ...
            endpoint_type="auth",
            business_validators=[self._validate_user_info_response] if response.status_code == 200 else None
        )
    ...
    async def test_logout(self, endpoint_client, endpoint_validator, auth_headers):
        ...
        result = endpoint_validator.validate_response(
            response,
            metrics,
            expected_status=[200, 204, 401, 404],  # 204 for no content, 404 if not implemented
            endpoint_type="auth"
        )
        ...
        if response.status_code in [200, 204]:
            ...
            try:
                ...
                if me_response.status_code == 401:
                    ...
                    logger.warning(f"Token still valid after logout: {me_response.status_code}")
    ...
    @pytest.mark.asyncio
    async def test_authentication_flow_end_to_end(self, endpoint_client, endpoint_validator, firebase_secrets_setup):
        ...
        for username, password in test_credentials:
            ...
            try:
                ...
                me_response, me_metrics = await endpoint_client.get("/auth/me", headers=headers)
                ...
                if me_response.status_code == 200:
                    ...
                    logger.warning(f"Protected endpoint failed: {me_response.status_code}")
                
                # Step 3: Test logout (if implemented)
                logout_response, logout_metrics = await endpoint_client.post("/auth/logout", headers=headers)
        ...
        if successful_auth:
            ...
            pytest.skip("No valid authentication credentials available for end-to-end test")
...
async def test_auth_endpoints_summary(endpoint_client, firebase_secrets_setup):
        ...
        if response.status_code == 200:
            try:
                ...
                if auth_token:
                    ...
                    # Test /auth/me
                    me_response, _ = await endpoint_client.get("/auth/me", headers=headers)
                    ...
                    # Test logout
                    logout_response, _ = await endpoint_client.post("/auth/logout", headers=headers)
                    results["logout"]["tested"] = True
                    results["logout"]["status"] = logout_response.status_code
                    results["logout"]["success"] = logout_response.status_code in [200, 204, 404]
            except Exception as e:
    ...
    for endpoint, result in results.items():
        if result["tested"]:
            ...
            logger.warning(f"? {endpoint}: Not tested - {result.get('error', 'Unknown error')}")
