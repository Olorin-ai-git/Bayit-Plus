...
def test_enhanced_console_logging():
    ...
    print(
        f"⚠️  Expected Risk Indicators: inconsistent_fingerprint, rapid_changes, bot_patterns"
    )
    ...
    for i, score in enumerate(risk_scores, 1):
        ...
        print(
            f"      ✅ Extracted risk score: {score} (path: risk_assessment.risk_level)"
        )
    ...
    print(
        f"      🐌 Slowest: Logs Agent ({slowest_duration}ms, {slowdown_pct:.0f}% slower)"
    )
