...
class TestRunner:
    ...
    def __init__(self, args):
        ...
        if not api_key:
            logger.error(
                f"Anthropic API key not configured in Firebase Secrets Manager!"
            )
            logger.error(
                f"Please configure the secret '{self.settings.anthropic_api_key_secret}' in Firebase"
            )
            sys.exit(1)
    ...
    def generate_report(self):
        ...
        report = {
            ...
            "api_key_configured": bool(
                get_firebase_secret(self.settings.anthropic_api_key_secret)
            ),
