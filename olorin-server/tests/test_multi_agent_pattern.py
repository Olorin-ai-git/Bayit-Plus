...
from app.service.agent.patterns.openai.multi_agent_pattern import (
...
class TestOpenAIMultiAgentPattern:
    ...
    def openai_config(self):
        ...
        return OpenAIPatternConfig(model="gpt-4o", temperature=0.1, stream=False)
    ...
    def multi_agent_pattern(self, pattern_config, openai_config, mock_tools):
        ...
        return OpenAIMultiAgentPattern(
            config=pattern_config, openai_config=openai_config, tools=mock_tools
        )
    ...
    def test_initialization(self, multi_agent_pattern):
        ...
        assert (
            len(multi_agent_pattern._function_definitions) == 0
        )  # Mock tools don't convert
    ...
    async def test_result_synthesis(self, multi_agent_pattern):
        ...
        synthesis = await multi_agent_pattern._synthesizer.synthesize_results(
            agent_results
        )
    ...
    async def test_handoff_context_creation(self, multi_agent_pattern):
        ...
        handoff_context = (
            await multi_agent_pattern._handoff_manager.create_handoff_context(
...
if __name__ == "__main__":
    ...
    async def run_basic_test():
        ...
        context = {
            "investigation_type": "ato",
            "data_types": ["ip", "device_fingerprint"],
        }
