...
class TestRiskScoreAggregation:
    ...
    def test_basic_risk_score_extraction(self):
        ...
        assert (
            abs(overall_risk_score - expected_average) < 0.001
        ), f"Expected {expected_average}, got {overall_risk_score}"
    ...
    def test_uniform_risk_scores(self):
        ...
        ), f"Expected 0.5, got {overall_risk_score} - original bug not fixed!"
        assert len(risk_scores) == 4, f"Expected 4 risk scores, got {len(risk_scores)}"
    ...
    def test_zero_risk_scores_handling(self):
        ...
        ), f"Expected 0 for all-zero scores, got {overall_risk_score}"
        assert (
            len(risk_scores) == 0
        ), f"Expected no non-zero scores, got {len(risk_scores)}"
    ...
    def test_mixed_zero_nonzero_scores(self):
        ...
        agent_results = {
            ...
            "network_analysis": self.create_mock_agent_result(
                "network", 0.8
            ),  # Non-zero
        ...
        assert (
            len(risk_scores) == 2
        ), f"Expected 2 non-zero scores, got {len(risk_scores)}"
    ...
    def test_malformed_agent_results(self):
        ...
        malformed_results = {
            ...
            "missing_risk_fields": {
                "messages": [Mock(content='{"other_field": "value"}')]
            },
        ...
        for result_name, malformed_result in malformed_results.items():
            ...
            assert (
                risk_score == 0
            ), f"Malformed result '{result_name}' should return 0, got {risk_score}"
    ...
    def test_location_agent_format(self):
        ...
        assert (
            risk_score == 0.3
        ), f"Location agent risk score not extracted correctly, got {risk_score}"
    ...
    def test_edge_case_risk_values(self):
        ...
        for test_value in test_values:
            ...
            assert (
                extracted_score == test_value
            ), f"Risk value {test_value} not preserved, got {extracted_score}"
