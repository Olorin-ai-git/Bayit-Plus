...
def test_investigation_state_management_complete_lifecycle():
    ...
    assert created_state["investigation_id"] == investigation_id or created_state.get("id") == investigation_id, f"ID mismatch: {created_state}"
    ...
    print(f"✓ Snapshot retrieved (version: {snapshot['version']}, etag: {snapshot.get('etag', 'N/A')})")
    ...
    for update in progress_updates:
        ...
        if update_response.status_code == 409:
            print(f"  ⚠ Conflict detected at {update['progress_percentage']}%, refetching...")
    ...
    if events_response.status_code == 200:
        ...
        print(f"✓ Event feed working: {len(events_data.get('items', []))} events, cursor: {events_data.get('next_cursor', 'N/A')}")
    else:
        print(f"ℹ Event feed not available (expected if not implemented): {events_response.status_code}")
    ...
    assert final_state["progress"]["progress_percentage"] == 100, f"Expected 100%, got {final_state['progress']['progress_percentage']}%"
