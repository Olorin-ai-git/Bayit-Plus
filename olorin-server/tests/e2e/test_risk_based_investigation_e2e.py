...
def test_risk_based_investigation_with_auto_select():
    ...
    for entity in entities:
        ...
        assert (
            entity_value != "auto-select"
        ), f"Placeholder was not replaced! Got: {entity_value}"
        assert (
            "@" in entity_value or len(entity_value) > 0
        ), f"Invalid entity value: {entity_value}"
        print(
            f"   ✓ Entity: {entity_value[:50]}... (type: {entity.get('entity_type')})"
        )

    print(
        f"✓ Auto-select successfully populated {len(entities)} top-risk entities from PostgreSQL"
    )
    ...
    assert len(persisted_state.get("settings", {}).get("entities", [])) == len(
        final_entities
    )
    ...
    print(
        f"  • Auto-Select Entities:      {len(final_entities)} from PostgreSQL top 10%"
    )
