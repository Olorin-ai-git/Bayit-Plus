...
class TestStructuredLLM:
    ...
    @pytest.mark.asyncio
    async def test_real_llm_initialization(
        self, real_anthropic_client, api_cost_monitor
    ):
        ...
        messages = [
            ...
            HumanMessage(
                content="Analyze this pattern: User logged in from 5 different countries in 1 hour."
            ),
        ]
        ...
        assert (
            "fraud" in response.content.lower()
            or "suspicious" in response.content.lower()
        )
    ...
    async def test_llm_with_tools_binding(self, api_cost_monitor):
        ...
        messages = [
            ...
            HumanMessage(
                content="Check if this IP address 198.51.100.42 is suspicious."
            ),
        ]
...
class TestDomainAgents:
    ...
    async def test_network_agent_real_investigation(
        ...
        config = RunnableConfig(
            tags=["test", "network_agent"], metadata={"test_type": "real_api"}
        )
        ...
        assert any(
            ...
            for term in ["ip", "network", "connection", "traffic"]
        )
    ...
    async def test_device_agent_real_investigation(
        ...
        config = RunnableConfig(
            tags=["test", "device_agent"], metadata={"test_type": "real_api"}
        )
        ...
        findings = await structured_device_agent(real_investigation_context, config)
        ...
        assert any(
            term in all_text.lower()
            for term in ["device", "fingerprint", "browser", "screen"]
        )
    ...
    async def test_location_agent_real_investigation(
        ...
        config = RunnableConfig(
            tags=["test", "location_agent"], metadata={"test_type": "real_api"}
        )
    ...
    async def test_logs_agent_real_investigation(
        ...
        config = RunnableConfig(
            tags=["test", "logs_agent"], metadata={"test_type": "real_api"}
        )
        ...
        logger.info(
            f"Logs agent findings: {len(findings.key_findings)} patterns detected"
        )
    ...
    async def test_risk_agent_aggregation(
        ...
        config = RunnableConfig(
            tags=["test", "risk_agent"], metadata={"test_type": "real_api"}
        )
        ...
        findings = await structured_risk_agent(real_investigation_context, config)
...
class TestAgentFactory:
    ...
    def test_configure_domain_tools(self):
        ...
        assert any(
            "device" in name.lower() or "fingerprint" in name.lower()
    ...
    def test_get_default_domain_objectives(self):
        ...
        assert any(
            "travel" in obj.lower() or "location" in obj.lower() for obj in location_obj
        )
...
class TestRealScenarioInvestigations:
    ...
    async def test_account_takeover_scenario(
        ...
        all_findings = " ".join(
            network_findings.key_findings + network_findings.suspicious_indicators
        )
        ato_detected = any(
        ...
        logger.info(
            f"ATO scenario detected with risk: {network_findings.risk_score:.2f}"
        )
    ...
    async def test_payment_fraud_scenario(
        ...
        logger.info(
            f"Payment fraud indicators: {device_findings.suspicious_indicators[:3]}"
        )
    ...
    async def test_multi_domain_investigation(
        ...
        config = RunnableConfig(
            tags=["test", "multi_domain"], metadata={"test_type": "comprehensive"}
        )
        ...
        for domain, findings in domain_results.items():
            logger.info(
                f"  {domain}: risk={findings.risk_score:.2f}, confidence={findings.confidence:.2f}"
            )
    ...
    async def test_result_variability(
        ...
        config = RunnableConfig(
            tags=["test", "variability"], metadata={"test_type": "variability_check"}
        )
        ...
        real_investigation_context.request_metadata["timestamp"] = (
        ...
        assert (
            abs(first_result.risk_score - second_result.risk_score) < 0.3
        )  # Similar range
        ...
        assert (
            first_findings_str != second_findings_str
            or first_result.confidence != second_result.confidence
        )
...
class TestPerformanceAndCost:
    ...
    async def test_api_response_time(
        ...
        config = RunnableConfig(
            tags=["test", "performance"], metadata={"test_type": "response_time"}
        )
    ...
    @pytest.mark.asyncio
    async def test_cost_tracking(self, real_investigation_context, api_cost_monitor):
        """Test cost tracking for API calls."""
        config = RunnableConfig(
            tags=["test", "cost_tracking"], metadata={"test_type": "cost_analysis"}
        )
    ...
    async def test_parallel_investigations(
        ...
        config = RunnableConfig(
            tags=["test", "parallel"], metadata={"test_type": "parallel_execution"}
        )
        ...
        tasks = [structured_network_agent(ctx, config) for ctx in contexts]
...
class TestIntegrationWorkflows:
    ...
    async def test_complete_investigation_workflow(
        ...
        final_assessment = await structured_risk_agent(
            real_investigation_context, config
        )
        api_cost_monitor.track_call(2000, 1800)
