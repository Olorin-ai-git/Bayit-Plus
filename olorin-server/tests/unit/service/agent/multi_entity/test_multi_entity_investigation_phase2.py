...
class TestMultiEntityInvestigationModels:
    ...
    def test_investigation_result_model(self):
        ...
        result = InvestigationResult(
            ...
            tool_results=[
                {"tool": "device_analyzer", "execution_time_ms": 1250, "result": "completed"}
            ],
            risk_score=0.6,
...
class TestMultiEntityInvestigationOrchestrator:
    ...
    @pytest.mark.asyncio
    async def test_investigation_context_initialization(self, orchestrator, sample_request):
    ...
    async def test_investigation_context_validation(self, orchestrator):
        ...
        with pytest.raises(ValueError, match="Relationship target entity not found"):
            await orchestrator._initialize_investigation_context(invalid_relationship_request)
    ...
    async def test_execute_cross_entity_analysis(self, orchestrator, sample_request):
        ...
        analysis = await orchestrator._execute_cross_entity_analysis(context, sample_request)
    ...
    @pytest.mark.asyncio
    async def test_execute_cross_entity_analysis_disabled(self, orchestrator, sample_request):
    ...
    async def test_execute_relationship_analysis(self, orchestrator, sample_request):
        ...
        insights = await orchestrator._execute_relationship_analysis(context, sample_request)
        
        assert isinstance(insights, list)
        assert len(insights) == len(context.relationships)  # One insight per relationship
    ...
    async def test_execute_risk_assessment(self, orchestrator, sample_request):
        ...
        assessment = await orchestrator._execute_risk_assessment(context, sample_request, boolean_result)
        ...
        assert assessment.individual_entity_scores["user_123"] == 0.6  # Average of 0.7 and 0.5
    ...
    def test_get_investigation_status(self, orchestrator, sample_request):
        ...
        context = InvestigationContext(
            ...
            entity_types={"user_123": EntityType.USER, "txn_456": EntityType.TRANSACTION},
            relationships=[],
            boolean_logic="user_123 AND txn_456",
            investigation_scope=["device", "location"],
            agent_results={"user_123": [InvestigationResult(
                investigation_id=sample_request.investigation_id,
                entity_id="user_123",
                agent_type="device_agent"
            )]},
...
class TestBooleanQueryParser:
    ...
    def test_boolean_query_parser_parse_placeholder(self):
        ...
        parser = BooleanQueryParser(
            expression="A AND B",
            entity_mapping={"A": "entity_1", "B": "entity_2"}
        )
...
class TestMultiEntityInvestigationIntegration:
    ...
    def test_performance_metrics_calculation(self):
        ...
        orchestrator._update_average_execution_time(5000)  # New investigation took 5 seconds
    ...
    def test_timeline_event_tracking(self):
        ...
        # Add timeline events
        context.add_timeline_event("test_event", "Test event description", {"key": "value"})
