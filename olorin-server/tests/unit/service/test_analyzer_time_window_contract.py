...
def test_analyzer_generates_correct_time_window_sql(monkeypatch):
    ...
    assert f'DATEADD(day, -{max_lookback_days}, CURRENT_TIMESTAMP())' in query, \
        "Query should cap end date at 6 months ago"
    
    # Verify window duration is subtracted from end date
    assert 'DATEADD(day, -1, DATEADD(hour, -0,' in query or 'DATEADD(hour, -24,' in query, \
...
def test_analyzer_fraud_inclusion_when_disabled(monkeypatch):
    ...
    assert 'IS_FRAUD_TX' in query, "Query should still reference IS_FRAUD_TX for counting"
...
def test_analyzer_approved_filter_included(monkeypatch):
    ...
    assert 'NSURE_LAST_DECISION' in query or 'UPPER(' in query, \
...
def test_analyzer_preserves_top_10_percent_logic(monkeypatch):
    ...
    assert 'CEIL' in query or 'risk_rank <=' in query, \
...
def test_analyzer_time_window_calculation_accuracy(monkeypatch):
    ...
    for hours in [24, 48, 168, 720]:  # 1 day, 2 days, 1 week, 30 days
        query = analyzer._build_risk_query(hours=hours, group_by='EMAIL', top_percentage=10.0)
        ...
        if remaining_hours > 0:
            assert f'DATEADD(day, -{days}, DATEADD(hour, -{remaining_hours},' in query or \
                   f'DATEADD(hour, -{hours},' in query, \
                f"Query should properly calculate {hours} hours window"
        else:
            assert f'DATEADD(day, -{days},' in query or f'DATEADD(hour, -{hours},' in query, \
                f"Query should properly calculate {hours} hours window"
