...
class TestCostOptimizationFramework:
    ...
    @pytest.mark.asyncio
    async def test_device_spoofing_investigation_optimization(self, cost_optimizer, api_cost_monitor):
    ...
    async def test_token_compression_optimization(self, cost_optimizer):
        ...
        # Test compression
        compressed_prompt, compression_ratio = await cost_optimizer._compress_prompt(verbose_prompt)
    ...
    async def test_usage_tracking_across_periods(self, cost_optimizer):
        ...
        assert all(usage == total_expected for usage in cost_optimizer.usage_by_period.values())

    @pytest.mark.asyncio
    async def test_synthetic_identity_investigation_optimization(self, cost_optimizer, api_cost_monitor):
    ...
    async def test_cache_management_and_cleanup(self, cost_optimizer):
        ...
        test_entries = [
            ("prompt1", "task1", {"result": "data1"}, 10.0, "claude-3-haiku-20240307", 3600),
            ("prompt2", "task2", {"result": "data2"}, 15.0, "claude-3-sonnet-20240229", 1800),  
            ("prompt3", "task3", {"result": "data3"}, 20.0, "claude-opus-4-1-20250805", 1),  # Short TTL
        ]
    ...
    async def test_optimization_statistics_accuracy(self, cost_optimizer):
        ...
        test_scenarios = [
            ("data_extraction", "Extract key data points from transaction log", "claude-3-haiku-20240307"),
            ("risk_assessment", "Assess overall fraud risk for this investigation", "claude-opus-4-1-20250805"),
            ("pattern_recognition", "Identify suspicious patterns in user behavior", "claude-3-sonnet-20240229"),
        ]
        ...
        assert updated_stats['optimization']['total_requests'] == initial_requests + len(test_scenarios)
    ...
    async def test_strategy_enable_disable(self, cost_optimizer):
        ...
        assert OptimizationStrategy.TOKEN_COMPRESSION not in cost_optimizer.active_strategies
        
        # Re-enable token compression
        cost_optimizer.enable_strategy(OptimizationStrategy.TOKEN_COMPRESSION)
        assert OptimizationStrategy.TOKEN_COMPRESSION in cost_optimizer.active_strategies
        
        # Disable caching
        cost_optimizer.disable_strategy(OptimizationStrategy.REQUEST_DEDUPLICATION)
        assert OptimizationStrategy.REQUEST_DEDUPLICATION not in cost_optimizer.active_strategies
    ...
    async def test_budget_configuration_updates(self, cost_optimizer):
        ...
        # Update budgets
        cost_optimizer.configure_budgets(
            daily=750.0,
            weekly=3000.0,
            monthly=10000.0
        )
    ...
    @pytest.mark.asyncio
    async def test_money_laundering_investigation_workflow(self, cost_optimizer, api_cost_monitor):
        ...
        for phase in investigation_phases:
            ...
            api_cost_monitor.track_call(
                1500, 1500, model_selection.selected_model
            )
...
class TestOptimizationStrategies:
    ...
    async def test_model_downgrade_strategy(self, cost_optimizer):
        ...
        if OptimizationStrategy.MODEL_DOWNGRADE in optimization_result.strategies_applied:
    ...
    async def test_period_reset_timing(self, cost_optimizer):
        ...
        now = datetime.now()
        expected_hourly = now.replace(minute=0, second=0, microsecond=0) + timedelta(hours=1)
        ...
        expected_daily = now.replace(hour=0, minute=0, second=0, microsecond=0) + timedelta(days=1)
    ...
    def test_time_until_reset_calculation(self, cost_optimizer):
        ...
        assert timedelta(hours=1, minutes=55) <= time_remaining <= timedelta(hours=2, minutes=5)
    ...
    async def test_compression_effectiveness_scenarios(self, cost_optimizer):
        ...
        for scenario in test_scenarios:
            compressed_text, ratio = await cost_optimizer._compress_prompt(scenario["text"])
...
class TestBudgetManagement:
    ...
    async def test_budget_threshold_detection(self, cost_optimizer):
        ...
        alerts = await cost_optimizer._check_budget_constraints(10.0)  # Small additional cost
