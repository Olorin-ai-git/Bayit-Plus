...
def test_fraud_column_pattern_exclusion():
    ...
    assert 'FIRST_FRAUD_STATUS_DATETIME' not in query, "FIRST_FRAUD_STATUS_DATETIME should be excluded"
    ...
    assert 'LAST_FRAUD_ALERT_DATETIME' not in query, "LAST_FRAUD_ALERT_DATETIME should be excluded"
    assert 'IS_LAST_DISPUTE_FRAUD_RELATED_REASON' not in query, "IS_LAST_DISPUTE_FRAUD_RELATED_REASON should be excluded"
...
def test_model_score_exclusion():
    ...
    assert 'MODEL_SCORE' not in query, "MODEL_SCORE should be excluded from investigation"
...
def test_non_fraud_columns_included():
    ...
    assert 'PAID_AMOUNT_VALUE_IN_CURRENCY' in query, "PAID_AMOUNT_VALUE_IN_CURRENCY should be included"
...
def test_maxmind_fraud_columns_excluded():
    ...
    assert 'LAST_MAXMIND_MIN_FRAUD_ALERT_DATETIME' not in query, \
        "LAST_MAXMIND_MIN_FRAUD_ALERT_DATETIME should be excluded"
    assert 'MAXMIND_MIN_FRAUD_ALERTS' not in query, \
        "MAXMIND_MIN_FRAUD_ALERTS should be excluded"
    assert 'COUNT_MAXMIND_MIN_FRAUD_ALERTS' not in query, \
...
def test_processor_fraud_columns_excluded():
    ...
    assert 'IS_PROCESSOR_REJECTED_DUE_TO_FRAUD' not in query, \
...
def test_different_investigation_focus():
    ...
    for focus in focus_types:
        ...
        assert 'IS_FRAUD_TX' not in query, \
