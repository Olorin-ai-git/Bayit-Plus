...
class TestMultiEntityInvestigationAPIEndpoints:
    ...
    def sample_multi_entity_request(self):
        ...
        return {
            ...
            "metadata": {"test_case": "api_integration_test", "created_by": "pytest"},
        }

    def test_start_multi_entity_investigation_endpoint(
        ...
        for i, entity in enumerate(data["entities"]):
            assert (
                entity["entity_id"]
                == sample_multi_entity_request["entities"][i]["entity_id"]
            )
            assert (
                entity["entity_type"]
                == sample_multi_entity_request["entities"][i]["entity_type"]
            )
    ...
    def test_start_multi_entity_investigation_validation_errors(self, client):
        ...
        response = client.post(
            "/v1/structured/multi-entity/start", json=invalid_request_1
        )
        ...
        response = client.post(
            "/v1/structured/multi-entity/start", json=invalid_request_2
        )
        assert response.status_code == 422  # Validation error
        ...
        response = client.post(
            "/v1/structured/multi-entity/start", json=invalid_request_3
        )
        assert response.status_code == 422  # Validation error
        ...
        response = client.post(
            "/v1/structured/multi-entity/start", json=invalid_request_4
        )
        assert (
            response.status_code == 400
        )  # Bad request (will fail during orchestrator processing)

    def test_get_multi_entity_investigation_status_endpoint(
        self, client, sample_multi_entity_request
    ):
        ...
        status_response = client.get(
            f"/v1/structured/multi-entity/{investigation_id}/status"
        )
    ...
    def test_update_multi_entity_relationships_endpoint(self, client):
        ...
        response = client.put(
            "/v1/structured/multi-entity/test_id/relationships", json=relationships
        )
    ...
    @pytest.mark.asyncio
    async def test_multi_entity_investigation_request_model_validation(
        self, async_client
    ):
        ...
        response = await async_client.post(
            "/v1/structured/multi-entity/start", json=valid_request
        )
        assert response.status_code == 200
        ...
        response = await async_client.post(
            "/v1/structured/multi-entity/start", json=invalid_strength_request
        )
        assert response.status_code == 422  # Validation error
    ...
    def test_entity_relationship_enum_values(self, client):
        ...
        for relationship_type in relationship_types:
            ...
            ), f"Failed for relationship type: {relationship_type}"
    ...
    def test_boolean_logic_parameter(self, client):
        ...
        for expression in boolean_expressions:
            ...
            assert (
                response.status_code == 200
            ), f"Failed for boolean logic: {expression}"
    ...
    def test_metadata_handling(self, client):
        ...
        response = client.post(
            "/v1/structured/multi-entity/start", json=request_with_metadata
        )
        assert response.status_code == 200
...
class TestMultiEntityInvestigationErrorHandling:
    ...
    def test_missing_required_fields(self, client):
        ...
        request_missing_entities = {"boolean_logic": "A AND B"}

        response = client.post(
            "/v1/structured/multi-entity/start", json=request_missing_entities
        )
    ...
    def test_empty_entities_list(self, client):
        ...
        request_empty_entities = {"entities": []}

        response = client.post(
            "/v1/structured/multi-entity/start", json=request_empty_entities
        )
    ...
    def test_invalid_relationship_references(self, client):
        ...
        response = client.post(
            "/v1/structured/multi-entity/start", json=request_invalid_relationship
        )
        assert response.status_code == 400  # Bad request from orchestrator validation

    @patch(
        "app.service.agent.multi_entity.investigation_orchestrator.MultiEntityInvestigationOrchestrator.start_multi_entity_investigation"
    )
