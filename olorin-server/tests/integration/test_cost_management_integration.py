...
class TestCostManagementSystemIntegration:
    ...
    def cost_management_stack(self):
        ...
        return {
            ...
            'circuit_breaker': get_circuit_breaker('integration_test', CircuitBreakerConfig(
                failure_threshold=3,
    ...
    @pytest.mark.asyncio
    async def test_complete_investigation_cost_workflow(self, cost_management_stack, api_cost_monitor):
        ...
        for phase_data in investigation_phases:
            try:
                ...
                is_affordable, reason = await credit_monitor.validate_request_affordability(cost_estimate)
                ...
                if is_affordable:
                    ...
                    await cost_optimization.track_actual_usage(optimization_result.optimized_cost)
        ...
        assert total_investigation_cost < 100.0  # Should be under $100 for full investigation
        
        # Verify optimization was applied
        optimization_stats = cost_optimization.get_optimization_stats()
        assert optimization_stats['optimization']['total_requests'] >= len(investigation_phases)
    ...
    async def test_budget_exhaustion_graceful_degradation(self, cost_management_stack):
        ...
        model_selection, optimization_result = await cost_optimization.optimize_request(**constrained_investigation)

        # Under budget pressure, should fallback to cheaper model
        assert model_selection.fallback_reason is not None or optimization_result.savings > 0
    ...
    @pytest.mark.asyncio
    async def test_circuit_breaker_integration_with_api_failures(self, cost_management_stack):
        ...
        async def simulate_anthropic_api_call():
            ...
            if failure_count <= 2:
                ...
                raise Exception("RateLimitError: API rate limit exceeded - please retry after 60 seconds")
                ...
                return {
                    ...
                    "risk_score": 0.87,
                    "recommended_actions": ["Block transaction", "Flag account", "Manual review"]
                }
    ...
    @pytest.mark.asyncio
    async def test_real_time_cost_tracking_during_investigation(self, cost_management_stack, api_cost_monitor):
        ...
        try:
            ...
            investigation_steps = [
                {"step": "user_verification", "cost": 8.50, "model": "claude-3-haiku-20240307"},
                {"step": "transaction_analysis", "cost": 22.75, "model": "claude-3-sonnet-20240229"},
                {"step": "behavioral_analysis", "cost": 45.25, "model": "claude-opus-4-1-20250805"},
                {"step": "final_decision", "cost": 18.50, "model": "claude-3-sonnet-20240229"},
            ]
    ...
    @pytest.mark.asyncio
    async def test_multi_investigation_concurrent_cost_management(self, cost_management_stack, api_cost_monitor):
        ...
        async def run_investigation(investigation_id: str, investigation_type: str):
            ...
            return {
                ...
                "strategies_applied": [s.value for s in optimization_result.strategies_applied]
            }
    ...
    async def test_cost_management_performance_overhead(self, cost_management_stack):
        ...
        assert avg_time_per_operation < 0.1, f"Cost management overhead too high: {avg_time_per_operation:.4f}s per operation"
    ...
    async def test_system_health_check_integration(self, cost_management_stack):
        ...
        health_checks['credit_monitor'] = await cost_management_stack['credit_monitor'].health_check()
        health_checks['model_fallback'] = await cost_management_stack['model_fallback'].health_check()
        health_checks['cost_optimization'] = await cost_management_stack['cost_optimization'].health_check()
        health_checks['cost_tracker'] = await cost_management_stack['cost_tracker'].health_check()

        # Verify all components report healthy status
        for component_name, health_status in health_checks.items():
            assert health_status["status"] == "healthy", f"{component_name} reported unhealthy status: {health_status}"
...
class TestCostManagementConfigurationIntegration:
    ...
    def test_configuration_loading_and_validation(self):
        ...
        config_path = Path(__file__).parent.parent.parent / "config" / "cost_management_config.yaml"
    ...
    async def test_configuration_integration_with_components(self):
        ...
        assert cost_optimization.budget_limits[cost_optimization.BudgetPeriod.DAILY] == 500.0
...
class TestInvestigationScenarioIntegration:
    ...
    async def test_money_laundering_investigation_end_to_end(self, api_cost_monitor):
        ...
        model_fallback = get_model_fallback()
        cost_optimization = get_cost_optimization()
        circuit_breaker = get_circuit_breaker("ml_investigation", CircuitBreakerConfig(
            failure_threshold=2,
            ...
            try:
                ...
                async def execute_ml_phase():
                    ...
                    return {
                        ...
                        "model_used": model_selection.selected_model,
                        "risk_indicators": ["structured_transactions", "shell_entities", "geographic_risk"],
                        "confidence": 0.89
                    }
        ...
        assert total_investigation_cost < 200.0  # Even complex ML investigation should be reasonable
        ...
        assert any("opus" in model.lower() for model in models_used)  # Should use premium models

    @pytest.mark.asyncio
    async def test_insider_fraud_investigation_with_budget_constraints(self, api_cost_monitor):
        ...
        cost_optimization.usage_by_period[cost_optimization.BudgetPeriod.DAILY] = 450.0  # 90% used
        ...
        model_selection, optimization_result = await cost_optimization.optimize_request(**insider_fraud_scenario)
        ...
        budget_check = await cost_optimization._check_budget_constraints(optimization_result.optimized_cost)
        # Should have warnings but still allow critical investigation
        warning_alerts = [a for a in budget_check if a.threshold_type in ['warning', 'critical']]
...
class TestCostManagementResilience:
    ...
    async def test_system_resilience_under_api_failures(self):
        ...
        circuit_breaker = get_circuit_breaker("resilience_test", CircuitBreakerConfig(
            failure_threshold=2,
    ...
    async def test_graceful_degradation_under_extreme_load(self):
        ...
        try:
            ...
            critical_failures = [r for r in failed_results if "critical" in str(r).lower()]
    ...
    async def test_cost_management_system_recovery(self):
        ...
        for component_name, result in recovery_results.items():
            assert result["recovery_successful"], f"{component_name} failed to recover: {result}"