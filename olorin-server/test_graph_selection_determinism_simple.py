    ...
    try:
        ...
        for i in range(5):
            ...
            hybrid_enabled = feature_flags.is_enabled(
                "hybrid_graph_v1", investigation_id
            )
        ...
        if all_same:
            print(
                f"   ✅ Feature flag deterministic: {results[0]} (all {len(results)} calls)"
            )
        else:
...
def test_rollback_trigger_logic():
    ...
    try:
        ...
        if all_same:
            print(
                f"   ✅ Rollback triggers deterministic: {results[0]} (all {len(results)} calls)"
            )
        else:
...
def test_selection_priority_logic():
    ...
    test_scenarios = [
        ...
        },
        {
            ...
            "params": {"hybrid_flag_enabled": True},
    ...
    for scenario in test_scenarios:
        ...
        if consistent and correct:
            ...
            print(
                f"   ❌ {scenario['name']}: got {results}, expected {scenario['expected']}"
            )
            all_passed = False
