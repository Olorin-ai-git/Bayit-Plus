...
async def test_on_fraud():
    ...
    if features.get("burst_score_3h", 0) > 2:
        print(
            f"  🚨 BURST DETECTED: {features.get('max_tx_in_3h', 0)} transactions in 3 hours"
        )

    if features.get("max_repeated_amount_ratio", 0) > 0.5:
        print(
            f"  🚨 REPEATED AMOUNTS: {features.get('max_repeated_amount_count', 0)} identical amounts"
        )

    if features.get("rapid_succession", 0) > 0:
        print(
            f"  🚨 RAPID SUCCESSION: Min interval {features.get('min_interval_minutes', 0):.1f} minutes"
        )

    if features.get("single_merchant", 0) > 0 and features.get("tx_count", 0) > 5:
        print(
            f"  🚨 SINGLE MERCHANT: All {features.get('tx_count', 0)} transactions at same merchant"
        )
    ...
    for tx in fraud_txs[:5]:  # Show first 5
        ...
        flag = (
            "✅ CAUGHT" if score >= risk_assessment["risk_threshold"] else "❌ MISSED"
        )
    ...
    if caught_fraud > 0:
        print(
            f"✅ SUCCESS! Enhanced scoring would catch {caught_fraud}/{len(fraud_txs)} fraud transactions"
        )
