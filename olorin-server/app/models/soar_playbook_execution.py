...
class SOARPlaybookExecution(Base):
    ...
    playbook_id = Column(
        String(255),
        nullable=False,
        comment="SOAR playbook identifier"
    )
    ...
    trigger_reason = Column(
        Text,
        nullable=True,
        comment="Reason for playbook trigger"
    )
    ...
    actions_executed = Column(
        Text,
        nullable=True,
        comment="JSON array of action execution results"
    )
    ...
    __table_args__ = (
        CheckConstraint("status IN ('running', 'completed', 'failed', 'cancelled')", name='chk_soar_execution_status'),
