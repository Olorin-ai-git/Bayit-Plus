...
from sqlalchemy import Column, String, Text, DateTime, CheckConstraint, Index, UniqueConstraint
...
class ComposioConnection(Base):
    ...
    refresh_token = Column(
        Text,
        nullable=True,
        comment="Encrypted refresh token (optional)"
    )
    ...
    __table_args__ = (
        UniqueConstraint('tenant_id', 'toolkit_name', 'connection_id', name='uq_composio_connections_tenant_toolkit_connection'),
        CheckConstraint("status IN ('active', 'expired', 'revoked')", name='chk_composio_connection_status'),
    ...
    def is_expired(self) -> bool:
        ...
        if self.expires_at is None:
            return False
        return datetime.now(self.expires_at.tzinfo) >= self.expires_at if self.expires_at.tzinfo else datetime.now() >= self.expires_at.replace(tzinfo=None)
