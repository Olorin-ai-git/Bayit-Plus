...
class ValidatedDeviceID(BaseModel):
    ...
    device_id: str = Field(..., min_length=1, max_length=200, description="Device identifier")
...
class ValidatedInvestigationID(BaseModel):
    ...
    investigation_id: str = Field(..., min_length=1, max_length=100, description="Investigation identifier")
...
class ValidatedTimeRange(BaseModel):
    ...
    def validate_time_range(cls, v):
        ...
        if not re.match(r'^\d+[dhm]$', v):
            raise ValueError("Time range must be in format: number followed by d/h/m (e.g., '30d', '7d', '1h')")
...
class ValidatedEntityType(BaseModel):
    ...
    entity_type: Literal["user_id", "device_id"] = Field(..., description="Type of entity being analyzed")
...
class ValidatedComment(SecureString):
    ...
    sender: Literal["Investigator", "Policy Team"] = Field(..., description="Comment sender role")
    timestamp: Optional[datetime] = Field(default_factory=datetime.utcnow, description="Comment timestamp")
...
class ValidatedInvestigationRequest(BaseModel):
    ...
    entity_id: str = Field(..., min_length=1, max_length=200, description="Entity identifier")
    entity_type: Literal["user_id", "device_id"] = Field(..., description="Type of entity")
    investigation_id: str = Field(..., min_length=1, max_length=100, description="Investigation identifier")
    time_range: str = Field(..., description="Time range for analysis")
    mode: Optional[Literal["manual", "autonomous"]] = Field(default="manual", description="Analysis mode")
