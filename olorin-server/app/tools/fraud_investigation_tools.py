...
class TransactionAnalysisTool(BaseFraudTool):
    ...
    async def analyze(
        ...
        try:
            ...
            # Query transactions for the entity
            if entity_type == 'email':
                query = f"SELECT * FROM TRANSACTION_DATA WHERE EMAIL = '{entity_id}' ORDER BY TX_DATETIME DESC LIMIT 1000"
            elif entity_type == 'ip':
                query = f"SELECT * FROM TRANSACTION_DATA WHERE IP = '{entity_id}' ORDER BY TX_DATETIME DESC LIMIT 1000"
            ...
            if tx_count > 0:
                ...
                for tx in transactions:
                    tx_datetime = datetime.fromisoformat(tx.get('TX_DATETIME', '').replace('Z', '+00:00'))
                ...
                round_amounts = [amt for amt in amounts if amt == round(amt) and amt >= 100]
                ...
                if round_amount_freq > 0.3:
                    suspicious_patterns.append("Frequent round-amount transactions (card testing)")
                if max_amount > avg_amount * 5:
                    suspicious_patterns.append("Unusual high-value transaction detected")

                # 5. Calculate risk indicators
                risk_indicators = []
                if max_hourly > 5:
                    risk_indicators.append(f"High transaction velocity: {max_hourly} transactions in one hour")
                if round_amount_freq > 0.2:
                    risk_indicators.append(f"Suspicious round amounts: {round_amount_freq:.1%} of transactions")

                # Calculate confidence based on data quality and patterns
                confidence = min(0.95, 0.6 + (len(suspicious_patterns) * 0.1) + (tx_count / 1000 * 0.2))
                ...
                result.data = {
                    ...
                    "velocity_metrics": {
                        "avg_daily": statistics.mean(tx_by_day.values()) if tx_by_day else 0,
                    ...
                    "amount_analysis": {
                        ...
                        "unusual_amounts": [amt for amt in amounts if amt > avg_amount * 3][:5],
                ...
                recommendations = []
                if max_hourly > 10:
                    recommendations.append("Immediate review of high-velocity transactions")
                if round_amount_freq > 0.3:
                    recommendations.append("Investigate potential card testing activity")
                if max_amount > avg_amount * 5:
                    recommendations.append("Verify authorization for high-value transactions")
...
class AccountBehaviorTool(BaseFraudTool):
    ...
    async def analyze(
        ...
        try:
            ...
            recent_countries = list(countries)[-5:] if len(countries) > 5 else list(countries)
            new_locations = [c for c in recent_countries if c not in ['US', 'CA', 'GB']]  # Assume normal locations

            # 3. Calculate spending patterns
            amounts = [float(record.get('PAID_AMOUNT_VALUE_IN_CURRENCY', 0)) for record in auth_data]
            avg_spending = statistics.mean(amounts) if amounts else 0
            recent_spending = statistics.mean(amounts[:10]) if len(amounts) >= 10 else avg_spending

            spending_change = (recent_spending - avg_spending) / avg_spending * 100 if avg_spending > 0 else 0

            # 4. Check account age and dormancy
            if auth_data:
                first_activity = min(login_times)
                last_activity = max(login_times)
                account_age = (datetime.now() - datetime.fromisoformat(first_activity.replace('Z', '+00:00'))).days
            ...
            if len(new_locations) > 0:
                risk_indicators.append(f"Activity from {len(new_locations)} unusual countries")
            if abs(spending_change) > 200:
                risk_indicators.append(f"Significant spending pattern change: {spending_change:.1f}%")
            if failed_attempts > len(auth_data) * 0.1:
                risk_indicators.append(f"High failure rate: {failed_attempts} failed transactions")
        ...
        try:
            ...
            identity_query = f"""
                SELECT UNIQUE_USER_ID, EMAIL, FIRST_NAME, LAST_NAME, PHONE_NUMBER,
                       ...
                       IS_FRAUD_TX, NSURE_LAST_DECISION
            ...
            emails = set(record.get('EMAIL') for record in identity_data if record.get('EMAIL'))
            names = set((record.get('FIRST_NAME'), record.get('LAST_NAME')) for record in identity_data
                       if record.get('FIRST_NAME') and record.get('LAST_NAME'))
            phones = set(record.get('PHONE_NUMBER') for record in identity_data if record.get('PHONE_NUMBER'))
            countries = set(record.get('IP_COUNTRY_CODE') for record in identity_data if record.get('IP_COUNTRY_CODE'))
            ...
            fraud_transactions = sum(1 for record in identity_data if record.get('IS_FRAUD_TX') == 1)
            total_transactions = len(identity_data)
            fraud_rate = fraud_transactions / total_transactions if total_transactions > 0 else 0

            # 4. Calculate risk scores
            avg_model_score = statistics.mean([float(record.get('MODEL_SCORE', 0)) for record in identity_data if record.get('MODEL_SCORE')])

            # 5. Verification status assessment
            verification_score = 0
            if email_consistency: verification_score += 0.25
            if name_consistency: verification_score += 0.25
            if phone_consistency: verification_score += 0.25
            if geo_consistency: verification_score += 0.25

            # Adjust for fraud indicators
            if fraud_rate > 0.1: verification_score -= 0.3
            if avg_model_score > 0.7: verification_score -= 0.2
            ...
            result.data = {
                ...
                "verification_status": {
                    ...
                    "status": "verified" if verification_score > 0.7 else "requires_review" if verification_score > 0.4 else "high_risk",
...
class ATODetectionTool(BaseFraudTool):
    ...
    async def analyze(
        ...
        try:
            ...
            if not ato_data:
                result.status = "partial"
                result.data = {"message": "No authentication data found for ATO analysis"}
                return result
            ...
            for record in ato_data:
                if record.get('TX_DATETIME'):
                    login_times.append(datetime.fromisoformat(record['TX_DATETIME'].replace('Z', '+00:00')))
                if record.get('IP'):
            ...
            rapid_attempts = min_time_between < 60  # Less than 1 minute between attempts
            distributed_sources = unique_ips > 10
            high_failure_rate = failed_attempts / total_attempts > 0.3 if total_attempts > 0 else False
            geographic_spread = unique_countries > 5

            # 4. Calculate ATO likelihood
            ato_score = 0
            if rapid_attempts: ato_score += 0.3
            if distributed_sources: ato_score += 0.25
            if high_failure_rate: ato_score += 0.25
            if geographic_spread: ato_score += 0.2
            ...
            result.data = {
                ...
                "security_indicators": {
                    ...
                    "failure_rate": round(failed_attempts / total_attempts, 3) if total_attempts > 0 else 0,
                    "high_failure_rate_detected": high_failure_rate,
                },
                "ato_assessment": {
                    "ato_likelihood_score": round(ato_score, 3),
                    "risk_level": "high" if ato_score > 0.7 else "medium" if ato_score > 0.4 else "low",
            ...
            risk_indicators = []
            if rapid_attempts:
                risk_indicators.append(f"Rapid login attempts detected (minimum {min_time_between}s apart)")
            if distributed_sources:
                risk_indicators.append(f"Distributed IP sources: {unique_ips} different IPs")
            if high_failure_rate:
                risk_indicators.append(f"High failure rate: {failed_attempts}/{total_attempts} failed")
            if geographic_spread:
                risk_indicators.append(f"Geographic spread: {unique_countries} countries")
            ...
            if distributed_sources:
                recommendations.append("Review and potentially block suspicious IP ranges")
        ...
        try:
            ...
            ml_query = f"""
                SELECT TX_ID_KEY, EMAIL, IP, IP_COUNTRY_CODE, DEVICE_ID, USER_AGENT,
                       ...
                       PAYMENT_METHOD, CARD_BRAND, TX_DATETIME
                FROM TRANSACTION_DATA
            ...
            amounts = [float(tx.get('PAID_AMOUNT_VALUE_IN_CURRENCY', 0)) for tx in ml_data]
            model_scores = [float(tx.get('MODEL_SCORE', 0)) for tx in ml_data if tx.get('MODEL_SCORE')]
            ...
            # Geographic features
            countries = [tx.get('IP_COUNTRY_CODE') for tx in ml_data if tx.get('IP_COUNTRY_CODE')]
            unique_countries = len(set(countries))
            ...
            transaction_risk = min(1.0, avg_amount / 1000 * 0.1 + (max_amount / avg_amount if avg_amount > 0 else 0) * 0.1)
            ...
            risk_factors = [
                {"factor": "account_risk_score", "weight": account_risk, "impact": 0.30},
                {"factor": "transaction_patterns", "weight": transaction_risk, "impact": 0.25},
                {"factor": "device_diversity", "weight": device_risk, "impact": 0.15},
                {"factor": "geographic_spread", "weight": network_risk, "impact": 0.15},
                {"factor": "historical_fraud_rate", "weight": behavioral_risk, "impact": 0.15},
            ]
            ...
            result.data = {
                ...
                "risk_classification": "high" if ensemble_score > 0.7 else "medium" if ensemble_score > 0.4 else "low",
            }
            ...
            if account_risk > 0.8:
                risk_indicators.append(f"High account risk score from existing model: {account_risk:.3f}")
            if unique_countries > 3:
                risk_indicators.append(f"Geographic risk: activity from {unique_countries} countries")
            if fraud_rate > 0.1:
                risk_indicators.append(f"Historical fraud pattern: {fraud_rate:.1%} fraud rate")
            ...
            if ensemble_score > 0.8:
                ...
                recommendations.append("Consider blocking transaction until verification")
    ...
    async def analyze(
        ...
        try:
            ...
            graph_query = f"""
                ...
                related_entities AS (
                    SELECT t.EMAIL, t.IP, t.DEVICE_ID, t.PHONE_NUMBER, t.IS_FRAUD_TX, t.MODEL_SCORE
                    FROM TRANSACTION_DATA t
                    INNER JOIN target_data td ON (
                        t.IP = td.IP OR
                        t.DEVICE_ID = td.DEVICE_ID OR
            ...
            if not graph_data:
                result.status = "partial"
                result.data = {"message": "No relationship data found for graph analysis"}
                return result

            # 1. Build relationship graph metrics
            connected_emails = set(record.get('EMAIL') for record in graph_data if record.get('EMAIL'))
            shared_ips = set(record.get('IP') for record in graph_data if record.get('IP'))
            shared_devices = set(record.get('DEVICE_ID') for record in graph_data if record.get('DEVICE_ID'))
            shared_phones = set(record.get('PHONE_NUMBER') for record in graph_data if record.get('PHONE_NUMBER'))
            ...
            fraud_entities = [record for record in graph_data if record.get('IS_FRAUD_TX') == 1]
            fraud_rate_in_network = len(fraud_entities) / len(graph_data) if graph_data else 0

            # Calculate average risk score in network
            risk_scores = [float(record.get('MODEL_SCORE', 0)) for record in graph_data if record.get('MODEL_SCORE')]
            ...
            if ip_connections > 10:
                ring_indicators.append(f"Shared IP addresses: {ip_connections} entities")
                ring_score += 0.25

            if fraud_rate_in_network > 0.3:
                ring_indicators.append(f"High fraud rate in network: {fraud_rate_in_network:.1%}")
                ring_score += 0.25

            if avg_network_risk > 0.6:
                ring_indicators.append(f"High average risk score: {avg_network_risk:.3f}")
                ring_score += 0.2

            # 5. Calculate centrality and clustering metrics
            degree_centrality = min(1.0, total_connections / 100)  # Normalized by max expected connections
            clustering_coefficient = min(1.0, (device_connections + ip_connections) / max(1, total_connections))
            ...
            result.data = {
                "graph_metrics": {
                    ...
                    "network_density": round(total_connections / max(1, len(graph_data)), 3),
                },
                "shared_attributes": {
            ...
            if device_connections > 10:
                risk_indicators.append(f"Extensive device sharing: {device_connections} shared devices")
            if fraud_rate_in_network > 0.4:
                risk_indicators.append(f"High fraud rate in connected network: {fraud_rate_in_network:.1%}")
            if avg_network_risk > 0.7:
                risk_indicators.append(f"Connected to high-risk entities (avg score: {avg_network_risk:.3f})")
