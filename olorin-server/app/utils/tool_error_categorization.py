...
class ToolErrorCategorizer:
    ...
    ERROR_PATTERNS = {
        ...
        ToolErrorCategory.API_SERVICE_UNAVAILABLE: [
            "503",
            "service unavailable",
    ...
    def categorize_error(
        cls, error: Exception, tool_name: str = "", http_status: Optional[int] = None
    ) -> ToolErrorCategory:
        """
    ...
    def get_recovery_suggestion(cls, category: ToolErrorCategory) -> tuple[str, bool]:
        ...
        recovery_map = {
            ...
            ToolErrorCategory.TIMEOUT_ERROR: (
                "Increase timeout or check service performance",
                True,
            ),
            ToolErrorCategory.AUTH_INVALID_CREDENTIALS: (
            ...
            ToolErrorCategory.AUTH_RATE_LIMITED: (
                "Wait for rate limit reset or increase quota",
                True,
            ),
            ToolErrorCategory.API_SERVER_ERROR: ("Wait for service recovery", True),
            ToolErrorCategory.API_BAD_REQUEST: (
                "Check request parameters and format",
                False,
            ),
            ToolErrorCategory.DATA_PARSING_ERROR: (
                "Verify data format and parsing logic",
                False,
            ),
            ToolErrorCategory.DB_CONNECTION_FAILED: (
...
def create_tool_error_details(
    ...
    suggested_action, is_retryable = ToolErrorCategorizer.get_recovery_suggestion(
        category
    )
    ...
    stack_trace_hash = (
        hashlib.md5(stack_trace.encode()).hexdigest()[:8] if stack_trace else None
    )
