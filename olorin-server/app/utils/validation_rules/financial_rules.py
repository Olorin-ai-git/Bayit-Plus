...
class FinancialValidationRules:
    ...
    def validate_currency_amount(
        ...
        try:
            ...
            if actual_precision > expected_precision:
                return (
                    False,
                    f"Too many decimal places for {currency} (max {expected_precision})",
                )
    ...
    def validate_payment_method(
        self, payment_method: str, payment_data: Dict[str, Any]
    ) -> Tuple[bool, Optional[str]]:
        """
    ...
    def validate_transaction_risk(
        ...
        should_block = (
            risk_score > self._get_blocking_threshold() or "test_amount" in risk_factors
        )
        error_message = None

        if should_block:
            error_message = (
                f"Transaction blocked due to high risk (score: {risk_score:.2f})"
            )
    ...
    def _validate_paypal(
        self, paypal_data: Dict[str, Any]
    ) -> Tuple[bool, Optional[str]]:
    ...
    def _calculate_velocity_risk_score(
