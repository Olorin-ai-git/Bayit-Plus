...
class GeographicValidationRules:
    ...
    COUNTRY_DATA = {
        'US': {'name': 'United States', 'continent': 'NA', 'postal_pattern': r'^\d{5}(-\d{4})?$'},
        'GB': {'name': 'United Kingdom', 'continent': 'EU', 'postal_pattern': r'^[A-Z]{1,2}\d[A-Z\d]?\s*\d[A-Z]{2}$'},
        'DE': {'name': 'Germany', 'continent': 'EU', 'postal_pattern': r'^\d{5}$'},
        'FR': {'name': 'France', 'continent': 'EU', 'postal_pattern': r'^\d{5}$'},
        'CA': {'name': 'Canada', 'continent': 'NA', 'postal_pattern': r'^[A-Z]\d[A-Z]\s*\d[A-Z]\d$'},
        ...
        'NL': {'name': 'Netherlands', 'continent': 'EU', 'postal_pattern': r'^\d{4}\s*[A-Z]{2}$'},
        'SE': {'name': 'Sweden', 'continent': 'EU', 'postal_pattern': r'^\d{3}\s*\d{2}$'},
        ...
        'BR': {'name': 'Brazil', 'continent': 'SA', 'postal_pattern': r'^\d{5}-?\d{3}$'},
        ...
        'HK': {'name': 'Hong Kong', 'continent': 'AS', 'postal_pattern': r'^$'},  # No postal codes
    ...
    RISK_REGIONS = {
        'high_risk_countries': {
            'NG', 'PK', 'BD', 'ID', 'PH', 'VN', 'LK', 'GH', 'KE', 'EG',
            'MA', 'DZ', 'TN', 'LB', 'JO', 'IQ', 'AF', 'MM', 'KH', 'LA'
        },
        'medium_risk_countries': {
            'RU', 'UA', 'BY', 'MD', 'RS', 'BA', 'MK', 'AL', 'BG', 'RO',
            'TR', 'GE', 'AM', 'AZ', 'KZ', 'UZ', 'KG', 'TJ', 'TM'
        },
        'sanctions_countries': {
            'IR', 'KP', 'SY', 'CU', 'VE', 'MM'  # Example sanctioned countries
        }
    }
    ...
    def validate_country_code(self, country_code: str) -> Tuple[bool, Optional[str], Dict[str, Any]]:
        """
        ...
        if country_analysis['is_sanctioned']:
            return False, f"Transactions from {country_info['name']} are not permitted", country_analysis
        
        return True, None, country_analysis
    
    def validate_postal_code(self, postal_code: str, country_code: str) -> Tuple[bool, Optional[str]]:
        """
        ...
        country_code = country_code.upper()
        if country_code not in self.COUNTRY_DATA:
            return False, f"Cannot validate postal code for unknown country: {country_code}"
    ...
    def validate_coordinates(self, latitude: float, longitude: float) -> Tuple[bool, Optional[str], Dict[str, Any]]:
        """
        ...
        try:
            ...
            location_analysis = {
                ...
                'nearest_major_city': self._find_nearest_major_city(latitude, longitude)
            }
    ...
    def validate_location_consistency(self, location_data: Dict[str, Any]) -> Tuple[bool, Optional[str], Dict[str, Any]]:
        """
        ...
        if country_code and timezone:
            ...
            if expected_timezones and timezone not in expected_timezones:
                inconsistencies.append(f"Timezone {timezone} not typical for {country_code}")
        ...
        if coordinates and country_code:
            ...
            if lat is not None and lon is not None:
                ...
                if estimated_country and estimated_country != country_code:
                    inconsistencies.append(f"Coordinates suggest {estimated_country} but country is {country_code}")
            ...
            if ip_country and ip_country != country_code:
                ...
                if ip_coords and coordinates:
                    distance = self._calculate_distance(
                        ip_coords.get('lat'), ip_coords.get('lon'),
                        coordinates.get('lat'), coordinates.get('lon')
                    )
                    if distance > 1000:  # More than 1000 km apart
                        inconsistencies.append(f"IP location ({ip_country}) far from stated location ({country_code})")
        ...
        if not is_consistent:
            error_message = f"Location inconsistencies detected: {', '.join(inconsistencies[:2])}"
    ...
    def validate_ip_geolocation(self, ip: str, stated_location: Dict[str, Any]) -> Tuple[bool, Optional[str], Dict[str, Any]]:
        ...
        if stated_country and mock_ip_location['country_code'] != stated_country:
            inconsistencies.append(f"IP country ({mock_ip_location['country_code']}) differs from stated ({stated_country})")
            risk_score += 0.4
        
        # Compare coordinates if available
        if stated_coords and mock_ip_location['coordinates']:
            distance = self._calculate_distance(
                stated_coords.get('lat'), stated_coords.get('lon'),
                mock_ip_location['coordinates']['lat'],
                mock_ip_location['coordinates']['lon']
            )
            
            if distance > 500:  # More than 500 km difference
                inconsistencies.append(f"IP location {distance:.0f} km from stated location")
                risk_score += 0.3
        ...
        if not is_consistent:
            error_message = f"IP geolocation inconsistencies: {', '.join(inconsistencies[:2])}"
    ...
    def _find_nearest_major_city(self, latitude: float, longitude: float) -> Dict[str, Any]:
    ...
    def _estimate_country_from_coordinates(self, latitude: float, longitude: float) -> Optional[str]:
    ...
    def _get_city_coordinates(self, city: str, country_code: str) -> Optional[Tuple[float, float]]:
