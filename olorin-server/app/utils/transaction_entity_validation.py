...
class TransactionEntityValidator:
    ...
    PATTERNS = {
        ...
        'payment_method_type': re.compile(r'^(credit_card|debit_card|paypal|apple_pay|google_pay|bank_transfer|crypto)$'),
        ...
        # Status patterns
        'authorization_stage': re.compile(r'^(authorized|declined|pending_review|flagged_suspicious|flagged_high_risk|under_investigation)$'),
    ...
    @classmethod
    def validate_all_csv_columns(cls, row_data: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
        """
    ...
    def _get_csv_entity_mappings(cls) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            'TABLE_RECORD_UPDATED_AT': {
                ...
                'validation_rules': ['datetime_iso', 'not_empty', 'reasonable_date', 'after_created']
            },
            'TX_DATETIME': {
            ...
            'TX_TIMESTAMP_MS': {
                ...
                'validation_rules': ['timestamp_ms', 'not_empty', 'reasonable_timestamp_ms']
            },
            ...
            'DATE_OF_BIRTH': {
                ...
                'validation_rules': ['datetime_iso', 'allow_empty', 'reasonable_birth_date']
            },
            ...
            'LAST_NAME': {
                ...
                'validation_rules': ['first_name', 'allow_empty', 'length_max_50']  # Same pattern as first name
            },
            'UNIQUE_USER_ID': {
            ...
            'PAID_AMOUNT_VALUE_IN_CURRENCY': {
                ...
                'validation_rules': ['decimal_amount', 'allow_empty', 'positive_amount']
            },
            ...
            'PROCESSING_FEE_VALUE_IN_CURRENCY': {
                ...
                'validation_rules': ['decimal_amount', 'allow_empty', 'non_negative_amount']
            },
    ...
    def _apply_validation_rule(
        ...
        if rule_name == 'not_empty':
            ...
            if value and not cls.PATTERNS['datetime_iso'].match(value):
                errors.append(f'{entity_type.value} must be in ISO datetime format (YYYY-MM-DD HH:MM:SS.f)')
        
        elif rule_name == 'timestamp_ms':
            if value and not cls.PATTERNS['timestamp_ms'].match(value):
                errors.append(f'{entity_type.value} must be a 13-digit millisecond timestamp')
            ...
            if value and not cls.PATTERNS['hex_id'].match(value):
                errors.append(f'{entity_type.value} must be a 32-character hexadecimal ID')
        
        elif rule_name == 'composite_id':
            if value and not cls.PATTERNS['composite_id'].match(value):
                errors.append(f'{entity_type.value} must be in composite format (PREFIX::SUFFIX)')
        
        elif rule_name == 'app_id_pattern':
            if value and not cls.PATTERNS['app_id_pattern'].match(value):
                errors.append(f'{entity_type.value} must be a 16-character alphanumeric ID')
        
        elif rule_name == 'store_id_pattern':
            if value and not cls.PATTERNS['store_id_pattern'].match(value):
                errors.append(f'{entity_type.value} must contain only alphanumeric characters, underscores, and hyphens')
            ...
            if value and not cls.PATTERNS['email_normalized'].match(value):
                errors.append(f'{entity_type.value} must be a normalized (lowercase) email address')
        
        elif rule_name == 'first_name':
            if value and not cls.PATTERNS['first_name'].match(value):
                errors.append(f'{entity_type.value} must contain only letters, spaces, apostrophes, and hyphens')
        
        elif rule_name == 'authorization_stage':
            if value and not cls.PATTERNS['authorization_stage'].match(value):
                errors.append(f'{entity_type.value} must be one of: authorized, declined, pending_review, flagged_suspicious, flagged_high_risk, under_investigation')
        
        elif rule_name == 'event_type':
            if value and not cls.PATTERNS['event_type'].match(value):
                errors.append(f'{entity_type.value} must be one of: tx, refund, chargeback, adjustment')
            ...
            if value:
                try:
                    ...
                    if dt.year < 2020 or dt.year > 2030:
                        warnings.append(f'{entity_type.value} date seems unreasonable: {dt.year}')
                except ValueError:
            ...
            if value:
                try:
                    ...
                    if dt.year < 2020 or dt.year > 2030:
                        warnings.append(f'{entity_type.value} timestamp seems unreasonable: {dt.year}')
            ...
            if value:
                try:
                    ...
                    if dt.year < 1900 or dt.year > current_year - 13:  # Minimum 13 years old
                        warnings.append(f'{entity_type.value} seems unreasonable for birth date: {dt.year}')
                except ValueError:
            ...
            if value and not cls.PATTERNS['country_code'].match(value):
                errors.append(f'{entity_type.value} must be a valid country code (1-4 digits)')
        
        elif rule_name == 'phone_number_format':
            if value and not cls.PATTERNS['phone_number_format'].match(value):
                errors.append(f'{entity_type.value} must be a valid phone number format')
        
        elif rule_name == 'currency_code':
            if value and not cls.PATTERNS['currency_code'].match(value):
                errors.append(f'{entity_type.value} must be a valid ISO 4217 currency code (3 letters)')
            ...
            if value and not cls.PATTERNS['payment_method_type'].match(value):
                errors.append(f'{entity_type.value} must be a valid payment method type')
        
        elif rule_name == 'token_format':
            if value and not cls.PATTERNS['token_format'].match(value):
                errors.append(f'{entity_type.value} must be a valid token format (10-100 characters)')
        
        elif rule_name == 'identifier_format':
            if value and not cls.PATTERNS['identifier_format'].match(value):
                errors.append(f'{entity_type.value} must be a valid identifier format (5-50 characters)')
        
        elif rule_name == 'three_d_secure_status':
            if value and not cls.PATTERNS['three_d_secure_status'].match(value):
                errors.append(f'{entity_type.value} must be a valid 3D Secure result (Y/N/A/U/R/E)')
    ...
    @classmethod
    def get_validation_summary(cls, validation_results: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        valid_columns = sum(1 for result in validation_results.values() if result['is_valid'])
        invalid_columns = sum(1 for result in validation_results.values() if not result['is_valid'])
        columns_with_warnings = sum(1 for result in validation_results.values() if result['warnings'])
        ...
        return {
            ...
            'validation_success_rate': (valid_columns / total_columns * 100) if total_columns > 0 else 0,
            'all_errors': all_errors,
            'all_warnings': all_warnings,
            'columns_missing_entity_mapping': [
                col for col in validation_results 
                if validation_results[col]['errors'] and 'missing from row data' in validation_results[col]['errors'][0]
            ]
        }
    ...
    def validate_csv_row_completeness(cls, row_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'completeness_percentage': (len(provided_columns & expected_columns) / len(expected_columns)) * 100
        }
...
def validate_transaction_row(row_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    completeness_result = TransactionEntityValidator.validate_csv_row_completeness(row_data)
    
    # Validate all columns
    validation_results = TransactionEntityValidator.validate_all_csv_columns(row_data)
    
    # Generate summary
    validation_summary = TransactionEntityValidator.get_validation_summary(validation_results)
    
    return {
        'row_completeness': completeness_result,
        'column_validations': validation_results,
        'validation_summary': validation_summary,
        'overall_valid': completeness_result['is_complete'] and validation_summary['invalid_columns'] == 0
    }