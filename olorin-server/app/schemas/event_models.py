...
class Actor(BaseModel):
    ...
    type: ActorType = Field(..., description="Type of actor generating the event")
    ...
    def validate_service(cls, v: Optional[str], info) -> Optional[str]:
        ...
        if actor_type in [ActorType.SYSTEM, ActorType.WEBHOOK] and not v:
            raise ValueError(
                "service is required when actor type is 'system' or 'webhook'"
            )
        return v
...
class InvestigationEvent(BaseModel):
    ...
    ts: str = Field(..., description="ISO 8601 timestamp when event occurred")
    actor: Actor = Field(
        ..., description="Information about who/what generated this event"
    )
    op: OperationType = Field(..., description="Type of operation performed")
    entity: EntityType = Field(..., description="Type of entity that was modified")
...
class EventsFeedResponse(BaseModel):
    ...
    next_cursor: Optional[str] = Field(
        None, pattern=r"^\d{13}_\d{6}$", description="Cursor for next page of results"
    )
    has_more: bool = Field(
        False, description="Whether more results exist after this page"
    )
    poll_after_seconds: Optional[int] = Field(
        None, ge=1, le=3600, description="Recommended seconds to wait before next poll"
    )
    etag: Optional[str] = Field(
        None, description='ETag for conditional requests (format: W/"version-hash")'
    )
...
class SummaryResponse(BaseModel):
    ...
    investigation_id: str = Field(
        ..., min_length=1, max_length=255, description="Investigation ID"
    )
    ...
    current_phase: Optional[str] = Field(
        None, description="Current execution phase if in progress"
    )
    progress_percentage: float = Field(
        0.0, ge=0.0, le=100.0, description="Overall progress percentage (0-100)"
    )
    created_at: datetime = Field(..., description="Investigation creation timestamp")
