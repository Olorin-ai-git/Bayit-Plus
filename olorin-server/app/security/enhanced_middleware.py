...
class ThreatDetector:
    ...
    def detect_threats(self, request_data: str, headers: Dict[str, str]) -> List[Dict[str, Any]]:
...
class RateLimiter:
    ...
    def is_rate_limited(
        ...
        if self.redis_client:
            ...
            return self._memory_rate_limit(identifier, limit, window, current_time, burst_limit)
    
    def _redis_rate_limit(self, identifier: str, limit: int, window: int, current_time: float) -> tuple[bool, Dict[str, Any]]:
...
class SecurityEventLogger:
    ...
    def log_security_event(
        ...
        logger.warning(f"SECURITY_EVENT: {json.dumps(event)}", extra={"security_event": True})
        
        # Store in Redis for analysis
        if self.redis_client:
            event_key = f"security_event:{datetime.utcnow().strftime('%Y%m%d_%H')}:{time.time()}"
            self.redis_client.setex(event_key, 86400, json.dumps(event))  # 24 hour retention
...
class EnhancedSecurityMiddleware(BaseHTTPMiddleware):
    ...
    async def _validate_request(self, request: Request) -> Optional[Dict[str, Any]]:
        ...
        if len(str(request.url)) > self.config.max_url_length:
            errors.append(f"URL too long: {len(str(request.url))} > {self.config.max_url_length}")
        
        # Check header size
        total_header_size = sum(len(k) + len(v) for k, v in request.headers.items())
        if total_header_size > self.config.max_header_size:
            errors.append(f"Headers too large: {total_header_size} > {self.config.max_header_size}")
        
        # Check content length if present
        content_length = request.headers.get("content-length")
        if content_length and int(content_length) > self.config.max_request_size:
            errors.append(f"Request body too large: {content_length} > {self.config.max_request_size}")
    ...
    def _handle_rate_limit_violation(self, request: Request, client_ip: str, rate_info: Dict[str, Any]):
    ...
    def _handle_validation_violation(self, request: Request, client_ip: str, validation_result: Dict[str, Any]):
    ...
    def _handle_threat_detection(self, request: Request, client_ip: str, threats: List[Dict[str, Any]]):
    ...
    def _track_violation(self, client_ip: str, violation_type: str):
        ...
        self.violation_tracker[client_ip] = [
            (timestamp, vtype) for timestamp, vtype in self.violation_tracker[client_ip]
        ...
        if len(self.violation_tracker[client_ip]) >= self.config.max_violations_per_hour:
            logger.warning(f"Auto-blocking IP {client_ip} due to {len(self.violation_tracker[client_ip])} violations")
            self.rate_limiter.block_ip(client_ip, self.config.block_duration_minutes)
...
def setup_enhanced_security_middleware(
    ...
    if redis_url:
        try:
            ...
            logger.warning(f"Redis connection failed: {e}. Using memory-based security.")
    ...
    app.add_middleware(EnhancedSecurityMiddleware, config=security_config, redis_client=redis_client)
