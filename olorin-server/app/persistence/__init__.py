...
def _add_to_memory(investigation):
    ...
    investigation_id = (
        ...
        else investigation.get("id") if isinstance(investigation, dict) else None
    )
...
def delete_investigation(investigation_id: str):
    ...
    try:
        with get_db_session() as session:
            ...
            if db_inv:
                ...
                session.commit()
                logger.info(
                    f"[DELETE_INVESTIGATION] Deleted investigation {investigation_id} from database"
                )
            else:
                logger.debug(
                    f"[DELETE_INVESTIGATION] Investigation {investigation_id} not found in database"
                )
    except Exception as db_error:
        logger.error(
            f"[DELETE_INVESTIGATION] Database deletion failed: {db_error}",
            exc_info=True,
        )
    ...
    if deleted_inv:
        ...
        if hasattr(inv_memory, "id"):
            return {
                "investigation_id": inv_memory.id,
                "user_id": getattr(inv_memory, "user_id", None),
                "status": getattr(inv_memory, "status", None),
            }
    ...
    try:
        ...
        try:
            ...
            with get_db_session() as session:
                ...
                logger.debug(
                    f"[LIST_INVESTIGATIONS] Found {len(db_investigations)} investigations in database"
                )
                    ...
                    investigation_data.append(
                        {
                            ...
                            "created_at": (
                                db_inv.created_at.isoformat()
                                if db_inv.created_at
                                else None
                            ),
                            "updated_at": (
                                db_inv.updated_at.isoformat()
                                if db_inv.updated_at
                                else None
                            ),
                        }
                    )
        ...
        if not investigation_data:
            logger.debug(
                "[LIST_INVESTIGATIONS] No investigations found in database, returning in-memory store"
            )
            return list(IN_MEMORY_INVESTIGATIONS.values())
        ...
        for idx, db_inv_data in enumerate(investigation_data):
            try:
                ...
                if not db_inv_data or "investigation_id" not in db_inv_data:
                    logger.warning(
                        f"[LIST_INVESTIGATIONS] Skipping invalid investigation data at index {idx}: missing investigation_id"
                    )
                    continue
                ...
                owner: Optional[str] = (
                    None  # Extract from settings_json, NO FALLBACK to user_id
                )
                ...
                if db_inv_data["settings_json"]:
                    try:
                        ...
                        if settings.get("entity_id"):
                            ...
                            and isinstance(settings.get("entities"), list)
                            and len(settings.get("entities", [])) > 0
                        ):
                            ...
                            if isinstance(first_entity, dict):
                                ...
                                if (
                                    "entityValue" in first_entity
                                    and first_entity["entityValue"]
                                ):
                                    entity_id = first_entity["entityValue"]
                                elif (
                                    "entity_value" in first_entity
                                    and first_entity["entity_value"]
                                ):
                        ...
                        if settings.get("entity_type"):
                            ...
                            if isinstance(first_entity, dict):
                                # Check camelCase first (actual format in DB)
                                if (
                                    "entityType" in first_entity
                                    and first_entity["entityType"]
                                ):
                                    entity_type = first_entity["entityType"]
                                elif (
                                    "entity_type" in first_entity
                                    and first_entity["entity_type"]
                                ):
                        ...
                        if "sources" in settings and settings["sources"]:
                            sources_raw = (
                                settings["sources"]
                                if isinstance(settings["sources"], list)
                                else []
                            )
                            if sources_raw:
                                ...
                                extracted_sources = [
                                    (
                                        s
                                        if isinstance(s, str)
                                        else (s.get("source_name") or s.get("name"))
                                    )
                                    ...
                                    if (
                                        ...
                                        or (
                                            isinstance(s, dict)
                                            and (s.get("source_name") or s.get("name"))
                                        )
                                    )
                                ]
                        ...
                        if "tools" in settings and settings["tools"]:
                            tools_raw = (
                                settings["tools"]
                                if isinstance(settings["tools"], list)
                                else []
                            )
                            if tools_raw:
                            ...
                            # If tools is an empty list [], keep it as None (not an empty list) to match API contract
                                ...
                                isinstance(settings.get("tools"), list)
                                and len(settings.get("tools", [])) == 0
                            ):
                        ...
                        time_range = settings.get("time_range") or settings.get(
                            "timeRange"
                        )
                        ...
                        logger.debug(
                            f"Error parsing settings_json for {db_inv_data['investigation_id']}: {e}"
                        )
                        pass
                ...
                if db_inv_data["progress_json"]:
                    try:
                                ...
                                phases_list = [
                                    {
                                        ...
                                        "status": (
                                            "in-progress"
                                            if db_inv_data["status"] == "IN_PROGRESS"
                                            else "completed"
                                        ),
                                        "pct": progress_value,
                                    }
                        ...
                        logger.debug(
                            f"Error parsing progress_json for {db_inv_data['investigation_id']}: {e}"
                        )
                        pass
                ...
                if progress_data is not None:
                    try:
                        ...
                        if "overall_risk_score" in progress_data:
                            ...
                            overall_risk_score = float(
                                progress_data["final_risk_score"]
                            )
                        ...
                        if (
                            overall_risk_score is None
                            and isinstance(domain_findings_raw, dict)
                            and "risk" in domain_findings_raw
                        ):
                            ...
                            if (
                                ...
                                if risk_score_value is not None:
                                    try:
                                        ...
                                        logger.debug(
                                            f"[LIST_INVESTIGATIONS] Extracted overall_risk_score from domain_findings.risk.risk_score: {overall_risk_score}"
                                        )
                        ...
                        if domain_findings:
                            try:
                                domain_keys = (
                                    list(domain_findings.keys())
                                    if domain_findings
                                    else []
                                )
                                logger.debug(
                                    f"[LIST_INVESTIGATIONS] Found domain_findings in progress_json for {db_inv_data['investigation_id']}: {domain_keys}"
                                )
                                ...
                                for domain_name in [
                                    ...
                                    if domain_name in domain_findings:
                                        ...
                                        if isinstance(domain_data, dict):
                                            domain_keys_list = (
                                                list(domain_data.keys())[:10]
                                                if domain_data
                                                else []
                                            )
                                            logger.debug(
                                                f"[LIST_INVESTIGATIONS] Domain '{domain_name}' keys: {domain_keys_list}"
                                            )
                                            logger.debug(
                                                f"[LIST_INVESTIGATIONS] Domain '{domain_name}' risk_score: {domain_data.get('risk_score')}"
                                            )
                                            llm_analysis = domain_data.get(
                                                "llm_analysis", {}
                                            )
                                            if isinstance(llm_analysis, dict):
                                                llm_keys = (
                                                    list(llm_analysis.keys())
                                                    if llm_analysis
                                                    else []
                                                )
                                                logger.debug(
                                                    f"[LIST_INVESTIGATIONS] Domain '{domain_name}' llm_analysis keys: {llm_keys}"
                                                )
                            except Exception as debug_error:
                                logger.debug(
                                    f"[LIST_INVESTIGATIONS] Error in debug logging: {debug_error}"
                                )
                        ...
                        if isinstance(domain_findings, dict) and domain_findings:
                            try:
                                ...
                                logger.debug(
                                    f"[LIST_INVESTIGATIONS] Processing progress_json domain_findings for {db_inv_data['investigation_id']}: {domain_keys_list}"
                                )
                            except Exception as key_error:
                                logger.debug(
                                    f"[LIST_INVESTIGATIONS] Error getting domain_findings keys: {key_error}"
                                )
                                domain_keys_list = []
                            ...
                            if domain_findings.get("device"):
                                ...
                                if isinstance(device_data, dict):
                                    ...
                                    if (
                                        "risk_score" in device_data
                                        and device_data["risk_score"] is not None
                                    ):
                                        device_risk_score = float(
                                            device_data["risk_score"]
                                        )
                                        logger.debug(
                                            f"[LIST_INVESTIGATIONS] Extracted device_risk_score from progress_json: {device_risk_score}"
                                        )
                                    elif "metrics" in device_data and isinstance(
                                        device_data["metrics"], dict
                                    ):
                                        if (
                                            "avg_model_score" in device_data["metrics"]
                                            and device_data["metrics"][
                                                "avg_model_score"
                                            ]
                                            is not None
                                        ):
                                            ...
                                            logger.debug(
                                                f"[LIST_INVESTIGATIONS] Extracted device_risk_score from metrics.avg_model_score: {device_risk_score}"
                                            )
                                    ...
                                    progress_llm_thoughts = (
                                        device_data.get("llm_response_text")
                                        or (
                                            llm_analysis.get("reasoning")
                                            if isinstance(llm_analysis, dict)
                                            and llm_analysis.get("reasoning")
                                            else None
                                        )
                                        or (
                                            llm_analysis.get("llm_response")
                                            if isinstance(llm_analysis, dict)
                                            and llm_analysis.get("llm_response")
                                            else None
                                        )
                                        or device_data.get("llm_thoughts")
                                        or device_data.get("reasoning")
                                        or None
                                    )
                                    ...
                                    if progress_llm_thoughts:
                                        ...
                                        logger.debug(
                                            f"[LIST_INVESTIGATIONS] Extracted device_llm_thoughts from progress_json: length={len(device_llm_thoughts) if device_llm_thoughts else 0}"
                                        )
                            ...
                            if domain_findings.get("location"):
                                ...
                                if isinstance(location_data, dict):
                                    # Extract risk score from progress_json - check risk_score first, then metrics.avg_model_score
                                    if (
                                        "risk_score" in location_data
                                        and location_data["risk_score"] is not None
                                    ):
                                        location_risk_score = float(
                                            location_data["risk_score"]
                                        )
                                        logger.debug(
                                            f"[LIST_INVESTIGATIONS] Extracted location_risk_score from progress_json: {location_risk_score}"
                                        )
                                    elif "metrics" in location_data and isinstance(
                                        location_data["metrics"], dict
                                    ):
                                        if (
                                            "avg_model_score"
                                            in location_data["metrics"]
                                            and location_data["metrics"][
                                                "avg_model_score"
                                            ]
                                            is not None
                                        ):
                                            ...
                                            logger.debug(
                                                f"[LIST_INVESTIGATIONS] Extracted location_risk_score from metrics.avg_model_score: {location_risk_score}"
                                            )
                                    ...
                                    progress_llm_thoughts = (
                                        location_data.get("llm_response_text")
                                        or (
                                            llm_analysis.get("reasoning")
                                            if isinstance(llm_analysis, dict)
                                            and llm_analysis.get("reasoning")
                                        ...
                                        or location_data.get("llm_thoughts")
                                        or location_data.get("reasoning")
                                        or None
                                    )
                                    ...
                                    if progress_llm_thoughts:
                                        ...
                                        logger.debug(
                                            f"[LIST_INVESTIGATIONS] Extracted location_llm_thoughts from progress_json: length={len(location_llm_thoughts) if location_llm_thoughts else 0}"
                                        )
                            ...
                            if domain_findings.get("network"):
                                ...
                                if isinstance(network_data, dict):
                                    # Extract risk score from progress_json - check risk_score first, then metrics.avg_model_score
                                    if (
                                        "risk_score" in network_data
                                        and network_data["risk_score"] is not None
                                    ):
                                        network_risk_score = float(
                                            network_data["risk_score"]
                                        )
                                        logger.debug(
                                            f"[LIST_INVESTIGATIONS] Extracted network_risk_score from progress_json: {network_risk_score}"
                                        )
                                    elif "metrics" in network_data and isinstance(
                                        network_data["metrics"], dict
                                    ):
                                        if (
                                            "avg_model_score" in network_data["metrics"]
                                            and network_data["metrics"][
                                                "avg_model_score"
                                            ]
                                            is not None
                                        ):
                                            ...
                                            logger.debug(
                                                f"[LIST_INVESTIGATIONS] Extracted network_risk_score from metrics.avg_model_score: {network_risk_score}"
                                            )
                                    ...
                                    progress_llm_thoughts = (
                                        network_data.get("llm_response_text")
                                        or (
                                            llm_analysis.get("reasoning")
                                            if isinstance(llm_analysis, dict)
                                            and llm_analysis.get("reasoning")
                                        ...
                                        or network_data.get("llm_thoughts")
                                        or network_data.get("reasoning")
                                        or None
                                    )
                                    # Use progress_json value - only if not None
                                    if progress_llm_thoughts:
                                        ...
                                        logger.debug(
                                            f"[LIST_INVESTIGATIONS] Extracted network_llm_thoughts from progress_json: length={len(network_llm_thoughts) if network_llm_thoughts else 0}"
                                        )
                            ...
                            if domain_findings.get("logs"):
                                ...
                                if isinstance(logs_data, dict):
                                    # Extract risk score from progress_json - check risk_score first, then metrics.avg_model_score
                                    if (
                                        "risk_score" in logs_data
                                        and logs_data["risk_score"] is not None
                                    ):
                                        logs_risk_score = float(logs_data["risk_score"])
                                        logger.debug(
                                            f"[LIST_INVESTIGATIONS] Extracted logs_risk_score from progress_json: {logs_risk_score}"
                                        )
                                    elif "metrics" in logs_data and isinstance(
                                        logs_data["metrics"], dict
                                    ):
                                        if (
                                            "avg_model_score" in logs_data["metrics"]
                                            and logs_data["metrics"]["avg_model_score"]
                                            is not None
                                        ):
                                            logs_risk_score = float(
                                                logs_data["metrics"]["avg_model_score"]
                                            )
                                            logger.debug(
                                                f"[LIST_INVESTIGATIONS] Extracted logs_risk_score from metrics.avg_model_score: {logs_risk_score}"
                                            )
                                    ...
                                    progress_llm_thoughts = (
                                        logs_data.get("llm_response_text")
                                        or (
                                            llm_analysis.get("reasoning")
                                            if isinstance(llm_analysis, dict)
                                            and llm_analysis.get("reasoning")
                                        ...
                                        or logs_data.get("llm_thoughts")
                                        or logs_data.get("reasoning")
                                        or None
                                    )
                                    # Use progress_json value - only if not None
                                    if progress_llm_thoughts:
                                        ...
                                        logger.debug(
                                            f"[LIST_INVESTIGATIONS] Extracted logs_llm_thoughts from progress_json: length={len(logs_llm_thoughts) if logs_llm_thoughts else 0}"
                                        )
                ...
                device_risk_score_final = (
                    float(device_risk_score) if device_risk_score is not None else None
                )
                location_risk_score_final = (
                    float(location_risk_score)
                    if location_risk_score is not None
                    else None
                )
                network_risk_score_final = (
                    float(network_risk_score)
                    if network_risk_score is not None
                    else None
                )
                logs_risk_score_final = (
                    float(logs_risk_score) if logs_risk_score is not None else None
                )
                overall_risk_score_final = (
                ...
                if overall_risk_score_final is not None:
                    logger.debug(
                        f"[LIST_INVESTIGATIONS] Investigation {db_inv_data['investigation_id']} overall_risk_score={overall_risk_score_final} extracted from progress_json"
                    )
                else:
                    logger.debug(
                        f"[LIST_INVESTIGATIONS] Investigation {db_inv_data['investigation_id']} has no overall_risk_score in progress_json"
                    )

                # Log extracted values for debugging (only log if we found data)
                if (
                    device_risk_score_final is not None
                    or location_risk_score_final is not None
                    or network_risk_score_final is not None
                    or logs_risk_score_final is not None
                ):
                    logger.debug(
                        f"[LIST_INVESTIGATIONS] Investigation {db_inv_data['investigation_id']} extracted risk scores: device={device_risk_score_final}, location={location_risk_score_final}, network={network_risk_score_final}, logs={logs_risk_score_final}"
                    )

                # Safely check LLM thoughts before logging lengths
                device_llm_safe = device_llm_thoughts if device_llm_thoughts else ""
                location_llm_safe = (
                    location_llm_thoughts if location_llm_thoughts else ""
                )
                ...
                if (
                    device_llm_thoughts
                    or location_llm_thoughts
                    or network_llm_thoughts
                    or logs_llm_thoughts
                ):
                    logger.debug(
                        f"[LIST_INVESTIGATIONS] Investigation {db_inv_data['investigation_id']} extracted LLM thoughts lengths: device={len(device_llm_safe)}, location={len(location_llm_safe)}, network={len(network_llm_safe)}, logs={len(logs_llm_safe)}"
                    )
                ...
                if location_llm_thoughts or network_llm_thoughts or logs_llm_thoughts:
                    logger.debug(
                        f"[LIST_INVESTIGATIONS] Creating Investigation {db_inv_data['investigation_id']} with LLM thoughts: location={len(location_llm_safe)}, network={len(network_llm_safe)}, logs={len(logs_llm_safe)}"
                    )
                ...
                investigation_dict = {
                    ...
                    "user_id": db_inv_data[
                        "user_id"
                    ],  # This comes from DB column, not a fallback
                    "status": investigation_status,
                    "policy_comments": policy_comments if policy_comments else "",
                    "investigator_comments": (
                        investigator_comments if investigator_comments else ""
                    ),
                    ...
                    ),  # CRITICAL: Include domain_findings so map_investigation_to_transactions can extract risk.risk_score
                }
                ...
                try:
                    investigation = Investigation(
                        ...
                        entity_id=(
                            investigation_dict["entity_id"]
                            if investigation_dict["entity_id"] is not None
                            else ""
                        ),
                        entity_type=(
                            investigation_dict["entity_type"]
                            if investigation_dict["entity_type"] is not None
                            else ""
                        ),
                        user_id=investigation_dict["user_id"],
                        status=investigation_dict["status"],
                        policy_comments=investigation_dict["policy_comments"],
                        investigator_comments=investigation_dict[
                            "investigator_comments"
                        ],
                        overall_risk_score=(
                            investigation_dict["overall_risk_score"]
                            if investigation_dict["overall_risk_score"] is not None
                            else 0.0
                        ),
                        device_llm_thoughts=(
                            investigation_dict["device_llm_thoughts"]
                            if investigation_dict["device_llm_thoughts"] is not None
                            else ""
                        ),
                        location_llm_thoughts=(
                            investigation_dict["location_llm_thoughts"]
                            if investigation_dict["location_llm_thoughts"] is not None
                            else ""
                        ),
                        network_llm_thoughts=(
                            investigation_dict["network_llm_thoughts"]
                            if investigation_dict["network_llm_thoughts"] is not None
                            else ""
                        ),
                        logs_llm_thoughts=(
                            investigation_dict["logs_llm_thoughts"]
                            if investigation_dict["logs_llm_thoughts"] is not None
                            else ""
                        ),
                        device_risk_score=(
                            investigation_dict["device_risk_score"]
                            if investigation_dict["device_risk_score"] is not None
                            else 0.0
                        ),
                        location_risk_score=(
                            investigation_dict["location_risk_score"]
                            if investigation_dict["location_risk_score"] is not None
                            else 0.0
                        ),
                        network_risk_score=(
                            investigation_dict["network_risk_score"]
                            if investigation_dict["network_risk_score"] is not None
                            else 0.0
                        ),
                        logs_risk_score=(
                            investigation_dict["logs_risk_score"]
                            if investigation_dict["logs_risk_score"] is not None
                            else 0.0
                        ),
                    )
                    ...
                    if investigation:
                        try:
                            loc_thoughts = (
                                getattr(investigation, "location_llm_thoughts", None)
                                or ""
                            )
                            net_thoughts = (
                                getattr(investigation, "network_llm_thoughts", None)
                                or ""
                            )
                            logs_thoughts = (
                                getattr(investigation, "logs_llm_thoughts", None) or ""
                            )
                            ...
                            if loc_thoughts or net_thoughts or logs_thoughts:
                                logger.debug(
                                    f"[LIST_INVESTIGATIONS] Investigation {investigation.id} created successfully with LLM thoughts: location={len(loc_thoughts)}, network={len(net_thoughts)}, logs={len(logs_thoughts)}"
                                )
                        except Exception as log_error:
                            logger.warning(
                                f"[LIST_INVESTIGATIONS] Error logging investigation LLM thoughts: {log_error}"
                            )
                except Exception as inv_create_error:
                    logger.warning(
                        f"[LIST_INVESTIGATIONS] Failed to create Investigation object for {investigation_dict.get('id', 'unknown')}: {inv_create_error}"
                    )
                    investigation = None
                ...
                investigation_id = (
                    db_inv_data.get("investigation_id", f"unknown-{idx}")
                    if db_inv_data
                    else f"unknown-{idx}"
                )
                ...
                logger.error(
                    f"[LIST_INVESTIGATIONS] Failed to convert investigation {investigation_id} (index {idx}): {error_type}: {error_msg}"
                )
        ...
        if investigations:
            logger.debug(
                f"[LIST_INVESTIGATIONS] Successfully converted {len(investigations)} investigations"
            )
        ...
        logger.debug(
            "[LIST_INVESTIGATIONS] No investigations converted, returning in-memory store"
        )
        return list(IN_MEMORY_INVESTIGATIONS.values())
        ...
        logger = get_bridge_logger(__name__)
        logger.error(
            f"[LIST_INVESTIGATIONS] Unexpected error: {e}, falling back to in-memory store"
        )
