...
from sqlalchemy import create_engine, TypeDecorator, JSON, String, text
...
def init_database():
    ...
    logger.info(f"Database initialized with URL: {database_url[:50]}...")  # Don't log full URL with password
...
def ensure_required_tables_exist():
    ...
    try:
        ...
        if missing_tables:
            logger.info(f"📋 Creating {len(missing_tables)} missing table(s): {missing_tables}")
