...
class RAGColumnMigration:
    ...
    async def rename_column(self, table_name: str, old_name: str, new_name: str) -> bool:
        ...
        try:
            ...
            if not await self.check_column_exists(table_name, old_name):
                logger.info(f"Column {old_name} does not exist in {table_name} (may already be migrated)")
                return True
            
            # Check if new column already exists
            if await self.check_column_exists(table_name, new_name):
                logger.warning(f"Column {new_name} already exists in {table_name} - skipping migration")
                return True
            ...
            if self.is_postgresql:
                ...
                async with self.db_config.session() as session:
                    ...
                    logger.info(f"✅ Renamed {old_name} to {new_name} in {table_name} (PostgreSQL)")
                    return True
            else:
                ...
                with sync_engine.connect() as conn:
                    ...
                    conn.execute(query)
                    conn.commit()
                    logger.info(f"✅ Renamed {old_name} to {new_name} in {table_name} (SQLite)")
                    return True
                    
        except Exception as e:
            logger.error(f"❌ Failed to rename column {old_name} to {new_name} in {table_name}: {e}")
            return False
    ...
    async def run_migration(self) -> bool:
        ...
        try:
            ...
            success2 = await self.rename_column("document_chunks", "metadata", "meta_data")
    ...
    async def verify_migration(self) -> dict:
        ...
        try:
            ...
            verification["documents_table"]["old_column_exists"] = await self.check_column_exists("documents", "metadata")
            verification["documents_table"]["new_column_exists"] = await self.check_column_exists("documents", "meta_data")
            verification["documents_table"]["migrated"] = (
                not verification["documents_table"]["old_column_exists"] and
                verification["documents_table"]["new_column_exists"]
            )
            
            # Check document_chunks table
            verification["document_chunks_table"]["old_column_exists"] = await self.check_column_exists("document_chunks", "metadata")
            verification["document_chunks_table"]["new_column_exists"] = await self.check_column_exists("document_chunks", "meta_data")
            verification["document_chunks_table"]["migrated"] = (
                not verification["document_chunks_table"]["old_column_exists"] and
                verification["document_chunks_table"]["new_column_exists"]
            )
            
            verification["all_migrated"] = (
                verification["documents_table"]["migrated"] and
                verification["document_chunks_table"]["migrated"]
            )
            ...
            if verification["all_migrated"]:
                ...
                logger.warning(f"   Document chunks table: {verification['document_chunks_table']}")
