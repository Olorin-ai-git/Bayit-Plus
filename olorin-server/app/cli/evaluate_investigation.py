...
def parse_entity_type(value: str) -> str:
    ...
    valid_types = [
        'email', 'phone', 'device_id', 'ip',
        'account_id', 'card_fingerprint', 'merchant_id'
    ]
...
Examples:
  ...
  python -m app.cli.evaluate_investigation \\
    ...
    parser.add_argument(
        '--entity-value',
        type=str,
        help='Entity value (normalized automatically)'
    )
    ...
    parser.add_argument(
        '--merchant-ids',
        nargs='+',
        help='Merchant IDs to filter (space-separated)'
    )
    ...
    parser.add_argument(
        '--include-histograms',
        action='store_true',
        help='Include risk histograms'
    )
    parser.add_argument(
        '--include-timeseries',
        action='store_true',
        help='Include daily timeseries'
    )
...
async def main():
    ...
    if not args.entity_type and not args.merchant_ids:
        parser.error("Either --entity-type/--entity-value or --merchant-ids must be specified")

    # Build request
    request = ComparisonRequest(
        entity={
            'type': args.entity_type,
            'value': args.entity_value
        } if args.entity_type and args.entity_value else None,
    ...
    try:
        if not args.quiet:
            ...
            logger.info(f"Entity: {args.entity_type}={args.entity_value}" if args.entity_type else "Entity: Global")
        ...
        if not args.quiet:
            ...
            print(f"  TP: {response.A.TP}, FP: {response.A.FP}, TN: {response.A.TN}, FN: {response.A.FN}")
            print(f"  Precision: {response.A.precision:.2%}, Recall: {response.A.recall:.2%}, F1: {response.A.f1:.2%}")
            print(f"  Accuracy: {response.A.accuracy:.2%}, Fraud rate: {response.A.fraud_rate:.2%}")
            ...
            print(f"  TP: {response.B.TP}, FP: {response.B.FP}, TN: {response.B.TN}, FN: {response.B.FN}")
            print(f"  Precision: {response.B.precision:.2%}, Recall: {response.B.recall:.2%}, F1: {response.B.f1:.2%}")
            print(f"  Accuracy: {response.B.accuracy:.2%}, Fraud rate: {response.B.fraud_rate:.2%}")
            if response.B.pending_label_count:
                print(f"  Pending labels: {response.B.pending_label_count}")

            print(f"\nDeltas (B - A):")
            print(f"  Precision: {response.delta.precision:+.2%}, Recall: {response.delta.recall:+.2%}")
            print(f"  F1: {response.delta.f1:+.2%}, Accuracy: {response.delta.accuracy:+.2%}")
            ...
            artifact_path = response.metadata.get('artifact_path') if hasattr(response, 'metadata') else None
            if artifact_path:
