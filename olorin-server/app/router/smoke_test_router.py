...
async def run_smoke_tests(
    ...
    try:
        ...
        return JSONResponse(
            ...
            status_code=200 if not report.has_critical_failures else 206  # 206 = Partial Content for warnings
        )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Smoke tests failed: {str(e)}"
        )
...
async def list_services():
    ...
    try:
        ...
        return {
            ...
            "critical_services": len([s for s in services if s["severity"] == "critical" and s["enabled"]])
        }
    ...
    try:
        ...
        if os.getenv("USE_SNOWFLAKE", "false").lower() == "true":
            ...
            validation_results.append({
                ...
                "message": "Snowflake configuration valid" if not missing_snowflake else f"Missing: {', '.join(missing_snowflake)}"
            })
        
        # Check threat intelligence API keys
        api_services = [
            ("abuseipdb", "ABUSEIPDB_API_KEY", ["USE_ABUSEIPDB_IP_REPUTATION", "USE_ABUSEIPDB_BULK_ANALYSIS"]),
            ("virustotal", "VIRUSTOTAL_API_KEY", ["USE_VIRUSTOTAL_IP_ANALYSIS", "USE_VIRUSTOTAL_DOMAIN_ANALYSIS"]),
            ("shodan", "SHODAN_API_KEY", ["USE_SHODAN_INFRASTRUCTURE", "USE_SHODAN_SEARCH"])
        ]
        
        for service_name, api_key_var, enable_vars in api_services:
            enabled = any(os.getenv(var, "false").lower() == "true" for var in enable_vars)
            ...
            if enabled:
                ...
                validation_results.append({
                    ...
                    "enabled": True,
                    "valid": bool(api_key and len(api_key) > 10),  # Basic length check
                    "missing_config": [] if api_key else [api_key_var],
                    "message": f"{service_name.title()} API key configured" if api_key else f"Missing {api_key_var}"
                })
        ...
        if os.getenv("USE_SPLUNK", "false").lower() == "true":
            ...
            validation_results.append({
                ...
                "message": "Splunk configuration valid" if not missing_splunk else f"Missing: {', '.join(missing_splunk)}"
            })
