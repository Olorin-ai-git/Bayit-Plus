...
class QueryValidationMetrics(BaseModel):
    ...
    expression_length: int = Field(
        description="Length of Boolean expression in characters"
    )
    complexity_score: float = Field(description="Overall complexity score")
    complexity_level: QueryComplexityLevelSchema = Field(
...
class QueryValidationErrorResponse(BaseModel):
    ...
    complexity_metrics: QueryValidationMetrics = Field(
...
class MultiEntityMetricsResponse(BaseModel):
    ...
    completed_investigations: int = Field(
        description="Successfully completed investigations"
    )
    ...
    complexity_distribution: Dict[str, int] = Field(
        description="Distribution of query complexity levels"
    )
...
class EnhancedEntityTypesResponse(BaseModel):
    ...
    core_entity_types: List[str] = Field(
        description="Core entity types for basic investigations"
    )
    transaction_entity_types: List[str] = Field(
        description="Transaction-specific entity types"
    )
    extended_entity_types: List[str] = Field(
        description="Extended entity types for advanced use cases"
    )
    all_entity_types: List[str] = Field(
        description="Complete list of all available entity types"
    )
    total_types: int = Field(description="Total number of entity types available")
    type_descriptions: Dict[str, EntityTypeInfo] = Field(
        description="Detailed descriptions for each type"
    )
...
class HealthCheckResponse(BaseModel):
    ...
    websocket_connections_count: int = Field(
        description="Number of active WebSocket connections"
    )
    multi_entity_investigations: int = Field(
        description="Number of active multi-entity investigations"
    )
