...
class WindowSpec(BaseModel):
    ...
    preset: WindowPreset = Field(
        ...,
        description="Window preset type"
    )
    start: Optional[datetime] = Field(
        ...
        None,
        description="Custom end time (ISO 8601) - required if preset is 'custom'"
    )
    label: Optional[str] = Field(
        None,
        description="Custom label for the window"
    )
...
class ComparisonOptions(BaseModel):
    ...
    include_per_merchant: bool = Field(
        True,
        description="Include per-merchant breakdown"
    )
    max_merchants: int = Field(
        25,
        ge=1,
        le=1000,
        description="Maximum number of merchants in breakdown"
    )
    include_histograms: bool = Field(
        False,
        description="Include risk histogram data"
    )
    include_timeseries: bool = Field(
        False,
        description="Include daily timeseries data"
    )
...
class ComparisonRequest(BaseModel):
    ...
    windowA: WindowSpec = Field(
        ...,
        description="Window A specification"
    )
    windowB: WindowSpec = Field(
        ...,
        description="Window B specification"
    )
    ...
    merchant_ids: Optional[List[str]] = Field(
        None,
        description="Optional list of merchant IDs to filter"
    )
...
class ConfusionMatrix(BaseModel):
    ...
    entity_type: str = Field(..., description="Entity type (e.g., 'email', 'device_id', 'ip')")
    entity_id: str = Field(..., description="Entity identifier")
    investigation_id: Optional[str] = Field(None, description="Investigation ID if available")
    
    TP: int = Field(0, ge=0, description="True Positives: predicted Fraud AND IS_FRAUD_TX = 1")
    FP: int = Field(0, ge=0, description="False Positives: predicted Fraud AND IS_FRAUD_TX = 0")
    TN: int = Field(0, ge=0, description="True Negatives: predicted Not Fraud AND IS_FRAUD_TX = 0")
    FN: int = Field(0, ge=0, description="False Negatives: predicted Not Fraud AND IS_FRAUD_TX = 1")
    
    excluded_count: int = Field(0, ge=0, description="Transactions with NULL IS_FRAUD_TX excluded from confusion matrix")
    
    precision: float = Field(0.0, ge=0.0, le=1.0, description="TP / (TP + FP), or 0.0 if TP + FP = 0")
    recall: float = Field(0.0, ge=0.0, le=1.0, description="TP / (TP + FN), or 0.0 if TP + FN = 0")
    f1_score: float = Field(0.0, ge=0.0, le=1.0, description="2 * (precision * recall) / (precision + recall), or 0.0 if precision + recall = 0")
    accuracy: float = Field(0.0, ge=0.0, le=1.0, description="(TP + TN) / (TP + FP + TN + FN), or 0.0 if total = 0")
    
    investigation_risk_score: Optional[float] = Field(None, ge=0.0, le=1.0, description="Risk score from investigation")
    risk_threshold: float = Field(..., ge=0.0, le=1.0, description="Threshold used for classification (RISK_THRESHOLD_DEFAULT)")
    total_transactions: int = Field(0, ge=0, description="Total transactions in investigation window")
...
class PowerAssessment(BaseModel):
    ...
    reasons: List[str] = Field(default_factory=list, description="Reasons for low power status")
...
class WindowMetrics(BaseModel):
    ...
    brier: Optional[float] = Field(None, ge=0.0, description="Brier score for calibration (lower is better)")
    log_loss: Optional[float] = Field(None, ge=0.0, description="Log loss for calibration (lower is better)")
    source: Optional[str] = Field(None, description="Investigation ID or 'fallback' if no investigation")
    ci: Optional[Dict[str, Optional[Tuple[float, float]]]] = Field(
        None, 
        description="Confidence intervals for precision, recall, accuracy"
    )
    support: Optional[SupportMetrics] = Field(None, description="Support metrics for power assessment")
    power: Optional[PowerAssessment] = Field(None, description="Power/stability assessment")
...
class AutoExpandMetadata(BaseModel):
    ...
    reasons: List[str] = Field(default_factory=list, description="Reasons expansion stopped")
    effective_support: Optional[Dict[str, int]] = Field(None, description="Support metrics after expansion")
...
class WindowInfo(BaseModel):
    ...
    auto_expand_meta: Optional[AutoExpandMetadata] = Field(
        None, 
        description="Metadata about auto-expansion if enabled"
    )
...
class DeltaMetrics(BaseModel):
    ...
    psi_predicted_risk: Optional[float] = Field(None, description="PSI statistic for predicted_risk distribution drift")
    ks_stat_predicted_risk: Optional[float] = Field(None, description="KS statistic for predicted_risk distribution comparison")
...
class FraudClassification(BaseModel):
    ...
    predicted_label: str = Field(..., description="Predicted label: 'Fraud' or 'Not Fraud'")
    predicted_risk: Optional[float] = Field(None, ge=0.0, le=1.0, description="Investigation risk score")
    actual_label: Optional[str] = Field(None, description="Actual label from IS_FRAUD_TX: 'Fraud' or 'Not Fraud', or None if NULL")
    actual_outcome: Optional[int] = Field(None, description="IS_FRAUD_TX value: 1, 0, or None")
    confusion_category: Optional[str] = Field(None, description="Confusion category: 'TP', 'FP', 'TN', 'FN', or None if excluded")
...
class AggregatedConfusionMatrix(BaseModel):
    ...
    total_excluded: int = Field(0, ge=0, description="Sum of excluded transactions across all entities")
    
    aggregated_precision: float = Field(0.0, ge=0.0, le=1.0, description="total_TP / (total_TP + total_FP)")
    aggregated_recall: float = Field(0.0, ge=0.0, le=1.0, description="total_TP / (total_TP + total_FN)")
    aggregated_f1_score: float = Field(0.0, ge=0.0, le=1.0, description="2 * (precision * recall) / (precision + recall)")
    aggregated_accuracy: float = Field(0.0, ge=0.0, le=1.0, description="(total_TP + total_TN) / (total_TP + total_FP + total_TN + total_FN)")
    
    entity_matrices: List[ConfusionMatrix] = Field(default_factory=list, description="Individual entity confusion matrices")
    
    entity_count: int = Field(0, ge=0, description="Number of entities analyzed")
    risk_threshold: float = Field(..., ge=0.0, le=1.0, description="Threshold used for classification")
    calculation_timestamp: datetime = Field(..., description="When metrics were calculated")
...
class ComparisonResponse(BaseModel):
    ...
    per_merchant: Optional[List[PerMerchantMetrics]] = Field(
        None,
        description="Per-merchant breakdown"
    )
    excluded_missing_predicted_risk: Optional[int] = Field(
        None,
        ge=0,
        description="Transactions excluded due to missing predicted_risk"
    )
