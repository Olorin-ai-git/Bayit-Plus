...
async def execute_structured_investigation(
    ...
    """
    logger.info(
        f"⚙️ BACKGROUND TASK STARTED: execute_structured_investigation for {investigation_id}"
    )
        ...
        with get_db_session() as db:
            ...
            if state:
                ...
                if state.status not in (
                    ...
                    logger.info(
                        f"✅ Set investigation {investigation_id} to IN_PROGRESS at executor start (was: {state.status})"
                    )
                elif state.status == "IN_PROGRESS":
                    logger.info(
                        f"ℹ️  Investigation {investigation_id} already in IN_PROGRESS status"
                    )
                else:
                    logger.warning(
                        f"⚠️  Investigation {investigation_id} is already {state.status}, skipping IN_PROGRESS update"
                    )
    except Exception as status_error:
        logger.warning(
            f"⚠️ Failed to set IN_PROGRESS status at executor start: {str(status_error)}"
        )
    ...
    try:
        ...
        if request.entity_id == "risk-based-auto-select":
            logger.info(
                f"🔍 Risk-based investigation detected - auto-selecting entities for {investigation_id}"
            )
            ...
            try:
                ...
                if results.get("status") == "critical_no_data":
                    critical_msg = results.get(
                        "message",
                        "CRITICAL: No entities found in any time window. Nothing to investigate.",
                    )
                    ...
                    logger.critical(
                        f"🚨 Investigation {investigation_id} terminated with CRITICAL/ERROR status: No data to investigate"
                    )
                    return {
                        "investigation_id": investigation_id,
                        "status": "CRITICAL",
                ...
                logger.info(
                    f"✅ Auto-selected entity for {investigation_id}: {entity_type}={entity_value[:50]}..."
                )
                ...
                logger.error(f"❌ {error_msg}")
                logger.exception(
                    f"Full auto-selection error traceback for investigation {investigation_id}:"
                )
                ...
                try:
                    ...
                    logger.info(
                        f"✅ Persisted auto-selection error status for investigation {investigation_id}"
                    )
                except Exception as update_error:
                    # If update_investigation_status fails, try direct database update as fallback
                    logger.error(
                        f"❌ Failed to update investigation status via update_investigation_status: {str(update_error)}"
                    )
                    ...
                    try:
                        ...
                        with get_db_session() as db:
                            state = (
                                db.query(InvestigationState)
                                .filter(
                                    InvestigationState.investigation_id
                                    == investigation_id
                                )
                                .first()
                            )
                            ...
                            if state:
                                ...
                                progress_data = (
                                    json.loads(state.progress_json)
                                ...
                                db.commit()
                                logger.info(
                                    f"✅ Persisted auto-selection error status directly to database for investigation {investigation_id}"
                                )
                            else:
                                logger.error(
                                    f"❌ Investigation {investigation_id} not found in database for error persistence"
                                )
                    except Exception as db_error:
                        logger.error(
                            f"❌ CRITICAL: Failed to persist auto-selection error even with direct DB update: {str(db_error)}"
                        )
        ...
        logger.info(
            f"✅ Verified entity information: {request.entity_type}={request.entity_id[:50]}..."
        )
        ...
        try:
            ...
            logger.info(
                f"🛡️ Created RecursionGuard context for investigation {investigation_id} with thread_id {thread_id}"
            )
        ...
        logger.info(
            f"✅ Verification complete - execution can proceed for {investigation_id}"
        )
        ...
        with get_db_session() as db:
            ...
            if state:
                ...
                if state.status != "IN_PROGRESS":
                    ...
                    logger.info(
                        f"✅ Updated investigation {investigation_id} to IN_PROGRESS after verification (was: {state.status})"
                    )
                else:
                    logger.info(
                        f"ℹ️  Investigation {investigation_id} already in IN_PROGRESS status"
        ...
        update_investigation_status(
            investigation_id,
            {"status": "in_progress", "current_phase": "agent_initialization"},
        )
        ...
        await execute_context_preparation_phase(
            investigation_id, investigation_context, request
        )

        # Execute the main agent investigation
        from .investigation_executor_core import _execute_agent_investigation_phase

        result = await _execute_agent_investigation_phase(
            investigation_id, investigation_context, request, thread_id
        )
        ...
        try:
            ...
            logger.info(
                f"🛡️ Cleaned up RecursionGuard context for investigation {investigation_id}"
            )
        ...
        logger.exception(
            f"Full exception traceback for investigation {investigation_id}:"
        )
        ...
        try:
            ...
            logger.info(
                f"🛡️ Cleaned up RecursionGuard context after failure for investigation {investigation_id}"
            )
        except Exception as cleanup_error:
            logger.warning(
                f"Failed to cleanup RecursionGuard context after failure: {cleanup_error}"
            )
        ...
        try:
            ...
            logger.info(
                f"✅ Persisted error status for investigation {investigation_id}"
            )
        except Exception as update_error:
            # If update_investigation_status fails, try direct database update as fallback
            logger.error(
            ...
            try:
                ...
                with get_db_session() as db:
                    ...
                    if state:
                        ...
                        db.commit()
                        logger.info(
                            f"✅ Persisted error status directly to database for investigation {investigation_id}"
                        )
                    else:
                        logger.error(
                            f"❌ Investigation {investigation_id} not found in database for error persistence"
                ...
                logger.error(
                    f"❌ CRITICAL: Failed to persist error even with direct DB update: {str(db_error)}"
                )
                logger.exception("Exception during direct database update:")
