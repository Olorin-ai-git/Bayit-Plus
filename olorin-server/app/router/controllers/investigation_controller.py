    ...
    try:
        ...
        try:
            ...
            metadata = {
                ...
                "entity_id": request.entity_id,
                "entity_type": request.entity_type,
                "investigation_type": getattr(request, 'investigation_type', 'structured'),
            ...
            logger.error(f"Failed to start investigation logging for {investigation_id}: {e}", exc_info=True)
...
def update_investigation_status(investigation_id: str, updates: Dict[str, Any]) -> None:
    ...
    try:
        ...
        with get_db_session() as db:
            ...
            if not state:
                logger.warning(f"Investigation {investigation_id} not found in database - cannot update")
                return
            ...
            if investigation_id in active_investigations:
                ...
                logger.debug(f"Investigation {investigation_id} exists in database but not in active_investigations dict (database-backed investigation)")
            ...
            if investigation_completed and investigation_id in _active_investigation_handlers:
                try:
                    ...
                    event_type = 'completion' if status_upper == 'COMPLETED' else ('cancellation' if status_upper == 'CANCELLED' else 'failure')
                    log_manager.log_lifecycle_event(investigation_id, event_type, {'status': status_upper})
                    ...
                    logger.error(f"Failed to stop investigation logging for {investigation_id}: {e}", exc_info=True)
            
            # Generate confusion table when investigation completes
            if investigation_completed and updates.get("status", "").upper() == "COMPLETED":
                try:
                    ...
                    confusion_table_path = generate_confusion_table_sync(investigation_id, investigation_folder)
                    if confusion_table_path:
                        logger.info(f"✅ Confusion table generated for investigation {investigation_id}: {confusion_table_path}")
                    else:
                        logger.debug(f"📊 Confusion table generation scheduled for investigation {investigation_id}")
                except Exception as e:
                    logger.warning(f"⚠️ Failed to generate confusion table for investigation {investigation_id}: {e}", exc_info=True)
            ...
            if "status" in updates:
                ...
                valid_statuses = ("CREATED", "SETTINGS", "IN_PROGRESS", "COMPLETED", "ERROR", "CANCELLED")
                if new_status not in valid_statuses:
                    logger.warning(f"Invalid status '{new_status}' (from '{raw_status}'). Skipping status update.")
                    new_status = None

                if new_status and new_status != state.status:
                    state.status = new_status
                    # Only update lifecycle_stage if the new status is a valid lifecycle value
                    if new_status in ("CREATED", "SETTINGS", "IN_PROGRESS", "COMPLETED"):
            ...
            if "findings_summary" in updates:
                try:
                    ...
                    if "domain_findings" not in findings_summary and progress_data.get("domain_findings"):
                        findings_summary["domain_findings"] = progress_data["domain_findings"]
                        logger.debug(f"Added domain_findings to findings_summary for {investigation_id}")
                    ...
                    if "risk_score" in findings_summary and findings_summary["risk_score"] is not None:
                        ...
                        logger.debug(f"Set overall_risk_score={findings_summary['risk_score']} from risk_score for investigation {investigation_id}")
                    elif progress_data.get("risk_score") is not None:
                        # Fallback: use risk_score from progress_data if not in findings_summary
                        findings_summary["overall_risk_score"] = progress_data["risk_score"]
                        logger.debug(f"Set overall_risk_score={progress_data['risk_score']} from progress_data for investigation {investigation_id}")
                    
                    # Merge findings_summary into progress_data
                    progress_data.update(findings_summary)
                    logger.debug(f"Updated progress_json with findings_summary for investigation {investigation_id}")
                except Exception as json_error:
                    logger.error(f"Failed to merge findings_summary into progress_json: {str(json_error)}", exc_info=True)
                        ...
                        if risk_score_from_domain is not None:
                            try:
                                ...
                                if 0 <= risk_score_float <= 1.0:
                                    ...
                                    progress_data["overall_risk_score"] = risk_score_float
                                    logger.info(
            ...
            if "findings_summary" in updates:
                logger.debug(f"Committed progress_json with findings_summary for investigation {investigation_id}")
            ...
            try:
                ...
                if action_type in ["PROGRESS", "PHASE_CHANGE"]:
                    enhanced_payload.update({
                        ...
                        "tool_executions_count": len(progress_data.get("tool_executions", [])),
                        "completed_tools": sum(1 for te in progress_data.get("tool_executions", []) if te.get("status") == "completed"),
                        "domain_findings_count": len(progress_data.get("domain_findings", {})),
                        "domains_completed": list(progress_data.get("domain_findings", {}).keys())
                    })
                ...
                if action_type == "COMPLETED":
                    enhanced_payload.update({
                        ...
                        "total_domains": len(progress_data.get("domain_findings", {})),
                        "has_results": bool(state.progress_json and any(key in progress_data for key in ['risk_score', 'findings', 'domain_findings']))
                    })
                    # Include risk score if available
                    # Results are now in progress_json
                    if progress_data.get('risk_score') is not None or progress_data.get('findings') or progress_data.get('domain_findings'):
                        try:
                            results_data = progress_data
                            if "risk_score" in results_data:
                                enhanced_payload["final_risk_score"] = results_data["risk_score"]
                        except:
                            pass
                ...
                logger.debug(f"Created audit entry for investigation {investigation_id}: {action_type}")
            except Exception as audit_error:
                logger.error(f"Failed to create audit entry: {str(audit_error)}", exc_info=True)
            ...
            if isinstance(updates, dict):
                ...
                if isinstance(findings_summary, dict):
                    ...
                    logger.info(f"✅ Investigation {investigation_id} status updated: {status} | Completed: {completion_time}")
            else:
                logger.info(f"✅ Investigation {investigation_id} status updated: {updates}")

    except Exception as e:
        logger.error(f"Failed to persist investigation status update: {str(e)}", exc_info=True)
