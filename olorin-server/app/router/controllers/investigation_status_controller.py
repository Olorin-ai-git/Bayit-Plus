...
async def get_investigation_status(
    investigation_id: str, active_investigations: Dict[str, Dict[str, Any]]
) -> InvestigationStatusResponse:
    """
    ...
    start_time = datetime.fromisoformat(
        investigation["start_time"].replace("Z", "+00:00")
    )
...
async def get_investigation_logs(investigation_id: str) -> InvestigationLogsResponse:
    ...
    try:
        ...
        log_summary = structured_investigation_logger.generate_investigation_summary(
            investigation_id
        )

        # Get specific interaction types
        all_logs = structured_investigation_logger.get_investigation_logs(
            investigation_id
        )
...
async def get_investigation_journey(investigation_id: str) -> LangGraphJourneyResponse:
    ...
    try:
        ...
        journey_visualization = journey_tracker.generate_journey_visualization(
            investigation_id
        )
        ...
        response = LangGraphJourneyResponse(
            ...
            performance_analytics={
                ...
                "agent_count": len(
                    set(
                        node.get("agent_name")
                        for node in journey_visualization.get("nodes", [])
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve journey: {str(e)}"
        )
