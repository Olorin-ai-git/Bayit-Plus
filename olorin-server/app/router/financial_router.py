...
def _extract_entity_info(state) -> Tuple[str, str, str]:
    """Extract entity_type, entity_value, and merchant_name from state settings."""
    entity_type = "unknown"
    entity_value = ""
    merchant_name = "Unknown"

    settings_json = getattr(state, "settings_json", None)
    if not settings_json:
        return entity_type, entity_value, merchant_name
    ...
    try:
        ...
        if entities and len(entities) > 0:
            ...
            if isinstance(first_entity, dict):
                entity_type = first_entity.get("entity_type") or "unknown"
                entity_value = first_entity.get("entity_value") or ""
            else:
                entity_type = getattr(first_entity, "entity_type", "unknown") or "unknown"
                entity_value = getattr(first_entity, "entity_value", "") or ""

        merchant_name = settings.get("merchant_name") or "Unknown"
        if merchant_name == "Unknown":
            metadata = settings.get("metadata", {})
            if isinstance(metadata, dict):
                merchant_name = metadata.get("merchant_name") or "Unknown"
        if merchant_name == "Unknown" and entity_type == "MERCHANT_NAME":
            merchant_name = entity_value or "Unknown"
...
@router.get(
    ...
    description="Returns summary and list of all completed investigations with financial data",
)
async def get_financial_dashboard(
    db: Session = Depends(get_db),
    current_user: User = Depends(require_read),
):
    """Get dashboard data with summary and investigation list for authenticated user."""
    service = InvestigationStateService(db)
    completed_states = service.get_completed_investigations_with_auth(current_user.username)

    total_saved, total_lost = Decimal("0"), Decimal("0")
    total_tp, total_fp, total_tn, total_fn = 0, 0, 0, 0
    precision_sum, recall_sum = 0.0, 0.0
    successful_count, precision_count = 0, 0
    investigations_list = []

    for state in completed_states:
        revenue = _extract_revenue_metrics(state)
        confusion = _extract_confusion_metrics(state)
        entity_type, entity_value, merchant_name = _extract_entity_info(state)
        ...
        inv_item = {
            ...
            "status": state.status,
            "saved_fraud_gmv": float(revenue.saved_fraud_gmv if revenue else 0),
            "lost_revenues": float(revenue.lost_revenues if revenue else 0),
            "net_value": float(revenue.net_value if revenue else 0),
            "precision": confusion.precision if confusion else None,
            "completed_at": state.updated_at.isoformat() if state.updated_at else "",
        }
        investigations_list.append(inv_item)

        if revenue and not revenue.skipped_due_to_prediction:
            total_saved += revenue.saved_fraud_gmv
            total_lost += revenue.lost_revenues
            successful_count += 1

        if confusion:
            total_tp += confusion.true_positives
            total_fp += confusion.false_positives
            total_tn += confusion.true_negatives
            total_fn += confusion.false_negatives
            precision_sum += confusion.precision
            recall_sum += confusion.recall
            precision_count += 1
    ...
    summary = {
        "total_saved_fraud_gmv": float(total_saved),
        "total_lost_revenues": float(total_lost),
        "total_net_value": float(total_saved - total_lost),
        ...
        } if precision_count > 0 else None,
        "investigation_count": len(completed_states),
        "successful_calculations": successful_count,
        "failed_calculations": len(completed_states) - successful_count,
        "aggregated_at": datetime.utcnow().isoformat(),
    }

    return {"summary": summary, "investigations": investigations_list}
