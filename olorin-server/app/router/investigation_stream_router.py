...
@router.get(
    ...
    responses={
        ...
        304: {
            "description": "Not Modified - ETag matches current state"
        },
...
async def get_investigation_events(
    ...
    if if_none_match:
        ...
        if if_none_match == current_etag:
            ...
            response.headers["X-Recommended-Interval"] = str(poll_interval * 1000)  # Convert to ms
    ...
    response.headers["Cache-Control"] = "no-cache, must-revalidate"
    response.headers["X-Recommended-Interval"] = str(events_response.poll_after_seconds * 1000)
