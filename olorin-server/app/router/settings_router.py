...
def get_default_agent_tools_mapping() -> Dict[str, List[str]]:
    ...
    try:
        ...
        logger.info(f"Assigned {len(all_available_tools)} tools to each of {len(agents)} agents by default")
...
class UserSettings(BaseModel):
    ...
    default_entity_type: str = Field(default="user_id", description="Default entity type for investigations")
    selected_agents: List[str] = Field(default_factory=get_default_agents, description="Default selected agents")
    comment_prefix: str = Field(default="", description="Prefix for comments")
    agent_tools_mapping: Dict[str, List[str]] = Field(default_factory=get_default_agent_tools_mapping, description="Mapping of agent to tools")
...
async def get_available_tools():
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Could not access tool registry: {e}. Using fallback tools.")
...
async def get_tools_with_display_names():
    ...
    try:
        ...
        try:
            ...
            # Fallback list
            fallback_tools = [
                "http_request", "json_api", "web_search", "splunk_query",
                "database_query", "file_read", "file_search", "vector_search"
            ]
            available_tools = [{"name": tool, "description": ""} for tool in fallback_tools]
        ...
        for tool_info in available_tools:
            ...
            display_name = display_names.get(tool_name, tool_name.replace("_", " ").title())
            
            tools_with_display.append(ToolDisplayInfo(
                name=tool_name,
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get tools with display names: {str(e)}")
...
async def get_agent_tools_mapping():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get agent tools mapping: {str(e)}")
...
async def import_settings(
    ...
    try:
        ...
        if "settings" not in import_data:
            raise HTTPException(status_code=400, detail="Invalid import data: missing 'settings' field")
        ...
        raise HTTPException(status_code=400, detail=f"Failed to import settings: {str(e)}")
...
async def get_tools_by_category():
    ...
    try:
        ...
        logger.info(f"Categorized tools: {len(olorin_tools)} Olorin tools, {len(mcp_tools)} MCP tools")
        return {
