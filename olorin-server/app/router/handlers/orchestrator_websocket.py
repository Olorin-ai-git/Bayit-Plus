...
from app.service.websocket_manager import websocket_manager, AgentPhase
from app.service.agent.agent_coordination import AgentType, HandoffTrigger, HandoffContext
from app.service.agent.orchestrator_state import get_orchestrator_state_manager, StateTransition
...
class OrchestratorWebSocketHandler:
    ...
    async def handle_websocket_connection(self, websocket: WebSocket, investigation_id: str):
        """
        ...
        try:
            ...
            while True:
                try:
                    ...
                    await self._handle_websocket_message(websocket, investigation_id, message)
    ...
    async def broadcast_orchestrator_decision(
        ...
        """
        logger.info(f"📡 Broadcasting orchestrator decision: {decision_type} for {investigation_id}")
        ...
        try:
            ...
            decision_event = DecisionEvent(
                ...
                expected_outcome=self._predict_decision_outcome(decision_type, decision_factors),
            ...
            orchestrator_event = OrchestratorEvent(
                ...
                reasoning=reasoning,
                confidence_score=confidence_score,
                impact_level=self._assess_decision_impact(decision_type, confidence_score),
    ...
    async def broadcast_agent_handoff(
        ...
        """
        logger.info(f"🔄 Broadcasting agent handoff: {handoff_context.from_agent.value} → {handoff_context.to_agent.value}")
    ...
    async def broadcast_investigation_milestone(
        ...
        """
        logger.info(f"🎯 Broadcasting investigation milestone: {milestone_type.value} ({progress_percentage:.1f}%)")
        ...
        try:
            ...
            next_milestones = self._predict_next_milestones(milestone_type, agents_status)
            ...
            orchestrator_event = OrchestratorEvent(
                ...
                confidence_score=self._calculate_milestone_confidence(progress_percentage, agents_status),
                impact_level=self._assess_milestone_impact(milestone_type),
                metadata={
                    "total_milestones_reached": len(self.milestone_tracker.get(investigation_id, [])),
                    "agents_active": len([s for s in agents_status.values() if s == "active"])
                }
            )
    ...
    async def broadcast_strategy_adaptation(
        ...
        """
        logger.info(f"🔧 Broadcasting strategy adaptation: {old_strategy} → {new_strategy}")
        ...
        try:
            ...
            orchestrator_event = OrchestratorEvent(
                ...
                event_data={
                    ...
                    "strategy_effectiveness": self._assess_strategy_effectiveness(new_strategy)
                },
                ...
                metadata={
                    ...
                    "strategy_change_count": self._get_strategy_change_count(investigation_id)
                }
            )
        ...
        try:
            ...
            orchestrator_event = OrchestratorEvent(
                ...
                event_data={
                    ...
                    "progress_velocity": self._calculate_progress_velocity(investigation_id, progress_percentage),
                ...
                confidence_score=self._calculate_progress_confidence(progress_percentage, active_agents),
                impact_level="medium",
                metadata={
    ...
    async def _send_initial_state(self, websocket: WebSocket, investigation_id: str):
        ...
        try:
            ...
            summary = await self.state_manager.get_investigation_summary(investigation_id)
    ...
    async def _handle_websocket_message(self, websocket: WebSocket, investigation_id: str, message: str):
    ...
    def _buffer_event(self, investigation_id: str, event: OrchestratorEvent):
        ...
        if len(self.event_buffer[investigation_id]) > 50:
            self.event_buffer[investigation_id] = self.event_buffer[investigation_id][-50:]
    ...
    def _predict_decision_outcome(self, decision_type: str, factors: Dict[str, Any]) -> str:
    ...
    def _assess_decision_impact(self, decision_type: str, confidence: float) -> str:
        ...
        if confidence > 0.8 and decision_type in ["strategy_selection", "critical_handoff"]:
            return "high"
        elif confidence > 0.6:
            return "medium"
        else:
    ...
    def _predict_next_milestones(self, current_milestone: MilestoneType, agents_status: Dict[str, str]) -> List[str]:
        """Predict next investigation milestones"""
        active_agents = sum(1 for status in agents_status.values() if status == "active")
        completed_agents = sum(1 for status in agents_status.values() if status == "completed")
    ...
    def _calculate_milestone_confidence(self, progress: float, agents_status: Dict[str, str]) -> float:
        """Calculate confidence score for milestone"""
        base_confidence = progress / 100.0
        
        # Boost confidence if agents are performing well
        successful_agents = sum(1 for status in agents_status.values() if status in ["completed", "active"])
    ...
    def _assess_milestone_impact(self, milestone_type: MilestoneType) -> str:
        ...
        high_impact = [MilestoneType.INVESTIGATION_STARTED, MilestoneType.ALL_AGENTS_COMPLETE, MilestoneType.INVESTIGATION_FINALIZED]
    ...
    def _get_strategy_change_count(self, investigation_id: str) -> int:
        ...
        return sum(1 for event in events if event.event_type == OrchestratorEventType.STRATEGY_ADAPTED)
    
    def _calculate_progress_velocity(self, investigation_id: str, current_progress: float) -> float:
    ...
    def _calculate_progress_confidence(self, progress: float, active_agents: List[str]) -> float:
