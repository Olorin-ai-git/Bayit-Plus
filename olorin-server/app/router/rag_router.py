    ...
    try:
            ...
            try:
                ...
                if db_config.is_postgresql:
                    ...
                    with sync_engine.connect() as conn:
                        result = conn.execute(text("SELECT name FROM sqlite_master WHERE type='table' AND name='rag_data_sources'"))
        ...
        return {
            **status_info,
            ...
            "ready": status_info.get("ready_for_queries", False) and db_initialized and tables_exist
        }
    except Exception as e:
...
async def update_chat_session(
    ...
    try:
        ...
        meta_data_value = session.meta_data if hasattr(session, 'meta_data') and isinstance(session.meta_data, dict) else {}
...
@router.post("/chats/{session_id}/messages", response_model=ChatMessageResponse, status_code=status.HTTP_201_CREATED)
