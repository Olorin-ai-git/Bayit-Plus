...
async def get_investigation_events(
    ...
    since: Optional[str] = Query(
        None, description="Cursor for pagination (timestamp_sequence format)"
    ),
    limit: int = Query(
        DEFAULT_LIMIT, ge=1, le=MAX_LIMIT, description="Events per page"
    ),
    action_types: Optional[list[str]] = Query(
        None, description="Filter by action types"
    ),
    sources: Optional[list[str]] = Query(
        None, description="Filter by sources (UI, API, SYSTEM, WEBHOOK, POLLING)"
    ),
    user_ids: Optional[list[str]] = Query(None, description="Filter by user IDs"),
    since_timestamp: Optional[int] = Query(
        None, description="Events after this timestamp (ms)"
    ),
    until_timestamp: Optional[int] = Query(
        None, description="Events before this timestamp (ms)"
    ),
    if_none_match: Optional[str] = Header(
        None, alias="If-None-Match", description="ETag for conditional requests"
    ),
    ...
    **ETag Support**: Pass `If-None-Match` header with previous ETag
    ...
    if if_none_match:
        ...
        if if_none_match == etag:
            # Log 304 response
            logger.info(
                f"📊 /events endpoint response construction for {investigation_id}:"
            )
            ...
            logger.info(
                f"   ⏱️  Total elapsed: {(time.time() - start_time) * 1000:.1f}ms"
            )
    ...
    for i, event in enumerate(result.items[:5]):
        ...
        if len(str(event.payload)) > 200:
            logger.info(
                f"         - Payload full length: {len(str(event.payload))} chars"
            )
    ...
    if domain_findings_events:
        logger.info(
            f"   🎯 Domain findings events found: {len(domain_findings_events)}"
        )
        for i, event in enumerate(domain_findings_events[:3]):
            ...
            logger.info(
                f"         - Risk score: {event.payload.get('risk_score', 'N/A')}"
            )
            logger.info(
                f"         - Confidence: {event.payload.get('confidence', 'N/A')}"
            )
            logger.info(
                f"         - Has LLM analysis: {event.payload.get('has_llm_analysis', False)}"
            )
            if event.payload.get("llm_response_preview"):
                logger.info(
                    f"         - LLM preview: {event.payload.get('llm_response_preview')[:100]}..."
                )
    ...
    if tool_execution_events:
        ...
        for i, event in enumerate(tool_execution_events[:3]):
            ...
            logger.info(
                f"         - Agent: {event.payload.get('agent_type', 'unknown')}"
            )
            logger.info(
                f"         - Duration: {event.payload.get('execution_time_ms', 0)}ms"
            )
...
async def get_complete_audit_log(
    ...
    # Get all events
    all_events = service.get_audit_trail_summary(
        investigation_id, current_user.username
    )
