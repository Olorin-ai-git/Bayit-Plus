...
async def liveness_probe():
    ...
    """
    return LivenessProbe(alive=True, timestamp=get_current_timestamp())
...
async def startup_probe():
    ...
    return StartupProbe(
        started=True, timestamp=get_current_timestamp(), startup_time_seconds=uptime
    )
...
async def detailed_health(request: Request):
    ...
    if request and hasattr(request, "app") and hasattr(request.app, "state"):
        ...
        if hasattr(request.app.state, "database_provider_connected"):
            services["database_provider"] = (
                request.app.state.database_provider_connected
            )
        ...
        if hasattr(request.app.state, "anomaly_detection_available"):
            services["anomaly_detection"] = (
                request.app.state.anomaly_detection_available
            )
        ...
        if hasattr(request.app.state, "detection_scheduler"):
            services["detection_scheduler"] = (
                request.app.state.detection_scheduler is not None
            )
    ...
    all_dependencies_healthy = all(
        isinstance(dep, bool) and dep
        for dep in dependencies.values()
        if isinstance(dep, bool)
    ) and all(
        all(svc.values()) for svc in dependencies.values() if isinstance(svc, dict)
    )
