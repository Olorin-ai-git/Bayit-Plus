...
async def execute_autonomous_investigation(
    ...
    try:
        ...
        journey_tracker.track_node_execution(
            ...
            duration_ms=75,
            status=NodeStatus.COMPLETED,
            metadata={"phase_type": "context_preparation", "query_length": len(investigation_query) if 'investigation_query' in locals() else 0}
        )
        ...
        # Location Analysis Agent Node
        journey_tracker.track_node_execution(
            investigation_id=investigation_id,
            node_name="location_analysis_agent",
            node_type=NodeType.AGENT,
            input_state={"location_data": "loaded", "travel_history": "available", "impossible_travel_detected": True},
            output_state={"location_analysis": "in_progress", "travel_impossibility": "confirmed"},
            duration_ms=0,
            status=NodeStatus.IN_PROGRESS,
            agent_name="LocationAnalysisAgent",
            metadata={"analysis_type": "location_travel_patterns", "risk_level": "critical", "confidence": 90.0, "distance_miles": 2572.8}
        )
        ...
        # Network Analysis Agent Node
        journey_tracker.track_node_execution(
            investigation_id=investigation_id,
            node_name="network_analysis_agent",
            node_type=NodeType.AGENT,
            input_state={"network_data": "loaded", "ip_reputation": "suspicious", "vpn_detected": True},
            output_state={"network_analysis": "in_progress", "vpn_anomaly": "detected"},
            duration_ms=0,
            status=NodeStatus.IN_PROGRESS,
            agent_name="NetworkAnalysisAgent",
            metadata={"analysis_type": "network_reputation_vpn", "risk_level": "high", "confidence": 88.0, "vpn_provider": "NordVPN"}
        )
        ...
        # Logs Analysis Agent Node
        journey_tracker.track_node_execution(
            investigation_id=investigation_id,
            node_name="logs_analysis_agent",
            node_type=NodeType.AGENT,
            input_state={"session_logs": "loaded", "behavioral_data": "available", "velocity_metrics": "high"},
            output_state={"logs_analysis": "in_progress", "behavioral_anomalies": "detected"},
            duration_ms=0,
            status=NodeStatus.IN_PROGRESS,
            agent_name="LogsAnalysisAgent",
            metadata={"analysis_type": "behavioral_velocity", "risk_level": "high", "confidence": 92.0, "transaction_velocity": "abnormally_high"}
        )
        ...
        journey_tracker.track_node_execution(
            ...
            node_type=NodeType.AGENT,
            input_state={"all_agent_findings": "available", "fraud_indicators": "multiple", "convergence_analysis": "ready"},
            output_state={"risk_assessment": "in_progress", "final_score": "calculating"},
            duration_ms=0,
            status=NodeStatus.IN_PROGRESS,
            agent_name="RiskAssessmentAgent",
            metadata={"analysis_type": "fraud_risk_synthesis", "risk_level": "critical", "confidence": 85.0, "indicators_count": 4}
        )
