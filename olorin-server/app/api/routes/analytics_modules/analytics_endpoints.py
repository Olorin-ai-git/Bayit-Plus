...
class AnalyticsEndpoints:
    ...
    async def get_top_risk_entities(
        ...
        top_percentage: Optional[float] = Query(
            None, description="Top percentage (1-100)"
        ),
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Risk analysis failed: {str(e)}"
            )

    async def post_top_risk_entities(
        self, request: Request, body: RiskAnalyticsRequest
    ):
    ...
    async def analyze_entity(self, body: EntityAnalysisRequest):
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Entity analysis failed: {str(e)}"
            )
    ...
    async def get_metrics(
        ...
        include_throughput: bool = Query(
            True, description="Include throughput metrics"
        ),
    ):
