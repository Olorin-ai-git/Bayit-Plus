...
from fastapi import APIRouter, HTTPException, Query, Request, WebSocket, WebSocketDisconnect, Depends, BackgroundTasks
...
from app.models.analytics import AnalyticsDashboardResponse, FraudMetrics, PrecisionRecallResponse
...
async def create_experiment(experiment_data: dict):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Create experiment failed: {str(e)}")
...
async def update_experiment(experiment_id: str, updates: dict):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Update experiment failed: {str(e)}")


@router.post("/experiments/{experiment_id}/promote")
async def promote_experiment(
    experiment_id: str,
    variant_id: str = Query(..., description="Winning variant ID")
):
    """Promote winning variant to production."""
    try:
        experiment = await experiment_manager.promote_experiment(experiment_id, variant_id)
        return experiment
    except Exception as e:
        logger.error(f"Promote experiment endpoint error: {e}")
        raise HTTPException(status_code=500, detail=f"Promote experiment failed: {str(e)}")
...
async def get_experiment_results(
    ...
    try:
        start_dt = datetime.fromisoformat(start_date)
        end_dt = datetime.fromisoformat(end_date)
        return await experiment_manager.get_experiment_results(experiment_id, start_dt, end_dt)
    except Exception as e:
        logger.error(f"Experiment results endpoint error: {e}")
        raise HTTPException(status_code=500, detail=f"Experiment results failed: {str(e)}")
...
async def check_experiment_guardrails(experiment_id: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Check guardrails failed: {str(e)}")
...
async def detect_drift(
    ...
    try:
        ...
        return await drift_detector.detect_drift(feature, ref_start, ref_end, curr_start, curr_end)
    except Exception as e:
...
async def list_replay_scenarios(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"List replay scenarios failed: {str(e)}")
...
async def create_replay_scenario(request: Request):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Create replay scenario failed: {str(e)}")
...
async def get_replay_scenario(scenario_id: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Get replay scenario failed: {str(e)}")
...
async def run_replay_scenario(scenario_id: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Run replay scenario failed: {str(e)}")
...
async def get_replay_scenario_results(scenario_id: str):
    ...
    try:
        ...
        if not results:
            raise HTTPException(status_code=404, detail="Replay scenario results not found")
        return results
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Get replay scenario results endpoint error: {e}")
        raise HTTPException(status_code=500, detail=f"Get replay scenario results failed: {str(e)}")
...
async def get_cohort_top_drivers(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Cohort top drivers failed: {str(e)}")
...
async def check_slo_violations():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"SLO violations check failed: {str(e)}")
...
async def get_audit_logs(
    ...
    try:
        start_dt = datetime.fromisoformat(start_date) if start_date else None
        end_dt = datetime.fromisoformat(end_date) if end_date else None
        logs = await pipeline_monitor.get_audit_logs(start_dt, end_dt, action_type, user_id, limit)
...
async def export_data(request: Request):
    ...
    try:
        ...
        return {"message": "Export functionality coming soon", "format": body.get("format")}
    except Exception as e:
...
async def detect_anomalies(request: DetectRequest):
    ...
    if request.window_from >= request.window_to:
        raise HTTPException(
            status_code=400,
            detail="window_from must be before window_to"
        )
    
    # Check window size (prevent excessive ranges)
    window_duration = (request.window_to - request.window_from).total_seconds()
    max_window_seconds = int(os.getenv("ANOMALY_DETECTION_MAX_WINDOW_SECONDS", "2592000"))  # 30 days default
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Snowflake connection error during detection: {e}", exc_info=True)
            raise HTTPException(
                status_code=503,
...
async def list_anomalies(
    ...
    try:
        ...
        total = query.count()
        anomalies = query.order_by(AnomalyEvent.created_at.desc()).offset(offset).limit(limit).all()
        ...
        if detector_id_filter:
            try:
                ...
                await websocket.send_json({
                    "error": "Invalid detector_id format",
                    "type": "error"
                })
            ...
            try:
                ...
                try:
                    ...
                    if last_anomaly_time:
                        query = query.filter(AnomalyEvent.created_at > last_anomaly_time)
                    
                    # Get new anomalies, ordered by creation time
                    new_anomalies = query.order_by(AnomalyEvent.created_at.asc()).limit(100).all()
    ...
    try:
        ...
        if not cohorts:
            ...
            if entities:
                ...
                entity_value = entities[0].get('entity_value') or entities[0].get('entityValue')
        ...
        if time_range:
            ...
            if isinstance(time_range, dict):
                start_time = time_range.get('start_time') or time_range.get('start_date') or time_range.get('startTime') or time_range.get('startDate')
                end_time = time_range.get('end_time') or time_range.get('end_date') or time_range.get('endTime') or time_range.get('endDate')
                ...
                if start_time:
                    ...
                    if isinstance(start_time, str):
                        from datetime import datetime
                        try:
                            investigation_window_from = datetime.fromisoformat(start_time.replace('Z', '+00:00'))
                ...
                if end_time:
                    if isinstance(end_time, str):
                        from datetime import datetime
                        try:
                            investigation_window_to = datetime.fromisoformat(end_time.replace('Z', '+00:00'))
                        except (ValueError, AttributeError):
        ...
        detector_params = {
            ...
            'investigation_window_from': investigation_window_from.isoformat() if investigation_window_from else None,
            'investigation_window_to': investigation_window_to.isoformat() if investigation_window_to else None,
        }
...
async def bulk_delete_detectors(request: BulkDeleteRequest):
    ...
    try:
        if not request.detector_ids or len(request.detector_ids) == 0:
            raise HTTPException(status_code=400, detail="detector_ids list cannot be empty")
        ...
        logger.info(f"Bulk deleted {deleted_count} detectors. Not found: {len(not_found_ids)}")
        ...
        db.rollback()
        raise HTTPException(status_code=500, detail=f"Failed to bulk delete detectors: {str(e)}")
    finally:
        db.close()
...
async def preview_detector(detector_id: str, request: PreviewRequest):
    ...
    try:
        ...
        detection_run_to_delete = db.query(DetectionRun).filter(
        ...
        db.rollback()
        raise HTTPException(status_code=500, detail=f"Failed to preview detector: {str(e)}")
    finally:
        db.close()
...
async def get_sample_cohort_values(
    dimension: str = Query(..., description="Cohort dimension name (e.g., merchant_id, channel, geo)"),
    limit: int = Query(10, description="Number of sample values to return", ge=1, le=100)
):
    """
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Found {len(values)} sample values for dimension '{dimension}' (column: {actual_column})")
            ...
            logger.warning(f"Failed to query sample cohort values for {dimension}: {e}", exc_info=True)
            return {
                "dimension": dimension,
                "values": [],
                "count": 0
        ...
        logger.warning(f"Unexpected error fetching sample cohort values: {e}", exc_info=True)
        return {
            "dimension": dimension,
            "values": [],
            "count": 0
...
class ReplayRequest(PydanticBaseModel):
    ...
    detector_config: Dict[str, Any]  # Detector configuration (without id, created_at, updated_at)
    window_from: datetime
    window_to: datetime
    production_detector_id: Optional[str] = None  # Optional: compare against specific production detector
...
async def replay_detection(request: ReplayRequest):
    ...
    try:
        ...
        if request.production_detector_id:
            ...
            db = next(get_db())
            try:
                production_detector = db.query(Detector).filter(
                    Detector.type == detector_config.type,
        ...
        try:
            ...
            logger.error(f"Snowflake connection error during replay: {e}", exc_info=True)
            raise HTTPException(
                status_code=503,
                detail=f"Data source unavailable: {str(e)}"
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Replay detection failed: {str(e)}"
        )
...
async def promote_detector(detector_id: str):
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=500, detail=f"Failed to promote detector: {str(e)}")
    finally:
        db.close()
    ...
    try:
        ...
        if is_entityless:
            ...
            db_state = db.query(InvestigationStateModel).filter(
                InvestigationStateModel.investigation_id == investigation_state.investigation_id
            ).first()
...
async def trigger_startup_analysis(
    ...
    try:
        ...
        logger.info(f"🚀 Triggering startup analysis flow for top {top_n} entities (manual trigger)...")
        ...
        async def run_analysis():
            try:
                ...
                logger.info(f"✅ Startup analysis flow completed: {len(comparison_results)} investigations")
            except Exception as e:
