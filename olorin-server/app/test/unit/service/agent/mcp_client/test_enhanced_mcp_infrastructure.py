...
class TestEnhancedMCPClient:
    ...
    async def test_server_registration(self, enhanced_mcp_client):
        ...
        await enhanced_mcp_client.register_server(
            server_name="test_server", endpoint="stdio://test", priority=1
        )

        assert "test_server" in enhanced_mcp_client.server_configs
        assert (
            enhanced_mcp_client.server_configs["test_server"]["endpoint"]
            == "stdio://test"
        )
        assert enhanced_mcp_client.server_configs["test_server"]["priority"] == 1

    @patch(
        "app.service.agent.mcp_client.enhanced_mcp_client.EnhancedMCPClient._execute_mcp_call"
    )
    async def test_tool_execution_with_caching(self, mock_execute, enhanced_mcp_client):
        ...
        await enhanced_mcp_client.register_server(
            server_name="test_server", endpoint="stdio://test"
        )
            ...
            for _ in range(4):
                try:
                    await enhanced_mcp_client.execute_tool(
                        server_name="test_server", tool_name="test_tool", params={}
                    )
                except Exception:
                    pass
...
class TestMCPConnectionPool:
    ...
    async def test_connection_acquisition_and_release(self, connection_pool):
        ...
        with patch(
            "app.service.agent.mcp_client.mcp_connection_pool.MCPConnectionPool._establish_mcp_connection",
...
class TestMCPHealthMonitor:
    ...
    async def test_server_registration(self, health_monitor):
        ...
        health_monitor.register_server(
            server_name="test_server", service_type="test_service", priority=1
        )
    ...
    async def test_health_check_execution(self, health_monitor):
        ...
        health_monitor.register_server(
            server_name="test_server", service_type="test_service"
        )

        # Mock health check methods
        with (
            patch.object(health_monitor, "_check_response_time", return_value=0.1),
    ...
    async def test_failover_trigger(self, health_monitor):
        ...
        health_monitor.register_server(
            server_name="primary_server", service_type="test_service", priority=2
        )
        health_monitor.register_server(
            server_name="backup_server", service_type="test_service", priority=1
        )

        # Mock unhealthy conditions
        with (
            patch.object(health_monitor, "_check_response_time", return_value=10.0),
...
class TestMCPMetricsCollector:
    ...
    def test_counter_operations(self, metrics_collector):
        ...
        assert (
            metrics_collector.get_counter_value("test_counter", {"server": "test"})
            == 2.0
        )
    ...
    def test_request_tracking(self, metrics_collector):
        ...
        metrics_collector.track_request(
            "server1", "tool_call", success=True, response_time=0.5
        )

        # Track failed request
        metrics_collector.track_request(
            "server1", "tool_call", success=False, response_time=1.0
        )
    ...
    def test_cache_tracking(self, metrics_collector):
        ...
        # Check counters
        assert (
            metrics_collector.get_counter_value(
                "mcp_cache_operations", {"operation": "get", "result": "hit"}
            )
            == 1
        )
        assert (
            metrics_collector.get_counter_value(
                "mcp_cache_operations", {"operation": "get", "result": "miss"}
            )
            == 1
        )
        assert (
            metrics_collector.get_counter_value(
                "mcp_cache_operations", {"operation": "set", "result": "miss"}
            )
            == 1
        )
    ...
    def test_circuit_breaker_tracking(self, metrics_collector):
        ...
        assert (
            metrics_collector.get_gauge_value(
                "mcp_circuit_breaker_state", {"server": "server1"}
            )
            == 0.0
        )
        ...
        assert (
            metrics_collector.get_counter_value(
                "mcp_circuit_breaker_transitions",
                {"server": "server1", "state": "open"},
            )
            == 1
        )
        assert (
    ...
    def test_all_metrics_export(self, metrics_collector):
        ...
        metrics_collector.track_request(
            "server1", "test", success=True, response_time=0.2
        )
...
class TestIntegration:
    ...
    async def test_end_to_end_tool_execution(self, full_infrastructure):
        ...
        with patch(
            ...
            assert (
                metrics["server_stats"]["integration_test_server"]["total_requests"]
                >= 1
            )
