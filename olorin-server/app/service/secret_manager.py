...
class SecretManagerClient:
    ...
    def _initialize_client(self):
        ...
        try:
            ...
            logger.info(
                f"✅ Secret Manager client initialized for project: {self.project_id}"
            )
        except Exception as e:
            logger.error(
                f"❌ Failed to initialize Secret Manager client - Firebase Secrets Manager is required: {e}"
            )
            raise ValueError(
                f"Firebase Secret Manager client initialization failed: {e}"
            )
    ...
    def get_secret(self, secret_name: str, version: str = "latest") -> Optional[str]:
        ...
        if cache_key in self._cache:
            ...
            if time.time() < expiry_time:
                ...
                logger.debug(
                    f"Using cached secret {firebase_secret_name} (value: {masked_value}, TTL remaining: {int(expiry_time - time.time())}s)"
                )
                return cached_value
            else:
        ...
        if not self._client:
            logger.error(
                "No Secret Manager client available - Firebase connection required"
            )
            raise ValueError(
        ...
        try:
            ...
            logger.info(
                f"🔐 Attempting to retrieve secret from Firebase Secret Manager..."
            )
            logger.info(f"   Project: {self.project_id}")
            ...
            logger.debug(
                f"Successfully retrieved secret {firebase_secret_name} from Secret Manager (value: {masked_value})"
            )
            return secret_value

        except google_exceptions.NotFound:
            logger.warning(
                f"Secret not found in Secret Manager: {firebase_secret_name} (original: {secret_name}) in project {self.project_id}"
            )
            return None

        except google_exceptions.PermissionDenied:
            logger.error(
                f"Permission denied accessing secret {firebase_secret_name} (original: {secret_name}) in project {self.project_id}"
            )
            return None

        except google_exceptions.DeadlineExceeded:
            logger.error(
                f"⏰ Timeout retrieving secret {firebase_secret_name} (original: {secret_name}) from Secret Manager"
            )
            logger.error(
                "   This usually indicates network connectivity issues or service unavailability"
            )
            return None

        except Exception as e:
            logger.error(
                f"❌ Error retrieving secret {firebase_secret_name} (original: {secret_name}) from Secret Manager: {e}"
            )
    ...
    def get_cache_stats(self) -> dict:
        ...
        active_entries = sum(
            1 for _, (_, expiry) in self._cache.items() if expiry > current_time
        )
