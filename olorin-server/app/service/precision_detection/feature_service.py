    ...
    def get_transaction_features(self, txn_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            with get_db_session() as db:
                ...
                if "mv_features_txn" not in table_names and "mv_features_txn" not in view_names:
                    logger.debug("mv_features_txn view/table does not exist - precision detection tables not migrated yet")
                    return None
            ...
            if "no such table" in error_msg or "does not exist" in error_msg or "table" in error_msg and "not found" in error_msg:
                logger.debug(f"mv_features_txn view/table does not exist - precision detection tables not migrated yet")
                return None
            logger.error(f"Failed to get transaction features for {txn_id}: {e}", exc_info=True)
            raise
    ...
    def get_merchant_burst_signals(
        ...
        try:
            with get_db_session() as db:
                ...
                if "mv_burst_flags" not in table_names and "mv_burst_flags" not in view_names:
                    logger.debug("mv_burst_flags view/table does not exist - precision detection tables not migrated yet")
                    return None
                ...
                if not result:
                    logger.debug(f"Merchant burst signals not found for {merchant_id} on {date}")
                    return None
                ...
                return {
                    ...
                    "tiny_amt_rate": float(result.tiny_amt_rate) if result.tiny_amt_rate else 0.0,
                    "z_unique_cards_30d": float(result.z_uniq_cards) if result.z_uniq_cards else 0.0
                }
        ...
        try:
            ...
            with get_db_session() as db:
                ...
                if "pg_alerts" not in table_names:
                    logger.debug("pg_alerts table does not exist - precision detection tables not migrated yet")
                    return None
                
                query = text("SELECT score FROM pg_alerts WHERE txn_id = :txn_id ORDER BY created_at DESC LIMIT 1")
                result = db.execute(query, {"txn_id": txn_id}).fetchone()
                check_query_latency(query_start, threshold_ms=100.0)
                if not result:
            ...
            error_msg = str(e).lower()
            if "no such table" in error_msg or "does not exist" in error_msg or "table" in error_msg and "not found" in error_msg:
                logger.debug(f"pg_alerts table does not exist - precision detection tables not migrated yet")
                return None
