...
class ScoreCache:
    ...
    def set_cached_score(self, ip_address: str, score_data: Dict[str, Any]) -> bool:
        """
        ...
        try:
            ...
            success = self.cache_service.set(
                cache_key, cached_data, ttl=self.CACHE_TTL_SECONDS
            )

            if success:
                logger.debug(
                    f"Cached score for IP {ip_address} with TTL {self.CACHE_TTL_SECONDS}s"
                )
            else:
