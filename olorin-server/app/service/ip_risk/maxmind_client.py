...
class MaxMindClient:
    ...
    async def score_transaction(
        ...
        **additional_data: Any,
        ...
        try:
            ...
            payload = {"device": {"ip_address": ip_address}}
            ...
            if transaction_amount is not None and currency:
                payload["event"] = {"amount": transaction_amount, "currency": currency}
            ...
            logger.info(
                f"Calling MaxMind API for transaction {transaction_id}, IP {ip_address}"
            )
            ...
            if response.status_code == 200:
                    ...
                    "geolocation": (
                        {
                            ...
                            "region": (
                                ip_location.get("subdivisions", [{}])[0]
                                if ip_location.get("subdivisions")
                                else {}
                            ),
                ...
                raise MaxMindServerError(
                    f"MaxMind server error: {response.status_code}"
                )
            else:
    ...
    async def _persist_to_postgresql(self, score_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            await loop.run_in_executor(
                None, lambda: self._sync_persist_to_postgresql(score_data)
            )
        ...
        try:
            with get_db_session() as db:
                ...
                params = {
                    ...
                    "velocity_signals": json.dumps(
                        score_data.get("velocity_signals", {})
                    ),
                ...
                logger.info(
                    f"Persisted IP risk score to PostgreSQL: {score_data.get('transaction_id')}"
                )
    ...
    async def _publish_to_kafka(self, score_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            topic = (
                self.config_loader.load_secret("KAFKA_IP_RISK_TOPIC")
                or "ip_risk_scores"
            )
            ...
            success = await loop.run_in_executor(
                None, lambda: producer.publish_event(topic=topic, event=event, key=key)
            )

            if success:
                logger.debug(
                    f"Published IP risk score event to Kafka: {event['event_id']}"
                )
            else:
                logger.warning(
                    f"Failed to publish IP risk score event to Kafka: {event['event_id']}"
                )
    ...
    async def _get_abuseipdb_fallback(self, ip_address: str) -> Dict[str, Any]:
        ...
        try:
            ...
            reputation_response = await self._abuseipdb_client.check_ip_reputation(
                ip_address
            )
            ...
            if reputation_response.success and reputation_response.ip_info:
                            ...
                            "country": (
                                {
                                    ...
                                    "name": ip_info.country_name,
                                }
                                if ip_info.country_code
                                else None
                            )
                        }
                        if ip_info.country_code
                ...
                logger.info(
                    f"Using AbuseIPDB fallback for IP {ip_address}, risk_score={risk_score}"
                )
                return result
            else:
    ...
    async def score_transaction_with_fallback(
        self, transaction_id: str, ip_address: str, **kwargs: Any
    ) -> Dict[str, Any]:
        """
        ...
        try:
            return await self.score_transaction(
                transaction_id=transaction_id, ip_address=ip_address, **kwargs
            )
