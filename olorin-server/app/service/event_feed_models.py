...
class EventActor(BaseModel):
    ...
    type: str = Field(
        ..., pattern="^(USER|SYSTEM|WEBHOOK|POLLING)$", description="Actor type"
    )
...
class InvestigationEvent(BaseModel):
    ...
    ts: int = Field(
        ..., ge=0, description="Event timestamp in milliseconds since epoch"
    )
    op: str = Field(
        ...,
...
class EventsFeedResponse(BaseModel):
    ...
    has_more: bool = Field(
        default=False, description="Whether more events exist beyond this page"
    )
    poll_after_seconds: int = Field(
        default=5, ge=1, description="Recommended polling interval in seconds"
    )
    etag: str = Field(
        ...
        try:
            ...
            for i in range(len(timestamps) - 1):
                ...
                if ts1 > ts2 or (ts1 == ts2 and seq1 > seq2):
                    raise ValueError(
                        f"Events not ordered correctly: {ts1}_{seq1} > {ts2}_{seq2}"
                    )
...
class EventFilterParams(BaseModel):
    ...
    user_ids: Optional[List[str]] = Field(
        None, description="Filter by user IDs who performed actions"
    )
    since_timestamp: Optional[int] = Field(
        None, ge=0, description="Include events after this timestamp (ms since epoch)"
    )
    until_timestamp: Optional[int] = Field(
        None, ge=0, description="Include events before this timestamp (ms since epoch)"
    )
...
class AuditTrailSummary(BaseModel):
    ...
    action_counts: Dict[str, int] = Field(
        default_factory=dict, description="Event count by action type"
    )
    source_counts: Dict[str, int] = Field(
        default_factory=dict, description="Event count by source"
    )
    earliest_event_ts: Optional[int] = Field(
        None, ge=0, description="Timestamp of first event (ms)"
    )
    latest_event_ts: Optional[int] = Field(
        None, ge=0, description="Timestamp of last event (ms)"
    )
