...
from app.schemas.investigation_state import InvestigationStateResponse, InvestigationProgress, InvestigationSettings, InvestigationResults
...
class InvestigationStateAuthHelper:
    ...
    def get_state_with_auth(
        ...
        from app.models.progress_models import InvestigationProgress as ProgressModelsProgress
        ...
        try:
            ...
            if state.progress_json:
                try:
                    ...
                    if domain_findings:
                        logger.debug(f"   ✅ Extracted {len(domain_findings)} domain findings from progress_json: {list(domain_findings.keys())}")
                    else:
                        logger.debug(f"   ℹ️  No domain_findings found in progress_json")
                except Exception as e:
                    logger.warning(f"   ⚠️  Failed to extract domain_findings from progress_json: {str(e)}")
            ...
            if new_progress.tool_executions:
                for tool in new_progress.tool_executions:
                    ...
                    if tool.status in ["completed", "running"]:
                        ...
                        if agent_type and agent_type.lower() not in ["graph", "graph_agent", "unknown"]:
            ...
            logger.warning(f"Failed to build progress for investigation {investigation_id}: {str(e)}")
        ...
        response = InvestigationStateResponse.model_validate(state, from_attributes=True)
        ...
        logger.debug(f"   - Progress object: {'present' if response.progress else 'None'}")
        if response.progress:
            logger.debug(f"      Domain findings in progress: {len(response.progress.domain_findings) if response.progress.domain_findings else 0}")
            if response.progress.domain_findings:
                logger.debug(f"      Domains: {list(response.progress.domain_findings.keys())}")
        logger.debug(f"   - Settings object: {'present' if response.settings else 'None'}")
        logger.debug(f"✅ InvestigationStateResponse constructed for {investigation_id}")
