...
def apply_state_updates(
    state: InvestigationState, data: InvestigationStateUpdate
) -> Dict[str, Any]:
        ...
        if "risk_score" in progress_dict and progress_dict["risk_score"] is not None:
            ...
            try:
                ...
                if risk_score_float < 0 or risk_score_float > 1.0:
                    logger.error(
                        f"🚨 CRITICAL BUG: Invalid risk_score {risk_score_float} outside [0, 1] range!"
                    )
                    logger.error(
                        f"   This indicates domain scores are being incorrectly aggregated (likely summed instead of averaged)"
                    )
                    logger.error(f"   Investigation ID: {state.investigation_id}")
                    logger.error(
                        f"   This is a critical bug that must be fixed at the source - rejecting invalid score"
                    )
...
def _index_investigation_on_completion(state: InvestigationState):
    ...
    try:
        ...
        title = (
            settings_dict.get("name") or f"Investigation {state.investigation_id[:8]}"
        )
        ...
        registry.index_investigation(
            ...
            completed_at=(
                state.updated_at if state.status in ("COMPLETED", "completed") else None
            ),
            metadata={
        ...
        logger.warning(
            f"Failed to index investigation {state.investigation_id} in registry: {e}"
        )
...
def _generate_investigation_manifest_on_completion(state: InvestigationState):
    ...
    try:
        ...
        manifest = manifest_generator.generate_investigation_manifest(
            investigation_id=state.investigation_id,
            title=settings_dict.get("name")
            or f"Investigation {state.investigation_id[:8]}",
            investigation_type=settings_dict.get("investigation_type"),
            graph_type=settings_dict.get("graph_type") or settings_dict.get("graph"),
            trigger_source=settings_dict.get("trigger_source") or "unknown",
            status=state.status,
        ...
        logger = logging.getLogger(__name__)
        logger.warning(
            f"Failed to generate manifest for investigation {state.investigation_id}: {e}"
        )
    ...
    try:
        ...
        if state.progress_json:
            try:
                ...
                investigation_data["final_risk_score"] = progress_dict.get(
                    "overall_risk_score"
                ) or progress_dict.get("final_risk_score")

                # Extract tool usage
                investigation_data["tools_used"] = progress_dict.get("tools_used", 0)
                investigation_data["tool_results"] = progress_dict.get(
                    "tool_results", []
                )

                # Extract domain findings
                investigation_data["domain_findings"] = progress_dict.get(
                    "domain_findings", {}
                )
        ...
        if issues:
            ...
            logger.warning(
                f"Linter found issues for investigation {state.investigation_id}:\n{summary}"
            )
...
def _trigger_package_generation_on_completion(state: InvestigationState):
    ...
    try:
        ...
        investigation_type = settings_dict.get(
            "investigation_type"
        ) or settings_dict.get("trigger_source", "")
        is_auto_comparison = (
            investigation_type == "auto_comparison"
            or state.investigation_id.startswith("auto-comp-")
        ...
        logger = logging.getLogger(__name__)
        logger.info(
            f"📦 Auto-comparison investigation completed: {state.investigation_id}"
        )
        logger.info(
            "   Package generation will be handled by background task in _wait_and_create_final_zip_package"
        )
        ...
        logger = logging.getLogger(__name__)
        logger.warning(
            f"Failed to trigger package generation for investigation {state.investigation_id}: {e}"
        )
