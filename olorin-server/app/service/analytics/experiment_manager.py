...
class ExperimentManager:
    ...
    def __init__(self):
        ...
        logger.info(f"ExperimentManager initialized with {db_provider.upper()} provider")
    ...
    async def create_experiment(self, experiment_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with get_db_session() as db:
            ...
            db.execute(query, {
                ...
                'end_date': datetime.fromisoformat(experiment_data['endDate']) if experiment_data.get('endDate') else None,
    ...
    async def get_experiment(self, experiment_id: str) -> Optional[Dict[str, Any]]:
        ...
        with get_db_session() as db:
            ...
            """)
            variants_result = db.execute(variants_query, {'experiment_id': experiment_id})
        ...
        experiment['variants'] = [
            {
                **v,
                ...
                'metrics': json.loads(v.get('metrics', '{}')) if v.get('metrics') else None,
                'statisticalSignificance': json.loads(v.get('statistical_significance', '{}')) if v.get('statistical_significance') else None
            }
            for v in variants
        ]
        ...
        if result:
            experiment['results'] = {
                ...
                'impactEstimate': json.loads(result[0].get('impact_estimate', '{}')) if result[0].get('impact_estimate') else None
            }
    ...
    async def list_experiments(self, status: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        with get_db_session() as db:
            if status:
                query = text("SELECT * FROM experiments WHERE status = :status ORDER BY created_at DESC")
        ...
        return [
            {
                **exp,
                ...
                'startDate': exp.get('start_date').isoformat() if exp.get('start_date') else None,
                'endDate': exp.get('end_date').isoformat() if exp.get('end_date') else None,
                'createdAt': exp.get('created_at').isoformat() if exp.get('created_at') else None,
                'updatedAt': exp.get('updated_at').isoformat() if exp.get('updated_at') else None,
    ...
    async def update_experiment(self, experiment_id: str, updates: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'endDate' in updates:
            ...
            params['end_date'] = datetime.fromisoformat(updates['endDate']) if updates['endDate'] else None
        ...
        with get_db_session() as db:
            query = text(f"UPDATE experiments SET {', '.join(set_clauses)} WHERE experiment_id = :experiment_id")
            db.execute(query, params)

        return await self.get_experiment(experiment_id)

    async def promote_experiment(self, experiment_id: str, variant_id: str) -> Dict[str, Any]:
        """
        ...
        with get_db_session() as db:
            ...
            check_query = text("SELECT result_id FROM experiment_results WHERE experiment_id = :experiment_id")
            existing = db.execute(check_query, {'experiment_id': experiment_id}).fetchone()
    ...
    async def get_experiment_results(
        ...
        experiment_col = 'EXPERIMENT_ID' if db_provider == 'snowflake' else 'experiment_id'
        ...
        for row in results:
            ...
            metrics = await self.metrics_calc.calculate_metrics(
                start_date, end_date, {'experiment_id': experiment_id, 'variant_id': variant_id}
            )
    ...
    def calculate_lift(self, control_metrics: Dict[str, Any], treatment_metrics: Dict[str, Any]) -> float:
        """
    ...
    def calculate_statistical_significance(
        ...
        t_stat, p_value = stats.ttest_ind(control_array, treatment_array, equal_var=False)
        ...
        margin = stats.t.ppf(1 - alpha/2, min(len(control_array), len(treatment_array)) - 1) * se_diff
    ...
    async def check_guardrails(self, experiment_id: str) -> List[Dict[str, Any]]:
        ...
        results = await self.get_experiment_results(
            ...
            datetime.utcnow() if not experiment.get('end_date') else datetime.fromisoformat(experiment['end_date'])
        )
        ...
        for guardrail in guardrails:
            ...
            for variant in results['variants']:
                ...
                if metric == 'conversion_rate':
                    ...
                    current_value = variant_metrics.get('modelLatency', {}).get('p95', 0)
                elif metric == 'manual_review_rate':
                    review_count = variant_metrics.get('totalDecisions', 0) - variant_metrics.get('approvedCount', 0) - variant_metrics.get('declinedCount', 0)
                    current_value = review_count / variant_metrics.get('totalDecisions', 1) if variant_metrics.get('totalDecisions', 0) > 0 else 0
                ...
                if current_value is not None:
                    ...
                    status = 'violated' if violated else ('warning' if abs(current_value - threshold) / threshold < 0.1 else 'ok')
    ...
    def _get_experiment(self, experiment_id: str) -> Optional[Dict[str, Any]]:
        ...
        with get_db_session() as db:
            query = text("SELECT * FROM experiments WHERE experiment_id = :experiment_id")
