...
class ReplayEngine:
    ...
    async def get_scenario(self, scenario_id: str) -> Optional[Dict[str, Any]]:
        ...
        with get_db_session() as db:
            query = text(
                "SELECT * FROM replay_scenarios WHERE scenario_id = :scenario_id"
            )
            ...
            scenario["startDate"] = (
                scenario.get("start_date").isoformat()
                if scenario.get("start_date")
                else None
            )
            scenario["endDate"] = (
                scenario.get("end_date").isoformat()
                if scenario.get("end_date")
                else None
            )
            scenario["createdAt"] = (
                scenario.get("created_at").isoformat()
                if scenario.get("created_at")
                else None
            )
            scenario["updatedAt"] = (
                scenario.get("updated_at").isoformat()
    ...
    async def list_scenarios(
        ...
        with get_db_session() as db:
            if status:
                query = text(
                    "SELECT * FROM replay_scenarios WHERE status = :status ORDER BY created_at DESC"
                )
    ...
    async def run_scenario(self, scenario_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            production_metrics = await self.metrics_calc.calculate_metrics(
                start_date, end_date
            )
            ...
            impact_metrics = self._calculate_impact_metrics(
                production_metrics, replay_results
            )

            # Store results
            result_id = await self._store_results(
                scenario_id, replay_results, diff_comparison, impact_metrics
            )
    ...
    async def _deterministic_replay(
        ...
        recall = (
            true_positives / (true_positives + would_miss_fraud)
            if (true_positives + would_miss_fraud) > 0
            else 0.0
        )
        f1_score = (
    ...
    def _calculate_diff(
        self, production_metrics: Any, replay_results: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        prod_dict = (
            ...
            if hasattr(production_metrics, "dict")
            else production_metrics
        )

        return {
            "precisionDiff": replay_results.get("precision", 0)
            - prod_dict.get("precision", 0),
            "recallDiff": replay_results.get("recall", 0) - prod_dict.get("recall", 0),
            "f1ScoreDiff": replay_results.get("f1Score", 0)
            - prod_dict.get("f1_score", 0),
            "declineRateDiff": (
                replay_results.get("wouldDecline", 0)
                / replay_results.get("totalDecisions", 1)
            )
            - (prod_dict.get("false_positives", 0) + prod_dict.get("true_positives", 0))
            / prod_dict.get("total_decisions", 1),
            "falsePositiveDiff": replay_results.get("falsePositives", 0)
            - prod_dict.get("false_positives", 0),
            "truePositiveDiff": replay_results.get("truePositives", 0)
            - prod_dict.get("true_positives", 0),
        }
    ...
    def _calculate_impact_metrics(
        ...
        false_positive_reduction = prod_dict.get(
            "false_positives", 0
        ) - replay_results.get("falsePositives", 0)
        cost_savings = false_positive_reduction * false_positive_cost

        return {
            "falsePositiveReduction": false_positive_reduction,
            "costSavings": cost_savings,
            "fraudCatchImprovement": replay_results.get("wouldCatchFraud", 0)
            - prod_dict.get("true_positives", 0),
            "precisionImprovement": replay_results.get("precision", 0)
            - prod_dict.get("precision", 0),
            "recallImprovement": replay_results.get("recall", 0)
            - prod_dict.get("recall", 0),
        }
        ...
        with get_db_session() as db:
            ...
            return {
                **result_row,
                ...
                "productionComparison": json.loads(
                    result_row.get("production_comparison", "{}")
                ),
