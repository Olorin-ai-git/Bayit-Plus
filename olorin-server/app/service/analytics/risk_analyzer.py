...
class RiskAnalyzer:
    ...
    def _validate_column_name(self, column_name: str) -> str:
        ...
        if not is_valid_column(column_upper):
            raise ValueError(
                f"Invalid column '{column_name}' - not found in database schema. Valid columns must match the 333-column schema definition."
            )
    ...
    async def get_top_risk_entities(
        ...
        try:
            logger.info(
                f"🔄 Starting FRAUD ENTITY ANALYSIS: time_window={time_window}, group_by={group_by}"
            )
            logger.info(f"⏱️ Time window: {time_window} ({hours} hours)")
            logger.info(
                f"🎯 New pattern: Retrieve ALL entities with APPROVED=TRUE AND IS_FRAUD_TX=1"
            )
            ...
            logger.info(
                f"📅 Query window dates: {start_date.strftime('%Y-%m-%d %H:%M:%S')} to {end_date.strftime('%Y-%m-%d %H:%M:%S')} (UTC)"
            )
            ...
            logger.info(
                f"🔍 Query filters: {approved_filter} AND {fraud_col}=1 (APPROVED FRAUD ONLY)"
            )
            ...
            logger.info(
                f"✅ {db_provider.upper()} connection initialized (lazy connection)"
            )
            ...
            logger.info(
                f"🔍 Executing {db_provider.upper()} query for {group_by} filtering:"
            )
            ...
            window_duration_hours = hours % 24
            datetime_col = (
                "TX_DATETIME" if db_provider == "snowflake" else "tx_datetime"
            )
            decision_col = (
                "NSURE_LAST_DECISION"
                if db_provider == "snowflake"
            ...
            approved_filter_expr = self._build_approved_filter(
                decision_col, db_provider
            )
            ...
            try:
                diagnostic_results = await self.client.execute_query_async(
                    diagnostic_query
                )
                if diagnostic_results:
                    ...
                    logger.info(
                        f"   Total transactions in date range: {diag.get('total_transactions', 0)}"
                    )
                    logger.info(
                        f"   Approved transactions: {diag.get('approved_count', 0)}"
                    )
                    logger.info(
                        f"   Date range: {diag.get('earliest_transaction')} to {diag.get('latest_transaction')}"
                    )
                    logger.info(
                        f"   Unique {group_by}: {diag.get('unique_entities', 0)}"
                    )

                    # If no data in filtered range, check what data exists in the database
                    if diag.get("total_transactions", 0) == 0:
                        logger.warning(
                            f"⚠️ No transactions found in filtered date range. Checking database for available data..."
                        )
                        ...
                        if db_provider == "snowflake":
                            ...
                            """
                        try:
                            table_check_results = await self.client.execute_query_async(
                                table_check_query
                            )
                            if table_check_results:
                                approximate_count = table_check_results[0].get(
                                    "approximate_row_count", 0
                                )
                                logger.info(
                                    f"📊 Table {table_name} exists (approximate row count: ~{approximate_count:,} rows)"
                                )
                        except Exception as table_check_e:
                            # If sampling fails, just verify table exists
                            logger.debug(
                                f"Sampling not available, verifying table exists: {table_check_e}"
                            )
                            try:
                                ...
                                logger.error(
                                    f"❌ Error checking table {table_name}: {simple_e}"
                                )
                                logger.error(
                                    f"   This might indicate the table doesn't exist or there's a permissions issue"
                                )
                        ...
                        """
                        logger.info(
                            f"🔍 Database-wide query (limited to last {max_lookback_days} days to avoid timeout):"
                        )
                        logger.info(f"{db_range_query}")
                        try:
                            db_range_results = await self.client.execute_query_async(
                                db_range_query
                            )
                        except Exception as db_range_error:
                            logger.warning(
                                f"⚠️ Database-wide statistics query failed (non-critical): {db_range_error}"
                            )
                            logger.warning(
                                "   Continuing without database-wide statistics..."
                            )
                        ...
                        if db_range_results:
                            ...
                            logger.info(
                                f"   Total transactions in database: {db_info.get('total_all_transactions', 0)}"
                            )
                            logger.info(
                                f"   Total approved transactions: {db_info.get('total_approved', 0)}"
                            )
                            logger.info(
                                f"   Database date range: {db_info.get('db_earliest_transaction')} to {db_info.get('db_latest_transaction')}"
                            )
                            logger.info(
                                f"   Total unique {group_by}: {db_info.get('total_unique_entities', 0)}"
                            )
                            ...
                            if db_info.get("total_all_transactions", 0) > 0:
                                logger.info(
                                    f"🔍 Analyzing which filter is excluding data..."
                                )
                                ...
                                try:
                                    no_date_results = (
                                        await self.client.execute_query_async(
                                            no_date_filter_query
                                        )
                                    )
                                    if no_date_results:
                                        no_date_info = no_date_results[0]
                                        logger.info(
                                            f"📊 Data WITHOUT 6-month constraint (WITH APPROVED filter, last 12 months):"
                                        )
                                        logger.info(
                                            f"   Transactions: {no_date_info.get('transactions_no_date_filter', 0)}"
                                        )
                                        logger.info(
                                            f"   Date range: {no_date_info.get('earliest_no_date')} to {no_date_info.get('latest_no_date')}"
                                        )
                                except Exception as e:
                                    logger.warning(
                                        f"⚠️ Error checking data without date filter: {e}"
                                    )
                                ...
                                try:
                                    no_approved_results = (
                                        await self.client.execute_query_async(
                                            no_approved_filter_query
                                        )
                                    )
                                    if no_approved_results:
                                        no_approved_info = no_approved_results[0]
                                        logger.info(
                                            f"📊 Data WITH 6-month constraint (WITHOUT APPROVED filter):"
                                        )
                                        logger.info(
                                            f"   Total transactions: {no_approved_info.get('transactions_no_approved_filter', 0)}"
                                        )
                                        logger.info(
                                            f"   Approved: {no_approved_info.get('approved_count', 0)}"
                                        )
                                        logger.info(
                                            f"   Non-approved: {no_approved_info.get('non_approved_count', 0)}"
                                        )
                                        logger.info(
                                            f"   Date range: {no_approved_info.get('earliest_no_approved')} to {no_approved_info.get('latest_no_approved')}"
                                        )
                                except Exception as e:
                                    logger.warning(
                                        f"⚠️ Error checking data without APPROVED filter: {e}"
                                    )
                                ...
                                try:
                                    ...
                                    if no_filters_results:
                                        ...
                                        logger.info(
                                            f"   Transactions: {no_filters_info.get('transactions_no_filters', 0)}"
                                        )
                                        logger.info(
                                            f"   Date range: {no_filters_info.get('earliest_no_filters')} to {no_filters_info.get('latest_no_filters')}"
                                        )
                                except Exception as e:
                                    logger.warning(
                                        f"⚠️ Error checking data without filters: {e}"
                                    )
                            ...
                            if db_latest and db_earliest:
                                logger.info(
                                    f"   Database date range: {db_earliest} to {db_latest}"
                                )
                                logger.info(
                                    f"   Query date range: {start_date.strftime('%Y-%m-%d %H:%M:%S')} to {end_date.strftime('%Y-%m-%d %H:%M:%S')} UTC"
                                )
                                if isinstance(db_latest, str) and isinstance(
                                    db_earliest, str
                                ):
                                    from datetime import datetime as dt

                                    try:
                                        db_latest_dt = dt.fromisoformat(
                                            db_latest.replace("Z", "+00:00")
                                        )
                                        db_earliest_dt = dt.fromisoformat(
                                            db_earliest.replace("Z", "+00:00")
                                        )
                                        ...
                                        if end_date <= db_earliest_dt:
                                            logger.warning(
                                                f"⚠️ Query end date ({end_date.date()}) is BEFORE database earliest transaction ({db_earliest_dt.date()})"
                                            )
                                            logger.warning(
                                                f"   The 6-month lookback window ({end_date.date()}) is too far in the past!"
                                            )
                                            logger.warning(
                                                f"   Database only has data from {db_earliest_dt.date()} onwards"
                                            )
                                            logger.warning(
                                                f"   Consider reducing ANALYTICS_MAX_LOOKBACK_MONTHS or checking if older data exists"
                                            )
                                        elif start_date >= db_latest_dt:
                                            logger.warning(
                                                f"⚠️ Query start date ({start_date.date()}) is AFTER database latest transaction ({db_latest_dt.date()})"
                                            )
                                            logger.warning(
                                                f"   This shouldn't happen - query is looking in the future!"
                                            )
                                        elif end_date > db_latest_dt:
                                            logger.info(
                                                f"ℹ️  Query end date ({end_date.date()}) is AFTER database latest transaction ({db_latest_dt.date()})"
                                            )
                                            logger.info(
                                                f"   Query range partially overlaps with database range"
                                            )
                                            logger.info(
                                                f"   Query will only return data up to {db_latest_dt.date()}"
                                            )
                                        else:
                                            logger.info(
                                                f"✅ Query date range overlaps with database date range"
                                            )
                                            logger.info(
                                                f"   Overlap: {max(start_date, db_earliest_dt).date()} to {min(end_date, db_latest_dt).date()}"
                                            )
                                    except Exception as parse_e:
                                        logger.debug(
                                            f"Could not parse dates for comparison: {parse_e}"
                                        )
            ...
            try:
                ...
                if (
                    ...
                    logger.error(
                        f"❌ Schema authorization error: Schema '{full_schema}' does not exist or user does not have access"
                    )
                    logger.error(f"   Please verify:")
                    logger.error(f"   1. Schema '{full_schema}' exists in Snowflake")
                    logger.error(
                        f"   2. User has USAGE privilege on schema '{full_schema}'"
                    )
                    logger.error(
                        f"   3. User has SELECT privilege on table in schema '{full_schema}'"
                    )
                    logger.error(
                        f"   4. Environment variables SNOWFLAKE_DATABASE and SNOWFLAKE_SCHEMA are set correctly"
                    )
                    logger.error(
                        f"   Current values: SNOWFLAKE_DATABASE={database}, SNOWFLAKE_SCHEMA={schema}"
                    )
                    return {
                        "status": "error",
            ...
            # Process results
            analysis = self._process_results(
            ...
            for fallback_window, fallback_hours in fallback_windows:
                if entities_count == 0 and current_hours < fallback_hours:
                    ...
                    # End date: capped at max_lookback_days ago
                    fallback_end_date = datetime.utcnow() - timedelta(
                        days=max_lookback_days
                    )
                    # Start date: end_date - window_duration
                    fallback_start_date = fallback_end_date - timedelta(
                        days=fallback_window_duration_days,
                        hours=fallback_window_duration_hours,
                    )

                    logger.info(
                        f"🔄 No entities found in {time_window} ({current_hours}h), trying {fallback_window} fallback window..."
                    )
                    logger.info(
                        f"📅 Fallback window dates: {fallback_start_date.strftime('%Y-%m-%d %H:%M:%S')} to {fallback_end_date.strftime('%Y-%m-%d %H:%M:%S')} (UTC)"
                    )
                    fallback_attempted = True

                    # Try fallback window
                    extended_query = self._build_risk_query(
                        fallback_hours, group_by, top_percentage
                    )
                    logger.info(f"📝 Full Fallback SQL Query:")
                    logger.info(f"{extended_query}")
                    extended_results = await self.client.execute_query_async(
                        extended_query
                    )
                    ...
                    if extended_results and len(extended_results) > 0:
                        logger.info(
                            f"✅ Found {len(extended_results)} entities in {fallback_window} window"
                        )
                        analysis = self._process_results(
                            extended_results, fallback_window, group_by, top_percentage
                        )
                        ...
                        logger.info(
                            f"⚠️ No entities found in {fallback_window} window, trying next fallback..."
                        )
                        time_window = fallback_window  # Update for next iteration check
                        current_hours = fallback_hours
            ...
            if fallback_attempted and entities_count == 0:
                logger.critical(
                    f"🚨 CRITICAL: No entities found even after trying all fallback windows (up to 90d) for {group_by}"
                )
                logger.critical(
                    f"🚨 CRITICAL: No data to investigate - terminating investigation"
                )
                analysis["status"] = "critical_no_data"
                analysis["fallback_used"] = False
                analysis["original_time_window"] = original_time_window
                analysis["message"] = (
                    f"CRITICAL: No entities found in any time window (7d, 14d, 30d, 60d, 90d) for {group_by}. Nothing to investigate."
                )
                return analysis
            ...
            entities_count = len(analysis.get("entities", []))
            logger.info(
                f"✅ Risk analysis completed: {entities_count} entities identified (from {final_query_rows} query rows)"
            )
            ...
            if entities:
                ...
                for i, entity in enumerate(entities[:3], 1):
                    ...
                    risk_score = entity.get(
                        "risk_score", entity.get("avg_risk_score", 0)
                    )
                    logger.info(
                        f"   {i}. Entity: {entity_id} | Transactions: {tx_count} | Risk Score: {risk_score:.3f}"
                    )

            # If query returned rows but no entities after processing, log diagnostic info
            if len(results) > 0 and entities_count == 0:
                logger.warning(
                    f"⚠️ Query returned {len(results)} rows but 0 entities after processing. "
    ...
    def _build_risk_query(
        self, hours: int, group_by: str, top_percentage: float
    ) -> str:
        """
        ...
        if validated_column == IP:
            ...
            excluded_patterns = os.getenv(
                ...
                "10.%,192.168.%,172.16.%,172.17.%,172.18.%,172.19.%,172.2_.%,172.30.%,172.31.%,127.%,169.254.%",
            ).split(",")
            excluded_values = os.getenv(
                "EXCLUDED_IP_VALUES", ",,0.0.0.0,::,localhost,unknown"
            ).split(",")
            ...
            like_filters = " ".join(
                [
                    f"AND {IP} NOT LIKE '{pattern.strip()}'"
                    for pattern in excluded_patterns
                    if pattern.strip()
                ]
            )
            quoted_values = [
                f"'{val.strip()}'" for val in excluded_values if val.strip()
            ]
    ...
    def _process_results(
        ...
        total_risk_value = sum(
            (r.get("RISK_WEIGHTED_VALUE") or r.get("risk_weighted_value") or 0)
            for r in results
        )
        total_transactions = sum(
            (r.get("TRANSACTION_COUNT") or r.get("transaction_count") or 0)
            for r in results
        )
        total_amount = sum(
            (r.get("TOTAL_AMOUNT") or r.get("total_amount") or 0) for r in results
        )
        total_fraud = sum(
            (r.get("FRAUD_COUNT") or r.get("fraud_count") or 0) for r in results
        )
            ...
            entities.append(
                {
                    ...
                    "risk_weighted_value": round(
                        (
                            r.get("RISK_WEIGHTED_VALUE")
                            or r.get("risk_weighted_value")
                            or 0
                        ),
                        2,
                    ),
                    "transaction_count": r.get("TRANSACTION_COUNT")
                    or r.get("transaction_count"),
                    "total_amount": round(
                        (r.get("TOTAL_AMOUNT") or r.get("total_amount") or 0), 2
                    ),
                    "fraud_count": (r.get("FRAUD_COUNT") or r.get("fraud_count") or 0),
                    "rejected_count": (
                        r.get("REJECTED_COUNT") or r.get("rejected_count") or 0
                    ),
                    "max_risk_score": round(
                        (r.get("MAX_RISK_SCORE") or r.get("max_risk_score") or 0), 3
                    ),
                    "percentile": round(
                        (r.get("PERCENTILE") or r.get("percentile") or 0) * 100, 1
                    ),
    ...
    async def analyze_entity(
        ...
        try:
            ...
            escaped_entity_value = entity_value.replace(
                "'", "''"
            )  # SQL standard escape for single quotes
    ...
    def _get_dynamic_threshold(self, level: str, current_score: float) -> float:
        ...
        """
        try:
            from app.service.analytics.threshold_calculator import (
    ...
    def _get_device_threshold(self) -> int:
        ...
        try:
            from app.service.analytics.threshold_calculator import (
                get_dynamic_device_threshold,
            )

            entity_type = getattr(self, "_current_entity_type", None)

            threshold = get_dynamic_device_threshold(
                entity_type=entity_type, lookback_days=7
            )
    ...
    def _get_velocity_threshold(self) -> int:
        ...
        try:
            from app.service.analytics.threshold_calculator import (
                get_dynamic_velocity_threshold,
            )
    ...
    def _assess_risk(self, profile: Dict[str, Any]) -> Dict[str, Any]:
        ...
        fraud_rate = (
            (fraud_count / transaction_count * 100) if transaction_count > 0 else 0
        )
        ...
        return {
            ...
            "indicators": {
                "high_risk_score": avg_risk
                > self._get_dynamic_threshold("high", avg_risk),
                "confirmed_fraud": fraud_count > 0,
                "multiple_devices": profile.get("unique_devices", 0)
                > self._get_device_threshold(),
                "multiple_cards": profile.get("unique_cards", 0)
                > self._get_card_threshold(),
                "suspicious_velocity": transaction_count
                > self._get_velocity_threshold(),
            },
        }
