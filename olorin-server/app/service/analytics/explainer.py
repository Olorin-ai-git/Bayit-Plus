...
class Explainer:
    ...
    def calculate_shap_values(self, decision_data: Dict[str, Any], baseline_data: List[Dict[str, Any]]) -> Dict[str, float]:
        ...
        features = ['MODEL_SCORE', 'EMAIL', 'DEVICE_ID', 'IP_COUNTRY_CODE', 'MERCHANT_ID', 'AMOUNT']
        shap_values = {}
        
        # Calculate mean baseline values
        baseline_means = {}
        for feature in features:
            values = [float(row.get(feature, 0) or 0) for row in baseline_data if row.get(feature) is not None]
    ...
    async def get_cohort_drivers(
        ...
        for feature_name, feature_values in features.items():
            if len(feature_values) > 1:
                ...
                impacts[feature_name] = abs(correlation) if not np.isnan(correlation) else 0.0
        ...
        if len(results) > 0:
            email_fraud_rate = sum(1 for row in results if row.get('EMAIL') and row.get('IS_FRAUD_TX') == 1) / email_present if email_present > 0 else 0
            no_email_fraud_rate = sum(1 for row in results if not row.get('EMAIL') and row.get('IS_FRAUD_TX') == 1) / (len(results) - email_present) if (len(results) - email_present) > 0 else 0
            impacts['email'] = abs(email_fraud_rate - no_email_fraud_rate)
            
            device_fraud_rate = sum(1 for row in results if row.get('DEVICE_ID') and row.get('IS_FRAUD_TX') == 1) / device_present if device_present > 0 else 0
            no_device_fraud_rate = sum(1 for row in results if not row.get('DEVICE_ID') and row.get('IS_FRAUD_TX') == 1) / (len(results) - device_present) if (len(results) - device_present) > 0 else 0
    ...
    async def get_confusion_matrix(
        ...
        for row in results:
            matrix_over_time.append({
                "timestamp": row.get('time_bucket').isoformat() if row.get('time_bucket') else None,
