...
from app.schemas.kpi_schemas import KPIDashboardResponse, KPIDailyMetricsResponse, KPIThresholdSweepResponse, KPIBreakdownResponse
...
class KPIService:
    ...
    def get_dashboard_metrics(
        ...
        latest_metrics = self.db.query(KPIDailyMetrics).filter(
            and_(
        ...
        ).order_by(KPIDailyMetrics.metric_date.desc()).first()
        ...
        daily_metrics_list = daily_metrics_query.order_by(KPIDailyMetrics.metric_date.asc()).all()
        ...
        threshold_sweep_list = sweep_query.order_by(KPIThresholdSweep.threshold.asc()).all()
        ...
        return KPIDashboardResponse(
            ...
            daily_metrics=[KPIDailyMetricsResponse.model_validate(m, from_attributes=True) for m in daily_metrics_list],
            threshold_sweep=[KPIThresholdSweepResponse.model_validate(s, from_attributes=True) for s in threshold_sweep_list],
            breakdowns=[KPIBreakdownResponse.model_validate(b, from_attributes=True) for b in breakdowns_list],
            pilot_id=pilot_id,
            tenant_id=tenant_id,
    ...
    def get_daily_metrics(
        ...
        return [KPIDailyMetricsResponse.model_validate(m, from_attributes=True) for m in metrics]
    ...
    def get_threshold_sweep(
        ...
        return [KPIThresholdSweepResponse.model_validate(s, from_attributes=True) for s in sweeps]
    ...
    def get_breakdowns(
        ...
        return [KPIBreakdownResponse.model_validate(b, from_attributes=True) for b in breakdowns]
