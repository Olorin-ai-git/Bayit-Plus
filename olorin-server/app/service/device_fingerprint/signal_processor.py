...
class SignalProcessor:
        ...
        try:
            with get_db_session() as db:
                ...
                params = {
                    ...
                    "browser_fingerprint": json.dumps(signal_dict.get("browser_fingerprint")) if signal_dict.get("browser_fingerprint") else None,
                    "behavioral_signals": json.dumps(signal_dict.get("behavioral_signals")) if signal_dict.get("behavioral_signals") else None,
                ...
                logger.info(f"Inserted device signal to PostgreSQL: {signal_dict['device_id']}")
    ...
    def _mirror_to_splunk(self, signal_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            splunk_event = {
                ...
                "timestamp": signal_dict.get("captured_at", datetime.utcnow().isoformat()),
            }
            
            # Send to Splunk (placeholder - actual implementation would use Splunk client)
            logger.info(f"Would send device signal to Splunk: {signal_dict['device_id']}")
    ...
    def _publish_to_kafka(
        ...
        try:
            ...
            topic = self.config_loader.load_secret("KAFKA_DEVICE_SIGNALS_TOPIC") or "device_signals"
            ...
            if success:
                logger.debug(f"Published device signal event to Kafka: {event['event_id']}")
                return {"status": "success", "event_id": event["event_id"]}
            else:
                logger.warning(f"Failed to publish device signal event to Kafka: {event['event_id']}")
