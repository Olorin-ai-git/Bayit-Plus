...
def generate_startup_report(
    ...
    startup_data = _collect_startup_data(app_state, startup_duration_seconds, reports_dir, output_path)
    ...
    if output_path is None:
        ...
        logger.info(
            f"Resolved top-level startup report path: {output_path}"
        )
    else:
...
def _collect_startup_data(
    ...
    if not top_risk_entities or not isinstance(top_risk_entities, dict) or not top_risk_entities.get('entities'):
        ...
        if comparison_results:
            logger.info(f"No top_risk_entities in app_state, extracting from {len(comparison_results)} comparison results")
            for result in comparison_results:
                ...
                if investigation_id:
                    ...
                    if inv_data:
                        risk_score = inv_data.get("overall_risk_score") or inv_data.get("risk_score") or 0.0
        ...
        if not entities_from_comparisons and reports_dir and Path(reports_dir).exists():
            ...
            if manifests_file.exists():
                try:
                    ...
                    logger.info(f"Extracting entities from {len(manifests)} comparison manifests")
                    for manifest in manifests:
                        entity = manifest.get("entity", {})
                        entity_type = entity.get("entity_type", "email")
                        entity_id = entity.get("entity_id", "N/A")
                        investigation_id = manifest.get("metadata", {}).get("investigation_id") or manifest.get("left_investigation")
                    ...
                    logger.info(f"Extracted {len(entities_from_comparisons)} entities from comparison manifests")
                except Exception as e:
                    logger.warning(f"Failed to extract entities from manifests: {e}")
        
        if entities_from_comparisons:
            top_risk_entities = {
                'summary': {'group_by': entities_from_comparisons[0].get('entity_type', 'email')},
                'entities': entities_from_comparisons
            }
            logger.info(f"Created top_risk_entities with {len(entities_from_comparisons)} entities")
    ...
    if top_risk_entities and isinstance(top_risk_entities, dict):
        ...
        for entity in entities[:10]:  # Limit to top 10
            ...
            if risk_score == 0.0 or risk_score == 'N/A':
                ...
                comparison_results = data["auto_comparisons"]["results"]
                for result in comparison_results:
                    if result.get("entity_value") == entity_id or result.get("entity") == entity_id:
                        investigation_id = result.get("investigation_id")
                        if investigation_id:
                            inv_data = _get_investigation_from_db(investigation_id)
                            if inv_data and inv_data.get("overall_risk_score") is not None:
            ...
            if isinstance(entity, dict):
                mapped_entity.update({k: v for k, v in entity.items() if k not in ['entity_type', 'entity_id', 'risk_score']})
            
            mapped_entities.append(mapped_entity)
            seen_entities[entity_id] = True
        
        data["risk_entities"]["entities"] = mapped_entities
        data["risk_entities"]["count"] = len(mapped_entities)  # Update count to unique entities
    ...
    if data["risk_entities"]["entities"]:
        for entity in data["risk_entities"]["entities"]:
            ...
            if entity_id and (entity.get("risk_score") == 0.0 or entity.get("risk_score") == "N/A"):
                # Look for investigation with this entity
                investigation_summaries = data["auto_comparisons"].get("investigation_summaries", [])
...
def _generate_summary_section(data: Dict[str, Any]) -> str:
    ...
    components_status = [
        ("Database", data["database"]["available"], "Provides data access for investigations and analytics"),
        ("Risk Entities Loader", data["risk_entities"]["loaded"], "Loads top risk entities from Snowflake for auto-investigation"),
        ("Auto Comparisons", data["auto_comparisons"]["completed"], "Automatically runs investigations and generates comparison reports"),
        ("RAG System", data["rag_system"]["available"], "Retrieval-Augmented Generation for enhanced context"),
        ("Anomaly Detection", data["anomaly_detection"]["available"], "Detects anomalous patterns in transaction data"),
        ("Detection Scheduler", data["detection_scheduler"]["available"], "Schedules and manages detection tasks"),
    ]
...
def _generate_zero_metrics_explanation(window_a: Dict[str, Any], window_b: Dict[str, Any], artifact_data: Optional[Dict[str, Any]]) -> str:
    ...
    if total_tx_a == 0 and total_tx_b == 0:
        reasons.append("<strong>CRITICAL:</strong> No transactions found in either time window (Window A: 0, Window B: 0). This means:")
        reasons.append("  • The entity had no transactions during the investigation or validation periods")
        reasons.append("  • The time windows may be incorrect or outside the entity's transaction history")
        reasons.append("  • The database query may have failed or returned empty results")
    elif total_tx_a == 0:
        reasons.append("<strong>CRITICAL:</strong> No transactions found in Window A (Historical Investigation period)")
    elif total_tx_b == 0:
        ...
        reasons.append("All performance metrics (Precision, Recall, F1, Accuracy) are 0.0% because no transactions were flagged as fraud (TP=0, FP=0).")
        reasons.append("")
        reasons.append("Possible reasons:")
        reasons.append("  • <strong>Risk score not applied:</strong> The investigation's risk score was not applied to transactions (predicted_risk is missing or None)")
        reasons.append("  • <strong>Risk score extraction failed:</strong> overall_risk_score is 0.0 and domain_findings.risk.risk_score was not extracted")
        reasons.append("  • <strong>Threshold too high:</strong> The risk threshold ({:.1%}) was higher than all predicted_risk values".format(threshold))
        reasons.append("  • <strong>No fraud predictions:</strong> No transactions had predicted_risk >= threshold, so none were flagged as fraud")
    
    reasons_html = "<br>".join([f"<li style='margin: 4px 0;'>{r}</li>" if not r.startswith("  •") else f"<li style='margin: 2px 0; margin-left: 20px;'>{r}</li>" for r in reasons])
...
def _generate_comparison_summary(window_a: Dict[str, Any], window_b: Dict[str, Any], delta: Dict[str, Any], artifact_data: Optional[Dict[str, Any]]) -> str:
...
def _generate_confusion_table_section(data: Dict[str, Any]) -> str:
    ...
    if not aggregated_matrix:
        ...
        if hasattr(confusion_data.get("aggregated"), 'total_TP'):
            ...
            logger.warning("[CONFUSION_TABLE] No aggregated confusion matrix found in app.state")
            return """
          <div class="panel">
...
def _generate_comparison_metrics_section(data: Dict[str, Any]) -> str:
    ...
    if not zip_dir or not zip_dir.exists():
        ...
        if output_path:
            ...
            if "comparison_package" in str(output_path_obj) or "auto_startup" in str(output_path_obj):
                zip_dir = output_path_obj.parent if output_path_obj.is_file() else output_path_obj
    ...
    if not results and zip_dir and zip_dir.exists():
        ...
        if manifests_file.exists():
            try:
                ...
                logger.info(f"Loaded {len(results)} comparisons from comparison_manifests.json")
            except Exception as e:
            ...
            if investigations_dir.exists():
                ...
                for artifact_file in artifact_files:
                    try:
                        ...
                        investigation_id = _extract_investigation_id_from_path(artifact_file)
    ...
    for i, result in enumerate(results, 1):
        ...
        if zip_dir and zip_dir.exists():
            investigations_dir = zip_dir / "investigations"
            logger.info(f"Looking for artifacts in: {investigations_dir}, exists: {investigations_dir.exists()}")
            if investigations_dir.exists():
                # Search for artifact files matching this investigation_id
                artifact_files = list(investigations_dir.rglob("*artifact.json"))
                logger.info(f"Found {len(artifact_files)} artifact files, looking for investigation_id: {investigation_id}")
                for artifact_file in artifact_files:
                    logger.info(f"Checking artifact: {artifact_file.name}, contains investigation_id: {investigation_id in artifact_file.name}")
                    if investigation_id != "N/A" and investigation_id in artifact_file.name:
                        try:
                            with open(artifact_file, 'r') as f:
                                artifact_data = json.load(f)
                            logger.info(f"✅ Loaded comparison metrics from zip_dir artifact: {artifact_file}")
                            logger.info(f"   Artifact data keys: {list(artifact_data.keys())}")
                            logger.info(f"   Window A TP: {artifact_data.get('A', {}).get('TP')}, total_transactions: {artifact_data.get('A', {}).get('total_transactions')}")
                            break
                        except Exception as e:
                            logger.warning(f"Failed to load zip_dir artifact {artifact_file}: {e}")
                        ...
                        for artifact_file in artifact_files:
                            if investigation_id != "N/A" and investigation_id in artifact_file.name:
                                try:
                                    with open(artifact_file, 'r') as f:
                                        artifact_data = json.load(f)
                                    logger.debug(f"Loaded comparison metrics from entity artifact: {artifact_file}")
                                    break
                                except Exception as e:
                                    logger.debug(f"Failed to load entity artifact {artifact_file}: {e}")
                        ...
                        if not artifact_data and artifact_files:
                            try:
                                with open(artifact_files[0], 'r') as f:
                                    artifact_data = json.load(f)
                                logger.debug(f"Loaded most recent comparison metrics from entity artifact: {artifact_files[0]}")
                            except Exception as e:
                                logger.debug(f"Failed to load entity artifact {artifact_files[0]}: {e}")
        ...
        if use_comparison_response:
            ...
            if isinstance(comparison_response, dict):
                ...
                window_a = {
                    "total_transactions": getattr(comparison_response.A, "total_transactions", 0),
                    ...
                    "over_threshold": getattr(comparison_response.A, "over_threshold", 0),
                }
                window_b = {
                    "total_transactions": getattr(comparison_response.B, "total_transactions", 0),
                    ...
                    "over_threshold": getattr(comparison_response.B, "over_threshold", 0),
                }
                delta = {
                    "precision": getattr(comparison_response.delta, "precision", 0.0) if hasattr(comparison_response, "delta") else 0.0,
                    "recall": getattr(comparison_response.delta, "recall", 0.0) if hasattr(comparison_response, "delta") else 0.0,
                    "f1": getattr(comparison_response.delta, "f1", 0.0) if hasattr(comparison_response, "delta") else 0.0,
                    "accuracy": getattr(comparison_response.delta, "accuracy", 0.0) if hasattr(comparison_response, "delta") else 0.0,
                    "fraud_rate": getattr(comparison_response.delta, "fraud_rate", 0.0) if hasattr(comparison_response, "delta") else 0.0,
                }
                window_a_info = {
                    "label": getattr(comparison_response.windowA, "label", "Window A") if hasattr(comparison_response, "windowA") else "Window A",
                    "start": getattr(comparison_response.windowA, "start", "") if hasattr(comparison_response, "windowA") else "",
                    "end": getattr(comparison_response.windowA, "end", "") if hasattr(comparison_response, "windowA") else "",
                }
                window_b_info = {
                    "label": getattr(comparison_response.windowB, "label", "Window B") if hasattr(comparison_response, "windowB") else "Window B",
                    "start": getattr(comparison_response.windowB, "start", "") if hasattr(comparison_response, "windowB") else "",
                    "end": getattr(comparison_response.windowB, "end", "") if hasattr(comparison_response, "windowB") else "",
                }
        ...
        if artifact_data and not use_comparison_response:
            ...
            logger.info(f"   window_a: TP={window_a.get('TP')}, total_transactions={window_a.get('total_transactions')}")
            logger.info(f"   window_b: TP={window_b.get('TP')}, total_transactions={window_b.get('total_transactions')}")
        elif not use_comparison_response:
            # Fallback to metrics dict if comparison_response not available
            logger.warning(f"⚠️  No artifact_data found for {entity_value}, artifact_data={artifact_data is not None}, use_comparison_response={use_comparison_response}")
        ...
        if window_a_info.get("start") and window_a_info.get("end"):
            window_a_label += f" ({window_a_info.get('start')} to {window_a_info.get('end')})"
        if window_b_info.get("start") and window_b_info.get("end"):
            window_b_label += f" ({window_b_info.get('start')} to {window_b_info.get('end')})"
...
def _enrich_with_investigation_details(
    ...
    comp_data.setdefault("aggregated_metrics", {
        "total_comparisons": 0,
    ...
    if reports_dir and reports_dir.exists():
        ...
        if "reports/startup" in str(output_path):
            ...
            if timestamp_match:
                ...
                zip_candidates = list(Path("artifacts/comparisons/auto_startup").glob(f"*{timestamp_str}*"))
                if zip_candidates:
                    zip_dir = zip_candidates[0] if zip_candidates[0].is_dir() else zip_candidates[0].parent
                    logger.info(f"Inferred zip_dir from output_path: {zip_dir}")
    ...
    if not zip_dir or not zip_dir.exists():
        ...
        if auto_startup_dir.exists():
            ...
            package_dirs = [d for d in auto_startup_dir.iterdir() if d.is_dir() and "comparison_package" in d.name]
            if package_dirs:
                # Sort by modification time, most recent first
                package_dirs.sort(key=lambda x: x.stat().st_mtime, reverse=True)
                zip_dir = package_dirs[0]
                logger.info(f"Found most recent comparison package directory: {zip_dir}")
                ...
                for artifact_file in inv_folder.rglob("*.json"):
                    if "artifact" in artifact_file.name.lower():
                        try:
                            ...
                            if not investigation_id or investigation_id in seen_investigation_ids:
                                continue
                            ...
                            if "type" not in entity_data:
                                if "entity_type" in entity_data:
                                    ...
                                    entity_value = entity_data.get("value") or entity_data.get("entity_id")
                                    if entity_value and entity_value in manifest_investigation_map:
                                        # Get entity type from manifest
                                        for manifest in manifests if 'manifests' in locals() else []:
                                            m_entity = manifest.get("entity", {})
                                            if m_entity.get("entity_id") == entity_value:
                                                entity_data["type"] = m_entity.get("entity_type", "email")
                                                break
                                    if "type" not in entity_data:
                            ...
                            inv_summary = {
                                "investigation_id": investigation_id,
                                "entity": entity_data,
                                "investigation_summary": artifact_data.get("investigation_summary", ""),
                                "metrics": {
                                    "window_a": _extract_metrics(artifact_data.get("A", {})),
                                    "window_b": _extract_metrics(artifact_data.get("B", {})),
                                },
                                "artifact_path": str(artifact_file.relative_to(zip_dir)),
                            }
                            
                            logger.info(f"Created investigation summary for {investigation_id}, entity: {entity_data.get('type')}={entity_data.get('value')}")
                            
                            if inv_db_data:
                                inv_summary["investigation_risk_score"] = inv_db_data.get("overall_risk_score")
                                inv_summary["investigation_status"] = inv_db_data.get("status")
                                inv_summary["investigation_created"] = inv_db_data.get("created_at")
                                inv_summary["domain_findings"] = inv_db_data.get("domain_findings", {})
                                inv_summary["tools_used"] = inv_db_data.get("tools_used", [])
                                inv_summary["current_phase"] = inv_db_data.get("current_phase")
                                inv_summary["percent_complete"] = inv_db_data.get("percent_complete")
                                inv_summary["lifecycle_stage"] = inv_db_data.get("lifecycle_stage")
                                inv_summary["started_at"] = inv_db_data.get("started_at")
                                inv_summary["completed_at"] = inv_db_data.get("completed_at")
                                inv_summary["tool_executions"] = inv_db_data.get("tool_executions", [])
                                inv_summary["findings"] = inv_db_data.get("findings", [])
                                inv_summary["summary"] = inv_db_data.get("summary", "")
                            
                            comp_data["investigation_summaries"].append(inv_summary)
                        except Exception as e:
                            logger.debug(f"Failed to read investigation artifact {artifact_file}: {e}")
    ...
    for summary in comp_data["investigation_summaries"]:
        ...
        if unique_key not in seen_entities:
            ...
            logger.debug(f"Skipping duplicate investigation summary: {investigation_id} for entity {entity_value}")
...
def _get_investigation_from_db(investigation_id: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        with get_db_session() as session:
            ...
            mapped_status = status_map.get(db_inv.status, db_inv.status.lower() if db_inv.status else 'unknown')
                    ...
                    if isinstance(progress_data, dict):
                        ...
                        if overall_risk_score is None:
                            if 'overall_risk_score' in progress_data:
                                ...
                                if top_level_score is not None and float(top_level_score) != 0.0:
                        ...
                        investigation_data['current_phase'] = progress_data.get('current_phase', 'unknown')
                        investigation_data['percent_complete'] = progress_data.get('percent_complete', 0)
                        investigation_data['lifecycle_stage'] = progress_data.get('lifecycle_stage', 'unknown')
                        investigation_data['started_at'] = progress_data.get('started_at')
                        investigation_data['completed_at'] = progress_data.get('completed_at')
                        investigation_data['tool_executions'] = progress_data.get('tool_executions', [])
                        investigation_data['findings'] = progress_data.get('findings', [])
                        investigation_data['summary'] = progress_data.get('summary', '')
                except (json.JSONDecodeError, TypeError, ValueError) as e:
                    logger.debug(f"Failed to parse progress_json for {investigation_id}: {e}")
        ...
        logger.debug(f"Failed to query investigation {investigation_id} from database: {e}")
    return None
...
def _generate_investigation_details_section(data: Dict[str, Any]) -> str:
    ...
    if len(unique_summaries) < len(investigation_summaries):
        logger.debug(f"Deduplicated investigation summaries in details section: {len(investigation_summaries)} -> {len(unique_summaries)}")
    ...
    for i, inv_summary in enumerate(unique_summaries, 1):
        ...
        journey_html = "<br>".join(journey_items) if journey_items else "No journey data available"
        ...
        for domain_name in domain_order:
            if domain_name in domain_findings:
                ...
                if isinstance(domain_data, dict):
                    ...
                    if confidence is not None and confidence < 0.5 and isinstance(evidence, list) and len(evidence) > 0:
                    ...
                    if isinstance(evidence, list) and evidence:
                        ...
                        for ev in evidence[:5]:
                            if isinstance(ev, str):
                                evidence_items.append(f"<li style='margin: 4px 0; padding-left: 8px;'>{ev[:200]}{'...' if len(ev) > 200 else ''}</li>")
                            elif isinstance(ev, dict):
                                ev_text = ev.get('text', ev.get('description', str(ev)))
                                evidence_items.append(f"<li style='margin: 4px 0; padding-left: 8px;'>{str(ev_text)[:200]}{'...' if len(str(ev_text)) > 200 else ''}</li>")
                        if evidence_items:
                    ...
                    if isinstance(risk_indicators, list) and risk_indicators:
                        ...
                        for indicator in risk_indicators[:5]:
                            if isinstance(indicator, str):
                                indicator_items.append(f"<span style='margin: 2px 4px; padding: 4px 8px; border-radius: 4px; display: inline-block; background: var(--chip); font-size: 12px;'>{indicator}</span>")
                            elif isinstance(indicator, dict):
                                ind_text = indicator.get('type', indicator.get('name', str(indicator)))
                                indicator_items.append(f"<span style='margin: 2px 4px; padding: 4px 8px; border-radius: 4px; display: inline-block; background: var(--chip); font-size: 12px;'>{ind_text}</span>")
                        if indicator_items:
        ...
        domain_analysis_html = "".join(domain_analysis) if domain_analysis else "<p style='color: var(--muted);'>No domain analysis available</p>"
        ...
        if isinstance(tools_used, list) and tools_used:
            ...
            for tool in tools_used[:10]:
                if isinstance(tool, str):
                    tool_items.append(f"<span style='margin: 4px 8px 4px 0; padding: 6px 12px; border-radius: 6px; display: inline-block; background: var(--chip); font-size: 12px;'>{tool}</span>")
                elif isinstance(tool, dict):
                    tool_name = tool.get('tool_name', tool.get('name', str(tool)))
                    tool_items.append(f"<span style='margin: 4px 8px 4px 0; padding: 6px 12px; border-radius: 6px; display: inline-block; background: var(--chip); font-size: 12px;'>{tool_name}</span>")
            if tool_items:
        ...
        if isinstance(tool_executions, list) and tool_executions:
            ...
            for exec_item in tool_executions[:10]:
                if isinstance(exec_item, dict):
                    ...
                    exec_items.append(f"<li style='margin: 4px 0; font-size: 13px;'><strong>{tool_name}</strong> - {status}{f' ({timestamp})' if timestamp else ''}</li>")
        ...
        if investigation_risk_score is not None:
            risk_level = "High" if investigation_risk_score >= 0.7 else "Medium" if investigation_risk_score >= 0.4 else "Low"
            risk_color = "var(--danger)" if investigation_risk_score >= 0.7 else "var(--warn)" if investigation_risk_score >= 0.4 else "var(--ok)"
            ...
            formatted_summary = _format_investigation_summary(summary) if summary else ""
...
def _format_llm_analysis(llm_analysis: Any) -> str:
    ...
    if isinstance(llm_analysis, dict):
        ...
        if risk_score is not None:
            formatted_parts.append(f"<div style='margin-bottom: 12px;'><strong style='color: var(--accent);'>Risk Score:</strong> <span style='font-size: 16px; font-weight: bold;'>{risk_score:.3f}</span></div>")
        
        if confidence is not None:
            formatted_parts.append(f"<div style='margin-bottom: 12px;'><strong style='color: var(--accent);'>Confidence:</strong> <span style='font-size: 16px; font-weight: bold;'>{confidence:.1%}</span></div>")
        
        # Format risk factors (handle newlines)
        if risk_factors:
            if isinstance(risk_factors, str):
                risk_factors_formatted = _parse_text_with_newlines(risk_factors)
                formatted_parts.append(f"<div style='margin-bottom: 12px;'><strong style='color: var(--accent);'>Risk Factors:</strong><div style='margin-top: 4px;'>{risk_factors_formatted}</div></div>")
            else:
                formatted_parts.append(f"<div style='margin-bottom: 12px;'><strong style='color: var(--accent);'>Risk Factors:</strong> {str(risk_factors)}</div>")
        
        # Format reasoning (handle newlines)
        if reasoning:
            if isinstance(reasoning, str):
                reasoning_formatted = _parse_text_with_newlines(reasoning)
                formatted_parts.append(f"<div style='margin-bottom: 12px;'><strong style='color: var(--accent);'>Reasoning:</strong><div style='margin-top: 4px;'>{reasoning_formatted}</div></div>")
            else:
                formatted_parts.append(f"<div style='margin-bottom: 12px;'><strong style='color: var(--accent);'>Reasoning:</strong> {str(reasoning)}</div>")
        
        # Format summary
        if summary:
            if isinstance(summary, str):
                summary_formatted = _parse_text_with_newlines(summary)
                formatted_parts.append(f"<div style='margin-bottom: 12px;'><strong style='color: var(--accent);'>Summary:</strong><div style='margin-top: 4px;'>{summary_formatted}</div></div>")
            else:
                formatted_parts.append(f"<div style='margin-bottom: 12px;'><strong style='color: var(--accent);'>Summary:</strong> {str(summary)}</div>")
        
        # Add any remaining fields
        for key, value in llm_analysis.items():
            if key not in ['risk_score', 'confidence', 'risk_factors', 'reasoning', 'summary']:
                if isinstance(value, str):
                    value_formatted = _parse_text_with_newlines(str(value))
                    formatted_parts.append(f"<div style='margin-bottom: 8px;'><strong>{key.replace('_', ' ').title()}:</strong> {value_formatted}</div>")
                else:
                    formatted_parts.append(f"<div style='margin-bottom: 8px;'><strong>{key.replace('_', ' ').title()}:</strong> {str(value)}</div>")
...
def _format_investigation_summary(summary: str) -> str:
    ...
    formatted = re.sub(r'^#+\s+(.+)$', r'<h4 style="margin: 12px 0 8px 0; color: var(--accent); font-size: 14px;">\1</h4>', formatted, flags=re.MULTILINE)
