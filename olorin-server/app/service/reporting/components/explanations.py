...
class ExplanationsComponent(BaseVisualizationComponent):
    ...
    def process_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        all_explanations.sort(
            key=lambda x: x["parsed_timestamp"] or datetime.min, reverse=True
        )
        ...
        return {
            ...
            "metrics": {
                ...
                "unique_agents": len(
                    set(exp["agent_name"] for exp in all_explanations)
                ),
                "avg_confidence": self._calculate_avg_confidence(all_explanations),
                "most_active_agent": (
                    explanation_stats["by_agent"].most_common(1)[0][0]
                    if explanation_stats["by_agent"]
                    else "None"
                ),
                "most_common_type": (
                    explanation_stats["by_type"].most_common(1)[0][0]
                    if explanation_stats["by_type"]
                    else "None"
                ),
            },
        }

    def _create_explanation_entry(
        self,
        source: str,
        timestamp: str,
        agent_name: str,
    ...
    def _contains_code_patterns(self, content: str) -> bool:
        ...
        return any(
            re.search(pattern, content, re.IGNORECASE) for pattern in code_patterns
        )
    ...
    def _extract_key_insights(
        ...
        critical_decisions = [
            exp
            for exp in explanations
            if exp["explanation_type"] == "decision_logic"
            and "high" in exp["urgency_indicators"]
        ]
        ...
        error_analyses = [
            exp for exp in explanations if exp["explanation_type"] == "error_analysis"
        ]
        if error_analyses:
            insights.append(
                {
    ...
    def _calculate_quality_metrics(
        ...
        with_confidence = len(
            [exp for exp in explanations if exp["confidence"] is not None]
        )
        high_confidence = len(
            [exp for exp in explanations if (exp["confidence"] or 0) >= 0.7]
        )
        ...
        return {
            ...
            "high_confidence_rate": (
                (high_confidence / with_confidence) * 100 if with_confidence > 0 else 0
            ),
            "detailed_explanations_rate": (detailed / total) * 100 if total > 0 else 0,
            "technical_explanations_rate": (
                (with_code / total) * 100 if total > 0 else 0
            ),
    ...
    def _calculate_avg_confidence(self, explanations: List[Dict[str, Any]]) -> float:
        ...
        confidences = [
            exp["confidence"] for exp in explanations if exp["confidence"] is not None
        ]
    ...
    def generate_html(self, processed_data: Dict[str, Any]) -> str:
        ...
        stats_items = [
            ...
            (
                "High Confidence",
                f"{quality_metrics.get('high_confidence_rate', 0):.1f}%",
            ),
        ]
        ...
        for i, explanation in enumerate(all_explanations[:20]):  # Limit to 20
            ...
            urgency_indicators = (
                ", ".join(explanation["urgency_indicators"])
                if explanation["urgency_indicators"]
                else "Normal"
            )
