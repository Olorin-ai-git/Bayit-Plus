...
class ToolsAnalysisComponent(BaseVisualizationComponent):
    ...
    def process_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for tool_name, stats in tool_stats.items():
            ...
            reliability_score = self._calculate_reliability_score(success_rate, avg_execution_time, len(stats['errors']))
        ...
        for agent_name, stats in agent_stats.items():
            success_rate = (stats['success_count'] / stats['tool_usage_count']) * 100 if stats['tool_usage_count'] > 0 else 0
            avg_execution_time = statistics.mean(stats['execution_times']) if stats['execution_times'] else 0
            efficiency_score = self._calculate_efficiency_score(success_rate, avg_execution_time, len(stats['unique_tools']))
        ...
        execution_times = [ex.get('execution_time_ms', 0) for ex in tool_executions if ex.get('execution_time_ms', 0) > 0]
        avg_execution_time = statistics.mean(execution_times) if execution_times else 0
        
        # Performance categorization
        performance_categories = self._categorize_performance(processed_tools, processed_agents)
        ...
        return {
            ...
            'metrics': {
                ...
                'most_used_tool': processed_tools[0]['name'] if processed_tools else 'None',
                'most_active_agent': processed_agents[0]['name'] if processed_agents else 'None'
            }
        }
    
    def _calculate_reliability_score(self, success_rate: float, avg_execution_time: float, error_count: int) -> float:
    ...
    def _calculate_efficiency_score(self, success_rate: float, avg_execution_time: float, unique_tools: int) -> float:
    ...
    def _get_most_common_errors(self, errors: List[str]) -> List[Dict[str, Any]]:
        ...
        return [{'error': error, 'count': count} for error, count in error_counter.most_common(3)]
    
    def _categorize_performance(self, tools: List[Dict], agents: List[Dict]) -> Dict[str, Any]:
    ...
    def generate_html(self, processed_data: Dict[str, Any]) -> str:
        ...
        insights_html = ""
        insights = [
            ('High Performance Tools', performance_categories.get('high_performance_tools', []), 'success'),
            ('Problematic Tools', performance_categories.get('problematic_tools', []), 'danger'),
            ('Efficient Agents', performance_categories.get('efficient_agents', []), 'success'),
            ('Struggling Agents', performance_categories.get('struggling_agents', []), 'warning')
        ]
