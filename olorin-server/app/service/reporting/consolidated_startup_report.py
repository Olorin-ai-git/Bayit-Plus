...
def _generate_selector_section(selector_metadata: Optional[Dict[str, Any]]) -> str:
    ...
    for entity in entities[:20]:  # Show top 20
        ...
        entity_rows += f"""
            <tr>
                ...
                <td style="padding: 8px; text-align: center; border-bottom: 1px solid var(--border);"><span style="color: var(--error); font-weight: bold;">{fraud_count}</span></td>
    ...
    if len(entities) > 20:
        entity_rows += f"""
            <tr>
                <td colspan="4" style="padding: 8px; text-align: center; color: var(--muted); font-style: italic;">
    ...
    <div class="panel">
        <div class="entity-header" onclick="toggleSelector()" style="cursor: pointer; padding: 20px; background: linear-gradient(135deg, rgba(74, 158, 255, 0.15) 0%, rgba(74, 158, 255, 0.05) 100%); border-radius: 12px; margin-bottom: 20px; transition: all 0.3s ease;">
            ...
            <div style="margin-top: 12px; display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 16px;">
                ...
                <div>
                    ...
                    <div style="font-size: 1.2em; font-weight: bold; color: var(--error); margin-top: 4px;">
                        {sum(e.get('fraud_count', 0) for e in entities)}
                    </div>
                </div>
            </div>
        </div>
        ...
        <div id="selector-content" style="display: block;">
            <div style="margin-bottom: 20px;">
                <div style="background: rgba(74, 158, 255, 0.1); border-left: 4px solid var(--accent); padding: 16px; border-radius: 8px;">
                    <div style="display: grid; grid-template-columns: auto 1fr; gap: 12px 20px; align-items: center;">
                        ...
                        <div style="font-family: 'Monaco', monospace; color: var(--text);">
            ...
            <h3 style="color: var(--accent-secondary); margin-bottom: 16px; font-size: 1.1em;">🚨 Entities Flagged for Investigation</h3>
            <div style="overflow-x: auto;">
                <table style="width: 100%; border-collapse: collapse; font-size: 0.9em;">
                    <thead>
                        <tr style="background: rgba(74, 158, 255, 0.2);">
                            <th style="padding: 12px 8px; text-align: left; border-bottom: 2px solid var(--border);">Email</th>
                            <th style="padding: 12px 8px; text-align: left; border-bottom: 2px solid var(--border);">Merchant</th>
                            <th style="padding: 12px 8px; text-align: center; border-bottom: 2px solid var(--border);">Fraud TX</th>
                            <th style="padding: 12px 8px; text-align: center; border-bottom: 2px solid var(--border);">Total TX</th>
                        </tr>
                    </thead>
                    <tbody>
                        {entity_rows}
