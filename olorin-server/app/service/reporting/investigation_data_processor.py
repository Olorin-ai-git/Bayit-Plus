...
class InvestigationPhase:
    ...
    @classmethod
    def from_activity_data(
        cls, activity: Dict[str, Any]
    ) -> Optional["InvestigationPhase"]:
        """Create InvestigationPhase from structured activity data"""
        if (
            activity.get("interaction_type")
            != InteractionType.INVESTIGATION_PROGRESS.value
        ):
            return None
...
class ProcessedInvestigationData:
    ...
    def get_agent_network_data(self) -> Dict[str, Any]:
        ...
        for agent in self.agents_used:
            nodes[agent] = {
                ...
                "type": "agent",
                "interactions": len(
                    [i for i in self.llm_interactions if i.agent_name == agent]
                ),
            }
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        if self.llm_interactions:
            response_times = [
                i.response_time_ms
                for i in self.llm_interactions
        ...
        if self.tool_executions:
            ...
            if execution_times:
                ...
                metrics["tool_success_rate"] = sum(
                    1 for t in self.tool_executions if t.success
                ) / len(self.tool_executions)
...
class InvestigationDataProcessor:
    ...
    def process_investigation_folder(
        ...
        try:
            ...
            if metadata_file.exists():
                try:
                    ...
                    result.investigation_id = metadata.get(
                        "investigation_id", "unknown"
                    )
            ...
            if activities_file.exists():
                try:
                    interactions_data = self._process_activities_file_streaming(
                        activities_file
                    )
                    ...
                    result.investigation_phases = interactions_data[
                    ...
                    error_msg = (
                        f"Error processing structured_activities.jsonl: {str(e)}"
                    )
                    processing_errors.append(error_msg)
                    logger.error(error_msg)
            ...
            if results_dir.exists() and results_dir.is_dir():
                try:
                    ...
                    logger.info(
                        f"Processed {len(results_data)} files from results folder"
                    )
                except Exception as e:
            ...
            if processing_errors:
                ...
                result.processing_status = (
                    ...
                    if files_processed > 0
                    else ProcessingStatus.EMPTY
                )

            # Add performance metrics
            if self.enable_performance_monitoring:
                result.processing_metrics = self._generate_processing_metrics(
                    files_processed, result.total_interactions, len(processing_errors)
                )

            logger.info(
                f"Processed investigation folder {folder_path.name}: "
            ...
            logger.error(
                f"Failed to process investigation folder {folder_path}: {str(e)}"
            )
            raise
    ...
    def _process_activities_file_streaming(
        self, activities_file: Path
    ) -> Dict[str, List[Any]]:
        ...
        if use_streaming:
            logger.info(
                f"Using streaming mode for large file: {file_size / (1024*1024):.1f} MB"
            )
        ...
        try:
            with open(activities_file, "r", encoding="utf-8") as f:
                    ...
                    try:
                        ...
                        if use_streaming and len(batch_buffer) >= self.batch_size:
                            self._process_activity_batch(
                                batch_buffer, interactions_data
                            )
                            batch_buffer = []

                    except json.JSONDecodeError as e:
                        logger.warning(
                            f"Invalid JSON on line {line_count} in {activities_file}: {e}"
                        )
                        continue
                ...
                logger.info(
                    f"Processed {line_count} activities from {activities_file.name}"
                )
    ...
    def _process_activity_batch(
        self, batch: List[Dict[str, Any]], interactions_data: Dict[str, List[Any]]
    ) -> None:
        ...
        for activity in batch:
            ...
            if interaction_type == InteractionType.LLM_CALL.value:
                ...
                if investigation_phase:
                    interactions_data["investigation_phases"].append(
                        investigation_phase
                    )
                ...
                for risk_entry in risk_progression:
                    if isinstance(risk_entry, dict) and "risk_score" in risk_entry:
                        risk_score_entry = RiskScoreEntry.from_progression_data(
                            risk_entry
                        )
    ...
    def _process_log_file_streaming(self, log_file: Path) -> List[LogEntry]:
        ...
        if use_streaming:
            logger.info(
                f"Using streaming mode for large log file: {file_size / (1024*1024):.1f} MB"
            )
                ...
                for line in f:
                    ...
                    try:
                        ...
                        if use_streaming and len(log_entries) >= 10000:
                            logger.info(
                                f"Truncated log processing at {len(log_entries)} entries"
                            )
                            break
    ...
    def _calculate_aggregated_statistics(
        self, result: ProcessedInvestigationData
    ) -> None:
        ...
        if all_timestamps:
            try:
                parsed_timestamps = [
                    result._parse_timestamp(ts) for ts in all_timestamps
                ]
    ...
    def _generate_processing_metrics(
        self, files_processed: int, total_records: int, errors_encountered: int
    ) -> ProcessingMetrics:
...
def process_investigation_folder(
    folder_path: Union[str, Path], **processor_kwargs
) -> ProcessedInvestigationData:
    """
