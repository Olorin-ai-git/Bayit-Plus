...
class InvestigationFolderAdapter(BaseAdapter):
    ...
    def _create_summary_from_folder(
        ...
        # Extract timing
        start_time = self.safe_extract_datetime(metadata, "start_time") or datetime.now()
        ...
        llm_calls = sum(1 for activity in activities if activity.get("type") == "llm_call")
        tool_executions = sum(1 for activity in activities if activity.get("type") == "tool_execution")
        ...
        if error_count > 0 or any(activity.get("status") == "failed" for activity in activities):
    ...
    def _extract_timeline_from_activities(self, activities: List[Dict[str, Any]]) -> List[TimelineEvent]:
        """Extract timeline events from activities."""
        events = []
        ...
        for activity in activities:
            ...
            title = activity.get("title", activity.get("action", f"{event_type.title()} Event"))
    ...
    def _create_risk_analysis_from_activities(
        ...
        for activity in activities:
            ...
            if risk_score > 0:
                ...
                agent = activity.get("agent")
                if agent:
                    if agent not in agent_risk_scores or risk_score > agent_risk_scores[agent]:
    ...
    def _extract_agents_from_activities(self, activities: List[Dict[str, Any]]) -> AgentAnalysisData:
        ...
        for agent, data in agent_interactions.items():
            ...
            if data["total"] > 0:
                agents_data.agent_success_rates[agent] = data["successes"] / data["total"]
    ...
    def _extract_tools_from_activities(self, activities: List[Dict[str, Any]]) -> ToolsAnalysisData:
    ...
    def _extract_flow_from_activities(self, activities: List[Dict[str, Any]]) -> InvestigationFlowData:
        ...
        for activity in activities:
            ...
            if "phase" in activity_type.lower() or activity.get("phase"):
                ...
                if not any(p.get("name") == phase_name for p in phases):
                    phases.append({
                        "name": phase_name,
                        "start_time": self.safe_extract_datetime(activity, "timestamp"),
                        "status": activity.get("status", "unknown"),
                        "agent": activity.get("agent")
                    })
                
                # Track completed phases
                if (activity.get("status") in ["completed", "success", "passed"] and 
                    phase_name not in completed_phases):
    ...
    def _convert_journey_data(self, journey_data: Dict[str, Any]) -> JourneyTrackingData:
    ...
    def _extract_performance_from_activities(self, activities: List[Dict[str, Any]]) -> PerformanceData:
        ...
        return PerformanceData(
            total_execution_time=total_execution_time if total_execution_time > 0 else None,
            average_response_time=average_response_time if average_response_time > 0 else None,
            api_call_counts=api_calls
        )
    
    def _extract_explanations_from_activities(self, activities: List[Dict[str, Any]]) -> List[ExplanationData]:
        """Extract explanations from activities."""
        explanations = []
        
        for i, activity in enumerate(activities):
            # Look for explanation-type activities
            if (activity.get("type") in ["explanation", "reasoning", "analysis"] or
                "explanation" in activity.get("description", "").lower()):
                
                timestamp = self.safe_extract_datetime(activity, "timestamp") or datetime.now()
