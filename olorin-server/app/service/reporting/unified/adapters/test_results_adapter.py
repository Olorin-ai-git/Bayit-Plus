...
class TestResultsAdapter(BaseAdapter):
    ...
    def _create_summary(
        self, test_data: Dict[str, Any], all_results: Dict[str, Any]
    ) -> InvestigationSummary:
        """Create investigation summary from test data."""
        investigation_id = test_data.get("investigation_id", "unknown")
        scenario_name = (
            list(all_results.keys())[0] if all_results else "Unknown Scenario"
        )

        # Extract timing information
        start_time = (
            self.safe_extract_datetime(test_data, "start_time") or datetime.now()
        )
        ...
        passed_tests = sum(
            1 for result in all_results.values() if result.get("status") == "PASSED"
        )
                ...
                if timestamp:
                    events.append(
                        TimelineEvent(
                            ...
                            risk_score=self.safe_extract_float(
                                phase_data, "risk_score"
                            ),
                            metadata=phase_data,
                        )
                    )
    ...
    def _extract_agents_data(self, test_data: Dict[str, Any]) -> AgentAnalysisData:
        ...
        for phase_name, phase_data in phases.items():
            if isinstance(phase_data, dict):
                ...
                if agent:
                    # Count usage
                    agents_data.agent_usage[agent] = (
                        agents_data.agent_usage.get(agent, 0) + 1
                    )
                    ...
                    agents_data.agent_success_rates[agent].append(
    ...
    def _extract_flow_data(self, test_data: Dict[str, Any]) -> InvestigationFlowData:
        ...
        return InvestigationFlowData(
            phases=phase_list, completed_phases=completed_phases
        )
                ...
                if explanation_text:
                    explanations.append(
                        ExplanationData(
                            ...
                            timestamp=self.safe_extract_datetime(
                                phase_data, "start_time"
                            )
                            or datetime.now(),
                            reasoning=explanation_text,
                            confidence=self.safe_extract_float(
                                phase_data, "confidence"
                            ),
                            metadata=phase_data,
                        )
                    )
