...
class EnhancedEventFeedService:
    ...
    def fetch_events_with_pagination(
        ...
        try:
            ...
            cursor_ts, cursor_seq = self._parse_cursor(cursor) if cursor else (None, None)
            ...
            if cursor_ts:
                query = query.filter(
                    or_(
                        ...
                        and_(
                            InvestigationAuditLog.timestamp == cursor_ts,
                            InvestigationAuditLog.entry_id > f"{cursor_ts}_{cursor_seq:06d}"
                        )
                    )
                )
            ...
            # Convert to events
            events = [
                self.filter_service.audit_log_to_event(entry)
            ...
            logger.debug(f"   - Converting {len(entries)} audit log entries to {len(events)} events")
            ...
            logger.debug(f"✅ EventsFeedResponse constructed: {len(response.items)} items")
            return response
    ...
    def get_audit_trail_summary(
        ...
        try:
            ...
            logger.error(f"Error getting audit summary for {investigation_id}: {str(e)}")
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
