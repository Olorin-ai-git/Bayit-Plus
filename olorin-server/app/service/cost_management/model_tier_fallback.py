...
class ModelTierFallback:
    ...
    async def select_model(self, 
                          ...
                          quality_requirement: Optional[float] = None) -> ModelSelection:
        """
        ...
        if preferred_model and preferred_model in suitable_models:
            selection = await self._try_model_selection(preferred_model, estimated_tokens)
            if selection:
                return selection
        ...
        for model_name in model_priority:
            ...
            if selection:
                ...
                if preferred_model and model_name != preferred_model:
                    ...
                    selection.fallback_reason = f"Cost constraints - fell back from {preferred_model}"
                    
                    # Calculate cost savings
                    if preferred_model in self.MODEL_CONFIGS:
                        preferred_cost = self._estimate_cost(preferred_model, estimated_tokens)
                        actual_cost = selection.estimated_cost
                        savings = preferred_cost - actual_cost
                        self.fallback_stats["cost_savings"] += savings
                        
                        # Calculate quality impact
                        preferred_quality = self.MODEL_CONFIGS[preferred_model].quality_score
    ...
    def _get_task_complexity(self, task_type: str) -> TaskComplexity:
        ...
        return self.INVESTIGATION_TASK_COMPLEXITY.get(
            task_type, 
            TaskComplexity.MODERATE  # Default to moderate
        )
    ...
    def _get_suitable_models(self, complexity: TaskComplexity) -> List[str]:
        ...
        if not suitable:
            for model_name, config in self.MODEL_CONFIGS.items():
                if any(task.value >= complexity.value for task in config.suitable_tasks):
                    suitable.append(model_name)
    ...
    def _prioritize_models(self, models: List[str]) -> List[str]:
        ...
        if self.force_economy_mode:
            ...
            return sorted(models, key=lambda m: self.MODEL_CONFIGS[m].cost_per_1k_output)
    ...
    async def _try_model_selection(self, model_name: str, estimated_tokens: int) -> Optional[ModelSelection]:
        """
        ...
        is_affordable, reason = await self.credit_monitor.validate_request_affordability(cost_estimate)
    ...
    def get_model_recommendations(self, task_type: str) -> Dict[str, Any]:
        ...
        for model_name in suitable_models:
            ...
            recommendations.append({
                ...
                "suitability": "optimal" if complexity in config.suitable_tasks else "capable"
            })
    ...
    def get_fallback_statistics(self) -> Dict[str, Any]:
        ...
        fallback_rate = (self.fallback_stats["fallback_count"] / total * 100) if total > 0 else 0
        ...
        return {
            ...
            "average_savings_per_fallback": (
                self.fallback_stats["cost_savings"] / self.fallback_stats["fallback_count"]
                if self.fallback_stats["fallback_count"] > 0 else 0
            )
        }
