...
from .anthropic_credit_monitor import get_credit_monitor, CreditStatus
from .model_tier_fallback import get_model_fallback
from .cost_optimization_framework import get_cost_optimization, BudgetAlert, BudgetPeriod
...
class RealTimeCostTracker:
    ...
    async def _monitor_credit_balance(self):
        ...
        while not self._shutdown_event.is_set():
            try:
                ...
                # Broadcast update
                await self._broadcast_update('budget_status', {
    ...
    async def _monitor_performance(self):
        ...
        while not self._shutdown_event.is_set():
            try:
                ...
                successful_requests = total_requests - circuit_health.get('open_breakers', 0)
                ...
                summary = PerformanceSummary(
                    ...
                    optimization_rate=opt_stats['optimization']['optimization_rate_percent'],
                ...
                # Update metrics
                perf_metrics = [
                    CostMetric("total_requests", MetricType.PERFORMANCE, 
                              summary.total_requests, "count", datetime.now()),
                    CostMetric("success_rate", MetricType.PERFORMANCE,
                              summary.successful_requests / max(summary.total_requests, 1) * 100,
                              "percent", datetime.now()),
                    CostMetric("average_cost", MetricType.COST,
                              summary.average_cost, "USD", datetime.now()),
                    CostMetric("optimization_rate", MetricType.OPTIMIZATION,
                              summary.optimization_rate, "percent", datetime.now()),
                    CostMetric("cache_hit_rate", MetricType.OPTIMIZATION,
                              summary.cache_hit_rate, "percent", datetime.now())
                ]
    ...
    async def _check_budget_threshold(self, period: BudgetPeriod):
        ...
        try:
            ...
            self.logger.error(f"Error checking budget threshold for {period.value}: {e}")
    
    async def _create_alert(self, alert_id: str, severity: AlertSeverity, 
                           metric_type: MetricType, title: str, message: str, 
                           data: Dict[str, Any]):
        """Create a new alert"""
        ...
        self.logger.log(log_level, f"Cost Alert: {title}", extra={
            'alert_id': alert_id,
    ...
    def get_current_dashboard_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'alerts': {alert_id: asdict(alert) for alert_id, alert in self.active_alerts.items()},
            'performance_history': [asdict(p) for p in self.performance_history[-10:]],  # Last 10
            'stats': self.stats,
            'status': {
