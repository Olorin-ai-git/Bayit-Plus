...
class AnthropicCreditMonitor:
    ...
    async def validate_request_affordability(
        ...
        if balance.balance < cost_estimate.estimated_cost:
            return (
                False,
                f"Insufficient balance: ${balance.balance:.4f} < ${cost_estimate.estimated_cost:.4f}",
            )

        # Check minimum balance buffer
        if (balance.balance - cost_estimate.estimated_cost) < self.thresholds[
            "minimum_balance"
        ]:
            return (
                False,
                f"Request would leave balance below minimum threshold: ${self.thresholds['minimum_balance']}",
            )

        # Check daily budget
        if (self._daily_usage + cost_estimate.estimated_cost) > self.thresholds[
            "daily_budget"
        ]:
            return (
                False,
                f"Request would exceed daily budget: ${self.thresholds['daily_budget']}",
            )

        # Check weekly budget
        if (self._weekly_usage + cost_estimate.estimated_cost) > self.thresholds[
            "weekly_budget"
        ]:
            return (
                False,
                f"Request would exceed weekly budget: ${self.thresholds['weekly_budget']}",
            )

        # Check monthly budget
        if (self._monthly_usage + cost_estimate.estimated_cost) > self.thresholds[
            "monthly_budget"
        ]:
            return (
                False,
                f"Request would exceed monthly budget: ${self.thresholds['monthly_budget']}",
            )

        return True, "Request approved"

    async def track_request_usage(
        self, cost_estimate: CostEstimate, actual_tokens: Optional[Dict] = None
    ):
        """
    ...
    async def get_usage_summary(self) -> Dict:
        ...
        return {
            ...
            "usage_percentages": {
                ...
                "monthly": (self._monthly_usage / self.thresholds["monthly_budget"])
                * 100,
            },
    ...
    def _generate_recommendations(self, balance: CreditBalance) -> List[str]:
        ...
        if balance.status == CreditStatus.CRITICAL:
            recommendations.append(
                "URGENT: Credit balance critically low - consider adding funds"
            )
        ...
        if weekly_pct > 80:
            recommendations.append(
                f"Weekly usage at {weekly_pct:.1f}% - consider budget increase"
            )
    ...
    async def health_check(self) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "cache_valid": self._cache_expiry
                and datetime.now() < self._cache_expiry,
            }
        except Exception as e:
            return {"status": "unhealthy", "error": str(e), "cache_valid": False}
