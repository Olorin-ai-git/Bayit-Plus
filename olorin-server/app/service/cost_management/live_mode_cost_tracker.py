...
class LiveModeCostTracker:
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "api_timeouts": {"anthropic": 30, "snowflake": 30, "external": 15},
            "cost_rates": {
    ...
    async def track_cost_event(
        ...
        try:
            ...
            # Log the cost event
            logger.info(
                f"Cost event tracked: {service} ${amount:.4f} ({category.value})"
            )
    ...
    async def _check_alert_thresholds(self):
        ...
        try:
            ...
            session_percentage = (
                total_cost / session_budget if session_budget > 0 else 0
            )
    ...
    async def _create_alert(
        ...
        try:
            ...
            existing_alert = next(
                (a for a in self.active_alerts if a.alert_id == alert_id), None
            )
    ...
    def get_current_costs(self) -> Dict[str, float]:
        ...
        return {category.value: total for category, total in self.cost_totals.items()}
    ...
    def generate_cost_report(self, period_hours: int = None) -> CostSummary:
        ...
        try:
            ...
            for category in CostCategory:
                ...
                cost_by_category[category.value] = sum(
                    e.amount for e in category_events
                )
            ...
            for event in events:
                ...
                cost_by_service[service] = (
                    cost_by_service.get(service, 0) + event.amount
                )
            ...
            for event in events:
                if event.investigation_id:
                    ...
                    cost_by_investigation[inv_id] = (
                        cost_by_investigation.get(inv_id, 0) + event.amount
                    )
