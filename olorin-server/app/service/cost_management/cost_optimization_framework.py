...
from .anthropic_credit_monitor import CostEstimate, get_credit_monitor
from .api_circuit_breaker import CircuitBreakerConfig, get_circuit_breaker
from .model_tier_fallback import ModelSelection, get_model_fallback
...
class CostOptimizationFramework:
    ...
    def __init__(self):
        ...
        self.budget_thresholds = {"warning": 0.8, "critical": 0.95}  # 80%  # 95%

        # Cache management
        self.request_cache: Dict[str, CacheEntry] = {}
        self.cache_stats = {"hits": 0, "misses": 0, "evictions": 0, "savings": 0.0}
    ...
    async def optimize_request(
    ...
    ) -> Tuple[ModelSelection, OptimizationResult]:
        """
        ...
        original_cost = await self._estimate_original_cost(
            preferred_model, prompt, max_tokens
        )
        ...
        if OptimizationStrategy.REQUEST_DEDUPLICATION in self.active_strategies:
            ...
            if cached_result:
                ...
                model_selection = ModelSelection(
                    ...
                    tier=self.model_fallback.MODEL_CONFIGS[
        ...
        optimization_result.savings = (
            optimization_result.original_cost - optimization_result.optimized_cost
        )
    ...
    async def _estimate_original_cost(
    ...
    def get_optimization_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "budget": {
                "limits": {
                    period.value: limit for period, limit in self.budget_limits.items()
                },
                "current_usage": {
                    period.value: usage
