...
class OrchestratorDashboardService:
    ...
    async def initialize_dashboard(self, investigation_id: str) -> Dict[str, Any]:
        ...
        try:
            self.logger.info(f"Initializing dashboard for investigation: {investigation_id}")
            ...
            investigation_state = await self.state_manager.get_investigation_state(investigation_id)
            ...
            self.flow_progress[investigation_id] = InvestigationFlowProgress(
                ...
                current_phase=investigation_state.get("current_phase", InvestigationPhase.INITIALIZATION),
                completed_phases=[],
                phase_durations={},
                bottlenecks=[],
                critical_path=[],
    ...
    async def update_decision_visualization(
        ...
        try:
            ...
            visualization = DashboardDecisionVisualization(
                ...
                visualization_type=self._determine_visualization_type(decision_data["decision_type"])
            )
            ...
            if len(self.decision_history[investigation_id]) > self.max_decision_history:
                self.decision_history[investigation_id] = \
                    self.decision_history[investigation_id][-self.max_decision_history:]
            ...
            self.logger.debug(f"Updated decision visualization: {decision_data['decision_id']}")
    ...
    async def update_coordination_timeline(
        ...
        try:
            ...
            timeline_entry = AgentCoordinationTimeline(
                ...
                coordination_mode=coordination_data.get("coordination_mode", "sequential"),
                dependencies=coordination_data.get("dependencies", []),
                estimated_completion=datetime.fromisoformat(
                    coordination_data.get("estimated_completion", 
            ...
            self.logger.debug(f"Updated coordination timeline: {coordination_data['coordination_id']}")
    ...
    async def update_investigation_flow(
        ...
        try:
            ...
            if "current_phase" in flow_data:
                flow_progress.current_phase = InvestigationPhase(flow_data["current_phase"])
            ...
            if "phase_duration" in flow_data:
                phase_name = flow_data.get("phase_name", str(flow_progress.current_phase))
            ...
            flow_progress.bottlenecks = await self._identify_bottlenecks(investigation_id)
            
            # Update quality score
            flow_progress.quality_score = await self._calculate_quality_score(investigation_id)
    ...
    async def update_performance_metrics(
        ...
        try:
            ...
            if "agent_handoff_efficiency" in metrics_data:
                metrics.agent_handoff_efficiency = metrics_data["agent_handoff_efficiency"]
            
            if "investigation_success_rate" in metrics_data:
                metrics.investigation_success_rate = metrics_data["investigation_success_rate"]
            
            if "resource_utilization" in metrics_data:
                metrics.resource_utilization.update(metrics_data["resource_utilization"])
            
            # Generate optimization recommendations
            metrics.optimization_recommendations = await self._generate_optimization_recommendations(
            ...
            # Detect anomalies
            metrics.anomalies = await self._detect_performance_anomalies(investigation_id, metrics)
            
            # Broadcast to dashboard connections
            await self._broadcast_dashboard_update(
                investigation_id,
                DashboardViewType.PERFORMANCE_METRICS,
                asdict(metrics)
            )
        ...
        try:
            if view_type == DashboardViewType.REAL_TIME_DECISIONS:
                ...
                return {
                    ...
                    "active_coordinations": len([t for t in timeline if t.status == "in_progress"])
                }
            
            elif view_type == DashboardViewType.INVESTIGATION_FLOW:
                flow = self.flow_progress.get(investigation_id)
                if not flow:
                    return {"view_type": view_type.value, "error": "Flow data not available"}
                return {
                    "view_type": view_type.value,
                    "flow_progress": asdict(flow)
                }
            
            elif view_type == DashboardViewType.PERFORMANCE_METRICS:
                metrics = self.performance_metrics.get(investigation_id)
                if not metrics:
                    return {"view_type": view_type.value, "error": "Metrics data not available"}
                return {
                    "view_type": view_type.value,
                ...
                return {
                    ...
                    "recommendations": await self._get_bottleneck_recommendations(bottlenecks)
                }
    ...
    async def _update_critical_path(self, investigation_id: str) -> None:
        ...
        try:
            ...
            for entry in timeline:
                ...
                edges.append((entry.source_agent, entry.target_agent, entry.estimated_completion))
    ...
    async def _identify_bottlenecks(self, investigation_id: str) -> List[Dict[str, Any]]:
    ...
    async def _calculate_quality_score(self, investigation_id: str) -> float:
        ...
        try:
            ...
            scores = {
                ...
                "completion_rate": len(self.flow_progress[investigation_id].completed_phases) / 
                                 len(InvestigationPhase) if investigation_id in self.flow_progress else 0.0,
                "response_time": min(1.0, 30.0 / max(1.0, metrics.decision_latency_ms / 1000.0)),
                "data_quality": 0.8  # Placeholder - would calculate from actual data quality metrics
            }
            
            # Weighted average
            quality_score = sum(scores[factor] * weight for factor, weight in factors.items())
    ...
    async def _generate_optimization_recommendations(
        ...
        try:
            ...
            if metrics.decision_latency_ms > 1000:  # > 1 second
                ...
                recommendations.append("Implement decision caching for similar scenarios")
            ...
            if metrics.investigation_success_rate < 0.9:
                recommendations.append("Review failure patterns and implement additional fallbacks")
    ...
    async def _get_bottleneck_recommendations(
        ...
        try:
            ...
            if len(bottlenecks) > 3:
                ...
                recommendations.append("Review overall investigation strategy for optimization")
