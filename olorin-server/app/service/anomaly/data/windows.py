...
async def fetch_windows_snowflake(
    ...
    if provider is None:
        ...
        logger.info(
            f"Using {database_provider} for transaction windows (analytics always uses PostgreSQL)"
        )
    ...
    try:
        ...
        provider_type = type(provider).__name__
        logger.error(
            f"Unexpected error connecting to {provider_type}: {e}", exc_info=True
        )
        raise ConnectionError(f"{provider_type} connection failed: {e}") from e
    ...
    if cohort_filters:
        for key, value in cohort_filters.items():
            if value is not None:
                ...
                db_col = column_mapping.get(
                    key.lower(), key.upper() if is_snowflake else key.lower()
                )
                if is_snowflake:
    ...
    if cohort_by:
        mapped_cohort_by = [
            column_mapping.get(
                col.lower(), col.upper() if is_snowflake else col.lower()
            )
    ...
    if metrics:
        ...
        mapped_metrics = [
            metric.upper() if is_snowflake else metric.lower() for metric in metrics
        ]
    ...
    if is_snowflake:
        ...
        where_clauses = ["window_start >= :window_from", "window_end <= :window_to"]
    ...
    try:
        ...
        if is_postgresql:
            ...
            results = await loop.run_in_executor(
                None, lambda: provider.execute_query(query, params)
            )
        ...
        if should_disconnect:
            try:
                ...
                logger.info(
                    f"Disconnected from {provider_type_name} after batch cohort processing"
                )
            except Exception as e:
                provider_type_name = "PostgreSQL" if is_postgresql else "Snowflake"
