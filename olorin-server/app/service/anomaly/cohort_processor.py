...
from app.models.anomaly import AnomalyEvent, Detector
    ...
    for cohort_idx, cohort in enumerate(cohorts):
        try:
            ...
            for anomaly_data in cohort_anomalies:
                ...
                log_anomaly_detected(
                    anomaly_id, run_id, detector.id, cohort, anomaly_data
                )
            ...
            logger.warning(f"Failed to process cohort {cohort}: {e}", exc_info=True)
            continue
        ...
        try:
            ...
            if postgres_provider:
                try:
                    ...
                    logger.debug(
                        "Disconnected PostgreSQL provider after batch cohort processing"
                    )
                except Exception as e:
