...
from app.models.anomaly import Detector, AnomalyEvent, DetectionRun
...
class ReplayComparisonService:
    ...
    def compare_replay(
        ...
        try:
            ...
            production_detector = db.query(Detector).filter(
                Detector.id == production_detector_id
            ).first()
            if not production_detector:
                raise ValueError(f"Production detector {production_detector_id} not found")
            ...
            replay_anomalies = db.query(AnomalyEvent).filter(
                AnomalyEvent.run_id == replay_run.id
            ).all()

            # Get production anomalies for the same window
            production_anomalies = db.query(AnomalyEvent).filter(
            ...
            # Compare anomalies
            comparison = self._compare_anomalies(
                replay_anomalies,
    ...
    def _anomaly_key(self, anomaly: AnomalyEvent) -> str:
        ...
        cohort_str = ",".join(
            f"{k}:{v}" for k, v in sorted(anomaly.cohort.items())
        )
