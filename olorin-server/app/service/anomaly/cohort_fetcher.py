...
def get_cohorts(
    ...
    if use_snowflake and database_provider == 'snowflake':
        ...
        provider_name = database_provider if database_provider in ['postgresql', 'snowflake'] else 'snowflake'
    
    logger.info(f"Using database provider: {provider_name} (USE_SNOWFLAKE={use_snowflake}, DATABASE_PROVIDER={database_provider})")
    
    provider = get_database_provider(provider_name)
    try:
        provider.connect()
    except Exception as e:
        logger.error(f"Failed to connect to {provider_name} for cohort query: {e}", exc_info=True)
        # Convert to ConnectionError so it can be properly handled by callers
        error_str = str(e).lower()
        if provider_name == 'snowflake' and ('oauth' in error_str or 'invalid' in error_str or 'authentication' in error_str):
            raise ConnectionError(f"Snowflake authentication failed. Please check your OAuth token: {e}") from e
        raise ConnectionError(f"Database connection failed ({provider_name}): {e}") from e
    ...
    try:
        ...
        for row in results:
            ...
            for db_col, value in row.items():
                ...
                detector_col = reverse_mapping.get(db_col.lower()) or reverse_mapping.get(db_col) or db_col
        ...
        logger.error(f"Failed to query cohorts from {provider_name}: {e}", exc_info=True)
        # Check if it's a database/connection error
        error_str = str(e).lower()
        if provider_name == 'snowflake' and ('snowflake' in error_str or 'oauth' in error_str or 'invalid' in error_str):
