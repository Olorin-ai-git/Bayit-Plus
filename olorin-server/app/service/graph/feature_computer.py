...
from app.service.graph.graph_client_abstraction import GraphClientAbstraction, GraphProvider
...
class FeatureComputer:
    ...
    def compute_cluster_detection(
        ...
        if not self.graph_client.is_available():
            logger.warning("Graph database not available, returning empty cluster detection")
        ...
        try:
            ...
            # Cache result
            self.redis_client.set(cache_key, json.dumps(result), ttl=self.cache_ttl_seconds)
    ...
    def compute_cluster_risk_score(
        ...
        try:
            ...
            total_shared_devices = sum(c.get("shared_devices", 0) for c in clusters.get("clusters", []))
            
            # Risk score formula
            risk_score = min(1.0, (cluster_count * 0.2) + (total_shared_devices * 0.1))
            
            # Cache result
            self.redis_client.set(cache_key, str(risk_score), ttl=self.cache_ttl_seconds)
    ...
    def compute_shared_device_count(
        ...
        try:
            ...
            # Cache result
            self.redis_client.set(cache_key, str(shared_count), ttl=self.cache_ttl_seconds)
    ...
    def compute_velocity_across_clusters(
        ...
        try:
            ...
            # Check cache
            cache_key = f"velocity_clusters:{entity_type}:{entity_id}:{time_window_hours}"
            cached_result = self.redis_client.get(cache_key)
            if cached_result:
                return json.loads(cached_result)
            ...
            for cluster in clusters.get("clusters", []):
                ...
                results = self.graph_client.execute_query(
                    query,
                    {"entity_id": cluster_entity_id, "time_window_hours": time_window_hours}
                )
                
                transaction_count = results[0].get("transaction_count", 0) if results else 0
                velocity = transaction_count / time_window_hours  # Transactions per hour
            ...
            logger.error(f"Failed to compute velocity across clusters: {e}", exc_info=True)
