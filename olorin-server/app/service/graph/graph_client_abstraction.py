...
class GraphClientAbstraction:
    ...
    def __init__(
        ...
        # Initialize clients
        self.neo4j_client = Neo4jClient() if self.provider == GraphProvider.NEO4J else None
        self.tigergraph_client = TigerGraphClient() if self.provider == GraphProvider.TIGERGRAPH else None
    ...
    def _get_configured_provider(self) -> GraphProvider:
        ...
        if self.tenant_id:
            tenant_provider = self.config_loader.load_secret(f"TENANT_{self.tenant_id}_GRAPH_PROVIDER")
            if tenant_provider:
                try:
    ...
    def _verify_availability(self) -> None:
        ...
        if self.provider == GraphProvider.NEO4J:
            if not self.neo4j_client or not self.neo4j_client.is_available():
                logger.warning("Neo4j not available, graph operations will fail gracefully")
        elif self.provider == GraphProvider.TIGERGRAPH:
            if not self.tigergraph_client or not self.tigergraph_client.is_available():
                logger.warning("TigerGraph not available, graph operations will fail gracefully")
    ...
    def is_available(self) -> bool:
        ...
        if self.provider == GraphProvider.NEO4J:
            ...
            return self.tigergraph_client is not None and self.tigergraph_client.is_available()
        return False
    ...
    def load_entity(
        ...
        if not self.is_available():
            logger.warning(f"Graph database not available, skipping entity load: {entity_type}:{entity_id}")
            return False
        ...
        try:
            if self.provider == GraphProvider.NEO4J:
                ...
                return self.tigergraph_client.load_entity(entity_type, entity_id, properties)
    ...
    def create_relationship(
        ...
        """
        if not self.is_available():
            logger.warning("Graph database not available, skipping relationship creation")
            return False
    ...
    def execute_query(
        ...
        """
        if not self.is_available():
            logger.warning("Graph database not available, returning empty query results")
            return []
