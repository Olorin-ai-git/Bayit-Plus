    ...
    try:
        ...
        try:
            ...
            if reported_is_public is False and actual_is_public:
                ...
                logger.info(
                    f"Corrected AbuseIPDB is_public flag for {ip_str}: false -> true"
                )
...
def normalize_virustotal(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        reputation_summary["detection_ratio"] = (
            detection_ratio if total_engines > 0 else 0.0
        )
...
def resolve_provider_conflicts(provider_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info(
        f"🛡️ PROVIDER CONFLICT RESOLUTION: Selected {conservative_provider} "
        f"(risk: {conservative_risk:.3f}) as most conservative"
    )

    if len(provider_risks) > 1:
        other_risks = {
            k: v for k, v in provider_risks.items() if k != conservative_provider
        }
...
def canonical_is_public(
    ip: str, vt_is_malicious: bool = False, abuse_is_public: bool = None
) -> bool:
    """
    ...
    try:
        ...
        local_public = not ip.startswith(
