...
from app.router.models.investigation_comparison_models import (
    ComparisonRequest, WindowSpec, WindowPreset, ComparisonOptions
)
...
async def find_entity_transaction_date_range(
    ...
    try:
        ...
        if not results or len(results) == 0:
            logger.debug(f"No transaction date range found for {entity_type}={entity_value}")
            return None
        ...
        if not earliest_str or not latest_str or tx_count == 0:
            logger.debug(f"Empty transaction date range for {entity_type}={entity_value}")
            return None
        ...
        logger.info(f"📅 Found transaction date range for {entity_type}={entity_value}: "
                   f"{earliest.date()} to {latest.date()} ({tx_count} transactions)")
        
        return (earliest, latest)
        
    except Exception as e:
        logger.warning(f"⚠️ Failed to find transaction date range for {entity_type}={entity_value}: {e}")
        return None
...
def detect_entity_type(entity_value: str) -> str:
    ...
    phone_pattern = r'^\+?[\d\s\-\(\)]+$'
    if re.match(phone_pattern, entity_value) and len(entity_value.replace(' ', '').replace('-', '').replace('(', '').replace(')', '')) >= 10:
        return "phone"
...
async def create_and_wait_for_investigation(
    ...
    try:
        ...
        logger.info(f"🔨 Creating investigation {investigation_id} for {entity_type}={entity_value}")
        ...
        try:
            ...
            if not structured_request:
                logger.error(f"❌ Failed to extract structured request for investigation {investigation_id}")
                return None
            ...
            logger.info(f"🚀 Triggering investigation execution for {investigation_id}...")
            # Use asyncio.create_task to run in background
            task = asyncio.create_task(execute_structured_investigation(
                investigation_id=investigation_id,
            ...
            logger.info(f"✅ Investigation {investigation_id} created and execution triggered, waiting for completion...")
                ...
                try:
                    ...
                    if status == "COMPLETED" and lifecycle_stage == "COMPLETED":
                        ...
                        if investigation:
                            ...
                            logger.warning(f"⚠️ Investigation {investigation_id} completed but not found in persistence layer")
                            return None
                    
                    elif status in ["ERROR", "FAILED", "CANCELLED"]:
                        logger.error(f"❌ Investigation {investigation_id} failed with status: {status}")
                        return None
            ...
            logger.warning(f"⏱️ Investigation {investigation_id} did not complete within {max_wait_seconds} seconds")
            return None
...
async def run_auto_comparison_for_entity(
    ...
    now = datetime.now(pytz.timezone("America/New_York"))
    window_duration_days = int(os.getenv('INVESTIGATION_DEFAULT_WINDOW_DAYS', '60'))  # Default: 60 days (changed from 14)
    ...
    historical_window_start = historical_window_end - timedelta(days=window_duration_days)  # window_duration_days before that
    
    logger.info(f"🔍 Looking for investigations that cover time window: {historical_window_start.date()} to {historical_window_end.date()}")
    logger.info(f"   (Matching by investigation window dates, not creation time)")
    
    # Check if we should use existing investigations
    use_existing = os.getenv('USE_EXISTING_INVESTIGATIONS_FOR_COMPARISON', 'true').lower() == 'true'
    
    if not use_existing:
        logger.info(f"⚠️ USE_EXISTING_INVESTIGATIONS_FOR_COMPARISON=false - will create new investigations instead of using existing ones")
        all_investigations = []
    else:
    ...
    for inv in all_investigations:
        ...
        within_tolerance = (
            start_diff_start <= tolerance_days or  # Investigation start near target start
            start_diff_end <= tolerance_days or    # Investigation start near target end
            end_diff_start <= tolerance_days or    # Investigation end near target start
            end_diff_end <= tolerance_days or      # Investigation end near target end
            has_overlap                            # Or windows overlap
        )
        
        if within_tolerance:
            # Calculate overlap percentage for ranking
            if has_overlap:
                target_duration = (historical_window_end - historical_window_start).total_seconds()
                overlap_duration = (overlap_end - overlap_start).total_seconds()
                overlap_pct = (overlap_duration / target_duration) * 100 if target_duration > 0 else 0
            else:
    ...
    if matching_investigations:
        ...
        matching_investigations.sort(key=lambda x: (x['overlap_pct'], x['risk_score']), reverse=True)
        best_match = matching_investigations[0]
        historical_inv = best_match['inv']
        
        logger.info(f"✅ Found matching investigation: {historical_inv.get('id', 'unknown')}")
        logger.info(f"   Investigation window: {best_match['inv_from'].date()} to {best_match['inv_to'].date()}")
        logger.info(f"   Target window: {historical_window_start.date()} to {historical_window_end.date()}")
    ...
    if historical_inv:
        ...
        if inv_from and inv_to:
            ...
            if window_duration > 30:
                logger.warning(f"⚠️ Investigation {historical_inv.get('id', 'unknown')} has unreasonable window duration ({window_duration} days), skipping...")
                historical_inv = None
    ...
    if historical_inv:
        ...
        if validation_period_end_candidate.tzinfo is None:
            validation_period_end_candidate = tz.localize(validation_period_end_candidate)
        else:
            validation_period_end_candidate = validation_period_end_candidate.astimezone(inv_to.tzinfo)
        ...
        logger.info(f"✅ Found valid historical investigation: {historical_inv.get('id', 'unknown')}")
        ...
        logger.info(f"   Window A (Historical Investigation): {window_a_start.date()} to {window_a_end.date()}")
        logger.info(f"   Window B (Validation Period): {window_b_start.date()} to {window_b_end.date()}")
        ...
        if not availability.get('available', False):
            ...
            logger.warning(f"⚠️ No transaction data found in investigation windows (Window A: {window_a_count}, Window B: {window_b_count})")
            
            # Check if we should use existing investigations
            use_existing = os.getenv('USE_EXISTING_INVESTIGATIONS_FOR_COMPARISON', 'true').lower() == 'true'
            
            if not use_existing:
                logger.info(f"⚠️ USE_EXISTING_INVESTIGATIONS_FOR_COMPARISON=false - skipping search for existing investigations")
                all_investigations = []
            else:
                logger.info(f"🔍 Searching for oldest investigation with transaction data...")
            ...
            valid_investigations = [
                ...
                if inv.get('status', '').upper() == 'COMPLETED' and inv.get('overall_risk_score') is not None
            ]
            ...
            if valid_investigations:
                ...
                valid_investigations.sort(key=lambda x: x.get('_parsed_from') or datetime.min.replace(tzinfo=tz))
                ...
                for oldest_inv in valid_investigations:
                    ...
                    if isinstance(oldest_from, str):
                        oldest_from = datetime.fromisoformat(oldest_from.replace('Z', '+00:00'))
                    if isinstance(oldest_to, str):
                        oldest_to = datetime.fromisoformat(oldest_to.replace('Z', '+00:00'))
                    ...
                    oldest_availability = await check_data_availability(
                        ...
                        window_b_end=min(now, oldest_to + timedelta(days=90)),  # Up to 90 days after
                        merchant_ids=None,
                        is_snowflake=None
                    )
                    
                    if oldest_availability.get('available', False):
                        logger.info(f"✅ Found oldest investigation with transaction data: {oldest_inv.get('id', 'unknown')}")
                        logger.info(f"   Window: {oldest_from.date()} to {oldest_to.date()}")
                        logger.info(f"   Risk Score: {oldest_inv.get('overall_risk_score', 'N/A')}")
                        ...
                        if validation_period_end_candidate.tzinfo is None:
                            ...
                            validation_period_end_candidate = validation_period_end_candidate.astimezone(oldest_to.tzinfo)
                        ...
                        if window_b_end <= window_b_start:
                            ...
                            if window_b_end <= window_b_start:
                                logger.error(f"❌ Cannot create valid validation window for oldest investigation. Skipping.")
                                continue
                    ...
                    if valid_investigations:
                        oldest_inv_fallback = valid_investigations[0]  # Already sorted by oldest first
                        ...
                        if oldest_from_fallback and oldest_to_fallback:
                            ...
                            if isinstance(oldest_from_fallback, str):
                                oldest_from_fallback = datetime.fromisoformat(oldest_from_fallback.replace('Z', '+00:00'))
                            if isinstance(oldest_to_fallback, str):
                                oldest_to_fallback = datetime.fromisoformat(oldest_to_fallback.replace('Z', '+00:00'))
                            
                            if oldest_from_fallback.tzinfo is None:
                                oldest_from_fallback = tz.localize(oldest_from_fallback)
                            else:
                                oldest_from_fallback = oldest_from_fallback.astimezone(tz)
                            ...
                            logger.warning(f"⚠️ No investigations found with transaction data")
                            logger.info(f"📅 Using oldest investigation window (no transaction data): {oldest_inv_fallback.get('id', 'unknown')}")
                            logger.info(f"   Window: {oldest_from_fallback.date()} to {oldest_to_fallback.date()}")
                            logger.info(f"   Risk Score: {oldest_inv_fallback.get('overall_risk_score', 'N/A')}")
                            ...
                            validation_period_end = min(now, oldest_to_fallback + timedelta(days=90))
                            window_b_start = oldest_to_fallback
                            window_b_end = validation_period_end
                        else:
                            logger.warning(f"⚠️ Oldest investigation has invalid dates, will skip comparison")
                            historical_inv = None
                    else:
                        logger.warning(f"⚠️ No valid completed investigations found, will skip comparison")
                        historical_inv = None
            else:
        ...
        logger.info(f"⚠️ No historical investigation found - creating new investigation...")
        ...
        historical_window_end = now - timedelta(days=max_lookback_days)
        historical_window_start = historical_window_end - timedelta(days=window_duration_days)
        ...
        if availability.get('window_a_count', 0) == 0:
            logger.info(f"⚠️ No transaction data found in desired historical window ({historical_window_start.date()} to {historical_window_end.date()})")
            logger.info(f"🔍 Finding actual transaction date range for {entity_type}={entity_value}...")
            
            date_range = await find_entity_transaction_date_range(entity_type, entity_value, lookback_days=180)
                ...
                if days_span >= min_required_span:
                    ...
                    if target_start < earliest_date:
                        ...
                        if target_end > effective_max_end:
                            ...
                            target_start = max(earliest_date, target_end - timedelta(days=actual_window_days))
                    ...
                    if target_start >= target_end:
                        ...
                        if earliest_date < effective_max_end:
                            ...
                            logger.warning(f"⚠️ Skipping transaction date range adjustment - using default windows")
                    ...
                    if target_start is not None and target_end is not None:
                        ...
                        logger.info(f"   Historical window: {historical_window_start.date()} to {historical_window_end.date()}")
                        logger.info(f"   Window duration: {(historical_window_end - historical_window_start).days} days (tolerance: ±{span_tolerance_days} days)")
                    else:
                        logger.info(f"📅 Using default window dates (transaction range adjustment skipped)")
                else:
                    logger.warning(f"⚠️ Insufficient transaction span ({days_span} days, minimum {min_required_span} days required with tolerance)")
                    # Fall through to use default windows
            else:
                logger.warning(f"⚠️ No transaction data found for {entity_type}={entity_value} in last 180 days")
                # Fall through to use default windows
        ...
        if historical_window_end_utc > max_allowed_end_utc:
            logger.warning(f"⚠️ Window end {historical_window_end_utc.date()} exceeds max lookback ({max_allowed_end_utc.date()}), capping to {max_allowed_end_utc.date()}")
        ...
        if historical_window_start_utc >= historical_window_end_utc:
            logger.warning(f"⚠️ Invalid window: start {historical_window_start_utc.date()} >= end {historical_window_end_utc.date()}, adjusting...")
            # Recalculate: set end to max_allowed_end, start to window_duration_days before that
            historical_window_end = max_allowed_end
            historical_window_end_utc = max_allowed_end_utc
            historical_window_start = historical_window_end - timedelta(days=window_duration_days)
            historical_window_start_utc = historical_window_start.astimezone(utc) if historical_window_start.tzinfo else utc.localize(historical_window_start)
            
            # Final check: if start is still >= end, use minimum valid window
            if historical_window_start_utc >= historical_window_end_utc:
                logger.error(f"⚠️ Cannot create valid window with duration {window_duration_days} days within max lookback constraint")
                # Use a minimal 1-day window ending at max_allowed_end
                historical_window_end = max_allowed_end
                historical_window_start = historical_window_end - timedelta(days=1)
                logger.warning(f"⚠️ Using minimal 1-day window: {historical_window_start.date()} to {historical_window_end.date()}")
        ...
        if start_utc >= end_utc:
            raise ValueError(f"Invalid window: start ({start_utc.date()}) must be before end ({end_utc.date()})")
        
        logger.info(f"🔨 Creating historical investigation:")
        logger.info(f"   Window: {historical_window_start.date()} to {historical_window_end.date()}")
        ...
        if historical_inv:
            ...
            if historical_inv:
                logger.info(f"✅ Created and completed historical investigation: {historical_inv.get('id', 'unknown')}")
                logger.info(f"📊 Prediction Validation Setup:")
                logger.info(f"   Window A (Historical Investigation): {window_a_start.date()} to {window_a_end.date()}")
                logger.info(f"   Window B (Validation Period): {window_b_start.date()} to {window_b_end.date()}")
                    ...
                    if domain_findings and isinstance(domain_findings, dict):
                        ...
                        if isinstance(risk_domain, dict):
                            ...
                            if risk_score is not None and risk_score != 0.0:
                                ...
                                logger.info(f"   Predicted Risk: {predicted_risk_display:.3f} (extracted from domain_findings.risk.risk_score)")
                            else:
                                logger.info(f"   Predicted Risk: {predicted_risk_display} (no risk score available)")
                        else:
                            logger.info(f"   Predicted Risk: {predicted_risk_display} (no risk domain findings)")
                    else:
                        logger.info(f"   Predicted Risk: {predicted_risk_display} (no domain_findings available)")
                else:
            ...
            logger.warning(f"⚠️ Failed to create historical investigation, falling back to transaction-based comparison...")
            date_range = await find_entity_transaction_date_range(entity_type, entity_value, lookback_days=180)
            ...
            if date_range:
                ...
                if days_span >= 30:
                    ...
                    logger.info(f"📊 Using transaction-based comparison (no investigation found):")
                    logger.info(f"   Window A: {window_a_start.date()} to {window_a_end.date()}")
                    logger.info(f"   Window B: {window_b_start.date()} to {window_b_end.date()}")
                    ...
                    logger.info(f"📊 Using default windows (insufficient transaction span)")
                    request = ComparisonRequest(
                        entity={
                            "type": entity_type,
                            "value": entity_value
                        ...
                        windowB=WindowSpec(
                            preset=WindowPreset.RECENT_14D,
                            label="Recent 14d"
                        ),
                        risk_threshold=float(os.getenv("RISK_THRESHOLD_DEFAULT", "0.3")),
                        options=ComparisonOptions(
                ...
                logger.info(f"📊 Using default windows (no transaction date range found)")
                request = ComparisonRequest(
                    entity={
                        "type": entity_type,
                        "value": entity_value
    ...
    if not historical_inv:
        ...
        if 'request' not in locals():
            logger.warning(f"⚠️ No valid investigation found for {entity_type}={entity_value} after all checks")
            return {
                "status": "skipped",
                "entity_type": entity_type,
                "entity_value": entity_value,
    ...
    try:
        ...
        if request.windowA.preset == WindowPreset.CUSTOM:
            ...
            window_a_start, window_a_end, _ = compute_window(
                request.windowA.preset,
                request.windowA.start,
                request.windowA.end
            )
        ...
        if request.windowB.preset == WindowPreset.CUSTOM:
            ...
            window_b_start, window_b_end, _ = compute_window(
                request.windowB.preset,
                request.windowB.start,
                request.windowB.end
            )
        ...
        if historical_inv:
            ...
            logger.info(f"   Investigation Risk Score: {historical_inv.get('overall_risk_score', 'N/A')}")
        logger.info(f"   Window A: {window_a_start.date()} to {window_a_end.date()} ({final_availability.get('window_a_count', 0)} transactions)")
        logger.info(f"   Window B: {window_b_start.date()} to {window_b_end.date()} ({final_availability.get('window_b_count', 0)} transactions)")
        ...
        if response.windowA.auto_expand_meta and response.windowA.auto_expand_meta.expanded:
            ...
            logger.info(f"✅ Window A expanded: {original_days}d → {effective_days}d to reach minimum support")
            if response.windowA.auto_expand_meta.reasons:
                logger.info(f"   Expansion reasons: {', '.join(response.windowA.auto_expand_meta.reasons)}")
        ...
        if response.windowB.auto_expand_meta and response.windowB.auto_expand_meta.expanded:
            ...
            logger.info(f"✅ Window B expanded: {original_days}d → {effective_days}d to reach minimum support")
            if response.windowB.auto_expand_meta.reasons:
                logger.info(f"   Expansion reasons: {', '.join(response.windowB.auto_expand_meta.reasons)}")
        ...
        if response.A.ci:
            for metric, ci in response.A.ci.items():
                if ci and len(ci) == 2:
                    width = ci[1] - ci[0]
                    if width > 0.10:
                        logger.warning(f"⚠️ Wide CI for Window A {metric}: {width:.3f} (95% CI {format_percentage(ci[0], 1)}–{format_percentage(ci[1], 1)})")
        ...
        if response.B.ci:
            for metric, ci in response.B.ci.items():
                if ci and len(ci) == 2:
                    width = ci[1] - ci[0]
                    if width > 0.10:
                        logger.warning(f"⚠️ Wide CI for Window B {metric}: {width:.3f} (95% CI {format_percentage(ci[0], 1)}–{format_percentage(ci[1], 1)})")
        ...
        try:
            ...
            logger.warning(f"⚠️ Failed to create individual entity package: {e}", exc_info=True)
        ...
        return {
            ...
            "entity_package_path": str(entity_package_path) if entity_package_path else None,
            "comparison_response": response,
            "investigation_id": historical_inv.get('id') if historical_inv else None,
            "metrics": {
        ...
        error_msg = str(e)
        logger.error(f"❌ Auto-comparison failed for {entity_type}={entity_value}: {error_msg}")
        ...
        error_msg = str(e)
        logger.error(f"❌ Auto-comparison failed for {entity_type}={entity_value}: {error_msg}", exc_info=True)
        return {
            "status": "error",
            "entity_type": entity_type,
            "entity_value": entity_value,
...
async def run_auto_comparisons_for_top_entities(
    ...
    if not entities:
        logger.warning("⚠️ No entities found in risk analyzer results - skipping investigation creation (no crash)")
        return []
    ...
    if actual_count < requested_count:
        logger.info(f"🚀 Starting auto-comparisons for top {actual_count} riskiest entities (requested {requested_count}, but only {actual_count} available)")
    else:
        logger.info(f"🚀 Starting auto-comparisons for top {actual_count} riskiest entities (unconditional execution)")
    ...
    for i, entity_data in enumerate(top_entities, 1):
        ...
        entity_type = detected_type if detected_type != 'email' or default_entity_type == 'email' else default_entity_type
        
        risk_score = entity_data.get('risk_score') or entity_data.get('avg_risk_score', 0)
    ...
    if ci_widths['precision'] is not None or ci_widths['recall'] is not None or ci_widths['accuracy'] is not None:
        precision_str = f"{ci_widths['precision']:.3f}" if ci_widths['precision'] is not None else "N/A"
        recall_str = f"{ci_widths['recall']:.3f}" if ci_widths['recall'] is not None else "N/A"
        accuracy_str = f"{ci_widths['accuracy']:.3f}" if ci_widths['accuracy'] is not None else "N/A"
        logger.info(f"   Avg CI widths - Precision: {precision_str}, "
                   f"Recall: {recall_str}, "
...
async def _create_individual_entity_package(
    ...
    package_dir = reports_dir / f"entity_{entity_type}_{entity_value[:30]}_{timestamp_str}"
    package_dir.mkdir(parents=True, exist_ok=True)
    
    # Create ZIP filename
    zip_filename = f"entity_package_{entity_type}_{entity_value[:30]}_{timestamp_str}.zip"
    ...
    with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
        ...
        confusion_table_pattern = f"confusion_table_{investigation_id}_*.html" if investigation_id else None
                    ...
                    if year_dir.is_dir() and year_dir.name.isdigit():
                        for month_dir in year_dir.iterdir():
                            if month_dir.is_dir() and month_dir.name.isdigit():
                                ...
                                if potential_inv_folder.exists() and potential_inv_folder.is_dir():
...
<body>
    <div class="container">
        ...
        metadata = {
            ...
            "metrics": {
                "window_a_transactions": comparison_response.A.total_transactions if comparison_response and comparison_response.A else None,
                "window_b_transactions": comparison_response.B.total_transactions if comparison_response and comparison_response.B else None,
                "precision_a": comparison_response.A.precision if comparison_response and comparison_response.A else None,
                "recall_a": comparison_response.A.recall if comparison_response and comparison_response.A else None,
            }
        }
...
async def package_comparison_results(
    ...
    logger.info(f"📦 Creating comparison package: {zip_path} (using FileOrganizationService)")
    ...
    with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
        ...
        for i, result in enumerate(comparison_results, 1):
            if result.get('status') != 'success':
                logger.warning(f"⚠️ Skipping failed comparison {i}: {result.get('entity_value', 'unknown')}")
                continue
            ...
            if investigation_id:
                ...
                entity_artifacts_dir_alt = artifacts_base / entity_type / entity_value.replace('@', '-at-')
                    ...
                    if entity_type_dir.exists():
                        ...
                        for possible_folder in entity_type_dir.iterdir():
                            if possible_folder.is_dir():
                                ...
                                investigation_files = list(possible_folder.glob(f"investigation_{entity_type}_*.json"))
                ...
                if inv_folder and inv_folder.exists():
                    ...
                    folder_name_in_zip = f"investigations/investigation_{i}_{investigation_id}"
                    
                    # Ensure confusion table exists - generate if missing
                    confusion_table_path = inv_folder / f"confusion_table_{investigation_id}.html"
                    if not confusion_table_path.exists() and investigation_id:
                        logger.info(f"   📊 Confusion table not found, generating for {investigation_id}...")
                        try:
                            from app.service.investigation.confusion_table_generator import generate_confusion_table_sync
                            generated_path = generate_confusion_table_sync(investigation_id, inv_folder)
                            if generated_path and generated_path.exists():
                                logger.info(f"   ✅ Generated confusion table: {generated_path.name}")
                            else:
                                logger.warning(f"   ⚠️ Confusion table generation scheduled but not yet complete")
                        except Exception as e:
                            logger.warning(f"   ⚠️ Failed to generate confusion table: {e}")
                    ...
                    for root, dirs, files in os.walk(inv_folder):
                        ...
                        for file in files:
                            ...
                            original_file_path = file_path  # Keep original for arcname calculation
                            ...
                            if file_path.is_symlink():
                                try:
                                    ...
                                    if os.path.isabs(symlink_target):
                                        ...
                                        resolved_path = workspace_root / Path(symlink_target).relative_to('workspace')
                                    else:
                                        # Target is relative to symlink's directory
                                        resolved_path = (file_path.parent / symlink_target).resolve()
                                    ...
                                    logger.debug(f"   Resolved symlink: {original_file_path} -> {symlink_target} (resolved: {resolved_path})")
                            ...
                            if not file_path.exists():
                                logger.warning(f"   ⚠️ File does not exist: {file_path}. Skipping.")
                                continue
                            ...
                            arcname = folder_name_in_zip / original_file_path.relative_to(inv_folder)
                            try:
                                zipf.write(file_path, str(arcname))
                            except FileNotFoundError as e:
                                logger.warning(f"   ⚠️ Failed to add file to zip: {file_path}: {e}. Skipping.")
                                continue
                    
                    logger.info(f"   ✅ Added investigation folder: {investigation_id} from {inv_folder}")
                    ...
                    if not investigation_metadata:
                        investigation_json_files = list(inv_folder.glob(f"investigation_{entity_type}_*.json"))
                        if investigation_json_files:
                            # Use the most recent JSON file
                            latest_json = max(investigation_json_files, key=lambda p: p.stat().st_mtime)
                            try:
                                ...
                                logger.warning(f"   ⚠️ Failed to load investigation JSON: {e}")
                    ...
                    logger.warning(f"   ⚠️ Investigation artifacts folder not found for {investigation_id}")
                    logger.warning(f"      Looked in: {entity_artifacts_dir} and {entity_artifacts_dir_alt}")
        ...
        try:
            ...
            for i, result in enumerate(comparison_results, 1):
                ...
                manifest = manifest_generator.generate_comparison_manifest(
                    ...
                    right_investigation=investigation_id or "unknown",  # Same investigation for auto-comparisons
                ...
                try:
                    ...
                    logger.warning(f"Failed to index comparison {comparison_id} in registry: {e}")
            ...
            if comparison_manifests:
                ...
                logger.info(f"   ✅ Added comparison manifests ({len(comparison_manifests)} manifests)")
        except Exception as e:
        ...
        if startup_report_path:
            ...
            if startup_report.exists():
                ...
                logger.info(f"   ✅ Added startup analysis report: startup_analysis_report.html")
            else:
...
"""
    ...
    total_comparisons = sum(1 for inv in investigation_data if inv.get('comparison_response'))
    
    html_parts.append("""
        <div class="summary-stats">
            <div class="stat-card">
                <div class="stat-value">""" + str(total_investigations) + """</div>
                <div class="stat-label">Investigations</div>
            </div>
            <div class="stat-card">
                <div class="stat-value">""" + str(total_comparisons) + """</div>
    ...
    for inv in investigation_data:
        ...
        if metrics:
            ...
            if precision_delta is not None:
                delta_class = "delta-positive" if precision_delta > 0 else "delta-negative" if precision_delta < 0 else "delta-neutral"
                html_parts.append(f"""
                <div class="metric-card">
            ...
            if recall_delta is not None:
                delta_class = "delta-positive" if recall_delta > 0 else "delta-negative" if recall_delta < 0 else "delta-neutral"
                html_parts.append(f"""
                <div class="metric-card">
