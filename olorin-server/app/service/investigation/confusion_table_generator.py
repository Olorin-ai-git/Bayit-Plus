...
async def generate_confusion_table_for_investigation(
    ...
    if not lock.acquire(blocking=False):
        logger.info(f"⏭️  Confusion table generation already in progress for {investigation_id}, skipping duplicate request")
        return None
    
    try:
        # Import here to avoid circular dependencies
        from scripts.generate_confusion_table_for_investigation import generate_confusion_table
        ...
        if investigation_folder:
            output_path = investigation_folder / f"confusion_table_{investigation_id}.html"
        else:
        ...
        if output_path.exists():
            logger.info(f"⏭️  Confusion table already exists for {investigation_id} at {output_path}, skipping generation")
            return output_path
        
        logger.info(f"📊 Generating confusion table for investigation {investigation_id} -> {output_path}")
        ...
        if result and result.get("html_path"):
            ...
            logger.warning(f"⚠️ Confusion table generation returned no result for {investigation_id}")
            return None
            
    except Exception as e:
        logger.error(f"❌ Failed to generate confusion table for investigation {investigation_id}: {e}", exc_info=True)
        return None
    finally:
        lock.release()
...
def generate_confusion_table_sync(
    ...
    try:
        ...
        if loop.is_running():
            ...
            logger.info(f"📊 Scheduling confusion table generation as background task for {investigation_id}")
            task = asyncio.create_task(
                generate_confusion_table_for_investigation(investigation_id, investigation_folder)
            )
