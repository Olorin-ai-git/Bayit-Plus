...
class EnhancedRiskScorer:
    ...
    def __init__(self):
        ...
        self.merchant_risk_multipliers = self.feature_calculator.merchant_risk_multipliers
    ...
    def _get_progressive_threshold(self, tx_count: int, merchant_name: str = None) -> Tuple[float, str]:
        """
        ...
        if merchant_name:
            ...
            for merchant_key, multiplier in self.merchant_risk_multipliers.items():
                if merchant_key in merchant_lower:
                    ...
                    if multiplier < 1.0:
                        reason = f'high_risk_merchant ({merchant_name}, {threshold:.2f})'
                    else:
                        reason = f'low_risk_merchant ({merchant_name}, {threshold:.2f})'
                    logger.debug(f"   Merchant adjustment: {merchant_name} → {original_threshold:.2f} * {multiplier} = {threshold:.2f}")
                    break
    ...
    def _is_borderline_fraud(self, features: Dict[str, float], anomalies: List[Dict]) -> bool:
        """
        ...
        if features.get('tx_count', 0) > 8:
            if (features.get('single_merchant', 0) > 0 or 
                features.get('single_device', 0) > 0 or
        ...
        if features.get('max_repeated_amount_ratio', 0) > 0.6 and features.get('tx_count', 0) > 4:
            return True
        
        return False
    
    def _is_whitelisted_pattern(self, features: Dict[str, float], primary_merchant: str = None) -> bool:
        """
        ...
        if primary_merchant:
            ...
            subscription_merchants = ['netflix', 'spotify', 'apple', 'amazon prime', 'hulu', 'disney']
    ...
    def calculate_entity_risk(
        ...
        logger.info(f"📊 Calculating enhanced risk for {entity_type}:{entity_id} with {len(transactions)} transactions")
        ...
        merchants = [tx.get('MERCHANT') or tx.get('MERCHANT_NAME') for tx in transactions if tx.get('MERCHANT') or tx.get('MERCHANT_NAME')]
        merchant_counts = {}
        for m in merchants:
            if m:
                merchant_counts[m] = merchant_counts.get(m, 0) + 1
        primary_merchant = max(merchant_counts.keys(), key=lambda k: merchant_counts[k]) if merchant_counts else None
        
        # Get progressive threshold
        tx_count = len(transactions)
        adaptive_threshold, threshold_reason = self._get_progressive_threshold(tx_count, primary_merchant)
        ...
        for tx in transactions:
            tx_id = (tx.get('TX_ID_KEY') or tx.get('tx_id_key') or 
                    tx.get('TX_ID') or tx.get('tx_id') or
                    str(hash(str(tx))))
        ...
        if is_fraud:
            whitelisted = self._is_whitelisted_pattern(overall_assessment['features'], primary_merchant)
            if whitelisted:
                is_fraud = False
        ...
        if not is_fraud and 0.10 <= risk_score < adaptive_threshold:
            if self._is_borderline_fraud(overall_assessment['features'], overall_assessment['anomalies']):
                is_fraud = True
                refinement_applied = True
                logger.info(f"   ⚠️ Borderline case flagged as fraud (score: {risk_score:.3f}, threshold: {adaptive_threshold:.3f})")
        ...
        logger.info(f"   Overall risk: {risk_score:.3f} ({overall_assessment['risk_level']})")
    ...
    def enhance_investigation_results(
        ...
        enhanced_results['transaction_scores'].update(risk_assessment['transaction_scores'])
        ...
        if risk_assessment['overall_risk_score'] > original_risk:
            enhanced_results['overall_risk_score'] = risk_assessment['overall_risk_score']
            logger.info(f"🔄 Risk score updated: {original_risk:.3f} → {risk_assessment['overall_risk_score']:.3f}")
