...
def _build_approved_filter(decision_col: str, db_provider: str, use_fallback: bool = True) -> str:
    """
...
def classify_transaction_fraud(risk_score: Optional[float], risk_threshold: float) -> str:
    """
...
async def query_isfraud_tx_for_transactions(
    ...
    logger.info(f"[QUERY_ISFRAUD_TX] Querying {len(transaction_ids)} transactions for IS_FRAUD_TX values")
    logger.info(f"[QUERY_ISFRAUD_TX] Window end: {window_end_utc} (UTC)")
    logger.info(f"[QUERY_ISFRAUD_TX] Full query: {query}")
    logger.info(f"[QUERY_ISFRAUD_TX] Sample transaction IDs (first 5): {transaction_ids[:5]}")
    logger.info(f"[QUERY_ISFRAUD_TX] Transaction ID types: {[type(tx_id).__name__ for tx_id in transaction_ids[:5]]}")
    logger.info(f"[QUERY_ISFRAUD_TX] Transaction ID samples (as strings): {[str(tx_id) for tx_id in transaction_ids[:5]]}")
    
    # Check if diagnostic queries are enabled (default: false for performance)
    enable_diagnostics = os.getenv('ENABLE_ISFRAUD_TX_DIAGNOSTICS', 'false').lower() == 'true'
    
    if not enable_diagnostics:
        logger.debug("[QUERY_ISFRAUD_TX] Diagnostic queries disabled (ENABLE_ISFRAUD_TX_DIAGNOSTICS=false). Set to 'true' to enable.")
    ...
    if enable_diagnostics:
        ...
        try:
            ...
            if diag_results:
                ...
                total_in_table = diag_row.get('TOTAL_IN_TABLE') or diag_row.get('total_in_table', 0)
                non_null = diag_row.get('NON_NULL_COUNT') or diag_row.get('non_null_count', 0)
                ...
                not_fraud = diag_row.get('NOT_FRAUD_COUNT') or diag_row.get('not_fraud_count', 0)
        ...
        try:
            if is_async_diag:
                diag_results2 = await db_provider.execute_query_async(diagnostic_query_with_date)
            else:
                diag_results2 = db_provider.execute_query(diagnostic_query_with_date)
            
            if diag_results2:
                diag_row2 = diag_results2[0]
                total_with_filter = diag_row2.get('TOTAL_WITH_DATE_FILTER') or diag_row2.get('total_with_date_filter', 0)
                non_null2 = diag_row2.get('NON_NULL_COUNT') or diag_row2.get('non_null_count', 0)
                null_count2 = diag_row2.get('NULL_COUNT') or diag_row2.get('null_count', 0)
                fraud2 = diag_row2.get('FRAUD_COUNT') or diag_row2.get('fraud_count', 0)
                not_fraud2 = diag_row2.get('NOT_FRAUD_COUNT') or diag_row2.get('not_fraud_count', 0)
        ...
        try:
            if is_async_diag:
                diag_results3 = await db_provider.execute_query_async(diagnostic_query_timing)
            else:
                diag_results3 = db_provider.execute_query(diagnostic_query_timing)
            
            if diag_results3:
                logger.info(f"[QUERY_ISFRAUD_TX] DIAGNOSTIC 3 - IS_FRAUD_TX population by transaction age:")
                for row in diag_results3:
                    age_group = row.get('AGE_GROUP') or row.get('age_group')
                    total_age = row.get('TOTAL') or row.get('total', 0)
                    non_null_age = row.get('NON_NULL_COUNT') or row.get('non_null_count', 0)
    ...
    try:
        ...
        logger.info(f"[QUERY_ISFRAUD_TX] Query returned {len(results) if results else 0} rows")
        if results:
            logger.info(f"[QUERY_ISFRAUD_TX] Sample result row keys: {list(results[0].keys())}")
            logger.info(f"[QUERY_ISFRAUD_TX] Sample result row: transaction_id={results[0].get('transaction_id') or results[0].get('TRANSACTION_ID')}, is_fraud_tx={results[0].get('is_fraud_tx') or results[0].get('IS_FRAUD_TX')}")
            
            # Log first 5 rows for detailed inspection
            logger.info(f"[QUERY_ISFRAUD_TX] First 5 result rows:")
            for i, row in enumerate(results[:5]):
                tx_id = row.get('transaction_id') if row.get('transaction_id') is not None else row.get('TRANSACTION_ID')
                is_fraud = row.get('is_fraud_tx') if row.get('is_fraud_tx') is not None else row.get('IS_FRAUD_TX')
                logger.info(f"  Row {i+1}: transaction_id={tx_id}, is_fraud_tx={is_fraud} (type: {type(is_fraud)})")
        ...
        for row in results:
            ...
            tx_id = row.get('transaction_id') if row.get('transaction_id') is not None else row.get('TRANSACTION_ID')
            is_fraud_value = row.get('is_fraud_tx') if row.get('is_fraud_tx') is not None else row.get('IS_FRAUD_TX')
            
            # Skip if transaction_id is missing
            if not tx_id:
                logger.warning(f"[QUERY_ISFRAUD_TX] Row missing transaction_id, keys: {list(row.keys())}")
                continue
            ...
            if is_fraud_value is None:
                ...
                try:
                    ...
                    logger.warning(f"[QUERY_ISFRAUD_TX] Unexpected IS_FRAUD_TX value for {tx_id}: {is_fraud_value} (type: {type(is_fraud_value)})")
                    is_fraud_map[tx_id] = None
                    null_count += 1
        ...
        logger.debug(f"[QUERY_ISFRAUD_TX] Mapped transaction IDs: {list(is_fraud_map.keys())[:5]}...")
        logger.debug(f"[QUERY_ISFRAUD_TX] Requested transaction IDs (sample): {transaction_ids[:5]}")
        logger.debug(f"[QUERY_ISFRAUD_TX] Transaction ID types - requested: {type(transaction_ids[0]) if transaction_ids else 'N/A'}, mapped: {type(list(is_fraud_map.keys())[0]) if is_fraud_map else 'N/A'}")
        
        # Log missing transactions with detailed diagnostics
        missing_tx_ids = set(str(tx_id) for tx_id in transaction_ids) - set(str(tx_id) for tx_id in is_fraud_map.keys())
        if missing_tx_ids:
            logger.warning(f"⚠️ IS_FRAUD_TX query did not return values for {len(missing_tx_ids)} transactions: {list(missing_tx_ids)[:5]}...")
            ...
            try:
                if is_async:
                    diag_results_no_date = await db_provider.execute_query_async(diagnostic_no_date)
                else:
                    diag_results_no_date = db_provider.execute_query(diagnostic_no_date)
                
                if diag_results_no_date:
                    logger.info(f"[QUERY_ISFRAUD_TX] DIAGNOSTIC: Found {len(diag_results_no_date)} transactions WITHOUT date filter")
                    for diag_row in diag_results_no_date[:5]:
                        diag_tx_id = str(diag_row.get('transaction_id') if diag_row.get('transaction_id') is not None else diag_row.get('TRANSACTION_ID', ''))
                        diag_datetime = diag_row.get('tx_datetime') if diag_row.get('tx_datetime') is not None else diag_row.get('TX_DATETIME')
                        logger.info(f"  - Transaction {diag_tx_id}: datetime={diag_datetime}, window_end={window_end_str}")
                        if diag_datetime:
                            try:
                                from datetime import datetime
                                if isinstance(diag_datetime, str):
                                    diag_dt = datetime.fromisoformat(diag_datetime.replace('Z', '+00:00'))
                                else:
                                    diag_dt = diag_datetime
                                if diag_dt > window_end_utc:
                                    logger.warning(f"    ⚠️ Transaction datetime {diag_dt} is AFTER window_end {window_end_utc} - date filter excluded it")
                            except Exception as dt_e:
                                logger.debug(f"    Could not parse datetime: {dt_e}")
                else:
                    logger.warning(f"[QUERY_ISFRAUD_TX] DIAGNOSTIC: No transactions found even WITHOUT date filter - transaction IDs may not exist in database")
            except Exception as diag_e:
            ...
            if is_fraud_map:
                ...
                logger.debug(f"[QUERY_ISFRAUD_TX] Type comparison - requested: {type(transaction_ids[0])}={transaction_ids[0]}, mapped: {type(sample_mapped_id)}={sample_mapped_id}")
                # Try converting to string for comparison
                if sample_requested_id == str(sample_mapped_id):
                    logger.warning(f"[QUERY_ISFRAUD_TX] ⚠️ Type mismatch detected - IDs match as strings but not as original types")
...
def get_investigation_by_id(investigation_id: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        with get_db_session() as session:
            ...
            if db_inv.settings_json:
                try:
                    ...
                    if entities and len(entities) > 0:
                        entity = entities[0] if isinstance(entities[0], dict) else entities[0].__dict__ if hasattr(entities[0], '__dict__') else {}
                        entity_id = entity.get('entity_value') or entity.get('entity_id')
            ...
            if db_inv.progress_json:
                try:
                    ...
                    overall_risk_score = progress_data.get('overall_risk_score') or progress_data.get('risk_score')
                    # Try to get window dates from progress_json
                    from_date = progress_data.get('from_date') or progress_data.get('window_start')
                    to_date = progress_data.get('to_date') or progress_data.get('window_end')
            ...
            if (not from_date or not to_date) and db_inv.settings_json:
                try:
                    ...
                    if isinstance(time_range, dict):
                        if not from_date:
                            from_date = time_range.get('start_time') or time_range.get('start')
                        if not to_date:
                            to_date = time_range.get('end_time') or time_range.get('end')
                    
                    # Fallback to top-level settings if time_range didn't have dates
                    if not from_date:
                        from_date = settings_data.get('from_date') or settings_data.get('window_start')
                    if not to_date:
                        to_date = settings_data.get('to_date') or settings_data.get('window_end')
                except json.JSONDecodeError:
                    pass
        ...
        logger.error(f"Failed to get investigation {investigation_id}: {e}", exc_info=True)
...
async def map_investigation_to_transactions(
    ...
    if investigation:
        ...
        source = 'fallback'
        logger.warning(f"[MAP_INVESTIGATION_TO_TRANSACTIONS] No investigation provided, using fallback source")
    ...
    decision_filter = _build_approved_filter(decision_col, 'snowflake' if is_snowflake else 'postgresql')
    filter_mode = os.getenv("TRANSACTION_DECISION_FILTER", "FINALIZED").upper()
    ...
    logger.info(f"🔍 Filtering transactions: mode={filter_mode}, filter={decision_filter}")
    logger.debug(f"[MAP_INVESTIGATION_TO_TRANSACTIONS] Entity clause: {entity_clause}")
    logger.debug(f"[MAP_INVESTIGATION_TO_TRANSACTIONS] Window: {window_start} to {window_end}")
    ...
    if (not transactions or len(transactions) == 0) and filter_mode == "APPROVED_ONLY":
        logger.warning("⚠️ No APPROVED transactions found - trying fallback decision values...")
        
        # Try FINALIZED filter (APPROVED, AUTHORIZED, SETTLED)
        fallback_filter = _build_approved_filter(decision_col, 'snowflake' if is_snowflake else 'postgresql', use_fallback=True)
        ...
        if 'WHERE' in fallback_query.upper():
            fallback_query = fallback_query.rstrip(';').rstrip() + f" AND {fallback_filter}"
        else:
            fallback_query = fallback_query.rstrip(';').rstrip() + f" WHERE {fallback_filter}"
        ...
        if transactions and len(transactions) > 0:
            ...
            logger.warning("⚠️ Fallback query also returned 0 transactions - may need to use ALL filter")
    ...
    if transactions and investigation_risk_score is not None:
        try:
            ...
            logger.info(f"💾 Stored {stored_count} predictions to Postgres PREDICTIONS table")
        except Exception as e:
            logger.warning(f"⚠️ Failed to store predictions to Postgres: {e}")
    
    logger.info(f"[MAP_INVESTIGATION_TO_TRANSACTIONS] Query returned {len(transactions) if transactions else 0} transactions")
    ...
    use_enhanced_scoring = os.getenv('USE_ENHANCED_RISK_SCORING', 'true').lower() == 'true'
    if use_enhanced_scoring and transactions:
        logger.info(f"🎯 Using Enhanced Risk Scoring (without MODEL_SCORE) for {len(transactions)} transactions")
        ...
        try:
            ...
            if investigation_risk_score is None or enhanced_score > investigation_risk_score:
                logger.info(f"🔄 Risk score updated: {investigation_risk_score} → {enhanced_score:.3f}")
    ...
    if transactions:
        ...
        logger.info(f"[MAP_INVESTIGATION_TO_TRANSACTIONS] Sample transaction keys: {all_keys}")
        logger.info(f"[MAP_INVESTIGATION_TO_TRANSACTIONS] Sample transaction 'transaction_id' value: {sample_tx.get('transaction_id', 'KEY_NOT_FOUND')}")
        ...
        logger.info(f"[MAP_INVESTIGATION_TO_TRANSACTIONS] Transaction ID variations: {tx_id_variations}")
        logger.info(f"[MAP_INVESTIGATION_TO_TRANSACTIONS] Sample transaction full structure (first 3): {[list(tx.keys()) for tx in transactions[:3]]}")
        ...
        for key in all_keys:
            if 'transaction' in key.lower() or 'tx_id' in key.lower():
                ...
                logger.info(f"[MAP_INVESTIGATION_TO_TRANSACTIONS] Found potential transaction ID key: '{actual_tx_id_key}' with value: {sample_tx.get(actual_tx_id_key)}")
                break
        
        if not actual_tx_id_key:
            logger.error(f"[MAP_INVESTIGATION_TO_TRANSACTIONS] ⚠️ No transaction ID key found in transaction keys: {all_keys}")
    else:
        logger.warning(f"[MAP_INVESTIGATION_TO_TRANSACTIONS] No transactions returned from query - this may explain empty transaction_ids")
    ...
    if entity_type and entity_id:
        ...
        if entity_label:
            logger.info(f"Found entity label for {entity_type}:{entity_id}: {entity_label}")
        else:
            logger.debug(f"No entity label found for {entity_type}:{entity_id} (will use transaction-level labels if available)")
    ...
    for tx in transactions:
        ...
        if tx_id:
            ...
            logger.warning(f"[MAP_INVESTIGATION_TO_TRANSACTIONS] Transaction missing transaction_id, keys: {list(tx.keys())}")
    ...
    transaction_ids = [tx.get('transaction_id') for tx in mapped_transactions if tx.get('transaction_id')]
    logger.info(f"[MAP_INVESTIGATION_TO_TRANSACTIONS] Preparing to query IS_FRAUD_TX for {len(transaction_ids)} transactions")
    
    # DEBUG: Log details about transaction_ids extraction
    if mapped_transactions:
        logger.info(f"[MAP_INVESTIGATION_TO_TRANSACTIONS] Total mapped_transactions: {len(mapped_transactions)}")
        logger.info(f"[MAP_INVESTIGATION_TO_TRANSACTIONS] Sample mapped_tx keys: {list(mapped_transactions[0].keys())}")
        logger.info(f"[MAP_INVESTIGATION_TO_TRANSACTIONS] Sample mapped_tx 'transaction_id' value: {mapped_transactions[0].get('transaction_id', 'KEY_NOT_FOUND')}")
        # Check how many transactions have transaction_id key
        tx_with_id = [tx for tx in mapped_transactions if tx.get('transaction_id')]
        tx_without_id = [tx for tx in mapped_transactions if not tx.get('transaction_id')]
        logger.info(f"[MAP_INVESTIGATION_TO_TRANSACTIONS] Transactions with 'transaction_id' key: {len(tx_with_id)}, without: {len(tx_without_id)}")
        if tx_without_id:
            logger.warning(f"[MAP_INVESTIGATION_TO_TRANSACTIONS] Sample transaction without 'transaction_id': {list(tx_without_id[0].keys())}")
    else:
        logger.warning(f"[MAP_INVESTIGATION_TO_TRANSACTIONS] mapped_transactions is empty - this explains why transaction_ids is empty")
    
    if transaction_ids:
        try:
            logger.info(f"[MAP_INVESTIGATION_TO_TRANSACTIONS] Calling query_isfraud_tx_for_transactions with {len(transaction_ids)} transaction IDs, window_end={window_end}, is_snowflake={is_snowflake}")
            ...
            logger.info(f"[MAP_INVESTIGATION_TO_TRANSACTIONS] Received IS_FRAUD_TX map with {len(is_fraud_map)} entries")
            ...
            for mapped_tx in mapped_transactions:
                ...
                if tx_id and mapped_tx.get('actual_outcome') is None:
                    ...
                    if tx_id_str in is_fraud_map:
                        ...
                        updated_count += 1
                        logger.debug(f"Applied IS_FRAUD_TX={is_fraud_map[tx_id_str]} to transaction {tx_id}")
                    else:
                        logger.debug(f"[MAP_INVESTIGATION_TO_TRANSACTIONS] Transaction {tx_id} not found in is_fraud_map (keys: {list(is_fraud_map.keys())[:3]}...)")
            
            logger.info(f"[MAP_INVESTIGATION_TO_TRANSACTIONS] Updated {updated_count}/{len(mapped_transactions)} transactions with IS_FRAUD_TX values")
        except Exception as e:
            logger.error(f"❌ Failed to query IS_FRAUD_TX values: {e}", exc_info=True)
            # Continue without IS_FRAUD_TX values - transactions will have actual_outcome=None
    else:
        logger.warning(f"[MAP_INVESTIGATION_TO_TRANSACTIONS] No transaction IDs found - skipping IS_FRAUD_TX query")
    
    # Log summary with proper formatting
    logger.info(f"[MAP_INVESTIGATION_TO_TRANSACTIONS] ✅ Mapping complete:")
    logger.info(f"  - Query returned: {len(transactions)} transactions (filtered by date window and decision status)")
    if transaction_scores and isinstance(transaction_scores, dict):
        ...
        if excluded_by_filter > 0:
            ...
            logger.info(f"  - Transactions with scores excluded by query filters: {excluded_by_filter}")
            logger.info(f"    Reason: These transactions don't match confusion matrix criteria:")
            logger.info(f"    - Must be within date window: {window_start} to {window_end}")
            logger.info(f"    - Must have decision status: APPROVED/AUTHORIZED/SETTLED (REJECTED transactions excluded)")
            logger.info(f"    - Must match entity filter")
            logger.info(f"    Note: transaction_scores includes ALL transactions from investigation (including REJECTED),")
            logger.info(f"    but confusion matrix only evaluates APPROVED transactions for prediction quality.")
    logger.info(f"  - Mapped with per-transaction scores: {len(mapped_transactions)} transactions")
    ...
    if excluded_no_score > 0:
        logger.warning(f"  - Excluded (no per-transaction score): {excluded_no_score} transactions")
        logger.warning(f"    ⚠️ These transactions match query filters but don't have scores in transaction_scores dict")
    ...
    return mapped_transactions, source, None if transaction_scores else investigation_risk_score
...
async def get_transactions_for_investigation_window(
    ...
    return await map_investigation_to_transactions(investigation, window_start, window_end)
...
def select_best_investigation(
    ...
    fully_covering = [
        p for p in parsed_investigations
    ...
    if fully_covering:
        ...
        if selected_inv and 'overall_risk_score' in selected_inv:
            logger.debug(f"[SELECT_BEST_INVESTIGATION] Selected investigation {selected_inv.get('id', 'unknown')} with overall_risk_score={selected_inv.get('overall_risk_score')}")
        return selected_inv
