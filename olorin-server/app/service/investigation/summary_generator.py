...
def generate_investigation_summary(
    ...
    if window_b_metrics.total_transactions > window_a_metrics.total_transactions:
        change_pct = (
            (
                (
                    window_b_metrics.total_transactions
                    - window_a_metrics.total_transactions
                )
                / window_a_metrics.total_transactions
                * 100
            )
            if window_a_metrics.total_transactions > 0
            else 0
        )
        sentences.append(
        ...
        change_pct = (
            ...
            if window_b_metrics.total_transactions > 0
            else 0
        )
        sentences.append(
    ...
    if (
        window_b_metrics.pending_label_count
        and window_b_metrics.pending_label_count > 0
    ):
        sentences.append(
