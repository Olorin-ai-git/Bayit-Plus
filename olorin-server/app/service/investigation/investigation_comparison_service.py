...
async def compare_investigations(
    ...
    """
    logger.info(f"Validating prediction quality: {investigation_id_a} vs {investigation_id_b}")
    ...
    for inv in all_investigations:
        ...
        if inv_id == investigation_id_a:
            inv_a = inv if isinstance(inv, dict) else inv.__dict__ if hasattr(inv, "__dict__") else inv
        if inv_id == investigation_id_b:
            inv_b = inv if isinstance(inv, dict) else inv.__dict__ if hasattr(inv, "__dict__") else inv
    ...
    inv_a_dict = inv_a if isinstance(inv_a, dict) else inv_a.__dict__ if hasattr(inv_a, "__dict__") else {}
    inv_b_dict = inv_b if isinstance(inv_b, dict) else inv_b.__dict__ if hasattr(inv_b, "__dict__") else {}
    ...
    if not from_date_a or not from_date_b:
        raise ValueError("Both investigations must have time windows for prediction quality validation")
    ...
    older_inv_id = investigation_id_a if from_date_a < from_date_b else investigation_id_b
    ...
    newer_inv_id = investigation_id_b if from_date_a < from_date_b else investigation_id_a
...
async def _validate_prediction_quality(
    ...
    if len(transactions) == 0:
        logger.warning(f"No transactions found in validation window for prediction quality validation")
        return {
            "validation_period": {
    ...
    return {
        ...
        "validation_quality": "good" if total_labeled >= 10 else "limited" if total_labeled > 0 else "insufficient"
    }
...
def _generate_prediction_quality_summary(
    ...
    summary_parts = [
        f"Prediction Quality Validation for {entity_type}:{entity_id}."
    ]
    ...
    if prediction_quality:
        if prediction_quality.get("error"):
            summary_parts.append(f"Validation error: {prediction_quality.get('error')}.")
        else:
            ...
            if actual_fraud_rate is not None:
                ...
                if prediction_accurate is True:
                    ...
                    direction = "overestimated" if prediction_quality.get("predicted_high_risk") and not prediction_quality.get("actual_high_fraud") else "underestimated"
                    summary_parts.append(
                ...
                validation_quality = prediction_quality.get("validation_quality", "unknown")
        ...
        summary_parts.append("Could not validate prediction quality - insufficient data.")
