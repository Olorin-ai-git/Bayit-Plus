...
class SnowflakeReadConfig:
    ...
    def __init__(self):
        ...
        self.read_warehouse = os.getenv('SNOWFLAKE_READ_WAREHOUSE', 'manual_review_agent_wh')
...
async def verify_snowflake_read_path() -> Dict[str, Any]:
    ...
    logger.info(f"📅 Testing window: {window_start.date()} to {window_end.date()} ({window_duration_days} days)")
    ...
    try:
        ...
        if not result or len(result) == 0:
            raise RuntimeError(f"Failed to access table {table_name} - query returned no results")
        ...
        if window_result and len(window_result) > 0:
            approved_count = window_result[0].get('row_count', window_result[0].get('ROW_COUNT', 0))
        ...
        if approved_count == 0:
            ...
            if fallback_result and len(fallback_result) > 0:
                ...
                authorized_count = row.get('authorized_count', row.get('AUTHORIZED_COUNT', 0))
                settled_count = row.get('settled_count', row.get('SETTLED_COUNT', 0))
                null_count = row.get('null_count', row.get('NULL_COUNT', 0))
                distinct_decisions = row.get('distinct_decisions', row.get('DISTINCT_DECISIONS', 0))
                ...
                if authorized_count > 0:
                    ...
                    logger.warning("⚠️ Using ALL transactions (no APPROVED/AUTHORIZED/SETTLED found)")
                else:
        ...
        logger.info(f"✅ Snowflake read path verified: {approved_count:,} transactions in test window")
        if fallback_used:
