...
from app.router.models.investigation_comparison_models import (
    ComparisonRequest, ComparisonResponse, WindowMetrics, DeltaMetrics,
    WindowInfo, ConfusionMatrix, AggregatedConfusionMatrix
)
...
from .metrics_calculation import (
    compute_confusion_matrix, compute_derived_metrics,
    compute_histogram
)
from .timeseries_calculation import compute_timeseries_daily
from .distribution_statistics import compute_psi, compute_ks_statistic
...
from .workload_metrics import calculate_workload_metrics, calculate_precision_at_k, calculate_recall_at_budget
...
async def compare_windows(request: ComparisonRequest) -> ComparisonResponse:
    ...
    risk_threshold = float(os.getenv("RISK_THRESHOLD_DEFAULT", "0.3"))  # Default changed from 0.7 to 0.3
    ...
    if request.entity:
        ...
        investigation_a = select_best_investigation(investigations_a, window_a_start, window_a_end)
        investigation_b = select_best_investigation(investigations_b, window_b_start, window_b_end)
        
        if investigation_a:
            source_a = investigation_a.get('id', 'unknown')
            predicted_risk_a = investigation_a.get('overall_risk_score')
            logger.info(f"Window A: Using investigation {source_a} with risk_score={predicted_risk_a}")
        else:
        ...
        if investigation_b:
            ...
            logger.info(f"Window B: Using investigation {source_b} with risk_score={predicted_risk_b}")
        else:
    ...
    if auto_expand_enabled and request.entity:
        ...
        async def count_only_a(start: datetime, end: datetime) -> Dict[str, int]:
            ...
            test_tx, _, _ = await map_investigation_to_transactions(
                investigation_a, start, end,
                entity_type=request.entity.get("type"),
                entity_id=request.entity.get("value")
            )
            known = [tx for tx in test_tx if tx.get("actual_outcome") in (0, 1, True, False, "FRAUD", "NOT_FRAUD")]
            actual_frauds = sum(1 for tx in known if tx.get("actual_outcome") in (1, True, "FRAUD"))
            predicted_positives = len(test_tx) if predicted_risk_a and predicted_risk_a >= risk_threshold else 0
            return {
        ...
        async def count_only_b(start: datetime, end: datetime) -> Dict[str, int]:
            ...
            test_tx, _, _ = await map_investigation_to_transactions(
                investigation_b, start, end,
                entity_type=request.entity.get("type"),
                entity_id=request.entity.get("value")
            )
            known = [tx for tx in test_tx if tx.get("actual_outcome") in (0, 1, True, False, "FRAUD", "NOT_FRAUD")]
            actual_frauds = sum(1 for tx in known if tx.get("actual_outcome") in (1, True, "FRAUD"))
            predicted_positives = len(test_tx) if predicted_risk_b and predicted_risk_b >= risk_threshold else 0
            return {
                "known_transactions": len(known),
                "actual_frauds": actual_frauds,
                "predicted_positives": predicted_positives
        ...
        # Expand windows
        window_a_effective_start, window_a_effective_end, expand_meta_a = await expand_window_until_support(
            count_only_a, 
            window_a_start, window_a_end, is_retro_a, today
        )
        window_b_effective_start, window_b_effective_end, expand_meta_b = await expand_window_until_support(
            count_only_b,
            window_b_start, window_b_end, is_retro_b, today
        )
    ...
    transactions_a, source_a, predicted_risk_a = await map_investigation_to_transactions(
        investigation_a, window_a_effective_start, window_a_effective_end,
    ...
    transactions_b, source_b, predicted_risk_b = await map_investigation_to_transactions(
        investigation_b, window_b_effective_start, window_b_effective_end,
        entity_type=request.entity.get("type") if request.entity else None,
        entity_id=request.entity.get("value") if request.entity else None
    )
    
    logger.info(f"Fetched {len(transactions_a)} transactions for Window A (source={source_a}), {len(transactions_b)} for Window B (source={source_b})")
    ...
    if len(transactions_a) > 0:
        try:
            ...
            posthoc_results_a = run_all_posthoc_detectors(transactions_a, merchant_id=merchant_id)
            
            if posthoc_results_a.get("high_precision"):
                logger.info(f"✅ Post-hoc detectors flagged high-precision patterns in Window A: {posthoc_results_a.get('risk_factors', [])}")
    ...
    if len(transactions_b) > 0:
        try:
            ...
            merchant_id = list(merchant_ids)[0] if merchant_ids else None
            posthoc_results_b = run_all_posthoc_detectors(transactions_b, merchant_id=merchant_id)
            
            if posthoc_results_b.get("high_precision"):
                logger.info(f"✅ Post-hoc detectors flagged high-precision patterns in Window B: {posthoc_results_b.get('risk_factors', [])}")
        except Exception as e:
    ...
    tp_a, fp_a, tn_a, fn_a, excluded_a = compute_confusion_matrix(transactions_a, risk_threshold)
    precision_a, recall_a, f1_a, accuracy_a, fraud_rate_a, pending_a, ci_a, support_a, power_a = compute_derived_metrics(
        tp_a, fp_a, tn_a, fn_a, transactions_a, ci_confidence=EVAL_DEFAULTS["ci_confidence"]
    )
    over_threshold_a = sum(1 for tx in transactions_a if tx.get("predicted_risk") and tx.get("predicted_risk") >= risk_threshold)
    ...
    from app.router.models.investigation_comparison_models import SupportMetrics, PowerAssessment
    ...
    if window_b_empty:
        ...
        precision_b, recall_b, f1_b, accuracy_b, fraud_rate_b, pending_b, ci_b, support_b, power_b = (
        ...
        tp_b, fp_b, tn_b, fn_b, excluded_b = compute_confusion_matrix(transactions_b, risk_threshold)
        precision_b, recall_b, f1_b, accuracy_b, fraud_rate_b, pending_b, ci_b, support_b, power_b = compute_derived_metrics(
            tp_b, fp_b, tn_b, fn_b, transactions_b, ci_confidence=EVAL_DEFAULTS["ci_confidence"]
        )
        over_threshold_b = sum(1 for tx in transactions_b if tx.get("predicted_risk") and tx.get("predicted_risk") >= risk_threshold)
    ...
    workload_a = calculate_workload_metrics(transactions_a, window_a_start, window_a_end, risk_threshold)
    if window_b_empty:
        ...
        workload_b = calculate_workload_metrics(transactions_b, window_b_start, window_b_end, risk_threshold)
    
    # Calculate precision@k (top 100, 500, 1000 anomalies)
    precision_at_k_a = {}
    precision_at_k_b = {}
    for k in [100, 500, 1000]:
        precision_at_k_a[k] = calculate_precision_at_k(transactions_a, k, risk_threshold)
        if window_b_empty:
            precision_at_k_b[k] = None  # No data for Window B
        else:
            precision_at_k_b[k] = calculate_precision_at_k(transactions_b, k, risk_threshold)
    
    # Calculate recall at fixed alert budgets (50, 100, 150 alerts/day)
    recall_at_budget_a = {}
    recall_at_budget_b = {}
    for daily_budget in [50, 100, 150]:
        recall_at_budget_a[daily_budget] = calculate_recall_at_budget(transactions_a, daily_budget, window_a_start, window_a_end)
        if window_b_empty:
            recall_at_budget_b[daily_budget] = None  # No data for Window B
        else:
            recall_at_budget_b[daily_budget] = calculate_recall_at_budget(transactions_b, daily_budget, window_b_start, window_b_end)
    ...
    if options and options.include_timeseries:
        metrics_a.timeseries_daily = compute_timeseries_daily(transactions_a, window_a_start, window_a_end, risk_threshold)
        if not window_b_empty:
            metrics_b.timeseries_daily = compute_timeseries_daily(transactions_b, window_b_start, window_b_end, risk_threshold)
        else:
            metrics_b.timeseries_daily = None  # No data for Window B

    # Compute deltas
    # Extract predicted_risk values for PSI/KS calculation
    risks_a = [tx.get("predicted_risk") for tx in transactions_a if tx.get("predicted_risk") is not None]
    risks_b = [tx.get("predicted_risk") for tx in transactions_b if tx.get("predicted_risk") is not None]

    psi_value = compute_psi(risks_a, risks_b) if len(risks_a) >= 10 and len(risks_b) >= 10 else None
    ks_value = compute_ks_statistic(risks_a, risks_b) if len(risks_a) >= 10 and len(risks_b) >= 10 else None
    ...
    if options and options.include_per_merchant:
        per_merchant = compute_per_merchant_metrics(
            transactions_a, transactions_b, risk_threshold,
            options.max_merchants
        )

    # Generate summary
    summary = generate_investigation_summary(
        request.entity, metrics_a, metrics_b, delta,
        window_a_label, window_b_label
    )
    ...
    response = ComparisonResponse(
        ...
        windowA=WindowInfo(
            ...
            auto_expand_meta=AutoExpandMetadata(**expand_meta_a) if expand_meta_a else None
        ),
        windowB=WindowInfo(
            label=window_b_label,
            start=window_b_effective_start.isoformat(),
            end=window_b_effective_end.isoformat(),
            auto_expand_meta=AutoExpandMetadata(**expand_meta_b) if expand_meta_b else None
        ),
        A=metrics_a,
        B=metrics_b,
        delta=delta,
        per_merchant=per_merchant,
        excluded_missing_predicted_risk=excluded_a + excluded_b if (excluded_a + excluded_b) > 0 else None,
    ...
    logger.info(f"JSON artifact saved: {artifact_path} (investigation_id={artifact_investigation_id or 'None'})")
    ...
    try:
        html_content = generate_html_report(response, html_output_path, additional_metrics)
...
def calculate_confusion_matrix(
    ...
    excluded_missing_actual_outcome = sum(
        ...
        if tx.get('actual_outcome') is None
    )
    ...
    precision, recall, f1, accuracy, fraud_rate, pending_count, ci_dict, support_dict, power_dict = compute_derived_metrics(
        tp, fp, tn, fn, transactions
    )
...
def aggregate_confusion_matrices(
    ...
    aggregated_precision = total_tp / (total_tp + total_fp) if (total_tp + total_fp) > 0 else 0.0
    if (total_tp + total_fp) == 0:
        logger.debug("Divide-by-zero in aggregated precision calculation: TP+FP=0 (no positive predictions)")
    
    # Recall: total_TP / (total_TP + total_FN)
    aggregated_recall = total_tp / (total_tp + total_fn) if (total_tp + total_fn) > 0 else 0.0
    if (total_tp + total_fn) == 0:
        logger.debug("Divide-by-zero in aggregated recall calculation: TP+FN=0 (no actual fraud cases)")
    ...
    aggregated_accuracy = (total_tp + total_tn) / total_known if total_known > 0 else 0.0
    if total_known == 0:
        logger.debug("Divide-by-zero in aggregated accuracy calculation: no known labels")
