    ...
    try:
        ...
        with get_db_session() as db:
            ...
            for tx in transactions:
                ...
                transaction_id = tx.get("TX_ID_KEY") or tx.get("tx_id_key") or tx.get("transaction_id")
                ...
                if tx_predicted_risk is None:
                    logger.debug(f"Skipping transaction {transaction_id} without predicted_risk")
                    continue
                ...
                # Extract merchant_id
                merchant_id = (
                    tx.get("STORE_ID") or tx.get("store_id") or 
                    tx.get("MERCHANT_ID") or tx.get("merchant_id") or
                    None
                )
...
def compute_confusion_matrix_with_join(
    ...
    if not predictions:
        ...
        return {
            "overall": {"TP": 0, "FP": 0, "TN": 0, "FN": 0, "precision": 0.0, "recall": 0.0, "f1": 0.0, "accuracy": 0.0},
            "by_merchant": {}
        }
    
    # Step 2: Join to Snowflake IS_FRAUD_TX using resilient label joiner
    from app.service.investigation.resilient_label_joiner import join_actual_labels_resilient_async
    ...
    try:
        ...
        if loop.is_running():
            ...
            labels_map = asyncio.run(join_actual_labels_resilient_async(
                transaction_ids=transaction_ids,
                use_secondary_sources=True
            ))
        else:
            labels_map = loop.run_until_complete(join_actual_labels_resilient_async(
                transaction_ids=transaction_ids,
                use_secondary_sources=True
            ))
    except RuntimeError:
    ...
    logger.info(f"📊 Retrieved {len([v for v in labels_map.values() if v is not None])} ground truth labels from Snowflake (with fallbacks)")
    ...
    def calculate_metrics(tp_val, fp_val, tn_val, fn_val):
        ...
        accuracy = (tp_val + tn_val) / (tp_val + fp_val + tn_val + fn_val) if (tp_val + fp_val + tn_val + fn_val) > 0 else 0.0
        return {
