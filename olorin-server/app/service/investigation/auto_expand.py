...
async def expand_window_until_support(
    ...
    label_gap = timedelta(days=auto["label_maturity_days"]) if is_retro else timedelta(0)
    ...
    def meets_support(snapshot: Dict[str, int]) -> bool:
        ...
        return (
            snapshot.get("known_transactions", 0) >= ms["min_transactions"] and
            snapshot.get("actual_frauds", 0) >= ms["min_actual_frauds"] and
            snapshot.get("predicted_positives", 0) >= ms["min_predicted_positives"]
        )
    
    # Check initial window
    snap = await fetch_count_fn(cur_start, cur_end) if inspect.iscoroutinefunction(fetch_count_fn) else fetch_count_fn(cur_start, cur_end)
        ...
        if is_retro:
            ...
            if new_end > maturity_boundary:
                ...
                if new_end <= cur_end:
                    ...
                    logger.info(f"Window expansion stopped: hit label maturity boundary")
                    break
