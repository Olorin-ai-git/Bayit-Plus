...
def normalize_transaction_keys(transactions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    ...
    for tx in transactions:
        ...
        if normalized_id:
            ...
            missing_keys += 1
            logger.debug(f"Transaction missing transaction ID: {list(tx.keys())[:5]}...")
    
    if missing_keys > 0:
        logger.warning(f"⚠️ {missing_keys} transactions missing transaction ID (out of {len(transactions)})")
