...
async def join_actual_labels_resilient_async(
    transaction_ids: List[str], use_secondary_sources: bool = True
    ...
    """
    return await _join_actual_labels_resilient_impl(
        transaction_ids, use_secondary_sources, is_async=True
    )
...
def join_actual_labels_resilient(
    ...
    try:
        ...
        return asyncio.run(
            _join_actual_labels_resilient_impl(
                transaction_ids, use_secondary_sources, is_async=True
            )
        )
    except RuntimeError:
...
async def _join_actual_labels_resilient_impl(
    transaction_ids: List[str], use_secondary_sources: bool, is_async: bool
) -> Dict[str, Optional[int]]:
    """
    ...
    logger.info(
        f"📊 Primary label source (IS_FRAUD_TX): {labeled_count}/{len(transaction_ids)} transactions labeled"
    )

    # If primary source is sparse and secondary sources enabled, try fallbacks
    if (
        use_secondary_sources and labeled_count < len(transaction_ids) * 0.5
    ):  # Less than 50% labeled
        logger.info(
            f"⚠️ Primary label source sparse ({labeled_count}/{len(transaction_ids)}) - trying secondary sources..."
        )
        ...
        unlabeled_ids = [
            ...
            if tx_id not in labels_map or labels_map[tx_id] is None
        ]
        ...
        if unlabeled_ids:
            ...
            if is_async:
                secondary_result = await db_provider.execute_query_async(
                    secondary_query
                )
            else:
            ...
            for row in secondary_result:
                ...
                if case_outcome:
                    ...
                    if "FRAUD" in case_upper or case_upper == "FRAUDULENT":
                        ...
                        "NOT_FRAUD" in case_upper
                        or "LEGITIMATE" in case_upper
                        or "APPROVED" in case_upper
                    ):
                        labels_map[tx_id] = 0
                        secondary_labeled += 1
                        continue
                ...
                # Try CHARGEBACK_FLAG
                chargeback = (
                    row.get("chargeback_flag")
                    or row.get("CHARGEBACK_FLAG")
                    or row.get("HAS_CHARGEBACK")
                    or row.get("has_chargeback")
                )
                ...
                manual_decision = row.get("MANUAL_REVIEW_DECISION") or row.get(
                    "manual_review_decision"
                )
                if manual_decision:
            ...
            logger.info(
                f"📊 Secondary label sources: {secondary_labeled} additional transactions labeled"
            )
            logger.info(
                f"📊 Total labeled: {labeled_count + secondary_labeled}/{len(transaction_ids)} transactions"
            )
