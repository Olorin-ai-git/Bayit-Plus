...
def detect_ip_device_velocity_reuse(
    ...
    for email, email_txs in email_groups.items():
        ...
        for i in range(len(email_txs)):
            ...
            window_count = sum(
                1 for tx in email_txs[i:] if tx["datetime"] <= window_end
            )
        ...
        for hour_start, merchants in merchants_per_hour.items():
            ...
            max_distinct_merchants_60min = max(
                max_distinct_merchants_60min, len(merchants_in_window)
            )
        ...
        if max_distinct_merchants_60min >= min_merchants or max_tx_per_5min >= 3:
            return {
                ...
                "unique_merchants": len(
                    set(tx["merchant_id"] for tx in email_txs if tx["merchant_id"])
                ),
                "unique_ips": len(set(tx["ip"] for tx in email_txs if tx["ip"])),
                "unique_devices": len(
                    set(tx["device_id"] for tx in email_txs if tx["device_id"])
                ),
                "severity": (
                    "high"
                    if max_distinct_merchants_60min >= 5 or max_tx_per_5min >= 5
                    else "medium"
                ),
    ...
    for i in range(len(parsed_txs) - 1):
        ...
        if (
            ...
            distance_km = _haversine_distance(
                tx1["latitude"], tx1["longitude"], tx2["latitude"], tx2["longitude"]
            )
            ...
            if time_diff_hours > 0:
                speed_mph = (
                    distance_km / 1.60934
                ) / time_diff_hours  # Convert km to miles

                if speed_mph > min_speed_mph:
                    # Check for corroboration signals
                    device_overlap = (
                        tx1["device_id"] and tx1["device_id"] == tx2["device_id"]
                    )
                    payment_overlap = (
                        tx1["card_hash"] and tx1["card_hash"] == tx2["card_hash"]
                    )
...
def detect_merchant_local_anomaly(
    ...
    merchant_txs = [
        ...
        for tx in transactions
        if (
            (tx.get("STORE_ID") or tx.get("store_id") or "") == merchant_id
            or (tx.get("MERCHANT_ID") or tx.get("merchant_id") or "") == merchant_id
        )
    ]
    ...
    now = (
        datetime.now(parsed_txs[-1]["datetime"].tzinfo)
        if parsed_txs[-1]["datetime"].tzinfo
        else datetime.now()
    )
    recent_txs = [
        tx for tx in parsed_txs if (now - tx["datetime"]).total_seconds() <= 86400
    ]
        ...
        if recent_amounts:
            ...
            if all_bins and recent_bins:
                ...
                for bin_val, count in recent_bin_dist.items():
                    ...
                    all_p = (
                        all_bin_dist.get(bin_val, 0) / len(all_bins) if all_bins else 0
                    )
                    if all_p > 0:
                        bin_mix_drift += (
                            recent_p * (recent_p / all_p) if recent_p > 0 else 0
                        )
            ...
            if not should_suppress and (
                abs(aov_z_score) > 2.0 or bin_mix_drift > 0.5 or night_ratio > 0.3
            ):
                return {
                    "detected": True,
                    ...
                    "severity": (
                        "high"
                        if abs(aov_z_score) > 3.0 or bin_mix_drift > 1.0
                        else "medium"
                    ),
                }
...
def detect_fraud_ring_link_analysis(
    ...
    if fraud_rings:
        ...
        return {
            ...
            "severity": (
                "high"
                if max_ring["chargeback_rate"] > 0.5 and max_ring["size"] >= 5
                else "medium"
            ),
...
def run_all_posthoc_detectors(
    transactions: List[Dict[str, Any]], merchant_id: Optional[str] = None
) -> Dict[str, Any]:
    """
    ...
    try:
        results["ip_device_velocity_reuse"] = detect_ip_device_velocity_reuse(
            transactions
        )
    except Exception as e:
    ...
    try:
        results["geo_impossibility_corroborated"] = (
            detect_geo_impossibility_corroborated(transactions)
        )
    except Exception as e:
    ...
    if merchant_id:
        try:
            results["merchant_local_anomaly"] = detect_merchant_local_anomaly(
    ...
    try:
        results["fraud_ring_link_analysis"] = detect_fraud_ring_link_analysis(
            transactions
        )
    except Exception as e:
