...
class FraudDetectionFeatures:
    ...
    def calculate_transaction_features(
        self, transactions: List[Dict[str, Any]], entity_id: str, window_hours: int = 24
    ) -> Dict[str, Any]:
        """
    ...
    def _calculate_composite_risk_score(self, features: Dict[str, float]) -> float:
        ...
        if (
            features.get("merchant_diversity", 1) < 0.3 and tx_count > 3
        ):  # Lowered from 5
    ...
    def calculate_per_transaction_risk(
        self, transaction: Dict[str, Any], historical_transactions: List[Dict[str, Any]]
    ) -> float:
        """
