...
def compute_derived_metrics(
...
) -> Tuple[float, float, float, float, float, int, Dict[str, Any], Dict[str, Any], Dict[str, Any]]:
    """
    ...
    if n_pred_pos == 0:
        logger.debug("Divide-by-zero in precision calculation: TP+FP=0 (no positive predictions)")

    # Recall: TP / (TP + FN)
    n_actual_fraud = tp + fn
    recall = tp / n_actual_fraud if n_actual_fraud > 0 else 0.0
    if n_actual_fraud == 0:
        logger.debug("Divide-by-zero in recall calculation: TP+FN=0 (no actual fraud cases)")

    # F1: 2 * (precision * recall) / (precision + recall)
    f1 = 2 * (precision * recall) / (precision + recall) if (precision + recall) > 0 else 0.0
    if (precision + recall) == 0:
        logger.debug("Divide-by-zero in F1 calculation: precision+recall=0 (no valid predictions)")

    # Accuracy: (TP + TN) / (TP + FP + TN + FN)
    total_known = tp + fp + tn + fn
    accuracy = (tp + tn) / total_known if total_known > 0 else 0.0
    if total_known == 0:
        logger.debug("Divide-by-zero in accuracy calculation: no known labels (all transactions pending)")
    ...
    ci_precision = wilson_interval(tp, n_pred_pos, ci_confidence) if n_pred_pos > 0 else None
    ci_recall = wilson_interval(tp, n_actual_fraud, ci_confidence) if n_actual_fraud > 0 else None
    ci_accuracy = wilson_interval(tp + tn, total_known, ci_confidence) if total_known > 0 else None
    ...
    return precision, recall, f1, accuracy, fraud_rate, pending_label_count, ci_dict, support_dict, power_dict
...
def compute_histogram(
    ...
    risks = [tx.get("predicted_risk") for tx in transactions if tx.get("predicted_risk") is not None]

    if not risks:
        return [HistogramBin(bin=f"{i/num_bins:.1f}-{(i+1)/num_bins:.1f}", n=0) for i in range(num_bins)]

    min_risk = min(risks)
    max_risk = max(risks)
    bin_width = (max_risk - min_risk) / num_bins if max_risk > min_risk else 1.0 / num_bins
