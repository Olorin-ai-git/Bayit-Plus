...
from .models import SmokeTestResult, SmokeTestStatus, SmokeTestSeverity
    ...
    async def run_all_tests(self) -> List[SmokeTestResult]:
        ...
        try:
            ...
            if connectivity_result.status == SmokeTestStatus.FAILED:
                logger.warning(f"{self.service_name} connectivity failed, skipping remaining tests")
                results.append(self._create_skipped_result("Authentication test skipped - connectivity failed", "authentication_test"))
                results.append(self._create_skipped_result("Functionality test skipped - connectivity failed", "functionality_test"))
                return results
            ...
            if auth_result.status == SmokeTestStatus.FAILED:
                logger.warning(f"{self.service_name} authentication failed, skipping functionality test")
                results.append(self._create_skipped_result("Functionality test skipped - authentication failed", "functionality_test"))
                return results
            ...
            logger.info(f"Completed smoke tests for {self.service_name}: {len(results)} tests")
            
        except Exception as e:
            logger.error(f"Unexpected error running smoke tests for {self.service_name}: {e}")
            results.append(self._create_error_result(f"Unexpected error: {str(e)}", "unexpected_error"))
            
        return results
    
    async def _run_test_with_timeout(self, test_coroutine, test_name: str) -> SmokeTestResult:
