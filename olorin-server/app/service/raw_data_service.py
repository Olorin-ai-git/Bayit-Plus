...
class RawDataService:
    ...
    def ensure_investigation_exists(self, investigation_id: str) -> None:
        ...
        if not existing_investigation:
            logger.info(
                f"Creating new investigation {investigation_id} for raw data upload"
            )
    ...
    async def process_raw_data_file(
        self, file_content: bytes, filename: str, investigation_id: str
    ) -> Tuple[bool, dict]:
        """
        ...
        try:
            ...
            logger.info(
                f"Processing CSV file {filename} for investigation {investigation_id}"
            )

            # Process the CSV data
            processing_result = await raw_data_processor.process_csv_data(
                csv_content=file_content, filename=filename
            )
        ...
        if processing_result.get("success", False):
            ...
            for record in processing_result.get("data", []):
                try:
                    ...
                    logger.warning(
                        f"Failed to convert record to RawTransactionData: {e}"
                    )
                    continue
            ...
            return RawDataProcessingResult(
                ...
                data=processed_data[
                    :100
                ],  # Limit response size, return first 100 records
