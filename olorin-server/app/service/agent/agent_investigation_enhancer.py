...
async def investigate_with_mcp_enhancement(
    ...
    try:
        ...
        if domain == "device":
            ...
            findings = await _investigate_user_behavior_with_mcp(mcp_tools, context, findings)
        elif domain == "location":
            findings = await _investigate_location_with_mcp(mcp_tools, context, findings)
        elif domain == "logs":
            findings = await _investigate_logs_with_mcp(mcp_tools, context, findings)
        else:
            findings = await _generic_mcp_investigation(mcp_tools, context, findings, domain)
        ...
        logger.info(f"✅ MCP-enhanced {domain} investigation completed in {duration:.2f}s "
...
async def _investigate_device_with_mcp(
    ...
    if len(results) > 0 and not isinstance(results[0], Exception):
        ...
        if ml_result.get("success"):
            ...
            findings.key_findings.extend([f"ML Analysis: {indicator}" for indicator in risk_indicators])
    ...
    if len(results) > 1 and not isinstance(results[1], Exception):
        ...
        if intel_result.get("success"):
            ...
            findings.key_findings.extend([f"Threat Intel: {threat}" for threat in threats])
    
    # Device-specific risk analysis
    device_risk_score = _calculate_device_risk(device_info, findings.ml_predictions, findings.threat_intelligence)
    ...
    if device_risk_score > 70:
        findings.key_findings.append(f"High-risk device detected (score: {device_risk_score})")
    
    logger.debug(f"Device investigation completed with {len(findings.key_findings)} findings")
    return findings
...
async def _investigate_network_with_mcp(
    ...
    if len(results) > 0 and not isinstance(results[0], Exception):
        ...
        if intel_result.get("success"):
            ...
            findings.key_findings.extend([f"Network Intel: {anomaly}" for anomaly in anomalies])
    ...
    if len(results) > 1 and not isinstance(results[1], Exception):
        ...
        if compliance_result.get("success"):
            ...
            findings.key_findings.extend([f"Compliance: {violation}" for violation in violations])
    
    # Network-specific analysis
    network_risk_score = _calculate_network_risk(network_info, findings.threat_intelligence, findings.compliance_status)
    ...
    if network_risk_score > 60:
        findings.key_findings.append(f"Suspicious network activity detected (score: {network_risk_score})")
    
    logger.debug(f"Network investigation completed with {len(findings.key_findings)} findings")
    return findings
...
async def _investigate_user_behavior_with_mcp(
    ...
    if len(results) > 0 and not isinstance(results[0], Exception):
        ...
        if ml_result.get("success"):
            ...
            findings.key_findings.extend([f"Behavior ML: {anomaly}" for anomaly in anomalies])
    ...
    if len(results) > 1 and not isinstance(results[1], Exception):
        ...
        if intel_result.get("success"):
            ...
            findings.key_findings.extend([f"Behavior Intel: {pattern}" for pattern in patterns])
    
    # User behavior risk analysis
    behavior_risk_score = _calculate_behavior_risk(behavior_info, findings.ml_predictions, findings.threat_intelligence)
    ...
    if behavior_risk_score > 75:
        findings.key_findings.append(f"Anomalous user behavior detected (score: {behavior_risk_score})")
    
    logger.debug(f"User behavior investigation completed with {len(findings.key_findings)} findings")
    return findings
...
async def _investigate_location_with_mcp(
    ...
    if intel_result.get("success"):
        ...
        findings.key_findings.extend([f"Location Intel: {risk}" for risk in location_risks])
    
    # Location-specific risk analysis
    location_risk_score = _calculate_location_risk(location_info, findings.threat_intelligence)
    ...
    if location_risk_score > 50:
        findings.key_findings.append(f"High-risk location detected (score: {location_risk_score})")
...
async def _investigate_logs_with_mcp(
    ...
    for i, result in enumerate(results):
        if not isinstance(result, Exception) and result.get("success"):
            if i == 0:  # ML analysis
                ...
                findings.key_findings.extend([f"Log ML: {anomaly}" for anomaly in anomalies])
            elif i == 1:  # Compliance
                findings.compliance_status = result["data"]
                issues = result["data"].get("compliance_issues", [])
                findings.key_findings.extend([f"Log Compliance: {issue}" for issue in issues])
    
    # Logs-specific risk analysis
    logs_risk_score = _calculate_logs_risk(logs_info, findings.ml_predictions, findings.compliance_status)
    ...
    if logs_risk_score > 65:
        findings.key_findings.append(f"Suspicious log patterns detected (score: {logs_risk_score})")
...
async def _generic_mcp_investigation(
    ...
    try:
        result = await mcp_tools.call_intelligence_gathering({
            "domain": domain,
            "context": "generic_investigation"
        })
        
        if result.get("success"):
            findings.threat_intelligence = result["data"]
            insights = result["data"].get("insights", [])
            findings.key_findings.extend([f"Generic Intel: {insight}" for insight in insights])
...
async def _analyze_device_with_ml(mcp_tools: MCPToolWrapper, device_info: Dict[str, Any]) -> Dict[str, Any]:
    """Analyze device using ML models."""
    return await mcp_tools.call_ml_models({
        "analysis_type": "device_fingerprint",
        "device_data": device_info
    })


async def _get_device_threat_intel(mcp_tools: MCPToolWrapper, device_info: Dict[str, Any]) -> Dict[str, Any]:
    """Get threat intelligence for device."""
    return await mcp_tools.call_intelligence_gathering({
        "lookup_type": "device_patterns",
        "device_data": device_info
    })


async def _analyze_network_intelligence(mcp_tools: MCPToolWrapper, network_info: Dict[str, Any]) -> Dict[str, Any]:
...
async def _check_network_compliance(mcp_tools: MCPToolWrapper, network_info: Dict[str, Any]) -> Dict[str, Any]:
...
async def _analyze_behavior_with_ml(mcp_tools: MCPToolWrapper, behavior_info: Dict[str, Any]) -> Dict[str, Any]:
...
async def _get_behavior_threat_intel(mcp_tools: MCPToolWrapper, behavior_info: Dict[str, Any]) -> Dict[str, Any]:
    """Get threat intelligence for user behavior."""
    return await mcp_tools.call_intelligence_gathering({
        "lookup_type": "behavior_patterns",
        "behavior_data": behavior_info
    })


async def _analyze_location_intelligence(mcp_tools: MCPToolWrapper, location_info: Dict[str, Any]) -> Dict[str, Any]:
...
async def _analyze_logs_with_ml(mcp_tools: MCPToolWrapper, logs_info: Dict[str, Any]) -> Dict[str, Any]:
...
async def _check_logs_compliance(mcp_tools: MCPToolWrapper, logs_info: Dict[str, Any]) -> Dict[str, Any]:
    """Check logs compliance."""
    return await mcp_tools.call_compliance_check({
        "check_type": "log_compliance",
        "log_data": logs_info
    })
...
def _calculate_device_risk(device_info: Dict[str, Any], ml_predictions: Optional[Dict[str, Any]], threat_intel: Optional[Dict[str, Any]]) -> float:
...
def _calculate_network_risk(network_info: Dict[str, Any], threat_intel: Optional[Dict[str, Any]], compliance: Optional[Dict[str, Any]]) -> float:
...
def _calculate_behavior_risk(behavior_info: Dict[str, Any], ml_predictions: Optional[Dict[str, Any]], threat_intel: Optional[Dict[str, Any]]) -> float:
...
def _calculate_location_risk(location_info: Dict[str, Any], threat_intel: Optional[Dict[str, Any]]) -> float:
...
def _calculate_logs_risk(logs_info: Dict[str, Any], ml_predictions: Optional[Dict[str, Any]], compliance: Optional[Dict[str, Any]]) -> float:
