...
class LLMPromptParser:
    ...
    @staticmethod
    def parse_prompt(prompt: str, domain: str, investigation_id: str, entity_id: str) -> str:
        """
        ...
        if numbered_items:
            for i, (number, content) in enumerate(numbered_items):
                if i == 0:  # Risk Level
                    ...
                    formatted_lines.append(f"   {number}. risk_score: {content} {risk_indicator}")
...
class LLMInteractionFormatter:
    ...
    def format_console_interaction(
        ...
        parsed_prompt = LLMPromptParser.parse_prompt(prompt, domain, investigation_id, entity_id)
    ...
    def validate_response_format(response: str) -> Dict[str, Any]:
        ...
        compliant_elements = sum(1 for elem in required_elements if elem not in validation_result["missing_elements"])
        validation_result["format_compliance"] = compliant_elements / len(required_elements)
