...
class AgentCommunicationHub:
    ...
    def __init__(
        ...
        self.agent_channels: Dict[str, Set[str]] = defaultdict(
            set
        ...
        self.route_table: Dict[str, str] = (
            {}
        )  # recipient_id -> channel_id for direct routing
        ...
        self.conversations: Dict[str, List[str]] = defaultdict(
            list
        )  # correlation_id -> message_ids
    ...
    async def send_message(
        ...
        try:
            ...
            message = AgentMessage(
                ...
                protocol=protocol,
                sender_id=sender_id,
                sender_type=self.registered_agents.get(sender_id, {}).get(
                    "agent_type", "unknown"
                ),
    ...
    async def send_response(
        ...
        )  # Could be more specific based on original message
    ...
    async def _deliver_message_to_channel(
        self, message: AgentMessage, channel_id: str
    ) -> None:
        """Deliver message to a specific channel"""

        if channel_id not in self.channels:
            self.logger.warning(
                f"Channel {channel_id} not found for message {message.message_id}"
            )
            return
    ...
    async def _deliver_message_to_agent(
        self, message: AgentMessage, agent_id: str
    ) -> None:
        ...
        try:
            ...
            if agent_id in self.agent_message_handlers:
                ...
                try:
                    ...
                    self.logger.error(
                        f"Message handler failed for agent {agent_id}: {str(e)}"
                    )
            else:
                # No handler - just mark as delivered
                message.mark_delivered()
                self.logger.debug(
                    f"No message handler for agent {agent_id}, message stored"
                )

        except Exception as e:
            self.logger.error(
                f"Failed to deliver message to agent {agent_id}: {str(e)}"
            )
    ...
    async def _handle_response_timeout(self, message: AgentMessage) -> None:
        ...
        if (
            ...
            and not message.response_received
        ):
            # Response timed out
    ...
    async def cleanup_old_messages(self) -> None:
        ...
        for correlation_id, message_ids in list(self.conversations.items()):
            ...
            recent_messages = [
                msg_id for msg_id in message_ids if msg_id in self.message_history
            ]
