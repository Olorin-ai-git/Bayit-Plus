...
class VerificationLogger:
    ...
    def log_model_health_change(self, model: str, is_healthy: bool, reason: str = ""):
        ...
        self.logger.info(
            f"{emoji} Model {model} marked as {status}" + (f": {reason}" if reason else ""),
            extra={
    ...
    def get_recent_audit_entries(self, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        recent_entries = self.audit_entries[-limit:] if limit < len(self.audit_entries) else self.audit_entries
