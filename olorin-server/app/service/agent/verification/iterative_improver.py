...
class IterativeImprover:
    ...
    def analyze_verification_failure(
        self, verification_result: Dict[str, Any], context: Dict[str, Any] = None
        ...
        if explanation:
            suggestions.extend(
                self._generate_suggestions_from_explanation(explanation, context)
            )
    ...
    def _generate_suggestions_for_issue(
        self, issue: str, context: Dict[str, Any] = None
    ) -> List[ImprovementSuggestion]:
        ...
        if any(
            keyword in issue_lower for keyword in ["incomplete", "missing", "lacks"]
        ):
            suggestions.append(
                ImprovementSuggestion(
                    strategy=ImprovementStrategy.ENHANCE_CONTEXT,
            ...
            keyword in issue_lower for keyword in ["unclear", "confusing", "ambiguous"]
        ):
            suggestions.append(
                ImprovementSuggestion(
                    strategy=ImprovementStrategy.CLARIFY_REQUIREMENTS,
            ...
            for keyword in ["format", "structure", "organization"]
        ):
            suggestions.append(
                ImprovementSuggestion(
            ...
            keyword in issue_lower for keyword in ["complex", "difficult", "technical"]
        ):
            suggestions.append(
                ImprovementSuggestion(
            ...
            keyword in issue_lower
            for keyword in ["example", "demonstration", "illustration"]
        ):
            suggestions.append(
                ImprovementSuggestion(
    ...
    def _generate_suggestions_from_explanation(
        self, explanation: str, context: Dict[str, Any] = None
    ) -> List[ImprovementSuggestion]:
        ...
        if (
            "too brief" in explanation_lower
            or "insufficient detail" in explanation_lower
        ):
            suggestions.append(
                ImprovementSuggestion(
                    strategy=ImprovementStrategy.ENHANCE_CONTEXT,
        ...
        if (
            "wrong format" in explanation_lower
            or "format mismatch" in explanation_lower
        ):
            suggestions.append(
                ImprovementSuggestion(
                    strategy=ImprovementStrategy.RESTRUCTURE_RESPONSE,
    ...
    def create_improvement_request(
        ...
        selected_strategy = self._select_best_strategy(
            suggestions, iteration_context.failed_strategies, current_attempt
        )

        # Create improvement prompt based on selected strategy
        improvement_prompt = self._create_improvement_prompt(
            selected_strategy, iteration_context, suggestions, current_attempt
        )
        ...
        system_content = self._create_enhanced_system_message(
            selected_strategy, iteration_context, current_attempt
        )
        ...
        for message in iteration_context.original_request:
            if isinstance(message, SystemMessage):
                ...
                enhanced_content = self._enhance_user_message(
                    message.content, improvement_prompt, iteration_context
                )
        ...
        logger.debug(
            f"🔄 Created improvement request (attempt {current_attempt}, strategy: {selected_strategy.value})"
        )
    ...
    def _select_best_strategy(
        ...
        available_suggestions = [
            s for s in suggestions if s.strategy not in failed_strategies
        ]
    ...
    def _create_improvement_prompt(
        ...
        if strategy == ImprovementStrategy.SPECIFIC_FEEDBACK:
            ...
            base_prompt += (
                "- Address any potential confusion from the previous attempt\n"
            )
            for suggestion in relevant_suggestions:
            ...
            base_prompt += (
                "Your previous response lacked sufficient context or detail. Please:\n"
            )
            ...
            base_prompt += (
                "Your previous response had structural or format issues. Please:\n"
            )
            ...
            base_prompt += (
                "Your previous response was too complex or technical. Please:\n"
            )
            ...
            base_prompt += (
                "Your previous response would benefit from examples. Please:\n"
            )
    ...
    def _create_enhanced_system_message(
        self, strategy: ImprovementStrategy, context: IterationContext, attempt: int
    ) -> str:
...
"""
    ...
    def _enhance_user_message(
        self, original_content: str, improvement_prompt: str, context: IterationContext
    ) -> str:
    ...
    def record_strategy_outcome(
        ...
        if confidence_improvement != 0.0:
            ...
            stats["avg_confidence_improvement"] = (
                current_avg * (total_attempts - 1) + confidence_improvement
            ) / total_attempts

        logger.debug(
            f"📊 Strategy {strategy.value} outcome recorded: success={success}, improvement={confidence_improvement:.3f}"
        )
    ...
    def get_strategy_effectiveness_report(self) -> Dict[str, Any]:
        ...
        for strategy, stats in self.strategy_effectiveness.items():
            total_attempts = stats["successes"] + stats["failures"]
            success_rate = (
                stats["successes"] / total_attempts if total_attempts > 0 else 0
            )
            ...
            report[strategy.value] = {
                ...
                "avg_confidence_improvement": round(
                    stats["avg_confidence_improvement"], 3
                ),
            }
