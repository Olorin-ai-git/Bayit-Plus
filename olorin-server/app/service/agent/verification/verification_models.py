...
class VerificationModels:
    ...
    def _initialize_available_models(self):
        ...
        if not self.initialized_models:
            raise RuntimeError(
                "No verification models could be initialized! Check API keys."
            )

        logger.info(
            f"🎯 Successfully initialized {len(self.initialized_models)} verification models"
        )
    ...
    def _select_most_cost_effective(
        self, models: List[VerificationModel]
    ) -> VerificationModel:
        """Select model with lowest cost per token."""
        return min(
            models, key=lambda m: self.config.get_model_config(m).cost_per_1k_tokens
        )
    ...
    def _select_most_accurate(
        ...
        return max(
            models, key=lambda m: self.config.get_model_config(m).accuracy_rating
        )
    ...
    async def verify_with_best_model(
        ...
        for attempt in range(max_attempts):
            try:
                ...
                logger.debug(
                    f"🔍 Attempting verification with {selected_model.value} (attempt {attempt + 1})"
                )
                ...
                if response.error is None:
                    logger.debug(
                        f"✅ Verification successful with {selected_model.value}"
                    )
                    return response
                else:
                    logger.warning(
                        f"⚠️  Model {selected_model.value} returned error: {response.error}"
                    )
...
Be thorough but concise. Focus on accuracy, relevance, and completeness."""
    ...
    def _parse_verification_response(self, content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            is_valid = any(
                ...
                for phrase in ["valid", "correct", "accurate", "appropriate"]
            )
    ...
    def get_model_health_status(self) -> Dict[str, Any]:
        ...
        return {
            "healthy_models": [
                m.value for m, healthy in self.model_health.items() if healthy
            ],
            "unhealthy_models": [
                m.value for m, healthy in self.model_health.items() if not healthy
            ],
            "total_models": len(self.initialized_models),
            "health_percentage": (
                sum(self.model_health.values()) / len(self.model_health)
        ...
        for model in self.initialized_models.keys():
            try:
                ...
                if is_healthy:
                    ...
                    logger.warning(
                        f"❌ Health check failed for {model.value}: {response.error}"
                    )
