...
class VerificationMetrics:
    ...
    def record_verification_success(
        ...
        self.response_times.append(MetricPoint(current_time, response_time_ms, {'model': model}))
        self.confidence_scores.append(MetricPoint(current_time, confidence_score, {'model': model}))
        self.verification_results.append(MetricPoint(current_time, 1.0, {'result': 'success', 'model': model}))
        ...
        logger.debug(f"📈 Recorded successful verification: {model} ({response_time_ms}ms, {confidence_score:.2f} confidence)")
    ...
    def record_verification_failure(
        ...
        current_time = time.time()
        self.response_times.append(MetricPoint(current_time, response_time_ms, {'model': model}))
        self.verification_results.append(MetricPoint(current_time, 0.0, {'result': 'failure', 'model': model, 'error': error_type}))
        ...
        logger.debug(f"📉 Recorded failed verification: {model} ({error_type}, {response_time_ms}ms)")
    ...
    def get_current_performance(self) -> Dict[str, Any]:
        ...
        if self.response_times:
            ...
            p95_response_time_ms = statistics.quantiles(recent_times, n=20)[18] if len(recent_times) > 10 else avg_response_time_ms
        else:
        ...
        if self.confidence_scores:
            recent_confidence = [point.value for point in list(self.confidence_scores)[-100:]]
    ...
    def get_model_performance(self) -> Dict[str, Any]:
        ...
        for model, stats in self.model_metrics.items():
            ...
            avg_cost_per_call = stats['total_cost'] / total_calls if stats['total_cost'] > 0 else 0
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        health_checks['response_time'] = {
            ...
            'healthy': avg_response_time <= self.health_thresholds['max_avg_response_time_ms'],
    ...
    def get_trend_analysis(self, hours: int = 24) -> Dict[str, Any]:
        ...
        for window in relevant_windows:
            if window.total_verifications > 0:
                success_rate = window.successful_verifications / window.total_verifications
                avg_response_time = window.total_response_time_ms / window.total_verifications
        ...
        if not success_rates:
            return {'message': f'No verification data available for {hours}h trend analysis'}
