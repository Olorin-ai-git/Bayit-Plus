...
class VerificationConfig:
    ...
    def __init__(self):
        ...
        logger.info(
            f"🔧 Verification configuration initialized - Enabled: {self.enabled}"
        )
    ...
    def _load_configuration(self):
        ...
        self.primary_model = VerificationModel(
            self._get_str_env(
                "LLM_VERIFICATION_MODEL", VerificationModel.GPT_3_5_TURBO.value
            )
        )
        ...
        self.parallel_enabled = self._get_bool_env(
            "LLM_VERIFICATION_PARALLEL_ENABLED", False
        )
        ...
        self.retry_exponential_backoff = self._get_bool_env(
            "LLM_VERIFICATION_RETRY_EXPONENTIAL_BACKOFF", True
        )

        # Emergency and debugging settings
        self.bypass_for_tools = self._get_bool_env(
            "LLM_VERIFICATION_BYPASS_FOR_TOOLS", False
        )
        self.log_level = self._get_str_env("LLM_VERIFICATION_LOG_LEVEL", "INFO")
        self.metrics_enabled = self._get_bool_env(
            "LLM_VERIFICATION_METRICS_ENABLED", True
        )

        # Advanced settings
        self.batch_size = self._get_int_env("LLM_VERIFICATION_BATCH_SIZE", 1)
        self.fallback_models = self._get_list_env(
            "LLM_VERIFICATION_FALLBACK_MODELS",
            [
    ...
    def _load_api_credentials(self):
        ...
        try:
            ...
            self.openai_api_key = config_loader.load_secret(
                "VERIFICATION_OPENAI_API_KEY"
            ) or config_loader.load_secret("OPENAI_API_KEY")

            self.anthropic_api_key = config_loader.load_secret(
                "VERIFICATION_ANTHROPIC_API_KEY"
            ) or config_loader.load_secret("ANTHROPIC_API_KEY")

            self.google_api_key = config_loader.load_secret(
                "VERIFICATION_GOOGLE_API_KEY"
            ) or config_loader.load_secret("GOOGLE_API_KEY")

        except Exception as e:
            logger.warning(
                f"Failed to load API credentials from config loader/Firebase Secrets: {e}"
            )
    ...
    def _validate_configuration(self):
        ...
        if self.confidence_threshold < 0.5:
            issues.append(
                "Confidence threshold is very low (<0.5), may accept poor responses"
            )

        if self.confidence_threshold > 0.95:
            issues.append(
                "Confidence threshold is very high (>0.95), may cause excessive retries"
            )
        ...
        if self.enabled:
            ...
            if not self._has_required_api_key(self.primary_model):
                issues.append(
                    f"No API key available for primary model: {primary_config.name}"
                )
