...
from .communication.ice_events import ICEEventType, get_event_bus
from .communication.investigation_state import InvestigationState, get_state_manager
from .multi_entity.entity_manager import EntityType, get_entity_manager
from .patterns import PatternResult, PatternType, get_pattern_registry
from .rag.rag_orchestrator import RAGRequest, get_rag_orchestrator
...
class IntegratedAgentSystem:
    ...
    async def execute_integrated_investigation(
        ...
        try:
            ...
            pattern_results = await self._execute_investigation_patterns(
                request, result, ws_streaming
            )
    ...
    async def _execute_multi_entity_analysis(
        ...
        try:
            ...
            related_entities = self.entity_manager.get_related_entities(
                primary_entity_id, max_depth=2
            )

            # Analyze entity network
            network_analysis = self.entity_manager.analyze_entity_network(
                primary_entity_id
            )
    ...
    async def _execute_investigation_patterns(
        ...
        try:
            ...
            result.patterns_executed = [
                pattern.value for pattern in patterns_to_execute
            ]
            ...
            for pattern_type in patterns_to_execute:
                try:
                    ...
                    self.logger.error(
                        f"Pattern {pattern_type.value} execution failed: {str(e)}"
                    )
                    pattern_results[pattern_type.value] = PatternResult.error_result(
                        str(e)
                    )
    ...
    async def _integrate_results(
        ...
        try:
            ...
            if confidence_scores:
                result.confidence_score = sum(confidence_scores) / len(
                    confidence_scores
                )
            ...
            result.risk_assessment = {
                "overall_risk_score": min(
                    1.0, result.confidence_score * 0.8
                ...
                "patterns_consensus": len(
                    [
                        pr
                        for pr in result.pattern_results.values()
                        if pr.success and pr.confidence_score >= 0.7
                    ]
                ),
        ...
        try:
            ...
            if request.require_evidence and result.evidence_strength < 0.3:
                ...
                result.error_message = (
                    f"Insufficient evidence strength ({result.evidence_strength:.2f})"
                )
    ...
    async def _finalize_investigation(
        ...
        try:
            ...
            await self.event_bus.publish(
                (
                    ICEEventType.INVESTIGATION_COMPLETED
                    if result.success
                    else ICEEventType.INVESTIGATION_FAILED
                ),
                {
                    "investigation_id": request.investigation_id,
    ...
    def _select_patterns_for_investigation(
        self, request: IntegratedInvestigationRequest
    ) -> List[PatternType]:
        ...
        return pattern_mapping.get(
            request.investigation_type, [PatternType.AUGMENTED_LLM]  # Default pattern
        )
    ...
    def get_system_statistics(self) -> Dict[str, Any]:
        ...
        system_stats = {
            ...
            "processing_stats": {
                ...
                "success_rate": self.stats["successful_investigations"]
                / max(1, self.stats["investigations_processed"]),
