...
class MCPConnectionPool:
    ...
    async def get_connection(
        self, server_name: str, endpoint: MCPServerEndpoint
        ...
        async with self.locks[server_name]:
            ...
            if server_name in self.connections and self.connections[server_name]:
                ...
                try:
                    ...
                    if (
                        len(self.connections[server_name])
...
class MCPCache:
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.warning(
                f"Failed to initialize MCP cache with Redis, cache disabled: {e}"
            )
            self.redis_client = None
    ...
    async def set(
        ...
        try:
            ...
            success = self.redis_client.set(
                cache_key, json.dumps(cache_data), ex=ttl or self.default_ttl
            )
    ...
    async def invalidate(
        self, server_name: str, method: str = None, params: Dict[str, Any] = None
    ):
        ...
        try:
            if method and params:
                ...
                logger.info(
                    f"Server-wide cache invalidation requested for {server_name}"
                )
...
class EnhancedMCPClient:
    ...
    def register_server(self, endpoint: MCPServerEndpoint):
        ...
        self.circuit_breakers[endpoint.name] = MCPCircuitBreaker(
            endpoint.name, failure_threshold=endpoint.circuit_breaker_threshold
        )
        ...
        try:
            ...
            if use_cache and mcp_response.success:
                await self.cache.set(
                    server_name, method, params, mcp_response, cache_ttl
                )
    ...
    def _update_metrics(self, server_name: str, success: bool, execution_time_ms: int):
        ...
        metrics["avg_response_time"] = (
            current_avg * (request_count - 1) + execution_time_ms
        ) / request_count
...
async def _register_default_servers(client: EnhancedMCPClient):
    ...
    try:
        ...
        if settings.mcp_blockchain_enabled:
            ...
            if not api_key:
                ...
                try:
                    ...
                    logger.warning(
                        f"Could not get blockchain MCP API key from secrets: {e}"
                    )
        ...
        if settings.mcp_intelligence_enabled:
            ...
            if not api_key:
                ...
                try:
                    ...
                    logger.warning(
                        f"Could not get intelligence MCP API key from secrets: {e}"
                    )
        ...
        if os.getenv("USE_BLOCKCHAIN_MCP_CLIENT") == "true":
            client.register_server(
                MCPServerEndpoint(
                    name="blockchain_analysis",
                    url=os.getenv(
                        "BLOCKCHAIN_MCP_ENDPOINT", "http://localhost:8080/mcp"
                    ),
        ...
        if os.getenv("USE_INTELLIGENCE_MCP_CLIENT") == "true":
            client.register_server(
                MCPServerEndpoint(
                    name="intelligence_gathering",
                    url=os.getenv(
                        "INTELLIGENCE_MCP_ENDPOINT", "http://localhost:8081/mcp"
                    ),
        ...
        logger.info(
            "Default MCP servers registered from fallback environment variables"
        )
