...
class MCPHealthMonitor:
    ...
    def __init__(
        ...
        self.server_priorities: Dict[str, Dict[str, int]] = {}  # service -> {server: priority}
    ...
    def register_server(
        ...
        if service_type not in self.primary_servers:
            ...
            current_priority = self.server_priorities[service_type].get(current_primary, -1)
    ...
    def _default_failover_rules(self) -> List[FailoverRule]:
        ...
        return [
            ...
            FailoverRule(
                trigger_condition="health_score",
                threshold=0.5,
                action="alert_only"
            ),
            FailoverRule(
    ...
    async def check_server_health(self, server_name: str) -> ServerHealthReport:
        ...
        try:
            ...
            metrics['response_time'] = HealthMetric(
                ...
                status=HealthStatus.HEALTHY if response_time < 5.0 else HealthStatus.UNHEALTHY
            )
            ...
            metrics['error_rate'] = HealthMetric(
                ...
                value=error_rate,
                threshold=0.1,  # 10%
                status=HealthStatus.HEALTHY if error_rate < 0.1 else HealthStatus.UNHEALTHY
            )
            ...
            metrics['memory_usage'] = HealthMetric(
                ...
                value=memory_usage,
                threshold=0.8,  # 80%
                status=HealthStatus.HEALTHY if memory_usage < 0.8 else HealthStatus.DEGRADED
            )
            ...
            metrics['connection_count'] = HealthMetric(
                ...
                threshold=100,
                status=HealthStatus.HEALTHY if connection_count < 100 else HealthStatus.DEGRADED
            )
    ...
    async def _switch_primary_server(self, failed_server: str) -> None:
        ...
        healthy_servers = [
            (server, priority) for server, priority in available_servers.items()
            if server != failed_server and
            self.server_health.get(server, {}).get('overall_status') == HealthStatus.HEALTHY
        ]
        ...
        if healthy_servers:
            ...
            logger.info(f"Switched primary server for {service_type}: {old_primary} -> {new_primary}")
    ...
    async def _trigger_alert(
        ...
        alert = {
            ...
            'message': message,
            'health_status': self.server_health.get(server_name, {}).overall_status.value
        }
