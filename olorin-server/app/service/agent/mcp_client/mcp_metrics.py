...
import time
import psutil
from typing import Dict, List, Optional, Any, Counter, DefaultDict
...
class MCPMetricsCollector:
    ...
    def __init__(self, max_history_size: int = 1000):
        ...
        self.histograms: DefaultDict[str, deque] = defaultdict(lambda: deque(maxlen=max_history_size))
        self.timers: DefaultDict[str, deque] = defaultdict(lambda: deque(maxlen=max_history_size))
        ...
        self.response_times: DefaultDict[str, deque] = defaultdict(lambda: deque(maxlen=max_history_size))
    ...
    def track_request(self, server_name: str, operation: str, success: bool = True, response_time: Optional[float] = None) -> None:
    ...
    def track_circuit_breaker(self, server_name: str, state: str) -> None:
        ...
        self.set_gauge('mcp_circuit_breaker_state', 1.0 if state == 'open' else 0.0, 
                      {'server': server_name})
    ...
    def get_all_metrics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            for server_name in set(list(self.request_counts.keys()) + list(self.error_counts.keys())):
                metrics['server_stats'][server_name] = self.get_server_stats(server_name)
