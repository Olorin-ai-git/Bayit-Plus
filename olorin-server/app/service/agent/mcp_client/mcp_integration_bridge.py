...
from .mcp_client_manager import MCPServerCategory, MCPClientManager
...
class MCPIntegrationBridge:
    ...
    async def _migrate_legacy_servers(self):
        ...
        for server_name, legacy_config in self._legacy_manager.servers.items():
            ...
            try:
                ...
                logger.info(f"Migrated MCP server: {server_name} ({legacy_config.category.value})")
    ...
    async def invalidate_cache(self, server_name: str, tool_name: Optional[str] = None):
        ...
        try:
            ...
            logger.info(f"Cache invalidated for {server_name}" + (f".{tool_name}" if tool_name else ""))
        ...
        try:
            ...
            for server_name, server_health in system_status.get("servers", {}).items():
                metrics["servers"][server_name] = {
                    ...
                    "circuit_breaker_state": server_health.get("circuit_breaker_state", "unknown"),
                    "total_requests": server_health.get("metrics", {}).get("requests", 0),
                    "success_rate": (
                        server_health.get("metrics", {}).get("successes", 0) /
                        max(server_health.get("metrics", {}).get("requests", 1), 1) * 100
                    ),
                    "avg_response_time": server_health.get("metrics", {}).get("avg_response_time", 0)
                }
