...
class BulletproofExceptionHandler:
        ...
        def decorator(func):
            ...
            async def wrapper(*args, **kwargs):
                ...
                if circuit_breaker and not await self._check_circuit_breaker(circuit_breaker):
                    logger.warning(f"🚨 Circuit breaker {circuit_breaker} is OPEN, using fallback")
                    return await self._execute_fallback_strategy(fallback_strategy, *args, **kwargs)
                ...
                for attempt in range(max_retries + 1):
                    try:
                        ...
                        if attempt < max_retries and await self._should_retry(exception_context):
                            retry_delay = await self._calculate_retry_delay(attempt, exception_context)
                            logger.warning(f"⚠️ Attempt {attempt + 1} failed, retrying in {retry_delay}s: {str(e)}")
                            await asyncio.sleep(retry_delay)
                            continue
                        
                        # All retries exhausted, apply resilience strategy
                        return await self._apply_resilience_strategy(
                            resilience_level, exception_context, fallback_strategy, *args, **kwargs
                        )
    ...
    async def transform_exception_to_response(
        ...
        """
        logger.info(f"🔧 Transforming exception to actionable response for {investigation_id}")
        ...
        try:
            ...
            recovery_suggestions = await self._generate_recovery_suggestions(exception_context)
            ...
            actionable_response = await self._create_actionable_response(exception_context)
            
            logger.info(f"✅ Transformed {failure_type.value} exception to actionable response")
    ...
    async def prevent_cascading_failures(
        ...
        try:
            ...
            dependent_components = await self._identify_dependent_components(failed_component)
            ...
            prevention_plan = {
                ...
                "recovery_strategy": await self._select_cascade_recovery_strategy(failure_context),
                "estimated_impact": await self._estimate_cascade_impact(failed_component),
                "prevention_actions": [
            ...
            logger.info(f"✅ Created cascade prevention plan isolating {len(isolation_plan)} components")
            return prevention_plan
            
        except Exception as e:
            logger.error(f"🚨 Failed to prevent cascading failures: {str(e)}")
            return {"status": "emergency_isolation", "failed_component": failed_component}
    ...
    async def handle_service_degradation(
        ...
        """
        logger.info(f"⚡ Handling service degradation: {degraded_services} at {degradation_level:.2f} level")
        
        try:
            # Assess criticality of degraded services
            criticality_assessment = await self._assess_service_criticality(degraded_services)
            ...
            degradation_plan = {
                ...
                "recovery_conditions": await self._define_recovery_conditions(degraded_services),
                "alternative_capabilities": await self._identify_alternative_capabilities(degraded_services)
            }
            ...
            return await self._create_emergency_degradation_plan(investigation_id, degraded_services)
    ...
    async def preserve_error_context(
        ...
        try:
            ...
            enhanced_context = {
                ...
                "orchestration_state": await self._capture_orchestration_state(investigation_id),
                "agent_states": await self._capture_agent_states(investigation_id),
                "performance_metrics": await self._capture_performance_context(investigation_id),
                "recent_decisions": await self._capture_recent_decisions(investigation_id),
                ...
                "recovery_recommendations": await self._generate_context_recovery_recommendations(error_data)
            }
            ...
            logger.info(f"✅ Preserved error context {context_id} with {len(enhanced_context)} metadata fields")
            return context_id
    ...
    def _initialize_recovery_actions(self):
        ...
        self.recovery_actions = {
            FailureType.EXTERNAL_SERVICE: [
                RecoveryAction(
                    "retry_with_backoff", "retry", "Exponential backoff retry",
                    self._retry_with_backoff, 0.8, 2.5
                ),
                RecoveryAction(
                    "switch_to_fallback_service", "fallback", "Use alternative service",
                    self._switch_to_fallback_service, 0.9, 1.0
                ),
                RecoveryAction(
                    "use_cached_data", "cache", "Use cached results",
                    self._use_cached_data, 0.7, 0.5
                )
            ],
            FailureType.LLM_API_FAILURE: [
                RecoveryAction(
                    "fallback_to_alternative_llm", "llm_fallback", "Switch LLM provider",
                    self._fallback_to_alternative_llm, 0.85, 3.0
                ),
                RecoveryAction(
                    "use_rule_based_response", "rule_based", "Rule-based decision",
                    self._use_rule_based_response, 0.6, 0.8
                )
            ],
            FailureType.AGENT_EXECUTION: [
                RecoveryAction(
                    "restart_agent", "restart", "Restart failed agent",
                    self._restart_agent, 0.75, 2.0
                ),
                RecoveryAction(
                    "delegate_to_alternative_agent", "delegation", "Use backup agent",
                    self._delegate_to_alternative_agent, 0.8, 1.5
                )
            ]
        }
    ...
    def _initialize_circuit_breakers(self):
        ...
        services = [
            ...
            "splunk_api", "threat_intelligence", "websocket_manager"
        ]
    ...
    def _classify_failure_type(self, exception: Exception) -> FailureType:
        ...
        if any(term in exception_str for term in ["connection", "timeout", "network", "503", "502", "504"]):
            return FailureType.EXTERNAL_SERVICE
        
        # LLM API specific failures
        if any(term in exception_str for term in ["openai", "anthropic", "rate_limit", "api_key", "model"]):
            return FailureType.LLM_API_FAILURE
        
        # Data validation failures
        if any(term in exception_type for term in ["validation", "parse", "json", "schema"]):
            return FailureType.DATA_VALIDATION
        
        # Resource exhaustion
        if any(term in exception_str for term in ["memory", "quota", "limit", "timeout"]):
            return FailureType.RESOURCE_EXHAUSTION
        
        # Configuration errors
        if any(term in exception_str for term in ["config", "environment", "missing", "not_found"]):
    ...
    async def _assess_exception_impact(self, exception_context: ExceptionContext) -> Dict[str, Any]:
        """Assess the impact of exception on investigation"""
        
        severity = "medium"
        if exception_context.failure_type in [FailureType.LLM_API_FAILURE, FailureType.EXTERNAL_SERVICE]:
            severity = "high"
    ...
    async def _generate_recovery_suggestions(self, exception_context: ExceptionContext) -> List[str]:
    ...
    async def _create_actionable_response(self, exception_context: ExceptionContext) -> Dict[str, Any]:
    ...
    async def _create_emergency_response(self, investigation_id: str, exception: Exception) -> Dict[str, Any]:
    ...
    async def _calculate_retry_delay(self, attempt: int, exception_context: ExceptionContext) -> float:
    ...
    async def _execute_fallback_strategy(self, strategy_name: Optional[str], *args, **kwargs):
    ...
    async def _transparent_recovery(self, exception_context: ExceptionContext, *args, **kwargs):
        """Attempt transparent recovery without user awareness"""
        return {"status": "transparent_recovery", "recovered": True}
    
    async def _graceful_degradation(self, exception_context: ExceptionContext, *args, **kwargs):
        """Gracefully degrade functionality"""
        return {"status": "graceful_degradation", "reduced_functionality": True}
    
    async def _partial_completion(self, exception_context: ExceptionContext, *args, **kwargs):
        """Complete with available resources"""
        return {"status": "partial_completion", "partial_results": True}
    
    async def _fail_safe_mode(self, exception_context: ExceptionContext, *args, **kwargs):
        """Enter fail-safe mode"""
        return {"status": "fail_safe_mode", "minimal_functionality": True}
    
    async def _controlled_failure(self, exception_context: ExceptionContext, *args, **kwargs):
    ...
    async def _retry_with_backoff(self, *args, **kwargs): return {"action": "retry_with_backoff"}
    async def _switch_to_fallback_service(self, *args, **kwargs): return {"action": "fallback_service"}
    async def _use_cached_data(self, *args, **kwargs): return {"action": "cached_data"}
    async def _fallback_to_alternative_llm(self, *args, **kwargs): return {"action": "alternative_llm"}
    async def _use_rule_based_response(self, *args, **kwargs): return {"action": "rule_based"}
    async def _restart_agent(self, *args, **kwargs): return {"action": "restart_agent"}
    async def _delegate_to_alternative_agent(self, *args, **kwargs): return {"action": "delegate_agent"}
    
    # Service degradation strategy implementations
    
    async def _reduce_analysis_depth(self, *args, **kwargs): return {"strategy": "reduced_depth"}
    async def _skip_non_critical_agents(self, *args, **kwargs): return {"strategy": "skip_non_critical"}
    async def _use_cached_results(self, *args, **kwargs): return {"strategy": "cached_results"}
    async def _enable_fast_mode(self, *args, **kwargs): return {"strategy": "fast_mode"}
    async def _emergency_summary(self, *args, **kwargs): return {"strategy": "emergency_summary"}
    
    # Utility methods for various operations
    
    def _generate_exception_id(self, investigation_id: str, exception: Exception) -> str:
    ...
    def _generate_error_context_id(self, investigation_id: str, error_data: Dict[str, Any]) -> str:
    ...
    async def _capture_orchestration_state(self, investigation_id: str) -> Dict[str, Any]: 
        return {"orchestration_active": True}
    
    async def _capture_agent_states(self, investigation_id: str) -> Dict[str, Any]: 
        return {"active_agents": []}
    
    async def _capture_performance_context(self, investigation_id: str) -> Dict[str, Any]: 
        return {"performance_metrics": []}
    
    async def _capture_recent_decisions(self, investigation_id: str) -> List[Dict[str, Any]]: 
        return []
    ...
    async def _generate_context_recovery_recommendations(self, error_data: Dict[str, Any]) -> List[str]: 
    ...
    async def _create_debugging_breadcrumbs(self, context_id: str, context: Dict[str, Any]):
        """Create debugging breadcrumbs"""
    ...
    async def _identify_alternative_paths(self, component: str, context: Dict[str, Any]) -> List[str]:
    ...
    async def _select_degradation_strategy(self, services: List[str], level: float, assessment: Dict[str, str]) -> str:
        if level > 0.7:
    ...
    async def _apply_degradation_strategy(self, investigation_id: str, strategy: str) -> Dict[str, Any]:
        return {"strategy_applied": strategy, "result": "success"}
    
    async def _create_degradation_monitoring(self, services: List[str], strategy: str) -> Dict[str, Any]:
    ...
    async def _identify_alternative_capabilities(self, services: List[str]) -> Dict[str, List[str]]:
        return {service: ["alternative_1", "alternative_2"] for service in services}
    
    async def _create_emergency_degradation_plan(self, investigation_id: str, services: List[str]) -> Dict[str, Any]:
        return {
            "plan_type": "emergency",
            "investigation_id": investigation_id,
