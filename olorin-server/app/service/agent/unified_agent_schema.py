...
class AgentRiskResponse(BaseModel):
    ...
    risk_factors: List[str] = Field(
        ..., min_length=0, description="Specific risk indicators found during analysis"
    )

    mitigation_measures: List[str] = Field(
        ..., min_length=0, description="Recommended actions to address identified risks"
    )
    ...
    analysis_duration_ms: Optional[int] = Field(
        None, description="Time taken for analysis in milliseconds"
    )
    ...
    def validate_mitigation_measures(cls, v):
        ...
        filtered = [
            measure
            for measure in v
            if measure and isinstance(measure, str) and measure.strip()
        ]
        return filtered
...
def convert_legacy_response(
    ...
    from .schema_validator_fix import AgentType as SchemaAgentType
    from .schema_validator_fix import get_unified_validator
    ...
    risk_result = validator.extract_risk_score(legacy_response, schema_agent_type)
...
def _extract_domain_specific(
    agent_type: AgentType, response: Union[str, Dict]
) -> Dict[str, Any]:
    ...
    try:
        ...
        if agent_type == AgentType.NETWORK:
            ...
            for field in [
                ...
                if field not in domain_data:
                    domain_data[field] = (
                        {} if "scores" in field or "metadata" in field else []
                    )
