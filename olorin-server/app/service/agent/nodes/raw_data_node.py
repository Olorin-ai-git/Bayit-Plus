...
class RawDataNode:
    ...
    async def process_csv_data(
        self, csv_content: Union[str, bytes], filename: Optional[str] = None
    ) -> Dict[str, Any]:
        """
            ...
            for message in state.get("messages", []):
                if isinstance(message, HumanMessage):
                    ...
                    if (
                        ...
                        csv_content = message.additional_kwargs.get(
                            "csv_data"
                        ) or message.additional_kwargs.get("file_content")
                    ...
                    if isinstance(content, str):
                        ...
                        try:
                            ...
                            if isinstance(parsed_content, dict):
                                csv_content = parsed_content.get(
                                    "csv_data"
                                ) or parsed_content.get("file_content")
            ...
            if result["success"] and investigation_id:
                try:
                    ...
                    from app.service.agent.orchestration.investigation_coordinator import (
                        update_investigation_with_raw_data,
                    )

                    await update_investigation_with_raw_data(investigation_id, result)
                except Exception as e:
                    self.logger.warning(
                        f"Failed to update investigation with raw data results: {e}"
                    )
            ...
            for delimiter in delimiters:
                try:
                    ...
                    if len(df.columns) > 1:  # Successfully parsed
                        self.logger.info(
                            f"Successfully parsed CSV with {len(df)} rows, {len(df.columns)} columns"
                        )
                        return df
    ...
    async def _validate_transaction_data(
        ...
        validated_df = (
            pd.DataFrame(validated_records) if validated_records else pd.DataFrame()
        )
        ...
        quality_report.quality_score = self._calculate_quality_score(
            quality_report, validated_df
        )
    ...
    async def _process_data_batches(self, df: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        for start_idx in range(0, len(df), self.batch_size):
            ...
            batch_summary = {
                ...
                "amount_avg": (
                    batch["amount"].mean() if "amount" in batch.columns else 0
                ),
                "unique_merchants": (
                    batch["merchant"].nunique() if "merchant" in batch.columns else 0
                ),
                "date_range": {
                    "start": (
                        batch["timestamp"].min().isoformat()
                        if "timestamp" in batch.columns
                        else None
                    ),
                    "end": (
                        batch["timestamp"].max().isoformat()
                        if "timestamp" in batch.columns
                        else None
                    ),
                },
    ...
    def _calculate_quality_score(
        self, quality_report: DataQualityReport, df: pd.DataFrame
    ) -> float:
        ...
        # Combined score
        final_score = max(
            0.0, min(1.0, (validity_score + completeness_score) / 2 - issue_penalty)
        )
    ...
    def _initialize_validation_rules(self) -> List[TransactionValidationRule]:
        ...
        return [
            ...
            TransactionValidationRule(
                "amount", required=True, data_type=float, min_value=0.0
            ),
    ...
    def _update_processing_stats(self, quality_report: DataQualityReport) -> None:
        ...
        new_avg = (
            (old_avg * (files_processed - 1)) + quality_report.quality_score
        ) / files_processed
        self.processing_stats["average_quality_score"] = round(new_avg, 3)

        self.processing_stats["anomalies_detected"] += len(
            quality_report.anomalies_detected
        )
