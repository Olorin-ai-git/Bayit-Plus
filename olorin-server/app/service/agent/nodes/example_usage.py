...
from app.service.agent.nodes.raw_data_node import RawDataNode, MessagesState
...
async def generate_data_quality_report(state: MessagesState, config=None) -> MessagesState:
    """
...
async def generate_investigation_summary(state: MessagesState, config=None) -> MessagesState:
    ...
    for message in reversed(messages):
        if isinstance(message, AIMessage):
            ...
            if kwargs.get('node_type') == 'raw_data_processing' and not raw_data_results:
    ...
    if raw_data_results:
        ...
        summary_content += f"Anomalies Detected: {raw_data_results['anomalies_count']}\n\n"
    ...
    summary_message = AIMessage(
        ...
        additional_kwargs={
            'node_type': 'investigation_summary',
            'status': 'complete'
        }
    )
