...
from .unified_agent_schema import (
    AgentRiskResponse, AgentType, ensure_valid_response,
    RiskLevel, create_agent_response
)
...
class FallbackStrategy(str, Enum):
    ...
    STRUCTURED_FALLBACK = "structured_fallback"  # Use predefined response based on input
...
class GuardedLLMCaller:
        ...
        try:
            ...
            result = await asyncio.wait_for(
                self._execute_guarded_call(
                    llm_function, agent_type, investigation_id, 
                    fallback_data, result, **llm_kwargs
                ),
            ...
            result.error = f"Total operation timeout ({self.config.total_timeout}s) exceeded"
            result.used_fallback = True
            result.fallback_reason = "total_timeout"
            
            if self.config.log_failures:
                logger.error(f"🚨 Guarded LLM call total timeout for {agent_type.value}: {result.error}")
            ...
            if self.config.log_failures:
                logger.error(f"🚨 Unexpected error in guarded LLM call for {agent_type.value}: {str(e)}")
    ...
    async def _execute_guarded_call(
        ...
        **llm_kwargs
        ...
        for attempt_num in range(1, self.config.max_retries + 1):
            ...
            try:
                if self.config.log_attempts:
                    logger.info(f"🔄 LLM call attempt {attempt_num}/{self.config.max_retries} for {agent_type.value}")
                
                # Execute LLM call with per-call timeout
                try:
                    llm_response = await asyncio.wait_for(
                        llm_function(**llm_kwargs),
                    ...
                    if not validation_errors:
                        ...
                        if self.config.log_attempts:
                            logger.info(f"✅ LLM call successful on attempt {attempt_num} for {agent_type.value}")
                        ...
                        if self.config.log_failures:
                            logger.warning(f"⚠️ LLM response validation failed on attempt {attempt_num}: {validation_errors}")
                ...
                if self.config.log_failures:
                    logger.warning(f"❌ LLM call exception on attempt {attempt_num}: {str(e)}")
        ...
        if self.config.log_fallbacks:
            logger.warning(f"🛡️ Using fallback response for {agent_type.value} after {self.config.max_retries} failed attempts")
    ...
    def _validate_llm_response(self, response: Any) -> List[str]:
        ...
        try:
            ...
            if self.config.require_risk_score:
                ...
                elif risk_score < self.config.min_risk_score or risk_score > self.config.max_risk_score:
                    errors.append(f"Risk score {risk_score} out of valid range [{self.config.min_risk_score}, {self.config.max_risk_score}]")
    ...
    def _generate_fallback_response(
        ...
        if self.config.fallback_strategy == FallbackStrategy.ZERO_RISK:
            ...
            raise Exception(f"LLM call failed and configured to raise exception: {reason}")
        ...
        domain_specific = self._generate_fallback_domain_specific(agent_type, fallback_data)
    ...
    def _infer_risk_from_fallback_data(self, fallback_data: Optional[Dict[str, Any]]) -> tuple[float, float]:
        ...
        suspicious_keywords = [
            'suspicious', 'anomaly', 'unusual', 'irregular', 'fraud', 
            'malicious', 'threat', 'violation', 'breach', 'attack'
        ]
    ...
    def _generate_fallback_risk_factors(
        ...
        return base_factors.get(agent_type, ["Analysis completed with processing limitations"])
    
    def _generate_fallback_mitigation_measures(self, agent_type: AgentType) -> List[str]:
        ...
        return base_measures.get(agent_type, ["Conduct manual review of analysis results"])
    ...
    def _generate_fallback_domain_specific(
        ...
        if agent_type == AgentType.NETWORK:
            domain_data.update({
                'network_red_flags': ['Processing constraints prevented full analysis'],
            ...
            domain_data.update({
                ...
                'activity_timeline': ['Timeline analysis limited by processing constraints']
            })
            
        elif agent_type == AgentType.RISK_AGGREGATION:
            domain_data.update({
                'cross_domain_correlations': ['Limited cross-domain analysis available'],
                'risk_classification': ['Manual risk classification recommended'],
                'aggregation_metadata': {'fallback_used': True, 'processing_limited': True},
                'individual_agent_scores': {'fallback_note': 'Individual scores may be incomplete'}
            })
        
        # Add any available fallback data
        if fallback_data:
            domain_data['fallback_data_available'] = True
            domain_data['fallback_data_summary'] = str(fallback_data)[:500]  # Truncate for safety
    ...
    def get_call_statistics(self) -> Dict[str, Any]:
        ...
        avg_duration = sum(result.total_duration for result in self.call_history) / total_calls
        avg_attempts = sum(result.total_attempts for result in self.call_history) / total_calls
        ...
        return {
            ...
            'retry_strategies_used': list(set(
