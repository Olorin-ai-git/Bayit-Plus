...
class ExecutionContext:
    ...
    def can_enter_node(self, node_name: str) -> bool:
        ...
        if self.current_depth >= self.max_depth:
            logger.warning(f"Max depth {self.max_depth} reached for investigation {self.investigation_id}")
            return False
            
        # Check duration limit
        if time.time() - self.start_time > self.max_duration_seconds:
            logger.warning(f"Max duration {self.max_duration_seconds}s exceeded for investigation {self.investigation_id}")
            return False
            
        # Check for immediate loops (same node consecutively)
        if len(self.node_stack) >= 2 and self.node_stack[-1] == node_name and self.node_stack[-2] == node_name:
            logger.warning(f"Immediate loop detected for node {node_name} in investigation {self.investigation_id}")
            return False
    ...
    def can_call_tool(self, tool_name: str) -> bool:
        ...
        if len(self.tool_calls) >= self.max_tool_calls:
            logger.warning(f"Max tool calls {self.max_tool_calls} reached for investigation {self.investigation_id}")
            return False
        ...
        if len(same_tool_calls) >= 5:
            logger.warning(f"Too many calls to {tool_name} in investigation {self.investigation_id}")
            return False
    ...
    def enter_node(self, node_name: str) -> None:
        ...
        logger.debug(f"Entered node {node_name}, depth: {self.current_depth}, investigation: {self.investigation_id}")
    ...
    def exit_node(self, node_name: str) -> None:
        ...
        if self.node_stack and self.node_stack[-1] == node_name:
            self.node_stack.pop()
            self.current_depth = len(self.node_stack)
            logger.debug(f"Exited node {node_name}, depth: {self.current_depth}, investigation: {self.investigation_id}")
        else:
            logger.warning(f"Node stack mismatch: expected {node_name}, got {self.node_stack[-1] if self.node_stack else 'empty'}")
    
    def record_tool_call(self, tool_name: str) -> None:
        """Record a tool call"""
        self.tool_calls.append(tool_name)
        logger.debug(f"Tool call {tool_name}, total: {len(self.tool_calls)}, investigation: {self.investigation_id}")
    ...
    def get_execution_summary(self) -> Dict[str, any]:
        ...
        return {
            ...
            "max_depth_reached": max(len(self.node_stack), self.current_depth) if self.node_stack else 0,
...
class RecursionGuard:
    ...
    def create_context(
        ...
        with self._lock:
            ...
            if len(self._contexts) >= self._max_concurrent_investigations:
                logger.error(f"Max concurrent investigations {self._max_concurrent_investigations} reached")
            ...
            if context_key in self._contexts:
                logger.warning(f"Context already exists for {context_key}, returning existing")
            ...
            logger.info(f"Created execution context for investigation {investigation_id}, thread {thread_id}")
            return context
    
    def get_context(self, investigation_id: str, thread_id: str) -> Optional[ExecutionContext]:
    ...
    def can_call_tool(self, investigation_id: str, thread_id: str, tool_name: str) -> bool:
    ...
    def record_tool_call(self, investigation_id: str, thread_id: str, tool_name: str) -> bool:
    ...
    def get_system_stats(self) -> Dict[str, any]:
        ...
        with self._lock:
            ...
            total_tool_calls = sum(len(ctx.tool_calls) for ctx in self._contexts.values())
            
            return {
                "active_investigations": active_contexts,
                "total_investigations_processed": total_investigations,
                "average_depth": total_depth / active_contexts if active_contexts > 0 else 0,
    ...
    def cleanup_stale_contexts(self, max_age_seconds: int = 3600) -> int:
        ...
        with self._lock:
            ...
            for key in stale_keys:
                ...
                logger.warning(f"Removed stale context {key}, age: {current_time - context.start_time}s")
                removed_count += 1
