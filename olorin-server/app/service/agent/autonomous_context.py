...
class StructuredInvestigationContext:
    ...
    def update_phase(self, phase: InvestigationPhase) -> None:
        ...
        self.updated_at = datetime.now()
        logger.info(
            f"Investigation {self.investigation_id} phase updated to {phase.value}"
        )
    ...
    def add_objective(self, objective: InvestigationObjective) -> None:
        ...
        self.updated_at = datetime.now()
        logger.debug(
            f"Added objective {objective.objective_id} to investigation {self.investigation_id}"
        )
    ...
    def record_domain_findings(self, domain: str, findings: DomainFindings) -> None:
        ...
        logger.info(
            f"Recorded findings for domain {domain} in investigation {self.investigation_id}"
        )
    ...
    def _update_overall_risk(self) -> None:
        ...
        for domain, findings in self.progress.domain_findings.items():
            ...
            if findings.risk_score is not None:
                ...
                logger.warning(
                    f"Domain {domain} has no risk_score - excluding from overall calculation"
                )
        ...
        if total_weight > 0:
            ...
            self.progress.overall_risk_score = (
                0.0  # Keep as 0.0 since it's an overall metric
            )
                ...
                if isinstance(source_data, dict):
                    ...
                    if "entity_id" in source_data:
                        important_fields.append(
                            f"Entity ID: {source_data['entity_id']}"
                        )
...
"""
        ...
        for obj in sorted(self.objectives, key=lambda x: x.priority, reverse=True):
            status = (
                "✓ COMPLETED"
                if obj.objective_id in self.progress.completed_domains
                else "○ PENDING"
            )
            context_parts.append(
                f"""
    ...
    def get_ip_addresses(self) -> List[str]:
        ...
        for source_name, source_data in self.data_sources.items():
            if isinstance(source_data, dict):
                ...
                if "network" in source_data and isinstance(
                    source_data["network"], dict
                ):
                    network_data = source_data["network"]
                    for field in ip_fields:
                        if field in network_data:
                            ip_value = network_data[field]
                            if isinstance(ip_value, str) and self._is_valid_ip(
                                ip_value
                            ):
                                ip_addresses.add(ip_value)
                            elif isinstance(ip_value, list):
        ...
        if ip_addresses:
            logger.info(
                f"Extracted {len(ip_addresses)} IP addresses from context: {list(ip_addresses)[:5]}..."
            )
        else:
            logger.warning(
                f"No IP addresses found in context for entity {self.entity_id}"
            )
            # Add a test IP for development/testing purposes if needed
            # This should be removed in production
            if self.data_sources:
                logger.info(
                    "Data sources available but no IPs found - may need to check data structure"
                )
