...
class MCPCircuitBreaker:
    ...
    def __init__(self, server_name: str, failure_threshold: int = 3, recovery_timeout: float = 60.0):
    ...
    def can_execute(self) -> bool:
        ...
        if self.state['state'] == 'closed':
            ...
            if current_time - self.state['last_failure_time'] > self.state['recovery_timeout']:
                ...
                logger.info(f"Circuit breaker for {self.server_name} moved to half-open state")
                return True
            return False
    ...
    def record_failure(self) -> None:
        ...
        if self.state['failure_count'] >= self.state['failure_threshold']:
            ...
            logger.warning(f"Circuit breaker for {self.server_name} opened after {self.state['failure_count']} failures")
