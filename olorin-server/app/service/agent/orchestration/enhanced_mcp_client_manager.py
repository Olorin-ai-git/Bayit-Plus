...
class EnhancedMCPClientManager:
    ...
    async def register_server(
        self, server_info: MCPServerInfo, initialize_resilience: bool = True
    ) -> None:
        ...
        logger.info(
            f"Registered MCP server: {server_info.name} ({server_info.category.value})"
        )
        ...
        try:
            ...
            if self.routing_policy.fallback_enabled:
                ...
                if fallback_server:
                    ...
                    logger.warning(
                        f"Falling back from {server_name} to {fallback_server}"
                    )
    ...
    async def _select_server(
        ...
        for server_name in self.category_mapping[category]:
            ...
            if (
                ...
                and server.status != ConnectionStatus.UNAVAILABLE
                and not server.circuit_breaker_open
            ):
        ...
        if strategy == "capability_based":
            ...
            return candidate_servers[
                self.metrics["total_requests"] % len(candidate_servers)
            ].name
    ...
    async def _execute_with_resilience(
        self, server_name: str, payload: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        async def _make_request() -> Dict[str, Any]:
            ...
            try:
                response = await client.post(f"{server.endpoint}/execute", json=payload)
    ...
    async def _select_fallback_server(
        self, category: MCPServerCategory, failed_server: str
    ) -> Optional[str]:
        """Select a fallback server when primary fails."""
        fallback_candidates = [
            name
            for name in self.category_mapping[category]
            if (
                name != failed_server
                and self.servers[name].status != ConnectionStatus.UNAVAILABLE
                and not self.servers[name].circuit_breaker_open
            )
        ]
        ...
        if fallback_candidates:
            ...
            return max(
                fallback_candidates, key=lambda name: self.servers[name].success_rate
            )
