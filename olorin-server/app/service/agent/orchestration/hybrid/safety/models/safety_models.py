...
class SafetyLimits:
    ...
    def __post_init__(self):
        ...
        if self.max_orchestrator_loops <= 0:
            logger.warning(f"Invalid orchestrator loops limit: {self.max_orchestrator_loops}")
        ...
        if not 0.0 <= self.confidence_threshold_for_override <= 1.0:
            logger.warning(f"Invalid confidence threshold: {self.confidence_threshold_for_override}")
        if not 0.0 <= self.resource_pressure_threshold <= 1.0:
            logger.warning(f"Invalid resource pressure threshold: {self.resource_pressure_threshold}")
...
class SafetyStatus:
    ...
    def __post_init__(self):
        ...
        if self.requires_immediate_termination and self.allows_ai_control:
            logger.warning("Inconsistent safety status: termination required but AI control allowed")
        ...
        if self.safety_level == SafetyLevel.EMERGENCY:
            if not self.requires_immediate_termination and self.allows_ai_control:
                logger.warning("Emergency safety level but no termination required and AI control allowed")
    ...
    def is_safe_to_continue(self) -> bool:
        ...
        return (
            not self.requires_immediate_termination and
            self.safety_level != SafetyLevel.EMERGENCY and
            self.resource_pressure < self.current_limits.resource_pressure_threshold
        )
    ...
    def get_primary_concern(self) -> str:
        ...
        if self.has_critical_concerns:
            critical_concerns = [c for c in self.safety_concerns if "critical" in c.lower()]
            return critical_concerns[0] if critical_concerns else "Unknown critical concern"
