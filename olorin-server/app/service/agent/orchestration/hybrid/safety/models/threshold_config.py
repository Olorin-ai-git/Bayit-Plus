...
class SafetyThresholdConfig:
    ...
    def get_safety_multipliers_for_level(
        self, safety_level: SafetyLevel
    ) -> Dict[str, float]:
        """Get multipliers for given safety level"""
        return self.safety_multipliers.get(
            safety_level, self.safety_multipliers[SafetyLevel.STANDARD]
        )

    def get_strategy_adjustments(
        self, strategy: InvestigationStrategy
    ) -> Dict[str, float]:
        """Get adjustments for given investigation strategy"""
        return self.strategy_adjustments.get(
            strategy, self.strategy_adjustments[InvestigationStrategy.ADAPTIVE]
        )
    ...
    def validate_configuration(self) -> bool:
        ...
        try:
            ...
            for level, multipliers in self.safety_multipliers.items():
                if not self._validate_multipliers(
                    multipliers, f"safety_level_{level.value}"
                ):
                    return False
            ...
            for strategy, adjustments in self.strategy_adjustments.items():
                if not self._validate_multipliers(
                    adjustments, f"strategy_{strategy.value}"
                ):
                    return False
    ...
    def _validate_base_limits(self, limits: Dict[str, Any], mode: str) -> bool:
        ...
        required_keys = [
            ...
            "max_tool_executions",
            "max_domain_attempts",
            "max_investigation_time_minutes",
    ...
    def _validate_multipliers(
        self, multipliers: Dict[str, float], context: str
    ) -> bool:
