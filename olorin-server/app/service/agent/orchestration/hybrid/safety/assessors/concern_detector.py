...
class ConcernDetector:
    ...
    def identify_safety_concerns(
        ...
        concerns.extend(self._check_resource_pressure_concerns(resource_pressure, limits))
    ...
    def _check_loop_concerns(
        ...
        if orchestrator_loops >= limits.max_orchestrator_loops * self.loop_warning_threshold:
            severity = self.threshold_manager.get_loop_concern_severity(orchestrator_loops, limits.max_orchestrator_loops)
    ...
    def _check_resource_pressure_concerns(
        ...
        if resource_pressure >= limits.resource_pressure_threshold:
            severity = self.threshold_manager.get_resource_pressure_severity(resource_pressure)
            concerns.append(SafetyConcern(
            ...
            concerns.append(SafetyConcern(
                ...
                metrics={"pressure": resource_pressure, "threshold": self.threshold_manager.config.high_pressure_threshold},
                recommended_action="Monitor resource usage closely"
            ))
        
        return concerns
    
    def _check_confidence_concerns(self, state: 'HybridInvestigationState') -> List[SafetyConcern]:
    ...
    def _check_evidence_concerns(self, state: 'HybridInvestigationState') -> List[SafetyConcern]:
        ...
        if self.evidence_validator.should_trigger_safety_concerns(evidence_quality, orchestrator_loops):
            concerns.append(SafetyConcern(
    ...
    def _check_timeout_concerns(
        ...
        try:
            ...
            if elapsed_minutes >= limits.max_investigation_time_minutes * self.time_warning_threshold:
