...
from ..models import SafetyLimits, SafetyConcern
...
class TerminationChecker:
    ...
    def requires_immediate_termination(
        ...
        if self.enable_critical_concern_termination and self._has_critical_concerns(concerns):
            return True
        
        # Hard limits exceeded
        if self.enable_hard_limit_termination and self._hard_limits_exceeded(state, limits):
            return True
        
        # Time limit exceeded
        if self.enable_time_limit_termination and self._time_limit_exceeded(state, limits):
            return True
    ...
    def _hard_limits_exceeded(self, state: 'HybridInvestigationState', limits: SafetyLimits) -> bool:
        ...
        if orchestrator_loops >= loop_limit:
            logger.warning(f"   🚨 Immediate termination: orchestrator loops exceeded {loop_limit}")
            return True
        ...
        if tools_used >= tool_limit:
            logger.warning(f"   🚨 Immediate termination: tool executions exceeded {tool_limit}")
            return True
        ...
        if domains_completed >= limits.max_domain_attempts:
            logger.warning(f"   🚨 Immediate termination: domain attempts exceeded {limits.max_domain_attempts}")
            return True
        
        return False
    
    def _time_limit_exceeded(self, state: 'HybridInvestigationState', limits: SafetyLimits) -> bool:
        ...
        try:
            ...
            time_limit = limits.max_investigation_time_minutes - self.time_termination_margin
            
            if elapsed_minutes >= time_limit:
                logger.warning(f"   🚨 Immediate termination: time limit exceeded {time_limit} minutes")
                return True
        except Exception:
    ...
    def get_termination_reason(
        ...
        if self.enable_critical_concern_termination:
            ...
            if critical_concerns:
                ...
                reasons.append(f"Critical safety concerns: {'; '.join(concern_messages)}")
        
        # Check hard limits
        if self.enable_hard_limit_termination:
            orchestrator_loops = state.get("orchestrator_loops", 0)
            if orchestrator_loops >= limits.max_orchestrator_loops:
                reasons.append(f"Orchestrator loop limit exceeded: {orchestrator_loops}/{limits.max_orchestrator_loops}")
            
            tools_used = len(state.get("tools_used", []))
            if tools_used >= limits.max_tool_executions:
                reasons.append(f"Tool execution limit exceeded: {tools_used}/{limits.max_tool_executions}")
            
            domains_completed = len(state.get("domains_completed", []))
            if domains_completed >= limits.max_domain_attempts:
                reasons.append(f"Domain attempt limit exceeded: {domains_completed}/{limits.max_domain_attempts}")
            ...
            if start_time:
                try:
                    ...
                    if elapsed_minutes >= limits.max_investigation_time_minutes:
                        reasons.append(f"Time limit exceeded: {elapsed_minutes:.1f}/{limits.max_investigation_time_minutes} minutes")
                except Exception:
                    pass
    ...
    def should_gracefully_finalize(
        ...
        if orchestrator_loops >= loop_threshold:
            reasons.append(f"Orchestrator loops approaching limit: {orchestrator_loops}/{limits.max_orchestrator_loops}")
        
        # Check tool executions at 80% threshold
        tools_used = len(state.get("tools_used", []))
        tool_threshold = int(limits.max_tool_executions * 0.8)
        if tools_used >= tool_threshold:
            reasons.append(f"Tool executions approaching limit: {tools_used}/{limits.max_tool_executions}")
        ...
        if start_time:
            try:
                ...
                if elapsed_minutes >= time_threshold:
                    reasons.append(f"Time approaching limit: {elapsed_minutes:.1f}/{limits.max_investigation_time_minutes} minutes")
            except Exception:
                pass
        ...
        return should_finalize, "; ".join(reasons) if reasons else "No finalization needed"
    
    def _has_adequate_domain_coverage(self, state: 'HybridInvestigationState', limits: SafetyLimits) -> bool:
        """
        ...
        """
        domains_completed = len(state.get("domains_completed", []))
        min_domains_for_finalization = max(1, int(limits.max_domain_attempts * 0.5))  # At least 50% coverage
        
        has_coverage = domains_completed >= min_domains_for_finalization
        if not has_coverage:
            logger.info(f"   📊 Insufficient domain coverage: {domains_completed}/{min_domains_for_finalization} minimum")
    ...
    def get_termination_urgency(
        ...
        if (orchestrator_loops >= limits.max_orchestrator_loops or
            tools_used >= limits.max_tool_executions):
            return "urgent"
        
        # Moderate urgency for other termination conditions
        return "moderate"
    
    def configure_termination_settings(self,
        ...
        if enable_hard_limits is not None:
            ...
            logger.info(f"Hard limit termination: {'enabled' if enable_hard_limits else 'disabled'}")
        
        if enable_time_limits is not None:
            self.enable_time_limit_termination = enable_time_limits
            logger.info(f"Time limit termination: {'enabled' if enable_time_limits else 'disabled'}")
        
        if enable_critical_concerns is not None:
            self.enable_critical_concern_termination = enable_critical_concerns
            logger.info(f"Critical concern termination: {'enabled' if enable_critical_concerns else 'disabled'}")
