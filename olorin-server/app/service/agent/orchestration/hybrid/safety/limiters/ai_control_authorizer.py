...
class AIControlAuthorizer:
    ...
    def should_allow_ai_control(
        ...
        if auth_decision is not None:
            ...
            logger.debug(f"   {'✅' if auth_decision else '🚫'} AI control {reason}: "
    ...
    def _check_confidence_authorization(
        ...
        if confidence_level == AIConfidenceLevel.HIGH:
            if resource_pressure < self.confidence_thresholds["high_confidence_pressure_limit"]:
                return True
        
        # Medium confidence with reasonable resource pressure
        if confidence_level == AIConfidenceLevel.MEDIUM:
            if resource_pressure < self.confidence_thresholds["medium_confidence_pressure_limit"]:
                return True
        
        # UNKNOWN confidence during warmup period (first few loops)
        if confidence_level == AIConfidenceLevel.UNKNOWN:
            if (orchestrator_loops < self.warmup_loop_threshold and 
                resource_pressure < self.confidence_thresholds["unknown_warmup_pressure_limit"]):
                return True
    ...
    def get_authorization_reasoning(
        ...
        if confidence_level == AIConfidenceLevel.HIGH:
            pressure_limit = self.confidence_thresholds["high_confidence_pressure_limit"]
        ...
        if confidence_level == AIConfidenceLevel.MEDIUM:
            pressure_limit = self.confidence_thresholds["medium_confidence_pressure_limit"]
            if resource_pressure < pressure_limit:
        ...
        if confidence_level == AIConfidenceLevel.UNKNOWN:
            if orchestrator_loops < self.warmup_loop_threshold:
                pressure_limit = self.confidence_thresholds["unknown_warmup_pressure_limit"]
                if resource_pressure < pressure_limit:
    ...
    def update_thresholds(self,
        ...
        """
        if high_confidence_pressure is not None and 0.0 <= high_confidence_pressure <= 1.0:
            self.confidence_thresholds["high_confidence_pressure_limit"] = high_confidence_pressure
            logger.info(f"Updated high confidence pressure limit to {high_confidence_pressure}")
        
        if medium_confidence_pressure is not None and 0.0 <= medium_confidence_pressure <= 1.0:
            self.confidence_thresholds["medium_confidence_pressure_limit"] = medium_confidence_pressure
            logger.info(f"Updated medium confidence pressure limit to {medium_confidence_pressure}")
        
        if unknown_warmup_pressure is not None and 0.0 <= unknown_warmup_pressure <= 1.0:
            self.confidence_thresholds["unknown_warmup_pressure_limit"] = unknown_warmup_pressure
            logger.info(f"Updated unknown warmup pressure limit to {unknown_warmup_pressure}")
    ...
    def configure_authorization(self,
        ...
        if deny_on_critical is not None:
            ...
            logger.info(f"Deny on critical concerns: {'enabled' if deny_on_critical else 'disabled'}")
        
        if consider_age is not None:
            self.consider_concern_age = consider_age
            logger.info(f"Consider concern age: {'enabled' if consider_age else 'disabled'}")
        
        if track_decisions is not None:
            self.track_authorization_decisions = track_decisions
            logger.info(f"Track authorization decisions: {'enabled' if track_decisions else 'disabled'}")
