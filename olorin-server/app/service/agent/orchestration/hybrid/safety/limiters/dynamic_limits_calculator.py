...
class DynamicLimitsCalculator:
    ...
    def calculate_dynamic_limits(
        ...
        safety_multipliers = self.threshold_config.get_safety_multipliers_for_level(
            safety_level
        )
        ...
        limits = SafetyLimits(
            ...
            confidence_threshold_for_override=base_limits[
                "confidence_threshold_for_override"
            ],
            resource_pressure_threshold=base_limits["resource_pressure_threshold"],
        )

        self._log_limits_calculation(
            limits, safety_level, strategy, is_test_mode, context_adjustments
        )
    ...
    def _calculate_confidence_adjustments(
        ...
        if all(conf > 0.8 for conf in recent_confidences):
            logger.debug(
                f"   📈 High confidence bonus applied: {self.confidence_scaling['high_confidence_bonus']}"
            )
        ...
        if all(conf < 0.4 for conf in recent_confidences):
            logger.debug(
                f"   📉 Low confidence penalty applied: {self.confidence_scaling['low_confidence_penalty']}"
            )
    ...
    def _calculate_performance_adjustments(
        ...
        if domains_per_loop > 0.5:
            logger.debug(
                f"   🚀 Fast progress bonus applied: {self.performance_adjustments['fast_completion_bonus']}"
            )
        ...
        if orchestrator_loops >= 5 and domains_per_loop < 0.2:
            logger.debug(
                f"   🐌 Slow progress penalty applied: {self.performance_adjustments['slow_progress_penalty']}"
            )
            return self.performance_adjustments["slow_progress_penalty"]

        return 1.0

    def _log_limits_calculation(
        self,
        limits: SafetyLimits,
        ...
        non_neutral_adjustments = {
            k: v for k, v in context_adjustments.items() if abs(v - 1.0) > 0.01
        }
    ...
    def get_limits_explanation(
        ...
        if significant_adjustments:
            explanation_parts.append(
                f"Adjustments: {', '.join(significant_adjustments)}"
            )
    ...
    def update_scaling_factors(
        ...
        if fast_completion_bonus is not None:
            self.performance_adjustments["fast_completion_bonus"] = (
        ...
        if slow_progress_penalty is not None:
            self.performance_adjustments["slow_progress_penalty"] = (
                slow_progress_penalty
            )
