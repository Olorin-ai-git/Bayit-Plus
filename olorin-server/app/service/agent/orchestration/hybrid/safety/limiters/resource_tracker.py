...
class ResourceTracker:
    ...
    def calculate_remaining_resources(
        ...
        logger.debug(f"      Loops: {remaining['orchestrator_loops']}/{limits.max_orchestrator_loops}")
        logger.debug(f"      Tools: {remaining['tool_executions']}/{limits.max_tool_executions}")
        logger.debug(f"      Domains: {remaining['domain_attempts']}/{limits.max_domain_attempts}")
        logger.debug(f"      Time: {remaining['time_minutes']} minutes")
        
        return remaining
    
    def _calculate_remaining_time(self, state: 'HybridInvestigationState', limits: SafetyLimits) -> int:
        ...
        try:
            ...
            remaining_minutes = max(0, int(limits.max_investigation_time_minutes - elapsed_minutes))
    ...
    def get_resource_utilization(
        ...
        utilization = {
            "orchestrator_loops": orchestrator_loops / max(1, limits.max_orchestrator_loops),
    ...
    def _calculate_time_utilization(self, state: 'HybridInvestigationState', limits: SafetyLimits) -> float:
    ...
    def get_consumption_rates(self, state: 'HybridInvestigationState') -> Dict[str, float]:
        """
        ...
        if orchestrator_loops == 0:
            return {"tools_per_loop": 0.0, "domains_per_loop": 0.0, "minutes_per_loop": 0.0}
        ...
        rates = {
            ...
            "minutes_per_loop": self._calculate_time_per_loop(state, orchestrator_loops)
        }
    ...
    def _calculate_time_per_loop(self, state: 'HybridInvestigationState', orchestrator_loops: int) -> float:
    ...
    def get_resource_projections(
        ...
        if rates["tools_per_loop"] > 0:
            loops_until_tool_exhaustion = remaining["tool_executions"] / rates["tools_per_loop"]
        ...
        if rates["minutes_per_loop"] > 0:
            loops_until_time_exhaustion = remaining["time_minutes"] / rates["minutes_per_loop"]
    ...
    def _calculate_projection_confidence(self, state: 'HybridInvestigationState') -> float:
    ...
    def get_resource_efficiency(self, state: 'HybridInvestigationState') -> Dict[str, float]:
        """
        ...
        if orchestrator_loops == 0:
            return {"domain_efficiency": 0.0, "tool_efficiency": 0.0, "overall_efficiency": 0.0}
    ...
    def configure_tracking(self,
        ...
        if track_rates is not None:
            ...
            logger.info(f"Consumption rate tracking: {'enabled' if track_rates else 'disabled'}")
        
        if provide_projections is not None:
            self.provide_projections = provide_projections
            logger.info(f"Resource projections: {'enabled' if provide_projections else 'disabled'}")
