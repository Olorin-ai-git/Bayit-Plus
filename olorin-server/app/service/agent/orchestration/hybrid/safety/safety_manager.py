...
class SafetyManager:
    ...
    def validate_current_state(self, state: 'HybridInvestigationState') -> SafetyStatus:
        ...
        current_limits = self.dynamic_limits_calculator.calculate_dynamic_limits(state, safety_level)
        
        # Step 3: Assess resource pressure
        resource_pressure = self.resource_pressure_calculator.calculate_resource_pressure(state, current_limits)
        
        # Step 4: Check for safety concerns
        safety_concerns = self.concern_detector.identify_safety_concerns(state, current_limits, resource_pressure)
        
        # Step 5: Determine if AI control is allowed
        allows_ai_control = self.ai_control_authorizer.should_allow_ai_control(state, safety_concerns, resource_pressure)
        
        # Step 6: Check for immediate termination needs
        logger.debug(f"   🔄 STEP 6: Checking immediate termination requirements...")
        requires_termination = self.termination_checker.requires_immediate_termination(state, safety_concerns, current_limits)
        ...
        remaining_resources = self.resource_tracker.calculate_remaining_resources(state, current_limits)
        ...
        recommended_actions = self.action_recommender.generate_recommended_actions(state, safety_concerns, resource_pressure)
        ...
        logger.debug(f"   🕰️ PERFORMANCE: Validation completed in {validation_time:.1f}ms")
    ...
    def _log_validation_results(self, safety_status: SafetyStatus, safety_concerns, validation_time: float):
        ...
        logger.info(f"   Termination required: {safety_status.requires_immediate_termination}")
        ...
        if validation_time > 20:  # 20ms threshold
            logger.warning(f"   ⏱️ Safety validation slower than expected: {validation_time:.1f}ms")
    ...
    def get_detailed_safety_report(self, state: 'HybridInvestigationState') -> dict:
        ...
        pressure_breakdown = self.resource_pressure_calculator.get_pressure_breakdown(state, current_limits)
        resource_utilization = self.resource_tracker.get_resource_utilization(state, current_limits)
        ...
        report = {
            ...
            "safety_concerns": [
                ...
                for concern in self.concern_detector.identify_safety_concerns(state, current_limits, resource_pressure)
            ],
            "recommendations": {
                "override_reasoning": safety_status.override_reasoning,
                "recommended_actions": safety_status.recommended_actions,
                "action_categories": self.action_recommender.get_action_categories(safety_status.recommended_actions)
            },
            "component_info": {
                "validation_count": self.validation_count,
                "last_validation_time_ms": self.last_validation_time,
                "safety_level_reasoning": self.safety_level_detector.get_safety_level_reasoning(state),
                "limits_explanation": self.dynamic_limits_calculator.get_limits_explanation(state, safety_level),
                "authorization_reasoning": self.ai_control_authorizer.get_authorization_reasoning(state, [], resource_pressure),
                "resource_summary": self.resource_tracker.get_resource_summary(state, current_limits)
            }
        }
