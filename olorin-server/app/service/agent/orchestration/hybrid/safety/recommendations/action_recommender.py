...
class ActionRecommender:
    ...
    def generate_recommended_actions(
        ...
        prioritized_actions = self._prioritize_actions(action_list, concerns, resource_pressure)
    ...
    def _calculate_action_score(
        ...
        elif any(word in action.lower() for word in ["monitor", "validate", "implement"]):
            base_score = 30
        ...
        critical_concerns = [c for c in concerns if c.severity == "critical"]
        if critical_concerns and any(word in action.lower() for word in ["completion", "termination"]):
    ...
    def get_action_categories(self, actions: List[str]) -> dict:
        ...
        for action in actions:
            ...
            if any(word in action_lower for word in ["immediate", "emergency", "force"]):
                categories["emergency"].append(action)
            elif any(word in action_lower for word in ["resource", "consumption", "minimal"]):
            ...
            elif any(word in action_lower for word in ["safety", "validation", "checks"]):
                categories["safety_measures"].append(action)
            elif any(word in action_lower for word in ["optimize", "efficiency", "performance"]):
    ...
    def add_concern_actions(self, concern_type: SafetyConcernType, actions: List[str]):
        ...
        logger.info(f"Added {len(actions)} actions for concern type {concern_type.value}")
