...
class OverrideReasoner:
    ...
    def _build_ai_control_reasoning(
        ...
        if critical_concerns:
            reasoning.append(f"Critical safety violations detected: {len(critical_concerns)} issues")
        elif high_concerns:
            reasoning.append(f"High-priority safety concerns present: {len(high_concerns)} issues")
        
        # Add resource pressure reasoning if provided
        if resource_pressure is not None and resource_pressure > 0.7:
            if self.include_metrics:
                reasoning.append(f"Excessive resource pressure: {resource_pressure:.2f}")
            else:
    ...
    def _build_concern_reasoning(self, concerns: List[SafetyConcern]) -> List[str]:
        ...
        for severity in ["critical", "high", "medium", "low"]:
            if severity in concern_groups:
                ...
                reasoning.extend(self._build_severity_reasoning(severity, severity_concerns))
    ...
    def _build_severity_reasoning(self, severity: str, concerns: List[SafetyConcern]) -> List[str]:
        """Build reasoning for concerns of a specific severity"""
        reasoning = []
        
        if severity == "critical":
            reasoning.append(f"Critical concerns requiring immediate action: {len(concerns)}")
        else:
            reasoning.append(f"Active safety concerns: {len(concerns)} {severity} priority")
        ...
        for concern in concerns[:max_detail_concerns]:
            if self.include_metrics and concern.metrics:
                ...
                if key_metric:
                    reasoning.append(f"{severity.capitalize()} concern: {concern.message} ({key_metric})")
                else:
    ...
    def _extract_key_metric(self, concern: SafetyConcern) -> str:
        ...
        metric_keys = ["current_loops", "pressure", "elapsed_minutes", "evidence_quality", "current", "limit"]
    ...
    def _build_state_reasoning(self, state: 'HybridInvestigationState') -> List[str]:
        ...
        if orchestrator_loops > 15:
            ...
            if self.include_metrics:
                reasoning.append(f"Elevated orchestrator loop count: {orchestrator_loops}")
            else:
    ...
    def _calculate_reasoning_priority(self, reason: str, concerns: List[SafetyConcern]) -> int:
        ...
        if any(word in reason_lower for word in ["critical", "emergency", "immediate"]):
            ...
            base_score = 20
        elif any(word in reason_lower for word in ["ai control denied", "safety violations"]):
            base_score = 15
    ...
    def configure_reasoning(self,
        ...
        if include_metrics is not None:
            ...
            logger.info(f"Include metrics in reasoning: {'enabled' if include_metrics else 'disabled'}")
        
        if include_thresholds is not None:
            self.include_thresholds = include_thresholds
            logger.info(f"Include thresholds in reasoning: {'enabled' if include_thresholds else 'disabled'}")
