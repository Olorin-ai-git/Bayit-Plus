...
class CanonicalOutcomeBuilder:
    ...
    def build_outcome(
        ...
        """
        logger.debug(
            f"Building canonical outcome for investigation {state.get('investigation_id')}"
        )
        ...
        outcome = CanonicalFinalOutcome(
            ...
            in [
                ...
                InvestigationStatus.COMPLETED_WITH_WARNINGS,
            ],
            ...
            graph_selection_reason=state.get(
                "graph_selection_reason", "Hybrid intelligence selected"
            ),
        ...
        logger.info(
            f"✅ Canonical outcome built: {status.value} for {state.get('investigation_id')}"
        )
        return outcome

    def _determine_investigation_status(
        self, state: HybridInvestigationState, completion_reason: str
    ) -> InvestigationStatus:
    ...
    def _build_evidence_assessment(
        ...
        strategy = state.get(
            "investigation_strategy", InvestigationStrategy.ADAPTIVE
        ).value

        validation_passed, validation_reason, _ = (
            self.evidence_validator.validate_evidence_for_completion(
        ...
        return EvidenceAssessment(
            ...
            quality_level=self.evidence_validator.get_evidence_quality_level(
                evidence_quality
            ),
    ...
    def _build_performance_metrics(
        ...
        return PerformanceMetrics(
            ...
            investigation_efficiency=coerce_float(
                state.get("investigation_efficiency"), 0.0
            ),
    ...
    def _build_ai_intelligence_metrics(
        self, state: HybridInvestigationState
    ...
    def _generate_recommendations(
        self, state: HybridInvestigationState, risk_assessment: RiskAssessment
    ) -> List[str]:
        """Generate actionable recommendations based on investigation results."""
        recommendations = []

        # Risk-based recommendations
        if risk_assessment.fraud_likelihood in [
            FraudLikelihood.VERY_HIGH,
        ...
        if evidence_quality < 0.6:
            recommendations.append(
                "Consider gathering additional evidence before final decision"
            )
