...
class AdvancedSafetyManager:
    ...
    def validate_current_state(self, state: HybridInvestigationState) -> SafetyStatus:
        ...
        allows_ai_control = self._should_allow_ai_control(
            state, safety_concerns, resource_pressure
        )

        # Check for immediate termination
        requires_termination = self._requires_immediate_termination(
            state, safety_concerns, current_limits
        )

        # Build override reasoning
        override_reasoning = self._build_override_reasoning(
            state, safety_concerns, allows_ai_control
        )
    ...
    def _calculate_dynamic_limits(
        self, state: HybridInvestigationState, safety_level: SafetyLevel
    ) -> SafetyLimits:
        ...
        strategy_adj = self.strategy_adjustments.get(
            strategy, {"loops": 1.0, "tools": 1.0, "domains": 1.0, "time": 1.0}
        )

        # Calculate final limits
        limits = SafetyLimits(
            max_orchestrator_loops=int(
                base["max_orchestrator_loops"]
                * multipliers["loops"]
                * strategy_adj["loops"]
            ),
            max_tool_executions=int(
                base["max_tool_executions"]
                * multipliers["tools"]
                * strategy_adj["tools"]
            ),
            max_domain_attempts=int(
                base["max_domain_attempts"]
                * multipliers["domains"]
                * strategy_adj["domains"]
            ),
            max_investigation_time_minutes=int(
                base["max_investigation_time_minutes"]
                * multipliers["time"]
                * strategy_adj["time"]
            ),
        ...
        logger.debug(
            f"      Time limit: {limits.max_investigation_time_minutes} minutes"
        )
    ...
    def _calculate_resource_pressure(
        ...
        if orchestrator_loops < 3:
            logger.debug(
                f"   🔄 Warmup period: {orchestrator_loops}/3 loops - pressure limited to 0.0"
            )
            return 0.0
        ...
        total_tool_executions = max(
            tools_used, tool_execution_attempts
        ...
        def calculate_progressive_pressure(
            current: int, limit: int, early_threshold: float = 0.7
        ) -> float:
        ...
        loop_pressure = calculate_progressive_pressure(
            orchestrator_loops, limits.max_orchestrator_loops
        )
        tool_pressure = calculate_progressive_pressure(
            total_tool_executions, limits.max_tool_executions
        )
        domain_pressure = calculate_progressive_pressure(
            domains_completed, limits.max_domain_attempts
        )
        ...
        if start_time:
            try:
                ...
                )  # More realistic 30 min default
                time_pressure = calculate_progressive_pressure(
                    int(elapsed_minutes), time_budget_minutes
                )
        ...
        logger.debug(
            f"   📈 FIXED Hybrid Intelligence resource pressure: {overall_pressure:.3f}"
        )
        logger.debug(
            f"      Loops: {loop_pressure:.3f} ({orchestrator_loops}/{limits.max_orchestrator_loops}) [70% threshold: {limits.max_orchestrator_loops * 0.7:.0f}]"
        )
        logger.debug(
            f"      Tools: {tool_pressure:.3f} ({total_tool_executions}/{limits.max_tool_executions}) [unique: {tools_used}, attempts: {tool_execution_attempts}]"
        )
        logger.debug(
            f"      Domains: {domain_pressure:.3f} ({domains_completed}/{limits.max_domain_attempts})"
        )
        logger.debug(f"      Time: {time_pressure:.3f}")
        logger.debug(
            f"   Progressive pressure scaling: gentle increase until 70% capacity, then sharper curve"
        )
    ...
    def _identify_safety_concerns(
        ...
        if state.get("ai_decisions"):
            ...
            evidence_quality = (
                getattr(ai_decision, "evidence_quality", 0.0) if ai_decision else 0.0
            )
            evidence_quality = evidence_quality if evidence_quality is not None else 0.0

        evidence_validator = get_evidence_validator()
        if evidence_validator.should_trigger_safety_concerns(
    ...
    def _should_allow_ai_control(
        ...
        if critical_concerns:
            logger.debug(
                f"   🚫 AI control denied: {len(critical_concerns)} critical concerns"
            )
            return False
        ...
        if resource_pressure < MIN_PRESSURE_THRESHOLD:
            logger.debug(
                f"   ✅ AI control allowed: resource pressure {resource_pressure:.3f} below threshold {MIN_PRESSURE_THRESHOLD}"
            )
            return True
        ...
        if confidence_level == AIConfidenceLevel.MEDIUM and resource_pressure < 0.8:
            logger.debug(
                f"   ✅ AI control allowed: medium confidence, moderate pressure"
            )
            return True
        ...
        if confidence_level == AIConfidenceLevel.UNKNOWN and resource_pressure < 0.5:
            logger.debug(
                f"   ✅ AI control allowed: unknown confidence, acceptable pressure (loop {orchestrator_loops})"
            )
            return True

        # Low confidence, high pressure -> Deny AI control
        logger.debug(
            f"   🚫 AI control denied: confidence={confidence_level.value}, pressure={resource_pressure:.3f}, loop={orchestrator_loops}"
        )
        return False
    ...
    def _requires_immediate_termination(
        ...
        if orchestrator_loops >= limits.max_orchestrator_loops:
            logger.warning(
                f"   🚨 Immediate termination: orchestrator loops exceeded {limits.max_orchestrator_loops}"
            )
            return True
        ...
        if tools_used >= limits.max_tool_executions:
            logger.warning(
                f"   🚨 Immediate termination: tool executions exceeded {limits.max_tool_executions}"
            )
            return True
        ...
        if start_time:
            try:
                ...
                if elapsed_minutes >= limits.max_investigation_time_minutes:
                    logger.warning(
                        f"   🚨 Immediate termination: time limit exceeded {limits.max_investigation_time_minutes} minutes"
                    )
                    return True
            except Exception:
                pass
    ...
    def _build_override_reasoning(
        ...
        if not allows_ai_control:
            ...
            for concern in concerns:
                if concern.severity in ["critical", "high"]:
                    reasoning.append(
                        f"{concern.severity.capitalize()} concern: {concern.message}"
                    )
        ...
        if concerns:
            ...
            if critical_count > 0:
                reasoning.append(
                    f"Critical concerns requiring immediate action: {critical_count}"
                )

        resource_pressure = self._calculate_resource_pressure(
            state, self._calculate_dynamic_limits(state, SafetyLevel.STANDARD)
        )
