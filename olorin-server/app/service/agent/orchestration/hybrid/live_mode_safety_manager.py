...
class LiveModeSafetyManager:
    ...
    def _initialize_monitoring(self):
        ...
        logger.info(
            f"Live Mode Safety Manager initialized with level: {self.safety_level.value}"
        )
        logger.info(
            f"Cost limits: ${self.cost_limits.per_investigation}/investigation, ${self.cost_limits.per_session}/session"
        )
        logger.info(
            f"Time limits: {self.time_limits.per_investigation}min/investigation, {self.time_limits.per_phase}min/phase"
        )
    ...
    async def start_investigation_monitoring(self, investigation_id: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Starting live mode investigation monitoring: {investigation_id}"
            )
    ...
    def can_start_investigation(self) -> bool:
        ...
        if any(self.circuit_breakers.values()):
            ...
            self.record_safety_violation(
                "Circuit breakers active", {"active_breakers": active_breakers}
            )
            return False
    ...
    async def track_cost(
        self, cost_type: str, amount: float, details: Dict[str, Any] = None
    ):
        ...
        try:
            ...
            # Log cost update
            logger.debug(
                f"Cost tracked: {cost_type}=${amount:.2f}, total=${self.cost_tracking.total_costs:.2f}"
            )
    ...
    async def _check_cost_circuit_breakers(self):
        ...
        if (
            self.cost_tracking.investigation_start_time
            and self.cost_tracking.total_costs >= self.cost_limits.per_investigation
        ):
    ...
    async def track_api_usage(self, api_type: str, usage_details: Dict[str, Any]):
        ...
        try:
            ...
            if api_type == "snowflake":
                ...
                # Check credit limits
                if (
                    self.cost_tracking.snowflake_credits_used
                ...
                # Check token limits
                if (
                    self.cost_tracking.claude_tokens_used
                    >= self.cost_limits.claude_tokens
                ):
                    await self.activate_emergency_stop(
                        EmergencyStopReason.API_QUOTA_EXHAUSTED,
                ...
                # Check call limits
                if (
                    self.cost_tracking.external_api_calls_made
        ...
        try:
            ...
            if len(recent_errors) >= self.error_limits.consecutive_failures:
                ...
                for i in range(len(recent_errors) - 1):
                    ...
                    if (
                        time2 - time1
                    ).total_seconds() > 60:  # Not consecutive if >1 minute apart
                        consecutive = False
                        break
    ...
    def record_safety_violation(self, violation_type: str, details: Dict[str, Any]):
        ...
        logger.warning(
            f"Safety violation recorded: {violation_type}, details: {details}"
        )
    ...
    def _get_session_duration(self) -> float:
        ...
        if self.cost_tracking.session_start_time:
            return (
                datetime.now() - self.cost_tracking.session_start_time
            ).total_seconds() / 60
        return 0.0
    ...
    def generate_cost_report(self) -> Dict[str, Any]:
        ...
        return {
            "session_summary": {
                ...
                "cost_per_minute": self.cost_tracking.total_costs
            ...
            "safety_events": {
                ...
                "circuit_breaker_activations": sum(
                    1 for v in self.circuit_breakers.values() if v
                ),
            },
        }
