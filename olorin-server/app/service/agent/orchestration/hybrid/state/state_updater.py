...
from .ai_decision_models import AIRoutingDecision, SafetyOverride, create_safety_override
...
def add_safety_override(
    ...
    if safety_level not in MIN_LEVEL_FOR_OVERRIDE:
        logger.debug(f"🛡️ Safety override BLOCKED: level '{safety_level}' insufficient (needs elevated/emergency)")
        return state
        
    if current_pressure < MIN_PRESSURE_THRESHOLD:
        logger.debug(f"🛡️ Safety override BLOCKED: pressure {current_pressure:.3f} below threshold {MIN_PRESSURE_THRESHOLD}")
        return state
    ...
    recent_overrides = [
        override for override in state.get("safety_overrides", [])
    ...
    for recent_override in recent_overrides:
        ...
        if recent_key == override_key:
            ...
            logger.debug(f"🛡️ Safety override suppressed (duplicate): {original_ai_decision} → {safety_decision}")
            logger.debug(f"   Concern: {concern_type.value} (suppressed #{state['override_duplicates_suppressed'][override_key]})")
            logger.debug(f"   Cooldown: 5.0s active, {len(recent_overrides)} recent overrides")
            return state
    ...
    if len(recent_overrides) >= 8:  # More than 8 overrides in 5 seconds = storm
        logger.error(f"🚨 SAFETY OVERRIDE STORM DETECTED: {len(recent_overrides)} overrides in 5 seconds")
        logger.error(f"   Latest attempt: {original_ai_decision} → {safety_decision}")
        logger.error(f"   Storm prevention: Suppressing further overrides for 10 seconds")
...
def _is_recent_override(override: SafetyOverride, current_time: datetime, cooldown_seconds: float) -> bool:
    """
