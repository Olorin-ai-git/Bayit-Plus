...
class EvidenceValidator:
    ...
    def validate_evidence_for_completion(
        ...
        if evidence_quality < self.thresholds.minimum_acceptable:
            return False, f"Evidence quality {evidence_quality:.3f} below minimum threshold {self.thresholds.minimum_acceptable}", validation_result
        ...
        if domains_completed < min_domains:
            return False, f"Insufficient domains completed: {domains_completed} < {min_domains} required for {strategy} strategy", validation_result
        
        # Validate tool execution (flexible for basic strategy)
        if strategy != "basic" and tools_used < min_tools:
            return False, f"Insufficient tools executed: {tools_used} < {min_tools} required for {strategy} strategy", validation_result
        
        # Additional quality checks for higher strategies
        if strategy == "comprehensive" and evidence_quality < self.thresholds.adequate_threshold:
            return False, f"Comprehensive strategy requires evidence quality ≥ {self.thresholds.adequate_threshold}, got {evidence_quality:.3f}", validation_result
        
        validation_result["validation_passed"] = True
        return True, "Evidence validation passed", validation_result
    
    def should_trigger_safety_concerns(self, evidence_quality: float, loops_completed: int) -> bool:
