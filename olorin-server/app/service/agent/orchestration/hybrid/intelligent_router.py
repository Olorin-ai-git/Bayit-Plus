...
from .state.state_updater import (
    add_safety_override,  # CRITICAL FIX: Use version with gating logic
)
...
class IntelligentRouter:
    ...
    async def get_hybrid_routing_decision(
        ...
        logger.debug(
            f"     Previous routing decisions: {len(state.get('routing_decisions', []))}"
        )
        ...
        logger.debug(
            f"     Router Type: Intelligent Router (AI confidence + Advanced safety validation)"
        )
        logger.debug(
            f"     Decision factors: Confidence levels, investigation strategy, safety concerns"
        )
        logger.debug(f"     Available strategies: {list(self.strategy_routing.keys())}")
        logger.debug(
            f"     Available confidence levels: {list(self.confidence_routing.keys())}"
        )
        ...
        try:
            ...
            logger.debug(
                f"     Allows AI Control: {getattr(safety_status, 'allows_ai_control', 'N/A')}"
            )
            logger.debug(
                f"     Requires Termination: {getattr(safety_status, 'requires_immediate_termination', 'N/A')}"
            )
            logger.debug(
                f"     Resource Pressure: {getattr(safety_status, 'resource_pressure', 'N/A')}"
            )
            logger.debug(
            ...
            logger.debug(
                f"     Proposed Next Node: {routing_decision.get('next_node', 'N/A')}"
            )
            logger.debug(
                f"     Safety Override: {routing_decision.get('safety_override', False)}"
            )
            logger.debug(
                f"     Override Reason: {routing_decision.get('override_reason', 'None')}"
            )
            logger.debug(
                f"     Reasoning Items: {len(routing_decision.get('reasoning', []))}"
            )
            ...
            metadata = {
                ...
                "confidence_level": (
                    ...
                    if hasattr(getattr(ai_decision, "confidence_level", None), "value")
                ...
                "safety_level": (
                    getattr(safety_status, "safety_level", "UNKNOWN").value
                    if hasattr(getattr(safety_status, "safety_level", None), "value")
                    else str(getattr(safety_status, "safety_level", "UNKNOWN"))
                ),
            ...
            confidence_level = (
                getattr(ai_decision, "confidence_level", "UNKNOWN")
                if ai_decision
                else "UNKNOWN"
            )
            strategy = (
                getattr(ai_decision, "strategy", "ADAPTIVE")
                if ai_decision
                else "ADAPTIVE"
            )
            ...
            logger.debug(
                f"     State at failure: {state.get('investigation_id', 'N/A')} - phase {state.get('current_phase', 'N/A')}"
            )
            ...
            fallback_decision = {
                ...
                "reasoning": [
                    ...
                    "Falling back to safe completion",
                ],
    ...
    async def _apply_hybrid_decision_logic(
        self, state: HybridInvestigationState, ai_decision, safety_status
    ) -> Dict[str, Any]:
        ...
        recommended_action = (
            getattr(ai_decision, "recommended_action", "N/A") if ai_decision else "N/A"
        )
        ...
        logger.debug(
            f"     Allows AI Control: {getattr(safety_status, 'allows_ai_control', False)}"
        )
        logger.debug(
            f"     Requires Termination: {getattr(safety_status, 'requires_immediate_termination', False)}"
        )
        logger.debug(
            f"     Safety Level: {getattr(safety_status, 'safety_level', 'UNKNOWN')}"
        )
        logger.debug(
            f"     Resource Pressure: {getattr(safety_status, 'resource_pressure', 0.0)}"
        )
        logger.debug(
            f"     Safety Concerns Count: {len(getattr(safety_status, 'safety_concerns', []))}"
        )
        logger.debug(f"   🔄 DECISION FUSION PROCESS:")
        logger.debug(f"     Combining AI intelligence with safety validation")
        logger.debug(
            f"     Decision hierarchy: Emergency check → AI control check → Safety-first fallback"
        )
        ...
        if getattr(safety_status, "requires_immediate_termination", False):
            ...
            logger.debug(
                f"     Safety concerns: {getattr(safety_status, 'safety_concerns', [])}"
            )
            logger.debug(
                f"     Override reasoning: {getattr(safety_status, 'override_reasoning', [])}"
            )

            emergency_decision = {
                "next_node": "summary",
                "reasoning": ["Emergency termination required by safety manager"]
                + getattr(safety_status, "override_reasoning", []),
                "safety_override": True,
        ...
        high_confidence = (
            confidence_level in [AIConfidenceLevel.HIGH, AIConfidenceLevel.MEDIUM]
            if confidence_level
            else False
        )
        ...
        logger.debug(
            f"     Combined condition: {allows_ai_control and high_confidence}"
        )
        ...
        if allows_ai_control and high_confidence:
            ...
            if routing_method:
                ...
                logger.warning(
                    f"     No routing method found for confidence level: {confidence_level}"
                )
                decision = {
                    "next_node": "summary",
                    "reasoning": ["No routing method available"],
                }
            ...
            decision.update(
                {
                    ...
                    + ["AI control granted by safety validation"],
        ...
        logger.debug(
            f"     Reason: allows_ai_control={allows_ai_control}, high_confidence={high_confidence}"
        )
        ...
        safety_decision = await self._safety_first_routing(
            state, ai_decision, safety_status
        )
        ...
        if override_applied:
            clean_reasoning = [
                "Safety-first routing applied"
            ] + safety_status.override_reasoning
    ...
    async def _high_confidence_routing(
        self, state: HybridInvestigationState, ai_decision
    ) -> Dict[str, Any]:
        ...
        decision.update(
            {
                ...
                "reasoning": ["High confidence allows AI optimization"]
        ...
        decision.update(
            {
                ...
                "reasoning": ["Medium confidence with validation checks"]
                + decision.get("reasoning", []),
            }
        )
    ...
    async def _low_confidence_routing(
        ...
        return await self._sequential_routing(
            state, "Low confidence requires sequential execution"
        )
    ...
    async def _unknown_confidence_routing(
        ...
        return await self._sequential_routing(
            state, "Unknown confidence requires comprehensive analysis"
        )
    ...
    async def _critical_path_routing(
        ...
        if len(domain_findings) == 0 and len(domains_completed) == 0:
            logger.debug(
                f"⚡ Critical path: Need at least one domain analysis before risk assessment"
            )

            # CRITICAL FIX: Use circuit breaker to avoid repeated failures
            from app.service.agent.orchestration.circuit_breaker import (
                get_next_available_domain_node,
                is_node_disabled,
            )
            ...
            if not is_node_disabled(state, "network_agent"):
                return {
                    ...
                    "reasoning": [
                        "Critical path: Priority domain analysis first",
                        "Network analysis critical for fraud detection",
                    ],
                }
            else:
                ...
                if next_available and next_available != "summary":
                    return {
                        ...
                        "reasoning": [
                            "Critical path: Network agent disabled, using next available domain",
                            f"Fallback to {next_available}",
                        ],
                    }
                else:
                    # All domain agents disabled, skip to summary
                    logger.warning(
                        "⚡ Critical path: All domain agents disabled, skipping to summary"
                    )
                    return {
                        "next_node": "summary",
                        "reasoning": [
                            "Critical path: All domain agents failed",
                            "Emergency fallback to summary",
                        ],
                    }
        ...
        if "risk" not in domains_completed:
            return {
                ...
                "reasoning": [
                    "Critical path: Risk assessment with domain evidence",
    ...
    async def _minimal_routing(
        ...
        if len(domain_findings) == 0 and len(domains_completed) == 0:
            ...
            return {
                ...
                "reasoning": [
                    "Minimal strategy: One domain analysis required",
                    "Device analysis for basic evidence",
                ],
            }
        ...
        if "risk" not in domains_completed:
            return {
                "next_node": "risk_agent",
                "reasoning": [
                    "Minimal strategy: Risk assessment with minimal evidence",
        ...
        for agent in priority_agents:
            ...
            if domain not in domains_completed:
                return {
                    "next_node": agent,
                    "reasoning": [
                        f"Focused strategy: {domain} analysis priority",
                        f"Evidence suggests {domain} anomalies",
                    ],
                }
    ...
    async def _adaptive_routing(
        ...
        if tools_used < 2 and ai_decision.tools_recommended:
            return {
                "next_node": "tools",
                "reasoning": [
                    "Adaptive: Additional tools recommended",
                    f"Tools to use: {ai_decision.tools_recommended}",
                ],
            }
            ...
            for agent in recommended_agents:
                ...
                if domain not in domains_completed:
                    return {
                        "next_node": agent,
                        "reasoning": [
                            f"Adaptive: {domain} analysis recommended",
                            "AI selected based on current evidence",
                        ],
                    }
        ...
        return {
            "next_node": "summary",
            "reasoning": [
                "Adaptive investigation sufficient",
                f"Completed {len(domains_completed)} domains",
            ],
        }
    ...
    async def _comprehensive_routing(
        ...
        return await self._sequential_routing(
            state, "Comprehensive strategy: Full analysis required"
        )

    async def _sequential_routing(
        self, state: HybridInvestigationState, reason: str
    ) -> Dict[str, Any]:
        ...
        if snowflake_data and len(domain_findings) == 0:
            logger.debug(
                f"🎯 CRITICAL ROUTING D3: Database data available ({snowflake_data.get('row_count', 0)} rows), triggering domain analysis"
            )
            ...
            domain_decision["reasoning"] = [
                reason,
                "CRITICAL D3: Start domain analysis with database data",
            ] + domain_decision["reasoning"]
        ...
        if (
            len(domain_agents_completed) < 6
        ):  # Need 6 domain agents (risk runs separately after)
            domain_decision = await self._get_next_sequential_domain(state)
            # CRITICAL FIX: Only proceed if we got a valid domain agent, not summary
            if domain_decision["next_node"] != "summary":
                logger.debug(
                    f"🔄 Continue domain analysis: {domain_decision['next_node']} (completed: {len(domain_agents_completed)}/6)"
                )
                domain_decision["reasoning"] = [
                    reason,
                    f"Domain analysis progress: {len(domain_agents_completed)}/6 complete",
                ] + domain_decision["reasoning"]
                return domain_decision
            else:
                # If _get_next_sequential_domain returned summary but we haven't completed 6 domains, there's a tracking issue
                logger.warning(
                    f"⚠️ Routing issue: _get_next_sequential_domain returned summary but only {len(domain_agents_completed)}/6 domains completed"
                )
                ...
                for domain in domain_order:
                    if domain not in domain_agents_completed:
                        ...
                        return {
                            ...
                            "reasoning": [
                                reason,
                                f"Force routing: {domain} not yet completed",
                                f"Progress: {len(domain_agents_completed)}/6",
                            ],
                        }
        ...
        if len(domain_agents_completed) == 6:
            ...
            if "risk" not in agent_results:
                logger.info(
                    f"🎯 All 6 domain agents complete - routing to risk aggregation agent"
                )
                return {
                    "next_node": "risk_agent",
                    "reasoning": [
                        reason,
                        "6 domain agents complete",
                        "Running risk aggregation on domain findings",
                    ],
                }
        ...
        return {
            ...
            "reasoning": [
                ...
                f"Analyzed {len(domain_findings)} domains + risk aggregation",
            ],
        }
    ...
    async def _get_next_sequential_domain(
        ...
        logger.debug(
            f"   domains_completed list: {list(state.get('domains_completed', []))}"
        )
    ...
    async def _safety_first_routing(
        ...
        if threshold_manager.should_force_termination_for_safety_overrides(
            len(safety_overrides)
        ):
            logger.warning(
                f"🚨 LOOP PREVENTION: {len(safety_overrides)} safety overrides detected - forcing completion"
            )
            return {
                "next_node": "summary",
                "reasoning": [
        ...
        critical_concerns = [
            c for c in safety_status.safety_concerns if "critical" in c.lower()
        ]
        ...
        if safety_status.resource_pressure > 0.8:
            return {
                "next_node": "summary",
                "reasoning": [
                    f"High resource pressure: {safety_status.resource_pressure:.2f}",
                    "Force completion to prevent resource exhaustion",
                ],
            }
        ...
        if len(domain_findings) > 0:
            ...
            valid_domain_findings = sum(
                1
                for k, v in domain_findings.items()
                if v and isinstance(v, dict) and v.get("status") != "no_results"
            )
            actual_domains_completed = max(
                actual_domains_completed, valid_domain_findings
            )
        ...
        has_sufficient_evidence = (
            actual_domain_agents_completed
            >= 6  # CRITICAL FIX: Require all 6 domain agents
        ...
        if has_sufficient_evidence:
            logger.debug(
                f"🛡️ Safety routing with sufficient evidence: {len(tools_used)} tools, {actual_domain_agents_completed}/6 domain agents - moving to summary"
            )
            return {
                "next_node": "summary",
                "reasoning": [
        ...
        if actual_domain_agents_completed < 6:
            logger.debug(
                f"🛡️ Safety routing: Only {actual_domain_agents_completed}/6 domain agents completed - continuing domain analysis"
            )
            return await self._sequential_routing(
                state,
                f"Safety-first: Need {6 - actual_domain_agents_completed} more domain agents (current: {actual_domain_agents_completed}/6)",
            )

        # CRITICAL FIX: If no domain agents executed yet, force at least one
        if actual_domain_agents_completed == 0:
            logger.debug(
                f"🛡️ CRITICAL: No domain agents executed - forcing domain analysis"
            )
        ...
        # If some tools but insufficient evidence, try more domain analysis
            ...
            logger.debug(
                f"🛡️ Insufficient evidence: {len(tools_used)} tools, {actual_domain_agents_completed}/6 domain agents - require more domain analysis"
            )
            return await self._sequential_routing(
                state,
                f"Safety-first: Need more domain analysis ({actual_domain_agents_completed}/6 complete)",
            )

        # Default to conservative sequential routing for initial state
        return await self._sequential_routing(
            state, "Safety-first mode: Conservative approach"
        )

    async def _add_validation_checks(
        self, state: HybridInvestigationState, decision: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def hybrid_routing_function(self, state: HybridInvestigationState) -> str:
        """
        ...
        try:
            ...
            if routing_decision.get("safety_override"):
                logger.warning(
                    f"   ⚠️ Safety override: {routing_decision.get('override_reason')}"
                )
