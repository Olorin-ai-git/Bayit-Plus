...
class HealthMonitor:
    ...
    def check_error_rate(self) -> bool:
        ...
        if error_rate > threshold:
            logger.warning(f"🚨 Error rate threshold exceeded: {error_rate:.2%} > {threshold:.2%}")
            return True
    ...
    def check_performance_degradation(self) -> bool:
        ...
        if degradation > threshold:
            logger.warning(f"🚨 Performance degradation threshold exceeded: {degradation:.2%} > {threshold:.2%}")
            return True
    ...
    def check_safety_override_rate(self) -> bool:
        ...
        if override_rate > threshold:
            logger.warning(f"🚨 Safety override rate threshold exceeded: {override_rate:.2%} > {threshold:.2%}")
            return True
    ...
    def check_resource_usage(self) -> bool:
        ...
        if memory_usage and memory_usage > memory_threshold:
            logger.warning(f"🚨 Memory usage threshold exceeded: {memory_usage:.2%} > {memory_threshold:.2%}")
            return True
        
        if cpu_usage and cpu_usage > cpu_threshold:
            logger.warning(f"🚨 CPU usage threshold exceeded: {cpu_usage:.2%} > {cpu_threshold:.2%}")
            return True
