...
class MetricsCollector:
    ...
    def record_investigation_start(self, investigation_id: str, graph_type: str):
        ...
        logger.debug(f"📊 Investigation start recorded: {investigation_id} ({graph_type})")
    ...
    def record_investigation_completion(
        ...
        self.metrics_buffer.append(metric)
        logger.debug(f"📊 Investigation completion recorded: {investigation_id} (success={success})")
    ...
    def record_safety_override(
        ...
        self.metrics_buffer.append(metric)
        logger.debug(f"📊 Safety override recorded: {investigation_id} - {override_reason}")
    ...
    def get_average_duration(self, time_window: timedelta = timedelta(minutes=10)) -> float:
        """
        ...
        completions = [
            m for m in self.metrics_buffer
            if (m["type"] == "investigation_completion" 
    ...
    def get_safety_override_rate(self, time_window: timedelta = timedelta(minutes=10)) -> float:
        """
    ...
    def get_metrics_summary(self, time_window: timedelta = timedelta(minutes=10)) -> Dict[str, Any]:
        """
    ...
    def export_metrics(self, time_window: timedelta = timedelta(hours=1)) -> List[Dict[str, Any]]:
        """
