...
class RollbackTriggers:
    ...
    def record_investigation_result(
        ...
        if success:
            ...
            logger.warning(f"⚠️ Investigation failure #{self.consecutive_failure_count}: {investigation_id}")
    ...
    def _check_consecutive_failures(self) -> bool:
        ...
        if self.consecutive_failure_count >= threshold:
            logger.warning(f"🚨 Consecutive failure threshold exceeded: {self.consecutive_failure_count} >= {threshold}")
            return True
