...
from .feature_flags.flag_manager import DeploymentMode, FeatureFlags, GraphType
...
class MigrationManager:
    ...
    async def get_investigation_graph(
        ...
        try:
            ...
            if recovery_result["success"]:
                logger.warning(
                    f"🔧 Graph selection recovered: {recovery_result['action']}"
                )

                # If recovery suggests fallback to clean graph, do that
                if recovery_result.get("fallback_graph_type") == "clean":
                    return await self.graph_selector.graph_builders.build_graph(
                        GraphType.CLEAN
                    )
    ...
    async def complete_investigation(
        ...
        logger.info(
            f"🎯 Migration Manager: Completing investigation {investigation_id}"
        )
    ...
    def disable_hybrid_graph(self, reason: str = "manual_disable"):
        ...
        self.metrics_reporter.record_rollback_event(
            event_type="manual_disable", reason=reason
        )
    ...
    def start_ab_test(self, test_split: int = 50):
        ...
        logger.info(
            f"🧪 Starting A/B test: {test_split}% hybrid, {100-test_split}% clean"
        )

        self.graph_selector.ab_test_manager.start_ab_test(
            test_name="hybrid_vs_clean", test_split=test_split
        )

        # Record in metrics
        self.metrics_reporter.record_performance_metric(
            metric_name="ab_test_started", value=test_split, unit="percentage"
        )
    ...
    def stop_ab_test(self):
        ...
        # Record in metrics
        self.metrics_reporter.record_performance_metric(
            metric_name="ab_test_stopped", value=1.0, unit="boolean"
        )
    ...
    def trigger_rollback(self, reason: str):
        ...
        # Record in metrics
        self.metrics_reporter.record_rollback_event(
            event_type="triggered", reason=reason
        )
    ...
    def clear_rollback(self):
        ...
        # Record in metrics
        self.metrics_reporter.record_rollback_event(
            event_type="cleared", reason="manual_clear"
        )
    ...
    def get_migration_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_investigations": len(
                self.service_adapter.get_active_investigations()
            ),
        }
