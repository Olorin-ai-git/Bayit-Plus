...
class MetricsReporter:
    ...
    def record_graph_selection(
        ...
        logger.debug(f"📊 Graph selection metric recorded: {graph_type} ({selection_reason})")
    ...
    def generate_usage_report(
        ...
        logger.info(f"✅ Usage report generated with {self._count_metrics_in_window(cutoff_time)} metrics")
        return report
    ...
    def _analyze_graph_selections(self, cutoff_time: datetime) -> Dict[str, Any]:
        ...
        selections = [
            m for m in self.metrics_cache["graph_selection"]
        ...
        return {
            ...
            "hybrid_usage_percentage": graph_type_counts.get("hybrid", 0) / len(selections) * 100
        }
    ...
    def _analyze_feature_flags(self, cutoff_time: datetime) -> Dict[str, Any]:
        ...
        flag_usages = [
            m for m in self.metrics_cache["feature_flag_usage"]
            if datetime.fromisoformat(m["timestamp"]) >= cutoff_time
        ]
        ...
        for flag_name, stats in flag_stats.items():
            enablement_rate = stats["enabled"] / stats["total"] * 100 if stats["total"] > 0 else 0
    ...
    def _analyze_ab_tests(self, cutoff_time: datetime) -> Dict[str, Any]:
        ...
        assignments = [
            m for m in self.metrics_cache["ab_test_assignment"]
            if datetime.fromisoformat(m["timestamp"]) >= cutoff_time
        ]
    ...
    def _analyze_performance(self, cutoff_time: datetime) -> Dict[str, Any]:
        ...
        performance_metrics = [
            m for m in self.metrics_cache["performance_metric"]
            if datetime.fromisoformat(m["timestamp"]) >= cutoff_time
        ]
    ...
    def _analyze_rollbacks(self, cutoff_time: datetime) -> Dict[str, Any]:
        ...
        rollback_events = [
            m for m in self.metrics_cache["rollback_event"]
            if datetime.fromisoformat(m["timestamp"]) >= cutoff_time
        ]
