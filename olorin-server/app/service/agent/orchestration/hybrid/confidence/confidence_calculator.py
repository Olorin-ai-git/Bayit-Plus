...
class ConfidenceCalculator:
    ...
    def calculate_weighted_confidence(
        self, confidence_values: Union[Dict[ConfidenceFieldType, float], Dict[str, Any]]
    ) -> float:
        """
        ...
        if not validated_values:
            logger.warning(
                "No valid confidence values after validation, returning fallback"
            )
        ...
        if ConfidenceFieldType.OVERALL_CONFIDENCE in confidence_values:
            ...
            logger.debug(
                f"Using pre-calculated overall confidence: {fmt_num(overall, 3)}"
            )
            return overall
        ...
        for field_type, value in validated_values.items():
            ...
            if field_type in self.component_weights:
                ...
                total_weight += weight
                logger.debug(
                    f"  {field_type.value}: {fmt_num(value, 3)} × {fmt_num(weight, 2)} = {fmt_num(value * weight, 3)}"
                )
        ...
        if total_weight == 0:
            valid_values = [
                v
                for v in validated_values.values()
                if isinstance(v, (int, float)) and -float("inf") < v < float("inf")
            ]
            if not valid_values:
            ...
            logger.debug(
                f"No weighted components found, using simple average: {fmt_num(simple_avg, 3)}"
            )
            return simple_avg
    ...
    def calculate_confidence_trend(
        self, historical_confidences: list
    ) -> Dict[str, float]:
        """
        ...
        variance = sum((x - mean_conf) ** 2 for x in historical_confidences) / len(
    ...
    def _validate_weights(self):
        ...
        if abs(total_weight - 1.0) > 0.1:
            logger.warning(
                f"Component weights sum to {fmt_num(total_weight, 3)}, expected ~1.0"
            )
    ...
    def _validate_and_sanitize_input(
        ...
        if all(isinstance(k, ConfidenceFieldType) for k in confidence_values.keys()):
            ...
            for field_type, value in confidence_values.items():
                try:
                    validated_value = self._validate_single_confidence_value(
    ...
    def _validate_single_confidence_value(self, value: Any, field_name: str) -> float:
        ...
        if not (0.0 <= float_value <= 1.0):
            raise ValueError(
                f"{field_name} must be between 0.0 and 1.0, got {float_value}"
            )

        return float_value

    def calculate_with_validation(
        self, raw_confidence_data: Dict[str, Any]
    ) -> tuple[float, List[str]]:
        """
