...
class ConsolidatedConfidence:
    ...
    def _validate_quality_scores(self):
        ...
        if not (0.0 <= self.consistency_score <= 1.0):
            raise ValueError(f"Consistency score must be between 0.0 and 1.0, got {self.consistency_score}")
        
        if not (0.0 <= self.reliability_score <= 1.0):
            raise ValueError(f"Reliability score must be between 0.0 and 1.0, got {self.reliability_score}")
...
class ConfidencePayload(BaseModel):
    ...
    snowflake_score: Optional[float] = Field(None, ge=0.0, le=1.0, description="Snowflake evidence confidence (0.0-1.0)")
    tool_score: Optional[float] = Field(None, ge=0.0, le=1.0, description="Tool execution confidence (0.0-1.0)")
    domain_score: Optional[float] = Field(None, ge=0.0, le=1.0, description="Domain analysis confidence (0.0-1.0)")
    pattern_score: Optional[float] = Field(None, ge=0.0, le=1.0, description="Pattern recognition confidence (0.0-1.0)")
    velocity_score: Optional[float] = Field(None, ge=0.0, le=1.0, description="Investigation velocity confidence (0.0-1.0)")
    ...
    @field_validator('snowflake_score', 'tool_score', 'domain_score', 'pattern_score', 'velocity_score', mode='before')
    @classmethod
    ...
    def has_sufficient_data(self, min_components: int = 2) -> bool:
        ...
        """
        valid_components = sum(1 for field in ['snowflake_score', 'tool_score', 'domain_score', 'pattern_score', 'velocity_score']
    ...
    def get_data_quality_issues(self) -> List[str]:
        ...
        for field_name in ['snowflake_score', 'tool_score', 'domain_score', 'pattern_score', 'velocity_score']:
            value = getattr(self, field_name, None)
            if value is not None:
                if value == 0.0:
