...
class ConfidenceExtractor:
    ...
    def extract_confidence_values(
        ...
        try:
            ...
            logger.debug(f"Extracted {len(confidence_values)} confidence values from sources")
    ...
    def _extract_from_state(self, state: Dict[str, Any]) -> Dict[ConfidenceFieldType, float]:
    ...
    def _extract_from_agent_results(self, agent_results: List) -> Dict[ConfidenceFieldType, float]:
        ...
        confidences = {}
        if tool_confidences:
            confidences[ConfidenceFieldType.TOOL_CONFIDENCE] = sum(tool_confidences) / len(tool_confidences)
        
        if domain_confidences:
            confidences[ConfidenceFieldType.DOMAIN_CONFIDENCE] = sum(domain_confidences) / len(domain_confidences)
    ...
    def _extract_from_single_result(self, result) -> Dict[str, List[float]]:
        ...
        return {
            'tool': tool_confidences,
            'domain': domain_confidences
        }
    
    def _extract_from_context(self, investigation_context: Dict[str, Any]) -> Dict[ConfidenceFieldType, float]:
