...
class ConfidenceValidator:
    ...
    def _check_suspicious_patterns(
        ...
        if len(set(values)) == 1 and len(values) > 2:
            issues.append(f"Suspicious uniformity: all {len(values)} confidence values are identical ({values[0]})")
        ...
        if extreme_low > len(values) * 0.8:
            issues.append(f"Suspicious clustering: {extreme_low}/{len(values)} values below 0.1")
        
        if extreme_high > len(values) * 0.8:
            issues.append(f"Suspicious clustering: {extreme_high}/{len(values)} values above 0.9")
    ...
    def assess_confidence_reliability(
        ...
        missing_critical = sum(1 for ctype in critical_types if ctype not in confidence_values)
    ...
    def validate_consolidated_confidence(self, overall_score: float) -> List[str]:
        ...
        if not (0.0 <= overall_score <= 1.0):
            issues.append(f"Consolidated confidence score out of range: {overall_score}")
        
        # Check for suspicious edge values
        if overall_score == 0.0:
            issues.append("Consolidated confidence is exactly 0.0 - may indicate calculation error")
        
        if overall_score == 1.0:
            issues.append("Consolidated confidence is exactly 1.0 - may indicate over-confidence")
