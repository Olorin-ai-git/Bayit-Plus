...
class StrategySelector:
    ...
    async def determine_strategy(
        ...
        strategy = self._apply_strategy_logic(confidence, risk_score, snowflake_completed)
        
        logger.debug(f"   🎯 Strategy: {strategy.value}")
        
        return strategy
    
    def _apply_strategy_logic(self, confidence: float, risk_score: float, snowflake_completed: bool) -> InvestigationStrategy:
        ...
        if confidence >= 0.9 and risk_score >= 0.8:
            logger.debug(f"   🎯 Strategy logic: CRITICAL_PATH (high confidence + high risk)")
            return InvestigationStrategy.CRITICAL_PATH
        
        # High confidence and clear patterns -> Focused
        if confidence >= 0.7 and risk_score >= 0.6:
            logger.debug(f"   🎯 Strategy logic: FOCUSED (high confidence + clear patterns)")
        ...
        logger.debug(f"   🎯 Strategy logic: COMPREHENSIVE (low confidence or uncertain)")
