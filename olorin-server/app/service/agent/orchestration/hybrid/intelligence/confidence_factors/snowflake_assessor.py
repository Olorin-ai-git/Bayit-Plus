...
class SnowflakeAssessor:
    ...
    async def assess_evidence(self, state: HybridInvestigationState) -> float:
        ...
        fraud_boosted_score = self._apply_fraud_pattern_boost(
            evidence_score, snowflake_data
        )

        # Apply data completeness factor
        final_confidence = self._apply_completeness_factor(
            fraud_boosted_score, snowflake_data
        )
    ...
    def _apply_fraud_pattern_boost(
        self, base_score: float, snowflake_data: Dict[str, Any]
    ) -> float:
    ...
    def _apply_completeness_factor(
        self, score: float, snowflake_data: Dict[str, Any]
    ) -> float:
