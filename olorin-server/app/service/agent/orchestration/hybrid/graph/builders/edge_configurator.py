...
def force_tools_routing(state):
    ...
    if consecutive_ai_messages >= max_retries:
        logger.warning(
            f"⚠️ Max retries ({max_retries}) reached for tool calls ({consecutive_ai_messages} consecutive AI messages without tools). Proceeding without tools."
        )
        return "ai_confidence_assessment"

    # No tool calls - retry with stronger prompts
    logger.warning(
        f"⚠️ No tool calls detected ({consecutive_ai_messages} consecutive attempts). Retrying fraud_investigation with stronger prompts."
    )
...
class EdgeConfigurator:
    ...
    def define_workflow_edges(
        self, builder: StateGraph, use_enhanced_tools: bool = False
    ) -> None:
        """
    ...
    def _configure_tool_routing(
        self, builder: StateGraph, use_enhanced_tools: bool
    ) -> None:
