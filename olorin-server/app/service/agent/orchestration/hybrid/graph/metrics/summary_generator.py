...
from ....metrics.safe import coerce_float, fmt_num, safe_div, safe_gt
from ...hybrid_state_schema import AIConfidenceLevel, HybridInvestigationState
...
class SummaryGenerator:
    ...
    def generate_hybrid_summary(self, state: HybridInvestigationState) -> str:
        ...
        summary_parts.extend(
            [self._generate_performance_insights(summary_data), self._generate_footer()]
        )
    ...
    def _collect_summary_data(self, state: HybridInvestigationState) -> Dict[str, Any]:
        ...
        return {
            ...
            "confidence_level": state.get(
                "ai_confidence_level", AIConfidenceLevel.UNKNOWN
            ).value,
            "strategy": self._get_strategy_value(
                state.get("investigation_strategy", "unknown")
            ),
            "orchestrator_loops": state.get("orchestrator_loops", 0),
            "domains_completed": len(state.get("domains_completed", [])),
            "tools_used": len(
                state.get("tool_results", {})
            ),  # Only count tools with actual results
            "safety_overrides": len(state.get("safety_overrides", [])),
            "risk_score": state.get(
                "risk_score"
            ),  # CRITICAL: Preserve None values, don't default to 0.0
            ...
            "domain_findings": state.get(
                "domain_findings", {}
            ),  # CRITICAL: Include domain findings to show domain risk scores
        }
...
"""
    ...
    def _generate_investigation_coverage(self, data: Dict[str, Any]) -> str:
        ...
        data_sources = "Snowflake" + (
            ", Additional Tools" if data["tools_used"] > 0 else ""
        )
...
"""
    ...
    def _generate_risk_assessment(self, data: Dict[str, Any]) -> str:
        ...
        risk_score_display = (
            fmt_num(data["risk_score"], 3) if data["risk_score"] is not None else "N/A"
        )
...
"""
    ...
    def _generate_safety_compliance(self, data: Dict[str, Any]) -> str:
        ...
        completion_status = (
            "Successfully" if data["current_phase"] == "complete" else "With Issues"
        )
        resource_utilization = (
            "Efficient"
            if data["safety_overrides"] == 0
            else "Required Safety Intervention"
        )
...
"""
    ...
    def _generate_ai_decision_analysis(self, data: Dict[str, Any]) -> str:
        ...
        evidence_quality = (
            getattr(final_decision, "evidence_quality", 0.0) if final_decision else 0.0
        )
        investigation_completeness = (
            getattr(final_decision, "investigation_completeness", 0.0)
            if final_decision
            else 0.0
        )
        resource_impact = (
            getattr(final_decision, "resource_impact", "Unknown")
            if final_decision
            else "Unknown"
        )
        strategy_value = getattr(final_decision, "strategy", None)
        strategy_str = (
            getattr(strategy_value, "value", "Unknown") if strategy_value else "Unknown"
        )
...
"""
    ...
    def _generate_performance_insights(self, data: Dict[str, Any]) -> str:
        ...
        optimization_type = (
            "AI-Optimized" if safe_gt(data["final_confidence"], 0.7) else "Safety-First"
        )
        resource_efficiency = (
            "Excellent"
            if data["safety_overrides"] == 0
            else "Good with Safety Controls"
        )
