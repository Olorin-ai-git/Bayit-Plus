...
from ...hybrid_state_schema import update_ai_confidence
...
class IntelligenceNodes:
    ...
    async def ai_confidence_assessment_node(
        ...
        logger.debug(f"     Factor weights: Snowflake(35%) + Tool(25%) + Domain(20%) + Pattern(15%) + Velocity(5%)")
        ...
        try:
            ...
            logger.debug(f"   🔄 STATE UPDATE PHASE: Updating state with AI confidence...")
            updated_state = update_ai_confidence(state, ai_decision, "confidence_assessment")
            logger.debug(f"   ✅ STATE UPDATED: AI confidence integrated into investigation state")
            ...
            audit_entry = {
                ...
                "details": {
                    ...
                    "recommended_action": getattr(ai_decision, 'recommended_action', 'N/A'),
                    "reasoning": getattr(ai_decision, 'reasoning', [])
                }
            }
            
            # Add confidence assessment to audit trail
            logger.debug(f"   🔄 AUDIT TRAIL PHASE: Adding assessment to audit trail...")
            ...
            logger.debug(f"     Investigation context: {state.get('investigation_id', 'N/A')} - {state.get('current_phase', 'N/A')}")
            logger.debug(f"     State at failure: tool_count={state.get('tool_count', 0)}, loops={state.get('orchestrator_loops', 0)}")
    ...
    async def safety_validation_node(
        ...
        logger.debug(f"     Tool Count: {state.get('tool_count', 0)}")
        logger.debug(f"     Existing Safety Concerns: {len(state.get('safety_concerns', []))}")
        logger.debug(f"     Current AI Confidence: {state.get('ai_confidence', 0.0)}")
        logger.debug(f"   🛡️ SAFETY FRAMEWORK:")
        logger.debug(f"     Manager Type: Advanced Safety Manager (Dynamic limits & resource monitoring)")
        logger.debug(f"     Validation scope: AI control authorization & termination checks")
        ...
        try:
            ...
            if limits:
                dynamic_limits = {
                    "max_orchestrator_loops": getattr(limits, 'max_orchestrator_loops', 50),
                    "max_tool_executions": getattr(limits, 'max_tool_executions', 20),
                    "max_domain_attempts": getattr(limits, 'max_domain_attempts', 10),
                    "max_investigation_time_minutes": getattr(limits, 'max_investigation_time_minutes', 30)
                }
                state["dynamic_limits"] = dynamic_limits
                logger.debug(f"   ✅ DYNAMIC LIMITS UPDATED: {dynamic_limits}")
            else:
                logger.warning(f"   ⚠️ No current_limits in safety_status, skipping limits update")
            ...
            if safety_concerns:
                safety_level_value = getattr(safety_status.safety_level, 'value', str(safety_status.safety_level)) if hasattr(safety_status, 'safety_level') else 'UNKNOWN'
                concern_records = [{
                    "timestamp": datetime.now().isoformat(),
                    "concern": concern,
                ...
                logger.debug(f"   ✅ SAFETY CONCERNS RECORDED: {len(concern_records)} new concerns added")
            else:
                logger.debug(f"   ✅ NO NEW SAFETY CONCERNS: Current operation within safe parameters")
            ...
            audit_entry = {
                ...
                "details": {
                    "safety_level": safety_level_value,
                    "ai_control_allowed": getattr(safety_status, 'allows_ai_control', False),
                    "termination_required": getattr(safety_status, 'requires_immediate_termination', False),
                    "resource_pressure": getattr(safety_status, 'resource_pressure', 0.0),
            ...
            logger.debug(f"   🔄 AUDIT TRAIL PHASE: Adding validation to audit trail...")
            state["decision_audit_trail"].append(audit_entry)
            logger.debug(f"   ✅ AUDIT ENTRY ADDED: Total audit entries: {len(state['decision_audit_trail'])}")
