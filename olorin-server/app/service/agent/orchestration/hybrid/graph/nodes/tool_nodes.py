...
def _is_successful_tool_execution(tool_name: str, tool_content: str) -> bool:
    ...
    if tool_content.strip().startswith('{"error"') or tool_content.strip().startswith(
        '{"success": false'
    ):
        return False
...
class ToolNodes:
    ...
    def add_tool_nodes(
        ...
        if use_enhanced_tools:
            ...
            try:
                logger.debug(
                    f"[Hybrid] Creating EnhancedToolNode with investigation_id={investigation_id}"
                )
                enhanced_tool_executor = EnhancedToolNode(
                    tools, investigation_id=investigation_id
                )
                builder.add_node("tools", enhanced_tool_executor)
                logger.info(
                    f"✅ [Hybrid] Using EnhancedToolNode with {len(tools)} tools and persistence enabled"
                )
            except Exception as e:
                logger.warning(
                    f"[Hybrid] Failed to create EnhancedToolNode: {e}, falling back to standard ToolNode"
                )
        ...
        try:
            ...
            if database_provider == "snowflake":
                ...
                if snowflake_tool not in tools:
                    ...
                    logger.info(
                        f"✅ Added SnowflakeQueryTool as PRIMARY tool (DATABASE_PROVIDER={database_provider})"
                    )
            elif database_provider == "postgresql":
                # Use PostgreSQL provider - create DatabaseQueryTool with PostgreSQL connection
                try:
                    from app.service.agent.tools.database_tool.database_tool import (
                        DatabaseQueryTool,
                    )
                    ...
                    postgres_tool = DatabaseQueryTool(
                    ...
                    if postgres_tool not in tools:
                        ...
                        logger.info(
                            f"✅ Added DatabaseQueryTool with PostgreSQL provider as PRIMARY tool"
                        )
                        logger.info(
                            f"✅ PostgreSQL connection: {host}:{port}/{database}"
                        )
                except Exception as e:
                    logger.error(
                        f"❌ Failed to create PostgreSQL DatabaseQueryTool: {str(e)}"
                    )
                    logger.warning(
                        "⚠️ Falling back to tools from registry - data queries may fail"
                    )
                    raise
            else:
                logger.error(f"❌ Invalid DATABASE_PROVIDER: {database_provider}")
                raise ValueError(
                    f"DATABASE_PROVIDER must be 'postgresql' or 'snowflake', got: {database_provider}"
                )
    ...
    def _create_metadata_tracker(self):
        ...
        def track_tool_metadata(
            state: HybridInvestigationState, config: Optional[Dict] = None
                ...
                for msg in reversed(messages[-10:]):  # Check last 10 messages
                    if (
                        ...
                        if tool_name == "composio_webcrawl":
                            ...
                            logger.debug(
                                f"📊 Stored composio_webcrawl result (length: {len(str(tool_content))} chars)"
                            )
                            continue

                        # Check if this is a successful tool execution (not skipped/failed)
                        is_successful_execution = _is_successful_tool_execution(
                        ...
                        if is_successful_execution:
                            ...
                            logger.debug(
                                f"📊 Skipping tool telemetry for {tool_name} (not successfully executed)"
                            )
                ...
                for msg in reversed(
                    ...
                    if (
                        ...
                        if "snowflake" in tool_name or "database" in tool_name:
                            ...
                            snowflake_data = self._normalize_data_type(
                                tool_content, "snowflake"
                            )
                            metadata_update["snowflake_data"] = snowflake_data
                            metadata_update["snowflake_completed"] = True
                            logger.info(
                                f"📊 DATABASE DATA: Extracted from {msg.name} (type: {type(snowflake_data).__name__})"
                            )
                        elif "network" in tool_name:
                            # CRITICAL FIX B0: Normalize network data for LLM processing
                            network_data = self._normalize_data_type(
                                tool_content, "network"
                            )
                            metadata_update["network_data"] = network_data
                            logger.info(
                                f"📊 NETWORK DATA: Extracted from {msg.name} (type: {type(network_data).__name__})"
                            )
                        elif "device" in tool_name:
                            # CRITICAL FIX B0: Normalize device data for LLM processing
                            device_data = self._normalize_data_type(
                                tool_content, "device"
                            )
                            metadata_update["device_data"] = device_data
                            logger.info(
                                f"📊 DEVICE DATA: Extracted from {msg.name} (type: {type(device_data).__name__})"
                            )
                        elif "location" in tool_name:
                            # CRITICAL FIX B0: Normalize location data for LLM processing
                            location_data = self._normalize_data_type(
                                tool_content, "location"
                            )
                            metadata_update["location_data"] = location_data
                            logger.info(
                                f"📊 LOCATION DATA: Extracted from {msg.name} (type: {type(location_data).__name__})"
                            )
                        elif (
                            "logs" in tool_name
                            or "splunk" in tool_name
                            or "sumologic" in tool_name
                        ):
                            ...
                            logger.info(
                                f"📊 LOGS DATA: Extracted from {msg.name} (type: {type(logs_data).__name__})"
                            )

                # Update phase after first successful tool execution
                if (
                    len(tools_used) > 0
                    and state.get("current_phase") == "initialization"
                ):
                    metadata_update["current_phase"] = "tool_execution"
                    logger.info(f"📊 PHASE UPDATE: initialization → tool_execution")

                logger.info(
                    f"✅ TRACKING: Updated metadata for {len(tools_used)} tools"
                )
        ...
        try:
            if isinstance(data, str):
                ...
                try:
                    ...
                    logger.debug(
                        f"📊 {data_type.upper()} DATA: Parsed JSON string to {type(parsed_data).__name__}"
                    )
                    return parsed_data
                    ...
                    try:
                        ...
                        logger.debug(
                            f"📊 {data_type.upper()} DATA: Parsed Python literal to {type(parsed_data).__name__}"
                        )
                        return parsed_data
                ...
                logger.debug(
                    f"📊 {data_type.upper()} DATA: Already {type(data).__name__}, no parsing needed"
                )
                return data

        except Exception as e:
            logger.error(
                f"📊 {data_type.upper()} DATA: Error normalizing data type: {str(e)}"
            )
