...
from app.service.agent.orchestration.metrics.safe import fmt_num, safe_div, safe_mean
from app.service.intel.normalize import (
    abuseipdb_risk_component,
    normalize_abuseipdb,
...
from .policy import (
    apply_confirmed_fraud_floor,
    derive_confirmed_fraud,
    fuse_risk,
    isolate_llm_narrative,
    prepublish_validate,
)
...
def compute_final_risk(state: Dict[str, Any]) -> Optional[float]:
    ...
    logger.info(
        f"🔍 compute_final_risk: domain_findings type={type(domain_findings)}, keys={list(domain_findings.keys()) if isinstance(domain_findings, dict) else 'NOT A DICT'}"
    )
    if isinstance(domain_findings, dict):
        for domain_name in [
            ...
            if domain_name in domain_findings:
                risk_score = (
                    domain_findings[domain_name].get("risk_score")
                    if isinstance(domain_findings[domain_name], dict)
                    else "NOT A DICT"
                )
    ...
    if llm_risk_score is not None:
        try:
            ...
            if 0 <= llm_score_float <= 1.0:
                ...
                logger.info(
                    f"🧠 LLM risk score found ({llm_score_float:.4f}), blending 50/50 with risk agent score ({risk_agent_score:.4f}) = {blended_score:.4f}"
                )
                final_risk = min(1.0, max(0.0, blended_score))
                return final_risk
            else:
                logger.warning(
                    f"⚠️ LLM risk score out of range ({llm_score_float}), using risk agent score only"
                )
        except (ValueError, TypeError) as e:
            logger.warning(
                f"⚠️ Invalid LLM risk score format: {llm_risk_score}, using risk agent score only"
            )
...
def finalize_risk(state: Dict[str, Any]) -> None:
    ...
    if final_risk is None:
        logger.info(
            "🚫 EVIDENCE GATING: Final risk blocked by aggregator - investigation needs more evidence"
        )
        state["risk_score"] = None
        state["investigation_status"] = "insufficient_evidence"
        gating_reason = state.get(
    ...
    if confirmed_fraud_detected:
        logger.info(
            "🚨 CONFIRMED FRAUD BYPASS: Skipping evidence gate due to ground truth fraud confirmation"
        )
        ...
        logger.warning(
            f"🚫 EVIDENCE GATE: Blocked numeric risk publication - {validation_result['status']}"
        )
    ...
    if confirmed_fraud_detected:
        ...
        if final_risk is None or final_risk < fraud_floor:
            ...
            logger.info(
                f"🚨 CONFIRMED FRAUD FLOOR: Risk elevated from {fmt_num(original_risk_for_floor, 3)} to {fmt_num(final_risk, 3)} due to ground truth"
            )
    ...
    if validation_result.get("discordant_signals", False):
        ...
        if state.get("confirmed_fraud_present") and max_allowed < 0.60:
            max_allowed = 0.60
            logger.info(
                "🛡️ FRAUD FLOOR PROTECTION: Discordance cap raised to respect confirmed fraud floor"
            )
        ...
        if final_risk > max_allowed:
            ...
            logger.warning(
                f"🔒 DISCORDANCE CAP: Risk capped from {fmt_num(original_risk, 3)} to {fmt_num(final_risk, 3)} due to discordant signals"
            )
    ...
    from app.service.agent.orchestration.domain.linter import (
        assert_investigation_safety,
        lint_investigation,
    )
    ...
    for domain_name in ["logs", "network", "device", "location", "authentication"]:
        ...
        if isinstance(domain_data, dict):
            domain_result = DomainResult(
                ...
                status=(
                    "OK"
                    if domain_data.get("risk_score") is not None
                    else "INSUFFICIENT_EVIDENCE"
                ),
                signals=domain_data.get("evidence", [])[
                    :3
                ],  # First 3 evidence items as signals
    ...
    if current_state_risk != final_risk:
        ...
        if current_state_risk is not None:
            logger.debug(
                f"🔄 Updated state.risk_score from {fmt_num(current_state_risk, 3)} to {fmt_num(final_risk, 3)} before linting"
            )
    ...
    if lint_errors:
        logger.warning(
            f"🚨 LINTING VIOLATIONS DETECTED: {len(lint_errors)} issues before publishing risk"
        )
    ...
    logger.debug(
        f"✅ Risk finalization complete - numeric risk published: {fmt_num(final_risk, 3)}"
    )
...
def validate_risk_consistency(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not validation["is_consistent"]:
        validation["issues"].append(
            f"Risk score mismatch: state={final_risk}, computed={computed_risk}"
        )
    ...
    for domain, data in domain_findings.items():
        ...
        if risk_score is not None:
            if not isinstance(risk_score, (int, float)):
                validation["issues"].append(
                    f"Domain {domain} has invalid risk_score type: {type(risk_score)}"
                )
            elif risk_score < 0 or risk_score > 1:
                validation["issues"].append(
                    f"Domain {domain} risk_score out of range: {risk_score}"
                )
...
def _apply_intel_fusion(state: Dict[str, Any]) -> None:
    ...
    try:
            ...
            if "confirmed fraud" in combined_text:
                ...
                if llm_risk is not None:
                    ...
                    if isinstance(llm_risk, (int, float)) and llm_risk > 1.0:
                        ...
                        logger.info(
                            f"C2 FIX: Normalized confirmed fraud LLM score from {original_llm_risk} to {fmt_num(llm_risk, 3)}"
                        )

                    network_findings["risk_score"] = llm_risk
                    evidence = network_findings.setdefault("evidence", [])
                    evidence.append(
                        f"Preserved LLM confirmed fraud assessment: {fmt_num(llm_risk, 3)} (no external fusion applied)"
                    )
                    logger.info(
                        f"Confirmed fraud detected: preserving LLM risk score {fmt_num(llm_risk, 3)} (skipped intel fusion)"
                    )
                    return
        ...
        vt_result = tool_results.get("virustotal_ip_analysis") or tool_results.get(
            "virustotal_analysis"
        )
        if vt_result:
            try:
                parsed_vt = (
                    json.loads(vt_result) if isinstance(vt_result, str) else vt_result
                )
        ...
        if provider_data:
            ...
            if len(provider_data) > 1:
                ...
                conservative_provider = unified_assessment.get(
                    "consensus_provider", "unknown"
                )

                # Apply intel fusion with unified assessment
                fused_risk = fuse_network_risk(
                    internal_risk, unified_assessment.get("base_data", {})
                )

                # Add evidence of conflict resolution and fusion
                evidence = network_findings.setdefault("evidence", [])
                evidence.append(
                    f"Applied provider conflict resolution: selected {conservative_provider} "
                    f"(risk: {fmt_num(external_risk, 3)}) as most conservative"
                )
                evidence.append(
                    f"Applied intel fusion with unified assessment (weighted internal:external = 70:30)"
                )

                logger.info(
                    f"Provider conflict resolution + fusion: internal={fmt_num(internal_risk, 3)}, "
                ...
                evidence = network_findings.setdefault("evidence", [])
                evidence.append(
                    f"Applied intel fusion with {provider_name} (weighted internal:external = 70:30)"
                )

                logger.info(
                    f"Single provider fusion ({provider_name}): internal={fmt_num(internal_risk, 3)} -> fused={fmt_num(fused_risk, 3)}"
                )
    ...
    try:
        ...
        for domain_name, domain_data in domain_findings.items():
            if isinstance(domain_data, dict) and "llm_analysis" in domain_data:
                ...
                domain_data["llm_analysis"] = clean_llm_analysis_sections(
                    domain_data["llm_analysis"]
                )
    ...
    try:
        ...
        for domain_name in ["logs", "network"]:
            ...
            if isinstance(domain_obj, dict):
                ...
                logger.debug(
                    f"Applied confirmed fraud floor check to {domain_name} domain"
                )
    ...
    if start_time and end_time:
        try:
            ...
            start_str = (
                ...
                if "Z" in str(start_time)
                else str(start_time)
            )
            end_str = (
                end_time.replace("Z", "+00:00")
                if "Z" in str(end_time)
                else str(end_time)
            )
...
def _assert_sanity_checks(state: Dict[str, Any], final_risk: float) -> None:
    ...
    for domain_name, domain_data in domain_findings.items():
        if isinstance(domain_data, dict) and "risk_score" in domain_data:
            ...
            assert (
                0.0 <= domain_risk <= 1.0
            ), f"domain {domain_name} risk_score out of range: {domain_risk}"
    ...
    if not state.get("end_time"):
        ...
        if start_time_str and duration_ms > 0:
            try:
                ...
                logger.info(
                    f"✅ DEFENSIVE: computed end_time = {computed_end_time} (start + {duration_ms}ms)"
                )
                ...
                logger.warning(
                    f"⚠️ FALLBACK: Using current time as end_time = {fallback_end_time}"
                )
        else:
            ...
            state["end_time"] = fallback_end_time
            logger.warning(
                f"⚠️ FALLBACK: No valid timing data, using current time as end_time = {fallback_end_time}"
            )
    ...
    try:
        ...
        if entity_type == "ip" or (entity_id and _looks_like_ip(entity_id)):
            ...
            if network_findings:
                ...
                analysis = network_findings.get(
                    "analysis", {}
                )  # FIXED: Look in analysis, not llm_analysis
                ...
                if unique_ips_analysis in (None, 0):
                    ...
                    unique_ips_analysis = max(
                        1, len(set([entity_ip])) if entity_ip else 1
                    )
                ...
                assert (
                    unique_ips_analysis >= 1
                ), f"IP investigation unique_ips in analysis must be >= 1, got {unique_ips_analysis}"
                assert (
                    unique_ips_metrics >= 1
                ), f"IP investigation unique_ip_count in metrics must be >= 1, got {unique_ips_metrics}"

                # Consistency check between analysis and metrics
                assert (
                    unique_ips_analysis == unique_ips_metrics
        ...
        ), f"evidence_strength must be numeric, got {type(evidence_strength)}"
        assert (
            0.0 <= evidence_strength <= 1.0
        ), f"evidence_strength must be 0.0-1.0, got {evidence_strength}"
        ...
        if tools_used:
            ...
            if len(tools_with_results) == 0 and len(tool_results) == 0:
                ...
                logger.warning(
                    f"⚠️ Business logic warning: tools_used={len(tools_used)} but no tool_results found. Tools: {tools_used[:5]}"
                )
                ...
                missing_tools = [
                    tool for tool in tools_used if tool not in tool_results
                ]
                logger.debug(
                    f"📊 Some tools in tools_used don't have results: {missing_tools[:3]}"
                )
        ...
        if snowflake_data and snowflake_data.get("results"):
            ...
            assert isinstance(
                results, list
            ), f"snowflake results must be list, got {type(results)}"
            ...
            if results:
                ...
                if isinstance(first_record, dict):
                    ...
                    if has_fraud_column:
                        logger.error(
                            "🚨 CRITICAL DATA LEAKAGE: IS_FRAUD_TX column found in Snowflake results - this should NOT happen!"
                        )
                        logger.error(
                            "   Investigation queries must exclude IS_FRAUD_TX to prevent data leakage"
                        )
                    else:
                        logger.debug(
                            "✅ Data leakage check passed: IS_FRAUD_TX not found in results"
                        )
        ...
        for domain in expected_domains:
            if domain in domain_findings:
                ...
                assert isinstance(
                    domain_data, dict
                ), f"domain {domain} data must be dict, got {type(domain_data)}"

                # Each domain should have evidence list
                evidence = domain_data.get("evidence", [])
                assert isinstance(
                    evidence, list
                ), f"domain {domain} evidence must be list, got {type(evidence)}"
        ...
        ), f"investigation took too long: {duration_ms}ms > {max_duration_ms}ms"
...
def _compute_evidence_strength(state: Dict[str, Any]) -> None:
    ...
    try:
        ...
        if len(risk_scores) >= 2:
            ...
            max_deviation = (
                max(abs(score - mean_score) for score in risk_scores)
                if risk_scores
                else 0
            )
        ...
        if isinstance(snowflake_data, dict) and "results" in snowflake_data:
            ...
            if isinstance(results, list) and len(results) > 0:
                volume_recency_score = min(
                    0.2, len(results) * 0.05
                )  # More records = better
            else:
        ...
        base_evidence_strength = min(
            1.0,
            source_diversity_score
            + corroboration_score
            + volume_recency_score
            + data_quality_score,
        )
        ...
        if (
            ...
            try:
                from app.service.agent.orchestration.risk.fraud_pattern_detectors import (
                    calculate_fraud_pattern_score,
                )

                results = snowflake_data["results"]
                if isinstance(results, list) and len(results) >= 3:
                    fraud_pattern_score, pattern_details = (
                        calculate_fraud_pattern_score(results)
                    )

                    if fraud_pattern_score > 0.0:
                        logger.info(
                            f"🔍 Fraud pattern score: {fmt_num(fraud_pattern_score, 3)}"
                        )
                        logger.info(
                            f"   Patterns detected: velocity={pattern_details['velocity_burst'] is not None}, "
        ...
        if fraud_pattern_score >= 0.6 and base_evidence_strength < 0.6:
            ...
            logger.info(
                f"✅ Fraud pattern boost: evidence_strength elevated from {fmt_num(base_evidence_strength, 3)} to {fmt_num(evidence_strength, 3)} (pattern score: {fmt_num(fraud_pattern_score, 3)})"
            )
...
def _deduplicate_cross_domain_prose(state: Dict[str, Any]) -> None:
    ...
    try:
        ...
        paragraph_registry = (
            {}
        )  # hash -> (domain, section, paragraph_text, first_occurrence)
        duplicates_found = []
        ...
        for duplicate_info in duplicates_found:
            ...
            if original_para_text in section_text:
                ...
                new_section_text = "\n\n".join(
                    [p.strip() for p in new_section_text.split("\n\n") if p.strip()]
                )
        ...
        if removed_count > 0:
            logger.info(
                f"Cross-domain deduplication: removed {removed_count} duplicate paragraphs"
            )
            logger.debug(
                f"Duplicates removed from: {[d['duplicate_location'] for d in duplicates_found]}"
            )
        else:
...
def _apply_decisive_evidence_policy(state: Dict[str, Any]) -> None:
    ...
    try:
        ...
        if evidence_strength < 0.7:
            logger.debug(
                f"Decisive evidence policy: evidence strength {fmt_num(evidence_strength, 3)} < 0.7, no confidence floor applied"
            )
            return
        ...
        if not has_confirmed_fraud:
            logger.debug(
                "Decisive evidence policy: no confirmed fraud detected, no confidence floor applied"
            )
            return
        ...
        for domain_name, domain_data in domain_findings.items():
            if isinstance(domain_data, dict):
                ...
                if (
                    ...
                    and current_confidence < confidence_floor
                ):
                    domain_data["confidence"] = confidence_floor
                    domains_updated.append(domain_name)
                    logger.info(
                        f"Applied confidence floor to {domain_name}: {fmt_num(current_confidence, 3)} -> {fmt_num(confidence_floor, 3)}"
                    )

        # Also apply to overall state confidence if it exists
        state_confidence = state.get("confidence", 0.0)
        if (
            isinstance(state_confidence, (int, float))
            and state_confidence < confidence_floor
        ):
            state["confidence"] = confidence_floor
            logger.info(
                f"Applied confidence floor to investigation: {fmt_num(state_confidence, 3)} -> {fmt_num(confidence_floor, 3)}"
            )

        if domains_updated or state_confidence < confidence_floor:
            logger.info(
                f"Decisive evidence policy applied: evidence_strength={fmt_num(evidence_strength, 3)}, confirmed_fraud=True, confidence_floor={fmt_num(confidence_floor, 3)}"
            )
        else:
            logger.debug(
                f"Decisive evidence policy: conditions met but no confidence adjustments needed"
            )
...
def _apply_evidence_gate(state: Dict[str, Any]) -> None:
    ...
    try:
        ...
        if not has_minimum_evidence(state):
            ...
            logger.error(
                "🚫 EVIDENCE GATE VIOLATION: Investigation lacks minimum external evidence"
            )
            logger.error(
                "   This investigation relies too heavily on Snowflake data without external validation"
            )
            ...
            risk_indicators.append(
                "Investigation quality: Insufficient external validation"
            )

            logger.warning(
                "⚠️ Investigation marked as low-quality due to insufficient evidence"
            )
        else:
            state["evidence_gate_passed"] = True
            state["investigation_quality"] = "sufficient_evidence"
            logger.debug(
                "✅ Evidence gate passed - investigation has sufficient external validation"
            )

    except Exception as e:
        logger.warning(
            f"Evidence gate check failed, allowing investigation to proceed: {e}"
        )
...
def _generate_action_plan(state: Dict[str, Any], final_risk: float) -> None:
    ...
    try:
        ...
        if actions:
            logger.info(
                f"🎯 Action plan generated: {len(actions)} recommendations for risk {fmt_num(final_risk, 3)}"
            )
...
def _apply_anti_flap_guard(
    entity_id: str, final_risk: float, state: Dict[str, Any]
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if flap_result.get("flap_detected"):
            ...
            state.setdefault("investigation_warnings", []).append(
                "Risk flapping detected and limited"
            )
        else:
