...
class DomainAnalysisHandler:
    ...
    def _should_complete_domain_analysis(self, state: InvestigationState) -> bool:
        ...
        self._log_domain_safety_check(domains_completed, orchestrator_loops, min_domains_required, max_orchestrator_loops)

        should_complete = (len(domains_completed) >= min_domains_required or
                          orchestrator_loops >= max_orchestrator_loops)

        if should_complete:
            triggered_conditions = self._get_completion_triggers(domains_completed, min_domains_required, orchestrator_loops, max_orchestrator_loops)
            logger.debug(f"[SAFETY-CHECK-5]   ✅ TRIGGERED: Domain analysis completion - {', '.join(triggered_conditions)}")
            logger.info(f"✅ Domain analysis sufficient: {len(domains_completed)} domains, {orchestrator_loops} loops - moving to summary")
            logger.info(f"🎯 ORCHESTRATOR: Force completing domain analysis - reason: {', '.join(triggered_conditions)}")
            return True

        return False

    def _log_domain_safety_check(self, domains_completed: List[str], orchestrator_loops: int, min_domains_required: int, max_orchestrator_loops: int):
    ...
    def _get_completion_triggers(self, domains_completed: List[str], min_domains_required: int, orchestrator_loops: int, max_orchestrator_loops: int) -> List[str]:
        """Get list of completion triggers."""
        triggered_conditions = []

        if len(domains_completed) >= min_domains_required:
            triggered_conditions.append(f"sufficient_domains({len(domains_completed)}>={min_domains_required})")
