...
class LoggerUtilities:
    ...
    @staticmethod
    def log_tool_execution_interaction(state: Dict[str, Any], messages: List, snowflake_data: Dict[str, Any], tools_count: int):
        ...
        if snowflake_data:
            ...
            if isinstance(snowflake_data, dict) and 'results' in snowflake_data:
                ...
                logger.debug(f"   📊 Snowflake context available: Yes ({len(results)} records)")
    ...
    def log_response_analysis(response, tools_used: List[str]):
        ...
        if hasattr(response, 'content'):
            logger.debug(f"   📄 Response content length: {len(str(response.content))} chars")
        ...
        if hasattr(response, 'tool_calls') and response.tool_calls:
            ...
            logger.debug(f"   ✅ Reasoning successful: Generated {len(response.tool_calls)} new tool calls (total tools used: {len(tools_used)})")
        else:
            logger.debug(f"   🔧 Tool calls present: No")
            logger.debug(f"   💭 Reasoning decision: No additional tools needed (current tools used: {len(tools_used)})")