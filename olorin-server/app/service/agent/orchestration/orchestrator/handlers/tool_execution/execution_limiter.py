...
class ToolExecutionLimiter:
    ...
    @staticmethod
    def should_exit_mock_mode(snowflake_data: Dict[str, Any], tools_used: List[str]) -> bool:
    ...
    def should_exit_due_to_limits(state: InvestigationState) -> bool:
        ...
        ToolExecutionLimiter._log_safety_check(tool_execution_attempts, max_attempts, tools_used, max_tools, orchestrator_loops, max_orchestrator_loops)

        should_exit = (tool_execution_attempts >= max_attempts or
                      len(tools_used) >= max_tools or
                      orchestrator_loops >= max_orchestrator_loops)

        if should_exit:
            triggered_conditions = ToolExecutionLimiter._get_triggered_conditions(tool_execution_attempts, max_attempts, tools_used, max_tools, orchestrator_loops, max_orchestrator_loops)
            logger.debug(f"[SAFETY-CHECK-3]   ✅ TRIGGERED: Tool execution limit reached - {', '.join(triggered_conditions)}")
    ...
    @staticmethod
    def _log_safety_check(tool_execution_attempts: int, max_attempts: int, tools_used: List[str], max_tools: int, orchestrator_loops: int, max_orchestrator_loops: int):
    ...
    @staticmethod
    def _get_triggered_conditions(tool_execution_attempts: int, max_attempts: int, tools_used: List[str], max_tools: int, orchestrator_loops: int, max_orchestrator_loops: int) -> List[str]:
        """Get list of triggered exit conditions."""
        triggered_conditions = []

        if tool_execution_attempts >= max_attempts:
            triggered_conditions.append(f"max_attempts({tool_execution_attempts}>={max_attempts})")
        ...
        if orchestrator_loops >= max_orchestrator_loops:
            triggered_conditions.append(f"max_loops({orchestrator_loops}>={max_orchestrator_loops})")
