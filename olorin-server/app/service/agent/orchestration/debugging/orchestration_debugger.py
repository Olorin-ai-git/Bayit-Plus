...
class OrchestrationDebugger:
    ...
    def _analyze_state(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        state_analysis = {
            "schema_type": "InvestigationState" if "current_phase" in state else "Unknown",
        ...
        required_fields = [
            "investigation_id", "entity_id", "entity_type", "current_phase",
            "snowflake_data", "snowflake_completed", "tool_results", 
            "domain_findings", "domains_completed"
        ]
        
        for field in required_fields:
            state_analysis["required_fields_present"][field] = field in state
            if field not in state:
                state_analysis["state_field_issues"].append(f"Missing required field: {field}")
        ...
        state_analysis["data_flow_status"] = {
            "snowflake_to_tools": bool(state.get("snowflake_data")) and len(state.get("tools_used", [])) > 0,
            "tools_to_domains": len(state.get("tool_results", {})) > 0 and len(state.get("domain_findings", {})) > 0,
            "domains_to_completion": len(state.get("domain_findings", {})) > 0 and state.get("current_phase") in ["summary", "complete"]
        }
    ...
    def _analyze_routing_logic(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        state_fields = ["domains_completed", "snowflake_completed"]  # These should match
        ...
        routing_analysis["routing_conditions"] = {
            ...
            "should_route_to_domains": snowflake_completed and tools_used >= 3 and domains_completed < 6,
    ...
    def _analyze_flow_gaps(
        ...
        if journey_data:
            flow_analysis["execution_tracking"] = {
                "total_nodes_executed": journey_data.get("journey_metadata", {}).get("total_nodes_executed", 0),
                "total_state_transitions": journey_data.get("journey_metadata", {}).get("total_state_transitions", 0),
                "execution_path": journey_data.get("journey_metadata", {}).get("execution_path", []),
        ...
        if snowflake_data and not tool_results:
            flow_analysis["flow_bottlenecks"].append("Snowflake data collected but no tools executed")
        
        if tool_results and not domain_findings:
            flow_analysis["flow_bottlenecks"].append("Tools executed but no domain analysis performed")
        ...
        flow_analysis["termination_analysis"] = {
            ...
            "investigation_success": bool(domain_findings) and len(domain_findings) >= 3
        }
    ...
    def _detect_flow_gap(self, state: Dict[str, Any]) -> bool:
        ...
        if snowflake_data and not tool_results and current_phase in ["summary", "complete"]:
            return True
        
        # Gap 2: Tools executed but domain analysis skipped
        if tool_results and not domain_findings and current_phase in ["summary", "complete"]:
            return True
    ...
    def _generate_recommendations(self, state: Dict[str, Any]) -> List[str]:
        ...
        if "snowflake_completed" not in state:
            recommendations.append("Add snowflake_completed field tracking to state schema")
        
        if "domains_completed" not in state:
            recommendations.append("Add domains_completed field tracking to state schema")
        
        # Routing logic recommendations
        if len(state.get("tool_results", {})) > 0 and len(state.get("domain_findings", {})) == 0:
            recommendations.append("CRITICAL: Fix routing logic to ensure domain agents are triggered after tool execution")
        
        # Flow control recommendations
        current_phase = state.get("current_phase", "unknown")
        if current_phase in ["summary", "complete"] and len(state.get("domain_findings", {})) == 0:
            recommendations.append("CRITICAL: Prevent premature investigation completion - require domain analysis")
        
        # Data flow recommendations
        if not state.get("snowflake_data") and current_phase != "initialization":
            recommendations.append("Fix Snowflake data collection - ensure data is retrieved before analysis")
