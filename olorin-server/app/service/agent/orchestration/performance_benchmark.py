...
class InvestigationPerformanceBenchmark:
    ...
    async def benchmark_investigation_types(
        ...
        for inv_type in investigation_types:
            ...
            result = await self._run_single_benchmark(
                graph_builder, inv_type, test_data, config
            )
    ...
    async def _run_single_benchmark(
        ...
        benchmark_id = (
            f"{investigation_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
    ...
    def _generate_test_data(
        self, investigation_type: str, config: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    def update_baseline(
        self, investigation_type: str, result: BenchmarkResult, version: str = "1.0"
    ):
        """
    ...
    def _generate_recommendations(self, result: BenchmarkResult) -> List[str]:
        ...
        if result.metrics.get("avg_latency", 0) > 10:
            recommendations.append(
                "High average latency detected - consider parallel execution"
            )
        ...
        if result.metrics.get("error_rate", 0) > 0.05:
            recommendations.append(
                "High error rate - improve error handling and retry logic"
            )
...
class ContinuousPerformanceMonitor:
        ...
        if "latency" in current_metrics:
            ...
            if recent_latencies:
                ...
                if (
                    ...
                    > avg_recent * self.alert_thresholds["latency_spike"]
                ):
                    alerts.append(
                        f"Latency spike detected: {current_metrics['latency']:.2f}s"
                    )
        ...
        if "memory_mb" in current_metrics:
            ...
            if len(recent_memory) >= 2:
                ...
                if memory_growth > self.alert_thresholds["memory_leak"]:
                    alerts.append(
                        f"Potential memory leak: {memory_growth:.0f} MB growth"
                    )
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "time_window": self.metrics_buffer[-1]["timestamp"]
            - self.metrics_buffer[0]["timestamp"],
        }
