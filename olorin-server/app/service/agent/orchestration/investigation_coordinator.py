...
async def start_investigation(state: MessagesState, config) -> dict:
    ...
    agent_context = rehydrate_agent_context(
        get_config_value(config, ["configurable", "agent_context"])
    )
    ...
    if agent_context is not None:
        ...
        investigation_id = metadata.get("investigation_id") or metadata.get(
            "investigationId"
        )
        ...
        logger.info(
            "[start_investigation] Using hybrid state structure - extracting from state"
        )
        ...
        logger.info(
            f"🔍 Hybrid state parameters: entity_id={entity_id}, entity_type={entity_type}, investigation_id={investigation_id}"
        )
    ...
    if agent_context is not None:
        agent_context.metadata.additional_metadata["investigation_id"] = (
            investigation_id
        )
    ...
    if (
        ...
        and hasattr(agent_context, "metadata")
        and agent_context.metadata
    ):
        additional_metadata = getattr(agent_context.metadata, "additional_metadata", {})
        csv_data = additional_metadata.get("csv_data") or additional_metadata.get(
            "file_content"
        )
...
async def update_investigation_with_raw_data(
    investigation_id: str, raw_data_results: dict
) -> None:
    """
    ...
    try:
        ...
        update_data = InvestigationUpdate(id=investigation_id)
        ...
        logger.error(
            f"Failed to update investigation {investigation_id} with raw data: {e}"
        )
