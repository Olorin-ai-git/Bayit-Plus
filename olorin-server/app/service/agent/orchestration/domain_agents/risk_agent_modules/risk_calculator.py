    ...
    def calculate_real_risk_score(
        ...
        if isinstance(device_findings, dict):
            ...
            if any("NULL" in str(ev) or "not available" in str(ev).lower() or
                ...
                self.logger.info(f"🚨 Device data missing - adding {device_penalty:.3f} risk penalty")
        ...
        if not weighted_scores:
            ...
            if transaction_data and total_amount > 0:
                ...
                self.logger.warning(f"⚠️ No domain scores available, using volume-weighted transaction risk: {volume_weighted_risk:.3f}")
                return volume_weighted_risk
            else:
                raise ValueError("CRITICAL: Insufficient REAL data for risk calculation")
        ...
        if transaction_data and total_amount > 0:
            ...
            self.logger.info(f"📊 Volume weighting: domain_risk={weighted_avg:.3f}, tx_risk={volume_weighted_tx_risk:.3f}, blended={blended_risk:.3f}")
