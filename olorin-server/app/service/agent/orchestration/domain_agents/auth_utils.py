...
def analyze_login_attempts(results: list, findings: Dict[str, Any]) -> None:
    ...
    if login_attempts:
        ...
        findings["evidence"].append(
            f"Login attempts: max={max_attempts}, avg={avg_attempts:.1f}"
        )

        if max_attempts > 20:
            findings["risk_indicators"].append(
                f"Brute force detected: {max_attempts} login attempts"
            )
            ...
            findings["evidence"].append(
                f"CRITICAL: {max_attempts} login attempts indicates brute force attack"
            )
            ...
            findings["evidence"].append(
                f"SUSPICIOUS: {max_attempts} login attempts may indicate attack"
            )
...
def analyze_failed_login_ratios(results: list, findings: Dict[str, Any]) -> None:
    ...
    if failed_ratios:
        ...
        findings["evidence"].append(
            f"Failed login ratios: max={max_failed_ratio:.1%}, avg={avg_failed_ratio:.1%}"
        )

        if max_failed_ratio > 0.8:
            findings["risk_indicators"].append(
                f"High failure rate: {max_failed_ratio:.1%}"
            )
            # CRITICAL: Only modify risk_score if it exists (no fallback scores)
            current_score = findings.get("risk_score")
            if current_score is not None:
            ...
            findings["evidence"].append(
                f"CRITICAL: {max_failed_ratio:.1%} failure rate indicates potential attack"
            )
        elif max_failed_ratio > 0.5:
            findings["risk_indicators"].append(
                f"Moderate failure rate: {max_failed_ratio:.1%}"
            )
            # CRITICAL: Only modify risk_score if it exists (no fallback scores)
            current_score = findings.get("risk_score")
            if current_score is not None:
            ...
            findings["evidence"].append(
                f"SUSPICIOUS: {max_failed_ratio:.1%} failure rate above normal"
            )
...
def analyze_security_indicators(results: list, findings: Dict[str, Any]) -> None:
    ...
    if mfa_bypass_attempts:
        findings["risk_indicators"].append(
            f"MFA bypass attempts detected: {len(mfa_bypass_attempts)}"
        )
        # CRITICAL: Only modify risk_score if it exists (no fallback scores)
        current_score = findings.get("risk_score")
        if current_score is not None:
        ...
        findings["evidence"].append(
            f"CRITICAL: {len(mfa_bypass_attempts)} MFA bypass attempts detected"
        )
    ...
    if travel_scores:
        ...
        findings["risk_indicators"].append(
            f"Impossible travel detected (confidence: {max_travel_score:.2f})"
        )
        # CRITICAL: Only modify risk_score if it exists (no fallback scores)
        current_score = findings.get("risk_score")
        if current_score is not None:
            findings["risk_score"] = current_score + 0.6
        findings["evidence"].append(
            f"CRITICAL: Impossible travel with {max_travel_score:.2f} confidence"
        )
    ...
    if stuffing_indicators:
        findings["risk_indicators"].append(
            f"Credential stuffing detected: {len(stuffing_indicators)} batches"
        )
        # CRITICAL: Only modify risk_score if it exists (no fallback scores)
        current_score = findings.get("risk_score")
        if current_score is not None:
            findings["risk_score"] = current_score + 0.5
        findings["evidence"].append(
            f"SUSPICIOUS: {len(stuffing_indicators)} credential stuffing batches"
        )
        findings["metrics"]["credential_stuffing_batches"] = len(stuffing_indicators)
        findings["analysis"]["credential_stuffing_indicators"] = len(
            stuffing_indicators
        )
    ...
    if sim_swap_indicators:
        findings["risk_indicators"].append(
            f"SIM swap indicators: {len(sim_swap_indicators)}"
        )
        # CRITICAL: Only modify risk_score if it exists (no fallback scores)
        current_score = findings.get("risk_score")
        if current_score is not None:
...
def analyze_auth_threat_intelligence(
    ...
    logger.debug(
        f"[Step 5.2.5.2] 🔍 Category-based authentication analysis: Processing {len(tool_results)} tools"
    )
    ...
    for tool_name, result in tool_results.items():
        ...
        if auth_signals:
            logger.debug(
                f"[Step 5.2.5.2]   ✅ {tool_name}: Found {len(auth_signals)} authentication signals"
            )
            _process_auth_signals(tool_name, auth_signals, findings)
        else:
            logger.debug(
                f"[Step 5.2.5.2]   ➖ {tool_name}: No authentication intelligence signals detected"
            )
...
def _extract_auth_signals(tool_name: str, result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.debug(
        f"[Step 5.2.5.2] 🔍 Extracting authentication signals from {tool_name} with {len(result)} top-level fields"
    )
    ...
    for indicator in auth_indicators:
        if indicator in result:
            ...
            logger.debug(
                f"[Step 5.2.5.2]     → Found auth indicator: {indicator} = {result[indicator]}"
            )
    ...
    for indicator in auth_score_indicators:
        if indicator in result:
            try:
                ...
                logger.debug(
                    f"[Step 5.2.5.2]     → Found auth score: {indicator} = {result[indicator]}"
                )
            except (ValueError, TypeError):
                logger.debug(
                    f"[Step 5.2.5.2]     → Skipped non-numeric score: {indicator} = {result[indicator]}"
                )
                pass
        ...
        if isinstance(value, dict):
            ...
            for i, item in enumerate(value[:5]):  # Limit to first 5 items
                if isinstance(item, dict):
                    nested_signals = _extract_auth_signals(
    ...
    logger.debug(
        f"[Step 5.2.5.2] ✅ Extracted {len(signals)} authentication signals from {tool_name}"
    )
    return signals


def _process_auth_signals(
    tool_name: str, signals: Dict[str, Any], findings: Dict[str, Any]
) -> None:
    """Process extracted authentication signals to adjust risk score."""

    logger.debug(
        f"[Step 5.2.5.3] 🔍 Processing {len(signals)} authentication signals from {tool_name}"
    )
        ...
        if key.startswith("auth_") and value:
            if value is True or str(value).lower() in [
                ...
                if any(
                    ...
                    threat in key
                    for threat in ["mfa_bypass", "session_hijacking", "token_abuse"]
                ):
    ...
    for key, value in signals.items():
        if key.startswith("score_") and isinstance(value, (int, float)):
            ...
            if normalized_score > 0.7:  # High authentication risk
                ...
                findings["evidence"].append(
                    f"{tool_name}: Normal authentication behavior {key} = {value}"
                )
    ...
    if current_score is not None:
        if auth_risk_level > 0.5:
            ...
            findings["risk_indicators"].append(
                f"{tool_name}: Authentication threats detected (level: {auth_risk_level:.2f})"
            )
        elif auth_risk_level < -0.15:
            # Normal authentication behavior - reduce risk
            risk_multiplier = 1.0 + max(
                -0.1, auth_risk_level * 0.15
            )  # auth_risk_level is negative
            findings["risk_score"] = max(0.1, current_score * risk_multiplier)
            findings["evidence"].append(
                f"{tool_name}: Authentication behavior appears normal (level: {auth_risk_level:.2f})"
            )
    ...
    if evidence_count > 0:
        ...
        logger.debug(
            f"[Step 5.2.5.3]   ✅ {tool_name}: Processed {evidence_count} authentication signals, risk level: {auth_risk_level:.2f}"
        )
    else:
        logger.debug(
            f"[Step 5.2.5.3]   ➖ {tool_name}: No actionable authentication signals found"
        )
