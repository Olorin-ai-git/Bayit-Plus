...
async def location_agent_node(
    ...
    if results:
        ...
        if isinstance(snowflake_data, str):
            location_findings["risk_indicators"].append(
    ...
    if risk_score is not None:
        logger.info(
            f"[Step 5.2.3] ✅ Location analysis complete - Risk: {risk_score:.2f}"
        )
    else:
        logger.info(
            f"[Step 5.2.3] ✅ Location analysis complete - Risk: INSUFFICIENT_DATA"
        )
...
def _analyze_impossible_travel(results: list, findings: Dict[str, Any]) -> None:
    ...
    if len(locations_by_time) < 2:
        findings["evidence"].append(
            "Insufficient location data for impossible travel analysis"
        )
        return
    ...
    unique_countries = len(
        set(loc["country"] for loc in locations_by_time if loc["country"])
    )
            ...
            if prev_coords and curr_coords and hours > 0:
                ...
                if is_impossible:
                    ...
                    findings["risk_indicators"].append(
                        f"IMPOSSIBLE TRAVEL: {prev_loc['country']}→{curr_loc['country']} in {minutes:.1f} min (requires {speed_mph:.0f} mph)"
                    )
                    findings["evidence"].append(
                        f"CRITICAL: Impossible travel {prev_loc['country']}→{curr_loc['country']} in {minutes:.1f} minutes requiring {speed_mph:.0f} mph (max possible ~600 mph)"
                    )

                elif is_highly_suspicious:
                    findings["risk_indicators"].append(
                        f"SUSPICIOUS TRAVEL: {prev_loc['country']}→{curr_loc['country']} requires {speed_mph:.0f} mph"
                    )
                    findings["evidence"].append(
                        f"SUSPICIOUS: Very fast travel {prev_loc['country']}→{curr_loc['country']} in {hours:.1f}h requiring {speed_mph:.0f} mph"
                    )

                else:
                    findings["evidence"].append(
                        f"Geographic movement: {prev_loc['country']}→{curr_loc['country']} in {hours:.1f}h ({speed_mph:.0f} mph - feasible)"
                    )
    ...
    if impossible_travels:
        ...
        if "llm_risk_score" not in findings:
            ...
            if current_score is not None:
                findings["risk_score"] = max(current_score, 0.8)
        findings["evidence"].append(
            f"FRAUD INDICATOR: {len(impossible_travels)} impossible travel pattern(s) detected"
        )
        ...
        findings["evidence"].append(
            f"GEOGRAPHIC SPREAD: Activity from {unique_countries} different countries"
        )
...
def _analyze_high_risk_countries(results: list, findings: Dict[str, Any]) -> None:
    ...
    if risk_countries_found:
        ...
        findings["evidence"].append(
            f"Activity from high-risk regions: {list(risk_countries_found)}"
        )
...
def _analyze_geographic_diversity(results: list, findings: Dict[str, Any]) -> None:
    ...
    if not countries and not cities:
        logger.debug(
            "[Step 5.2.3.1] 🔄 Snowflake geographic data empty, attempting extraction from tool results"
        )
    ...
    if countries or cities:
        findings["evidence"].append(
            f"Geographic spread: {len(cities)} cities across {len(countries)} countries"
        )
    else:
        findings["evidence"].append(
    ...
    if len(countries) > 5:
        findings["risk_indicators"].append(
            f"High country diversity: {len(countries)} countries"
        )
        findings["evidence"].append(
            f"SUSPICIOUS: Activity spans {len(countries)} different countries"
        )
        # CRITICAL FIX: Only modify risk_score if LLM hasn't set it yet AND risk_score is not None
        if "llm_risk_score" not in findings:
            current_score = findings.get("risk_score")
    ...
    if len(cities) > 10:
        ...
        findings["evidence"].append(
            f"SUSPICIOUS: Activity across {len(cities)} different cities"
        )
        # CRITICAL FIX: Only modify risk_score if LLM hasn't set it yet AND risk_score is not None
        if "llm_risk_score" not in findings:
            current_score = findings.get("risk_score")
...
def _analyze_geolocation_intelligence(
    tool_results: Dict[str, Any], findings: Dict[str, Any]
) -> None:
    """Analyze geolocation and travel intelligence from any tool that provides location data."""

    logger.debug(
        f"[Step 5.2.3.2] 🔍 Category-based location analysis: Processing {len(tool_results)} tools"
    )
    ...
    for tool_name, result in tool_results.items():
        ...
        if location_signals:
            logger.debug(
                f"[Step 5.2.3.2]   ✅ {tool_name}: Found {len(location_signals)} location signals"
            )
            _process_location_signals(tool_name, location_signals, findings)
        else:
            logger.debug(
                f"[Step 5.2.3.2]   ➖ {tool_name}: No geolocation intelligence signals detected"
            )
...
def _extract_location_signals(tool_name: str, result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.debug(
        f"[Step 5.2.3.2] 🔍 Extracting location signals from {tool_name} with {len(result)} top-level fields"
    )
    ...
    for indicator in location_indicators:
        if indicator in result:
            ...
            logger.debug(
                f"[Step 5.2.3.2]     → Found location indicator: {indicator} = {result[indicator]}"
            )
    ...
    for indicator in location_risk_indicators:
        if indicator in result:
            try:
                ...
                logger.debug(
                    f"[Step 5.2.3.2]     → Found location risk score: {indicator} = {result[indicator]}"
                )
            except (ValueError, TypeError):
                logger.debug(
                    f"[Step 5.2.3.2]     → Skipped non-numeric score: {indicator} = {result[indicator]}"
                )
                pass
        ...
        if isinstance(value, dict):
            ...
            for i, item in enumerate(value[:5]):  # Limit to first 5 items
                if isinstance(item, dict):
                    nested_signals = _extract_location_signals(
                        f"{tool_name}_{key}_{i}", item
                    )
                    signals.update(nested_signals)
                    if nested_signals:
                        nested_count += 1
    ...
    logger.debug(
        f"[Step 5.2.3.2] ✅ Extracted {len(signals)} location signals from {tool_name}"
    )
    return signals


def _process_location_signals(
    tool_name: str, signals: Dict[str, Any], findings: Dict[str, Any]
) -> None:
    """Process extracted location signals to adjust risk score."""

    logger.debug(
        f"[Step 5.2.3.3] 🔍 Processing {len(signals)} location signals from {tool_name}"
    )
    ...
    for key, value in signals.items():
        if key.startswith("location_") and value:
            ...
            # Check for specific high-risk countries or regions
                ...
                risk_term in value.lower()
                for risk_term in ["tor", "proxy", "vpn", "hosting"]
            ):
                location_risk_level += 0.15
                evidence_count += 1
                findings["evidence"].append(
                    f"{tool_name}: Suspicious location type {key} = {value}"
                )
    ...
    if "llm_risk_score" in findings:
        ...
        logger.debug(
            f"[Step 5.2.3.3]   ℹ️ LLM risk score already set ({findings.get('llm_risk_score', 'N/A')}), skipping location risk adjustments"
        )
        # Still add indicators to evidence, but don't modify score
        if location_risk_level > 0.4:
            findings["risk_indicators"].append(
                f"{tool_name}: High-risk location detected (level: {location_risk_level:.2f})"
            )
        elif location_risk_level < -0.15:
            findings["evidence"].append(
                f"{tool_name}: Location appears safe (level: {location_risk_level:.2f})"
            )
    else:
        # Pre-LLM analysis: Apply risk adjustment based on location assessment
        ...
        if current_score is not None:
            if location_risk_level > 0.4:
                ...
                risk_multiplier = 1.0 + max(
                    -0.08, location_risk_level * 0.1
                )  # location_risk_level is negative
    ...
    if evidence_count > 0:
        ...
        logger.debug(
            f"[Step 5.2.3.3]   ✅ {tool_name}: Processed {evidence_count} location signals, risk level: {location_risk_level:.2f}"
        )
    else:
        logger.debug(
            f"[Step 5.2.3.3]   ➖ {tool_name}: No actionable location signals found"
        )
...
def _extract_geographic_data_from_findings(findings: Dict[str, Any]) -> tuple[set, set]:
    ...
    logger.debug(
        f"[Step 5.2.3.1] 📍 Extracted from findings: {len(countries)} countries, {len(cities)} cities"
    )
