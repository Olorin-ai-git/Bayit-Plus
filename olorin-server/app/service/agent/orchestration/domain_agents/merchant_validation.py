...
class MerchantValidationService:
    ...
    def _get_postgres_engine(self) -> Optional[Engine]:
        ...
        if self._postgres_engine is None:
            ...
            postgres_db = os.getenv("POSTGRES_DATABASE") or os.getenv(
                "POSTGRES_DB", "olorin_db"
            )
            ...
            if not postgres_password:
                logger.warning(
                    "⚠️ Postgres password not configured, merchant risk level enrichment will be skipped"
                )
                return None
            ...
            try:
                ...
                logger.info(
                    "✅ Postgres engine initialized for merchant risk level queries"
                )
    ...
    async def run_validation(
        ...
        try:
            ...
            logger.info(
                f"🔍 Running merchant validation for investigation {investigation_id}"
            )
            ...
            validation_results = self._validate_predictions(
                predicted_risk, predicted_confidence, actual_outcomes, merchant_findings
            )
            ...
            if investigation_folder:
                await self._save_validation_results(
                    validation_results, investigation_folder
                )

            logger.info(f"✅ Merchant validation completed")
            logger.info(
                f"   Prediction accuracy: {validation_results.get('prediction_correct', False)}"
            )
            logger.info(
    ...
    def _validate_predictions(
        ...
        return {
            ...
            "validation_quality": self._assess_validation_quality(
                risk_correlation_error, prediction_correct, actual_total
            ),
        }
    ...
    async def _enrich_with_merchant_risk_levels(
        self, snowflake_results: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
        ...
        if not postgres_engine:
            logger.warning(
                "⚠️ Postgres engine not available, merchant risk levels will be set to 'UNKNOWN'"
            )
        ...
        merchant_names_list = "', '".join(
            [name.replace("'", "''") for name in merchant_names]
        )
        ...
        try:
            with postgres_engine.connect() as conn:
                ...
                if not column_exists:
                    logger.debug(
                        "⚠️ merchant_risk_level column does not exist in transactions_enriched table, skipping enrichment"
                    )
                ...
                for result in snowflake_results:
                    merchant_name = result.get("MERCHANT_NAME") or result.get(
                        "merchant_name"
                    )
                ...
                logger.debug(
                    f"✅ Enriched {len(snowflake_results)} transactions with merchant risk levels from Postgres"
                )
                return snowflake_results

        except Exception as e:
            logger.warning(
                f"⚠️ Failed to fetch merchant risk levels from Postgres (column may not exist): {e}"
            )
    ...
    async def _save_validation_results(
        self, validation_results: Dict[str, Any], investigation_folder: Path
    ) -> None:
        """Save validation results to investigation folder."""
        try:
