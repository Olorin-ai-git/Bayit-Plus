...
from app.service.agent.tools.snowflake_tool.schema_constants import (
    MERCHANT_NAME, MERCHANT_SEGMENT_ID,  # MERCHANT_RISK_LEVEL not in schema_constants - handled dynamically
    MERCHANT_DECISIONS, MERCHANT_LAST_DECISION, COUNT_MERCHANT_DECISIONS,
    MERCHANT_LAST_DECISION_DATETIME, DAYS_FROM_FIRST_MERCHANT_ACCEPTANCE_TO_TX,
    TX_DATETIME, IS_FRAUD_TX, MODEL_SCORE, PAID_AMOUNT_VALUE_IN_CURRENCY
)
    ...
    try:
        ...
        if results:
            ...
            if results and isinstance(results[0], dict):
                ...
                merchant_fields = [k for k in results[0].keys() if 'MERCHANT' in k.upper() or 'merchant' in k.lower()]
            ...
            if len(merchant_findings.get('evidence', [])) == 0:
                logger.warning("⚠️ No evidence collected from merchant analysis, adding basic transaction count")
                merchant_findings["evidence"].append(f"Merchant data analyzed: {len(results)} records processed")
                merchant_findings["evidence"].append("Merchant analysis completed on transaction dataset")
            
            logger.info(f"📊 Evidence collected from Snowflake data: {len(merchant_findings.get('evidence', []))} items")
        else:
            logger.warning(f"⚠️ Merchant agent: No results extracted from snowflake_data")
            if isinstance(snowflake_data, str):
                merchant_findings["risk_indicators"].append("Snowflake data in non-structured format")
        ...
        DomainAgentBase.finalize_findings(
            merchant_findings, snowflake_data, tool_results, analysis_duration, "merchant"
        )
        ...
        logger.info(f"[Step 5.2.7] ✅ Merchant analysis complete - Evidence collected: {len(merchant_findings['evidence'])} points")
...
def _analyze_merchant_risk_distribution(results: List[Dict], findings: Dict[str, Any]) -> None:
    ...
    for r in results:
        ...
        risk_level = r.get("MERCHANT_RISK_LEVEL") or r.get("merchant_risk_level") or "UNKNOWN"
        ...
        # Track amounts
        amount = r.get("PAID_AMOUNT_VALUE_IN_CURRENCY") or r.get("paid_amount_value_in_currency") or 0.0
        try:
            merchant_names[merchant_name]["total_amount"] += float(amount) if amount else 0.0
    ...
    high_risk_count = merchant_risk_levels.get("high", 0) + merchant_risk_levels.get("HIGH", 0)
    medium_risk_count = merchant_risk_levels.get("medium", 0) + merchant_risk_levels.get("MEDIUM", 0)
    low_risk_count = merchant_risk_levels.get("low", 0) + merchant_risk_levels.get("LOW", 0)
    
    findings["evidence"].append(f"Unique merchants: {len(merchant_names)} across {total_transactions} transactions")
    findings["evidence"].append(f"Merchant risk distribution: High={high_risk_count}, Medium={medium_risk_count}, Low={low_risk_count}")
    
    # Risk indicators
    if high_risk_count > 0:
        high_risk_pct = (high_risk_count / total_transactions) * 100 if total_transactions > 0 else 0
        findings["evidence"].append(f"High-risk merchant transactions: {high_risk_count} ({high_risk_pct:.1f}%)")
        
        if high_risk_pct > 50:
            findings["risk_indicators"].append(f"High concentration of high-risk merchant transactions ({high_risk_pct:.1f}%)")
            findings["evidence"].append(f"SUSPICIOUS: >50% transactions with high-risk merchants")
    
    # Identify merchants with fraud
    fraud_merchants = {name: data for name, data in merchant_names.items() if data["fraud_count"] > 0}
    if fraud_merchants:
        findings["evidence"].append(f"Merchants with fraud: {len(fraud_merchants)}")
        findings["risk_indicators"].append(f"{len(fraud_merchants)} merchants associated with fraud transactions")
        
        # List top fraud merchants
        top_fraud_merchants = sorted(fraud_merchants.items(), key=lambda x: x[1]["fraud_count"], reverse=True)[:5]
        for name, data in top_fraud_merchants:
            findings["evidence"].append(f"  - {name}: {data['fraud_count']} fraud transactions, risk={data['risk_level']}")
    
    findings["analysis"]["merchant_risk_distribution"] = merchant_risk_levels
    findings["analysis"]["merchant_names"] = {name: {"count": data["count"], "risk_level": data["risk_level"]} 
...
def _analyze_merchant_velocity(results: List[Dict], findings: Dict[str, Any]) -> None:
    ...
    if not merchant_timestamps:
        findings["evidence"].append("Merchant velocity analysis: No timestamp data available")
        return
    ...
    if len(merchant_timestamps) > 1:
        ...
        for timestamps in merchant_timestamps.values():
            all_timestamps.extend([(ts, merchant) for merchant, timestamps_list in merchant_timestamps.items() 
                                  for ts in timestamps_list if ts in timestamps_list])
        ...
        findings["metrics"]["merchants_per_transaction"] = unique_merchants / total_transactions if total_transactions > 0 else 0
        
        findings["evidence"].append(f"Merchant velocity: {unique_merchants} unique merchants across {total_transactions} transactions")
        
        if rapid_switches > 0:
            findings["risk_indicators"].append(f"Rapid merchant switching detected: {rapid_switches} instances of 3+ merchants within 1 hour")
            findings["evidence"].append(f"SUSPICIOUS: Rapid merchant switching pattern detected")
        
        # First-time merchant usage
        if len(merchant_timestamps) > 1:
            first_merchant = min(merchant_timestamps.items(), key=lambda x: min(x[1]))[0]
            findings["evidence"].append(f"First merchant used: {first_merchant}")
            
            # Check if many merchants are used only once
            single_use_merchants = sum(1 for timestamps in merchant_timestamps.values() if len(timestamps) == 1)
            if single_use_merchants > unique_merchants * 0.5:
                findings["risk_indicators"].append(f"High single-use merchant rate: {single_use_merchants}/{unique_merchants} merchants used only once")
                findings["evidence"].append(f"SUSPICIOUS: >50% merchants used only once (potential card testing)")
    else:
        findings["evidence"].append(f"Merchant velocity: Single merchant ({list(merchant_timestamps.keys())[0] if merchant_timestamps else 'UNKNOWN'})")
    
    findings["analysis"]["merchant_velocity"] = {
        "unique_merchants": unique_merchants,
        "rapid_switches": findings["metrics"].get("merchant_velocity_rapid_switches", 0)
    }


def _analyze_merchant_category_patterns(results: List[Dict], findings: Dict[str, Any]) -> None:
    ...
    for r in results:
        ...
        category = (r.get("MERCHANT_CATEGORY") or r.get("merchant_category") or 
                   r.get("MERCHANT_CATEGORY_CODE") or r.get("merchant_category_code") or 
                   "UNKNOWN")
        
        merchant_categories[merchant_name] = category
        
        if category not in category_merchants:
            category_merchants[category] = {"merchants": set(), "transactions": 0, "fraud_count": 0}
    ...
    if len(category_merchants) > 1:
        ...
        findings["evidence"].append(f"Merchant categories: {len(category_merchants)} unique categories")
        ...
        for category, data in category_merchants.items():
            if data["transactions"] > 0:
                ...
                if fraud_rate > 0.3:  # >30% fraud rate
                    findings["risk_indicators"].append(f"High fraud rate category: {category} ({fraud_rate*100:.1f}% fraud)")
                    findings["evidence"].append(f"SUSPICIOUS: Category '{category}' has {fraud_rate*100:.1f}% fraud rate")
        
        # Detect rapid category switching
        categories_used = list(set(merchant_categories.values()))
        if len(categories_used) > 3:
            findings["risk_indicators"].append(f"High category diversity: {len(categories_used)} different categories")
            findings["evidence"].append(f"SUSPICIOUS: Transactions span {len(categories_used)} different categories")
        
        findings["analysis"]["merchant_categories"] = {
            category: {
                "merchant_count": len(data["merchants"]),
                "transaction_count": data["transactions"],
                "fraud_rate": data["fraud_count"] / data["transactions"] if data["transactions"] > 0 else 0
            }
            for category, data in category_merchants.items()
        }
    else:
        findings["evidence"].append("Merchant category analysis: Single category or category data not available")
...
def _analyze_merchant_decisions(results: List[Dict], findings: Dict[str, Any]) -> None:
    ...
    for r in results:
        ...
        decision = (r.get("MERCHANT_LAST_DECISION") or r.get("merchant_last_decision") or 
                   r.get("NSURE_LAST_DECISION") or r.get("nsure_last_decision") or "UNKNOWN")
        ...
        merchant_decisions[merchant_name]["decision_counts"][decision] = merchant_decisions[merchant_name]["decision_counts"].get(decision, 0) + 1
    ...
    for merchant_name, data in merchant_decisions.items():
        ...
        if len(set(decisions)) > 1:  # Multiple different decisions
            findings["evidence"].append(f"Merchant {merchant_name}: Multiple decision types {set(decisions)}")
        
        # Check for unusual decision patterns
        accept_count = sum(1 for d in decisions if 'accept' in str(d).lower() or 'approve' in str(d).lower())
        reject_count = sum(1 for d in decisions if 'reject' in str(d).lower() or 'decline' in str(d).lower() or 'block' in str(d).lower())
        
        if reject_count > accept_count * 2:  # More than 2x rejections vs acceptances
            findings["risk_indicators"].append(f"Merchant {merchant_name}: High rejection rate ({reject_count} rejections vs {accept_count} acceptances)")
            findings["evidence"].append(f"SUSPICIOUS: Merchant {merchant_name} has high rejection rate")
...
def _analyze_merchant_fraud_correlation(results: List[Dict], findings: Dict[str, Any]) -> None:
    ...
    for r in results:
        ...
        if model_score is not None:
            try:
                merchant_fraud_data[merchant_name]["model_scores"].append(float(model_score))
            except (ValueError, TypeError):
                pass
        
        try:
            merchant_fraud_data[merchant_name]["amounts"].append(float(amount) if amount else 0.0)
        except (ValueError, TypeError):
            pass
    ...
    for merchant_name, data in merchant_fraud_data.items():
        if data["total"] > 0:
            ...
            avg_model_score = sum(data["model_scores"]) / len(data["model_scores"]) if data["model_scores"] else 0.0
    ...
    if high_fraud_merchants:
        findings["risk_indicators"].append(f"{len(high_fraud_merchants)} merchants with >20% fraud rate")
        findings["evidence"].append(f"High fraud rate merchants: {len(high_fraud_merchants)}")
