...
IP_REPUTATION_CLEAN_VALUES = ["VERY_LOW", "MINIMAL", "LOW"]
...
def apply_clean_intel_veto(
    ...
    if not high_weight_features and risk_score < CLEAN_INTEL_VETO_THRESHOLD:
        ...
        logger.info(f"🔽 Applied clean-intel veto: {risk_score:.3f} → {adjusted_score:.3f} ({reason})")
...
def apply_impossible_travel_hard_block(
    ...
    if max_speed > IMPOSSIBLE_TRAVEL_HARD_BLOCK_THRESHOLD:
        ...
        logger.warning(f"🚨 Applied impossible travel hard block: {risk_score:.3f} → {adjusted_score:.3f} ({reason})")
        return adjusted_score, reason
...
def apply_rule_overrides(
    ...
    blocked_score, block_reason = apply_impossible_travel_hard_block(current_score, advanced_features)
...
def calibrate_risk_score(
    ...
    """
    Calibrate risk score using historical performance data.
    
    This is a placeholder for future model-based calibration.
    For now, applies simple isotonic-style adjustment.
    ...
    """
    # Simple calibration: if historical precision is low, adjust scores upward
    # This is a placeholder - full calibration requires training data
    if historical_performance:
        ...
        logger.debug(f"📊 Calibrated score: {base_score:.3f} → {calibrated_score:.3f} (precision: {precision:.2f})")
