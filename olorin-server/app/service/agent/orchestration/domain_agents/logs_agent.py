...
async def logs_agent_node(
    ...
    if results:
        ...
        if isinstance(snowflake_data, str):
            logs_findings["risk_indicators"].append(
...
def _analyze_failed_transactions(results: list, findings: Dict[str, Any]) -> None:
    ...
    findings["metrics"]["failure_rate"] = (
        len(failed_txs) / total_txs if total_txs > 0 else 0
    )

    findings["evidence"].append(
        f"Transaction failures: {len(failed_txs)} out of {total_txs} transactions"
    )

    if len(failed_txs) > 5:
        findings["risk_indicators"].append(
            f"High number of rejected transactions: {len(failed_txs)}"
        )
        ...
        findings["evidence"].append(
            f"SUSPICIOUS: {len(failed_txs)} rejected transactions indicates potential fraud attempts"
        )

    # Analyze rejection reasons
    rejection_reasons = [
        tx.get("REJECTION_REASON") for tx in failed_txs if tx.get("REJECTION_REASON")
    ]
...
def _analyze_transaction_timing(results: list, findings: Dict[str, Any]) -> None:
    ...
    findings["evidence"].append(
        f"Transaction frequency: {len(tx_times)} transactions analyzed"
    )
    ...
    if len(tx_times) > 10:
        ...
        findings["evidence"].append(
            f"SUSPICIOUS: {len(tx_times)} transactions may indicate automated behavior"
        )

    findings["analysis"]["transaction_timeline"] = tx_times[
        :10
    ]  # Store first 10 for analysis
...
def _analyze_error_patterns(results: list, findings: Dict[str, Any]) -> None:
    ...
    if error_codes:
        findings["evidence"].append(
            f"Error patterns: {len(error_codes)} errors with {len(unique_errors)} unique codes"
        )

    if len(error_codes) > 3:
        findings["risk_indicators"].append(
            f"Multiple error codes detected: {len(unique_errors)}"
        )
        # CRITICAL FIX: Only modify risk_score if LLM hasn't set it yet AND risk_score is not None
        if "llm_risk_score" not in findings:
            current_score = findings.get("risk_score")
        ...
        findings["evidence"].append(
            f"SUSPICIOUS: {len(unique_errors)} different error types may indicate probing behavior"
        )


def _analyze_log_intelligence(
    ...
    logger.debug(
        f"[Step 5.2.4.2] 🔍 Category-based log analysis: Processing {len(tool_results)} tools"
    )
    ...
    for tool_name, result in tool_results.items():
        ...
        if log_signals:
            logger.debug(
                f"[Step 5.2.4.2]   ✅ {tool_name}: Found {len(log_signals)} log signals"
            )
            _process_log_signals(tool_name, log_signals, findings)
        else:
            logger.debug(
                f"[Step 5.2.4.2]   ➖ {tool_name}: No log intelligence signals detected"
            )
...
def _extract_log_signals(tool_name: str, result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.debug(
        f"[Step 5.2.4.2] 🔍 Extracting log signals from {tool_name} with {len(result)} top-level fields"
    )
    ...
    for indicator in log_indicators:
        if indicator in result:
            ...
            logger.debug(
                f"[Step 5.2.4.2]     → Found log indicator: {indicator} = {result[indicator]}"
            )
    ...
    for indicator in log_score_indicators:
        if indicator in result:
            try:
                ...
                logger.debug(
                    f"[Step 5.2.4.2]     → Found log score: {indicator} = {result[indicator]}"
                )
            except (ValueError, TypeError):
                logger.debug(
                    f"[Step 5.2.4.2]     → Skipped non-numeric score: {indicator} = {result[indicator]}"
                )
                pass
        ...
        if isinstance(value, dict):
            ...
            for i, item in enumerate(value[:5]):  # Limit to first 5 items
                if isinstance(item, dict):
                    nested_signals = _extract_log_signals(
    ...
    logger.debug(
        f"[Step 5.2.4.2] ✅ Extracted {len(signals)} log signals from {tool_name}"
    )
    return signals


def _process_log_signals(
    tool_name: str, signals: Dict[str, Any], findings: Dict[str, Any]
) -> None:
    """Process extracted log signals to adjust risk score."""

    logger.debug(
        f"[Step 5.2.4.3] 🔍 Processing {len(signals)} log signals from {tool_name}"
    )
    ...
    for key, value in signals.items():
        if key.startswith("score_") and isinstance(value, (int, float)):
            ...
            if normalized_score > 0.6:  # High log risk
                ...
                findings["evidence"].append(
                    f"{tool_name}: Normal log activity {key} = {value}"
                )
    ...
    if "llm_risk_score" in findings:
        ...
        logger.debug(
            f"[Step 5.2.4.3]   ℹ️ LLM risk score already set ({findings.get('llm_risk_score', 'N/A')}), skipping log risk adjustments"
        )
        # Still add indicators to evidence, but don't modify score
        if log_risk_level > 0.4:
            findings["risk_indicators"].append(
                f"{tool_name}: Suspicious log activity detected (level: {log_risk_level:.2f})"
            )
        elif log_risk_level < -0.1:
            findings["evidence"].append(
                f"{tool_name}: Log activity appears normal (level: {log_risk_level:.2f})"
            )
    else:
        ...
        if current_score is not None:
            if log_risk_level > 0.4:
                ...
                risk_multiplier = 1.0 + max(
                    -0.05, log_risk_level * 0.1
                )  # log_risk_level is negative
    ...
    if evidence_count > 0:
        ...
        logger.debug(
            f"[Step 5.2.4.3]   ✅ {tool_name}: Processed {evidence_count} log signals, risk level: {log_risk_level:.2f}"
        )
    else:
        logger.debug(
            f"[Step 5.2.4.3]   ➖ {tool_name}: No actionable log signals found"
        )
