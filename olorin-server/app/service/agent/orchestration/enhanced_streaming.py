...
class InvestigationStreamer:
    ...
    def __init__(self, graph: CompiledGraph, checkpointer: Optional[BaseCheckpointer] = None):
        """
    ...
    async def stream_investigation(
        ...
        try:
            ...
            completion_metrics = self.performance_tracker.get_investigation_metrics(investigation_id)
    ...
    async def _process_chunk(self, chunk: Dict[str, Any], investigation_id: str) -> Optional[StreamEvent]:
        """
...
class PerformanceTracker:
    ...
    def record_metric(self, investigation_id: str, metric_data: Dict[str, Any]):
        ...
        if "agent" in metric_data:
            ...
            if "duration" in metric_data:
                investigation["agents"][agent_name]["total_duration"] += metric_data["duration"]
        
        # Track tool performance
        if "tool" in metric_data:
            tool_name = metric_data["tool"]
            if tool_name not in investigation["tools"]:
                investigation["tools"][tool_name] = {
                    "calls": 0,
                    "total_duration": 0
                }
            investigation["tools"][tool_name]["calls"] += 1
            if "duration" in metric_data:
                investigation["tools"][tool_name]["total_duration"] += metric_data["duration"]
    ...
    def end_investigation(self, investigation_id: str):
        ...
        if investigation_id in self.investigations:
            ...
            investigation["total_duration"] = investigation["end_time"] - investigation["start_time"]
    ...
    def get_investigation_metrics(self, investigation_id: str) -> Dict[str, Any]:
        ...
        return {
            "total_duration": investigation.get("total_duration", time.time() - investigation["start_time"]),
            "agents_executed": len(investigation["agents"]),
            "tools_called": sum(tool["calls"] for tool in investigation["tools"].values()),
