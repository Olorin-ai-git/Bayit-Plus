...
def finalize_metrics(state: Dict[str, Any]) -> None:
    ...
    state["performance_metrics"]["decisions_per_second"] = safe_div(len(trail), dur_s, 0.0)
    
    # Calculate investigation velocity safely
    domains_completed = len(state.get("domains_completed", []))
    state["performance_metrics"]["investigation_velocity"] = safe_div(domains_completed, dur_s, 0.0)
    ...
    state["performance_metrics"]["resource_efficiency"] = max(0.0, 1.0 - (loop_penalty * 0.1))
...
def finalize_duration_metrics(state: Dict[str, Any]) -> None:
    ...
    state["performance_metrics"]["time_efficiency"] = max(0.1, min(1.0, 1.0 / (1.0 + abs(time_ratio - 1.0))))
...
def finalize_coverage_metrics(state: Dict[str, Any]) -> None:
    ...
    state["performance_metrics"]["domain_completion_percentage"] = safe_div(domains_completed, 6.0, 0.0) * 100.0
    
    # Calculate tool utilization safely
    max_tools = state.get("max_tools", 52)
    state["performance_metrics"]["tool_utilization"] = safe_div(tools_used, max_tools, 0.0) * 100.0
