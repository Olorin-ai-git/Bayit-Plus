...
def score_network_domain(
    ...
    try:
        ...
        is_public = not (
            ip.is_private or ip.is_loopback or ip.is_reserved or ip.is_multicast
        )
    ...
    logger.info(
        f"Network domain assessed: score={result.score}, status={result.status}, signals={len(result.signals)}, is_public={is_public}"
    )
...
def extract_network_signals_from_tools(tool_results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if abuseipdb_result:
        try:
            ...
            data = (
                json.loads(abuseipdb_result)
                if isinstance(abuseipdb_result, str)
                else abuseipdb_result
            )
