...
def filter_domain_fields(domain: str, raw_fields: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for field_name, field_value in raw_fields.items():
        ...
        if field_name in FORBIDDEN_FIELDS:
            ...
            logger.debug(f"🚫 BLOCKED forbidden field '{field_name}' from {domain} domain")
            continue
    ...
    if blocked_fields:
        logger.info(f"🛡️ WHITELIST FILTER ({domain}): Blocked {len(blocked_fields)} non-domain fields: {blocked_fields[:5]}")
        
    logger.debug(f"✅ WHITELIST FILTER ({domain}): Allowed {len(filtered_fields)} domain-specific fields")
...
def validate_domain_metrics(domain: str, metrics: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for metric_name, metric_value in metrics.items():
        ...
        if metric_name not in whitelist:
            violations.append(f"Non-whitelisted field '{metric_name}' in {domain} metrics") 
            continue
...
def log_domain_field_usage(domain: str, fields_used: List[str]) -> None:
    ...
    other_used = [f for f in fields_used if f not in whitelist and f not in FORBIDDEN_FIELDS]
