...
def validate_domain(r: DomainResult) -> None:
    ...
    if r.status == "INSUFFICIENT_EVIDENCE":
        if r.score is not None:
            logger.warning(f"Domain {r.name}: Clearing score {r.score} due to INSUFFICIENT_EVIDENCE status")
        r.score = None
    ...
    if r.score is not None:
        ...
        if r.score != original_score:
            logger.warning(f"Domain {r.name}: Bounded score from {original_score} to {r.score}")
    
    # 3) If no signals, no numeric score (domain contract violation)
    if not r.signals and r.score is not None:
        logger.warning(f"Domain {r.name}: No signals but score={r.score} - marking insufficient evidence")
    ...
    logger.debug(f"Domain {r.name} validated: score={r.score}, status={r.status}, signals={len(r.signals)}")
