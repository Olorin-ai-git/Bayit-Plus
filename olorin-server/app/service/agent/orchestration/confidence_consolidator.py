...
class ConfidenceConsolidator:
    ...
    def consolidate_confidence_scores(
        ...
        try:
            ...
            confidence_values = self._extract_confidence_values(state, agent_results, investigation_context)
            
            # Detect inconsistencies between confidence sources
            consistency_issues = self._detect_confidence_inconsistencies(confidence_values)
            
            # Calculate weighted overall confidence
            overall_confidence = self._calculate_weighted_confidence(confidence_values)
            
            # Assess confidence reliability
            reliability_score = self._assess_confidence_reliability(confidence_values, consistency_issues)
            ...
            evidence_confidence = confidence_values.get(ConfidenceFieldType.EVIDENCE_CONFIDENCE)
            tool_confidence = confidence_values.get(ConfidenceFieldType.TOOL_CONFIDENCE)
            domain_confidence = confidence_values.get(ConfidenceFieldType.DOMAIN_CONFIDENCE)
            ...
            consolidated = ConsolidatedConfidence(
                ...
                source_fields={field_type.value: value for field_type, value in confidence_values.items()},
                consolidation_method="weighted_average_with_consistency_check"
            )
            
            logger.debug(f"✅ Confidence consolidated: {overall_confidence:.3f} ({level_description})")
            logger.debug(f"   Consistency: {consistency_score:.3f} | Reliability: {reliability_score:.3f}")
            if consistency_issues:
                logger.warning(f"   Data quality issues: {len(consistency_issues)} detected")
    ...
    def _extract_confidence_values(
        ...
        if state:
            ...
            if conf_score is not None:
                confidence_values[ConfidenceFieldType.CONFIDENCE_SCORE] = float(conf_score)
            ...
            if ev_conf is not None:
                confidence_values[ConfidenceFieldType.EVIDENCE_CONFIDENCE] = float(ev_conf)
        ...
        if agent_results:
            ...
            if tool_confidences:
                confidence_values[ConfidenceFieldType.TOOL_CONFIDENCE] = sum(tool_confidences) / len(tool_confidences)
            
            if domain_confidences:
                confidence_values[ConfidenceFieldType.DOMAIN_CONFIDENCE] = sum(domain_confidences) / len(domain_confidences)
    ...
    def _detect_confidence_inconsistencies(
        ...
        if len(values) > 1:
            ...
            if variance > 0.5:  # 50% difference between highest and lowest
                issues.append(f"High confidence variance detected: {variance:.3f} (min: {min_conf:.3f}, max: {max_conf:.3f})")
        
        # Check for confidence values outside valid range
        for field_type, value in confidence_values.items():
            if not (0.0 <= value <= 1.0):
                issues.append(f"Invalid confidence range for {field_type.value}: {value}")
        
        # Check for suspicious patterns
        if ConfidenceFieldType.AI_CONFIDENCE in confidence_values and ConfidenceFieldType.CONFIDENCE_SCORE in confidence_values:
    ...
    def apply_consolidated_confidence(
        ...
        self._safe_set_confidence(state, "ai_confidence", consolidated.ai_confidence or consolidated.overall_score)
        ...
        logger.debug(f"✅ Applied consolidated confidence {consolidated.overall_score:.3f} to investigation state")
