...
class PerformanceMonitor:
    ...
    def record_execution(
        ...
        metrics["avg_duration"] = metrics["total_duration"] / max(
        ...
        metrics["avg_input_size"] = (
            (metrics["avg_input_size"] * (total - 1)) + input_size
        ) / total
        metrics["avg_output_size"] = (
            (metrics["avg_output_size"] * (total - 1)) + output_size
        ) / total
    ...
    def check_performance_targets(
        self, tool_name: str, targets: Dict[str, float]
    ) -> Dict[str, bool]:
        ...
        try:
            if self.spec.async_implementation:
                ...
                result = await loop.run_in_executor(
                    None, self.spec.implementation, **kwargs
                )
                success = True
                return json.dumps(result) if not isinstance(result, str) else result
            else:
...
class InvestigationToolBuilder:
        ...
        templates["geo_validator"] = ToolSpecification(
            ...
            input_schema={
                ...
                "properties": {
                    ...
                    "claimed_location": {
                        "type": "object",
                        "description": "Claimed location data",
                    },
                },
                "required": ["ip"],
            },
    ...
    def create_tool(self, spec: ToolSpecification) -> InvestigationTool:
        ...
        if spec.implementation or spec.async_implementation:
            ...
            if impl_errors:
                raise ValueError(
                    f"Invalid tool implementation: {', '.join(impl_errors)}"
                )
...
def example_tool_creation():
    ...
    device_tool = builder.create_from_template(
        "device_analyzer", implementation=analyze_device_fingerprint
    )
    ...
    def check_ip_reputation(ip: str, threshold: float = 0.5) -> dict:
        ...
        score = random.random()
        return {"ip": ip, "reputation_score": score, "is_suspicious": score > threshold}
