...
class EnhancedFraudRouter:
    ...
    def _initialize_routing_rules(
        self,
    ) -> Dict[InvestigationComplexity, Dict[str, Any]]:
        ...
        if "domain_findings" in state:
            ...
            if "device" in findings:
                ...
                if "risk_score" in device_findings:
                    ...
                    indicators.device_anomaly_score = (
            ...
            if "location" in findings:
                ...
                if "risk_score" in location_findings:
                    ...
                    indicators.location_risk_score = (
                        risk_score if risk_score is not None else 0.0
                    )
            ...
            if "logs" in findings:
                ...
                if "risk_score" in logs_findings:
                    ...
                    indicators.activity_risk_score = (
                        risk_score if risk_score is not None else 0.0
                    )
    ...
    def determine_complexity(
        self, indicators: FraudIndicators
        ...
        if (
            composite_score
            >= self.complexity_thresholds[InvestigationComplexity.CRITICAL]
        ):
            return InvestigationComplexity.CRITICAL
        elif (
            composite_score >= self.complexity_thresholds[InvestigationComplexity.HIGH]
        ):
            return InvestigationComplexity.HIGH
        elif (
            composite_score
            >= self.complexity_thresholds[InvestigationComplexity.MEDIUM]
        ):
    ...
    def route_investigation(self, state: Dict[str, Any]) -> str:
        ...
        logger.info(
            f"Investigation complexity: {complexity.value}, "
    ...
    def _should_end_early(
        self, state: Dict[str, Any], indicators: FraudIndicators
    ) -> bool:
        """
...
def raw_data_or_investigation_routing(state: Dict[str, Any]) -> str:
    ...
    has_database_data = (
        ...
        and snowflake_data.get("success")
        and snowflake_data.get("row_count", 0) > 0
    )

    # Log what data sources are available
    if has_csv_data:
        logger.info(
            "Raw CSV data detected in messages - routing to raw data processing"
        )
        return "raw_data_node"
        ...
        logger.info(
            "No raw data detected (no CSV in messages, no database data) - routing to standard fraud investigation"
        )
        return "fraud_investigation"
