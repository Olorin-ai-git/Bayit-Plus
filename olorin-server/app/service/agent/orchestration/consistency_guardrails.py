...
def validate_narrative_numeric_agreement(
    domain: str, findings: Dict[str, Any]
) -> List[str]:
    """
    ...
    if risk_score <= 0.4:  # Low risk numeric
        ...
        for phrase in high_risk_phrases:
            if phrase.lower() in reasoning.lower():
                issues.append(
                    f"{domain}: Low numeric risk ({risk_score:.2f}) but narrative mentions '{phrase}'"
                )
    ...
    if "limited data" in reasoning.lower() or "1 transaction" in reasoning.lower():
        if risk_score > 0.4:
            issues.append(
                f"{domain}: Narrative mentions limited data but risk_score is {risk_score:.2f} > 0.4"
            )
    ...
    if "minimal" in reasoning.lower() and "external" in reasoning.lower():
        if risk_score > 0.4:
            issues.append(
                f"{domain}: Narrative mentions MINIMAL external TI but risk_score is {risk_score:.2f} > 0.4"
            )
...
def validate_no_model_score_driver(domain: str, findings: Dict[str, Any]) -> bool:
    ...
    if uses_model_score(reasoning) or uses_model_score(risk_factors):
        logger.error(
            f"🚫 {domain}: Uses MODEL_SCORE as risk driver - domain weight set to 0"
        )
        return True
...
def validate_aggregator_vs_publish_consistency(
    calculated_risk: float, published_risk: float, tolerance: float = 0.05
) -> bool:
    """
    ...
    if difference > tolerance:
        logger.error(
            f"🚫 PUBLISH_MISMATCH: calculated={calculated_risk:.3f}, published={published_risk:.3f}, diff={difference:.3f}"
        )
        return False
...
def apply_consistency_guardrails(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if domains_with_model_score_usage:
        ...
        logger.warning(
            f"🚫 Zeroing weights for domains using MODEL_SCORE: {domains_with_model_score_usage}"
        )
...
def sanity_check_before_publish(calculated: float, final: float) -> None:
    ...
    if calculated is not None and final is not None:
        ...
        assert (
            difference <= 0.05
        ), f"publish_mismatch: calculated={calculated:.3f}, final={final:.3f}, diff={difference:.3f}"
