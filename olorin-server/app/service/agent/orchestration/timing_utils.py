...
def calculate_performance_metrics(state: Any) -> Dict[str, float]:
    ...
    return {
        ...
        "investigation_velocity": safe_divide(1.0, duration_seconds, 0.1),  # Higher is faster
        "resource_efficiency": 1.0,  # Can be calculated based on actual resource usage
        "final_efficiency": min(1.0, safe_divide(60.0, duration_seconds, 0.64))  # Target 60s completion
    }
...
def ensure_timing_integrity(state: Any) -> None:
    ...
    if isinstance(state, dict):
        ...
        if state.get("current_phase") == "complete" and ("end_time" not in state or state["end_time"] is None):
        ...
        if (hasattr(state, "current_phase") and state.current_phase == "complete" and 
            (not hasattr(state, "end_time") or state.end_time is None)):
            state.end_time = current_time
