...
async def segment_hunter_node(state: EntitylessInvestigationState) -> Dict[str, Any]:
    ...
    if not anomaly_id:
        return {"messages": [AIMessage(content="Error: No anomaly_id in state")]}
    ...
    for dimension in dimensions:
        try:
            logger.info(
                f"🔍 SegmentHunter checking dimension {dimension} for anomaly {anomaly_id}"
            )
            result = segment_hunter._run(
                anomaly_id=anomaly_id, dimension=dimension, top_k=10, min_support=50
            )
            ...
            if has_error:
                logger.warning(
                    f"⚠️ SegmentHunter error for dimension {dimension}: {result.get('error')}"
                )
                ...
                logger.info(
                    f"✅ SegmentHunter found {len(result['segments'])} segments for dimension {dimension}"
                )
        except Exception as e:
            logger.error(
                f"❌ SegmentHunter failed for dimension {dimension}: {e}", exc_info=True
            )
            continue
    ...
    logger.info(
        f"SegmentHunter found {len(top_segments)} top segments for anomaly {anomaly_id}"
    )
    if len(top_segments) == 0:
        logger.warning(
            f"⚠️ SegmentHunter returned 0 segments for anomaly {anomaly_id}. All segments: {len(all_segments)}, Dimensions checked: {dimensions}"
        )
...
async def entity_extractor_node(state: EntitylessInvestigationState) -> Dict[str, Any]:
    ...
    if not segments:
        return {"messages": [AIMessage(content="No segments to extract entities from")]}
    ...
    for segment in segments[:5]:
        ...
        try:
            ...
            logger.warning(
                f"EntityExtractor failed for {dimension}={dimension_value}: {e}"
            )
            continue
...
"""
    ...
    try:
        ...
        return {"messages": [AIMessage(content=f"Investigation Summary:\n\n{summary}")]}
    except Exception as e:
        logger.error(f"Narrator error: {e}", exc_info=True)
        return {"messages": [AIMessage(content=f"Error generating summary: {str(e)}")]}
