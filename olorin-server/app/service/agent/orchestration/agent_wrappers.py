...
def wrap_agent_for_graph(agent_func):
    ...
    async def wrapped(state: Dict[str, Any], config: RunnableConfig) -> Dict[str, Any]:
        logger.info(f"Wrapper called for {agent_func.__name__}, state keys: {state.keys()}")
        ...
        logger.info(f"Extracted context - investigation_id: {investigation_id}, entity_id: {entity_id}")
        ...
        if not agent_context:
            ...
            # Check dict fields
            if not agent_context.get("investigation_id") or not agent_context.get("entity_id"):
                needs_context = True
        else:
            # It's an object, check attributes
            if not getattr(agent_context, "investigation_id", None) or not getattr(agent_context, "entity_id", None):
        ...
        if needs_context:
            ...
            logger.info(f"Created/replaced agent_context for {agent_func.__name__}: {investigation_id}")
