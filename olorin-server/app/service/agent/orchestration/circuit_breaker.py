...
def record_node_failure(state: Any, node_name: str, error: Exception) -> None:
    ...
    if failure_count >= FAILURE_THRESHOLD:
        ...
        logger.warning(f"🔥 CIRCUIT BREAKER: Node '{node_name}' disabled after {failure_count} failures")
...
def get_available_nodes(state: Any, all_nodes: Set[str]) -> Set[str]:
    ...
    if len(available) == 0:
        logger.warning("🔥 CIRCUIT BREAKER: All nodes disabled, resetting to allow summary")
...
def get_next_available_domain_node(state: Any) -> Optional[str]:
    ...
    for node in DOMAIN_NODES:
        ...
        if (node_domain not in domains_completed and 
            not is_node_disabled(state, node)):
            return node
