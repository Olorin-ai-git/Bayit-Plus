...
from app.service.agent.agent_coordination import AgentType, HandoffTrigger, HandoffContext, CoordinationPlan
from app.service.agent.flow_continuity import CheckpointType, ContinuityStrategy, CompletionStatus
...
class OrchestratorStateManager:
    ...
    def __init__(self):
        ...
        self.metric_aggregations: Dict[str, Dict[str, deque]] = defaultdict(lambda: defaultdict(lambda: deque(maxlen=100)))
    ...
    async def initialize_investigation_state(
        ...
        """
        logger.info(f"🚀 Initializing state tracking for investigation {investigation_id}")
        ...
        with self._state_lock:
            try:
                ...
                snapshot = await self._create_state_snapshot(
                    ...
                    agent_states={agent: {"status": "pending", "assigned_at": datetime.now().isoformat()} 
                                 for agent in coordination_plan.agent_sequence},
                    coordination_metadata={
                        "execution_mode": coordination_plan.execution_mode.value,
                        "agent_sequence": [agent.value for agent in coordination_plan.agent_sequence],
                ...
                logger.info(f"✅ Initialized state tracking with {len(coordination_plan.agent_sequence)} agents")
                return snapshot
    ...
    async def record_orchestration_decision(
        ...
        """
        logger.info(f"📝 Recording orchestration decision: {decision_type} for {investigation_id}")
        
        with self._state_lock:
            try:
                # Generate unique decision ID
                decision_id = self._generate_decision_id(investigation_id, decision_type)
                ...
                self.coordination_metadata[investigation_id]["orchestration_decisions"] += 1
                ...
                logger.info(f"✅ Recorded decision {decision_id} with confidence {confidence_score:.2f}")
    ...
    async def track_agent_execution(
        ...
        """
        logger.info(f"🔍 Tracking agent execution: {agent_type.value} -> {execution_state}")
        ...
        with self._state_lock:
            try:
                ...
                snapshot = await self._create_state_snapshot(
                    ...
                    orchestration_state={"active_agents": len(active_inv.get("current_agents", []))},
                    agent_states={agent_type: agent_state},
                    context={"agent": agent_type.value, "execution_state": execution_state}
                )
                
                logger.info(f"✅ Tracked {agent_type.value} execution: {execution_state}")
                return snapshot
    ...
    async def record_handoff_completion(
        ...
        """
        logger.info(f"🔄 Recording handoff completion: {handoff_context.from_agent.value} -> {handoff_context.to_agent.value}")
        ...
        with self._state_lock:
            try:
                ...
                for snapshot in reversed(snapshots):
                    if snapshot.transition not in [StateTransition.AGENT_FAILED, StateTransition.INVESTIGATION_FAILED]:
                ...
                await self._create_state_snapshot(
                    ...
                    orchestration_state={"recovery_id": recovery_id, "recovery_strategy": recovery_strategy.value},
                    recovery_data={"recovery_state": asdict(recovery_state)},
                    context=failure_context
                )
                
                logger.info(f"✅ Created recovery state {recovery_id} with strategy {recovery_strategy.value}")
        ...
        with self._state_lock:
            try:
                ...
                if active_inv.get("start_time"):
                    duration = (datetime.now() - active_inv["start_time"]).total_seconds()
                
                # Agent completion statistics
                total_agents = len(active_inv.get("completed_agents", [])) + len(active_inv.get("failed_agents", [])) + len(active_inv.get("current_agents", []))
                completion_rate = len(active_inv.get("completed_agents", [])) / total_agents if total_agents > 0 else 0
                ...
                summary = {
                    ...
                    "recovery_status": {
                        ...
                        "is_recoverable": recovery_state.is_recoverable if recovery_state else None,
                        "recovery_priority": recovery_state.priority_level if recovery_state else None
                    },
                    "timeline": [
                        {
    ...
    def _generate_snapshot_id(self, investigation_id: str, transition: StateTransition) -> str:
        """Generate unique snapshot ID"""
        content = f"{investigation_id}_{transition.value}_{datetime.now().isoformat()}"
        return hashlib.md5(content.encode()).hexdigest()[:12]
    
    def _generate_metric_id(self, investigation_id: str, metric_type: MetricType, agent_type: Optional[AgentType]) -> str:
    ...
    async def _cleanup_old_snapshots(self, investigation_id: str, max_snapshots: int = 50):
