...
class RecommendationGenerator:
    ...
    def generate_recommendations(
        ...
        recommendations.extend(self._get_priority_recommendations(
            overall_score, risk_assessments
        ))
    ...
    def _get_specific_risk_recommendations(self, risk_type: str, score: float) -> List[str]:
    ...
    def _get_model_based_recommendations(self, model_results: Dict[str, Any]) -> List[str]:
        ...
        if "composite" in model_results:
            ...
            if isinstance(composite_result, dict):
                ...
                if consensus_factor < 0.5:
                    recommendations.append("Low model consensus - consider additional validation")
        ...
        if "ml_based" in model_results:
            ...
            if isinstance(ml_result, dict):
                ...
                if prediction_interval:
                    range_width = prediction_interval.get("upper_bound", 0) - prediction_interval.get("lower_bound", 0)
                    if range_width > 0.3:
                        recommendations.append("High prediction uncertainty - consider additional data collection")
    ...
    def _get_priority_recommendations(
        ...
        if len(high_risk_assessments) >= 3:
            recommendations.append("Multiple high-risk factors detected - comprehensive review recommended")

        # Check for critical combinations
        fraud_score = next(
            (assessment.score for assessment in risk_assessments if assessment.risk_type == "fraud"),
            0.0
        )
        behavioral_score = next(
            (assessment.score for assessment in risk_assessments if assessment.risk_type == "behavioral"),
            0.0
        )

        if fraud_score > 0.7 and behavioral_score > 0.6:
            recommendations.append("URGENT: High fraud and behavioral risk combination detected")
