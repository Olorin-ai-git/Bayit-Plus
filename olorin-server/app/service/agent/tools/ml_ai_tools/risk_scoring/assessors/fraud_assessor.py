...
class FraudRiskAssessor(BaseRiskAssessor):
    ...
    def assess(self, processed_data: Dict[str, Any], risk_tolerance: str) -> RiskAssessmentResult:
        ...
        self._assess_financial_fraud_indicators(financial_data, risk_factors, contributing_factors)

        # Analyze risk indicators
        risk_indicators = processed_data.get("risk_indicators", {})
        self._assess_risk_indicators(risk_indicators, risk_factors, contributing_factors)

        # Analyze behavioral patterns
        behavioral_data = processed_data.get("behavioral_data", {})
        self._assess_behavioral_fraud_patterns(behavioral_data, risk_factors, contributing_factors)
    ...
    def _assess_behavioral_fraud_patterns(
        ...
        for field, value in behavioral_data.items():
            ...
            if isinstance(value, (int, float)):
                ...
                elif "device" in field_lower and "inconsistency" in field_lower and value > 0.3:
                    risk_factors.append({
                        "score": value * 0.7,
                        "weight": 1.4,
