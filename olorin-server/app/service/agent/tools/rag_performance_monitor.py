...
class RAGToolPerformanceMonitor:
    ...
    def __init__(
        ...
        self.tool_metrics: Dict[str, Deque[PerformanceMetric]] = defaultdict(lambda: deque(maxlen=1000))
        ...
        logger.info(f"RAG Tool Performance Monitor initialized with {performance_target_ms}ms target")
    ...
    def _update_tool_summary(self, tool_name: str, metric: PerformanceMetric) -> None:
        ...
        self.tool_performance_summary[tool_name] = {
            "avg_overhead_ms": statistics.mean(overhead_times) if overhead_times else 0.0,
            "median_overhead_ms": statistics.median(overhead_times) if overhead_times else 0.0,
            "p95_overhead_ms": statistics.quantiles(overhead_times, n=20)[18] if len(overhead_times) >= 20 else max(overhead_times, default=0.0),
            "target_compliance_rate": sum(target_compliance) / len(target_compliance) if target_compliance else 0.0,
            "avg_knowledge_chunks": statistics.mean(knowledge_chunks) if knowledge_chunks else 0.0,
    ...
    def _check_performance_alerts(self, metric: PerformanceMetric) -> None:
        ...
        if metric.total_enhancement_ms > self.performance_target_ms * 3:
            self._generate_alert(
                ...
                {"overhead_ms": metric.total_enhancement_ms, "target_ms": self.performance_target_ms},
                "critical"
            )
        
        # Warning: Consecutive violations of target
        if self.current_session_stats["consecutive_violations"] >= self.alert_threshold_violations:
            self._generate_alert(
                "consecutive_target_violations",
                metric.tool_name,
                f"{self.current_session_stats['consecutive_violations']} consecutive target violations",
                {"consecutive_count": self.current_session_stats["consecutive_violations"]},
                "warning"
            )
        
        # Tool-specific degradation
        tool_summary = self.tool_performance_summary.get(metric.tool_name, {})
        if tool_summary.get("target_compliance_rate", 1.0) < 0.7:  # Less than 70% compliance
            self._generate_alert(
    ...
    def get_system_performance_summary(self) -> Dict[str, Any]:
        ...
        if total_executions > 0:
            target_compliance_rate = session_stats["target_compliant_executions"] / total_executions
        ...
        if recent_metrics:
            ...
            p95_overhead = statistics.quantiles(overhead_times, n=20)[18] if len(overhead_times) >= 20 else max(overhead_times)
        ...
        return {
            ...
            "active_alerts": len([a for a in self.alerts_history if a.timestamp > datetime.now() - timedelta(minutes=5)]),
            "tools_monitored": len(self.tool_metrics),
            "session_duration_minutes": (datetime.now() - session_stats["session_start"]).total_seconds() / 60,
    ...
    def get_tool_performance_details(self, tool_name: str) -> Optional[Dict[str, Any]]:
        ...
        compliance_rate = sum(1 for m in tool_metrics if m.meets_target) / len(tool_metrics)
        ...
        return {
            ...
            "performance_metrics": {
                ...
                "std_dev_ms": statistics.stdev(overhead_times) if len(overhead_times) > 1 else 0.0
            },
            "target_compliance": {
                "compliance_rate": compliance_rate,
                "target_ms": self.performance_target_ms,
                "violations": len(tool_metrics) - sum(1 for m in tool_metrics if m.meets_target)
            },
            "performance_distribution": {
                level.value: sum(1 for m in tool_metrics if m.performance_level == level)
                for level in PerformanceLevel
            },
            "recent_trend": self._calculate_performance_trend(tool_metrics[-20:]) if len(tool_metrics) >= 10 else "insufficient_data"
        }
    ...
    def _calculate_system_health_score(self) -> float:
        ...
        compliance_rate = self.current_session_stats["target_compliant_executions"] / total_executions
        health_score = compliance_rate
        
        # Penalty for consecutive violations
        consecutive_penalty = min(0.3, self.current_session_stats["consecutive_violations"] * 0.05)
        health_score -= consecutive_penalty
        
        # Penalty for critical alerts
        recent_critical_alerts = sum(
            1 for alert in self.alerts_history 
            if alert.severity == "critical" and alert.timestamp > datetime.now() - timedelta(minutes=10)
        )
    ...
    def _calculate_performance_trend(self, recent_metrics: List[PerformanceMetric]) -> str:
    ...
    def get_recent_alerts(self, severity_filter: Optional[str] = None) -> List[PerformanceAlert]:
        ...
        if severity_filter:
            recent_alerts = [alert for alert in recent_alerts if alert.severity == severity_filter]
