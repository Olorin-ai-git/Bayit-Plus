...
class AbuseReportInput(BaseModel):
    ...
    ip: str = Field(
        ..., description="IP address to report for abuse", examples=["[IP_1]", "[IP_2]"]
    )
...
class AbuseReportingTool(BaseTool):
    ...
    def _generate_report_analysis(
        self, response, ip: str, categories: str, comment: str
    ) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            "abuseipdb_response": {
                "confidence_percentage": (
                    ...
                    if hasattr(response, "confidence_percentage")
                    else None
                ),
                "country_code": (
                    response.country_code if hasattr(response, "country_code") else None
                ),
                "isp": response.isp if hasattr(response, "isp") else None,
                "usage_type": (
                    response.usage_type if hasattr(response, "usage_type") else None
                ),
            },
    ...
    async def _arun(self, ip: str, categories: str, comment: str, **kwargs) -> str:
        ...
        try:
            ...
            report_response = await client.report_ip_abuse(
                ip=ip, categories=category_ids, comment=comment
            )

            if report_response.success:
                # Generate comprehensive analysis
                analysis_data = self._generate_report_analysis(
                    report_response, ip, categories, comment
                )
