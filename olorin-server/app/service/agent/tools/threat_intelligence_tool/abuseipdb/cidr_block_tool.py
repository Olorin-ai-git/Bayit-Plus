...
class CIDRBlockAnalysisTool(BaseTool):
    ...
    def _generate_cidr_analysis(self, response, cidr_network: str) -> Dict[str, Any]:
        ...
        threat_density = self._calculate_threat_density(total_reported, network_stats.get("usable_addresses", 1))
        ...
        # Risk assessment
        overall_risk = self._assess_overall_risk(risk_percentage, threat_density, len(critical_ips), len(high_risk_ips))
        
        # Generate recommendations
        recommendations = self._generate_cidr_recommendations(
            overall_risk, threat_density, critical_ips, high_risk_ips, 
            infrastructure_analysis, network_stats
        )
        ...
        analysis = {
            ...
            "metadata": {
                ...
                "source": "AbuseIPDB",
                "network_info": response.network_info.model_dump() if response.network_info else None
            }
        }
        
        return analysis

    def _calculate_threat_density(self, reported_count: int, usable_addresses: int) -> float:
    ...
    def _analyze_infrastructure(self, reported_ips) -> Dict[str, Any]:
        ...
        dominant_isp = max(isp_counts.items(), key=lambda x: x[1]) if isp_counts else ("Unknown", 0)
        dominant_usage = max(usage_types.items(), key=lambda x: x[1]) if usage_types else ("Unknown", 0)
        ...
        return {
            ...
            "isp_distribution": sorted(isp_counts.items(), key=lambda x: x[1], reverse=True)[:10],
            "dominant_usage_type": {"type": dominant_usage[0], "count": dominant_usage[1]},
    ...
    def _analyze_geographic_distribution(self, reported_ips) -> Dict[str, Any]:
        ...
        dominant_country = max(country_counts.items(), key=lambda x: x[1]) if country_counts else ("Unknown", 0)
        ...
        return {
            ...
            "dominant_country": {"country": dominant_country[0], "count": dominant_country[1]},
            "country_distribution": sorted(country_counts.items(), key=lambda x: x[1], reverse=True),
            "geographic_diversity": "HIGH" if total_countries > 5 else "MEDIUM" if total_countries > 2 else "LOW",
    ...
    def _analyze_temporal_patterns(self, reported_ips) -> Dict[str, Any]:
        ...
        return {
            ...
            "activity_trend": "INCREASING" if very_recent > recent_reports * 0.3 else "STABLE"
        }

    def _assess_overall_risk(self, risk_percentage: float, threat_density: float, 
                           critical_count: int, high_risk_count: int) -> str:
    ...
    def _generate_cidr_recommendations(self, overall_risk: str, threat_density: float,
                                     critical_ips: List, high_risk_ips: List,
                                     infrastructure_analysis: Dict, network_stats: Dict) -> List[str]:
        ...
        if threat_density >= 10.0:
            recommendations.append("🔥 SEVERE: >10% of network compromised - likely coordinated attack")
        elif threat_density >= 1.0:
            recommendations.append("⚠️ WARNING: >1% of network showing malicious activity")
        elif threat_density >= 0.1:
            recommendations.append("📈 NOTABLE: Elevated threat activity within network")
        
        # Infrastructure-based recommendations
        if infrastructure_analysis.get("coordination_indicator"):
            recommendations.append("🎯 COORDINATED: Evidence suggests organized malicious infrastructure")
        
        if infrastructure_analysis.get("infrastructure_risk_level") == "HIGH":
            recommendations.append("🏢 ISP ALERT: Single ISP concentration suggests provider-level issue")
        
        # Network type recommendations
        if network_stats.get("is_private"):
            recommendations.append("🔐 INTERNAL: Private network range - check for insider threats or compromised systems")
        else:
            recommendations.append("🌐 PUBLIC: Internet-routable space - external threat focus")
        
        # Specific IP recommendations
        if len(critical_ips) > 0:
            recommendations.append(f"🎯 IMMEDIATE: {len(critical_ips)} critical-threat IPs require instant blocking")
        
        if len(high_risk_ips) > 5:
            recommendations.append(f"⚠️ PRIORITY: {len(high_risk_ips)} high-risk IPs need investigation")
        
        # Network size considerations
        total_addresses = network_stats.get("usable_addresses", 0)
        if total_addresses > 1000000:  # Large networks
            recommendations.append("📊 LARGE NETWORK: Consider subnet-level analysis for better granularity")
        elif total_addresses < 256:  # Small networks
            recommendations.append("🔍 SMALL NETWORK: Individual IP investigation recommended")
        ...
        **kwargs
        ...
        try:
            ...
            if cidr_response.success:
                ...
                analysis_data = self._generate_cidr_analysis(cidr_response, cidr_network)
                
                logger.info(f"CIDR analysis completed: {analysis_data['threat_summary']['total_reported_ips']} reported IPs found")
