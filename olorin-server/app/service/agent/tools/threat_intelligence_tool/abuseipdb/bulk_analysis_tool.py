...
class BulkIPAnalysisTool(BaseTool):
    ...
    def _generate_bulk_analysis(self, response) -> Dict[str, Any]:
        ...
        risk_summary = {
            ...
            "overall_risk_level": self._determine_overall_risk(
                high_risk_ips, medium_risk_ips, total_analyzed
            ),
        }
        ...
        for ip_data in response.ips_analyzed:
            analysis = {
                ...
                "investigation_priority": self._get_investigation_priority(
                    ip_data.abuse_confidence_percentage
                ),
            }
        ...
        recommendations = self._generate_bulk_recommendations(
            risk_summary, high_risk_ips, medium_risk_ips
        )

        # Geographic analysis
        geographic_analysis = self._analyze_geographic_distribution(
            response.ips_analyzed
        )
    ...
    def _determine_overall_risk(
        self, high_risk_ips: List[str], medium_risk_ips: List[str], total: int
    ) -> str:
    ...
    def _analyze_geographic_distribution(self, ips_analyzed) -> Dict[str, Any]:
        ...
        top_risk_countries = sorted(
            high_risk_countries.items(), key=lambda x: x[1], reverse=True
        )[:5]

        return {
            "total_countries": len(country_counts),
            "top_countries": [
                {"country": c, "count": count} for c, count in top_countries
            ],
            "high_risk_countries": [
                {"country": c, "high_risk_count": count}
                for c, count in top_risk_countries
            ],
            "geographic_diversity": (
                "HIGH"
                if len(country_counts) > 10
                else "MEDIUM" if len(country_counts) > 5 else "LOW"
            ),
        }

    def _generate_bulk_recommendations(
        self, risk_summary: Dict, high_risk_ips: List[str], medium_risk_ips: List[str]
    ) -> List[str]:
        ...
        if high_count > 0:
            recommendations.append(
                f"🚨 CRITICAL: {high_count} high-risk IPs detected - immediate investigation required"
            )
            recommendations.append(
                "🔒 Consider blocking or restricting access from high-risk IPs"
            )
            recommendations.append(
                "📊 Cross-reference high-risk IPs with recent fraud incidents"
            )

        if medium_count > 0:
            recommendations.append(
                f"⚠️ ATTENTION: {medium_count} medium-risk IPs require enhanced monitoring"
            )
            recommendations.append(
                "🔍 Apply additional verification for users from medium-risk IPs"
            )

        overall_risk = risk_summary["overall_risk_level"]
        if overall_risk in ["CRITICAL", "HIGH"]:
            recommendations.append(
                "🚨 URGENT: High concentration of malicious IPs suggests coordinated attack"
            )
            recommendations.append(
                "📋 Implement emergency security measures and alert security team"
            )
            recommendations.append(
        ...
        if high_percentage >= 50:
            recommendations.append(
                "🔥 SEVERE: Over 50% of IPs are high-risk - possible botnet or fraud ring"
            )
        elif high_percentage >= 25:
            recommendations.append(
                "⚠️ WARNING: 25%+ of IPs are high-risk - investigate for patterns"
            )

        if total >= 1000:
            recommendations.append(
                "📈 Large-scale analysis complete - consider automated response rules"
            )

        if not recommendations:
            recommendations.append(
                "✅ Good news: No immediate security concerns detected"
            )
    ...
    async def _arun(self, ip_addresses: str, max_age_days: int = 90, **kwargs) -> str:
        ...
        try:
            ...
            bulk_response = await client.bulk_check_ips(
                ip_addresses=ip_list, max_age_days=max_age_days
            )
            ...
            if bulk_response.success:
                ...
                logger.info(
                    f"Bulk analysis completed: {analysis_data['risk_summary']['total_analyzed']} IPs processed"
                )
            ...
            return json.dumps(
                {"success": False, "error": str(e), "source": "AbuseIPDB"}, indent=2
            )
