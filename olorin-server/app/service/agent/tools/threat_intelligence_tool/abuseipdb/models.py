...
class AbuseIPDBConfig(BaseModel):
    ...
    api_key_secret: str = Field(
        default="ABUSEIPDB_API_KEY", description="Firebase secret name for API key"
    )
    base_url: str = Field(
        default="https://api.abuseipdb.com/api/v2", description="AbuseIPDB API base URL"
    )
...
class IPInfo(BaseModel):
    ...
    abuse_confidence_percentage: int = Field(
        ..., description="Abuse confidence percentage"
    )
    ...
    num_distinct_users: int = Field(
        default=0, description="Number of distinct reporting users"
    )
    last_reported_at: Optional[datetime] = Field(
        default=None, description="Last report timestamp"
    )
...
class BulkIPData(BaseModel):
    ...
    last_reported_at: Optional[datetime] = Field(
        default=None, description="Last reported"
    )
...
class BulkAnalysisResponse(BaseModel):
    ...
    success: bool = Field(..., description="Whether request was successful")
    ips_analyzed: List[BulkIPData] = Field(
        default_factory=list, description="Analyzed IP data"
    )
    total_ips: int = Field(default=0, description="Total IPs analyzed")
    high_risk_ips: List[str] = Field(
        default_factory=list, description="High risk IP addresses"
    )
    error: Optional[str] = Field(default=None, description="Error message if any")
    response_time_ms: int = Field(..., description="Response time in milliseconds")
...
class CIDRAnalysisResponse(BaseModel):
    ...
    success: bool = Field(..., description="Whether request was successful")
    network_info: Optional[CIDRBlockInfo] = Field(
        default=None, description="Network information"
    )
    reported_ips: List[BulkIPData] = Field(
        default_factory=list, description="Reported IPs in range"
    )
...
class AbuseReportRequest(BaseModel):
    ...
    timestamp: Optional[datetime] = Field(
        default=None, description="Incident timestamp"
    )
...
class AbuseReportResponse(BaseModel):
    ...
    report_id: Optional[str] = Field(
        default=None, description="Report ID if successful"
    )
