...
class ShodanClient:
        ...
        try:
            async with session.request(method, url, params=params) as response:
                if response.status == 200:
                    ...
                    if (
                        "membership" in error_text.lower()
                        or "upgrade" in error_text.lower()
                    ):
                        raise ValueError(
                            "Shodan API endpoint requires paid subscription plan"
                        )
                    else:
    ...
    async def host_info(
        self, ip: str, history: bool = False, minify: bool = False
    ) -> ShodanHostResponse:
        """
    ...
    async def search(
        ...
        Returns:
            Search results
        """
        params = {"query": query, "page": page, "minify": "true" if minify else "false"}
        ...
        return ShodanSearchResponse(
            matches=matches, total=data.get("total", 0), facets=data.get("facets", {})
        )

    async def search_exploits(
        self, query: str, facets: Optional[str] = None, page: int = 1
    ...
    async def search_count(
        self, query: str, facets: Optional[str] = None
    ) -> Dict[str, Any]:
        """
