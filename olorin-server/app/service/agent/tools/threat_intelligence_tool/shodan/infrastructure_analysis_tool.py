...
class InfrastructureAnalysisInput(BaseModel):
    ...
    include_history: bool = Field(
        default=False, description="Include historical scan data"
    )
    include_vulnerabilities: bool = Field(
        default=True, description="Include vulnerability analysis"
    )
    ...
    def validate_ip(cls, v):
        ...
        try:
            ...
            raise ValueError(
                f"Invalid IP address format: {v}. Expected IPv4 (e.g., 192.168.1.1) or IPv6 (e.g., 2001:db8::1) address."
            )
...
class ShodanInfrastructureAnalysisTool(BaseTool):
        ...
        **kwargs,
        ...
        try:
            ...
            host_data = await self.client.host_info(
                ip=ip, history=include_history, minify=False
            )
            ...
            if "paid subscription" in str(e).lower():
                logger.debug(
                    f"Shodan analysis skipped for {ip}: paid subscription required"
                )
                return json.dumps(
                    {
                        "status": "skipped",
    ...
    async def _build_infrastructure_analysis(
        ...
        if include_vulnerabilities:
            host_info["vulnerability_analysis"] = self._analyze_vulnerabilities(
                host_data
            )
        ...
        host_info["investigation_recommendations"] = self._generate_recommendations(
            host_data
        )
        ...
        for service in host_data.data:
            if service.ssl and isinstance(service.ssl, dict):
                ssl_info = {
                    ...
                    "cert_issuer": service.ssl.get("cert", {})
                    .get("issuer", {})
                    .get("O"),
    ...
    def _assess_infrastructure_risk(
        self, host_data: ShodanHostResponse
    ) -> Dict[str, Any]:
        ...
        if unencrypted_critical:
            risk_score += 20
            risk_factors.append(
                f"Unencrypted critical services: {unencrypted_critical[:3]}"
            )
        ...
        if host_data.infrastructure_type in ["iot_device", "network_device"]:
            risk_score += 15
            risk_factors.append(
                f"Sensitive device type: {host_data.infrastructure_type}"
            )
    ...
    def _generate_recommendations(self, host_data: ShodanHostResponse) -> List[str]:
        ...
        if host_data.total_vulnerabilities > 5:
            recommendations.append(
                "CRITICAL: Multiple vulnerabilities detected - investigate exploitation attempts"
            )
            recommendations.append(
                "Review access logs for suspicious activity targeting vulnerable services"
            )
        elif host_data.total_vulnerabilities > 0:
            recommendations.append(
                "Known vulnerabilities present - check for exploitation indicators"
            )

        # Critical services recommendations
        if host_data.has_critical_ports:
            recommendations.append(
                "Critical services exposed - verify legitimate business purpose"
            )
        ...
        if db_ports:
            recommendations.append(
                f"Database services exposed on ports {list(db_ports)} - high risk for data breach"
            )
        ...
        if remote_ports:
            recommendations.append(
                f"Remote access services on ports {list(remote_ports)} - verify authorized usage"
            )

        # Infrastructure type specific
        if host_data.infrastructure_type == "iot_device":
            recommendations.append(
                "IoT device detected - often vulnerable and rarely updated"
            )
        elif host_data.infrastructure_type == "network_device":
            recommendations.append(
                "Network infrastructure device - critical for network security"
            )
