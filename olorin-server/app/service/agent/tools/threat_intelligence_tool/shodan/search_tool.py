...
class ShodanSearchInput(BaseModel):
    ...
    facets: Optional[str] = Field(
        None, description="Comma-separated facets to return (e.g., 'country,port,org')"
    )
...
class ShodanSearchTool(BaseTool):
    ...
    async def _arun(
        self, query: str, facets: Optional[str] = None, page: int = 1, **kwargs
    ) -> str:
        """Execute search asynchronously."""
        try:
            # Perform search
            search_results = await self.client.search(
                query=query, facets=facets, page=page, minify=True
            )

            # Build analysis result
            analysis_result = await self._build_search_analysis(
                search_results=search_results, query=query, page=page
            )
            ...
            if "paid subscription plan" in error_str.lower():
                logger.warning(
                    f"Shodan search unavailable for query '{query}': requires paid plan"
                )
                return json.dumps(
                    {
                        "status": "unavailable",
                ...
                logger.warning(
                    f"Shodan search unavailable for query '{query}': insufficient credits"
                )
                return json.dumps(
                    {
                ...
                logger.error(
                    f"Shodan search authentication failed for query '{query}': {error_str}"
                )
                return json.dumps(
                    {
    ...
    async def _build_search_analysis(
        self, search_results: ShodanSearchResponse, query: str, page: int
    ) -> Dict[str, Any]:
            ...
            for match in search_results.matches[:10]:  # Top 10 results
                ...
                if match.location:
                    result_summary["location"] = {
                        ...
                        or match.location.country_code,
        ...
        result["search_analysis"]["unique_organizations"] = (
            search_results.unique_organizations[:10]
        )
    ...
    def _generate_search_recommendations(
        self, search_results: ShodanSearchResponse, query: str
    ) -> List[str]:
