...
from ..enhanced_tool_base import EnhancedToolBase, RetryStrategy, ValidationLevel
...
class ThreatIntelligenceConfig(BaseModel):
    ...
    api_key_secret_name: str = Field(
        ..., description="Firebase secret name for API key"
    )
...
class BaseThreatIntelligenceTool(EnhancedToolBase):
    ...
    def __init__(
        self, name: str, description: str, config: ThreatIntelligenceConfig, **kwargs
    ):
    ...
    def _initialize_cache(self) -> EnhancedCache:
        ...
        try:
            ...
            logger.info(
                f"Initialized enhanced cache for {self.name} with Redis: {redis_client is not None}"
            )
            return cache
        ...
        if self._api_key is None:
            try:
                ...
                if not self._api_key:
                    raise ValueError(
                        f"API key not found in Firebase secrets: {self.config.api_key_secret_name}"
                    )
            except Exception as e:
    ...
    async def execute_threat_intelligence_query(
        ...
        try:
            ...
            cache_key = self._generate_cache_key(
                {"query_type": query_type, **query_data}
            )
            ...
            return ThreatIntelligenceResponse(
                success=False, error=str(e), source=self.name, timestamp=start_time
            )
