...
class VirusTotalConfig(BaseModel):
    ...
    api_key_secret: str = Field(default="VIRUSTOTAL_API_KEY", description="Firebase secret name for VirusTotal API key")
    base_url: str = Field(default="https://www.virustotal.com/api/v3", description="VirusTotal API base URL")
    timeout_seconds: int = Field(default=30, description="Request timeout in seconds")
    rate_limit_requests: int = Field(default=500, description="Requests per day (free tier)")
...
class VirusTotalIPResponse(BaseModel):
    ...
    vendor_results: List[VirusTotalVendorResult] = Field(default_factory=list, description="Individual vendor results")
...
class VirusTotalDomainResponse(BaseModel):
    ...
    categories: Dict[str, str] = Field(default_factory=dict, description="Domain categories from vendors")
    reputation: Optional[int] = Field(None, description="Community reputation score")
    harmless_votes: int = Field(default=0, description="Harmless community votes")
    malicious_votes: int = Field(default=0, description="Malicious community votes")
...
class VirusTotalFileResponse(BaseModel):
    ...
    # Timestamps
    first_submission_date: Optional[datetime] = Field(None, description="First submission date")
    last_analysis_date: Optional[datetime] = Field(None, description="Last analysis timestamp")
    last_submission_date: Optional[datetime] = Field(None, description="Last submission date")
...
class VirusTotalURLResponse(BaseModel):
    ...
    # Categories and reputation
    categories: Dict[str, str] = Field(default_factory=dict, description="URL categories from vendors")
    reputation: Optional[int] = Field(None, description="Community reputation score")
    harmless_votes: int = Field(default=0, description="Harmless community votes")
    malicious_votes: int = Field(default=0, description="Malicious community votes")
