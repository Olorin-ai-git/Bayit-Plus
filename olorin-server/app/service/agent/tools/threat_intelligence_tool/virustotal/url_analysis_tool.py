...
from .models import VirusTotalConfig, VirusTotalURLResponse
...
class URLAnalysisInput(BaseModel):
    ...
    url: str = Field(
        ...,
        description="URL to analyze (must include protocol, e.g., 'https://example.com')",
    )
    ...
    include_screenshots: bool = Field(
        default=False, description="Whether to include screenshot analysis information"
    )
...
class VirusTotalURLAnalysisTool(BaseTool):
    ...
    async def _build_url_analysis(
        ...
        if (
            include_redirects
            and hasattr(url_response, "redirection_chain")
            and url_response.redirection_chain
        ):
            redirect_info = []
                ...
                redirect_info.append(
                    {
                        ...
                        "url": redirect_url,
                        "domain": (
                            urlparse(redirect_url).netloc if redirect_url else "Unknown"
                        ),
                    }
                )
        ...
        if (
            include_screenshots
            and hasattr(url_response, "has_content")
            and url_response.has_content
        ):
            screenshot_info = {
            ...
            if hasattr(url_response, "outgoing_links") and url_response.outgoing_links:
                screenshot_info["outgoing_links_count"] = len(
                    url_response.outgoing_links
                )
                screenshot_info["sample_outgoing_links"] = url_response.outgoing_links[
                    :5
                ]
        ...
        submission_info = {
            "first_submission_date": (
                str(url_response.first_submission_date)
                if url_response.first_submission_date
                else None
            ),
            "last_analysis_date": (
                str(url_response.last_analysis_date)
                if url_response.last_analysis_date
                else None
            ),
            "last_modification_date": (
                str(url_response.last_modification_date)
                if url_response.last_modification_date
                else None
            ),
            "times_submitted": getattr(url_response, "times_submitted", 0),
            "total_votes": {
                "harmless": (
                    url_response.total_votes.get("harmless", 0)
                    if url_response.total_votes
                    else 0
                ),
                "malicious": (
                    url_response.total_votes.get("malicious", 0)
                    if url_response.total_votes
                    else 0
                ),
            },
        ...
        return {"url_analysis": url_info}

    def _assess_url_risk(
        self, url_response: VirusTotalURLResponse, url: str
    ) -> Dict[str, Any]:
        ...
        if hasattr(url_response, "categories") and url_response.categories:
            ...
            found_dangerous_cats = [
                cat
                for cat in url_response.categories.values()
        ...
        # Redirect-based risk
        if (
            hasattr(url_response, "redirection_chain")
            and url_response.redirection_chain
        ):
        ...
        if hasattr(url_response, "threat_names") and url_response.threat_names:
            ...
            found_threats = [
                threat
                for threat in url_response.threat_names
        ...
        if any(keyword in url.lower() for keyword in suspicious_keywords):
            if (
                stats.malicious > 0 or stats.suspicious > 0
            ):  # Only flag if also detected by engines
                risk_score += 10
    ...
    def _generate_url_recommendations(
        ...
        if not url_response.last_analysis_stats:
            return [
                "Insufficient data for recommendations - consider resubmitting URL for analysis"
            ]
        ...
        if stats.malicious > 10:
            recommendations.append(
                "CRITICAL: URL flagged as malicious by multiple engines - block immediately"
            )
            recommendations.append(
                "Investigate all users who may have accessed this URL"
            )
            recommendations.append("Check for credential theft or malware installation")
        elif stats.malicious > 5:
            recommendations.append(
                "URL flagged as malicious - block access and investigate"
            )
        elif stats.malicious > 0:
        ...
        if hasattr(url_response, "categories") and url_response.categories:
            ...
            if any("phishing" in cat for cat in categories):
                recommendations.append(
                    "PHISHING DETECTED: Check for credential theft and notify affected users"
                )
            elif any("malware" in cat for cat in categories):
                recommendations.append(
                    "MALWARE HOSTING: Scan systems that accessed this URL for infections"
                )
            elif any("scam" in cat or "fraud" in cat for cat in categories):
                recommendations.append(
                    "Fraud/scam site detected: Investigate financial transactions and user reports"
                )
        ...
        if (
            ...
            if redirect_count > 3:
                recommendations.append(
                    f"Multiple redirects detected ({redirect_count}) - investigate redirect chain for malicious redirections"
                )
            ...
            if url_response.redirection_chain:
                ...
                if final_domain != original_domain:
                    recommendations.append(
                        f"URL redirects to different domain ({final_domain}) - verify legitimacy"
                    )
        ...
        if hasattr(url_response, "reputation") and url_response.reputation is not None:
            if url_response.reputation < -20:
                recommendations.append(
                    "Very poor reputation - high probability of malicious activity"
                )

        # General security recommendations
        if stats.suspicious > 3:
            recommendations.append(
                "High suspicious activity - monitor user access and behavior"
            )
        ...
        if not recommendations:
            if stats.harmless > stats.malicious + stats.suspicious:
                ...
                recommendations.append(
                    "Mixed detection results - investigate URL context and purpose"
                )
