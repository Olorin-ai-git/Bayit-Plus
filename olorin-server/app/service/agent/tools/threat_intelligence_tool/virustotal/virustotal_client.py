            ...
            try:
                ...
                if not self._api_key:
                    raise VirusTotalAuthError(
                        f"VirusTotal API key not found: {self.config.api_key_secret}"
                    )
    ...
    async def analyze_ip(self, ip: str) -> VirusTotalIPResponse:
        ...
        try:
            ...
            if "last_analysis_stats" in attributes:
                analysis_stats = self._parse_analysis_stats(
            ...
            if "last_analysis_results" in attributes:
                vendor_results = self._parse_vendor_results(
            ...
            response = VirusTotalIPResponse(
                ...
                last_analysis_date=self._safe_get_datetime(
            ...
            return VirusTotalIPResponse(success=False, ip=ip, error=str(e))
    ...
    async def analyze_domain(self, domain: str) -> VirusTotalDomainResponse:
        ...
        try:
            ...
            response = VirusTotalDomainResponse(
                ...
                last_update_date=self._safe_get_datetime(
                ...
                subdomains=list(attributes.get("subdomains", {}).keys())[
                    :50
            ...
            return VirusTotalDomainResponse(success=False, domain=domain, error=str(e))
    ...
    async def analyze_file_hash(self, file_hash: str) -> VirusTotalFileResponse:
        ...
        try:
            ...
            response = VirusTotalFileResponse(
                ...
                harmless_votes=votes.get("harmless", 0),
                malicious_votes=votes.get("malicious", 0),
                first_submission_date=self._safe_get_datetime(
                    attributes.get("first_submission_date")
                ),
                last_analysis_date=self._safe_get_datetime(
                    attributes.get("last_analysis_date")
                ),
                last_submission_date=self._safe_get_datetime(
                    attributes.get("last_submission_date")
                ),
                response_time_ms=result["response_time_ms"],
            )
            ...
            return VirusTotalFileResponse(
                success=False, file_hash=file_hash, hash_type=hash_type, error=str(e)
            )
