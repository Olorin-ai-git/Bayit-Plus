...
from .models import VirusTotalConfig, VirusTotalDomainResponse
...
class DomainAnalysisInput(BaseModel):
    ...
    domain: str = Field(
        ...,
        description="Domain name to analyze (e.g., 'example.com'). IP addresses are not supported - use IP analysis tools instead.",
    )
    include_subdomains: bool = Field(
        default=False, description="Whether to include subdomain analysis in the report"
    )
    include_whois: bool = Field(
        default=True, description="Whether to include WHOIS information in the analysis"
    )
    max_detections: int = Field(
        default=10, description="Maximum number of detection details to include (1-50)"
    )
    ...
    def validate_domain(cls, v):
        ...
        try:
            ...
            logger.info(
                f"IP address {domain} detected, fetching associated email domain from Snowflake"
            )
            ...
            try:
                ...
                from app.service.agent.tools.async_helpers import safe_run_async
                from app.service.agent.tools.snowflake_tool.client import (
                    SnowflakeClient,
                )
                ...
                async def get_email_domain():
                    ...
                    if results and results[0]:
                        email_value = (
                            results[0].get("EMAIL")
                            or results[0].get("email")
                            or results[0].get(email_column)
                        )
                    ...
                    if email_value:
                        ...
                        if "@" in email_value:
                            ...
                            logger.info(
                                f"Found email domain {email_domain} for IP {domain}"
                            )
                            return email_domain
                    return None
                ...
                if email_domain:
                    ...
                    raise ValueError(
                        f"Domain analysis cannot be performed on IP address '{domain}'. No associated email domain found."
                    )

            except Exception as e:
                logger.error(f"Failed to fetch email domain for IP {domain}: {e}")
                raise ValueError(
                    f"Domain analysis cannot be performed on IP address '{domain}'. Failed to fetch associated email domain."
                )
...
class VirusTotalDomainAnalysisTool(BaseTool):
        ...
        **kwargs,
        ...
        try:
            ...
            if test_mode:
                ...
                logger = get_bridge_logger(__name__)
                logger.info(
                    f"Mock mode active - returning mock response for domain {domain}"
                )
                return json.dumps(
                    {
                        "domain": domain,
                        ...
                        "community_votes": {"harmless": 5, "malicious": 0},
            ...
            if not domain_response.success:
                ...
                if (
                    "InvalidArgumentError" in error_msg
                    and "not a valid domain pattern" in error_msg
                ):
                    logger = get_bridge_logger(__name__)
                    logger.debug(
                        f"VirusTotal domain analysis validation failed for {domain}: {error_msg}"
                    )
                    return json.dumps(
                        {
                    ...
                    logger = get_bridge_logger(__name__)
                    logger.warning(
                        f"VirusTotal domain analysis failed for {domain}: {error_msg}"
                    )
                    return json.dumps(
                        {
                            "status": "error",
            ...
            if (
                "InvalidArgumentError" in error_str
                and "not a valid domain pattern" in error_str
            ):
                logger.debug(
                    f"VirusTotal domain analysis skipped for {domain}: invalid domain pattern (likely IP address)"
                )
                return json.dumps(
                    {
                        "status": "skipped",
                ...
                "VirusTotal API error 400" in error_str
                and "not a valid domain pattern" in error_str
            ):
                # Handle the specific error format seen in the logs
                logger.debug(
                    f"VirusTotal domain analysis failed for {domain}: API validation error"
                )
                return json.dumps(
                    {
                        "status": "validation_error",
                ...
                "quota" in error_str.lower() or "limit" in error_str.lower()
            ):
                logger.debug(
                    f"VirusTotal domain analysis rate limited for {domain}: {error_str}"
                )
                return json.dumps(
                    {
                        "status": "rate_limited",
                ...
                logger.error(
                    f"VirusTotal domain analysis authentication failed for {domain}: {error_str}"
                )
                return json.dumps(
                    {
                ...
                logger.warning(
                    f"VirusTotal domain analysis unavailable for {domain}: {type(e).__name__}"
                )
                return json.dumps(
                    {
                        "status": "unavailable",
    ...
    async def _build_domain_analysis(
        ...
        if (
            include_whois
            and hasattr(domain_response, "whois_date")
            and domain_response.whois_date
        ):
        ...
        domain_info["investigation_recommendations"] = (
            self._generate_domain_recommendations(domain_response, domain)
        )

        # Additional context
        domain_info["analysis_metadata"] = {
            "last_analysis_date": (
                str(domain_response.last_analysis_date)
                if domain_response.last_analysis_date
                else None
            ),
            "last_modification_date": (
                str(domain_response.last_modification_date)
                if domain_response.last_modification_date
                else None
            ),
            "popularity_ranks": domain_response.popularity_ranks or {},
            "total_votes": {
                "harmless": (
                    domain_response.total_votes.get("harmless", 0)
                    if domain_response.total_votes
                    else 0
                ),
                "malicious": (
                    domain_response.total_votes.get("malicious", 0)
                    if domain_response.total_votes
                    else 0
                ),
            },
        ...
        return {"domain_analysis": domain_info}

    def _assess_domain_risk(
        self, domain_response: VirusTotalDomainResponse
    ) -> Dict[str, Any]:
        ...
        if domain_response.tags:
            found_dangerous_tags = [
                tag
                for tag in domain_response.tags
                if any(d in tag.lower() for d in dangerous_tags)
            ]
    ...
    def _generate_domain_recommendations(
        self, domain_response: VirusTotalDomainResponse, domain: str
    ) -> List[str]:
        ...
        if not domain_response.last_analysis_stats:
            return [
                "Insufficient data for recommendations - consider additional analysis"
            ]
        ...
        if stats.malicious > 5:
            recommendations.append(
                "CRITICAL: Multiple security vendors flagged this domain - block immediately"
            )
            recommendations.append(
                "Investigate all connections to this domain in your environment"
            )
        elif stats.malicious > 0:
            recommendations.append(
                "Domain flagged as malicious - investigate further before allowing access"
            )

        if stats.suspicious > 3:
            recommendations.append(
                "High suspicious activity - monitor traffic to this domain"
            )

        # Reputation-based recommendations
        if domain_response.reputation < -50:
            recommendations.append(
                "Extremely poor reputation - likely compromised or malicious"
            )
        elif domain_response.reputation < -10:
            recommendations.append(
                "Poor reputation - exercise caution with this domain"
            )

        # Tag-based recommendations
        if domain_response.tags:
            if any(
                tag.lower() in ["phishing", "malware"] for tag in domain_response.tags
            ):
                recommendations.append(
                    "Domain associated with phishing/malware - high priority threat"
                )
            if any(tag.lower() in ["botnet", "c2"] for tag in domain_response.tags):
                recommendations.append(
                    "Potential command & control infrastructure - investigate network traffic"
                )
        ...
        if hasattr(domain_response, "subdomains") and domain_response.subdomains:
            if len(domain_response.subdomains) > 50:
                recommendations.append(
                    "Large number of subdomains - possible domain generation algorithm (DGA)"
                )
        ...
        if not recommendations:
            if stats.harmless > stats.malicious + stats.suspicious:
                ...
                recommendations.append(
                    "Mixed results - investigate domain context and usage"
                )
