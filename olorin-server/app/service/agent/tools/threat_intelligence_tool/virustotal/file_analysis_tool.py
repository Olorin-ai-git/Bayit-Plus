...
from .models import VirusTotalConfig, VirusTotalFileResponse
...
class FileAnalysisInput(BaseModel):
    ...
    file_hash: str = Field(
        ..., description="File hash to analyze (MD5, SHA1, or SHA256)"
    )
    ...
    max_detections: int = Field(
        default=15, description="Maximum number of detection details to include (1-50)"
    )
    ...
    def validate_file_hash(cls, v):
        ...
        if len(hash_value) == 32:  # MD5
            ...
            raise ValueError(
                "Hash must be MD5 (32 chars), SHA1 (40 chars), or SHA256 (64 chars)"
            )
...
class VirusTotalFileAnalysisTool(BaseTool):
    ...
    def _calculate_risk_level_from_stats(self, stats) -> str:
        ...
        detection_rate = (
            (stats.malicious + stats.suspicious) / stats.total_engines
        ) * 100
    ...
    async def _build_file_analysis(
        ...
        if (
            hasattr(file_response, "type_description")
            and file_response.type_description
        ):
        ...
        # Hash values
        if (
            hasattr(file_response, "md5")
            or hasattr(file_response, "sha1")
            or hasattr(file_response, "sha256")
        ):
        ...
        if (
            hasattr(file_response, "popular_threat_classification")
            and file_response.popular_threat_classification
        ):
            classification = file_response.popular_threat_classification
            file_info["threat_classification"] = {
                "suggested_threat_label": classification.get(
                    "suggested_threat_label", "Unknown"
                ),
                "popular_threat_category": list(
                    classification.get("popular_threat_category", [])
                ),
                "popular_threat_name": list(
                    classification.get("popular_threat_name", [])
                ),
            }
                    ...
                    detections.append(
                        {
                            ...
                            "engine_update": (
                                str(result.engine_update)
                                if result.engine_update
                                else None
                            ),
                        }
                    )
        ...
        if (
            include_behavior
            and hasattr(file_response, "sandbox_verdicts")
            and file_response.sandbox_verdicts
        ):
            behavioral_info = {}
        ...
        if include_network:
            ...
            if hasattr(file_response, "contacted_ips") and file_response.contacted_ips:
                network_indicators["contacted_ips"] = file_response.contacted_ips[
                    :10
                ]  # Limit to 10

            if (
                hasattr(file_response, "contacted_domains")
                and file_response.contacted_domains
            ):
                network_indicators["contacted_domains"] = (
                    file_response.contacted_domains[:10]
                )  # Limit to 10

            if (
                hasattr(file_response, "contacted_urls")
                and file_response.contacted_urls
            ):
                network_indicators["contacted_urls"] = file_response.contacted_urls[
                    :10
                ]  # Limit to 10
        ...
        file_info["investigation_recommendations"] = (
            self._generate_file_recommendations(file_response, file_hash)
        )

        # Additional context
        file_info["analysis_metadata"] = {
            "first_submission_date": (
                str(file_response.first_submission_date)
                if file_response.first_submission_date
                else None
            ),
            "last_analysis_date": (
                str(file_response.last_analysis_date)
                if file_response.last_analysis_date
                else None
            ),
            "last_modification_date": (
                str(file_response.last_modification_date)
                if file_response.last_modification_date
                else None
            ),
            ...
            "total_votes": {
                "harmless": (
                    file_response.total_votes.get("harmless", 0)
                    if file_response.total_votes
                    else 0
                ),
                "malicious": (
                    file_response.total_votes.get("malicious", 0)
                    if file_response.total_votes
                    else 0
                ),
            },
        ...
        return {"file_analysis": file_info}
    ...
    def _assess_file_risk(
        self, file_response: VirusTotalFileResponse
    ) -> Dict[str, Any]:
        ...
        detection_ratio = (stats.malicious + stats.suspicious) / max(
        ...
        if (
            hasattr(file_response, "sandbox_verdicts")
            and file_response.sandbox_verdicts
        ):
            malicious_sandboxes = [
                name
                for name, verdict in file_response.sandbox_verdicts.items()
                if verdict.get("category") == "malicious"
            ]
            if malicious_sandboxes:
                risk_score += len(malicious_sandboxes) * 15
                risk_factors.append(
                    f"Malicious sandbox verdicts: {', '.join(malicious_sandboxes[:3])}"
                )
    ...
    def _generate_file_recommendations(
        self, file_response: VirusTotalFileResponse, file_hash: str
    ) -> List[str]:
        ...
        if not file_response.last_analysis_stats:
            return [
                "Insufficient data for recommendations - consider submitting file for analysis"
            ]
        ...
        if stats.malicious > 15:
            recommendations.append(
                "CRITICAL: File detected as malware by multiple engines - quarantine immediately"
            )
            recommendations.append(
                "Investigate all systems where this file was present"
            )
            recommendations.append(
            ...
            recommendations.append(
                "File flagged as malicious - isolate affected systems"
            )
            recommendations.append(
                "Perform forensic analysis to determine attack vector"
            )
        elif stats.malicious > 0:
            recommendations.append(
                "Potential malware detected - investigate file origin and distribution"
            )

        # Suspicious activity recommendations
        if stats.suspicious > 5:
            recommendations.append(
                "High suspicious activity - analyze file behavior in sandbox environment"
            )
        ...
        if (
            ...
            if "ransomware" in threat_label:
                recommendations.append(
                    "RANSOMWARE DETECTED: Check for encrypted files and backup integrity"
                )
            elif "trojan" in threat_label:
                recommendations.append(
                    "Trojan detected: Monitor for unauthorized access and data exfiltration"
                )
            elif "keylogger" in threat_label or "stealer" in threat_label:
                recommendations.append(
                    "Credential theft malware: Reset all passwords for affected users"
                )
            elif "rootkit" in threat_label:
                recommendations.append(
                    "Rootkit detected: Perform deep system scan and consider reimaging"
                )

        # Network-based recommendations
        if hasattr(file_response, "contacted_ips") and file_response.contacted_ips:
            recommendations.append(
                "File makes network connections - monitor network traffic for C&C communication"
            )
        ...
        if not recommendations:
            if stats.harmless > stats.malicious + stats.suspicious:
                ...
                recommendations.append(
                    "Mixed detection results - investigate file context and usage"
                )
