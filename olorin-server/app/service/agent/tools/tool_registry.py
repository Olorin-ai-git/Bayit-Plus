...
if env_path.exists():
    ...
    tool_count = sum(
        1 for k in os.environ if k.startswith("USE_") and os.getenv(k) == "true"
    )
    if tool_count > 0:
    ...
    if alt_env_path.exists():
        ...
        if tool_count > 0:
            print(
                f"✅ Loaded .env file from alternate path with {tool_count} enabled tools"
            )
    else:
...
try:
    ...
    from .threat_intelligence_tool.unified_threat_intelligence_tool import (
...
class ToolRegistry:
    ...
    def _safe_async_run(self, coro_func):
        ...
        try:
            ...
            logger.info(
                "🔄 Detected running event loop - creating task for MCP server registration"
            )
            ...
            try:
                ...
                logger.info(
                    "📅 MCP server registration task scheduled in existing event loop"
                )
                return None
    ...
    def _ensure_initialized(self) -> None:
        ...
        with self._initialization_lock:
            ...
            logger.warning(
                "⚠️ Tool registry requires (re)initialization - performing emergency initialization"
            )
            try:
                self.initialize()
                ...
                if not self._initialized:
                    raise RuntimeError(
                        "Registry initialization flag not set after initialization"
                    )

                if len(self._tools) == 0:
                    logger.error(
                        "❌ Emergency initialization failed - no tools loaded!"
                    )
                    raise RuntimeError(
                        "No tools were loaded during emergency initialization"
                    )
                ...
                for tool in sample_tools:
                    if not hasattr(tool, "invoke") or not hasattr(tool, "name"):
                        logger.error(
                            f"❌ Tool {getattr(tool, 'name', 'unknown')} is missing required methods"
                        )
                        raise RuntimeError(
                            f"Tool {getattr(tool, 'name', 'unknown')} improperly initialized"
                        )

                logger.info(
                    f"✅ Emergency initialization successful: {len(self._tools)} tools loaded and validated"
                )
                ...
                self._initialized = False
                raise RuntimeError(
                    f"Tool registry initialization failed: {init_error}"
                ) from init_error
            finally:
    ...
    def initialize(
        ...
        if self._initialized:
            ...
            if not has_database_connection and not has_no_tools:
                logger.debug(
                    "Tool registry already initialized (skipping duplicate initialization)"
                )
                return
            ...
            if has_database_connection or has_no_tools:
                if has_database_connection:
                    logger.info(
                        "Tool registry reinitializing with database connection string"
                    )
                if has_no_tools:
                    logger.warning(
                        "Tool registry detected 0 tools - performing reinitialization"
                    )
                self._tools.clear()
        ...
        try:
            ...
            if (
                ...
                and os.getenv("USE_DATABASE_QUERY", "false").lower() == "true"
            ):
                self._register_tool(
            ...
            if (
                database_connection_string
                and os.getenv("USE_DATABASE_SCHEMA", "false").lower() == "true"
            ):
                self._register_tool(
            ...
            if os.getenv("USE_WEB_SEARCH", "false").lower() == "true":
                self._register_tool(
                    WebSearchTool(user_agent=web_search_user_agent), "web"
                )
            if os.getenv("USE_WEB_SCRAPE", "false").lower() == "true":
                self._register_tool(
                    WebScrapeTool(user_agent=web_search_user_agent), "web"
                )
            ...
            if (
                os.getenv("USE_SPLUNK", "false").lower() == "true"
                and SPLUNK_TOOL_AVAILABLE
            ):
                try:
                ...
                os.getenv("USE_SPLUNK", "false").lower() == "true"
                and not SPLUNK_TOOL_AVAILABLE
            ):
                logger.warning(
                    "Splunk tool requested but not available due to missing dependencies"
                )
            else:
            ...
            if os.getenv("USE_SUMO_LOGIC", "false").lower() == "true":
                try:
                    ...
                    logger.info(
                        "SumoLogic tool registered (enabled via USE_SUMO_LOGIC=true)"
                    )
                except Exception as e:
            ...
            if os.getenv("USE_SNOWFLAKE", "false").lower() == "true":
                try:
                    ...
                    logger.info(
                        "Snowflake tool registered (enabled via USE_SNOWFLAKE=true)"
                    )
                except Exception as e:
            ...
            try:
                ...
                from app.service.agent.tools.anomaly_tools.attach_evidence import (
                    AttachEvidenceTool,
                )
                from app.service.agent.tools.anomaly_tools.detect_anomalies import (
                    DetectAnomaliesTool,
                )
                from app.service.agent.tools.anomaly_tools.entity_extractor import (
                    EntityExtractorTool,
                )
                from app.service.agent.tools.anomaly_tools.fetch_series import (
                    FetchSeriesTool,
                )
                from app.service.agent.tools.anomaly_tools.list_anomalies import (
                    ListAnomaliesTool,
                )
                from app.service.agent.tools.anomaly_tools.open_investigation import (
                    OpenInvestigationTool,
                )
                from app.service.agent.tools.anomaly_tools.segment_hunter import (
                    SegmentHunterTool,
                )
                ...
                logger.info(
                    "Anomaly detection tools registered (fetch_series, detect_anomalies, list_anomalies, open_investigation, attach_evidence, segment_hunter, entity_extractor, anomaly_correlator)"
                )
            except Exception as e:
            ...
            if os.getenv("USE_DEVICE_FINGERPRINT", "true").lower() == "true":
                try:
                    ...
                    logger.info(
                        "Device fingerprint tool registered (enabled via USE_DEVICE_FINGERPRINT=true)"
                    )
                except Exception as e:
            ...
            if os.getenv("USE_MAXMIND_MINFRAUD", "true").lower() == "true":
                try:
                    ...
                    logger.info(
                        "MaxMind minFraud tool registered (enabled via USE_MAXMIND_MINFRAUD=true)"
                    )
                except Exception as e:
            ...
            if os.getenv("USE_COMPOSIO", "true").lower() == "true":
                try:
                    ...
                    logger.info(
                        "Composio tool registered (enabled via USE_COMPOSIO=true)"
                    )
                except Exception as e:
            ...
            if os.getenv("USE_COMPOSIO_SEARCH", "true").lower() == "true":
                try:
                    ...
                    logger.info(
                        "Composio Search tool registered (enabled via USE_COMPOSIO_SEARCH=true)"
                    )
                except Exception as e:
            ...
            if os.getenv("USE_COMPOSIO_WEBCRAWL", "true").lower() == "true":
                try:
                    ...
                    logger.info(
                        "Composio WebCrawl tool registered (enabled via USE_COMPOSIO_WEBCRAWL=true)"
                    )
                except Exception as e:
            ...
            if os.getenv("USE_VERIPHONE", "true").lower() == "true":
                try:
                    ...
                    logger.info(
                        "Veriphone tool registered (enabled via USE_VERIPHONE=true)"
                    )
                except Exception as e:
            ...
            if os.getenv("USE_IPQS_EMAIL", "true").lower() == "true":
                try:
                    ...
                    logger.info(
                        "IPQS Email tool registered (enabled via USE_IPQS_EMAIL=true)"
                    )
                except Exception as e:
            ...
            if os.getenv("USE_GRAPH_FEATURES", "true").lower() == "true":
                try:
                    ...
                    logger.info(
                        "Graph feature tool registered (enabled via USE_GRAPH_FEATURES=true)"
                    )
                except Exception as e:
            ...
            if MCP_CLIENTS_AVAILABLE:
                ...
                if os.getenv("USE_INTELLIGENCE_MCP_CLIENT", "false").lower() == "true":
                    try:
                        ...
                        logger.warning(
                            f"Failed to register Intelligence MCP client: {e}"
                        )
            ...
            if THREAT_INTEL_AVAILABLE:
                ...
                if os.getenv("USE_ABUSEIPDB_IP_REPUTATION", "false").lower() == "true":
                    try:
                        self._register_tool(
                            SimpleIPReputationTool(), "threat_intelligence"
                        )
                        logger.info("AbuseIPDB IP reputation tool registered")
                    except Exception as e:
                        logger.warning(
                            f"Failed to register AbuseIPDB IP reputation tool: {e}"
                        )
                ...
                if os.getenv("USE_ABUSEIPDB_BULK_ANALYSIS", "false").lower() == "true":
                    try:
                        ...
                        logger.warning(
                            f"Failed to register AbuseIPDB bulk IP analysis tool: {e}"
                        )

                if os.getenv("USE_ABUSEIPDB_CIDR_BLOCK", "false").lower() == "true":
                    try:
                        self._register_tool(
                            CIDRBlockAnalysisTool(), "threat_intelligence"
                        )
                        logger.info("AbuseIPDB CIDR block analysis tool registered")
                    except Exception as e:
                        logger.warning(
                            f"Failed to register AbuseIPDB CIDR block analysis tool: {e}"
                        )

                if (
                    os.getenv("USE_ABUSEIPDB_ABUSE_REPORTING", "false").lower()
                    == "true"
                ):
                    try:
                        ...
                        logger.warning(
                            f"Failed to register AbuseIPDB abuse reporting tool: {e}"
                        )
                ...
                if os.getenv("USE_VIRUSTOTAL_IP_ANALYSIS", "false").lower() == "true":
                    try:
                        self._register_tool(
                            VirusTotalIPAnalysisTool(), "threat_intelligence"
                        )
                        logger.info("VirusTotal IP analysis tool registered")
                    except Exception as e:
                        logger.warning(
                            f"Failed to register VirusTotal IP analysis tool: {e}"
                        )

                if (
                    os.getenv("USE_VIRUSTOTAL_DOMAIN_ANALYSIS", "false").lower()
                    == "true"
                ):
                    try:
                        self._register_tool(
                            VirusTotalDomainAnalysisTool(), "threat_intelligence"
                        )
                        logger.info("VirusTotal domain analysis tool registered")
                    except Exception as e:
                        logger.warning(
                            f"Failed to register VirusTotal domain analysis tool: {e}"
                        )

                if os.getenv("USE_VIRUSTOTAL_FILE_ANALYSIS", "false").lower() == "true":
                    try:
                        self._register_tool(
                            VirusTotalFileAnalysisTool(), "threat_intelligence"
                        )
                        logger.info("VirusTotal file analysis tool registered")
                    except Exception as e:
                        logger.warning(
                            f"Failed to register VirusTotal file analysis tool: {e}"
                        )

                if os.getenv("USE_VIRUSTOTAL_URL_ANALYSIS", "false").lower() == "true":
                    try:
                        self._register_tool(
                            VirusTotalURLAnalysisTool(), "threat_intelligence"
                        )
                        logger.info("VirusTotal URL analysis tool registered")
                    except Exception as e:
                        logger.warning(
                            f"Failed to register VirusTotal URL analysis tool: {e}"
                        )
                ...
                if os.getenv("USE_SHODAN_INFRASTRUCTURE", "false").lower() == "true":
                    try:
                        self._register_tool(
                            ShodanInfrastructureAnalysisTool(), "threat_intelligence"
                        )
                        logger.info("Shodan infrastructure analysis tool registered")
                    except Exception as e:
                        logger.warning(
                            f"Failed to register Shodan infrastructure analysis tool: {e}"
                        )
                ...
                if os.getenv("USE_SHODAN_EXPLOIT_SEARCH", "false").lower() == "true":
                    try:
                        self._register_tool(
                            ShodanExploitSearchTool(), "threat_intelligence"
                        )
                        logger.info("Shodan exploit search tool registered")
                    except Exception as e:
                        logger.warning(
                            f"Failed to register Shodan exploit search tool: {e}"
                        )
                ...
                if os.getenv("USE_UNIFIED_THREAT_INTEL", "false").lower() == "true":
                    try:
                        self._register_tool(
                            UnifiedThreatIntelligenceTool(), "threat_intelligence"
                        )
                        logger.info("Unified threat intelligence tool registered")
                    except Exception as e:
                        logger.warning(
                            f"Failed to register unified threat intelligence tool: {e}"
                        )

            # Blockchain Tools
            if BLOCKCHAIN_TOOLS_AVAILABLE:
                if (
                    os.getenv("USE_BLOCKCHAIN_WALLET_ANALYSIS", "false").lower()
                    == "true"
                ):
                    try:
                        self._register_tool(
                            BlockchainWalletAnalysisTool(), "blockchain"
                        )
                        logger.info("Blockchain wallet analysis tool registered")
                    except Exception as e:
                        logger.warning(
                            f"Failed to register blockchain wallet analysis tool: {e}"
                        )
                ...
                if os.getenv("USE_CRYPTOCURRENCY_TRACING", "false").lower() == "true":
                    try:
                        ...
                        logger.warning(
                            f"Failed to register cryptocurrency tracing tool: {e}"
                        )
                ...
                if os.getenv("USE_DEFI_PROTOCOL_ANALYSIS", "false").lower() == "true":
                    try:
                        ...
                        logger.warning(
                            f"Failed to register DeFi protocol analysis tool: {e}"
                        )
                ...
                if os.getenv("USE_NFT_FRAUD_DETECTION", "false").lower() == "true":
                    try:
                        ...
                        logger.warning(
                            f"Failed to register NFT fraud detection tool: {e}"
                        )
                ...
                if os.getenv("USE_BLOCKCHAIN_FORENSICS", "false").lower() == "true":
                    try:
                        ...
                        logger.warning(
                            f"Failed to register blockchain forensics tool: {e}"
                        )
                ...
                if os.getenv("USE_CRYPTO_EXCHANGE_ANALYSIS", "false").lower() == "true":
                    try:
                        ...
                        logger.warning(
                            f"Failed to register crypto exchange analysis tool: {e}"
                        )
                ...
                if os.getenv("USE_DARKWEB_CRYPTO_MONITOR", "false").lower() == "true":
                    try:
                        ...
                        logger.warning(
                            f"Failed to register dark web crypto monitor tool: {e}"
                        )

                if (
                    os.getenv("USE_CRYPTOCURRENCY_COMPLIANCE", "false").lower()
                    == "true"
                ):
                    try:
                        self._register_tool(
                            CryptocurrencyComplianceTool(), "blockchain"
                        )
                        logger.info("Cryptocurrency compliance tool registered")
                    except Exception as e:
                        logger.warning(
                            f"Failed to register cryptocurrency compliance tool: {e}"
                        )
            ...
            if INTELLIGENCE_TOOLS_AVAILABLE:
                if os.getenv("USE_SOCIAL_MEDIA_PROFILING", "false").lower() == "true":
                    try:
                        ...
                        logger.warning(
                            f"Failed to register social media profiling tool: {e}"
                        )
                ...
                if os.getenv("USE_SOCIAL_NETWORK_ANALYSIS", "false").lower() == "true":
                    try:
                        ...
                        logger.warning(
                            f"Failed to register social network analysis tool: {e}"
                        )
                ...
                if os.getenv("USE_SOCIAL_MEDIA_MONITORING", "false").lower() == "true":
                    try:
                        ...
                        logger.warning(
                            f"Failed to register social media monitoring tool: {e}"
                        )
                ...
                if os.getenv("USE_OSINT_DATA_AGGREGATOR", "false").lower() == "true":
                    try:
                        ...
                        logger.warning(
                            f"Failed to register OSINT data aggregator tool: {e}"
                        )
                ...
                if os.getenv("USE_BUSINESS_INTELLIGENCE", "false").lower() == "true":
                    try:
                        ...
                        logger.warning(
                            f"Failed to register business intelligence tool: {e}"
                        )
                ...
                if os.getenv("USE_DARKWEB_MONITORING", "false").lower() == "true":
                    try:
                        ...
                        logger.warning(
                            f"Failed to register dark web monitoring tool: {e}"
                        )
            ...
            if ML_AI_TOOLS_AVAILABLE:
                if os.getenv("USE_BEHAVIORAL_ANALYSIS", "false").lower() == "true":
                    try:
                        ...
                        logger.warning(
                            f"Failed to register behavioral analysis ML tool: {e}"
                        )
                ...
                if os.getenv("USE_ANOMALY_DETECTION", "false").lower() == "true":
                    try:
                        ...
                        logger.warning(
                            f"Failed to register anomaly detection ML tool: {e}"
                        )
                ...
                if os.getenv("USE_PATTERN_RECOGNITION", "false").lower() == "true":
                    try:
                        ...
                        logger.warning(
                            f"Failed to register pattern recognition ML tool: {e}"
                        )
                ...
                if os.getenv("USE_FRAUD_DETECTION", "false").lower() == "true":
                    try:
                        ...
                        logger.warning(
                            f"Failed to register fraud detection ML tool: {e}"
                        )
    ...
    def is_initialized(self) -> bool:
        ...
        return (
            ...
            and not self._initialization_in_progress
        )
...
def get_tools_for_agent(
    ...
    """
    logger.info(
        f"🔍 get_tools_for_agent called with categories: {categories}, tool_names: {tool_names}"
    )
    ...
    logger.info(
        f"📊 Registry state: initialized={tool_registry._initialized}, tools_count={len(tool_registry._tools)}"
    )
    if len(tool_registry._tools) > 0:
        logger.info(
            f"🔧 Available tool names: {list(tool_registry._tools.keys())[:10]}..."
        )  # First 10

    if not tool_registry.is_initialized():
        logger.error(
            "❌ Tool registry initialization failed - returning empty tool list"
        )
        return []
    ...
    if len(tool_registry._tools) == 0:
        ...
        try:
            ...
            logger.info(
                f"✅ Emergency re-initialization completed: {len(tool_registry._tools)} tools now available"
            )
        except Exception as e:
    ...
    if categories:
        ...
        for category in categories:
            ...
            logger.debug(
                f"✅ Added {len(category_tools)} tools from category '{category}'"
            )
    ...
    logger.info(
        f"✅ Retrieved {len(unique_tools)} tools for agent (categories: {categories}, specific: {tool_names})"
    )
    ...
    if len(unique_tools) > 0:
        ...
        if (
            hasattr(test_tool, "invoke")
            and hasattr(test_tool, "name")
            and hasattr(test_tool, "description")
        ):
            logger.info(
                f"✅ Tool validation passed: sample tool '{test_tool.name}' is properly initialized"
            )
        else:
            logger.error(
                f"❌ Tool validation FAILED: sample tool '{test_tool.name}' missing required attributes"
            )
