...
class BlockchainProvider(ABC):
    ...
    @abstractmethod
    async def get_address_info(self, address: str, network: BlockchainNetwork) -> Optional[AddressInfo]:
        """Get information about a blockchain address."""
        pass
    
    @abstractmethod
    async def get_transaction_info(self, tx_id: str, network: BlockchainNetwork) -> Optional[TransactionAnalysis]:
...
class ChainalysisProvider(BlockchainProvider):
    ...
    async def trace_funds(
        ...
        logger.info(f"Tracing funds from {address} on {network.value} with depth {depth}")
        return []
    ...
    def _parse_address_response(self, data: Dict[str, Any], address: str, network: BlockchainNetwork) -> AddressInfo:
...
class BlockchainAnalysisSuite:
    ...
    async def assess_risk(
        ...
        if self.address_validator.validate_bitcoin_address(subject) or \
           self.address_validator.validate_ethereum_address(subject):
    ...
    def _select_provider(self, preference: Optional[str] = None) -> Optional[BlockchainProvider]:
    ...
    async def _enhance_address_analysis(self, address_info: AddressInfo, provider: BlockchainProvider) -> None:
