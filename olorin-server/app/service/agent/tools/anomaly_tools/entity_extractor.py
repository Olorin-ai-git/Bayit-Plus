...
class _EntityExtractorArgs(BaseModel):
    ...
    dimension: str = Field(
        ..., description="Dimension name (e.g., reason_code, issuer, asn)"
    )
    dimension_value: str = Field(
        ..., description="Specific dimension value to extract entities from"
    )
...
class EntityExtractorTool(BaseTool):
    ...
    def _build_entity_extraction_query(
        ...
        for key, value in cohort_filters.items():
            if key.upper() == "MERCHANT_ID":
                col_name = (
                    "STORE_ID" if is_snowflake else "store_id"
                )  # TXS uses STORE_ID
                cohort_conditions.append(f"{col_name} = '{value}'")
            elif key.upper() == "CHANNEL":
                col_name = (
                    "DEVICE_TYPE" if is_snowflake else "device_type"
                )  # TXS uses DEVICE_TYPE
                cohort_conditions.append(f"{col_name} = '{value}'")
            elif key.upper() == "GEO":
    ...
    def _run(
        ...
        try:
            ...
            if not query:
                return {"error": "No valid entity types specified", "entities": []}
            ...
            logger.info(
                f"EntityExtractor: Using database provider: {database_provider} (is_snowflake={is_snowflake})"
            )
            ...
            if is_snowflake:
                ...
                if database_provider == "postgresql" or not database_provider:
                    ...
                    if (
                        ...
                        logger.debug(
                            f"EntityExtractor: Built PostgreSQL connection string from individual env vars"
                        )
            ...
            for row in rows:
                ...
                entities_by_type[entity_type].append(
                    {"entity_value": entity_value, "tx_count": tx_count}
                )
