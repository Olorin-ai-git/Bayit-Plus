...
class _AnomalyCorrelatorArgs(BaseModel):
    ...
    candidate_entities: List[Dict[str, Any]] = Field(
        ...
        description="Candidate entities from EntityExtractor"
    )
    check_sumo: bool = Field(
        True,
        description="Whether to check SumoLogic logs for correlation"
    )
    check_bursts: bool = Field(
        True,
        description="Whether to check for bursty patterns"
    )
    ...
    def _check_burst_patterns(
        ...
        for entity_group in entities:
            ...
            if len(entity_list) > 10:
                avg_tx = sum(e.get('tx_count', 0) for e in entity_list) / len(entity_list)
                bursts.append({
        ...
        try:
            ...
            return {
                ...
                'summary': {
                    ...
                    'high_confidence': len([p for p in root_patterns if p.get('confidence') == 'high']),
                    'has_bursts': burst_patterns.get('has_bursts', False),
                    'has_concentration': concentration_patterns.get('has_concentration', False)
                }
            }
