...
class MaxMindMinFraudInput(BaseModel):
    ...
    billing_country: Optional[str] = Field(
        None, description="Optional billing country code"
    )
    transaction_amount: Optional[float] = Field(
        None, description="Optional transaction amount"
    )
...
class MaxMindMinFraudTool(BaseTool):
    ...
    def _run(
        ...
        logger.info(
            f"🔍 [MaxMindMinFraudTool] Starting IP risk scoring: ip_address={ip_address}, transaction_id={transaction_id}, email={email}, billing_country={billing_country}, amount={transaction_amount}, currency={currency}"
        )
        ...
        try:
            ...
            logger.debug(
                f"🔍 [MaxMindMinFraudTool] MaxMindClient initialized: {self.maxmind_client is not None}"
            )

            # Use async client in sync context
            loop = asyncio.get_event_loop()
            logger.debug(
                f"🔍 [MaxMindMinFraudTool] Event loop status: running={loop.is_running()}"
            )

            if loop.is_running():
                # If loop is running, create a task
                logger.debug(
                    f"🔍 [MaxMindMinFraudTool] Event loop is running, using ThreadPoolExecutor"
                )
                ...
                logger.debug(
                    f"🔍 [MaxMindMinFraudTool] Event loop not running, using run_until_complete"
                )
            ...
            logger.info(
                f"✅ [MaxMindMinFraudTool] IP scored successfully: ip_address={ip_address}, risk_score={risk_score}, result_keys={list(result.keys())}"
            )
            logger.debug(
                f"🔍 [MaxMindMinFraudTool] Full result: {json.dumps(result, indent=2)}"
            )

            result_json = json.dumps(result, indent=2)
            logger.info(
                f"✅ [MaxMindMinFraudTool] Returning result, length={len(result_json)}"
            )
            ...
            logger.error(
                f"❌ [MaxMindMinFraudTool] IP scoring failed: ip_address={ip_address}, error={e}",
                exc_info=True,
            )
            ...
            logger.error(
                f"❌ [MaxMindMinFraudTool] Returning error result: {error_result}"
            )
            return error_result
    ...
    async def _arun(
        ...
        logger.info(
            f"🔍 [MaxMindMinFraudTool] Starting async IP risk scoring: ip_address={ip_address}, transaction_id={transaction_id}"
        )
        ...
        try:
            if self.maxmind_client is None:
                raise RuntimeError("MaxMindClient not initialized")
            logger.debug(
                f"🔍 [MaxMindMinFraudTool] MaxMindClient initialized for async call"
            )
            ...
            risk_score = result.get("risk_score")
            logger.info(
                f"✅ [MaxMindMinFraudTool] Async IP scored successfully: ip_address={ip_address}, risk_score={risk_score}"
            )
            logger.debug(
                f"🔍 [MaxMindMinFraudTool] Async result keys: {list(result.keys())}"
            )
            ...
            logger.error(
                f"❌ [MaxMindMinFraudTool] Async IP scoring failed: ip_address={ip_address}, error={e}",
                exc_info=True,
            )
            return json.dumps(
                {
