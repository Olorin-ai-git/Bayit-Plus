...
class SplunkQueryInput(BaseModel):
    ...
    max_results: int = Field(
        1000, description="Maximum number of results to return", ge=1, le=10000
    )
    timeout_seconds: int = Field(
        300, description="Query timeout in seconds", ge=1, le=3600
    )
    ...
    search_mode: str = Field(
        "normal", description="Search mode: normal, fast, smart, verbose"
    )
    enable_preview: bool = Field(
        False, description="Enable search preview for faster results"
    )
...
class EnhancedSplunkTool(EnhancedToolBase):
    ...
    def __init__(
        ...
        config = ToolConfig(
            ...
            output_schema={"required_fields": ["results", "query_info"]},
    ...
    async def _execute_impl(
        ...
        if self.enable_query_optimization:
            ...
            if optimized_query != query_input.query:
                self.logger.info(
                    f"Query optimized: {query_input.query[:50]}... -> {optimized_query[:50]}..."
                )
        ...
        execution_context = {
            ...
            "execution_id": (
                context.get("execution_id", "unknown") if context else "unknown"
            ),
        ...
        try:
            ...
            async with self.connection_semaphore:
                ...
                try:
                    result = await self._execute_splunk_query(
                        query_input, execution_context
                    )

                    # Post-process results
                    processed_result = await self._post_process_results(
                        result, query_input, execution_context
                    )
            ...
            error_context = {
                "query": (
                    query_input.query[:200] + "..."
                    if len(query_input.query) > 200
            ...
            self.logger.error(
                f"Splunk query execution failed: {str(e)}", extra=error_context
            )
            raise

    async def _execute_splunk_query(
        self, query_input: SplunkQueryInput, execution_context: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if os.getenv("OLORIN_USE_DEMO_DATA", "false").lower() == "true":
            ...
            return {
                ...
                "metadata": {"demo_mode": True, "data_source": "mock_splunk_data"},
            }
        ...
        try:
            ...
            enhanced_results = {
                ...
                "performance": {
                    ...
                    "slow_query_threshold_exceeded": query_execution_time
            ...
            if "timeout" in str(e).lower():
                raise TimeoutError(
                    f"Splunk query timed out after {query_input.timeout_seconds}s: {str(e)}"
                )
                ...
                raise HTTPException(
                    status_code=500,
                    detail=f"Splunk query failed ({error_type}): {str(e)}",
                )
    ...
    async def _post_process_results(
        ...
        results["processing"] = {
            ...
            "suggested_optimizations": await self._analyze_query_for_optimizations(
                query_input.query
            ),
        }
        ...
        if query_results:
            results["data_quality"] = {
                ...
                "estimated_total_matches": self._estimate_total_matches(
            ...
            results["data_quality"] = {
                "has_results": False,
                "empty_result_analysis": await self._analyze_empty_results(
                    query_input.query
                ),
            }
    ...
    async def _optimize_query(self, query: str) -> str:
        ...
        if re.match(self.optimization_patterns["add_time_index"], query, re.IGNORECASE):
            if not re.search(r"index\s*=", query, re.IGNORECASE):
                ...
                self.logger.info(
                    "Query optimization: Consider adding 'index=<your_index>' for better performance"
                )

        # Optimize regex usage
        if re.search(
            self.optimization_patterns["inefficient_regex"], query, re.IGNORECASE
        ):
            # Simple optimization: suggest using search terms instead of regex where possible
            self.logger.info(
                "Query optimization: Consider using search terms instead of regex for better performance"
            )
        ...
        if not re.search(r"\|\s*fields\s+", query, re.IGNORECASE):
            if len(query.split("|")) > 1:  # Multi-command query
                self.logger.info(
                    "Query optimization: Consider adding '| fields <specific_fields>' to reduce data transfer"
                )
    ...
    async def _analyze_query_for_optimizations(self, query: str) -> List[str]:
        ...
        if not re.search(r"earliest\s*=", query, re.IGNORECASE):
            suggestions.append(
                "Consider adding time range constraints with 'earliest=' and 'latest=' for better performance"
            )

        # Index specification
        if not re.search(r"index\s*=", query, re.IGNORECASE):
            suggestions.append(
                "Specify target index(es) with 'index=' to improve search performance"
            )

        # Field extraction
        if "|" in query and not re.search(r"\|\s*fields\s+", query, re.IGNORECASE):
            suggestions.append(
                "Use '| fields <field_list>' to reduce data transfer and improve performance"
            )
        ...
        if re.search(r"\|\s*stats\s+", query, re.IGNORECASE):
            if not re.search(r"\|\s*stats\s+.*\s+by\s+", query, re.IGNORECASE):
                suggestions.append(
                    "Consider using 'by' clause in stats command for better aggregation"
                )
        ...
        if query.strip().startswith("search "):
            if "*" in query[:50]:  # Wildcard in first 50 chars
                suggestions.append(
                    "Avoid leading wildcards in search terms for better performance"
                )
        ...
        if len(results) > 1:
            ...
            for i, result in enumerate(results[1:], 1):
                ...
                if result_fields != first_fields:
                    ...
                    if missing or extra:
                        issues.append(
                            f"Result {i}: field mismatch (missing: {missing}, extra: {extra})"
                        )
    ...
    def _estimate_total_matches(
        self, results: List[Dict[str, Any]], query_input: SplunkQueryInput
    ) -> Optional[int]:
    ...
    async def _analyze_empty_results(self, query: str) -> Dict[str, Any]:
        ...
        analysis = {"possible_causes": [], "suggestions": []}
        ...
        if re.search(r"index\s*=", query, re.IGNORECASE):
            analysis["possible_causes"].append(
                "Data might not exist in specified index"
            )
    ...
    def _is_valid_time_format(self, time_str: str) -> bool:
        ...
        return any(
            re.match(pattern, time_str, re.IGNORECASE) for pattern in valid_patterns
        )
