...
NETWORK_LOCATION_FIELDS = [IP, IP_COUNTRY_CODE, "ASN", "ISP", "MAXMIND_IP_RISK_SCORE"]
...
class SnowflakeQueryTool(BaseTool):
    ...
    account: str = Field(
        "olorin_fraud.snowflakecomputing.com", description="Snowflake account URL"
    )
    warehouse: str = Field(
        "FRAUD_ANALYSIS_WH", description="Snowflake warehouse for query execution"
    )
    ...
    def build_comprehensive_investigation_query(
        ...
        if include_priority_only:
            ...
            logger.info(
                f"🎯 Using priority evidence fields ({len(PRIORITY_EVIDENCE_FIELDS)} columns)"
            )
        else:
            columns_to_use = REAL_COLUMNS
            logger.info(
                f"📊 Using comprehensive evidence fields ({len(REAL_COLUMNS)} columns)"
            )
        ...
        # Build date filter
        date_filter = (
            f"TX_DATETIME >= DATEADD(day, -{date_range_days}, CURRENT_TIMESTAMP())"
        )
    ...
    def get_optimized_investigation_query(
        ...
        logger.info(
            f"🎨 Generated optimized {investigation_focus} query for {entity_type}={entity_id}"
        )
        logger.info(
            f"   Evidence coverage: {query_info['validation']['evidence_coverage_score']}"
        )
        logger.info(
            f"   Performance tier: {query_info['metadata']['performance_estimate']['performance_tier']}"
        )
    ...
    def _build_entity_where_clause(self, entity_type: str, entity_id: str) -> str:
        ...
        if entity_type_upper == "IP":
            ...
            logger.warning(
                f"⚠️ Unknown entity type '{entity_type}', using multi-field search"
            )
            return (
                f"("
                f"{IP} = '{entity_id}' OR "
    ...
    def validate_query_fields(self, query: str) -> Dict[str, Any]:
        ...
        if device_coverage < 2:
            recommendations.append(
                "Include more device analysis fields for better device fingerprinting"
            )
        ...
        if risk_coverage < 2:
            recommendations.append(
                "Include more risk analysis fields for comprehensive fraud assessment"
            )
        ...
        return {
            ...
            "total_evidence_score": max(
                0.0, 1.0 - (len(missing_fields) / len(critical_fields))
            ),
        }
    ...
    def _correct_column_names(self, query: str) -> str:
        ...
        for wrong, correct in corrections.items():
            ...
            if wrong != "IP\\b":  # Special handling for IP pattern
                corrected_query = re.sub(
                    pattern, correct, corrected_query, flags=re.IGNORECASE
                )
            else:
                # For IP, only replace if it's not part of another word
                corrected_query = re.sub(
                    r"\bIP\b(?!_)", "IP", corrected_query, flags=re.IGNORECASE
                )
        ...
        corrected_query = re.sub(
            r",\s*(FROM|WHERE|GROUP|ORDER|LIMIT)",
            r" \1",
            corrected_query,
            flags=re.IGNORECASE,
        )
        ...
        try:
            ...
            if not password:
                ...
                try:
                    ...
                    logger.warning(
                        f"⚠️ Failed to retrieve Snowflake password from secrets: {secret_error}"
                    )
            ...
            try:
                ...
                await tool_logger.log_tool_execution_failure(
                    execution_id=execution_id, error=conn_error, tool_args=tool_args
                )
                ...
                if "authentication" in error_msg or "invalid username" in error_msg:
                    detailed_error = (
                        "Authentication failed - check Snowflake credentials"
                    )
                elif "warehouse" in error_msg:
                    detailed_error = (
                        "Warehouse connection failed - warehouse may be suspended"
                    )
                elif "network" in error_msg or "timeout" in error_msg:
                    detailed_error = (
                        "Network connection failed - check connectivity to Snowflake"
                    )
                else:
            ...
            try:
                ...
                if not results:
                    ...
                    logger.warning(
                        f"   - Column names or table references are incorrect"
                    )
                    logger.warning(
                        f"   - Data may not be available for the specified time period"
                    )
                ...
                query_insights = {
                    "contains_fraud_analysis": any(
                        keyword in corrected_query.upper()
                        for keyword in ["FRAUD", "MODEL_SCORE", "RISK"]
                    ),
                    "contains_user_analysis": any(
                        keyword in corrected_query.upper()
                        for keyword in ["EMAIL", "USER", "CUSTOMER"]
                    ),
                    "contains_payment_analysis": any(
                        keyword in corrected_query.upper()
                        for keyword in ["PAYMENT", "CARD", "BIN"]
                    ),
                    "contains_time_analysis": any(
                        keyword in corrected_query.upper()
                        for keyword in ["TX_DATETIME", "DATE", "TIME"]
                    ),
                    ...
                    "query_complexity": (
                        "complex"
                        if len(corrected_query) > 500
                        else "medium" if len(corrected_query) > 200 else "simple"
                    ),
                    "estimated_data_coverage": (
                        "comprehensive"
                        if len(results) > 100
                        else "partial" if len(results) > 10 else "limited"
                    ),
                }

                # Convert results to JSON-serializable format
                json_safe_results = json.loads(
                    json.dumps(results, cls=SnowflakeJSONEncoder)
                )
                ...
                result_object = {
                    ...
                    "evidence_completeness": {
                        ...
                        "coverage_percentage": round(
                            (len(columns) / len(REAL_COLUMNS)) * 100, 1
                        ),
                        "critical_fields_included": len(
                            [
                                c
                                for c in columns
                                if c
                    ...
                    "schema_info": {
                        ...
                        "field_categories": {
                            "core_transaction": len(
                                [
                                    f
                                    for f in columns
                                    if f
                                    in [TX_ID_KEY, EMAIL, PAID_AMOUNT_VALUE_IN_CURRENCY]
                                ]
                            ),
                            "risk_analysis": len(
                                [
                                    ...
                                    for f in columns
                                    if f
                                    in [MODEL_SCORE, IS_FRAUD_TX, MAXMIND_RISK_SCORE]
                                ]
                            ),
                            "device_analysis": len(
                                [
                                    f
                                    for f in columns
                                    if f in [DEVICE_ID, USER_AGENT, DEVICE_TYPE]
                                ]
                            ),
                            "network_analysis": len(
                                [f for f in columns if f in [IP, IP_COUNTRY_CODE]]
                            ),
                        },
                        "main_table": get_required_env_var(
                            "SNOWFLAKE_TRANSACTIONS_TABLE"
                        ),
                    },
                }
                ...
                await tool_logger.log_tool_execution_success(
                    execution_id=execution_id, result=result_object
                )
                ...
                await tool_logger.log_tool_execution_failure(
                    execution_id=execution_id, error=query_error, tool_args=tool_args
                )
                ...
                if "syntax error" in error_msg or "sql compilation error" in error_msg:
                    ...
                    suggestion = (
                        "Check if the Snowflake warehouse is active and not suspended."
                    )
                else:
            ...
            if client:
                try:
                    ...
                    logger.warning(
                        f"⚠️ Error closing Snowflake connection: {disconnect_error}"
                    )
