...
def get_correct_column_name(old_name: str) -> str:
    ...
    if old_name in ALL_COLUMN_NAMES:
        return old_name
    raise ValueError(
        f"Invalid column name '{old_name}' - not found in schema. Only valid schema columns allowed."
    )
...
print(
    f"✅ Schema constants loaded: {ACTUAL_COLUMN_COUNT} columns validated for {table_name} ({db_provider.upper()})"
)
...
def get_required_env_var(var_name: str) -> str:
    ...
    if not value:
        raise ValueError(
            f"❌ Required environment variable {var_name} not found in .env file!"
        )
    return value


def get_full_table_name(
    database: str = None, schema: str = None, table_name: str = None
) -> str:
    """
