...
class RealSnowflakeClient:
    ...
    def _load_configuration(self):
        ...
        self.auth_method = config.get("auth_method") or os.getenv(
            "SNOWFLAKE_AUTH_METHOD", "password"
        )
        self.private_key_path = config.get("private_key_path") or os.getenv(
            "SNOWFLAKE_PRIVATE_KEY_PATH"
        )
        self.private_key_passphrase = config.get("private_key_passphrase") or os.getenv(
        ...
        if self.auth_method == "private_key":
            if not self.private_key_path:
                missing.append(
                    "SNOWFLAKE_PRIVATE_KEY_PATH (required for private_key auth)"
                )
            elif not Path(self.private_key_path).exists():
                missing.append(
                    f"SNOWFLAKE_PRIVATE_KEY_PATH file not found: {self.private_key_path}"
                )
            if not CRYPTOGRAPHY_AVAILABLE:
                logger.error(
                    "cryptography library not available - required for private key authentication"
                )
                missing.append(
                    "cryptography library (install with: poetry add cryptography)"
                )
        ...
        if missing:
            logger.error(
                f"Missing critical Snowflake configuration: {', '.join(missing)}"
            )
    ...
    def _load_private_key(self) -> bytes:
        ...
        try:
            ...
            key_name = (
                Path(self.private_key_path).name if self.private_key_path else "not set"
            )
        ...
        if self.connection is None:
            try:
                ...
                if self.auth_method == "private_key":
                    ...
                    logger.info(
                        f"Using private key authentication for user: {self.user}"
                    )
                    try:
                    ...
                    logger.info(
                        f"Using OAuth token authentication for user: {self.user}"
                    )
                elif self.auth_method == "externalbrowser":
                    # External browser (SSO) authentication
                    conn_params["authenticator"] = "externalbrowser"
                    logger.info(
                        f"Using external browser (SSO) authentication for user: {self.user}"
                    )
                else:
                    # Default password authentication
                ...
                if self.auth_method == "private_key":
                    ...
                    key_name = (
                        Path(self.private_key_path).name
                ...
                try:
                    ...
                    logger.info(
                        f"✅ Successfully connected to Snowflake: {self.database}.{self.schema}"
                    )
                except Exception as cursor_error:
                    logger.error(
                        f"Failed to create cursor after connection: {cursor_error}"
                    )
                ...
                logger.error(
                    "snowflake-connector-python not installed. Run: poetry add snowflake-connector-python"
                )
                raise
            except Exception as e:
    ...
    def validate_query(self, query: str, limit: Optional[int] = None) -> str:
        ...
        if limit and "LIMIT" not in query_upper:
            query = (
                f"{query.rstrip(';')} LIMIT {min(limit, self.max_transactions_limit)}"
            )
    ...
    def _execute_query_sync(self, query: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if "'NoneType' object has no attribute" in str(
                e
    ...
    async def execute_query(
        ...
        results = await loop.run_in_executor(
            executor, self._execute_query_sync, safe_query
        )
    ...
    async def test_connection(self) -> bool:
        ...
        try:
            ...
            results = await self.execute_query(
                "SELECT CURRENT_VERSION() as version, CURRENT_DATABASE() as db"
            )
            if results:
