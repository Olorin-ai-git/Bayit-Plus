...
class EnhancedCache:
            ...
            if self.enable_content_deduplication:
                ...
                if content_hash in self.content_hashes:
                    ...
                    if existing_key in self.entries:
                        ...
                        self.logger.debug(f"Content deduplication: {key} -> {existing_key}")
    ...
    async def delete(self, key: str) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Cache delete error for key {key}: {str(e)}", exc_info=True)
            return False
    ...
    async def _ensure_capacity(self, new_entry_size: int) -> None:
        ...
        # Size-based eviction
        while (self.stats.total_size_bytes + new_entry_size > self.max_memory_bytes and 
               self.entries):
    ...
    def _update_get_stats(self, start_time: float, hit: bool) -> None:
        ...
        self.stats.avg_get_time_ms = (
            (self.stats.avg_get_time_ms * (total_ops - 1) + duration_ms) / total_ops
        )
    ...
    async def _clear_redis(self) -> None:
        ...
        try:
            if hasattr(self.redis_client, 'keys') and hasattr(self.redis_client, 'delete'):
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'last_activity': {
                'last_hit': self.stats.last_hit_time.isoformat() if self.stats.last_hit_time else None,
                'last_miss': self.stats.last_miss_time.isoformat() if self.stats.last_miss_time else None,
                'last_eviction': self.stats.last_eviction_time.isoformat() if self.stats.last_eviction_time else None
            },
            'content_deduplication': {
                'unique_content_hashes': len(self.content_hashes),
                'deduplication_ratio': len(self.content_hashes) / max(1, self.stats.entry_count)
            },
            'tags': {
