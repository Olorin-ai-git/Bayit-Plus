...
class SumoLogicClient:
    ...
    async def search(
        self, query: str, from_time: str = "-1d", to_time: str = "now"
    ) -> List[Dict[str, Any]]:
        """
        ...
        def _search():
            try:
                ...
                logger.info(
                    f"Created search job with time range: {from_time} to {to_time}"
                )
                ...
                while True:
                    ...
                    if elapsed > timeout_seconds:
                        raise Exception(
                            f"SumoLogic search timed out after {timeout_seconds} seconds"
                        )
