...
class SumoLogicQueryTool(BaseTool):
    ...
    endpoint: str = Field(
        "https://api.sumologic.com/api/v1", description="SumoLogic API endpoint"
    )
    ...
    async def _arun(self, query: str, time_range: str) -> Dict[str, Any]:
        ...
        client = SumoLogicClient(
            access_id=access_id, access_key=access_key, endpoint=self.endpoint
        )
        ...
        try:
            ...
            logger.info(
                f"SumoLogic query completed, returned {len(results.get('results', []))} results"
            )
            return results
            ...
            if "secret" in str(e).lower() and "not found" in str(e).lower():
                logger.debug(
                    f"SumoLogic analysis skipped: API credentials not configured"
                )
                return {
                    "status": "skipped",
