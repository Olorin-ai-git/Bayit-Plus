...
class QueryPerformanceMonitor:
    ...
    def __init__(self, slow_query_threshold_ms: int = 1000):
        ...
        if slow_query_threshold_ms <= 0:
            raise ValueError(
                f"slow_query_threshold_ms must be > 0, got {slow_query_threshold_ms}"
            )
        ...
        logger.info(
            f"Initialized QueryPerformanceMonitor (slow threshold: {slow_query_threshold_ms}ms)"
        )

    def record_query_execution(
        self, query: str, duration_ms: float, row_count: int, success: bool
    ) -> None:
        """
        ...
        with self._lock:
            if query:
                ...
                return {
                    ...
                    "success_rate": sum(1 for e in executions if e["success"])
                    / len(executions),
                }
            else:
                # Aggregate stats for all queries
                total_executions = sum(
                    len(execs) for execs in self._query_stats.values()
                )
                all_durations = [
                    e["duration_ms"]
                ...
                return {
                    ...
                    "slow_queries_count": sum(
                        1 for d in all_durations if d > self.slow_query_threshold_ms
                    ),
                }
