...
class SocialMediaMonitoringTool(BaseTool):
    ...
    def _run(
        ...
        """
        logger.info(
            f"Monitoring {len(monitoring_keywords)} keywords across social media"
        )
        ...
        try:
            ...
            # Validate duration
            monitoring_duration_hours = max(
                1, min(monitoring_duration_hours, 168)
            )  # 1 hour to 1 week
            ...
            monitoring_results = {
                ...
                "monitoring_start": (
                    datetime.utcnow() - timedelta(hours=monitoring_duration_hours)
                ).isoformat(),
                "monitoring_end": datetime.utcnow().isoformat(),
                "duration_hours": monitoring_duration_hours,
                "content_analysis": self._analyze_content(
                    monitoring_keywords, platforms, monitoring_duration_hours
                ),
                "sentiment_analysis": (
                    self._analyze_sentiment(monitoring_keywords, platforms)
                    if include_sentiment_analysis
                    else None
                ),
                "threat_detection": (
                    self._detect_threats(monitoring_keywords, platforms)
                    if include_threat_detection
                    else None
                ),
                "trending_analysis": self._analyze_trends(
                    monitoring_keywords, platforms
                ),
            ...
            monitoring_results["summary_metrics"] = self._calculate_summary_metrics(
                monitoring_results
            )
    ...
    def _analyze_content(
        self, keywords: List[str], platforms: List[str], duration: int
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "content_types": {"text": 0.60, "image": 0.25, "video": 0.10, "link": 0.05},
            "engagement_metrics": {
                "total_likes": 15600,
                "total_shares": 3200,
                "total_comments": 8900,
    ...
    def _analyze_sentiment(
        ...
        return {
            "overall_sentiment": {"positive": 0.52, "neutral": 0.31, "negative": 0.17},
            "sentiment_trends": {
    ...
    def _analyze_trends(
        ...
        return {
            "trending_keywords": [
                {
                    "keyword": keywords[0] if keywords else "scam",
                    "mentions": 456,
                    "growth": 2.3,
                },
    ...
    def _generate_alerts(
        self, keywords: List[str], thresholds: Dict[str, float]
    ) -> List[Dict[str, Any]]:
    ...
    def _calculate_summary_metrics(self, monitoring_results: Dict) -> Dict[str, Any]:
        ...
        return {
            "monitoring_score": 75,
            "activity_level": (
                "high" if content.get("total_mentions", 0) > 1000 else "medium"
            ),
            "sentiment_health": (
                "positive"
                if sentiment
                and sentiment.get("overall_sentiment", {}).get("positive", 0) > 0.5
                else "mixed"
            ),
