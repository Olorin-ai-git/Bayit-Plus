...
class SocialNetworkAnalysisTool(BaseTool):
    ...
    def _run(
        ...
        try:
            ...
            if not analysis_types:
                analysis_types = [NetworkAnalysisType.CENTRALITY.value, NetworkAnalysisType.COMMUNITY.value]
            ...
            analysis_results = {
                ...
                "network_metrics": self._calculate_network_metrics(target_profiles, analysis_depth),
                "centrality_analysis": self._analyze_centrality(target_profiles) if "centrality" in analysis_types else None,
                "community_detection": self._detect_communities(target_profiles) if "community" in analysis_types else None,
                "influence_mapping": self._map_influence(target_profiles) if include_influence_mapping else None,
                "anomaly_detection": self._detect_network_anomalies(target_profiles) if detect_anomalies else None,
                "relationship_patterns": self._analyze_relationship_patterns(target_profiles),
                "risk_assessment": {}
            }
            
            # Calculate risk assessment
            analysis_results["risk_assessment"] = self._assess_network_risks(analysis_results)
    ...
    def _calculate_network_metrics(self, profiles: List[str], depth: int) -> Dict[str, Any]:
    ...
    def _assess_network_risks(self, analysis_results: Dict) -> Dict[str, Any]:
        ...
        communities = analysis_results.get("community_detection", {}).get("communities", [])
        suspicious_communities = [c for c in communities if c.get("type") == CommunityType.SUSPICIOUS.value]
