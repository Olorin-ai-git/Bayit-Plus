...
from app.service.agent.tools.pii_sanitizer import (
    SanitizationMethod,
...
class _WebSearchArgs(BaseModel):
    ...
    num_results: int = Field(
        default=10, description="Number of search results to return (1-20)"
    )
...
class WebSearchTool(BaseTool):
    ...
    def _run(
        self, query: str, num_results: int = 10, search_engine: str = "duckduckgo"
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for result in results:
                ...
                sanitized_title, title_pii = sanitize_web_content(
                    result.get("title", ""), method=SanitizationMethod.REDACT
                )

                # Sanitize snippet
                sanitized_snippet, snippet_pii = sanitize_web_content(
                    result.get("snippet", ""), method=SanitizationMethod.REDACT
                )
            ...
            if pii_summary:
                response["pii_sanitization"] = {
                    ...
                    "total_instances": sum(
                        sum(
                            len(instances)
                            for instances in detection.get("detected_pii", {}).values()
                        )
...
class _WebScrapeArgs(BaseModel):
    ...
    extract_links: bool = Field(
        default=False, description="Whether to extract all links from the page"
    )
    max_length: int = Field(
        default=5000, description="Maximum length of extracted text"
    )
    ...
    def _extract_links(
        ...
        for link in soup.find_all("a", href=True):
            ...
            if href.startswith(("http://", "https://")) and text:
                links.append(
                    {"url": href, "text": text, "title": link.get("title", "")}
                )

        return links

    def _run(
        self, url: str, extract_links: bool = False, max_length: int = 5000
    ) -> Dict[str, Any]:
        """Scrape the web page."""
        try:
            # Validate URL
