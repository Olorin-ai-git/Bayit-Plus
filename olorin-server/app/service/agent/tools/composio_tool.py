...
class ComposioActionInput(BaseModel):
    ...
    toolkit: str = Field(..., description="Toolkit name (e.g., 'stripe', 'shopify', 'okta')")
    action: str = Field(..., description="Action name (e.g., 'void_payment', 'cancel_order', 'suspend_user')")
    connection_id: str = Field(..., description="Composio connection ID for the tenant")
    parameters: Dict[str, Any] = Field(default_factory=dict, description="Action parameters")
    execution_id: Optional[str] = Field(None, description="Optional SOAR execution ID for audit")
    tenant_id: Optional[str] = Field(None, description="Tenant ID for scoping (required for action execution). If not provided, will attempt to retrieve from investigation context or use DEFAULT_TENANT_ID environment variable.")
...
class ComposioTool(BaseTool):
    ...
    def _run(
        ...
        **kwargs
        ...
        logger.info(f"🔍 [ComposioTool] Starting action execution: toolkit={toolkit}, action={action}, connection_id={connection_id}, tenant_id={tenant_id}, execution_id={execution_id}")
        logger.debug(f"🔍 [ComposioTool] Parameters: {json.dumps(parameters or {}, indent=2)}")
        logger.debug(f"🔍 [ComposioTool] Additional kwargs: {kwargs}")
        
        # Try to get tenant_id from kwargs if not provided directly
        if not tenant_id:
            tenant_id = kwargs.get('tenant_id')
            logger.debug(f"🔍 [ComposioTool] Retrieved tenant_id from kwargs: {tenant_id}")
        ...
        if not tenant_id:
            try:
                ...
                if metadata and isinstance(metadata, dict):
                    ...
                    if tenant_id:
                        logger.debug(f"🔍 [ComposioTool] Retrieved tenant_id from investigation metadata: {tenant_id}")
            except Exception as e:
                logger.debug(f"🔍 [ComposioTool] Could not get tenant_id from investigation metadata: {e}")
        
        # Use default tenant_id if still not available (for testing/demo)
        if not tenant_id:
            tenant_id = os.getenv('DEFAULT_TENANT_ID', 'default')
            logger.warning(f"⚠️ [ComposioTool] Using default tenant_id from environment: {tenant_id}")
        ...
        if parameters is None:
            parameters = {}
            logger.debug(f"🔍 [ComposioTool] Parameters were None, initialized as empty dict")
        
        try:
            # Validate executor initialization
            if self.action_executor is None:
                raise RuntimeError("ActionExecutor not initialized")
            logger.debug(f"🔍 [ComposioTool] ActionExecutor initialized: {self.action_executor is not None}")
            
            logger.info(f"🔍 [ComposioTool] Calling ActionExecutor.execute_action with toolkit={toolkit}, action={action}")
            ...
            logger.info(f"✅ [ComposioTool] Action executed successfully: toolkit={toolkit}, action={action}, tenant={tenant_id}, connection={connection_id}")
            logger.debug(f"🔍 [ComposioTool] Action result type: {type(result)}, result_keys: {list(result.keys()) if isinstance(result, dict) else 'N/A'}")
            ...
            logger.info(f"✅ [ComposioTool] Returning success result, length={len(result_json)}")
            
            return result_json
            
        except ComposioConnectionError as e:
            logger.warning(f"⚠️ [ComposioTool] Connection error: toolkit={toolkit}, action={action}, error={e}")
            return json.dumps({
                "status": "failed",
                "error": str(e),
            ...
            logger.error(f"❌ [ComposioTool] Action error: toolkit={toolkit}, action={action}, error={e}")
            return json.dumps({
                "status": "failed",
                "error": str(e),
            ...
            if isinstance(e, RetryError):
                ...
                if last_attempt:
                    ...
                    if isinstance(exception, ComposioConnectionError):
                        logger.warning(f"⚠️ [ComposioTool] Connection error (from retry): toolkit={toolkit}, action={action}, error={exception}")
                        return json.dumps({
                            "status": "failed",
                            "error": str(exception),
                            "error_type": "connection_error",
            ...
            error_str = str(e)
            if "ComposioConnectionError" in error_str or ("Connection" in error_str and "not found" in error_str):
                logger.warning(f"⚠️ [ComposioTool] Connection error (detected from message): toolkit={toolkit}, action={action}, error={e}")
                return json.dumps({
                    "status": "failed",
                    "error": str(e),
                    "error_type": "connection_error",
            ...
            logger.error(f"❌ [ComposioTool] Unexpected error: toolkit={toolkit}, action={action}, error={e}", exc_info=True)
            return json.dumps({
                "status": "failed",
                "error": str(e),
