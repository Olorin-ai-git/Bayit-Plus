...
class ToolMetrics:
    ...
    def update_execution(self, duration: float, success: bool, error_type: Optional[str] = None):
    ...
    def cache_hit_rate(self) -> float:
        ...
        return self.cache_hit_count / total_cache_requests if total_cache_requests > 0 else 0.0
...
class EnhancedToolBase(ABC):
        ...
        try:
            ...
            while retry_count <= self.config.max_retries:
                try:
                    ...
                    self.logger.info(f"Tool {self.config.name} executed successfully in {exec_time:.3f}s")
                    ...
                    last_error = ToolTimeoutError(f"Tool execution timed out after {self.config.timeout_seconds}s")
                    self.logger.warning(f"Tool {self.config.name} timed out (attempt {retry_count + 1})")
                    ...
                    self.logger.warning(f"Tool {self.config.name} failed: {str(e)} (attempt {retry_count + 1})")
            ...
            self.logger.error(f"Tool {self.config.name} failed after {retry_count} attempts: {error_message}")
            ...
            self.logger.error(f"Tool {self.config.name} unexpected error: {str(e)}", exc_info=True)
        ...
        if self.config.input_schema:
            ...
            for field in required_fields:
                if field not in input_data:
                    raise ToolValidationError(f"Required field '{field}' missing from input")
    ...
    async def _validate_output(self, result: Any) -> None:
        ...
        if self.config.output_schema:
            ...
            if isinstance(result, dict) and 'required_fields' in self.config.output_schema:
                required_fields = self.config.output_schema['required_fields']
                for field in required_fields:
                    if field not in result:
                        raise ToolValidationError(f"Required output field '{field}' missing from result")
    ...
    def _generate_cache_key(self, input_data: Dict[str, Any], context: Optional[Dict[str, Any]] = None) -> str:
    ...
    def _update_circuit_breaker_failure(self) -> None:
        ...
        if self.config.retry_strategy == RetryStrategy.CIRCUIT_BREAKER:
            ...
            if self.circuit_breaker_failures >= self.circuit_breaker_threshold:
                ...
                self.logger.warning(f"Circuit breaker opened for tool {self.config.name}")
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Health check failed for tool {self.config.name}: {str(e)}")
            return False