...
from .enhanced_tool_base import EnhancedToolBase, ToolConfig, ToolMetrics, ToolResult
...
class RAGEnhancedToolBase(EnhancedToolBase):
    ...
    def __init__(
        ...
        try:
            ...
            if self.rag_available:
                ...
                logger.info(
                    f"RAG enhancement initialized but not available for tool {config.name}"
                )
        except Exception as e:
            logger.warning(
                f"RAG context enhancer initialization failed for {config.name}: {str(e)}"
            )
        ...
        if RESULT_AUGMENTATION_AVAILABLE:
            try:
                ...
                self.result_augmentation_available = (
                    self.result_augmentation_service.rag_available
                )
                if self.result_augmentation_available:
                    logger.info(f"Result augmentation enabled for tool {config.name}")
            except Exception as e:
                logger.warning(
                    f"Result augmentation service initialization failed for {config.name}: {str(e)}"
                )
    ...
    async def execute(
        ...
        if self.enable_rag and self.rag_available and self.context_enhancer:
            try:
                return await self._rag_enhanced_execute(
                    input_data, context, investigation_context, domain
                )
            except Exception as e:
                logger.warning(
                    f"RAG-enhanced execution failed for {self.config.name}, falling back to standard: {str(e)}"
                )
    ...
    async def _rag_enhanced_execute(
        ...
        enhanced_context_dict = self._create_enhanced_context_dict(
    ...
    async def _enhance_result_with_comprehensive_augmentation(
        ...
        if self.result_augmentation_available and self.result_augmentation_service:
            try:
                ...
                augmented_result = (
                    await self.result_augmentation_service.augment_result(
                        result=result,
                        context=enhanced_context,
                        investigation_context=investigation_context,
                        domain=domain,
        ...
        return await self._enhance_result_with_basic_interpretation(
            result, enhanced_context
        )

    async def _enhance_result_with_basic_interpretation(
        self, result: ToolResult, enhanced_context: ToolExecutionContext
    ) -> ToolResult:
        ...
        try:
            ...
            interpretation = await self._create_rag_interpretation(
                result, enhanced_context
            )
            ...
            if interpretation:
                ...
                result.metadata["interpretation_confidence"] = (
            ...
            logger.warning(
                f"RAG result interpretation failed for {self.config.name}: {str(e)}"
            )
    ...
    def _calculate_interpretation_confidence(self, knowledge_context) -> float:
        ...
        confidence = min(
            1.0, (critical_weight + supporting_weight + background_weight) / 10.0
        )
        return confidence
    ...
    def _update_rag_metrics(
        ...
        self.rag_metrics[
            "knowledge_chunks_utilized"
        ] += enhanced_context.total_knowledge_chunks

        # Update parameter enhancement count
        self.rag_metrics["parameter_enhancements"] += len(
            enhanced_context.parameter_enhancements
        )
        ...
        new_avg = (
            (current_avg * (total_rag_executions - 1)) + rag_overhead_ms
        ) / total_rag_executions
    ...
    def get_rag_metrics(self) -> Dict[str, Any]:
        ...
        if total_executions > 0:
            rag_usage_rate = (
                self.rag_metrics["rag_enhanced_executions"] / total_executions
            )
