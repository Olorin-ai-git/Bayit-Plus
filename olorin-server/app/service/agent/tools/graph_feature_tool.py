...
class GraphFeatureInput(BaseModel):
    ...
    entity_id: str = Field(
        ..., description="Entity identifier (user_id, device_id, etc.)"
    )
    entity_type: str = Field(
        default="User", description="Entity type: User, Device, Card, IP"
    )
...
class GraphFeatureTool(BaseTool):
    ...
    def __init__(self, tenant_id: Optional[str] = None, **kwargs):
        ...
        object.__setattr__(
            self, "feature_computer", FeatureComputer(tenant_id=tenant_id)
        )
    ...
    def _run(
        ...
        logger.info(
            f"🔍 [GraphFeatureTool] Starting feature computation: entity_id={entity_id}, entity_type={entity_type}, feature_type={feature_type}, transaction_id={transaction_id}, tenant_id={tenant_id}"
        )

        # Use provided tenant_id or instance tenant_id
        effective_tenant_id = tenant_id or self.tenant_id
        logger.debug(
            f"🔍 [GraphFeatureTool] Effective tenant_id: {effective_tenant_id}"
        )
        ...
        try:
            ...
            logger.debug(
                f"🔍 [GraphFeatureTool] FeatureComputer initialized: {self.feature_computer is not None}"
            )
            ...
            if feature_type in ("all", "cluster_risk"):
                logger.debug(
                    f"🔍 [GraphFeatureTool] Computing cluster_risk_score for entity_id={entity_id}, entity_type={entity_type}"
                )
                results["cluster_risk_score"] = (
                    self.feature_computer.compute_cluster_risk_score(
                        entity_id, entity_type
                    )
                )
                logger.debug(
                    f"🔍 [GraphFeatureTool] cluster_risk_score computed: {results.get('cluster_risk_score')}"
                )

            if feature_type in ("all", "shared_devices"):
                logger.debug(
                    f"🔍 [GraphFeatureTool] Computing shared_device_count for entity_id={entity_id}, entity_type={entity_type}"
                )
                results["shared_device_count"] = (
                    self.feature_computer.compute_shared_device_count(
                        entity_id, entity_type
                    )
                )
                logger.debug(
                    f"🔍 [GraphFeatureTool] shared_device_count computed: {results.get('shared_device_count')}"
                )

            if feature_type in ("all", "co_travel"):
                logger.debug(
                    f"🔍 [GraphFeatureTool] Computing co_travel_patterns for entity_id={entity_id}, entity_type={entity_type}"
                )
                results["co_travel_patterns"] = (
                    self.feature_computer.compute_co_travel_patterns(
                        entity_id, entity_type
                    )
                )
                logger.debug(
                    f"🔍 [GraphFeatureTool] co_travel_patterns computed: {len(results.get('co_travel_patterns', []))} patterns"
                )

            if feature_type in ("all", "velocity"):
                logger.debug(
                    f"🔍 [GraphFeatureTool] Computing velocity_across_clusters for entity_id={entity_id}, entity_type={entity_type}"
                )
                results["velocity_across_clusters"] = (
                    self.feature_computer.compute_velocity_across_clusters(
                        entity_id, entity_type
                    )
                )
                logger.debug(
                    f"🔍 [GraphFeatureTool] velocity_across_clusters computed: {results.get('velocity_across_clusters')}"
                )

            if feature_type == "all":
                logger.debug(
                    f"🔍 [GraphFeatureTool] Computing cluster_detection for entity_id={entity_id}, entity_type={entity_type}"
                )
                results["cluster_detection"] = (
                    self.feature_computer.compute_cluster_detection(
                        entity_id, entity_type
                    )
                )
                logger.debug(
                    f"🔍 [GraphFeatureTool] cluster_detection computed: {results.get('cluster_detection')}"
                )

            # Export to Snowflake if transaction_id provided
            if transaction_id:
                logger.info(
                    f"🔍 [GraphFeatureTool] Exporting features to PostgreSQL for transaction_id={transaction_id}"
                )
                ...
                logger.info(
                    f"🔍 [GraphFeatureTool] Export result: success={export_success}"
                )
            ...
            logger.debug(
                f"🔍 [GraphFeatureTool] Full results: {json.dumps(results, indent=2, default=str)}"
            )

            result_json = json.dumps(results, indent=2, default=str)
            logger.info(
                f"✅ [GraphFeatureTool] Returning result, length={len(result_json)}"
            )
            ...
            logger.error(
                f"❌ [GraphFeatureTool] Feature computation failed: entity_id={entity_id}, entity_type={entity_type}, feature_type={feature_type}, error={e}",
                exc_info=True,
            )
            ...
            logger.error(
                f"❌ [GraphFeatureTool] Returning error result: {error_result}"
            )
            return error_result
