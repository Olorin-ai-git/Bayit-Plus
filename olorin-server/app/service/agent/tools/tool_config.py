...
class ToolConfig:
    ...
    def get_tool_status_summary(cls) -> Dict[str, Dict[str, any]]:
        ...
        logger.info(f"Tool Status: {len(summary['enabled'])} enabled, "
    ...
    def validate_configuration(cls) -> bool:
        ...
        if not enabled_tools:
            issues.append("WARNING: No tools are enabled. Application may have limited functionality.")
        ...
        if cls.is_tool_enabled('snowflake'):
            ...
            for var in snowflake_vars:
                if not os.getenv(var):
                    issues.append(f"WARNING: Snowflake enabled but {var} not configured")
