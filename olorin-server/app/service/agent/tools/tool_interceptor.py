...
class ToolExecutionInterceptor:
    ...
    def register_hook(self, hook: InterceptorHook) -> None:
        ...
        self.logger.info(f"Registered {hook.hook_type.value} hook with priority {hook.priority}")
    ...
    async def execute_tool(
        ...
        async with self.execution_semaphore:
            try:
                ...
                # Pre-execution hooks
                await self._execute_hooks(HookType.PRE_EXECUTION, {
                    'tool': tool,
                    'input_data': input_data,
                    'context': context,
                    'execution_id': execution_id
                ...
                await self._execute_hooks(HookType.POST_EXECUTION, {
                    'tool': tool,
                    'input_data': input_data,
                    'context': context,
                    'result': result,
                ...
                if result.success:
                    await self._execute_hooks(HookType.ON_SUCCESS, {
                        'tool': tool,
                        'result': result,
                        'execution_id': execution_id,
                        'execution_time': execution_time
                    ...
                    if result.from_cache:
                        await self._execute_hooks(HookType.ON_CACHE_HIT, {
                            'tool': tool,
                            'result': result,
                            'execution_id': execution_id
                        })
                    else:
                        await self._execute_hooks(HookType.ON_CACHE_MISS, {
                            'tool': tool,
                            'result': result,
                            'execution_id': execution_id
                        })
                else:
                    await self._execute_hooks(HookType.ON_FAILURE, {
                        'tool': tool,
                        'result': result,
                        'execution_id': execution_id,
                        'error': result.error,
                ...
                if result.retry_count > 0:
                    await self._execute_hooks(HookType.ON_RETRY, {
                        'tool': tool,
                        'result': result,
                        'execution_id': execution_id,
                ...
                if self.config.enable_execution_tracking:
                    await self._update_execution_stats(tool.config.name, result, execution_time)
                
                # Performance monitoring
                if self.config.enable_performance_monitoring:
                    await self._update_performance_metrics(tool.config.name, execution_time)
                ...
                # Timeout hooks
                await self._execute_hooks(HookType.ON_TIMEOUT, {
                    'tool': tool,
                    'input_data': input_data,
                    'context': context,
                    'execution_id': execution_id,
                ...
                execution_time = time.time() - start_time
                self.logger.error(f"Interceptor error during tool execution: {str(e)}", exc_info=True)
    ...
    async def _execute_hooks(self, hook_type: HookType, event_data: Dict[str, Any]) -> None:
        ...
        for hook in hooks:
            ...
            try:
                ...
                self.logger.warning(f"Hook {hook_type.value} timed out after {self.config.hook_timeout_seconds}s")
            except Exception as e:
                self.logger.error(f"Hook {hook_type.value} failed: {str(e)}", exc_info=True)
    ...
    async def _track_error_pattern(self, tool_name: str, result: ToolResult) -> None:
        ...
        if result.error_type:
            ...
            if len(self.error_patterns[error_key]) > max_history:
                self.error_patterns[error_key] = self.error_patterns[error_key][-max_history:]
    
    async def _update_execution_stats(self, tool_name: str, result: ToolResult, execution_time: float) -> None:
    ...
    async def _update_performance_metrics(self, tool_name: str, execution_time: float) -> None:
        ...
        if len(self.performance_metrics[tool_name]) > max_metrics:
            self.performance_metrics[tool_name] = self.performance_metrics[tool_name][-max_metrics:]
    ...
    async def _store_execution_history(self, execution_info: Dict[str, Any]) -> None:
        ...
        history_entry = {
            ...
            'result_summary': str(execution_info.get('result', {}).result)[:200] if execution_info.get('result') else None
        }
        ...
        if len(self.execution_history) > self.config.replay_history_size:
            self.execution_history = self.execution_history[-self.config.replay_history_size:]
    ...
    def get_execution_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            'total_tools_monitored': len(set(
                key.split(':')[0] for key in self.execution_stats.keys()
            )),
        ...
        for key, count in self.execution_stats.items():
            if key.endswith(':total_executions') and count > 0:
                ...
                successful = self.execution_stats.get(f"{tool_name}:successful_executions", 0)
    ...
    def get_error_analysis(self) -> Dict[str, Any]:
        ...
        for error_key, error_history in self.error_patterns.items():
            ...
            analysis['error_patterns'][error_key] = {
                ...
                'avg_execution_time': sum(e['execution_time'] for e in error_history) / len(error_history),
                'avg_retry_count': sum(e['retry_count'] for e in error_history) / len(error_history),
                'last_occurrence': error_history[-1]['timestamp'].isoformat() if error_history else None
            }
    ...
    async def replay_execution(self, execution_id: str) -> Optional[ToolResult]:
        ...
        self.logger.info(f"Replaying execution {execution_id} for tool {target_execution['tool_name']}")
    ...
    def add_standard_hooks(self) -> None:
        ...
        self.register_hook(InterceptorHook(
        ...
        # Error logging hook
        self.register_hook(InterceptorHook(
            hook_type=HookType.ON_FAILURE,
            handler=self._log_failure,
            priority=1
        ))
    ...
    async def _log_pre_execution(self, event_data: Dict[str, Any]) -> None:
        ...
        self.logger.info(f"Starting execution {execution_id} for tool {tool.config.name}")
