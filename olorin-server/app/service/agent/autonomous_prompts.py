...
def create_investigation_prompt(
    ...
    prompt_parts.extend([
        ...
        "1. SELECT TOOLS based on what data you need AND what data is available",
        "2. CRITICAL: Check if context.data_sources['user']['ip_address'] exists BEFORE using any IP-based tools",
        "3. If NO IP address is available: SKIP tools that require IP (AbuseIPDB, Shodan IP analysis, VirusTotal IP)",
        "4. NEVER pass entity_id (like 'K1F6HIIGBVHH20TX') to IP analysis tools - these are NOT IP addresses",
        "5. Use alternative analysis methods when IP is not available (logs, behavioral patterns, etc.)",
        "6. Use multiple tools if necessary to gather comprehensive evidence",
        "7. EXPLAIN your tool selection reasoning and data availability", 
        "8. Analyze all collected data for fraud indicators",
        "9. Correlate findings across data sources",
        "10. Assign risk scores based on evidence strength",
        "11. Provide confidence levels for all assessments",
