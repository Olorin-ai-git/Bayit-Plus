...
def create_investigation_prompt(
    ...
    if available_tools:
        tool_names = [tool.name if hasattr(tool, 'name') else str(tool) for tool in available_tools]
        logger.info(f"Creating unified prompt for {domain} with {len(tool_names)} available tools")
    else:
        tool_names = []
