...
class AgentRAGToolOrchestrator:
    ...
    async def execute_tool_with_rag_context(
        ...
        tool_name = tool.config.name if hasattr(tool, 'config') else str(type(tool).__name__)
        
        # Check if tool supports RAG enhancement
        if isinstance(tool, RAGEnhancedToolBase) and self.enable_rag_tools and self.rag_available:
    ...
    async def _execute_rag_enhanced_tool(
        ...
        if hasattr(result, 'metadata') and result.metadata.get("rag_enhanced"):
            self.agent_rag_stats["total_rag_overhead_ms"] += result.metadata.get("rag_overhead_ms", 0)
            self.agent_rag_stats["total_knowledge_chunks_used"] += result.metadata.get("knowledge_chunks_used", 0)
    ...
    def _update_tool_execution_stats(self, tool_name: str, execution_time_ms: float, result: Any) -> None:
        ...
        tool_stats["avg_time_ms"] = tool_stats["total_time_ms"] / tool_stats["executions"]
    ...
    def get_agent_rag_performance(self) -> Dict[str, Any]:
        ...
        if rag_tools > 0:
            ...
            avg_chunks_per_tool = self.agent_rag_stats["total_knowledge_chunks_used"] / rag_tools
        ...
        return {
            ...
            "rag_performance": {
                ...
                "total_knowledge_chunks": self.agent_rag_stats["total_knowledge_chunks_used"],
...
class RAGToolFactory:
    ...
    def wrap_existing_tool_with_rag(
        tool_instance: EnhancedToolBase,
        enable_rag: bool = True
    ) -> RAGEnhancedToolBase:
        """
        ...
        class ToolWrapper(RAGEnhancedToolBase):
            ...
            def __init__(self, wrapped_tool: EnhancedToolBase, rag_enabled: bool = True):
...
def enable_rag_for_agent_tools(
    ...
    orchestrator.set_investigation_context(investigation_id, agent_id or f"{agent_domain}_agent")
    
    logger.info(f"RAG capabilities enabled for {agent_domain} agent in investigation {investigation_id}")
