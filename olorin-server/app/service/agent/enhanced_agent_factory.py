...
from .mcp_client.mcp_integration_bridge import (
    MCPIntegrationBridge,
    get_mcp_integration_bridge,
)
...
class MCPToolWrapper:
    ...
    async def call_blockchain_analysis(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return await self._call_mcp_tool(
            "blockchain_analysis", "analyze_transaction", params
        )

    async def call_intelligence_gathering(
        self, params: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Call intelligence gathering MCP tools."""
        return await self._call_mcp_tool(
            "intelligence_gathering", "gather_threat_intel", params
        )
    ...
    async def _call_mcp_tool(
        ...
        try:
            ...
            if response and response.success:
                ...
                logger.error(
                    f"MCP tool call failed: {server_name}.{tool_name}: {error_msg}"
                )
                self.error_count += 1
...
class EnhancedAgentFactory:
    ...
    async def create_enhanced_agent(
        self, config: EnhancedAgentConfig, base_agent_creator: Callable = None
    ) -> Any:
        """
        ...
        class EnhancedInvestigationAgent:
            ...
            async def structured_investigate(
                self, context, config, specific_objectives=None
            ):
                """Enhanced structured investigation with MCP integration."""
                self._investigation_count += 1

                logger.info(
                    f"🔍 Starting enhanced {self.domain} investigation with MCP integration"
                )

                # Check server health before investigation
                if not await self._verify_mcp_health():
                    logger.warning(
                        f"Some MCP servers unhealthy for {self.domain} domain"
                    )
                ...
                logger.info(
                    f"✅ Enhanced {self.domain} investigation completed "
            ...
            async def _run_investigation_with_mcp(self, context, config, objectives):
                ...
                from .agent_investigation_enhancer import (
                    investigate_with_mcp_enhancement,
                )
        ...
        logger.info(
            f"🤖 Created enhanced {config.domain} agent with MCP integration "
            f"(total agents: {self.agents_created})"
        )

        return enhanced_agent

    async def _get_relevant_server_health(
...
async def create_enhanced_device_agent() -> Any:
    ...
    config = EnhancedAgentConfig(
        domain="device",
        preferred_mcp_categories=[
            MCPServerCategory.ML_AI,
            MCPServerCategory.INTELLIGENCE,
        ],
