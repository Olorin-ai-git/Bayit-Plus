...
class OpenAIStreamingPattern(OpenAIBasePattern):
    ...
    def __init__(self, config, openai_config, tools=None, ws_streaming=None):
        ...
        if self.tools:
            ...
            logger.info(f"Streaming pattern initialized with {len(self._function_definitions)} tools")
        ...
        try:
            ...
            if streaming_result["success"]:
                ...
                return PatternResult.error_result(
                    error_message=streaming_result.get("error", "Advanced streaming execution failed")
                )
    ...
    async def _initialize_advanced_handlers(self, investigation_id: str):
        ...
        if not self._completion_handler:
            self._completion_handler = ChatCompletionHandler(
                self._openai_client, 
                self.tools, 
                self._advanced_streaming_handler
            )
...
class AdvancedStreamingHandler(StreamingHandler):
    ...
    async def broadcast_function_complete(self, function_name: str, result: Any):
        """Broadcast function call completion"""
        await self._broadcast_function_status("function_complete", function_name, {"result": str(result)[:200]})
    
    async def broadcast_function_error(self, function_name: str, error: str):
        """Broadcast function call error"""
        await self._broadcast_function_status("function_error", function_name, {"error": error})
    
    async def _broadcast_function_status(self, status: str, function_name: str, data: Dict[str, Any]):
...
class ProgressiveRiskAnalyzer:
    ...
    async def update_analysis(self, content_chunk: str, chunk_number: int):
        ...
        chunk_risk = sum(1 for keyword in risk_keywords if keyword.lower() in content_chunk.lower())
    ...
    async def _broadcast_analysis_start(self):
        ...
        await self._broadcast_analysis_event("analysis_start", {"initial_risk_score": 0.0})
    
    async def _broadcast_analysis_update(self):
        """Broadcast progressive analysis update"""
        await self._broadcast_analysis_event("analysis_update", {
    ...
    async def _broadcast_analysis_event(self, event_type: str, data: Dict[str, Any]):
        ...
        try:
            if hasattr(self.streaming_handler.ws_streaming, 'broadcast_investigation_update'):
...
class StreamingCoordinator:
    ...
    async def complete_session(self):
        """Complete streaming session"""
        await self._broadcast_session_event("session_complete", {
            "session_id": self.session_id,
    ...
    async def handle_error(self, error_message: str):
        ...
        await self._broadcast_session_event("session_error", {
            "session_id": self.session_id,
            "error": error_message,
    ...
    async def cleanup(self):
        ...
        await self._broadcast_session_event("session_cleanup", {
            "session_id": self.session_id,
