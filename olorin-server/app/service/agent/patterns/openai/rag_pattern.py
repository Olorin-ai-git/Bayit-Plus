...
class OpenAIRAGPattern(OpenAIBasePattern):
    ...
    async def execute_openai_pattern(
        ...
        try:
            ...
            knowledge_response = await self._rag_orchestrator.process_request(
                rag_request
            )

            if knowledge_response.success:
                # Enhance with knowledge
                enhanced_messages = await self._augment_messages(
                    messages, knowledge_response
                )
                ...
                if self.openai_config.stream and self.ws_streaming:
                    result = await self._generate_streaming(
                        enhanced_messages, context, knowledge_response
                    )
                else:
    ...
    async def _augment_messages(
        self, messages: List[BaseMessage], knowledge_response
    ) -> List[BaseMessage]:
    ...
    async def _generate_streaming(
        self, messages, context, knowledge_response
    ) -> PatternResult:
        ...
        try:
            ...
            enhanced_result = self._enhance_result(
                result["response"], knowledge_response
            )
