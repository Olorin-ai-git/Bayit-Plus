...
class ResultSynthesizer:
    ...
    async def synthesize_results(self, agent_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Generate summary
        synthesis["investigation_summary"] = self._generate_summary(agent_results, synthesis)
    ...
    def _find_correlations(self, results: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        network_vpn = results.get("network", {}).get("findings", {}).get("vpn_detected", False)
        device_new = results.get("device", {}).get("findings", {}).get("new_device", False)
        location_anomaly = results.get("location", {}).get("findings", {}).get("location_anomaly", False)
    ...
    def _generate_summary(self, results: Dict[str, Any], synthesis: Dict[str, Any]) -> str:
        """Generate human-readable investigation summary"""
        risk_level = "LOW" if synthesis["overall_risk_score"] < 0.3 else \
                    "MEDIUM" if synthesis["overall_risk_score"] < 0.7 else "HIGH"
        
        return f"Multi-agent fraud investigation completed with {risk_level} risk assessment. " \
