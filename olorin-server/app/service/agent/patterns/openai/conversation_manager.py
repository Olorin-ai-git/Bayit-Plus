...
class ConversationManager:
    ...
    async def manage_history(
        self, investigation_id: str, new_messages: List[BaseMessage]
    ) -> None:
    ...
    async def enrich_context(
        self, investigation_id: str, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if investigation_id in self._context_cache:
            ...
            cached_time = datetime.fromisoformat(
                cached_context.get("cached_at", "1970-01-01")
            )

            if datetime.now() - cached_time < timedelta(
                hours=self.max_context_age_hours
            ):
    ...
    async def inject_history(
        self, investigation_id: str, openai_messages: List[Dict[str, str]]
    ) -> List[Dict[str, str]]:
        ...
        history_messages = []
        for hist_msg in (
            history[: -len(user_messages)] if len(history) > len(user_messages) else []
        ):
    ...
    async def clear_history(self, investigation_id: str) -> None:
        ...
        logger.info(
            f"Cleared conversation history for investigation {investigation_id}"
        )
