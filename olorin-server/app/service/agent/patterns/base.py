...
class PatternResult:
    ...
    @classmethod
    def success_result(
        cls, result: Any, confidence: float = 1.0, reasoning: str = None
    ) -> "PatternResult":
    ...
    @classmethod
    def error_result(
        cls, error_message: str, confidence: float = 0.0
    ) -> "PatternResult":
...
class BasePattern(ABC):
    ...
    def _cache_result(self, cache_key: str, result: PatternResult) -> None:
        ...
        if len(self._cache) >= self.MAX_CACHE_SIZE:
            ...
            logger.debug(
                f"Evicted {len(oldest_keys)} cache entries to prevent memory leak"
            )
        ...
        if self._openai_client is None:
            try:
                ...
                self._openai_client = AsyncOpenAI(
                    api_key=api_key, timeout=self.openai_config.request_timeout
                )
