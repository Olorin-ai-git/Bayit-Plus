...
class RoutingPattern(BasePattern):
        ...
        try:
            ...
            if not handler:
                return PatternResult.error_result(
                    f"No handler found for route: {classification.route}"
                )
            ...
            if self.ws_streaming:
                await self._stream_routing_complete(
                    classification.route, result.success, context
                )
    ...
    async def _calculate_route_score(
        self, content: str, context: Dict[str, Any], rules: Dict[str, Any]
    ) -> float:
        ...
        context_matches = sum(
            1 for req in context_requirements if req in context and context[req]
        )
    ...
    def _get_classification_reasoning(
        self, route: str, content: str, context: Dict[str, Any]
    ) -> str:
    ...
    def _get_suggested_parameters(
        self, route: str, context: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _handle_device_investigation(
        self, messages: List[BaseMessage], context: Dict[str, Any]
    ) -> PatternResult:
    ...
    async def _handle_location_investigation(
        self, messages: List[BaseMessage], context: Dict[str, Any]
    ) -> PatternResult:
    ...
    async def _handle_network_investigation(
        self, messages: List[BaseMessage], context: Dict[str, Any]
    ) -> PatternResult:
    ...
    async def _handle_comprehensive_investigation(
        self, messages: List[BaseMessage], context: Dict[str, Any]
    ) -> PatternResult:
    ...
    async def _handle_correlation_analysis(
        self, messages: List[BaseMessage], context: Dict[str, Any]
    ) -> PatternResult:
    ...
    async def _stream_classification(
        self, classification: RouteClassification, context: Dict[str, Any]
    ) -> None:
        """Stream classification result"""
        if self.ws_streaming:
            await self.ws_streaming.send_agent_thought(
    ...
    async def _stream_routing_complete(
        self, route: str, success: bool, context: Dict[str, Any]
    ) -> None:
