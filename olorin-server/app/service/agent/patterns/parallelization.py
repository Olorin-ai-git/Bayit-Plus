...
class ParallelizationPattern(BasePattern):
    ...
    def __init__(self, config, tools=None, ws_streaming=None):
        ...
        self.sectioning_strategy = config.custom_params.get(
            "sectioning_strategy", "domain_based"
        )
    ...
    async def execute(
        ...
        try:
            ...
            parallel_tasks = await self._create_parallel_tasks(
                messages, context, strategy
            )
            ...
            final_result = await self._aggregate_results(
                results, parallel_tasks, context
            )

            # Stream completion
            if self.ws_streaming:
                await self._stream_parallel_complete(
                    len(results), final_result.success, context
                )
            ...
            logger.error(
                f"Parallelization pattern execution failed: {str(e)}", exc_info=True
            )
    ...
    async def _create_parallel_tasks(
        self, messages: List[BaseMessage], context: Dict[str, Any], strategy: str
    ) -> List[ParallelTask]:
    ...
    async def _create_domain_based_tasks(
        ...
        for domain, description in domains.items():
            ...
            domain_messages = messages + [
                BaseMessage(
                    content=f"Focus specifically on {description} for entity {context.get('entity_id', 'unknown')}"
                )
            ]
    ...
    async def _create_aspect_based_tasks(
        ...
        for aspect, description in aspects.items():
            ...
            aspect_messages = messages + [
                BaseMessage(
                    content=f"{description} for the current investigation case."
                )
            ]
    ...
    async def _create_voting_tasks(
        ...
        for approach, instruction in voting_approaches.items():
            ...
            voting_messages = messages + [
                BaseMessage(
                    content=f"Investigation instruction: {instruction}. Provide your analysis and conclusion."
                )
            ]
    ...
    async def _create_tool_based_tasks(
        ...
        for category, category_tools in tool_categories.items():
            ...
            task_context.update(
                {
                    ...
                    "assigned_tools": [
                        tool.name for tool in category_tools if hasattr(tool, "name")
                    ],
            ...
            tool_messages = messages + [
                BaseMessage(
                    content=f"Use {category} tools to analyze the investigation case thoroughly."
                )
            ]
            ...
            task_llm_pattern = AugmentedLLMPattern(
                self.config, category_tools, self.ws_streaming
            )
    ...
    def _categorize_tools(self) -> Dict[str, List[Any]]:
        ...
        for tool in self.tools:
            ...
            if any(
                keyword in tool_name for keyword in ["search", "vector", "retriever"]
            ):
                categories["search"].append(tool)
            elif any(
                keyword in tool_name for keyword in ["splunk", "query", "database"]
            ):
    ...
    async def _aggregate_results(
        ...
        if self.voting_strategy == "majority":
            ...
            return await self._best_confidence_voting(
                successful_results, tasks, context
            )
        else:
        ...
        for i, result in enumerate(results):
            if result.success and i < len(tasks):
                ...
                weighted_results.append(
                    {"result": result.result, "weight": weight, "task": task.name}
                )
                total_weight += weight
    ...
    async def _consensus_voting(
        ...
        all_content = [
            str(result.result).lower() for result in results if result.success
        ]
    ...
    async def _simple_aggregation(
        ...
        combined_result = {
            ...
            "task_names": [
                task.name for task in tasks if task.result and task.result.success
            ],
        }
    ...
    async def _stream_tasks_created(
        self, tasks: List[ParallelTask], context: Dict[str, Any]
    ) -> None:
    ...
    async def _stream_parallel_complete(
        self, successful_count: int, overall_success: bool, context: Dict[str, Any]
    ) -> None:
