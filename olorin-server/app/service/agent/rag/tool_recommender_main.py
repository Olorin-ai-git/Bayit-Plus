    ...
    async def recommend_tools(
        ...
        try:
            ...
            if strategy == ToolRecommendationStrategy.EFFECTIVENESS_BASED:
                recommendations = (
                    await self.strategies.effectiveness_based_recommendations(
            ...
            return await self.generate_fallback_recommendations(
                domain, requested_categories
            )
    ...
    async def get_enhanced_tool_list(
        ...
        try:
            ...
            recommendations = await self.recommend_tools(
                investigation_context, domain, requested_categories=categories
            )
            ...
            if categories and not recommended_tools:
                ...
                for category in categories:
                    category_tools.extend(
                        self.tool_registry.get_tools_by_category(category)
                    )
                recommended_tools.extend(
                    category_tools[: self.config.max_recommended_tools]
                )

            self.logger.debug(
                f"Enhanced tool list: {len(recommended_tools)} tools for {domain}"
            )
