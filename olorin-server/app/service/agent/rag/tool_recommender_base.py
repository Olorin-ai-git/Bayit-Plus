...
class ToolRecommenderBase:
    ...
    async def filter_and_rank_recommendations(
        ...
        filtered = [r for r in recommendations if r.confidence >= self.config.min_confidence_threshold]
        ...
        for rec in filtered:
            ...
            if category_counts.get(tool_category, 0) < self.config.max_tools_per_category:
    ...
    async def generate_fallback_recommendations(
        ...
        for tool in tools[:self.config.max_recommended_tools]:
            recommendation = ToolRecommendation(
                ...
                domain_match=any(domain_term in tool.name.lower() for domain_term in [domain])
            )
    ...
    def update_recommendation_statistics(
        ...
        self.recommendation_stats["knowledge_enhanced_recommendations"] += knowledge_enhanced
        ...
        if recommendations:
            ...
            self.recommendation_stats["avg_recommendation_confidence"] = (
                (current_avg * (total_recs - 1) + avg_confidence) / total_recs
            )
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        enhanced_rate = self.recommendation_stats["knowledge_enhanced_recommendations"] / total_recs
        fallback_rate = self.recommendation_stats["fallback_recommendations"] / total_recs
