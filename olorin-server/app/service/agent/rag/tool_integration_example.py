    ...
    if _tool_recommender is None:
        try:
            ...
            if rag_orchestrator and tool_registry.is_initialized():
                ...
                logger.warning("Cannot initialize tool recommender: RAG orchestrator or tool registry not available")
    ...
    try:
        if use_rag_recommendations and RAG_AVAILABLE:
            ...
            if tool_recommender:
                ...
                if enhanced_tools:
                    logger.debug(f"RAG recommender provided {len(enhanced_tools)} tools for {domain}")
                    return enhanced_tools
                else:
                    logger.warning(f"RAG recommender returned no tools, falling back to standard selection")
        ...
        logger.error(f"Enhanced tool selection failed: {e}, falling back to standard selection")
        return get_tools_for_agent(categories=categories, tool_names=tool_names)
        ...
        for rec in recommendations:
            rec_dict = {
                ...
                "effectiveness_metrics": {
                    "success_rate": rec.effectiveness_metrics.success_rate if rec.effectiveness_metrics else 0.0,
                    "domain_relevance": rec.effectiveness_metrics.domain_relevance if rec.effectiveness_metrics else 0.0
                } if rec.effectiveness_metrics else None
            }
            recommendation_details.append(rec_dict)
        
        logger.info(f"Generated {len(recommendation_details)} detailed recommendations for {domain}")
...
class EnhancedAgentToolSelection:
    ...
    async def get_risk_agent_tools(investigation_context: AutonomousInvestigationContext) -> List[BaseTool]:
        ...
        risk_categories = ["olorin", "search", "database", "threat_intelligence", "ml_ai"]
...
def get_tool_recommender_stats() -> dict:
    ...
    if not tool_recommender:
        return {"available": False, "reason": "RAG not available or tool recommender not initialized"}
