...
class ToolAnalysisUtils:
    ...
    def extract_tool_mentions(self, content: str) -> List[str]:
        ...
        for tool_name in all_tools:
            ...
            if (
                tool_name.lower() in content_lower
                or tool_name.replace("_", " ").lower() in content_lower
            ):
    ...
    def extract_success_indicators(self, content: str) -> Dict[str, float]:
        ...
        if any(
            ...
            for word in ["successful", "effective", "accurate", "reliable"]
        ):
        ...
        if any(
            word in content_lower
            for word in ["failed", "ineffective", "inaccurate", "unreliable"]
        ):
            success_indicators["success_rate"] -= 0.2
            success_indicators["confidence"] -= 0.15
    ...
    def calculate_effectiveness_confidence(
        self, metrics: ToolEffectivenessMetrics, knowledge_context: KnowledgeContext
    ) -> float:
        ...
        if any(
            metrics.tool_name in chunk.content
    ...
    def calculate_domain_relevance(
        self, tool: BaseTool, domain: str, domain_preferences: Dict[str, float]
    ) -> float:
        ...
        if any(
            pattern in tool.name.lower() or pattern in tool.description.lower()
            for pattern in domain_tools
        ):
            relevance += 0.2
    ...
    def extract_effectiveness_metrics(
        self, knowledge_context: KnowledgeContext, domain: str
        ...
        for chunk in knowledge_context.critical_knowledge:
            ...
            for tool_name in tools_mentioned:
                ...
                metrics[tool_name].success_rate = success_indicators.get(
                    "success_rate", 0.8
                )
                metrics[tool_name].avg_confidence = success_indicators.get(
                    "confidence", 0.75
                )
                metrics[tool_name].domain_relevance = (
                    0.9 if domain in chunk.content.lower() else 0.6
                )
    ...
    def extract_case_similarity_patterns(
        ...
        for chunk in knowledge_context.supporting_knowledge:
            ...
            if (
                ...
                and investigation_context.entity_type.value in chunk.content
            ):
                similarity_score += 0.2

            for tool_name in tools_mentioned:
                patterns[tool_name] = max(
                    patterns.get(tool_name, 0.0), similarity_score
                )

        return patterns

    def extract_domain_preferences(
        self, knowledge_context: KnowledgeContext, domain: str
    ) -> Dict[str, float]:
    ...
    def generate_effectiveness_reasoning(
        ...
        reasoning_parts = [f"Historical success rate: {metrics.success_rate:.1%}"]
        ...
        if knowledge_context.critical_knowledge:
            reasoning_parts.append(
                f"mentioned in {len(knowledge_context.critical_knowledge)} critical knowledge sources"
            )
    ...
    def generate_hybrid_reasoning(
        ...
        if (
            recommendation.effectiveness_metrics
            and recommendation.effectiveness_metrics.success_rate > 0.8
        ):
    ...
    def determine_tool_category(self, tool: BaseTool) -> str:
        ...
        if any(
            ...
            for pattern in ["threat", "virus", "abuse", "shodan"]
        ):
            return "threat_intelligence"
        elif any(
            pattern in tool_name_lower for pattern in ["splunk", "sumo", "snowflake"]
        ):
            return "olorin"
            ...
            pattern in tool_name_lower
            for pattern in ["ml", "ai", "behavioral", "anomaly"]
        ):
            return "ml_ai"
        else:
            return "utility"
