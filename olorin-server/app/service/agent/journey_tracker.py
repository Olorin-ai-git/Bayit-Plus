...
class LangGraphJourneyTracker:
    ...
    def __init__(self, output_directory: Optional[Path] = None):
        ...
        logger.info(
            f"Initialized LangGraphJourneyTracker with output: {self.output_directory}"
        )

    def start_journey_tracking(
        self, investigation_id: str, initial_state: Dict[str, Any]
    ) -> None:
        ...
        with self._global_lock:
            ...
            if investigation_id in self._active_journeys:
                logger.debug(
                    f"Journey tracking already active for investigation: {investigation_id}"
                )
                return
            ...
            logger.info(
                f"Started journey tracking for investigation: {investigation_id}"
            )
    ...
    def track_node_execution(
        ...
        with journey_lock:
            ...
            if investigation_id not in self._active_journeys:
                logger.error(
                    f"Failed to create journey for investigation: {investigation_id}"
                )
                return None
        ...
        if nx:
            ...
            graph.add_node(
                ...
                tool_name=tool_name,
                execution_count=len(
                    [n for n in journey.node_executions if n.node_name == node_name]
                ),
            )

        # Notify monitoring callbacks
        self._notify_callbacks(
            investigation_id, "node_execution", asdict(node_execution)
        )

        logger.debug(
            f"Tracked node execution: {node_name} ({duration_ms}ms) - {status.value}"
        )
        return execution_id
    ...
    def track_state_transition(
        ...
        with journey_lock:
            ...
            if investigation_id not in self._active_journeys:
                logger.error(
                    f"Failed to create journey for state transition: {investigation_id}"
                )
                return None
        ...
        self._notify_callbacks(
            investigation_id, "state_transition", asdict(state_transition)
        )

        logger.debug(
            f"Tracked state transition: {from_node} -> {to_node} ({transition_reason})"
        )
    ...
    def track_agent_coordination(
        ...
        with journey_lock:
            ...
            if investigation_id not in self._active_journeys:
                logger.error(
                    f"Failed to create journey for agent coordination: {investigation_id}"
                )
                return None
        ...
        self._notify_callbacks(
            investigation_id, "agent_coordination", asdict(agent_coordination)
        )

        logger.debug(
            f"Tracked agent coordination: {from_agent} -> {to_agent} ({coordination_type})"
        )
    ...
    def complete_journey(
        ...
        total_duration = sum(
            execution.duration_ms for execution in journey.node_executions
        )
        unique_agents = set(
        ...
        journey.journey_metadata.update(
            {
                ...
                "execution_path": [
                    execution.node_name for execution in journey.node_executions
                ],
            }
        )
        ...
        logger.info(
            f"Completed journey tracking for investigation: {investigation_id} "
    ...
    def get_journey_status(self, investigation_id: str) -> Dict[str, Any]:
        ...
        with self._global_lock:
            if investigation_id not in self._journey_locks:
                return {
                    "error": f"No active journey for investigation: {investigation_id}"
                }
            journey_lock = self._journey_locks[investigation_id]
        ...
        recent_executions = (
            journey.node_executions[-5:] if journey.node_executions else []
        )
        current_agents = set(
            execution.agent_name
        ...
        return {
            ...
            "graph_structure": {
                "total_nodes": (
                    ...
                    if nx and hasattr(graph, "number_of_nodes")
                    else 0
                ),
                "total_edges": (
                    graph.number_of_edges()
                    if nx and hasattr(graph, "number_of_edges")
                    else 0
                ),
                "execution_path": [
                    execution.node_name for execution in journey.node_executions
            ...
            with journey_lock:
                ...
                execution = NodeExecution(
                    ...
                    state_changes={
                        ...
                        "performance_delta": selection_time_ms
                        - 100,  # Difference from target
                    },
                    metadata={
                ...
                if investigation_id in self._active_journeys:
                    self._active_journeys[investigation_id].node_executions.append(
                        execution
                    )
                else:
    ...
    def generate_journey_visualization(self, investigation_id: str) -> Dict[str, Any]:
        ...
        if not journey:
            ...
            try:
                ...
                if investigation_folder_path:
                    journey_file = (
                        investigation_folder_path / f"journey_{investigation_id}.json"
                    )
            except Exception:
                pass
            ...
            if not journey_file or not journey_file.exists():
                journey_file = (
                    self.output_directory / f"journey_{investigation_id}.json"
                )
        ...
        for transition in journey.state_transitions:
            if (
                transition.from_node in node_positions
                and transition.to_node in node_positions
            ):
                edge_data = {
    ...
    def _calculate_state_changes(
        ...
        for key in set(input_state.keys()) & set(output_state.keys()):
            if input_state[key] != output_state[key]:
                modified[key] = {"old": input_state[key], "new": output_state[key]}
    ...
    def _save_journey(self, journey: InvestigationJourney) -> None:
        ...
        try:
            ...
            folder_manager = get_folder_manager()
            investigation_folder_path = folder_manager.get_investigation_folder(
                journey.investigation_id
            )
            if investigation_folder_path:
                investigation_folder = investigation_folder_path
                journey_file = (
                    investigation_folder / f"journey_{journey.investigation_id}.json"
                )
            else:
                # Fall back to default directory
                journey_file = (
                    self.output_directory / f"journey_{journey.investigation_id}.json"
                )
        except Exception as e:
    ...
    def _notify_callbacks(
        self, investigation_id: str, event_type: str, data: Dict[str, Any]
    ) -> None:
