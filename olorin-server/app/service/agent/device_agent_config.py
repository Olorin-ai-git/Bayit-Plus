...
def get_device_objectives(rag_enabled: bool = False, mcp_enhanced: bool = False) -> List[str]:
...
def create_device_agent_metadata(rag_enabled: bool, rag_stats: Dict[str, Any], mcp_enhanced: bool = False) -> Dict[str, Any]:
...
def format_completion_message(rag_enabled: bool, findings_count: int, risk_score: float, rag_stats: Dict[str, Any], mcp_enhanced: bool = False) -> str:
    ...
    if mcp_enhanced:
        base_message += ", MCP services: connection pooling, circuit breakers, distributed caching"
...
def create_result_structure(findings, rag_enabled: bool, rag_stats: Dict[str, Any], mcp_enhanced: bool = False) -> Dict[str, Any]:
    ...
    if mcp_enhanced:
        summary_suffix += " (MCP: enhanced reliability & performance)" if summary_suffix else " (MCP: enhanced reliability & performance)"
    ...
    return {
        "risk_assessment": {
            ...
            "summary": f"{'MCP-enhanced' if mcp_enhanced else ('RAG-enhanced' if rag_enabled else 'Autonomous')} device analysis: {len(findings.key_findings)} findings{summary_suffix}",
