...
class MockLLM(BaseChatModel):
    ...
    async def _agenerate(
        ...
        **kwargs: Any,
        ...
        if is_orchestrator_init:
            logger.warning(
                f"🎯 MockLLM: Orchestrator initialization detected - generating database query tool call"
            )
        ...
        try:
            ...
            if domain:
                logger.warning(
                    f"🎯 MockLLM: Attempting to use REAL LLM response for domain: {domain}"
                )
                real_response = get_real_response(
                    domain, entity_id, entity_risk_score or 0.5
                )
                ...
                if real_response and "content" in real_response:
                    ...
                    logger.warning(
                        f"✅ Using REAL LLM response with {len(tool_calls)} tool calls"
                    )
                    logger.warning(
                        f"📊 Real response risk score: {real_response.get('risk_score', 'N/A')}"
                    )
        ...
        if use_enhanced:
            try:
                ...
                logger.warning(
                    "🧠 Using ENHANCED mock responses with chain-of-thought reasoning"
                )
                ...
                logger.warning(
                    f"Could not use enhanced mock responses, falling back to standard: {e}"
                )
    ...
    def _extract_domain(self, messages: List[BaseMessage]) -> str:
        ...
        for message in messages:
            ...
            if (
                "verify the quality" in content
                or "verification" in content
                or "verify quality" in content
            ):
                return "verification"
    ...
    def _extract_entity_risk_score(
        self, messages: List[BaseMessage], kwargs: Dict
    ) -> Optional[float]:
    ...
    def _generate_database_tool_call(self, entity_id: str) -> ChatResult:
        ...
        tool_call = {
            ...
            "args": {"query": sql_query.strip(), "limit": 100},
            "id": "call_db_001",
            "type": "tool_call",
        }
        ...
        logger.warning(
            f"   Query: SELECT from transactions_enriched WHERE ip = '{entity_id}'"
        )
    ...
    def bind_tools(self, tools: List[Any], **kwargs) -> "MockLLM":
        ...
        logger.info(
            f"MockLLM: Binding {len(tools)} tools (mock - tools not actually used)"
        )
        return self
