"""
Structured Risk Assessment Agent

Risk domain structured investigation agent using LLM-driven tool selection.
"""
...
async def structured_risk_agent(state, config) -> dict:
    """Structured risk assessment using LLM-driven tool selection with optional RAG enhancement"""
    ...
    from .risk_agent_config import (
        create_risk_agent_metadata,
        create_risk_rag_config,
        get_risk_objectives,
    ...
    journey_tracker.track_node_execution(
        ...
        input_state={
            "entity_id": entity_id,
            "risk_analysis": "starting",
            "investigation_phase": "risk_assessment",
        ...
        agent_name="RAG-Enhanced-RiskAgent" if RAG_AVAILABLE else "StructuredRiskAgent",
    ...
    structured_context.start_domain_analysis("risk")
    ...
    try:
        ...
        if RAG_AVAILABLE and rag_config:
            ...
            logger.info(
                "⚠️ Created risk agent with intelligent RAG-enhanced tool selection"
            )
        else:
        ...
        risk_objectives = get_risk_objectives(
        ...
        structured_context.record_domain_findings("risk", findings)

        # Track risk agent completion with RAG metrics
        completion_metadata = create_risk_agent_metadata(
        ...
        journey_tracker.track_node_execution(
            ...
            agent_name=(
                "RAG-Enhanced-RiskAgent" if RAG_AVAILABLE else "StructuredRiskAgent"
            ),
        ...
        from .unified_agent_schema import AgentType, ensure_valid_response
        ...
        fallback_data = {
            ...
            "context_summary": (
                str(structured_context)[:500] if structured_context else ""
            ),
        }
        ...
        try:
            ...
            domain_specific = {
                ...
                "aggregation_metadata": {
                    "rag_enabled": RAG_AVAILABLE and rag_config is not None,
                    "knowledge_retrievals": rag_stats.get(
                        "knowledge_retrieval_count", 0
                    ),
                    "context_augmentation": rag_stats.get(
                        "context_augmentation_success", False
                    ),
                },
            ...
            logger.info(
                f"✅ Risk agent created unified response with score: {unified_response.overall_risk_score}"
            )
            ...
            return {
                "messages": [AIMessage(content=fallback_response.model_dump_json())]
            }
        ...
        structured_context.fail_domain_analysis("risk", str(e))
        ...
        from .unified_agent_schema import AgentType, create_agent_response
