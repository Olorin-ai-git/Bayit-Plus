...
class MultiEntityInvestigationCoordinator:
    ...
    async def coordinate_multi_entity_investigation(
        self, request: MultiEntityInvestigationRequest
    ) -> Dict[str, Any]:
        """
        ...
        self.logger.info(
            f"🔄 Coordinating multi-entity investigation: {investigation_id}"
        )
        ...
        try:
            ...
            self.logger.info(
                f"✅ Multi-entity coordination completed: {investigation_id}"
            )
            return coordination_result

        except Exception as e:
            self.logger.error(
                f"❌ Multi-entity coordination failed {investigation_id}: {str(e)}"
            )
            raise
    ...
    def _build_multi_entity_workflow(self) -> StateGraph:
        ...
        workflow.add_node(
            "parallel_entity_investigation", self._parallel_entity_investigation_node
        )
    ...
    def _initialize_workflow_state(
        ...
        return MultiEntityWorkflowState(
            messages=[
                HumanMessage(
                    content=f"Starting multi-entity investigation {request.investigation_id}"
                )
            ],
            investigation_id=request.investigation_id,
    ...
    async def _initialize_investigation_node(
        self, state: MultiEntityWorkflowState
    ) -> MultiEntityWorkflowState:
        """Initialize investigation node"""

        self.logger.info(
            f"🚀 Initializing multi-entity investigation: {state['investigation_id']}"
        )
        ...
        state["messages"].append(
            AIMessage(
                content=f"Initialized investigation for {len(state['entities'])} entities"
            )
        )

        return state

    async def _parallel_entity_investigation_node(
        self, state: MultiEntityWorkflowState
    ) -> MultiEntityWorkflowState:
        ...
        self.logger.info(
            f"🔍 Starting parallel entity investigations: {investigation_id}"
        )
        ...
        for entity_id, context in state["entity_contexts"].items():
            ...
            task = self._create_entity_investigation_task(entity_id, context, state)
            investigation_tasks.append(task)

        # Execute all entity investigations in parallel
        try:
            entity_results = await asyncio.gather(
                *investigation_tasks, return_exceptions=True
            )
            ...
            for i, (entity_id, context) in enumerate(state["entity_contexts"].items()):
                ...
                if isinstance(result, Exception):
                    self.logger.error(
                        f"Entity investigation failed for {entity_id}: {str(result)}"
                    )
        ...
        state["messages"].append(
            AIMessage(
                content=f"Completed investigations for {len(state['completed_entities'])} entities"
            )
        )
            ...
            if agent_scope in self.agent_functions:
                try:
                    ...
                    agent_state = MessagesState(
                        messages=[
                            HumanMessage(
                                content=f"Investigate {entity_id} for {agent_scope} analysis"
                            )
                        ]
                    )
                    ...
                    self.logger.info(
                        f"✅ {agent_scope} agent completed for entity {entity_id}"
                    )

                except Exception as e:
                    self.logger.error(
                        f"❌ {agent_scope} agent failed for entity {entity_id}: {str(e)}"
                    )
                    # Create error result
    ...
    def _extract_agent_result(
        ...
        return InvestigationResult(
            ...
            agent_reasoning=content.get(
                "reasoning", f"Executed {agent_scope} analysis for {entity_id}"
            ),
        )

    async def _cross_entity_analysis_node(
        self, state: MultiEntityWorkflowState
    ) -> MultiEntityWorkflowState:
        ...
        state["messages"].append(
            AIMessage(
                content=f"Cross-entity analysis completed with {len(state['cross_entity_findings'])} findings"
            )
        )
    ...
    def _find_cross_entity_patterns(
        self, state: MultiEntityWorkflowState
    ) -> List[Dict[str, Any]]:
    ...
    def _calculate_risk_correlations(
        self, state: MultiEntityWorkflowState
    ) -> List[Dict[str, Any]]:
        ...
        for i in range(len(entity_ids)):
            for j in range(i + 1, len(entity_ids)):
                ...
                correlation = {
                    ...
                    - abs(risk1 - risk2),  # Higher when risks are similar
                    "correlation_type": (
                        "risk_similarity"
                        if abs(risk1 - risk2) < 0.3
                        else "risk_divergence"
                    ),
                }
    ...
    async def _finalize_investigation_node(
        ...
        investigation_id = state["investigation_id"]
        self.logger.info(
            f"🏁 Finalizing multi-entity investigation: {investigation_id}"
        )
        ...
        state["messages"].append(
            AIMessage(
                content=f"Multi-entity investigation {investigation_id} finalized"
            )
        )
    ...
    async def _execute_workflow(
        self, workflow: StateGraph, initial_state: MultiEntityWorkflowState
    ) -> MultiEntityWorkflowState:
    ...
    def _extract_coordination_results(
        self, final_state: MultiEntityWorkflowState
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": (
                len(final_state["completed_entities"]) / len(final_state["entities"])
    ...
    def _calculate_average_risk(
        self, agent_results: List[InvestigationResult]
    ) -> float:
    ...
    def _detect_agent_pattern(
        self, agent_type: str, findings_list: List[Dict[str, Any]]
    ...
    def get_coordination_metrics(self) -> Dict[str, Any]:
        ...
        return {
            **self.metrics,
            "success_rate": (
                self.metrics["successful_investigations"]
                / self.metrics["total_coordinated_investigations"]
                if self.metrics["total_coordinated_investigations"] > 0
                else 0.0
            ),
            "average_entities_per_investigation": (
                self.metrics["parallel_entity_investigations"]
                / self.metrics["total_coordinated_investigations"]
                if self.metrics["total_coordinated_investigations"] > 0
                else 0.0
            ),
