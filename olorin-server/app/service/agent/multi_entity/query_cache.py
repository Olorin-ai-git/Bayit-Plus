...
class MultiEntityQueryCache:
    ...
    def __init__(
        ...
        logger.info(f"MultiEntityQueryCache initialized (max_size: {max_size}, ttl: {default_ttl}s, max_memory: {max_memory_mb}MB)")
    ...
    def _calculate_query_complexity(self, boolean_logic: str, entity_count: int) -> float:
    ...
    def put(
        ...
        try:
            ...
            # Generate cache key
            cache_key = self._generate_cache_key(query_type, entities, boolean_logic, extra_params)
            
            # Calculate entry size
            entry_size = self._calculate_entry_size(result)
            
            # Check if entry is too large
            if entry_size > self.max_memory_bytes * 0.1:  # Don't cache entries > 10% of max memory
                logger.warning(f"Query result too large to cache: {entry_size} bytes")
                return False
            
            with self._lock:
                # Evict entries if needed
                if (entry_size + self._stats["memory_usage_bytes"] > self.max_memory_bytes or
                ...
                self._stats["avg_query_complexity"] = (current_avg * 0.9) + (complexity * 0.1)
                
                logger.debug(f"Cached query result (key: {cache_key}, size: {entry_size} bytes, complexity: {complexity:.2f})")
                return True
    ...
    def warm_cache_for_entities(self, entities: List[str], common_queries: List[str]):
        ...
        """
        try:
            logger.info(f"Warming cache for {len(entities)} entities with {len(common_queries)} query patterns")
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                "memory_usage_percent": (self._stats["memory_usage_bytes"] / self.max_memory_bytes) * 100,
                "hit_rate": hit_rate,
