...
from .entity_manager import EntityType, Entity, get_entity_manager
...
class TransactionEntityFactory:
        ...
        for csv_column, entity_type in self.CSV_COLUMN_MAPPINGS.items():
            if csv_column in row_data and row_data[csv_column] is not None:
                ...
                try:
                    ...
                    if entity_type in [EntityType.RECORD_CREATED, EntityType.RECORD_UPDATED, 
                                     EntityType.TX_DATETIME, EntityType.TX_RECEIVED, EntityType.TIMESTAMP]:
                        parsed_timestamp = self._parse_timestamp(value)
                        if parsed_timestamp:
                            attributes['parsed_timestamp'] = parsed_timestamp.isoformat()
                    ...
                    logger.info(f"Created {entity_type.value} entity {entity_id} from CSV column {csv_column}")
    ...
    async def create_entities_from_csv_batch(
        ...
        for i, row_data in enumerate(csv_data):
            try:
                entity_ids = await self.create_entities_from_csv_row(row_data, investigation_id)
                all_entity_ids.extend(entity_ids)
                
                # Log progress for large batches
                if (i + 1) % 100 == 0:
                    logger.info(f"Processed {i + 1} CSV rows, created {len(all_entity_ids)} entities")
        ...
        return {
            ...
            'entities_per_second': len(all_entity_ids) / processing_time if processing_time > 0 else 0
        }
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'supported_entity_types': [et.value for et in self.CSV_COLUMN_MAPPINGS.values()],
        }
