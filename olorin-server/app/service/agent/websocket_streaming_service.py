...
class WebSocketStreamingService:
    ...
    async def send_agent_thought(self, thought_data: Dict[str, Any]) -> None:
        ...
        event = self._create_base_event(
            StreamEventType.AGENT_THOUGHT, StreamPriority.NORMAL
        )
        event.update(
            {
                ...
                "metadata": {
                    ...
                    if k not in ["message", "pattern", "confidence", "type"]
                },
            }
        )
    ...
    async def send_tool_execution(self, tool_data: Dict[str, Any]) -> None:
        ...
        event = self._create_base_event(
            StreamEventType.TOOL_EXECUTION, StreamPriority.HIGH
        )
        event.update(
            {
    ...
    async def send_tool_result(
        ...
        event = self._create_base_event(
            StreamEventType.TOOL_RESULT, StreamPriority.HIGH
        )
        event.update(
            {
                "tool_name": tool_name,
    ...
    async def send_tool_error(self, tool_name: str, error_message: str) -> None:
        ...
        event = self._create_base_event(
            StreamEventType.TOOL_ERROR, StreamPriority.CRITICAL
        )
        event.update(
            {
                "tool_name": tool_name,
    ...
    async def send_chain_start(self, chain_type: str, steps: List[str]) -> None:
        ...
        event = self._create_base_event(
            StreamEventType.CHAIN_START, StreamPriority.NORMAL
        )
        event.update(
            {
    ...
    async def send_chain_complete(
        self, chain_type: str, success: bool, results: Dict[str, Any]
    ) -> None:
        """Send prompt chain completion event"""

        event = self._create_base_event(
            StreamEventType.CHAIN_COMPLETE, StreamPriority.NORMAL
        )
        event.update(
            {
                "chain_type": chain_type,
    ...
    async def send_step_progress(
        self, step_name: str, step_number: int, total_steps: int
    ) -> None:
    ...
    async def send_route_classification(
        self, route: str, confidence: float, reasoning: str
    ) -> None:
        """Send routing classification result"""

        event = self._create_base_event(
            StreamEventType.ROUTE_CLASSIFICATION, StreamPriority.NORMAL
        )
        event.update(
            {
    ...
    async def send_parallel_start(self, task_count: int, strategy: str) -> None:
        ...
        event = self._create_base_event(
            StreamEventType.PARALLEL_START, StreamPriority.NORMAL
        )
        event.update(
            {
    ...
    async def send_parallel_progress(
        self, completed_tasks: int, total_tasks: int, current_task: str
    ) -> None:
    ...
    async def send_orchestration_start(
        self, worker_count: int, task_breakdown: List[Dict[str, Any]]
    ) -> None:
        """Send orchestration start event"""

        event = self._create_base_event(
            StreamEventType.ORCHESTRATION_START, StreamPriority.NORMAL
        )
        event.update(
            {
                ...
                "estimated_duration": len(task_breakdown)
                * 45,  # Orchestrated task estimate
            }
        )
    ...
    async def send_worker_assignment(
        self, worker_name: str, task_name: str, specialization: str
    ) -> None:
        """Send worker task assignment event"""

        event = self._create_base_event(
            StreamEventType.WORKER_ASSIGNMENT, StreamPriority.LOW
        )
        event.update(
            {
    ...
    async def send_optimization_cycle(
        self, cycle: int, quality_score: float, improvements: List[str]
    ) -> None:
        """Send optimization cycle event"""

        event = self._create_base_event(
            StreamEventType.OPTIMIZATION_CYCLE, StreamPriority.NORMAL
        )
        event.update(
            {
                "cycle_number": cycle,
    ...
    async def send_investigation_start(
        self, investigation_type: str, entity_details: Dict[str, Any]
    ) -> None:
        """Send investigation start event"""

        event = self._create_base_event(
            StreamEventType.INVESTIGATION_START, StreamPriority.HIGH
        )
        event.update(
            {
                ...
                "estimated_completion": entity_details.get(
    ...
    async def send_investigation_complete(
        self, success: bool, results: Dict[str, Any], execution_summary: Dict[str, Any]
    ) -> None:
        """Send investigation completion event"""

        event = self._create_base_event(
            StreamEventType.INVESTIGATION_COMPLETE, StreamPriority.HIGH
        )
        event.update(
            {
                "success": success,
    ...
    async def send_warning(
        self, warning_message: str, context: Dict[str, Any] = None
    ) -> None:
        """Send warning event"""
    ...
    async def send_info(
        self, info_message: str, details: Dict[str, Any] = None
    ) -> None:
    ...
    def _create_base_event(
        self, event_type: StreamEventType, priority: StreamPriority
    ) -> Dict[str, Any]:
    ...
    async def _send_single_event(self, event: Dict[str, Any]) -> None:
        ...
        if self.websocket_manager:
            try:
                ...
                ws_message = {"type": "investigation_update", "data": event}

                # Send via WebSocket manager
                await self.websocket_manager.send_to_investigation(
                    self.investigation_id, json.dumps(ws_message)
                )
            ...
            logger.info(
                f"Stream event: {event['event_type']} - {event.get('thought', event.get('info_message', 'Event'))}"
            )
    ...
    async def _flush_batch(self) -> None:
        ...
        if self.websocket_manager:
            try:
                await self.websocket_manager.send_to_investigation(
                    self.investigation_id, json.dumps(batch_message)
                )
            except Exception as e:
    ...
    async def close(self) -> None:
        ...
        await self.send_info(
            "Streaming service closed",
            {
                "total_events_sent": self.events_sent,
                "total_execution_time": self._calculate_total_execution_time(),
                "average_event_rate": self.events_sent
                / max(self._calculate_total_execution_time() or 1, 1),
            },
        )
