...
class AuditTrailManager:
    ...
    def __init__(
        self, retention_days: int = 365, pii_config: Optional[PIIMaskingConfig] = None
    ):
        """
        ...
        self._encryption_key = (
            self._pii_config.encryption_key or Fernet.generate_key().decode()
        )
    ...
    def generate_compliance_report(
        ...
        """
        start_date = start_date or datetime.utcnow() - timedelta(
        ...
        return {
            ...
            "events_by_type": {
                event_type.name: len(
                    [r for r in filtered_records if r.event_type == event_type]
                )
                for event_type in AuditEventType
            },
            "regulations_covered": list(
                set(
                    reg.name
                    for record in filtered_records
                    for reg in record.regulations
                )
            ),
    ...
    def purge_expired_records(self):
        ...
        self._audit_records = [
            record for record in self._audit_records if record.timestamp > cutoff_time
        ]
