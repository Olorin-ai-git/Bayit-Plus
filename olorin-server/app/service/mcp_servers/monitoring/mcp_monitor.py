...
class MCPMonitor:
    ...
    async def _check_server_health(self, server_name: str) -> HealthCheck:
        ...
        try:
            ...
            checks_passed["response_time"] = response_time < self.sla_targets["response_time_p95"]
            ...
            if metrics:
                checks_passed["error_rate"] = metrics.error_rate <= self.sla_targets["error_rate"]
    ...
    async def _check_resources(self, server_name: str) -> bool:
        ...
        return (
            metrics.memory_usage_mb < 1000 and  # 1GB threshold
    ...
    async def _aggregate_metrics(self, metrics: ServerMetrics):
        ...
        if metrics.total_requests > 0:
            metrics.success_rate = (metrics.success_count / metrics.total_requests) * 100
            metrics.error_rate = (metrics.error_count / metrics.total_requests) * 100
        
        # Update Prometheus metrics
        if self.enable_prometheus:
            self.prom_success_rate.labels(server=metrics.server_name).set(metrics.success_rate)
    ...
    async def _create_alert(
        ...
        # Set cooldown
        self.alert_cooldowns[alert_key] = datetime.now() + timedelta(seconds=self.alert_cooldown)
        ...
        for server_name, metrics in self.servers.items():
            ...
            server_report = {
                ...
                "sla_compliance": {
                    ...
                    "response_time": metrics.p95_response_time <= self.sla_targets["response_time_p95"],
