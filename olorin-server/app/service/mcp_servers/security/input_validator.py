...
class MCPInputValidator:
    ...
    def validate(
        ...
        try:
            ...
            # Security validation
            result = self._security_validation(
                result.sanitized_value, input_type, result
            )
            ...
            if additional_rules:
                result = self._apply_custom_rules(
                    result.sanitized_value, additional_rules, result
                )
            ...
            return ValidationResult(
                is_valid=False, errors=[f"Validation failed: {str(e)}"], risk_score=1.0
            )
            ...
            if isinstance(value, str):
                try:
                    ...
                    result.errors.append(
                        f"{field_name} contains invalid JSON: {str(e)}"
                    )
                    return result
                ...
                if self.validation_level in [
                    ...
                    if ip.is_private or ip.is_loopback:
                        result.warnings.append(
                            f"{field_name} is a private/localhost address"
                        )
    ...
    def _security_validation(
        self, value: Any, input_type: InputType, result: ValidationResult
    ) -> ValidationResult:
        ...
        for pattern in self.command_injection_regex:
            if pattern.search(value):
                result.is_valid = False
                result.errors.append(
                    "Input contains potentially dangerous command characters"
                )
    ...
    def _apply_custom_rules(
        self, value: Any, rules: List[ValidationRule], result: ValidationResult
    ) -> ValidationResult:
    ...
    def _sanitize_string(self, value: str) -> str:
        ...
        if BLEACH_AVAILABLE and self.validation_level in [
            ValidationLevel.STRICT,
            ValidationLevel.PARANOID,
        ]:
    ...
    def validate_mcp_tool_input(
        ...
        results["tool_name"] = self.validate(
            tool_name, InputType.TOOL_NAME, "tool_name"
        )
    ...
    def _determine_input_type(
        self, field_name: str, value: Any, schema: Optional[Dict[str, Any]] = None
    ) -> InputType:
        ...
        if "user_id" in field_name_lower or "userid" in field_name_lower:
            ...
            "transaction_id" in field_name_lower or "transactionid" in field_name_lower
        ):
