...
class ComposioEncryption:
    ...
    def _load_encryption_key(self) -> bytes:
        ...
        if kms_key:
            ...
            try:
                ...
                logger.warning(
                    f"Failed to decode KMS key: {e}, falling back to environment"
                )
        ...
        if password:
            salt = self.config_loader.load_secret(
                "COMPOSIO_ENCRYPTION_SALT", "default_salt"
            ).encode()
            kdf = PBKDF2HMAC(
    ...
    def decrypt(
        self, encrypted_data: str, associated_data: Optional[bytes] = None
    ) -> str:
        """
