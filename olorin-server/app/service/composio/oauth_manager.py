...
class OAuthManager:
    ...
    def initiate_oauth(
        ...
        try:
            ...
            logger.info(f"Initiated OAuth flow for toolkit {toolkit}, tenant {tenant_id}")
            return oauth_url
        ...
        try:
            ...
            with get_db_session() as db:
                ...
                if existing:
                    ...
                    db.commit()
                    db.refresh(existing)
                    logger.info(f"Updated existing connection {connection_id} for tenant {tenant_id}")
                    return existing
                else:
                    ...
                    logger.info(f"Created new connection {connection_id} for tenant {tenant_id}")
                    return connection
    ...
    def refresh_connection_token(
        ...
        try:
            with get_db_session() as db:
                ...
                if not connection:
                    raise ComposioConnectionError(f"Connection {connection_id} not found for tenant {tenant_id}")
                
                if not connection.refresh_token:
                    raise ComposioConnectionError(f"Connection {connection_id} has no refresh token")
                ...
                logger.info(f"Refreshed token for connection {connection_id}, tenant {tenant_id}")
                return connection
        ...
        try:
            with get_db_session() as db:
                ...
                if is_expired and connection.refresh_token:
                    try:
                        connection = self.refresh_connection_token(connection_id, tenant_id)
                        is_expired = False
                        is_active = True
                    except Exception as e:
                        logger.warning(f"Auto-refresh failed for connection {connection_id}: {e}")
