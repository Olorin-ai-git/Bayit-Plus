...
class PlaybookExecutor:
    ...
    async def execute_playbook(
        ...
        try:
            ...
            if response.status_code == 202:
                ...
                logger.info(f"SOAR playbook {playbook_id} accepted, execution_id={execution.id}")
                return execution
                ...
                execution.status = 'failed'
                execution.error_message = f"SOAR API error: {response.status_code} - {error_text}"
                execution.completed_at = datetime.utcnow()
                with get_db_session() as db:
                    db.merge(execution)
                    db.commit()
                raise Exception(f"SOAR API error: {response.status_code} - {error_text}")
                
        except Exception as e:
            logger.error(f"Failed to execute SOAR playbook {playbook_id}: {e}", exc_info=True)
            raise
    ...
    async def get_execution_status(
        ...
        try:
            with get_db_session() as db:
                execution = db.query(SOARPlaybookExecution).filter(
                    ...
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            if 'actions_executed' in soar_status:
                                execution.set_actions_executed(soar_status['actions_executed'])
