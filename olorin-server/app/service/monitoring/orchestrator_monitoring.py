...
>>>>>>> 001-modify-analyzer-method
    ...
    def __init__(self):
        ...
        self.metrics_history: Dict[MonitoringMetric, deque] = defaultdict(lambda: deque(maxlen=1000))
    ...
    async def raise_alert(
        ...
        try:
            ...
            alert_key = f"{alert_type}:{investigation_id}" if investigation_id else str(alert_type)
            if self._is_in_cooldown(alert_key):
                self.logger.debug(f"Alert {alert_key} in cooldown, skipping")
                return ""
            
            # Create alert
            alert_id = f"alert_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{alert_type.value}"
            alert = Alert(
                alert_id=alert_id,
            ...
            self.logger.warning(f"Alert raised: {title} (ID: {alert_id}, Severity: {severity.value})")
    ...
    def _initialize_default_thresholds(self) -> Dict[MonitoringMetric, MonitoringThreshold]:
    ...
    async def _collect_system_metrics(self) -> None:
        ...
        try:
            ...
            await self.record_metric(MonitoringMetric.MEMORY_UTILIZATION, memory_percent)
            
            # Investigation metrics
            active_investigations = len(await self.state_manager.get_active_investigations())
            await self.record_metric(MonitoringMetric.ACTIVE_INVESTIGATIONS, active_investigations)
            
            self.logger.debug(f"Collected system metrics: CPU={cpu_percent}%, Memory={memory_percent}%")
    ...
    async def _check_orchestrator_state(self) -> Dict[str, Any]:
        ...
        try:
            ...
            test_state = await self.state_manager.get_investigation_state("health_check_test")
    ...
    async def _evaluate_threshold(
        ...
        try:
            ...
            cutoff_time = datetime.now() - timedelta(minutes=threshold.evaluation_window_minutes)
            recent_data = [
                data for data in self.metrics_history[metric]
            ...
            if self._compare_value(aggregate_value, threshold.critical_threshold, threshold.comparison_operator):
                await self._raise_threshold_alert(
                    metric, threshold, aggregate_value, AlertSeverity.CRITICAL
                )
            elif self._compare_value(aggregate_value, threshold.warning_threshold, threshold.comparison_operator):
                await self._raise_threshold_alert(
    ...
    async def _raise_threshold_alert(
        ...
        try:
            ...
            recommended_actions = self._get_threshold_recommendations(metric, current_value)
    ...
    def _get_threshold_recommendations(self, metric: MonitoringMetric, value: float) -> List[str]:
    ...
    async def _cleanup_old_alerts(self) -> None:
        ...
        try:
            ...
            self.logger.debug(f"Cleaned up old alerts, {len(self.alert_history)} remaining")
