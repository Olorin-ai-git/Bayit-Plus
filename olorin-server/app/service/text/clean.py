...
def deduplicate_recommendations(recommendations: List[str], has_confirmed_fraud: bool = False) -> List[str]:
    """
...
def _apply_fraud_severity_guard(recommendation: str) -> str:
    ...
    if changes_made > 0:
        logger.debug(f"🚨 FRAUD SEVERITY GUARD: Upgraded {changes_made} monitoring phrases to escalation")
