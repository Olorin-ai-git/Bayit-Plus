...
def validate_required_secrets(config_instance) -> bool:
    ...
    if env in ["production", "prd", "staging", "stg", "e2e", "prf"]:
        ...
        if (
            hasattr(config_instance, "enabled_log_sources")
            and "splunk" in config_instance.enabled_log_sources
        ):
    ...
    if missing_secrets:
        ...
        if env in ["production", "prd", "staging", "stg"]:
            raise ValueError(
                error_msg + " - Cannot start application safely without these secrets."
            )
    ...
    if required_secrets:
        logger.info(
            f"All {len(required_secrets)} required secrets validated for environment '{env}'"
        )
