...
class DomainFindingsService:
    ...
    def persist_domain_findings(
        ...
        try:
            ...
            if not state:
                logger.warning(f"Investigation {investigation_id} not found for domain findings persistence")
                return False
            ...
            if state.progress_json:
                try:
                    ...
                    logger.warning(f"Failed to parse progress_json for {investigation_id}: {e}")
                    progress_data = {}
            ...
            try:
                ...
                payload = {
                    ...
                    "llm_confidence": findings.get("llm_analysis", {}).get("confidence") if findings.get("llm_analysis") else None,
                    "analysis_duration": findings.get("llm_analysis", {}).get("analysis_duration") if findings.get("llm_analysis") else None,
                    # Include summary of LLM response (first 500 chars)
                    "llm_response_preview": (
                        findings.get("llm_analysis", {}).get("llm_response", "")[:500]
                        if findings.get("llm_analysis") and findings.get("llm_analysis", {}).get("llm_response")
                        else None
                    )
                }
                ...
                create_audit_entry(
                    ...
                    changes_json=json_module.dumps(payload),  # Payload is the changes_json directly
                    from_version=from_version if from_version is not None else state.version - 1,
                ...
                logger.warning(f"Failed to create audit entry for domain findings: {str(audit_error)}")
    ...
    def ensure_domain_findings_in_results(
        ...
        logger.debug(f"ensure_domain_findings_in_results called for {investigation_id} - no action needed (domain_findings are in progress_json)")
        return True
