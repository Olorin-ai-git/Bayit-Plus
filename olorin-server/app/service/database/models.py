...
class Document(VectorBase, TimestampMixin):
    ...
    __table_args__ = (Index("idx_documents_collection", "collection_id", "is_active"),)
...
class DocumentChunk(VectorBase, TimestampMixin):
    ...
    __table_args__ = (Index("idx_chunks_document", "document_id", "chunk_index"),)
...
class RAGDataSource(VectorBase, TimestampMixin):
    ...
    def __repr__(self):
        return (
            f"<RAGDataSource(id={self.id}, name={self.name}, type={self.source_type})>"
        )
...
class RAGChatMessage(VectorBase, TimestampMixin):
    ...
    query_metadata = Column(
        JSON, nullable=True
    )  # execution_time, result_count, sources, confidence, citations
