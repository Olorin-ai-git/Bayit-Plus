...
from app.service.rag.investigation_indexing_background import (
    get_investigation_indexing_background_service,
)
...
class RAGSystemStartup:
    ...
    async def initialize_rag_system(self, skip_on_failure: bool = True) -> bool:
        ...
        if not use_rag_service:
            logger.info(
                "⏭️  RAG service is disabled (USE_RAG_SERVICE=false) - skipping initialization"
            )
            self.initialized = False
            self.database_available = False
            self.embedding_service_available = False
        ...
        try:
            ...
            if db_config.is_postgresql:
                ...
                if not self._check_database_config():
                    logger.warning(
                        "PostgreSQL configuration not found - RAG will use SQLite fallback"
                    )
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Table creation warning (may already exist): {table_error}"
                        )
    ...
    async def _initialize_embedding_service(self) -> bool:
        ...
        try:
            ...
            if not providers:
                logger.warning(
                    "No embedding providers available - RAG search will be limited"
                )
                return False

            self.embedding_service_available = True
            logger.info(
                f"✅ Embedding service initialized with providers: {', '.join(providers)}"
            )
            return True
    ...
    async def _initialize_knowledge_base(self) -> bool:
        ...
        try:
            ...
            if not (self.database_available and self.embedding_service_available):
                logger.warning(
                    "Knowledge base requires database and embedding service - skipping"
                )
                return False
    ...
    async def _perform_health_check(self):
        ...
        if health_status["overall"]:
            ...
            try:
                ...
                logger.info(
                    f"📈 RAG health check: {len(collections)} collections available"
                )
    ...
    async def _start_background_indexing(self) -> None:
        ...
        try:
            if not (self.database_available and self.embedding_service_available):
                logger.warning(
                    "Background indexing requires database and embedding service - skipping"
                )
                return
    ...
    def get_status(self) -> dict:
        ...
        return {
            ...
            "ready_for_queries": self.database_available
            and self.embedding_service_available,
        }
