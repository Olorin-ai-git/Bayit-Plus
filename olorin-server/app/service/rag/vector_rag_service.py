...
class VectorRAGService:
    ...
    async def ingest_document(
        ...
        try:
            ...
            logger.info(
                f"Ingesting document {document_id}: {len(chunks)} chunks to create"
            )
                    ...
                    try:
                        ...
                        if generate_embeddings:
                            ...
                            if embedding_result.success and embedding_result.embeddings:
                                ...
                                logger.warning(
                                    f"Failed to generate embedding for chunk {idx}: {embedding_result.error_message if hasattr(embedding_result, 'error_message') else 'Unknown error'}"
                                )
                        ...
                        logger.error(
                            f"Failed to create chunk {idx} for document {document_id}: {e}",
                            exc_info=True,
                        )
                        raise
            ...
            return DocumentIngestResult(
                success=True, document_id=document_id, chunk_count=len(chunks)
            )
        except Exception as e:
            logger.error(f"Document ingestion failed: {e}")
            return DocumentIngestResult(
                success=False, document_id="", chunk_count=0, error_message=str(e)
            )
    ...
    async def _search_postgresql(
        ...
        params = {"query_vec": str(query_embedding), "limit": limit}
        if collection_ids:
    ...
    async def regenerate_embeddings(self, embedding_type: str = "openai") -> int:
        ...
        async with self.db_config.session() as session:
            query = (
                select(DocumentChunk).where(DocumentChunk.is_active == True).limit(100)
            )
