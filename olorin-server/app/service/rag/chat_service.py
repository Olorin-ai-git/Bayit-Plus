...
from app.service.database.models import RAGChatMessage, RAGChatSession
...
class ChatService:
    ...
    async def create_session(
        ...
        try:
            ...
            async with self.db_config.session() as session:
                ...
                logger.info(
                    f"Created chat session: {chat_session.id} for user {user_id}"
                )
                ...
                meta_data_value = (
                    chat_session.meta_data if chat_session.meta_data is not None else {}
                )
                _ = chat_session.id
        ...
        try:
            ...
            async with self.db_config.session() as session:
                ...
                if chat_session:
                    ...
                    meta_data_value = (
                        chat_session.meta_data
                        if chat_session.meta_data is not None
                        else {}
                    )
    ...
    async def list_sessions(
        self, user_id: str, include_inactive: bool = False, limit: int = 50
    ) -> List[RAGChatSession]:
        ...
        try:
            ...
            async with self.db_config.session() as session:
                query = select(RAGChatSession).where(RAGChatSession.user_id == user_id)
                ...
                for sess in sessions:
                    # Store meta_data value before making transient (to avoid SQLAlchemy metadata conflict)
                    meta_data_value = (
                        sess.meta_data if sess.meta_data is not None else {}
                    )
                    _ = sess.id
                    _ = sess.user_id
                    _ = sess.title
