...
class DataSourceService:
    ...
    async def get_enabled_data_sources(self) -> List[RAGDataSource]:
        ...
        try:
            ...
            async with self.db_config.session() as session:
                query = select(RAGDataSource).where(
                    RAGDataSource.enabled == True
    ...
    async def test_connection(self, source_id: str) -> Dict[str, Any]:
        ...
        try:
            if data_source.source_type == "postgresql":
                ...
                return {"success": False, "error": f"Unknown source type: {data_source.source_type}"}
        except Exception as e:
    ...
    async def _test_investigation_results(self, data_source: RAGDataSource) -> Dict[str, Any]:
    ...
    async def _update_status(
        ...
        async with self.db_config.session() as session:
            query = update(RAGDataSource).where(
