...
class LoggingConfigManager:
    ...
    def load_configuration(self, 
                          ...
                          force_reload: bool = False) -> LoggingConfig:
        """
    ...
    def _convert_env_value(self, key: str, value: str) -> Any:
        ...
        # Boolean values
        if key in ['async_logging', 'lazy_initialization', 'suppress_noisy_loggers', 
    ...
    def _merge_yaml_config(self, config: LoggingConfig, yaml_config: Dict[str, Any]) -> LoggingConfig:
        ...
        for key, value in yaml_config.items():
            if hasattr(config, key):
                ...
                if key == 'log_format' and isinstance(value, str):
                    ...
                    try:
                        value = [LogOutput(item.lower()) if isinstance(item, str) else item 
                               for item in value]
                    except ValueError:
    ...
    def _merge_env_config(self, config: LoggingConfig, env_config: Dict[str, Any]) -> LoggingConfig:
    ...
    def _merge_args_config(self, config: LoggingConfig, args: argparse.Namespace) -> LoggingConfig:
                    ...
                    if isinstance(value, str):
                        ...
                        for output in value.split(','):
                            try:
                                ...
                                self._logger.warning(f"Invalid log output argument: {output.strip()}")
                        ...
                        try:
                            ...
                            self._logger.warning(f"Invalid log outputs argument: {value}")
                            continue
    ...
    def _validate_configuration(self, config: LoggingConfig) -> None:
        ...
        if config.log_level.upper() not in valid_levels:
            self._logger.warning(f"Invalid log level: {config.log_level}, defaulting to WARNING")
        ...
        if config.buffer_size <= 0:
            self._logger.warning(f"Invalid buffer size: {config.buffer_size}, defaulting to 1000")
        ...
        if config.max_file_size <= 0:
            self._logger.warning(f"Invalid max file size: {config.max_file_size}, defaulting to 10MB")
            config.max_file_size = 10 * 1024 * 1024
        
        # Validate backup count
        if config.backup_count < 0:
            self._logger.warning(f"Invalid backup count: {config.backup_count}, defaulting to 5")
    ...
    def get_enhanced_command_line_parser(self) -> argparse.ArgumentParser:
        ...
        """
        parser = argparse.ArgumentParser(description='Olorin Server with Unified Logging')
