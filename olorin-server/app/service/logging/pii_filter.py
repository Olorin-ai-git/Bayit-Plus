        ...
        if redaction_service and redaction_service.is_enabled():
            ...
            if hasattr(record, "msg") and record.msg:
                if isinstance(record.msg, str):
                    ...
                    record.msg = (
                        redaction_service.redact_context(record.msg)
                        if isinstance(record.msg, dict)
                        else [
                            (
                                redaction_service.redact_message(str(item))
                                if isinstance(item, str)
                                else (
                                    redaction_service.redact_context(item)
            ...
            if hasattr(record, "args") and record.args:
                if isinstance(record.args, tuple):
                    record.args = tuple(
                        (
                            redaction_service.redact_message(str(arg))
                            if isinstance(arg, str)
                            else (
                                redaction_service.redact_context(arg)
                ...
                for key, value in record.__dict__.items():
                    if key not in [
                        ...
                        if isinstance(value, str):
                            record.__dict__[key] = redaction_service.redact_message(
                                value
                            )
                        elif isinstance(value, dict):
                            record.__dict__[key] = redaction_service.redact_context(
                                value
                            )
