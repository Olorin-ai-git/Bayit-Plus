...
from .investigation_folder_manager import get_folder_manager, InvestigationMode
...
class JourneyTracker:
    ...
    def log_node_execution(
        ...
        if investigation_id not in self._journey_data:
            logger.warning(f"Journey tracking not started for investigation: {investigation_id}")
            return
        ...
        logger.debug(f"Logged node execution: {node_name} for investigation: {investigation_id}")
    ...
    def log_state_transition(
        ...
        self._journey_data[investigation_id]["state_transitions"].append(state_transition)
        self._write_journey_file(investigation_id)
        
        logger.debug(f"Logged state transition: {from_state} → {to_state} for investigation: {investigation_id}")
    ...
    def log_agent_coordination(
        ...
        logger.debug(f"Logged agent coordination: {coordinator_agent} → {target_agent} ({action}) for investigation: {investigation_id}")
    ...
    def complete_journey_tracking(
        ...
        logger.info(f"Completed journey tracking for investigation: {investigation_id} with status: {status}")
        return journey_data
    ...
    def _write_journey_file(self, investigation_id: str) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to write journey file for investigation {investigation_id}: {e}")
    ...
    def get_journey_data(self, investigation_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Failed to load journey data for investigation {investigation_id}: {e}")
    ...
    def get_journey_summary(self, investigation_id: str) -> Dict[str, Any]:
        ...
        if not journey_data:
            return {"error": f"Journey data not found for investigation: {investigation_id}"}
