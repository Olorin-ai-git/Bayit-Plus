...
class PIIRedactionConfig(BaseSettings):
    ...
    enable_email_redaction: bool = Field(default=True, env="PII_REDACT_EMAILS")
    enable_ssn_redaction: bool = Field(default=True, env="PII_REDACT_SSN")
    enable_credit_card_redaction: bool = Field(
        default=True, env="PII_REDACT_CREDIT_CARDS"
    )
    enable_phone_redaction: bool = Field(default=True, env="PII_REDACT_PHONES")
    enable_ip_redaction: bool = Field(default=False, env="PII_REDACT_IPS")
    enable_api_key_redaction: bool = Field(default=True, env="PII_REDACT_API_KEYS")

    email_replacement: str = Field(
        default="[EMAIL_REDACTED]", env="PII_EMAIL_REPLACEMENT"
    )
    ssn_replacement: str = Field(default="[SSN_REDACTED]", env="PII_SSN_REPLACEMENT")
    credit_card_replacement: str = Field(
        default="[CC_REDACTED]", env="PII_CC_REPLACEMENT"
    )
    phone_replacement: str = Field(
        default="[PHONE_REDACTED]", env="PII_PHONE_REPLACEMENT"
    )
    ip_replacement: str = Field(default="[IP_REDACTED]", env="PII_IP_REPLACEMENT")
    api_key_replacement: str = Field(
        default="[API_KEY_REDACTED]", env="PII_API_KEY_REPLACEMENT"
    )
