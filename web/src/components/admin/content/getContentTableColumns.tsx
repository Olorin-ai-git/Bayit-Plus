  ...
  icon: (
    <GlassTooltip content={tooltip}>
      ...
      >
        <Sparkles
          size={16}
          color={hasSubtitles ? colors.primary.DEFAULT : colors.textSecondary}
        />
      </Pressable>
    </GlassTooltip>
  ),
        ...
        const thumbnailType =
          content.content_type === "series"
            ...
            : content.content_type === "podcast"
              ? "podcast"
              : content.content_type === "radio"
                ? "radio"
                : content.content_type === "audiobook"
                  ? "audiobook"
                  : "movie";
        return <ThumbnailCell uri={value} type={thumbnailType} size="medium" />;
      },
      renderChild: (value: any) => (
        <ThumbnailCell uri={value} type="episode" size="small" />
      ),
    },
    {
      key: "title",
      ...
      render: (value: any, row: any) => {
        ...
        const hasNoEpisodes =
          ...
          (!content.episode_count || content.episode_count === 0);
      ...
      render: (_: any, row: any) => {
        ...
        const getViewUrl = () => {
          switch (content.content_type) {
            ...
            default:
              ...
              return isSeriesContent(content)
        ...
        const actions = [
          createStarAction(
            ...
            content.is_featured,
            content.is_featured
              ? t("admin.content.unfeature", "Remove from Featured")
              : t("admin.content.feature", "Add to Featured"),
          ),
          createViewAction(
            () => {
        ...
        if (onHebrewAI) {
          const hasEnglish =
            content.available_subtitles?.includes("en") || false;
          const hasAnySubtitles = hasHebrew || hasEnglish;
          actions.splice(
            1,
            0,
            createSubtitleAIAction(
        ...
        return <ActionsCell actions={actions} align="right" showTooltips />;
      },
