...
export default function VideoPlayerControlsOverlay({
  ...
  const controlBarHeight = responsive.isMobile
    ? MOBILE_CONTROL_BAR_HEIGHT
    : DESKTOP_CONTROL_BAR_HEIGHT;
  ...
  return (
    <View style={styles.controlsOverlay} pointerEvents="auto">
            ...
            <View style={styles.subtitleFlagsRow}>
              {availableSubtitles.slice(0, 6).map((track) => {
                ...
                return langInfo?.flag ? (
                  ...
                  <Icon key={track.id} name="globe" size="md" color="#FFFFFF" />
                );
              })}
              {availableSubtitles.length > 6 && (
                <Text style={styles.subtitleFlagMore}>
                  +{availableSubtitles.length - 6}
                </Text>
              )}
            </View>
      ...
      {/* Bottom Controls */}
      <GlassView
        style={[styles.bottomControls, { minHeight: controlBarHeight }]}
        intensity="high"
        noBorder
      >
        {!isLive && (
...
const styles = StyleSheet.create({
  controlsOverlay: {
    ...
    backgroundImage:
      "linear-gradient(to top, rgba(17, 17, 34, 0.9), transparent 40%, transparent 60%, rgba(17, 17, 34, 0.4))" as any,
    zIndex: 10,
