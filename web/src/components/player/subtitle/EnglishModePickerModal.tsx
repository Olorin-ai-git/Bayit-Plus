...
export default function EnglishModePickerModal({
  ...
  const [generatingMode, setGeneratingMode] = useState<EnglishMode | null>(
    null,
  );
  ...
  const pollJobStatus = useCallback(
    async (jobId: string, mode: EnglishMode) => {
      try {
        const status = (await subtitlesService.getJobStatus(
          jobId,
        )) as JobStatus;
        logger.debug(`Job status: ${status.status}`, "EnglishModePickerModal", {
          jobId,
        ...
        if (status.status === "completed" || status.status === "failed") {
          ...
          if (status.status === "completed") {
            ...
            logger.error(
              ...
              "EnglishModePickerModal",
              { contentId, error: status.error_message },
            );
          }
        ...
        logger.error("Failed to poll job status", "EnglishModePickerModal", {
          jobId,
          error,
        });
  ...
  const handleCancelJob = useCallback(async () => {
    ...
    try {
      ...
      setJobProgress(0);
      setCurrentJobId(null);
      logger.info("Job cancelled", "EnglishModePickerModal", {
        jobId: currentJobId,
      });
    } catch (error) {
      logger.error("Failed to cancel job", "EnglishModePickerModal", {
        jobId: currentJobId,
        error,
      });
  ...
  const handleRestartJob = useCallback(
    async (mode: EnglishMode) => {
      ...
      try {
        ...
        let result;
        if (mode === "heblish") {
          result = await subtitlesService.generateHeblish(
            contentId,
            "en",
            true,
          );
        } else if (mode === "grammarFlip") {
          result = await subtitlesService.generateGrammarFlip(
            contentId,
            "en",
            true,
          );
        } else if (mode === "slangSynthesis") {
          result = await subtitlesService.generateSlangSynthesis(
            contentId,
            "en",
            true,
          );
  ...
  const handleGenerate = async (e: React.MouseEvent, mode: EnglishMode) => {
    ...
    logger.info(`Generate ${mode} clicked`, "EnglishModePickerModal", {
      contentId,
      isAdmin,
      generatingMode,
      mode,
    ...
    try {
      logger.info(`Starting ${mode} generation`, "EnglishModePickerModal", {
        contentId,
        mode,
      });
      ...
      logger.info(`${mode} job started`, "EnglishModePickerModal", {
        contentId,
        result,
      });
      ...
      logger.error(
        `Failed to start ${mode} generation`,
        "EnglishModePickerModal",
        { contentId, error: errorMessage },
      );
  ...
  const memoizedOptions = useMemo(() => ENGLISH_MODE_OPTIONS, []);
      ...
      >
          ...
          <div className="mb-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-3">
            <div className="flex items-start gap-2">
              ...
              <p className="text-sm text-yellow-200">
                {t(
                  "subtitles.noEnglishSubtitles",
                  "No English subtitles available for this content.",
                )}
              </p>
            </div>
          </div>
          ...
          <div className="mb-4 bg-indigo-500/10 border border-indigo-500/30 rounded-lg p-3">
            <div className="flex items-start gap-2">
              ...
              <p className="text-sm text-indigo-200">
                {t(
                  "subtitles.englishMode.firstTimeHint",
                  'Heblish mode injects Hebrew words into English subtitles for immersive language learning. Example: "Hello friends!" becomes "Shalom chaverim!"',
                )}
              </p>
              <GlassButton
        ...
        <div className="space-y-2 sm:space-y-3">
          {memoizedOptions.map((option) => {
            ...
            const isAIMode =
              option.mode === "heblish" ||
              option.mode === "grammarFlip" ||
              option.mode === "slangSynthesis";
            const canShowGenerateButton =
              !isAvailable && isAIMode && isAdmin && contentId;
                    ...
                    isAIMode && (
                      <div className="sm:w-32 sm:flex-shrink-0">
                        <p className="text-xs text-amber-500/90 italic">
                          {t(
                            `subtitles.englishMode.${option.mode}.unavailableReason`,
                            `${option.mode} processing not available for this content`,
                          )}
                        </p>
                      </div>
                    )}
                      ...
                      {isAdmin && contentId ? (
                        <div className="flex items-center gap-2 flex-wrap">
                          {generatingMode === option.mode ? (
                            ...
                            <GlassButton
                              ...
                              aria-label={t("subtitles.generateMode", {
                                mode: option.mode,
                              })}
                            >
                        ...
                        >
                          <span className="text-xs text-red-400 font-semibold">
                            {t(
                              "subtitles.englishMode.unavailable",
                              "Unavailable",
                            )}
                          </span>
                        </div>
