...
const CulturalReferencePopup: React.FC<CulturalReferencePopupProps> = ({
  ...
  const [explanation, setExplanation] = useState<ReferenceExplanation | null>(null);
        ...
        {!loading && !error && explanation && (
          <>
            ...
            <View style={styles.content}>
              <Text style={styles.hebrewName}>{explanation.canonical_name_hebrew}</Text>
              <Text style={styles.englishName}>{explanation.canonical_name_english}</Text>

              <Text style={styles.explanation}>{explanation.explanation}</Text>

              {explanation.related_references.length > 0 && (
                <View style={styles.relatedSection}>
                  <Text style={styles.relatedTitle}>Related References</Text>
                  {explanation.related_references.map((related, index) => (
                    <View key={`${related.reference_id}-${index}`} style={styles.relatedItem}>
                      <Text style={styles.relatedName}>{related.name}</Text>
                      <Text style={styles.relatedRelationship}>{related.relationship}</Text>
                    </View>
                  ))}
                </View>
              )}
