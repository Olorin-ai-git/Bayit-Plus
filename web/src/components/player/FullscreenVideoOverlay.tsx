...
import {
  AvatarDialoguePanel,
  CharacterSelectBar,
} from "@/components/vod-interactions";
...
export default function FullscreenVideoOverlay() {
  ...
  const { isOpen, content, startTime, closePlayer } =
  ...
  useEffect(() => {
    ...
    logger.info(
      "Fetching watch history for overlay",
      "FullscreenVideoOverlay",
      {
        contentId: content.id,
      },
    ...
    historyService
      ...
      .then((response) => {
        ...
        logger.info(
          "Watch history received in overlay",
          "FullscreenVideoOverlay",
          {
        ...
        if (!Array.isArray(items)) {
          logger.info(
            "No watch history items found in overlay",
            "FullscreenVideoOverlay",
          );
          return;
        }
        ...
        if (saved?.position > 0) {
          ...
          logger.info(
            "Set saved watch position in overlay",
            "FullscreenVideoOverlay",
            {
              contentId: content.id,
          ...
          logger.info(
            "No saved position found in overlay",
            "FullscreenVideoOverlay",
            {
              contentId: content.id,
            },
      ...
      .catch((err) => {
        logger.error(
          "Failed to fetch watch history in overlay",
          "FullscreenVideoOverlay",
          {
  ...
  const handleProgress = useCallback(
    async (currentTime: number, duration: number) => {
      ...
      try {
        ...
        logger.error(
          "Failed to save watch progress in overlay",
          "FullscreenVideoOverlay",
          {
  ...
  const handleCharacterSelect = useCallback(
    async (character: ContentCharacter) => {
  ...
  const handleSendDialogueMessage = useCallback(
  ...
  const handleEnded = useCallback(() => {
    ...
    if (!advancePlaylistOrClose()) {
      ...
      if (content.type === "series" && content.seriesId) {
        ...
        logger.debug(
          "Content ended, no playlist active",
          "FullscreenVideoOverlay",
        );
      }
    }
  ...
  const handleRestartComplete = useCallback(() => {
    setSavedPosition(null);
    logger.info(
      "Watch position cleared after restart in overlay",
      "FullscreenVideoOverlay",
      {
        contentId: content?.id,
      },
  ...
  const overlay = (
    ...
    >
      ...
      {/* Audio Player for podcast/radio/audiobook content */}
      ...
      {streamUrl &&
        ...
        (isYouTubeUrl(streamUrl) ? (
          ...
          (() => {
            ...
            logger.info(
              "Rendering VideoPlayer with initial subtitle",
              "FullscreenVideoOverlay",
              {
                contentId: content.id,
          ...
          <View style={styles.nextEpisodeCard}>
            ...
            <View style={styles.nextEpisodeInfo}>
              ...
              <Text style={styles.nextEpisodeTitle} numberOfLines={2}>
                {t("content.episode")} {nextEpisode.episode_number}:{" "}
                {nextEpisode.title}
              </Text>
            ...
            <View style={styles.nextEpisodeActions}>
              <Pressable
                ...
                style={styles.nextEpisodePlayBtn}
              >
                <Text style={styles.nextEpisodePlayText}>
                  {t("player.playNow", "Play Now")}
                </Text>
              </Pressable>
              <Pressable
                onPress={cancelAutoPlay}
                style={styles.nextEpisodeCancelBtn}
              >
                <Text style={styles.nextEpisodeCancelText}>
                  {t("common.cancel")}
                </Text>
              </Pressable>
            </View>
        ...
        content.type !== "live" && (
          ...
          >
            <MessageCircle
              size={20}
              color={
                vodInteraction.isFreeDialogueActive ? "#c084fc" : colors.text
              }
            />
            <Text
              style={[
                ...
                vodInteraction.isFreeDialogueActive &&
                  styles.talkButtonTextActive,
              ]}
            >
              {t("player.dialogue.talkToCharacter")}
            </Text>
          </Pressable>
        )}
