...
export default function DubbingControls({
  ...
  useEffect(() => {
    if (availableVoices.length === 0) {
      LiveDubbingService.getVoices()
        .then((voices) => {
          logger.debug("Loaded ElevenLabs voices", "DubbingControls", {
            count: voices.length,
          });
          setAvailableVoices(
            voices.map((v) => ({
  ...
  const handlePress = () => {
    ...
    if (!isEnabled && onDisableSubtitles) {
      logger.debug(
        "Disabling subtitles for mutual exclusivity",
        "DubbingControls",
        {},
      );
      onDisableSubtitles();
          ...
          <GlassLiveControlButton
            icon={
              <Radio
                size={18}
                color={
                  isEnabled ? colors.primary.DEFAULT : colors.textSecondary
                }
              />
            }
            ...
            tooltip={
              quotaExceeded
                ? t(
                    "quota.dubbingExceeded",
                    "Dubbing quota exceeded. Please try again later.",
                  )
                : isEnabled
                  ? t("dubbing.active", "Live Dubbing Active")
                  : isPremium
                    ? t("dubbing.clickToEnable", "Click to enable live dubbing")
                    : t("dubbing.premiumRequired", "Plus subscription required")
            }
            splitIcon={
              <Text style={styles.splitFlag}>
                {LANG_FLAGS[inputLang] || inputLang.toUpperCase()}
              </Text>
            }
            onSplitPress={() => setShowLangPicker(true)}
            splitAccessibilityLabel={t(
              "dubbing.selectInputLanguage",
              "Select input language",
            )}
            splitTooltip={t(
              "dubbing.inputLanguageTooltip",
