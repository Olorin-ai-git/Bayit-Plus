...
function isDismissed(channelId: string): boolean {
  try {
    return localStorage.getItem(`catchup-dismissed-${channelId}`) === "true";
  } catch {
    return false;
  }
}
...
function setDismissed(channelId: string): void {
  try {
    localStorage.setItem(`catchup-dismissed-${channelId}`, "true");
  } catch {
    /* storage failure */
  }
}

export function useCatchUp({ channelId }: UseCatchUpOptions) {
  ...
  const checkAvailability = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/v1/live/${channelId}/catchup/available`,
        {
  ...
  useEffect(() => {
    if (channelId) checkAvailability();
  }, [channelId, checkAvailability]);

  const fetchSummary = useCallback(
    async (windowMinutes?: number, targetLanguage?: string) => {
      if (!channelId) {
      ...
      try {
        ...
        if (response.status === 402) {
          ...
          setState((prev) => ({
            ...
            hasCredits: false,
            balance: data.remaining_credits || 0,
          }));
          return;
        }
        ...
        setState((prev) => ({
          ...
          showSummary: true,
          showAutoPrompt: false,
          hasCredits: data.remainingCredits > 0,
