...
function cleanupExpiredStreams(): void {
  ...
  if (removedCount > 0) {
    logger.info(
      `Cleaned up ${removedCount} expired failed streams`,
...
function isStreamBlocked(url: string): boolean {
  ...
  if (isStreamFailureExpired(timestamp)) {
    failedStreamUrls.delete(url);
    logger.info(
      "Failed stream TTL expired, allowing retry",
      "isStreamBlocked",
      { url },
    );
    return false;
...
export function killStaleHLS(): void {
  logger.info(
    `Killing ${activeHlsInstances.size} stale HLS instances`,
    "killStaleHLS",
  );
...
export function clearFailedStreams(): void {
  ...
  failedStreamUrls.clear();
  logger.info(
    `Cleared ${count} failed streams from blocklist`,
    "clearFailedStreams",
  );
}
...
export function useHLSPlayer({
    ...
    (data: { type: string; details: string; fatal: boolean }) => {
      ...
      if (data.type === Hls.ErrorTypes.NETWORK_ERROR) {
        ...
        if (networkErrorCountRef.current >= maxNetworkErrors && isLive) {
          logger.error(
            "Multiple network errors detected, clearing live session",
            "useHLSPlayer",
          );
          clearPersistedSession();
  ...
  useEffect(() => {
    ...
    if (isStreamBlocked(streamUrl)) {
      ...
      const timeUntilExpiry = Math.ceil(
        (FAILED_STREAM_TTL - (Date.now() - timestamp)) / 1000,
      );

      logger.warn(
        "Stream URL previously failed, not retrying",
        "useHLSPlayer",
        {
          streamUrl,
    ...
    const preferNativeHLS =
      !isLive && !!video.canPlayType("application/vnd.apple.mpegurl");
    if (!preferNativeHLS && Hls.isSupported() && streamUrl.includes(".m3u8")) {
      const hls = new Hls({
        ...
        // VOD: start at the lowest quality and ramp up as bandwidth is measured.
        // This mirrors iOS startsOnFirstEligibleVariant and prevents multi-minute
        // buffering when the ABR algorithm overestimates bandwidth on cold start.
        // Live: let HLS.js auto-select (-1) since latency matters more than startup.
        startLevel: isLive ? -1 : 0,
        // Buffer settings: VOD gets a deeper buffer (30-60s) for resilience against
        // network dips; live keeps a tight window to stay near the edge.
        maxBufferLength: isLive ? Math.max(10, targetLatencySeconds + 5) : 30,
        maxMaxBufferLength: isLive
          ? Math.max(30, targetLatencySeconds + 20)
          : 60,
      ...
      hls.on(Hls.Events.MANIFEST_PARSED, (_, data) => {
        ...
        logger.info("HLS manifest parsed", "useHLSPlayer", {
          ...
          videoAudioTracksCount:
            (video as any).audioTracks?.length ?? "not supported",
        });
        if (autoPlay) {
          ...
          video
            ...
            .then(() => {
              ...
              if (video.muted) {
                logger.warn(
                  "Video unexpectedly muted after autoplay success",
                  "useHLSPlayer",
                );
              }
            })
            .catch((err) => {
              logger.info(
                "Autoplay blocked by browser, trying muted",
                "useHLSPlayer",
                {
                  error: err.message,
                  errorName: err.name,
      ...
      hls.on(Hls.Events.ERROR, (event, data) => {
        ...
        if (data.type === Hls.ErrorTypes.NETWORK_ERROR) {
          ...
          if (isLive && networkErrorCountRef.current >= maxNetworkErrors) {
            logger.error(
              "Stale stream detected - stopping HLS player",
              "useHLSPlayer",
              { streamUrl },
            );
            hls.stopLoad();
            ...
            logger.info(
              "Added stream to failed list, will not retry",
              "useHLSPlayer",
              {
                streamUrl,
      ...
      video.addEventListener("loadedmetadata", () => {
        ...
        if (autoPlay) {
          ...
          video
            ...
            .catch((err) => {
              logger.info(
                "Autoplay blocked (Safari), trying muted",
                "useHLSPlayer",
                {
                  error: err.message,
                  errorName: err.name,
              ...
              video
                ...
                .then(() => {
                  logger.info(
                    "Muted autoplay succeeded (Safari)",
                    "useHLSPlayer",
                  );
                })
                .catch((e) => {
                  logger.warn(
                    "Autoplay blocked even when muted (Safari)",
                    "useHLSPlayer",
                    {
                      error: e.message,
                    },
      ...
      video.addEventListener("loadeddata", () => {
        ...
        if (autoPlay) {
          ...
          video
            ...
            .catch((err) => {
              logger.info(
                "Autoplay blocked (fallback), trying muted",
                "useHLSPlayer",
                {
                  error: err.message,
                  errorName: err.name,
              ...
              video
                ...
                .then(() => {
                  logger.info(
                    "Muted autoplay succeeded (fallback)",
                    "useHLSPlayer",
                  );
                })
                .catch((e) => {
                  logger.warn(
                    "Autoplay blocked even when muted (fallback)",
                    "useHLSPlayer",
                    {
                      error: e.message,
                    },
  ...
  useEffect(() => {
    ...
    const handleDubbingSwitch = (
      event: CustomEvent<{ url: string; delay_ms: number }>,
    ) => {
      const video = videoRef.current;
      ...
      logger.info(
        `Reconfiguring HLS for dubbing: ${delaySeconds}s latency`,
        "useHLSPlayer",
      );
      ...
      hls.on(Hls.Events.MANIFEST_PARSED, () => {
        ...
        logger.info(
          "HLS reconfigured for dubbing, manifest parsed",
          "useHLSPlayer",
        );
        if (wasPlaying) {
          video.play().catch((err) => {
            logger.warn(
              "Failed to resume after dubbing switch",
              "useHLSPlayer",
              err,
            );
          });
    ...
    const handleDubbingRestore = (event: CustomEvent<{ url: string }>) => {
      ...
      hls.on(Hls.Events.MANIFEST_PARSED, () => {
        ...
        if (wasPlaying) {
          video.play().catch((err) => {
            logger.warn(
              "Failed to resume after dubbing restore",
              "useHLSPlayer",
              err,
            );
          });
    ...
    window.addEventListener(
      "dubbing-stream-switch",
      handleDubbingSwitch as EventListener,
    );
    window.addEventListener(
      "dubbing-stream-restore",
      handleDubbingRestore as EventListener,
    );
  ...
  const destroyHLS = useCallback(() => {
    if (hlsRef.current) {
      logger.info(
        "Destroying HLS.js instance (external request)",
        "useHLSPlayer",
      );
      activeHlsInstances.delete(hlsRef.current);
      hlsRef.current.stopLoad();
