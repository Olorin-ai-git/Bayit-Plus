...
import { Play, SkipForward, Gauge, Zap, FastForward } from 'lucide-react';
...
export function PlaybackSection() {
  ...
  const updatePref = async <K extends keyof PlaybackPrefs>(key: K, value: PlaybackPrefs[K]) => {
    const prev = prefs;
    setPrefs((p) => ({ ...p, [key]: value }));
    try {
      await profilesService.updatePlaybackPreferences({ ...prefs, [key]: value });
    } catch (error) {
  ...
  return (
    <SettingSection title={t('settings.playback', 'Playback')} isRTL={isRTL}>
      ...
      <SettingRow
        ...
        description={t('settings.continueWatchingDesc', 'Resume from where you left off')}
      ...
      <SettingRow
        ...
        description={t('settings.hardwareAccelerationDesc', 'Use GPU for video decoding')}
