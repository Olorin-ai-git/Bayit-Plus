  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Pressable onPress={handleClose} style={styles.closeButton} accessibilityRole="button" accessibilityLabel={t('common.close')}>
      ...
      {!activeUrl && (
        <View style={styles.inputSection}>
          ...
          <View style={styles.inputRow}>
            <TextInput style={[styles.urlInput, error && styles.urlInputError, isValid && styles.urlInputValid]}
              value={urlInput} onChangeText={handleUrlChange} placeholder="https://archive.org/embed/..."
            ...
            <Pressable onPress={handleLoadUrl} style={[styles.loadButton, !isValid && styles.loadButtonDisabled]}
              disabled={!isValid} accessibilityRole="button" accessibilityLabel={t('common.load', 'Load')}>
              <Text style={styles.loadButtonText}>{t('common.load', 'Load')}</Text>
            </Pressable>
          </View>
          {error && <View style={styles.statusRow}><AlertCircle size={16} color={colors.error.DEFAULT} /><Text style={styles.errorText}>{error}</Text></View>}
          {isValid && urlInput && <View style={styles.statusRow}><CheckCircle size={16} color={colors.success.DEFAULT} /><Text style={styles.validText}>{t('widgets.urlValid', 'URL is valid')}</Text></View>}
      ...
      {activeUrl && (
        <View style={styles.iframeContainer}>
          <iframe src={activeUrl} style={{ width: '100%', height: '100%', border: 'none' }} title="Custom content"
            allow="autoplay; fullscreen; picture-in-picture" sandbox="allow-scripts allow-same-origin allow-presentation" allowFullScreen />
          <Pressable onPress={() => setActiveUrl(null)} style={styles.changeUrlButton} accessibilityRole="button"
