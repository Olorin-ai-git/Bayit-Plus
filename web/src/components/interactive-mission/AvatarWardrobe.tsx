...
export function AvatarWardrobe({
  ...
  const fetchCatalog = async () => {
    setLoading(true);
    try {
      const data = (await api.get("/avatar-outfits/catalog", {
        params: { avatar_id: avatarId },
      })) as { outfits: OutfitItem[] };
      setOutfits(data.outfits || []);
    } catch (err: any) {
      setError(
        err?.detail || err?.message || t("avatarWardrobe.errors.loadFailed"),
      );
    ...
    async (outfit: OutfitItem) => {
      ...
      try {
        ...
        setError(
          err?.detail ||
            err?.message ||
            t("avatarWardrobe.errors.purchaseFailed"),
        );
  ...
  const handleEquip = useCallback(
    async (outfitId: string | null) => {
      try {
        await api.post(`/avatar-outfits/avatars/${avatarId}/equip`, {
          outfit_id: outfitId,
        });
        ...
        setError(
          err?.detail || err?.message || t("avatarWardrobe.errors.equipFailed"),
        );
      }
    },
    [avatarId],
    ...
    ({ item }: { item: OutfitItem }) => {
      ...
      return (
        <Pressable
          style={[
            styles.outfitCard,
            selectedOutfit?.id === item.id && styles.outfitCardSelected,
          ]}
        ...
        >
          ...
          <View style={[styles.rarityBadge, { backgroundColor: rarityColor }]}>
            <Text style={styles.rarityText}>
              {t(`avatarWardrobe.rarity.${item.rarity}`)}
            </Text>
          </View>
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        keyExtractor={(item: OutfitItem) => item.id}
        numColumns={3}
        contentContainerStyle={styles.grid}
      />

      {selectedOutfit &&
        !selectedOutfit.owned &&
        selectedOutfit.is_purchasable && (
          <View style={styles.purchaseBar}>
            <Text style={styles.purchaseText}>
              {isHebrew ? selectedOutfit.name_he : selectedOutfit.name} -{" "}
              {selectedOutfit.shekel_price}
            </Text>
            <GlassButton
              title={
                purchasing
                  ? t("avatarWardrobe.purchasing")
                  : t("avatarWardrobe.purchase")
              }
              onPress={() => handlePurchase(selectedOutfit)}
              variant="primary"
              size="md"
              disabled={
                purchasing || shekelBalance < selectedOutfit.shekel_price
              }
            />
          </View>
        )}
      ...
      {selectedOutfit && selectedOutfit.owned && (
        <View style={styles.purchaseBar}>
          <GlassButton
            title={t("avatarWardrobe.equip")}
            onPress={() => handleEquip(selectedOutfit.outfit_id)}
            variant="primary"
            size="md"
          />
          <GlassButton
            title={t("avatarWardrobe.unequip")}
