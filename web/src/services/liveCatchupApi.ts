...
/**
 * Generate AI catch-up summary for live channels.
 */
export async function generateCatchupSummary(
  ...
  logger.debug("Generating catchup summary", LOG_CONTEXT, {
    channelId,
    windowMinutes,
    targetLanguage,
  });
 ...
 */
export async function checkCatchupAvailability(
  channelId: string,
): Promise<CatchUpAvailabilityResponse> {
...
export async function getTranscriptTimeline(
  ...
  logger.debug("Fetching transcript timeline", LOG_CONTEXT, {
    channelId,
    windowMinutes,
  });
