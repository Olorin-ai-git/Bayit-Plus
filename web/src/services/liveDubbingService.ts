...
const AUDIO_CONFIG = {
  ...
  defaultSyncDelayMs: parseInt(
    import.meta.env.VITE_DUBBING_SYNC_DELAY_MS || "600",
    10,
  ),
  // Default original audio volume (0 = muted)
  defaultOriginalVolume: parseFloat(
    import.meta.env.VITE_DUBBING_ORIGINAL_VOLUME || "0",
  ),
  // Default dubbed audio volume (1 = full volume)
  defaultDubbedVolume: parseFloat(
    import.meta.env.VITE_DUBBING_DUBBED_VOLUME || "1",
  ),
  // Volume transition time (seconds) for smooth fades
  volumeTransitionTime: parseFloat(
    import.meta.env.VITE_DUBBING_VOLUME_TRANSITION || "0.1",
  ),
  // Log interval for chunk statistics
  chunkLogInterval: parseInt(
    import.meta.env.VITE_DUBBING_CHUNK_LOG_INTERVAL || "100",
    10,
  ),
} as const;
...
class LiveDubbingService {
  ...
  private continuousPlaybackController: ContinuousPlaybackController | null =
    null;
  ...
  private calculateAdaptiveSyncDelay(channelId: string): number {
    const settings = useDubbingSettingsStore
      .getState()
    ...
    // Get latency history
    const avgLatency = useLatencyHistoryStore
      .getState()
      .getAverageLatency(channelId);
  ...
  private processLatencyReport(report: LatencyReport, channelId: string): void {
    ...
    if (settings.autoAdaptiveSync) {
      ...
      if (Math.abs(newSyncDelay - this.syncDelayMs) > 50) {
        ...
        logger.info(
          `Adaptive sync updated: ${newSyncDelay}ms`,
          "liveDubbingService",
        );
      }
    }
  ...
  async connect(
    ...
    try {
      logger.info(
        `Requesting synced stream for channel ${channelId}`,
        "liveDubbingService",
      );
      ...
      if (
        this.syncedStreamInfo.mode === "server-side" &&
        this.syncedStreamInfo.video_url
      ) {
        ...
        if ((window as any).Hls && (window as any).Hls.isSupported()) {
          ...
          const event = new CustomEvent("dubbing-stream-switch", {
            detail: {
              url: delayedUrl,
              delay_ms: this.syncedStreamInfo.video_delay_ms,
            },
          });
        ...
        logger.info(
          "Client-side video buffering initialized",
          "liveDubbingService",
        );
      }
    } catch (error) {
    ...
    try {
      ...
      let wsUrl = buildWsUrl(
        `/api/v1/ws/live/${channelId}/dubbing?target_lang=${targetLang}&platform=${platform}&continuous_flow=${enableContinuousFlow}`,
      );
      if (voiceId) {
      ...
      this.ws.onopen = async () => {
        ...
        const pipelineReady = await this.setupAudioPipeline(
          videoElement,
          onError,
        );
        if (!pipelineReady) {
      ...
      this.ws.onmessage = async (event) => {
        try {
          ...
          if (msg.type === "connected") {
            this.syncDelayMs =
              msg.sync_delay_ms || AUDIO_CONFIG.defaultSyncDelayMs;
            logger.info(
            ...
            logger.debug(
              `Dubbed audio #${audioMsg?.sequence}: "${audioMsg?.translated_text?.substring(0, 30)}..."`,
              "liveDubbingService",
            );
            ...
            if (!this.firstDubbedAudioReceived && this.videoBufferManager) {
              ...
              logger.debug(
                "Completed latency measurement (first dubbed audio)",
                "liveDubbingService",
              );
            }
            ...
            if (
              this.enableContinuousFlow &&
              this.continuousPlaybackController
            ) {
            ...
            if (this.continuousPlaybackController) {
              this.continuousPlaybackController.onBufferStatusReceived(
                msg as BufferStatus,
              );
            }
            ...
            logger.error(
              `Server error: ${msg.error || msg.message}`,
              "liveDubbingService",
            );
  ...
  private async setupAudioPipeline(
    ...
    try {
      ...
      logger.debug(
        `Capture AudioContext created at ${AUDIO_CONFIG.sampleRate}Hz (for STT)`,
        "liveDubbingService",
      );
      ...
      logger.debug(
        `Playback AudioContext created at ${this.playbackContext.sampleRate}Hz (for TTS)`,
        "liveDubbingService",
      );
      ...
      logger.debug(
        "Gain nodes created and connected to playback context",
        "liveDubbingService",
      );
          ...
          onAudioData: (audioBuffer: ArrayBuffer) => {
            ...
            if (this.ws && this.ws.readyState === WebSocket.OPEN) {
              ...
              if (!this.firstAudioChunkSent && this.videoBufferManager) {
                ...
                logger.debug(
                  "Started latency measurement (first audio chunk)",
                  "liveDubbingService",
                );
              }
              ...
              logger.warn(
                "WebSocket not ready, skipping audio chunk",
                "liveDubbingService",
              );
            }
          },
      ...
      if (!this.audioCapture.isActive()) {
        logger.warn(
          "Audio capture did not start - pipeline incomplete",
          "liveDubbingService",
        );
        return false;
      }
      ...
      logger.debug(
        "Audio pipeline ready (using SharedAudioCapture)",
        "liveDubbingService",
      );
      return true;
    } catch (error) {
  ...
  private waitForVideoReady(
    ...
    return new Promise((resolve, reject) => {
      ...
      if (videoElement.readyState >= 2) {
        logger.debug(
          "Video already ready for audio capture",
          "liveDubbingService",
        );
        resolve();
        return;
      ...
      logger.debug(
        "Waiting for video to be ready for audio capture",
        "liveDubbingService",
      );
      ...
      const timeout = setTimeout(() => {
        ...
        logger.warn(
          "Video ready timeout - proceeding anyway",
          "liveDubbingService",
        );
  ...
  private async playDubbedAudio(base64Audio: string): Promise<void> {
    ...
    try {
      ...
      logger.debug(
        `Playing dubbed audio (${audioBuffer.duration.toFixed(2)}s at ${this.playbackContext.sampleRate}Hz)`,
        "liveDubbingService",
      );
    } catch (error) {
  ...
  setOriginalVolume(volume: number): void {
    ...
    logger.debug(
      `Original volume: ${(this.originalVolume * 100).toFixed(0)}%`,
      "liveDubbingService",
    );
  }
  ...
  setDubbedVolume(volume: number): void {
    ...
    logger.debug(
      `Dubbed volume: ${(this.dubbedVolume * 100).toFixed(0)}%`,
      "liveDubbingService",
    );
  }
  ...
  setAutoAdaptiveSync(enabled: boolean): void {
    if (this.channelId) {
      ...
      if (enabled) {
        ...
        logger.info(
          `Auto-adaptive sync enabled: ${this.syncDelayMs}ms`,
          "liveDubbingService",
        );
      } else {
    ...
    if (this.originalVideoSrc && this.videoElementRef) {
      ...
      if ((window as any).Hls && (window as any).Hls.isSupported()) {
        ...
        this.videoElementRef.play().catch((err) => {
          logger.warn(
            "Failed to play restored stream",
            "liveDubbingService",
            err,
          );
        });
  ...
  getAppliedVideoDelay(): number {
    return (
      this.videoBufferManager?.getAppliedDelay() ??
      this.syncedStreamInfo?.video_delay_ms ??
      0
    );
  }
  ...
  static async checkAvailability(
    ...
    try {
      ...
      logger.error(
        "Error checking dubbing availability",
        "liveDubbingService",
        error,
      );
