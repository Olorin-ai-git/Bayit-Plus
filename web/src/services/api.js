...
const RETRYABLE_STATUS_CODES = (
  import.meta.env.VITE_API_RETRY_STATUS_CODES || "408,429,500,502,503,504"
)
  .split(",")
  .map(Number);
...
const apiContentService = {
  ...
  getSeasonEpisodes: (seriesId, seasonNum) =>
  ...
  getIsraelisInCity: (city, state, params = {}) =>
    api.get("/content/israelis-in-city", {
      params: { city, state, ...params },
    }),
  getIsraeliBusinessesInCity: (city, state, params = {}) =>
    api.get("/content/israeli-businesses-in-city", {
      params: { city, state, ...params },
    }),
};
...
const apiLiveService = {
  ...
  getEPG: (channelId, date) =>
    api.get(`/live/${channelId}/epg`, { params: { date } }),
...
const apiPodcastService = {
  ...
  getEpisodes: (showId, params) =>
    api.get(`/podcasts/${showId}/episodes`, { params }),
  getEpisode: (showId, episodeId) =>
    api.get(`/podcasts/${showId}/episodes/${episodeId}`),
  getCategories: () => api.get("/podcasts/categories"),
  getFeatured: () =>
    api.get("/podcasts", { params: { is_featured: true, page_size: 20 } }),
  syncPodcasts: () => api.post("/podcasts/sync"),
  resolveUrl: (url, provider) =>
    api.post("/podcasts/resolve-url", { url, provider }),
  addFromUrl: (rssUrl, overrides = {}) =>
    api.post("/podcasts/add-from-url", { rss_url: rssUrl, ...overrides }),
};
...
const apiAudiobookService = {
  ...
  getAudiobookWithChapters: (audiobookId) =>
    api.get(`/audiobooks/${audiobookId}/chapters`),
  getFeatured: () =>
    api.get("/audiobooks", { params: { is_featured: true, page_size: 20 } }),
...
const apiChatService = {
  ...
  resolveContent: async (items, language = "he") => {
    const response = await api.post("/chat/resolve-content", {
      items,
      language,
    });
    return response.data;
...
const apiTrendingService = {
  ...
  getByCategory: (category) =>
    api.get(`/trending/category/${category}`, { timeout: 20000 }),
};
...
const apiSubtitlesService = {
  ...
  generateNikud: (contentId, language = "he", force = false) =>
    api.post(`/subtitles/${contentId}/nikud`, null, {
      params: { language, force },
    }),
  generateShoresh: (contentId, language = "he", force = false) =>
    api.post(`/subtitles/${contentId}/shoresh`, null, {
      params: { language, force },
    }),
  generateHeblish: (contentId, language = "en", force = false) =>
    api.post(`/subtitles/${contentId}/heblish`, null, {
      params: { language, force },
    }),
  generateGrammarFlip: (contentId, language = "en", force = false) =>
    api.post(`/subtitles/${contentId}/grammar-flip`, null, {
      params: { language, force },
    }),
  generateSlangSynthesis: (contentId, language = "en", force = false) =>
    api.post(`/subtitles/${contentId}/slang-synthesis`, null, {
      params: { language, force },
    }),
  // Job status polling
  ...
  searchOpenSubtitles: ({ query, imdb_id, language }) =>
    api.get("/subtitles/opensubtitles/search", {
      params: { query, imdb_id, language },
    }),
};

// Subtitle Preferences Service (API)
const apiSubtitlePreferencesService = {
  getPreference: (contentId) => api.get(`/subtitles/preferences/${contentId}`),
  setPreference: (
    contentId,
    language,
    hebrewMode = "regular",
    ...
    api.post(`/subtitles/preferences/${contentId}`, null, {
      params: { language, hebrew_mode: hebrewMode, english_mode: englishMode },
    }),
  setHebrewMode: (contentId, mode) =>
    api.patch(`/subtitles/preferences/${contentId}/hebrew-mode`, null, {
      params: { hebrew_mode: mode },
    }),
  setEnglishMode: (contentId, mode) =>
    api.patch(`/subtitles/preferences/${contentId}/english-mode`, null, {
      params: { english_mode: mode },
    }),
  deletePreference: (contentId) =>
    api.delete(`/subtitles/preferences/${contentId}`),
  getAllPreferences: () => api.get("/subtitles/preferences"),
};
...
const apiChildrenService = {
  getContent: (category, maxAge, limit) =>
    api.get("/children/content", {
      params: { category, max_age: maxAge, limit },
    }),
  ...
  getContentBySubcategory: (slug, maxAge, limit) =>
    api.get(`/children/subcategory/${slug}`, {
      params: { age_max: maxAge, limit },
    }),
...
const apiYoungstersService = {
  getContent: (category, maxAge, limit) =>
    api.get("/youngsters/content", {
      params: { category, age_max: maxAge, limit },
    }),
  getCategories: () => api.get("/youngsters/categories"),
  getSubcategories: () => api.get("/youngsters/subcategories"),
  getContentBySubcategory: (slug, maxAge, limit) =>
    api.get(`/youngsters/subcategory/${slug}`, {
      params: { age_max: maxAge, limit },
    }),
...
const apiJudaismService = {
  ...
  getShabbatTimes: (city, state, geonameId) =>
    api.get("/judaism/calendar/shabbat", {
      params: { city, state, geoname_id: geonameId },
    }),
  ...
  getSynagogues: (region, denomination, page = 1, limit = 20) =>
    api.get("/judaism/community/synagogues", {
      params: { region, denomination, page, limit },
    }),
  getKosherRestaurants: (
    ...
    api.get("/judaism/community/kosher", {
      params: { region, city, state, certification, page, limit },
    }),
  ...
  getCommunityEvents: (region, eventType, days = 14, page = 1, limit = 20) =>
    api.get("/judaism/community/events", {
      params: { region, event_type: eventType, days, page, limit },
    }),
  getOrganization: (orgId) =>
    api.get(`/judaism/community/organization/${orgId}`),
  searchCommunity: (params) => api.get("/judaism/community/search", { params }),

  // Torah shiurim endpoints
  getShiurim: (category, rabbi, source, page = 1, limit = 20) =>
    api.get("/judaism/shiurim", {
      params: { category, rabbi, source, page, limit },
    }),
...
const apiDownloadsService = {
  ...
  getDownloadProgress: (downloadId) =>
    api.get(`/downloads/${downloadId}/progress`),
};
...
const apiQuizService = {
  ...
  getHistory: (profileId, limit = 20, skip = 0) =>
    api.get("/quiz/history/me", {
      params: { profile_id: profileId, limit, skip },
    }),
};
...
const apiWidgetService = {
  ...
  bulkSoftDelete: (widgetIds) =>
    api.post("/widgets/bulk-delete", { widget_ids: widgetIds }),
};
...
const apiPartyService = {
  ...
  addReaction: (partyId, messageId, emoji) =>
    api.post(`/party/${partyId}/chat/${messageId}/react`, null, {
      params: { emoji },
    }),
  removeReaction: (partyId, messageId, emoji) =>
    api.delete(`/party/${partyId}/chat/${messageId}/react`, {
      params: { emoji },
    }),
  syncPlayback: (partyId, position, isPlaying = true) =>
    api.post(`/party/${partyId}/sync`, null, {
      params: { position, is_playing: isPlaying },
    }),
};
...
const apiStarStoryService = {
  ...
  revokeConsent: (profileId) => api.delete(`/star-story/consent/${profileId}`),
};

// Discover Service (API)
const apiDiscoverService = {
  getConfig: () => api.get("/discover/config"),
  completeWalkthrough: (featureId, stepsCompleted, skipped) =>
    api.post("/discover/walkthrough-complete", {
      feature_id: featureId,
      steps_completed: stepsCompleted,
      skipped,
    }),
  getCharacterGenStatus: () => api.get("/discover/character-generation-status"),
  generateCharacters: (contentId) =>
    api.post(`/discover/generate-characters/${contentId}`),
};

// BYOC Service (API)
const apiBYOCService = {
  enrich: (item) => api.post("/byoc/enrich", item),
  enrichBatch: (items) => api.post("/byoc/enrich/batch", { items }),
  getBatchStatus: (jobId) => api.get(`/byoc/enrich/batch/${jobId}/status`),
  normalize: (entries, healthSample, sourceType) =>
    api.post("/byoc/normalize", {
      entries,
      health_sample: healthSample,
      source_type: sourceType,
    }),
  getNormalizationStatus: (jobId) => api.get(`/byoc/normalize/${jobId}/status`),
  applyNormalization: (
    jobId,
    acceptedPrimaries,
    dismissedIndices,
    ...
    api.post("/byoc/normalize/apply", {
      job_id: jobId,
      accepted_duplicate_primaries: acceptedPrimaries,
      dismissed_indices: dismissedIndices,
      category_overrides: categoryOverrides,
    }),
  getProviders: () => api.get("/byoc/providers"),
  searchChannelIndex: (query, limit) =>
...
const apiHouseholdService = {
  ...
  create: (name) => api.post("/household/create", { name }),
  update: (householdId, name) =>
    api.patch(`/household/${householdId}`, { name }),
  remove: (householdId) => api.delete(`/household/${householdId}`),
  invite: (householdId, email, role) =>
    api.post(`/household/${householdId}/invite`, { email, role }),
  acceptInvitation: (invitationCode) =>
    api.post("/household/accept-invitation", {
      invitation_code: invitationCode,
    }),
  listMembers: (householdId) => api.get(`/household/${householdId}/members`),
  removeMember: (householdId, userId) =>
    api.delete(`/household/${householdId}/members/${userId}`),
};

// Direct Messages Service (API)
const apiDMService = {
  getConversations: () => api.get("/dm/conversations"),
  getMessages: (friendId, limit, before) =>
    api.get(`/dm/${friendId}`, { params: { limit, before } }),
  sendMessage: (friendId, message, messageType) =>
    api.post(`/dm/${friendId}`, { message, message_type: messageType }),
  markRead: (messageId) => api.patch(`/dm/${messageId}/read`),
  markAllRead: (friendId) => api.patch(`/dm/read-all/${friendId}`),
  translateMessage: (messageId) => api.post(`/dm/${messageId}/translate`),
  addReaction: (messageId, emoji) =>
    api.post(`/dm/${messageId}/react`, null, { params: { emoji } }),
  removeReaction: (messageId, emoji) =>
    api.delete(`/dm/${messageId}/react`, { params: { emoji } }),
};
...
export const discoverService = apiDiscoverService;
