...
class ChannelChatService {
  ...
  async connect(
    channelId: string,
    callbacks: ChannelChatCallbacks,
    isLive = true,
  ): Promise<void> {
    try {
      ...
      const wsUrl = buildWsUrl(`/api/v1/ws/${wsPathPrefix}/${channelId}/chat`);

      logger.info(
        "Connecting to channel chat WebSocket",
        "channelChatService",
        {
          channelId,
          isLive,
  ...
  private handleMessage(
    ...
    try {
      ...
      logger.debug("WebSocket message received", "channelChatService", {
        type: msg.type,
        data: msg,
      });
  ...
  static async translateMessage(
    ...
    try {
      ...
      const data = await api.get(`/${pathPrefix}/${channelId}/chat/translate`, {
        params,
      });
