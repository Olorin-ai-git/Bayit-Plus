...
class LiveSubtitleService {
  ...
  async connect(
    ...
    if (this.ws) {
      logger.debug(
        "Cleaning up existing WebSocket before new connection",
        "liveSubtitleService",
      );
      this.disconnect();
    }
    ...
    try {
      ...
      if (this.syncedStreamInfo.mode === "client-side") {
        ...
        logger.info(
          "Server-side video buffering active",
          "liveSubtitleService",
        );
      }
    } catch (error) {
      ...
      try {
        ...
        this.ws.onopen = () => {
          ...
          if (this.ws && this.ws.readyState === WebSocket.OPEN) {
            ...
            logger.error(
              "WebSocket not in OPEN state during onopen",
              "liveSubtitleService",
              {
        ...
        this.ws.onmessage = async (event) => {
          try {
            ...
            logger.debug("Message received", "liveSubtitleService", {
              type: msg.type,
              msg,
            });
            ...
            if (msg.type === "connected") {
              ...
              try {
                ...
                const errorMsg =
                  audioCaptureError instanceof Error
                    ? audioCaptureError.message
                    : "Audio capture failed";
                logger.error(
                  "Audio capture failed after authentication",
                  "liveSubtitleService",
                  { error: errorMsg },
                );
                onError(errorMsg);
              ...
              logger.debug("Subtitle received", "liveSubtitleService", {
                type: msg.type,
                text: msg.data.text,
                data: msg.data,
              });
              ...
              if (!this.firstSubtitleReceived && this.videoBufferManager) {
                ...
                logger.debug(
                  "Completed latency measurement (first subtitle)",
                  "liveSubtitleService",
                );
              }
              ...
              if (
                ...
                logger.debug(
                  "Skipping partial subtitle (awaiting translation)",
                  "liveSubtitleService",
                );
                return;
              }
              ...
              logger.debug(
                "Calling onSubtitle callback",
                "liveSubtitleService",
                msg.data,
              );
              onSubtitle(msg.data);
              logger.debug(
                "onSubtitle callback completed",
                "liveSubtitleService",
              );
  ...
  private async startAudioCapture(
    ...
    try {
      ...
      logger.debug(
        `AudioContext created with sampleRate: ${liveSubtitleConfig.sampleRate}Hz`,
        "liveSubtitleService",
      );
      ...
      if (audioTracks.length === 0) {
        logger.error("No audio tracks in video stream", "liveSubtitleService", {
          message:
            "This usually means: 1. The video has no audio, OR 2. CORS is blocking audio capture (cross-origin video) 3. The video element is muted",
        });
      ...
      audioTracks.forEach((track: any, i: number) => {
        logger.debug(
          `Track ${i}: ${track.label || "unnamed"}, enabled=${track.enabled}, muted=${track.muted}`,
          "liveSubtitleService",
        );
      });
      ...
      logger.debug(
        "Audio processor created (buffer size: 2048, ~128ms latency)",
        "liveSubtitleService",
      );
      ...
      this.processor.onaudioprocess = (e) => {
        ...
        if (maxAmplitude < 0.001) {
          ...
          if (silentChunks === 100) {
            logger.warn(
              "100 consecutive silent chunks detected - Audio may be blocked by CORS or video is muted",
              "liveSubtitleService",
            );
          }
        } else {
        ...
        if (chunkCount % 100 === 0) {
          const dbLevel =
            maxAmplitude > 0 ? 20 * Math.log10(maxAmplitude) : -100;
          logger.debug(
            `Sent ${chunkCount} chunks, level: ${dbLevel.toFixed(1)}dB`,
            "liveSubtitleService",
          );
        }
      };
      ...
      logger.debug(
        "Audio capture started - capturing DIRECTLY from video (not microphone)",
        "liveSubtitleService",
      );
    } catch (error) {
