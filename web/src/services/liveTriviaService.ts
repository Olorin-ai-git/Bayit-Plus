...
if (!API_BASE_URL) {
  logger.warn(
    "VITE_API_URL not configured - trivia features will fail",
    LOG_CONTEXT,
  );
}
...
class LiveTriviaService {
  ...
  connect(
    ...
    if (!isValidChannelId(channelId)) {
      onError({
        message: i18n.t("errors.trivia.invalidChannel"),
        recoverable: false,
      });
      return;
    }
    ...
    if (!token) {
      onError({
        message: i18n.t("errors.auth.notAuthenticated"),
        recoverable: false,
      });
      return;
    }

    const wsUrl = buildWsUrl(`/api/v1/ws/live/${channelId}/trivia`);

    logger.info(
      `Connecting to trivia WebSocket for channel ${channelId}`,
      LOG_CONTEXT,
    );
    ...
    const connectionTimeout = setTimeout(() => {
      if (!this.isConnected) {
        ...
        this.onError?.({
          message: i18n.t("errors.connection.timeout"),
          recoverable: true,
        });
        this.disconnect();
      }
        ...
        switch (msg.type) {
          ...
          case "trivia_fact":
            if (msg.data && isValidTriviaFact(msg.data)) {
              ...
              logger.error(
                "Invalid trivia fact data received",
                LOG_CONTEXT,
                msg.data,
              );
            }
          ...
          case "quota_exceeded":
            ...
            clearTimeout(connectionTimeout);
            this.onError?.({
              message: msg.message || "Usage limit reached",
              recoverable: false,
            });
            this.disconnect();
            break;
    ...
    this.ws.onerror = () => {
      ...
      clearTimeout(connectionTimeout);
      this.onError?.({
        message: i18n.t("errors.connection.error"),
        recoverable: true,
      });
