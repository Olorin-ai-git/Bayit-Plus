...
export function useContentData() {
  ...
  const loadContent = useCallback(async () => {
    ...
    try {
      if (
        ...
        filters.content_type === "radio" ||
        filters.content_type === "audiobooks"
      ) {
        ...
        log.debug("Podcasts/Radio/Audiobooks loaded", {
        ...
        const apiFilters = {
          ...
          is_published: filters.is_published,
          content_type:
            filters.content_type === "series"
              ? "series"
              : filters.content_type === "movies"
                ? "movies"
                : undefined,
          sort_by: sortBy,
        ...
        const response =
          await adminContentService.getContentHierarchical(apiFilters);

        log.debug("VOD content loaded", {
          count: response.items.length,
          total: response.total,
          firstItems: response.items
            .slice(0, 3)
      ...
      if (!expanded) {
        ...
        if (!episodeCache[rowId]) {
          try {
            ...
            log.info("Episodes loaded", {
              seriesId: rowId,
              count: response.episodes.length,
            });
          } catch (err) {
    ...
    try {
      ...
      if (result.errors && result.errors.length > 0) {
        if (result.deleted_count > 0) {
          ...
          // Complete failure
          notifications.showError(t("common.error"), result.errors.join(", "));
        ...
        notifications.showSuccess(
          t("common.success"),
          t("admin.content.batchDeleteSuccess", {
  ...
  const handleBatchFeature = useCallback(
    async (featured: boolean) => {
      ...
      try {
        ...
        setSelectedItemsData([]);
        await loadContent();
        log.info("Batch feature update successful", {
          featured,
  ...
  const hierarchicalData = useMemo<HierarchicalTableRow[]>(() => {
    ...
    const filtered = showOnlyWithSubtitles
      ? items.filter(
          (item) =>
            item.available_subtitles && item.available_subtitles.length > 0,
        )
      : items;
