...
const INTERACTION_EVENT_TYPES = new Set([
  'interaction_start', 'turn_change', 'character_response',
  'turn_warning', 'turn_skipped', 'interaction_end',
])
...
interface UseSharedInteractionResult {
  ...
  isSending: boolean
  startSharedInteraction: (partyId: string, contentId: string, momentTimestamp: number, characterName: string) => Promise<void>
...
export function useSharedInteraction(): UseSharedInteractionResult {
  ...
  const isMyTurn = currentTurnUserId !== null && currentTurnUserId === currentUserId
  ...
  const sendMessage = useCallback(async (text: string, addressedCharacter?: string): Promise<void> => {
  ...
  return {
    session, participants, currentTurnUserId, isMyTurn,
    turnCountdown, conversation, isSending,
    startSharedInteraction, sendMessage, endSharedInteraction,
  }
}
