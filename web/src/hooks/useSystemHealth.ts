  ...
  const loadInitialData = useCallback(async () => {
    try {
      ...
      if (allClients.length > 0) {
        const avgCpu =
          allClients.reduce((sum, c) => sum + (c.metrics.cpu_usage || 0), 0) /
          allClients.length;
        const avgMemory =
          allClients.reduce(
            (sum, c) => sum + (c.metrics.memory_usage || 0),
            0,
          ) / allClients.length;
        ...
        setMetrics({
          ...
          active_requests: allClients.reduce(
            (sum, c) => sum + c.metrics.active_users,
            0,
          ),
        });
      }
  ...
  const connectWebSocket = useCallback(() => {
    ...
    try {
      const ws = new WebSocket(
        buildWsUrl(`/ws/admin/diagnostics?token=${token}`),
      );
      wsRef.current = ws;
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (data.type === "snapshot" || data.type === "update") {
            ...
            const allClients = Object.values(
              diagnosticsData.clients ?? {},
            ).flat();
            if (allClients.length > 0) {
              const avgCpu =
                allClients.reduce(
                  (sum, c) => sum + (c.metrics.cpu_usage || 0),
                  0,
                ) / allClients.length;
              const avgMemory =
                allClients.reduce(
