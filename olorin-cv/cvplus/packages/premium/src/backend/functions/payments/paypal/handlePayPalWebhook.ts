...
export const handlePayPalWebhook = https.onRequest(async (req, res) => {
  ...
  if (!authAlgo || !transmission || !certId || !transmissionSig || !transmissionTime) {
    logger.error('[handlePayPalWebhook] Missing required webhook headers');
  ...
  try {
    ...
    logger.info('[handlePayPalWebhook] Webhook received:', {
    ...
    if (!result.processed) {
      logger.error('[handlePayPalWebhook] Failed to process webhook:', {
    ...
    logger.info('[handlePayPalWebhook] Webhook processed successfully:', {
      event_id: webhookEvent.id,
    ...
    logger.error('[handlePayPalWebhook] Error processing webhook:', error);
