...
export class PayPalTestRunner {
  ...
  async runAllTests(): Promise<TestSummary> {
    logger.info('🚀 Starting PayPal Integration Test Suite...');
  ...
  private async runBasicPaymentTests(): Promise<void> {
    ...
    logger.info('✅ Basic Payment Tests Completed');
  }
  ...
  private async runWebhookTests(): Promise<void> {
    logger.info('📡 Running Webhook Tests...');
  ...
  private async runErrorScenarioTests(): Promise<void> {
    logger.info('⚠️ Running Error Scenario Tests...');
  ...
  private async runPerformanceTests(): Promise<void> {
    ...
    await this.runTest('High Volume Payment Processing', async () => {
      ...
      logger.info(`  📊 Processed 100 payments in ${processingTime}ms`);
    });
    ...
    await this.runTest('Concurrent Webhook Processing', async () => {
      ...
      logger.info(`  📊 Processed 50 webhooks in ${processingTime}ms`);
    });
  ...
  private async runTest(name: string, testFn: () => Promise<void>): Promise<void> {
    ...
    try {
      ...
      logger.info(`  ❌ ${name} (${duration}ms): ${error instanceof Error ? error.message : String(error)}`);
    }
  }
