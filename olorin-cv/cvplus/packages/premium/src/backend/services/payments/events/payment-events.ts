...
export class PaymentEventBus implements IProviderEventBus {
    ...
    try {
      ...
      for (const middleware of this.getSortedMiddleware()) {
        try {
          ...
          logger.error(`[PaymentEventBus] Middleware error in ${middleware.name}:`, error);
        }
      }
      ...
      for (const middleware of this.getSortedMiddleware()) {
        for (const { result } of handlerResults) {
          try {
            ...
            await middleware.onError(error as Error, processedEvent);
            logger.error(`[PaymentEventBus] Middleware after-handler error in ${middleware.name}:`, error);
          }
        }
      }
  ...
  addMiddleware(middleware: EventMiddleware): void {
    ...
    logger.info(`[PaymentEventBus] Middleware '${middleware.name}' added with priority ${middleware.priority}`);
  }
  ...
  removeMiddleware(name: string): void {
    ...
    if (index !== -1) {
      ...
      logger.info(`[PaymentEventBus] Middleware '${name}' removed`);
    }
  }
  ...
  clearHistory(): void {
    ...
    logger.info('[PaymentEventBus] Event history cleared');
  }
  ...
  private async handleHandlerError(
    ...
    logger.error(`[PaymentEventBus] Handler '${handler.name}' failed:`, error);
    ...
    try {
      ...
      logger.error(`[PaymentEventBus] Handler onError callback failed:`, onErrorError);
    }
    ...
    this.emit(errorEvent).catch(err => {
      logger.error('Payment event emission failed:', err instanceof Error ? err : new Error(String(err)));
    });
  }
  ...
  private logEventProcessing(
    ...
    logger.info(`[PaymentEventBus] Event ${status}:`, {
      event_id: event.id,
  ...
  private logSubscription(
    ...
    logger.info(`[PaymentEventBus] ${action}:`, {
...
export class MetricsCollectionEventHandler implements PaymentEventHandler {
  ...
  async handle(event: PaymentEvent): Promise<EventHandlerResult> {
    ...
    logger.info(`[Metrics] Collecting metrics for ${event.type} from ${event.provider}`);
