...
export class PaymentSystem {
  ...
  async initialize(): Promise<void> {
    logger.info('[PaymentSystem] Initializing advanced payment processing system...');
    ...
    logger.info('[PaymentSystem] Payment system initialized successfully');
    logger.info(`[PaymentSystem] Registered providers: ${this.registry.getAll().map(p => p.providerName).join(', ')}`);
  }
   ...
   * Shutdown the payment system gracefully
    */
  shutdown(): void {
    logger.info('[PaymentSystem] Shutting down payment system...');
    this.registry.shutdown();
    logger.info('[PaymentSystem] Payment system shutdown completed');
  }
}
