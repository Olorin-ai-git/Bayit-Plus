...
export class PaymentMetricsCollector implements IPaymentMetricsCollector {
  ...
  async recordTransactionMetrics(
    ...
    try {
      ...
      logger.error(`[PaymentMetricsCollector] Failed to record transaction metrics:`, error);
    }
  }
  ...
  async recordProviderMetrics(
    ...
    try {
      ...
      if (!currentMetrics) {
        logger.warn(`[PaymentMetricsCollector] Provider ${provider} not initialized`);
        return;
      }
      ...
      logger.error(`[PaymentMetricsCollector] Failed to record provider metrics:`, error);
    }
  }
  ...
  async recordTransactionCost(
    ...
    try {
      ...
      logger.error(`[PaymentMetricsCollector] Failed to record transaction cost:`, error);
    }
  }
      ...
      try {
        ...
        this.realtimeSubscribers.forEach(callback => {
          try {
            ...
            logger.error('[PaymentMetricsCollector] Subscriber callback error:', error);
          }
        });
      } catch (error) {
        logger.error('[PaymentMetricsCollector] Failed to update realtime metrics:', error);
      }
  ...
  private logMetricsEvent(
    ...
    logger.info(`[PaymentMetricsCollector] ${type}:`, {
      provider,
