...
export class PaymentOrchestrator implements IPaymentOrchestrator {
  ...
  private logOrchestrationEvent(
    ...
    logger.info(`[PaymentOrchestrator] ${type}:`, JSON.stringify(event, null, 2));

    // Emit to event bus
    paymentEventBus.emit(event).catch(err => {
      logger.error('Event bus emission failed:', err instanceof Error ? err : new Error(String(err)));
    });
  }
}
