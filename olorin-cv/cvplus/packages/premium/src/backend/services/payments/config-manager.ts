...
export class ProviderConfigurationManager implements IProviderConfigurationManager {
  ...
  async updateConfig<T extends PaymentProviderConfig>(
    ...
    this.configChangeCallbacks.forEach(callback => {
      try {
        ...
        logger.error(`[ConfigManager] Config change callback error:`, error);
      }
    });
  ...
  async rotateSecrets(provider: PaymentProviderName): Promise<void> {
    ...
    logger.info(`[ConfigManager] Secrets rotation for ${provider} - would be implemented with secrets manager`);
    ...
    this.secretsRotationCallbacks.forEach(callback => {
      try {
        ...
        logger.error(`[ConfigManager] Secrets rotation callback error:`, error);
      }
    });
  ...
  private parseIntEnvVar(name: string, defaultValue: number): number {
    ...
    if (isNaN(parsed)) {
      logger.warn(`[ConfigManager] Invalid integer value for ${name}: ${value}, using default: ${defaultValue}`);
      return defaultValue;
    }
  ...
  private logConfigEvent(
    ...
    logger.info(`[ConfigManager] ${type} for ${provider}:`, JSON.stringify(data, null, 2));
  }
