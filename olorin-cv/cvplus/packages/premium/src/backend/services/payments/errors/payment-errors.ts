...
export class PaymentErrorHandler implements IPaymentErrorHandler {
  ...
  async reportError(error: ProviderError, context: PaymentContext): Promise<void> {
    try {
      ...
      logger.info('[PaymentErrorHandler] Error reported:', JSON.stringify(reportData, null, 2));
      ...
      logger.error('[PaymentErrorHandler] Failed to report error:', reportingError);
    }
  }
  ...
  private updateErrorTrends(): void {
    ...
    if (recentErrors.length > 0) {
      logger.info(`[PaymentErrorHandler] Analyzed ${recentErrors.length} recent errors`);
    }
  }
  ...
  private cleanupOldErrors(): void {
    ...
    if (removed > 0) {
      logger.info(`[PaymentErrorHandler] Cleaned up ${removed} old error records`);
    }
  }
  ...
  private logErrorEvent(
    ...
    logger.info(`[PaymentErrorHandler] ${type}:`, {
      user_id: context.userId,
      currency: context.currency,
      amount: context.amount,
      ...data,
