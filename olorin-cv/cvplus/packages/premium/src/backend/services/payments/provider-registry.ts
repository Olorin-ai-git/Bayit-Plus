...
export class ProviderRegistry implements IPaymentProviderRegistry {
  ...
  register(provider: IPaymentProvider): void {
    ...
    if (this.providers.has(providerName)) {
      logger.warn(`[ProviderRegistry] Provider '${providerName}' is already registered. Replacing...`);
    }
    ...
    this.registrationCallbacks.forEach(callback => {
      try {
        ...
        logger.error(`[ProviderRegistry] Registration callback error:`, error);
      }
    });
  ...
  remove(providerName: PaymentProviderName): void {
    ...
    this.removalCallbacks.forEach(callback => {
      try {
        ...
        logger.error(`[ProviderRegistry] Removal callback error:`, error);
      }
    });
   ...
   *
   * Filters providers based on their regional availability and compliance.
   * Supports ISO 3166-1 alpha-2 country codes (e.g., 'US', 'EU', 'GB', etc.)
   *
   * Each provider has regional restrictions based on:
   * - Payment processor licensing and compliance
  ...
  getByRegion(region: string): IPaymentProvider[] {
    ...
    return this.getAll().filter(provider => {
      // Get provider's supported regions from configuration
      const supportedRegions = this.getProviderSupportedRegions(provider);
  ...
  /**
   * Get supported regions for a specific provider
   * Implementation details depend on provider configuration
   */
  private getProviderSupportedRegions(provider: IPaymentProvider): string[] {
    // Provider-specific region mappings
    const regionMap: Record<string, string[]> = {
      stripe: [
        // Stripe supports ~46 countries
        'US', 'CA', 'GB', 'IE', 'DE', 'FR', 'ES', 'IT', 'NL', 'BE', 'AT', 'CH',
        'SE', 'NO', 'DK', 'FI', 'CZ', 'PL', 'HU', 'RO', 'BG', 'HR', 'SI', 'SK',
        'GR', 'PT', 'LU', 'MT', 'CY', 'EE', 'LV', 'LT', 'RU', 'UA', 'BY', 'KZ',
        'AU', 'NZ', 'JP', 'SG', 'HK', 'IN', 'MY', 'TH', 'BR', 'MX', 'AE', 'SA',
      ],
      paypal: [
        // PayPal supports ~190+ countries
        'US', 'CA', 'GB', 'IE', 'DE', 'FR', 'ES', 'IT', 'NL', 'BE', 'AT', 'CH',
        'SE', 'NO', 'DK', 'FI', 'CZ', 'PL', 'HU', 'RO', 'BG', 'HR', 'SI', 'SK',
        'GR', 'PT', 'LU', 'MT', 'CY', 'EE', 'LV', 'LT', 'RU', 'UA', 'BY', 'KZ',
        'AU', 'NZ', 'JP', 'SG', 'HK', 'IN', 'MY', 'TH', 'BR', 'MX', 'AE', 'SA',
        'ZA', 'EG', 'NG', 'KE', 'GH', 'TW', 'PH', 'VN', 'TL', 'BD', 'PK',
        'CN', 'KR', 'CO', 'AR', 'CL', 'PE', 'CW', 'CR', 'DO', 'SV', 'GT',
      ],
    };

    const regions = regionMap[provider.providerName.toLowerCase()] || [];

    // Log warning if provider has no known regions (fail secure)
    if (regions.length === 0) {
      logger.warn(
        `[ProviderRegistry] No regions configured for provider: ${provider.providerName}. ` +
        `Defaulting to empty list (fail secure - deny all regions).`
      );
    }
        ...
        try {
          ...
          this.healthCallbacks.forEach(callback => {
            try {
              ...
              logger.error(`[ProviderRegistry] Health callback error:`, error);
            }
          });
  ...
  async discoverProviders(): Promise<void> {
    ...
    if (process.env.STRIPE_SECRET_KEY && process.env.STRIPE_PUBLISHABLE_KEY) {
      try {
        ...
        logger.error('[ProviderRegistry] Failed to auto-register Stripe provider:', error);
      }
    }
    ...
    if (process.env.PAYPAL_CLIENT_ID && process.env.PAYPAL_CLIENT_SECRET) {
      try {
        ...
        logger.error('[ProviderRegistry] Failed to auto-register PayPal provider:', error);
      }
    }
  }
  ...
  shutdown(): void {
    ...
    logger.info('[ProviderRegistry] Registry shutdown completed');
  }
  ...
  private startHealthMonitoring(): void {
    this.healthMonitorInterval = setInterval(async () => {
      try {
        ...
        logger.error('[ProviderRegistry] Health check failed:', error);
      }
  ...
  private logProviderEvent(
    ...
    logger.info(`[ProviderRegistry] ${type}:`, JSON.stringify(event, null, 2));
  }
