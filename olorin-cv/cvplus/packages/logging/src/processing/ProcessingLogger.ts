...
import { LogLevel, LogDomain } from '../backend/types';
import type { LogPerformance } from '../backend/types';
import { BaseLogger } from '../backend/core/BaseLogger';
import { getCurrentCorrelationId, withCorrelation } from '../backend/utils/correlation';
...
export class ProcessingLogger {
  ...
  withCorrelation<T>(correlationId: string, callback: () => T): T {
    return withCorrelation(correlationId, callback);
  }
