...
export class RAGService {
  ...
  async initializeIndex(): Promise<boolean> {
    try {
      ...
      logger.info(`Pinecone index ${this.INDEX_NAME} is ready. Stats:`, indexStats);
      return true;
    } catch (error) {
  ...
  async storeCVEmbeddings(embeddings: CVEmbeddingDocument[]): Promise<boolean> {
    ...
    try {
      ...
      logger.info(`Storing ${embeddings.length} embeddings in namespace: ${namespace}`);
      ...
      for (let i = 0; i < vectors.length; i += batchSize) {
        ...
        logger.info(`Stored batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(vectors.length / batchSize)}`);
      ...
      logger.info(`Successfully stored all ${embeddings.length} embeddings for CV ${processedCvId}`);
      return true;

    } catch (error) {
      logger.error('Error storing CV embeddings:', error);
  ...
  async searchRelevantContent(
    ...
    try {
      logger.info(`Searching for relevant content in CV ${processedCvId} with query: "${query}"`);
      ...
      logger.info(`Found ${results.length} relevant content chunks with average confidence: ${confidence.toFixed(3)}`);
  ...
  async deleteCVEmbeddings(processedCvId: string): Promise<boolean> {
    try {
      ...
      logger.info(`Successfully deleted all embeddings for CV ${processedCvId}`);
      return true;

    } catch (error) {
      logger.error('Error deleting CV embeddings:', error);
      return false;
    }
  }
  ...
  async hasCVEmbeddings(processedCvId: string): Promise<boolean> {
    try {
      ...
      logger.warn(`Could not check embeddings for CV ${processedCvId}:`, error);
      return false;
    }
  }
  ...
  async processAndStoreCVEmbeddings(processedCV: any): Promise<boolean> {
    try {
      logger.info(`Processing and storing embeddings for CV: ${processedCV.id}`);

      // Check if embeddings already exist
      const hasExisting = await this.hasCVEmbeddings(processedCV.id);
      if (hasExisting) {
        logger.info(`Embeddings already exist for CV ${processedCV.id}, skipping`);
        return true;
      }
      ...
      if (embeddings.length === 0) {
        logger.warn(`No embeddings generated for CV ${processedCV.id}`);
        return false;
      }
      ...
      if (success) {
        logger.info(`Successfully processed and stored ${embeddings.length} embeddings for CV ${processedCV.id}`);
      }

      return success;

    } catch (error) {
      logger.error('Error processing and storing CV embeddings:', error);
      return false;
    }
  }
