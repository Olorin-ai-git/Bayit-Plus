...
export class EmbeddingService {
  ...
  async generateCVEmbeddings(processedCV: any): Promise<CVEmbeddingDocument[]> {
    try {
      logger.info(`Generating embeddings for CV: ${processedCV.id}`);

      // Step 1: Chunk CV content into optimal sizes
      const chunks = await this.chunkCVContent(processedCV);

      if (chunks.length === 0) {
        logger.warn('No content chunks generated for CV:', processedCV.id);
        return [];
      }

      logger.info(`Generated ${chunks.length} content chunks`);
      ...
      logger.error('Error generating CV embeddings:', error);
  ...
  private splitTextIntoChunks(
    ...
    if (estimatedTokens <= this.MAX_CHUNK_TOKENS) {
      ...
      while (startIndex < words.length) {
        ...
        if (chunkIndex > 100) {
          logger.warn(`Too many chunks generated for section ${section}, stopping at 100`);
          break;
        }
      }
    }
