...
export class PodcastGenerationService {
  ...
  private async validateEnvironment(): Promise<void> {
    ...
    try {
      ...
      logger.error('❌ ElevenLabs API validation failed:', {
...
Focus: ${options.focus || 'balanced'}`;
  ...
  private async generateAudioSegments(
    ...
    for (let i = 0; i < script.segments.length; i++) {
      ...
      try {
        ...
        logger.error(`Error generating audio for segment ${i}:`, {
          speaker: segment.speaker,
  ...
  private async mergeAudioSegments(
    ...
    try {
      ...
      logger.error('Error merging audio segments:', {
        message: error.message,
        stack: error.stack,
        tempDir,
  ...
  private cleanupTempFiles(filePaths: string[]): void {
    ...
    filePaths.forEach(filePath => {
      try {
        ...
        logger.error(`Failed to cleanup ${filePath}:`, {
          error: error.message,
          code: error.code,
          errno: error.errno
        });
