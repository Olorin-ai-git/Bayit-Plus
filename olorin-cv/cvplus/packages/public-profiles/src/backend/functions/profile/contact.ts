...
export const contactProfileOwner = onRequest(
  ...
  async (req: Request, res: Response) => {
    try {
      logger.info('Profile contact request received');
      ...
      logger.info(`Processing contact request for profile: ${profileId}`);
      ...
      if (spamScore > 0.8) {
        logger.warn(`High spam score (${spamScore}) for contact request from ${requestData.senderEmail}`);
      ...
      logger.info(`Contact request processed successfully: ${messageId}`);
      ...
      logger.error('Profile contact error:', error);
...
async function sendContactNotification(
  ...
  try {
    ...
    logger.info(`Sending contact notification to: ${recipientEmail}`);
