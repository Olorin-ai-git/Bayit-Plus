...
async function handleStartChatSession(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (!hasEmbeddings) {
      logger.info(`No embeddings found for CV ${portalData.processedCvId}, attempting to generate...`);
    ...
    if (userMessage?.trim()) {
      // Note: Should process initial message and add AI response
      // This will be implemented in T035 (sendChatMessage)
      logger.info('Initial message received:', userMessage);
    }
    ...
    logger.error('Error in startChatSession:', error);
    res.status(500).json({
      success: false,
...
async function updatePortalAnalytics(
  ...
  try {
    ...
    logger.warn('Failed to update portal analytics:', error);
  }
}
