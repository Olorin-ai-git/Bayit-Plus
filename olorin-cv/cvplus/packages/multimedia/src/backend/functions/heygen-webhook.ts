...
export const heygenWebhook = onRequest(
  ...
  async (request, response) => {
    ...
    try {
      ...
      logger.info('[HeyGen Webhook] Processing payload:', {
      ...
      logger.info(`[HeyGen Webhook] Successfully processed in ${processingTime}ms:`, {
        jobId: result.jobId,
      ...
      logger.error('[HeyGen Webhook] Processing failed:', {
        error: error.message,
...
export const videoWebhook = onRequest(
  ...
  async (request, response) => {
    ...
    try {
      ...
      logger.info(`[Video Webhook] ${provider} processed successfully:`, {
        jobId: result.jobId,
        status: result.status,
        processingTime
      });
