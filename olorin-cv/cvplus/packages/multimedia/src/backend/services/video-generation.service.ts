...
export class VideoGenerationService {
      ...
      if (options.useAdvancedPrompts !== false) { // Default to enhanced
        try {
          const promptOptions: PromptEngineOptions = {
            ...
            optimizationLevel: (options.optimizationLevel || 'premium') as 'basic' | 'standard' | 'advanced' | 'premium'
          };

          enhancedScript = await advancedPromptEngine.generateEnhancedScript(
            parsedCV.personalInfo?.name || 'Professional',
            promptOptions
          );
  ...
  async generateEnhancedScriptOnly(
    ...
    try {
      ...
      return await advancedPromptEngine.generateEnhancedScript(parsedCV.personalInfo?.name || 'Professional', promptOptions);
    } catch (error: any) {
