...
/**
 * Validate that a Stripe price ID is properly configured (not a placeholder)
 * Throws an error if the price ID is invalid or missing
 */
const validateStripePriceId = (priceId: string | undefined, environment: string): string => {
  if (!priceId) {
    throw new Error(
      `CRITICAL: Stripe price ID is not configured for ${environment} environment. ` +
      `Please set the appropriate env var: STRIPE_PRICE_ID_${environment.toUpperCase()} ` +
      `The price ID must be a valid Stripe price identifier (e.g., price_xxxxx).`
    );
  }

  if (priceId.includes('placeholder') || priceId.includes('template')) {
    throw new Error(
      `CRITICAL: Stripe price ID contains a placeholder: "${priceId}". ` +
      `This is not a valid Stripe configuration. ` +
      `Please configure a real Stripe price ID via environment variables.`
    );
  }
...
const PRICING_CONFIG = {
  stripe: {
    priceIds: {
      development: validateStripePriceId(process.env.STRIPE_PRICE_ID_DEV, 'development'),
      staging: validateStripePriceId(process.env.STRIPE_PRICE_ID_STAGING, 'staging'),
      production: validateStripePriceId(process.env.STRIPE_PRICE_ID_PROD, 'production')
    }
  },
  pricing: {
    free: {
...
// Fails explicitly if NODE_ENV or FUNCTIONS_EMULATOR are not properly configured
const getEnvironment = (): 'development' | 'staging' | 'production' => {
  ...
  // Fail explicitly if NODE_ENV is not configured
  throw new Error(
    'CRITICAL: NODE_ENV environment variable is not properly configured. ' +
    `Received: "${nodeEnv}". ` +
    'Please set NODE_ENV to one of: development, staging, or production.'
  );
};
 ...
 * All prices are validated at module load time to ensure they are real Stripe price IDs
 */
export const getStripePriceId = (_tier: SubscriptionTier): string => {
  const env = getEnvironment();
  const priceId = PRICING_CONFIG.stripe.priceIds[env];

  // Note: priceId is guaranteed to be valid and not a placeholder due to
  // validation that happens when the module loads (validateStripePriceId)
  return priceId;
};
