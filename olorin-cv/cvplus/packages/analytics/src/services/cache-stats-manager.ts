...
export class CacheStatsManager {
  ...
  */
  recordCacheHit(tier: 'memory' | 'redis' | 'firestore', latency: number): void {
    ...
    logger.info(`[CacheStatsManager] Cache HIT (${tier}) - latency: ${latency}ms`);
    this.updateStats();
  }
  ...
  */
  recordCacheMiss(latency: number): void {
    ...
    logger.info(`[CacheStatsManager] Cache MISS - latency: ${latency}ms`);
    this.updateStats();
  }
  ...
  */
  performCleanup(): void {
    ...
    if (expiredKeys.length > 0) {
      logger.info(`[CacheStatsManager] Cleaned up ${expiredKeys.length} expired entries`);
      this.updateStats();
    }
  }
