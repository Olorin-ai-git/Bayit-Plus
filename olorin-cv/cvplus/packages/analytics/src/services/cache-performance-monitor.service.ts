...
class PricingCacheService {
  ...
  async getHitRate() {
    try {
      ...
      logger.error('Failed to calculate hit rate:', error);
      return 0;
    }
  }
  ...
  async warmCache(userIds?: string[], plans?: string[]) {
    try {
      ...
      logger.error('Failed to warm pricing cache:', error);
      throw error;
    }
  }
  ...
  async getWriteReduction() {
    try {
      ...
      logger.error('Failed to calculate write reduction:', error);
      return 0;
    }
  }
}
...
class SubscriptionCacheService extends PricingCacheService {
  ...
  async warmCache(userIds?: string[]) {
    try {
      ...
      logger.error('Failed to warm subscription cache:', error);
      throw error;
    }
  }
}
...
class FeatureAccessCacheService extends PricingCacheService {
  async warmCache(userIds?: string[]) {
    try {
      ...
      logger.error('Failed to warm feature access cache:', error);
      throw error;
    }
  }
}
...
class UsageBatchCacheService extends PricingCacheService {
  async getMetrics() {
    ...
    try {
      ...
      logger.error('Failed to get usage batch metrics:', error);
      return {
        ...baseMetrics,
        eventsQueued: 0,
        averageBatchSize: 0,
  ...
  async warmCache() {
    try {
      ...
      logger.error('Failed to warm usage batch cache:', error);
      throw error;
    }
  }
}
