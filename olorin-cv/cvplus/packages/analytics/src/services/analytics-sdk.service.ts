...
export class CVPlusAnalyticsSDK {
    ...
    try {
      ...
      if (!consentStatus.hasConsent && this.config.privacy.consentRequired) {
        if (this.config.debug) {
          logger.info('[CVPlus Analytics] Consent required but not given, initializing in minimal mode');
        }
      ...
      if (this.config.debug) {
        logger.info('[CVPlus Analytics] SDK initialized successfully');
      }

    } catch (error) {
      logger.error('[CVPlus Analytics] Failed to initialize:', error);
      throw error;
    }
  }
  ...
  */
  async track(eventName: string, properties: EventProperties = {}): Promise<void> {
    if (!this.initialized) {
      logger.warn('[CVPlus Analytics] SDK not initialized, call initialize() first');
      return;
    }
    ...
    if (!await this.privacyManager.hasConsent(ConsentCategory.ANALYTICS)) {
      if (this.config.debug) {
        logger.info('[CVPlus Analytics] Analytics consent not given, skipping event:', eventName);
      }
      return;
    }
    ...
    try {
      ...
      if (this.config.debug) {
        logger.info('[CVPlus Analytics] Event tracked:', eventName, properties);
      }

    } catch (error) {
      logger.error('[CVPlus Analytics] Failed to track event:', error);
      if (this.config.autoTrackErrors) {
  ...
  */
  async page(category?: string, name?: string, properties: EventProperties = {}): Promise<void> {
    if (!this.initialized) {
      logger.warn('[CVPlus Analytics] SDK not initialized');
      return;
    }
    ...
    try {
      ...
      logger.error('[CVPlus Analytics] Failed to track page view:', error);
    }
  }
  ...
  */
  async identify(userId: string, traits: UserIdentification['traits'] = {}): Promise<void> {
    ...
    if (!await this.privacyManager.hasConsent(ConsentCategory.ANALYTICS)) {
      if (this.config.debug) {
        logger.info('[CVPlus Analytics] Analytics consent not given for user identification');
      }
      return;
    }
    ...
    try {
      ...
      if (this.config.debug) {
        logger.info('[CVPlus Analytics] User identified:', userId);
      }

    } catch (error) {
      logger.error('[CVPlus Analytics] Failed to identify user:', error);
    }
  }
  ...
  */
  ...
  private async initializeMinimalMode(): Promise<void> {
    ...
    if (this.config.debug) {
      logger.info('[CVPlus Analytics] Initialized in minimal mode');
    }
  }
  ...
  private async enqueueEvent(event: AnalyticsEvent): Promise<void> {
    ...
    if (this.config.validateEvents) {
      ...
      if (!validation.valid) {
        logger.warn('[CVPlus Analytics] Event validation failed:', validation.errors);
        return;
      }
  ...
  private trackWebVitals(): void {
    ...
    try {
      ...
      if (this.config.debug) {
        logger.warn('[CVPlus Analytics] Web Vitals tracking failed:', error);
      }
    }
  }
  ...
  private async getAuthToken(): Promise<string> {
    ...
    try {
      ...
      logger.warn('[CVPlus Analytics] Auth token unavailable:', error);
      return this.config.transport?.apiKey || '';
    }
  }
...
class PrivacyManager {
  ...
  private initializeConsent(): void {
    ...
    if (storedConsent) {
      try {
        ...
        logger.warn('[CVPlus Analytics] Failed to load stored consent');
      }
    }
  ...
  async updateConsent(consent: Record<ConsentCategory, boolean>): Promise<void> {
    ...
    try {
      ...
      logger.warn('[CVPlus Analytics] Failed to store consent record:', error);
    }
  }
  ...
  async requestUserData(): Promise<DataAccessRequest> {
    try {
      ...
      logger.error('Failed to request user data:', error);
      throw error;
    }
  }
...
class EventQueue {
  ...
  async flush(): Promise<void> {
    ...
    try {
      ...
      if (this.config.debug) {
        logger.info('[CVPlus Analytics] Successfully flushed batch:', batch.length, 'events');
      }

    } catch (error) {
      logger.error('[CVPlus Analytics] Failed to flush events:', error);
...
class EventTransport {
    ...
    try {
      ...
      if (typeof navigator !== 'undefined' && navigator.geolocation) {
        try {
          ...
          logger.warn('Geolocation failed, falling back to IP-based detection:', geolocationError);
        }
      }
      ...
      try {
        ...
        logger.warn('IP-based location detection failed:', ipError);
      }
      ...
      logger.error('Error getting location with consent:', error);
      return {
        timezone: Intl.DateTimeFormat().resolvedOptions().timeZone
      };
    }
  ...
  private async getIPBasedLocation(): Promise<{ country?: string; region?: string; city?: string; timezone?: string }> {
    try {
      ...
      logger.error('IP-based location detection failed:', error);
      throw error;
    }
  }
}