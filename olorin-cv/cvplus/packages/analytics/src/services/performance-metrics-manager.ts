...
export class PerformanceMetricsManager {
  ...
  */
  recordSuccess(duration: number, fromCache: boolean): void {
    ...
    logger.info(`[PerformanceMetricsManager] Success recorded - Duration: ${duration}ms, Cache: ${fromCache}`);
  }
  ...
  */
  recordError(error: RecommendationError, duration: number): void {
    ...
    logger.error(`[PerformanceMetricsManager] Error recorded - Type: ${error.type}, Duration: ${duration}ms`);
  }
  ...
  */
  private reportMetrics(): void {
    logger.info('[PerformanceMetricsManager] Performance Metrics:', {
  ...
  */
  private checkPerformanceThresholds(): void {
    ...
    if (warnings.length > 0) {
      logger.warn('[PerformanceMetricsManager] Performance threshold warnings:', warnings);
    }
  }
  ...
  */
  resetMetrics(): void {
    ...
    logger.info('[PerformanceMetricsManager] Metrics reset');
  }
  ...
  */
  ...
  private calculateQueueDepth(): number {
    try {
      ...
      logger.error('Error calculating queue depth:', error);
      return 0;
    }
  }
  ...
  private getPendingFirestoreWrites(): number {
    try {
      ...
      logger.error('Error calculating Firestore queue depth:', error);
      return 0;
    }
  }
}