...
export const getAnalytics = onRequest(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      logger.info(`Getting analytics for ${entityType}/${entityId} with period: ${requestParams.period}`);
      ...
      logger.info(`Analytics data retrieved successfully for ${entityType}/${entityId}`);
...
async function verifyEntityOwnership(
  ...
  try {
    ...
    logger.error('Entity ownership verification failed:', error);
    return { isOwner: false };
  }
}
