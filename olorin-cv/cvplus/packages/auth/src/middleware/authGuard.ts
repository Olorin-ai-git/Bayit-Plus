...
enum AdminLevel {
  ...
  L3_ADMIN = 3,
  L4_SYSTEM = 4,
  L4_SUPER_ADMIN = 4, // Alias for backward compatibility
  L5_SYSTEM_ADMIN = 5
}
...
export interface AuthenticatedRequest extends Omit<CallableRequest, 'auth'> {
  auth: {
    ...
    rawToken?: string; // Required by CallableRequest auth property
  };
}
 ...
 * This maintains compatibility while transitioning from architectural violations
  */
export const requireAuthHTTP = async (req: AuthenticatedHTTPRequest, res: Response, next: () => void) => {
  try {
 ...
 * HTTP Admin authentication middleware
  */
export const requireAdminHTTP = async (req: AuthenticatedHTTPRequest, res: Response, next: () => void) => {
  try {
