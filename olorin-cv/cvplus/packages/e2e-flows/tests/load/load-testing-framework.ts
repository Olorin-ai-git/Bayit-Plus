...
export class LoadTestFramework extends EventEmitter {
  ...
  async executeLoadTest(
    ...
    logger.info(`Target: ${config.targetUsers} concurrent users`);
  ...
  private async executeSustainPhase(
    ...
    logger.info(`Sustaining ${config.targetUsers} users for ${config.sustainDuration} seconds`);
    ...
    results.forEach((result, index) => {
      if (result.status === 'fulfilled') {
        ...
        logger.error(`User sustain-${index} failed:`, result.reason);
      }
    });
  ...
  private async executeRampDownPhase(
    ...
    logger.info(`Ramp-down phase for ${config.name} - letting users complete naturally`);
    logger.info(`Test function available: ${typeof testFunction === 'function'}`);
    return [];
  }
  ...
  async stop(): Promise<void> {
    logger.info('Stopping load test framework...');
