...
export class LoadTestMonitor extends EventEmitter {
  ...
  startMonitoring(): void {
    ...
    logger.info('🔍 Load test monitoring started');
  }
  ...
  stopMonitoring(): void {
    ...
    logger.info('⏹️  Load test monitoring stopped');
  }
  ...
  displayRealtimeDashboard(): void {
    ...
    console.clear();
    logger.info('📊 LOAD TEST REAL-TIME DASHBOARD');
    logger.info('================================');
    logger.info(`⚡ Concurrent Users: ${latest.concurrentUsers}`);
    logger.info(`🔄 Requests/sec: ${latest.requestsPerSecond.toFixed(1)}`);
    logger.info(`⏱️  Avg Response: ${latest.averageResponseTime.toFixed(0)}ms`);
    logger.info(`❌ Error Rate: ${latest.errorRate.toFixed(2)}%`);
    logger.info(`💾 Memory Usage: ${latest.systemLoad.memory.usedPercent.toFixed(1)}%`);
    logger.info(`🖥️  CPU Load: ${latest.systemLoad.loadAverage[0].toFixed(2)}`);
    logger.info(`⏰ Time: ${new Date(latest.timestamp).toLocaleTimeString()}`);
    logger.info('================================');
    ...
    if (this.metrics.length > 1) {
      ...
      logger.info('📈 TRENDS:');
      logger.info(`   RPS: ${rpsChange >= 0 ? '↗️' : '↘️'} ${rpsChange.toFixed(1)}`);
      logger.info(`   Response: ${responseChange <= 0 ? '↗️' : '↘️'} ${responseChange.toFixed(0)}ms`);
    }
  }
}
...
export class ConcurrentUserSimulator {
  ...
  constructor(public readonly maxWorkers: number = cpus().length * 2) {
    logger.info(`User simulator initialized with max ${maxWorkers} workers`);
  }
  ...
  async simulateUsers(
    ...
    logger.info(`🚀 Starting ${userCount} concurrent users for ${duration} seconds`);
  ...
  private async simulateIndividualUser(
    ...
    while (Date.now() < endTime) {
      ...
      try {
        ...
        logger.error(`User ${userId} error:`, errorObj.message);
      }
...
`;
  ...
  static exportToJson(report: LoadTestReport, outputPath: string): void {
    ...
    logger.info(`📊 JSON report exported to: ${outputPath}`);
  }
}
...
export class LoadTestValidator {
  ...
  static validateSystemCapacity(targetUsers: number): { capable: boolean; warnings: string[] } {
    ...
    logger.info(`System capacity check: ${totalMemoryGB.toFixed(1)}GB total, ${freeMemoryGB.toFixed(1)}GB free`);
