...
class LoadTestRunner {
  ...
  private setupEventListeners(): void {
    ...
    this.monitor.on('systemStress', (metrics) => {
      logger.warn('🚨 SYSTEM STRESS DETECTED!');
      logger.warn(`   Memory: ${metrics.systemLoad.memory.usedPercent.toFixed(1)}%`);
      logger.warn(`   CPU Load: ${metrics.systemLoad.loadAverage[0].toFixed(2)}`);
      logger.warn('   Consider reducing load or scaling resources');
    });

    this.monitor.on('performanceDegradation', (metrics) => {
      logger.warn('📉 PERFORMANCE DEGRADATION DETECTED!');
      logger.warn(`   Error Rate: ${metrics.errorRate.toFixed(2)}%`);
      logger.warn(`   Response Time: ${metrics.averageResponseTime.toFixed(0)}ms`);
    });
  }
  ...
  async runScenario(scenario: string, options: LoadTestOptions): Promise<LoadTestResults> {
    ...
    try {
      switch (scenario.toLowerCase()) {
        ...
        case 'complete':
          logger.info('⚠️  Complete suite will take 2-3 hours to finish');
  ...
  private async runCustomScenario(options: LoadTestOptions): Promise<LoadTestResults> {
    ...
    if (!options.skipValidation) {
      ...
      if (!capacity.capable) {
        logger.warn('⚠️  System capacity warnings:');
        capacity.warnings.forEach(warning => logger.warn(`   • ${warning}`));
  ...
  private displayResultsSummary(results: LoadTestResults): void {
    ...
    logger.info(`🎯 Target Users: ${results.config.targetUsers.toLocaleString()}`);
    logger.info(`✅ Achieved Users: ${metrics.concurrentUsersAchieved.toLocaleString()}`);
    logger.info(`📊 Success Rate: ${((metrics.concurrentUsersAchieved / results.config.targetUsers) * 100).toFixed(1)}%`);
    logger.info();
    logger.info(`📈 Total Requests: ${metrics.totalRequests.toLocaleString()}`);
    logger.info(`✅ Successful: ${metrics.successfulRequests.toLocaleString()}`);
    logger.info(`❌ Failed: ${metrics.failedRequests.toLocaleString()}`);
    logger.info(`📊 Error Rate: ${metrics.errorRate.toFixed(2)}%`);
    logger.info();
    logger.info(`⏱️  Avg Response: ${metrics.averageResponseTime.toFixed(0)}ms`);
    logger.info(`📊 P95 Response: ${metrics.p95ResponseTime.toFixed(0)}ms`);
    logger.info(`🚀 Peak Throughput: ${metrics.peakThroughput.toFixed(1)} req/s`);
    logger.info(`📈 Sustained Throughput: ${metrics.sustainedThroughput.toFixed(1)} req/s`);
    logger.info();
    ...
    if (results.recommendations && results.recommendations.length > 0) {
      ...
      results.recommendations.forEach((rec, index) => {
        ...
        logger.info(`      ${rec.recommendation}`);
...
async function main(): Promise<void> {
  ...
  try {
    logger.info('🔧 CVPlus E2E Flows Load Test Runner');
    logger.info(`📅 Started at: ${new Date().toLocaleString()}`);
    logger.info(`🎯 Scenario: ${options.scenario}`);

    if (options.reportOnly) {
      logger.info('📊 Report-only mode: No tests will be executed');
      return;
    }
    ...
    if (options.scenario === 'stress' || (options.users && parseInt(options.users.toString()) >= 10000)) {
      ...
      if (
        ...
        logger.info('   System meets all target performance criteria');
      } else {
        logger.info('⚠️  PARTIAL SUCCESS: System approaches 10K capacity');
        logger.info('   Review recommendations for optimization');
      }
    }
