...
describe('Integration Performance Tests', () => {
  ...
  beforeAll((done) => {
    ...
    mockServer.listen(0, () => {
      ...
      logger.info(`Integration test mock server started on port ${mockServerPort}`);
      done();
    });
  });
  ...
  afterAll((done) => {
    ...
    logger.info(JSON.stringify(performanceResults, null, 2));
    ...
    logger.info(`Integration performance report saved to: ${reportPath}`);
  ...
  describe('Performance Benchmarks Validation', () => {
    test('should meet all integrated performance targets', () => {
      ...
      logger.info(`📊 Overall efficiency score: ${results.resourceEfficiency?.efficiencyScore?.toFixed(2) || 'N/A'}`);
    });
  });
