...
export class PerformanceTestRunner {
  ...
  public async runTest<T>(
    ...
    try {
      ...
      logger.info(`  Operations/sec: ${metrics.operationsPerSecond.toFixed(2)}`);
      logger.info(`  Error Rate: ${metrics.errorRate.toFixed(2)}%`);
      logger.info(`  Passed: ${thresholdCheck.passed ? '✅' : '❌'}`);

      if (!thresholdCheck.passed) {
        logger.info(`  Violations:`);
        thresholdCheck.violations.forEach(v => logger.info(`    - ${v}`));
      }
...
export class PerformanceUtils {
  ...
  public static async benchmark<T>(
    ...
    logger.info(`  Min Time: ${minTime.toFixed(2)}ms`);
    logger.info(`  Max Time: ${maxTime.toFixed(2)}ms`);
...
export const performanceAssertions = {
  ...
  expectDurationUnder: (actual: number, threshold: number, message?: string) => {
    ...
    if (message) {
      logger.info(`✅ ${message}: ${actual}ms < ${threshold}ms`);
    }
  },
  ...
  expectMemoryUnder: (actual: number, threshold: number, message?: string) => {
    expect(actual).toBeLessThan(threshold);
    if (message) {
      logger.info(`✅ ${message}: ${(actual / 1024 / 1024).toFixed(2)}MB < ${(threshold / 1024 / 1024).toFixed(2)}MB`);
    }
  },
  ...
  expectThroughputAbove: (actual: number, threshold: number, message?: string) => {
    ...
    if (message) {
      logger.info(`✅ ${message}: ${actual.toFixed(2)} ops/sec > ${threshold} ops/sec`);
    }
  },
  ...
  expectErrorRateUnder: (actual: number, threshold: number, message?: string) => {
    expect(actual).toBeLessThan(threshold);
    if (message) {
      logger.info(`✅ ${message}: ${actual.toFixed(2)}% < ${threshold}%`);
    }
  }
};