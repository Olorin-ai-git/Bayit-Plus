...
class PerformanceTestRunner {
  ...
  async runAllTests(): Promise<void> {
    logger.info('🚀 Starting CVPlus E2E Flows Performance Test Suite');
    ...
    try {
      ...
      logger.info(`Total execution time: ${(totalDuration / 1000 / 60).toFixed(2)} minutes`);
  ...
  private async runMockDataServiceTests(): Promise<void> {
    ...
    for (const test of tests) {
      try {
        ...
        logger.info(`    ❌ ${test.name} failed:`, (error as Error).message);
        serviceResults[test.name] = {
          passed: false,
  ...
  private async generateReports(): Promise<void> {
    ...
    try {
      ...
      logger.info(`💾 Raw Results: ${rawResultsPath}`);
