...
export const createRoleProfile: HttpsFunction = onRequest(
  ...
  async (request, response) => {
    try {
      // Note: Implementation pending for after migration from main functions
      ...
      logger.error('Error creating role profile:', error);
...
export const getRoleProfile: HttpsFunction = onRequest(
  ...
  async (request, response) => {
    try {
      ...
      logger.error('Error getting role profile:', error);
      response.status(500).json({
        error: 'Internal server error'
      });
    }
