...
export async function getJobData(jobId: string, userId: string): Promise<CVParsedData | null> {
  try {
    logger.info(`[JobProcessing] Retrieving CV data for job ${jobId}, user ${userId}`);
    ...
    if (!jobDoc.exists) {
      logger.warn(`[JobProcessing] Job ${jobId} not found`);
      return null;
    }
    ...
    if (jobData?.userId !== userId) {
      logger.warn(`[JobProcessing] Job ${jobId} does not belong to user ${userId}`);
      return null;
    }
    ...
    if (!cvData) {
      logger.warn(`[JobProcessing] No parsed CV data found for job ${jobId}`);
    ...
    logger.error(`[JobProcessing] Error retrieving CV data for job ${jobId}:`, error);
    return createMockCVData(); // Return mock data for development
  }
}
