...
// Import supportedLanguages directly instead of relying on type-only import
...
export function useTranslation(namespace?: string): UseTranslationReturn {
  ...
  // Get language safely - ensures consistent access pattern
  const language = i18n?.language || 'en';
  const currentLanguage = language as SupportedLanguage;

  // Safe language config access with fallback
  const defaultLangConfig = { dir: 'ltr' as const, locale: 'en-US' };
  const currentLangConfig = (supportedLanguages && supportedLanguages[currentLanguage])
