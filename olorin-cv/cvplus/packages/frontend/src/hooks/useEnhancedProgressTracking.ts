...
export const useEnhancedProgressTracking = ({
  ...
  const setupProgressTracking = useCallback((trackingJobId: string) => {
    ...
    logger.warn('🔄 [ENHANCED] Setting up progress tracking for job:', trackingJobId);
    logger.warn('📊 [ENHANCED] Tracking features:', trackingFeatures.map(f => f.name));
    ...
    const unsubscribe = jobSubscriptionManager.subscribeToProgress(
      ...
      (job: Job | null) => {
        if (!job || !isMountedRef.current) {
          logger.warn('📊 [ENHANCED] No job data or component unmounted');
          return;
        }
      ...
      logger.warn('📊 [ENHANCED] Real-time update received:', {
        status: data.status,
      ...
      trackingFeatures.forEach(feature => {
        ...
        if (featureData) {
          ...
          logger.warn(`📊 [ENHANCED] Feature ${feature.id}:`, {
      ...
      if (allCompleted) {
        logger.warn('🎉 [ENHANCED] All features completed!');
      ...
      logger.warn(`📊 [ENHANCED] Progress summary: ${completedCount}/${trackingFeatures.length} completed, ${failedCount} failed, ${processingCount} processing`);
      },
      {
        enableLogging: true,
