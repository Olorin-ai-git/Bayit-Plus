...
export const useCVGeneration = (jobId: string) => {
  ...
  const triggerCVGeneration = async (jobData: Job, generationConfig?: CVGenerationConfig) => {
    logger.warn('🎯 [DEBUG] triggerCVGeneration started');
    ...
    try {
      ...
      if (!configToUse) {
        ...
        if (storedConfig) {
          try {
            ...
            logger.error('❌ [DEBUG] Error parsing stored config:', error);
          }
        }
      }
      ...
      logger.warn('🔥 [DEBUG] Calling generateCV service with features:', selectedFeatures);
      ...
      if (podcastGeneration && isMountedRef.current) {
        try {
          ...
          logger.error('Podcast generation failed:', podcastError);
  ...
  const triggerQuickCreateWorkflow = async (jobData: Job) => {
    logger.warn('🎯 [DEBUG] triggerQuickCreateWorkflow started - generating with ALL features');
    ...
    try {
      ...
      logger.warn('🔥 [DEBUG] Quick Create: Calling generateCV with ALL features:', allFeatures);
      const result = await generateCV(jobData.id, 'modern', allFeatures);
      
      // Generate podcast for quick create
      if (isMountedRef.current) {
        try {
          logger.warn('🎙️ Quick Create: Generating podcast');
          await generateEnhancedPodcast(jobData.id, 'professional');
      ...
      const poll = async () => {
        ...
        try {
          ...
          if (updatedJob?.generatedCV?.html || updatedJob?.generatedCV?.htmlUrl) {
            logger.warn('✅ [DEBUG] CV generation completed');
            resolve(updatedJob);
          } else if (attempts < maxAttempts) {
            setTimeout(poll, pollInterval);
          } else {
            logger.error('❌ [DEBUG] CV generation timeout');
            reject(new Error('CV generation timed out. Please try again.'));
          }
        } catch (pollError) {
          logger.error('❌ [DEBUG] Error polling for CV completion:', pollError);
