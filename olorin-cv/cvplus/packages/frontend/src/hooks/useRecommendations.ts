...
export function useRecommendations(
  ...
  const loadRecommendations = useCallback(async (forceRefresh = false): Promise<RecommendationItem[]> => {
    ...
    if (!forceRefresh) {
      ...
      if (cached) {
        ...
        if (now - cached.timestamp < CACHE_DURATION && cached.data.length > 0) {
          logger.warn(`[useRecommendations] Using cached data for job ${jobId}`);
          return cached.data;
        }
        
        // Return existing loading promise if one is in progress
        if (cached.loading) {
          logger.warn(`[useRecommendations] Joining existing load for job ${jobId}`);
    ...
    const loadingPromise = (async (): Promise<RecommendationItem[]> => {
      try {
        logger.warn(`[useRecommendations] Loading recommendations for job ${jobId}`);
        ...
        logger.warn(`[useRecommendations] Loaded ${transformedRecommendations.length} recommendations for job ${jobId}`);
        return transformedRecommendations;
        
      } catch (err: unknown) {
        logger.error(`[useRecommendations] Error loading recommendations for job ${jobId}:`, err);
