...
export const useRetryManager = ({
  ...
  const retryFeature = useCallback(async (featureId: string): Promise<boolean> => {
    if (!jobId || activeRetries.has(featureId)) {
      logger.warn(`Cannot retry ${featureId}: ${!jobId ? 'No jobId' : 'Already retrying'}`);
      return false;
    }

    try {
      logger.warn(`🔄 [RETRY] Attempting to retry feature: ${featureId}`);
      ...
      logger.warn(`✅ [RETRY] Feature ${featureId} retry initiated successfully`);
      ...
      logger.error(`❌ [RETRY] Failed to retry feature ${featureId}:`, errorMessage);
  ...
  const autoRetryFeature = useCallback(async (featureId: string, error: string): Promise<void> => {
    ...
    logger.warn(`⏳ [AUTO-RETRY] Scheduling auto-retry for ${featureId} in ${delay}ms`);
    
    setTimeout(async () => {
      const success = await retryFeature(featureId);
      if (success) {
        logger.warn(`✅ [AUTO-RETRY] Auto-retry successful for ${featureId}`);
      } else {
        logger.warn(`❌ [AUTO-RETRY] Auto-retry failed for ${featureId}`);
      }
    }, delay);
