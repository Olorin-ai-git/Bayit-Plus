...
export const useJobEnhanced = (
  ...
  const setupSubscription = useCallback(() => {
    ...
    try {
      ...
      unsubscribeRef.current = jobSubscriptionManager.subscribeToJob(
        ...
        (updatedJob) => {
          if (updatedJob) {
            ...
            if (enableLogging) {
              logger.warn(`[useJobEnhanced] Job ${jobId} not found or subscription error`);
            }
      ...
      logger.error(`[useJobEnhanced] Failed to setup subscription for ${jobId}:`, err);
      handleSubscriptionError();
    }
  ...
  const startPolling = useCallback(() => {
    ...
    const poll = async () => {
      ...
      try {
        ...
        logger.error(`[useJobEnhanced] Polling error for job ${jobId}:`, err);
      }
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      logger.error('Error refreshing job:', err);
    }
  ...
  useEffect(() => {
    ...
    const loadJob = async () => {
      try {
        ...
        logger.error('Error loading job:', err);
