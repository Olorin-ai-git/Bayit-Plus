...
class NavigationDebugger {
  ...
  log(action: string, data: Partial<NavigationDebugInfo> = {}): void {
    ...
    logger.warn(`🔍 [NAV-DEBUG] ${action}:`, debugInfo);
  }
  ...
  clearLogs(): void {
    this.logs = [];
    logger.warn('🗑️ [NAV-DEBUG] Logs cleared');
  }

  printReport(): void {
    console.group('📊 [NAV-DEBUG] Navigation Report');
    logger.warn(`Total navigation events: ${this.logs.length}`);
    ...
    this.logs.slice(-10).forEach((log, index) => {
      ...
      logger.warn(`${status} ${log.timestamp} - ${log.action}`, {
        from: log.fromPath,
        to: log.toPath,
        jobId: log.jobId,
        error: log.error
