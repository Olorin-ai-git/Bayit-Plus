...
const withCVProcessingErrorBoundary = <T extends {}>(
  ...
  return (props: T) => {
    try {
      ...
      logger.error('CV Processing module error:', error);
...
const CVAnalysisResultsModule = lazy(() =>
  import('@cvplus/processing/frontend')
    ...
    .catch(error => {
      logger.error('Failed to load CVAnalysisResults from submodule:', error);
...
const GeneratedCVDisplayModule = lazy(() =>
  import('@cvplus/processing/frontend')
    ...
    .catch(error => {
      logger.error('Failed to load GeneratedCVDisplay from submodule:', error);
...
const LivePreviewModule = lazy(() =>
  import('@cvplus/processing/frontend')
    ...
    .catch(error => {
      logger.error('Failed to load LivePreview from submodule:', error);
...
const CVComparisonViewModule = lazy(() =>
  import('@cvplus/processing/frontend')
    ...
    .catch(error => {
      logger.error('Failed to load CVComparisonView from submodule:', error);
...
export const monitorCVProcessingPerformance = (componentName: string) => {
  ...
  return {
    recordLoadTime: () => {
      ...
      logger.info(`CV Processing ${componentName} loaded in ${loadTime}ms`);
