...
export class FirestoreListenerValidator {
  ...
  startMonitoring(intervalMs = 10000): void {
    ...
    logger.warn('🔍 [FirestoreValidator] Starting listener monitoring...');
  ...
  stopMonitoring(): void {
    ...
    logger.warn('🔍 [FirestoreValidator] Stopped listener monitoring');
  }
  ...
  validateListeners(): ValidationReport {
    ...
    this.validationCallbacks.forEach(callback => {
      try {
        ...
        logger.error('[FirestoreValidator] Callback error:', error);
      }
    });
  ...
  private logValidationResults(report: ValidationReport): void {
    ...
    if (issues.length > 0) {
      logger.warn(`🔍 [FirestoreValidator] Health Score: ${healthScore}/100 - Issues detected:`);
      
      issues.forEach(issue => {
        const severity = issue.severity === 'CRITICAL' ? '🚨' : 
                        issue.severity === 'HIGH' ? '⚠️' : 
                        issue.severity === 'MEDIUM' ? '⚡' : 'ℹ️';
        logger.warn(`${severity} [${issue.type}] ${issue.message}`);
        ...
        if (issue.suggestedActions.length > 0) {
          logger.warn('   Actions:', issue.suggestedActions.join(', '));
        }
      });
    } else {
      logger.warn(`🔍 [FirestoreValidator] ✅ All listeners healthy (Score: ${healthScore}/100)`);
    }

    logger.warn('🔍 [FirestoreValidator] System Stats:', {
  ...
  private handleCriticalIssues(issues: ValidationIssue[]): void {
    ...
    if (criticalIssues.length > 0 && !this.emergencyCleanupInProgress) {
      ...
      if (duplicateListenerIssues.length > 0) {
        logger.warn('🔍 [FirestoreValidator] 🚨 CRITICAL: Auto-triggering emergency cleanup due to duplicate listeners');
        this.handleEmergencyCleanup();
      }
    }
  }
  ...
  private handleEmergencyCleanup(): void {
    if (this.emergencyCleanupInProgress) {
      logger.warn('🔍 [FirestoreValidator] Emergency cleanup already in progress');
      return;
    }

    this.emergencyCleanupInProgress = true;
    logger.warn('🔍 [FirestoreValidator] 🚨 EMERGENCY CLEANUP INITIATED');
    ...
    try {
      ...
      setTimeout(() => {
        ...
        logger.warn('🔍 [FirestoreValidator] 🚨 EMERGENCY CLEANUP COMPLETED');
        logger.warn('🔍 [FirestoreValidator] Pre-cleanup:', preCleanupReport);
        logger.warn('🔍 [FirestoreValidator] Post-cleanup:', postCleanupReport);
        ...
        setTimeout(() => {
          logger.warn('🔍 [FirestoreValidator] Restarting monitoring after cleanup...');
      ...
      logger.error('🔍 [FirestoreValidator] Emergency cleanup failed:', error);
      this.emergencyCleanupInProgress = false;
    }
  }
   ...
   */
  forceCleanup(): void {
    logger.warn('🔍 [FirestoreValidator] 🚨 LEGACY FORCE CLEANUP - Use handleEmergencyCleanup for enhanced cleanup');
    this.handleEmergencyCleanup();
  }
  ...
  clearIssueHistory(): void {
    this.issueHistory = [];
    logger.warn('🔍 [FirestoreValidator] Issue history cleared');
  }
  ...
  updateThresholds(newThresholds: Partial<typeof this.alertThresholds>): void {
    ...
    logger.warn('🔍 [FirestoreValidator] Alert thresholds updated:', this.alertThresholds);
  }
...
if (typeof window !== 'undefined' && process.env.NODE_ENV === 'development') {
  ...
  window._firestoreDebug = {
    ...
    quickCheck: () => {
      ...
      logger.warn('🔍 Quick Health Check:', result);
      return result;
    },
    fullReport: () => {
      const report = validator.getValidationReport();
      logger.warn('🔍 Full Validation Report:', report);
      return report;
    },
    cleanup: () => {
    ...
    issues: () => {
      ...
      logger.warn('🔍 Recent Issues:', issues);
      return issues;
    }
  };
  
  logger.warn('🔍 [FirestoreValidator] Enhanced debugging available:');
  logger.warn('  - window.firestoreValidator (full API)');
  logger.warn('  - window._firestoreDebug.quickCheck() (health summary)');
  logger.warn('  - window._firestoreDebug.fullReport() (detailed report)');
  logger.warn('  - window._firestoreDebug.cleanup() (emergency cleanup)');
  logger.warn('  - window._firestoreDebug.issues() (recent issues)');
}
