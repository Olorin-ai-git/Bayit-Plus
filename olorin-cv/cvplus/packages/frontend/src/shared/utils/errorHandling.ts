...
const logger = {
  logError: (message: string, error: Error, data?: any) => logger.error(message, error, data),
  logDebug: (message: string, data?: any) => logger.debug(message, data),
  logInfo: (message: string, data?: any) => console.info(message, data),
  logWarning: (message: string, data?: any) => logger.warn(message, data),
  logCritical: (message: string, error: Error, data?: any) => logger.error('[CRITICAL]', message, error, data),
};
...
export function logError(context: string, error: unknown, additionalData?: Record<string, unknown>): void {
  ...
  if (process.env.NODE_ENV === 'development') {
    ...
    if (isError(error)) {
      logger.error(`[${context}] Error:`, error.message, error.stack, logData);
    } else if (isFirebaseError(error)) {
      logger.error(`[${context}] Firebase Error [${error.code}]:`, error.message, logData);
    } else {
      logger.error(`[${context}] Unknown Error:`, errorMessage, logData);
    }
  }
}
