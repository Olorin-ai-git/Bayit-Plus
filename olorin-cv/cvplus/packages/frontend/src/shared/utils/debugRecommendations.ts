...
class RecommendationsDebugger {
  ...
  trackCall(jobId: string, caller = 'unknown', blocked = false, requestKey?: string): void {
    ...
    if (blocked) {
      logger.warn(`🚫 [RecommendationsDebugger] BLOCKED request for job ${jobId} from ${caller}`);
      logger.warn(`🚫 [RecommendationsDebugger] Request key: ${requestKey}`);
    } else {
      logger.warn(`✅ [RecommendationsDebugger] EXECUTING request for job ${jobId} from ${caller}`);
      logger.warn(`✅ [RecommendationsDebugger] Request key: ${requestKey}`);
    }
    ...
    if (now - this.lastCallTime < 1000 && this.calls.length > 1) {
      ...
      logger.warn(`🚨 MULTIPLE CALLS DETECTED! ${recentCalls.length} total calls within 1 second:`);
      logger.warn(`   - ${actualCalls.length} actual requests`);
      logger.warn(`   - ${blockedCalls.length} blocked requests`);
      
      recentCalls.forEach((call, index) => {
        const status = call.blocked ? '🚫 BLOCKED' : '✅ EXECUTED';
        logger.warn(`  ${status} Call ${index + 1}:`, {
          caller: call.caller,
          jobId: call.jobId,
...
if (typeof window !== 'undefined') {
  ...
  (window as any).getRequestDebugInfo = () => {
    ...
    import('../services/cv/CVAnalyzer').then(({ CVAnalyzer }) => {
      ...
      logger.warn('🔍 Current Request Debug Info:', debugInfo);
      
      const stats = recommendationsDebugger.getStats();
      logger.warn('📊 Call Statistics:', stats);
  ...
  (window as any).clearAllRequestTracking = () => {
    ...
    import('../services/cv/CVAnalyzer').then(({ CVAnalyzer }) => {
      ...
      logger.warn('🧹 All request tracking cleared');
    }).catch(console.error);
  };
}