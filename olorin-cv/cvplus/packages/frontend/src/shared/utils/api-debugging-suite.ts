...
export class FirebaseDebugger {
  ...
  private static async testFirebaseFunctions(results: DebuggingResults): Promise<void> {
    ...
    try {
      ...
      if (!functions) {
        ...
        logger.error('❌ Firebase Functions not initialized');
        return;
      }
      ...
      logger.error('❌ Firebase Functions test failed:', error);
    } finally {
      console.groupEnd();
    }
  }
  ...
  private static async testCallableFunction(results: DebuggingResults, jobId: string): Promise<void> {
    ...
    try {
      ...
      logger.info('✅ Callable function works', testResponse.data);
  ...
  private static async testHttpFallback(results: DebuggingResults, jobId: string): Promise<void> {
    ...
    try {
      ...
      logger.info('🧪 Testing HTTP endpoint:', `${baseUrl}/getRecommendations`);
      ...
      logger.info('✅ HTTP fallback works', data);
  ...
  private static async testBackendFunction(results: DebuggingResults): Promise<void> {
    ...
    try {
      ...
      if (response.status === 404) {
        ...
        logger.error('❌ Backend function not found');
      } else {
        logger.info('✅ Backend function endpoint exists', {
          status: response.status,
      ...
      logger.error('❌ Backend function test failed:', error);
    } finally {
      console.groupEnd();
    }
  }
  ...
  private static generateRecommendations(results: DebuggingResults): void {
    ...
    logger.info('Overall Success:', results.overall.success);
    logger.info('Critical Issues:', issues);
    logger.info('Recommendation:', results.overall.recommendation);
    console.groupEnd();
  }
  ...
  static async quickDebugTest(jobId: string): Promise<{ success: boolean; error?: string; details: any }> {
    try {
      logger.info('🚀 Quick Debug Test Started');
...
export const debugRecommendationsCall = async (jobId: string) => {
  ...
  if (!debugResult.success) {
    logger.error('❌ Debug test failed:', debugResult.error);
    logger.error('Details:', debugResult.details);
  } else {
    logger.info('✅ Debug test passed:', debugResult.details);
  }
