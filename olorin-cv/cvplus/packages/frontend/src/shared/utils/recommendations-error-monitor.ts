...
class RecommendationsErrorMonitor {
  ...
  static logError(error: any, context: {
    ...
    if (context.response) {
      logger.error('API Response:', context.response);
    }
    console.groupEnd();
    ...
    try {
      ...
      logger.warn('Could not save error to localStorage:', e);
    }
  }
  ...
  static clearErrors(): void {
    ...
    try {
      ...
      logger.warn('Could not clear errors from localStorage:', e);
    }
  }
  ...
  static loadStoredErrors(): void {
    try {
      ...
      logger.warn('Could not load stored errors:', e);
    }
  }
  ...
  static monitorResponse(response: any, jobId: string): void {
    ...
    if (!hasSuccess) {
      logger.warn('⚠️ Response missing "success" field');
    }

    if (!hasDirectRecommendations && !hasNestedRecommendations) {
      logger.warn('⚠️ Response missing "recommendations" field in expected locations');
    }

    if (hasSuccess && response.success && !hasDirectRecommendations && !hasNestedRecommendations) {
      logger.warn('⚠️ Success=true but no recommendations found');
    }
