...
class EnhancedLogger {
  ...
  error(message: string, ...args: any[]) {
    if (this.legacyLogLevel >= LogLevel.ERROR) {
      logger.error(`❌ [CVPlus] ${message}`, ...args);
    }
  ...
  warn(message: string, ...args: any[]) {
    if (this.legacyLogLevel >= LogLevel.WARN) {
      logger.warn(`⚠️ [CVPlus] ${message}`, ...args);
    }
  ...
  debug(message: string, ...args: any[]) {
    if (this.isDevelopment && this.legacyLogLevel >= LogLevel.DEBUG) {
      logger.debug(`🔍 [CVPlus] ${message}`, ...args);
    }
  ...
  test(testName: string) {
    return {
      start: (message: string) => this.isDevelopment && logger.warn(`🧪 [TEST:${testName}] ${message}`),
      success: (message: string) => this.isDevelopment && logger.warn(`✅ [TEST:${testName}] ${message}`),
      error: (message: string, ...args: any[]) => logger.error(`❌ [TEST:${testName}] ${message}`, ...args),
      info: (message: string) => this.isDevelopment && logger.warn(`📝 [TEST:${testName}] ${message}`)
    };
  }
