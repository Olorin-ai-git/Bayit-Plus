...
export const sanitizeAndValidateCVContent = (
  ...
  try {
    ...
    if (!validation.success) {
      logger.warn(`Content validation failed for ${type}:`, validation.error);
...
export const sanitizeCVData = (data: any): any => {
  try {
    ...
    if (!validation.success) {
      logger.warn('CV data validation failed:', validation.error);
    ...
    logger.error('CV data sanitization failed:', error);
