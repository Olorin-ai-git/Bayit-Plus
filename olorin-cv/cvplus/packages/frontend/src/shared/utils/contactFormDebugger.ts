...
export function debugContactFormRendering(container?: Element): DebugInfo {
  ...
  try {
    ...
    logger.warn(`🔍 [DEBUG] Found ${placeholders.length} placeholders in ${container ? 'container' : 'document'}`);
    ...
    placeholders.forEach((placeholder, index) => {
      ...
      logger.warn(`📦 [DEBUG] Placeholder ${index + 1}:`, {
        componentName,
        hasProps: !!propsAttr,
      ...
      if (componentName === 'ContactForm') {
        logger.warn(`🎯 [DEBUG] ContactForm placeholder found:`, {
          parsedProps,
    ...
    if (loadingSpinners.length > 0) {
      ...
      logger.warn(`⏳ [DEBUG] Found ${loadingSpinners.length} loading spinners`);
    }

  } catch (error) {
    debugInfo.errors.push(`Debug function failed: ${error}`);
    logger.error('🚨 [DEBUG] Debug function failed:', error);
  }
...
export function forceInitializeWithDebug(container?: Element): void {
  logger.warn('🚀 [DEBUG] Force initializing React components...');
  ...
  if (debugInfo.rendererAvailable) {
    try {
      if (container) {
        ...
        logger.warn(`🎯 [DEBUG] Manually rendering ${placeholders.length} components in container`);
          ...
          if (componentName && (window as any).renderReactComponent) {
            ...
            if (propsJson) {
              try {
                ...
                logger.error(`🚨 [DEBUG] Failed to parse props for ${componentName}:`, error);
                return;
              }
            }
            
            logger.warn(`🔄 [DEBUG] Rendering ${componentName} with props:`, props);
      ...
      logger.error('🚨 [DEBUG] Failed to force initialize:', error);
    }
  } else {
    logger.error('🚨 [DEBUG] Component renderer not available - check if componentRenderer.ts is loaded');
  }
}
...
export function initializeWithDebug(container?: Element, retryCount = 0): void {
  ...
  logger.warn(`🔄 [DEBUG] Initialize attempt ${retryCount + 1}/${maxRetries + 1}`);
  ...
  if (retryCount === 0) {
    ...
    setTimeout(() => {
      logger.warn(`🔄 [DEBUG] Retry ${retryCount} after ${delay}ms delay`);
      forceInitializeWithDebug(container);
      ...
      setTimeout(() => {
        ...
        if (remainingPlaceholders.length > 0 && retryCount < maxRetries) {
          logger.warn(`⚠️ [DEBUG] Still ${remainingPlaceholders.length} loading placeholders, retrying...`);
    ...
    logger.error('🚨 [DEBUG] Max retries exceeded, component initialization failed');
