...
export class MemoryMonitor {
  ...
  start(): void {
    ...
    if (!this.isMemoryAPIAvailable()) {
      logger.warn('Memory API not available - monitoring disabled');
      return;
    }
    ...
    logger.warn('Memory monitoring started');
  }
  ...
  private emitAlert(alert: MemoryAlert): void {
    logger.warn(`Memory Alert [${alert.type}]: ${alert.message}`);
...
export function initializeMemoryMonitoring(): void {
  ...
  memoryMonitor.onAlert((alert) => {
    // Log to console
    logger.warn(`Memory Alert: ${alert.message}`, alert);
  ...
  logger.warn('Memory monitoring initialized');
}
