...
export const safeConsole = {
  log: (message: string, obj?: unknown): void => {
    if (obj !== undefined) {
      try {
        logger.warn(message, JSON.stringify(obj, null, 2));
      } catch (error) {
        logger.warn(message, '[Object could not be serialized]', obj);
      }
    } else {
      logger.warn(message);
    }
  ...
  error: (message: string, obj?: unknown): void => {
    if (obj !== undefined) {
      try {
        logger.error(message, JSON.stringify(obj, null, 2));
      } catch (error) {
        logger.error(message, '[Object could not be serialized]', obj);
      }
    } else {
