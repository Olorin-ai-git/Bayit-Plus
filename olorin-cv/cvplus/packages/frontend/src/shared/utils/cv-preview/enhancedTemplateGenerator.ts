...
export class EnhancedTemplateGenerator {
  ...
  static async generateHTML(
    ...
    if (!securityAudit.isSecure) {
      logger.warn('Security violations detected:', securityAudit.violations);
    }
    ...
    try {
      ...
      logger.warn('🎨 [ENHANCED GENERATOR] Processing template:', {
      ...
      logger.warn(`✅ [ENHANCED GENERATOR] Template generated successfully in ${generationTime.toFixed(2)}ms`);
      ...
      logger.error(`❌ [ENHANCED GENERATOR] Template generation failed for ${templateId}:`, error);
  ...
  private static async generateEnhancedTemplate(
    ...
    logger.warn(`🎨 [ENHANCED TEMPLATE] Generating ${template.name} (${template.category})`);
...
logger.warn('🚀 Enhanced Template Generator loaded and ready');