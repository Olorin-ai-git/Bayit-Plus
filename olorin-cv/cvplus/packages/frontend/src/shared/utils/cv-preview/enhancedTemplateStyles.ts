...
export class EnhancedTemplateStyles {
  ...
  static generateTemplateCSS(template: CVTemplate): string {
    ...
    logger.warn(`🎨 [TEMPLATE STYLES] Generating CSS for ${template.name}`);
  ...
  static clearCache(): void {
    ...
    logger.warn('🧹 Template styles cache cleared');
  }
...
export function applyTemplateTheme(template: CVTemplate, element?: HTMLElement): void {
  ...
  logger.warn(`🎨 Applied ${template.name} theme with ${Object.keys(properties).length} CSS properties`);
}
...
export function removeTemplateTheme(template: CVTemplate, element?: HTMLElement): void {
  ...
  logger.warn(`🧹 Removed ${template.name} theme`);
}

logger.warn('🎨 Enhanced Template Styles system loaded');
