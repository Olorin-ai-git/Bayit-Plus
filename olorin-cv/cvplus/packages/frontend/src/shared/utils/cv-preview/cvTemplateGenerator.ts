...
export class CVTemplateGenerator {
  ...
  static async generateHTML(
    ...
    logger.warn('🎨 [CV TEMPLATE GENERATOR] Generating template:', selectedTemplate);
    ...
    try {
      ...
      logger.warn('⚠️ [CV TEMPLATE GENERATOR] Compatibility layer failed, falling back to legacy generator:', error);
  ...
  static generateLegacyHTML(
    ...
    logger.warn('🔧 [LEGACY GENERATOR] Generating legacy template HTML');
