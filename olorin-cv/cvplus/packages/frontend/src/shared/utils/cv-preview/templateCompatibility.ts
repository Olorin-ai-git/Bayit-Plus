...
export class TemplateCompatibility {
  ...
  static async generateHTML(
    ...
    logger.warn(`🔄 [TEMPLATE COMPATIBILITY] Using ${generatorType} generator for template: ${templateId}`);
  ...
  static migrateLegacyTemplate(legacyTemplate: LegacyTemplate): CVTemplate {
    logger.warn(`🔄 [TEMPLATE MIGRATION] Migrating legacy template: ${legacyTemplate.id}`);
    ...
    logger.warn(`✅ [TEMPLATE MIGRATION] Successfully migrated ${legacyTemplate.id} to ${category} template`);
  ...
  static clearCompatibilityData(): void {
    ...
    logger.warn('🧹 Template compatibility data cleared');
  }
}
...
logger.warn('🔄 Template Compatibility Layer loaded');
