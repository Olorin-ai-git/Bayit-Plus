...
export class EnhancedCacheManager {
  ...
  private initializeCaches(): void {
    ...
    logger.warn('Enhanced cache manager initialized');
  }
  ...
  invalidate(pattern: string | RegExp, cacheType?: 'css' | 'template' | 'metadata'): number {
    ...
    if (invalidatedCount > 0) {
      logger.warn(`Invalidated ${invalidatedCount} cache entries matching pattern: ${pattern}`);
    }
  ...
  async warmCache(templateIds: string[]): Promise<void> {
    logger.warn(`Warming cache for ${templateIds.length} templates...`);
    ...
    const warmPromises = templateIds.map(async (templateId) => {
      try {
        ...
        logger.warn(`Failed to warm cache for template ${templateId}:`, error);
      }
    });
    
    await Promise.allSettled(warmPromises);
    logger.warn('Cache warming completed');
  }
  ...
  cleanup(): {
    ...
    logger.warn('Cache cleanup completed:', result);
    return result;
  }
  ...
  releaseMemoryPressure(): {
    ...
    logger.warn('Memory pressure relief completed:', result);
    return result;
  }
  ...
  destroy(): void {
    ...
    logger.warn('Enhanced cache manager destroyed');
  }
  ...
  private initializeMonitoring(): void {
    ...
    memoryMonitor.onAlert((alert) => {
      if (alert.type === 'critical' || alert.type === 'leak_detected') {
        logger.warn('Memory alert received, performing cache cleanup:', alert);
  ...
  private checkMemoryPressure(): void {
    ...
    if (memoryUsagePercent > 85) {
      logger.warn(`High cache memory usage: ${memoryUsagePercent.toFixed(1)}%`);
      this.releaseMemoryPressure();
    }
  }
...
export function initializeEnhancedCaching(config?: Partial<CacheConfig>): void {
  ...
  logger.warn('Enhanced caching system initialized');
}
