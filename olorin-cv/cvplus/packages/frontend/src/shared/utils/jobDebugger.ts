...
export const debugJobState = (job: Job | null, context = 'UNKNOWN') => {
  if (!job) {
    logger.warn(`🐛 [${context}] No job data available`);
    return;
  }
  ...
  logger.warn(`🐛 [${context}] Job Debug Info:`, debugInfo);
  ...
  if (issues.length > 0) {
    logger.warn(`⚠️ [${context}] Potential issues detected:`, issues);
  } else {
    logger.warn(`✅ [${context}] Job state looks healthy`);
  }
...
export const shouldDisplayCV = (job: Job | null): boolean => {
  ...
  logger.warn(`🤔 [CV-DISPLAY-CHECK] Should display CV?`, {
    hasContent,
    isCompleted,
    hasGeneratedCV,
