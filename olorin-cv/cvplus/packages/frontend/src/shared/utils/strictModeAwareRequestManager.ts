...
export class StrictModeAwareRequestManager {
  ...
  private constructor() {
    ...
    logger.warn(`[StrictModeAwareRequestManager] Initialized - StrictMode: ${this.isStrictMode}`);
  }
  ...
  async executeOnce<T>(
    ...
    logger.warn(`[StrictModeAware] Request #${callCount} for key: ${key}`, {
      context,
    ...
    if (!forceRegenerate && this.requestResults.has(key)) {
      ...
      if (now - cached.timestamp < this.CACHE_DURATION) {
        logger.warn(`[StrictModeAware] Returning cached result for ${key}`, {
    ...
    if (isLikelyStrictModeDuplicate) {
      logger.warn(`[StrictModeAware] 🚨 STRICTMODE DUPLICATE DETECTED for ${key}`, {
      ...
      if (this.activeRequests.has(key)) {
        logger.warn(`[StrictModeAware] Waiting for active request to complete...`);
        try {
          ...
          logger.warn(`[StrictModeAware] Active request failed, proceeding with new request`);
        }
      } else {
        // No active request, but this is likely a duplicate
        // In StrictMode, we still proceed but log it
        logger.warn(`[StrictModeAware] StrictMode duplicate but no active request, proceeding`);
      }
    }
    
    // Check for active request (non-StrictMode or non-duplicate case)
    if (this.activeRequests.has(key) && !isLikelyStrictModeDuplicate) {
      logger.warn(`[StrictModeAware] Active request exists, waiting for completion`);
      try {
        const result = await this.activeRequests.get(key)!;
        return {
          data: result as T,
    ...
    // Create new request
    logger.warn(`[StrictModeAware] Starting new request for ${key}`);
    ...
    try {
      ...
      logger.warn(`[StrictModeAware] Request completed successfully for ${key}`, {
      ...
      logger.error(`[StrictModeAware] Request failed for ${key}:`, error);
      throw error;
  ...
  private cleanupExpiredCache(): void {
    ...
    if (cleanedCount > 0) {
      logger.warn(`[StrictModeAware] Cleaned up ${cleanedCount} expired cache entries`);
    }
  }
  ...
  private cleanupOldTimestamps(): void {
    ...
    if (cleanedCount > 0) {
      logger.warn(`[StrictModeAware] Cleaned up ${cleanedCount} old timestamps`);
    }
  }
  ...
  clearKey(key: string): void {
    ...
    logger.warn(`[StrictModeAware] Cleared all caches for key: ${key}`);
  }
