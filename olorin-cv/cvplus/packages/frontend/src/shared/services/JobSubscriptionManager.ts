...
export class JobSubscriptionManager {
  ...
  private setupApplicationShutdownHandlers(): void {
    ...
    if (typeof process !== 'undefined') {
      ...
      process.on('uncaughtException', (error) => {
        logger.error('[JobSubscriptionManager] Uncaught exception, shutting down:', error);
        if (JobSubscriptionManager.instance) {
          JobSubscriptionManager.instance.shutdown();
        }
      });
  ...
  public subscribeToJob(
    ...
    if (this.isShuttingDown) {
      if (opts.enableLogging) {
        logger.warn(`[JobSubscriptionManager] Ignoring subscription to ${jobId} - shutting down`);
      }
    ...
    if (!subscriptionRateLimiter.isAllowed(jobId)) {
      ...
      logger.warn(`[JobSubscriptionManager] Rate limit exceeded for job ${jobId}. Try again in ${timeUntilReset}ms`);
    ...
    if (opts.enableLogging) {
      logger.warn(`[JobSubscriptionManager] Subscribing to job: ${jobId}`);
    }
  ...
  public forceRefresh(jobId: string): void {
    const subscription = this.subscriptions.get(jobId);
    if (subscription) {
      logger.warn(`[JobSubscriptionManager] Force refreshing job: ${jobId}`);
  ...
  private checkMemoryUsage(): void {
    ...
    if (process.env.NODE_ENV === 'development') {
      logger.warn('[JobSubscriptionManager] Memory Stats:', stats);
    }
    
    // Warn about potential memory issues
    if (stats.subscriptionsCount > 100) {
      logger.warn(`[JobSubscriptionManager] High subscription count: ${stats.subscriptionsCount}`);
    }
    
    if (stats.debounceTimersCount > 50) {
      logger.warn(`[JobSubscriptionManager] High debounce timer count: ${stats.debounceTimersCount}`);
    }
    
    if (stats.cleanupTimersCount > 20) {
      logger.warn(`[JobSubscriptionManager] High cleanup timer count: ${stats.cleanupTimersCount}`);
    }
    
    if (stats.memoryUsageKB > 10240) { // 10MB threshold
      logger.warn(`[JobSubscriptionManager] High estimated memory usage: ${stats.memoryUsageKB}KB`);
   ...
   */
  public cleanup(): void {
    logger.warn(`[JobSubscriptionManager] Starting comprehensive cleanup of ${this.subscriptions.size} subscriptions`);
    ...
    try {
      ...
      for (const subscription of this.subscriptions.values()) {
        try {
          ...
          logger.error('[JobSubscriptionManager] Error unsubscribing:', error);
        }
      }
      ...
      for (const [key, timer] of this.debounceTimers.entries()) {
        try {
          ...
          logger.error(`[JobSubscriptionManager] Error clearing debounce timer ${key}:`, error);
        }
      }
      ...
      for (const [jobId, cleanupTimer] of this.cleanupTimers.entries()) {
        try {
          ...
          logger.error(`[JobSubscriptionManager] Error clearing cleanup timer for job ${jobId}:`, error);
        }
      }
      ...
      for (const interval of this.intervals) {
        try {
          ...
          logger.error('[JobSubscriptionManager] Error clearing interval:', error);
        }
      }
      ...
      if (this.memoryMonitoringInterval) {
        try {
          ...
          logger.error('[JobSubscriptionManager] Error clearing memory monitoring interval:', error);
        }
      }
      ...
      logger.warn('[JobSubscriptionManager] Comprehensive cleanup completed successfully');
    } catch (error) {
      logger.error('[JobSubscriptionManager] Error during cleanup:', error);
    }
  }
  ...
  public shutdown(): void {
    ...
    logger.warn('[JobSubscriptionManager] Initiating graceful shutdown');
    
    // Log final memory stats before shutdown
    const finalStats = this.getMemoryStats();
    logger.warn('[JobSubscriptionManager] Final memory stats before shutdown:', finalStats);
  ...
  private createNewSubscription(jobId: string, options: Required<SubscriptionOptions>): JobSubscription {
    if (options.enableLogging) {
      logger.warn(`[JobSubscriptionManager] Creating new Firestore subscription for job: ${jobId}`);
    }
      ...
      (docSnapshot) => {
        try {
          ...
          if (options.enableLogging && docSnapshot.exists() && hasActualChange) {
            logger.warn(`[JobSubscriptionManager] Job update for ${jobId}:`, docSnapshot.data()?.status);
          }
          ...
          logger.error(`[JobSubscriptionManager] Error processing job update for ${jobId}:`, error);
          subscription.errorCount++;
          
          // If too many errors, mark as inactive
          if (subscription.errorCount >= options.maxRetries) {
            logger.warn(`[JobSubscriptionManager] Max errors reached for job ${jobId}, marking as inactive`);
            subscription.isActive = false;
          }
        }
      },
      (error) => {
        logger.error(`[JobSubscriptionManager] Firestore error for job ${jobId}:`, error);
        subscription.errorCount++;
        ...
        for (const [callback, registration] of subscription.callbacks) {
          try {
            ...
            logger.error(`[JobSubscriptionManager] Callback error for job ${jobId}:`, callbackError);
          }
        }
      }
    );
  ...
  private unsubscribeCallback(jobId: string, callback: (job: Job | null) => void, enableLogging?: boolean): void {
    ...
    if (enableLogging) {
      logger.warn(`[JobSubscriptionManager] Unsubscribed from job: ${jobId}, remaining callbacks: ${subscription.callbacks.size}`);
    }
      ...
      const timerId = setTimeout(() => {
        try {
          ...
          if (currentSubscription && currentSubscription.callbacks.size === 0) {
            if (enableLogging) {
              logger.warn(`[JobSubscriptionManager] Cleaning up unused subscription for job: ${jobId}`);
            }
            ...
            try {
              ...
              logger.error(`[JobSubscriptionManager] Error unsubscribing from job ${jobId}:`, error);
            }
          ...
          logger.error(`[JobSubscriptionManager] Error during delayed cleanup for job ${jobId}:`, error);
  ...
  private debouncedCallback(
    ...
    if (existingTimer) {
      try {
        ...
        logger.error(`[JobSubscriptionManager] Error clearing existing debounce timer for ${key}:`, error);
      }
    }
    ...
    const timer = setTimeout(() => {
      try {
        ...
        logger.error(`[JobSubscriptionManager] Callback execution error for job ${jobId}:`, error);
      } finally {
        // Always clean up the timer reference, even on error
        try {
          this.debounceTimers.delete(key);
        } catch (deleteError) {
          logger.error(`[JobSubscriptionManager] Error deleting debounce timer ${key}:`, deleteError);
        }
      }
    }, debounceMs);
  ...
  private cleanupInactiveSubscriptions(): void {
    ...
    for (const jobId of jobsToClean) {
      try {
        const subscription = this.subscriptions.get(jobId);
        if (subscription) {
          logger.warn(`[JobSubscriptionManager] Cleaning up inactive subscription for job: ${jobId}`);
        ...
        logger.error(`[JobSubscriptionManager] Error cleaning up subscription for job ${jobId}:`, error);
      }
    }

    if (cleanedCount > 0) {
      logger.warn(`[JobSubscriptionManager] Cleaned up ${cleanedCount} inactive subscriptions`);
      
      // Log memory stats after cleanup in development
      if (process.env.NODE_ENV === 'development') {
        const memStats = this.getMemoryStats();
        logger.warn('[JobSubscriptionManager] Memory stats after cleanup:', memStats);
      }
    }
  ...
  private cleanupOrphanedTimers(): void {
    ...
    for (const jobId of timersToRemove) {
      ...
      if (cleanupTimer) {
        try {
          ...
          logger.error(`[JobSubscriptionManager] Error clearing orphaned timer for job ${jobId}:`, error);
        }
        this.cleanupTimers.delete(jobId);
      }
    }
    ...
    if (timersToRemove.length > 0) {
      logger.warn(`[JobSubscriptionManager] Cleaned up ${timersToRemove.length} orphaned cleanup timers`);
    }
  }

  private logSubscriptionStats(): void {
    const stats = this.getStats();
    logger.warn('[JobSubscriptionManager] Stats:', {
