...
export class CheckpointManager {
  ...
  public async createCheckpoint(
    ...
    logger.warn(`Checkpoint created: ${checkpointId} for job ${jobId} at ${type}`);
    return checkpoint;
  }
  ...
  public async getCheckpointByType(
    ...
    try {
      ...
      logger.error('Error getting checkpoint by type:', error);
      return null;
    }
  }
  ...
  public async getJobCheckpoints(jobId: string): Promise<ProcessingCheckpoint[]> {
    ...
    try {
      ...
      logger.error('Error getting job checkpoints:', error);
      return [];
    }
  }
  ...
  public async deleteJobCheckpoints(jobId: string): Promise<boolean> {
    try {
      ...
      logger.warn(`Deleted ${checkpoints.length} checkpoints for job ${jobId}`);
      return true;
    } catch (error) {
      logger.error('Error deleting job checkpoints:', error);
      return false;
    }
  }
  ...
  public async cleanupExpiredCheckpoints(): Promise<number> {
    ...
    try {
      ...
      logger.warn(`Cleaned up ${cleanedCount} expired checkpoints`);
      return cleanedCount;

    } catch (error) {
      logger.error('Error cleaning up expired checkpoints:', error);
      return cleanedCount;
    }
  }
  ...
  private getAllFromLocalStorage(): ProcessingCheckpoint[] {
    try {
      ...
      logger.error('Error getting checkpoints from localStorage:', error);
      return [];
    }
  }
