...
export class SchedulingService {
  ...
  static async submitSchedulingRequest(data: SchedulingRequestData): Promise<SchedulingResponse> {
    try {
      logger.warn('Submitting scheduling request:', data);
      
      const result = await this.sendSchedulingEmailFunction(data);
      
      logger.warn('Scheduling request submitted successfully:', result.data);
      return result.data;
      
    } catch (error: any) {
      logger.error('Error submitting scheduling request:', error);
