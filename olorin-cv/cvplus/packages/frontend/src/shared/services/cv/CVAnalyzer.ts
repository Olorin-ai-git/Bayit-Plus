...
export class CVAnalyzer {
  ...
  static async getRecommendations(
    ...
    logger.warn('[CVAnalyzer] getRecommendations is deprecated. Use CVServiceCore.getRecommendations for proper duplicate prevention.');
    ...
    logger.warn(`[CVAnalyzer] LEGACY getRecommendations called for jobId: ${jobId} (request #${currentCount})`);
    
    // IMMEDIATE BLOCKING: Check if request is already active
    if (activeRequests.has(requestKey)) {
      logger.warn(`[CVAnalyzer] BLOCKING duplicate legacy request for job: ${jobId} (request #${currentCount})`);
      logger.warn(`[CVAnalyzer] Active request key: ${requestKey}`);
      ...
      if (existingPromise) {
        logger.warn(`[CVAnalyzer] Returning cached promise for legacy job: ${jobId}`);
        return existingPromise;
      }
      
      // If no cached promise, return empty result to prevent hanging
      logger.warn(`[CVAnalyzer] No cached promise found, returning null for legacy job: ${jobId}`);
      return null;
    }
    ...
    logger.warn(`[CVAnalyzer] Starting new legacy request for job: ${jobId} (request #${currentCount})`);
    logger.warn(`[CVAnalyzer] Request key: ${requestKey}`);
    logger.warn(`[CVAnalyzer] Active requests count: ${activeRequests.size}`);
    logger.warn(`[CVAnalyzer] Cached promises count: ${cachedPromises.size}`);
    ...
    const requestPromise = this._executeGetRecommendations(jobId, targetRole, industryKeywords, forceRegenerate)
      .finally(() => {
        ...
        logger.warn(`[CVAnalyzer] Cleaning up legacy request tracking for job: ${jobId}`);
  ...
  static async _executeGetRecommendationsDirectly(
    ...
    logger.warn(`[CVAnalyzer] _executeGetRecommendationsDirectly called for job: ${jobId}`);
  ...
  private static async _executeGetRecommendations(
    ...
    logger.warn(`[CVAnalyzer] Executing actual Firebase request for job: ${jobId}`);
    ...
    try {
      ...
      logger.warn(`[CVAnalyzer] Attempting callable function for job: ${jobId}`);
      ...
      logger.warn(`[CVAnalyzer] Callable function succeeded for job: ${jobId}`);
      return result.data;
    } catch (error: unknown) {
      logger.warn(`[CVAnalyzer] Callable function failed for job: ${jobId}, trying direct HTTP call:`, error);
      ...
      logger.warn(`[CVAnalyzer] Direct HTTP call succeeded for job: ${jobId}`);
      logger.warn(`[CVAnalyzer] HTTP response structure:`, result);
  ...
  static clearRequestTracking() {
    ...
    requestCounts.clear();
    logger.warn('[CVAnalyzer] Request tracking cleared');
  }
