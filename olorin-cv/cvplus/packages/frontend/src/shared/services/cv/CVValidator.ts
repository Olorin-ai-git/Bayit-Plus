...
export class CVValidator {
  ...
  static async updateJobFeatures(jobId: string, selectedFeatures: string[]): Promise<{
    ...
    try {
      ...
      logger.warn('🔒 Calling secure updateJobFeatures function:', { jobId, selectedFeatures });
      ...
      logger.warn('✅ Server validation completed:', {
      ...
      logger.error('❌ Error updating job features via secure function:', error);
