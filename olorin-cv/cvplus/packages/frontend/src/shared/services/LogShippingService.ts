...
export class LogShippingService {
  ...
  addLogs(logs: LogEntry[], metadata: Partial<ShippingMetadata> = {}): void {
    ...
    if (this.queue.length > this.config.maxQueueSize) {
      ...
      logger.warn(`Log shipping queue overflow: ${overflow.length} batches dropped`);
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        if (batch.retries < this.config.maxRetries) {
          ...
          logger.error('Log batch failed after max retries:', batch.id, result.error);
        }
      }
  ...
  private handleOnlineStatusChange(): void {
    ...
    if (!wasOnline && this.isOnline) {
      ...
      logger.info('Connection restored, attempting to ship queued logs');
      setTimeout(() => {
        this.shipImmediately();
  ...
  private saveQueueToStorage(): void {
    try {
      ...
      logger.warn('Failed to save log shipping queue to storage:', error);
    }
  }
  ...
  private loadQueueFromStorage(): void {
    try {
      ...
      logger.info(`Loaded ${this.queue.length} batches from storage`);

    } catch (error) {
      logger.warn('Failed to load log shipping queue from storage:', error);
      localStorage.removeItem(this.storageKey);
    }
  }
