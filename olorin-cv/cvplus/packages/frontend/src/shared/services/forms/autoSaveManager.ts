...
export class AutoSaveManager {
  ...
  private async executeSave(formId: string, activeForm: ActiveForm): Promise<boolean> {
    try {
      ...
      logger.warn(`Auto-saving form ${formId}:`, {
      ...
      logger.error(`Auto-save failed for form ${formId}:`, error);
      ...
      if (retryCount < this.defaultOptions.maxRetries!) {
        ...
        logger.warn(`Retrying auto-save for form ${formId} in ${retryDelay}ms (attempt ${retryCount + 1})`);
      }
