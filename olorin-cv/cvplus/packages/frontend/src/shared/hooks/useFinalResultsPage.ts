...
export const useFinalResultsPage = (jobId: string) => {
  ...
  const loadBaseHTML = async (job: Job) => {
    logger.warn('💼 [HTML-LOAD] Starting HTML load process:', {
    ...
    try {
      ...
      if (job.generatedCV?.htmlUrl) {
        logger.warn('🌍 [HTML-LOAD] Attempting to fetch from URL:', job.generatedCV.htmlUrl);
        ...
        if (job.generatedCV.htmlUrl.includes('firebasestorage') || job.generatedCV.htmlUrl.includes('localhost:9199')) {
          try {
            ...
            if (response.ok) {
              ...
              logger.warn('✅ [HTML-LOAD] Successfully fetched HTML from URL:', htmlContent.length, 'characters');
              setBaseHTML(htmlContent);
              setEnhancedHTML(htmlContent);
              return;
            } else {
              logger.warn('⚠️ [HTML-LOAD] Failed to fetch HTML from URL:', response.status, response.statusText);
            }
          } catch (fetchError) {
            logger.error('❌ [HTML-LOAD] Error fetching HTML from URL:', fetchError);
          }
        }
      }
      ...
      if (job.generatedCV?.html) {
        logger.warn('📝 [HTML-LOAD] Using inline HTML content:', job.generatedCV.html.length, 'characters');
        setBaseHTML(job.generatedCV.html);
        setEnhancedHTML(job.generatedCV.html);
        return;
      }
      
      logger.warn('⚠️ [HTML-LOAD] No HTML content available in job data');
      
    } catch (error) {
      logger.error('❌ [HTML-LOAD] Error in loadBaseHTML:', error);
      
      // Final fallback attempt
      if (job.generatedCV?.html) {
        logger.warn('🔄 [HTML-LOAD] Fallback: Using inline HTML after error');
        setBaseHTML(job.generatedCV.html);
        setEnhancedHTML(job.generatedCV.html);
      }
    }
  ...
  const setupFeatureQueue = (selectedFeatures: string[]) => {
    logger.warn('🔧 [DEBUG] setupFeatureQueue called with features:', selectedFeatures);
    ...
    const progressiveEnhancementFeatures = selectedFeatures.filter(feature => {
      ...
      if (isBackendFeature) {
        logger.warn(`🔧 [DEBUG] Filtering out backend feature: ${feature} (already processed)`);
      }
      return !isBackendFeature;
    });
    
    logger.warn('🔧 [DEBUG] Progressive enhancement features after filtering:', progressiveEnhancementFeatures);
    
    const normalizedFeatures = progressiveEnhancementFeatures.map(feature => 
      feature === 'embed-q-r-code' ? 'embed-qr-code' : feature
    );
    logger.warn('🔧 [DEBUG] Normalized features:', normalizedFeatures);
    
    const camelCaseFeatures = normalizedFeatures.map(feature => 
      feature === 'embed-qr-code' ? 'embedQRCode' : kebabToCamelCase(feature)
    );
    logger.warn('🔧 [DEBUG] CamelCase features:', camelCaseFeatures);
    
    const queue = camelCaseFeatures
      .filter(featureId => {
        const hasConfig = !!FEATURE_CONFIGS[featureId];
        if (!hasConfig) {
          logger.warn(`🔧 [DEBUG] No config found for feature: ${featureId}`);
        }
        return hasConfig;
      })
      .map(featureId => FEATURE_CONFIGS[featureId]);
    
    logger.warn('🔧 [DEBUG] Final feature queue (progressive enhancement only):', queue);
    ...
    logger.warn(`📊 [FEATURE-SUMMARY] Backend features: ${backendCount}, Progressive enhancement features: ${progressiveCount}`);
    
    if (backendCount > 0) {
      logger.warn('✅ [FEATURE-SUMMARY] Backend features are already integrated in the base CV HTML');
    }
    
    if (progressiveCount === 0) {
      logger.warn('🎯 [FEATURE-SUMMARY] No progressive enhancement needed - CV is ready to display');
    } else {
      logger.warn(`🚀 [FEATURE-SUMMARY] Will progressively enhance CV with ${progressiveCount} features`);
    }
  ...
  const loadJobData = async () => {
    ...
    try {
      ...
      setJob(jobData);
      logger.warn('🔧 [DEBUG] Job data loaded:', {
      ...
      if (jobData.generatedCV?.html || jobData.generatedCV?.htmlUrl || jobData.status === 'completed') {
        logger.warn('🚀 [DEBUG] Attempting to load HTML content for completed job');
        await loadBaseHTML(jobData);
      } else {
        logger.warn('⏳ [DEBUG] Job not ready for HTML display:', {
      ...
      if (jobData.generatedCV?.features && jobData.generatedCV.features.length > 0) {
        logger.warn('🔧 [DEBUG] Setting up feature queue with features from generatedCV:', jobData.generatedCV.features);
      ...
      // If no features in generatedCV, check enhancedFeatures
        ...
        logger.warn('🔧 [DEBUG] Setting up feature queue with features from enhancedFeatures:', Object.keys(jobData.enhancedFeatures));
      ...
      // Check selectedFeatures as fallback
        ...
        logger.warn('🔧 [DEBUG] Setting up feature queue with selectedFeatures:', jobData.selectedFeatures);
        setupFeatureQueue(jobData.selectedFeatures);
      } else {
        logger.warn('🔧 [DEBUG] No features found in job data - feature queue will be empty');
      }
      ...
      if (jobData.status === 'completed' && jobData.enhancedFeatures) {
        ...
        logger.warn('🏁 [DEBUG] Job completed, features processing status:', !featuresComplete);
      }
    } catch (err: any) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = jobSubscriptionManager.subscribeToProgress(
      ...
      (updatedJob: Job | null) => {
        ...
        logger.warn('🔄 [REAL-TIME] Job status update received:', {
        ...
        if ((updatedJob.status === 'completed' && job?.status !== 'completed') || 
          ...
          logger.warn('🎉 [REAL-TIME] Job completed or CV generated, reloading job data');
          setJob(updatedJob);
