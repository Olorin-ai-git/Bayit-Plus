...
export const useLogStream = (config: LogStreamConfig = {}): UseLogStreamReturn => {
  ...
  const handleStreamEvent = useCallback((event: LogStreamEvent) => {
    ...
    switch (event.type) {
      ...
      case 'disconnect':
        logger.info('Stream disconnected:', event.data?.reason);
        break;

      default:
        logger.info('Unknown stream event:', event);
    }
  }, [addLogToBuffer]);
    ...
    try {
      ...
      eventSource.onmessage = (event) => {
        try {
          ...
          logger.error('Error parsing stream event:', error, event.data);
        }
      };
      ...
      eventSource.addEventListener('log', (event: any) => {
        try {
          ...
          logger.error('Error parsing log event:', error);
        }
      });
      ...
      eventSource.addEventListener('heartbeat', (event: any) => {
        try {
          ...
          logger.error('Error parsing heartbeat event:', error);
        }
      });
      ...
      eventSource.addEventListener('stats', (event: any) => {
        try {
          ...
          logger.error('Error parsing stats event:', error);
        }
      });
  ...
  useEffect(() => {
    ...
    const healthCheckInterval = setInterval(() => {
      ...
      if (lastEvent && Date.now() - new Date(lastEvent.timestamp).getTime() > 60000) {
        logger.warn('No events received in 60 seconds, reconnecting...');
        disconnect();
