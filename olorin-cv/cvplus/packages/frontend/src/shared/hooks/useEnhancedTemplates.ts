...
export function useEnhancedTemplates(options: TemplateHookOptions = {}): UseEnhancedTemplatesReturn {
  ...
  const loadTemplates = async () => {
    try {
      ...
      logger.warn('✅ Templates loaded successfully:', templateDisplayData.length);
    } catch (err) {
  ...
  const loadRecommendations = () => {
    try {
      ...
      logger.warn('🎯 Template recommendations updated:', recommended.length);
    } catch (err) {
      logger.warn('⚠️ Failed to load recommendations:', err);
    }
  };
  ...
  const selectTemplate = useCallback((templateId: string) => {
    ...
    if (template) {
      ...
      logger.warn('🎨 Template selected:', template.name);
    } else {
      logger.warn('⚠️ Template not found:', templateId);
    }
  }, [templates]);

  const setCategory = useCallback((category: TemplateCategory | 'all') => {
    setActiveCategory(category);
    logger.warn('📂 Category changed:', category);
  }, []);
  ...
  const trackTemplateUsage = useCallback((templateId: string, metadata?: Record<string, any>) => {
    if (enableAnalytics) {
      ...
      logger.warn('📊 Template usage tracked:', templateId, metadata);
    }
  }, [enableAnalytics]);
  ...
  const generateCV = useCallback(async (params: {
    ...
    try {
      logger.warn('🚀 Starting enhanced CV generation:', params.templateId);
      ...
      if (result.success) {
        logger.warn('✅ CV generation completed:', result.metadata);
        return {
          success: true,
          data: {
        ...
        logger.error('❌ CV generation failed:', result.error);
        return {
          success: false,
...
logger.warn('🪝 Enhanced Templates Hooks initialized');
