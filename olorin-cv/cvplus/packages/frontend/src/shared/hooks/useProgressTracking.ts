...
export const useProgressTracking = (jobId: string, features: FeatureConfig[]) => {
  ...
  const setupProgressTracking = (trackingFeatures: FeatureConfig[]) => {
    logger.warn('📡 [DEBUG] Setting up progress tracking for job:', jobId);
    logger.warn('📡 [DEBUG] Tracking features:', trackingFeatures.map(f => ({ id: f.id, name: f.name })));
    ...
    const unsubscribe = jobSubscriptionManager.subscribeToProgress(
      ...
      (job: Job | null) => {
        if (!job || !isMountedRef.current) {
          logger.warn('📡 [DEBUG] No job data or component unmounted');
          return;
        }
        
        const enhancedFeatures = job.enhancedFeatures || {};
        logger.warn('🗺️ [DEBUG] Enhanced features received:', enhancedFeatures);
        ...
        trackingFeatures.forEach(feature => {
          ...
          if (featureData && typeof featureData === 'object' && !Array.isArray(featureData)) {
            ...
            logger.warn(`🔍 [DEBUG] Feature ${feature.id} progress:`, {
            ...
            logger.warn(`✅ [DEBUG] Feature ${feature.id} mapped to:`, {
            ...
            if (featureData) {
              logger.warn(`⚠️ [DEBUG] Feature ${feature.id} has invalid data structure:`, typeof featureData, featureData);
            }
          }
        });
        
        logger.warn(`📡 [DEBUG] Progress update: ${updatedFeatures}/${trackingFeatures.length} features have data`);
        logger.warn(`📊 [DEBUG] Feature status summary: ${completedFeatures} completed, ${processingFeatures} processing`);
        logger.warn(`📊 [DEBUG] New progress state:`, newProgressState);
        ...
        setProgressState(prevState => {
          ...
          if (hasChanges) {
            logger.warn(`🔄 [DEBUG] Progress state updated with changes`);
            return newProgressState;
          } else {
            logger.warn(`⏭️ [DEBUG] No progress changes detected, keeping previous state`);
            return prevState;
          }
        });
      },
