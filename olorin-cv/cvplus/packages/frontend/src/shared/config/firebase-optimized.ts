...
try {
  ...
  if (existingApps.length === 0) {
    ...
    logger.warn('[Firebase] App initialized successfully');
  } else {
    // App already exists, use the existing one
    app = getApp();
    logger.warn('[Firebase] Using existing app instance');
  }
} catch (error) {
  logger.error('[Firebase] Error during app initialization:', error);
  ...
  if (!auth) {
    ...
    if (import.meta.env.DEV && !emulatorConnections.auth) {
      try {
        ...
        logger.warn('[Firebase] Connected to Auth emulator on localhost:9099');
      } catch (error) {
        // Emulator might already be connected
        if (error instanceof Error && !error.message.includes('already')) {
          logger.warn('[Firebase] Failed to connect to Auth emulator:', error.message);
        }
  ...
  if (!firestore) {
    ...
    if (import.meta.env.DEV && !emulatorConnections.firestore) {
      try {
        ...
        logger.warn('[Firebase] Connected to Firestore emulator on localhost:8090');
      } catch (error) {
        // Emulator might already be connected
        if (error instanceof Error && !error.message.includes('already')) {
          logger.warn('[Firebase] Failed to connect to Firestore emulator:', error.message);
        }
  ...
  if (!functions) {
    ...
    if (import.meta.env.DEV && !emulatorConnections.functions) {
      try {
        ...
        logger.warn('[Firebase] Connected to Functions emulator on localhost:5001');
      } catch (error) {
        // Emulator might already be connected
        if (error instanceof Error && !error.message.includes('already')) {
          logger.warn('[Firebase] Failed to connect to Functions emulator:', error.message);
        }
