...
export const emergencyFirestoreReset = async (
  ...
  try {
    logger.warn('[EmergencyReset] Starting Firestore emergency reset due to assertion failures');
    ...
    try {
      ...
      logger.warn('[EmergencyReset] Network disabled successfully');
    } catch (error) {
      logger.warn('[EmergencyReset] Network disable failed:', error);
    ...
    if (clearCache) {
      try {
        ...
        logger.warn('[EmergencyReset] Cache cleared successfully');
      } catch (error) {
        logger.warn('[EmergencyReset] Cache clear failed:', error);
    ...
    if (reinitializeConnection) {
      try {
        ...
        logger.warn('[EmergencyReset] Network re-enabled successfully');
        
        // Wait for connection to stabilize
        await new Promise(resolve => setTimeout(resolve, 2000));
        steps.push('Connection stabilization wait (2s)');
      } catch (error) {
        logger.error('[EmergencyReset] Network re-enable failed:', error);
    ...
    logger.warn(`[EmergencyReset] Reset completed successfully in ${recoveryTime}ms`);
    ...
    const recoveryTime = Date.now() - startTime;
    logger.error('[EmergencyReset] Emergency reset failed:', error);
...
export class FirestoreHealthMonitor {
  ...
  reportError(error: Error): boolean {
    ...
    this.errorCount++;
    logger.warn(`[HealthMonitor] Firestore assertion error detected (${this.errorCount}/${this.maxErrors})`);
    ...
    if (this.errorCount >= this.maxErrors && now - this.lastResetTime > this.resetCooldown) {
      logger.error('[HealthMonitor] Error threshold exceeded, triggering emergency reset');
  ...
  private async triggerEmergencyReset(): Promise<void> {
    ...
    try {
      ...
      if (result.success) {
        logger.warn('[HealthMonitor] Emergency reset successful, resetting error count');
        this.errorCount = 0;
        this.onHealthChanged?.(true);
      } else {
        logger.error('[HealthMonitor] Emergency reset failed:', result.error);
      }
    } catch (error) {
      logger.error('[HealthMonitor] Emergency reset exception:', error);
    }
  }
...
export const firestoreHealthMonitor = new FirestoreHealthMonitor(
  (healthy) => {
    logger.warn(`[HealthMonitor] Firestore health status: ${healthy ? 'HEALTHY' : 'UNHEALTHY'}`);
    
    // Optional: Show user notification
    if (!healthy) {
      logger.warn('Firestore connection experiencing issues, attempting automatic recovery...');
    }
  }
);
