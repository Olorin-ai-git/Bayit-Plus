...
class FirestoreDebuggingSuite {
  ...
  public registerListener(
    ...
    if (this.duplicateDetectionEnabled) {
      ...
      if (duplicates.length > 0) {
        logger.warn(`🚨 [FirestoreDebugger] DUPLICATE LISTENER DETECTED for path: ${path}`, {
  ...
  public unregisterListener(id: string): void {
    ...
    if (listener) {
      ...
      if (process.env.NODE_ENV === 'development') {
        logger.warn(`✅ [FirestoreDebugger] Listener unregistered: ${id} (${listener.path})`);
      }
    }
  }
  ...
  public recordAssertionError(error: Error, context?: any): AssertionError | null {
    ...
    logger.error(`🚨 [FirestoreDebugger] ASSERTION ERROR DETECTED: ${errorType.toUpperCase()}`, {
  ...
  public cleanupDuplicateListeners(
    ...
    logger.warn(`🧹 [FirestoreDebugger] Cleaning up ${toCleanup.length} duplicate listeners for path: ${path}`);
    ...
    for (const listener of toCleanup) {
      try {
        ...
        logger.warn(`  ✅ Cleaned up listener: ${listener.id} (source: ${listener.componentSource || 'unknown'})`);
      } catch (error) {
        logger.error(`  ❌ Error cleaning up listener ${listener.id}:`, error);
      }
    }
  ...
  public performComprehensiveCleanup(): {
    ...
    logger.warn('🧽 [FirestoreDebugger] Starting comprehensive cleanup...');
    ...
    for (const listener of inactiveListeners) {
      try {
        ...
        logger.error(`Error cleaning up inactive listener ${listener.id}:`, error);
      }
    }
    ...
    try {
      ...
      logger.error('Error cleaning up JobSubscriptionManager:', error);
    }
    ...
    logger.warn('✅ [FirestoreDebugger] Comprehensive cleanup completed:', results);
    return results;
  }
  ...
  private async attemptAutomaticRecovery(error: AssertionError): Promise<boolean> {
    logger.warn(`🔧 [FirestoreDebugger] Attempting automatic recovery for ${error.errorType} error...`);
    ...
    for (const strategy of strategies) {
      try {
        ...
        if (success) {
          logger.warn(`✅ [FirestoreDebugger] Recovery successful using strategy: ${strategy}`);
          error.recovery = strategy;
          return true;
        }
      } catch (recoveryError) {
        logger.error(`❌ [FirestoreDebugger] Recovery strategy ${strategy} failed:`, recoveryError);
      }
    }
    
    logger.warn(`⚠️ [FirestoreDebugger] All recovery strategies failed for ${error.errorType} error`);
    return false;
  }
  ...
  private startMonitoring(): void {
    ...
    logger.warn('🔍 [FirestoreDebugger] Started monitoring Firestore listeners');
  }
  ...
  public stopMonitoring(): void {
    ...
    logger.warn('⏹️ [FirestoreDebugger] Stopped monitoring');
  }
  ...
  private performHealthCheck(): void {
    ...
    if (process.env.NODE_ENV === 'development') {
      logger.warn('📊 [FirestoreDebugger] Health Check:', {
    ...
    if (stats.duplicateListeners > 10 || stats.memoryUsageKB > 5000 || stats.errorCount > 5) {
      logger.warn('⚠️ [FirestoreDebugger] Thresholds exceeded, performing auto-cleanup');
  ...
  private logListenerRegistration(listener: FirestoreListener): void {
    if (process.env.NODE_ENV === 'development') {
      logger.warn(`📝 [FirestoreDebugger] Listener registered:`, {
        id: listener.id,
        path: listener.path,
...
## Error Summary`;
  ...
  public shutdown(): void {
    ...
    for (const listener of this.listeners.values()) {
      if (listener.unsubscribe) {
        try {
          ...
          logger.error('Error cleaning up listener during shutdown:', error);
        }
      }
    }
    ...
    logger.warn('🔚 [FirestoreDebugger] Shutdown completed');
  }
}
...
if (process.env.NODE_ENV === 'development') {
  ...
  if (typeof window !== 'undefined') {
    ...
    logger.warn('🛠️ [FirestoreDebugger] Available globally as window.firestoreDebugger');
    logger.warn('Commands: .getDebugStats(), .performComprehensiveCleanup(), .generateDiagnosticReport()');
  }
}
