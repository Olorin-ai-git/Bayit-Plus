...
export class RateLimiter {
  ...
  public recordRequest(key: string, success = true): void {
    ...
    if (this.options.enableLogging) {
      logger.warn(`[RateLimiter] Recorded ${success ? 'successful' : 'failed'} request for key: ${key}`);
    }
  }
  ...
  private cleanup(): void {
    ...
    if (cleanedCount > 0 && this.options.enableLogging) {
      logger.warn(`[RateLimiter] Cleaned up ${cleanedCount} expired rate limit keys`);
    }
  }
}
  ...
  return ((...args: Parameters<T>) => {
    ...
    timeoutId = setTimeout(() => {
      ...
      if (rateLimiter && key && !rateLimiter.isAllowed(key)) {
        logger.warn(`[DebouncedFunction] Rate limit exceeded for key: ${key}`);
        return;
      }
...
export function createThrottledFunction<T extends (...args: never[]) => unknown>(
  ...
  return ((...args: Parameters<T>) => {
    ...
    const execute = () => {
      // Check rate limiting if provided
      if (rateLimiter && key && !rateLimiter.isAllowed(key)) {
        logger.warn(`[ThrottledFunction] Rate limit exceeded for key: ${key}`);
        return;
      }
