  ...
  placeholders.forEach((placeholder, index) => {
    ...
    if (propsJson) {
      try {
        ...
        logger.error(`Failed to parse props for ${componentName}:`, error);
        return;
      }
    }
    ...
    if (componentName === 'AchievementCards') {
      logger.warn('🏆 Initializing AchievementCards component with props:', props);
    ...
    if (componentName === 'AIPodcastPlayer') {
      logger.warn('🎙️ Initializing AIPodcastPlayer component with props:', props);
    ...
    if (componentName === 'DynamicQRCode') {
      logger.warn('📱 Initializing DynamicQRCode component with props:', props);
      logger.warn('📱 QRCode Component from registry:', COMPONENT_REGISTRY[componentName]);
    ...
    if (componentName === 'VideoIntroduction') {
      logger.warn('🎥 Initializing VideoIntroduction component with props:', props);
      ...
      props = {
        ...
        onRegenerateVideo: async (customScript?: string, options?: any) => {
          try {
            ...
            logger.error('Video regeneration error:', error);
            throw error;
          }
        },
    ...
    if (['PortfolioGallery', 'TestimonialsCarousel', 'CertificationBadges', 'PersonalityInsights'].includes(componentName)) {
      logger.warn(`🎯 Initializing ${componentName} component with props:`, props);
