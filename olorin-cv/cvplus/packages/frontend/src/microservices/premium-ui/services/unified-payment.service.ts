...
// Type definitions (temporarily inline due to missing type files)
...
export class UnifiedPaymentService {
  ...
  async getAvailableProviders(context?: PaymentContext): Promise<PaymentProviderCapabilities[]> {
    try {
      ...
      logger.error('Failed to fetch provider capabilities:', error);
  ...
  async getExchangeRate(fromCurrency: string, toCurrency: string): Promise<number> {
    ...
    try {
      ...
      logger.warn('Failed to fetch exchange rate:', error);
