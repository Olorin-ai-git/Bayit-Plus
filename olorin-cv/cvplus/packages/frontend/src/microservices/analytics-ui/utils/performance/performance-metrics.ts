...
export class PerformanceMetrics {
  ...
  private checkPerformanceBudget(metrics: TemplatePerformanceMetrics): void {
    ...
    if (violations.length > 0) {
      logger.warn(`Performance budget violations for template ${metrics.templateId}:`, violations);
...
export function initializePerformanceMonitoring(): void {
  ...
  performanceMetrics.onMetrics((type, metrics) => {
    ...
    if (type === 'template') {
      logger.warn(`Template ${metrics.templateId} generated in ${metrics.generationTime}ms`);
    }
  ...
  logger.warn('Performance monitoring initialized');
}
...
export async function measureAsync<T>(
  ...
  try {
    ...
    logger.warn(`${name} completed in ${duration.toFixed(2)}ms`);
    return result;
  } catch (error) {
    const end = performance.now();
    const duration = end - start;
