...
export const useRoleDetection = (): UseRoleDetectionReturn => {
  ...
  const startDetection = useCallback(async (jobData: Job) => {
    logger.warn('[useRoleDetection] startDetection called with jobData:', {
      id: jobData?.id,
    ...
    if (!jobData.id) {
      ...
      logger.error('[useRoleDetection] Error: No job ID provided');
    ...
    try {
      logger.warn('[useRoleDetection] Starting role detection for job:', jobData.id);
      setLocalError(null);
      ...
      const timeoutPromise = new Promise<never>((_, reject) => {
        timeoutRef.current = setTimeout(() => {
          logger.warn(`[useRoleDetection] Role detection timeout reached after ${baseTimeout / 1000} seconds`);
      ...
      logger.warn('[useRoleDetection] About to call roleProfileService.detectRole with jobId:', jobData.id);
      logger.warn(`[useRoleDetection] Timeout set to ${Math.round(baseTimeout / 1000)} seconds (retry attempt: ${retryCount})`);
      ...
      logger.warn('[useRoleDetection] Received response from detectRole:', detectionResponse);
      setProgressMessage('');
      ...
      logger.error('[useRoleDetection] Role detection failed:', error);
      ...
      if (shouldProvideFallbacks) {
        logger.warn('[useRoleDetection] Auto-providing fallback roles due to timeout/error');
        setTimeout(() => {
  ...
  const retry = useCallback(() => {
    if (state.jobData) {
      ...
      setLocalError(null);
      setProgressMessage('');
      logger.warn(`[useRoleDetection] Retrying detection (attempt ${retryCount + 2})`);
      startDetection(state.jobData);
    }
  }, [state.jobData, startDetection, retryCount]);
  
  const provideFallbackRoles = useCallback(() => {
    logger.warn('[useRoleDetection] Providing fallback roles');
