...
export async function setupServices(config?: any): Promise<void> {
  try {
    ...
    if (parentIntegration.isConnectedToParent()) {
      logger.info('[CVProcessing] Connected to parent application');
      ...
      parentIntegration.on('auth-token-updated', (token) => {
        ...
        logger.info('[CVProcessing] Auth token updated from parent');
      });
    } else {
      logger.info('[CVProcessing] Running in standalone mode');
    }

    // Validate service setup
    await validateServices();

    logger.info('[CVProcessing] All services initialized successfully');

  } catch (error) {
    logger.error('[CVProcessing] Service setup failed:', error);
    throw error;
  }
}
