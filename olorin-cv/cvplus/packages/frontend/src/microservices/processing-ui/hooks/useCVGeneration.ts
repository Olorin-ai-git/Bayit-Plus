...
export const useCVGeneration = (): UseCVGenerationReturn => {
  ...
  useEffect(() => {
    if (state.editor.settings.autosave && state.cv) {
      ...
      autosaveTimer.current = setTimeout(() => {
        ...
        logger.info('[CV-GENERATION] Auto-save triggered');
  ...
  const loadCV = useCallback(async (jobId: string) => {
    ...
    try {
      ...
      logger.info('[CV-GENERATION] Loading CV:', jobId);
      ...
      logger.error('[CV-GENERATION] Failed to load CV:', error);
      setState(prev => ({
        ...prev,
        loading: false,
  ...
  const changeTemplate = useCallback(async (templateId: string) => {
    ...
    try {
      logger.info('[CV-GENERATION] Changing template:', templateId);
      ...
      logger.error('[CV-GENERATION] Failed to change template:', error);
      setState(prev => ({
        ...prev,
        loading: false,
  ...
  const saveVersion = useCallback(async (description: string) => {
    ...
    try {
      logger.info('[CV-GENERATION] Saving version:', description);
      ...
      logger.error('[CV-GENERATION] Failed to save version:', error);
      setState(prev => ({
        ...prev,
        saving: false,
  ...
  const exportCV = useCallback(async (format: ExportFormat, options?: ExportOptions): Promise<string> => {
    ...
    try {
      logger.info('[CV-GENERATION] Exporting CV:', { format, options });
      ...
      logger.error('[CV-GENERATION] Failed to export CV:', error);
      setState(prev => ({
        ...prev,
        exporting: false,
