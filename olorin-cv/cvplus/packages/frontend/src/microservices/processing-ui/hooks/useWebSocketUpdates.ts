...
export const useWebSocketUpdates = ({
  ...
  const sendHeartbeat = useCallback(() => {
    if (websocketRef.current?.readyState === WebSocket.OPEN) {
      try {
        ...
        logger.warn('[WebSocket] Heartbeat send failed:', error);
      }
    }
  }, [heartbeatInterval]);
  ...
  const connect = useCallback(() => {
    ...
    try {
      ...
      websocket.onopen = () => {
        logger.info('[WebSocket] Connected to', url.toString());
      ...
      websocket.onclose = (event) => {
        ...
        if (enabled && event.code !== 1000 && state.reconnectAttempts < maxReconnectAttempts) {
          ...
          logger.info(`[WebSocket] Reconnecting in ${delay}ms (attempt ${state.reconnectAttempts + 1}/${maxReconnectAttempts})`);
