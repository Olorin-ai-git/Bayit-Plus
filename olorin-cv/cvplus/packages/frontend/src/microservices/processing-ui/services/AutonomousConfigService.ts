...
export class AutonomousConfigService {
  ...
  private loadUserPreferences(): Partial<CVProcessingConfig> | null {
    try {
      ...
      logger.warn('[Config] Failed to load user preferences:', error);
      return null;
    }
  }
  ...
  private saveUserPreferences(config: Partial<CVProcessingConfig>): void {
    try {
      ...
      logger.warn('[Config] Failed to save user preferences:', error);
    }
  }
  ...
  resetConfig(): void {
    ...
    try {
      ...
      logger.warn('[Config] Failed to clear preferences:', error);
    }
  }
