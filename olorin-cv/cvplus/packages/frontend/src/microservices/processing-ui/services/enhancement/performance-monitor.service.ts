...
export class PerformanceMonitorService {
  ...
  startFeatureMonitoring(
    ...
    logger.warn(`📊 Started monitoring: ${featureName}`);
  }
  ...
  completeFeatureMonitoring(
    ...
    if (!metrics) {
      logger.warn(`⚠️ No active monitoring found for feature: ${featureId}`);
      return null;
    }
    ...
    logger.warn(`📈 Completed monitoring: ${metrics.featureName} (${completedMetrics.duration?.toFixed(2)}ms)`);
  ...
  recordRetryAttempt(featureId: string): void {
    ...
    if (metrics) {
      ...
      logger.warn(`🔄 Retry recorded for ${metrics.featureName}: ${metrics.retryCount}`);
    }
  }
  ...
  startSystemMonitoring(): void {
    ...
    logger.warn('🖥️ Started system performance monitoring');
  }
  ...
  stopSystemMonitoring(): void {
    ...
    logger.warn('🛑 Stopped system performance monitoring');
  }
  ...
  clearHistory(): void {
    ...
    logger.warn('🧹 Performance history cleared');
  }
