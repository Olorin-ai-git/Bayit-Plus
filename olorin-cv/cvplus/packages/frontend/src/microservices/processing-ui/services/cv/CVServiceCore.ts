...
export class CVServiceCore {
  ...
  static async getRecommendations(
    ...
    logger.warn(`[CVServiceCore] getRecommendations called for jobId: ${jobId}`, {
      requestKey,
      forceRegenerate,
    ...
    const result = await strictModeAwareRequestManager.executeOnce(
      requestKey,
      async () => {
        logger.warn(`[CVServiceCore] Executing actual request for jobId: ${jobId}`);
    ...
    if (result.wasFromCache) {
      ...
      logger.warn(`[CVServiceCore] Returned ${cacheType} result for jobId: ${jobId}`, {
        wasStrictModeDuplicate: result.wasStrictModeDuplicate,
        wasFromCache: result.wasFromCache
      });
    } else {
      logger.warn(`[CVServiceCore] Returned fresh result for jobId: ${jobId}`);
    }
