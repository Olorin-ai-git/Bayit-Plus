...
export class CVProcessingService {
  ...
  async uploadCV(file: File, options: UploadOptions): Promise<UploadResult> {
    try {
      ...
      this.startStatusTracking(jobId).catch(error => {
        logger.warn('[CVProcessing] Failed to start real-time tracking:', error);
      });
      ...
      logger.error('[CVProcessing] Upload error:', error);
  ...
  async startProcessing(jobId: string, features: ProcessingFeatures): Promise<ProcessingJob> {
    try {
      ...
      logger.error('[CVProcessing] Start processing error:', error);
      throw error;
    }
  }
  ...
  async getProcessingStatus(jobId: string): Promise<ProcessingStatus> {
    try {
      ...
      logger.error('[CVProcessing] Get status error:', error);
      throw error;
    }
  }
  ...
  async cancelProcessing(jobId: string): Promise<void> {
    try {
      ...
      logger.error('[CVProcessing] Cancel processing error:', error);
      throw error;
    }
  }
  ...
  async getProcessingResults(jobId: string): Promise<ProcessingResults> {
    try {
      ...
      logger.error('[CVProcessing] Get results error:', error);
      throw error;
    }
  }
  ...
  async getAnalysisResults(jobId: string): Promise<AnalysisResults> {
    try {
      ...
      logger.error('[CVProcessing] Get analysis results error:', error);
      throw error;
    }
  }
  ...
  async getTemplates(): Promise<CVTemplate[]> {
    try {
      ...
      logger.error('[CVProcessing] Get templates error:', error);
      throw error;
    }
  }
  ...
  async applyTemplate(jobId: string, templateId: string): Promise<ProcessedCVData> {
    try {
      ...
      logger.error('[CVProcessing] Apply template error:', error);
      throw error;
    }
  }
  ...
  async exportCV(jobId: string, format: ExportFormat): Promise<ExportResult> {
    try {
      ...
      logger.error('[CVProcessing] Export CV error:', error);
      throw error;
    }
  }
    ...
    try {
      ...
      eventSource.onmessage = (event) => {
        try {
          ...
          logger.error('[CVProcessing] Status update parse error:', error);
        }
      };

      // Handle errors
      eventSource.onerror = (error) => {
        logger.error('[CVProcessing] EventSource error:', error);
      ...
      logger.error('[CVProcessing] Subscribe to updates error:', error);
      throw error;
    }
  }
  ...
  private async startStatusTracking(jobId: string): Promise<void> {
    ...
    try {
      ...
      logger.warn('[CVProcessing] Could not start real-time tracking:', error);
      // Fallback to polling
  ...
  private startPollingStatusUpdates(jobId: string): void {
    const pollInterval = setInterval(async () => {
      try {
        ...
        logger.error('[CVProcessing] Polling error:', error);
        clearInterval(pollInterval);
      }
    ...
    setTimeout(() => {
      if (eventSource.readyState === EventSource.CLOSED) {
        try {
          ...
          logger.error('[CVProcessing] Reconnection failed:', error);
        }
      }
    }, 5000);
  }
  ...
  private async withRetry<T>(
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        logger.warn(`[CVProcessing] Retry ${attempt + 1}/${maxRetries} for ${operationId} in ${delay}ms:`, lastError.message);
    ...
    try {
      ...
      while (this.requestQueue.length > 0) {
        ...
        try {
          ...
          logger.error('[CVProcessing] Queue processing error:', error);
  ...
  private async processQueuedRequest(request: RequestQueueItem): Promise<void> {
    switch (request.type) {
      ...
      case 'template':
        ...
        break;
      default:
        logger.warn('[CVProcessing] Unknown request type:', request.type);
    }
  }
