...
export class HTMLValidatorService {
  ...
  validateHTML(
    ...
    try {
      ...
      logger.warn(`✅ HTML validation completed for ${featureId}: ${score}/100`);
      return result;

    } catch (error) {
      logger.error(`❌ Error validating HTML for ${featureId}:`, error);
  ...
  clearCache(): void {
    ...
    logger.warn('🧹 HTML validation cache cleared');
  }
