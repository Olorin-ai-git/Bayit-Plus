...
export class CVProcessingService {
  ...
  async updateProcessingData(
    ...
    try {
      ...
      logger.error('[CVProcessing] Update data error:', error);
      return {
        success: false,
        error: error instanceof Error ? error.message : String(error)
      };
  ...
  async getQueueStatus(): Promise<{
    ...
    try {
      ...
      logger.error('[CVProcessing] Queue status error:', error);
      return {
        queueLength: 0,
        averageWaitTime: 0,
        estimatedWaitTime: 0
...
// Export convenience functions are already defined above