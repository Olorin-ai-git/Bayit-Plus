...
export function useRecommendations() {
  ...
  const loadRecommendations = useCallback(async (params: GetRecommendationsParams) => {
    ...
    try {
      logger.info('[useRecommendations] Loading recommendations:', params);
      ...
      logger.info(`[useRecommendations] Loaded ${recommendations.length} recommendations`);

    } catch (error) {
      if (error instanceof Error && error.name === 'AbortError') {
        logger.info('[useRecommendations] Request aborted');
        return;
      }
      ...
      logger.error('[useRecommendations] Error loading recommendations:', error);
  ...
  const applyRecommendations = useCallback(async (params: ApplyImprovementsParams) => {
    ...
    try {
      logger.info('[useRecommendations] Applying recommendations:', params);
      ...
      logger.info('[useRecommendations] Recommendations applied successfully');
      return response;
      ...
      logger.error('[useRecommendations] Error applying recommendations:', error);
      throw error;
    }
  }, []);
  ...
  const previewRecommendation = useCallback(async (params: PreviewImprovementParams) => {
    try {
      logger.info('[useRecommendations] Previewing recommendation:', params);
      ...
      logger.info('[useRecommendations] Preview generated successfully');
      return response;

    } catch (error) {
      logger.error('[useRecommendations] Error previewing recommendation:', error);
      throw error;
    }
  }, []);
  ...
  const retryFailedRequest = useCallback(async () => {
    if (!state.error?.retryable) {
      logger.warn('[useRecommendations] Current error is not retryable');
      return;
    }
    ...
    logger.info('[useRecommendations] Retrying failed request...');
  }, [state.error]);
  ...
  const refreshCache = useCallback(async () => {
    try {
      ...
      logger.info('[useRecommendations] Cache refreshed');
    } catch (error) {
      logger.error('[useRecommendations] Error refreshing cache:', error);
    }
  }, []);
