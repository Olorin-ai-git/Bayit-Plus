...
export function useProgressiveEnhancement({
  ...
  const initializeFeatures = useCallback(() => {
    ...
    if (SERVER_SIDE_FEATURES.some(f => memoizedSelectedFeatures.includes(f))) {
      ...
      logger.warn(`🏗️ Server-side features detected and excluded from progressive enhancement: ${serverSideSelected.join(', ')}`);
      logger.warn(`🔄 Client-side features for progressive enhancement: ${clientSideFeatures.map(id => FEATURE_NAMES[id] || id).join(', ')}`);
    }
  ...
  const fetchBaseHtml = useCallback(async (): Promise<string> => {
    try {
      ...
      logger.warn('✅ Base HTML fetched successfully');

      return html;
    } catch (error) {
      logger.error('❌ Error fetching base HTML:', error);
  ...
  const callLegacyFunction = useCallback(async (featureId: string): Promise<string> => {
    ...
    try {
      logger.warn(`🚀 Calling ${functionName} for feature: ${featureId}`);
      ...
      logger.warn(`✅ ${functionName} completed successfully`);
      ...
      if (featureId === 'availability-calendar' && data.featureData) {
        logger.warn(`🔧 [availability-calendar] Extracted feature data:`, data.featureData);
  ...
  const mergeFeatureHtml = useCallback((baseHtml: string, featureHtml: string, featureId: string): string => {
    try {
      ...
      logger.error(`❌ Error merging feature HTML for ${featureId}:`, error);
  ...
  const processFeature = useCallback(async (featureId: string) => {
    ...
    try {
      ...
      logger.warn(`🔍 HTML validation for ${featureName}: ${validationResult.score}/100`);
      ...
      if (validationResult.warnings.length > 0) {
        logger.warn(`⚠️ HTML validation warnings for ${featureName}:`, validationResult.warnings);
      }
      ...
      logger.error(`❌ Error processing feature ${featureId}:`, error);
      ...
      if (recoveryResult.shouldRetry && retryCountRef.current[retryKey] < retryAttempts) {
        ...
        logger.warn(`🔄 Enhanced retry for ${featureName} (attempt ${currentAttempt}/${retryAttempts})`);
        logger.warn(`📊 Recovery strategy: ${recoveryResult.strategy}`);
        logger.warn(`🎯 Success probability: ${(recoveryResult.estimatedSuccessProbability * 100).toFixed(1)}%`);
        logger.warn(`⚡ Recovery actions: ${recoveryResult.recoveryActions.join(', ')}`);

        if (recoveryResult.alternativeApproach) {
          logger.warn(`🔀 Alternative approach available: ${recoveryResult.alternativeApproach}`);
        }
  ...
  const startEnhancement = useCallback(async () => {
    ...
    if (clientSideFeatures.length === 0) {
      logger.warn('🏗️ [useProgressiveEnhancement] All selected features are server-side generated, skipping progressive enhancement');
      ...
      try {
        ...
        logger.warn('✅ [useProgressiveEnhancement] Server-side generated CV loaded successfully');
      } catch (error: any) {
        logger.error('❌ Error loading base HTML:', error);
        setState(prev => ({
          ...prev,
          error: error.message,
          isLoading: false
    ...
    logger.warn('🔥 [useProgressiveEnhancement] startEnhancement called for job:', jobId);

    try {
      setState(prev => {
        // Check if already processing to prevent duplicate calls
        if (prev.isLoading) {
          logger.warn('⚠️ [useProgressiveEnhancement] Already processing, ignoring duplicate call');
          return prev;
        }
      ...
      logger.warn('📄 Fetching base HTML...');
      const baseHtml = await fetchBaseHtml();
      setState(prev => ({
        ...prev,
        baseHtml,
      ...
      try {
        logger.warn('🧠 Calculating optimal feature priorities...');
        ...
        if (prioritizedFeatures && prioritizedFeatures.length > 0) {
          ...
          logger.warn('📊 Priority Analysis:', analysis);
          logger.warn('🎯 Recommendations:', analysis.recommendations);
          ...
          logger.warn('🚀 Processing features in optimized order:', orderedFeatureIds.map(id => FEATURE_NAMES[id] || id));
        } else {
          throw new Error('Priority service returned empty results');
        }

      } catch (priorityError: any) {
        logger.warn('⚠️ Priority calculation failed, falling back to original order:', priorityError.message);
        ...
        logger.warn('🚀 Processing features in original order:', orderedFeatureIds.map(id => FEATURE_NAMES[id] || id));
        toast('Processing features in default order', { duration: 3000 });
      }

      // Ensure we have features to process
      if (orderedFeatureIds.length === 0) {
        logger.error('❌ No features to process after priority calculation and fallback');
      ...
      for (let i = 0; i < orderedFeatureIds.length; i++) {
        ...
        logger.warn(`🔄 Processing feature ${i + 1}/${orderedFeatureIds.length}: ${featureName} (${progress.toFixed(1)}% complete)`);
        ...
        if (i < orderedFeatureIds.length - 1) {
          ...
          logger.warn(`⏱️ Waiting ${delay}ms before next feature...`);
          await new Promise(resolve => setTimeout(resolve, delay));
        }
      }

      logger.warn('🎉 All features processed successfully!');
      ...
      if (user) {
        try {
          ...
          logger.warn('📊 Performance report generated successfully');
        } catch (reportError) {
          logger.error('❌ Error generating performance report:', reportError);
        }
        ...
        try {
          setState(currentState => {
            ...
            featurePriorityService.updateUserPreferences(user.uid, completedFeatures, failedFeatures)
              .then(() => logger.warn('👤 User preferences updated based on session results'))
              .catch((prefsError: any) => logger.error('❌ Error updating user preferences:', prefsError));

            return currentState;
          });
        } catch (prefsError) {
          logger.error('❌ Error updating user preferences:', prefsError);
        }
      }
      ...
      logger.error('❌ Error starting progressive enhancement:', error);
  ...
  useEffect(() => {
    ...
    if (unsubscribeRef.current) {
      logger.warn('⚠️ [useProgressiveEnhancement] Cleaning up existing subscription before creating new one');
    ...
    const handleJobUpdate = (job: Job | null) => {
      ...
      logger.warn('📊 Job update received via JobSubscriptionManager for job:', jobId);
    ...
    logger.warn('🔔 [useProgressiveEnhancement] Setting up job subscription for:', jobId);
    ...
    return () => {
      logger.warn('🧹 [useProgressiveEnhancement] Cleaning up job subscription for:', jobId);
      unsubscribeFromJob();
      unsubscribeRef.current = null;
    };
  }, [user, jobId]);
  ...
  useEffect(() => {
    logger.warn('🚀 [useProgressiveEnhancement] Auto-start check:', {
      autoStart,
    ...
    if (autoStart && memoizedSelectedFeatures.length > 0 && !hasStartedRef.current) {
      logger.warn('✅ [useProgressiveEnhancement] Starting enhancement for job:', jobId);
