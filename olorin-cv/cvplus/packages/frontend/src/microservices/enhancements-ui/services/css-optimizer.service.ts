...
export class CSSOptimizerService {
  ...
  optimizeFragmentCSS(
    ...
    try {
      ...
      logger.error(`❌ Error optimizing CSS for ${featureId}:`, error);
      return this.createEmptyResult();
    }
  }
  ...
  optimizeHTMLFragment(
    ...
    try {
      ...
      logger.error(`❌ Error optimizing HTML fragment for ${featureId}:`, error);
      return htmlFragment;
    }
  }
  ...
  clearCache(): void {
    ...
    this.cssStats.clear();
    logger.warn('🧹 CSS optimization cache cleared');
  }
