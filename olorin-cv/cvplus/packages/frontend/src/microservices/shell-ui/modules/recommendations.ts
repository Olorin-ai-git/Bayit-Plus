...
export const loadRecommendationsModule = async () => {
  if (MODULE_FLAGS.USE_RECOMMENDATIONS_MODULE) {
    try {
      // Note: Enable when @cvplus/recommendations package is ready
      ...
      logger.warn('[@cvplus/recommendations] Module not available:', error);
      return null;
    }
  }
  return null;
...
export const isRecommendationsModuleAvailable = (): boolean => {
  // Note: Enable when package is available
  // try {
