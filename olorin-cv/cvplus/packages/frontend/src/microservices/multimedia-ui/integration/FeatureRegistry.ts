...
export class FeatureRegistry {
  ...
  updateFeature(featureName: string, updates: Partial<FeatureConfig>): void {
    ...
    if (!namespaceConfig) {
      logger.warn(`[FeatureRegistry] Namespace "${namespace}" not found`);
      return;
    }

    const currentConfig = namespaceConfig.features[feature];
    if (!currentConfig) {
      logger.warn(`[FeatureRegistry] Feature "${featureName}" not found`);
      return;
    }
  ...
  private notifyListeners(featureName: string, config: FeatureConfig): void {
    this.listeners.get(featureName)?.forEach(callback => {
      try {
        ...
        logger.error(`[FeatureRegistry] Error in feature listener for ${featureName}:`, error);
      }
    });
  }
}
