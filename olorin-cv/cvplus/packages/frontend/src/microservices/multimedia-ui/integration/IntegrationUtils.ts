...
class MultimediaErrorBoundary extends React.Component<MultimediaErrorBoundaryProps, ErrorBoundaryState> {
  ...
  componentDidCatch(error: Error, errorInfo: React.ErrorInfo) {
    logger.error(`[MultimediaErrorBoundary] Error in ${this.props.componentName}:`, error, errorInfo);
  ...
  try {
    for (const [key, expectedType] of Object.entries(schema)) {
      ...
      if (expectedType === 'required' && (value === undefined || value === null)) {
        logger.warn(`[validateProps] Required prop "${key}" is missing`);
        return false;
      }
      ...
      if (value !== undefined && value !== null) {
        ...
        if (expectedType !== 'required' && actualType !== expectedType) {
          logger.warn(`[validateProps] Prop "${key}" expected ${expectedType}, got ${actualType}`);
          return false;
        }
      }
    }
    ...
    logger.error('[validateProps] Validation error:', error);
    return false;
  }
}
...
export function createLazyComponent<P extends Record<string, any>>(
  ...
  const LazyComponent = React.lazy(() => 
    importFn().catch((error) => {
      logger.error(`[createLazyComponent] Failed to load ${componentName}:`, error);
  ...
  private notifyListeners(key: string, value: any): void {
    this.listeners.get(key)?.forEach(callback => {
      try {
        ...
        logger.error(`[IntegrationStateManager] Error in listener for ${key}:`, error);
      }
    });
  }
}
