...
export class MultimediaService {
  ...
  subscribeToGeneration(jobId: string, onUpdate?: (status: GenerationStatus) => void): EventSource {
    ...
    eventSource.onmessage = (event) => {
      try {
        ...
        logger.error('Failed to parse generation update:', error);
      }
    };

    eventSource.onerror = (error) => {
      logger.error('Generation EventSource error:', error);
      eventSource.close();
      this.eventSources.delete(jobId);
    };
  ...
  subscribeToPlayback(sessionId: string, onUpdate?: (session: PlaybackSession) => void): EventSource {
    ...
    eventSource.onmessage = (event) => {
      try {
        ...
        logger.error('Failed to parse playback update:', error);
      }
    };

    eventSource.onerror = (error) => {
      logger.error('Playback EventSource error:', error);
      eventSource.close();
  ...
  async trackEngagement(mediaId: string, event: {
    ...
    try {
      ...
      logger.warn('Failed to track engagement:', error);
    }
  }
