...
export class MultimediaServiceInitializer {
  ...
  static initialize(options: MultimediaServiceInitOptions): MultimediaService {
    if (MultimediaServiceInitializer.initialized) {
      if (options.enableLogging) {
        logger.info('MultimediaService already initialized');
      }
    ...
    try {
      ...
      if (options.enableLogging) {
        logger.info('MultimediaService initialized successfully');
      }

      return multimediaService;
    } catch (error) {
      logger.error('Failed to initialize MultimediaService:', error);
  ...
  private static setupEmulators(
    ...
    try {
      ...
      logger.info('Firebase emulators connected for MultimediaService');
    } catch (error) {
      // Emulator connection might fail if already connected, which is fine
      logger.warn('Emulator connection warning:', error);
    }
  }
