...
export class PlaceholderService {
  ...
  static async customizeRecommendation(
    ...
    try {
      logger.warn(`Customizing recommendation ${recommendationId} with placeholders:`, placeholderValues);
      ...
      if (result.data.success) {
        logger.warn('Placeholder customization successful:', result.data.data);
        return result.data;
      } else {
        throw new Error(result.data.error || 'Failed to customize placeholders');
      }
    } catch (error: any) {
      logger.error('Error customizing placeholders:', error);
