    ...
    try {
      ...
      if (insertPosition === 'placeholder' && placeholder) {
        ...
        if (placeholderRegex.test(mergedHtml)) {
          ...
          logger.warn(`Placeholder ${placeholder} not found, falling back to append`);
      ...
      logger.error(`Error merging fragment for feature ${featureId}:`, error);
...
`;
  ...
  private static mergeStyles(html: string, styles: string[]): string {
    ...
    if (headCloseIndex === -1) {
      logger.warn('No head section found, skipping style merge');
      return html;
    }
  ...
  private static mergeScripts(html: string, scripts: string[]): string {
    ...
    const bodyCloseIndex = html.lastIndexOf('</body>');
    if (bodyCloseIndex === -1) {
      logger.warn('No body section found, appending scripts to end');
  ...
  static removeFeature(html: string, featureId: string): string {
    try {
      ...
      logger.error(`Error removing feature ${featureId}:`, error);
      return html;
    }
  }
  ...
  static optimize(html: string): string {
    try {
      ...
      if (scriptBlocks.length !== uniqueScripts.length) {
        logger.warn('Removed duplicate scripts');
