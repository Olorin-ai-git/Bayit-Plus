...
export function getMappedTemplate(legacyId: string): CVTemplate | null {
  ...
  if (!modernId) {
    logger.warn(`No mapping found for legacy template ID: ${legacyId}`);
    return null;
  }
  
  const template = getTemplate(modernId);
  if (!template) {
    logger.error(`Mapped template not found: ${modernId} (from legacy ${legacyId})`);
    return null;
  }
...
export class TemplateSelectionService {
  ...
  setUserProfile(profile: {
    ...
    logger.warn('📋 User profile updated for template recommendations:', profile);
  }
  ...
  trackTemplateSelection(templateId: string, userData?: {
    ...
    logger.warn('📊 Template selection tracked:', {
      templateId,
...
export class EnhancedCVGenerationService {
  ...
  async generateCV(params: {
    ...
    try {
      ...
      logger.warn('🎨 Generating CV with template:', template.name);
      logger.warn('📊 Template features:', template.features);
      logger.warn('🎯 ATS compatibility:', template.ats.formats.ats.compatibility.score);
...
logger.warn('🔗 Template Integration Service initialized');
logger.warn('📋 Available templates:', getAllTemplates().length);
logger.warn('🎨 Template categories:', Object.keys(templateRegistry.categories));
