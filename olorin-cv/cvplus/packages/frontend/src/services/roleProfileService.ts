...
export class RoleProfileService {
  ...
  async detectRole(
    ...
    try {
      ...
      logger.warn('[RoleProfileService] Detecting role for job:', jobId);
      logger.warn('[RoleProfileService] Initial auth check - Current user:', currentUser ? currentUser.uid : 'NOT AUTHENTICATED');
      logger.warn('[RoleProfileService] Functions instance:', this.functions);
      logger.warn('[RoleProfileService] Using force regenerate:', forceRegenerate);
      
      // If no current user, wait for auth state to settle
      if (!currentUser) {
        logger.warn('[RoleProfileService] No current user, waiting for auth state...');
        
        // Wait for auth state with timeout
        currentUser = await new Promise((resolve) => {
          const unsubscribe = auth.onAuthStateChanged((user) => {
            logger.warn('[RoleProfileService] Auth state changed:', user ? user.uid : 'null');
            unsubscribe();
            resolve(user);
          });
          ...
          setTimeout(() => {
            logger.warn('[RoleProfileService] Auth state timeout after 5 seconds');
            unsubscribe();
            resolve(null);
          }, 5000);
        });
      ...
      logger.warn('[RoleProfileService] Final auth check - Current user:', currentUser ? currentUser.uid : 'NOT AUTHENTICATED');
      ...
      if (cachedRequest) {
        logger.warn('[RoleProfileService] Returning existing request for:', requestKey);
      ...
      logger.warn('[RoleProfileService] Calling detectRoleProfile with data:', requestData);
      ...
      const requestPromise = (async (): Promise<RoleDetectionResponse> => {
        try {
          ...
          logger.warn('[RoleProfileService] Raw result from detectRoleProfile:', result);

          if (!result.data?.success) {
            throw new Error(result.data?.error || 'Role detection failed');
          }

          logger.warn('[RoleProfileService] Successful detection result:', result.data.data);
      ...
      this.activeRequests.delete(requestKey);
      logger.error('[RoleProfileService] Detection error:', error);
      logger.error('[RoleProfileService] Error details:', {
  ...
  private cleanupExpiredRequests(): void {
    ...
    if (expiredKeys.length > 0) {
      logger.warn(`[RoleProfileService] Cleaned up ${expiredKeys.length} expired requests`);
    }
  }
  ...
  async getAllRoleProfiles(): Promise<RoleProfile[]> {
    try {
      logger.warn('[RoleProfileService] Fetching all role profiles');
      ...
      logger.error('[RoleProfileService] Fetch error:', error);
  ...
  async getRoleProfilesByCategory(category: string): Promise<RoleProfile[]> {
    try {
      logger.warn('[RoleProfileService] Fetching role profiles for category:', category);
      ...
      logger.error('[RoleProfileService] Fetch by category error:', error);
  ...
  async searchRoleProfiles(
    ...
    try {
      logger.warn('[RoleProfileService] Searching role profiles:', searchQuery);
      ...
      logger.error('[RoleProfileService] Search error:', error);
  ...
  async applyRole(
    ...
    try {
      logger.warn('[RoleProfileService] Applying role profile:', { jobId, roleProfileId });
      ...
      logger.error('[RoleProfileService] Apply role error:', error);
  ...
  async getRoleRecommendations(
    ...
    try {
      logger.warn('[RoleProfileService] Getting role-based recommendations:', {
        jobId,
        roleProfileId,
        targetRole
      });
      ...
      logger.error('[RoleProfileService] Get recommendations error:', error);
  ...
  async autoDetectAndApply(
    ...
    try {
      logger.warn('[RoleProfileService] Auto-detecting and applying role for job:', jobId);
      ...
      logger.error('[RoleProfileService] Auto-detect and apply error:', error);
  ...
  async getEnhancedRecommendations(
    ...
    try {
      logger.warn('[RoleProfileService] Getting enhanced recommendations for job:', jobId);
      ...
      try {
        ...
        logger.warn('[RoleProfileService] Role detection failed, proceeding without role context:', detectionError);
      }
      ...
      logger.error('[RoleProfileService] Enhanced recommendations error:', error);
  ...
  async validateRoleCompatibility(
    ...
    try {
      logger.warn('[RoleProfileService] Validating role compatibility:', { jobId, roleProfileId });
      ...
      logger.error('[RoleProfileService] Compatibility validation error:', error);
