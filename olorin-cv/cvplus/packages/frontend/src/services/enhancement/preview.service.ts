...
export class PreviewService {
  ...
  async initializePreview(
    ...
    try {
      ...
      logger.error('❌ Error initializing preview:', error);
      this.updateState({ 
        isLoading: false, 
  ...
  private async loadBaseHtml(jobId: string, userId: string): Promise<void> {
    try {
      ...
      logger.warn('✅ Base HTML loaded for preview');
    } catch (error: unknown) {
  ...
  private startRealTimeMonitoring(jobId: string): void {
    ...
    this.unsubscribe = jobSubscriptionManager.subscribeToPreview(
      ...
      (job: Job | null) => {
        if (job) {
          ...
          logger.warn('📡 No job data received in preview monitoring');
        }
      },
      {
        enableLogging: true,
    ...
    logger.warn('📡 Started real-time preview monitoring via JobSubscriptionManager');
  }
  ...
  private mergeFeatureHtml(baseHtml: string, featureHtml: string, featureId: string): string {
    try {
      ...
      logger.error(`❌ Error merging preview HTML for ${featureId}:`, error);
      return baseHtml;
    }
  }
  ...
  generateSafePreviewHtml(html: string): string {
    ...
    try {
      ...
      logger.error('❌ Error generating safe preview HTML:', error);
  ...
  private updateState(updates: Partial<PreviewState>): void {
    ...
    this.listeners.forEach(listener => {
      try {
        ...
        logger.error('❌ Error in preview listener:', error);
      }
    });
  }
  ...
  toggleFeatureHighlight(featureId: string, highlight: boolean): void {
    ...
    logger.warn(`${highlight ? 'Highlighting' : 'Unhighlighting'} feature: ${featureId}`);
  }
  ...
  cleanup(): void {
    ...
    logger.warn('🧹 Preview service cleaned up');
  }
}
