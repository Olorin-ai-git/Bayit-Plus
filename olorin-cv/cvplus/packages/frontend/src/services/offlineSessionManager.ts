...
export class OfflineSessionManager {
  ...
  public async syncPendingActions(): Promise<void> {
    ...
    for (const action of pendingActions) {
      try {
        ...
        logger.error('Failed to execute queued action:', error);
        action.attempts++;
  ...
  private async setupServiceWorker(): Promise<void> {
    ...
    try {
      ...
      logger.warn('Service Worker registered successfully');
    } catch (error) {
  ...
  private async handleFormSaveAction(action: QueuedAction): Promise<void> {
    ...
    logger.warn('Executing form save action:', { sessionId, formId, formData });
  }
  ...
  private async handleFailedAction(action: QueuedAction): Promise<void> {
    logger.error('Action failed after maximum attempts:', action);
