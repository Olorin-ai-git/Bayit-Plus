...
export class CVParser {
  ...
  private static async retryOperation<T>(
    ...
    for (let i = 0; i <= maxRetries; i++) {
      try {
        ...
        logger.warn(`Operation attempt ${i + 1} failed:`, error);
  ...
  static async createJob(params: JobCreateParams): Promise<string> {
    ...
    return this.retryOperation(async () => {
      ...
      logger.warn('🔄 Creating job with ID:', jobId);
      await setDoc(doc(db, 'jobs', jobId), jobData);
      logger.warn('✅ Job created successfully:', jobId);
      return jobId;
    });
  }
  ...
  static async uploadCV(params: FileUploadParams): Promise<string> {
    ...
    return this.retryOperation(async () => {
      logger.warn('🔄 Uploading file:', file.name, 'for job:', jobId);
      ...
      logger.warn('✅ File uploaded successfully, updating job...');
      ...
      logger.warn('✅ Job updated with file info');
      return downloadURL;
    });
  }
  ...
  static async processCV(params: CVProcessParams) {
    ...
    logger.warn('ProcessCV called with:', { 
      jobId, 
      fileUrl: fileUrl.substring(0, 100) + '...', 
      mimeType, 
      isUrl 
    });
    logger.warn('User authenticated:', currentUser.uid);
  ...
  static async initiateCVGeneration(params: AsyncCVGenerationParams): Promise<AsyncCVGenerationResponse> {
    ...
    try {
      logger.warn('🚀 Calling Firebase initiateCVGeneration function with:', {
        jobId,
        templateId,
      ...
      logger.warn('✅ Firebase initiateCVGeneration function completed:', {
        hasResult: !!result,
  ...
  static async generateCV(jobId: string, templateId: string, features: string[]) {
    ...
    logger.warn(`🎯 CV Generation Mode: ${isAsyncEnabled ? 'ASYNC' : 'SYNC'}`, {
  ...
  private static async _generateCVSync(jobId: string, templateId: string, features: string[]) {
    ...
    try {
      logger.warn('🚀 Calling Firebase generateCV function (SYNC) with:', {
        jobId,
        templateId,
        originalFeatures: features,
        convertedFeatures: kebabCaseFeatures,
      ...
      logger.warn('✅ Firebase generateCV function (SYNC) completed:', {
        hasResult: !!result,
        hasData: !!(result as any)?.data,
        timestamp: new Date().toISOString()
      });
