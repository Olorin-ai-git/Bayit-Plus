...
export class MagicTransformService {
  ...
  static async executeEnhancedMagicTransform(
    ...
    try {
      ...
      logger.warn(`[MagicTransform] User tier: ${userTier}`);
      ...
      try {
        ...
        logger.warn(`[MagicTransform] Applied ${selectedRecommendationIds.length} improvements`);
      } catch (error) {
        logger.error('[MagicTransform] Failed to apply improvements:', error);
      ...
      for (let i = 0; i < coreFeatures.length; i++) {
        ...
        try {
          ...
          logger.warn(`[MagicTransform] Generated feature: ${feature}`);
        } catch (error) {
          logger.error(`[MagicTransform] Failed to generate ${feature}:`, error);
      ...
      if (userTier === 'PREMIUM') {
        ...
        for (let i = 0; i < premiumFeatures.length; i++) {
          ...
          try {
            ...
            logger.warn(`[MagicTransform] Generated premium feature: ${feature}`);
          } catch (error) {
            logger.error(`[MagicTransform] Failed to generate premium ${feature}:`, error);
            result.failedFeatures.push(feature);
            progress.failedTasks.push({
      ...
      logger.warn(`[MagicTransform] Completed in ${result.totalProcessingTime}s:`, {
      ...
      logger.error('[MagicTransform] Critical error:', error);
  ...
  private static async determineUserTier(): Promise<SubscriptionTier> {
    try {
      ...
      logger.warn('[MagicTransform] Failed to determine user tier, defaulting to FREE:', error);
      return 'FREE';
    }
  }
  ...
  private static async generatePrivacyModeSetup(jobId: string): Promise<any> {
    ...
    try {
      ...
      logger.warn('[MagicTransform] Privacy settings will be applied when public profile is created');
      return {
        type: 'privacy-mode',
        settings: privacySettings,
        updated: false,
