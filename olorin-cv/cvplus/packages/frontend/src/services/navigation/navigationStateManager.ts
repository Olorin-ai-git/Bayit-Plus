...
export class NavigationStateManager {
  ...
  public pushStateToHistory(state: NavigationState): void {
    ...
    try {
      ...
      logger.warn('Error pushing state to browser history:', error);
    }
  }
  ...
  private saveNavigationStateToStorage(): void {
    try {
      ...
      logger.warn('Failed to save navigation state to storage:', error);
    }
  }
  ...
  private restoreNavigationStateFromStorage(): NavigationState | null {
    try {
      ...
      logger.warn('Failed to restore navigation state from storage:', error);
  ...
  private setupNetworkListener(): void {
    const onlineHandler = () => {
      logger.warn('🌐 Network connection restored');
      // Retry queued operations
      this.retryQueuedOperations();
    };

    const offlineHandler = () => {
      logger.warn('🌐 Network connection lost - switching to cached mode');
    };
  ...
  private retryQueuedOperations(): void {
    ...
    for (const [operationId, promise] of this.networkRetryQueue.entries()) {
      ...
      logger.warn(`Retrying queued operation: ${operationId}`);
    }
  }
  ...
  private cacheNavigationContext(sessionId: string, context: NavigationContext): void {
    try {
      ...
      logger.warn('Failed to cache navigation context:', error);
    }
  }
  ...
  private getCachedNavigationContext(sessionId: string): NavigationContext | null {
    try {
      ...
      logger.warn('Failed to retrieve cached navigation context:', error);
    }
    return null;
  }
  ...
  public cleanup(): void {
    ...
    logger.warn('🧹 NavigationStateManager cleanup completed');
  }
}
