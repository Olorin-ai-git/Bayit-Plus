...
export const Sidebar: React.FC = () => {
  ...
  const partnerName = organization?.name || "Partner";
  const nameInitial = partnerName.charAt(0).toUpperCase();
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 px-6 py-5 border-b border-white/5">
        <div className="h-10 w-10 rounded-xl bg-partner-primary flex items-center justify-center shrink-0 overflow-hidden">
          {logoUrl ? (
            <img
              src={logoUrl}
              alt={partnerName}
          ...
          <span
            className={`text-white font-bold text-lg ${logoUrl ? "hidden" : ""}`}
          >
            {nameInitial}
          </span>
        </div>
          ...
          {filteredItems.map((item) => {
            ...
            return (
              <li key={item.path}>
                ...
                >
                  <span
                    className={`shrink-0 ${isActive ? "text-partner-primary" : ""}`}
                  >
                    {item.icon}
                  </span>
                  {!isSidebarCollapsed && (
