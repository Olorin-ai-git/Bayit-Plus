...
#   - All 11 required secrets in GCP Secret Manager
#   - Run ./SETUP_SECRETS.sh first to create secrets from backend/.env
...
# Determine project root (olorin monorepo root)
...
if [ ${#MISSING_SECRETS[@]} -gt 0 ]; then
  ...
  echo ""
  echo "Run the secret setup script first:"
  echo "  cd olorin-infra"
  echo "  ./SETUP_SECRETS.sh"
  echo ""
  echo "This will create all Olorin secrets from backend/.env"
  exit 1
fi
echo "✓ All 11 required secrets verified"
echo ""

# Step 3: Validate Cloud Build Configuration
echo "📍 STEP 3: Validating Cloud Build configuration..."
if [ ! -f "$PROJECT_ROOT/olorin-infra/cloudbuild.yaml" ]; then
  echo "❌ ERROR: olorin-infra/cloudbuild.yaml not found"
  exit 1
fi
...
if command -v docker &> /dev/null; then
  ...
  cd "$PROJECT_ROOT"
  docker build -f olorin-infra/Dockerfile -t olorin-backend:test . > /dev/null 2>&1
  if [ $? -eq 0 ]; then
...
gcloud builds submit \
  --config=olorin-infra/cloudbuild.yaml \
  --project="$PROJECT_ID"
