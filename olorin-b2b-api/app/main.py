"""
Olorin B2B API Service.

Extracted Cloud Run service for Olorin platform B2B routes. Connects to the
same MongoDB cluster and Redis instance as the monolith, but handles only
Olorin partner/dubbing/search/context/recap/webhook routes with independent
...
async def lifespan(service_app: FastAPI):
    """Service lifespan: MongoDB subset + Redis + content metadata."""
    ...
    # Initialize content metadata service (used by Olorin search/context routes)
    try:
        ...
        logger.info(
            "Content metadata service initialized",
            extra={"service": SERVICE_NAME},
        )
    except Exception as e:
        logger.warning("Content metadata service initialization failed: %s", e)
