...
Extracted Cloud Run service for user routes. Connects to the same
MongoDB cluster and Redis instance as the monolith, but handles only
user profile, subscription, favorites, downloads, history, family
controls, household, devices, playback, and notification endpoints
with independent scaling and deployment.
...
SERVICE_TITLE = "Bayit+ User Service"
...
async def lifespan(service_app: FastAPI):
    ...
    # Initialize Redis client (session caching, rate limiting)
    try:
