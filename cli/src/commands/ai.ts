...
export function registerAiCommand(program: Command): void {
  ...
  aiCommand
    ...
    .action(async (_options) => {
      ...
      console.log(chalk.gray('Note: Voice recording requires additional setup\n'));
      console.log(chalk.red('❌ Voice command not yet implemented in CLI'));
      console.log(chalk.yellow('💡 Voice commands are available via the backend API:'));
      console.log(chalk.white('   POST /api/v1/nlp/voice-command'));
      process.exit(1);
    });
