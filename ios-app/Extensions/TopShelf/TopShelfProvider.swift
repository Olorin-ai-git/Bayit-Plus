    ...
    final class TopShelfProvider: TVTopShelfContentProvider {
        ...
        private func loadSections() async -> [TVTopShelfItemCollection<TVTopShelfSectionedItem>] {
            ...
            let liveChannels = buildLiveChannelsSection()
            if !liveChannels.items.isEmpty {
                sections.append(liveChannels)
            }
        ...
        private func buildLiveChannelsSection() -> TVTopShelfItemCollection<TVTopShelfSectionedItem> {
            let cached = loadCachedItems(forKey: TopShelfConstants.liveChannelsKey)
