    ...
    private enum DS {
        ...
        static let r2xl: CGFloat = 16 // rounded-2xl  (cards, panels)
        static let rxl: CGFloat = 12 // rounded-xl   (icon boxes, rows)
        static let rmd: CGFloat = 8 // rounded-md   (status pill)

        // MARK: Spacing

        static let gap: CGFloat = 16 // gap-4
        static let pagePad: CGFloat = 20 // container padding
    }
    ...
    private struct HeaderRow: View {
        ...
        var body: some View {
            HStack(alignment: .top) {
                Text("Help &\nSupport")
                    // SF Pro Display is used automatically for size ≥ 20pt on iOS
                    .font(.system(size: 44, weight: .bold, design: .default))
                    .foregroundColor(.white)
                    .lineSpacing(6) // lineHeight 1.15 → adds ~6pt extra
                    .kerning(-0.88) // letterSpacing -0.02em × 44pt
                    .padding(.top, 8)
    ...
    private struct FeatureCardsSection: View {
        var body: some View {
            VStack(spacing: DS.gap) {
                // Primary highlighted card — full width on iPhone
                FeatureCardView(
                    systemIcon: "sparkles",
    ...
    private struct FeatureCardView: View {
        ...
        var body: some View {
            Button(action: {}) {
                HStack(spacing: 16) {
                    // Icon container: 48×48, rounded-xl (12pt)
                    ZStack {
                        iconGradient
                    ...
                    // Text stack: label (13/400/white50) + title (17/600/white)
