...
struct TVLanguageRatioView: View {
    ...
    private var englishSegment: some View {
        ...
        .frame(
            width: englishPercentage > 0
                ? (UIScreen.main.bounds.width * 0.3 * (englishPercentage / 100))
                : 0
        )
        .background(
            LinearGradient(
    ...
    private var hebrewSegment: some View {
        HStack(spacing: TVDesignTokens.Spacing.xxs) {
        ...
        .frame(
            width: hebrewPercentage > 0
                ? (UIScreen.main.bounds.width * 0.3 * (hebrewPercentage / 100))
                : 0
        )
        .background(
            LinearGradient(
