    ...
    struct TVTalkBackOverlayView: View {
        ...
        private var submitAndSkipButtons: some View {
            HStack(spacing: TVDesignTokens.Spacing.lg) {
                GlassButton(localization.t("common.submit"), variant: .primary, size: .large) { submitAnswer() }
                    .tvFocusStyle().disabled(selectedIndex == nil)
                GlassButton(localization.t("common.skip"), variant: .ghost, size: .large) { viewModel.dismiss() }
                    .tvFocusStyle()
            }
        }
