...
/// before pausing playback. Uses absolute `Date` target timestamps so the
/// countdown survives app backgrounding on iOS.
@MainActor
@Observable
final class SleepTimerManager {
    ...
    private let logger = BayitLogger(category: "SleepTimer")
    ...
    private static let fadeDurationMs = 5000
    ...
    func start(
        ...
        logger.info("Sleep timer started", context: ["durationMinutes": "\(clamped)"])
    ...
    func extend(additionalMinutes: Int) {
        ...
        logger.info("Sleep timer extended", context: ["additionalMinutes": "\(additionalMinutes)"])
    }
