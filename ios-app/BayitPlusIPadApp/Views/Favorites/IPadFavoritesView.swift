...
/// iPad-optimized favorites with 4-column grid and filter chips
struct IPadFavoritesView: View {
    ...
    var body: some View {
        ScrollView(.vertical, showsIndicators: false) {
            PageHeader(icon: "heart.fill", title: localization.t("profile.favorites"))
            ...
            ForEach(vm.items) { item in
                ...
                .contextMenu {
                    Button(role: .destructive) {
                        Task { await vm.removeFavorite(contentId: item.contentId ?? item.id) }
                    } label: {
                        Label(localization.t("favorites.remove"), systemImage: "heart.slash")
                    }
                }
            }
            if vm.items.count < vm.total {
                Color.clear.frame(height: 1).onAppear { Task { await vm.loadMore() } }
            }
        }
