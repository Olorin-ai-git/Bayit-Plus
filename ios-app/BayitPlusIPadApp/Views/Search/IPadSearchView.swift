...
/// iPad-optimized search with full-width layout and multi-column results grid
struct IPadSearchView: View {
    ...
    var body: some View {
        ...
        .task {
            if viewModel == nil {
                let vm = SearchViewModel(
                    searchRepository: repos.search, featureFlags: featureFlags
                )
                viewModel = vm
            ...
            if let vm = viewModel {
                SortOptionsSheet(
                    selectedSort: Binding(
                        get: { vm.sortOption }, set: { vm.sortOption = $0 }
                    ),
        ...
        .sheet(isPresented: sheetBinding(\.showFilterSheet)) {
            if let vm = viewModel {
                SearchFilterSheet(
                    filters: Binding(
                        get: { vm.advancedFilters }, set: { vm.advancedFilters = $0 }
                    ),
    ...
    private func sheetBinding(
        _ keyPath: ReferenceWritableKeyPath<SearchViewModel, Bool>
    ) -> Binding<Bool> {
        Binding(
    ...
    private func searchBar(_ vm: SearchViewModel) -> some View {
        GlassSearchBar(
            text: Binding(
                get: { vm.query }, set: { vm.query = $0; vm.onQueryChanged() }
            ),
    ...
    private func searchToolbar(_ vm: SearchViewModel) -> some View {
        HStack(spacing: DesignTokens.Spacing.md) {
            Button { vm.showSortSheet = true } label: {
                Label(
                    localization.t(vm.sortOption.localizationKey),
                    systemImage: "arrow.up.arrow.down"
                )
                .font(.system(size: DesignTokens.FontSize.sm))
                .foregroundColor(
                    vm.sortOption != .relevance
                        ? DesignTokens.Primary.default : DesignTokens.Text.secondary
                )
            }
            Spacer()
            Button { vm.showFilterSheet = true } label: {
                ...
                .font(.system(size: DesignTokens.FontSize.sm))
                .foregroundColor(
                    vm.advancedFilters.isEmpty
                        ? DesignTokens.Text.secondary : DesignTokens.Primary.default
                )
            }
        }
        .padding(.horizontal, DesignTokens.Spacing.xl)
    ...
    private func searchContent(_ vm: SearchViewModel) -> some View {
        if vm.isSearching {
            ...
            SearchResultsGridView(results: vm.results) {
                coordinator.navigate(to: $0)
            }
            if vm.hasMore {
                if vm.isLoadingMore {
                    ProgressView().tint(DesignTokens.Primary.default).padding()
                } else {
    ...
    private func emptyState(_ vm: SearchViewModel) -> some View {
        VStack(spacing: DesignTokens.Spacing.lg) {
            ...
            if !vm.advancedFilters.isEmpty || vm.selectedFilter != .all {
                GlassButton(
                    localization.t("search.clearFilters"),
