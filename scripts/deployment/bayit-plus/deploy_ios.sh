...
main() {
    ...
    if [[ "$SKIP_UPLOAD" != "true" ]]; then
        if [[ -z "$APP_STORE_CONNECT_API_KEY_ID" ]] || [[ -z "$APP_STORE_CONNECT_ISSUER_ID" ]] || [[ -z "$APP_STORE_CONNECT_API_KEY_PATH" ]]; then
            log_warning "App Store Connect API credentials not set. Upload will be skipped."
            log_info "Set APP_STORE_CONNECT_API_KEY_ID, APP_STORE_CONNECT_ISSUER_ID, and APP_STORE_CONNECT_API_KEY_PATH"
            SKIP_UPLOAD=true
        else
    ...
    if [[ ! -f "$EXPORT_OPTIONS_PLIST" ]]; then
        log_substep "Creating ExportOptions-iOS.plist..."
...
EOF
    ...
    if [[ "$SKIP_UPLOAD" != "true" ]]; then
        ...
        log_info "IPA available at: $IPA_FILE"
    fi
    ...
    # Summary
    log_step "iOS Deployment Summary"
