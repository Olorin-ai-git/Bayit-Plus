...
echo "   ${BLUE}gcloud run services describe bayit-backend-production --region=${REGION} --format='value(spec.serviceAccountName)'${NC}"
echo ""
echo "3. Test GCS access from Cloud Run:"
echo "   ${BLUE}curl https://bayit-backend-production-<hash>.a.run.app/api/v1/health${NC}"
echo ""
