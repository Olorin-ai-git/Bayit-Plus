...
# Configuration
# =============================================================================
# Centralized deployment script paths
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
OLORIN_ROOT="$(cd "$SCRIPT_DIR/../../.." && pwd)"
PROJECT_ROOT="$OLORIN_ROOT/olorin-fraud/backend"
...
check_deployment_prerequisites() {
    ...
    check_prerequisites "gcloud" "docker" "poetry" || exit 1
...
verify_deployment() {
    ...
    # Use shared Cloud Run verification with custom retry count
    if verify_cloud_run_deployment "$SERVICE_NAME" "$REGION" "$PROJECT_ID" 18; then
        # Additional check for OpenAPI docs
        local service_url
        service_url=$(get_cloud_run_url "$SERVICE_NAME" "$REGION" "$PROJECT_ID")
