...
# Get script directory and root (centralized deployment script)
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
OLORIN_ROOT="$(cd "$SCRIPT_DIR/../../.." && pwd)"
FRONTEND_ROOT="$OLORIN_ROOT/olorin-fraud/frontend"
...
print_header "Olorin Frontend Deployment"
...
check_environment() {
    ...
    if [ ! -d "dist" ] && [ "$SKIP_BUILD" = "true" ]; then
        log_error "No build found and SKIP_BUILD=true. Run build first."
        exit 1
    fi
    ...
    if ! firebase projects:list >/dev/null 2>&1; then
        log_error "Firebase not authenticated. Run: firebase login"
        exit 1
    fi
...
pre_deployment_health_check() {
    log_step "Pre-deployment Health Check"
    ...
    for service in "${services[@]}"; do
        if [ ! -f "dist/$service/remoteEntry.js" ]; then
            log_error "$service build missing"
            exit 1
        fi
    done
    ...
    for service in "${services[@]}"; do
        ...
        if [ $size_mb -gt 50 ]; then
            log_warning "$service bundle is large: ${size_mb}MB"
        fi
    done
...
EOF
    ...
    log_success "Production URLs configured"
}
...
invalidate_cdn() {
    if [ -z "$CDN_DISTRIBUTION_ID" ]; then
        log_warning "CDN_DISTRIBUTION_ID not set, skipping cache invalidation"
        return 0
    fi

    log_step "Invalidating CDN Cache"

    if [ "$DRY_RUN" = "true" ]; then
        log_warning "DRY RUN: Would invalidate CDN cache"
        return 0
    fi
    ...
    if command -v aws >/dev/null 2>&1; then
        ...
        log_warning "AWS CLI not found, skipping CDN invalidation"
    fi
}
...
post_deployment_health_check() {
    ...
    for service in "${!SERVICE_URLS[@]}"; do
        ...
        if curl -sf --max-time 10 "$remote_entry_url" >/dev/null; then
            log_success "$service remote entry accessible"
        else
            log_error "$service remote entry not accessible"
    ...
    log_success "All post-deployment health checks passed"
}

# Rollback on failure
rollback_deployment() {
    if [ "$ROLLBACK_ON_FAILURE" != "true" ]; then
        log_warning "Rollback disabled (ROLLBACK_ON_FAILURE=false)"
        return 0
    fi

    log_step "Rolling Back Deployment"

    log_warning "Automatic rollback not implemented yet"
    log_warning "Please use Firebase console or CLI to rollback manually"
...
EOF
    ...
    if [ "$DRY_RUN" = "true" ]; then
        log_warning "DRY RUN: Would send notification"
        return 0
    fi
...
main() {
    ...
    trap 'deployment_status="failed"; log_error "Deployment failed on line $LINENO"' ERR
