#!/usr/bin/env python3
"""
Generate character response videos for BTTF Part II interactive moments.
...
Usage:
    cd backend && poetry run python ../scripts/backend/generate_bttf2_character_responses.py
"""
...
async def generate_tts_audio(
    ...
    with tempfile.NamedTemporaryFile(
        suffix=".mp3", delete=False, prefix=f"tts_bttf2_{safe_name}_",
    ) as f:
        f.write(resp.content)
        audio_local = f.name

    gcs_path = (
        f"vod-interactions/character-audio/bttf2_{safe_name}_{text_hash}.mp3"
    )
...
async def run_aurora_lipsync(
    ...
    with tempfile.NamedTemporaryFile(
        suffix=".mp4", delete=False, prefix=f"aurora_bttf2_{moment_idx}_",
    ) as f:
    ...
    video_gcs_path = (
        ...
        f"bttf2_moment_{moment_idx}_{video_hash}.mp4"
    )
...
async def main():
    ...
    if not content:
        logger.error("BTTF Part II not found (imdb_id=%s)", BTTF2_IMDB_ID)
        client.close()
        sys.exit(1)

    moments = content.get("interactive_moments", [])
    logger.info("Found %d interactive moments for BTTF2", len(moments))
    ...
    for idx, moment in enumerate(moments):
        ...
        try:
            ...
            label = f"bttf2_moment_{idx}_{char_name}"
