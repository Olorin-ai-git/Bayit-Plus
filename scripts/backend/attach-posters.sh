...
async def process_vod_content(
    ...
    for idx, item in enumerate(items, 1):
        ...
        try:
            ...
            if is_series:
                metadata = await tmdb.enrich_series_content(item.title, item.year)
            else:
                metadata = await tmdb.enrich_movie_content(item.title, item.year)
            ...
            if metadata.get("poster"):
                ...
                if not dry_run:
                    ...
                    # Update original title and language
                    if metadata.get("original_title") and not item.original_title:
                        item.original_title = metadata["original_title"]
                        fields_updated.append("original_title")

                    if metadata.get("original_language") and not item.original_language:
                        item.original_language = metadata["original_language"]
                        fields_updated.append("language")

                    # Update tagline
                    if metadata.get("tagline") and not item.tagline:
                        item.tagline = metadata["tagline"]
                        fields_updated.append("tagline")

                    # Update status
                    if metadata.get("status") and not item.status:
                        item.status = metadata["status"]

                    # Update popularity
                    if metadata.get("popularity"):
                        item.popularity = metadata["popularity"]
