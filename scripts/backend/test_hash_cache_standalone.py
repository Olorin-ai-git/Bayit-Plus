...
async def test_hash_caching():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        monitor_service_2._hash_cache = monitor_service_2._load_hash_cache(
            fake_folder_id
        )
        ...
        for file_path in test_files:
            ...
            file_hash = await monitor_service_2._get_or_calculate_hash(
                str(file_path), file_stat
            )
        ...
        calc_start = time.time()
        new_hash = await monitor_service_2._get_or_calculate_hash(
            str(modified_file), file_stat
        )
        calc_time = time.time() - calc_start
        ...
        calc_start = time.time()
        cached_hash = await monitor_service_2._get_or_calculate_hash(
            str(modified_file), file_stat
        )
        calc_time = time.time() - calc_start
