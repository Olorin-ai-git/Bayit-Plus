#!/usr/bin/env python3
"""
Upload The Two Towers Extended Part 2 from local MKV to HLS.
...
SOURCE_FILE = (
    "/Users/olorin/Movies/"
    "THE LORD OF THE RINGS- THE TWO TOWERS (EXT.) PT. 2/"
    "THE LORD OF THE RINGS- THE TWO TOWERS (EXT.) PT. 2_t00.mkv"
)

GCS_PATH = "movies/The_Lord_of_the_Rings_The_Two_Towers_Extended_Part_2/hls"
CONTENT_TITLE = "The Lord of the Rings: The Two Towers (Extended) - Part 2"
...
async def get_tmdb_metadata(title: str, api_key: str) -> dict | None:
    """Fetch movie metadata from TMDB API, including collection data."""
    try:
        import httpx
        ...
        async with httpx.AsyncClient() as client:
            response = await client.get(
                ...
                params={"api_key": api_key, "query": "The Lord of the Rings The Two Towers"},
                timeout=10.0,
            )
                ...
                if details_resp.status_code == 200:
                    ...
                    if btc:
                        ...
                        if btc.get("poster_path"):
                            result["collection_poster"] = (
                                f"https://image.tmdb.org/t/p/w500{btc['poster_path']}"
                            )
                        logger.info(
                            "  Collection: %s (ID: %s)",
                            result["collection_name"], result["collection_id"],
                        )
            return result
    except Exception as e:
...
async def detect_collection(
    db, content_id: str, tmdb_collection_id: int,
    tmdb_collection_name: str, api_key: str,
):
    ...
    if len(movies) < 2:
        logger.info(
            "  Collection has %d movie(s) - need 2+ to create collection parent",
            len(movies),
        )
        return
    parent = await db.content.find_one(
        {"tmdb_collection_id": tmdb_collection_id, "is_collection_parent": True},
    )
    import httpx
    collection_meta = {
        "name": tmdb_collection_name, "overview": None,
        "poster": None, "backdrop": None, "total": 0,
    }
    try:
        async with httpx.AsyncClient() as client:
            ...
            if resp.status_code == 200:
                ...
                if cdata.get("poster_path"):
                    collection_meta["poster"] = (
                        f"https://image.tmdb.org/t/p/w500{cdata['poster_path']}"
                    )
                if cdata.get("backdrop_path"):
                    collection_meta["backdrop"] = (
                        f"https://image.tmdb.org/t/p/w1280{cdata['backdrop_path']}"
                    )
    except Exception as e:
    ...
    if not parent:
        ...
        logger.info(
            "  Created collection parent: %s (ID: %s)",
            collection_meta["name"], parent_id,
        )
    else:
    ...
    for idx, movie_doc in enumerate(sorted_movies, start=1):
        ...
        logger.info(
            "  Linked '%s' to collection (order: %d)",
            movie_doc.get("title"), idx,
        )
...
async def main():
    ...
    logger.info("=" * 70)
    logger.info("Two Towers Extended Part 2 - HLS Upload Pipeline")
    logger.info("=" * 70)
    ...
    temp_dir = tempfile.mkdtemp(prefix="hls_two_towers_pt2_")
    ...
    try:
        ...
        logger.info("")
        logger.info("=" * 70)
        logger.info("STAGE 1: Converting to HLS (this will take 40-60 minutes)...")
        logger.info("=" * 70)
        ...
        content_data = {
            ...
            "description": (
                ...
                if tmdb
                else "The Two Towers Extended Edition - Part 2"
            ),
            "content_type": "movie",
