#!/usr/bin/env python3
"""Upload Back to the Future (Blu-ray) from local MKV to HLS."""
...
SOURCE_FILE = "/Users/olorin/Movies/Back to the Future/Back to the Future_t08.mkv"
GCS_PATH = "movies/Back_to_the_Future/hls"
CONTENT_TITLE = "Back to the Future"
IMDB_ID = "tt0088763"
TMDB_SEARCH = "Back to the Future"
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.get(
                ...
                params={"api_key": api_key, "query": title}, timeout=10.0,
            )
...
async def detect_collection(db, content_id: str, tmdb_collection_id: int, tmdb_collection_name: str, api_key: str):
    ...
    # Link movies to collection parent, sorted by year
...
async def main():
    ...
    logger.info("=" * 70)
    logger.info("Back to the Future - HLS Upload Pipeline")
    logger.info("=" * 70)
    ...
    temp_dir = tempfile.mkdtemp(prefix="hls_bttf1_")
    ...
    try:
        ...
        content_data = {
            ...
            "year": int(tmdb["year"]) if (tmdb and tmdb.get("year")) else 1985,
