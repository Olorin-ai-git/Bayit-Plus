#!/usr/bin/env python3
"""Attach OCR'd PGS subtitles to The Karate Kid (1984) and mark as featured."""
...
SRT_DIR = "/Users/olorin/Movies/Karate Kid, The (1984)"
SRT_FILES = {
    "en": "Karate Kid, The (1984)_t00.en.srt",
    "es": "Karate Kid, The (1984)_t00.es.srt",
}
GCS_SUBS_PATH = "movies/The_Karate_Kid_1984/subtitles"
IMDB_ID = "tt0087538"
...
def parse_srt_to_cues(content: str) -> list[dict]:
    """Parse SRT content into list of cue dicts."""
    cues = []
    blocks = re.split(r"\n\s*\n", content.strip())
...
def upload_srt_to_gcs(
    srt_path: str, gcs_key: str, bucket_name: str,
) -> str:
    """Upload SRT file to GCS and return public URL."""
...
async def main():
    ...
    logger.info("=" * 70)
    logger.info("Attach Subtitles + Mark Featured: The Karate Kid (1984)")
    logger.info("=" * 70)

    client = AsyncIOMotorClient(settings.MONGODB_URI)
    db = client[settings.MONGODB_DB_NAME]

    content = await db.content.find_one({"imdb_id": IMDB_ID, "year": 1984})
    if not content:
        logger.error("Content not found for IMDB %s (year 1984)", IMDB_ID)
        logger.info("The upload pipeline may not have completed yet.")
        client.close()
        sys.exit(1)
    ...
    for lang_code, srt_filename in SRT_FILES.items():
        ...
        source_url = upload_srt_to_gcs(
            srt_path, gcs_key, settings.GCS_BUCKET_NAME,
        )

        # Check for existing subtitle track
        ...
        if existing:
            ...
            native_name, en_name = LANGUAGE_NAMES.get(
            ...
            track_doc = {
                ...
                "source_url": source_url,
                "cues": cues,
                "has_nikud_version": False,
                "has_shoresh_version": False,
                "has_heblish_version": False,
                "has_grammar_flip_version": False,
                "has_slang_synthesis_version": False,
                "has_engrew_version": False,
    ...
    update_fields = {"is_featured": True, "updated_at": datetime.now(UTC)}
    ...
    logger.info("  Subtitles: %s", inserted_languages or "none (OCR not ready)")
    logger.info("  Featured: True")
