#!/usr/bin/env python3
"""Upload Law Abiding Citizen (2009) (Blu-ray) from local MKV to HLS."""
...
SOURCE_FILE = "/Users/olorin/Movies/LAW ABIDING CITIZEN/title_t00.mkv"
GCS_PATH = "movies/Law_Abiding_Citizen_2009/hls"
CONTENT_TITLE = "Law Abiding Citizen"
IMDB_ID = "tt1197624"
TMDB_SEARCH = "Law Abiding Citizen"
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.get(
                ...
                params={"api_key": api_key, "query": title, "year": 2009}, timeout=10.0,
            )
...
async def main():
    ...
    logger.info("=" * 70)
    logger.info("Law Abiding Citizen (2009) - HLS Upload Pipeline")
    logger.info("=" * 70)
    ...
    logger.info("  Source: %.0f MB, %.0f min, 1080p MKV (VC-1)", file_size_mb, duration_s / 60)

    temp_dir = tempfile.mkdtemp(prefix="hls_law_abiding_citizen_")
    ...
    try:
        logger.info("")
        logger.info("=" * 70)
        logger.info("STAGE 1: Converting to HLS (VC-1 -> H.264, ~40-60 minutes)...")
        logger.info("=" * 70)
        ...
        content_data = {
            ...
            "year": int(tmdb["year"]) if (tmdb and tmdb.get("year")) else 2009,
            ...
            "metadata": {
                ...
                "source_format": "mkv", "source_codec": "vc1",
