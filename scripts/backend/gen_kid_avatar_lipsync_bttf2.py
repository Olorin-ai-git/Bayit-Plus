#!/usr/bin/env python3
"""
Generate kid avatar lipsync videos for all BTTF Part II moments.

Uses ElevenLabs TTS (Etai's cloned voice) + fal.ai Aurora lip-sync
+ GCS upload + MongoDB update.

Usage:
    cd backend && poetry run python ../scripts/backend/gen_kid_avatar_lipsync_bttf2.py
"""
...
async def main():
    ...
    logger.info("Found %d moments for BTTF2", len(moments))
        ...
        for idx, moment in enumerate(moments):
            ...
            try:
                ...
                with tempfile.NamedTemporaryFile(
                    suffix=".mp3",
                    delete=False,
                    prefix=f"kid_tts_bttf2_{safe_name}_",
                ) as f:
                    f.write(audio_data)
                    audio_local = f.name
                logger.info(
                ...
                audio_gcs_path = (
                    ...
                    f"bttf2_kid_{safe_name}_{audio_hash}.mp3"
                )
                ...
                with tempfile.NamedTemporaryFile(
                    suffix=".mp4",
                    delete=False,
                    prefix=f"kid_aurora_bttf2_{safe_name}_",
                ) as f:
                ...
                video_gcs_path = (
                    ...
                    f"bttf2_kid_{safe_name}_{video_hash}.mp4"
                )
    ...
    mongo.close()
    logger.info("Done generating kid avatar lipsync for BTTF2")
