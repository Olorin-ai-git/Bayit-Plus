# Real-time/Offline Trivia and Fun Facts Feature - V8
...
**V8 CHANGES**: This version addresses the 4 critical issues from V7 review to achieve 13/13 approval.

### V8 Critical Fixes Summary

| # | Issue | Severity | V8 Fix |
|---|-------|----------|--------|
| 1 | ContentTrivia not registered | Critical (BLOCKING) | Added explicit database.py registration section |
| 2 | jest.config.js missing | Critical | Changed from "ADD/UPDATE" to "CREATE" with full file |
| 3 | useEffect cleanup dependency | Critical | Fixed dependency array from `[setCurrentFact]` to `[]` |
| 4 | GlassSelect missing minHeight | Critical | Added `minHeight: SETTINGS_CONFIG.touchMinHeight` |

### V7 Critical Fixes (All Verified)

| # | Issue | Status | Verification |
|---|-------|--------|--------------|
| 1 | tvOS touch targets | ✅ FIXED | Changed to 250x100pt via OVERLAY_CONFIG |
| 2 | Rate limiting | ✅ FIXED | Added to RATE_LIMITS dict |
| 3 | content_id validation | ✅ FIXED | Added validate_object_id() |
| 4 | AI sanitization | ✅ FIXED | Added sanitize_ai_output() |
| 5 | Prompt injection | ✅ FIXED | Added sanitize_for_prompt() |
| 6 | Installation paths | ✅ FIXED | Corrected to mobile-app/ios, etc. |
| 7 | NetInfo mock location | ✅ FIXED | Moved to mobile-app/src/setupTests.js |
| 8 | GlassSelect | ✅ VERIFIED | Exported at native/index.ts:31 |
...
### Database Registration (V8 CRITICAL FIX - BLOCKING)

**THIS IS A BLOCKING ISSUE: Without this registration, Beanie will not initialize the ContentTrivia collection and ALL trivia endpoints will fail with errors.**

```python
# /backend/app/core/database.py - MUST ADD TO document_models LIST

# V8 CRITICAL FIX: Add import at module level (around line 71)
from app.models.trivia import ContentTrivia

# V8 CRITICAL FIX: Add to document_models list (after ContentDiscoveryQueue, around line 202)
document_models: List[Type[Document]] = [
    # ... existing models ...
    ContentDiscoveryQueue,
    # Trivia models - V8 ADDITION (BLOCKING FIX)
    ContentTrivia,
]
```

**Verification Steps:**
1. Confirm import is at module level, not inside a function
2. Confirm ContentTrivia is in the document_models list
3. Run: `poetry run python -c "from app.core.database import document_models; print([m.__name__ for m in document_models])"` - must include "ContentTrivia"
...
### Jest Configuration (V8 CRITICAL FIX - CREATE NEW FILE)
...
export function useTrivia({
  ...
  // V8 CRITICAL FIX: Cleanup on unmount - empty dependency array
  // The previous version had [setCurrentFact] which caused issues because:
  // 1. setCurrentFact is a stable reference from zustand, but including it
  //    in deps can cause unnecessary cleanup/re-registration
  // 2. This is a cleanup-only effect that should run ONLY on unmount
  useEffect(() => {
    return () => {
  ...
  }, []);  // V8 FIX: Empty array - cleanup only on unmount
      ...
      {preferences.enabled && (
        <>
          <View style={styles.row}>
            ...
            {/* V8 CRITICAL FIX: Added minHeight for tvOS 250x100pt requirement */}
            <GlassSelect
              ...
              style={[styles.select, {
                ...
                minHeight: SETTINGS_CONFIG.touchMinHeight,  // V8 FIX: Added minHeight
              }]}
            />
          </View>
...
- `mobile-app/jest.config.js` - **V8 FIX: CREATE NEW FILE** (not "ADD/UPDATE")
...
- `backend/app/core/database.py` - **V8 CRITICAL: Register ContentTrivia model** (BLOCKING)
...
### V8 Critical Fixes
- [ ] **database.py registration**: ContentTrivia added to document_models list
  - Verify: `poetry run python -c "from app.core.database import document_models; print([m.__name__ for m in document_models])"` includes "ContentTrivia"
- [ ] **jest.config.js created**: File exists at `/mobile-app/jest.config.js`
  - Verify: `ls mobile-app/jest.config.js` succeeds
- [ ] **useEffect cleanup**: Dependency array is `[]` (empty), not `[setCurrentFact]`
  - Verify: Line ~1210 has `}, []);` not `}, [setCurrentFact]);`
- [ ] **GlassSelect minHeight**: Both minWidth AND minHeight set in TriviaSettingsSection
  - Verify: Line ~1574 has both `minWidth` and `minHeight`
...
| # | Issue | Version | Status | Verification |
|---|-------|---------|--------|--------------|
| 1 | ContentTrivia not registered | V8 | ✅ FIXED | Added to database.py with explicit instructions |
| 2 | jest.config.js missing | V8 | ✅ FIXED | Changed to "CREATE NEW FILE" |
| 3 | useEffect dependency array | V8 | ✅ FIXED | Changed `[setCurrentFact]` to `[]` |
| 4 | GlassSelect missing minHeight | V8 | ✅ FIXED | Added `minHeight: SETTINGS_CONFIG.touchMinHeight` |
| 5 | tvOS touch targets | V7 | ✅ FIXED | 250x100pt via OVERLAY_CONFIG |
| 6 | Rate limiting | V7 | ✅ FIXED | Added to RATE_LIMITS dict |
| 7 | content_id validation | V7 | ✅ FIXED | validate_object_id() |
| 8 | AI sanitization | V7 | ✅ FIXED | sanitize_ai_output() |
| 9 | Prompt injection | V7 | ✅ FIXED | sanitize_for_prompt() |
| 10 | Installation paths | V7 | ✅ FIXED | Corrected paths |
| 11 | NetInfo mock location | V7 | ✅ FIXED | mobile-app/src/setupTests.js |
| 12 | GlassSelect | V7 | ✅ VERIFIED | native/index.ts:31 |
