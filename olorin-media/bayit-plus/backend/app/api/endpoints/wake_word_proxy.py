...
class WakeWordRequest(BaseModel):
    ...
    sensitivity: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Detection sensitivity (0.0-1.0)"
    )
...
class WakeWordResponse(BaseModel):
    ...
    wake_word: Optional[str] = Field(
        default=None, description="Detected wake word phrase"
    )
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(url, headers=headers, data=files) as response:
                if response.status != 200:
                    ...
                    logger.error(
                        f"[WakeWord] Picovoice error: {response.status} - {error_text}"
                    )
                    raise HTTPException(
                ...
                return WakeWordResponse(
                    detected=detected, confidence=confidence, wake_word=wake_word
                )
