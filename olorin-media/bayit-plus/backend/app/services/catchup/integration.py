...
class CatchUpIntegration:
    ...
    def __init__(
        ...
        """Initialize catch-up integration with user/channel context."""
    ...
    async def generate_catchup_with_credits(
        ...
        credit_cost = settings.olorin.catchup.credit_cost

        if self._credit_service:
            await self._verify_and_authorize(feature_name, credit_cost)
        ...
        try:
            ...
            if self._credit_service:
                remaining_credits = await self._deduct_credits(
                    feature_name, credit_cost, target_language,
                    window_minutes, summary.get("cached", False),
                )
    ...
    async def _verify_and_authorize(
        self, feature_name: str, credit_cost: float,
    ) -> None:
        """Verify beta status and pre-authorize credits."""
        ...
        if not authorized:
            ...
            raise ValueError(
                f"Insufficient credits for catch-up. Available: {remaining}"
            )

    async def _deduct_credits(
        self, feature_name: str, credit_cost: float,
        target_language: str, window_minutes: Optional[int], cached: bool,
    ) -> Optional[float]:
    ...
    async def check_available(self) -> dict:
        ...
        available = await self._session_manager.check_catchup_available(
            self.channel_id,
        )
