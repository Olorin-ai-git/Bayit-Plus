...
async def hash_and_save_file(
    ...
    try:
        ...
        existing = await Content.find_one(
            Content.stream_url == str(file_path.absolute())
        )
        ...
        file_hash = await monitor_service._get_or_calculate_hash(
            str(file_path), file_stat
        )
        ...
        if existing_by_hash:
            logger.info(
                f"     ⚠️  DUPLICATE: Matches '{existing_by_hash.title}' in database"
            )
            stats["duplicates"] += 1
            return False
...
async def scan_folder(
    folder: MonitoredFolder, monitor_service: FolderMonitorService, stats: Dict
):
    ...
    patterns = (
        ...
        else monitor_service._get_default_patterns(folder.content_type)
    )
    ...
    for i, file_path in enumerate(found_files, 1):
        ...
        await hash_and_save_file(
            Path(file_path), folder.content_type, monitor_service, stats
        )
...
async def main():
    ...
    if cache_dir.exists():
        ...
        logger.info(
            f"   {len(cache_files)} cache file(s), {total_cache_size:,} bytes total"
        )
