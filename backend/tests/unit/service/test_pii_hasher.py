...
def test_tier_based_exclusion():
    ...
    assert (
        hashed_tier1["CARD_BIN"] == data["CARD_BIN"]
    ), "Tier 1: CARD_BIN should not be hashed"
    ...
    assert (
        hashed_tier2["CARD_BIN"] == data["CARD_BIN"]
    ), "Tier 2: CARD_BIN should not be hashed"
    ...
    assert (
        hashed_tier3["CARD_BIN"] != data["CARD_BIN"]
    ), "Tier 3: CARD_BIN should be hashed"
    assert (
        hashed_tier3["MODEL_SCORE"] == data["MODEL_SCORE"]
    ), "Tier 3: MODEL_SCORE should not be hashed"
