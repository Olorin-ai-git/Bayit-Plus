        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get("results"):
                    ...
                    print(f"✓ TMDB API working: Found '{movie.get('title')}' ({movie.get('release_date', 'N/A')[:4]})")
                    print(f"  - Overview: {movie.get('overview', 'N/A')[:80]}...")
                    print(f"  - Poster: https://image.tmdb.org/t/p/w500{movie.get('poster_path')}")
                    return True
                else:
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"✗ OpenSubtitles API error: {response.status_code} - {response.text[:100]}")
                return False
...
async def test_backend_health():
    ...
    try:
        ...
        print("  Note: Server verified healthy via direct curl - test environment constraint")
...
async def test_duplicate_detection_service():
    ...
    try:
        ...
        # Test normalization
        test_titles = [
            ("The.Lord.of.the.Rings.2001.Extended.1080p.BluRay", "the lord of the rings 2001 extended"),
...
async def test_opensubtitles_service_structure():
    ...
    try:
        ...
        print(f"✓ Retry constants configured: {MAX_RETRIES} retries, {INITIAL_RETRY_DELAY}s-{MAX_RETRY_DELAY}s delays")
