...
```
pydantic_core._pydantic_core.ValidationError: 2 validation errors for OlorinHeader
...
## Root Cause
Tests that use `ainvoke_agent` create an `OlorinHeader` object, which requires string values for:
- `olorin_experience_id`
- `olorin_originating_assetalias`
...
The fix ensures that all tests that invoke the LLM (via `ainvoke_agent`) have properly configured mock settings that satisfy the `OlorinHeader` Pydantic model validation requirements. 
