...
class TestStructuredInvestigationE2E:
    ...
    async def test_full_investigation_lifecycle(
        self, real_investigation_context, api_cost_monitor, db_session
    ):
        ...
        try:
            ...
            network_findings = await structured_network_agent(
                real_investigation_context, config
            )
            phases["network"] = {
            ...
            journey_tracker.complete_journey(
                ...
                final_output={
                    ...
                    "final_risk_score": final_risk.risk_score,
                    "total_findings": sum(
                        len(phase["findings"].key_findings) for phase in phases.values()
                    ),
                    "status": "completed",
                },
            )
            ...
            journey_tracker.complete_journey(
                journey_id, final_output={"error": str(e), "status": "failed"}
            )
            raise
    ...
    @pytest.mark.asyncio
    async def test_concurrent_investigations(self, real_database, api_cost_monitor):
        ...
        risk_scores = [
            assessment.get("risk_level", 0.0) for assessment in risk_assessments
        ]
    ...
    async def test_investigation_with_webhooks(
        self, real_investigation_context, api_cost_monitor, real_webhook_server
    ):
    ...
    async def test_investigation_error_handling(
        self, real_investigation_context, api_cost_monitor
    ):
        """Test investigation error handling and recovery with real API."""
        config = RunnableConfig(
            tags=["test", "error_handling"], metadata={"test_type": "error_recovery"}
        )
...
class TestScenarioBasedInvestigations:
    ...
    @pytest.mark.asyncio
    async def test_account_takeover_detection(self, real_database, api_cost_monitor):
        ...
        logger.info(
            f"ATO test - Final risk score: {risk_score}, confidence: {confidence}"
        )
        ...
        recommendations = (
            final_risk_assessment.get("recommended_actions", [])
            if final_risk_assessment
            else []
        )
        if recommendations:
            assert any(
                "block" in str(action).lower() or "freeze" in str(action).lower()
        ...
        logger.info(f"  Detected Indicators: {detected_indicators}")
        logger.info(
            f"  Recommendations: {recommendations[:3] if recommendations else 'None'}"
        )
    ...
    async def test_money_laundering_detection(
        ...
        config = RunnableConfig(
            tags=["test", "ml_detection"], metadata={"scenario": "money_laundering"}
        )
        ...
        findings["network"] = await structured_network_agent(
            real_investigation_context, config
        )
        api_cost_monitor.track_call(2100, 1900)

        findings["logs"] = await structured_logs_agent(
            real_investigation_context, config
        )
        api_cost_monitor.track_call(2000, 1800)

        findings["location"] = await structured_location_agent(
            real_investigation_context, config
        )
        api_cost_monitor.track_call(1900, 1700)
        ...
        assert any(
            "report" in action.lower()
            or "sar" in action.lower()
            or "suspicious" in action.lower()
...
class TestPerformanceAndReliability:
    ...
    async def test_investigation_performance_metrics(
        ...
        config = RunnableConfig(
            tags=["test", "performance"], metadata={"test": "performance_metrics"}
        )
        ...
        for agent_name, agent_func, (input_tokens, output_tokens) in agents_to_test:
            ...
            logger.info(
                f"{agent_name} agent: {duration:.2f}s, risk={findings.risk_score:.2f}"
            )
    ...
    async def test_api_retry_and_resilience(
        ...
        config = RunnableConfig(
            tags=["test", "resilience"], metadata={"test": "retry_mechanisms"}
        )
    ...
    async def test_investigation_consistency(
        ...
        config = RunnableConfig(
            tags=["test", "consistency"], metadata={"test": "result_consistency"}
        )
    ...
    async def test_cost_optimization(
        ...
        config = RunnableConfig(
            tags=["test", "cost_optimization"], metadata={"test": "cost_analysis"}
        )

        # Strategy 1: Selective agent execution based on initial risk
        initial_findings = await structured_network_agent(
            real_investigation_context, config
        )
        api_cost_monitor.track_call(1500, 1200)
...
class TestWebToolsIntegration:
    ...
    async def test_web_search_domain_reputation(
        ...
        findings_text = " ".join(
            findings.key_findings + findings.suspicious_indicators
        ).lower()
        ...
        assert (
            len(detected_web_analysis) >= 1
        ), "Web domain analysis should be present in findings"

        # Check for PII sanitization in findings - should not contain raw PII
        findings_combined = " ".join(
            findings.key_findings + findings.suspicious_indicators
        )

        # These PII patterns should be sanitized (redacted)
        assert (
            "john.doe@suspicious-domain.com" not in findings_combined
        ), "Email PII should be sanitized"
        ...
        sanitization_detected = any(
            marker in findings_combined for marker in pii_markers
        )
    ...
    async def test_web_scraping_business_verification(
        ...
        detected_business_analysis = [
            ind for ind in business_indicators if ind in findings_text
        ]

        # Should detect web-based business verification
        assert (
            len(detected_business_analysis) >= 1
        ), "Web business verification should be present"
        ...
        pii_found_unsanitized = [
            data for data in sensitive_data if data in findings_combined
        ]
        assert (
            len(pii_found_unsanitized) == 0
        ), f"PII should be sanitized: {pii_found_unsanitized}"
        ...
        redactions_found = [
            marker for marker in expected_redactions if marker in findings_combined
        ]
    ...
    async def test_web_threat_intelligence_research(
        ...
        detected_threat_research = [
            ind for ind in threat_indicators if ind in findings_text
        ]

        # Should detect web-based threat intelligence gathering
        assert (
            len(detected_threat_research) >= 1
        ), "Web threat research should be evident in findings"
        ...
        assert (
            len(unsanitized_pii) == 0
        ), f"Critical PII must be sanitized: {unsanitized_pii}"

        # Should contain security-appropriate redactions
        expected_security_redactions = ["[REDACTED-EMAIL]", "[REDACTED-CREDIT_CARD]"]
        security_redactions = [
            marker
            for marker in expected_security_redactions
            if marker in findings_combined
        ]
    ...
    async def test_web_fraud_trend_analysis(
        ...
        api_cost_monitor.track_call(
            2300, 2100
        )  # Risk analysis with web research uses more tokens
        ...
        risk_text = " ".join(
            final_risk.key_findings + final_risk.suspicious_indicators
        ).lower()
        fraud_trend_indicators = [
            "trend",
            "pattern",
            "fraud",
            "recent",
        ...
        detected_trend_analysis = [
            ind for ind in fraud_trend_indicators if ind in risk_text
        ]

        # Should detect web-based fraud trend analysis
        assert (
            len(detected_trend_analysis) >= 1
        ), "Web fraud trend analysis should be present"
        ...
        exposed_pii = [
            element for element in sensitive_elements if element in all_findings_text
        ]
        assert (
            len(exposed_pii) == 0
        ), f"All PII should be sanitized in final risk: {exposed_pii}"
        ...
        found_markers = [
            marker for marker in sanitization_markers if marker in all_findings_text
        ]
        ...
        logger.info(
            f"  Recommended Actions: {final_risk.recommended_actions[:2] if hasattr(final_risk, 'recommended_actions') else 'N/A'}"
        )
    ...
    async def test_full_web_tools_investigation_lifecycle(
        ...
        for phase_name, findings in phases:
            ...
            sanitization_count = sum(
                phase_text.count(marker) for marker in redaction_markers
            )
            sanitization_effectiveness[phase_name] = sanitization_count > 0

        # Validate no PII exposure across any phase
        total_exposed_pii = sum(
            len(exposed) for exposed in pii_exposure_report.values()
        )
        assert (
            total_exposed_pii == 0
        ), f"PII found exposed in phases: {pii_exposure_report}"
        ...
        for phase_name, findings in phases:
            ...
            detected_indicators = [
                ind for ind in expected_indicators if ind in phase_text
            ]
            web_usage_detected[phase_name] = len(detected_indicators)

        # Each phase should show evidence of web tools usage
        phases_with_web_analysis = sum(
            1 for count in web_usage_detected.values() if count > 0
        )
        assert (
            phases_with_web_analysis >= 4
        ), f"Web tools should be used across phases: {web_usage_detected}"
        ...
        logger.info(
            f"  Sanitization Applied: {list(sanitization_effectiveness.values())}"
        )
