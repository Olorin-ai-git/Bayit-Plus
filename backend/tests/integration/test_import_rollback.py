...
class TestImportRollback:
    ...
    def create_test_investigation_files(
        self, investigation_id: str, entity_type: str, entity_id: str
    ):
        ...
        # Create report file
        (inv_dir / "report.html").write_text(
            f"<html><body>Report for {investigation_id}</body></html>"
        )
    ...
    def test_import_investigation_files(self):
        ...
        # Create source files
        source_dir = self.create_test_investigation_files(
            investigation_id, entity_type, entity_id
        )

        # Import files (simplified - copy to target)
        target_inv_dir = (
            self.target_dir / "investigations" / "2025" / "11" / investigation_id
        )
    ...
    def test_rollback_import(self):
        ...
        with self.registry._get_connection() as conn:
            ...
            cursor.execute(
                "DELETE FROM files WHERE investigation_id = ?", (investigation_id,)
            )
    ...
    def test_import_with_conflicts(self):
        ...
        with self.registry._get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT COUNT(*) FROM files WHERE sha256_hash = ?", (file_hash,)
            )
