...
class MockMessagesAPI:
    ...
    async def list(self, thread_id, **kwargs):
        return MockMessageList(
            [
                MockMessage(
                    "msg_assistant", "Investigation completed successfully", "assistant"
                )
            ]
        )
    ...
    async def stream(self, **kwargs):
        ...
        events = [
            MockStreamEvent(
                ...
                MockMessageDelta("Starting fraud investigation..."),
            ),
            MockStreamEvent(
            ...
            MockStreamEvent(
                "thread.message.delta",
                MockMessageDelta("Analysis complete. High risk detected."),
            ),
...
class OpenAIAssistantPatternTest:
    ...
    async def test_tool_conversion(self):
        ...
        logger.info(
            f"✓ Converted {len(function_definitions)} tools to OpenAI functions"
        )

        # Validate function definitions structure
        for func_def in function_definitions:
            if not all(
                key in func_def for key in ["name", "description", "parameters"]
            ):
        ...
        logger.info(
            f"✓ Conversion stats: {stats['convertible_tools']}/{stats['total_tools']} tools converted"
        )
        ...
        for expected in expected_tools:
            if not any(expected in fname for fname in function_names):
                logger.warning(
                    f"Expected tool '{expected}' not found in converted functions"
                )
    ...
    async def test_function_calling(self):
        ...
        with patch(
            ...
            result = await execute_openai_function_call(
                "splunk_query_tool", {"query": "search fraud indicators"}, tools
            )
    ...
    async def test_pattern_creation(self):
        ...
        pattern = OpenAIAssistantPattern(
            config=pattern_config, openai_config=openai_config, tools=tools
        )
    ...
    async def test_registry_integration(self):
        ...
        pattern_config = PatternConfig(
            pattern_type=PatternType.OPENAI_ASSISTANT, timeout_seconds=30
        )

        openai_config = OpenAIPatternConfig(model="gpt-4o", temperature=0.1)
        ...
        logger.info(
            f"✓ Registry info: {registry_info['openai_patterns']} OpenAI patterns registered"
        )
    ...
    async def test_streaming_integration(self, mock_openai_client):
        ...
        openai_config = OpenAIPatternConfig(
            model="gpt-4o", temperature=0.1, stream=True  # Enable streaming
        )
