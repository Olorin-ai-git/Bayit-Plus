...
def run_all_phases(base_url: str, verbose: bool = False, timeout: int = 600) -> list:
    ...
    for i, (phase_name, test_file) in enumerate(test_phases, 1):
        ...
        if phase_result["success"]:
            print(
                f"✅ Phase {i} PASSED: {phase_name} ({phase_result['duration']:.1f}s)"
            )
        else:
            print(
                f"❌ Phase {i} FAILED: {phase_name} ({phase_result['duration']:.1f}s)"
            )
    ...
    print(
        f"✅ Successful Phases: {sum(1 for r in results if r['success'])}/{len(results)}"
    )
...
def generate_comprehensive_report(results: list, base_url: str) -> dict:
    ...
    report = {
        ...
        "summary": {
            ...
            "platform_health": (
                "GOOD"
                if success_rate >= 90
                else "FAIR" if success_rate >= 70 else "POOR"
            ),
            "critical_issues": [],
            "recommendations": [],
        },
        ...
        if not result["success"]:
            ...
            if result["return_code"] == -1:
                report["summary"]["critical_issues"].append(
                    f"{result['phase']}: Test timed out"
                )
            else:
                report["summary"]["critical_issues"].append(
                    f"{result['phase']}: Test failed"
                )
    ...
    if success_rate < 100:
        report["summary"]["recommendations"].append(
            "Some endpoint tests failed - review failed phases for issues"
        )

    if any(r["duration"] > 300 for r in results):  # 5 minutes
        report["summary"]["recommendations"].append(
            "Some test phases took over 5 minutes - consider performance optimization"
        )

    if success_rate >= 95:
        report["summary"]["recommendations"].append(
            "Excellent API health - all systems functioning well"
        )
    elif success_rate >= 80:
        report["summary"]["recommendations"].append(
            "Good API health with minor issues to address"
        )
    else:
        report["summary"]["recommendations"].append(
            "API health concerns - immediate attention required"
        )
    ...
    if format_type == "json":
        ...
        with open(filepath, "w") as f:
            ...
            for phase in report["phase_results"]:
                ...
                f.write(
                    f"{status_icon} {phase['phase']}: {phase['status']} ({phase['duration_seconds']:.1f}s)\n"
                )
...
Examples:
    ...
    parser.add_argument(
        "--phase", type=int, choices=range(1, 8), help="Run specific phase only (1-7)"
    )
    ...
    parser.add_argument(
        "--timeout", type=int, default=600, help="Test timeout in seconds"
    )
    ...
    if not args.skip_server_check:
        if not check_server_availability(args.base_url):
            print(
                "❌ Server is not available. Use --skip-server-check to proceed anyway."
            )
            sys.exit(1)
    ...
    if args.phase:
        ...
        result = run_pytest_phase(
            phase_name, test_file, args.base_url, args.verbose, args.timeout
        )
        results = [result]
    else:
        # Run all phases
