...
async def verify_hash_caching_system():
    ...
    try:
        ...
        required_methods = ['_load_hash_cache', '_save_hash_cache', '_get_or_calculate_hash']
    ...
    try:
        ...
        checks = [
            ...
            ("TIER 3: Hash comparison", "existing_by_hash = await db.content.find_one({'file_hash'"),
    ...
    try:
        ...
        checks = [
            ("Pre-calculated hash check", "pre_calculated_hash = job.metadata.get('pre_calculated_hash')"),
            ("Background hash calculation", "job.file_hash = await self._calculate_file_hash"),
            ("Hash completion stage", "job.stages[\"hash_calculation\"] = \"completed\""),
            ("Duplicate check after hash", "existing_content = await db.content.find_one({'file_hash'"),
