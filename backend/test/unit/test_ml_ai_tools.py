...
from app.service.agent.tools.ml_ai_tools.pattern_recognition import (
...
class TestAnomalyDetectionTool:
    ...
    async def test_z_score_method(self, tool, sample_transaction_data):
        ...
        result = await tool._arun(
            data=sample_transaction_data, detection_methods=["z_score"], z_threshold=2.0
        )
...
class TestFraudDetectionTool:
    ...
    async def test_payment_fraud_detection(self, tool, sample_transaction_data):
        ...
        result = await tool._arun(
            fraud_types=["payment_fraud"], **sample_transaction_data
        )
    ...
    async def test_identity_fraud_detection(self, tool, sample_transaction_data):
        ...
        result = await tool._arun(
            fraud_types=["identity_fraud"], **sample_transaction_data
        )
...
class TestMLAIToolsIntegration:
    ...
    async def test_workflow_integration(self, all_tools):
        ...
        behavioral_result = await all_tools["behavioral"]._arun(
            **sample_data["behavioral_data"]
        )
        anomaly_result = await all_tools["anomaly"]._arun(
            data=sample_data["transaction_data"]
        )
