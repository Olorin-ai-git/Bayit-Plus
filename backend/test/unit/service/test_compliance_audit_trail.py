...
def audit_manager():
    ...
    return AuditTrailManager(
        ...
        pii_config=PIIMaskingConfig(
            mask_email=True, mask_phone=True, mask_ssn=True, mask_credit_card=True
        ),
    )
...
def test_compliance_report(audit_manager):
    ...
    audit_manager.log_event(
        event_type=AuditEventType.ACCESS, regulations=[ComplianceRegulation.GDPR]
    )
    audit_manager.log_event(
        event_type=AuditEventType.MODIFICATION, regulations=[ComplianceRegulation.CCPA]
    )
