...
TXN003,not_a_number,2024-01-15 12:00:00,Electronics Store"""
    ...
    def sample_messages_state(self, sample_csv_content):
        ...
        return MessagesState(messages=[HumanMessage(content=sample_csv_content)])
    ...
    @pytest.mark.asyncio
    async def test_validate_transaction_data_success(
        self, raw_data_node, sample_csv_content
    ):
        """Test successful transaction data validation."""
        df = await raw_data_node._parse_csv_content(sample_csv_content)
        validated_df, quality_report = await raw_data_node._validate_transaction_data(
            df
        )
    ...
    @pytest.mark.asyncio
    async def test_validate_transaction_data_with_errors(
        self, raw_data_node, invalid_csv_content
    ):
    ...
    async def test_detect_anomalies(self, raw_data_node):
        ...
        assert (
            "amount_outlier" in anomaly_types
            or "future_timestamp" in anomaly_types
            or "duplicate_transaction" in anomaly_types
        )
    ...
    def test_calculate_quality_score(self, raw_data_node):
        ...
        perfect_report = DataQualityReport(
            total_records=100, valid_records=100, invalid_records=0, data_issues={}
        )
        ...
        perfect_score = raw_data_node._calculate_quality_score(
            perfect_report, perfect_df
        )
    ...
    @pytest.mark.asyncio
    async def test_process_csv_data_bytes_input(
        self, raw_data_node, sample_csv_content
    ):
    ...
    async def test_process_csv_data_file_too_large(self, raw_data_node):
        ...
        result = await raw_data_node.process_csv_data(
            large_content.encode(), "large.csv"
        )
    ...
    @pytest.mark.asyncio
    async def test_langgraph_node_call_success(
        self, raw_data_node, sample_messages_state
    ):
    ...
    @pytest.mark.asyncio
    async def test_langgraph_node_call_with_dict_content(
        self, raw_data_node, sample_csv_content
    ):
        ...
        dict_state = MessagesState(
            messages=[
                HumanMessage(
                    content=str(
                        {"csv_data": sample_csv_content, "filename": "test_data.csv"}
                    ),
                    additional_kwargs={
        ...
        result_success = response_message.additional_kwargs.get(
            "raw_data_results", {}
        ).get("success", False)
...
class TestAsyncFunctionality:
    ...
    async def test_async_batch_processing_memory_efficiency(self):
        ...
        for i in range(10):
            large_data.append(
                f"TXN{i:03d},100.{i},2024-01-15 10:{i:02d}:00,Store{i},USER{i},City{i}"
            )

        csv_content = (
            "transaction_id,amount,timestamp,merchant,user_id,location\n"
            + "\n".join(large_data)
        )
...
class TestErrorHandling:
    ...
    async def test_malformed_csv_handling(self, raw_data_node):
        ...
        assert (result["success"] is False and "error" in result) or (
            result["success"] is True
            and result["quality_report"]["quality_score"] == 0.0
        )
