...
class OrchestratorTestRunner:
    ...
    async def _generate_coverage_report(self) -> None:
        ...
        try:
            ...
            for file_path, file_data in files_data.items():
                if any(
                    source_path in file_path
                    for source_path in self.coverage_config["source_paths"]
                ):
                    self.test_results["coverage_report"]["files"][file_path] = {
                        "percentage": file_data.get("summary", {}).get(
                            "percent_covered", 0
                        ),
            ...
            overall_coverage = self.test_results["coverage_report"][
                "overall_percentage"
            ]
            ...
            if overall_coverage >= min_coverage:
                logger.info(
                    f"  ✅ Coverage: {overall_coverage:.1f}% (minimum: {min_coverage}%)"
                )
            else:
                logger.warning(
                    f"  ⚠️  Coverage: {overall_coverage:.1f}% (below minimum: {min_coverage}%)"
                )
    ...
    async def _generate_overall_summary(self, total_execution_time: float) -> None:
        ...
        try:
            ...
            for test_type in ["unit", "integration", "e2e"]:
                ...
                if test_key in self.test_results:
                    ...
                    if "detailed_results" in test_data:
                        ...
                        failed_tests += details.get("failed", 0) + details.get(
                            "errors", 0
                        )
            ...
            if failed_tests > 0:
                ...
                result.get("status") not in ["passed", "timeout"]
                for result in [
                    self.test_results.get(f"{t}_tests", {})
            ...
            # Coverage status
            coverage_percentage = self.test_results.get("coverage_report", {}).get(
                "overall_percentage", 0
            )
            coverage_meets_minimum = (
                coverage_percentage >= self.coverage_config["min_overall_coverage"]
            )
            ...
            self.test_results["overall_summary"] = {"status": "error", "error": str(e)}
    ...
    def _print_test_results(self) -> None:
        ...
        if coverage:
            coverage_emoji = (
                "✅" if summary.get("coverage_meets_minimum", False) else "⚠️"
            )
        ...
        if performance:
            ...
            print(
                f"   Error Recovery Rate: {reliability.get('error_recovery_rate_percent', 0)}%"
            )
        ...
        for test_type in ["unit", "integration", "e2e"]:
            ...
            if test_key in self.test_results:
                ...
                print(
                    f"   {status_emoji} {test_type.title()} Tests: {result.get('status', 'unknown')} ({result.get('execution_time', 0):.2f}s)"
                )
        ...
        print(
            f"   {ready_emoji} Production Ready: {summary.get('production_ready', False)}"
        )
        ...
        if not summary.get("production_ready", False):
            ...
            if not summary.get("coverage_meets_minimum", True):
                print(
                    f"   - Coverage below minimum ({coverage.get('overall_percentage', 0):.1f}% < {self.coverage_config['min_overall_coverage']}%)"
                )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Structured Investigation Orchestrator Test Runner"
    )
    parser.add_argument(
        "--test-types",
        nargs="+",
    ...
    parser.add_argument(
        "--output-file", help="Output file for test results (JSON format)"
    )
