...
async def test_virustotal_client_session_management():
    ...
    try:
        ...
        assert not hasattr(
            client, "_session"
        ), "Client should not manage its own session"
        ...
        with patch(
            "app.service.agent.tools.threat_intelligence_tool.virustotal.virustotal_client.get_firebase_secret",
...
async def test_concurrent_session_usage():
    ...
    try:
        ...
        final_stats = manager.get_session_stats()
        print(
            f"   Active sessions after concurrent usage: {final_stats['active_sessions']}"
        )

        assert (
            final_stats["active_sessions"] == 0
        ), "All concurrent sessions should be cleaned up"
...
async def run_all_tests():
    ...
    tests = [
        (
            "VirusTotal Client Session Management",
            test_virustotal_client_session_management,
        ),
