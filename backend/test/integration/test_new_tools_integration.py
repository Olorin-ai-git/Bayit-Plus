...
class TestNewToolsIntegration:
        ...
        try:
            ...
            with patch.object(blockchain_mcp_client, "_run") as mock_run:
                mock_result = {
                    ...
                    "chain": "bitcoin",
                    "analysis": {
                        "chainalysis": {"risk_score": 0.1, "categories": ["genesis"]}
                    },
                }
    ...
    async def test_intelligence_mcp_client(self):
        ...
        try:
            ...
            from app.service.agent.mcp_client.intelligence_client import (
                IntelligenceType,
            )
        ...
        try:
            ...
            with patch.object(ml_ai_mcp_client, "_run") as mock_run:
                ...
                if "predictions" in result and result["predictions"]:
                    ...
                    logger.info(
                        f"  Is Fraudulent: {predictions.get('is_fraudulent', 'N/A')}"
                    )
    ...
    def test_threat_intelligence_tools(self):
        ...
        try:
            ...
            assert any(
                found_tools.values()
            ), "None of the expected threat intelligence tools were found"
        ...
        try:
            ...
            threat_count = len(
                [
                    ...
                    for t in tool_names
                    if any(
                        x in t.lower() for x in ["threat", "virus", "abuse", "shodan"]
                    )
                ]
            )
            traditional_count = len(
                [
                    t
                    for t in tool_names
                    if any(x in t.lower() for x in ["splunk", "sumologic", "snowflake"])
                ]
            )
            ...
            assert (
                mcp_count > 0 or threat_count > 0
            ), "No MCP or threat intelligence tools found in graph builder"
    ...
    def test_domain_objectives_updated(self):
        ...
        try:
            ...
            for domain in domains:
                ...
                if (
                    "intelligence_mcp_client" in objectives_text
                    or "osint" in objectives_text
                ):
                    tools_mentioned["intelligence_mcp_client"] = True
                if "ml_ai_mcp_client" in objectives_text:
                    tools_mentioned["ml_ai_mcp_client"] = True
                if (
                    "threat intelligence" in objectives_text
                    or "abuseipdb" in objectives_text
                    or "virustotal" in objectives_text
                ):
            ...
            for tool, mentioned in tools_mentioned.items():
                ...
                logger.info(
                    f"  {status} {tool}: {'Integrated' if mentioned else 'Not explicitly mentioned'}"
                )

            # Pass test if at least some tools are mentioned
            assert any(
                tools_mentioned.values()
            ), "No new tools mentioned in domain objectives"
