"""
Test multi-speaker translation with Hebrew podcast "השבוע - פודקאסט הארץ" (This Week - Haaretz).
Tests Hebrew → English translation with background preservation and speaker diarization.
"""
import asyncio
...
async def test_haaretz_multi_speaker():
    ...
    # Find "השבוע - פודקאסט הארץ" podcast and get latest episode
    print("=" * 80)
    print("FINDING LATEST HAARETZ 'THIS WEEK' EPISODE")
    print("=" * 80)

    # Query for episodes from this podcast (sorted by pub_date descending)
    episodes = await PodcastEpisode.find(
        {"podcast_title": {"$regex": "השבוע"}}
    ).sort("-pub_date").limit(1).to_list()

    if not episodes:
        print("❌ No episodes found for 'השבוע - פודקאסט הארץ' podcast")
        client.close()
        return

    episode = episodes[0]

    print(f"Found latest episode:")
...
if __name__ == "__main__":
    asyncio.run(test_haaretz_multi_speaker())
