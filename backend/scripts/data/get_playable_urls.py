...
for title, item_id in items.items():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Find video files
            video_files = [f for f in files if f.get("format") in ["H.264", "MPEG4", "h.264", "MP4"]]
            
            if video_files:
                # Use the first/largest video file
                video_file = max(video_files, key=lambda x: int(x.get("size", 0)) if isinstance(x.get("size"), str) else 0)
...
working_urls["The Great Train Robbery (1903)"] = "https://archive.org/download/FreeRamble3013/FreeRamble3013.mp4"
