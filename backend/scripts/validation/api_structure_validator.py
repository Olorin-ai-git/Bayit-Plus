...
class APIStructureValidator:
    ...
    async def _validate_openapi_spec(self) -> Dict[str, Any]:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    f"{self.base_url}/openapi.json", timeout=10
                ) as response:
    ...
    async def _validate_agent_endpoints(self) -> Dict[str, Any]:
        ...
        endpoints_to_test = [
            ...
            {
                "path": "/v1/agent/start/test_entity",
                "method": "POST",
                "description": "Agent start endpoint",
            },
        ]
        ...
        return {
            ...
            "summary": {
                "reachable": len(
                    [r for r in results if r.get("status") == "reachable"]
                ),
                "service_errors": len(
                    [r for r in results if r.get("status") == "service_error"]
                ),
                "network_errors": len(
    ...
    async def _validate_authentication_structure(self) -> Dict[str, Any]:
        ...
        return {"endpoints_tested": auth_endpoints, "results": results}
    ...
    def _generate_validation_summary(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # WebSocket
        ws_ok = results["websocket_endpoints"].get("status") in [
            "available",
            "endpoint_available",
        ]
        ...
        return {
            "overall_status": (
                "healthy"
                if (server_ok and openapi_ok and agent_reachable > 0)
            ...
            "total_endpoints_discovered": results["openapi_spec"].get(
                "total_endpoints", 0
            ),
            "agent_specific_endpoints": len(
                results["openapi_spec"].get("agent_endpoints", [])
            ),
            "api_framework": (
                "FastAPI"
                if "uvicorn"
                in results["server_availability"].get("server_type", "").lower()
                else "Unknown"
            ),
        }
        ...
        if results["agent_endpoints"].get("results"):
            ...
            for result in results["agent_endpoints"]["results"]:
                status_icon = (
                    ...
                    else "⚠️" if result["status"] == "service_error" else "❌"
                )
                logger.info(
                    f"  {status_icon} {result['endpoint']} [{result['status_code']}] - {result['description']}"
                )
