        ...
        for i, txn in enumerate(transactions):
            ...
            if card_id and merchant_id:
                session.run(
                    ...
                    merchant_id=merchant_id,
                    txn_id=txn_id,
                    txn_ts=str(txn_ts) if txn_ts else None,
                )
...
def load_graph_features_batch(db, features: List[Dict[str, Any]]) -> int:
    """
    ...
    for record in features:
        ...
        shortest_path = None
        if (
            record.get("fraud_merchant_count", 0) > 0
            or record.get("fraud_card_count", 0) > 0
        ):
            shortest_path = 1
...
def run_gds_algorithms(neo4j_client: Neo4jClient) -> Dict[str, Any]:
    ...
    if fraud_node_ids:
        shortest_paths_result = neo4j_client.compute_shortest_paths_to_fraud(
            fraud_node_ids=fraud_node_ids, node_label="Card"
        )
...
def query_snowflake_transactions(
    sf_service: SnowflakeQueryService, limit: Optional[int] = None
) -> List[Dict[str, Any]]:
