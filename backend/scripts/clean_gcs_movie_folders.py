...
def clean_folder_name(folder_name):
    ...
    name = folder_name.replace('gs://bayit-plus-media-new-new/movies/', '').rstrip('/')
    ...
    if not name or len(name) < 2:
        return folder_name.replace('gs://bayit-plus-media-new-new/movies/', '').rstrip('/')
...
def get_all_movie_folders():
    ...
    result = subprocess.run(
        ['gcloud', 'storage', 'ls', 'gs://bayit-plus-media-new-new/movies/'],
        capture_output=True,
        text=True
    )
...
def rename_folder(old_path, new_name):
    ...
    old_name = old_path.replace('gs://bayit-plus-media-new-new/movies/', '').rstrip('/')

    # Skip if names are the same
    if old_name == new_name:
        return True, "No change needed"

    new_path = f"gs://bayit-plus-media-new-new/movies/{new_name}/"
...
def main():
    ...
    for folder in folders:
        original = folder.replace('gs://bayit-plus-media-new-new/movies/', '').rstrip('/')
