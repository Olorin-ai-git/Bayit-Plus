...
def check_environment_variables() -> ValidationResult:
    ...
    return ValidationResult("Environment Variables", True, message)
...
def check_database_tables() -> ValidationResult:
    ...
    try:
        ...
        required_tables = {"detectors", "detection_runs", "anomaly_events"}
...
def check_api_endpoints() -> ValidationResult:
    ...
    try:
        ...
        found_routes = [
            r for r in required_routes if any(r in route for route in routes)
        ]
        ...
        return ValidationResult(
            "API Endpoints", False, f"Error checking routes: {str(e)}"
        )
...
def check_models() -> ValidationResult:
    ...
    try:
        ...
        if not (detector_ok and run_ok and event_ok):
            ...
            return ValidationResult(
                "Models", False, f"Models missing attributes: {', '.join(missing)}"
            )

        return ValidationResult(
            "Models", True, "All models properly defined with required attributes"
        )
    except Exception as e:
        return ValidationResult("Models", False, f"Error checking models: {str(e)}")
...
def run_validation() -> Tuple[List[ValidationResult], bool]:
    ...
    for check in checks:
        try:
            ...
            results.append(
                ValidationResult(check.__name__, False, f"Exception: {str(e)}")
            )
