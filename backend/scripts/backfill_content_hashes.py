...
async def backfill_hashes(limit: int = None, dry_run: bool = False):
    ...
    for idx, item in enumerate(content_items, 1):
        try:
            ...
            if item.stream_url and not item.stream_url.startswith(('http://', 'https://', 'gs://')):
            ...
            if not local_path:
                ...
                skipped_gcs_only += 1
                logger.debug(f"[{idx}/{total}] Skipping {item.title} - GCS-only, no local path")
                continue
            
            path = Path(local_path)
            if not path.exists():
                skipped_file_not_found += 1
                logger.warning(f"[{idx}/{total}] Skipping {item.title} - file not found: {local_path}")
                continue
            
            # Calculate hash
            logger.info(f"[{idx}/{total}] Calculating hash for: {item.title} ({path.name})")
            file_hash = await asyncio.to_thread(calculate_file_hash_sync, str(path.absolute()))
