...
async def extract_subtitles_for_content(content: Content, ffmpeg: FFmpegService) -> dict:
    """
    ...
    try:
        ...
        print(f"  Languages detected: {content.available_subtitle_languages or 'unknown'}")
...
async def main():
    ...
    content_items = await Content.find(
        Content.embedded_subtitle_count > 0
    ).to_list()
    ...
    for content in content_items:
        ...
        if existing_subs == 0:
            ...
            print(f"  ✓ {content.title}: already has {existing_subs} extracted subtitles")
    ...
    print(f"Estimated time: {len(needs_extraction) * 1.5:.0f}-{len(needs_extraction) * 3:.0f} minutes")
    print("\nStarting extraction...")
