...
class SystematicFraudTester:
    ...
    async def get_fraud_entities_in_window(
    ...
    async def get_entity_transactions(
        self, email: str, fraud_window_start: datetime, context_days: int = 60
    ):
    ...
    async def test_entity(
        ...
        email: str,
        fraud_count: int,
        fraud_window_start: datetime,
        context_days: int = 60,
    ):
        """Test enhanced scoring on a specific entity"""
        # Get transactions
        transactions = await self.get_entity_transactions(
            email, fraud_window_start, context_days
        )
        ...
        # Calculate risk
        risk_assessment = self.scorer.calculate_entity_risk(
    ...
    async def test_window(
        self, window_start: datetime, window_end: datetime, max_entities: int = 5
    ):
        """Test a 24-hour window"""
        print("=" * 80)
        print(
            f"📅 TESTING WINDOW: {window_start.strftime('%Y-%m-%d %H:%M')} to {window_end.strftime('%Y-%m-%d %H:%M')}"
        )
        print("=" * 80)
        print()

        # Find fraud entities
        fraud_entities = await self.get_fraud_entities_in_window(
        ...
        for i, entity_info in enumerate(fraud_entities[:test_count], 1):
            ...
            if result:
                ...
                status = (
                    ...
                    else "⚠️" if result["recall"] >= 0.5 else "❌"
                )
                print(
                    f"  {status} Recall: {result['recall']:.1%} ({result['tp']}/{result['fraud_transactions']})"
                )
                print(
                    f"     Risk Score: {result['risk_score']:.3f}, Detected: {result['is_fraud_detected']}"
                )
    ...
    async def run_systematic_test(
        self, num_windows: int = 30, start_offset_months: int = 6
    ):
        ...
        for window_num in range(num_windows):
            ...
            window_result = await self.test_window(
                window_start, current_end, max_entities=3
            )
    ...
    def analyze_and_suggest_adjustments(self, windows_tested: int):
        ...
        if total_fraud > 0:
            ...
            overall_precision = (
                total_caught / (total_caught + total_fp)
                if (total_caught + total_fp) > 0
                else 0
            )

            print(
                f"Overall Recall: {overall_recall:.1%} ({total_caught}/{total_fraud})"
            )
            ...
            if self.missed_patterns:
                ...
                # Aggregate patterns
                avg_missed_count = sum(
                    p["missed_count"] for p in self.missed_patterns
                ) / len(self.missed_patterns)
                avg_amount = sum(p["avg_amount"] for p in self.missed_patterns) / len(
                    self.missed_patterns
                )
    ...
    def generate_final_report(self):
        ...
        windows_with_fraud = sum(
            1 for w in self.results if w["fraud_entities_found"] > 0
        )
        ...
        if recall_scores:
            ...
            print(
                f"  Avg Precision: {sum(precision_scores)/len(precision_scores):.1%} (if precision > 0)"
            )
        ...
        if recall_scores:
            ...
            print(
                f"  High (≥80%):     {high_recall:3d} entities ({high_recall/len(recall_scores)*100:.1f}%)"
            )
            print(
                f"  Medium (50-80%): {medium_recall:3d} entities ({medium_recall/len(recall_scores)*100:.1f}%)"
            )
            print(
                f"  Low (<50%):      {low_recall:3d} entities ({low_recall/len(recall_scores)*100:.1f}%)"
            )
            print()
        else:
        ...
        report_file = (
            f"systematic_test_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
            ...
            json.dump(
                {
                    "summary": {
                        ...
                        "overall_precision": (
                            overall_precision if total_fraud > 0 else 0
                        ),
...
async def main():
    ...
    parser.add_argument(
        "--windows", type=int, default=30, help="Number of 24-hour windows to test"
    )
    parser.add_argument(
        "--start-offset", type=int, default=6, help="Months back to start from"
    )
