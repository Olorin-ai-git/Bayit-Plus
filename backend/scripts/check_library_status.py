...
async def check_library_status():
    ...
    await init_beanie(database=db, document_models=[Content, SubtitleTrackDoc])
    ...
    print(
        f"   ✅ Complete metadata: {total - missing_metadata} ({(total - missing_metadata) / total * 100:.1f}%)"
    )
    ...
    print(
        f"   ✅ Complete (EN/HE/ES): {items_with_all_subs} ({items_with_all_subs / total * 100:.1f}%)"
    )
    print(
        f"   ❌ Missing subtitles: {items_missing_subs} ({items_missing_subs / total * 100:.1f}%)"
    )
    ...
    if missing_metadata > 0:
        print(
            f"   1. Run comprehensive audit to fix {missing_metadata} metadata issues:"
        )
