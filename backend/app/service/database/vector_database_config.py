...
class VectorDatabaseConfig:
    ...
    def __init__(self):
        ...
        if not self.is_postgresql:
            logger.warning(
                f"Non-PostgreSQL database URL detected: {self.database_url[:50]}... PostgreSQL is required for vector database."
            )
    ...
    async def initialize_engine(self) -> None:
        ...
        if not self.is_postgresql:
            raise ValueError(
                f"PostgreSQL is required for vector database. Got: {self.database_url[:50]}..."
            )
    ...
    async def _initialize_postgresql(self) -> None:
        ...
        try:
            ...
            self._async_engine = create_async_engine(
                async_url, pool_pre_ping=True, pool_size=5, max_overflow=10, echo=False
            )
