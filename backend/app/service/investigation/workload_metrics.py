...
def calculate_workload_metrics(
    ...
    pct_traffic_alerted = (
        (over_threshold / total_transactions * 100) if total_transactions > 0 else 0.0
    )
...
def calculate_precision_at_k(
    transactions: List[Dict[str, Any]], k: int, risk_threshold: Optional[float] = None
) -> Dict[str, Any]:
    """
    ...
    transactions_with_risk = [
        tx for tx in transactions if tx.get("predicted_risk") is not None
    ]
    ...
    fraud_in_top_k = sum(
        1 for tx in top_k if tx.get("actual_outcome") in ("FRAUD", 1, True)
    )

    # Count total fraud
    total_fraud = sum(
        1
        for tx in transactions_with_risk
...
def calculate_recall_at_budget(
    ...
    # Count fraud caught
    fraud_caught = sum(
        1 for tx in top_transactions if tx.get("actual_outcome") in ("FRAUD", 1, True)
    )
    ...
    precision_at_budget = (
        fraud_caught / len(top_transactions) if len(top_transactions) > 0 else 0.0
    )
