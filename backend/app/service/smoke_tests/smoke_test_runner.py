...
class SmokeTestRunner:
    ...
    def _load_service_configurations(self) -> dict:
        ...
        return {
            ...
            "abuseipdb": {
                "enabled": (
                    os.getenv("USE_ABUSEIPDB_IP_REPUTATION", "false").lower() == "true"
                    or os.getenv("USE_ABUSEIPDB_BULK_ANALYSIS", "false").lower()
                    == "true"
                ),
            ...
            "virustotal": {
                "enabled": (
                    os.getenv("USE_VIRUSTOTAL_IP_ANALYSIS", "false").lower() == "true"
                    or os.getenv("USE_VIRUSTOTAL_DOMAIN_ANALYSIS", "false").lower()
                    == "true"
                    or os.getenv("USE_VIRUSTOTAL_FILE_ANALYSIS", "false").lower()
                    == "true"
                ),
                "severity": SmokeTestSeverity.HIGH,
            ...
            "shodan": {
                "enabled": (
                    os.getenv("USE_SHODAN_INFRASTRUCTURE", "false").lower() == "true"
                    or os.getenv("USE_SHODAN_SEARCH", "false").lower() == "true"
                ),
                "severity": SmokeTestSeverity.HIGH,
    ...
    async def run_service_smoke_tests(
        self, service_name: str, enabled: bool
    ) -> ServiceSmokeTestSuite:
        ...
        suite = ServiceSmokeTestSuite(
            service_name=service_name.title(), enabled=enabled
        )
    ...
    async def run_all_smoke_tests(
        self, services: Optional[List[str]] = None, parallel: bool = True
    ) -> SmokeTestReport:
        """
        ...
        if parallel:
            ...
            logger.info(
                f"Running smoke tests in parallel for {len(valid_services)} services"
            )
            ...
            for i, result in enumerate(service_suites):
                if isinstance(result, Exception):
                    logger.error(
                        f"Service {valid_services[i]} smoke test failed: {result}"
                    )
                    # Create error suite
                    error_suite = ServiceSmokeTestSuite(
                        service_name=valid_services[i].title(), enabled=True
                    )
                    from .models import SmokeTestResult
            ...
            logger.info(
                f"Running smoke tests sequentially for {len(valid_services)} services"
            )

            for service_name in valid_services:
                config = self.service_configs[service_name]
                suite = await self.run_service_smoke_tests(
                    service_name, config["enabled"]
                )
        ...
        total_response_time = sum(
            suite.total_response_time_ms for suite in service_suites
        )
        ...
        if self.report.has_critical_failures:
            ...
            if self.report.should_block_investigations:
                logger.error(
                    "❌ INVESTIGATIONS SHOULD BE BLOCKED due to critical service failures"
                )
        else:
    ...
    async def run_quick_health_check(self) -> dict:
        ...
        report = await self.run_all_smoke_tests(
            services=critical_services, parallel=True
        )
