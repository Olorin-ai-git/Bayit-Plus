...
class UnifiedRAGService:
    ...
    async def _query_all_sources(
        ...
        for source in sources:
            if source.source_type == "investigation_results":
                tasks.append(
                    self._query_investigation_source(
                        query_text,
                        source,
                        limit,
                        threshold,
                ...
                tasks.append(
                    self._query_database_source(query_text, source, limit, threshold)
                )
            else:
                tasks.append(
                    self._query_vector_source(query_text, source, limit, threshold)
                )
    ...
    async def _query_investigation_source(
        ...
        try:
            collection_id = (
                await self.investigation_indexer.ensure_investigation_collection()
            )
            ...
            for r in search_results:
                ...
                if (
                    investigation_id
                    and metadata.get("investigation_id") != investigation_id
                ):
                    continue

                if entity_id:
                    entities = metadata.get("entities", [])
                    if isinstance(entities, list) and entity_id not in [
    ...
    async def _query_database_source(
        self, query_text: str, source, limit: int, threshold: float
    ) -> List[UnifiedSearchResult]:
    ...
    def _rank_and_deduplicate(
        self, results: List[UnifiedSearchResult], limit: int
    ) -> List[UnifiedSearchResult]:
    ...
    async def _generate_answer(
        self, query_text: str, results: List[UnifiedSearchResult]
    ) -> str:
        ...
        for i, result in enumerate(results[:3], 1):
            logger.debug(
                f"Result {i} (score: {result.similarity_score:.3f}): {result.content[:200]}..."
            )
        ...
        try:
            ...
            from app.service.agent.rag.rag_orchestrator import (
                RAGConfig,
                RAGOrchestrator,
                RAGRequest,
            )
            ...
            if hasattr(response, "generated_text") and response.generated_text:
                logger.info(
                    f"Generated answer length: {len(response.generated_text)} chars"
                )
                return response.generated_text
            elif hasattr(response, "success") and not response.success:
                # If generation failed, return a summary based on results
                logger.warning(
                    f"RAG generation failed: {response.error_message if hasattr(response, 'error_message') else 'Unknown error'}"
                )
            ...
            logger.warning(
                f"Answer generation failed, using simple summary: {e}", exc_info=True
            )
