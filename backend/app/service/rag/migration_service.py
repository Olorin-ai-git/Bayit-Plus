...
class RAGMigrationService:
    ...
    async def check_migration_status(self) -> Dict[str, Any]:
        ...
        if (
            ...
            and status["postgres_records"]["total_documents"] == 0
        ):
            status["migration_needed"] = True
            status["recommendations"].append(
                "Migration from SQLite to PostgreSQL recommended"
            )
        elif status["postgres_records"]["total_documents"] > 0:
            status["recommendations"].append(
                "PostgreSQL already has data - migration may not be needed"
            )
    ...
    async def _analyze_sqlite_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            document_tables = [
                t for t in tables if "document" in t.lower() or "chunk" in t.lower()
            ]
    ...
    async def _analyze_postgres_data(self) -> Dict[str, Any]:
        ...
        try:
            async with self.vector_db_config.session() as session:
                ...
                # Count chunks
                result = await session.execute(
                    text("SELECT COUNT(*) FROM document_chunks WHERE is_active = true")
                )
                ...
                return {
                    ...
                    "sentence_embeddings": (
                        embedding_counts[1] if embedding_counts else 0
                    ),
                    "openai_large_embeddings": (
                        embedding_counts[2] if embedding_counts else 0
                    ),
                }
    ...
    async def migrate_from_sqlite(
        ...
        try:
            ...
            if regenerate_embeddings and migration_stats["chunks"] > 0:
                ...
                embedding_stats = await self._regenerate_all_embeddings(
                    embedding_provider
                )
            ...
            logger.info(
                f"✅ Migration completed successfully in {result['processing_time_ms']}ms"
            )
        ...
        for col_info in collections:
            try:
                collection_id = await self.rag_service.create_collection(
                    name=col_info["name"], description=col_info["description"]
                )
    ...
    async def _migrate_documents_and_chunks(
        self, collection_mapping: Dict[str, uuid.UUID], batch_size: int
    ) -> Dict[str, int]:
            ...
            for i in range(0, len(documents), batch_size):
                ...
                for doc_row in batch:
                    try:
                        ...
                        title = doc_data.get(
                            "title",
                            doc_data.get("name", f'Document {stats["documents"] + 1}'),
                        )
                        content = doc_data.get("content", doc_data.get("text", ""))

                        if not content:
                            logger.warning(
                                f"Skipping document with no content: {title}"
                            )
                            continue

                        # Determine collection
                        collection_id = default_collection_id
                        doc_type = doc_data.get(
                            "type", doc_data.get("category", "knowledge_base")
                        )
                        ...
                        if ingest_result.success:
                            ...
                            logger.warning(
                                f"Failed to migrate document {title}: {ingest_result.error_message}"
                            )
                ...
                # Log progress
                logger.info(
                    f"Migrated {min(i + batch_size, len(documents))}/{len(documents)} documents"
                )
    ...
    async def validate_migration(self) -> Dict[str, Any]:
        ...
        validation = {"success": True, "issues": [], "stats": {}, "recommendations": []}
        ...
        try:
            ...
            if total_chunks > 0:
                openai_coverage = (
                    postgres_stats.get("openai_embeddings", 0) / total_chunks
                )
                sentence_coverage = (
                    postgres_stats.get("sentence_embeddings", 0) / total_chunks
                )

                if openai_coverage < 0.8 and sentence_coverage < 0.8:
                    validation["issues"].append(
                        "Low embedding coverage - consider regenerating embeddings"
                    )
            ...
            if total_chunks > 1000:
                validation["recommendations"].append(
                    "Consider creating vector indexes for better search performance"
                )

            if postgres_stats.get("total_collections", 0) == 1:
                validation["recommendations"].append(
                    "Consider organizing documents into multiple collections"
                )

            logger.info(
                f"✅ Migration validation completed: {'PASSED' if validation['success'] else 'FAILED'}"
            )
