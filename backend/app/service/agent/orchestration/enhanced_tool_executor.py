...
class EnhancedToolNode(ToolNode):
    ...
    def __init__(
        self, tools: Sequence[BaseTool], investigation_id: str = None, **kwargs
    ):
        ...
        for tool in self.tools:
            if isinstance(tool, BaseTool):
                ...
                self.health_manager.health_checks[tool.name] = ToolHealthMetrics(
                    tool_name=tool.name
                )
                # Ensure circuit breaker starts in CLOSED state (working)
                self.health_manager.health_checks[tool.name].circuit_state = (
                    CircuitState.CLOSED
                )
    ...
    async def ainvoke(
        self,
        input: Union[Dict[str, Any], MessagesState],
        config: Optional[RunnableConfig] = None,
    ) -> Union[Dict[str, Any], MessagesState]:
        """
        ...
        """
        logger.info(
            f"🔧 EnhancedToolNode.ainvoke called with input type: {type(input)}"
        )
        ...
        if isinstance(input, dict):
            ...
            if "investigation_id" in input:
                logger.info(
                    f"🔧 investigation_id found in input: {input['investigation_id']}"
                )
        ...
        if not self.investigation_id:
            if isinstance(input, dict) and "investigation_id" in input:
                ...
                logger.info(
                    f"🔧 CRITICAL: Extracted investigation_id from state: {self.investigation_id}"
                )
            elif config:
                ...
                if (
                    ...
                    and "investigation_id" in configurable
                ):
                    self.investigation_id = configurable["investigation_id"]
                    logger.info(
                        f"🔧 CRITICAL: Extracted investigation_id from config: {self.investigation_id}"
                    )
                    ...
                    logger.info(
                        f"🔧 CRITICAL: Extracted investigation_id from thread_id: {self.investigation_id}"
                    )
        ...
        logger.info(
            f"🔧 Composio forcing check: entity_id={entity_id}, entity_type={entity_type}, tools_used={tools_used}, orchestrator_loops={orchestrator_loops}, composio_search_in_messages={composio_search_in_messages}"
        )
        logger.info(
            f"🔧 Available composio tools: {[t.name for t in self.tools if 'composio' in t.name.lower()]}"
        )
            ...
            if isinstance(snowflake_data, dict) and "results" in snowflake_data:
                ...
                for r in results:
                    ...
                    if ip and isinstance(ip, str) and ip not in extracted_ips:
                        ...
                        if (
                            re.match(ipv4_pattern, ip)
                            or re.match(ipv6_pattern, ip)
                            or ":" in ip
                        ):
                    ...
                    if (
                        ...
                        and phone not in extracted_phones
                    ):
                        ...
                        if (
                            len(cleaned_phone) >= 10
                        ):  # Minimum 10 digits for a valid phone
                    ...
                    email = (
                        r.get("EMAIL")
                        or r.get("email")
                        or r.get("email_address")
                        or r.get("EMAIL_ADDRESS")
                        or r.get("user_email")
                        or r.get("USER_EMAIL")
                        or r.get("paypal_email")
                        or r.get("PAYPAL_EMAIL")
                    )
                    if (
                        email
                        and isinstance(email, str)
                        and email not in extracted_emails
                    ):
        ...
        logger.info(
            f"🔧 Extracted {len(extracted_ips)} IP address(es) from Snowflake data: {extracted_ips[:3]}..."
            if len(extracted_ips) > 3
            else f"🔧 Extracted {len(extracted_ips)} IP address(es): {extracted_ips}"
        )
        logger.info(
            f"🔧 Extracted {len(extracted_phones)} phone number(s) from Snowflake data: {extracted_phones[:3]}..."
            if len(extracted_phones) > 3
            else f"🔧 Extracted {len(extracted_phones)} phone number(s): {extracted_phones}"
        )
        logger.info(
            f"🔧 Extracted {len(extracted_emails)} email address(es) from Snowflake data: {extracted_emails[:3]}..."
            if len(extracted_emails) > 3
            else f"🔧 Extracted {len(extracted_emails)} email address(es): {extracted_emails}"
        )
        ...
        force_composio_search = (
            "composio_search" not in tools_used
            and not composio_search_in_messages  # Also check messages
            and entity_id
            and has_composio_search
            and (
                len(tools_used) > 0 or orchestrator_loops >= 0
            )  # Force after any tool OR always if we have entity
        )
        ...
        force_composio_webcrawl = (
            "composio_webcrawl" not in tools_used
            and (
                "composio_search" in tools_used or composio_search_in_messages
            )  # Check both sources
            and entity_id
            and has_composio_webcrawl
        )

        logger.info(
            f"🔧 Composio forcing decision: search={force_composio_search}, webcrawl={force_composio_webcrawl}, has_search={has_composio_search}, has_webcrawl={has_composio_webcrawl}"
        )
        ...
        if force_composio_search:
            logger.warning(
                f"⚠️ FORCING composio_search for {entity_type}={entity_id} (loop {orchestrator_loops}, tools_used: {tools_used})"
            )
        ...
        if (
            extracted_ips and len(tools_used) > 0
        ):  # Only force after some tools have been used (e.g., after Snowflake query)
        ...
        if (
            extracted_phones and len(tools_used) > 0
        ):  # Only force after some tools have been used (e.g., after Snowflake query)
            ...
            if has_veriphone and "veriphone_verify_phone" not in tools_used:
                logger.warning(
                    f"⚠️ FORCING veriphone_verify_phone for phone {first_phone}"
                )
                from uuid import uuid4
        ...
        if (
            extracted_emails and len(tools_used) > 0
        ):  # Only force after some tools have been used (e.g., after Snowflake query)
            ...
            if has_ipqs_email and "ipqs_email_verification" not in tools_used:
                logger.warning(
                    f"⚠️ FORCING ipqs_email_verification for email {first_email}"
                )
                from uuid import uuid4
        ...
        for message_idx, message in enumerate(messages):
            logger.debug(
                f"🔧 Processing message {message_idx + 1}/{len(messages)}: {type(message)}"
            )

            if isinstance(message, AIMessage) and message.tool_calls:
                tool_calls_found += len(message.tool_calls)
                logger.info(
                    f"🔧 Found {len(message.tool_calls)} tool calls in AIMessage"
                )
                ...
                for tool_idx, tool_call in enumerate(message.tool_calls):
                    ...
                    logger.info(
                        f"🔧 Executing tool {tool_idx + 1}/{len(message.tool_calls)}: {tool_name} (id: {tool_id})"
                    )

                    try:
                        # Execute with resilience - pass input state to infer agent name
                        result = await self._execute_tool_with_resilience(
                            tool_call, config, input_state=input
                        )
                        logger.info(
                            f"🔧 ✅ Tool {tool_name} executed successfully, result type: {type(result)}"
                        )
                        ...
                        if isinstance(result, (dict, list)):
                            ...
                            logger.debug(
                                f"🔧 Serialized {type(result).__name__} to JSON string (length: {len(content)})"
                            )
                        elif isinstance(result, str):
                            content = result
                            logger.debug(
                                f"🔧 Result already string (length: {len(content)})"
                            )
                        else:
                            content = str(result)
                            logger.debug(
                        ...
                        if tool_name == "snowflake_query_tool" and isinstance(
                            result, dict
                        ):
                            logger.info(
                                f"🔧 snowflake_query_tool completed, extracting IPs, phones, emails and checking threat intelligence tools..."
                            )
                            ...
                            for r in snowflake_results:
                                ...
                                if (
                                    ...
                                    and ip not in new_extracted_ips
                                ):
                                    import re
                                ...
                                if (
                                    phone
                                    and isinstance(phone, str)
                                    and phone not in new_extracted_phones
                                ):
                                    import re
                                ...
                                if (
                                    email
                                    and isinstance(email, str)
                                    and email not in new_extracted_emails
                                ):
                                    import re
                            ...
                            if new_extracted_ips:
                                logger.info(
                                    f"🔧 Extracted {len(new_extracted_ips)} IP(s) from Snowflake results: {new_extracted_ips[:3]}"
                                )
                                first_ip = new_extracted_ips[0]

                            if new_extracted_phones:
                                logger.info(
                                    f"🔧 Extracted {len(new_extracted_phones)} phone(s) from Snowflake results: {new_extracted_phones[:3]}"
                                )
                                first_phone = new_extracted_phones[0]

                            if new_extracted_emails:
                                logger.info(
                                    f"🔧 Extracted {len(new_extracted_emails)} email(s) from Snowflake results: {new_extracted_emails[:3]}"
                                )
                                ...
                                if (
                                    has_abuseipdb
                                    and "abuseipdb_ip_reputation" not in tools_used
                                ):
                                    logger.warning(
                                        f"⚠️ FORCING abuseipdb_ip_reputation after Snowflake query for IP {first_ip}"
                                    )
                                    from uuid import uuid4
                                    ...
                                    try:
                                        ...
                                        abuseipdb_result = (
                                            await self._execute_tool_with_resilience(
                                                abuseipdb_tool_call,
                                                config,
                                                input_state=input,
                                            )
                                        ...
                                        logger.info(
                                            f"🔧 ✅ Tool abuseipdb_ip_reputation executed successfully, result type: {type(abuseipdb_result)}"
                                        )

                                        # Serialize result
                                        if isinstance(abuseipdb_result, (dict, list)):
                                            abuseipdb_content = json.dumps(
                                                abuseipdb_result,
                                                default=str,
                                                ensure_ascii=False,
                                            )
                                        ...
                                        abuseipdb_tool_message = ToolMessage(
                                            ...
                                            tool_call_id=abuseipdb_tool_call.get(
                                                "id", ""
                                            ),
                                            name="abuseipdb_ip_reputation",
                                        )
                                        ...
                                        logger.debug(
                                            f"🔧 Created ToolMessage for abuseipdb_ip_reputation"
                                        )
                                    except Exception as e:
                                        safe_error_msg = sanitize_exception_message(e)
                                        logger.error(
                                            f"🔧 ❌ Tool abuseipdb_ip_reputation execution failed: {safe_error_msg}"
                                        )

                                # Force VirusTotal if available and not used
                                if (
                                    has_virustotal
                                    and "virustotal_ip_analysis" not in tools_used
                                ):
                                    logger.warning(
                                        f"⚠️ FORCING virustotal_ip_analysis after Snowflake query for IP {first_ip}"
                                    )
                                    from uuid import uuid4
                                    ...
                                    try:
                                        ...
                                        logger.info(
                                            f"🔧 ✅ Tool virustotal_ip_analysis executed successfully, result type: {type(virustotal_result)}"
                                        )

                                        # Serialize result
                                        if isinstance(virustotal_result, (dict, list)):
                                            virustotal_content = json.dumps(
                                                virustotal_result,
                                                default=str,
                                                ensure_ascii=False,
                                            )
                                        ...
                                        virustotal_tool_message = ToolMessage(
                                            ...
                                            tool_call_id=virustotal_tool_call.get(
                                                "id", ""
                                            ),
                                            name="virustotal_ip_analysis",
                                        )
                                        ...
                                        logger.debug(
                                            f"🔧 Created ToolMessage for virustotal_ip_analysis"
                                        )
                                    except Exception as e:
                                        safe_error_msg = sanitize_exception_message(e)
                                        logger.error(
                                            f"🔧 ❌ Tool virustotal_ip_analysis execution failed: {safe_error_msg}"
                                        )
                                ...
                                if (
                                    new_extracted_phones
                                    and has_veriphone
                                    and "veriphone_verify_phone" not in tools_used
                                ):
                                    logger.warning(
                                        f"⚠️ FORCING veriphone_verify_phone after Snowflake query for phone {first_phone}"
                                    )
                                    from uuid import uuid4
                                    ...
                                    try:
                                        ...
                                        logger.info(
                                            f"🔧 ✅ Tool veriphone_verify_phone executed successfully, result type: {type(veriphone_result)}"
                                        )

                                        # Serialize result
                                        if isinstance(veriphone_result, (dict, list)):
                                            veriphone_content = json.dumps(
                                                veriphone_result,
                                                default=str,
                                                ensure_ascii=False,
                                            )
                                        ...
                                        veriphone_tool_message = ToolMessage(
                                            ...
                                            tool_call_id=veriphone_tool_call.get(
                                                "id", ""
                                            ),
                                            name="veriphone_verify_phone",
                                        )
                                        ...
                                        logger.debug(
                                            f"🔧 Created ToolMessage for veriphone_verify_phone"
                                        )
                                    except Exception as e:
                                        safe_error_msg = sanitize_exception_message(e)
                                        logger.error(
                                            f"🔧 ❌ Tool veriphone_verify_phone execution failed: {safe_error_msg}"
                                        )
                                ...
                                if (
                                    new_extracted_emails
                                    and has_ipqs_email
                                    and "ipqs_email_verification" not in tools_used
                                ):
                                    logger.warning(
                                        f"⚠️ FORCING ipqs_email_verification after Snowflake query for email {first_email}"
                                    )
                                    from uuid import uuid4
                                    ...
                                    try:
                                        ...
                                        logger.info(
                                            f"🔧 ✅ Tool ipqs_email_verification executed successfully, result type: {type(ipqs_result)}"
                                        )

                                        # Serialize result
                                        if isinstance(ipqs_result, (dict, list)):
                                            ipqs_content = json.dumps(
                                                ipqs_result,
                                                default=str,
                                                ensure_ascii=False,
                                            )
                                        ...
                                        logger.debug(
                                            f"🔧 Created ToolMessage for ipqs_email_verification"
                                        )
                                    except Exception as e:
                                        safe_error_msg = sanitize_exception_message(e)
                                        logger.error(
                                            f"🔧 ❌ Tool ipqs_email_verification execution failed: {safe_error_msg}"
                                        )

                        # After composio_search completes, check if we should force webcrawl
                        if (
                            tool_name == "composio_search"
                            and not force_composio_webcrawl
                        ):
                            logger.info(
                                f"🔧 composio_search completed, checking if webcrawl should be forced..."
                            )
                            ...
                            if (
                                ...
                                logger.warning(
                                    f"⚠️ FORCING composio_webcrawl after composio_search completion for {entity_type}={entity_id}"
                                )
                                from uuid import uuid4
                                ...
                                try:
                                    ...
                                    logger.info(
                                        f"🔧 ✅ Tool composio_webcrawl executed successfully, result type: {type(webcrawl_result)}"
                                    )

                                    # Serialize webcrawl result
                                    if isinstance(webcrawl_result, (dict, list)):
                                        webcrawl_content = json.dumps(
                                            webcrawl_result,
                                            default=str,
                                            ensure_ascii=False,
                                        )
                                    ...
                                    logger.debug(
                                        f"🔧 Created ToolMessage for composio_webcrawl"
                                    )
                                except Exception as e:
                                    safe_error_msg = sanitize_exception_message(e)
                                    error_category = get_error_category(e)
                                    logger.error(
                                        f"🔧 ❌ Tool composio_webcrawl execution failed - {error_category}: {safe_error_msg}"
                                    )
                        ...
                        error_category = get_error_category(e)
                        logger.error(
                            f"🔧 ❌ Tool {tool_name} execution failed - {error_category}: {safe_error_msg}"
                        )
                ...
                logger.debug(
                    f"🔧 Message {message_idx + 1} is not an AIMessage with tool_calls"
                )

        logger.info(
            f"🔧 Tool execution summary: {tool_calls_found} tool calls found, {len(result_messages)} results generated"
        )
        ...
        if isinstance(input, dict):
            ...
            logger.info(
                f"🔧 Returning dict result with {len(result_messages)} messages"
            )
            return result
        else:
            logger.info(
                f"🔧 Returning list result with {len(result_messages)} messages"
            )
            return result_messages

    def _infer_agent_name_from_state(
        self, input: Union[Dict[str, Any], MessagesState]
    ) -> Optional[str]:
        """
    ...
    async def _execute_tool_with_resilience(
        self,
        tool_call: Dict[str, Any],
        config: Optional[RunnableConfig],
        input_state: Optional[Union[Dict[str, Any], MessagesState]] = None,
    ) -> Any:
        """
        ...
        if not metrics:
            logger.warning(
                f"No metrics found for tool {tool_name}, executing without resilience"
            )
            ...
            if tool:
                ...
                if self.investigation_id:
                    try:
                        ...
                        from app.service.tool_execution_service import (
                        ...
                        with get_db_session() as db:
                            ...
                            logger.info(
                                f"🔧 ✅ Tool execution persisted (no metrics): {tool_exec_id}"
                            )
                    except Exception as e:
                        logger.warning(
                            f"🔧 Failed to persist tool execution (no metrics): {e}"
                        )
                ...
                if self.investigation_id and hasattr(self, "_current_tool_exec_id"):
                    try:
                        ...
                        with get_db_session() as db:
                            ...
                            logger.info(
                                f"🔧 ✅ Tool execution completion persisted (no metrics)"
                            )
                    except Exception as e:
                        logger.warning(
                            f"🔧 Failed to persist tool completion (no metrics): {e}"
                        )
        ...
        with self._state_lock:
            if metrics.circuit_state == CircuitState.OPEN:
                if self._should_attempt_recovery(metrics):
                    ...
                    logger.info(
                        f"Circuit breaker for {tool_name} entering HALF_OPEN state"
                    )
                else:
        ...
        logger.info(
            f"🔧 Attempting to persist tool execution: investigation_id={self.investigation_id}, tool={tool_name}"
        )
        if self.investigation_id:
            try:
                from app.persistence.database import get_db_session
                ...
                with get_db_session() as db:
                    logger.info(
                        f"🔧 Database session obtained, creating ToolExecutionService"
                    )
                    service = ToolExecutionService(db)
                    logger.info(
                        f"🔧 Persisting tool execution: investigation_id={self.investigation_id}, agent_name={agent_name}, tool_name={tool_name}, status=running"
                    )
                    tool_exec_id = service.persist_tool_execution(
                        investigation_id=self.investigation_id,
                    ...
                    self._current_tool_exec_id = tool_exec_id
                    logger.info(
                        f"🔧 ✅ Tool execution persisted successfully: {tool_exec_id}"
                    )
            except Exception as e:
                logger.error(
                    f"🔧 ❌ Failed to persist tool execution start to database: {e}",
                    exc_info=True,
                )
        else:
            logger.warning(
                f"🔧 No investigation_id provided, tool execution persistence skipped"
            )
        ...
        for attempt in range(self.retry_config["max_retries"]):
            try:
                ...
                if (
                    ...
                    logger.info(
                        f"🔧 Mapped snowflake_query_tool args to database_query: query length={len(mapped_args['query'])}"
                    )
                    tool_args = mapped_args

                # Execute tool with timeout
                result = await self._execute_with_timeout(tool, tool_args, config)
                ...
                with self._state_lock:
                    if metrics.circuit_state == CircuitState.HALF_OPEN:
                        ...
                        logger.info(
                            f"Circuit breaker for {tool_name} CLOSED after successful recovery"
                        )
                ...
                if elapsed > self.performance_threshold:
                    ...
                    logger.warning(
                        f"Tool {tool_name} took {elapsed:.2f}s (threshold: {self.performance_threshold}s)"
                    )
                ...
                if self.investigation_id and hasattr(self, "_current_tool_exec_id"):
                    try:
                            ...
                            if result:
                                if isinstance(result, dict):
                                    ...
                                    output_result = {
                                        ...
                                        "summary": sanitize_tool_result(
                                            result, max_length=200
                                        ),
                                    }
                        ...
                        logger.warning(
                            f"Failed to persist tool execution completion to database: {e}"
                        )
                ...
                if not is_retryable or attempt == self.retry_config["max_retries"] - 1:
                    ...
                    with self._state_lock:
                        if (
                            ...
                            circuit_opened = True
                            logger.error(
                                f"Circuit breaker OPEN for {tool_name} after {metrics.consecutive_failures} failures"
                            )
                    ...
                    if self.investigation_id and hasattr(self, "_current_tool_exec_id"):
                        try:
                            ...
                            logger.warning(
                                f"Failed to persist tool execution failure to database: {db_e}"
                            )
                ...
                logger.warning(
                    f"Tool {tool_name} failed (attempt {attempt + 1}/{self.retry_config['max_retries']}), "
        ...
        if last_exception:
            ...
            logger.error(
                f"Tool {tool_name} failed after all retries - {error_category}: {safe_error_msg}"
            )
    ...
    def _get_tool_by_name(self, name: str) -> Optional[BaseTool]:
        ...
        logger.debug(
            f"🔧 Looking for tool: '{name}' among {len(self.tools)} available tools"
        )
        ...
        if mapped_name != name:
            logger.info(
                f"🔧 Mapping tool name '{name}' -> '{mapped_name}' (PostgreSQL mode)"
            )
        ...
        for tool in self.tools:
            if isinstance(tool, BaseTool) and hasattr(tool, "name"):
                ...
                if tool.name == name or tool.name == mapped_name:
                    logger.debug(
                        f"🔧 ✅ Found matching tool: {tool.name} (requested: {name})"
                    )
                    return tool

        logger.error(
            f"🔧 ❌ Tool '{name}' (mapped: '{mapped_name}') not found in available tools: {available_tools}"
        )
        return None
    ...
    def _should_attempt_recovery(self, metrics: ToolHealthMetrics) -> bool:
        ...
        recovery_time = metrics.last_failure_time + timedelta(
            seconds=self.circuit_config["recovery_timeout"]
        )
    ...
    async def _emit_tool_event(
        self, event_type: str, tool_name: str, event_data: Dict[str, Any]
    ):
        """
...
class ToolHealthManager:
    ...
    async def _check_tool_health(self, tool: BaseTool) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Health check failed for tool {getattr(tool, 'name', 'unknown')}: {e}"
            )
            return False
...
def clear_tool_event_handlers():
    ...
    def record_performance(self, tool_name: str, latency: float):
        ...
        if (
            len(self.performance_metrics) >= MAX_TOOL_METRICS
            and tool_name not in self.performance_metrics
        ):
            ...
            logger.warning(
                f"Removed metrics for {oldest_tool} to stay within resource limits"
            )
