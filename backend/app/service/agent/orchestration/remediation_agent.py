...
async def remediation_agent_node(
    ...
    try:
        ...
        logger.info(
            f"[Remediation] 🛡️ Starting remediation for investigation {investigation_id}"
        )
        ...
        if not entity_id or not entity_type:
            logger.warning(
                "[Remediation] ⚠️ Missing entity_id or entity_type, skipping remediation"
            )
            return updates

        if overall_risk_score is None:
            logger.warning(
                "[Remediation] ⚠️ No risk score available, skipping remediation"
            )
            return updates
        ...
        if overall_risk_score > REMEDIATION_RISK_THRESHOLD:
            ...
            logger.info(
                f"[Remediation] 🚨 Risk score {overall_risk_score:.3f} exceeds threshold {REMEDIATION_RISK_THRESHOLD}, labeling as {label}"
            )
        else:
            label = "NOT_FRAUD"
            logger.info(
                f"[Remediation] ✅ Risk score {overall_risk_score:.3f} below threshold {REMEDIATION_RISK_THRESHOLD}, labeling as {label}"
            )
        ...
        try:
            ...
            if entity_record:
                ...
                entity_record.status = label.lower().replace(
                    "_", " "
                )  # "suspected fraud" or "not fraud"
                entity_record.last_analyzed = datetime.now(
                ...
                entity_record.attributes["label_assigned_at"] = datetime.now(
                    pytz.timezone("America/New_York")
                ).isoformat()
                ...
                db.commit()
                logger.info(
                    f"[Remediation] ✅ Updated entity record with label: {label}"
                )
            else:
                ...
                entity_record = EntityRecord(
                    ...
                    attributes={
                        "label": label,
                        "label_assigned_at": datetime.now(
                            pytz.timezone("America/New_York")
                        ).isoformat(),
                ...
                db.commit()
                logger.info(
                    f"[Remediation] ✅ Created entity record with label: {label}"
                )
            ...
            logger.error(
                f"[Remediation] ❌ Failed to store entity label: {e}", exc_info=True
            )
            remediation_actions.append(
                {
