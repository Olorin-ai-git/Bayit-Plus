...
class EnhancedToolExecutionLogger:
    ...
    async def log_tool_execution_success(
        ...
        logger.info(
            f"   Data Completeness: {fmt_num(metrics.data_completeness_score, 3)}"
        )
        ...
        if metrics.duration_ms:
            if metrics.duration_ms > self.very_slow_execution_threshold_ms:
                logger.warning(
                    f"⚠️ VERY SLOW EXECUTION: {metrics.duration_ms}ms (threshold: {self.very_slow_execution_threshold_ms}ms)"
                )
                ...
                logger.warning(
                    f"⚠️ Slow execution: {metrics.duration_ms}ms (threshold: {self.slow_execution_threshold_ms}ms)"
                )
    ...
    async def log_tool_execution_failure(
        ...
        if state:
            ...
            logger.error(
                f"     AI Confidence: {fmt_num(state.get('ai_confidence', 0.0), 3)}"
            )
            logger.error(
                f"     Orchestrator Loops: {state.get('orchestrator_loops', 0)}"
            )
            logger.error(
                f"     Previous Tools Used: {len(state.get('tools_used', []))}"
            )
        ...
        if error_analysis["should_warn"]:
            ...
            logger.warning(
                f"   Common Error Category: {error_analysis['most_common_category']}"
            )
    ...
    def get_execution_summary(self) -> Dict[str, Any]:
        ...
        completed = sum(
            1
            for m in self._execution_metrics.values()
    ...
    def _analyze_result_quality(self, result: Any, tool_name: str) -> Dict[str, Any]:
        ...
        if isinstance(result, (list, tuple)):
            ...
            analysis["completeness_score"] = (
                non_empty_values / len(result) if result else 0.0
            )
        ...
        if "snowflake" in tool_name.lower():
            if isinstance(result, list) and len(result) == 0:
                analysis["warnings"].append(
                    "Snowflake query returned no results - check query parameters"
                )
        elif "threat" in tool_name.lower():
            if isinstance(result, dict) and not result.get("data"):
                analysis["warnings"].append(
                    "Threat intelligence API returned no threat data"
                )
    ...
    def _analyze_error_patterns(self, tool_name: str) -> Dict[str, Any]:
        ...
        total_executions = sum(
            1 for m in self._execution_metrics.values() if m.tool_name == tool_name
        )
        error_rate = (
            len(recent_errors) / total_executions if total_executions > 0 else 0.0
        )
        ...
        most_common_category = (
            max(category_counts.keys(), key=category_counts.get)
            if category_counts
            else "unknown"
        )
        ...
        if error_rate >= self.error_rate_critical_threshold:
            ...
            recommendation = (
                "Investigate recent failures and consider adjusting configuration"
            )
    ...
    def _log_investigation_impact(
        self, tool_name: str, state: HybridInvestigationState
    ):
...
def get_tool_execution_logger(
    ...
    if (
        _global_tool_logger is None
        or _global_tool_logger.investigation_id != investigation_id
    ):
