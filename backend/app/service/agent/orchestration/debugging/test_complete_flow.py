...
def simulate_investigation_flow():
    ...
    for i, domain in enumerate(domains):
        ...
        print(
            f"   Domains completed: {len(investigation_state['domains_completed'])}/6"
        )

        next_domain = domains[i + 1] if i + 1 < len(domains) else "summary"
        expected_next = (
            f"{next_domain}_agent" if next_domain != "summary" else "summary"
        )
    ...
    print(
        f"   Investigation status: {'✅ COMPLETE' if investigation_state.get('investigation_complete') else '❌ INCOMPLETE'}"
    )

    # Validate critical fix
    critical_phase = next(
        (phase for phase in flow_log if phase.get("critical_test")), None
    )
    ...
    if critical_phase:
        ...
        no_domain_findings = (
            len(critical_phase["current_state"]["domain_findings"]) == 0
        )

        if tool_results_present and no_domain_findings:
            print(
                f"   ✅ State correctly represents: Tool results available, no domain analysis yet"
            )
            print(
                f"   ✅ This state should trigger domain agents (network_agent first)"
            )
            print(
                f"   ✅ CRITICAL BUG FIXED: Investigation will now continue to domain analysis"
            )
    ...
    simulation_report = {
        ...
        "investigation_complete": investigation_state.get(
