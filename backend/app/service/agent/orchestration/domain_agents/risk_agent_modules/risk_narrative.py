...
class RiskNarrativeGenerator:
    ...
    def generate_aggregation_narrative(
        self, facts: Dict[str, Any], domain_findings: Dict[str, Any]
    ) -> str:
        ...
        if numeric_domains >= 2 or (numeric_domains >= 1 and total_signals >= 2):
            ...
            narrative_parts.append(
                "Evidence gating: BLOCK (insufficient corroboration)"
            )
    ...
    def analyze_cross_domain_patterns(
        self, domain_findings: Dict[str, Any], risk_findings: Dict[str, Any]
    ) -> None:
        ...
        if len(domain_findings) >= 3:
            ...
            if len(high_risk_domains) >= 2:
                ...
                risk_findings["metrics"]["high_risk_domains_count"] = len(
                    high_risk_domains
                )
