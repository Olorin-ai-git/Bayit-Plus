...
class ObservabilityCounter:
    ...
    def increment(
        self, amount: int = 1, metadata: Optional[Dict[str, Any]] = None
    ) -> None:
        """
...
class ObservabilityRegistry:
    ...
    def get_all_stats(self) -> Dict[str, CounterStats]:
        ...
        with self._lock:
            return {
                name: counter.get_stats() for name, counter in self._counters.items()
            }
    ...
    def get_dashboard_summary(self) -> Dict[str, Any]:
        ...
        hotfix_metrics = {
            "graph_selection": {
                "attempts": stats.get(
                    "graph_selection_attempts",
                    CounterStats(0, 0, 0, 0, datetime.now(), 0),
                ).total,
                "failures": stats.get(
                    "graph_selection_failures",
                    CounterStats(0, 0, 0, 0, datetime.now(), 0),
                ...
                "success_rate": self._calculate_success_rate(
                    "graph_selection_attempts", "graph_selection_failures"
                ),
            },
            "context_propagation": {
                "successes": stats.get(
                ...
                "entity_drifts": stats.get(
                    "entity_drift_detections",
                    CounterStats(0, 0, 0, 0, datetime.now(), 0),
                ).total,
                "validation_failures": stats.get(
                    "entity_validation_failures",
                    CounterStats(0, 0, 0, 0, datetime.now(), 0),
                ).total,
            },
            "safety_management": {
                "graceful_finalizations": stats.get(
                    "graceful_finalization_triggers",
                    CounterStats(0, 0, 0, 0, datetime.now(), 0),
                ).total,
                "hard_terminations": stats.get(
                    "hard_termination_triggers",
                    CounterStats(0, 0, 0, 0, datetime.now(), 0),
                ).total,
                "budget_breaches": stats.get(
                    "budget_threshold_breaches",
                    CounterStats(0, 0, 0, 0, datetime.now(), 0),
                ).total,
            },
            "async_clients": {
                "sessions_created": stats.get(
                    "async_sessions_created",
                    CounterStats(0, 0, 0, 0, datetime.now(), 0),
                ).total,
                "sessions_cleaned": stats.get(
                    "async_sessions_cleaned",
                    CounterStats(0, 0, 0, 0, datetime.now(), 0),
                ).total,
                "cleanup_failures": stats.get(
                    "session_cleanup_failures",
                    CounterStats(0, 0, 0, 0, datetime.now(), 0),
                ).total,
            },
        ...
        system_health = {
            "investigations": {
                "starts": stats.get(
                    "investigation_starts", CounterStats(0, 0, 0, 0, datetime.now(), 0)
                ).total,
                "completions": stats.get(
                    "investigation_completions",
                    CounterStats(0, 0, 0, 0, datetime.now(), 0),
                ).total,
                "completion_rate": self._calculate_success_rate(
                    "investigation_starts", "investigation_failures"
                ),
            },
            "performance": {
                "tool_executions": stats.get(
                    "tool_executions", CounterStats(0, 0, 0, 0, datetime.now(), 0)
                ).total,
                "orchestrator_loops": stats.get(
                    "orchestrator_loops", CounterStats(0, 0, 0, 0, datetime.now(), 0)
                ).total,
                "avg_response_time": stats.get(
                    "response_times_ms", CounterStats(0, 0, 0, 0, datetime.now(), 0)
                ).rate_per_minute,
            },
        }
    ...
    def _calculate_success_rate(
        self, attempts_counter: str, failures_counter: str
    ) -> float:
        """Calculate success rate percentage."""
        attempts = self._counters.get(
            attempts_counter, ObservabilityCounter("temp")
        ).get_value()
        failures = self._counters.get(
            failures_counter, ObservabilityCounter("temp")
        ).get_value()
...
def increment_counter(
    name: str, amount: int = 1, metadata: Optional[Dict[str, Any]] = None
) -> None:
