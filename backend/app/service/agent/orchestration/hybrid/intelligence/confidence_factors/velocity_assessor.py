...
class VelocityAssessor:
    ...
    def __init__(self):
        ...
        self.total_phases = (
            6  # initialization, snowflake, tools, domains, summary, complete
        )
        self.phase_order = [
            "initialization",
            "snowflake_analysis",
    ...
    def _calculate_velocity(
        self, state: HybridInvestigationState, start_time: str
    ) -> float:
        ...
        velocity = (phases_completed / self.total_phases) / max(
            1.0, elapsed_minutes / 10.0
        )
        velocity = min(1.0, max(0.1, velocity))  # Bound between 0.1 and 1.0

        logger.debug(
            f"      Elapsed: {elapsed_minutes:.1f} min, Progress: {phases_completed}/{self.total_phases}"
        )
