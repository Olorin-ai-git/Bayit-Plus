...
class EvidenceAnalyzer:
    ...
    async def calculate_evidence_quality(
        ...
        logger.debug(
            f"      Components: Snowflake({snowflake_quality:.2f}), Tools({tool_quality:.2f}), Domains({domain_quality:.2f})"
        )
    ...
    def _assess_domain_quality(self, state: HybridInvestigationState) -> float:
        ...
        high_quality_findings = sum(
            ...
            and findings.get("confidence", 0) > 0.6
        )
