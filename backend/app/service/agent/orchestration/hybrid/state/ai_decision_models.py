...
class AIRoutingDecision:
    ...
    def _validate_confidence(self):
        ...
        if not (0.0 <= self.evidence_quality <= 1.0):
            raise ValueError(
                f"Evidence quality must be between 0.0 and 1.0, got {self.evidence_quality}"
            )

        if not (0.0 <= self.investigation_completeness <= 1.0):
            raise ValueError(
                f"Investigation completeness must be between 0.0 and 1.0, got {self.investigation_completeness}"
            )
    ...
    def _validate_resource_impact(self):
        ...
        if self.resource_impact not in RESOURCE_IMPACT_LEVELS:
            raise ValueError(
                f"Resource impact must be one of {RESOURCE_IMPACT_LEVELS}, got {self.resource_impact}"
            )
...
def create_initial_ai_decision(
    ...
    return AIRoutingDecision(
        ...
        reasoning=[
            ...
            "Snowflake analysis provides essential context",
        ],
...
def create_safety_override(
    ...
    if ai_decisions:
        metrics["investigation_completeness"] = ai_decisions[
            -1
...
class AIRoutingDecisionPayload(BaseModel):
    ...
    recommended_action: str = Field(
        ..., min_length=1, description="Next action to take"
    )
    reasoning: List[str] = Field(..., min_length=1, description="AI reasoning chain")
    evidence_quality: float = Field(
        ..., ge=0.0, le=1.0, description="Evidence quality score (0.0-1.0)"
    )
    investigation_completeness: float = Field(
        ..., ge=0.0, le=1.0, description="Investigation completeness (0.0-1.0)"
    )
    ...
    tools_recommended: List[str] = Field(
        default_factory=list, description="Recommended tools"
    )

    required_safety_checks: List[str] = Field(
        default_factory=list, description="Required safety checks"
    )
    resource_impact: str = Field(..., description="Resource impact level")
    estimated_completion_time: Optional[int] = Field(
        None, ge=0, description="Estimated completion time in minutes"
    )

    model_used: Optional[str] = Field(None, description="AI model used for decision")
    calculation_time_ms: Optional[int] = Field(
        None, ge=0, description="Calculation time in milliseconds"
    )

    model_config = {"validate_assignment": True, "extra": "forbid"}  # Strict validation

    @field_validator(
        "confidence", "evidence_quality", "investigation_completeness", mode="before"
    )
    @classmethod
    ...
    def validate_resource_impact(cls, v):
        ...
        if v not in valid_levels:
            raise ValueError(
                f"Resource impact must be one of {valid_levels}, got '{v}'"
            )
        return v
    ...
    def to_ai_routing_decision(self) -> "AIRoutingDecision":
        ...
        try:
            ...
            confidence_level_enum = level_mapping.get(
                self.confidence_level.upper(), AIConfidenceLevel.MEDIUM
            )
        ...
        try:
            ...
            strategy_enum = strategy_mapping.get(
                self.strategy.lower(), InvestigationStrategy.ADAPTIVE
            )
