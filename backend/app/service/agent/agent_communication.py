...
def _extract_investigation_info(
    ...
    try:
        ...
        logger.debug(
            f"Extracted investigation info: id={investigation_id}, entity={entity_id}"
        )
    ...
    try:
        ...
        with context_lock:
            ...
            if context_key in _investigation_contexts:
                ...
                logger.debug(
                    f"Retrieved existing context for investigation {investigation_id}, entity {entity_id}"
                )
            ...
            logger.info(
                f"Created structured context for investigation {investigation_id}, entity {entity_id}"
            )
            return context
            ...
            if context_lock:
                with context_lock:
                    if context_key in _investigation_contexts:
                        ...
                        logger.info(
                            f"Cleaned up context for investigation {investigation_id}, entity {entity_id}"
                        )
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Failed to complete journey tracking: {str(je)}"
                        )
...
async def get_context_with_retry(
    ...
    logger.error(
        f"Failed to get context after {max_retries} attempts for investigation {investigation_id}"
    )
    return None
