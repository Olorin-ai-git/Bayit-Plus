...
class ICEEventSubscription:
    ...
    # Filtering criteria
    source_filters: Set[str] = field(
        default_factory=set
    )  # Source agent/pattern/tool IDs
    target_filters: Set[str] = field(
        default_factory=set
    )  # Target agent/pattern/tool IDs
...
class ICEEventBus:
    ...
    def subscribe(
        ...
        self.logger.info(
            f"Created subscription {subscription.subscription_id} for {len(event_types)} event types"
        )
    ...
    async def _process_event(self, event: ICEEvent) -> None:
        ...
        try:
            ...
            for subscription_id in self.event_type_subscribers.get(
                event.event_type, []
            ):
            ...
            self.logger.debug(
                f"Processed event {event.event_id} with {len(matching_subscriptions)} handlers"
            )

        except Exception as e:
            self.logger.error(
                f"Failed to process event {event.event_id}: {str(e)}", exc_info=True
            )
            self.stats["processing_errors"] += 1
    ...
    async def _cleanup_old_events(self) -> None:
        ...
        if old_processed_keys or old_correlations:
            self.logger.debug(
                f"Cleaned up {len(old_processed_keys)} old events and {len(old_correlations)} old correlations"
            )
        ...
        return {
            ...
            "correlation_stats": {
                ...
                "total_correlated_events": sum(
                    len(events) for events in self.correlation_chains.values()
                ),
            },
        }
    ...
    def get_subscription_stats(self) -> Dict[str, Any]:
        ...
        for sub_id, subscription in self.subscriptions.items():
            subscription_details[sub_id] = {
                ...
                "last_event_time": (
                    subscription.last_event_time.isoformat()
