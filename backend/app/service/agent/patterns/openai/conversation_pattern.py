...
class OpenAIConversationPattern(OpenAIBasePattern):
    ...
    def __init__(self, config, openai_config, tools=None, ws_streaming=None):
        ...
        if self.tools:
            ...
            logger.info(
                f"Conversation pattern initialized with {len(self._function_definitions)} tools"
            )
    ...
    async def execute_openai_pattern(
        ...
        try:
            ...
            enriched_context = await self._conversation_manager.enrich_context(
            ...
            completion_result = await self._completion_handler.execute_completion(
                openai_messages, function_calling_params, enriched_context
            )
            ...
            if completion_result["success"]:
                ...
                self._conversation_manager.update_context_cache(
                    investigation_id, enriched_context
                )
