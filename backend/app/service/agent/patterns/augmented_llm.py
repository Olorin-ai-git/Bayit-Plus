...
class AugmentedLLMPattern(BasePattern):
        ...
        try:
            ...
            if hasattr(response, "tool_calls") and response.tool_calls:
                response = await self._process_tool_calls(
                    response, augmented_messages, context
                )
    ...
    async def _augment_messages(
        ...
        if memory_context:
            augmented_messages.append(
                SystemMessage(content=f"Previous context: {memory_context}")
            )

        # Add retrieval augmentation if applicable
        if self._should_use_retrieval(context):
            retrieval_context = await self._retrieve_relevant_information(
                messages, context
            )
            if retrieval_context:
                augmented_messages.append(
                    SystemMessage(content=f"Relevant information: {retrieval_context}")
                )
    ...
    async def _build_system_context(self, context: Dict[str, Any]) -> str:
        ...
        if self.tools:
            tool_names = [
                getattr(tool, "name", str(tool.__class__.__name__))
    ...
    async def _retrieve_relevant_information(
        self, messages: List[BaseMessage], context: Dict[str, Any]
    ) -> str:
    ...
    async def _process_tool_calls(
        self, response: AIMessage, messages: List[BaseMessage], context: Dict[str, Any]
    ) -> AIMessage:
    ...
    def _calculate_confidence(
        self, response: AIMessage, context: Dict[str, Any]
    ) -> float:
        ...
        if any(
            ...
            for keyword in ["analysis", "conclusion", "recommendation", "evidence"]
        ):
            confidence += 0.2
    ...
    async def _stream_thinking_process(
        self, messages: List[BaseMessage], context: Dict[str, Any]
    ) -> None:
    ...
    async def _stream_reasoning_complete(
        self, response: AIMessage, context: Dict[str, Any]
    ) -> None:
