...
class ToolResultAugmentationService:
    ...
    def __init__(
        ...
        if self.rag_available:
            ...
            logger.warning(
                "Tool result augmentation service initialized without RAG support"
            )
    ...
    async def augment_result(
        ...
        if not self.rag_available or not result.success:
            logger.debug(
                f"Skipping result augmentation: RAG available={self.rag_available}, result success={result.success}"
            )
            return result
        ...
        try:
            ...
            logger.warning(
                f"Result augmentation failed after {elapsed_time:.1f}ms: {str(e)}"
            )
    ...
    def _update_performance_metrics(self, augmentation_time_ms: float) -> None:
        ...
        new_avg = (
            (current_avg * (successful_count - 1)) + augmentation_time_ms
        ) / successful_count
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        if self.metrics["total_augmentations"] > 0:
            success_rate = (
                self.metrics["successful_augmentations"]
                / self.metrics["total_augmentations"]
            )
