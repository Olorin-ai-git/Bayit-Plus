...
class MCPConnectionPool:
    ...
    def register_server(self, server_name: str, endpoint: str, **config) -> None:
        """Register an MCP server with the pool."""
        logger.info(f"Registering MCP server: {server_name}")

        self.server_configs[server_name] = {"endpoint": endpoint, **config}
    ...
    async def _create_connection(self, server_name: str) -> MCPConnection:
        ...
        connection = MCPConnection(
            server_name=server_name, endpoint=endpoint, state=ConnectionState.CONNECTING
        )
    ...
    async def _close_connection(self, connection: MCPConnection) -> None:
        ...
        if connection.connection:
            try:
                ...
                logger.warning(
                    f"Error closing connection to {connection.server_name}: {e}"
                )
        ...
        try:
            ...
            if health_result:
                ...
                if connection.consecutive_failures >= 3:
                    connection.state = ConnectionState.UNHEALTHY
                    logger.warning(
                        f"Connection to {connection.server_name} marked unhealthy"
                    )
        ...
        for server_name, connections in self.connections.items():
            server_stats = {
                ...
                "average_success_rate": (
                    sum(c.success_rate for c in connections) / len(connections)
                    if connections
                    else 0
                ),
                "connections": [],
