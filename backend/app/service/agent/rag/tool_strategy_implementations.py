...
from .tool_recommender_core import ToolEffectivenessMetrics, ToolRecommendation
...
class ToolStrategyImplementations:
    ...
    async def effectiveness_strategy(
        ...
        for tool in available_tools:
            metrics = effectiveness_data.get(
                tool.name, ToolEffectivenessMetrics(tool.name)
            )

            # Calculate effectiveness-based confidence
            confidence = self.utils.calculate_effectiveness_confidence(
                metrics, knowledge_context
            )

            if confidence >= self.config.min_confidence_threshold:
                reasoning = self.utils.generate_effectiveness_reasoning(
                    metrics, knowledge_context
                )
                ...
                recommendation = ToolRecommendation(
                    ...
                    domain_match=domain in tool.name.lower()
                    or any(
                        domain_term in tool.description.lower()
                        for domain_term in self.utils.domain_tool_preferences.get(
                            domain, []
                        )
                    ),
                )
        ...
        for tool in available_tools:
            ...
            if confidence >= self.config.min_confidence_threshold:
                ...
                recommendation = ToolRecommendation(
                    ...
                    knowledge_sources=[
                        chunk.document_id
                        for chunk in knowledge_context.supporting_knowledge[:2]
                    ],
    ...
    async def domain_specific_strategy(
        ...
        domain_preferences = self.utils.extract_domain_preferences(
            knowledge_context, domain
        )

        for tool in available_tools:
            domain_relevance = self.utils.calculate_domain_relevance(
                tool, domain, domain_preferences
            )
            ...
            if confidence >= self.config.min_confidence_threshold:
                ...
                recommendation = ToolRecommendation(
                    ...
                    knowledge_sources=[
                        chunk.document_id
                        for chunk in knowledge_context.background_knowledge[:2]
                    ],
                    domain_match=True,
                )
    ...
    def combine_hybrid_recommendations(
        ...
        for rec in effectiveness_recs:
            ...
            combined_recommendations[
                tool_name
            ].confidence *= self.config.effectiveness_weight

        # Add case similarity weights
        for rec in case_recs:
            tool_name = rec.tool.name
            if tool_name in combined_recommendations:
                combined_recommendations[tool_name].confidence += (
                    rec.confidence * self.config.case_similarity_weight
                )
                combined_recommendations[tool_name].case_similarity = (
                    rec.case_similarity
                )
            else:
                rec.confidence *= self.config.case_similarity_weight
