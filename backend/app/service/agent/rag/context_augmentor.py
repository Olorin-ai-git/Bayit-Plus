...
class ContextAugmentor:
    ...
    async def augment_investigation_context(
        ...
        try:
            self.logger.info(
                f"Augmenting {domain} context for investigation {investigation_context.investigation_id}"
            )

            # Check cache first
            cache_key = self._create_context_cache_key(
                investigation_context.investigation_id, domain, specific_objectives
            )
            ...
            return KnowledgeContext(
                ...
                entity_type=(
                    investigation_context.entity_type.value
                    if investigation_context.entity_type
                    else None
                ),
            )
    ...
    async def inject_context_into_prompt(
        ...
        try:
            ...
            if (
                include_all_levels
                and knowledge_context.background_knowledge
                and len("\n\n".join(context_sections))
                < self.config.max_context_length // 2
            ):
            ...
            if len(full_context) > self.config.max_context_length:
                full_context = (
                    full_context[: self.config.max_context_length - 100]
...
"""
            ...
            self.logger.debug(
                f"Injected {len(context_sections)} context sections into prompt"
            )
    ...
    async def _generate_domain_queries(
        ...
        if (
            hasattr(investigation_context, "investigation_type")
            and investigation_context.investigation_type
        ):
            inv_type_query = (
                f"{investigation_context.investigation_type} fraud detection {domain}"
            )
    ...
    async def _retrieve_domain_knowledge(
        ...
        try:
            ...
            rag_request = RAGRequest(
                ...
                required_tags=(
                    {domain} if self.config.enable_domain_filtering else set()
                ),
            )
            ...
            self.logger.error(
                f"Knowledge retrieval failed for query '{query}': {str(e)}"
            )
            return []
    ...
    async def _categorize_knowledge_by_relevance(
        ...
        for chunk, score in scored_chunks:
            if (
                ...
                and len(knowledge_context.critical_knowledge)
                < self.config.max_critical_chunks
            ):
                knowledge_context.critical_knowledge.append(chunk)
            elif (
                score >= self.config.supporting_threshold
                and len(knowledge_context.supporting_knowledge)
                < self.config.max_supporting_chunks
            ):
                knowledge_context.supporting_knowledge.append(chunk)
            elif (
                score >= self.config.background_threshold
                and len(knowledge_context.background_knowledge)
                < self.config.max_background_chunks
            ):
    ...
    async def _calculate_chunk_relevance(
        ...
        if domain in chunk.metadata.tags:
            ...
            domain_tag in chunk.metadata.tags
            for domain_tag in [
                f"{domain}_analysis",
                f"{domain}_fraud",
                f"{domain}_detection",
        ...
        if (
            ...
            and investigation_context.entity_type.value in chunk.metadata.entity_types
        ):
            relevance_score += 0.15
    ...
    def _create_context_cache_key(
        self, investigation_id: str, domain: str, objectives: Optional[List[str]]
    ) -> str:
    ...
    def _cache_context(self, cache_key: str, context: KnowledgeContext) -> None:
        ...
        if len(self.context_cache) >= 100:
            # Remove oldest entry
            oldest_key = min(
                self.context_cache.keys(), key=lambda k: self.context_cache[k][1]
            )
