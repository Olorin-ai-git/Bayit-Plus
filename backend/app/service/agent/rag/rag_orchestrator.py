...
from .knowledge_base import DocumentChunk, KnowledgeBase, KnowledgeBaseConfig
...
class RAGOrchestrator:
    ...
    def _initialize_llm_client(self) -> None:
        ...
        try:
            ...
            if self.llm_manager and self.llm_manager.selected_model:
                ...
                self.logger.warning(
                    "LLM manager initialized but no model available - will use fallback"
                )
    ...
    async def _generate_response(
        self, request: RAGRequest, chunks: List[DocumentChunk]
    ) -> RAGResponse:
        ...
        try:
            ...
            response = RAGResponse(
                ...
                retrieval_quality_score=self._calculate_retrieval_quality(
...
5. Do NOT say you don't have access to data - you DO have access through the knowledge context provided below"""
        ...
        if request.investigation_id or request.entity_type:
            ...
            if request.investigation_type:
                investigation_context += (
                    f"Investigation Type: {request.investigation_type}\n"
                )
    ...
    async def _call_llm(self, prompt: str, request: RAGRequest) -> tuple[str, float]:
        ...
        try:
                ...
                try:
                    ...
                    if "=== RELEVANT KNOWLEDGE ===" in prompt:
                        ...
                        if "User Query:" in prompt:
                            query_start = prompt.find("User Query:") + len(
                                "User Query:"
                            )
                    ...
                    self.logger.debug(
                        f"LLM generated response: {len(generated_text)} chars, confidence: {confidence:.2f}"
                    )
                    return generated_text, confidence

                except Exception as llm_error:
                    self.logger.warning(
                        f"LLM call failed, using fallback: {llm_error}", exc_info=True
                    )
            ...
            has_knowledge = (
                "=== RELEVANT KNOWLEDGE ===" in prompt
                or "knowledge_context" in prompt.lower()
            )
    ...
    def _update_metrics(
        ...
        if success:
            ...
            self.metrics.avg_chunks_per_request = (
                self.metrics.total_chunks_retrieved / self.metrics.requests_processed
            )
