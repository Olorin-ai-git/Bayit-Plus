...
class UnifiedSchemaValidator:
    ...
    def extract_risk_score(
        self, agent_output: Any, agent_type: AgentType, debug: bool = False
    ) -> RiskScore:
        """
        ...
        if candidates:
            ...
            if debug:
                logger.debug(
                    f"Selected {best_method} extraction with risk_level={best_result.risk_level}"
                )
        ...
        return RiskScore(
            ...
            validation_errors=[
                f"Could not extract risk score from {type(agent_output)} output"
            ],
        )
    ...
    def _extract_from_json(self, output: Any, agent_type: AgentType) -> RiskScore:
        ...
        try:
            ...
            if isinstance(output, str):
                if output.strip().startswith("{"):
                    ...
                    return RiskScore(
                        ...
                        agent_type.value,
                        output,
                        validation_errors=["Not JSON string"],
                    )
                ...
                return RiskScore(
                    ...
                    agent_type.value,
                    output,
                    validation_errors=["Not JSON format"],
                )
            ...
            if len(missing_elements) == len(required_elements) and required_elements:
                errors.append(
                    f"Missing all domain-specific elements. Expected any of: {required_elements}"
                )
            elif (
                len(missing_elements) > len(required_elements) * 0.7
            ):  # If more than 70% missing
                errors.append(
                    f"Missing most domain-specific elements: {missing_elements[:3]}"
                )  # Only show first 3
                ...
                for match in matches:
                    try:
                        ...
                        if 0.0 <= score <= 1.0:
                            ...
                            confidence = (
                                0.6  # Lower confidence for percentage conversion
                            )
    ...
    def _get_json_paths_for_agent(
        ...
        return paths.get(
            agent_type, {"risk_level": [["risk_level"]], "confidence": [["confidence"]]}
        )

    def _get_text_patterns_for_agent(
        self, agent_type: AgentType
    ) -> Dict[str, List[str]]:
    ...
    def _recursive_key_search(self, obj: Any, target_key: str) -> bool:
        ...
        if isinstance(obj, dict):
            for key, value in obj.items():
                ...
                target_normalized = (
                    target_key.replace("_", "").replace("-", "").replace(" ", "")
                )
            ...
            obj_normalized = (
                obj.lower().replace("_", "").replace("-", "").replace(" ", "")
            )
            target_normalized = (
                target_key.replace("_", "").replace("-", "").replace(" ", "")
            )
    ...
    def get_extraction_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": (total - self.extraction_stats["failed_extractions"])
            / max(total, 1),
