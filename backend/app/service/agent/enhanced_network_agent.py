    ...
    try:
        ...
        for source_name, source_data in data_sources.items():
            if isinstance(source_data, dict):
                ...
                for field in domain_fields:
                    if field in source_data:
                        ...
                        if isinstance(domain_value, str):
                            extracted_domain = _extract_domain_from_url_or_text(
                                domain_value
                            )
                            ...
                            for domain in domain_value:
                                if isinstance(domain, str):
                                    ...
                                    if extracted_domain and _is_valid_domain(
                                        extracted_domain
                                    ):
                                        valid_domains.append(extracted_domain)
                ...
                for field in text_fields:
                    if field in source_data and isinstance(source_data[field], str):
                        extracted_domains = _extract_domains_from_text(
                            source_data[field]
                        )
...
async def enhanced_structured_network_agent(state, config) -> dict:
    ...
    journey_tracker.track_node_execution(
        ...
        metadata={
            "domain": "network",
            "analysis_type": "threat_intelligence_enhanced",
            "objectives_count": 6,
        },
    )
    ...
    try:
        ...
        threat_intel_tools = [
            ...
            or "abuseipdb" in t.name.lower()
            or "virustotal" in t.name.lower()
            or "shodan" in t.name.lower()
        ]

        logger.info(
            f"🔍 Available threat intelligence tools for network agent: {[t.name for t in threat_intel_tools]}"
        )
        ...
        if valid_ips:
            logger.info(
                f"📍 Valid IPs to analyze: {valid_ips[:5]}..."
                if len(valid_ips) > 5
                else f"📍 Valid IPs to analyze: {valid_ips}"
            )
        if valid_domains:
            logger.info(
                f"🌍 Valid domains to analyze: {valid_domains[:3]}..."
                if len(valid_domains) > 3
                else f"🌍 Valid domains to analyze: {valid_domains}"
            )
        ...
        if valid_ips or valid_domains:
            ...
            structured_context.add_data_source(
                "extracted_network_indicators", extracted_network_data
            )
...
def _extract_threat_intelligence_summary(findings) -> Dict[str, Any]:
    ...
    if findings.raw_data:
        ...
        if any(
            key in str(findings.raw_data).lower()
            for key in ["abuseipdb", "abuse_confidence", "abuse_score"]
        ):
        ...
        if any(
            key in str(findings.raw_data).lower()
            for key in ["virustotal", "vt_", "malicious_votes"]
        ):
        ...
        if any(
            key in str(findings.raw_data).lower()
            for key in ["shodan", "open_ports", "infrastructure"]
        ):
    ...
    for finding in findings.key_findings:
        ...
        if (
            "port" in finding_lower
            or "service" in finding_lower
            or "vulnerability" in finding_lower
        ):
