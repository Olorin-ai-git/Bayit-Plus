...
class EnhancedSecurityConfig:
    ...
    def _get_secure_secret(self, env_var: str, byte_length: int = 64) -> str:
        ...
        if not secret or secret in [
            ...
            logger.warning(
                f"Generated new secret for {env_var}. Set this in environment: {masked_secret} (Value has been masked for security)"
            )
        return secret
    ...
    def _init_redis(self) -> Optional[redis.Redis]:
        ...
        try:
            ...
            logger.warning(
                f"Redis connection failed: {e}. Session management will use memory."
            )
            return None
...
class EnhancedAuthService:
    ...
    async def authenticate_user(
        ...
        try:
            ...
            db_user = (
                ...
                .filter(UserInDB.username == username.lower())
                .first()
            )
            ...
            if not db_user:
                ...
                self._log_security_event(
                    "LOGIN_FAILED", username, ip, "Invalid username"
                )
            ...
            if (
                ...
                and db_user.account_locked_until > datetime.utcnow()
            ):
                remaining = (
                    db_user.account_locked_until - datetime.utcnow()
                ).seconds // 60
                self._log_security_event(
                    "LOGIN_BLOCKED",
                    username,
            ...
            if not self.pwd_context.verify(password, db_user.hashed_password):
                ...
                self._log_security_event(
                    "LOGIN_FAILED", username, ip, "Invalid password"
                )
                return None, "Invalid credentials"
            ...
            if db_user.disabled:
                self._log_security_event(
                    "LOGIN_FAILED", username, ip, "Account disabled"
                )
            ...
            self._log_security_event(
                "LOGIN_SUCCESS", username, ip, "Authentication successful"
            )
            return user, "success"
            ...
            self._log_security_event(
                "LOGIN_ERROR", username, ip, f"Authentication error: {str(e)}"
            )
    ...
    async def validate_token(
        self, token: str, ip: str, required_scopes: List[str] = None
    ) -> Tuple[Optional[SecureUser], str]:
        ...
        try:
            ...
            if token_ip_hash and token_ip_hash != current_ip_hash:
                self._log_security_event(
                    "TOKEN_IP_MISMATCH", username, ip, "IP address mismatch"
                )
            ...
            # Get user from database
            db_user = (
                self.db.query(UserInDB).filter(UserInDB.username == username).first()
            )
    ...
    def revoke_token(self, token: str) -> bool:
        ...
        try:
            ...
            if self.config.redis_client and jti and exp:
                ...
                if remaining_ttl > 0:
                    self.config.redis_client.setex(
                        f"blacklist:{jti}", remaining_ttl, "revoked"
                    )
                    return True
    ...
    def _log_security_event(
        self, event_type: str, username: str, ip: str, details: str
    ):
...
async def get_current_user(
    ...
    user, error = await auth_service.validate_token(
        credentials.credentials, request.client.host
    )
...
def require_scopes(required_scopes: List[str]):
    ...
    async def check_scopes(
        ...
        user, error = await auth_service.validate_token(
            credentials.credentials, request.client.host, required_scopes
        )
