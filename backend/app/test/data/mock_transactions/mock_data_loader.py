...
class MockDataLoader:
    ...
    def load_all_scenarios(self) -> None:
        ...
        logger.info(
            f"Loaded {total_scenarios} scenarios "
    ...
    def _load_scenarios(self, scenario_type: ScenarioType) -> None:
        ...
        for json_file in scenarios_path.glob("*.json"):
            try:
                ...
                logger.debug(
                    f"Loaded scenario: {scenario.scenario_name} from {json_file.name}"
                )
    ...
    def get_scenarios_by_complexity(
        self, complexity: InvestigationComplexity
    ...
    def validate_investigation_results(
        self, scenario_name: str, actual_results: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        expected_classification = scenario.investigation_verdict.get(
            "fraud_classification", ""
        )
        actual_classification = actual_results.get("fraud_classification", "")
        classification_match = (
            expected_classification.upper() == actual_classification.upper()
        )
        ...
        validation_results.update(
            {
                "risk_score_validation": {
                    ...
                    "acceptable_deviation": risk_deviation
                    <= 10,  # Within 10 points acceptable
                },
                ...
                "overall_investigation_quality": {
                    ...
                    "agents_performed_well": all(
                        finding["risk_level_match"]
                        for finding in agent_findings_validation.values()
                    ),
                },
            }
        )
...
def validate_investigation_outcome(
    scenario_name: str, actual_results: Dict[str, Any]
) -> Dict[str, Any]:
    """
    Convenience function to validate investigation results against expected outcomes.
    """
    return mock_data_loader.validate_investigation_results(
        scenario_name, actual_results
    )
