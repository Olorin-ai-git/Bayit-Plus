...
def evaluate_board(board: chess.Board) -> int:
    ...
    board.pop()
    score += (
        (mobility - opponent_mobility) * 5
        if board.turn == chess.WHITE
        else (opponent_mobility - mobility) * 5
    )
...
def minimax(
    board: chess.Board, depth: int, alpha: int, beta: int, maximizing: bool
...
async def get_bot_move(
    board_fen: str, difficulty: BotDifficulty
