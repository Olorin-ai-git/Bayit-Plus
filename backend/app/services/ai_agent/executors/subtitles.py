...
async def _extract_subtitles_background(content_id: str, stream_url: str):
    ...
    try:
        logger.info(
            f"[Background] Starting subtitle extraction for content {content_id}"
        )
        ...
        extracted_subs = await ffmpeg_service.extract_all_subtitles(
            stream_url, languages=["en", "he", "es"], max_parallel=3, max_subtitles=10
        )
        ...
        for sub in extracted_subs:
            try:
                existing = await SubtitleTrackDoc.find_one(
                    {"content_id": content_id, "language": sub["language"]}
                )
                ...
                logger.info(
                    f"[Background] Saved {sub['language']} subtitles ({len(cues)} cues)"
                )

            except Exception as e:
                logger.error(
                    f"[Background] Failed to save {sub['language']} subtitles: {str(e)}"
                )

        content.has_subtitles = saved_count > 0
        content.available_subtitle_languages = saved_languages
        content.subtitle_extraction_status = (
            "completed" if saved_count > 0 else "failed"
        )
        await content.save()

        logger.info(
            f"[Background] Extraction complete for {content_id}: {saved_count} tracks saved"
        )

    except Exception as e:
        logger.error(
            f"[Background] Subtitle extraction failed for {content_id}: {str(e)}"
        )
        try:
            content = await Content.get(PydanticObjectId(content_id))
            if content:
...
async def execute_scan_video_subtitles(
    content_id: str, auto_extract: bool = True
) -> Dict[str, Any]:
    ...
    try:
        ...
        if auto_extract and len(metadata["subtitle_tracks"]) > 0:
            asyncio.create_task(
                _extract_subtitles_background(content_id, content.stream_url)
            )
...
async def execute_extract_video_subtitles(
    content_id: str, audit_id: str, languages: Optional[List[str]] = None
) -> Dict[str, Any]:
    ...
    try:
        ...
        asyncio.create_task(
            _extract_with_audit(content_id, content.stream_url, audit_id, languages)
        )
    ...
    try:
        ...
        for sub in extracted_subs:
            try:
                ...
                logger.error(
                    f"[Background] Failed to parse {sub['language']} subtitles: {str(e)}"
                )
        ...
        logger.info(
            f"[Background] Extraction complete for audit {audit_id}: {saved_count} tracks saved"
        )
...
async def execute_verify_required_subtitles(
    content_id: str, required_languages: List[str] = None
) -> Dict[str, Any]:
    ...
    try:
        ...
        missing_languages = [
            lang for lang in required_languages if lang not in existing_languages
        ]
...
async def execute_search_external_subtitles(
    content_id: str, language: str, sources: List[str] = None
) -> Dict[str, Any]:
    ...
    try:
        ...
        results = await service.search_subtitle_sources(
            content_id=content_id, language=language, sources=sources
        )
        return {
            "success": True,
            "found": results["found"],
...
async def execute_batch_download_subtitles(
    ...
    try:
        ...
        if audit_id:
            ...
            titles_affected = [
                d.get("title", d.get("content_id", "unknown"))
                for d in success_items[:5]
            ]
        ...
        if success_items:
            ...
            for item in success_items[:5]:
                ...
                summary_parts.append(
                    f"  - {item['title']} ({lang_short} from {item['source']})"
                )
            if len(success_items) > 5:
