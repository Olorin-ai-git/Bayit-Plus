...
async def execute_validate_taxonomy_compliance(
    ...
    try:
        ...
        # Load reference data
        sections = {str(s.id): s for s in await ContentSection.find(ContentSection.is_active == True).to_list()}
        genres = {str(g.id): g for g in await Genre.find(Genre.is_active == True).to_list()}
        audiences = {str(a.id): a for a in await Audience.find(Audience.is_active == True).to_list()}
...
async def execute_suggest_taxonomy_classification(
    ...
    try:
        ...
        if section_slug and section_slug in section_map:
            ...
            reasoning.append(f"Suggested section '{section_slug}' based on category '{content.category_name}'")
        ...
        if content_format:
            ...
            reasoning.append(f"Suggested format '{content_format}' based on is_series={content.is_series}, content_type={content.content_type}")
        ...
        if audience_slug and audience_slug in audience_map:
            ...
            reasoning.append(f"Suggested audience '{audience_slug}' based on age_rating={getattr(content, 'age_rating', None)}, content_rating={getattr(content, 'content_rating', None)}")
    ...
    try:
        ...
        if content_format is not None:
            if content_format not in CONTENT_FORMATS:
                return {"success": False, "error": f"Invalid content_format: {content_format}"}
        ...
        if dry_run:
            logger.info(f"[DRY RUN] Would apply taxonomy to {content_id}: {update_data}")
            return {
                "success": True,
                "dry_run": True,
                "content_id": content_id,
        ...
        if audit_id:
            action = LibrarianAction(
                ...
                details={"changes": changes, "update_data": {k: str(v) for k, v in update_data.items() if k != "updated_at"}},
            )
            await action.insert()
    ...
    try:
        ...
        for content in content_items:
            try:
                ...
                if dry_run:
                    logger.info(f"[DRY RUN] Would migrate {content.id} to section '{section_slug}'")
                else:
                    await content.set(update_data)
...
async def execute_get_taxonomy_summary(
    ...
    try:
        ...
        migrated = await Content.find({"section_ids": {"$exists": True, "$ne": []}}).count()
        ...
        return {
            ...
            "migration_status": {
                ...
                "migrated": migrated,
                "pending": pending,
                "percentage": round((migrated / total_content * 100) if total_content > 0 else 0, 2),
            },
        }
...
async def execute_list_taxonomy_violations(
    ...
    try:
        ...
        # Load valid IDs
        valid_section_ids = {str(s.id) for s in await ContentSection.find(ContentSection.is_active == True).to_list()}
        valid_genre_ids = {str(g.id) for g in await Genre.find(Genre.is_active == True).to_list()}
        valid_audience_ids = {str(a.id) for a in await Audience.find(Audience.is_active == True).to_list()}

        # Missing section
        if violation_type in ["missing_section", "all"]:
            missing_section = await Content.find({
                "$or": [
                    {"section_ids": {"$exists": False}},
                    {"section_ids": {"$size": 0}},
                    {"primary_section_id": None},
        ...
        if violation_type in ["missing_audience", "all"] and len(violations) < limit:
            ...
            missing_audience = await Content.find({
                "section_ids": {"$exists": True, "$ne": []},
                "$or": [
                    {"audience_id": None},
                    {"audience_id": {"$exists": False}},
                ]
            }).limit(remaining).to_list()
        ...
        if violation_type in ["invalid_format", "all"] and len(violations) < limit:
            remaining = limit - len(violations)
            invalid_format = await Content.find({
                "content_format": {"$nin": [None, "movie", "series", "documentary", "short", "clip"]}
            }).limit(remaining).to_list()
