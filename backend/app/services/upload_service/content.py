...
class ContentEntryCreator:
    ...
    async def _create_movie(self, job: UploadJob) -> Optional[str]:
        ...
        category = await self._get_category("movies", {
            "name": "סרטים",
            "name_en": "Movies",
            "name_es": "Películas",
            "icon": "film",
    ...
    async def _create_series_or_episode(self, job: UploadJob) -> Optional[str]:
        ...
        category = await self._get_category("series", {
            "name": "סדרות",
            "name_en": "Series",
            "name_es": "Series",
            "icon": "tv",
    ...
    async def _create_episode(self, job: UploadJob, category: ContentSection, m: dict, season: Optional[int], episode: Optional[int]) -> Content:
    ...
    async def _create_series_container(self, job: UploadJob, category: ContentSection, m: dict) -> Content:
    ...
    async def _find_or_create_series(self, name: str, m: dict, category: ContentSection) -> tuple[Optional[str], Optional[Content]]:
        ...
        if tmdb_id := m.get('tmdb_id'):
            if series := await Content.find_one({"is_series": True, "tmdb_id": tmdb_id}):
                return str(series.id), series
