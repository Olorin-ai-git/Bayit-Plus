...
from app.services.support.chat_utils import check_escalation, build_chat_result
...
async def chat(
    ...
    try:
        ...
        escalation_needed, escalation_reason = check_escalation(message, response_text, context)
        ...
        confidence = HIGH_CONFIDENCE_SCORE if not escalation_needed else LOW_CONFIDENCE_SCORE
        return build_chat_result(
            response_text, str(conversation.id), language, doc_paths,
            escalation_needed, escalation_reason, confidence
        )
