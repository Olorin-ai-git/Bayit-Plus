...
from app.services.support.constants import (
    URGENT_PRIORITY_WORDS, HIGH_PRIORITY_WORDS, LOW_PRIORITY_WORDS, DEFAULT_PRIORITY,
)
...
async def create_ticket(
    user: User, subject: str, message: str, category: str = 'general',
    ...
    ticket = SupportTicket(
        ...
        priority=priority, language=language, voice_conversation_id=voice_conversation_id,
    )
    await ticket.insert()
...
async def get_ticket(
    ticket_id: str, user: Optional[User] = None, is_admin: bool = False,
...
async def list_user_tickets(
    user: User, page: int = 1, page_size: int = 20, status: Optional[str] = None,
...
async def get_ticket_stats() -> dict:
    ...
    stats['by_priority'] = {result['_id']: result['count'] for result in priority_results}
...
async def update_ticket(
    ticket_id: str, admin: User, status: Optional[str] = None,
...
async def add_ticket_message(
    ticket_id: str, author: User, content: str, is_support: bool = False,
) -> Optional[SupportTicket]:
    ...
    message = TicketMessage(
        author_id=str(author.id), author_name=author.name,
        author_role='support' if is_support else 'user', content=content,
    )
...
async def add_ticket_note(
    ticket_id: str, admin: User, content: str,
) -> Optional[SupportTicket]:
    ...
    note = TicketNote(
        author_id=str(admin.id), author_name=admin.name, content=content,
    )
