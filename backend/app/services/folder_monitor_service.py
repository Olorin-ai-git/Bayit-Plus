...
class FolderMonitorService:
        ...
        try:
            ...
            for folder in folders:
                try:
                    ...
                    logger.error(
                        f"Error scanning folder {folder.path}: {e}", exc_info=True
                    )
                    stats["errors"] += 1
    ...
    async def _scan_folder(self, folder: MonitoredFolder) -> Dict[str, int]:
        ...
        patterns = (
            ...
            else self._get_default_patterns(folder.content_type)
        )
                ...
                if file_path not in self._known_files[folder_id]:
                    try:
                        ...
                        if file_hash:
                            ...
                            existing_by_hash = await db.content.find_one(
                                {"file_hash": file_hash}
                            )
