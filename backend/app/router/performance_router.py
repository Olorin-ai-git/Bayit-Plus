    ...
    try:
            ...
            return {
                "system_metrics": {
                    ...
                    "memory_usage_percent": (
                        process.memory_info().rss / system_memory.total
                    )
                    * 100,
        ...
        return {
            "system_metrics": {
                ...
                "memory_usage_percent": current_metrics.get(
            ...
            "cache_metrics": {
                ...
                "cache_size_mb": cache_stats.get("capacity", {}).get(
                    "total_size_bytes", 0
                )
                / 1024
                / 1024,
                "total_entries": cache_stats.get("capacity", {}).get("entry_count", 0),
                "memory_usage_percent": cache_stats.get("capacity", {}).get(
                    "memory_usage_percent", 0.0
                ),
                "evictions": cache_stats.get("performance", {}).get("evictions", 0),
                "expired_entries": cache_stats.get("performance", {}).get(
                    "expired_entries", 0
                ),
            },
            "agent_metrics": {
                ...
                "total_agent_calls": request_stats.get(
                    "total_count", 0
                ),  # Approximation
                "average_agent_response_time_ms": request_stats.get(
...
async def get_performance_metrics_history(
    minutes: int = Query(
        default=30, description="Number of minutes of history to retrieve"
    )
):
...
async def get_investigation_performance(investigation_id: str):
    ...
    try:
        ...
        if perf_manager:
            ...
            investigation_metrics = perf_manager.get_investigation_metrics(
                investigation_id
            )
        else:
        ...
        if investigation_metrics:
            ...
            base_metrics = {
                ...
                "performance_metrics": {
                    "total_execution_time_ms": investigation_metrics.get(
                        "total_time_ms", 0
                    ),
                    "agent_execution_times": investigation_metrics.get(
                        "agent_times", {}
                    ),
        ...
        if perf_manager:
            ...
            base_metrics["performance_metrics"].update(
                {
                    "average_response_time_ms": request_stats.get(
                        "avg_response_time_ms", 0.0
                    ),
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Investigation performance retrieval failed: {str(e)}",
        )
...
async def get_agent_performance():
    ...
    try:
        ...
        if perf_manager:
            ...
            global_cache_hit_rate = cache_stats.get("performance", {}).get(
                "hit_rate", 0.0
            )
            ...
            for agent_name in base_agent_data.keys():
                base_agent_data[agent_name].update(
                    {
                        "calls_today": request_stats.get("total_count", 0)
        ...
        return {
            ...
            "summary": {
                ...
                "active_agents": (
                    perf_manager.monitor.get_current_metrics().active_agents
                    if perf_manager
                    else 0
                ),
                "overall_success_rate": sum(
                    agent["success_rate"] for agent in base_agent_data.values()
                )
                / len(base_agent_data),
                "overall_avg_response_time_ms": sum(
                    agent["average_response_time_ms"]
                    for agent in base_agent_data.values()
                )
                / len(base_agent_data),
            },
            "timestamp": datetime.now(timezone.utc).isoformat(),
        ...
        raise HTTPException(
            status_code=500, detail=f"Agent performance retrieval failed: {str(e)}"
        )
    ...
    try:
        ...
        if perf_manager:
            ...
            if current_metrics:
                ...
                if (
                    current_metrics.cache_hit_rate < 0.7
                    and current_metrics.cache_hit_rate > 0
                ):
                    alerts.append(
                        {
                            "type": "warning",
        ...
        raise HTTPException(
            status_code=500, detail=f"Performance alerts retrieval failed: {str(e)}"
        )
