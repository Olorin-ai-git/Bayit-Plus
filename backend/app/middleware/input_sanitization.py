        ...
        if request.query_params:
            ...
            if sanitized_query != dict(request.query_params):
                if self.enable_logging:
                    logger.warning(f"Sanitized query params for {request.url.path} from IP: {request.client.host}")
            ...
            if "application/json" in content_type:
                try:
                    ...
                    if self._contains_dangerous_patterns(str(sanitized_body)):
                        logger.error(f"Blocked request with dangerous patterns to {request.url.path} from IP: {request.client.host}")
                    ...
                    if sanitized_body != body and self.enable_logging:
                        logger.warning(f"Sanitized JSON body for {request.url.path} from IP: {request.client.host}")
