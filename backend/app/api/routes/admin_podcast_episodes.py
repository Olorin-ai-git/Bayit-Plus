...
from .admin_content_schemas import PodcastEpisodeCreateRequest, PodcastEpisodeUpdateRequest
...
async def get_podcast_episodes(
    ...
    return {
        "items": [{
            "id": str(item.id), "podcast_id": item.podcast_id, "title": item.title,
            "description": item.description, "audio_url": item.audio_url, "duration": item.duration,
            "episode_number": item.episode_number, "season_number": item.season_number,
            "published_at": item.published_at.isoformat(), "thumbnail": item.thumbnail,
        } for item in items],
...
async def create_podcast_episode(
    ...
    episode = PodcastEpisode(
        podcast_id=podcast_id, title=data.title, description=data.description,
        audio_url=data.audio_url, duration=data.duration, episode_number=data.episode_number,
        season_number=data.season_number, published_at=data.published_at, thumbnail=data.thumbnail,
    )
    ...
    await log_audit(str(current_user.id), AuditAction.PODCAST_EPISODE_CREATED, "podcast_episode",
        str(episode.id), {"podcast_id": podcast_id, "title": episode.title}, request)
...
async def update_podcast_episode(
    ...
    if data.episode_number is not None:
        changes["episode_number"] = {"old": episode.episode_number, "new": data.episode_number}
        episode.episode_number = data.episode_number
    if data.season_number is not None:
        changes["season_number"] = {"old": episode.season_number, "new": data.season_number}
        episode.season_number = data.season_number
    if data.published_at is not None:
        changes["published_at"] = {"changed": True}
        episode.published_at = data.published_at
        if not podcast.latest_episode_date or episode.published_at > podcast.latest_episode_date:
    ...
    await log_audit(str(current_user.id), AuditAction.PODCAST_EPISODE_UPDATED, "podcast_episode",
                    episode_id, changes, request)
...
async def delete_podcast_episode(
    ...
    await log_audit(str(current_user.id), AuditAction.PODCAST_EPISODE_DELETED, "podcast_episode",
        episode_id, {"podcast_id": podcast_id, "title": episode.title}, request)
