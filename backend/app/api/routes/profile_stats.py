...
async def get_profile_stats(
    ...
    watchlist_count = await WatchlistItem.find(
        WatchlistItem.user_id == user_id
    ).count()

    # Count favorites
    favorites_count = await Favorite.find(
        Favorite.user_id == user_id
    ).count()

    # Calculate total watch time from watch history
    watch_history = await WatchHistory.find(
        WatchHistory.user_id == user_id
    ).to_list()

    total_watch_seconds = sum(
        item.position for item in watch_history
    )
