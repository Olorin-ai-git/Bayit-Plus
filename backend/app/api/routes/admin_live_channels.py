...
from .admin_content_schemas import LiveChannelCreateRequest, LiveChannelUpdateRequest
...
async def get_live_channels(is_active: Optional[bool] = None, page: int = Query(default=1, ge=1),
    ...
    items = await query.sort(LiveChannel.order).skip((page - 1) * page_size).limit(page_size).to_list()
    return {
...
async def get_live_channel(channel_id: str, current_user: User = Depends(has_permission(Permission.CONTENT_READ))):
...
async def create_live_channel(data: LiveChannelCreateRequest, request: Request,
...
async def update_live_channel(channel_id: str, data: LiveChannelUpdateRequest, request: Request,
    ...
    if data.is_drm_protected is not None:
        changes["is_drm_protected"] = {"old": channel.is_drm_protected, "new": data.is_drm_protected}
    ...
    if data.current_show is not None:
        changes["current_show"] = {"old": channel.current_show, "new": data.current_show}
...
async def delete_live_channel(channel_id: str, request: Request,
    ...
    if not channel:
        raise HTTPException(status_code=404, detail="Channel not found")
    await log_audit(str(current_user.id), AuditAction.LIVE_CHANNEL_DELETED, "live_channel", channel_id,
                    {"name": channel.name}, request)
    await channel.delete()
    return {"message": "Channel deleted"}

@router.post("/live-channels/reorder")
async def reorder_live_channels(order_data: dict, request: Request,
                                current_user: User = Depends(has_permission(Permission.CONTENT_UPDATE))):
    ...
    for cid, pos in order_data.items():
        try:
            ...
            pass
    await log_audit(str(current_user.id), AuditAction.LIVE_CHANNEL_UPDATED, "live_channel", None,
