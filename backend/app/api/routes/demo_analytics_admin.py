...
async def get_summary(
    ...
    track_counts = await PlaygroundEvent.aggregate(pipeline_sessions).to_list()
    ...
    event_counts = await PlaygroundEvent.aggregate(pipeline_events).to_list()
    ...
    stop_docs = await PlaygroundEvent.aggregate(pipeline_stops).to_list()
    viewed: dict[str, int] = {}
    ...
    daily_raw = await PlaygroundEvent.aggregate(pipeline_daily).to_list()
    ...
    cta_docs = await PlaygroundEvent.aggregate(pipeline_ctas).to_list()
    top_ctas = [
        TopCta(
            type=doc["_id"]["type"],
...
async def get_funnel(
    ...
    sessions = await PlaygroundEvent.aggregate(pipeline).to_list()
    total = len(sessions)
    if total == 0:
