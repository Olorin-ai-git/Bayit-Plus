...
async def _run_tool_loop(
    client: anthropic.Anthropic, response: anthropic.types.Message,
    messages: list[dict], system_prompt: str, max_iterations: int = 3
    ...
    while response.stop_reason == "tool_use" and max_iterations > 0:
        ...
        tool_msgs.append({
            "role": "assistant",
            "content": [{"type": b.type, **b.model_dump()} for b in response.content]
        })
        tool_msgs.append({"role": "user", "content": tool_results})
        response = client.messages.create(
            model=settings.CLAUDE_MODEL, max_tokens=1024,
            system=system_prompt, messages=tool_msgs, tools=CHAT_TOOLS
        )
    return response
...
async def send_message(
    ...
    conv = await _get_or_create_conversation(request.conversation_id, str(current_user.id))
    conv.messages.append({
        "role": "user", "content": request.message, "timestamp": datetime.utcnow().isoformat()
    })
    messages = [{"role": m["role"], "content": m["content"]} for m in conv.messages[-10:]]
    ...
    try:
        ...
        resp = client.messages.create(
            model=settings.CLAUDE_MODEL, max_tokens=1024,
            system=sys_prompt, messages=messages, tools=CHAT_TOOLS
        )
        resp = await _run_tool_loop(client, resp, messages, sys_prompt)
        raw = next((b.text for b in resp.content if b.type == "text"), "")
        asst_msg, _ = extract_json_from_response(raw)
        conv.messages.append({
            "role": "assistant", "content": asst_msg, "timestamp": datetime.utcnow().isoformat()
        })
        conv.updated_at = datetime.utcnow()
        await conv.save()
        recs = await get_recommendations_from_response(asst_msg, request.message, media_ctx)
        action = await extract_action_from_response(asst_msg, request.message, lang)
        final_msg = await align_message_with_action(asst_msg, action, request.message, lang)
        return ChatResponse(
            message=final_msg, conversation_id=str(conv.id), recommendations=recs,
            language=lang, spoken_response=strip_markdown(final_msg), action=action,
            content_ids=None, visual_action=None,
