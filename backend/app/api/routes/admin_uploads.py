...
async def run_integrity_cleanup(
    dry_run: bool = Query(default=True, description="If true, only report what would be done"),
    limit: int = Query(default=100, le=500, description="Maximum items to clean per category"),
    current_user: User = Depends(has_permission(Permission.CONTENT_CREATE))
) -> CleanupResponse:
    """
