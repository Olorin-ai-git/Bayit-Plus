...
async def get_usage_report(
    ...
    feature_type: Optional[FeatureType] = Query(None), platform: Optional[str] = Query(None),
    ...
    try:
        query = LiveFeatureUsageSession.find(LiveFeatureUsageSession.started_at >= start_date, LiveFeatureUsageSession.started_at <= end_date)
        if feature_type:
        ...
        return {
            "summary": {
                "total_sessions": total, "total_minutes": round(total_minutes, 2), "total_cost": round(total_cost, 4),
                "subtitle_sessions": len(subtitle_sessions), "subtitle_minutes": round(sum(s.duration_seconds / 60.0 for s in subtitle_sessions), 2),
                "dubbing_sessions": len(dubbing_sessions), "dubbing_minutes": round(sum(s.duration_seconds / 60.0 for s in dubbing_sessions), 2),
            },
            "sessions": [
                {"session_id": s.session_id, "user_id": s.user_id, "channel_id": s.channel_id, "feature_type": s.feature_type,
                 "started_at": s.started_at, "ended_at": s.ended_at, "duration_minutes": round(s.duration_seconds / 60.0, 2),
                 "status": s.status, "source_language": s.source_language, "target_language": s.target_language,
                 "platform": s.platform, "estimated_cost": round(s.estimated_total_cost, 4)}
                for s in sessions
            ],
...
async def get_system_stats(current_user: User = Depends(has_permission(Permission.ANALYTICS_READ))):
    ...
    try:
        ...
        active_sessions = await LiveFeatureUsageSession.find(LiveFeatureUsageSession.status == UsageSessionStatus.ACTIVE).count()
        today_pipeline = [
        ...
        return {
            "total_users_with_quotas": total_users, "active_sessions": active_sessions,
            "total_subtitle_minutes_today": round(subtitle_minutes_today, 2), "total_dubbing_minutes_today": round(dubbing_minutes_today, 2),
            "total_cost_today": round(cost_today, 4), "total_cost_month": round(cost_month, 4),
        }
    except Exception as e:
...
async def get_top_users(
    feature_type: Optional[FeatureType] = Query(None), days: int = Query(30, ge=1, le=365),
    limit: int = Query(20, ge=1, le=100), current_user: User = Depends(has_permission(Permission.ANALYTICS_READ)),
):
    ...
    try:
        ...
        pipeline = [
            match_stage,
            {"$group": {
                "_id": "$user_id", "total_sessions": {"$sum": 1},
                "total_minutes": {"$sum": {"$divide": ["$duration_seconds", 60]}},
                "total_cost": {"$sum": "$estimated_total_cost"},
                "subtitle_minutes": {"$sum": {"$cond": [{"$eq": ["$feature_type", FeatureType.SUBTITLE]}, {"$divide": ["$duration_seconds", 60]}, 0]}},
                "dubbing_minutes": {"$sum": {"$cond": [{"$eq": ["$feature_type", FeatureType.DUBBING]}, {"$divide": ["$duration_seconds", 60]}, 0]}},
            }},
        ...
        return {"period_days": days, "feature_type": feature_type.value if feature_type else "all", "top_users": top_users}
    except Exception as e:
