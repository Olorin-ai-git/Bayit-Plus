...
export function EffectiveControlsGrid({ effectiveControls, isLoading }: EffectiveControlsGridProps) {
    ...
    <View style={styles.section}>
      ...
      <View style={styles.controlsGrid}>
        <View style={styles.controlCard}>
          ...
          {effectiveControls.kids_enabled && (
            <Text style={styles.controlCardDetail}>
              {t('profileControls.effectiveControls.ageLimit', 'Age limit: {{age}}', { age: effectiveControls.kids_age_limit })}
            </Text>
          )}
        </View>
        ...
        <View style={styles.controlCard}>
          ...
          {effectiveControls.youngsters_enabled && (
            <Text style={styles.controlCardDetail}>
              {t('profileControls.effectiveControls.ageLimit', 'Age limit: {{age}}', { age: effectiveControls.youngsters_age_limit })}
            </Text>
          )}
        </View>
...
const styles = StyleSheet.create({
  ...
  controlsGrid: { gap: 20 },
  controlCard: { backgroundColor: 'rgba(255, 255, 255, 0.05)', borderRadius: 20, padding: 32 },
  controlCardLabel: { fontSize: 22, color: '#d1d5db', marginBottom: 8 },
  controlCardValue: { fontSize: 28, fontWeight: '600', color: '#ffffff' },
  controlCardDetail: { fontSize: 20, color: '#9ca3af', marginTop: 8 },
  noControlsText: { color: '#d1d5db', textAlign: 'center', fontSize: 24 },
  loadingContainer: { padding: 64, alignItems: 'center', justifyContent: 'center' },
});
