...
def get_firebase_secret(secret_name: str) -> Optional[str]:
    ...
    try:
        ...
        if env_var_value:
            ...
            logger.info(
                f"Successfully retrieved secret '{secret_name}' from environment variables"
            )
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if secret_value:
                ...
                logger.info(
                    f"Successfully retrieved latest version of secret '{secret_name}' from Firebase"
                )
                return secret_value
            else:
                logger.error(
                    f"Firebase returned empty value for secret '{secret_name}'"
                )
        else:
            logger.error(
                f"Firebase CLI failed to retrieve secret '{secret_name}': {result.stderr.strip()}"
            )
        ...
        logger.error(
            "Firebase CLI not found in PATH - install with: npm install -g firebase-tools"
        )
    except Exception as e:
...
def get_app_secret(secret_name: str) -> Optional[str]:
    ...
    if result is None:
        logger.error(
            f"Failed to retrieve secret '{secret_name}' from Firebase - no fallbacks allowed"
        )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if secret_value:
            logger.info(
                f"✓ Secret '{secret_name}' retrieved successfully from Firebase"
            )
