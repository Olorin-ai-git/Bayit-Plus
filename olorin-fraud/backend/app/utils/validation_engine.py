...
class ValidationEngine:
    ...
    def validate_entity(
        ...
        try:
            ...
            is_valid, error_msg = self.comprehensive_validator.validate_entity(
            ...
            result.add_error(
                f"Validation engine error: {str(e)}", ValidationSeverity.CRITICAL
            )
            return result
    ...
    def validate_entity_batch(
        ...
        for entity_type, fields in entity_data.items():
            for field_name, value in fields.items():
                ...
                results[key] = self.validate_entity(
                    entity_type, field_name, value, context
                )
        ...
        for entity_type, fields in entity_mapping.items():
            for field_name, value in fields.items():
                ...
                result = self.validate_entity(
                    entity_type, field_name, value, transaction_data
                )
    ...
    def _apply_specialized_validation(
        self, result: ValidationResult, context: Optional[Dict[str, Any]]
    ):
    ...
    def _apply_financial_validation(
        self, result: ValidationResult, context: Optional[Dict[str, Any]]
    ):
        ...
        if entity_type in [EntityType.AMOUNT, EntityType.TX_AMOUNT]:
            ...
            is_valid, error_msg = self.financial_validator.validate_currency_amount(
                value, currency
            )
            if not is_valid:
            ...
            is_valid, error_msg = self.financial_validator.validate_payment_method(
                str(value), payment_data
            )
            if not is_valid:
        ...
        if context:
            try:
                is_safe, risk_msg, risk_analysis = (
                    self.financial_validator.validate_transaction_risk(context)
                )
    ...
    def _apply_security_validation(
        ...
        is_safe, error_msg, security_analysis = (
            self.security_validator.validate_input_security(str(value), field_name)
        )
        result.security_analysis = security_analysis

        if not is_safe:
            result.add_error(
                f"Security validation failed: {error_msg}", ValidationSeverity.CRITICAL
            )
        ...
        if context:
            try:
                is_legit, fraud_msg, fraud_analysis = (
                    self.security_validator.validate_fraud_indicators(context)
                )
    ...
    def _apply_geographic_validation(
        self, result: ValidationResult, context: Optional[Dict[str, Any]]
    ):
        ...
        if entity_type == EntityType.COUNTRY_CODE:
            is_valid, error_msg, geo_analysis = (
                self.geographic_validator.validate_country_code(str(value))
            )
            ...
            if lat is not None and lon is not None:
                is_valid, error_msg, coord_analysis = (
                    self.geographic_validator.validate_coordinates(
                        float(lat), float(lon)
                    )
                )
    ...
    def _apply_temporal_validation(
        ...
        if (
            "timestamp" in entity_type.value.lower()
            or "time" in entity_type.value.lower()
        ):
            is_valid, error_msg, temporal_analysis = (
                self.temporal_validator.validate_timestamp(value)
            )
            ...
            if context:
                try:
                    is_business, business_msg, business_analysis = (
                        self.temporal_validator.validate_business_hours(value)
                    )
    ...
    def _apply_network_validation(
        ...
        if entity_type in [EntityType.IP, EntityType.CLIENT_IP]:
            is_valid, error_msg, network_analysis = (
                self.network_validator.validate_ip_address(str(value))
            )
            ...
            is_valid, error_msg, ua_analysis = (
                self.network_validator.validate_user_agent(str(value))
            )
            result.network_analysis = ua_analysis
            if not is_valid:
                result.add_warning(f"User agent analysis: {error_msg}")

    def _apply_cross_entity_validation(
        self,
    ...
    def _map_transaction_to_entities(
        self, transaction_data: Dict[str, Any]
    ...
    def _add_transaction_analysis(
        self, result: ValidationResult, transaction_data: Dict[str, Any]
    ):
