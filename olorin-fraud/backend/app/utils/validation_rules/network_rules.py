...
class NetworkValidationRules:
    ...
    RESERVED_RANGES = {
        ...
        "multicast": ["224.0.0.0/4", "ff00::/8"],  # IPv4 multicast  # IPv6 multicast
    ...
    DOMAIN_PATTERNS = {
        ...
        "suspicious_tld": re.compile(
            r"\.(tk|ml|ga|cf|click|download|loan|racing|review|trade|accountant|cricket|date|faith|party|science|stream|top|webcam|win|bid)$",
            re.IGNORECASE,
        ),
    ...
    def validate_network_consistency(
        ...
        if not is_consistent:
            error_message = (
                f"Network inconsistencies detected: {', '.join(inconsistencies[:2])}"
            )
    ...
    def _analyze_ip_address(
        ...
        return {
            ...
            "is_global": (
                ...
                else not (ip_obj.is_private or ip_obj.is_reserved)
            ),
            "network_class": (
                self._get_ip_class(ip_obj) if ip_obj.version == 4 else None
            ),
        }
    ...
    def _analyze_user_agent(self, user_agent: str) -> Dict[str, Any]:
        ...
        for os_name, pattern in self.USER_AGENT_PATTERNS["os_patterns"].items():
            ...
            if match:
                ...
                analysis["os_info"]["version"] = (
                    match.group(1) if match.group(1) else "unknown"
                )
                break
    ...
    def _is_suspicious_ip(self, ip: str) -> bool:
        ...
        try:
            ...
            for network_range in self.SUSPICIOUS_NETWORKS.get(
    ...
    def _check_header_consistency(
        self, headers: Dict[str, str], user_agent: str
    ) -> Dict[str, Any]:
    ...
    def _analyze_device_fingerprint(self, fingerprint: str) -> Dict[str, Any]:
        ...
        return {"is_suspicious": len(issues) > 0, "issues": issues}
