...
SENSITIVE_PATTERNS = {
    ...
    "jwt_token": re.compile(
        r"\beyJ[A-Za-z0-9+/=]+\.[A-Za-z0-9+/=]+\.[A-Za-z0-9+/=]*\b"
    ),
...
def sanitize_tool_result(result: Any, max_length: int = 200) -> Optional[str]:
    ...
    redaction_ratio = (
        len(result_str) - len(sanitized.replace("[REDACTED]", ""))
    ) / len(result_str)
    ...
    for key, value in event_data.items():
        if key in ["result", "response", "output"]:
            ...
            if isinstance(value, dict):
                sanitized_data[key] = {
                    k: sanitize_text(str(v)) if isinstance(v, str) else "[REDACTED]"
