...
class ActionExecutor:
    ...
    def __init__(
        ...
        self.oauth_manager = oauth_manager or OAuthManager(
            composio_client=self.composio_client, encryption=self.encryption
        )
    ...
    def execute_action(
        ...
        try:
            ...
            if connection.is_expired() and connection.refresh_token:
                logger.info(
                    f"Token expired for connection {connection_id}, refreshing..."
                )
                try:
                    connection = self.oauth_manager.refresh_connection_token(
                        connection_id, tenant_id
                    )
                except Exception as e:
                    logger.error(f"Failed to refresh token: {e}")
                    raise ComposioTokenExpiredError(
            ...
            try:
                ...
                composio_action_duration.labels(
                    toolkit=toolkit, action=action, tenant_id=tenant_id
    ...
    def _get_connection(self, connection_id: str, tenant_id: str) -> ComposioConnection:
        """
    ...
    def _update_last_used(self, connection_id: str, tenant_id: str) -> None:
