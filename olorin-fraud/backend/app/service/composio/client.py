...
class ComposioClient:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        try:
            ...
            raise ComposioConfigurationError(
                f"Failed to initialize Composio client: {e}"
            ) from e
    ...
    def get_oauth_url(
        ...
        try:
            ...
            auth_config = AuthConfig(
                redirect_uri=redirect_uri, scopes=scopes or [], state=state
            )

            # Create connection config
            connection = Connection(entity=toolkit)

            result = self._sdk_client.connected_accounts.create(
                auth_config=auth_config, connection=connection
            )
            ...
            logger.info(
                f"Generated OAuth URL for toolkit {toolkit}, tenant {tenant_id}"
            )
            return oauth_url
    ...
    def exchange_code_for_token(
        self, toolkit: str, code: str, redirect_uri: str, tenant_id: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            auth_config = AuthConfig(redirect_uri=redirect_uri, code=code)
            ...
            access_token = getattr(result, "access_token", None) or result.get(
                "access_token"
            )
            refresh_token = getattr(result, "refresh_token", None) or result.get(
                "refresh_token"
            )
            expires_in = getattr(result, "expires_in", None) or result.get(
                "expires_in"
            )  # seconds

            if not connection_id:
                # Try alternative field names
                connection_id = getattr(result, "connection_id", None) or result.get(
                    "connection_id"
                )

            if not connection_id:
                raise ComposioError(
                    "Invalid token exchange response: missing connection_id"
                )
            ...
            logger.info(
                f"Exchanged code for token, toolkit {toolkit}, tenant {tenant_id}, connection {connection_id}"
            )
    ...
    def execute_action(
        ...
        try:
            ...
            if toolkit.lower() == "veriphone":
                ...
                veriphone_account_id = config_loader.load_secret(
                    "COMPOSIO_VERIPHONE_ACCOUNT_ID"
                ) or os.getenv("COMPOSIO_VERIPHONE_ACCOUNT_ID")
                veriphone_api_key = config_loader.load_secret(
                    "COMPOSIO_VERIPHONE_API_KEY"
                ) or os.getenv("COMPOSIO_VERIPHONE_API_KEY")
                veriphone_user_id = config_loader.load_secret(
                    "COMPOSIO_VERIPHONE_USER_ID"
                ) or os.getenv("COMPOSIO_VERIPHONE_USER_ID")
    ...
    def refresh_token(
        self, toolkit: str, connection_id: str, refresh_token: str, tenant_id: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            new_access_token = getattr(result, "access_token", None) or result.get(
                "access_token"
            )
            new_refresh_token = getattr(result, "refresh_token", None) or result.get(
                "refresh_token"
            )
            ...
            return {
                ...
                "refresh_token": (
                    str(new_refresh_token) if new_refresh_token else refresh_token
                ),
                "expires_at": expires_at.isoformat() if expires_at else None,
                "expires_in": expires_in,
            }
