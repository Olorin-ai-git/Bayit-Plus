...
class APICircuitBreaker:
    ...
    async def call(self, func: Callable, *args, **kwargs) -> Any:
        ...
        if not self._can_execute():
            ...
            raise APICircuitBreakerError(
                f"Circuit breaker '{self.name}' is OPEN - calls not allowed", stats
            )
    ...
    def _should_open_circuit(self) -> bool:
        ...
        if (
            self.total_requests >= self.config.minimum_requests
            and self.slow_requests > 0
        ):
...
class CircuitBreakerRegistry:
    ...
    def get_all_stats(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            name: breaker.get_stats_dict() for name, breaker in self.breakers.items()
        }
