...
def get_database_password(settings: SvcSettings) -> str:
    ...
    logger.info(
        f"⚠️  Database password not found in .env, attempting Firebase Secrets Manager fallback..."
    )
    try:
        ...
        logger.warning(
            f"⚠️  Failed to retrieve database password from Firebase Secrets Manager: {e}"
        )
        logger.warning(
            "   This is expected if Firebase CLI is not installed or not authenticated"
        )
...
def get_jwt_secret_key(settings: SvcSettings) -> str:
    ...
    if len(jwt_secret) < 32:
        logger.warning(
            f"JWT secret key is only {len(jwt_secret)} characters - recommend minimum 32 characters"
        )
...
def get_api_key(settings: SvcSettings, api_name: str) -> Optional[str]:
    ...
    if not api_key:
        ...
        try:
            ...
            if api_key:
                logger.debug(
                    f"Retrieved {api_name} API key from Firebase Secrets Manager"
                )
        except Exception as e:
...
def build_database_url(
    ...
    database = (
        db_name or os.getenv("POSTGRES_DATABASE") or os.getenv("DB_NAME", "olorin")
    )
        ...
        try:
            ...
            if api_key and len(api_key) < 16:
                validation_errors.append(
                    f"{api_name} API key is too short (< 16 characters)"
                )
        except Exception as e:
