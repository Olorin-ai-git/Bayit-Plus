...
async def orchestrator_node(
    state: Dict[str, Any], config: Optional[Dict] = None
) -> Dict[str, Any]:
    ...
    logger.debug(
        f"Orchestrator database config: DATABASE_PROVIDER={database_provider}, USE_POSTGRES={use_postgres}"
    )
    ...
    if database_provider == "snowflake":
        logger.info(
            "✅ DATABASE_PROVIDER=snowflake - Skipping DatabaseQueryTool for orchestrator, will use SnowflakeQueryTool instead"
        )
        ...
        if not has_database_query:
            ...
            logger.debug(
                f"Orchestrator PostgreSQL env vars: host={bool(postgres_host)}, port={postgres_port}, db={bool(postgres_database)}, user={bool(postgres_user)}, password={'***' if postgres_password else None}"
            )
            ...
            if (
                ...
                logger.info(
                    f"✅ Built PostgreSQL connection string for orchestrator from .env (host={postgres_host}, db={postgres_database}, user={postgres_user})"
                )
            else:
                missing = []
                ...
                logger.warning(
                    f"⚠️ PostgreSQL config incomplete for orchestrator. Missing from .env: {', '.join(missing)}"
                )
            ...
            if not database_connection_string:
                ...
                if database_connection_string:
                    logger.info(
                        "✅ Using DATABASE_URL/POSTGRES_URL from .env for orchestrator"
                    )
            ...
            if database_connection_string:
                try:
                    ...
                    logger.info(
                        "✅ Added DatabaseQueryTool to orchestrator (PostgreSQL only)"
                    )
                except Exception as e:
                    logger.warning(
                        f"Could not add DatabaseQueryTool to orchestrator: {e}"
                    )
            else:
                logger.warning(
                    "⚠️ DatabaseQueryTool not available for orchestrator: No PostgreSQL connection string found"
                )

    # Verify database_query tool is available
    if not any(t.name == "database_query" for t in tools):
        logger.error(
            "❌ CRITICAL: database_query tool is NOT available for orchestrator! LLM cannot query database!"
        )
    else:
        logger.info(
            f"✅ database_query tool is available for orchestrator (total {len(tools)} tools)"
        )
