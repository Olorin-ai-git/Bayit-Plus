    ...
    try:
        ...
        if isinstance(snowflake_data, dict):
            ...
            if "row_count" in snowflake_data:
                logger.info(
                    f"   snowflake_data['row_count']: {snowflake_data.get('row_count')}"
                )
        ...
        if results:
            ...
            if results and isinstance(results[0], dict):
                ...
                logger.info(
                    f"   Record keys: {list(results[0].keys())[:20]}"
                )  # Show first 20 keys
                logger.info(
                    f"   Sample values: {dict(list(results[0].items())[:5])}"
            ...
            from app.service.agent.orchestration.domain.field_whitelist import (
                assert_no_cross_domain_pollution,
                filter_domain_fields,
            )
            ...
            if len(network_findings.get("evidence", [])) == 0:
                logger.warning(
                    "⚠️ No evidence collected from analysis functions, adding basic transaction count"
                )
                network_findings["evidence"].append(
                    f"Transaction data analyzed: {len(results)} records processed"
                )
                network_findings["evidence"].append(
                    "Network analysis completed on transaction dataset"
                )

            # Log evidence collected from Snowflake data
            logger.info(
                f"📊 Evidence collected from Snowflake data: {len(network_findings.get('evidence', []))} items"
            )
            if network_findings.get("evidence"):
                for i, ev in enumerate(
                    network_findings["evidence"][:5]
                ):  # Show first 5
            ...
            if isinstance(snowflake_data, str):
                network_findings["risk_indicators"].append(
        ...
        if total_evidence == 0:
            logger.warning(
                f"⚠️ Network agent: No evidence collected! Snowflake data had {snowflake_data.get('row_count', len(snowflake_data.get('results', [])))} rows but evidence list is empty"
            )
            ...
            if col in r:
                ...
                if (isinstance(value, bool) and value) or (
                    ...
                    findings["risk_indicators"].append(
                        f"VPN/Proxy detected ({col}: {value})"
                    )
                    ...
                    findings["evidence"].append(
                        f"Network traffic through VPN/Proxy service: {col}={value}"
                    )
                    break
...
def _analyze_geographic_patterns(results: list, findings: Dict[str, Any]) -> None:
    ...
    for r in results:
        country = (
            r.get("IP_COUNTRY_CODE")
            or r.get("ip_country_code")
            or r.get("IP_COUNTRY")
            or r.get("ip_country")
    ...
    logger.debug(
        f"🌍 Geographic analysis: Found {len(countries)} unique countries: {list(countries)[:10]}"
    )

    # Always add evidence about country count, even if it's 0 or 1
    if len(countries) > 0:
        findings["evidence"].append(
            f"Geographic analysis: {len(countries)} unique country/countries found: {list(countries)}"
        )

    if len(countries) > 3:
        findings["risk_indicators"].append(
            f"Activity from {len(countries)} different countries"
        )
        # CRITICAL FIX: Only modify risk_score if LLM hasn't set it yet AND risk_score is not None
        if "llm_risk_score" not in findings:
            current_score = findings.get("risk_score")
        ...
        findings["evidence"].append(
            f"Cross-border activity across {len(countries)} countries: {list(countries)}"
        )
    elif len(countries) > 1:
        findings["risk_indicators"].append(
            f"Cross-border activity ({len(countries)} countries)"
        )
        # CRITICAL FIX: Only modify risk_score if LLM hasn't set it yet AND risk_score is not None
        if "llm_risk_score" not in findings:
            current_score = findings.get("risk_score")
...
def _analyze_ip_diversity(results: list, findings: Dict[str, Any]) -> None:
    ...
    if len(ips) > 0:
        findings["evidence"].append(
            f"IP analysis: {len(ips)} unique IP address(es) found in transaction data"
        )
    ...
    if len(ips) > 10:
        ...
        findings["evidence"].append(
            f"Suspicious IP diversity pattern: {len(ips)} unique addresses"
        )
    elif len(ips) > 5:
        findings["evidence"].append(
            f"Moderate IP diversity: {len(ips)} unique addresses"
        )
...
def _analyze_threat_intelligence(
    ...
    logger.debug(
        f"[Step 5.2.1.2] 🔍 Category-based threat analysis: Processing {len(tool_results)} tools"
    )
    ...
    for tool_name, result in tool_results.items():
        ...
        if threat_signals:
            logger.debug(
                f"[Step 5.2.1.2]   ✅ {tool_name}: Found {len(threat_signals)} threat signals"
            )
            _process_threat_signals(tool_name, threat_signals, findings)
        else:
            logger.debug(
                f"[Step 5.2.1.2]   ➖ {tool_name}: No network threat signals detected"
            )
...
def _extract_threat_signals(tool_name: str, result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.debug(
        f"[Step 5.2.1.2] 🔍 Extracting threat signals from {tool_name} with {len(result)} top-level fields"
    )
    ...
    for indicator in threat_indicators:
        if indicator in result:
            ...
            logger.debug(
                f"[Step 5.2.1.2]     → Found threat indicator: {indicator} = {result[indicator]}"
            )
    ...
    for indicator in score_indicators:
        if indicator in result:
            try:
                ...
                logger.debug(
                    f"[Step 5.2.1.2]     → Found score indicator: {indicator} = {result[indicator]}"
                )
            except (ValueError, TypeError):
                logger.debug(
                    f"[Step 5.2.1.2]     → Skipped non-numeric score: {indicator} = {result[indicator]}"
                )
                pass
        ...
        if isinstance(value, dict):
            ...
            for i, item in enumerate(value[:5]):  # Limit to first 5 items
                if isinstance(item, dict):
                    nested_signals = _extract_threat_signals(
    ...
    logger.debug(
        f"[Step 5.2.1.2] ✅ Extracted {len(signals)} threat signals from {tool_name}"
    )
    return signals


def _process_threat_signals(
    tool_name: str, signals: Dict[str, Any], findings: Dict[str, Any]
) -> None:
    """Process extracted threat signals to adjust risk score."""

    logger.debug(
        f"[Step 5.2.1.3] 🔍 Processing {len(signals)} threat signals from {tool_name}"
    )
    ...
    for key, value in signals.items():
        if key.startswith("score_") and isinstance(value, (int, float)):
            ...
            if normalized_score > 0.5:  # High threat
                ...
                findings["evidence"].append(
                    f"{tool_name}: Clean reputation {key} = {value}"
                )
    ...
    if "llm_risk_score" in findings:
        ...
        logger.debug(
            f"[Step 5.2.1.3]   ℹ️ LLM risk score already set ({findings.get('llm_risk_score', 'N/A')}), skipping threat-level adjustments"
        )
        # Still add threat indicators to evidence, but don't modify score
        if threat_level > 0.5:
            findings["risk_indicators"].append(
                f"{tool_name}: High threat detected (level: {threat_level:.2f})"
            )
        elif threat_level < -0.3:
            findings["evidence"].append(
                f"{tool_name}: Network appears clean (level: {threat_level:.2f})"
            )
    else:
        ...
        if current_score is not None:
            if threat_level > 0.5:
                ...
                risk_multiplier = 1.0 + max(
                    -0.15, threat_level * 0.2
                )  # threat_level is negative
    ...
    if evidence_count > 0:
        ...
        logger.debug(
            f"[Step 5.2.1.3]   ✅ {tool_name}: Processed {evidence_count} threat signals, threat level: {threat_level:.2f}"
        )
    else:
        logger.debug(
            f"[Step 5.2.1.3]   ➖ {tool_name}: No actionable threat signals found"
        )
