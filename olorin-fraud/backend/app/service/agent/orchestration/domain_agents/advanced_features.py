...
VELOCITY_WINDOW_MINUTES = int(os.getenv("FEATURE_VELOCITY_WINDOW_MINUTES", "5"))
AMOUNT_THRESHOLD_BINS = json.loads(
    os.getenv("FEATURE_AMOUNT_THRESHOLD_BINS", "[[998, 1000], [499, 501], [99, 101]]")
)
MAX_FEASIBLE_SPEED_MPH = float(os.getenv("FEATURE_MAX_FEASIBLE_SPEED_MPH", "600"))
EARTH_RADIUS_KM = 6371.0  # Physical constant


def calculate_entity_scoped_velocity(
    transactions: List[Dict[str, Any]], entity_type: str, entity_value: str
) -> Dict[str, float]:
    """
            ...
            parsed_txs.append(
                {
                    ...
                    "ip_address": tx.get("IP")
                    or tx.get("ip")
                    or tx.get("IP_ADDRESS")
                    or tx.get("ip_address"),
    ...
    velocity_metrics = {
        "tx_per_5min_by_email": _calculate_velocity_by_entity(
            parsed_txs, "email", VELOCITY_WINDOW_MINUTES
        ),
        "tx_per_5min_by_device": _calculate_velocity_by_entity(
            parsed_txs, "device_id", VELOCITY_WINDOW_MINUTES
        ),
        "tx_per_5min_by_ip": _calculate_velocity_by_entity(
            parsed_txs, "ip_address", VELOCITY_WINDOW_MINUTES
        ),
        "merchant_local_velocity": _calculate_merchant_local_velocity(
            parsed_txs, VELOCITY_WINDOW_MINUTES
        ),
    }
...
def _calculate_velocity_by_entity(
    transactions: List[Dict[str, Any]], entity_key: str, window_minutes: int
) -> float:
...
def _calculate_merchant_local_velocity(
    transactions: List[Dict[str, Any]], window_minutes: int
) -> float:
...
def calculate_geovelocity_features(
    ...
    for i in range(len(location_data) - 1):
        ...
        if time_diff > 0:
            speed_mph = (
                distance_km / 1.60934
            ) / time_diff  # Convert km to miles, divide by hours
    ...
    distance_anomaly_score = (
        min(1.0, max(0.0, (max_speed - 100) / (MAX_FEASIBLE_SPEED_MPH - 100)))
        if max_speed > 100
        else 0.0
    )
...
def calculate_amount_patterns(transactions: List[Dict[str, Any]]) -> Dict[str, float]:
    """
    ...
    for tx in transactions:
        try:
            ...
            amount_txs.append({"amount": amount_float, "datetime": tx_datetime})
        except Exception as e:
    ...
    for amount_bin, txs in amount_groups.items():
        ...
        for i in range(len(txs) - 1):
            time_diff = (
                txs[i + 1]["datetime"] - txs[i]["datetime"]
            ).total_seconds() / 60
...
def calculate_device_ip_stability(
    ...
    if transactions:
        latest_tx = max(
            (
                tx.get("TX_DATETIME") or tx.get("tx_datetime")
                for tx in transactions
                if tx.get("TX_DATETIME") or tx.get("tx_datetime")
            ),
            default=None,
        )
        if latest_tx:
    ...
    # Calculate metrics
    devices_per_email_avg = (
        sum(len(devices) for devices in email_devices.values()) / len(email_devices)
        if email_devices
        else 0.0
    )
    ips_per_device_avg = (
        sum(len(ips) for ips in device_ips.values()) / len(device_ips)
        if device_ips
        else 0.0
    )
    user_agents_per_device_avg = (
        sum(len(uas) for uas in device_user_agents.values()) / len(device_user_agents)
        if device_user_agents
        else 0.0
    )
...
def calculate_merchant_consistency(
    ...
    if merchant_counts:
        ...
        single_merchant_concentration = (
            max_merchant_count / total_txs if total_txs > 0 else 0.0
        )
    else:
        single_merchant_concentration = 0.0
    ...
    diversity_score = (
        min(1.0, unique_merchants / max(total_txs, 1)) if total_txs > 0 else 0.0
    )
...
def extract_all_advanced_features(
    ...
    velocity_features = calculate_entity_scoped_velocity(
        transactions, entity_type, entity_value
    )
    ...
    try:
        if transactions:
            ...
            enhanced_velocity = velocity_analyzer.analyze_transaction_velocity(
                current_tx, historical_txs
            )

            if enhanced_velocity.get("success"):
                features["enhanced_velocity"] = enhanced_velocity
                features["sliding_window_velocities"] = enhanced_velocity.get(
                    "sliding_windows", {}
                )
                features["entity_velocities_enhanced"] = enhanced_velocity.get(
                    "entity_velocities", {}
                )
                features["merchant_concentration"] = enhanced_velocity.get(
                    "merchant_concentration", {}
                )
                features["cross_entity_correlation"] = enhanced_velocity.get(
