...
class DomainAgentBase:
    ...
    @staticmethod
    def log_agent_start(
        domain: str, entity_type: str, entity_id: str, is_test_mode: bool
    ) -> None:
        ...
        logger.debug(
            f"[Step {step}]   🤝 Agent handover: Orchestrator → {domain.title()} Agent"
        )
    ...
    def log_context_analysis(
        ...
        if snowflake_data:
            if isinstance(snowflake_data, dict) and "results" in snowflake_data:
                ...
                logger.debug(
                    f"[Step {step}.1]   Snowflake data: Yes ({len(results)} records)"
                )
            elif isinstance(snowflake_data, dict) and "row_count" in snowflake_data:
                logger.debug(
                    f"[Step {step}.1]   Snowflake data: Yes ({snowflake_data.get('row_count', 0)} rows)"
                )
            else:
        ...
        logger.debug(
            f"[Step {step}.1]   Tool results: {len(tool_results)} tools for category-based analysis"
        )
        if tool_results:
            logger.debug(
                f"[Step {step}.1]   Tool results keys: {list(tool_results.keys())}"
            )
            logger.debug(
                f"[Step {step}.1]   🔄 Category-based processing: Will extract domain-specific signals from all tools"
            )

    @staticmethod
    def start_chain_of_thought(
        ...
        cot_logger.log_reasoning_step(
            ...
            supporting_evidence=[
                {"type": "domain", "data": f"{domain} analysis specialization"},
                {
                    ...
                    "data": f"{agent_name} ready for analysis",
                },
            ],
            metadata={
                "agent": agent_name,
    ...
    def process_snowflake_results(snowflake_data: Dict[str, Any], domain: str) -> list:
        ...
        if isinstance(snowflake_data, dict):
            if "results" in snowflake_data:
                ...
                logger.debug(
                    f"[Step {step}]   📊 Processing {len(results)} Snowflake records for {domain} analysis"
                )
            else:
                # Dict but no "results" key - might be a different format, try to extract data
                step = DomainAgentBase._get_domain_step(domain)
                logger.info(
                    f"[Step {step}]   📊 Snowflake data is dict but no 'results' key. Keys: {list(snowflake_data.keys())}"
                )
                ...
                if "data" in snowflake_data:
                    ...
                    if isinstance(data_value, list):
                        results = data_value
                        logger.info(
                            f"[Step {step}]   ✅ Extracted {len(results)} records from 'data' key"
                        )
                    else:
                        logger.warning(
                            f"[Step {step}]   ⚠️ 'data' key exists but is not a list (type: {type(data_value).__name__})"
                        )
                        results = []
                    ...
                    if isinstance(rows_value, list):
                        results = rows_value
                        logger.info(
                            f"[Step {step}]   ✅ Extracted {len(results)} records from 'rows' key"
                        )
                    else:
                        logger.warning(
                            f"[Step {step}]   ⚠️ 'rows' key exists but is not a list (type: {type(rows_value).__name__})"
                        )
                        results = []
                else:
                    logger.warning(
                        f"[Step {step}]   ⚠️ No 'results', 'data', or 'rows' key found in snowflake_data"
                    )
                    results = []
        # Handle string type (JSON string that needs parsing)
            ...
            try:
                logger.debug(
                    f"[Step {step}]   📊 Parsing JSON string snowflake_data for {domain} analysis"
                )
                parsed_data = json.loads(snowflake_data)
                if isinstance(parsed_data, dict) and "results" in parsed_data:
                    results = parsed_data["results"]
                    logger.info(
                        f"[Step {step}]   ✅ Successfully parsed {len(results)} Snowflake records from JSON string"
                    )
                else:
                    logger.warning(
                        f"[Step {step}]   ⚠️ Parsed JSON but no 'results' key found. Keys: {list(parsed_data.keys()) if isinstance(parsed_data, dict) else 'not a dict'}"
                    )
            except (json.JSONDecodeError, Exception) as e:
                logger.error(
                    f"[Step {step}]   ❌ Failed to parse JSON snowflake_data: {str(e)}"
                )
                logger.debug(
                    f"[Step {step}]   String content length: {len(snowflake_data)} chars (preview: {snowflake_data[:50]}...)"
                )
        # Handle unexpected types (should not happen, but log for debugging)
            ...
            logger.debug(
                f"[Step {step}]   ℹ️ {domain.title()} Agent: Snowflake data type is {data_type} (expected dict or str). Attempting to extract results anyway."
            )
            # Try to extract results even from unexpected types
            if hasattr(snowflake_data, "get") and callable(
                getattr(snowflake_data, "get")
            ):
                # It's dict-like, try to get results
                results = snowflake_data.get("results", snowflake_data.get("data", []))
            elif hasattr(snowflake_data, "__iter__") and not isinstance(
                snowflake_data, str
            ):
                ...
                logger.warning(
                    f"[Step {step}] ⚠️ {domain.title()} Agent: Cannot extract results from Snowflake data type: {data_type}"
                )
                results = []
    ...
    def process_model_scores(
        results: list, findings: Dict[str, Any], domain: str
    ) -> None:
        """
        ...
        step = DomainAgentBase._get_domain_step(domain)
        logger.debug(
            f"[Step {step}]   📊 Processing {len(results)} records for {domain} anomaly detection"
        )
        ...
        for idx, r in enumerate(results[:3]):  # Log first 3 records
            ...
            logger.debug(
                f"[Step {step}]      Record {idx+1}: Raw data available (MODEL_SCORE present but ignored: {model_score is not None})"
            )
            logger.debug(
                f"[Step {step}]      Available fields for analysis: {list(r.keys())}"
            )
        ...
        findings["risk_score"] = (
            0.0  # Zero baseline - build risk score from evidence only
        )

        logger.debug(
            f"[Step {step}]   🎯 CORRECTED: Starting {domain} analysis from ZERO baseline (0.0)"
        )
        logger.debug(
            f"[Step {step}]   🚫 MODEL_SCORE completely ignored per system architecture"
        )
        logger.debug(
            f"[Step {step}]   🧠 LLM will analyze raw Snowflake patterns independently"
        )
    ...
    def finalize_findings(
        ...
        if not findings.get("_llm_confidence_set", False) and (
            "confidence" not in findings or findings.get("confidence") is None
        ):
            data_sources = sum(
                [
                    1 if snowflake_data else 0,
                    len(tool_results)
                    * 0.25,  # Each tool contributes 0.25 to confidence
                ]
            )
        ...
        if risk_score is not None:
            logger.info(
                f"[Step {step}] ✅ {domain.title()} analysis complete - Risk: {risk_score:.2f}"
            )
        else:
            logger.info(
                f"[Step {step}] ✅ {domain.title()} analysis complete - Risk: INSUFFICIENT_DATA"
            )
        ...
        if risk_score is not None:
            ...
            logger.debug(
                f"[Step {step}]   🎯 Risk score: INSUFFICIENT_DATA (LLM did not provide score)"
            )
        logger.debug(
            f"[Step {step}]   🔍 Risk indicators found: {len(findings['risk_indicators'])}"
        )
        ...
        if len(findings["risk_indicators"]) > 3:
            logger.debug(
                f"[Step {step}]      ... and {len(findings['risk_indicators']) - 3} more"
            )
        logger.debug(
            f"[Step {step}]   📊 Confidence level: {findings.get('confidence', 0):.2f}"
        )
        logger.debug(
            f"[Step {step}]   🧠 Evidence points collected: {len(findings.get('evidence', []))}"
        )
        logger.debug(
            f"[Step {step}]   🔄 Category-based signals processed from {_count_tool_metrics(findings)} tools"
        )
...
def log_agent_handover_complete(domain: str, findings: Dict[str, Any]) -> None:
    ...
    step = DomainAgentBase._get_domain_step(domain)
    logger.debug(
        f"[Step {step}]   🤝 Agent handover: {domain.title()} Agent → Orchestrator"
    )
    logger.debug(
        f"[Step {step}]   🎯 Findings delivered: {len(findings.get('risk_indicators', []))} indicators"
    )
    logger.debug(
        f"[Step {step}]   🧠 Chain of thought: Analysis complete, control returned to orchestrator"
    )
...
def _detect_confirmed_fraud(snowflake_data: Dict[str, Any]) -> bool:
    ...
    if isinstance(snowflake_data, str):
        try:
            ...
            logger.debug(
                "📊 _detect_confirmed_fraud: Parsed JSON string snowflake_data to dict"
            )
        except (json.JSONDecodeError, Exception) as e:
            logger.error(
                f"📊 _detect_confirmed_fraud: Failed to parse JSON snowflake_data: {str(e)}"
            )
            return False
...
def _compute_algorithmic_risk_score(
    domain: str, findings: Dict[str, Any], snowflake_data: Dict[str, Any]
) -> float:
    """
    ...
    if isinstance(snowflake_data, str):
        try:
            ...
            snowflake_data = json.loads(snowflake_data)
            logger.debug(
                f"📊 _compute_algorithmic_risk_score ({domain}): Parsed JSON string snowflake_data to dict"
            )
        except (json.JSONDecodeError, Exception) as e:
            logger.error(
                f"📊 _compute_algorithmic_risk_score ({domain}): Failed to parse JSON snowflake_data: {str(e)}"
            )
    ...
    from app.service.agent.orchestration.domain.network_scorer import (
        score_network_domain,
    )
    ...
    if domain == "logs":
        ...
        proxy_vpn = any(
            "vpn" in str(e).lower() or "proxy" in str(e).lower() for e in evidence
        )
        tor_detected = any("tor" in str(e).lower() for e in evidence)
        asn_risk = any(
            "high-risk" in str(e).lower() and "asn" in str(e).lower() for e in evidence
        )
        geo_anomaly = any(
            "geo" in str(e).lower()
            and ("anomaly" in str(e).lower() or "suspicious" in str(e).lower())
            for e in evidence
        )
        ...
        domain_result = score_network_domain(
            ti_hits, proxy_vpn, tor_detected, asn_risk, geo_anomaly, ip_str
        )
        ...
        fingerprint_anomaly = any(
            "fingerprint" in str(e).lower() and "anomaly" in str(e).lower()
            for e in evidence
        )
        ...
        risk_score = compute_device_risk(
            device_consistency,
            fingerprint_anomaly,
            browser_spoofing,
            device_velocity,
        ...
        location_consistency = not any(
            "inconsistent" in str(e).lower() for e in evidence
        )
        high_risk_country = any(
            "high-risk" in str(e).lower() and "country" in str(e).lower()
            for e in evidence
        )

        # CRITICAL: Pass None for model_score to prevent pollution
        risk_score = compute_location_risk(
            impossible_travel,
            travel_confidence,
        ...
        credential_stuffing = any(
            "credential stuffing" in str(e).lower() for e in evidence
        )
        session_anomaly = any(
            "session" in str(e).lower() and "anomaly" in str(e).lower()
            for e in evidence
        )

        # CRITICAL: Pass None for model_score to prevent pollution
        risk_score = compute_authentication_risk(
            failed_attempts,
            mfa_bypass,
            credential_stuffing,
            session_anomaly,
    ...
    if domain_result is not None:
        ...
        logger.debug(
            f"🎯 VALIDATED DOMAIN SCORER for {domain}: score={risk_score}, status={domain_result.status}, signals={len(domain_result.signals)}"
        )
    ...
    if risk_score is None:
        logger.debug(
            f"🧮 DETERMINISTIC RISK SCORE for {domain}: None (insufficient data - NO fallback score)"
        )
        return None

    logger.debug(
        f"🧮 DETERMINISTIC RISK SCORE for {domain}: {risk_score:.3f} (NO MODEL_SCORE used)"
    )
    return risk_score
...
async def analyze_evidence_with_llm(
    ...
    step = DomainAgentBase._get_domain_step(domain)
    logger.debug(
        f"[Step {step}.4] 🧠 LLM Evidence Analysis - Analyzing {len(findings.get('evidence', []))} evidence points"
    )

    # CRITICAL FIX: Ensure tool_results is at least an empty dict (NEVER None)
    if tool_results is None:
        logger.warning(
            f"[Step {step}.4] ⚠️ tool_results was None, converting to empty dict"
        )
        tool_results = {}

    logger.info(
        f"[Step {step}.4] 🔧 Tool Results Aggregation - {len(tool_results)} tool results available for LLM analysis"
    )
    ...
    if tool_results:
        ...
        logger.warning(
            f"[Step {step}.4] ⚠️ No tool results available - LLM will only see Snowflake data and manual evidence"
        )
    ...
    try:
        ...
        if llm_risk_score is not None and isinstance(llm_risk_score, (int, float)):
            ...
            if llm_risk_score > 1.0:
                ...
                llm_risk_score = min(
                    llm_risk_score / 100.0, 1.0
                )  # Cap at 1.0 for safety
                logger.info(
                    f"[Step {step}.4] 🔄 C2 FIX: Normalized LLM score from {original_score} (0-100 scale) to {llm_risk_score:.3f} (0-1 scale)"
                )
                ...
                llm_risk_score = 0.0
                logger.warning(
                    f"[Step {step}.4] ⚠️ C2 FIX: Clipped negative LLM score from {original_score} to 0.0"
                )
                llm_analysis["risk_score"] = llm_risk_score

        # Check if LLM analysis indicates insufficient data
        if (
            llm_analysis.get("risk_score_status") == "INSUFFICIENT_DATA"
            or llm_analysis.get("status") == "INSUFFICIENT_DATA"
        ):
            ...
            logger.warning(
                f"[Step {step}.4] ⚠️ INSUFFICIENT_DATA: LLM did not provide risk score for {domain} domain. Marking as insufficient data instead of using fallback."
            )
        elif (
            llm_risk_score is not None
            and isinstance(llm_risk_score, (int, float))
            and 0.0 <= llm_risk_score <= 1.0
        ):
            ...
            if llm_confidence is not None:
                ...
                findings["_llm_confidence_set"] = (
                    True  # Still mark as set to prevent fallback overwrite
                )

            findings["llm_risk_score"] = (
                llm_risk_score  # Store original LLM score for reference
            )
            logger.info(
                f"[Step {step}.4] ✅ Using LLM-determined risk score: {llm_risk_score:.3f} (stored directly, no modifications)"
            )
        else:
            ...
            findings["llm_risk_score"] = None
            logger.warning(
                f"[Step {step}.4] ⚠️ INSUFFICIENT_DATA: LLM risk score invalid ({llm_risk_score}) for {domain} domain. Marking as insufficient data instead of using algorithmic fallback."
            )

        # Clean and deduplicate all text content before storing
        from app.service.text.clean import (
        ...
        if has_confirmed_fraud and "recommendations" in llm_analysis:
            if isinstance(llm_analysis["recommendations"], list):
                ...
                guarded_list = deduplicate_recommendations(
                    rec_list, has_confirmed_fraud=True
                )
                llm_analysis["recommendations"] = (
                    guarded_list[0] if guarded_list else llm_analysis["recommendations"]
                )
        ...
        if risk_score is not None:
            logger.debug(
                f"[Step {step}.4] ✅ LLM analysis complete - Risk: {risk_score:.2f}, Confidence: {confidence:.2f}"
            )
        else:
            logger.debug(
                f"[Step {step}.4] ⚠️ LLM analysis complete - Insufficient data (no risk score)"
            )
        ...
        try:
            computed_risk_score = _compute_algorithmic_risk_score(
                domain, findings, snowflake_data
            )

            # Validate algorithmic score
            if (
                computed_risk_score is not None
                and isinstance(computed_risk_score, (int, float))
                and 0.0 <= computed_risk_score <= 1.0
            ):
                findings["risk_score"] = computed_risk_score
                findings["confidence"] = 0.3  # Lower confidence due to LLM failure
                findings["evidence"].append(
                    f"LLM analysis failed, using algorithmic fallback: {str(e)}"
                )
                findings["risk_indicators"].append(
                    "LLM evidence analysis unavailable - algorithmic score used"
                )
                logger.warning(
                    f"[Step {step}.4] 🔄 LLM completely failed, using algorithmic fallback: {computed_risk_score:.3f}"
                )
                return findings
            else:
...
def complete_chain_of_thought(
    process_id: str, findings: Dict[str, Any], domain: str
) -> None:
    ...
    risk_score = findings.get("risk_score")
    risk_score_str = (
        f"{risk_score:.2f}" if risk_score is not None else "INSUFFICIENT_DATA"
    )
    ...
    cot_logger.complete_agent_thinking(process_id=process_id, final_assessment=findings)
