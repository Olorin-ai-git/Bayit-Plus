...
REPEATED_AMOUNT_THRESHOLD_PCT = float(
    os.getenv("PATTERN_REPEATED_AMOUNT_THRESHOLD_PCT", "0.5")
)
REPEATED_AMOUNT_PENALTY = float(os.getenv("PATTERN_REPEATED_AMOUNT_PENALTY", "0.15"))

VELOCITY_BURST_THRESHOLD_MIN = float(
    os.getenv("PATTERN_VELOCITY_BURST_THRESHOLD_MIN", "5.0")
)
VELOCITY_BURST_PENALTY = float(os.getenv("PATTERN_VELOCITY_BURST_PENALTY", "0.10"))

IP_ROTATION_THRESHOLD = int(os.getenv("PATTERN_IP_ROTATION_THRESHOLD", "3"))
IP_ROTATION_PENALTY = float(os.getenv("PATTERN_IP_ROTATION_PENALTY", "0.10"))

APPROVAL_REJECTION_THRESHOLD_PCT = float(
    os.getenv("PATTERN_APPROVAL_REJECTION_THRESHOLD_PCT", "0.25")
)
APPROVAL_REJECTION_PENALTY = float(
    os.getenv("PATTERN_APPROVAL_REJECTION_PENALTY", "0.05")
)
...
def _calculate_velocity_burst_penalty(transactions: List[Dict[str, Any]]) -> float:
    ...
    for i in range(len(timestamped_txs) - 1):
        time_diff_minutes = (
            timestamped_txs[i + 1]["datetime"] - timestamped_txs[i]["datetime"]
        ).total_seconds() / 60.0
...
def _calculate_ip_rotation_penalty(transactions: List[Dict[str, Any]]) -> float:
    ...
    for tx in transactions:
        ...
        ip_address = (
            tx.get("IP") or tx.get("ip") or tx.get("IP_ADDRESS") or tx.get("ip_address")
        )
