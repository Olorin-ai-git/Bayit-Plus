    ...
    try:
        ...
        if results:
            ...
            if isinstance(snowflake_data, str):
                device_findings["risk_indicators"].append(
        ...
        logger.info(
            f"[Step 5.2.2] ✅ Device analysis complete - Evidence collected: {len(device_findings['evidence'])} points"
        )
...
def _analyze_device_id_patterns(results: list, findings: Dict[str, Any]) -> None:
    ...
    device_ids = set(
        r.get("DEVICE_ID") or r.get("device_id")
        for r in results
        if r.get("DEVICE_ID") or r.get("device_id")
    )
    ...
    if not device_ids and results:
        ...
        findings["evidence"].append(
            f"Device ID data not available (NULL values) across {len(results)} transactions"
        )
        findings["evidence"].append(
            "LIMITATION: Cannot analyze device patterns without device ID data"
        )
        ...
        findings["metrics"]["total_transactions"] = len(results)
        findings["evidence"].append(
            f"Unique devices: {len(device_ids)} across {len(results)} transactions"
        )

        if len(device_ids) > 5:
            findings["risk_indicators"].append(
                f"Multiple device IDs detected: {len(device_ids)}"
            )
            findings["evidence"].append(
                f"SUSPICIOUS: {len(device_ids)} different devices used"
            )
...
def _analyze_user_agent_patterns(results: list, findings: Dict[str, Any]) -> None:
    ...
    # Support both uppercase (Snowflake) and lowercase (PostgreSQL) column names
    user_agents = set(
        r.get("USER_AGENT") or r.get("user_agent")
        for r in results
        if r.get("USER_AGENT") or r.get("user_agent")
    )
...
def _analyze_browser_os_patterns(results: list, findings: Dict[str, Any]) -> None:
    ...
    # Support both uppercase (Snowflake) and lowercase (PostgreSQL) column names
    device_models = set(
        r.get("DEVICE_MODEL") or r.get("device_model")
        for r in results
        if r.get("DEVICE_MODEL") or r.get("device_model")
    )
    device_os_versions = set(
        r.get("DEVICE_OS_VERSION") or r.get("device_os_version")
        for r in results
        if r.get("DEVICE_OS_VERSION") or r.get("device_os_version")
    )
    ...
    has_parsed_ua = any(
        r.get("PARSED_USER_AGENT") or r.get("parsed_user_agent") for r in results
    )
    has_user_agent = any(r.get("USER_AGENT") or r.get("user_agent") for r in results)
    has_device_fields = any(
        r.get("DEVICE_MODEL") or r.get("device_model") for r in results
    ) or any(r.get("DEVICE_OS_VERSION") or r.get("device_os_version") for r in results)
    ...
    findings["analysis"]["unique_device_models"] = (
        len(device_models) if device_models else "unknown"
    )
    findings["analysis"]["unique_os_versions"] = (
        len(device_os_versions) if device_os_versions else "unknown"
    )
    ...
    findings["metrics"]["unique_device_models"] = (
        len(device_models) if device_models else None
    )
    findings["metrics"]["unique_os_versions"] = (
        len(device_os_versions) if device_os_versions else None
    )
...
def _analyze_ml_anomaly_detection(
    ...
    logger.debug(
        f"[Step 5.2.2.2] 🔍 Category-based device analysis: Processing {len(tool_results)} tools"
    )
    ...
    for tool_name, result in tool_results.items():
        ...
        if device_signals:
            logger.debug(
                f"[Step 5.2.2.2]   ✅ {tool_name}: Found {len(device_signals)} device signals"
            )
            _process_device_signals(tool_name, device_signals, findings)
        else:
            logger.debug(
                f"[Step 5.2.2.2]   ➖ {tool_name}: No device intelligence signals detected"
            )
...
def _extract_device_signals(tool_name: str, result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.debug(
        f"[Step 5.2.2.2] 🔍 Extracting device signals from {tool_name} with {len(result)} top-level fields"
    )
    ...
    for indicator in device_indicators:
        if indicator in result:
            ...
            logger.debug(
                f"[Step 5.2.2.2]     → Found device indicator: {indicator} = {result[indicator]}"
            )
    ...
    for indicator in anomaly_indicators:
        if indicator in result:
            try:
                ...
                logger.debug(
                    f"[Step 5.2.2.2]     → Found anomaly score: {indicator} = {result[indicator]}"
                )
            except (ValueError, TypeError):
                logger.debug(
                    f"[Step 5.2.2.2]     → Skipped non-numeric score: {indicator} = {result[indicator]}"
                )
                pass
        ...
        if isinstance(value, dict):
            ...
            for i, item in enumerate(value[:5]):  # Limit to first 5 items
                if isinstance(item, dict):
                    nested_signals = _extract_device_signals(
    ...
    logger.debug(
        f"[Step 5.2.2.2] ✅ Extracted {len(signals)} device signals from {tool_name}"
    )
    return signals


def _process_device_signals(
    tool_name: str, signals: Dict[str, Any], findings: Dict[str, Any]
) -> None:
    """Process extracted device signals to adjust risk score."""

    logger.debug(
        f"[Step 5.2.2.3] 🔍 Processing {len(signals)} device signals from {tool_name}"
    )
    ...
    for key, value in signals.items():
        if key.startswith("score_") and isinstance(value, (int, float)):
            ...
            if normalized_score > 0.7:  # High device risk
                ...
                findings["evidence"].append(
                    f"{tool_name}: Normal device behavior {key} = {value}"
                )
    ...
    if "llm_risk_score" in findings:
        ...
        logger.debug(
            f"[Step 5.2.2.3]   ℹ️ LLM risk score already set ({findings.get('llm_risk_score', 'N/A')}), skipping device risk adjustments"
        )
        # Still add indicators to evidence, but don't modify score
        if device_risk_level > 0.5:
            findings["risk_indicators"].append(
                f"{tool_name}: Device anomalies detected (level: {device_risk_level:.2f})"
            )
        elif device_risk_level < -0.2:
            findings["evidence"].append(
                f"{tool_name}: Device behavior appears normal (level: {device_risk_level:.2f})"
            )
    else:
        ...
        if current_score is not None:
            if device_risk_level > 0.5:
                ...
                risk_multiplier = 1.0 + max(
                    -0.1, device_risk_level * 0.15
                )  # device_risk_level is negative
    ...
    if evidence_count > 0:
        ...
        logger.debug(
            f"[Step 5.2.2.3]   ✅ {tool_name}: Processed {evidence_count} device signals, risk level: {device_risk_level:.2f}"
        )
    else:
        logger.debug(
            f"[Step 5.2.2.3]   ➖ {tool_name}: No actionable device signals found"
        )
