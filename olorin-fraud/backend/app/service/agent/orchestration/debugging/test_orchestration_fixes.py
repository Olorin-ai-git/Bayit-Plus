...
from app.service.agent.orchestration.debugging.orchestration_debugger import (
    debug_failed_investigation,
)
from app.service.agent.orchestration.hybrid.ai_confidence_engine import (
    AIConfidenceEngine,
)
...
class OrchestrationTestSuite:
    ...
    async def run_comprehensive_tests(self) -> Dict[str, Any]:
        ...
        test_results["tests"][
            "domain_triggering"
        ] = await self._test_domain_agent_triggering()

        # Test 4: Tool Results to Domain Flow
        test_results["tests"][
            "tool_to_domain_flow"
        ] = await self._test_tool_to_domain_flow()

        # Test 5: Sequential Domain Execution
        test_results["tests"][
            "sequential_domains"
        ] = await self._test_sequential_domain_execution()
    ...
    async def _test_routing_logic(self) -> Dict[str, Any]:
        ...
        routing_decision = await self.router.get_hybrid_routing_decision(
            state_with_tool_results
        )
        ...
        if routing_decision["next_node"].endswith("_agent"):
            ...
            test_result["fixes_validated"].append(
                "Tool results now correctly trigger domain analysis"
            )
        else:
            test_result["subtests"]["tool_to_domain"] = {
                "status": "failed",
            ...
            test_result["issues_found"].append(
                "Tool results still not triggering domain analysis"
            )
        ...
        routing_decision_2 = await self.router.get_hybrid_routing_decision(
            state_with_partial_domains
        )

        if routing_decision_2["next_node"] in [
            "device_agent",
            "location_agent",
            "logs_agent",
            ...
            test_result["fixes_validated"].append(
                "Sequential domain progression working correctly"
            )
        else:
            test_result["subtests"]["sequential_progression"] = {
                "status": "failed",
        ...
        passed_subtests = sum(
            1
            for subtest in test_result["subtests"].values()
    ...
    async def _test_state_transitions(self) -> Dict[str, Any]:
        ...
        passed_phases = sum(
            1
            for phase in test_result["phase_transitions"]
        ...
        if passed_phases == total_phases:
            ...
            test_result["status"] = "partial"
            test_result["issues_found"].append(
                f"Only {passed_phases}/{total_phases} phase transitions working"
            )
        else:
            test_result["status"] = "failed"
    ...
    async def _test_domain_agent_triggering(self) -> Dict[str, Any]:
        ...
        for i, domain in enumerate(domains):
            ...
            state.update(
                {
                    ...
                    "domain_findings": {
                        prev_domain: {"result": "test"} for prev_domain in domains[:i]
                    },
            ...
            test_result["domain_tests"][domain] = {
                ...
                "status": (
                    "passed"
                    if routing_decision["next_node"] == expected_agent
                    else "failed"
                ),
        ...
        passed_domains = sum(
            1
            for test in test_result["domain_tests"].values()
        ...
        if passed_domains == total_domains:
            test_result["status"] = "passed"
            test_result["fixes_validated"].append(
                "All domain agents triggered correctly"
            )
        elif passed_domains > 0:
            test_result["status"] = "partial"
            test_result["issues_found"].append(
                f"Only {passed_domains}/{total_domains} domain agents triggered correctly"
            )
        else:
            test_result["status"] = "failed"
            test_result["issues_found"].append(
                "Domain agent triggering completely broken"
            )
    ...
    async def _test_tool_to_domain_flow(self) -> Dict[str, Any]:
        ...
        for scenario in scenarios:
            ...
            test_result["flow_tests"][scenario["name"]] = {
                ...
                "status": (
                    "passed"
                    if triggers_domain == scenario["should_trigger_domains"]
                    else "failed"
                ),
            }
        ...
        passed_flows = sum(
            1
            for test in test_result["flow_tests"].values()
            if test["status"] == "passed"
        )
        ...
        if passed_flows == total_flows:
            test_result["status"] = "passed"
            test_result["fixes_validated"].append(
                "Tool-to-domain flow working correctly"
            )
        else:
            test_result["status"] = "failed"
            test_result["issues_found"].append(
                f"Tool-to-domain flow issues: {passed_flows}/{total_flows} working"
            )
    ...
    async def _test_sequential_domain_execution(self) -> Dict[str, Any]:
            ...
            state.update(
                {
                    ...
                    "tool_results": {"test_tool": {"result": "data"}},
                    "domain_findings": {
                        domain: {"result": "test"} for domain in completed_domains
                    },
                ...
                test_result["execution_order"].append(
                    {
                        ...
                        "status": (
                            "passed" if actual_next == expected_next else "failed"
                        ),
                    }
                )
        ...
        final_state.update(
            {
                ...
                "tool_results": {"test_tool": {"result": "data"}},
                "domain_findings": {
                    domain: {"result": "test"} for domain in expected_order
                },
        ...
        test_result["execution_order"].append(
            {
                ...
                "status": (
                    "passed" if final_routing["next_node"] == "summary" else "failed"
                ),
            }
        )
        ...
        # Calculate overall status
        passed_steps = sum(
            1 for step in test_result["execution_order"] if step["status"] == "passed"
        )
        ...
        if passed_steps == total_steps:
            test_result["status"] = "passed"
            test_result["fixes_validated"].append(
                "Sequential domain execution working correctly"
            )
        else:
            test_result["status"] = "failed"
            test_result["issues_found"].append(
                f"Sequential execution issues: {passed_steps}/{total_steps} steps working"
            )
