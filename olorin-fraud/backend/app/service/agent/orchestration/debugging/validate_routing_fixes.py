...
def simulate_routing_logic(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if len(tool_results) > 0 and len(domain_findings) == 0:
        print(
            f"🎯 CRITICAL ROUTING: Tool results available, triggering domain analysis"
        )
        ...
        return {
            ...
            "reasoning": [
                "CRITICAL: Start domain analysis with tool results",
                f"First domain: {next_domain}",
            ],
    ...
    return {
        ...
        "reasoning": [
            ...
            f"Analyzed {len(domain_findings)} domains",
        ],
        "phase": "completion",
    }
...
def test_routing_scenarios():
    ...
    test_results.append(
        {
            ...
            "status": (
                "✅ PASS"
                if result1["next_node"] == "fraud_investigation"
                else "❌ FAIL"
            ),
        }
    )
    print(
        f"   Result: {test_results[-1]['status']} - Expected: fraud_investigation, Got: {result1['next_node']}"
    )
    ...
    test_results.append(
        {
            ...
            "status": (
                "✅ PASS"
                if result2["next_node"] == "fraud_investigation"
                else "❌ FAIL"
            ),
        }
    )
    print(
        f"   Result: {test_results[-1]['status']} - Expected: fraud_investigation, Got: {result2['next_node']}"
    )
    ...
    test_results.append(
        {
            ...
            "status": (
                "✅ PASS"
                if result3["next_node"] == expected_domain_agent
                else "❌ FAIL"
            ),
        }
    )
    print(
        f"   Result: {test_results[-1]['status']} - Expected: {expected_domain_agent}, Got: {result3['next_node']}"
    )
    if test_results[-1]["status"] == "✅ PASS":
        print(
            f"   🎉 CRITICAL FIX VALIDATED: Tool results now trigger domain analysis!"
        )
    else:
    ...
    test_results.append(
        {
            ...
            "status": (
                "✅ PASS" if result4["next_node"] == expected_next_domain else "❌ FAIL"
            ),
        }
    )
    print(
        f"   Result: {test_results[-1]['status']} - Expected: {expected_next_domain}, Got: {result4['next_node']}"
    )
    ...
    print(
        f"   Result: {test_results[-1]['status']} - Expected: summary, Got: {result5['next_node']}"
    )
    ...
    # Check critical fix
    critical_test = next(
        (r for r in test_results if r["test"] == "CRITICAL_post_tools_trigger_domains"),
        None,
    )
