...
class TracedInvestigationGraph:
    ...
    async def execute_investigation(
        ...
        try:
            ...
            result = await self._execute_with_tracing(
                graph, input_data, traced_config, metrics
            )
            ...
            result["trace_metadata"] = {
                ...
                "tool_calls": sum(
                    m.get("calls", 0) for m in metrics.tool_metrics.values()
                ),
    ...
    def _create_traced_config(
        self, config: Optional[RunnableConfig], trace_id: str
    ) -> RunnableConfig:
    ...
    async def _execute_with_tracing(
        ...
        stages = {"initialization": time.time(), "execution": None, "completion": None}
        ...
        try:
            ...
            logger.info(
                f"Execution stages - Init: {init_duration:.2f}s, Exec: {exec_duration:.2f}s"
            )
    ...
    def _get_optimization_recommendations(self, metrics: TraceMetrics) -> List[str]:
        ...
        if metrics.duration and metrics.duration > 60:
            recommendations.append(
                f"Investigation took {metrics.duration:.1f}s - consider parallel execution"
            )
        ...
        for agent_name, agent_metrics in metrics.agent_metrics.items():
            ...
            if avg_duration > 10:
                recommendations.append(
                    f"Agent '{agent_name}' averaging {avg_duration:.1f}s - investigate bottleneck"
                )
        ...
        if slow_tools:
            ...
            recommendations.append(
                f"Tool '{slowest[0]}' is slowest at {slowest[1]:.1f}s avg"
            )
        ...
        if total_cache_ops > 0:
            ...
            if cache_hit_rate < 0.5:
                recommendations.append(
                    f"Low cache hit rate ({cache_hit_rate:.1%}) - review caching strategy"
                )

        # Check error rate
        if metrics.error_count > 0:
            recommendations.append(
                f"{metrics.error_count} errors occurred - review error handling"
            )

        # Check retry rate
        if metrics.retry_count > 5:
            recommendations.append(
                f"High retry count ({metrics.retry_count}) - check service reliability"
            )

        return recommendations

    def _log_performance_metrics(
        self, trace_id: str, metrics: TraceMetrics, result: Dict[str, Any]
    ):
        ...
        try:
            ...
            logger.info(
                f"Performance metrics logged for trace {trace_id}",
    ...
    def _log_error_metrics(
        self, trace_id: str, error: Exception, metrics: TraceMetrics
    ):
        ...
        try:
            ...
            logger.error(
                f"Investigation error logged for trace {trace_id}", extra=error_data
            )
...
class MetricsCollector:
    ...
    def record_agent_execution(self, agent_name: str, duration: float):
        ...
        agent_metrics["avg_duration"] = agent_metrics["total_duration"] / max(
            1, agent_metrics["executions"]
    ...
    def record_tool_call(self, tool_name: str, duration: float):
        ...
        tool_metrics["avg_duration"] = tool_metrics["total_duration"] / max(
            1, tool_metrics["calls"]
        )  # CRITICAL FIX: Division by zero protection
...
@asynccontextmanager
async def traced_investigation(
    graph_builder_func: Callable, project: Optional[str] = None
):
    """
