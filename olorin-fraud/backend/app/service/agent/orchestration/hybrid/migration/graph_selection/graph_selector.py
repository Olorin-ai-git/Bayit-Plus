...
class GraphSelector:
    ...
    CRITICAL: In LIVE mode, passes LLM to graph builders for intelligent routing.
             In DEMO mode, uses rule-based heuristics (no API costs).
    """

    def __init__(self, llm=None):
        """
        Initialize graph selector with optional LLM.
    ...
    async def select_investigation_graph(
        ...
        if not investigation_id:
            increment_counter(
                ...
                metadata={"reason": "missing_investigation_id"},
            )
        ...
        increment_counter(
            "graph_selection_attempts", metadata={"entity_type": entity_type}
        )
        ...
        try:
            ...
            if force_graph_type:
                ...
                try:
                    graph = await self.graph_builders.build_graph(
                        force_graph_type, investigation_id
                    )
                    self._record_graph_selection(
                        investigation_id, force_graph_type, SelectionReason.FORCED
                    )
            ...
            if self.rollback_triggers.should_rollback():
                ...
                increment_counter(
                    ...
                    metadata={"reason": "rollback_triggered"},
                )
                try:
                    graph = await self.graph_builders.build_graph(
                        GraphType.CLEAN, investigation_id
                    )
                    self._record_graph_selection(
                        investigation_id, GraphType.CLEAN, SelectionReason.ROLLBACK
                    )
                    return create_success_result(
                        graph,
                        GraphType.CLEAN,
                        ...
                        context={
                            "entity_type": entity_type,
                            "rollback_reason": "rollback_triggers_active",
                        },
                    )
                except Exception as e:
                    return create_failure_result(
            ...
            # Check A/B testing
            try:
                ab_assignment = self.ab_test_manager.get_ab_test_assignment(
                    investigation_id
                )
                if ab_assignment:
                    ...
                    try:
                        graph = await self.graph_builders.build_graph(
                            ab_assignment, investigation_id
                        )
                        self._record_graph_selection(
                            investigation_id, ab_assignment, SelectionReason.AB_TEST
                        )
                        return create_success_result(
                            graph,
                            ab_assignment,
                            SelectionReason.AB_TEST,
                            investigation_id,
                            context={
                                "entity_type": entity_type,
                                "ab_test_active": True,
                ...
                logger.warning(
                    f"A/B test assignment failed: {str(e)} - continuing with feature flags"
                )
            ...
            try:
                if self.feature_flags.is_enabled("hybrid_graph_v1", investigation_id):
                    ...
                    try:
                        graph = await self.graph_builders.build_graph(
                            GraphType.HYBRID, investigation_id
                        )
                        self._record_graph_selection(
                            investigation_id,
                            GraphType.HYBRID,
                        ...
                        return create_success_result(
                            ...
                            context={
                                "entity_type": entity_type,
                                "feature_flag": "hybrid_graph_v1",
                            },
                        )
                    except Exception as e:
                        logger.error(
                            f"Hybrid graph build failed: {str(e)} - falling back to clean"
                        )
                        self.fallback_counter += 1
                        increment_counter(
                            "graph_selection_fallbacks",
                            metadata={"reason": "hybrid_build_failed"},
                        )
            except Exception as e:
            ...
            try:
                ...
                self._record_graph_selection(
                    investigation_id, GraphType.HYBRID, SelectionReason.DEFAULT
                )
                return create_success_result(
                    graph,
                    GraphType.HYBRID,
            ...
            self.fallback_counter += 1
            increment_counter(
                "graph_selection_fallbacks", metadata={"reason": "emergency_fallback"}
            )
            ...
            try:
                ...
                self._record_graph_selection(
                    investigation_id,
                    GraphType.CLEAN,
                    SelectionReason.EMERGENCY_FALLBACK,
                )
                return create_success_result(
                    graph,
                    GraphType.CLEAN,
    ...
    async def get_investigation_graph(
        ...
        """
        result = await self.select_investigation_graph(
            investigation_id, entity_type, force_graph_type
        )
    ...
    def _record_graph_selection(
        ...
        selection_record = {
            ...
            "feature_flags": {
                "hybrid_graph_v1": self.feature_flags.get_flag_status(
                    "hybrid_graph_v1"
                ),
                "ab_test_hybrid_vs_clean": self.feature_flags.get_flag_status(
        ...
        logger.debug(
            f"   Migration system: Feature flag tracking & performance metrics"
        )
    ...
    def get_selection_stats(self) -> dict:
        ...
        for record in self.performance_metrics.values():
            ...
            stats["graph_type_counts"][graph_type] = (
                stats["graph_type_counts"].get(graph_type, 0) + 1
            )
            stats["selection_reason_counts"][reason] = (
                stats["selection_reason_counts"].get(reason, 0) + 1
            )

        # Add fallback tracking to stats
        stats["fallback_counter"] = self.fallback_counter
        stats["fallback_rate"] = self.fallback_counter / max(
            1, len(self.performance_metrics)
        )
