...
class EnvironmentLoader:
    ...
    def _apply_environment_override(self, flag_name: str, flag_config: Dict[str, Any]):
        """
        ...
        if self._is_truthy_value(env_value):
            ...
            logger.info(
                f"🚩 Environment override: {flag_name} enabled via {env_var_name}"
            )
            ...
            logger.info(
                f"🚩 Environment override: {flag_name} disabled via {env_var_name}"
            )

        else:
            logger.warning(
                f"🚩 Invalid environment value for {env_var_name}: {env_value}"
            )
