...
class DomainAssessor:
    ...
    async def assess_evidence(self, state: HybridInvestigationState) -> float:
        ...
        quality_factor = self._calculate_quality_factor(
            domains_completed, domain_findings
        )
        risk_factor = self._calculate_average_risk_score(
            domains_completed, domain_findings
        )

        # Weighted combination of factors
        domain_confidence = (
            completion_ratio * 0.4 + quality_factor * 0.4 + risk_factor * 0.2
        )

        logger.debug(f"   🎯 Domain evidence confidence: {domain_confidence:.3f}")
        logger.debug(
            f"      Completed: {len(domains_completed)}/{self.total_domains}, Quality: {quality_factor:.3f}"
        )
    ...
    def _calculate_quality_factor(
        self, domains_completed: List[Any], domain_findings: Dict[str, Any]
    ) -> float:
