...
from .reasoning import CompletenessTracker, EvidenceAnalyzer, ReasoningBuilder
from .strategy import ActionPlanner, AgentSelector, StrategySelector, ToolRecommender
...
class DecisionEngine:
    ...
    def __init__(self, llm=None):
        ...
        if self.use_llm_routing:
            logger.info(
                "🧠 LIVE MODE: Using real LLM for intelligent routing decisions"
            )
        else:
    ...
    async def calculate_investigation_confidence(
        ...
        logger.debug(
            f"   Investigation: {state.get('entity_type')} - {state.get('entity_id')}"
        )
        logger.debug(
            f"   Multi-factor analysis: Snowflake(35%) + Tool(25%) + Domain(20%) + Pattern(15%) + Velocity(5%)"
        )
        ...
        try:
            ...
            routing_decision = await self._generate_routing_decision(
                state, overall_confidence, confidence_scores
            )
            ...
            logger.info(
                f"🧠 AI confidence calculated: {overall_confidence:.3f} ({routing_decision.confidence_level.value})"
            )
            ...
            logger.debug(
                f"     State context: {state.get('investigation_id', 'N/A')} - {state.get('current_phase', 'N/A')}"
            )
        ...
        async def safe_assess(assessor, name: str) -> float:
            try:
                ...
                if not isinstance(score, (int, float)):
                    logger.warning(
                        f"     ⚠️  {name} assessor returned {type(score).__name__}, using 0.0"
                    )
                    return 0.0
                return float(score)
            except Exception as e:
    ...
    def _calculate_weighted_confidence(
        ...
        def safe_score(key: str) -> float:
            ...
            if not isinstance(score, (int, float)):
                logger.warning(
                    f"   ⚠️  {key} assessor returned {type(score).__name__}, using 0.0"
                )
                return 0.0
            return float(score)
        ...
        snowflake_weighted = (
            snowflake_score * self.confidence_weights["snowflake_evidence"]
        )
        ...
        pattern_weighted = (
            pattern_score * self.confidence_weights["pattern_recognition"]
        )
        velocity_weighted = (
            velocity_score * self.confidence_weights["investigation_velocity"]
        )

        logger.debug(
            f"     Snowflake: {snowflake_score:.3f} × 0.35 = {snowflake_weighted:.3f}"
        )
        logger.debug(f"     Tool: {tool_score:.3f} × 0.25 = {tool_weighted:.3f}")
        logger.debug(f"     Domain: {domain_score:.3f} × 0.20 = {domain_weighted:.3f}")
        logger.debug(
            f"     Pattern: {pattern_score:.3f} × 0.15 = {pattern_weighted:.3f}"
        )
        logger.debug(
            f"     Velocity: {velocity_score:.3f} × 0.05 = {velocity_weighted:.3f}"
        )
    ...
    async def _generate_routing_decision(
        ...
        if self.use_llm_routing:
            logger.info(
                "🧠 LIVE MODE: Calling real LLM for intelligent routing decision"
            )
            return await self._generate_llm_routing_decision(
                state, overall_confidence, confidence_scores
            )
        ...
        # Strategic decisions
        strategy = await self.strategy_selector.determine_strategy(
            state, overall_confidence
        )
        next_action = await self.action_planner.determine_next_action(
            state, overall_confidence, strategy
        )
        agents_to_activate = await self.agent_selector.determine_agents_to_activate(
            state, strategy, overall_confidence
        )
        tools_recommended = await self.tool_recommender.determine_recommended_tools(
            state, strategy, overall_confidence
        )
        ...
        reasoning = self.reasoning_builder.build_reasoning_chain(
            ...
            strategy,
            next_action,
            confidence_scores["snowflake"],
            confidence_scores["tool"],
            confidence_scores["domain"],
            confidence_scores["pattern"],
            confidence_scores["velocity"],
        )
        evidence_quality = await self.evidence_analyzer.calculate_evidence_quality(
            state
        )
        completeness = (
            await self.completeness_tracker.calculate_investigation_completeness(state)
        )
        ...
        resource_impact = self._assess_resource_impact(
            state, strategy, agents_to_activate
        )
...
{{
    ...
    "estimated_time_minutes": int
            ...
            logger.debug(
                f"   🤖 Calling LLM with routing prompt ({len(routing_prompt)} chars)"
            )
            ...
            return self._build_decision_from_llm(
                state, overall_confidence, routing_data
            )
            ...
            return await self._generate_heuristic_routing_decision(
                state, overall_confidence, confidence_scores
            )
    ...
    def _build_investigation_context(
        self, state: HybridInvestigationState, confidence_scores: Dict[str, float]
    ) -> str:
...
"""
    ...
    def _build_decision_from_llm(
        ...
        try:
            ...
            logger.warning(
                f"   ⚠️  Invalid strategy '{strategy_str}', defaulting to FOCUSED"
            )
        ...
        return AIRoutingDecision(
            ...
            investigation_completeness=routing_data.get(
                "investigation_completeness", 0.5
            ),
            strategy=strategy,
    ...
    def _assess_resource_impact(
        self, state: HybridInvestigationState, strategy, agents: List[str]
    ) -> str:
    ...
    def _estimate_completion_time(
        self, state: HybridInvestigationState, strategy, confidence: float
    ) -> Optional[int]:
        """Estimate time to completion in minutes."""
        strategy_times = {"minimal": 2, "critical_path": 3, "focused": 5}

        base_time = strategy_times.get(strategy.value, 12 if confidence <= 0.7 else 8)
        return base_time

    def _create_fallback_decision(
        self, state: HybridInvestigationState, start_time: float, error_msg: str
    ) -> AIRoutingDecision:
        ...
        return AIRoutingDecision(
            ...
            recommended_action=(
                "snowflake_analysis"
                if not state.get("snowflake_completed")
                else "summary"
            ),
            reasoning=[
                f"Confidence calculation failed: {error_msg}",
                "Falling back to safe sequential execution",
            ],
            evidence_quality=0.0,
            investigation_completeness=0.0,
            strategy=self.strategy_selector.strategy_confidence_thresholds.get(
