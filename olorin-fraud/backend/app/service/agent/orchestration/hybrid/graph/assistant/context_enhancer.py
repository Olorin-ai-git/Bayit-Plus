...
"""
    ...
    def enhance_messages_with_ai_context(
        self, messages: List[BaseMessage], ai_context: str
    ) -> List[BaseMessage]:
        """
        ...
        if ai_context.strip():
            ...
            if not system_message_found:
                ...
                if (
                    ...
                    and messages[0].type == "tool_use"
                ):
                    # Cannot safely add system message without breaking tool sequence
                    logger.warning(
                        "Cannot add AI context - would break tool_use/tool_result sequence"
                    )
    ...
    def get_ai_recommendation_summary(
        ...
        return {
            ...
            "latest_recommendation": {
                ...
                "strategy": (
                    latest_decision.strategy.value
                    if hasattr(latest_decision.strategy, "value")
                    else str(latest_decision.strategy)
                ),
                "reasoning": latest_decision.reasoning,
            },
            ...
            if hasattr(msg, "type"):
                if msg.type == "tool_use":
                    ...
                    if not expecting_tool_result:
                        logger.warning(
                            f"Tool result without preceding tool_use at position {i}"
                        )
                        tool_sequence_valid = False
                    expecting_tool_result = False
                elif expecting_tool_result and msg.type != "tool_result":
                    logger.warning(
                        f"Expected tool_result but found {msg.type} at position {i}"
                    )
                    tool_sequence_valid = False
                    expecting_tool_result = False
