...
class OrchestratorNode:
    ...
    async def hybrid_orchestrator_node(
        ...
        logger.debug(
            f"     AI Confidence Level: {state.get('ai_confidence_level', 'UNKNOWN')}"
        )
        logger.debug(f"   🧠 ROUTING CONTEXT:")
        logger.debug(f"     Intelligent Router: AI confidence + safety validation")
        logger.debug(
            f"     Available strategies: CRITICAL_PATH | MINIMAL | FOCUSED | ADAPTIVE | COMPREHENSIVE"
        )
        logger.debug(
            f"     Safety concerns: {len(state.get('safety_concerns', []))} active"
        )
        logger.debug(
        ...
        try:
            ...
            routing_decision = (
                await self.intelligent_router.get_hybrid_routing_decision(state)
            )
            ...
            logger.debug(
                f"   🔄 LOOP UPDATE: {old_loop_count} → {state['orchestrator_loops']}"
            )
            ...
            logger.debug(
                f"   📋 DECISION HISTORY: Total decisions: {len(state['routing_decisions'])}"
            )
            ...
            logger.debug(
                f"   📊 AUDIT TRAIL: Total entries: {len(state['decision_audit_trail'])}"
            )

            # Log final decision with context
            logger.info(
                f"✅ Hybrid orchestrator decision: {routing_decision['next_node']}"
            )
            ...
            if routing_decision.get("safety_override"):
                logger.warning(
                    f"⚠️ Safety override applied: {routing_decision.get('override_reason')}"
                )
                logger.debug(f"   🛡️ SAFETY OVERRIDE DETAILS:")
                logger.debug(
                    f"     Reason: {routing_decision.get('override_reason', 'No reason provided')}"
                )
                logger.debug(
                    f"     Override Type: {routing_decision.get('override_type', 'N/A')}"
                )
            ...
            logger.debug(
                f"     State at failure: investigation_id={state.get('investigation_id')}, phase={state.get('current_phase')}"
            )
            logger.debug(
                f"     Loop count at failure: {state.get('orchestrator_loops', 0)}"
            )
            ...
            logger.debug(
                f"   🔄 ERROR RECOVERY: Added error record to state, total errors: {len(state['errors'])}"
            )
    ...
    def get_orchestrator_stats(self, state: HybridInvestigationState) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_routing_decision": (
                state.get("routing_decisions", [])[-1]
                if state.get("routing_decisions")
                else None
            ),
            "safety_overrides_count": len(
                [
                    d
                    for d in state.get("routing_decisions", [])
                    if d.get("safety_override")
                ]
            ),
        }
