...
class DomainAgentEnhancer:
    ...
    def create_enhanced_domain_agent(
        self, domain_name: str, original_agent: Callable
    ) -> Callable:
        ...
        async def enhanced_domain_agent(
            ...
            logger.debug(
                f"   Domain analysis: Enhanced with confidence tracking & audit trail"
            )
            logger.debug(
                f"   Performance metrics: Real-time completion time monitoring"
            )
            logger.debug(
                f"   D1 FIX: Timeout enforcement: {DOMAIN_AGENT_TIMEOUT_SECONDS}s deadline"
            )
            ...
            try:
                ...
                try:
                    ...
                    logger.error(
                        f"⏱️ D1 FIX: {domain_name} agent exceeded {DOMAIN_AGENT_TIMEOUT_SECONDS}s timeout - short-circuiting"
                    )
                    ...
                    timeout_error = TimeoutError(
                        f"{domain_name} agent exceeded {DOMAIN_AGENT_TIMEOUT_SECONDS}s timeout"
                    )
                ...
                for domain in domains_completed_raw:
                    if isinstance(domain, dict):
                        domain_key = (
                            domain.get("domain") or domain.get("name") or str(domain)
                        )
                    else:
                ...
                if domain_name in domain_findings:
                    finding_quality = domain_findings[domain_name].get(
                        "confidence", 0.5
                    )
    ...
    def get_domain_completion_stats(
        ...
        return {
            ...
            "completion_percentage": len(domains_completed)
            / len(available_domains)
            * 100,
            "completed_domain_list": domains_completed,
            "pending_domains": [
                d for d in available_domains if d not in domains_completed
            ],
        }

    def get_domain_performance_metrics(
        self, state: HybridInvestigationState
    ) -> Dict[str, Any]:
    ...
    def get_domain_confidence_factors(
        self, state: HybridInvestigationState
    ) -> Dict[str, float]:
