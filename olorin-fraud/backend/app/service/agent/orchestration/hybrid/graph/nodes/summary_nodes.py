...
class SummaryNodes:
    ...
    async def enhanced_summary_node(
        ...
        logger.debug(
            f"   Comprehensive reporting: AI decisions + Safety overrides + Performance metrics"
        )
        logger.debug(
            f"   Investigation efficiency: Multi-factor calculation (time, coverage, safety)"
        )
        ...
        try:
            ...
            consolidated_confidence = (
                self.confidence_consolidator.consolidate_confidence_scores(
                    state=state,
            ...
            state = self.confidence_consolidator.apply_consolidated_confidence(
                state, consolidated_confidence
            )

            from app.service.agent.orchestration.metrics.safe import fmt_num

            logger.info(
                f"✅ Confidence consolidated: {fmt_num(consolidated_confidence.overall_score, 3)} ({consolidated_confidence.level_description})"
            )
            if consolidated_confidence.data_quality_issues:
                logger.warning(
                    f"   Data quality issues detected: {len(consolidated_confidence.data_quality_issues)}"
                )
            ...
            from app.service.agent.orchestration.risk.evidence_gating import (
                apply_evidence_gating,
            )

            apply_evidence_gating(state)
            logger.info(
                f"✅ Evidence gating applied: strength={fmt_num(state.get('evidence_strength', 0.0), 3)}"
            )
            ...
            need_transformation = (
                not existing_domain_findings  # domain_findings doesn't exist or is empty
                or not any(
                    df.get("risk_score") is not None
                    for df in existing_domain_findings.values()
                    if isinstance(df, dict)
                )  # or has no risk scores
            )

            if need_transformation and "agent_results" in state:
                logger.info(
                    "🔄 Transforming agent_results to domain_findings structure for finalize.py"
                )
                logger.info(
                    f"   Reason: existing_domain_findings empty={not existing_domain_findings}, has_scores={any(df.get('risk_score') is not None for df in existing_domain_findings.values() if isinstance(df, dict)) if existing_domain_findings else False}"
                )
                domain_findings = {}
                ...
                for domain_name in [
                    ...
                    if domain_name in agent_results:
                        ...
                        if isinstance(findings, dict):
                            ...
                            elif "analysis" in findings and isinstance(
                                findings["analysis"], list
                            ):
                        ...
                        domain_findings[domain_name] = {
                            ...
                            "summary": findings.get(
                                "summary", f"{domain_name} domain analysis"
                            ),
                            "status": (
                                "OK"
                ...
                logger.info(
                    f"✅ Transformed {len(domain_findings)} domains to domain_findings structure"
                )
                    ...
                    if isinstance(domain_data, dict):
                        ...
                        if domain_risk is not None:
                            domain_scores.append(
                                f"{domain_name}={fmt_num(domain_risk, 2)}"
                            )
                domain_info = (
                    f" (domain scores: {', '.join(domain_scores[:3])})"
                    if domain_scores
                    else ""
                )
                logger.info(
                    f"✅ Risk score finalized: N/A (blocked by evidence gating){domain_info}"
                )
            else:
            ...
            investigation_summary = self.summary_generator.generate_hybrid_summary(
                state
            )
            ...
            logger.info(
                f"   Final confidence: {fmt_num(state.get('ai_confidence', 0.0), 3)}"
            )
            ...
            try:
                ...
                state["canonical_final_outcome"] = outcome_to_dict(canonical_outcome)
                logger.info(
                    f"🛡️ Canonical outcome generated for error case: {canonical_outcome.status.value}"
                )
    ...
    async def enhanced_complete_node(
        ...
        logger.debug(
            f"   Final metrics calculation: Efficiency, resource utilization, AI optimization"
        )
        ...
        state["performance_metrics"]["final_efficiency"] = (
            self.performance_calculator.calculate_investigation_efficiency(state)
        )
        state["investigation_efficiency"] = state["performance_metrics"][
            "final_efficiency"
        ]
        ...
        if investigation_id:
                    ...
                    with get_db_session() as db:
                        ...
                        if db_state:
                            ...
                            if validated_scores:
                                ...
                                db.commit()
                                logger.info(
                                    f"✅ Persisted {len(validated_scores)} transaction scores to progress_json for investigation {investigation_id}"
                                )
                                if invalid_count > 0:
                                    logger.warning(
                                ...
                                logger.warning(
                                    f"⚠️ No valid transaction scores to persist for investigation {investigation_id}"
                                )
                        else:
                            logger.warning(
                                f"⚠️ Investigation state not found in database for {investigation_id}, cannot persist transaction_scores"
                            )
            except Exception as tx_error:
                logger.error(
                    f"❌ CRITICAL: Failed to persist transaction_scores for investigation {investigation_id}: {tx_error}",
                    exc_info=True,
                )
            ...
            if transaction_scores:
                logger.warning(
                    f"⚠️ transaction_scores exist in state but investigation_id is missing - cannot persist"
                )
    ...
    def _calculate_investigation_duration(
        ...
        try:
            if state.get("start_time"):
                ...
                if "start_time_ms" in perf_metrics:
                    ...
                    state["total_duration_ms"] = max(
                        0, int(current_time_ms - start_time_ms)
                    )
                else:
    ...
    def _log_final_statistics(self, state: HybridInvestigationState) -> None:
        ...
        logger.info(
            f"   Domains completed: {len(state.get('domains_completed', []))}/6"
        )
        ...
        logger.info(
            f"   Investigation efficiency: {fmt_num(state.get('investigation_efficiency', 0.0), 3)}"
        )
