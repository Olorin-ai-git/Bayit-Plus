...
class ConfidenceApplicator:
    ...
    def apply_consolidated_confidence(
        ...
        logger.debug(
            f"Applying consolidated confidence {fmt_num(consolidated.overall_score, 3)} to state"
        )
    ...
    def _update_primary_confidence_fields(
        ...
        if consolidated.evidence_confidence is not None:
            self._safe_set_confidence(
                state, "evidence_confidence", consolidated.evidence_confidence
            )

        logger.debug(
            f"Updated primary confidence fields to {fmt_num(consolidated.overall_score, 3)}"
        )
    ...
    def _add_consolidation_metadata(
        ...
        metadata["quality_indicators"] = {
            ...
            "reliability_rating": self._get_reliability_rating(
                consolidated.reliability_score
            ),
            "consistency_rating": self._get_consistency_rating(
                consolidated.consistency_score
            ),
        ...
        logger.debug(
            f"Added consolidation metadata with {len(consolidated.source_fields)} sources"
        )
    ...
    def _log_application_results(self, consolidated: ConsolidatedConfidence):
        ...
        logger.debug(
            f"✅ Applied consolidated confidence {fmt_num(consolidated.overall_score, 3)} ({consolidated.level_description})"
        )
        logger.debug(
            f"   Consistency: {fmt_num(consolidated.consistency_score, 3)} | Reliability: {fmt_num(consolidated.reliability_score, 3)}"
        )

        if consolidated.data_quality_issues:
            logger.warning(
                f"   Data quality issues: {len(consolidated.data_quality_issues)} detected"
            )
            for issue in consolidated.data_quality_issues[:3]:  # Log first 3 issues
                logger.warning(f"     - {issue}")
            if len(consolidated.data_quality_issues) > 3:
                logger.warning(
                    f"     ... and {len(consolidated.data_quality_issues) - 3} more"
                )
    ...
    def extract_confidence_summary(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "evidence_confidence": self._safe_get_confidence(
