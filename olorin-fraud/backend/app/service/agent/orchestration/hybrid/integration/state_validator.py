...
class StateValidator:
    ...
    def validate_investigation_state(
        ...
        try:
            ...
            if result.is_valid:
                ...
                error_count = len(
                    [i for i in result.issues if i["severity"] in ["error", "critical"]]
                )
                logger.warning(
                    f"⚠️ State validation issues: {investigation_id} ({error_count} errors)"
                )
    ...
    def _validate_required_fields(
        self, state: Dict[str, Any], result: ValidationResult
    ):
    ...
    def _validate_investigation_progress(
        self, state: Dict[str, Any], result: ValidationResult
    ):
    ...
    def _initialize_validation_rules(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "valid_entity_types": ["ip", "domain", "email", "hash", "url"],
        }
    ...
    def handle_validation_error(
        ...
        try:
            ...
            if revalidation_result.is_valid:
                logger.info(
                    f"✅ Validation error recovery successful: {investigation_id}"
                )
            else:
                logger.warning(
                    f"⚠️ Partial validation error recovery: {investigation_id}"
                )
    ...
    def _apply_error_correction(
        self, state: Dict[str, Any], issue: Dict[str, Any], investigation_id: str
    ) -> Dict[str, Any]:
    ...
    def register_error_handler(self, error_type: str, handler_function: callable):
        """
