...
class ServiceIntegrationProtocol(Protocol):
    ...
    async def initialize_investigation(
        self, investigation_id: str, parameters: Dict[str, Any]
    ) -> bool:
...
class ServiceAdapter:
        ...
        try:
            ...
            if investigation_context:
                ...
                try:
                    if (
                        self.graph_selector
                        and hasattr(self.graph_selector, "rollback_triggers")
                        and self.graph_selector.rollback_triggers
                    ):
                    ...
                    logger.warning(
                        f"Failed to record investigation result in rollback triggers: {e}"
                    )
                ...
                logger.info(
                    f"✅ Investigation completed: {investigation_id} (success={success})"
                )
            else:
                logger.warning(
                    f"⚠️ Investigation not found in active list: {investigation_id}"
                )

        except Exception as e:
            logger.error(f"❌ Investigation completion failed: {str(e)}")

    def register_service_hook(self, hook_name: str, hook_function: callable):
        """
    ...
    async def _initialize_investigation_context(
        ...
        await self._execute_service_hooks(
            "pre_investigation_init",
            {
                "investigation_id": investigation_id,
                "entity_type": entity_type,
        ...
        try:
            if (
                ...
                and hasattr(self.graph_selector, "rollback_triggers")
                and self.graph_selector.rollback_triggers
                and hasattr(self.graph_selector.rollback_triggers, "metrics_collector")
                and self.graph_selector.rollback_triggers.metrics_collector
            ):
                ...
                self.graph_selector.rollback_triggers.metrics_collector.record_investigation_start(
                    investigation_id=investigation_id, graph_type=graph_type
                )
        except Exception as e:
    ...
    async def _cleanup_investigation_context(self, investigation_id: str):
        ...
        await self._execute_service_hooks(
            "post_investigation_cleanup", {"investigation_id": investigation_id}
        )
    ...
    async def _handle_initialization_failure(
        ...
        await self._execute_service_hooks(
            "investigation_init_failed",
            {"investigation_id": investigation_id, "error_message": error_message},
        )
    ...
    def get_service_statistics(self) -> Dict[str, Any]:
        ...
        try:
            if self.graph_selector and hasattr(
                self.graph_selector, "get_selection_stats"
            ):
                stats["graph_selector_stats"] = (
                    self.graph_selector.get_selection_stats()
                )
            else:
                stats["graph_selector_stats"] = {
                    "error": "Graph selector not available"
                }
        except Exception as e:
    ...
    def ensure_hybrid_graph_config(self) -> bool:
        ...
        try:
            ...
            flag_status = self.graph_selector.feature_flags.get_flag_status(
                "hybrid_graph_v1"
            )

            if not flag_status or flag_status.get("error"):
                logger.error(
                    f"hybrid_graph_v1 flag not properly configured: {flag_status}"
                )
                return False
