...
class ResourcePressureCalculator:
    ...
    def calculate_resource_pressure(
        ...
        if orchestrator_loops < self.warmup_loops:
            logger.debug(
                f"   🔄 Warmup period: {orchestrator_loops}/{self.warmup_loops} loops - pressure limited to 0.0"
            )
            return 0.0
        ...
        domain_penalty = self._calculate_domain_penalty(
            domain_pressure, orchestrator_loops
        )
    ...
    def _calculate_loop_pressure(
        self, orchestrator_loops: int, limits: SafetyLimits
    ) -> float:
    ...
    def _calculate_domain_pressure(
        self, domains_completed: int, limits: SafetyLimits
    ) -> float:
    ...
    def _calculate_time_pressure(
        ...
        try:
            ...
            time_budget_minutes = getattr(
                limits, "max_investigation_time_minutes", 60
            )  # 60 min default
            return elapsed_minutes / max(1, time_budget_minutes)
        except Exception:
            return 0.0

    def _calculate_domain_penalty(
        self, domain_pressure: float, orchestrator_loops: int
    ) -> float:
        """Calculate penalty for lack of domain progress"""
        # Only apply penalty if behind on domain progress and after warmup
        if (
            domain_pressure < self.domain_progress_threshold
            and orchestrator_loops > self.domain_check_loops
        ):
    ...
    def _log_pressure_breakdown(
        ...
        logger.debug(
            f"   📈 Hybrid Intelligence resource pressure: {overall_pressure:.3f}"
        )
        logger.debug(
            f"      Loops: {loop_pressure:.3f} ({orchestrator_loops}/{limits.max_orchestrator_loops})"
        )
        logger.debug(
            f"      Tools: {tool_pressure:.3f} ({tools_used}/{limits.max_tool_executions})"
        )
        logger.debug(
        ...
        logger.debug(
            f"   Resource monitoring: Real-time utilization tracking for AI optimization"
        )
