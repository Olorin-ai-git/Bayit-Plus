...
def create_hybrid_initial_state(
    ...
    logger.debug(
        f"🧠 Creating hybrid investigation state with AI intelligence tracking"
    )
        ...
        **{
            **base_state,
            ...
            "routing_explanations": [
                "Investigation initialized with hybrid intelligence"
            ],
            "confidence_factors": {
    ...
    logger.info(
        f"   Initial confidence: {initial_confidence} ({confidence_level.value})"
    )
    ...
    override = SafetyOverride(
        ...
        metrics_at_override={
            ...
            "investigation_completeness": (
                state["ai_decisions"][-1].investigation_completeness
