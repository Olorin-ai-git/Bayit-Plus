...
class SafetyThresholdManager:
    ...
    def get_time_concern_severity(
        self, elapsed_minutes: float, max_minutes: int
    ) -> str:
        """
    ...
    def should_force_termination_for_safety_overrides(
        self, override_count: int
    ) -> bool:
        """
        ...
        """
        should_terminate = (
            override_count >= self.config.max_safety_overrides_before_termination
        )

        if should_terminate:
            logger.warning(
                f"🚨 Safety override limit exceeded: {override_count} >= {self.config.max_safety_overrides_before_termination}"
            )
    ...
    def should_trigger_rollback_for_override_rate(self, override_rate: float) -> bool:
        ...
        """
        should_rollback = (
            override_rate > self.config.safety_override_rate_rollback_threshold
        )

        if should_rollback:
            logger.warning(
                f"🚨 Safety override rate threshold exceeded: {override_rate:.2%} > {self.config.safety_override_rate_rollback_threshold:.2%}"
            )
    ...
    def log_current_thresholds(self) -> None:
        ...
        logger.debug(
            f"   Max safety overrides: {self.config.max_safety_overrides_before_termination}"
        )
        logger.debug(
            f"   Override rate rollback: {self.config.safety_override_rate_rollback_threshold:.1%}"
        )
        logger.debug(
            f"   Warning threshold: {self.config.warning_threshold_percentage:.1%}"
        )
        logger.debug(
            f"   Critical threshold: {self.config.critical_threshold_percentage:.1%}"
        )
        logger.debug(
            f"   Critical pressure: {self.config.critical_pressure_threshold:.1%}"
        )
