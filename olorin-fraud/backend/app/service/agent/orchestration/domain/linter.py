...
def _lint_cross_domain_consistency(domains: List[DomainResult]) -> List[str]:
    ...
    if len(set(val for _, val in public_values)) > 1:
        ...
        errors.append(
            f"is_public inconsistency: {values_str} (should be deterministic)"
        )
    ...
    if len(high_scores) > 2 and len(insufficient_domains) > 0:
        errors.append(
            f"Suspicious: {len(high_scores)} high scores but {len(insufficient_domains)} insufficient domains"
        )

    return errors


def _lint_aggregation_safety(
    domains: List[DomainResult], final_risk: Optional[float]
) -> List[str]:
        ...
        if not numeric_scores:
            ...
            if numeric_scores:
                ...
                if final_risk < min_score * 0.5:  # Shouldn't drop too far below minimum
                    errors.append(
                        f"final_risk {final_risk:.3f} suspiciously low vs domain range [{min_score:.3f}, {max_score:.3f}]"
                    )

    return errors


def _lint_rendering_safety(
    final_risk: Optional[float], domains: List[DomainResult]
) -> List[str]:
    ...
    if final_risk is None:
        ...
        for domain in domains:
            if domain.score is None and domain.status == "OK":
                errors.append(
                    f"Domain {domain.name} has status=OK but score=None (renderer risk)"
                )
    ...
    for domain in domains:
        if domain.score is None:
            ...
            if not any(
                ...
                for term in ["insufficient", "no", "limited", "unavailable"]
            ):
                errors.append(
                    f"Domain {domain.name} score=None but narrative doesn't indicate insufficient evidence"
                )
...
def _lint_state_consistency(
    state: Dict[str, Any], domains: List[DomainResult], final_risk: Optional[float]
) -> List[str]:
    ...
    for domain in domains:
        ...
        if finding_score != domain.score:
            errors.append(
                f"Domain {domain.name}: finding.risk_score ({finding_score}) != domain.score ({domain.score})"
            )
    ...
    float_none_errors = [
        ...
        if "float() argument must be a string or a real number, not 'NoneType'"
        in str(e)
    ]
    if float_none_errors:
        errors.append(
            f"CRITICAL: float(None) crashes detected in state.errors: {len(float_none_errors)} occurrences"
        )
...
def lint_specific_user_regressions(
    domains: List[DomainResult], final_risk: Optional[float]
) -> Dict[str, bool]:
    """
    ...
    if logs_domain:
        if (
            "low risk" in logs_domain.narrative.lower()
            and logs_domain.score
            and logs_domain.score > 0.4
        ):
    ...
    if network_domain:
        if (
            network_domain.status == "INSUFFICIENT_EVIDENCE"
            or "insufficient" in network_domain.narrative.lower()
        ) and network_domain.score:
