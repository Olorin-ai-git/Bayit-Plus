...
async def create_resilient_memory():
    ...
    try:
        ...
        logger.info(
            "🛡️ LangGraph RedisSaver not available - trying AsyncRedisSaver with mock"
        )
        ...
        try:
            ...
            logger.info(
                "🛡️ Using AsyncRedisSaver (mock/real IPS cache determined by TEST_MODE)"
            )
            return memory

        except Exception as e:
            logger.warning(
                f"🛡️ AsyncRedisSaver failed ({str(e)}) - falling back to MemorySaver"
            )
            from langgraph.checkpoint.memory import MemorySaver
        ...
        logger.warning(
            f"🛡️ Redis unavailable ({str(e)}) - falling back to MemorySaver for bulletproof operation"
        )
        from langgraph.checkpoint.memory import MemorySaver
...
async def create_parallel_agent_graph(use_enhanced_tools=False):
    ...
    logger.info(
        "Creating parallel graph with structured agents and RecursionGuard protection"
    )
...
async def create_sequential_agent_graph(use_enhanced_tools=False):
    ...
    builder.add_conditional_edges(
        ...
        {"tools": "tools", "network_agent": "network_agent"},
    )
...
async def create_and_get_agent_graph(
    ...
    if investigation_id:
        try:
            ...
            logger.info(
                f"🧠 Using Hybrid Intelligence Graph selection for investigation {investigation_id}"
            )
            ...
            logger.warning(
                "🧠 Hybrid system not available, falling back to traditional graph selection"
            )
        except Exception as e:
            logger.error(
                f"🧠 Hybrid graph selection failed: {e}, falling back to traditional selection"
            )

    # Traditional graph selection as fallback
    logger.info(
        f"Creating traditional agent graph: parallel={parallel}, enhanced_tools={use_enhanced_tools}, subgraphs={use_subgraphs}"
    )

    # Use subgraph architecture if requested (Phase 2)
    if use_subgraphs:
        return await create_modular_graph_with_subgraphs(
    ...
    if parallel:
        ...
        return await create_sequential_agent_graph(
            use_enhanced_tools=use_enhanced_tools
        )
...
def _get_configured_tools():
    ...
    try:
        ...
        logger.debug(
            f"Graph builder database config: DATABASE_PROVIDER={database_provider}, USE_POSTGRES={use_postgres}"
        )
        ...
        if database_provider == "snowflake":
            ...
            if not has_database_query:
                ...
                logger.debug(
                    f"Graph builder PostgreSQL env vars: host={bool(postgres_host)}, port={postgres_port}, db={bool(postgres_database)}, user={bool(postgres_user)}, password={'***' if postgres_password else None}"
                )
        ...
        has_database_query = any(t.name == "database_query" for t in tools)
        has_snowflake_query = any(
            isinstance(t, SnowflakeQueryTool)
            or (hasattr(t, "name") and t.name == "snowflake_query_tool")
            for t in tools
        )
        ...
        if not has_database_query and not has_snowflake_query:
            logger.error(
                "❌ CRITICAL: No database tool available! LLM cannot query database!"
            )
        elif database_provider == "snowflake" and has_snowflake_query:
            logger.info(
                f"✅ SnowflakeQueryTool is available (DATABASE_PROVIDER=snowflake, total {len(tools)} tools)"
            )
        elif has_database_query:
            logger.info(
                f"✅ database_query tool is available (total {len(tools)} tools)"
            )
        elif has_snowflake_query:
            logger.info(
                f"✅ SnowflakeQueryTool is available as fallback (total {len(tools)} tools)"
            )
        else:
            logger.warning(
                f"⚠️ Database tool availability unclear: database_query={has_database_query}, snowflake={has_snowflake_query}, provider={database_provider}"
            )
        ...
        logger.info(
            f"Graph builder loaded {len(tools)} tools including {threat_tools_count} threat intelligence tools and {mcp_tools_count} MCP client tools"
        )
        ...
        logger.error(
            f"Failed to get tools from registry, falling back to basic tools: {e}"
        )
    ...
    if use_enhanced:
        try:
            ...
            tool_node = EnhancedToolNode(
                healthy_tools, investigation_id=investigation_id
            )
            logger.info(
                f"✅ Created enhanced tool node with {len(healthy_tools)} healthy tools"
            )
            return tool_node

        except Exception as e:
            logger.error(
                f"Failed to create enhanced tool node: {e}, falling back to standard ToolNode"
            )
...
async def create_mcp_enhanced_graph(
    ...
    """
    logger.info(
        f"Creating MCP-enhanced graph: parallel={parallel}, enhanced={use_enhanced_tools}, mcp={use_mcp}"
    )
