...
async def orchestrator_conditional_routing(state: OrchestratorState) -> str:
    ...
    try:
        ...
        logger.info(
            f"🧭 Orchestrator routing decision: strategy={strategy}, agents={agents_to_activate}"
        )
        ...
        logger.warning(
            f"⚠️ Unknown orchestration strategy {strategy}, falling back to network_agent"
        )
        return "network_agent"

    except Exception as e:
        logger.error(
            f"🚨 Orchestrator routing failed: {str(e)}, routing to network_agent"
        )
        return "network_agent"


async def orchestration_decision_node(
    state: OrchestratorState, config
) -> OrchestratorState:
    """
    ...
    try:
        ...
        if orchestration_message:
            ...
            try:
                ...
                state["active_strategy"] = orchestration_data.get(
                ...
                logger.info(
                    f"✅ Applied orchestration decision: {state['active_strategy']}"
                )
...
async def create_orchestrator_driven_graph(
    use_enhanced_tools: bool = True, orchestration_mode: str = "ai_driven"
) -> StateGraph:
    """
    ...
    """
    logger.info(
        f"🏗️ Creating orchestrator-driven graph: tools={use_enhanced_tools}, mode={orchestration_mode}"
    )
    ...
    try:
        ...
        logger.info(
            "✅ Orchestrator-driven graph compiled successfully with AI coordination"
        )
        return graph
...
async def create_hybrid_orchestration_graph(
    parallel_execution: bool = True, use_enhanced_tools: bool = True
) -> StateGraph:
    """
    ...
    try:
        ...
        tools = _get_orchestrator_tools()
        tool_node = (
            await _create_orchestrator_enhanced_tools(tools)
            if use_enhanced_tools
            else ToolNode(tools)
        )
        builder.add_node("tools", tool_node)
        ...
        graph = builder.compile(
            checkpointer=memory, interrupt_before=["tools"], debug=True
        )
...
def _filter_working_tools(tools):
    ...
    try:
        from app.service.agent.orchestration.graph_builder import (
            _filter_working_tools as filter_tools,
        )
...
async def get_orchestrator_graph(
    ...
    """
    logger.info(
        f"🏭 Creating orchestrator graph: type={orchestration_type}, mode={execution_mode}"
    )

    if orchestration_type == "ai_driven":
        return await create_orchestrator_driven_graph(
            use_enhanced_tools=enhanced_tools, orchestration_mode="ai_driven"
        )
        ...
        return await create_and_get_agent_graph(
            parallel=(execution_mode == "parallel"), use_enhanced_tools=enhanced_tools
        )
