...
class HumanReviewRequest:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "priority": (
                self.priority.value
                if isinstance(self.priority, ReviewPriority)
                else self.priority
            ),
...
class HumanReviewManager:
    ...
    async def request_human_review(
        self, state: Dict[str, Any], reason: EscalationReason, priority: str = "medium"
    ) -> HumanReviewRequest:
        """
        ...
        logger.info(
            f"Created human review request {request_id} for investigation {investigation_id}"
        )

        return request

    async def wait_for_human_response(
        self, request: HumanReviewRequest, timeout: Optional[int] = None
        ...
        logger.warning(
            f"Timeout waiting for human response to request {request.request_id}"
        )
    ...
    def _recommend_action(
        self, risk_assessment: Dict[str, float], agent_findings: List[Dict[str, Any]]
    ) -> str:
    ...
    def _calculate_timeout(self, priority: str) -> int:
        ...
        timeouts = {"critical": 5, "high": 15, "medium": 30, "low": 60}
    ...
    def should_escalate(self, state: Dict[str, Any]) -> bool:
        ...
        if (
            ...
            and risk_score >= self.escalation_thresholds["risk_score"]
        ):
            return True

        # Check confidence threshold
        if (
            state.get("confidence_level", 1.0)
            <= self.escalation_thresholds["confidence"]
        ):
            return True
        ...
        if (
            ...
            and complexity_score >= self.escalation_thresholds["complexity"]
        ):
            return True
    ...
    def _has_ambiguous_signals(self, state: Dict[str, Any]) -> bool:
        ...
        if positive_signals > 0 and negative_signals > 0:
            ratio = min(positive_signals, negative_signals) / max(
