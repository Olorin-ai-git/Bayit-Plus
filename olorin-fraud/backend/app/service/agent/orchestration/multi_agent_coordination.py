...
class AgentCapabilities:
    ...
    def availability_score(self) -> float:
        ...
        speed_factor = 1.0 / (
            1.0 + self.avg_response_time / 10.0
        )  # Normalize to 10s baseline
...
class BaseCoordinationStrategy(ABC):
    ...
    async def coordinate(
        self, agents: List[AgentCapabilities], task: CoordinationTask
    ) -> Dict[str, Any]:
...
class ParallelStrategy(BaseCoordinationStrategy):
    ...
    async def coordinate(
        ...
        logger.info(
            f"Executing task {task.task_id} in parallel with {num_agents} agents"
        )
...
class SequentialStrategy(BaseCoordinationStrategy):
    ...
    async def coordinate(
        ...
        capable_agents.sort(
            key=lambda a: (
                len(set(a.specializations) & set(task.required_capabilities)),
                a.success_rate,
            ),
            reverse=True,
        )
...
class CommitteeStrategy(BaseCoordinationStrategy):
    ...
    async def coordinate(
        ...
        logger.info(
            f"Committee of {committee_size} agents voting on task {task.task_id}"
        )
        ...
        return {
            ...
            "confidence": decision_counts[final_decision]
...
class LoadBalancedStrategy(BaseCoordinationStrategy):
    ...
    async def coordinate(
        ...
        logger.info(
            f"Load-balanced task {task.task_id} to {best_agent.name} (load: {best_agent.current_load}/{best_agent.max_concurrent_tasks})"
        )
...
class AgentPool:
    ...
    def register_agent(self, agent: AgentCapabilities):
        ...
        logger.info(
            f"Registered agent {agent.name} with specializations: {agent.specializations}"
        )
    ...
    def get_available_agents(self) -> List[AgentCapabilities]:
        ...
        return [
            a for a in self.agents.values() if a.current_load < a.max_concurrent_tasks
        ]
...
class CoordinatedAgentOrchestrator:
    ...
    async def investigate_fraud_case(self, case_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(
            f"Starting coordinated investigation for case: {case_data.get('case_id', 'unknown')}"
        )
        ...
        for task in gathering_tasks:
            result = await self.agent_pool.execute_task(
                task, CoordinationStrategy.PARALLEL
            )
        ...
        risk_result = await self.agent_pool.execute_task(
            risk_task, CoordinationStrategy.COMMITTEE
        )
        ...
        decision_result = await self.agent_pool.execute_task(
            decision_task, CoordinationStrategy.SEQUENTIAL
        )
    ...
    async def optimize_agent_allocation(self):
        ...
        for agent in self.agent_pool.agents.values():
            ...
            if agent.success_rate < 0.85:
                ...
                logger.info(
                    f"Reduced capacity for {agent.name} due to low success rate"
                )
                ...
                logger.info(
                    f"Increased capacity for {agent.name} due to high performance"
                )
...
def create_coordinated_investigation_graph() -> StateGraph:
    ...
    async def coordinate_investigation(state: CoordinatedState) -> CoordinatedState:
        ...
        state["messages"].append(
            AIMessage(
                content=f"Coordinated investigation completed with {len(results['investigation_phases'])} phases"
            )
        )
...
async def example_coordinated_investigation():
    ...
    case_data = {
        ...
        "device": {"fingerprint": "abc123", "os": "iOS 17.0", "browser": "Safari"},
        "network": {
            "ip": "192.168.1.100",
            "location": "San Francisco, CA",
            "isp": "Comcast",
        },
        "logs": {"login_attempts": 5, "failed_attempts": 2, "session_duration": 3600},
    }
