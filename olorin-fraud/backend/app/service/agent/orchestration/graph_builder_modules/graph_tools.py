...
class GraphToolsManager:
    ...
    def get_all_tools(self) -> List[Any]:
        ...
        try:
            ...
            self.logger.debug(
                f"Database config: DATABASE_PROVIDER={database_provider}, USE_POSTGRES={use_postgres}"
            )

            # Only create DatabaseQueryTool if DATABASE_PROVIDER is PostgreSQL
            if database_provider == "snowflake":
                self.logger.info(
                    "✅ DATABASE_PROVIDER=snowflake - Using SnowflakeQueryTool"
                )
    ...
    def process_tool_results(self, tool_results: List[Any]) -> Dict[str, Any]:
        ...
        for result in tool_results:
            if hasattr(result, "name"):
                ...
                processed[tool_name] = {"status": "success", "result": result}
