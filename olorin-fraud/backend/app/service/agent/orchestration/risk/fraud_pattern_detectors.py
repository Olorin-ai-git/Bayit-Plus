...
def detect_amount_clustering(
    results: List[Dict[str, Any]], min_repeats: int = 2, time_window_minutes: int = 10
) -> Optional[Dict[str, Any]]:
    """
        ...
        if isinstance(result, dict):
            ...
            if datetime_str and amount:
                try:
                    ...
                    transactions.append(
                        {"datetime": tx_datetime, "amount": amount_float}
                    )
                except Exception as e:
                    ...
                    clusters.append(
                        {
                            ...
                            "time_span_minutes": (
                                window_txs[-1]["datetime"] - window_txs[0]["datetime"]
                            ).total_seconds()
                            / 60,
...
def detect_ip_rotation(
    results: List[Dict[str, Any]], time_window_hours: int = 4
) -> Optional[Dict[str, Any]]:
    """
    ...
    for device, data in device_ip_map.items():
        if len(data["ips"]) >= 2 and len(data["transactions"]) >= 2:
            ...
            # Calculate time span
            time_span = (
                transactions[-1]["datetime"] - transactions[0]["datetime"]
...
def calculate_fraud_pattern_score(
    results: List[Dict[str, Any]],
    ...
    velocity_score = (
        0.8
        if velocity_burst and velocity_burst.get("severity") == "high"
        else 0.5 if velocity_burst else 0.0
    )

    clustering_score = (
        0.7
        if amount_clustering and amount_clustering.get("severity") == "high"
        else 0.4 if amount_clustering else 0.0
    )

    rotation_score = (
        0.6
        if ip_rotation and ip_rotation.get("severity") == "high"
        else 0.3 if ip_rotation else 0.0
    )

    mismatch_score = (
        0.5
        if device_ip_mismatch and device_ip_mismatch.get("severity") == "high"
        else 0.2 if device_ip_mismatch else 0.0
    )
...
def has_strong_fraud_patterns(
    ...
    if len(results) >= 5:
        ...
        if fraud_pattern_score >= 0.5:
            logger.info(
                f"✅ Strong fraud patterns detected: score={fraud_pattern_score:.3f}"
            )
            logger.info(
                f"   Patterns: velocity={pattern_details['velocity_burst'] is not None}, "
