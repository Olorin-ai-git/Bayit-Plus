...
class DeviceAnalysisSubgraph(BaseDomainSubgraph):
    ...
    async def validate_device_data_node(self, state: DomainState) -> DomainState:
        ...
        if not self.validate_domain_data(state):
            return self.handle_domain_error(ValueError("Invalid device data"), state)
    ...
    async def calculate_device_risk_node(self, state: DomainState) -> DomainState:
        ...
        confidence = (
            0.8 if state["domain_findings"].get("validation") == "passed" else 0.3
        )
...
class NetworkAnalysisSubgraph(BaseDomainSubgraph):
    ...
    async def validate_network_data_node(self, state: DomainState) -> DomainState:
        ...
        if not self.validate_domain_data(state):
            return self.handle_domain_error(ValueError("Invalid network data"), state)
...
class LocationAnalysisSubgraph(BaseDomainSubgraph):
    ...
    def create_graph(self) -> StateGraph:
        ...
        graph.add_node(
            "analyze_geographic_patterns", self.analyze_geographic_patterns_node
        )
    ...
    async def validate_location_data_node(self, state: DomainState) -> DomainState:
        ...
        if not self.validate_domain_data(state):
            return self.handle_domain_error(ValueError("Invalid location data"), state)
    ...
    async def detect_impossible_travel_node(self, state: DomainState) -> DomainState:
        ...
        impossible_travel = {"detected": False, "locations": [], "time_delta": None}
...
class LogsAnalysisSubgraph(BaseDomainSubgraph):
    ...
    def create_graph(self) -> StateGraph:
        ...
        graph.add_node(
            "detect_suspicious_activity", self.detect_suspicious_activity_node
        )
    ...
    async def validate_logs_data_node(self, state: DomainState) -> DomainState:
        """Validate logs data."""
        if not self.validate_domain_data(state):
            return self.handle_domain_error(ValueError("Invalid logs data"), state)
...
class SubgraphOrchestrator:
    ...
    def _calculate_overall_risk(
        self, risk_scores: Dict[str, Dict[str, float]]
    ) -> float:
        """
    ...
    async def run_sequential_investigation(
        self, state: Dict[str, Any], domain_order: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        """
