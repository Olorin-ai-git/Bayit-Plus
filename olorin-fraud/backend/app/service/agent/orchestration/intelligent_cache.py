    ...
    async def put(
        ...
        async with self.lock:
            ...
            while (
                ...
                or self.total_size_bytes + size_bytes > self.max_memory_bytes
            ):
                if not self.cache:
...
class IntelligentCacheManager:
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "metrics": {
                "l1_hit_rate": (
                    self.metrics["l1_hits"] / l1_total if l1_total > 0 else 0
                ),
                "l2_hit_rate": (
                    self.metrics["l2_hits"] / l2_total if l2_total > 0 else 0
                ),
    ...
    async def execute_with_cache(
        ...
        try:
            ...
            if execution_time > 1.0:  # Cache if took more than 1 second
                ...
                logger.info(
                    f"Cached result for tool '{tool_name}' (took {execution_time:.2f}s)"
                )
