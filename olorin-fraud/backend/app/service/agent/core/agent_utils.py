...
def get_investigation_params(metadata: dict) -> tuple:
    ...
    investigation_id = metadata.get("investigationId") or metadata.get(
        "investigation_id"
    )
...
def create_standard_error_response(
    agent_name: str, error_msg: str, response_key: str = "risk_assessment"
) -> dict:
    ...
    return {"messages": [AIMessage(content=json.dumps({response_key: error_response}))]}


def log_agent_execution(
    agent_name: str, investigation_id: str, entity_id: str, action: str = "start"
):
    """Log agent execution events."""
    if action == "start":
        logger.info(
            f"🚀 Starting {agent_name} - Investigation: {investigation_id}, Entity: {entity_id}"
        )
