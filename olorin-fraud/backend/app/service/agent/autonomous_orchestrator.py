...
class StructuredOrchestrator:
    ...
    async def orchestrate_investigation(
        ...
        try:
            ...
            execution_plan = self._create_execution_plan(
                orchestration_decision, context
            )
            ...
            journey_tracker.track_node_execution(
                ...
                output_state={
                    ...
                    "agents_coordinated": len(
                        orchestration_decision.agents_to_activate
                    ),
            ...
            journey_tracker.track_node_execution(
                ...
                output_state={"orchestration": "failed", "error": str(e)},
                duration_ms=0,
    ...
    async def _make_orchestration_decision(
        self, investigation_id: str, entity_type: str, entity_id: str, context
        ...
        try:
            ...
            messages = [
                ...
                {
                    "role": "user",
                    "content": f"Analyze investigation for {entity_type} entity {entity_id} and provide orchestration strategy.",
                },
            ]
            ...
            logger.info(
                f"🧠 AI Orchestration Decision: {decision.strategy.value} with confidence {decision.confidence_score}"
            )
            ...
            logger.warning(
                f"⚠️ AI orchestration decision failed, using fallback: {str(e)}"
            )
    ...
    def _create_orchestration_prompt(
        ...
        try:
            ...
            logger.info(
                f"🧠 Generated sophisticated orchestration prompt for {entity_type} {entity_id}"
            )
            return sophisticated_prompt

        except Exception as e:
            logger.warning(
                f"⚠️ Sophisticated prompt generation failed: {str(e)}, using fallback"
            )
    ...
    def _parse_orchestrator_response(
        ...
        try:
            ...
            return OrchestrationDecision(
                ...
                confidence_score=(
                    float(response_data.get("confidence_score", 0.6))
                    if response_data.get("confidence_score") is not None
                    else 0.6
                ),
    ...
    def _create_execution_plan(
        self, decision: OrchestrationDecision, context
    ) -> Dict[str, Any]:
    ...
    async def _execute_orchestrated_investigation(
        ...
        try:
            ...
            if decision.strategy in [
                OrchestrationStrategy.COMPREHENSIVE,
                ...
                for agent_name in decision.execution_order:
                    try:
                        ...
                        if self._should_stop_sequential_execution(result):
                            logger.info(
                                f"🛑 Stopping sequential execution after {agent_name} due to sufficient findings"
                            )
                            break

                    except Exception as e:
                        logger.warning(
                            f"⚠️ Sequential agent {agent_name} failed: {str(e)}"
                        )
                        orchestration_results["failures"].append(
                            {
                                "agent": agent_name,
    ...
    async def _execute_agent_with_bulletproof(
        self, investigation_id: str, agent_name: str, context, state, config
    ) -> Dict[str, Any]:
    ...
    async def _import_device_agent(self, state, config):
        ...
        return await structured_device_agent(state, config)
    ...
    async def _consolidate_orchestration_results(
        ...
        consolidated_results = {
            ...
            "bulletproof_recovery_count": len(
                orchestration_results.get("failures", [])
            ),
        ...
        if successful_count > 0:
            consolidated_results["confidence_score"] = (
                total_confidence / successful_count
            )
        ...
        consolidated_results["orchestration_metadata"] = {
            ...
            "recovery_strategies_used": [
                f["recovery"] for f in orchestration_results.get("failures", [])
            ],
            "handoff_success_rate": (
                len([h for h in self.handoff_history if h.success])
                / len(self.handoff_history)
    ...
    def _create_bulletproof_fallback_result(
        self, investigation_id: str, error_message: str
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "risk_score": 0.5,
            "key_findings": [
                "Orchestrator encountered unexpected error - investigation partially completed"
            ],
            ...
            if investigation_id in self.orchestration_state:
                ...
                if self.handoff_history:
                    recent_handoffs = [
                        ...
                        if hasattr(h, "context_data")
                        and h.context_data.get("investigation_id") == investigation_id
                    ]
                    history.extend(
                        [f"agent_{h.to_agent}_executed" for h in recent_handoffs[-5:]]
                    )
