...
class AgentFactory:
    ...
    def __init__(
        ...
        if self.enable_rag:
            try:
                ...
                logger.info(
                    "AgentFactory initialized with RAG capabilities and tool recommender"
                )
            except Exception as e:
                logger.warning(
                    f"RAG orchestrator/tool recommender initialization failed: {e}"
                )
    ...
    def create_agent(
        self, domain: str, tools: List[Any], enable_rag: Optional[bool] = None
    ):
        ...
        if self.is_tool_recommender_available() and investigation_context:
            try:
                ...
                if selection_time_ms > 100:
                    ...
                    logger.debug(
                        f"🚀 Tool selection completed in {selection_time_ms:.1f}ms for {domain}"
                    )
                ...
                logger.warning(
                    f"Tool recommendation failed for {domain}, using fallback: {str(e)}"
                )
    ...
    def _update_performance_stats(self, selection_time_ms: float) -> None:
        ...
        total_selections = (
            self.stats["tool_recommendations_used"] + self.stats["static_tools_used"]
        )

        if total_selections > 0:
            new_avg = (
                (current_avg * (total_selections - 1)) + selection_time_ms
            ) / total_selections
...
def get_tool_recommender_factory() -> ToolRecommenderAgentFactory:
    ...
    if _tool_recommender_factory_instance is None:
        _tool_recommender_factory_instance = ToolRecommenderAgentFactory(
            enable_rag=True
        )
...
async def create_agent_with_intelligent_tools(
    ...
    try:
        ...
        return create_rag_agent(
            domain, fallback_tools, enable_rag=enable_rag if enable_rag else None
        )
