...
class ServiceResilienceManager:
    ...
    async def detect_service_failures(
        ...
        """
        logger.info(
            f"🔍 Detecting service failures for {service_name} using {detection_mode.value} mode"
        )
        ...
        try:
            ...
            if detection_mode in [
                FailureDetectionMode.PROACTIVE,
                FailureDetectionMode.HYBRID,
            ]:
                health = await self._proactive_health_check(service_name, health)

            if detection_mode in [
                FailureDetectionMode.PREDICTIVE,
                FailureDetectionMode.HYBRID,
            ]:
            ...
            logger.info(
                f"✅ Service {service_name} health: {health.health_score:.2f} (degradation: {health.degradation_level:.2f})"
            )
            return health

        except Exception as e:
            logger.error(
                f"🚨 Failed to detect service failures for {service_name}: {str(e)}"
            )
    ...
    async def handle_llm_api_failures(
        ...
        """
        logger.info(
            f"🤖 Handling LLM API failure for {primary_llm} in investigation {investigation_id}"
        )
            ...
            for strategy in fallback_strategies:
                try:
                    ...
                    if await self._check_strategy_prerequisites(strategy, context):
                        logger.info(
                            f"🔄 Attempting fallback strategy: {strategy.description}"
                        )

                        # Execute fallback strategy
                        result = await strategy.handler(
                            investigation_id, prompt, context
                        )

                        if result and result.get("success", False):
                            # Record successful fallback
                            await self._record_fallback_success(
                                strategy, investigation_id
                            )

                            logger.info(
                                f"✅ LLM fallback successful with {strategy.strategy_id}"
                            )
                            return {
                                "success": True,
                    ...
                    logger.warning(
                        f"⚠️ Fallback strategy {strategy.strategy_id} failed: {str(fallback_error)}"
                    )
                    continue

            # All fallback strategies failed, use rule-based response
            return await self._generate_rule_based_llm_response(
                investigation_id, prompt, context
            )
    ...
    async def manage_database_connectivity(
        ...
        try:
            ...
            db_health = await self.detect_service_failures(
                "database", FailureDetectionMode.PROACTIVE
            )
            ...
            if db_health.is_healthy and db_health.health_score > 0.8:
                ...
                if use_cache and operation.startswith("read"):
                    await self._cache_database_result(
                        investigation_id, operation, result
                    )
            ...
            if use_cache:
                cached_result = await self._get_cached_database_result(
                    investigation_id, operation
                )
                if cached_result:
                    return cached_result

            return await self._create_database_emergency_response(
                investigation_id, operation
            )

    async def ensure_websocket_resilience(
        self, investigation_id: str, message: Dict[str, Any], priority: str = "normal"
    ) -> bool:
        """
        ...
        """
        logger.info(
            f"📡 Ensuring WebSocket resilience for investigation {investigation_id}"
        )

        try:
            # Check WebSocket service health
            ws_health = await self.detect_service_failures(
                "websocket", FailureDetectionMode.REACTIVE
            )
        ...
        try:
            ...
            if cached_data:
                ...
                if await self._is_cache_valid(cached_data, cache_strategy):
                    ...
                    cached_data.quality_score = await self._calculate_cache_quality(
                        cached_data
                    )

                    logger.info(
                        f"✅ Cache hit for {data_type} (quality: {cached_data.quality_score:.2f})"
                    )
                    return {
    ...
    async def cache_investigation_data(
        ...
        """
        logger.info(
            f"💾 Caching investigation data: {data_type} with {cache_strategy.value} strategy"
        )
        ...
        try:
            ...
            logger.info(
                f"✅ Cached {data_type} until {expiry.strftime('%Y-%m-%d %H:%M:%S')}"
            )
            return True
    ...
    def _initialize_circuit_breakers(self):
        ...
        for service in services:
            self.circuit_breakers[service] = CircuitBreaker(
                service_name=service, failure_threshold=5, recovery_timeout=60
            )

    async def _initialize_service_health_record(
    ...
    async def _proactive_health_check(
        self, service_name: str, health: ServiceHealth
    ) -> ServiceHealth:
        ...
        try:
            ...
            if service_name == "database":
                ...
                health_check_result = await self._check_generic_service_health(
                    service_name
                )
            ...
            health.error_rate = (
                health.failure_count / total_checks if total_checks > 0 else 0
            )
    ...
    async def _predictive_failure_analysis(
        self, service_name: str, health: ServiceHealth
    ) -> ServiceHealth:
    ...
    async def _calculate_health_score(self, health: ServiceHealth) -> float:
        ...
        success_rate = (
            health.success_count / total_requests if total_requests > 0 else 1.0
        )
        ...
        health_score = (
            success_rate * 0.5 + response_factor * 0.3 + consecutive_penalty * 0.2
        )
    ...
    async def _fallback_anthropic_to_openai(
        self, investigation_id: str, prompt: str, context: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _fallback_openai_to_anthropic(
        self, investigation_id: str, prompt: str, context: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _rule_based_decision_making(
        self, investigation_id: str, prompt: str, context: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _ensure_websocket_retry_task(self):
        ...
        if (
            not hasattr(self, "_websocket_retry_task")
            or self._websocket_retry_task.done()
        ):
            self._websocket_retry_task = asyncio.create_task(
                self._websocket_retry_loop()
            )
    ...
    async def _is_cache_valid(
        self, cached_data: CachedData, strategy: CacheStrategy
    ) -> bool:
    ...
    async def _cleanup_expired_cache(self):
        ...
        expired_keys = [
            key
            for key, cached_data in self.cached_data.items()
    ...
    async def _check_strategy_prerequisites(
        self, strategy: FallbackStrategy, context: Dict[str, Any]
    ) -> bool:
    ...
    async def _record_fallback_success(
        self, strategy: FallbackStrategy, investigation_id: str
    ):
        """Record successful fallback strategy execution"""
        strategy.success_rate = min(1.0, strategy.success_rate + 0.01)
        logger.info(
            f"📈 Fallback strategy {strategy.strategy_id} success rate: {strategy.success_rate:.2f}"
        )
    ...
    async def _generate_rule_based_llm_response(
        self, investigation_id: str, prompt: str, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "processing_time": 100,
            "metadata": {"all_llm_apis_failed": True, "fallback_type": "rule_based"},
        }

    async def _create_emergency_llm_response(
        self, investigation_id: str, prompt: str
    ) -> Dict[str, Any]:
    ...
    async def _get_cached_database_result(
        self, investigation_id: str, operation: str
    ) -> Optional[Dict[str, Any]]:
        """Get cached database result"""
        cache_key = self._generate_cache_key(investigation_id, f"db_{operation}")
        cached_data = self.cached_data.get(cache_key)

        if cached_data and await self._is_cache_valid(
            cached_data, cached_data.cache_strategy
        ):
            return {
                "success": True,
                "data": cached_data.data,
    ...
    async def _cache_database_result(
        self, investigation_id: str, operation: str, result: Dict[str, Any]
    ):
    ...
    async def _create_database_emergency_response(
        self, investigation_id: str, operation: str
    ) -> Dict[str, Any]:
    ...
    async def _try_read_replica(
        self, operation: str, data: Dict[str, Any]
    ) -> Optional[Dict[str, Any]]:
        """Try to use read replica for database operations"""
        # Simulated read replica usage
        if operation.startswith("read"):
            return {"success": True, "data": data, "source": "read_replica"}
        return None

    async def _emergency_queue_websocket_message(
        self, investigation_id: str, message: Dict[str, Any]
    ):
