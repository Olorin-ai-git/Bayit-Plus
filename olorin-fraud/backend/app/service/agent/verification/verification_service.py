...
from .iterative_improver import ImprovementStrategy, IterationContext, IterativeImprover
...
from .verification_models import ModelPriority, VerificationModels
...
class LLMVerificationService:
    ...
    async def verify_response_with_retry(
        ...
        try:
            ...
            logger.error(
                f"❌ Verification pipeline failed for {verification_id}: {str(e)}"
            )
    ...
    async def _execute_verification_pipeline(
        ...
        for attempt in range(max_retries + 1):
            ...
            if (
                verification_result.result == VerificationResult.REQUIRES_RETRY
                and attempt < max_retries
                and llm_invoke_function is not None
            ):
                ...
                try:
                    ...
                    improved_response = await self._invoke_main_llm(
                        llm_invoke_function, improved_request
                    )
                    ...
                    if selected_strategy:
                        ...
                        verification_result.improvement_strategy = (
    ...
    async def _perform_single_verification(
        ...
        try:
            ...
            verification_prompt = self._create_verification_prompt(
                original_request=original_request, response=response, context=context
            )
            ...
            verification_response = VerificationResponse(
                result=(
                    ...
                    else VerificationResult.REQUIRES_RETRY
                ),
            ...
            if (
                verification_response.result == VerificationResult.VALID
                or verification_response.confidence_score >= 0.7
            ):
...
Be thorough but concise. Focus on accuracy, relevance, and completeness."""
    ...
    async def _invoke_main_llm(
        self, llm_invoke_function, improved_request: List[BaseMessage]
    ) -> str:
    ...
    def _generate_verification_id(
        self, original_request: List[BaseMessage], response: str
    ) -> str:
        ...
        try:
            ...
            if healthy_models == 0:
                ...
                health_results["issues"].append(
                    "No healthy verification models available"
                )
