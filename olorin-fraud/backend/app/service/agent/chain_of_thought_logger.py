...
class ChainOfThoughtLogger:
    ...
    def __init__(self, output_directory: Optional[Path] = None):
        ...
        logger.info(
            f"Initialized ChainOfThoughtLogger with directory: {self.output_directory}"
        )
    ...
    def start_agent_thinking(
        ...
        process_id = (
            f"{agent_name}_{investigation_id}_{int(datetime.now().timestamp())}"
        )
        ...
        logger.info(
            f"Started thought process tracking for {agent_name} on investigation {investigation_id}"
        )
        return process_id
    ...
    def log_reasoning_step(
        ...
        logger.debug(
            f"Logged reasoning step for {thought_process.agent_name}: "
    ...
    def log_tool_selection_reasoning(
        ...
        self._log_thinking_event(
            ...
            "tool_selection",
            {
                ...
                "rejected_tools": rejected_tools,
                "primary_reasoning": (
                    reasoning_chain[0] if reasoning_chain else "No reasoning provided"
                ),
        ...
        logger.debug(
            f"Logged tool selection for {thought_process.agent_name}: "
    ...
    def log_collaboration_thought(
        ...
        logger.debug(
            f"Logged collaboration thought for {thought_process.agent_name}: "
    ...
    def complete_agent_thinking(
        ...
        start_time = datetime.fromisoformat(
            thought_process.start_timestamp.replace("Z", "+00:00")
        )
        ...
        thought_process.performance_metrics.update(
            {
                ...
                "average_confidence": self._calculate_average_confidence(
                    thought_process.reasoning_steps
                ),
        ...
        self._log_thinking_event(
            ...
            "thinking_completed",
            {
                ...
                "average_confidence": thought_process.performance_metrics[
                    "average_confidence"
                ],
            },
        )
        ...
        logger.info(
            f"Completed thought process for {thought_process.agent_name}: "
    ...
    def get_agent_thinking_summary(self, process_id: str) -> Dict[str, Any]:
        ...
        recent_steps = (
            thought_process.reasoning_steps[-5:]
            if thought_process.reasoning_steps
            else []
        )
        ...
        return {
            ...
            "current_confidence": self._calculate_average_confidence(
                thought_process.reasoning_steps
            ),
    ...
    def generate_chain_of_thought_report(self, investigation_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            investigation_folder_path = folder_manager.get_investigation_folder(
                investigation_id
            )
        ...
        for search_path in search_paths:
            for file_path in search_path.glob(
                f"thought_process_{investigation_id}_*.json"
            ):
                try:
                    with open(file_path, "r") as f:
                        saved_process_data = json.load(f)
                        investigation_processes.append(
                            AgentThoughtProcess(**saved_process_data)
                        )
                except Exception as e:
                    logger.warning(
                        f"Failed to load saved thought process {file_path}: {e}"
                    )

        if not investigation_processes:
            return {
                "error": f"No thought processes found for investigation: {investigation_id}"
            }
        ...
        for process in investigation_processes:
            agent_summary = {
                ...
                "thinking_duration_ms": process.performance_metrics.get(
        ...
        report["cross_agent_analysis"] = self._perform_cross_agent_analysis(
            investigation_processes
        )
    ...
    def _assess_reasoning_complexity(
        self, thought_process: AgentThoughtProcess
    ) -> Dict[str, Any]:
        """Assess the complexity of agent's reasoning process"""

        reasoning_types = set(
            step.reasoning_type for step in thought_process.reasoning_steps
        )
        ...
        alt_considerations = sum(
            len(step.alternative_considerations)
    ...
    def _analyze_thinking_patterns(
        ...
        reasoning_sequence = [
            step.reasoning_type.value for step in thought_process.reasoning_steps
        ]
        reasoning_counts = {}
        for reasoning_type in reasoning_sequence:
            reasoning_counts[reasoning_type] = (
                reasoning_counts.get(reasoning_type, 0) + 1
            )

        # Analyze confidence patterns
        confidence_values = [
            step.confidence for step in thought_process.reasoning_steps
        ]
        ...
        return {
            "dominant_reasoning_types": sorted(
            ...
            "thinking_velocity": len(thought_process.reasoning_steps)
            / max(
                1,
                (
                    thought_process.performance_metrics.get("thinking_duration_ms")
                    or 1000
                )
                / 1000,
            ),
    ...
    def _extract_key_reasoning_chains(
        self, thought_process: AgentThoughtProcess
    ) -> List[Dict[str, Any]]:
    ...
    def _perform_cross_agent_analysis(
        self, processes: List[AgentThoughtProcess]
    ) -> Dict[str, Any]:
    ...
    def _extract_key_insights(self, processes: List[AgentThoughtProcess]) -> List[str]:
        ...
        avg_confidence = sum(
            self._calculate_average_confidence(p.reasoning_steps) for p in processes
        ) / len(processes)

        insights.append(
            f"Investigation involved {total_steps} reasoning steps across {len(processes)} agents"
        )
        insights.append(f"Average reasoning confidence was {avg_confidence:.2f}")

        # Find agents with exceptional performance
        for process in processes:
            agent_confidence = self._calculate_average_confidence(
                process.reasoning_steps
            )
            if agent_confidence > 0.8:
                insights.append(
                    f"{process.agent_name} demonstrated high confidence reasoning (avg: {agent_confidence:.2f})"
                )
        ...
        complex_processes = [
            p
            for p in processes
            if self._assess_reasoning_complexity(p)["complexity_level"] == "high"
        ]
        if complex_processes:
            agents = [p.agent_name for p in complex_processes]
            insights.append(
                f"Complex reasoning patterns observed in: {', '.join(agents)}"
            )
    ...
    def _save_thought_process(self, thought_process: AgentThoughtProcess) -> None:
        ...
        for step in process_dict["reasoning_steps"]:
            step["reasoning_type"] = (
                ...
                if hasattr(step["reasoning_type"], "value")
                else step["reasoning_type"]
            )
