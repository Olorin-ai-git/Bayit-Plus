...
class DocumentMetadata:
    ...
    entity_types: Set[str] = field(
        default_factory=set
    )  # device, location, network, etc.
    investigation_types: Set[str] = field(
        default_factory=set
    )  # behavioral, device, etc.
    ...
    def is_relevant_for_investigation(self, investigation_type: str) -> bool:
        ...
        return (
            investigation_type in self.investigation_types
            or not self.investigation_types
        )
...
class KnowledgeBase:
    ...
    async def add_document(
        ...
        try:
            ...
            self.logger.info(
                f"Added document {metadata.document_id} with {metadata.content_length} characters"
            )
    ...
    async def _chunk_semantic_split(
        self, content: str, document_id: str
    ) -> List[DocumentChunk]:
    ...
    async def _chunk_paragraph_based(
        self, content: str, document_id: str
    ) -> List[DocumentChunk]:
    ...
    async def _chunk_sentence_based(
        self, content: str, document_id: str
    ) -> List[DocumentChunk]:
    ...
    async def _generate_chunk_embedding(self, chunk: DocumentChunk) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to generate embedding for chunk {chunk.chunk_id}: {str(e)}"
            )
    ...
    async def search_chunks(
        ...
        try:
            ...
            cache_key = self._create_search_cache_key(
                query, entity_type, investigation_type, tags
            )
    ...
    def _filter_chunks_by_metadata(
        ...
        for chunk_id in chunk_ids:
            ...
            if investigation_type and not document.is_relevant_for_investigation(
                investigation_type
            ):
                continue
    ...
    def get_document_chunks(self, document_id: str) -> List[DocumentChunk]:
        ...
        return [
            self.chunks[chunk_id] for chunk_id in chunk_ids if chunk_id in self.chunks
        ]
