...
class RetrievalEngineConfig:
    ...
    default_expansion_method: QueryExpansionMethod = (
        QueryExpansionMethod.DOMAIN_SPECIFIC
    )
...
class RetrievalEngine:
    ...
    def __init__(
        ...
        self.search_semaphore = asyncio.Semaphore(
            config.max_concurrent_searches if config else 5
        )
    ...
    async def search(
        ...
        **kwargs,
        ...
        search_query = self._prepare_search_query(
            query, investigation_id, domain, **kwargs
        )
        ...
        try:
            ...
            self.logger.error(
                f"Search failed for query '{search_query.original_query}': {str(e)}"
            )
    ...
    async def _preprocess_query(self, query: SearchQuery) -> SearchQuery:
        ...
        if len(processed) > self.config.max_query_length:
            ...
            self.logger.warning(
                f"Query truncated to {self.config.max_query_length} characters"
            )
    ...
    async def _expand_query(self, query: SearchQuery) -> SearchQuery:
        ...
        if query.expansion_method == QueryExpansionMethod.SYNONYMS:
            ...
            expanded_queries.extend(
                self._expand_with_domain_terms(query.original_query, query.domain)
            )

        elif query.expansion_method == QueryExpansionMethod.RELATED_TERMS:
            expanded_queries.extend(
                self._expand_with_related_terms(query.original_query)
            )

        elif query.expansion_method == QueryExpansionMethod.CONTEXTUAL:
            expanded_queries.extend(
                self._expand_with_context(query.original_query, query.investigation_id)
            )
        ...
        if expanded_queries:
            ...
            self.logger.debug(
                f"Query expanded with {len(expanded_queries)} additional terms"
            )
    ...
    async def _vector_similarity_search(self, query: SearchQuery) -> SearchResult:
        ...
        chunks = await self.knowledge_base.search_chunks(
            ...
            entity_type=(
                list(query.entity_type_filters)[0]
    ...
    async def _hybrid_search(self, query: SearchQuery) -> SearchResult:
        ...
        for chunk in vector_result.chunks:
            chunk.hybrid_score = (
                getattr(chunk, "similarity_score", 0.7) * 0.7
            )  # Vector weight
        ...
        for chunk in keyword_result.chunks:
            existing_chunk = next(
                (c for c in combined_chunks if c.chunk_id == chunk.chunk_id), None
            )
            if existing_chunk:
                # Boost score for chunks found in both searches
                existing_chunk.hybrid_score += (
                    getattr(chunk, "similarity_score", 0.6) * 0.3
                )
            else:
                chunk.hybrid_score = (
                    getattr(chunk, "similarity_score", 0.6) * 0.5
                )  # Keyword weight
                combined_chunks.append(chunk)
        ...
        return SearchResult(
            ...
            total_candidates=vector_result.total_candidates
            + keyword_result.total_candidates,
    ...
    async def _apply_search_filters(
        self, chunks: List[DocumentChunk], query: SearchQuery
    ) -> List[DocumentChunk]:
        ...
        for chunk in chunks:
            # Domain filter
            if query.domain_filters and not any(
                domain in chunk.metadata.tags for domain in query.domain_filters
            ):
                continue

            # Entity type filter
            if (
                query.entity_type_filters
                and not query.entity_type_filters.intersection(
            ...
            if query.date_range_filter and hasattr(chunk.metadata, "creation_date"):
                ...
                if (
                    chunk.metadata.creation_date < start_date
                    or chunk.metadata.creation_date > end_date
                ):
                    continue
    ...
    async def _post_process_results(
        ...
        if query.domain:
            result.chunks = await self._apply_domain_boosting(
                result.chunks, query.domain
            )
    ...
    async def _apply_domain_boosting(
        self, chunks: List[DocumentChunk], domain: str
    ) -> List[DocumentChunk]:
        ...
        for chunk in chunks:
            if domain in chunk.metadata.tags:
                ...
                chunk.similarity_score = min(
                    1.0, original_score * self.config.domain_weight_multiplier
                )
    ...
    def _cache_result(self, query: SearchQuery, result: SearchResult) -> None:
        ...
        if len(self.results_cache) >= self.config.max_cache_entries:
            # Remove oldest entry
            oldest_key = min(
                self.results_cache.keys(), key=lambda k: self.results_cache[k][1]
            )
    ...
    def _update_search_stats(self, search_time_ms: float, success: bool) -> None:
        ...
        new_avg = (
            (current_avg * (self.search_stats["total_searches"] - 1)) + search_time_ms
...
def create_retrieval_engine(
    knowledge_base: KnowledgeBase, config: Optional[RetrievalEngineConfig] = None
) -> RetrievalEngine:
