...
class ToolInputValidator:
    ...
    def validate_tool_input(self, tool_type: ToolType, **kwargs) -> ValidationResult:
        ...
        try:
            if tool_type == ToolType.VIRUSTOTAL:
                ...
                result = ValidationResult(
                    valid=False, error_message=f"Unknown tool type: {tool_type}"
                )
            ...
            return ValidationResult(
                valid=False, error_message=f"Validation exception: {str(e)}"
            )
    ...
    def _validate_ip_reputation_input(self, **kwargs) -> ValidationResult:
        ...
        if not ip:
            return ValidationResult(
                valid=False, error_message="IP reputation check requires 'ip' parameter"
            )
        ...
        if not ip_validation.valid:
            return ValidationResult(
                ...
                suggestions=[
                    "Use valid IPv4 or IPv6 format (e.g. 8.8.8.8 or 2001:4860:4860::8888)"
                ],
            )
    ...
    def _validate_url_scanner_input(self, **kwargs) -> ValidationResult:
        ...
        if not url:
            return ValidationResult(
                valid=False, error_message="URL scanner requires 'url' parameter"
            )
    ...
    def _validate_dns_lookup_input(self, **kwargs) -> ValidationResult:
        ...
        if not domain:
            return ValidationResult(
                valid=False, error_message="DNS lookup requires 'domain' parameter"
            )
    ...
    def _validate_whois_input(self, **kwargs) -> ValidationResult:
        ...
        if not target:
            return ValidationResult(
                valid=False, error_message="WHOIS requires 'domain' or 'ip' parameter"
            )
        ...
        if self._is_ip_address(target):
            ...
            if ip_validation.valid:
                return ValidationResult(valid=True, cleaned_input={"ip": target})
        ...
        if domain_validation.valid:
            return ValidationResult(
                valid=True, cleaned_input={"domain": cleaned_domain}
            )

        return ValidationResult(
            valid=False, error_message="WHOIS target must be valid domain or IP address"
        )
    ...
    def _validate_domain_format(self, domain: str) -> ValidationResult:
        ...
        if not domain:
            return ValidationResult(valid=False, error_message="Empty domain")
        ...
        if not domain_pattern.match(domain):
            return ValidationResult(valid=False, error_message="Invalid domain format")
    ...
    def _validate_ip_address_format(self, ip: str) -> ValidationResult:
        ...
        if not ip:
            return ValidationResult(valid=False, error_message="Empty IP address")
        ...
        try:
            ...
            return ValidationResult(
                valid=False, error_message=f"Invalid IP address format: {str(e)}"
            )
    ...
    def _validate_url_format(self, url: str) -> ValidationResult:
        ...
        if not url:
            return ValidationResult(valid=False, error_message="Empty URL")
        ...
        try:
            ...
            if not parsed.scheme:
                return ValidationResult(
                    valid=False, error_message="URL missing scheme (http/https)"
                )

            if not parsed.netloc:
                return ValidationResult(valid=False, error_message="URL missing domain")
            ...
            return ValidationResult(
                valid=False, error_message=f"Invalid URL format: {str(e)}"
            )
    ...
    def get_validation_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": (
                self.validation_stats["successful_validations"] / max(total, 1)
            )
            * 100,
            "failure_rate": (
                self.validation_stats["failed_validations"] / max(total, 1)
            )
            * 100,
            "cleaning_rate": (self.validation_stats["cleaned_inputs"] / max(total, 1))
            * 100,
...
def safe_tool_call(
    tool_type: ToolType, tool_function: callable, **kwargs
) -> Tuple[bool, Any]:
    """
    ...
    if not validation_result.valid:
        logger.warning(
            f"Tool validation failed for {tool_type.value}: {validation_result.error_message}"
        )
        return False, {
