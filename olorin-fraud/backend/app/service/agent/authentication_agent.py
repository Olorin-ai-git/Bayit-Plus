...
async def structured_authentication_agent(state, config) -> dict:
    ...
    AUTHENTICATION_TIMEOUT_SECONDS = int(
        os.getenv("AUTHENTICATION_ANALYSIS_TIMEOUT", "300")
    )  # 5 minutes default
    PERFORMANCE_WARNING_THRESHOLD_SECONDS = int(
        os.getenv("AUTHENTICATION_PERFORMANCE_WARNING_THRESHOLD", "120")
    )  # 2 minutes
    ...
    from .authentication_agent_config import (
        create_authentication_agent_metadata,
        create_authentication_rag_config,
        get_authentication_objectives,
    ...
    journey_tracker.track_node_execution(
        ...
        input_state={
            "entity_id": entity_id,
            "authentication_analysis": "starting",
            "investigation_phase": "authentication_domain",
            "rag_enabled": RAG_AVAILABLE,
        },
        output_state={
            "authentication_analysis": "in_progress",
            "agent_status": "active",
        ...
        agent_name=(
            "RAG-Enhanced-AuthenticationAgent"
            if RAG_AVAILABLE
            else "StructuredAuthenticationAgent"
        ),
    ...
    try:
        ...
        if RAG_AVAILABLE and rag_config:
            ...
            logger.info(
                "🔐 Created authentication agent with intelligent RAG-enhanced tool selection"
            )
        else:
        ...
        authentication_objectives = get_authentication_objectives(
        ...
        try:
            ...
            return _create_error_response(
                f"Authentication analysis timeout: {error_message}"
            )
        ...
        if current_duration > PERFORMANCE_WARNING_THRESHOLD_SECONDS:
            logger.warning(
                f"⚠️ PERFORMANCE WARNING: Authentication analysis took {current_duration:.1f}s (>{PERFORMANCE_WARNING_THRESHOLD_SECONDS}s threshold)"
            )
        ...
        completion_metadata = create_authentication_agent_metadata(
        ...
        journey_tracker.track_node_execution(
            ...
            output_state={
                "authentication_analysis": "failed",
                "error": str(e),
                "rag_enabled": RAG_AVAILABLE,
            },
            duration_ms=0,
