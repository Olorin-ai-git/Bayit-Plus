...
class StructuredInvestigationAgent:
    ...
    def __init__(self, domain: str, tools: List[Any]):
        ...
        try:
            ...
            logger.info(
                f"Successfully bound {len(tools)} tools to {domain} structured agent"
            )
    ...
    def _format_tool_summary(self) -> str:
        ...
        for tool_name in self.tool_map.keys():
            ...
            if any(x in tool_lower for x in ["abuse", "virus", "shodan", "threat"]):
                ...
                x in tool_lower
                for x in ["snowflake", "splunk", "sumo", "database", "query"]
            ):
                categories["Database/Logs"].append(tool_name)
            elif any(
                x in tool_lower
                for x in ["blockchain", "crypto", "nft", "defi", "wallet"]
            ):
                categories["Blockchain"].append(tool_name)
            elif any(
                x in tool_lower for x in ["osint", "social", "people", "dark", "deep"]
            ):
        ...
        for cat, tools in categories.items():
            if tools:
                summary.append(
                    f"- {cat}: {', '.join(tools[:5])}{'...' if len(tools) > 5 else ''}"
                )
    ...
    async def refresh_tools(self, context: StructuredInvestigationContext) -> bool:
        ...
        try:
            ...
            if new_tools and new_tools != self.tools:
                ...
                logger.debug(
                    f"No tool changes needed for {self.domain} (refresh time: {refresh_time_ms:.1f}ms)"
                )
                return False
    ...
    async def structured_investigate(
        ...
        if self.supports_tool_refresh:
            try:
                ...
                if tools_refreshed:
                    logger.info(
                        f"🔄 Tools refreshed for {self.domain} investigation {context.investigation_id}"
                    )
            except Exception as e:
                logger.warning(
                    f"Tool refresh failed for {self.domain}, continuing with existing tools: {str(e)}"
                )
        ...
        logger.info(
            f"🔥 UNIFIED PROMPTS: Using comprehensive investigation prompt for {self.domain} domain"
        )
        logger.info(
            f"        📊 {len(self.tools)} tools available for comprehensive {self.domain.title()} analysis"
        )
...
"""
        ...
        try:
            ...
            logger.info(
                f"Starting structured {self.domain} investigation for {context.investigation_id}"
            )
            ...
            logger.info(
                f"        🔄 Invoking LLM for {self.domain.title()} analysis..."
            )
            ...
            if hasattr(result, "tool_calls") and result.tool_calls:
                logger.info(
                    f"🔧 Tool calls detected in {self.domain} analysis - executing tools and getting final analysis"
                )
                logger.info(
                    f"        🔧 Executing {len(result.tool_calls)} tools for {self.domain.title()} analysis..."
                )
                ...
                for tool_call in result.tool_calls:
                    ...
                    logger.debug(
                        f"📝 Fixed tool args for {tool_name}: {tool_args} -> {fixed_args}"
                    )
...
"""
                ...
                logger.info(
                    f"🔍 Requesting final analysis from LLM for {self.domain} domain"
                )
                logger.info(
                    f"        🔍 Getting final analysis with tool results for {self.domain.title()}..."
                )
                ...
                result = final_result
                logger.info(
                    f"✅ Final analysis received for {self.domain} domain with tool integration"
                )
            else:
                logger.info(
                    f"ℹ️ No tool calls detected in {self.domain} analysis - requesting formatted response"
                )
...
"""
                ...
                logger.info(
                    f"🔍 Requesting formatted response from LLM for {self.domain} domain"
                )
                logger.info(
                    f"        🔍 Getting formatted response for {self.domain.title()}..."
                )
            ...
            validation_result = LLMInteractionFormatter.validate_response_format(
                result.content
            )

            if validation_result["has_risk_score"]:
                logger.info(
                    f"        ✅ Risk Score Found: {validation_result['risk_score_value']}"
                )
            else:
            ...
            logger.info(
                f"        📊 Format Compliance: {validation_result['format_compliance']*100:.1f}%"
            )
            ...
            from app.service.agent.structured_parsing import (
                extract_content_from_response,
            )
            from app.service.agent.structured_prompts import (
            ...
            is_valid_format = validate_investigation_response(
                response_content, self.domain
            )
            if is_valid_format:
                logger.info(
                    f"✅ FORMAT VALIDATED: Response follows expected format for {self.domain}"
                )
                logger.info(
                    f"        ✅ Format validated for {self.domain.title()} response"
                )
            else:
                logger.warning(
                    f"⚠️ FORMAT WARNING: Response may not follow expected format for {self.domain}"
                )
                logger.warning(
                    f"        ⚠️ Format validation warning for {self.domain.title()} response"
                )
            ...
            logger.info(
                f"           Risk Score: {risk_display} | Confidence: {findings.confidence:.2f}"
            )
            logger.info(
                f"           Findings: {len(findings.key_findings)} | Quality: {findings.data_quality}"
            )

            # Log analysis completion
            logger.info(
                f"🔥 ANALYSIS COMPLETE: {self.domain} domain analysis using unified prompts completed successfully"
            )

            # Log comprehensive interaction summary
            logger.info(
                f"🔥 COMPREHENSIVE LLM INTERACTION SUMMARY for {self.domain.title()} Agent:"
            )
            ...
            if hasattr(result, "tool_calls") and result.tool_calls:
                ...
                logger.info(
                    f"   Tools Used: {', '.join(tools_used)} ({len(result.tool_calls)} total calls)"
                )
            ...
            logger.info(
                f"🔥 END LLM INTERACTION: {self.domain.title()} Agent - SUCCESS"
            )
            ...
            logger.error(
                f"        ❌ ERROR: {self.domain.title()} Agent failed completely - no risk_score available!"
            )
