...
class MLAIMCPClient(BaseTool):
    ...
    def _run(
        ...
        """
        return asyncio.run(
            self._arun(data, model_type, model_version, confidence_threshold)
        )
    ...
    async def _arun(
        ...
        try:
            ...
            if model_type in [ModelType.FRAUD_DETECTION]:
                ...
                ModelType.IMAGE_FORENSICS,
                ModelType.VIDEO_ANALYSIS,
    ...
    async def verify_media(self, media_path: str, media_type: str) -> Dict[str, Any]:
        ...
        return await self._arun(
            data=media_path, model_type=model_type, confidence_threshold=0.85
        )
