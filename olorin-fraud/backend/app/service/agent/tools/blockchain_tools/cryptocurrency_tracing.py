...
class CryptocurrencyTracingTool(BaseTool):
    ...
    def _run(
        ...
        try:
            ...
            trace_results = {
                ...
                "fund_flow": self._trace_fund_flow(
                    transaction_hash, source_network, tracing_depth
                ),
                "mixing_detection": (
                    self._detect_mixing_services(transaction_hash)
                    if detect_mixing
                    else None
                ),
                "exchange_interactions": self._identify_exchanges(transaction_hash),
                "cross_chain_transfers": (
                    self._trace_cross_chain(transaction_hash)
                    if follow_cross_chain
                    else None
                ),
                "summary": {},
            }
            ...
            trace_results["summary"] = {
                ...
                "mixing_detected": (
                    bool(trace_results["mixing_detection"]["services_detected"])
                    if detect_mixing
                    else False
                ),
                "exchanges_identified": len(
                    trace_results["exchange_interactions"]["exchanges"]
                ),
                "cross_chain_detected": (
                    bool(trace_results["cross_chain_transfers"]["bridges_used"])
                    if follow_cross_chain
                    else False
                ),
    ...
    def _trace_fund_flow(
        self, tx_hash: str, network: str, depth: str
    ) -> Dict[str, Any]:
    ...
    def _identify_exchanges(self, tx_hash: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "dex_interactions": [
                {"protocol": "Uniswap V3", "pool": "USDC/ETH", "hop_number": 5}
            ],
        }
