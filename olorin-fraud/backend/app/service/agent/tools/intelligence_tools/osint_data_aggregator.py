...
class OSINTDataAggregatorTool(BaseTool):
    ...
    def _run(
        ...
        try:
            ...
            aggregation_results = {
                ...
                "data_collection": self._collect_osint_data(
                    target_entity, entity_type, valid_sources
                ),
                "data_correlation": self._correlate_data(
                    target_entity, correlation_depth
                ),
                "timeline_analysis": (
                    self._build_timeline(target_entity) if include_timeline else None
                ),
                "reliability_assessment": self._assess_reliability(target_entity),
                "evidence_preservation": (
                    self._preserve_evidence(target_entity)
            ...
            aggregation_results["intelligence_summary"] = (
                self._generate_intelligence_summary(aggregation_results)
            )
    ...
    def _collect_osint_data(
        self, entity: str, entity_type: str, sources: List[str]
    ) -> Dict[str, Any]:
        ...
        for source in sources:
            if source == OSINTSource.PUBLIC_RECORDS.value:
                collected_data[source] = self._search_public_records(
                    entity, entity_type
                )
                ...
                collected_data[source] = self._search_government_data(
                    entity, entity_type
                )
        ...
        return {
            ...
            "sources_responded": len(
                [s for s in collected_data.values() if s.get("status") != "error"]
            ),
            "total_records_found": sum(
                s.get("record_count", 0) for s in collected_data.values()
            ),
