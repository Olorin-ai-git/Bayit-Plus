...
class ToolExecutionContext:
    ...
    # Context metadata
    context_injection_strategy: ContextInjectionStrategy = (
        ContextInjectionStrategy.FULL_INTEGRATION
    )
    ...
    def has_rag_context(self) -> bool:
        ...
        return (
            ...
            and self.knowledge_context.total_chunks > 0
        )
...
class ToolExecutionContextEnhancer:
    ...
    def __init__(
        ...
        try:
            ...
            logger.info(
                "ToolExecutionContextEnhancer initialized with RAG capabilities"
            )
        except Exception as e:
            logger.warning(
                f"RAG initialization failed - continuing without RAG: {str(e)}"
            )
        ...
        if exec_context.rag_enabled:
            try:
                ...
                if enhancement_result.success:
                    ...
                    exec_context.enhanced_parameters = (
                        enhancement_result.enhanced_parameters
                    )
                    exec_context.parameter_enhancements = [
                        enhancement_result.context_summary
                    ]
                    exec_context.context_retrieval_time_ms = (
                        enhancement_result.enhancement_time_ms
                    )
                    ...
                    self._update_success_stats(
                        enhancement_result.enhancement_time_ms,
                        enhancement_result.knowledge_chunks_used,
                    )
                    ...
                    logger.warning(
                        f"RAG enhancement failed for {tool_name}: {enhancement_result.error}"
                    )
    ...
    async def _perform_rag_enhancement(
        ...
        try:
            ...
            if not knowledge_context or knowledge_context.total_chunks == 0:
                return ToolContextEnhancementResult.failure_result(
    ...
    async def _retrieve_tool_knowledge_context(
        ...
        try:
            ...
            knowledge_context = (
                await self.context_augmentor.augment_investigation_context(
    ...
    def _create_tool_enhancement_query(
        self, tool_name: str, parameters: Dict[str, Any], domain: Optional[str]
    ) -> str:
    ...
    async def _apply_knowledge_to_parameters(
        ...
        if tool_name.lower() in ["splunk", "enhanced_splunk"]:
            enhanced_params = await self._enhance_splunk_parameters(
                enhanced_params, knowledge_context
            )
            ...
            enhanced_params = await self._enhance_generic_parameters(
                enhanced_params, knowledge_context
            )
    ...
    async def _enhance_splunk_parameters(
        self, params: Dict[str, Any], knowledge_context: KnowledgeContext
    ) -> Dict[str, Any]:
        ...
        if "query" in params and knowledge_context.critical_knowledge:
            ...
            if knowledge_terms:
                enhanced["query"] = (
                    f"({original_query}) AND ({' OR '.join(knowledge_terms)})"
                )
                enhanced["_rag_enhancement"] = (
                    f"Added {len(knowledge_terms)} knowledge-based search terms"
                )

        # Enhance time range based on knowledge patterns
        if knowledge_context.supporting_knowledge:
            time_patterns = self._extract_time_patterns_from_knowledge(
                knowledge_context.supporting_knowledge
            )
            if time_patterns and "earliest_time" not in params:
                enhanced["earliest_time"] = time_patterns.get(
                    "recommended_earliest", "-24h"
                )
                enhanced["_rag_time_enhancement"] = (
                    "Applied knowledge-based time range optimization"
                )
    ...
    async def _enhance_search_parameters(
        ...
        if "search_terms" in params or "query" in params:
            ...
            knowledge_terms = self._extract_relevant_terms_from_knowledge(
                knowledge_context
            )
            if knowledge_terms:
                if isinstance(original_terms, str):
                    enhanced[search_field] = (
                        f"{original_terms} {' '.join(knowledge_terms)}"
                    )
        ...
        if knowledge_context.total_chunks > 10:
            ...
            enhanced["_rag_result_enhancement"] = (
                "Increased result limit due to rich knowledge context"
            )
    ...
    async def _enhance_generic_parameters(
        ...
        enhanced["_rag_context"] = {
            ...
            "domains_covered": (
                list(knowledge_context.knowledge_sources)
    ...
    def _extract_search_terms_from_knowledge(
        self, knowledge_chunks: List[Any]
    ) -> List[str]:
    ...
    def _extract_time_patterns_from_knowledge(
        self, knowledge_chunks: List[Any]
    ) -> Dict[str, str]:
    ...
    def _extract_relevant_terms_from_knowledge(
    ...
    def _update_success_stats(
        self, enhancement_time_ms: float, knowledge_chunks: int
    ) -> None:
        ...
        new_avg = (
            (current_avg * (total_enhancements - 1)) + enhancement_time_ms
        ) / total_enhancements
