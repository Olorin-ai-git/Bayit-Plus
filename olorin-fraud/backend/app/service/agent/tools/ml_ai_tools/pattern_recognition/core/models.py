...
class PatternRecognitionInput(BaseModel):
    ...
    learning_enabled: bool = Field(
        default=True, description="Whether to learn new patterns from the data"
    )
...
class PatternResult(BaseModel):
    ...
    anomalies: List[Dict[str, Any]] = Field(
        default=[], description="Detected anomalies"
    )
...
class ProcessedData(BaseModel):
    ...
    categorical_data: Dict[str, str] = Field(
        default={}, description="Categorical features"
    )
    numerical_data: Dict[str, float] = Field(
        default={}, description="Numerical features"
    )
    ...
    transaction_data: List[Dict[str, Any]] = Field(
        default=[], description="Transaction records"
    )
...
class FraudPattern(BaseModel):
    ...
    pattern_type: str = Field(
        ...,
        description="Type of fraud pattern (card_testing, velocity_burst, amount_clustering, time_anomaly)",
    )
    ...
    confidence: float = Field(
        ..., ge=0.0, le=1.0, description="Confidence score for the pattern (0.0-1.0)"
    )
    risk_adjustment: float = Field(
        ..., description="Risk score adjustment (e.g., +0.20 for +20%)"
    )
    affected_count: int = Field(
        ..., ge=0, description="Number of transactions affected by this pattern"
    )
    evidence: Dict[str, Any] = Field(
        ..., description="Supporting evidence for the pattern"
    )
