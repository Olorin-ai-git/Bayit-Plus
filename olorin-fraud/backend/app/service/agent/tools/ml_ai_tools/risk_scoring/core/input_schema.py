...
class RiskScoringInput(BaseModel):
    ...
    risk_tolerance: str = Field(
        default="medium", description="Risk tolerance level: 'low', 'medium', 'high'"
    )
...
class ComprehensiveRiskResult(BaseModel):
    ...
    individual_assessments: List[RiskAssessmentResult] = Field(
        default_factory=list, description="Individual risk assessments"
    )
    model_scores: Dict[str, float] = Field(
        default_factory=dict, description="Scores from different models"
    )
    recommendations: List[str] = Field(
        default_factory=list, description="Risk mitigation recommendations"
    )
    metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Additional assessment metadata"
    )
