...
class ComposioWebCrawlInput(BaseModel):
    ...
    include_links: bool = Field(
        default=False, description="Whether to include links in results"
    )
...
class ComposioWebCrawlTool(BaseTool):
    ...
    def __init__(self, **kwargs):
        ...
        if not self._mcp_url:
            logger.warning(
                "COMPOSIO_FIRECRAWL_CRAWL_URL not set - Composio WebCrawl tool will not work"
            )
                    ...
                    for line in lines:
                        if line.startswith("data: "):
                            ...
                            try:
                                ...
                                if "error" in result:
                                    logger.error(
                                        f"Composio WebCrawl MCP error: {result['error']}"
                                    )
                                    return {
                                        "success": False,
    ...
    def _run(
        ...
        if result.get("success"):
                    ...
                    if "content" in mcp_result:
                        ...
                        if isinstance(mcp_content, list) and len(mcp_content) > 0:
                            ...
                            try:
                                ...
                                content = parsed.get(
                                    "content", parsed.get("data", text_content)
                                )
                            except:
            ...
            return json.dumps(
                {"success": False, "error": error_msg, "url": url}, indent=2
            )
    ...
    async def _arun(
        ...
        return await loop.run_in_executor(
            None, self._run, url, max_depth, include_links, entity_id
        )
