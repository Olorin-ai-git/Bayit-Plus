...
from ..base_threat_tool import (
    BaseThreatIntelligenceTool,
...
class IPReputationInput(BaseModel):
    ...
    include_details: bool = Field(
        default=True, description="Include detailed information about the IP"
    )
    ...
    def validate_ip_address(cls, v):
        ...
        for pattern in entity_patterns:
            if re.match(pattern, v):
                raise ValueError(
                    f"Entity ID detected where IP address expected: {v}. Please extract actual IP addresses from the investigation context data sources."
                )
        ...
        try:
            ...
            raise ValueError(
                f"Invalid IP address format: {v}. Expected valid IPv4 or IPv6 address format."
            )
...
class IPReputationTool(BaseThreatIntelligenceTool):
    ...
    def __init__(self, **kwargs):
        ...
        super().__init__(
            name=self.name, description=self.description, config=threat_config, **kwargs
        )
    ...
    async def _execute_threat_query(
        ...
        try:
            ...
            reputation_response = await self.client.check_ip_reputation(
                ip=ip, max_age_days=max_age_days, verbose=include_details
            )
            ...
            if e.status_code == 402:
                logger.debug(
                    f"AbuseIPDB analysis skipped for {ip}: subscription required"
                )
                return ThreatIntelligenceResponse(
                    success=False,
                ...
                logger.debug(
                    f"AbuseIPDB analysis limited for {ip}: subscription required"
                )
                return ThreatIntelligenceResponse(
                    success=False,
                    error=str(e),
                ...
                logger.warning(
                    f"AbuseIPDB analysis failed for {ip}: {type(e).__name__}"
                )
                return ThreatIntelligenceResponse(
                    success=False,
            ...
            logger.warning(
                f"AbuseIPDB analysis unavailable for {ip}: {type(e).__name__}"
            )
            return ThreatIntelligenceResponse(
                success=False,
    ...
    def _generate_threat_analysis(
        self, response: IPReputationResponse
    ) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            "reputation_summary": {
                ...
                "last_reported": (
                    ip_info.last_reported_at.isoformat()
            ...
            "investigation_recommendations": self._generate_recommendations(
                ip_info, risk_level
            ),
    ...
    def _calculate_normalized_risk_level(
        self, confidence: int, data_quality_score: float, quality_factors: List[str]
    ) -> str:
    ...
    def _assess_reputation_reliability(
        self, confidence: int, total_reports: int, data_quality_score: float
    ) -> str:
    ...
    def _get_normalization_recommendation(
        self, original_confidence: int, normalized_risk: str, data_quality_score: float
    ) -> str:
        ...
        if data_quality_score < 0.5:
            ...
            return (
                f"Reputation data appears reliable (quality: {data_quality_score:.2f})"
            )
    ...
    def _generate_threat_assessment(self, ip_info, risk_level: str) -> Dict[str, Any]:
        ...
        if ip_info.total_reports > 0:
            risk_factors.append(
                f"Reported {ip_info.total_reports} times by {ip_info.num_distinct_users} users"
            )
        ...
        return {
            ...
            "fraud_investigation_impact": self._assess_fraud_impact(
                ip_info, confidence
            ),
        }
    ...
    def _assess_fraud_impact(self, ip_info, confidence: int) -> Dict[str, Any]:
        ...
        if ip_info.country_code and ip_info.country_code in ["CN", "RU", "KP", "IR"]:
            recommendations.append(
                "Geographic risk factor - enhanced monitoring recommended"
            )
        ...
        return {
            ...
            "suggested_actions": self._suggest_investigation_actions(
                ip_info, confidence
            ),
        }
    ...
    def _generate_recommendations(self, ip_info, risk_level: str) -> List[str]:
        ...
        if ip_info.total_reports > 10:
            recommendations.append(
                f"📈 Multiple reports ({ip_info.total_reports}) indicate persistent malicious activity"
            )

        if (
            ip_info.last_reported_at
            and (datetime.utcnow() - ip_info.last_reported_at).days < 7
        ):
    ...
    def _calculate_confidence_score(self, response: IPReputationResponse) -> float:
        ...
        if response.ip_info.is_whitelisted:
            base_confidence = max(
                0.0, base_confidence - 0.3
            )  # Reduce confidence for whitelisted
    ...
    async def _arun(
        self, ip: str, max_age_days: int = 90, include_details: bool = True, **kwargs
    ) -> str:
        ...
        try:
            ...
            response = await self.execute_threat_intelligence_query(
                query_type="ip_reputation", query_data=query_data
            )
            ...
            return json.dumps(
                {"success": False, "error": str(e), "ip": ip, "source": "AbuseIPDB"},
                indent=2,
            )
