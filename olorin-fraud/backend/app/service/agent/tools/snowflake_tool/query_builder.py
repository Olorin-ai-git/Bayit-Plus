...
class SnowflakeQueryBuilder:
    ...
    def build_investigation_query(
        ...
        if investigation_focus not in cls.EVIDENCE_FIELD_COLLECTIONS:
            logger.warning(
                f"Unknown investigation focus '{investigation_focus}', using comprehensive"
            )
        ...
        # Pattern-based exclusion: any column containing "FRAUD" (case-insensitive)
        field_collection_filtered = []
        excluded_fraud_columns = []

        for field in field_collection:
            field_upper = field.upper()
            # Exclude MODEL_SCORE and any column containing "FRAUD"
            if field_upper == "MODEL_SCORE" or "FRAUD" in field_upper:
                excluded_fraud_columns.append(field)
            else:
                field_collection_filtered.append(field)

        field_collection = field_collection_filtered

        if excluded_fraud_columns:
            logger.info(
                f"🚫 Excluded {len(excluded_fraud_columns)} fraud-related columns from investigation query (unbiased evaluation)"
            )
            logger.debug(
                f"🚫 Excluded columns: {', '.join(excluded_fraud_columns[:10])}{'...' if len(excluded_fraud_columns) > 10 else ''}"
            )
        ...
        metadata = {
            ...
            "query_complexity": cls._assess_query_complexity(
                field_collection, date_range_days
            ),
            "evidence_categories": cls._categorize_fields(field_collection),
            "performance_estimate": cls._estimate_performance(
                field_collection, date_range_days, limit
            ),
        }

        logger.info(
            f"🔍 Built {investigation_focus} investigation query for {entity_type}={entity_id}"
        )
        logger.info(
            f"   Fields: {len(field_collection)}, Date range: {date_range_days} days"
        )
    ...
    def _assess_query_complexity(
        cls, field_collection: List[str], date_range_days: int
    ) -> str:
    ...
    def _estimate_performance(
        cls, field_collection: List[str], date_range_days: int, limit: int
    ) -> Dict[str, Any]:
        ...
        estimated_time_ms = int(
            (base_time_ms + field_overhead_ms + date_range_overhead_ms) * limit_factor
        )
        ...
        return {
            ...
            "performance_tier": (
                ...
                else "moderate" if estimated_time_ms < 5000 else "slow"
            ),
            "optimization_suggestions": cls._get_optimization_suggestions(
                field_collection, date_range_days, limit
            ),
        }
    ...
    def _get_optimization_suggestions(
        ...
        if limit > 5000:
            suggestions.append(
                "Large limit may impact performance, consider pagination"
            )
    ...
    @classmethod
    def _validate_query_completeness(
        cls, field_collection: List[str]
    ) -> Dict[str, Any]:
        ...
        coverage_score = sum(min(count, 3) for count in evidence_coverage.values()) / (
            len(evidence_coverage) * 3
        )
        ...
        return {
            ...
            "recommendations": cls._get_completeness_recommendations(
                missing_critical, evidence_coverage
            ),
        }

    @classmethod
    def _get_completeness_recommendations(
        cls, missing_critical: List[str], evidence_coverage: Dict[str, int]
    ) -> List[str]:
        ...
        if missing_critical:
            recommendations.append(
                f"Add critical fields: {', '.join(missing_critical)}"
            )

        for category, count in evidence_coverage.items():
            if count == 0:
                recommendations.append(
                    f"Consider adding {category} fields for comprehensive analysis"
                )
...
def get_recommended_query_for_entity(entity_type: str, entity_id: str, **kwargs) -> str:
    ...
    logger.info(
        f"   Evidence score: {query_info['validation']['evidence_coverage_score']}"
    )
