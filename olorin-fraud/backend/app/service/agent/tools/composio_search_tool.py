...
class ComposioSearchInput(BaseModel):
    ...
    max_results: int = Field(
        default=10, description="Maximum number of results to return (1-20)"
    )
...
class ComposioSearchTool(BaseTool):
    ...
    def __init__(self, **kwargs):
        ...
        self._mcp_url = (
            os.getenv("COMPOSIO_SEARCH_API_URL")
            or os.getenv("COMPOSIO_SEARCH_URL")
            or os.getenv("COMPOSIO_SEARCH_MCP_URL")
        )
        if not self._mcp_url:
            logger.warning(
                "COMPOSIO_SEARCH_API_URL not set - Composio Search tool will use fallback search"
            )
        self._timeout = 30.0
    ...
    def _make_mcp_request(self, method: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not self._mcp_url:
            return {"success": False, "error": "COMPOSIO_SEARCH_API_URL not configured"}
                    ...
                    for line in lines:
                        if line.startswith("data: "):
                            ...
                            try:
                                ...
                                if "error" in result:
                                    logger.error(
                                        f"Composio Search MCP error: {result['error']}"
                                    )
                                    return {
                                        "success": False,
    ...
    def _run(
        self, query: str, max_results: int = 10, entity_id: Optional[str] = None
    ) -> str:
        """
        ...
        if not self._mcp_url:
            logger.info(
                "Composio Search MCP URL not configured, using fallback web search"
            )
            try:
        ...
        result = self._make_mcp_request(
            "tools/call", {"name": "COMPOSIO_SEARCH_WEB", "arguments": {"query": query}}
        )
        ...
        if not result.get("success"):
            result = self._make_mcp_request(
                "tools/call",
                {"name": "COMPOSIO_SEARCH_DUCK_DUCK_GO", "arguments": {"query": query}},
            )
        ...
        if result.get("success"):
                            ...
                            if isinstance(parsed, dict):
                                ...
                                if isinstance(data, dict):
                                    ...
                                    if isinstance(results_data, dict):
                                        ...
                                        ai_overview = results_data.get(
                                            "ai_overview", {}
                                        )
                                        if ai_overview:
                                            text_blocks = ai_overview.get(
                                                "text_blocks", []
                                            )
                                    ...
                                    if (
                                        not results
                                        and "organic_results" in results_data
                                    ):
                                        results = results_data["organic_results"][
                                            :max_results
                                        ]
            ...
            logger.info(
                f"✅ Composio Search completed: query='{query}', results={len(results)}"
            )
            ...
            return json.dumps(
                {"success": False, "error": error_msg, "query": query}, indent=2
            )
    ...
    async def _arun(
        ...
        return await loop.run_in_executor(
            None, self._run, query, max_results, entity_id
        )
