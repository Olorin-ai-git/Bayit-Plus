...
class AsyncClientManager:
    ...
    def register_session(self, session: aiohttp.ClientSession) -> None:
        ...
        logger.debug(
            f"📡 Registered async session: {len(self._sessions)} active sessions"
        )
    ...
    def unregister_session(self, session: aiohttp.ClientSession) -> None:
        ...
        logger.debug(
            f"📡 Unregistered async session: {len(self._sessions)} active sessions"
        )
    ...
    async def cleanup_all_sessions(self) -> None:
        ...
        if cleanup_tasks:
            try:
                ...
                logger.info(
                    f"✅ Successfully closed {len(cleanup_tasks)} HTTP sessions"
                )
            except Exception as e:
            ...
            if loop and loop.is_running():
                ...
                def log_cleanup_result(task):
                    try:
                        if task.exception():
                            logger.warning(
                                f"📡 Cleanup task failed: {task.exception()}"
                            )
                    except Exception:
                        pass
