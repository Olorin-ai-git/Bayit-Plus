...
class ExtendedToolRegistry(ToolRegistry):
    ...
    def __init__(self):
        ...
        self._executor = ThreadPoolExecutor(
            max_workers=4, thread_name_prefix="tool_loader"
        )

    def register_tool_with_metadata(
        self, tool: BaseTool, category: str, metadata: ToolMetadata
    ) -> None:
    ...
    def register_tool_dependency(
        self, tool_name: str, dependency: ToolDependency
    ) -> None:
                ...
                try:
                    if not dependency.validation_function(dep_tool):
                        missing_dependencies.append(
                            f"{dependency.tool_name} (validation failed)"
                        )
                except Exception as e:
                    logger.warning(
                        f"Dependency validation failed for {dependency.tool_name}: {e}"
                    )
    ...
    async def preload_high_priority_tools(self) -> None:
        ...
        for name, metadata in self._tool_metadata.items():
            if (
                metadata.performance_tier == "fast"
                or metadata.usage_count > 100
                or name in self._lazy_loading
            ):
        ...
        if high_priority_tools:
            ...
            logger.info(
                f"Successfully preloaded {loaded_count}/{len(high_priority_tools)} tools"
            )
...
def register_blockchain_tool_lazy(
    name: str, loader: Callable, metadata: ToolMetadata
) -> None:
