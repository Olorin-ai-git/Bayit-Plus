...
class SegmentHunterTool(BaseTool):
    ...
    def _build_decomposition_query(
        ...
        is_snowflake = database_provider == "snowflake" or (
            use_snowflake and database_provider != "postgresql"
        )
        ...
        dimension_col = dimension_mapping.get(
            dimension.lower(), dimension.upper() if is_snowflake else dimension.lower()
        )
        ...
        if not is_snowflake:
            ...
            cohort_filter_actual = cohort_filter_actual.replace(
            ...
            cohort_filter_actual = re.sub(
                r"\b([A-Z_]+)\b", lower_column, cohort_filter_actual
            )
        ...
        if metric.lower() in ["decline_rate", "decline_rate_pct"]:
            ...
            if is_snowflake:
                decision_col = (
                    "NSURE_LAST_DECISION"  # TXS column: NSURE_LAST_DECISION (line 920)
                )
            ...
            amount_col = (
                ...
                if not is_snowflake
                else "PAID_AMOUNT_VALUE_IN_CURRENCY"
            )
        ...
        if database_provider == "snowflake" or (
            use_snowflake and database_provider != "postgresql"
        ):
        ...
        for key, value in anomaly.cohort.items():
            if key.upper() == "GEO":
                ...
                cohort_conditions.append(
                    f"{col_name} = '{value.upper() if is_snowflake else value.lower()}'"
                )
        ...
        try:
            ...
            if is_snowflake:
                ...
                anomaly_result = {
                    ...
                    "data": (
                        anomaly_result
                        if isinstance(anomaly_result, list)
                        else [anomaly_result] if anomaly_result else []
                    ),
                    "row_count": (
                        len(anomaly_result)
                        if isinstance(anomaly_result, list)
                        else 1 if anomaly_result else 0
                    ),
                }
                baseline_result = {
                    "success": True,
                    ...
                    "row_count": (
                        len(baseline_result)
                        if isinstance(baseline_result, list)
                        else 1 if baseline_result else 0
                    ),
                }
            else:
            ...
            if (
                isinstance(anomaly_result, dict)
                and "data" in anomaly_result
                and anomaly_result["data"]
            ):
                anomaly_count = anomaly_result["data"][0].get("count", 0)

            if (
                isinstance(baseline_result, dict)
                and "data" in baseline_result
                and baseline_result["data"]
            ):
    ...
    def _run(
        self, anomaly_id: str, dimension: str, top_k: int = 10, min_support: int = 50
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if not anomaly:
                return {"error": f"Anomaly {anomaly_id} not found", "segments": []}
            ...
            window_duration = (
                anomaly.window_end - anomaly.window_start
            ...
            if window_duration < 1.0:  # Less than 1 hour
                ...
                logger.info(
                    f"   Adaptive min_support: {min_support} -> {adaptive_min_support} (window={window_duration:.2f}h)"
                )
            elif window_duration < 24.0:  # Less than 1 day
                # Scale down slightly for short windows
                adaptive_min_support = max(
                    10, int(min_support * (window_duration / 24.0))
                )
                logger.info(
                    f"   Adaptive min_support: {min_support} -> {adaptive_min_support} (window={window_duration:.2f}h)"
                )
            ...
            logger.info(
                f"SegmentHunter executing query for dimension {dimension}, anomaly {anomaly_id}"
            )
            logger.info(
                f"   Anomaly window: {anomaly.window_start} to {anomaly.window_end}"
            )
            ...
            logger.info(
                f"   Metric: {anomaly.metric}, min_support: {adaptive_min_support} (original: {min_support})"
            )
            ...
            logger.info(
                f"   Using database provider: {database_provider} (is_snowflake={is_snowflake})"
            )
            ...
            if is_snowflake:
                ...
                result = {
                    ...
                    "data": (
                        ...
                        else [result_data] if result_data else []
                    ),
                    "row_count": (
                        len(result_data)
                        if isinstance(result_data, list)
                        else 1 if result_data else 0
                    ),
                    "query": query,
                }
            else:
                ...
                if database_provider == "postgresql" or not database_provider:
                    ...
                    if (
                        ...
                        connection_string = f"postgresql://{postgres_user}:{postgres_password}@{postgres_host}:{postgres_port}/{postgres_database}?gssencmode=disable"
                        logger.debug(
                            f"SegmentHunter: Built PostgreSQL connection string from individual env vars"
                        )
            ...
            if isinstance(result, dict):
                if not result.get("success", False):
                    ...
                    logger.error(
                        f"SegmentHunter database query error for dimension {dimension}: {error_msg}"
                    )
                    logger.error(f"   Full query: {query}")
                    return {"error": error_msg, "segments": []}
                if "error" in result:
                    logger.error(
                        f"SegmentHunter database query error for dimension {dimension}: {result['error']}"
                    )
                    logger.error(f"   Full query: {query}")
                    return {"error": result["error"], "segments": []}
            ...
            if isinstance(result, dict) and "data" in result:
                rows = result["data"]
                logger.info(
                    f"SegmentHunter query returned {len(rows)} rows for dimension {dimension}"
                )
                ...
                rows = result["results"]
                logger.info(
                    f"SegmentHunter query returned {len(rows)} rows (results format) for dimension {dimension}"
                )
            elif isinstance(result, list):
                rows = result
                logger.info(
                    f"SegmentHunter query returned {len(rows)} rows (list format) for dimension {dimension}"
                )
            else:
                rows = []
                logger.warning(
                    f"SegmentHunter query returned unexpected format: {type(result)} for dimension {dimension}. Keys: {list(result.keys()) if isinstance(result, dict) else 'N/A'}"
                )
