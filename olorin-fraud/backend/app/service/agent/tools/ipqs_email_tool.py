...
class IPQSEmailTool(BaseTool):
    ...
    def __init__(self, **kwargs):
        ...
        object.__setattr__(
            self, "base_url", "https://www.ipqualityscore.com/api/json/email"
        )
        self._load_api_key()
    ...
    def _load_api_key(self):
        ...
        try:
            ...
            api_key = self.config_loader.load_secret("IPQS_API_KEY") or os.getenv(
                "IPQS_API_KEY"
            )
            ...
            if not api_key:
                logger.warning(
                    "IPQS_API_KEY not configured. Set environment variable or configure secret."
                )
            else:
    ...
    def _run(self, email: str, entity_id: Optional[str] = None, **kwargs) -> str:
        """
        ...
        logger.info(
            f"📧 [IPQSEmailTool] Verifying email: {email}, entity_id: {entity_id}"
        )
        ...
        try:
            ...
            logger.error(
                f"❌ [IPQSEmailTool] HTTP error: {e.response.status_code} - {e.response.text}"
            )
            return json.dumps(
                {
                    "status": "error",
    ...
    async def _arun(self, email: str, entity_id: Optional[str] = None, **kwargs) -> str:
        """Async version of email verification."""
        import asyncio

        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(None, self._run, email, entity_id, **kwargs)
