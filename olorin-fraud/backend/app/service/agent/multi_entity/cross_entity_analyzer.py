...
from app.service.agent.multi_entity.entity_manager import Entity, EntityType
...
class CrossEntityAnalyzer:
    ...
    async def analyze_cross_entity_patterns(
        ...
        """
        self.logger.info(
            f"🔍 Starting cross-entity analysis for investigation: {investigation_id}"
        )
        ...
        try:
            ...
            entity_interactions = self._analyze_entity_interactions(
                entity_results, relationships, timeline
            )
            ...
            self.metrics["high_risk_correlations"] += len(
                [
                    r
                    for r in risk_correlations
                    if r.get("correlation_strength", 0) > self.correlation_threshold
                ]
            )
            ...
            self.logger.error(
                f"❌ Cross-entity analysis failed {investigation_id}: {str(e)}"
            )
            raise
    ...
    def _extract_timestamps_from_findings(
    ...
    def _analyze_risk_correlations(
        ...
        for i in range(len(entity_ids)):
            for j in range(i + 1, len(entity_ids)):
                ...
                if any(
                    (r.source_entity_id == entity1 and r.target_entity_id == entity2)
                    or (r.source_entity_id == entity2 and r.target_entity_id == entity1)
    ...
    def _detect_behavioral_patterns(
        ...
        for agent_type, events in events_by_agent.items():
            if len(events) >= 2:  # Need multiple events for pattern
                agent_patterns = self._analyze_agent_behavioral_patterns(
                    agent_type, events
                )
    ...
    def _analyze_agent_behavioral_patterns(
        self, agent_type: str, events: List[TimelineEvent]
    ) -> List[Dict[str, Any]]:
        ...
        for cluster in risk_clusters:
            if len(cluster) >= 2:
                ...
                pattern = {
                    ...
                    "evidence": [
                        self._extract_event_evidence(event) for event in cluster
                    ],
                }
    ...
    def _cluster_events_by_risk(
        self, events: List[TimelineEvent], threshold: float = 0.3
        ...
        for event in sorted_events[1:]:
            ...
            cluster_avg_risk = sum(e.risk_score for e in current_cluster) / len(
                current_cluster
            )
    ...
    def _detect_cross_agent_patterns(
        self, entity_results: Dict[str, List[InvestigationResult]]
    ) -> List[Dict[str, Any]]:
        ...
        for entity_id, results in entity_results.items():
            if len(results) >= 2:
                agent_risks = {
                    result.agent_type: result.risk_score for result in results
                }
                ...
                if risk_std < 0.2:  # Low variance indicates consistency
                    pattern = {
                        ...
                        "evidence": [
                            {"agent_type": k, "risk_score": v}
                            for k, v in agent_risks.items()
                        ],
                    }
            ...
            if len(cluster) >= 2:
                ...
                anomaly_cluster = {
                    ...
                    "temporal_span": {
                        ...
                        "duration_minutes": (
                            temporal_span[1] - temporal_span[0]
                        ).total_seconds()
                        / 60,
                    },
    ...
    def _cluster_events_by_time(
        self, events: List[TimelineEvent], window_minutes: int = 60
    ) -> List[List[TimelineEvent]]:
    ...
    def _analyze_temporal_patterns(
        self, timeline: List[TimelineEvent], relationships: List[EntityRelationship]
    ) -> List[Dict[str, Any]]:
        ...
        relationship_patterns = self._analyze_relationship_temporal_patterns(
            ...
            if len(events) >= 3:  # Need at least 3 events for sequence
                ...
                if self._is_escalating_sequence(risk_progression):
                    sequence = {
                        ...
                        "evidence": [
                            self._extract_event_evidence(event)
    ...
    def _is_escalating_sequence(
        self, risk_scores: List[float], threshold: float = 0.1
    ) -> bool:
    ...
    def _analyze_relationship_temporal_patterns(
        ...
        for relationship in relationships:
            source_events = [
                e for e in timeline if e.entity_id == relationship.source_entity_id
            ]
            target_events = [
                e for e in timeline if e.entity_id == relationship.target_entity_id
            ]
    ...
    def _calculate_temporal_correlation(
        self, events1: List[TimelineEvent], events2: List[TimelineEvent]
    ) -> float:
    ...
    def _analyze_specific_interaction(
        ...
        source_avg_risk = sum(r.risk_score for r in source_results) / len(
            source_results
        )
        target_avg_risk = sum(r.risk_score for r in target_results) / len(
            target_results
        )
    ...
    def _generate_interaction_summary(
        self, source_risk: float, target_risk: float, relationship: EntityRelationship
    ) -> str:
    ...
    def _calculate_overall_confidence(
        ...
        strong_correlations = len(
            [r for r in risk_correlations if r.get("correlation_strength", 0) > 0.7]
        )
        ...
        if isinstance(findings, dict):
            for key, value in findings.items():
                if any(
                    ...
                    for keyword in ["risk", "anomaly", "suspicious", "alert"]
                ):
