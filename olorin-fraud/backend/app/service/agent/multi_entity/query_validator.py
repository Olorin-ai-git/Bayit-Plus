...
class MultiEntityQueryValidator:
    ...
    def validate_query(
        ...
        try:
            ...
            logger.debug(
                f"Query validation completed: valid={is_valid}, "
    ...
    def _analyze_complexity(
        self, boolean_logic: str, entity_ids: List[str]
    ...
    def _validate_entity_limits(
        self, entity_ids: List[str], errors: List[str], warnings: List[str]
    ):
        ...
        if entity_count > self.max_entities:
            errors.append(
                f"Too many entities: {entity_count} (max: {self.max_entities})"
            )
        elif entity_count > self.max_entities * 0.8:
            warnings.append(
                f"High entity count: {entity_count} (approaching limit of {self.max_entities})"
            )
        ...
        if len(unique_entities) != entity_count:
            warnings.append(
                f"Duplicate entities detected: {entity_count - len(unique_entities)} duplicates"
            )

    def _validate_expression_syntax(
        self, boolean_logic: str, errors: List[str], warnings: List[str]
    ):
        ...
        if expression_length > self.max_expression_length:
            errors.append(
                f"Expression too long: {expression_length} chars (max: {self.max_expression_length})"
            )
    ...
    def _validate_complexity_limits(
        self, metrics: QueryComplexityMetrics, errors: List[str], warnings: List[str]
    ):
        """Validate complexity limits"""
        if metrics.nesting_depth > self.max_nesting_depth:
            errors.append(
                f"Nesting too deep: {metrics.nesting_depth} levels (max: {self.max_nesting_depth})"
            )

        if metrics.operator_count > self.max_operators:
            errors.append(
                f"Too many operators: {metrics.operator_count} (max: {self.max_operators})"
            )

        if metrics.complexity_score > self.complexity_threshold:
            errors.append(
                f"Query too complex: score {metrics.complexity_score:.1f} (max: {self.complexity_threshold})"
            )
        elif metrics.complexity_level == QueryComplexityLevel.COMPLEX:
            warnings.append(
                f"Complex query detected: score {metrics.complexity_score:.1f}"
            )

    def _generate_recommendations(
        self, metrics: QueryComplexityMetrics, recommendations: List[str]
    ):
        ...
        if metrics.estimated_execution_time_ms > 1000:
            recommendations.append(
                "Query may take >1 second to execute - consider optimization"
            )

        if metrics.operator_count > 50:
            recommendations.append(
                "High operator count - consider using entity groups instead"
            )
    ...
    def _should_cache_query(self, metrics: QueryComplexityMetrics) -> bool:
        ...
        return (
            ...
            in [QueryComplexityLevel.COMPLEX, QueryComplexityLevel.MODERATE]
            or metrics.entity_count >= 10
            or metrics.estimated_execution_time_ms > 500
        )
...
def validate_multi_entity_query(
    boolean_logic: str, entity_ids: List[str], context: Optional[Dict[str, Any]] = None
) -> QueryValidationResult:
    """
