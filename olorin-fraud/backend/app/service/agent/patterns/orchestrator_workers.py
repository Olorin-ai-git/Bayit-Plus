...
class OrchestratorWorkersPattern(BasePattern):
    ...
    async def execute(
        ...
        try:
            ...
            worker_tasks = await self._create_worker_tasks(
                task_breakdown.result, messages, context
            )
            ...
            final_result = await self._synthesize_worker_results(
                execution_results, context
            )

            # Stream orchestration completion
            if self.ws_streaming:
                await self._stream_orchestration_complete(
                    len(execution_results), final_result.success, context
                )
            ...
            logger.error(
                f"Orchestrator-Workers execution failed: {str(e)}", exc_info=True
            )
    ...
    async def _orchestrate_task_breakdown(
        self, messages: List[BaseMessage], context: Dict[str, Any]
    ) -> PatternResult:
    ...
    async def _create_worker_tasks(
        ...
        for i, task_def in enumerate(default_tasks):
            ...
            task_context.update(
                {
                    "worker_specialization": self.workers[
                        task_def["worker"]
                    ].specialization,
            ...
            task_messages = original_messages + [
                HumanMessage(
                    content=f"As a {self.workers[task_def['worker']].name}, {task_def['description']} for this investigation."
                )
            ]
    ...
    async def _execute_with_dependencies(
        ...
        while self.pending_tasks or any(
            worker.is_busy for worker in self.workers.values()
        ):
    ...
    def _assign_tasks_to_workers(
        self, ready_tasks: List[WorkerTask]
    ) -> Dict[str, WorkerTask]:
    ...
    def _find_best_worker(
        self, task: WorkerTask, available_workers: List[WorkerAgent]
    ...
    async def _execute_assigned_tasks(
        self, assignments: Dict[str, WorkerTask]
    ) -> List[PatternResult]:
        ...
        async def execute_worker_task(
            ...
            try:
                ...
                if self.ws_streaming:
                    await self._stream_worker_task_complete(
                        worker_id, task, result.success
                    )
                ...
                result = PatternResult.error_result(
        ...
        task_coroutines = [
            execute_worker_task(worker_id, task)
    ...
    def _update_worker_states(
        self, assignments: Dict[str, WorkerTask], results: List[PatternResult]
    ) -> None:
        ...
        for (worker_id, task), result in zip(assignments.items(), results):
            ...
            if result.success:
                worker.success_rate = (
                    worker.success_rate * (worker.completed_tasks - 1) + 1.0
                ) / worker.completed_tasks
            else:
                worker.success_rate = (
                    worker.success_rate * (worker.completed_tasks - 1) + 0.0
                ) / worker.completed_tasks
    ...
    async def _synthesize_worker_results(
        self, results: List[PatternResult], context: Dict[str, Any]
    ) -> PatternResult:
    ...
    async def _stream_task_assignment(
        self, tasks: List[WorkerTask], context: Dict[str, Any]
    ) -> None:
    ...
    async def _stream_worker_task_complete(
        self, worker_id: str, task: WorkerTask, success: bool
    ) -> None:
    ...
    async def _stream_worker_task_timeout(
        self, worker_id: str, task: WorkerTask
    ) -> None:
    ...
    async def _stream_worker_task_error(
        self, worker_id: str, task: WorkerTask, error_message: str
    ) -> None:
    ...
    async def _stream_orchestration_complete(
        self, completed_tasks: int, success: bool, context: Dict[str, Any]
    ) -> None:
        """Stream orchestration completion"""
        if self.ws_streaming:
            await self.ws_streaming.send_agent_thought(
