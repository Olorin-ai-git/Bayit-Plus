...
async def execute_openai_function_call(
    function_name: str, function_args: Dict[str, Any], tools: List[BaseTool]
...
def _find_tool_by_function_name(
    function_name: str, tools: List[BaseTool]
) -> Optional[BaseTool]:
    ...
    for tool in tools:
        if (
            function_name.lower() in tool.name.lower()
            or tool.name.lower() in function_name.lower()
        ):
            return tool
...
def _convert_openai_args_to_tool_args(
    openai_args: Dict[str, Any], tool: BaseTool
) -> Dict[str, Any]:
