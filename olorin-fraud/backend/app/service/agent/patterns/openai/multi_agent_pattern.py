...
from .multi_agent_coordinator import HandoffManager, MultiAgentCoordinator
...
class OpenAIMultiAgentPattern(OpenAIBasePattern):
    ...
    async def execute_openai_pattern(
        ...
        try:
            ...
            execution_strategy = await self._coordinator.determine_execution_strategy(
            ...
            if execution_strategy == "parallel":
                agent_results = await self._execute_parallel_agents(
                    required_agents, messages, context
                )
            else:
                agent_results = await self._execute_sequential_agents(
                    required_agents, messages, context
                )
            ...
            metrics.function_calls = sum(
                r.get("function_calls", 0) for r in agent_results.values()
            )
            ...
            return PatternResult.success_result(
                ...
                reasoning=synthesis.get(
                    "investigation_summary", "Multi-agent investigation completed"
                ),
            )
            ...
            return PatternResult.error_result(
                error_message=f"Multi-agent coordination failed: {str(e)}"
            )

    async def _execute_parallel_agents(
        self, agents: List[str], messages: List[BaseMessage], context: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _execute_sequential_agents(
        ...
        for i, agent in enumerate(agents):
            ...
            if result.get("success") and result.get("findings"):
                current_context.setdefault("findings", {}).update(
    ...
    async def _execute_single_agent(
        self, agent_type: str, messages: List[BaseMessage], context: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            assistant = (
                await self._assistant_manager.get_or_create_specialized_assistant(
    ...
    async def _initialize_components(self):
        ...
        if not self._coordinator:
            self._coordinator = MultiAgentCoordinator(
                self._openai_client, self.tools, self.ws_streaming
            )
        ...
        if not self._assistant_manager:
            self._assistant_manager = AssistantManager(
                self._openai_client, self.openai_config
            )
