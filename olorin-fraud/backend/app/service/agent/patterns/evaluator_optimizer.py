...
class EvaluatorOptimizerPattern(BasePattern):
    ...
    def __init__(self, config, tools=None, ws_streaming=None):
        ...
        # Configuration
        self.max_optimization_cycles = config.custom_params.get(
            "max_optimization_cycles", 3
        )
        self.quality_threshold = config.custom_params.get("quality_threshold", 0.8)
        self.improvement_threshold = config.custom_params.get(
        ...
        try:
            ...
            for cycle in range(self.max_optimization_cycles):
                ...
                evaluation = await self._evaluate_result(
                    current_result, messages, context
                )
                ...
                if self.ws_streaming:
                    await self._stream_evaluation_complete(
                        evaluation, cycle + 1, context
                    )

                # Check if optimization is needed
                if (
                    not evaluation.should_optimize
                    or evaluation.overall_score >= self.quality_threshold
                ):
                ...
                optimized_result = await self._optimize_result(
                    current_result, evaluation, messages, context, cycle + 1
                )

                # Check for improvement
                if optimized_result.success:
                    improvement = self._calculate_improvement(
                        current_result, optimized_result, evaluation
                    )
                    ...
                    if improvement >= self.improvement_threshold:
                        ...
                        if self.ws_streaming:
                            await self._stream_optimization_success(
                                cycle + 1, improvement, context
                            )
                    else:
                        # No significant improvement, stop optimization
                        if self.ws_streaming:
                            await self._stream_optimization_plateaued(
                                cycle + 1, improvement, context
                            )
                        break
                else:
            ...
            # Final evaluation
            final_evaluation = await self._evaluate_result(
                current_result, messages, context
            )

            # Enhance result with optimization metadata
            enhanced_result = self._create_enhanced_result(
                current_result, final_evaluation, optimization_history, context
            )

            # Stream optimization completion
            if self.ws_streaming:
                await self._stream_optimization_complete(
                    len(optimization_history), final_evaluation.overall_score, context
                )
            ...
            logger.error(
                f"Evaluator-Optimizer execution failed: {str(e)}", exc_info=True
            )
            ...
            return PatternResult.error_result(
                f"Evaluation-optimization failed: {str(e)}"
            )
    ...
    def _parse_evaluation_result(
        self, eval_content: Any, original_result: PatternResult
    ) -> EvaluationResult:
        ...
        overall_score = sum(
            metric.score * self.criteria_weights[metric.criterion] for metric in metrics
        )
        ...
        should_optimize = (
            ...
            and overall_score
            < original_result.confidence_score + 0.1  # Room for improvement
        )
        ...
        return EvaluationResult(
            ...
            improvement_plan=(
                ["Focus on lower-scoring criteria"] if should_optimize else []
            ),
            should_optimize=should_optimize,
        )

    def _extract_score_for_criterion(
        self, eval_text: str, criterion: EvaluationCriteria
    ) -> float:
    ...
    async def _optimize_result(
        ...
        optimization_messages = original_messages + [
    ...
    def _create_enhanced_result(
        ...
        enhanced_content = {
            ...
            "optimization_process": {
                ...
                "final_quality_met": final_evaluation.overall_score
        ...
        final_confidence = (
            final_result.confidence_score + final_evaluation.overall_score
        ) / 2
    ...
    async def _stream_evaluation_complete(
        self, evaluation: EvaluationResult, cycle: int, context: Dict[str, Any]
    ) -> None:
    ...
    async def _stream_optimization_success(
        self, cycle: int, improvement: float, context: Dict[str, Any]
    ) -> None:
    ...
    async def _stream_optimization_plateaued(
        self, cycle: int, improvement: float, context: Dict[str, Any]
    ) -> None:
    ...
    async def _stream_optimization_failed(
        self, cycle: int, context: Dict[str, Any]
    ) -> None:
    ...
    async def _stream_optimization_complete(
        self, cycles_completed: int, final_score: float, context: Dict[str, Any]
    ) -> None:
        """Stream optimization completion"""
        if self.ws_streaming:
            await self.ws_streaming.send_agent_thought(
