...
class EnhancedInvestigationValidator:
    ...
    def __init__(self):
        ...
        self.verification_enabled = (
            os.getenv("LLM_VERIFICATION_ENABLED", "true").lower() == "true"
        )
        ...
        if is_demo:
            self.verification_threshold = 0.60  # Lower threshold for demo mode
            self.min_evidence_threshold = 2  # Also lower min evidence for demo
            logger.info(
                "🎭 DEMO MODE: Using reduced verification threshold (0.60) and min evidence (2)"
            )
        else:
            self.verification_threshold = (
                float(os.getenv("VERIFICATION_THRESHOLD_DEFAULT", "85")) / 100
            )
            self.min_evidence_threshold = int(os.getenv("MIN_EVIDENCE_THRESHOLD", "3"))

        self.max_risk_delta_threshold = float(
            os.getenv("MAX_RISK_DELTA_THRESHOLD", "0.5")
        )

        logger.info(
            f"Enhanced Validator initialized: verification={self.verification_enabled}, "
    ...
    async def validate_investigation(
        ...
        logger.info(
            f"🔍 Starting enhanced validation for investigation {investigation_id}"
        )
        ...
        risk_delta = (
            abs(initial_risk - final_risk) if initial_risk and final_risk else 1.0
        )
        ...
        if agent_results:
            try:
                quality_assessment = (
                    await self.quality_assurance.validate_investigation_results(
                        investigation_id, agent_results, ValidationLevel.COMPREHENSIVE
                    )
                )
        ...
        if (
            self.verification_enabled
            and extraction_status != DataExtractionStatus.FAILED
        ):
            (
                llm_verification_score,
                llm_verification_passed,
                llm_verification_details,
        ...
        if extraction_status in [
            ...
            DataExtractionStatus.STRING_FORMAT_ERROR,
        ]:
            critical_issues.append(
                f"Data extraction failed: {', '.join(extraction_failures)}"
            )

        if (
            not risk_consistency_passed
            and extraction_status == DataExtractionStatus.SUCCESS
        ):
            # CRITICAL FIX: Handle None values before formatting
            initial_risk_str = (
                f"{initial_risk:.2f}" if initial_risk is not None else "None"
            )
            final_risk_str = f"{final_risk:.2f}" if final_risk is not None else "None"
            delta_str = f"{risk_delta:.2f}" if risk_delta is not None else "None"
            critical_issues.append(
        ...
        if (
            quality_assessment
            and quality_assessment.overall_quality_score < quality_threshold
        ):
            warnings.append(
                f"Low quality score: {quality_assessment.overall_quality_score:.2f} (threshold: {quality_threshold})"
            )

        # Generate recommendations
        if extraction_failures:
            recommendations.append(
                "Fix Snowflake data format to return structured JSON instead of strings"
            )

        if not risk_consistency_passed:
            recommendations.append(
                "Investigate why risk assessment changed dramatically from initial analysis"
            )

        if not minimum_evidence_met:
            recommendations.append(
                "Ensure more data sources are analyzed for comprehensive risk assessment"
            )
        ...
        if validation_status == ValidationStatus.CRITICAL_FAILURE:
            ...
            overall_score = min(
                overall_score, 40.0
            )  # Cap score for failed investigations
    ...
    def _extract_final_risk(
        self, investigation_result: Dict[str, Any], agent_results: Dict[str, Any]
    ) -> Optional[float]:
    ...
    def _validate_data_extraction(
        ...
        if snowflake_data:
            if isinstance(snowflake_data, str):
                failures.append(
                    "Database data is string format, cannot extract structured results"
                )
            ...
            for agent_name, result in agent_results.items():
                if isinstance(result, dict):
                    ...
                    if "extraction_error" in result or "parse_error" in result:
                        ...
                        if risk_score == 0.0 and agent_name in [
                            "device",
                            "location",
                            "logs",
                        ]:
                            ...
                            if isinstance(findings, dict):
                                ...
                                if all(
                                    ...
                                    failures.append(
                                        f"{agent_name}: Suspicious 0.00 risk with empty analysis (likely parsing failure)"
                                    )
                                else:
                                    any_success = True
                            else:
                    ...
                    findings = result.get("findings")
                    if (
                        isinstance(findings, str)
                        and "string format" in findings.lower()
                    ):
                        failures.append(
                            f"{agent_name}: String format error in findings"
                        )
        ...
        if len(suspicious_zeros) >= 2:
            failures.insert(
                0,
                f"CRITICAL: {len(suspicious_zeros)} agents returned 0.00 risk with empty data (parsing failure suspected)",
            )
        ...
        if failures and not any_success:
            ...
            return DataExtractionStatus.SUCCESS, (
                [] if not suspicious_zeros else failures
            )
        else:
    ...
    def _validate_risk_consistency(
        ...
        if extraction_status in [
            ...
            logger.warning(
                "Cannot validate risk consistency due to data extraction failure"
            )
            return False
        ...
        if initial_risk is None or final_risk is None:
            logger.warning(
                f"Missing risk scores: initial={initial_risk}, final={final_risk}"
            )
            return False
    ...
    async def _perform_llm_verification(
        ...
        try:
            ...
            if (
                not hasattr(self.llm_manager, "verification_model")
                or not self.llm_manager.verification_model
            ):
                logger.warning(
                    "Verification model not available, skipping LLM verification"
                )
