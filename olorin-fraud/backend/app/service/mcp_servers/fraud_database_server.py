...
class TransactionSearchInput(BaseModel):
    ...
    user_id: Optional[str] = Field(
        None, description="User ID to search transactions for"
    )
    device_id: Optional[str] = Field(
        None, description="Device ID to search transactions for"
    )
    start_date: Optional[str] = Field(
        None, description="Start date for transaction search (ISO format)"
    )
    end_date: Optional[str] = Field(
        None, description="End date for transaction search (ISO format)"
    )
    ...
    transaction_type: Optional[str] = Field(
        None, description="Type of transaction to filter"
    )
...
class DeviceFingerprintInput(BaseModel):
    ...
    include_history: bool = Field(
        False, description="Include historical fingerprint data"
    )
...
class FraudPatternInput(BaseModel):
    ...
    confidence_threshold: float = Field(
        0.7, description="Minimum confidence threshold (0-1)"
    )
    time_window_hours: int = Field(
        24, description="Time window for pattern search in hours"
    )
...
class RiskScoreInput(BaseModel):
    ...
    transaction_id: Optional[str] = Field(
        None, description="Specific transaction to evaluate"
    )
    include_factors: bool = Field(
        True, description="Include contributing factors in response"
    )
...
async def search_transaction_history(
    ...
    try:
        ...
        logger.info(
            f"Searching transactions: user={user_id}, device={device_id}, limit={limit}"
        )
        ...
        results = {
            ...
            "query_parameters": {
                ...
                "device_id": device_id,
                "date_range": (
                    f"{start_date} to {end_date}" if start_date and end_date else None
                ),
                "amount_range": (
                    f"{min_amount} to {max_amount}"
...
async def lookup_device_fingerprint(
    device_id: str, include_history: bool = False
) -> Dict[str, Any]:
    """
...
async def match_fraud_patterns(
    ...
    try:
        logger.info(
            f"Matching fraud patterns: type={pattern_type}, threshold={confidence_threshold}"
        )
        ...
        if pattern_type not in known_patterns:
            pattern_results["warning"] = (
                f"Unknown pattern type. Known types: {known_patterns}"
            )
...
async def calculate_risk_score(
    user_id: str, transaction_id: Optional[str] = None, include_factors: bool = True
) -> Dict[str, Any]:
    """
    ...
    try:
        logger.info(
            f"Calculating risk score: user={user_id}, transaction={transaction_id}"
        )
...
async def create_fraud_database_server(
    config: Optional[Dict[str, Any]] = None,
) -> FraudDatabaseMCPServer:
    """
