...
class SecureFraudInvestigationTool:
    ...
    def __init__(
        self, auth_service: MCPAuthenticationService, validator: MCPInputValidator
    ):
    ...
    async def investigate_user_fraud(
        ...
        try:
            ...
            logger.info(
                f"User {security_context.user_id} initiated fraud investigation"
            )
            ...
            validation_results = self.validator.validate_mcp_tool_input(
                tool_name="investigate_user_fraud", inputs=investigation_inputs
            )
            ...
            for field_name, result in validation_results.items():
                if not result.is_valid:
                    validation_errors.extend(
                        [f"{field_name}: {error}" for error in result.errors]
                    )
            ...
            if validation_errors:
                ...
                raise HTTPException(
                    status_code=400,
                    detail=f"Input validation failed: {validation_errors}",
                )
            ...
            if "security_context" in locals():
                await self.auth.audit_tool_execution(
                    ...
                    request_data=(
                        investigation_inputs
                        if "investigation_inputs" in locals()
                        else {}
                    ),
                    duration_ms=execution_time,
                )
            ...
            raise HTTPException(
                status_code=500, detail="Investigation failed due to internal error"
            )
                ...
                [
                    {
                        "type": "card_mismatch",
                        "risk_score": 0.9,
                        "details": "Card details don't match profile",
                    },
                    {
                        "type": "velocity_check",
                        "risk_score": 0.7,
                        "details": "High transaction velocity",
                    },
                ]
            )
...
async def query_user_transactions(
    ...
    inputs = {"user_id": user_id, "start_date": start_date, "end_date": end_date}

    validation_results = validator.validate_mcp_tool_input(
        tool_name="query_user_transactions", inputs=inputs
    )
