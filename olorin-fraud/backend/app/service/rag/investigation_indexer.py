...
from app.service.database.models import Document, DocumentChunk, DocumentCollection
...
class InvestigationIndexer:
        ...
        try:
            with get_db_session() as session:
                ...
                if not investigation:
                    logger.warning(
                        f"Investigation {investigation_id} not found or not completed"
                    )
                    return False

                text_content, entities_list = self.format_investigation_for_rag(
                    investigation
                )
                ...
                logger.info(
                    f"Indexed investigation {investigation_id} with {len(chunks)} chunks"
                )
                return True
    ...
    def format_investigation_for_rag(
                            ...
                            for i, tx in enumerate(
                                ...
                                if isinstance(tx, dict):
                                    ...
                                    if "tx_id" in tx or "transaction_id" in tx:
                                        tx_parts.append(
                                            f"ID: {tx.get('tx_id') or tx.get('transaction_id')}"
                                        )
                                    if (
                                        "amount" in tx
                                        or "paid_amount_value_in_currency" in tx
                                    ):
                                        amount = tx.get("amount") or tx.get(
                                            "paid_amount_value_in_currency"
                                        )
                                        currency = tx.get("currency") or tx.get(
                                            "paid_amount_currency", "USD"
                                        )
                                        tx_parts.append(f"Amount: {amount} {currency}")
                                    if (
                                        "date" in tx
                                        or "tx_datetime" in tx
                                        or "timestamp" in tx
                                    ):
                                        date = (
                                            tx.get("date")
            ...
            try:
                ...
                if isinstance(progress, dict):
                    ...
                    if "snowflake_data" in progress:
                        ...
                        if (
                            isinstance(snowflake_data, dict)
                            and "transactions" in snowflake_data
                        ):
                                ...
                                for i, tx in enumerate(
                                    ...
                                    if isinstance(tx, dict):
                                        ...
                                        if (
                                            "tx_id_key" in tx
                                            or "tx_id" in tx
                                            or "transaction_id" in tx
                                        ):
                                            tx_parts.append(
                                                f"ID: {tx.get('tx_id_key') or tx.get('tx_id') or tx.get('transaction_id')}"
                                            )
                                        if (
                                            "paid_amount_value_in_currency" in tx
                                            or "amount" in tx
                                        ):
                                            amount = tx.get(
                                                "paid_amount_value_in_currency"
                                        ...
                                        if "is_fraud_tx" in tx or "is_fraud" in tx:
                                            is_fraud = tx.get("is_fraud_tx") or tx.get(
                                                "is_fraud"
                                            )
                                            tx_parts.append(f"Fraud Flag: {is_fraud}")
                                        if tx_parts:
                    ...
                    if "tool_executions" in progress:
                        ...
                        if isinstance(tool_executions, list):
                            for tool_exec in tool_executions[
                                ...
                                if (
                                    isinstance(tool_exec, dict)
                                    and "result" in tool_exec
                                ):
                                    result = tool_exec["result"]
                                            ...
                                            if isinstance(tx_data, list) and tx_data:
                                                ...
                                                for tx in tx_data[:5]:  # Show first 5
                                                    if isinstance(tx, dict):
                                                        ...
                                                        if (
                                                            "amount" in tx
                                                            or "paid_amount_value_in_currency"
                                                            in tx
                                                        ):
                                                            amount = tx.get(
                                                                "amount"
        ...
        async with self.db_config.session() as session:
            ...
            if self.db_config.is_postgresql:
                ...
                if row:
                    ...
                    if "meta_data" in row_dict and isinstance(
                        row_dict["meta_data"], str
                    ):
                        try:
    ...
    async def index_new_investigations(self) -> int:
        ...
        with get_db_session() as session:
            ...
            all_investigations_query = (
                select(InvestigationState)
                .order_by(InvestigationState.updated_at.desc())
                .limit(10)
            )
            all_investigations = (
                session.execute(all_investigations_query).scalars().all()
            )
            ...
            logger.info(
                f"Found {len(investigations)} investigations with status='COMPLETED'"
            )

            # If none found with COMPLETED, try other variations
            if len(investigations) == 0:
                logger.debug(
                    "No investigations with status='COMPLETED', checking other status values..."
                )
                alt_query = (
                    select(InvestigationState)
                    .where(
                        InvestigationState.status.in_(
                            ["completed", "COMPLETE", "complete", "done", "DONE"]
                        )
                    )
                    .order_by(InvestigationState.updated_at.desc())
                )
                ...
                if alt_investigations:
                    logger.info(
                        f"Found {len(alt_investigations)} investigations with alternative status values"
                    )
            ...
            for investigation in investigations:
                ...
                async with self.db_config.session() as vec_session:
                    ...
                    if self.db_config.is_postgresql:
                        ...
                        doc_query = select(Document).where(
                            Document.collection_id == collection_id,
                            Document.meta_data["investigation_id"].astext
                        ...
                        doc_result = vec_session.execute(
                            query_text,
                            {
                                "collection_id": collection_id,
                        ...
                        if rows:
                            ...
                            if doc_id:
                                ...
                                if self.db_config.is_postgresql:
                                    doc_orm_result = await vec_session.execute(
                                        doc_query
                                    )
                                else:
                    ...
                    if existing_doc:
                        ...
                        doc_updated = (
                            datetime.fromisoformat(meta_data.get("updated_at", ""))
                            if meta_data
                            else None
                        )
                ...
                try:
                    if await self.index_investigation(investigation.investigation_id):
                        indexed_count += 1
                        logger.debug(
                            f"Successfully indexed investigation {investigation.investigation_id}"
                        )
                    else:
                        logger.debug(
                            f"Skipped indexing investigation {investigation.investigation_id} (already up to date or failed)"
                        )
                except Exception as e:
                    logger.error(
        ...
        logger.info(
            f"Indexed {indexed_count} new/updated investigations out of {len(investigations)} checked"
        )
