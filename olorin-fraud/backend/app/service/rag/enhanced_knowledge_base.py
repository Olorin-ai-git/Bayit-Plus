...
from app.service.rag.vector_rag_service import SearchResult, get_rag_service
...
class EnhancedKnowledgeBase:
        ...
        self.investigation_categories = {
            ...
            "money_laundering": {
                ...
                "tools": [
                    ...
                    "cash_intensity_calculator",
                    "suspicious_activity_reporter",
                ],
            },
            "account_takeover": {
                "keywords": [
    ...
    async def initialize(self):
        ...
        try:
            ...
            if empty_collections:
                logger.info(
                    f"Found {len(empty_collections)} empty collections - consider populating with domain knowledge"
                )
        ...
        required_collections = [
            {
                ...
                "metadata_schema": {
                    "fraud_type": {
                        "type": "string",
                        "enum": list(self.investigation_categories.keys()),
                    },
                    "severity": {
                        "type": "string",
        ...
        for collection_spec in required_collections:
            if collection_spec["name"] not in existing_names:
                try:
                    ...
                    logger.warning(
                        f"Failed to create collection {collection_spec['name']}: {e}"
                    )
        ...
        try:
            ...
            for result in search_results:
                enhanced_result = await self._enhance_search_result(
                    result, investigation_type
                )
                enhanced_results.append(enhanced_result)

            # Sort by relevance and investigation applicability
            enhanced_results = self._rank_results_by_context(
                enhanced_results, investigation_type
            )
    ...
    async def _enhance_query_with_context(
        self, query: str, investigation_type: Optional[str]
    ) -> str:
        """Enhance search query with investigation context."""
        if (
            not investigation_type
            or investigation_type not in self.investigation_categories
        ):
            return query
    ...
    async def _enhance_search_result(
        self, result: SearchResult, investigation_type: Optional[str]
    ) -> KnowledgeResult:
        ...
        if investigation_type:
            ...
            applicable_types = self._analyze_content_for_investigation_types(
                result.content
            )
        ...
        for inv_type in applicable_types:
            if inv_type in self.investigation_categories:
                applicable_tools.extend(
                    self.investigation_categories[inv_type]["tools"]
                )
        ...
        confidence_level = (
            ...
            else "medium" if result.similarity_score > 0.6 else "low"
        )
    ...
    def _rank_results_by_context(
        self, results: List[KnowledgeResult], investigation_type: Optional[str]
    ) -> List[KnowledgeResult]:
    ...
    async def add_domain_knowledge(
        ...
        try:
            ...
            if result.success:
                logger.info(
                    f"✅ Added domain knowledge: {title} ({result.chunk_count} chunks)"
                )
                return True
            else:
    ...
    async def get_investigation_context(self, investigation_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            search_queries = await self._get_investigation_search_history(
                investigation_id
            )
            ...
            if search_queries:
                ...
                context["recommended_knowledge"] = (
                    await self._suggest_knowledge_based_on_searches(search_queries)
                )
    ...
    async def _get_investigation_search_history(
    ...
    async def _suggest_knowledge_based_on_searches(
        ...
        for fraud_type, info in self.investigation_categories.items():
            keyword_matches = sum(
                1 for keyword in info["keywords"] if keyword in all_keywords
            )
            if keyword_matches > 0:
                suggestions.append(f"Review {fraud_type} investigation procedures")
                suggestions.extend(
                    [f"Consider using {tool}" for tool in info["tools"][:2]]
                )
