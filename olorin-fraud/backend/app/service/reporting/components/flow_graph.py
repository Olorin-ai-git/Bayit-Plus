...
class InvestigationFlowComponent(BaseVisualizationComponent):
    ...
    def process_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        avg_phase_duration = (
            total_duration / len(phase_durations) if phase_durations else 0
        )
        transition_count = len(processed_phases)

        # Identify critical path (phases with longest durations)
        critical_phases = sorted(
            phase_durations.items(), key=lambda x: x[1], reverse=True
        )[:3]
    ...
    def _analyze_flow_patterns(self, phases: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        durations = [
            p.get("duration_ms", 0) for p in phases if p.get("duration_ms", 0) > 0
        ]
        if durations:
    ...
    def generate_html(self, processed_data: Dict[str, Any]) -> str:
        ...
        stats_items = [
            ...
            (
                "Status",
                metrics.get("completion_status", "unknown").replace("_", " ").title(),
            ),
        ]
        ...
        for phase in processed_phases[:15]:  # Limit to first 15
            duration_display = (
                f"{phase['duration_ms']}ms" if phase["duration_ms"] > 0 else "N/A"
            )
    ...
    def _generate_mermaid_diagram(self, processed_data: Dict[str, Any]) -> str:
        ...
        completion_status = processed_data.get("metrics", {}).get(
