...
class RedisCloudClient:
    ...
    def _create_client(self) -> Redis:
        ...
        if not redis_password:
            logger.warning(
                "Redis password not found, attempting connection without authentication"
            )
        ...
        if redis_password:
            ...
            logger.info(
                f"Connecting to Redis Cloud at {host}:{port} with authentication"
            )
        else:
    ...
    def _test_connection(self) -> None:
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"Connected to Redis version: {info.get('redis_version', 'unknown')}"
                )
            except Exception:
...
def test_redis_connection(settings: SvcSettings) -> bool:
    ...
    try:
        ...
        if retrieved != test_value:
            logger.error(
                f"Retrieved value '{retrieved}' doesn't match expected '{test_value}'"
            )
            return False
