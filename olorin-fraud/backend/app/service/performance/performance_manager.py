        ...
        try:
            ...
            self._config = PerformanceOptimizationConfig(
                database_url=os.getenv(
                    "DATABASE_URL", "sqlite:///olorin_fraud_detection.db"
                ),
                redis_host=os.getenv("REDIS_HOST", "localhost"),
                redis_port=int(os.getenv("REDIS_PORT", "6379")),
                max_parallel_agents=int(os.getenv("MAX_PARALLEL_AGENTS", "8")),
                enable_alerts=os.getenv("ENABLE_PERFORMANCE_ALERTS", "true").lower()
                == "true",
            )

            self._init_result = await initialize_performance_optimization_system(
                self._config
            )
            ...
            if self._init_result.get("status") == "success":
                ...
                for metric, improvement in improvements.items():
                    ...
                    logger.info(
                        f"  - {metric}: {target_improvement} improvement target"
                    )
