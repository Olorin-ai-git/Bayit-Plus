...
class StructuredInvestigationLogger:
    ...
    def __init__(
        self, log_directory: Optional[Path] = None, enable_console_output: bool = True
    ):
        ...
        _get_bridge_logger().info(
            f"Initialized StructuredInvestigationLogger with directory: {self.log_directory}"
        )
    ...
    def start_investigation_logging(
        ...
        investigation_logger = logging.getLogger(
            f"structured_investigation_{investigation_id}"
        )
        ...
        self._log_interaction(
            investigation_id,
            InteractionType.INVESTIGATION_PROGRESS,
            asdict(progress_log),
        )

        _get_bridge_logger().info(
            f"Started investigation logging for: {investigation_id} in {folder_path}"
        )
        return folder_path
    ...
    def log_llm_interaction(
        ...
        self._log_interaction(
            investigation_id, InteractionType.LLM_CALL, asdict(llm_log)
        )
        ...
        if self.enable_console_output:
            ...
            _get_bridge_logger().info(
                f"        🧠 LLM Call: {agent_name} → {model_name}"
            )
            _get_bridge_logger().info(
                f"           Tools: {tools_used_str} | Tokens: {tokens_used.get('total_tokens', 0)}"
            )
            if reasoning_chain:
                _get_bridge_logger().info(
                    f"           Reasoning: {reasoning_chain[:100]}{'...' if len(reasoning_chain) > 100 else ''}"
                )
    ...
    def log_agent_decision(
        ...
        self._log_interaction(
            investigation_id, InteractionType.AGENT_DECISION, asdict(decision_log)
        )

        if self.enable_console_output:
            _get_bridge_logger().info(
                f"        🧠 Decision: {agent_name} → {decision_type}"
            )
            _get_bridge_logger().info(
    ...
    def log_tool_execution(
        ...
        self._log_interaction(
            investigation_id, InteractionType.TOOL_EXECUTION, asdict(tool_log)
        )
        ...
        try:
            ...
            _get_bridge_logger().error(
                f"Failed to persist tool execution to database: {str(e)}", exc_info=True
            )
        ...
        if self.enable_console_output:
            ...
            _get_bridge_logger().info(
                f"        🔧 Tool: {agent_name} → {tool_name} {status_icon}"
            )
            _get_bridge_logger().info(
                f"           Duration: {execution_time_ms}ms | Status: {status_text}"
            )
            _get_bridge_logger().info(
                f"           Params: {str(tool_parameters)[:80]}{'...' if len(str(tool_parameters)) > 80 else ''}"
            )
            if selection_reasoning:
                _get_bridge_logger().info(
                    f"           Selection: {selection_reasoning[:80]}{'...' if len(selection_reasoning) > 80 else ''}"
                )
    ...
    def log_langgraph_node(
        ...
        def _summarize_state_for_logging(state: Dict[str, Any]) -> Dict[str, Any]:
            ...
            if (
                "snowflake_data" in summarized_state
                and summarized_state["snowflake_data"]
            ):
        ...
        self._log_interaction(
            investigation_id, InteractionType.LANGGRAPH_NODE, asdict(node_log)
        )
        ...
        if self.enable_console_output:
            ...
            _get_bridge_logger().info(
                f"           Duration: {execution_time_ms}ms → {next_nodes_str}"
            )
    ...
    def log_investigation_progress(
        ...
        if self.enable_console_output:
            progress_pct = (
                len(completed_phases) / max(4, len(completed_phases) + 1)
            ) * 100
            _get_bridge_logger().info(
                f"        📈 Progress: {progress_type} → {current_phase}"
            )
            _get_bridge_logger().info(
                f"           Completion: {progress_pct:.0f}% | Phases: {len(completed_phases)} completed"
            )

            # Show risk score progression if available
            if risk_score_progression:
                latest_risk = (
                    risk_score_progression[-1] if risk_score_progression else {}
                )
    ...
    def complete_investigation_logging(
        self, investigation_id: str, final_status: str = "completed"
    ) -> None:
        """
        ...
        try:
            ...
            _get_bridge_logger().warning(
                f"Failed to generate HTML report for investigation {investigation_id}: {e}"
            )

        _get_bridge_logger().info(
            f"Completed investigation logging for: {investigation_id} with status: {final_status}"
        )
    ...
    def _generate_html_report(self, investigation_id: str) -> None:
        ...
        try:
            ...
            if not folder_path:
                _get_bridge_logger().warning(
                    f"Investigation folder not found for {investigation_id}"
                )
                return

            # Generate comprehensive HTML report using the unified generator
            try:
                from ..reporting.comprehensive_investigation_report import (
                ...
                _get_bridge_logger().info(
                    f"✅ Generated comprehensive HTML report: {report_path}"
                )
                ...
                for old_report in old_reports:
                    if old_report.exists():
                        try:
                            old_report.unlink()
                            _get_bridge_logger().debug(
                                f"🗑️ Removed old report: {old_report}"
                            )
                        except Exception as e:
                            _get_bridge_logger().warning(
                ...
                _get_bridge_logger().warning(
                    f"Comprehensive report generator not available, using simple generator: {import_error}"
                )
                ...
                _get_bridge_logger().info(
                    f"Generated simple HTML report: {report_file}"
                )

        except Exception as e:
            _get_bridge_logger().error(
                f"Failed to generate HTML report for {investigation_id}: {e}"
            )
        ...
        if metadata_file.exists():
            try:
                with open(metadata_file) as f:
                    ...
                    if investigation_id:
                        confusion_table_file = (
                            folder_path / f"confusion_table_{investigation_id}.html"
                        )
...
<body>
    <div class="container">
        ...
        for activity in activities:
            ...
            if activity_type in ["agent_decision", "investigation_progress"]:
                risk_score = data.get("decision_outcome", {}).get(
                    "risk_score"
                ) or data.get("findings_summary", {}).get("risk_score", 0)
    ...
    def _log_interaction(
        ...
        if (
            investigation_logger.handlers
        ):  # Only log if investigation-specific logger exists
    ...
    def get_investigation_logs(
        self, investigation_id: str, interaction_types: List[InteractionType] = None
    ) -> List[Dict[str, Any]]:
    ...
    def generate_investigation_summary(self, investigation_id: str) -> Dict[str, Any]:
        ...
        llm_calls = [
            log
            for log in logs
            if log["interaction_type"] == InteractionType.LLM_CALL.value
        ]
        total_tokens = sum(
            log["data"].get("tokens_used", {}).get("total_tokens", 0)
        ...
        agent_decisions = [
            log
            for log in logs
            if log["interaction_type"] == InteractionType.AGENT_DECISION.value
        ]
        avg_confidence = sum(
            log["data"].get("confidence_score", 0) for log in agent_decisions
        ) / max(len(agent_decisions), 1)

        progress_logs = [
            log
            for log in logs
            if log["interaction_type"] == InteractionType.INVESTIGATION_PROGRESS.value
        ]
...
structured_investigation_logger = StructuredInvestigationLogger(
