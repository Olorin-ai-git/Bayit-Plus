...
class InvestigationMetadata:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        data["mode"] = (
            data["mode"].value
            if isinstance(data["mode"], InvestigationMode)
            else data["mode"]
        )
        return data
...
class InvestigationFolderManager:
    ...
    def create_investigation_folder(
        ...
        folder_path = self._file_org_service.resolve_investigation_log_path(
            mode=mode.value, investigation_id=investigation_id, timestamp=timestamp_dt
        )
    ...
    def update_investigation_status(self, investigation_id: str, status: str) -> None:
        ...
        if investigation_id in self._active_investigations:
            ...
            folder_path = Path(
                self._active_investigations[investigation_id].folder_path
            )
            metadata_file = folder_path / "metadata.json"
            if metadata_file.exists():
                with open(metadata_file, "w") as f:
                    json.dump(
                        self._active_investigations[investigation_id].to_dict(),
                        f,
                        indent=2,
                    )

    def list_investigations(
        self, mode: Optional[InvestigationMode] = None
    ) -> List[InvestigationMetadata]:
        """
                ...
                if metadata_file.exists():
                    try:
                        with open(metadata_file) as f:
                            ...
                            if "mode" in metadata_dict and isinstance(
                                metadata_dict["mode"], str
                            ):
                                metadata_dict["mode"] = InvestigationMode(
                                    metadata_dict["mode"]
                                )
        ...
        for metadata in investigations:
            ...
            if created_at < cutoff_date:
                try:
                    ...
                    print(
                        f"Error cleaning up investigation {metadata.investigation_id}: {e}"
                    )
