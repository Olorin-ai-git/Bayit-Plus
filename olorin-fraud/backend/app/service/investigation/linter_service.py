...
class LinterService:
    ...
    def __init__(self, rule_severities: Optional[Dict[str, LinterSeverity]] = None):
        """
    ...
    def lint_investigation(
        self, investigation_data: Dict[str, Any], investigation_id: Optional[str] = None
    ) -> List[LinterIssue]:
        """
        ...
        issues.extend(
            self._check_risk_score_consistency(investigation_data, investigation_id)
        )

        # Rule 2: Tool usage validation
        issues.extend(
            self._check_tool_usage_validation(investigation_data, investigation_id)
        )
    ...
    def _check_risk_score_consistency(
        self, data: Dict[str, Any], investigation_id: Optional[str] = None
    ) -> List[LinterIssue]:
        """
        ...
        final_risk_score = data.get("final_risk_score") or data.get(
            "overall_risk_score"
        )
            ...
            issues.append(
                LinterIssue(
                    ...
                    details={
                        ...
                        "final_risk_score": final_risk_score,
                        "difference": abs(
                            float(state_risk_score) - float(final_risk_score)
                        ),
                    },
                )
            )
    ...
    def _check_end_time(
        ...
        end_time = (
            data.get("end_time") or data.get("completed_at") or data.get("finished_at")
        )
        ...
        if not end_time:
            issues.append(
                LinterIssue(
                    ...
                    resource_id=investigation_id,
                    details={"status": status, "has_end_time": False},
                )
            )
