...
class TestGraphSelector:
    ...
    async def test_get_investigation_graph_default_clean(self, graph_selector):
        ...
        with patch.object(
            graph_selector, "_build_clean_graph", new_callable=AsyncMock
        ) as mock_clean:
    ...
    async def test_get_investigation_graph_forced_hybrid(self, graph_selector):
        ...
        # Mock the graph building methods
        with patch.object(
            graph_selector, "_build_hybrid_graph", new_callable=AsyncMock
        ) as mock_hybrid:
            mock_hybrid.return_value = Mock()

            # Act
            graph = await graph_selector.get_investigation_graph(
                "test-001", force_graph_type=GraphType.HYBRID
            )
    ...
    async def test_get_investigation_graph_forced_orchestrator(self, graph_selector):
        ...
        # Mock the graph building methods
        with patch.object(
            graph_selector, "_build_orchestrator_graph", new_callable=AsyncMock
...
class TestGlobalFunctions:
    ...
    async def test_get_investigation_graph_global(self):
        ...
        with patch(
            "app.service.agent.orchestration.hybrid.migration_utilities.GraphSelector"
            ...
            mock_selector.get_investigation_graph.assert_called_once_with(
                investigation_id="test-001", entity_type="ip", force_graph_type=None
            )
            assert graph is not None
    ...
    def test_enable_hybrid_graph_global(self):
        ...
        with patch(
            "app.service.agent.orchestration.hybrid.migration_utilities.get_feature_flags"
        ) as mock_get_flags:
            mock_flags = Mock()
    ...
    def test_disable_hybrid_graph_global(self):
        ...
        with patch(
            ...
            # Assert
            mock_flags.disable_flag.assert_called_once_with(
                "hybrid_graph_v1", "test_disable"
            )
    ...
    def test_start_ab_test_global(self):
        ...
        with patch(
            ...
            assert (
                mock_flags.flags["ab_test_hybrid_vs_clean"]["rollout_percentage"] == 100
            )
    ...
    def test_stop_ab_test_global(self):
        ...
        with patch(
            ...
            # Assert
            mock_flags.disable_flag.assert_called_once_with(
                "ab_test_hybrid_vs_clean", "ab_test_complete"
            )
