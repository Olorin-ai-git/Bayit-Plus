...
class TestStructuredModeValidation:
    ...
    async def test_llm_driven_tool_selection_coverage(
        self, structured_graph, investigation_context
    ):
        """
            ...
            with patch(
                ...
                mock_findings.raw_data = {
                    "tool_selection_evidence": [
                        "Used splunk_query_tool based on LLM decision"
                    ],
                ...
                config = {
                    "configurable": {
                        "agent_context": self._create_mock_agent_context(
    ...
    async def test_investigation_quality_score(
        self, structured_graph, investigation_context
    ):
        """
        ...
        for i in range(num_tests):
            ...
            with patch(
                ...
                quality_score = self._calculate_investigation_quality(
                    result, mock_findings
                )
    ...
    async def test_tool_selection_accuracy(self):
        ...
        for scenario in scenarios:
            with patch(
                "app.service.agent.structured_agents.structured_llm"
            ) as mock_llm:
    ...
    async def test_completion_time_performance(
        self, structured_graph, investigation_context
    ):
        """
        ...
        assert (
            time_ratio <= 150.0 or average_time < 1.0
        ), (  # Allow for fast mock execution
            ...
            try:
                with patch(
                    ...
                    if i == 0:  # Simulate 1 failure out of 100 (1% failure rate)
                        mock_instance.structured_investigate.side_effect = Exception(
                            "Simulated failure"
                        )
                    else:
                        mock_findings = MagicMock()
                        mock_findings.domain = "reliability_test"
                        mock_instance.structured_investigate.return_value = (
                            mock_findings
                        )
    ...
    async def test_recursion_guard_effectiveness(self):
        ...
        for i in range(5):
            assert guard.record_tool_call(
                "recursion_test_001", "thread_001", f"tool_{i}"
            )

        # Should prevent excessive tool calls
        assert not guard.record_tool_call(
            "recursion_test_001", "thread_001", "overflow_tool"
        )
    ...
    def test_structured_context_comprehensive_coverage(self):
        ...
        for section in required_sections:
            assert (
                section in llm_context
            ), f"Missing required context section: {section}"
    ...
    def _verify_structured_tool_selection(self, result: Dict) -> bool:
        ...
        try:
            ...
            risk_assessment = message_content.get(
                "risk_assessment"
            ) or message_content.get("llm_assessment")
    ...
    def _calculate_investigation_quality(
        self, result: Dict, findings: MagicMock
    ) -> float:
        ...
        try:
            message_content = json.loads(result["messages"][0].content)
            assessment = message_content.get("llm_assessment") or message_content.get(
                "risk_assessment", {}
            )
    ...
    def _verify_tool_selection_accuracy(
        self,
        selected_tools: List[str],
        expected_tools: List[str],
        inappropriate_tools: List[str],
    ) -> bool:
