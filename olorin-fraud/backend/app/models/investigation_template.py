...
class InvestigationTemplate(Base):
    ...
    # Ownership
    user_id = Column(
        String(255), nullable=False, index=True, comment="User who owns this template"
    )

    # Template Metadata
    name = Column(
        String(255), nullable=False, comment="Template name (unique per user)"
    )

    description = Column(Text, nullable=True, comment="Template description")

    tags = Column(
        Text, nullable=True, comment="Comma-separated tags for categorization"
    )
    ...
    last_used = Column(
        DateTime, nullable=True, comment="When template was last applied"
    )
