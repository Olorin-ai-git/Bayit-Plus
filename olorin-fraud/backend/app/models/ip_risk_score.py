...
class IPRiskScoreRequest(BaseModel):
    ...
    transaction_amount: Optional[float] = Field(
        None, ge=0, description="Transaction amount"
    )
...
class IPRiskScore(BaseModel):
    ...
    geolocation_data: Optional[Dict[str, Any]] = Field(
        None, description="Geolocation data (JSON)"
    )
    velocity_signals: Optional[Dict[str, Any]] = Field(
        None, description="Velocity signals (JSON)"
    )
    scored_at: datetime = Field(
        default_factory=datetime.utcnow, description="Timestamp when scored"
    )
