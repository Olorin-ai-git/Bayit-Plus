...
class ComposioActionAudit(Base):
    ...
    action_id = Column(
        String(255), nullable=False, comment="Unique action execution identifier"
    )
    ...
    connection_id = Column(
        String(255), nullable=False, comment="Composio connection ID used for execution"
    )

    # Action Parameters and Results
    parameters = Column(Text, nullable=True, comment="JSON object of action parameters")

    result = Column(Text, nullable=True, comment="JSON object of action result")
    ...
    __table_args__ = (
        CheckConstraint(
            ...
            name="chk_composio_audit_status",
        ),
