...
async def execute_entityless_investigation(
    investigation_id: str, anomaly_id: str, investigation_context: Dict[str, Any]
) -> Dict[str, Any]:
    """
    ...
    """
    logger.info(
        f"🚀 Starting entityless investigation {investigation_id} for anomaly {anomaly_id}"
    )
    ...
    try:
        ...
        # Execute workflow
        logger.info(
            f"🔄 Invoking entityless investigation graph for {investigation_id}"
        )
        try:
            result = await graph.ainvoke(initial_state, config=config)
            logger.info(
                f"✅ Graph execution completed for {investigation_id}. Result keys: {list(result.keys()) if result else 'None'}"
            )
        except Exception as graph_error:
            logger.error(
                f"❌ Graph execution failed for {investigation_id}: {graph_error}",
                exc_info=True,
            )
            raise
        ...
        logger.error(
            f"Error executing entityless investigation {investigation_id}: {e}",
            exc_info=True,
        )
        raise
    finally:
...
async def _calculate_domain_risk_scores(
    ...
    logger.info(
        f"🧠 Calculating domain risk scores using LLM for investigation {investigation_id}"
    )
    ...
    for domain, domain_segment_list in domain_segments.items():
        ...
        logger.info(
            f"   Analyzing {domain} domain: {len(domain_segment_list)} segments"
        )
        ...
        if domain_segment_list:
            avg_importance = sum(
                s.get("importance", 0) for s in domain_segment_list
            ) / len(domain_segment_list)
        ...
        try:
            ...
            if risk_score is None:
                raise ValueError(
                    f"LLM analysis for {domain} domain did not return a risk_score - cannot proceed without real data"
                )
            ...
            if confidence is None:
                raise ValueError(
                    f"LLM analysis for {domain} domain did not return a confidence value - cannot proceed without real data"
                )

            reasoning = llm_analysis.get("reasoning")
            if not reasoning:
                raise ValueError(
                    f"LLM analysis for {domain} domain did not return reasoning - cannot proceed without real data"
                )
            ...
            raise ValueError(
                f"Cannot analyze {domain} domain without LLM analysis: {e}"
            )
    ...
    if not domain_findings:
        ...
        try:
            ...
            if risk_score is None:
                raise ValueError(
                    "LLM analysis did not return a risk_score - cannot proceed without real data"
                )
            ...
            if confidence is None:
                raise ValueError(
                    "LLM analysis did not return a confidence value - cannot proceed without real data"
                )

            reasoning = llm_analysis.get("reasoning")
            if not reasoning:
                raise ValueError(
                    "LLM analysis did not return reasoning - cannot proceed without real data"
                )
            ...
            logger.error(
                f"   ❌ Failed to create risk domain finding: {e}", exc_info=True
            )
            raise ValueError(
                f"Cannot create risk domain finding without LLM analysis: {e}"
            )
...
def _map_segments_to_domains(segments: list) -> Dict[str, list]:
    ...
    domain_mapping = {
        ...
        "risk": [
            ...
            "bin",
            "payment_method",
        ],  # gateway removed - column doesn't exist
...
def _calculate_overall_risk_score(domain_findings: Dict[str, Any]) -> int:
    ...
    if not domain_findings:
        raise ValueError(
            "Cannot calculate overall risk score without domain findings - no real data available"
        )
