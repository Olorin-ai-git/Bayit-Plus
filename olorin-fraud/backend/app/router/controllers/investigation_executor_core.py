...
from app.service.agent.journey_tracker import NodeStatus, NodeType, journey_tracker
...
async def _execute_agent_investigation_phase(
    ...
    structured_investigation_logger.log_investigation_progress(
        ...
        findings_summary={
            "current_phase_description": "EXECUTING REAL LangGraph agents with LLM calls"
        },
    ...
    journey_tracker.track_node_execution(
        ...
        input_state={
            ...
            "agents_ready": ["device", "location", "network", "logs", "risk"],
        },
        output_state={
            "phase": "agent_execution",
    ...
    update_investigation_status(
        investigation_id,
        {"current_phase": "agent_execution", "progress_percentage": 25.0},
    )
    ...
    agent_context = await _create_agent_context(
        investigation_id, investigation_query, request
    )
    ...
    logger.info(
        f"🤖 INVOKING REAL AGENTS: Device, Location, Network, Logs, Risk Analysis"
    )
    ...
    try:
        ...
        time_range = (
            investigation_context.get("time_range") if investigation_context else None
        )
        ...
        graph = await get_investigation_graph(
            investigation_id=investigation_id, entity_type=request.entity_type
        )

        # Set recursion limit based on production/test mode
        recursion_limit = (
            100  # Production mode - use higher limit for real investigations
        )
        ...
        logger.info(
            f"🚀 EXECUTING CLEAN GRAPH ORCHESTRATION (replacing old agent service)"
        )
        ...
        await log_agent_successful_execution(
            investigation_id,
            investigation_query,
            result,
            start_time,
        ...
        logger.exception(
            f"Full agent execution error traceback for investigation {investigation_id}:"
        )
        ...
        try:
            ...
            logger.error(
                f"Failed to update investigation status after agent error: {str(status_error)}"
            )
