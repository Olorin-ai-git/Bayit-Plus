...
class MockSnowflakeClient:
    ...
    async def connect(self, database: str, schema: str):
        logger.info(
            f"❄️ Mock Snowflake connected (Transaction Data: {database}.{schema})"
        )
...
async def demonstrate_multi_tool_execution():
    ...
    tools = [SplunkQueryTool(), SumoLogicQueryTool(), SnowflakeQueryTool()]
    ...
    with (
        patch(
            "app.service.agent.tools.splunk_tool.splunk_tool.SplunkClient",
            MockSplunkClient,
        ),
        patch(
            "app.service.agent.tools.sumologic_tool.sumologic_tool.SumoLogicClient",
            MockSumoLogicClient,
        ),
        patch(
        ...
        messages = [
            {
                "role": "user",
                "content": "Investigate user_fraud_demo for potential fraud",
            }
        ]
        ...
        for tool_call in response.tool_calls:
            ...
            if tool_name == "splunk_query_tool":
                ...
                task = tool._arun(
                    tool_call["args"]["query"], tool_call["args"]["time_range"]
                )
        ...
        for tool_name, task in tasks:
            ...
            if "results" in result:
                ...
                if result["results"] and isinstance(result["results"], list):
                    ...
                    print(
                        f"   📋 Sample: {json.dumps(first_result, indent=6)[:200]}..."
                    )
