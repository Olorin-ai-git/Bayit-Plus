...
class TestCLIContracts:
    ...
    def test_init_command_creates_structure(self):
        ...
        assert (
            "Workspace initialized" in result.output
    ...
    def test_new_command_creates_investigation(self):
        ...
        assert result.exit_code == 0
        assert (
            "Investigation created" in result.output
    ...
    def test_ls_command_lists_investigations(self):
        ...
        result = self.runner.invoke(
            cli, ["--root", str(self.workspace_root), "ls", "investigations"]
        )

        assert result.exit_code == 0
        assert (
            "investigations" in result.output.lower()
    ...
    def test_error_handling_missing_investigation(self):
        ...
        result = self.runner.invoke(
            cli,
            ["--root", str(self.workspace_root), "show", "--id", "nonexistent_inv_123"],
        )
    ...
    def test_error_handling_missing_workspace(self):
        ...
        result = self.runner.invoke(
            cli, ["--root", str(non_existent), "ls", "investigations"]
        )
