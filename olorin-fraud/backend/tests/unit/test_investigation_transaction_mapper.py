...
class TestPerTransactionScoreUsage:
    ...
    @patch(
        "app.service.investigation.investigation_transaction_mapper.get_database_provider"
    )
    @patch(
        "app.service.investigation.investigation_transaction_mapper.build_transaction_query"
    )
    @patch(
        "app.service.investigation.investigation_transaction_mapper.build_entity_where_clause"
    )
    @patch(
        "app.service.investigation.investigation_transaction_mapper.query_isfraud_tx_for_transactions"
    )
    def test_use_per_transaction_scores(
        self, mock_query_isfraud, mock_build_entity, mock_build_query, mock_get_provider
    ):
        ...
        progress_json = json.dumps({"transaction_scores": {"tx_1": 0.75, "tx_2": 0.35}})
    ...
    def test_exclude_transactions_without_scores(
        self, mock_query_isfraud, mock_build_entity, mock_build_query, mock_get_provider
    ):
    ...
    def test_exclude_all_when_no_transaction_scores(
        self, mock_query_isfraud, mock_build_entity, mock_build_query, mock_get_provider
    ):
