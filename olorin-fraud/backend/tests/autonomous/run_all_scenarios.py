...
class FraudScenarioRunner:
    ...
    def _setup_enhanced_console_logger(self):
        ...
        formatter = logging.Formatter("\033[96m[INVESTIGATION]\033[0m %(message)s")
    ...
    async def run_scenario(self, scenario: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.console_logger.info(
            f"Starting comprehensive investigation for {scenario['entity_id']}"
        )
        ...
        try:
            ...
            journey_tracker.start_journey_tracking(
                context.investigation_id, initial_state
            )
            ...
            def console_monitoring_callback(investigation_id, event_type, data):
                if event_type == "node_execution" and data.get("node_type") == "tool":
                    ...
                    print(
                        f"        🔧 Tool Executed: {tool_name} ({duration}ms) - {status}"
                    )
                    ...
                    print(
                        f"        🤝 Agent {coordination_type.title()}: {from_agent} → {to_agent}"
                    )
            ...
            print(
                f"   Entity-based agent selection: {'Device-focused' if scenario['entity_type'] == EntityType.DEVICE_ID else 'User-focused'}"
            )
            ...
            if scenario["entity_type"] == EntityType.DEVICE_ID:
                ...
                self.agent_performance_metrics.append(
                ...
                try:
                    ...
                    if isinstance(result, dict) and "messages" in result:
                        ...
                        if messages and len(messages) > 0:
                            ...
                            print(
                                f"      📝 Raw content preview: {str(content)[:100]}..."
                            )
                            ...
                            if isinstance(content, str):
                                ...
                                if "llm_assessment" in parsed_content:
                                    risk_score = parsed_content["llm_assessment"].get(
                                        "risk_level", 0
                                    )
                                    ...
                                    risk_score = risk_assessment.get(
                                        "risk_level"
                                    ) or risk_assessment.get("overall_risk_score", 0)
                                    extraction_path = (
                                        "risk_assessment.risk_level/overall_risk_score"
                                    )
                                elif "behavioral_analysis" in parsed_content:
                                    risk_score = parsed_content[
                                        "behavioral_analysis"
                                    ].get("risk_level", 0)
                                ...
                                if risk_score > 0:
                                    print(
                                        f"      ✅ Extracted risk score: {risk_score} (path: {extraction_path})"
                                    )
                                else:
                                    print(
                                        f"      ⚠️  No valid risk score found in content structure"
                                    )
                    ...
                    logger.warning(
                        f"Failed to extract risk score from {agent_name} result: {e}"
                    )
            ...
            if risk_scores:
                print(
                    f"   📊 Individual scores: {[f'{score:.3f}' for score in risk_scores]}"
                )
                ...
                logger.info(
                    f"Aggregated risk scores: {risk_scores} → Overall: {overall_risk_score:.3f}"
                )
            else:
            ...
            if self.agent_performance_metrics:
                ...
                for metric in self.agent_performance_metrics[-len(agent_results) :]:
                    print(
                        f"      {metric.agent_name}: {metric.duration_ms}ms ({metric.duration_seconds:.2f}s)"
                    )
                ...
                if len(recent_metrics) > 1:
                    ...
                    slowdown_pct = (
                        (slowest.duration_ms - fastest.duration_ms)
                        / fastest.duration_ms
                    ) * 100
                    print(
                        f"      ⚡ Fastest: {fastest.agent_name} ({fastest.duration_ms}ms)"
                    )
                    print(
                        f"      🐌 Slowest: {slowest.agent_name} ({slowest.duration_ms}ms, {slowdown_pct:.0f}% slower)"
                    )
    ...
    async def run_all_scenarios(self, scenario_ids: List[int] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            print(
                f"   🛠️  Available tools: device, network, location, logs, risk analysis"
            )
        except Exception as e:
        ...
        total_risk_scores = [
            r.get("overall_risk_score", 0)
            for r in self.results
            if "overall_risk_score" in r
        ]
        avg_risk_score = (
            sum(total_risk_scores) / len(total_risk_scores) if total_risk_scores else 0
        )
        ...
        print(f"{'='*80}")
        print(
            f"📅 Execution Period: {self.start_time.strftime('%H:%M:%S')} - {self.end_time.strftime('%H:%M:%S')}"
        )
        ...
        if total_risk_scores:
            ...
            print(
                f"   Range: {min(total_risk_scores):.3f} - {max(total_risk_scores):.3f}"
            )
