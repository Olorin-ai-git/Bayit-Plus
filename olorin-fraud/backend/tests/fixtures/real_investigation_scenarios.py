...
class RealScenarioGenerator:
    ...
    def generate_real_entity_data(
        self, entity_type: str = "merchant"
    ) -> Dict[str, Any]:
    ...
    def create_scenario(
        self, scenario_type: str, risk_level: str = "medium"
            ...
            patterns.append(
                {
                    ...
                    "success": success,
                    "ip": (
                        self._generate_test_ip()
                        if pattern_type != "normal"
                        else self._generate_test_ip()[:-1] + "1"
                    ),
    ...
    def _get_expected_indicators(
        self, scenario_type: str, risk_level: str
    ) -> List[str]:
...
def get_test_scenarios() -> List[RealInvestigationScenario]:
    ...
    for risk in ["high", "critical"]:
        scenarios.append(
            generator.create_scenario("authentication_impossible_travel", risk)
        )
        scenarios.append(
            generator.create_scenario("authentication_credential_stuffing", risk)
        )
...
def get_scenario_by_type(
    scenario_type: str, risk_level: Optional[str] = None
) -> RealInvestigationScenario:
