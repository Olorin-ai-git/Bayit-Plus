...
class AgentImportValidator:
    ...
    def validate_agent_instantiation(
        ...
        try:
            ...
            try:
                ...
                return (
                    True,
                    f"IMPORT_SUCCESS (init failed: {type(init_error).__name__})",
                )
    ...
    def run_comprehensive_validation(self) -> Dict[str, Any]:
        ...
        client_modules = [
            ...
            (
                "app.service.agent.ato_agents.clients.sumologic_client",
                "Sumo Logic Client",
            ),
        ]
        ...
        agent_implementations = [
            ...
            (
                "app.service.agent.ato_agents.location_data_agent.agent",
                "LocationDataAgent",
            ),
            (
                "app.service.agent.ato_agents.user_behavior_agent.agent",
                "UserBehaviorAgent",
            ),
            (
        ...
        for module_name, class_name in agent_implementations:
            success, message = self.validate_agent_instantiation(
        ...
        core_modules = [
            ...
            (
                "app.service.agent.ato_agents.fraud_detection_coordinator",
    ...
    def _generate_summary_report(self) -> Dict[str, Any]:
        ...
        print("=" * 60)
        print(
            f"📈 Import Success Rate: {self.success_count}/{self.total_count} ({success_rate:.1f}%)"
        )
        ...
        for category, stats in categories.items():
            ...
            print(
                f"  {category.replace('_', ' ').title()}: {stats['success']}/{stats['total']} ({cat_rate:.1f}%)"
            )
...
def test_agent_import_validation():
    ...
    ), f"Import success rate below 90%: {report['success_rate']:.1f}%"
    ...
    for module in critical_modules:
        ...
        assert report["all_results"][module][
            "success"
        ], f"Critical module {module} failed: {report['all_results'][module]['message']}"
...
def test_client_module_functionality():
    ...
    try:
        from app.service.agent.ato_agents.clients.databricks_client import (
            DatabricksClient,
        )
        ...
        ), "DatabricksClient missing execute_query method"
        ...
        ), "MySQLClient missing execute_query method"
