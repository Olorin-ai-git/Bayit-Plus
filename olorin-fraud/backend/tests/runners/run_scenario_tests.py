...
class ScenarioTestRunner:
    ...
    def _validate_real_response(self, result: dict):
        ...
        validations = [
            ...
            (
                "No mock data",
                not result.get("real_api_validation", {}).get("mock_data", True),
            ),
            (
                "Anthropic model used",
                "claude"
                in str(result.get("real_api_validation", {}).get("model", "")).lower(),
            ),
        ]
    ...
    def _print_final_report(self):
        ...
        print(
            f"Average Time per Scenario: {sum(r.get('total_time', 0) for r in self.results) / len(self.results):.2f}s"
        )
        print("\nScenario Results:")
        for result in self.results:
            status = "✅" if "error" not in str(result) else "❌"
            print(
                f"  {status} {result['scenario']}: ${result.get('total_cost', 0):.4f}"
            )
