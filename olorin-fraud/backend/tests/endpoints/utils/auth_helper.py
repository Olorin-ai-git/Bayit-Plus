...
async def authenticate_with_credentials(
    client: EndpointTestClient, username: str, password: str
    ...
    try:
        ...
        if response.status_code == 200:
            try:
                ...
                logger.info(
                    f"JSON authentication successful - token type: {token_type}"
                )
        ...
        if response.content:
            try:
                ...
                error_msg = (
                    response.text if response.text else "Unknown authentication error"
                )
        else:
...
async def get_test_auth_headers(client: EndpointTestClient) -> Dict[str, str]:
    ...
    raise AuthenticationError(
        f"All authentication attempts failed. Last error: {last_error}"
    )
...
async def refresh_token_if_needed(client: EndpointTestClient) -> None:
    ...
    try:
        ...
        response, metrics = await client.post(
            "/auth/refresh", json_data={"refresh_token": _token_manager.refresh_token}
        )
