...
class TestHealthEndpoints:
    ...
    async def test_root_endpoint(self, endpoint_client, endpoint_validator):
        ...
        ), f"Root endpoint too slow: {result.response_time_ms}ms"

        # Check if response has content
        if response.content:
            logger.info(
                f"Root endpoint returned content: {len(response.content)} bytes"
            )
    ...
    async def test_full_health_check(self, endpoint_client, endpoint_validator):
        ...
        assert (
            response.status_code == 200
        ), f"Full health check failed with status {response.status_code}"
        assert (
            result.response_time_ms < 2000
        ), f"Full health check too slow: {result.response_time_ms}ms"
        ...
        if response.content:
            try:
                ...
                assert (
                    len(data) >= 1
                ), "Full health check should have detailed information"
                ...
                found_components = [
                    comp for comp in expected_components if comp in data
                ]
        ...
        if result.errors:
            pytest.fail(
                f"Full health check validation failed: {'; '.join(result.errors)}"
            )
    ...
    async def test_version_endpoint(self, endpoint_client, endpoint_validator):
        ...
        assert (
            response.status_code == 200
        ), f"Version endpoint failed with status {response.status_code}"
        assert (
            result.response_time_ms < 500
        ), f"Version endpoint too slow: {result.response_time_ms}ms"
        ...
        if result.errors:
            pytest.fail(
                f"Version endpoint validation failed: {'; '.join(result.errors)}"
            )
    ...
    async def test_favicon_endpoint(self, endpoint_client, endpoint_validator):
        ...
        result = endpoint_validator.validate_response(
            response, metrics, expected_status=[200, 404], endpoint_type="health"
        )
        ...
        assert response.status_code in [
            200,
            404,
        ], f"Unexpected favicon status: {response.status_code}"
        assert (
            result.response_time_ms < 1000
        ), f"Favicon request too slow: {result.response_time_ms}ms"
        ...
        if response.status_code == 200:
            ...
            if content_type:
                ...
                if not is_image:
                    logger.warning(
                        f"Favicon has unexpected content-type: {content_type}"
                    )
        ...
        if result.errors:
            pytest.fail(
                f"Favicon endpoint validation failed: {'; '.join(result.errors)}"
            )

    @pytest.mark.asyncio
    async def test_performance_health_endpoint(
        self, endpoint_client, endpoint_validator
    ):
        ...
        assert response.status_code in [
            ...
            404,
            501,
        ], f"Unexpected performance health status: {response.status_code}"
        assert (
            result.response_time_ms < 2000
        ), f"Performance health check too slow: {result.response_time_ms}ms"
        ...
        if response.status_code == 200:
            ...
            if response.content:
                try:
                    ...
                    logger.warning(
                        f"Could not parse performance health response as JSON: {e}"
                    )

        elif response.status_code == 404:
            logger.info(
                "Performance health endpoint not found (404) - feature may not be implemented"
            )
        ...
        if result.errors:
            pytest.fail(
                f"Performance health validation failed: {'; '.join(result.errors)}"
            )
    ...
    def _validate_performance_health(self, data: Dict[str, Any], result):
        ...
        if not found_indicators:
            result.add_warning(
                "Performance health response missing performance indicators"
            )
...
async def test_health_endpoints_summary(endpoint_client):
    ...
    for endpoint, result in results.items():
        if result["success"]:
            logger.info(
                f"✓ {endpoint}: {result['status']} ({result.get('time_ms', 0):.1f}ms)"
            )
        else:
            logger.error(
                f"✗ {endpoint}: {result.get('status', 'ERROR')} - {result.get('error', 'Failed')}"
            )
