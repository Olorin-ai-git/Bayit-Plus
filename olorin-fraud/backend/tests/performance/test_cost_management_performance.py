...
class TestCostManagementPerformance:
    ...
    async def test_credit_monitor_performance_overhead(self, api_cost_monitor):
        ...
        for i in range(iterations):
            await credit_monitor.estimate_request_cost(
                "claude-3-sonnet-20240229", 1000 + i, 1500 + i
            )

            if i % 100 == 0:
                # Track some usage for realism
                api_cost_monitor.track_call(
                    1000 + i, 1500 + i, "claude-3-sonnet-20240229"
                )
        ...
        assert (
            avg_time_per_operation < 1.0
        ), f"Credit monitor overhead too high: {avg_time_per_operation:.2f}ms per operation"

        # Log performance metrics
        print(
            f"Credit Monitor Performance: {avg_time_per_operation:.4f}ms per operation"
        )
    ...
    async def test_model_fallback_selection_performance(self):
        ...
        assert (
            avg_time_per_selection < 10.0
        ), f"Model selection too slow: {avg_time_per_selection:.2f}ms per selection"

        print(
            f"Model Selection Performance: {avg_time_per_selection:.4f}ms per selection"
        )
    ...
    async def test_cost_optimization_throughput(self, api_cost_monitor):
        ...
        test_prompts = [
            (
                "data_extraction",
                "Extract relevant data from transaction logs for analysis",
            ),
            (
                "pattern_recognition",
                "Identify suspicious patterns in user behavior over the last 30 days",
            ),
            (
                "risk_assessment",
                "Assess fraud risk based on device fingerprint and transaction history",
            ),
            (
                "device_analysis",
                "Analyze device fingerprint for potential spoofing indicators",
            ),
        ...
        for i in range(iterations):
            ...
            try:
                ...
                if i % 10 == 0:
                    api_cost_monitor.track_call(
                        1000, 1000, model_selection.selected_model
                    )
        ...
        avg_time_per_optimization = (
            elapsed_time / successful_optimizations
        ) * 1000  # Milliseconds

        # Should maintain high throughput
        assert (
            throughput >= 20.0
        ), f"Cost optimization throughput too low: {throughput:.2f} ops/sec"
        assert (
            avg_time_per_optimization < 50.0
        ), f"Cost optimization too slow: {avg_time_per_optimization:.2f}ms per operation"

        print(
            f"Cost Optimization Performance: {throughput:.2f} ops/sec, {avg_time_per_optimization:.2f}ms per operation"
        )
    ...
    async def test_circuit_breaker_performance_impact(self):
        ...
        circuit_breaker = get_circuit_breaker(
            "performance_test",
            CircuitBreakerConfig(
                failure_threshold=5, recovery_timeout=10.0, timeout=5.0
            ),
        )
        ...
        assert (
            overhead_per_call < 1.0
        ), f"Circuit breaker overhead too high: {overhead_per_call:.4f}ms per call"
        assert (
            overhead_percentage < 10.0
        ), f"Circuit breaker overhead percentage too high: {overhead_percentage:.2f}%"

        print(
            f"Circuit Breaker Overhead: {overhead_per_call:.4f}ms per call ({overhead_percentage:.2f}%)"
        )
    ...
    async def test_real_time_tracker_monitoring_performance(self):
        ...
        try:
            ...
            assert (
                throughput >= 500.0
            ), f"Real-time tracker throughput too low: {throughput:.2f} metrics/sec"
    ...
    async def test_concurrent_access_performance(self, api_cost_monitor):
        ...
        avg_time_per_operation = (
            elapsed_time / concurrent_count
        ) * 1000  # Milliseconds

        # Should handle concurrent operations efficiently
        assert (
            throughput >= 10.0
        ), f"Concurrent throughput too low: {throughput:.2f} ops/sec"
        assert (
            avg_time_per_operation < 500.0
        ), f"Concurrent operations too slow: {avg_time_per_operation:.2f}ms per operation"

        print(
            f"Concurrent Access Performance: {throughput:.2f} ops/sec, {avg_time_per_operation:.2f}ms per operation"
        )
    ...
    async def test_memory_usage_under_load(self):
        ...
        ), f"Memory growth too high: {memory_growth_mb:.2f}MB for 1000 operations"
    ...
    async def test_cache_performance_characteristics(self):
        ...
        performance_improvement = (
            (first_pass_time - second_pass_time) / first_pass_time * 100
        )
        ...
        print(
            f"Cache Performance: {cache_hit_rate:.2f}% hit rate, {performance_improvement:.2f}% performance improvement"
        )
    ...
    async def test_system_scalability_limits(self):
                ...
                performance_results.append(
                    {
                        ...
                        "avg_latency": (
                            elapsed_time / successful_operations
        ...
        max_throughput = max(
            r["throughput"] for r in performance_results if "throughput" in r
        )
        ...
        assert (
            max_throughput >= 50.0
        ), f"Maximum throughput too low: {max_throughput:.2f} ops/sec"

        if degradation_point:
            assert (
                degradation_point >= 100
            ), f"Performance degradation too early at load level: {degradation_point}"
...
class TestCostManagementStressTests:
    ...
    async def test_sustained_high_load_stress(self, api_cost_monitor):
        ...
        for second in range(duration_seconds):
            ...
            batch_tasks = [
                stress_operation(second * target_rps + i) for i in range(target_rps)
            ]
        ...
        ), f"Throughput degraded too much: {actual_rps:.2f} < {target_rps * 0.8:.2f} rps"

        print(
            f"Stress Test Results: {success_rate:.2%} success rate, {actual_rps:.2f} rps"
        )
    ...
    async def test_memory_stress_and_recovery(self):
        ...
        print(
            f"Memory Stress: {memory_growth:.2f}MB growth, {recovery_rate:.2%} recovered"
        )
    ...
    async def test_circuit_breaker_stress_scenarios(self):
        ...
        circuit_breaker = get_circuit_breaker(
            "stress_test",
            CircuitBreakerConfig(
                failure_threshold=3, recovery_timeout=2.0, timeout=1.0
            ),
        )
        ...
        for i, (pattern_type, delay) in enumerate(
            call_patterns * 10
        ...
        state_transitions_expected = (
            stats["total_failures"] >= circuit_breaker.config.failure_threshold
        )

        print(
            f"Circuit Breaker Stress: {stats['total_requests']} requests, {stats['total_failures']} failures"
        )
    ...
    async def test_real_time_monitoring_stress(self):
        ...
        try:
            ...
            ), f"Real-time monitoring too slow under stress: {updates_per_second:.2f} updates/sec"

            # Verify data integrity
            assert (
                len(cost_tracker.current_metrics) >= metrics_count * 0.9
            )  # Allow some cleanup
...
class TestCostManagementBenchmarks:
    ...
    async def test_end_to_end_investigation_benchmark(self, api_cost_monitor):
        ...
        for iteration in range(benchmark_iterations):
            ...
            for task_type, prompt, preferred_model, max_tokens in investigation_phases:
                ...
                cost_estimate = await credit_monitor.estimate_request_cost(
                    model_selection.selected_model, max_tokens // 2, max_tokens // 2
                )

                is_affordable, _ = await credit_monitor.validate_request_affordability(
                    cost_estimate
                )
        ...
        # Performance targets
        assert (
            avg_investigation_time < 5.0
        ), f"Investigation too slow: {avg_investigation_time:.2f}s per investigation"
        assert (
            investigations_per_hour >= 720
        ), f"Throughput too low: {investigations_per_hour:.2f} investigations/hour"

        print(
            f"Investigation Benchmark: {avg_investigation_time:.2f}s per investigation"
        )
        print(
            f"Potential throughput: {investigations_per_hour:.2f} investigations/hour"
        )
    ...
    async def test_cost_management_overhead_benchmark(self):
        ...
        assert (
            overhead_per_operation < 10.0
        ), f"Cost management overhead too high: {overhead_per_operation:.2f}ms per operation"
    ...
    async def test_component_isolation_benchmark(self):
        ...
        for component_name, component in components.items():
            ...
            if component_name == "credit_monitor":
                for i in range(iterations):
                    await component.estimate_request_cost(
                        "claude-3-sonnet-20240229", 1000 + i, 1500 + i
                    )

            elif component_name == "model_fallback":
                for i in range(iterations):
                    await component.select_model(
                        "device_analysis", 2000 + i, "claude-3-sonnet-20240229"
                    )
        ...
        for component_name, metrics in component_benchmarks.items():
            print(
                f"{component_name}: {metrics['avg_time_ms']:.4f}ms per operation, {metrics['throughput']:.2f} ops/sec"
            )

            # Performance assertions
            assert (
                metrics["avg_time_ms"] < 50.0
            ), f"{component_name} too slow: {metrics['avg_time_ms']:.4f}ms"
            assert (
                metrics["throughput"] >= 20.0
            ), f"{component_name} throughput too low: {metrics['throughput']:.2f} ops/sec"
