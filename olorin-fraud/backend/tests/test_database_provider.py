...
class TestDatabaseProviderAbstraction:
    ...
    def test_database_provider_type_hints(self):
        ...
        assert connect_hints.get("return") is None or connect_hints["return"] == type(
            None
        )

        disconnect_hints = get_type_hints(BaseProvider.disconnect)
        assert disconnect_hints.get("return") is None or disconnect_hints[
            "return"
        ] == type(None)
