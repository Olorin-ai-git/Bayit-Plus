...
@click.option(
    "--type",
    "investigation_type",
    type=click.Choice(["structured", "hybrid"]),
    default="structured",
...
@click.option(
    "--graph",
    "graph_type",
    type=click.Choice(["clean", "hybrid"]),
    default="clean",
    help="Graph type",
)
@click.option(
    "--trigger",
    "trigger_source",
    type=click.Choice(["startup", "script", "ui"]),
    default="script",
    help="Trigger source",
)
@click.option(
    ...
    "entity_type",
    type=str,
    help="Entity type (e.g., email, device_id)",
)
@click.option("--entity-id", "entity_id", type=str, help="Entity ID")
@click.option("--title", type=str, help="Investigation title")
@click.pass_context
def new(
    ctx, investigation_type, graph_type, trigger_source, entity_type, entity_id, title
):
    """Create new investigation with manifest"""
    import uuid

    investigation_id = (
        f"inv_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{uuid.uuid4().hex[:8]}"
    )
    ...
    mode = (
        InvestigationMode.LIVE if trigger_source == "script" else InvestigationMode.DEMO
    )
...
@cli.command()
@click.option(
    "--id", "investigation_id", required=True, type=str, help="Investigation ID"
)
@click.option(
    "--path",
    "file_path",
    ...
    help="File path to add",
)
@click.option(
    "--kind",
    "file_kind",
    type=click.Choice(["artifact", "report", "log", "metadata"]),
    default="artifact",
    help="File kind",
)
@click.pass_context
def add_file(ctx, investigation_id, file_path, file_kind):
    ...
    try:
        file_handle = file_org_service.lock_file_for_write(
            target_path, create_if_missing=True
        )
        try:
            import shutil
...
@click.option(
    ...
    help="Extract zip file and generate comprehensive report from its contents",
)
@click.option(
    "--output",
    "output_path",
...
def report(ctx, investigation_id, zip_file_path, output_path):
    ...
    if not investigation_id and not zip_file_path:
        click.echo(
            "❌ Must specify either --id INVESTIGATION_ID or --unzip ZIP_FILE", err=True
        )
        sys.exit(1)

    if investigation_id and zip_file_path:
        click.echo(
            "❌ Cannot specify both --id and --unzip. Use one or the other.", err=True
        )
        sys.exit(1)
        ...
        if zip_file_path:
            ...
            try:
                ...
                if metadata_folders:
                    ...
                    click.echo(
                        f"   Found investigation folder via metadata.json: {investigation_folder.name}"
                    )
                else:
                    ...
                    for pattern in investigation_patterns:
                        ...
                        if pattern_files:
                            ...
                            click.echo(
                                f"   Found investigation folder via {pattern}: {investigation_folder.name}"
                            )
                            break
                    ...
                    if not investigation_folder:
                        ...
                        if len(top_level_dirs) == 1:
                            ...
                            click.echo(
                                f"   Using single top-level directory: {investigation_folder.name}"
                            )
                        else:
                            # Pattern 4: Use temp_dir as investigation folder (all files at root)
                            investigation_folder = temp_dir
                            click.echo(
                                f"   Using root directory as investigation folder"
                            )
                ...
                if output_path:
                    ...
                    report_output_path = (
                        zip_path.parent / f"{zip_stem}_comprehensive_report.html"
                    )

                click.echo(
                    f"📊 Generating comprehensive report from extracted files..."
                )
                ...
                if temp_dir and temp_dir.exists():
                    try:
                        ...
                        click.echo(
                            f"   ⚠️  Failed to clean up temporary directory: {e}",
                            err=True,
                        )
...
@click.option(
    ...
    help="Logs directory to import",
)
@click.option(
    "--artifacts",
    "artifacts_dir",
    type=click.Path(exists=True, file_okay=False),
    help="Artifacts directory to import",
)
@click.option(
    "--all",
    "import_all",
    is_flag=True,
    help="Import all existing logs and artifacts, overriding existing records",
)
...
def import_logs(ctx, logs_dir, artifacts_dir, import_all, dry_run, copy, move, link):
    ...
    if not logs_dir and not artifacts_dir:
        click.echo(
            "❌ Must specify --logs and/or --artifacts (or use --all to auto-detect)",
            err=True,
        )
        sys.exit(1)
    ...
    if artifacts_dir:
        ...
        pattern = re.compile(
            r"investigation_([^_]+)_(.+?)_(\d{8})_(\d{8})\.(json|html)"
        )
        ...
        for artifact_file in artifact_files:
            ...
            entity_type, entity_id_raw, date_start_str, date_end_str, file_ext = (
                match.groups()
            )
            ...
            inv_id_hash = hashlib.md5(
                f"{entity_type}:{entity_id}:{date_start_str}:{date_end_str}".encode()
            ).hexdigest()[:12]
            ...
            canonical_path, entity_view_path = (
                file_org_service.resolve_investigation_artifact_path(
                    entity_type=entity_type,
                    entity_id=entity_id,
            ...
            if not dry_run:
                try:
                    ...
                    if artifact_file.resolve() == canonical_path.resolve():
                        click.echo(
                            f"     ⏭️  Skipping (already in correct location): {canonical_path}"
                        )
                        imported_count += 1
                        continue
                    ...
                    if entity_view_path:
                        file_org_service.create_directory_structure(
                            entity_view_path.parent
                        )
                    ...
                    try:
                        file_handle = file_org_service.lock_file_for_write(
                            canonical_path, create_if_missing=True
                        )
                    ...
                    if entity_view_path and canonical_path.exists():
                        ...
                        if error:
                            ...
                            click.echo(
                                f"     ✓ Entity view ({view_type}): {entity_view_path}"
                            )
                        ...
                        try:
                            with registry._get_connection() as conn:
                                ...
                                if cursor.fetchone():
                                    ...
                                    click.echo(
                                        f"     ⚠️ File already indexed, skipping (use --all to override)"
                                    )
                        except Exception as e:
                    ...
                    if not file_already_indexed or import_all:
                        ...
                        if import_all and file_already_indexed:
                            try:
                                with registry._get_connection() as conn:
                                    cursor = conn.cursor()
                                    cursor.execute(
                                        "DELETE FROM files WHERE canonical_path = ?",
                                        (str(canonical_path),),
                                    )
                                    click.echo(f"     ↻ Overriding existing record")
                            except Exception as e:
                                click.echo(
                                    f"     ⚠️ Failed to delete existing record: {e}"
                                )
                        ...
                        registry.index_file(
                            ...
                            file_ext=file_ext,
                            entity_view_path=(
                                str(entity_view_path) if entity_view_path else None
                            ),
                        ...
                        with registry._get_connection() as conn:
                            ...
                            if not existing_inv or import_all:
                                if existing_inv and import_all:
                                    ...
                                    click.echo(
                                        f"     ↻ Overriding existing investigation record"
                                    )
                                ...
                                registry.index_investigation(
                                    ...
                                    canonical_path=(
                                        str(canonical_path.parent.parent.parent)
                                        if canonical_path
                                        else None
                                    ),
                                    created_at=date_end,
                                ...
                                click.echo(
                                    f"     ⚠️ Investigation already indexed (use --all to override)"
                                )
                    except Exception as e:
    ...
    if artifacts_dir:
        ...
        if comparisons_dir.exists():
            ...
            comparison_pattern = re.compile(
                r"comparison_([^_]+)_(.+?)_(\d{8}_\d{6})\.html"
            )
                    ...
                    for timestamp_dir in source_path.iterdir():
                        if timestamp_dir.is_dir():
                            comparison_files.extend(
                                timestamp_dir.glob("comparison_*.html")
                            )
            ...
            for comparison_file in comparison_files:
                ...
                source_type = (
                    "auto_startup"
                    if "auto_startup" in str(comparison_file)
                    else "manual"
                )
                ...
                if not dry_run:
                    try:
                        ...
                        if comparison_file.resolve() == report_path.resolve():
                            click.echo(
                                f"     ⏭️  Skipping (already in correct location): {report_path}"
                            )
                            imported_count += 1
                            continue
                        ...
                        if not import_all:
                            try:
                                with registry._get_connection() as conn:
                                    cursor = conn.cursor()
                                    cursor.execute(
                                        "SELECT comparison_id FROM comparisons WHERE comparison_id = ?",
                                        (comparison_id,),
                                    )
                                    if cursor.fetchone():
                                        comparison_already_indexed = True
                                        click.echo(
                                            f"     ⚠️ Comparison already indexed, skipping (use --all to override)"
                                        )
                            except Exception as e:
                                click.echo(
                                    f"     ⚠️ Failed to check existing comparison: {e}"
                                )
                            ...
                            if import_all and comparison_already_indexed:
                                try:
                                    with registry._get_connection() as conn:
                                        ...
                                        click.echo(
                                            f"     ↻ Overriding existing comparison record"
                                        )
                                except Exception as e:
                                    click.echo(
                                        f"     ⚠️ Failed to delete existing comparison: {e}"
                                    )
                        ...
                        click.echo(
                            f"     ❌ Failed to import {filename}: {e}", err=True
                        )
                        logger.exception(f"Import failed for {filename}")
                else:
            ...
            if reports_dir.exists():
                ...
                for timestamp_dir in reports_dir.iterdir():
                    if timestamp_dir.is_dir():
                        startup_files.extend(
                            timestamp_dir.glob("startup_analysis_*.html")
                        )
                ...
                for startup_file in startup_files:
                    ...
                    report_path = file_org_service.resolve_startup_report_path(
                        timestamp
                    )
                                ...
                                try:
                                    with registry._get_connection() as conn:
                                        ...
                                        if cursor.fetchone():
                                            ...
                                            click.echo(
                                                f"     ⚠️ Startup report already indexed, skipping (use --all to override)"
                                            )
                                except Exception as e:
                                    click.echo(
                                        f"     ⚠️ Failed to check existing startup report: {e}"
                                    )
                                ...
                                if import_all and startup_already_indexed:
                                    try:
                                        with registry._get_connection() as conn:
                                            ...
                                            click.echo(
                                                f"     ↻ Overriding existing startup report record"
                                            )
                                    except Exception as e:
                                        click.echo(
                                            f"     ⚠️ Failed to delete existing startup report: {e}"
                                        )
        ...
        for log_folder, match in log_folders:
            ...
            if not dry_run:
                try:
                        ...
                        for log_file in log_path.rglob("*"):
                            if log_file.is_file():
                                try:
                                    ...
                                    click.echo(
                                        f"     ⚠️ Failed to index log file {log_file.name}: {e}"
                                    )
                    ...
                    click.echo(
                        f"     ❌ Failed to import {log_folder.name}: {e}", err=True
                    )
...
@click.argument(
    "resource_type",
    type=click.Choice(["investigations", "files", "comparisons"]),
...
@click.option(
    "--date-range", "date_range", type=str, help="Date range (format: START,END)"
)
@click.pass_context
def ls(ctx, resource_type, entity_type, entity_id, date_range):
    ...
    if resource_type == "investigations":
        ...
        for inv in results:
            click.echo(
                f"  {inv.get('investigation_id', 'unknown')}: {inv.get('title', 'N/A')} ({inv.get('status', 'N/A')})"
            )
        ...
        for file in results:
            click.echo(
                f"  {file.get('canonical_path', 'unknown')} ({file.get('file_kind', 'N/A')})"
            )
        ...
        with registry._get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT * FROM comparisons ORDER BY created_at DESC LIMIT 100"
            )
            results = [dict(row) for row in cursor.fetchall()]
        ...
        for cmp in results:
            click.echo(
                f"  {cmp.get('comparison_id', 'unknown')}: {cmp.get('title', 'N/A')}"
            )
        ...
        for year_dir in workspace_base.iterdir():
            ...
            for month_dir in year_dir.iterdir():
                ...
                for inv_dir in month_dir.iterdir():
                    ...
                    investigation_id = inv_dir.name.replace("imported-", "").replace(
                        "inv_", ""
                    )
                    ...
                    if manifest_path.exists():
                        try:
                            ...
                            click.echo(
                                f"  ⚠️ Failed to parse manifest {manifest_path}: {e}"
                            )
                    ...
                    try:
                        with registry._get_connection() as conn:
                            ...
                            if not cursor.fetchone():
                                ...
                                indexed_count += 1
                                click.echo(
                                    f"  ✓ Indexed investigation: {investigation_id}"
                                )
                    except Exception as e:
                        click.echo(
                            f"  ⚠️ Failed to index investigation {investigation_id}: {e}"
                        )
                        error_count += 1
                    ...
                    for file_path in inv_dir.rglob("*"):
                        ...
                        try:
                            ...
                            click.echo(
                                f"  ⚠️ Failed to index file {file_path.name}: {e}"
                            )
                            error_count += 1
                ...
                for file_path in entity_id_dir.rglob("*"):
                    ...
                    try:
                        ...
                        with registry._get_connection() as conn:
                            cursor = conn.cursor()
                            cursor.execute(
                                "SELECT file_id FROM files WHERE entity_view_path = ?",
                                (str(file_path),),
                            )
                            if cursor.fetchone():
                                continue
                        ...
                        inv_match = re.match(
                            r"inv_([^_]+)__(\w+)\.(\w+)", file_path.name
                        )
                        ...
                        click.echo(
                            f"  ⚠️ Failed to index entity view file {file_path.name}: {e}"
                        )
                        error_count += 1
    ...
    if reports_base.exists():
        ...
        for startup_file in reports_base.rglob("startup_analysis_*.html"):
            ...
            try:
                ...
                click.echo(
                    f"  ⚠️ Failed to index startup report {startup_file.name}: {e}"
                )
                error_count += 1
...
@click.option(
    "--registry-only",
    is_flag=True,
    help="Only delete registry records, keep database investigations",
)
@click.pass_context
def cleanup(ctx, yes, files, registry_only):
    ...
    if not yes:
        ...
        if files:
            click.echo(
                "⚠️  This will also delete ALL investigation and comparison files!"
            )
        click.echo()
    ...
    registry = get_registry()
    deleted_counts = {"investigations": 0, "comparisons": 0, "files": 0, "audit_log": 0}
    ...
    try:
        ...
        with registry._get_connection() as conn:
            ...
            if files:
                cursor.execute(
                    "SELECT canonical_path FROM files WHERE canonical_path IS NOT NULL"
                )
            ...
            click.echo(
                f"   ✅ Deleted {deleted_counts['comparisons']} comparison(s) from registry"
            )
            click.echo(f"   ✅ Deleted {deleted_counts['files']} file(s) from registry")
            click.echo(
                f"   ✅ Deleted {deleted_counts['audit_log']} audit log entry/entries"
            )
        ...
        if not registry_only:
            ...
            try:
                with get_db_session() as db:
                    ...
                    click.echo(
                        f"   ✅ Deleted {deleted_count} investigation(s) from database"
                    )
                    ...
                    if in_memory_count > 0:
                        click.echo(
                            f"   ✅ Cleared {in_memory_count} investigation(s) from in-memory cache"
                        )
                ...
                click.echo(
                    f"   ❌ Error deleting investigations from database: {e}", err=True
                )
        ...
        if files:
            ...
            click.echo(
                f"   ✅ Deleted {deleted_file_count} file(s)/folder(s) from filesystem"
            )
