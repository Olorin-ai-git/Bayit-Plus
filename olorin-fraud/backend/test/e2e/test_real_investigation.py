...
async def test_real_device_agent():
    ...
    try:
        ...
        print(
            f"   📊 Agent result keys: {list(result.keys()) if isinstance(result, dict) else 'Not a dict'}"
        )
        ...
        if isinstance(result, dict):
            ...
            if isinstance(agent_output, dict):
                ...
                print(
                    f"   📊 Findings count: {len(findings) if isinstance(findings, list) else 'Not a list'}"
                )
                ...
                if sequential_metadata:
                    ...
                    print(
                        f"      Total phases: {sequential_metadata.get('total_phases', 0)}"
                    )
                    print(
                        f"      Successful phases: {sequential_metadata.get('successful_phases', 0)}"
                    )
                    print(
                        f"      Tools used: {sequential_metadata.get('total_tools_used', 0)}"
                    )
                    print(
                        f"      Tool coverage: {sequential_metadata.get('tool_coverage_percentage', 0)}%"
                    )
                    ...
                    for phase in phase_results:
                        ...
                        print(
                            f"      Phase {phase_name}: {'✅' if success else '❌'} ({tools_used} tools)"
                        )
...
async def main():
    ...
    if success:
        print(
            "📊 Real investigation test completed - check output above for tool usage"
        )
    else:
