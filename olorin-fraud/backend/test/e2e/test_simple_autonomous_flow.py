...
async def test_execution_plan_creation():
    ...
    comprehensive_plan = orchestrator._create_execution_plan(
    ...
    logger.info(
        f"   Phase Structure: {[p['phase_name'] for p in sequential_plan['phases']]}"
    )
...
async def test_bulletproof_coordination():
    ...
    logger.info(
        f"   Circuit Breakers: {len(coordination_result['config']['circuit_breakers'])}"
    )
    logger.info(
        f"   Retry Counters: {len(coordination_result['config']['retry_counters'])}"
    )

    # Verify circuit breaker configuration
    for agent, config in coordination_result["config"]["circuit_breakers"].items():
        logger.info(
            f"   Agent {agent}: threshold={config['failure_threshold']}, state={config['state']}"
        )
...
async def test_context_analysis():
    ...
    medium_risk_context = MockContext(
        warning_flags=["unusual_timing"], alert_level="medium", minor_irregularities=2
    )

    # Low-risk context
    low_risk_context = MockContext(
        normal_operation=True, standard_behavior=True, no_alerts=True
    )
...
async def test_result_consolidation():
    ...
    mock_results = {
        ...
        "agent_results": {
            ...
            "location": {"status": "failed", "error": "Service unavailable"},
        },
        "failures": [
            {
                "agent": "location",
    ...
    logger.info(
        f"   Bulletproof recoveries: {consolidated['bulletproof_recovery_count']}"
    )
