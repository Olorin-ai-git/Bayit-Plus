...
def create_agent_context(investigation_data: dict) -> AgentContext:
    ...
    session_id = f"session_{int(time.time())}"
    metadata = Metadata(
        interactionGroupId=session_id, supportedOutputFormats=[], additionalMetadata={}
    )
...
async def test_full_investigation_flow():
    ...
    try:
        ...
        logger.info(
            f'  Location: {investigation_data["network_info"]["city"]}, {investigation_data["network_info"]["country"]}'
        )
        logger.info(
            f'  Risk Flags: {list(investigation_data["behavioral_flags"].keys())}'
        )
        ...
        if isinstance(result, dict):
            ...
            if "agent_context" in result:
                ...
                if isinstance(ctx, dict):
                    logger.info(
                        f'  🔍 Investigation ID: {ctx.get("investigation_id", "N/A")}'
                    )
                    logger.info(
                        f'  ⏱️  Investigation Time: {time.time() - ctx.get("timestamp", time.time()):.2f}s'
                    )
                else:
            ...
            if "orchestration_decision" in result:
                ...
                logger.info(
                    f'  🤖 Agents Activated: {decision.get("agents_to_activate", [])}'
                )
        ...
        try:
            ...
            if checkpointed_state:
                ...
                logger.info(
                    f'  📊 Checkpoint contains: {list(checkpointed_state.values.keys()) if hasattr(checkpointed_state, "values") else "state data"}'
                )
            else:
