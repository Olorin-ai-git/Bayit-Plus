...
class TestEnhancedAgentFactory:
    ...
    def test_factory_init_with_rag_enabled(self):
        ...
        with patch("app.service.agent.agent_factory.RAG_AVAILABLE", True):
            with patch(
                "app.service.agent.agent_factory.get_rag_orchestrator"
            ) as mock_get_rag:
    ...
    def test_create_agent_rag_enhanced(self, mock_tools, mock_rag_orchestrator):
        ...
        with patch("app.service.agent.agent_factory.RAG_AVAILABLE", True):
            with patch(
                "app.service.agent.agent_factory.create_rag_enhanced_agent"
            ) as mock_create_rag:
                mock_agent = Mock()
    ...
    def test_create_agent_standard(self, mock_tools):
        ...
        with patch(
            "app.service.agent.agent_factory.create_structured_agent"
        ) as mock_create_standard:
            mock_agent = Mock()
    ...
    def test_create_rag_enhanced_agent_explicit(
...
class TestFactoryFunctions:
    ...
    def test_create_agent_function(self, mock_tools):
        ...
        with patch(
            "app.service.agent.agent_factory.get_agent_factory"
        ) as mock_get_factory:
            mock_factory = Mock()
            mock_agent = Mock()
    ...
    def test_create_rag_agent_function(self, mock_tools):
        ...
        with patch("app.service.agent.agent_factory.RAG_AVAILABLE", True):
            with patch(
                "app.service.agent.agent_factory.get_rag_enhanced_factory"
            ) as mock_get_factory:
                mock_factory = Mock()
                mock_agent = Mock()
