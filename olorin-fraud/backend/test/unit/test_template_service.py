...
def test_list_templates_filtered_by_tags(
    service: TemplateService, sample_template_json: dict
):
...
def test_get_template_success(service: TemplateService, sample_template_json: dict):
    ...
    created = service.create_template(
        user_id="user-123", name="Template", template_json=sample_template_json
    )

    result = service.get_template(
        template_id=created["template_id"], user_id="user-123"
    )
...
def test_update_template_success(service: TemplateService, sample_template_json: dict):
    ...
    created = service.create_template(
        user_id="user-123", name="Original", template_json=sample_template_json
    )
...
def test_delete_template_soft_delete_if_used(
    service: TemplateService, sample_template_json: dict, db_session: Session
):
    ...
    template = (
        db_session.query(InvestigationTemplate)
        .filter_by(template_id=created["template_id"])
        .first()
    )
...
def test_delete_template_hard_delete_if_unused(
    service: TemplateService, sample_template_json: dict, db_session: Session
):
...
def test_apply_template_increments_usage_count(
    service: TemplateService, sample_template_json: dict, db_session: Session
):
...
def test_template_json_validation(service: TemplateService):
    ...
    with pytest.raises((HTTPException, ValueError)):
        service.create_template(
            user_id="user-123", name="Invalid Template", template_json=invalid_json
        )
