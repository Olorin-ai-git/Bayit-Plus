...
from app.service.config_secrets import (
...
class TestSecretManagerClient:
    ...
    def mock_secret_client(self):
        ...
        with patch(
            "app.service.secret_manager.secretmanager.SecretManagerServiceClient"
        ) as mock:
            yield mock
    ...
    def secret_manager(self, mock_secret_client):
        ...
        client = SecretManagerClient(
            project_id="test-project", cache_ttl=60
        )  # 1 minute TTL for tests
        return client
    ...
    def test_get_secret_with_fallback(self, secret_manager):
        ...
        result = secret_manager.get_secret_with_fallback(
            "test_secret", env_var="FALLBACK_VAR", default="default_value"
        )
        assert result == "secret_value"
        ...
        with patch.dict(os.environ, {"FALLBACK_VAR": "fallback_value"}):
            result = secret_manager.get_secret_with_fallback(
                "missing_secret", env_var="FALLBACK_VAR", default="default_value"
            )
            assert result == "fallback_value"

        # Test with default value
        result = secret_manager.get_secret_with_fallback(
            "missing_secret", env_var="NONEXISTENT_VAR", default="default_value"
        )
    ...
    def test_env_var_name_conversion(self, secret_manager):
        ...
        assert (
            secret_manager._get_env_var_name("database_password") == "DATABASE_PASSWORD"
        )
        assert (
            secret_manager._get_env_var_name("DATABASE_PASSWORD") == "DATABASE_PASSWORD"
        )
    ...
    def test_cache_ttl_behavior(self, secret_manager):
        ...
        assert (
            secret_manager._client.access_secret_version.call_count == 1
        )  # Still 1, used cache
        ...
        assert (
            secret_manager._client.access_secret_version.call_count == 2
        )  # Called API again
...
class TestConfigLoader:
    ...
    def test_load_secret_with_env_specific(self, config_loader, mock_secret_manager):
        ...
        assert (
            first_call[0][0] == "test/database_password"
        )  # Check the actual path format
    ...
    def test_load_database_config_missing_password_production(
        self, config_loader, mock_secret_manager
    ):
        ...
        with patch.dict(os.environ, {"APP_ENV": "production"}):
            ...
            with pytest.raises(
                ValueError, match="CRITICAL: Database password not found"
            ):
    ...
    def test_load_jwt_config_missing_key_production(
        self, config_loader, mock_secret_manager
    ):
    ...
    def test_load_jwt_config_generates_dev_key(
        self, config_loader, mock_secret_manager
    ):
...
class TestConfigSecrets:
    ...
    def test_enhance_config_with_secrets(
        self, mock_config_instance, mock_config_loader
    ):
