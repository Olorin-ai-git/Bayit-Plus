...
async def test_anthropic_api_basic():
    ...
    try:
        ...
        messages = [
            HumanMessage(
                content="Hello! Please respond with 'API test successful' and nothing else."
            )
        ]
...
async def test_anthropic_with_simple_tools():
    ...
    try:
        ...
        messages = [
            HumanMessage(
                content="Please add 5 + 3 using the calculator tool, and also get the weather for San Francisco. You MUST use both tools."
            )
        ]
        ...
        if hasattr(response, "tool_calls") and response.tool_calls:
            ...
            print(
                f"       This suggests the LLM is not calling tools even when requested"
            )
...
async def test_tool_registry_tools():
    ...
    try:
        ...
        messages = [
            HumanMessage(
                content="Please analyze the IP address 1.1.1.1 using ALL available threat intelligence tools. You MUST use all tools to get comprehensive data."
            )
        ]
