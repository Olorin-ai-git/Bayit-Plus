...
class TestMCPServerRegistry:
    ...
    def test_unregister_server(self, registry, mock_server_info):
        ...
        assert "test_server" not in registry.capabilities_index.get(
            "fraud_detection", []
        )
...
class TestMCPIntegrationWithGraphBuilder:
    ...
    async def test_create_mcp_enhanced_graph(self):
        ...
        # Mock MCP client
        with patch(
            "app.service.agent.orchestration.graph_builder.MCPClientManager"
            ...
            graph = await create_mcp_enhanced_graph(
                parallel=True, use_enhanced_tools=False, use_mcp=True
            )
