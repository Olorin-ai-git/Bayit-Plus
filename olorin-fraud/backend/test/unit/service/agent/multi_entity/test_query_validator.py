...
class TestMultiEntityQueryValidator:
    ...
    def test_complex_query_validation(self):
        ...
        result = validator.validate_query(
            boolean_logic=boolean_logic, entity_ids=entities
        )

        assert result.is_valid is True
        assert result.complexity_metrics.complexity_level in [
    ...
    def test_entity_limit_validation(self):
        ...
        result = validator.validate_query(
            boolean_logic="entity_0 AND entity_1", entity_ids=entities
        )
    ...
    def test_nesting_depth_validation(self):
        ...
        boolean_logic = (
            "((((entity_0 AND entity_1) OR entity_2) AND entity_3) OR entity_4)"
        )
    ...
    def test_syntax_validation(self):
        ...
        assert result.is_valid is False
        assert any(
            "Unbalanced parentheses" in error for error in result.validation_errors
        )
        ...
        assert result.is_valid is False
        assert any(
            "Consecutive operators" in error for error in result.validation_errors
        )
    ...
    def test_complexity_scoring(self):
        ...
        assert (
            result2.complexity_metrics.complexity_score
            > result1.complexity_metrics.complexity_score
        )
        assert (
            result2.complexity_metrics.estimated_execution_time_ms
            > result1.complexity_metrics.estimated_execution_time_ms
        )
    ...
    def test_caching_recommendations(self):
        ...
        entities = [f"entity_{i}" for i in range(15)]
        result2 = validator.validate_query(
            "entity_0 AND entity_1 AND entity_2", entities
        )
    ...
    def test_duplicate_entity_detection(self):
        ...
        result = validator.validate_query(
            "entity_1 AND entity_2", entities_with_duplicates
        )
    ...
    def test_empty_query_validation(self):
        ...
        assert result.is_valid is False
        assert any(
            "Empty Boolean expression" in error for error in result.validation_errors
        )
        assert any(
            "No entities provided" in error for error in result.validation_errors
        )
    ...
    def test_convenience_validation_function(self):
        ...
        result = validate_multi_entity_query(
            boolean_logic="entity_1 AND entity_2", entity_ids=["entity_1", "entity_2"]
        )
...
class TestQueryValidatorPerformance:
    ...
    def test_validation_performance(self):
        ...
        )  # Higher threshold for performance test
...
class TestQueryValidatorEdgeCases:
    ...
    def test_case_insensitive_operators(self):
        ...
        # Test mixed case
        result2 = validator.validate_query(
            "entity_1 AND entity_2 or entity_1", entities
        )
