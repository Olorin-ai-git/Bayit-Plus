...
class TestVerificationCache:
    ...
    def cache_instance(self, mock_config):
        ...
        with patch(
            "app.service.agent.verification.verification_cache.asyncio.create_task"
        ):
    ...
    async def test_intelligent_eviction(self, cache_instance):
        ...
        await cache_instance.set(
            "high_confidence", {"confidence_score": 0.95}, "model1"
        )
        await asyncio.sleep(0.01)  # Small delay to differentiate timestamps

        await cache_instance.set(
            "medium_confidence", {"confidence_score": 0.75}, "model2"
        )
