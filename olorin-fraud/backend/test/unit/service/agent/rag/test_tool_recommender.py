...
def mock_tool_registry():
    ...
    def mock_get_tool(name):
        tools_dict = {
            "threat_intelligence_scanner": MockTool(
                "threat_intelligence_scanner", "Threat intel tool"
            ),
...
def mock_knowledge_context():
    ...
    supporting_chunk = DocumentChunk(
        ...
        metadata=DocumentMetadata(tags={"network", "case_study"}, entity_types={"ip"}),
    )
...
def mock_context_augmentor(mock_knowledge_context):
    ...
    augmentor = Mock()
    augmentor.augment_investigation_context = AsyncMock(
        return_value=mock_knowledge_context
    )
    return augmentor
...
def tool_recommender(mock_rag_orchestrator, mock_tool_registry, mock_context_augmentor):
    ...
    return KnowledgeBasedToolRecommender(
        ...
        config=ToolRecommenderConfig(
            max_recommended_tools=5, min_confidence_threshold=0.6
        ),
    )
...
class TestKnowledgeBasedToolRecommender:
    ...
    def test_init(self, tool_recommender):
        ...
        assert (
            "tool_effectiveness_patterns" in tool_recommender.tool_knowledge_categories
        )

    @pytest.mark.asyncio
    async def test_recommend_tools_effectiveness_strategy(
        self, tool_recommender, investigation_context, mock_knowledge_context
    ):
        """Test effectiveness-based tool recommendations"""
        # Mock the knowledge retrieval
        tool_recommender._retrieve_tool_knowledge = AsyncMock(
            return_value=mock_knowledge_context
        )
    ...
    async def test_recommend_tools_hybrid_strategy(
        self, tool_recommender, investigation_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_fallback_recommendations(self, tool_recommender):
        """Test fallback to standard tool selection"""
        with patch(
            "app.service.agent.rag.tool_recommender.get_tools_for_agent"
        ) as mock_get_tools:
            mock_get_tools.return_value = [MockTool("fallback_tool")]

            recommendations = await tool_recommender._fallback_recommendations(
                domain="network", categories=["threat_intelligence"]
            )

            assert len(recommendations) > 0
            assert (
                recommendations[0].reasoning
                == "Standard tool selection (RAG enhancement unavailable)"
            )
    ...
    def test_extract_success_indicators(self, tool_recommender):
        ...
        success_content = (
            "The tool was very successful and effective in detecting fraud"
        )
        failure_content = (
            "The tool failed to provide accurate results and was unreliable"
        )

        success_indicators_good = tool_recommender._extract_success_indicators(
            success_content
        )
        success_indicators_bad = tool_recommender._extract_success_indicators(
            failure_content
        )
    ...
    def test_determine_tool_category(self, tool_recommender):
        ...
        assert (
            tool_recommender._determine_tool_category(threat_tool)
            == "threat_intelligence"
        )
...
class TestToolRecommendation:
    ...
    def test_recommendation_creation(self):
        ...
        recommendation = ToolRecommendation(
            tool=tool, confidence=0.85, reasoning="Test reasoning", domain_match=True
        )
...
def test_create_tool_recommender(mock_rag_orchestrator, mock_tool_registry):
    ...
    recommender = create_tool_recommender(
        rag_orchestrator=mock_rag_orchestrator, tool_registry=mock_tool_registry
    )
