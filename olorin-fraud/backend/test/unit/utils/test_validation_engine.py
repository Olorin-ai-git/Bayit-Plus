...
class TestSingleEntityValidation:
    ...
    def test_validate_entity_calls_specialized_validation(
        self, mock_specialized, engine
    ):
    ...
    def test_validate_entity_with_context(self, engine):
        ...
        result = engine.validate_entity(
            EntityType.EMAIL, "email", "user@example.com", context
        )
    ...
    def test_validate_entity_exception_handling(self, engine):
        ...
        with patch.object(
            engine.comprehensive_validator, "validate_entity"
        ) as mock_validate:
...
class TestBatchEntityValidation:
    ...
    @patch(
        "app.utils.validation_engine.ValidationEngine._apply_cross_entity_validation"
    )
    def test_validate_entity_batch_calls_cross_validation(
        self, mock_cross_validation, engine
    ):
...
class TestTransactionValidation:
    ...
    def test_validate_transaction_data_partial_data(self, engine):
        ...
        transaction_data = {"amount": "50.00", "email": "user@example.com"}
    ...
    def test_validate_transaction_calls_transaction_analysis(
        self, mock_analysis, engine
    ):
        """Test transaction validation calls transaction-specific analysis"""
        transaction_data = {"amount": "100.50", "currency": "USD"}
    ...
    def test_map_transaction_to_entities(self, engine):
        ...
        assert "unknown_field" not in [
            field for fields in entity_mapping.values() for field in fields
        ]
...
class TestSpecializedValidation:
    ...
    @patch(
        "app.utils.validation_rules.financial_rules.FinancialValidationRules.validate_currency_amount"
    )
    ...
    @patch(
        "app.utils.validation_rules.security_rules.SecurityValidationRules.validate_input_security"
    )
    ...
    @patch(
        "app.utils.validation_rules.geographic_rules.GeographicValidationRules.validate_country_code"
    )
    ...
    @patch(
        "app.utils.validation_rules.temporal_rules.TemporalValidationRules.validate_timestamp"
    )
    ...
    @patch(
        "app.utils.validation_rules.network_rules.NetworkValidationRules.validate_ip_address"
    )
...
class TestPerformanceAndScalability:
    ...
    def test_single_validation_performance(self, engine):
        ...
        for entity_type, field_name, value in test_cases:
            ...
            assert (
                execution_time_ms < 100
            ), f"Validation took {execution_time_ms}ms for {entity_type}, should be <100ms"
    ...
    def test_transaction_validation_performance(self, engine):
        ...
        assert (
            execution_time_ms < 100
        ), f"Transaction validation took {execution_time_ms}ms, should be <100ms"
