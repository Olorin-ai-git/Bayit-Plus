...
class TestFinancialValidationRules:
    ...
    def test_financial_validator_initialization(self, financial_validator):
        ...
        assert (
            len(financial_validator.CURRENCY_PRECISION) >= 20
        ), "Should have currency precision data"
        ...
        assert (
            len(financial_validator.PAYMENT_PATTERNS) >= 6
        ), "Should have payment patterns"
...
class TestCurrencyAmountValidation:
    ...
    @pytest.mark.parametrize(
        "amount,currency,expected_valid",
        [
            ("100.50", "USD", True),
            ("0.00", "USD", True),
            ("999999.99", "USD", True),
    ...
    def test_validate_currency_amount_valid(
        self, financial_validator, amount, currency, expected_valid
    ):
        """Test validation of valid currency amounts"""
        is_valid, error = financial_validator.validate_currency_amount(amount, currency)
        assert (
            is_valid == expected_valid
        ), f"Amount {amount} {currency} should be {'valid' if expected_valid else 'invalid'}"
        if expected_valid:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ),  # USD allows only 2 decimal places
    ...
    def test_validate_currency_amount_invalid(
        self, financial_validator, amount, currency, expected_error
    ):
    ...
    def test_currency_precision_validation(self, financial_validator):
        ...
        for amount, currency, should_be_valid in currency_tests:
            is_valid, error = financial_validator.validate_currency_amount(
                amount, currency
            )
            assert (
                is_valid == should_be_valid
            ), f"Precision test failed for {amount} {currency}"
    ...
    def test_amount_type_conversion(self, financial_validator):
        ...
        for amount, currency, expected_valid in test_inputs:
            ...
            assert (
                is_valid == expected_valid
            ), f"Type conversion failed for {type(amount)} {amount}"
    ...
    def test_edge_case_amounts(self, financial_validator):
        ...
        for amount, currency, expected_valid in edge_cases:
            ...
            assert (
                is_valid == expected_valid
            ), f"Edge case failed for {amount} {currency}"
...
class TestPaymentMethodValidation:
    ...
    @pytest.mark.parametrize(
        "payment_method,payment_data,expected_valid",
        [
            ...
            (
                "bank_transfer",
                {"iban": "GB82WEST12345698765432", "swift": "DEUTDEFF"},
                True,
            ),
    ...
    def test_validate_payment_method_valid(
        self, financial_validator, payment_method, payment_data, expected_valid
    ):
        """Test validation of valid payment methods"""
        is_valid, error = financial_validator.validate_payment_method(
            payment_method, payment_data
        )
        assert (
            is_valid == expected_valid
        ), f"Payment method {payment_method} should be {'valid' if expected_valid else 'invalid'}"
        if expected_valid:
            assert error is None

    @pytest.mark.parametrize(
        "payment_method,payment_data,expected_error",
        [
    ...
    def test_validate_payment_method_invalid(
        self, financial_validator, payment_method, payment_data, expected_error
    ):
    ...
    def test_credit_card_luhn_validation(self, financial_validator):
        ...
        for card_number in valid_cards:
            ...
            assert (
                is_valid is True
            ), f"Valid card number should pass Luhn check: {card_number}"
        ...
        for card_number in invalid_cards:
            ...
            assert (
                is_valid is False
            ), f"Invalid card number should fail Luhn check: {card_number}"
    ...
    def test_card_network_detection(self, financial_validator):
        ...
        for card_number, expected_network in card_network_tests:
            ...
            assert (
                detected_network == expected_network
            ), f"Card {card_number} should be detected as {expected_network}"
...
class TestTransactionRiskValidation:
    ...
    def test_validate_transaction_risk_low_risk(self, financial_validator):
        ...
        is_safe, risk_msg, risk_analysis = (
            financial_validator.validate_transaction_risk(low_risk_transaction)
        )
    ...
    def test_validate_transaction_risk_high_risk(self, financial_validator):
        ...
        is_safe, risk_msg, risk_analysis = (
            financial_validator.validate_transaction_risk(high_risk_transaction)
        )
    ...
    def test_high_value_transaction_detection(self, financial_validator):
        ...
        for amount, currency in high_value_amounts:
            ...
            is_safe, risk_msg, risk_analysis = (
                financial_validator.validate_transaction_risk(transaction)
            )
            assert (
                risk_analysis["high_value_transaction"] is True
            ), f"Should detect high value: {amount} {currency}"
    ...
    def test_velocity_limit_detection(self, financial_validator):
        ...
        is_safe, risk_msg, risk_analysis = (
            financial_validator.validate_transaction_risk(high_velocity_transaction)
        )

        assert (
            risk_analysis["velocity_violations"] >= 3
        ), "Should detect multiple velocity violations"
    ...
    def test_suspicious_amount_patterns(self, financial_validator):
        ...
        for amount in suspicious_amounts:
            ...
            assert risk_analysis.get(
                "suspicious_amount_pattern", False
            ), f"Should detect suspicious amount: {amount}"
    ...
    def test_first_transaction_risk_assessment(self, financial_validator):
        ...
        is_safe, risk_msg, risk_analysis = (
            financial_validator.validate_transaction_risk(first_transaction)
        )

        assert (
            risk_analysis["first_transaction_risk"] is True
        ), "Should flag first transaction risk"
    ...
    def test_country_risk_assessment(self, financial_validator):
        ...
        for country in high_risk_countries:
            ...
            assert (
                risk_analysis.get("country_risk", 0) > 0.5
            ), f"Should detect high country risk: {country}"
        ...
        for country in low_risk_countries:
            ...
            assert (
                risk_analysis.get("country_risk", 0) < 0.3
            ), f"Should detect low country risk: {country}"
    ...
    def test_ip_country_mismatch_detection(self, financial_validator):
        ...
        is_safe, risk_msg, risk_analysis = (
            financial_validator.validate_transaction_risk(mismatch_transaction)
        )

        assert (
            risk_analysis["ip_country_mismatch"] is True
        ), "Should detect IP country mismatch"
...
class TestFraudDetection:
    ...
    def test_detect_card_testing_patterns(self, financial_validator):
        ...
        is_fraud, fraud_msg, fraud_analysis = financial_validator.detect_fraud_patterns(
            card_testing_data
        )

        assert is_fraud is True, "Should detect card testing pattern"
        assert (
            fraud_analysis["card_testing_indicators"] >= 3
        ), "Should have multiple card testing indicators"
    ...
    def test_detect_account_takeover_patterns(self, financial_validator):
        ...
        assert (
            fraud_analysis["account_takeover_indicators"] >= 4
        ), "Should have multiple ATO indicators"
    ...
    def test_detect_money_laundering_patterns(self, financial_validator):
        ...
        assert (
            fraud_analysis["money_laundering_indicators"] >= 3
        ), "Should have multiple ML indicators"
    ...
    def test_detect_synthetic_identity_fraud(self, financial_validator):
        ...
        assert (
            fraud_analysis["synthetic_identity_indicators"] >= 3
        ), "Should have multiple synthetic identity indicators"
    ...
    def test_legitimate_transaction_not_flagged(self, financial_validator):
        ...
        assert (
            is_fraud is False
        ), "Legitimate transaction should not be flagged as fraud"
...
class TestPerformanceRequirements:
    ...
    def test_currency_validation_performance(self, financial_validator):
        ...
        ), f"Currency validation took {execution_time_ms}ms, should be <50ms"
    ...
    def test_payment_method_validation_performance(self, financial_validator):
        ...
        assert (
            execution_time_ms < 50
        ), f"Payment validation took {execution_time_ms}ms, should be <50ms"
    ...
    def test_risk_assessment_performance(self, financial_validator):
        ...
        assert (
            execution_time_ms < 50
        ), f"Risk assessment took {execution_time_ms}ms, should be <50ms"
    ...
    def test_luhn_caching_performance(self, financial_validator):
        ...
        assert (
            warm_time <= cold_time * 1.1
        ), "Cached Luhn validation should not be significantly slower"
...
class TestEdgeCasesAndErrorHandling:
    ...
    def test_missing_payment_data_handling(self, financial_validator):
        ...
        for method, data in incomplete_data_sets:
            ...
            assert (
                is_valid is False
            ), f"Incomplete payment data should be invalid: {method}, {data}"
    ...
    def test_unsupported_currency_handling(self, financial_validator):
        ...
        for currency in unsupported_currencies:
            try:
                ...
                assert (
                    is_valid is False
                ), f"Unsupported currency should be invalid: {currency}"
