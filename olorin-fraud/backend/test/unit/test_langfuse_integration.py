...
async def test_agent_with_tracing(
    input_data: Dict[str, Any], config: RunnableConfig = None
):
    """
...
async def test_langchain_with_langfuse():
    ...
    with tracer.trace_investigation(
        ...
        llm = ChatOpenAI(model="gpt-3.5-turbo", temperature=0, callbacks=[handler])

        # Test simple completion
        messages = [
            HumanMessage(
                content="What are the main indicators of fraud in online transactions?"
            )
        ]
    ...
    tracer.flush()
    logger.info(
        f"✅ Test completed. Check Langfuse dashboard for trace: {investigation_id}"
    )
...
async def test_multiple_agents():
    ...
    with tracer.trace_investigation(
        ...
        for agent_name in agents:
            ...
            @tracer.trace_agent(
                agent_name=agent_name, trace_id=trace_context["trace"].id
            )
            async def run_agent():
