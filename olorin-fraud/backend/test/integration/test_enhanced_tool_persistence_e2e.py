...
def sample_investigation(test_db: Session):
    ...
    state = InvestigationState(
        ...
        settings_json=json.dumps(
            {"entities": [{"entity_type": "ip_address", "entity_value": "8.8.8.8"}]}
        ),
        progress_json=json.dumps({"tool_executions": [], "percent_complete": 0}),
...
async def test_enhanced_tool_persistence_e2e(test_db: Session, sample_investigation):
    ...
    assert (
        len(tool_executions) == 2
    ), f"Expected 2 tool executions, got {len(tool_executions)}"
    ...
    assert (
        len(progress.tool_executions) == 2
    ), f"Expected 2 tools in progress, got {len(progress.tool_executions)}"
    assert (
        progress.total_tools == 2
    ), f"Expected total_tools=2, got {progress.total_tools}"
    assert (
        progress.completed_tools == 2
    ), f"Expected completed_tools=2, got {progress.completed_tools}"
    ...
    device_exec = next(
        (t for t in progress.tool_executions if t.tool_name == "device_fingerprint"),
        None,
    )
    assert device_exec is not None, "Device fingerprint tool not found"
    assert (
        device_exec.status == "completed"
    ), f"Expected status=completed, got {device_exec.status}"
    assert device_exec.result is not None, "Tool result should not be None"
    assert (
        device_exec.result.risk_score == 0.35
    ), f"Expected risk_score=0.35, got {device_exec.result.risk_score}"
    ...
    assert (
        stats["total_tokens"] == 85
    ), f"Expected total_tokens=85, got {stats['total_tokens']}"
