...
class MockRAGOrchestrator:
    ...
    async def retrieve_knowledge(
        self, query: str, domain: str = None, max_chunks: int = 10
    ):
        return {
            "chunks": [
...
class MockContextAugmentor:
    ...
    async def augment_investigation_context(
        self, investigation_context, domain, specific_objectives=None
    ):
...
async def context_enhancer():
    ...
    with (
        patch(
            "app.service.agent.tools.rag_tool_context.get_rag_orchestrator"
        ) as mock_rag,
        patch(
            "app.service.agent.tools.rag_tool_context.create_context_augmentor"
        ) as mock_augmentor,
    ):
...
class TestToolExecutionContextEnhancer:
    ...
    @pytest.mark.asyncio
    async def test_context_enhancement_basic(
        self, context_enhancer, mock_investigation_context
    ):
    ...
    async def test_performance_target_compliance(
        ...
        assert (
            total_time < 50.0
        ), f"Enhancement took {total_time:.1f}ms, exceeding 50ms target"
    ...
    @pytest.mark.asyncio
    async def test_parameter_enhancement_splunk(
        self, context_enhancer, mock_investigation_context
    ):
        """Test Splunk-specific parameter enhancement"""

        input_params = {"query": "index=security error", "max_results": 100}
...
class TestRAGEnhancedToolBase:
    ...
    async def test_rag_enhanced_execution(self, test_tool, mock_investigation_context):
        ...
        with patch.object(test_tool, "context_enhancer") as mock_enhancer:
            ...
            mock_enhancer.enhance_tool_execution_context = AsyncMock(
                return_value=ToolExecutionContext(
...
class TestRAGToolPerformanceMonitor:
    ...
    def test_performance_target_violation_alert(self):
        ...
        violation_alerts = [
            a for a in recent_alerts if a.alert_type == "consecutive_target_violations"
        ]
    ...
    def test_system_health_calculation(self):
        ...
        for i in range(7):
            monitor.record_execution_performance(
                "good_tool", f"good_{i}", good_timing, context_data
            )

        for i in range(3):
            monitor.record_execution_performance(
                "poor_tool", f"poor_{i}", poor_timing, context_data
            )
        ...
        assert (
            0.6 <= health_score <= 0.8
        )  # Should reflect 70% compliance with some penalties
...
class TestAgentRAGToolOrchestrator:
    ...
    @pytest.mark.asyncio
    async def test_agent_tool_orchestration(
        self, test_tool, mock_investigation_context
    ):
        """Test agent-level RAG tool orchestration"""

        with patch(
            "app.service.agent.tools.rag_tool_integration.get_tool_context_enhancer"
        ) as mock_enhancer:
            mock_enhancer.return_value.rag_available = True

            orchestrator = AgentRAGToolOrchestrator()
            orchestrator.set_investigation_context(
                "test_investigation", "network_agent"
            )

            input_data = {"query": "orchestration_test"}
            agent_context = {"domain": "network", "agent_id": "network_agent"}
...
class TestRAGToolFactory:
    ...
    def test_wrap_existing_tool_with_rag(self):
        ...
        wrapped_tool = RAGToolFactory.wrap_existing_tool_with_rag(
            original_tool, enable_rag=True
        )
...
async def test_end_to_end_integration(mock_investigation_context):
    ...
    with (
        ...
        agent_context = {"domain": "network", "agent_id": "test_agent"}
