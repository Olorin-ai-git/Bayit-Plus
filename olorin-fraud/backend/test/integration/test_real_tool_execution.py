...
class ToolExecutionMonitor:
    ...
    def track_tool_call(
        self, tool_name: str, agent_name: str = "unknown", args: Any = None
    ):
    ...
    try:
        ...
        for tool in all_tools:
            if hasattr(tool, "invoke"):
                ...
                tool.invoke = lambda *args, t=tool, **kwargs: monitored_invoke(
                    t, *args, **kwargs
                )
        ...
        for domain in domains:
            agent = StructuredInvestigationAgent(
                domain=domain, tools=all_tools  # Give all tools to each agent
            )
            agents[domain] = agent
        ...
        config = RunnableConfig(
            tags=["test_investigation"], metadata={"test_mode": True}
        )
        ...
        for domain, agent in agents.items():
            ...
            try:
                ...
                if agent.use_sequential_prompting:
                    ...
                    print(
                        f"   ℹ️  Will execute through {len(agent.sequential_prompt_manager.phases)} phases"
                    )

                    # Simulate phase execution
                    for phase in agent.sequential_prompt_manager.phases:
                        monitor.track_phase_transition(
                            f"{domain}_phase_{phase.phase_id}"
                        )
                        ...
                        for category in phase.tool_categories:
                            category_tools = tool_registry.get_tools_by_category(
                                category
                            )
                            for tool in category_tools[
                                :2
                            ]:  # Execute 2 tools per category
                                monitor.track_tool_call(tool.name, domain)
                                phase_tools.append(tool.name)
                                TOOL_EXECUTION_LOG["tools_by_phase"][
                                    monitor.phase_counter
                                ].add(tool.name)

                        if phase_tools:
                            print(
                                f"      Executed {len(phase_tools)} tools: {', '.join(phase_tools[:3])}..."
                            )
                else:
        ...
        for phase_num, tools in TOOL_EXECUTION_LOG["tools_by_phase"].items():
            phase_info = next(
                (
                    p
                    for p in TOOL_EXECUTION_LOG["phase_transitions"]
        ...
        if TOOL_EXECUTION_LOG["execution_timeline"]:
            print(
                f"   ... total of {len(TOOL_EXECUTION_LOG['execution_timeline'])} tool executions"
            )
        ...
        print(
            f"   • Tool coverage: {used_tools}/{total_tools} ({usage_percentage:.1f}%)"
        )

        if TOOL_EXECUTION_LOG["total_executions"] >= 20:
            print(
                f"   ✅ PASSED: Multiple tools executed ({TOOL_EXECUTION_LOG['total_executions']} executions)"
            )
        else:
            print(
                f"   ⚠️  WARNING: Only {TOOL_EXECUTION_LOG['total_executions']} tool executions"
            )
        ...
        if len(TOOL_EXECUTION_LOG["phase_transitions"]) >= 4:
            print(
                f"   ✅ PASSED: Multiple phases executed ({len(TOOL_EXECUTION_LOG['phase_transitions'])} phases)"
            )
        else:
            print(
                f"   ⚠️  WARNING: Few phases executed ({len(TOOL_EXECUTION_LOG['phase_transitions'])} phases)"
            )
        ...
        with open(report_file, "w") as f:
            report_data = {
                ...
                "tools_by_agent": {
                    k: list(v) for k, v in TOOL_EXECUTION_LOG["tools_by_agent"].items()
                },
                "tools_by_phase": {
                    str(k): list(v)
                    for k, v in TOOL_EXECUTION_LOG["tools_by_phase"].items()
                },
                "phase_transitions": TOOL_EXECUTION_LOG["phase_transitions"],
                "execution_timeline": TOOL_EXECUTION_LOG["execution_timeline"][
                    :100
                ],  # First 100 events
            }
