...
class TestMCPSecurityIntegration:
    ...
    async def test_fraud_database_tool_security_flow(
        self, auth_service, input_validator, mock_base_auth, sample_user
    ):
        ...
        validation_results = input_validator.validate_mcp_tool_input(
            tool_name="fraud_database_query", inputs=tool_inputs
        )
        ...
        for field_name, validation_result in validation_results.items():
            ...
            ), f"Field {field_name} validation failed: {validation_result.errors}"
    ...
    async def test_unauthorized_tool_access(
        self, auth_service, mock_base_auth, sample_user
    ):
        ...
        context, result = await auth_service.validate_mcp_token(
            ...
            required_permissions=[
                MCPPermission.ADMIN_CONFIG_MANAGE
    ...
    async def test_rate_limiting_enforcement(
        ...
        context, result = await auth_service.validate_mcp_token(
            token="valid_jwt_token", ip="192.168.1.100", user_agent="OlorinAgent/1.0"
        )
    ...
    def test_fraud_specific_validation_rules(self, input_validator):
        ...
        results = input_validator.validate_mcp_tool_input(
            tool_name="fraud_pattern_matching", inputs=tool_inputs
        )
        ...
        for field_name, result in results.items():
            if field_name != "tool_name":  # Skip tool name validation
                ...
                ), f"Field {field_name} should be valid"
    ...
    async def test_complete_fraud_investigation_workflow(
        ...
        validation_results = input_validator.validate_mcp_tool_input(
            tool_name="comprehensive_fraud_investigation", inputs=investigation_params
        )

        # All parameters should be valid
        all_valid = all(
            result.is_valid for field_name, result in validation_results.items()
        )
        assert (
            all_valid
        ), f"Validation errors: {[(k, v.errors) for k, v in validation_results.items() if not v.is_valid]}"
        ...
        for server_name, tool_name, operation in tools_executed:
            await auth_service.audit_tool_execution(
                ...
                duration_ms=200
                + len(tool_name) * 10,  # Simulate varying response times
            )
        ...
        assert audit_calls >= len(
            tools_executed
        ), f"Expected at least {len(tools_executed)} audit calls, got {audit_calls}"
        ...
        print(
            f"   - Validation summary: {summary['valid_fields']}/{summary['total_fields']} fields valid"
        )
