...
def test_json_parsing_fix():
    ...
    parsed_data = (
        json.loads(mock_json_string)
        if isinstance(mock_json_string, str)
        else mock_json_string
    )
...
def test_risk_calculation_logic():
    ...
    model_scores = [
        float(r.get("MODEL_SCORE", 0)) for r in mock_results if "MODEL_SCORE" in r
    ]
    ...
    if model_scores:
        ...
        if final_risk_score >= 0.9:
            ...
            print(
                f"   ❌ Risk calculation failed: expected >= 0.9, got {final_risk_score}"
            )
            return False
    else:
...
def test_final_risk_aggregation():
    ...
    if risk_scores:
        ...
        print(
            f"   Weighted calculation: {weighted_score:.4f} / {total_weight:.2f} = {final_risk:.4f}"
        )
        ...
        if final_risk >= 0.6:  # High network score should push overall risk high
            ...
            print(
                f"   ❌ Final risk aggregation failed: expected >= 0.6, got {final_risk}"
            )
            return False
