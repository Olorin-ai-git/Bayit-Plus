...
from app.service.agent.orchestration.hybrid.integration.service_adapter import (
    ServiceAdapter,
)
...
def test_service_adapter_initialization():
    ...
    with patch(
        "app.service.agent.orchestration.hybrid.integration.service_adapter.GraphSelector"
    ) as MockGraphSelector:
    ...
    with patch(
        "app.service.agent.orchestration.hybrid.integration.service_adapter.GraphSelector",
        side_effect=Exception("Mock init failure"),
    ):
        adapter = ServiceAdapter()
...
def test_graph_type_determination_null_safety():
    ...
    mock_graph._nodes.__iter__ = Mock(
        side_effect=Exception("Mock node iteration failure")
    )
    graph_type = adapter._determine_graph_type(mock_graph)
...
async def run_all_tests():
    ...
    tests = [
        ...
        (
            "Investigation Completion Null Safety",
            test_investigation_completion_null_safety,
        ),
    ]
