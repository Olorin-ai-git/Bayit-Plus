...
async def test_virustotal_ip_analysis_in_event_loop():
    ...
    try:
        from app.service.agent.tools.threat_intelligence_tool.virustotal.ip_analysis_tool import (
        ...
        if "asyncio.run() cannot be called" in str(e):
            ...
            return (
                True  # Other errors are OK, we just care about the asyncio.run() issue
            )
...
async def test_virustotal_domain_analysis_in_event_loop():
    ...
    try:
        ...
        parsed_result = json.loads(result)
        print(
            f"   Success: {parsed_result.get('domain', 'unknown')} analysis completed"
        )
...
def test_async_helpers_utility():
    ...
    from app.service.agent.tools.async_helpers import (
...
async def run_all_tests():
    ...
    tests = [
        (
            "VirusTotal IP Analysis in Event Loop",
            test_virustotal_ip_analysis_in_event_loop,
        ),
        (
            "VirusTotal Domain Analysis in Event Loop",
            test_virustotal_domain_analysis_in_event_loop,
        ),
