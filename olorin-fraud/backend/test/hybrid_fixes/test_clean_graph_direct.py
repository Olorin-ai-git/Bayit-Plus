...
async def test_clean_graph_simple():
    ...
    try:
        ...
        print(
            f"   Snowflake completed: {final_state.get('snowflake_completed', False)}"
        )
        ...
        if tools_used == 0 and tool_attempts == 0:
            ...
            print(
                f"   Tools used: {tools_used}, Results: {tool_results}, Attempts: {tool_attempts}"
            )
...
async def main():
    ...
    if result["success"]:
        ...
        if result["tools_used"] > 0:
            ...
            print(
                "   Hybrid graph issue is likely in the enhanced tool node or routing"
            )
        else:
