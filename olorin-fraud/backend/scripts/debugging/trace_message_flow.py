...
async def trace_message_flow():
    ...
    try:
            ...
            tool_calls=[
                {
                    ...
                    "args": {
                        "query": "SELECT * FROM TRANSACTIONS WHERE IP = '67.76.8.209' LIMIT 5"
                    },
        ...
        print(
            f"   📝 AI Message created with tool_call id: {ai_message.tool_calls[0]['id']}"
        )
        ...
        if isinstance(tool_result, dict) and "messages" in tool_result:
            ...
            for i, msg in enumerate(tool_messages):
                ...
                if hasattr(msg, "tool_call_id"):
                    print(
                        f"   📝 Message {i+1}: {msg_type} with tool_call_id: {msg.tool_call_id}"
                    )
        ...
        for tool_call_id, result_pos in tool_results.items():
            if tool_call_id not in tool_uses:
                ...
                if use_pos < result_pos:
                    ...
                    print(
                        f"   ❌ INVALID ORDER: tool_result at {result_pos} before tool_use at {use_pos}"
                    )
        ...
        for tool_call_id, use_pos in tool_uses.items():
            if tool_call_id not in tool_results:
                ...
                print(
                    f"   ⚠️ ORPHANED tool_use: {tool_call_id} at position {use_pos} (no result)"
                )
        ...
        if orphaned_results:
            ...
            print(
                "   The tool_result references a tool_use_id that doesn't exist in the sequence."
            )
            return {
                "success": False,
