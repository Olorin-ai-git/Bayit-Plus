...
from app.service.agent.tools.threat_intelligence_tool.abuseipdb.ip_reputation_tool import (
    IPReputationTool,
)
from app.service.agent.tools.threat_intelligence_tool.shodan.infrastructure_analysis_tool import (
...
async def test_tool_error_handling():
    ...
    try:
        result = await sumologic_tool._arun(query="test query", time_range="24h")
    ...
    try:
        ...
        print(
            f"📄 Response success: {result.success}, error: {getattr(result, 'error', 'None')}"
        )
    except Exception as e:
    ...
    print(
        "📝 Check logs to verify that failed tools only generate DEBUG/WARNING messages, not ERROR messages"
    )
