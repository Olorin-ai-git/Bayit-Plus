...
def patched_assistant(state, config):
    ...
    if orphaned_results:
        print(
            f"🚨 CORRUPTION DETECTED! About to send corrupted sequence to Anthropic API"
        )
        ...
        raise Exception(
            f"CORRUPTION DETECTED: tool_result {orphaned_results[0][0]} has no corresponding tool_use"
        )
