...
def analyze_message_sequence(messages, step_name):
    ...
    for i, msg in enumerate(messages):
        ...
        if hasattr(msg, "tool_calls") and msg.tool_calls:
            ...
            for j, tc in enumerate(msg.tool_calls):
                ...
                tool_uses[tool_id] = i
                print(
                    f"      Tool call {j+1}: id={tool_id}, name={tc.get('name', 'NO_NAME')}"
                )
    ...
    for tool_call_id, result_pos in tool_results.items():
        if tool_call_id not in tool_uses:
            ...
            print(
                f"   ❌ ORPHANED tool_result: {tool_call_id} at position {result_pos}"
            )
        else:
            use_pos = tool_uses[tool_call_id]
            print(
                f"   ✅ Valid pair: tool_use at {use_pos} → tool_result at {result_pos}"
            )
...
async def trace_hybrid_flow():
    ...
    try:
        ...
        graph = await builder.build_hybrid_investigation_graph(
            use_enhanced_tools=True, enable_streaming=True, enable_interrupts=False
        )
        ...
        try:
            async for step in graph.astream(initial_state, config=config):
                ...
                for node_name, node_result in step.items():
                    ...
                    if messages:
                        is_valid, orphaned = analyze_message_sequence(
                            messages, f"After {node_name}"
                        )
                ...
                if step_count >= 7:
                    ...
                    print(
                        "   The corruption might occur during LLM invocation, not in graph steps"
                    )
                    break
...
if __name__ == "__main__":
    ...
    if result.get("corruption_detected"):
        print(
            f"✅ Found corruption source: {result['corruption_node']} at step {result['corruption_step']}"
        )
    elif result.get("error_caught"):
        print(
            f"✅ Caught error at step {result['error_step']} with tool_id {result.get('problematic_tool_id')}"
        )
    else:
