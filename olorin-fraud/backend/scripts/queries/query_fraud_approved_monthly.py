...
async def query_fraud_approved_monthly():
    ...
    try:
        ...
        if results:
            ...
            print(
                f"{'Month':<20} {'Count':<15} {'Total Amount':<20} {'Unique Emails':<15} {'Unique TXs':<15}"
            )
            print("-" * 85)
            logger.info(f"\n✅ Results: {len(results)} months found\n")
            logger.info(
            ...
            for row in results:
                ...
                unique_txs = row.get("UNIQUE_TRANSACTIONS") or row.get(
                ...
                print(
                    f"{month_str:<20} {count:<15} {amount_str:<20} {unique_emails:<15} {unique_txs:<15}"
                )
                logger.info(
                    f"{month_str:<20} {count:<15} {amount_str:<20} {unique_emails:<15} {unique_txs:<15}"
                )
        ...
        try:
            ...
            if hasattr(db_provider, "_client") and hasattr(
                db_provider._client, "disconnect"
            ):
