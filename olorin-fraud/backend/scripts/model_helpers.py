...
def load_features_and_labels() -> tuple[pd.DataFrame, pd.Series]:
    ...
    with get_db_session() as db:
        ...
        df = df.drop(
            columns=["txn_id", "txn_ts", "merchant_id", "card_id", "currency", "y_true"]
        )
...
def temporal_split(
    features: pd.DataFrame, labels: pd.Series, train_ratio: float = 0.8
) -> tuple:
...
def calibrate_model(
    ...
    if method not in ["isotonic", "sigmoid"]:
        raise ValueError(
            f"Invalid calibration method: {method}. Use 'isotonic' or 'sigmoid'"
        )
...
def score_all_transactions(
    model: CalibratedClassifierCV, features: pd.DataFrame
) -> pd.Series:
...
def store_scores(
    ...
    scores: pd.Series,
    model_version: str,
    threshold_cohort: Optional[str] = None,
