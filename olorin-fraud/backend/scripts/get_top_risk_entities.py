...
async def main():
    ...
    parser.add_argument(
        "--top", type=float, default=10, help="Top percentage to return (default: 10)"
    )
    parser.add_argument(
        "--force-refresh", action="store_true", help="Force refresh, bypass cache"
    )
    ...
    if args.json:
        ...
        if results.get("status") == "success":
            ...
            if entities:
                ...
                print(
                    f"{'Rank':<6} {'Entity':<40} {'Risk Score':<12} {'Risk Value':<15} {'Txns':<6} {'Fraud'}"
                )
                print("-" * 95)
                ...
                for i, entity in enumerate(entities, 1):
                    ...
                    print(
                        f"{i:<6} {entity_id:<40} {risk_score:<12.4f} ${risk_value:<14,.2f} {tx_count:<6} {fraud_count}"
                    )
            else:
