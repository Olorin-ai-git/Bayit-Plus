...
def check_investigation_risk_scores(entity_value: str = None, entity_type: str = None):
    ...
    if entity_value:
        investigations = [
            ...
            if inv.get("entity_id") == entity_value
            or inv.get("entity_value") == entity_value
        ]

    if entity_type:
        investigations = [
            inv for inv in investigations if inv.get("entity_type") == entity_type
        ]
    ...
    for inv in investigations:
        ...
        if status.upper() in ("COMPLETED", "COMPLETE"):
            if overall_risk_score is not None:
                ...
                print(
                    f"✅ {inv_id[:12]}... | {entity_type_val}:{entity_id[:30]} | Risk: {overall_risk_score:.3f}"
                )
            else:
                completed_without_score += 1
                print(
                    f"⚠️  {inv_id[:12]}... | {entity_type_val}:{entity_id[:30]} | Risk: MISSING"
                )
        else:
            incomplete += 1
    ...
    if completed_without_score > 0:
        ...
        print(
            f"   These will have predicted_risk=None in comparisons (NO FALLBACK to MODEL_SCORE)."
        )
