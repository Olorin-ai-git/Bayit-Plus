...
async def test_journey_initialization():
    ...
    try:
        ...
        logger.info(f"   Active journeys: {len(journey_tracker._active_journeys)}")
        logger.info(
            f"   Investigation {investigation_id} tracked: {investigation_id in journey_tracker._active_journeys}"
        )
        ...
        if investigation_id in journey_tracker._active_journeys:
            ...
            logger.info(
                f"   Journey metadata keys: {list(journey.journey_metadata.keys())}"
            )

            return True
        else:
            logger.error(
                f"❌ Investigation {investigation_id} not found in active journeys"
            )
            return False
