...
async def test_all_apis():
    ...
    results = {"AbuseIPDB": False, "VirusTotal": False, "Shodan": False}
    ...
    try:
        ...
        config = AbuseIPDBConfig(
            api_key_secret="ABUSEIPDB_API_KEY", timeout=30, max_retries=3
        )
        client = AbuseIPDBClient(config)

        # Test with Google DNS
        result = await client.check_ip_reputation(
            "8.8.8.8", max_age_days=90, verbose=False
        )
        if result.success:
