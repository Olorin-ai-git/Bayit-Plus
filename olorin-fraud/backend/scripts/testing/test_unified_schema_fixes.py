        ...
        try:
            ...
            if result:
                ...
                self.test_results.append(
                    {"test": test_name, "status": "PASSED", "details": ""}
                )
            else:
...
def test_unified_schema_creation():
    ...
    print(
        f"   Network response created with risk score: {network_response.overall_risk_score}"
    )
    return True
...
def test_legacy_response_conversion():
    ...
    legacy_string = (
        "risk_level: 0.65, confidence: 0.78, findings: suspicious activity detected"
    )
...
def test_json_serialization():
    ...
    print(
        f"   JSON serialization successful, risk score: {json_dict['overall_risk_score']}"
    )
    return True
