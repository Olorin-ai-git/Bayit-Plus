...
async def test_realtime_tool_execution():
    ...
    try:
        ...
        request = ToolCallRequest(arguments={"query": "SELECT 1 as connection_test"})
            ...
            with capture_server_logs(
                ...
                raw_data_count = len(
                    [
                        t
                        for t in tools
                        if "snowflake" in t.name.lower() or "database" in t.name.lower()
                    ]
                )
...
async def main():
    ...
    if all_passed:
        ...
        print(
            "✅ MCP Bridge provides real-time raw data access to investigation agents"
        )
