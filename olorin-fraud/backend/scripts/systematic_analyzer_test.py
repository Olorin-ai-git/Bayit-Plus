...
def run_analyzer_query(
    conn, window_start: datetime, window_end: datetime, ranking_type: str = "current"
) -> List[Dict[str, Any]]:
    """
...
def test_window(
    conn, offset_days: int, ranking_type: str = "current"
) -> Dict[str, Any]:
    """
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=180,
        help="Starting offset in days (default: 180 = 6 months)",
    )
    parser.add_argument(
        "--num-windows",
        type=int,
        default=30,
        help="Number of consecutive days to test (default: 30)",
    )
    parser.add_argument(
        "--ranking",
        choices=["current", "fraud_aware", "both"],
        default="both",
        help="Ranking type to test",
    )
    parser.add_argument(
        "--output", type=str, default="systematic_test_results.json", help="Output file"
    )
    ...
    print(f"╠{'═'*68}╣")
    print(
        f"║  Testing {args.num_windows} consecutive 24H windows                                ║"
    )
    print(
        f"║  Starting at {args.start_offset_days} days ago (~{args.start_offset_days/30:.1f} months)                          ║"
    )
    ...
    try:
        ...
        for ranking_type in ["current", "fraud_aware"]:
            ...
            avg_precision = (
                sum(w["precision"] for w in window_results) / total_windows
                if total_windows > 0
                else 0
            )
            overall_precision = (
            ...
            print(
                f"  Windows with fraud found: {windows_with_fraud} ({windows_with_fraud/total_windows*100:.1f}%)"
            )
            print(f"  Total entities investigated: {total_entities}")
            print(
                f"  Entities with fraud: {total_fraud_entities} ({overall_precision*100:.1f}%)"
            )
