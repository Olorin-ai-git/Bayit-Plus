...
async def generate_for_investigation(investigation_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        result = await generate_confusion_table_for_investigation(
        ...
        logger.error(
            f"Failed to generate confusion table for {investigation_id}: {e}",
            exc_info=True,
        )
        return {
            "investigation_id": investigation_id,
        ...
        with get_db_session() as session:
            ...
            if failed_count > 0 or error_count > 0:
                ...
                for r in results:
                    if r["status"] != "success":
                        print(
                            f"  - {r['investigation_id']}: {r.get('error', 'Unknown')}"
                        )
