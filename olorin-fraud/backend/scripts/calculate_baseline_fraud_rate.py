...
def main():
    ...
    try:
        ...
        for i in range(args.num_windows):
            ...
            if result:
                ...
                if total_entities and total_entities > 0:
                    ...
                    tx_fraud_rate = (
                        (total_fraud_txs / total_txs) * 100 if total_txs > 0 else 0
                    )
                    ...
                    if fraud_entities > 0:
                        ...
                        print(
                            f"  Fraud entities: {fraud_entities} ({entity_fraud_rate:.2f}%)"
                        )
                        print(f"  Total transactions: {total_txs:,}")
                        print(
                            f"  Fraud transactions: {total_fraud_txs} ({tx_fraud_rate:.2f}%)"
                        )
                        print()
        ...
        if result:
            ...
            entity_fraud_rate = (
                (fraud_entities / total_entities) * 100 if total_entities > 0 else 0
            )
