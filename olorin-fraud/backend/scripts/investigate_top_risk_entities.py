...
class RiskEntityInvestigator:
    ...
    async def get_top_risk_entities(
        ...
        if results["status"] == "success":
            ...
            print(
                f"❌ Failed to fetch entities: {results.get('error', 'Unknown error')}"
            )
            return []

    async def get_entity_historical_data(
        self, entity_value: str, entity_type: str, lookback_days: int = 90
    ) -> Dict[str, Any]:
        """
            ...
            if results:
                ...
                return {
                    ...
                    "historical_patterns": {
                        ...
                        "stddev_daily_transactions": float(
                            data.get("STDDEV_DAILY_TXNS", 0)
                        ),
    ...
    def trigger_structured_investigation(
        ...
        script_path = (
            Path(__file__).parent / "testing" / "unified_structured_test_runner.py"
        )
        ...
        print(
            f"   Volume Anomaly Score: {historical_data['anomaly_scores']['volume_anomaly']:.2f}"
        )
        print(
            f"   Risk Anomaly Score: {historical_data['anomaly_scores']['risk_anomaly']:.2f}"
        )
        ...
        try:
            ...
            return {"status": "timeout", "entity": entity_value, "scenario": scenario}
        except Exception as e:
            print(f"❌ Investigation error: {e}")
            return {"status": "error", "entity": entity_value, "error": str(e)}
    ...
    async def investigate_all_top_entities(
        ...
        for i, entity in enumerate(entities_to_investigate, 1):
            print(
                f"   {i}. {entity['entity']} (Risk Value: ${entity['risk_weighted_value']:,.2f})"
            )
        ...
        for i, entity in enumerate(entities_to_investigate, 1):
            print(f"\n{'='*80}")
            print(
                f"Investigation {i}/{len(entities_to_investigate)}: {entity['entity']}"
            )
            print("=" * 80)

            # Get historical data
            historical_data = await self.get_entity_historical_data(
                entity_value=entity["entity"], entity_type=group_by, lookback_days=90
            )
            ...
            if historical_data:
                ...
                print(
                    f"   Average Daily Transactions: {patterns['avg_daily_transactions']:.1f}"
                )
    ...
    def _generate_summary_report(self):
        ...
        successful = sum(
            1 for r in self.investigation_results if r["status"] == "success"
        )
        ...
        for result in self.investigation_results:
            ...
            print(
                f"{status_icon} {result['entity']}: {result.get('scenario', 'N/A')} - {result['status']}"
            )

        # Save report to file
        report_file = (
            f"investigation_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
...
async def main():
    ...
    parser.add_argument(
        "--top", type=float, default=10, help="Top percentage to investigate (1-100)"
    )
