...
async def analyze_false_negatives():
    ...
    if not fraud_results:
        print(
            "⚠️ No fraud transactions found for this entity in the investigation window"
        )
        return
    ...
    fraud_txs = [
        r for r in all_results if (r.get("IS_FRAUD") or r.get("is_fraud")) == 1
    ]
    clean_txs = [
        r for r in all_results if (r.get("IS_FRAUD") or r.get("is_fraud")) == 0
    ]
    ...
    print(
        f"  Amount: ${fraud_stats.get('avg_amount', 0):.2f} avg (${fraud_stats.get('min_amount', 0):.2f} - ${fraud_stats.get('max_amount', 0):.2f})"
    )
    print(
        f"  MODEL_SCORE: {fraud_stats.get('avg_model_score', 0):.4f} avg ({fraud_stats.get('min_model_score', 0):.4f} - {fraud_stats.get('max_model_score', 0):.4f})"
    )
    ...
    print(
        f"  Amount: ${clean_stats.get('avg_amount', 0):.2f} avg (${clean_stats.get('min_amount', 0):.2f} - ${clean_stats.get('max_amount', 0):.2f})"
    )
    print(
        f"  MODEL_SCORE: {clean_stats.get('avg_model_score', 0):.4f} avg ({clean_stats.get('min_model_score', 0):.4f} - {clean_stats.get('max_model_score', 0):.4f})"
    )
    ...
    if avg_fraud_score < avg_clean_score:
        ...
        print(
            f"   → Investigation excluded MODEL_SCORE (correctly, for unbiased evaluation)"
        )
        print(f"   → But this means investigation can't use this signal")
    else:
        print(
            f"   ⚠️ Fraud MODEL_SCORE ({avg_fraud_score:.4f}) vs Clean ({avg_clean_score:.4f})"
        )
    print()
    ...
    if fraud_tx_per_ip > clean_tx_per_ip:
        print(
            f"   ⚠️ Fraud shows IP concentration: {fraud_tx_per_ip:.1f} tx/IP vs {clean_tx_per_ip:.1f} for clean"
        )
    ...
    if fraud_tx_per_device > clean_tx_per_device:
        print(
            f"   ⚠️ Fraud shows device concentration: {fraud_tx_per_device:.1f} tx/device vs {clean_tx_per_device:.1f} for clean"
        )
    print()
    ...
    if fraud_txs:
        ...
        last_fraud = fraud_txs[-1].get("TX_DATETIME") or fraud_txs[-1].get(
            "tx_datetime"
        )
        ...
        if len(fraud_txs) > 1:
            fraud_dates = [
                datetime.fromisoformat(
                    str(tx.get("TX_DATETIME") or tx.get("tx_datetime"))[:19]
                )
                for tx in fraud_txs
            ]
            ...
            if fraud_span_days > 0:
                ...
                print(
                    f"   Fraud frequency: {fraud_frequency:.2f} transactions/day over {fraud_span_days} days"
                )
    print()
    ...
    print(
        "   - Investigation cannot use MODEL_SCORE (excluded for unbiased evaluation)"
    )
    print()
    print("3. APPROVED FRAUD LOOKS LEGITIMATE")
    print(
        "   - These transactions passed initial approval (NSURE_LAST_DECISION = APPROVED)"
    )
