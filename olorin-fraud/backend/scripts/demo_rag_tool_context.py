...
class DemoToolExecutionContextEnhancer:
    ...
    async def enhance_tool_execution_context(
        ...
        knowledge_data = await self.rag_orchestrator.retrieve_knowledge(
            knowledge_query, domain
        )

        # Step 2: Create knowledge context
        knowledge_context = MockKnowledgeContext(
            domain or "general", len(knowledge_data["chunks"])
        )
        ...
        print(
            f"   🎯 Target compliance: {'✅' if enhancement_time_ms < self.performance_target_ms else '❌'}"
        )
    ...
    async def _enhance_parameters(
        self, tool_name: str, params: Dict[str, Any], knowledge_context
    ) -> Dict[str, Any]:
        ...
        if "splunk" in tool_name.lower():
            if "query" in params:
                ...
                enhanced["_rag_enhancement"] = (
                    f"Added {len(knowledge_terms)} knowledge terms"
                )
            ...
            if "max_results" in params:
                enhanced["max_results"] = max(
                    params["max_results"], 50
                )  # Increase based on knowledge
                enhanced["_rag_enhancement"] = (
                    "Increased result limit based on knowledge complexity"
                )
...
class DemoRAGEnhancedTool:
    ...
    async def execute(self, input_data: Dict[str, Any], context: Dict[str, Any] = None):
        ...
        investigation_context = (
            context.get("investigation_context") if context else None
        )
        ...
        result = await self._execute_impl(
            enhanced_context["enhanced_parameters"], enhanced_context
        )
    ...
    async def _execute_impl(
        self, enhanced_params: Dict[str, Any], context: Dict[str, Any]
    ):
...
async def demo_rag_tool_context_injection():
    ...
    for i, scenario in enumerate(demo_scenarios, 1):
        ...
        try:
            ...
            print(
                f"   ⚡ Total time: {result['execution_time_ms'] + result['enhancement_time_ms']:.1f}ms"
            )
    ...
    avg_enhancement_time = sum(r.get("enhancement_time_ms", 0) for r in results) / max(
        1, successful_executions
    )
    avg_execution_time = sum(r.get("execution_time_ms", 0) for r in results) / max(
        1, successful_executions
    )
    total_knowledge_chunks = sum(r.get("knowledge_chunks_used", 0) for r in results)

    performance_compliant = sum(
        1 for r in results if r.get("performance_target_met", False)
    )
    ...
    print(
        f"Average enhancement time: {enhancer_stats['avg_enhancement_time_ms']:.1f}ms"
    )
