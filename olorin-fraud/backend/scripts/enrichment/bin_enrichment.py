...
def enrich_transactions_bin(transactions: List[Dict[str, Any]]) -> None:
    ...
    with get_db_session() as db:
        for txn in transactions:
            ...
            bin_number = (
                str(card_id)[:8] if len(str(card_id)) >= 8 else str(card_id)[:6]
            )
            ...
            if bin_data:
                ...
                issuer_geo_mismatch = (
                    merchant_country
                    and issuer_country
                    and merchant_country != issuer_country
                )
        ...
        db.commit()
        logger.info(
            f"BIN enrichment complete: {enriched_count}/{len(transactions)} transactions enriched"
        )
