...
class StructuredWorkflowValidator:
    ...
    async def _start_investigation(self):
        ...
        logger.info(
            f"🚀 Starting investigation for {self.entity_type}: {self.entity_id}"
        )
            ...
            try:
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        plain_text = response_data.get("agentOutput", {}).get(
                            "plainText", ""
                        )
                        self.investigation_id = self._extract_investigation_id(
                            plain_text
                        )

                        logger.info(
                            f"✅ Investigation started successfully: {self.investigation_id}"
                        )
                        return response_data
                    else:
                        logger.error(
                            f"❌ Failed to start investigation: {response.status}"
                        )
                        raise Exception(
                            f"Investigation start failed: {response.status}"
                        )
    ...
    async def _monitor_websocket_flow(self):
        ...
        logger.info(
            f"🔗 Connecting to WebSocket for investigation: {self.investigation_id}"
        )
            ...
            async with websockets.connect(ws_url, timeout=30) as websocket:
                ...
                while time.time() - start_time < timeout_seconds:
                    try:
                        ...
                        logger.info(
                            f"📡 [{phase.upper()}] {progress*100:.1f}% - {message_text}"
                        )
                        ...
                        logger.debug(
                            "⏱️ WebSocket message timeout - continuing to wait..."
                        )
                        continue
        ...
        if isinstance(data, dict):
            ...
            for key, value in data.items():
                if any(
                    ...
                    for indicator in ["trace", "id", "timestamp"]
                ):
            ...
            if "agentOutput" in data and "plainText" in data["agentOutput"]:
                ...
                if len(text) > 50 and not any(
                    mock_word in text.lower()
                    for mock_word in ["mock", "demo", "test", "example"]
                ):
                    return True
    ...
    def _validate_llm_usage(self):
        ...
        for response in self.llm_responses:
            ...
            if any(
                ...
                for field in ["confidence", "reasoning", "analysis", "risk_score"]
            ):
                self.validation_results["llm_validation"]["real_llm_confirmed"] = True
                self.validation_results["llm_validation"][
                    "response_quality_indicators"
                ].append(f"Phase {response['phase']}: Contains LLM analysis fields")
    ...
    def _generate_validation_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "workflow_validation": {
                ...
                "investigation_completed": any(
                    msg["data"].get("phase") == "completed"
        ...
        report["overall_status"] = (
            "PASSED" if (api_success and llm_success and workflow_success) else "FAILED"
        )
...
async def main():
    ...
    try:
        ...
        logger.info(
            f"Investigation ID: {report['test_configuration']['investigation_id']}"
        )
        logger.info(
            f"API Calls Made: {report['workflow_validation']['rest_api_calls']}"
        )
        logger.info(
            f"WebSocket Messages: {report['workflow_validation']['websocket_messages']}"
        )
        logger.info(f"LLM Responses: {report['workflow_validation']['llm_responses']}")
        logger.info(
            f"Investigation Completed: {report['workflow_validation']['investigation_completed']}"
        )
