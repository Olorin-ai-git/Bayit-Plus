    ...
    try:
        ...
        try:
            ...
            test_results["validation_results"]["enhanced_tool_executor"] = {
                ...
                "tools_tracked": (
                    len(enhanced_node.health_manager.health_checks)
                    if has_health_manager
                    else 0
                ),
            ...
            if has_health_manager:
                logger.info(
                    f"✅ Enhanced Tool Executor: Health manager initialized with {len(enhanced_node.health_manager.health_checks)} tools"
                )
            else:
        ...
        try:
            ...
            if not missing_fields:
                logger.info(
                    f"✅ State Management: All required fields present, starting in '{state.get('current_phase')}' phase"
                )
            else:
        ...
        try:
            ...
            logger.info(
                f"✅ Hybrid Graph Builder: Created successfully with intelligence mode '{builder.intelligence_mode}'"
            )
        ...
        if passed_tests == total_tests:
            ...
            logger.warning(
                f"⚠️ PARTIAL SUCCESS: {total_tests - passed_tests} tests failed"
            )
