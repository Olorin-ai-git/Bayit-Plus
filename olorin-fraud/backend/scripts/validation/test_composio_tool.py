...
async def test_composio_tool():
    ...
    try:
        ...
        with get_db_session() as db:
            ...
            if not existing:
                print(
                    f"🔧 Creating test ComposioConnection: tenant_id={test_tenant_id}, connection_id={test_connection_id}"
                )
    ...
    try:
        ...
        if result_dict.get("status") == "success":
            ...
            print(
                f"⚠️ Expected connection error (Composio SDK not configured): {result_dict.get('error')}"
            )
        elif "authentication" in result_dict.get("error", "").lower():
            print(
                f"⚠️ Expected authentication error (Composio credentials not configured): {result_dict.get('error')}"
            )
        else:
