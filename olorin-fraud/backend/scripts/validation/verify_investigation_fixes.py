...
async def run_investigation_test():
    ...
    try:
        ...
        print(
            f"✅ Initial state created: {state['entity_type']} - {state['entity_id']}"
        )
        ...
        if results:
            for i, result in enumerate(results):
                ...
                print(
                    f"   Record {i+1}: MODEL_SCORE={model_score}, IS_FRAUD_TX={is_fraud}"
                )
        ...
        mock_json_content = json.dumps(
            {"results": results, "row_count": len(results), "query_status": "success"}
        )

        # This is the critical fix - parse JSON string to dict
        parsed_snowflake_data = (
            json.loads(mock_json_content)
            if isinstance(mock_json_content, str)
            else mock_json_content
        )

        print(
            f"✅ JSON parsing works: {type(parsed_snowflake_data)} with {len(parsed_snowflake_data['results'])} results"
        )
        ...
        if parsed_snowflake_data and "results" in parsed_snowflake_data:
            ...
            model_scores = [
                float(r.get("MODEL_SCORE", 0)) for r in results if "MODEL_SCORE" in r
            ]
            ...
            if model_scores:
                ...
                print(
                    f"✅ Risk calculation works: MODEL_SCORE average={avg_model_score:.4f} → Risk Score={risk_score:.4f}"
                )
