...
internal fun DevicePairingScreen(uiState: DevicePairingUiState, onGenerateCode: () -> Unit, onRemoveDevice: (String) -> Unit, onNavigateBack: () -> Unit, onRetry: () -> Unit, modifier: Modifier = Modifier) {
    Column(modifier.fillMaxSize()) {
        GlassTopBar(bayitString("security.devices.title"))
        when (uiState) {
            ...
            is DevicePairingUiState.PairingCodeReady -> Column(modifier = Modifier.fillMaxSize().padding(DesignTokens.Spacing.base), verticalArrangement = Arrangement.Center, horizontalAlignment = Alignment.CenterHorizontally) {
                GlassCard(Modifier.fillMaxWidth()) {
                    Column(horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.spacedBy(DesignTokens.Spacing.md)) {
                        Text(bayitString("security.devices.pairingCode"), style = MaterialTheme.typography.titleLarge, fontWeight = FontWeight.Bold)
                        ...
                        Text(bayitString("security.devices.enterCodeOnTv"), color = DesignTokens.Colors.Text.muted)
                    }
                }
            }
            is DevicePairingUiState.Success -> LazyColumn(modifier = Modifier.fillMaxSize().padding(horizontal = DesignTokens.Spacing.base), verticalArrangement = Arrangement.spacedBy(DesignTokens.Spacing.sm)) {
                item { Spacer(Modifier.height(DesignTokens.Spacing.sm)); GlassButton(bayitString("security.devices.pairNewDevice"), onGenerateCode, Modifier.fillMaxWidth()); Spacer(Modifier.height(DesignTokens.Spacing.sm)) }
                if (uiState.devices.isEmpty()) {
                    item { Box(modifier = Modifier.fillMaxWidth().padding(DesignTokens.Spacing.xxl), contentAlignment = Alignment.Center) { Text(bayitString("security.devices.empty"), color = DesignTokens.Colors.Text.muted) } }
                }
                    ...
                    GlassCard(Modifier.fillMaxWidth()) {
                        Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.CenterVertically) {
                            Column(Modifier.weight(1f)) {
                                ...
                                Text(bayitString("security.devices.lastActiveNa"), fontSize = DesignTokens.FontSize.sm, color = DesignTokens.Colors.Text.muted)
                            }
                            GlassButton(bayitString("security.devices.remove"), { onRemoveDevice(device.hashCode().toString()) }, Modifier, enabled = false)
                        }
                    }
                }
            }
