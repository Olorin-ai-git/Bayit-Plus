...
class LLMSearchTest {
    ...
    private val feature = DiscoverFeatureCatalog.featureById("llm_search")!!

    @Test
    fun llmSearch_isOnAndroid() {
        assert(DiscoverPlatform.ANDROID in feature.platforms)
    }

    @Test
    fun llmSearch_hasNoPrerequisites() {
        assert(feature.prerequisites.isEmpty())
    }

    @Test
    fun llmSearch_ready_detailSheet_displaysDescription() {
    ...
    @Test
    fun llmSearch_ready_availabilityBadge_isReady() {
        composeTestRule.setContent {
            DiscoverFeatureDetailSheet(
                feature = feature,
                availability = FeatureAvailabilityState.Ready,
    ...
    @Test
    fun llmSearch_ready_walkthroughButton_fires() {
        var started = false
