...
class VODMomentsTest {

    @get:Rule
    val composeTestRule = createComposeRule()

    private val feature = DiscoverFeatureCatalog.featureById("vod_moments")!!

    @Test
    fun vodMoments_isIosOnlyInCatalog() {
        assert(DiscoverPlatform.ANDROID !in feature.platforms)
        assert(DiscoverPlatform.IOS in feature.platforms)
    }

    @Test
    fun vodMoments_platformOnly_badgeDisplayed() {
    ...
    @Test
    fun vodMoments_detailSheet_displaysDescription() {
        composeTestRule.setContent {
            DiscoverFeatureDetailSheet(
                feature = feature,
                availability = FeatureAvailabilityState.PlatformOnly(DiscoverPlatform.IOS),
    ...
    @Test
    fun vodMoments_platformOnly_noWalkthroughButton() {
        composeTestRule.setContent {
            DiscoverFeatureDetailSheet(
                feature = feature,
                availability = FeatureAvailabilityState.PlatformOnly(DiscoverPlatform.IOS),
