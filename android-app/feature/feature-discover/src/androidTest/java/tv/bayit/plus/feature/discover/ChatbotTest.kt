...
class ChatbotTest {
    ...
    private val feature = DiscoverFeatureCatalog.featureById("chatbot")!!

    @Test
    fun chatbot_isOnAndroid() {
        assert(DiscoverPlatform.ANDROID in feature.platforms)
    }

    @Test
    fun chatbot_hasNoPrerequisites() {
        assert(feature.prerequisites.isEmpty())
    }

    @Test
    fun chatbot_ready_detailSheet_displaysDescription() {
    ...
    @Test
    fun chatbot_ready_availabilityBadge_isReady() {
        composeTestRule.setContent {
            DiscoverFeatureDetailSheet(
                feature = feature,
                availability = FeatureAvailabilityState.Ready,
    ...
    @Test
    fun chatbot_ready_walkthroughButton_fires() {
        var started = false
