...
import { AGENT_COLORS, ANIMATION_TIMING, INITIAL_LOGS_COUNT, SIDEBAR_DIMENSIONS } from '../constants/definitions';
...
const Caret: React.FC = () => {
    ...
    useEffect(() => {
        const timer = setTimeout(() => setIsVisible(false), ANIMATION_TIMING.CARET_DURATION);
        return () => clearTimeout(timer);
    }, []);
 ...
 */
const AgentLogSidebar: React.FC<AgentLogSidebarProps> = ({ isOpen, onClose, logs, onClearLogs, onCopyLogs, cancelledRef, onLogDisplayed }) => {
    ...
    const [fadeState, setFadeState] = useState(isOpen ? 'slide-fade-in' : 'slide-fade-out');
    ...
    useEffect(() => {
        if (logs.length === INITIAL_LOGS_COUNT && (logs[0].message.includes('Initializing') || logs[0].message.includes('Investigation started'))) {
    ...
    useEffect(() => {
        ...
        const handleMouseMove = (e: MouseEvent) => {
            ...
            const newWidth = Math.max(SIDEBAR_DIMENSIONS.MIN_WIDTH, Math.min(SIDEBAR_DIMENSIONS.MAX_WIDTH, startWidthRef.current - dx));
     ...
     * @returns {string} The Tailwind CSS color class
     */
    const getAgentColor = (agentName: string): string => AGENT_COLORS[agentName] || 'text-gray-800';
    ...
    return (
        <>
            ...
            <div
                ...
                className={`w-72 flex flex-col h-full bg-white shadow-lg z-40 relative ${isDragging ? '' : 'transition-all duration-500 ease-in-out'} ${fadeState}`}
            ...
            >
                ...
                <div className="flex justify-between items-center p-4 border-b border-gray-200">
                    <h2 className="text-xl font-semibold text-gray-800">Agent Activity Log</h2>
                    <div className="flex gap-2 relative">
                        <div className="relative sidebar-btn">
                            ...
                            <span className="sidebar-tooltip" role="tooltip">Copy logs</span>
                            {copied && (
                                <span className="sidebar-tooltip" style={{bottom: '-2.2rem', background: '#16a34a'}} role="status">Copied!</span>
                            )}
                        </div>
                        <div className="relative sidebar-btn">
                            <button
                            ...
                            <span className="sidebar-tooltip" role="tooltip">Clear Logs</span>
                        </div>
                        <div className="relative sidebar-btn">
                            <button
                                type="button"
                            ...
                            <span className="sidebar-tooltip" role="tooltip">Close sidebar</span>
                        </div>
                    </div>
                </div>
                <div 
                ...
                >
                    {[...visibleLogs].reverse().map((log) => (
                        <div key={`${log.timestamp}-${log.message}`} className="mb-3 last:mb-0">
                            <div className="text-xs text-black-500 mb-1">
                                {new Date(log.timestamp).toLocaleTimeString()}
                            </div>
                            <div className={`text-xs ${getLogTypeColor(log.type)} flex items-baseline break-all`}>
                                <AnimatedText 
