...
export class InvestigationService extends BaseApiService {
   ...
   * - Lifecycle management (IN_PROGRESS → COMPLETED/FAILED)
  ...
  async getInvestigation(id: string): Promise<Investigation> {
    ...
    if (pendingRequest) {
      console.log('🔄 [InvestigationService.getInvestigation] Reusing pending request for ID:', id);
  ...
  async getProgress(id: string, etag?: string | null): Promise<{ data: import('../../../shared/types/investigation').InvestigationProgress | null; etag: string | null }> {
    ...
    if (pendingRequest) {
      console.log('🔄 [InvestigationService.getProgress] Reusing pending request for ID:', id);
      return pendingRequest;
    }
    ...
    const fetchPromise = (async () => {
      try {
        console.log('🚨🚨🚨 [InvestigationService.getProgress] Called with ID:', id, 'ETag:', etag);
        console.log('🚨🚨🚨 [InvestigationService.getProgress] Full endpoint:', `${this.baseEndpoint}/${id}`);
      ...
      if (response.status === 304) {
        console.log('🚨🚨🚨 [InvestigationService.getProgress] 304 Not Modified - no changes');
      ...
      console.log('🚨🚨🚨 [InvestigationService] Raw backend response:', investigationState);
      console.log('🚨🚨🚨 [InvestigationService] Raw response keys:', Object.keys(investigationState || {}));
      console.log('🚨🚨🚨 [InvestigationService] investigationId check:', {
      ...
      console.log('🚨🚨🚨 [InvestigationService] Progress data keys:', Object.keys(progressData || {}));
      console.log('🚨🚨🚨 [InvestigationService] Domain findings check:', {
      ...
      // Handle toolExecutions - BaseApiService transforms tool_executions → toolExecutions
      // Backend may provide:
      // 1. tool_executions (array of objects) → toolExecutions (camelCase)
      // 2. tools_executed (array of strings) → toolsExecuted (camelCase)
      ...
      // BaseApiService transforms tools_executed → toolsExecuted
      if (toolExecutions.length === 0) {
        ...
        if (toolsExecuted && Array.isArray(toolsExecuted) && toolsExecuted.length > 0) {
          ...
          console.log('🚨🚨🚨 [InvestigationService] Using toolsExecuted array:', {
      ...
      console.log('🚨🚨🚨 [InvestigationService] Tool extraction:', {
      ...
      // BaseApiService transforms investigation_id → investigationId, so check camelCase FIRST
      const investigationId = investigationState.investigationId || investigationState.investigation_id || investigationState.id;
      
      if (!investigationId) {
        console.error('🚨🚨🚨 [InvestigationService] CRITICAL: No investigation ID found!', {
      ...
      const backendData = {
        ...
        // CRITICAL: Extract domainFindings from progress object (BaseApiService transforms domain_findings → domainFindings)
      ...
      console.log('🚨🚨🚨 [InvestigationService] Backend data prepared:', {
      ...
      console.log('🚨🚨🚨 [InvestigationService] After transformation:', {
        ...
        console.error('🚨🚨🚨 [InvestigationService] Error in getProgress:', error);
        throw error;
      } finally {
        // Cleanup: Remove from maps when request completes
