...
export default function SubscribeScreen() {
  ...
  const handleSubscribe = useCallback(async () => {
    ...
    try {
      ...
      // For TV, show QR code or redirect to mobile app
      if (Platform.OS === 'web' && response.checkoutUrl) {
        window.location.href = response.checkoutUrl;
      } else {
        // Show QR code modal for native TV apps
        logger.info('Checkout created', 'SubscribeScreen', { url: response.checkoutUrl });
      }
    } catch (error) {
