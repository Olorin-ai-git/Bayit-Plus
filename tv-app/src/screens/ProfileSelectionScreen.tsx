...
const ProfileCard: React.FC<ProfileCardProps> = ({
  ...
  return (
    <TouchableOpacity
      className={`items-center ${isSelected ? 'opacity-70' : ''} ${isManageMode ? 'opacity-90' : ''}`}
    ...
    >
      <View
        ...
        style={[
          { width: PROFILE_SIZE, height: PROFILE_SIZE, backgroundColor: profile.avatar_color || AVATAR_COLORS[0] },
        ]}
      >
        {profile.avatar ? (
        ...
        {profile.is_kids_profile && (
          <View className="absolute -bottom-1 -right-1 bg-yellow-400 rounded-xl p-1">
            <Text className="text-xs"></Text>
          </View>
        )}
        {profile.has_pin && (
          <View className="absolute -top-1 -right-1 bg-black/60 rounded-[10px] p-1">
      ...
      {isManageMode && (
        <View className="absolute justify-center items-center bg-black/50 rounded-xl" style={{ top: 0, left: (PROFILE_SIZE + 20 - PROFILE_SIZE) / 2, width: PROFILE_SIZE, height: PROFILE_SIZE }}>
...
const PinModal: React.FC<PinModalProps> = ({
  ...
  return (
    <View className="absolute inset-0 bg-black/80 justify-center items-center">
      <GlassView intensity="high" className="w-80 p-6 rounded-2xl items-center">
        <Text className="text-xl font-semibold text-white mb-5">{t('profiles.enterPin', 'Enter PIN')}</Text>
        <TextInput
          ref={inputRef}
          className="w-full h-14 bg-white/10 rounded-lg px-4 text-2xl text-white text-center mb-4"
        ...
        <View className="flex-row gap-3 w-full">
          ...
          <TouchableOpacity
            className={`flex-1 h-11 justify-center items-center rounded-lg bg-purple-500 ${pin.length < 4 ? 'opacity-50' : ''}`}
...
export const ProfileSelectionScreen: React.FC = () => {
  ...
  return (
    <View className="flex-1 bg-[#0a0a1a]">
      {/* Background */}
      <View className="absolute inset-0 bg-purple-700/10" pointerEvents="none" />
      ...
      >
        ...
        {/* Profiles Grid */}
        <View className="flex-wrap justify-center items-start max-w-[700px] gap-6" style={{ flexDirection: isRTL ? 'row-reverse' : 'row' }}>
          ...
          {canAddProfile && (
            <TouchableOpacity
              ...
              activeOpacity={0.7}
            >
              <View className="justify-center items-center mb-3 bg-white/10 border-2 border-white/20 border-dashed rounded-xl" style={{ width: PROFILE_SIZE, height: PROFILE_SIZE }}>
