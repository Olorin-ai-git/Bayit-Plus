...
export const apiSubtitlesService = {
  ...
  generateEngrew: (
    ...
    api.post(`/subtitles/${contentId}/engrew`, null, {
      params: { language, force },
    }),
  generateGrammarFlip: (
    contentId: string,
  ...
  getJobStatus: (jobId: string) => api.get(`/subtitles/job/${jobId}`),
  cancelJob: (jobId: string) => api.post(`/subtitles/job/${jobId}/cancel`),
  ...
  }> => api.post("/subtitles/opensubtitles/search", params),
};
...
export const apiChaptersService = {
  ...
  getLiveChapters: (channelId: string) =>
    api.get(`/chapters/live/${channelId}`),
