...
export const apiChatService = {
  ...
  resolveContent: (
    ...
    language: string = "he",
  ): Promise<ResolveContentResponse> =>
    api.post("/chat/resolve-content", { items, language }),
  searchUsers: (
    name: string,
  ): Promise<{ users: Array<{ id: string; name: string; avatar?: string }> }> =>
...
export const apiDownloadsService = {
  ...
  deleteDownload: (downloadId: string) =>
    api.delete(`/downloads/${downloadId}`),
  checkDownload: (
    contentId: string,
  ): Promise<{ is_downloaded: boolean; download_id?: string }> =>
    api.get(`/downloads/check/${contentId}`),
  pauseDownload: (downloadId: string): Promise<void> =>
    api.post(`/downloads/${downloadId}/pause`),
  resumeDownload: (downloadId: string): Promise<void> =>
    api.post(`/downloads/${downloadId}/resume`),
};
