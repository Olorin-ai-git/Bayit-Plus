...
class StreamingVoicePipeline extends EventEmitter<StreamingVoicePipelineEvents> {
  ...
  private getWsEndpoint(): string {
    return buildWsUrl("/api/v1/support/voice");
  }
  ...
  async restartListening(): Promise<void> {
    if (!this.isConnected) {
      log.warn("Cannot restart listening - not connected");
      return;
    }
    if (this.isRecording) {
  ...
  commit(reason: "silence" | "button" = "button"): void {
    this.wsHandler.send({ type: "commit", reason });
  }
  cancel(): void {
    this.wsHandler.send({ type: "cancel" });
    this.audioPlayer?.stop();
    this.emitStateChange("idle");
  }
  ...
  private emitError(error: Error): void {
    ...
    setTimeout(() => this.emitStateChange("idle"), 3000);
  }
  ...
  isAudioPlaying(): boolean {
    return this.audioPlayer?.isCurrentlyPlaying() ?? false;
  }
  isSupported(): boolean {
