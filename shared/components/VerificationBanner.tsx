...
export const VerificationBanner: React.FC<VerificationBannerProps> = ({
  ...
  const getVerificationMessage = () => {
    ...
    if (!emailVerified && !phoneVerified) {
      return 'Verify your email and phone to unlock all features';
    }
    if (!emailVerified) {
      return 'Verify your email to continue';
    }
    if (!phoneVerified) {
      return 'Verify your phone to unlock all features';
    }
      ...
      <GlassView intensity="medium" className="w-full py-4 px-6 border-b border-purple-600/25">
        <View className="flex-row items-center justify-between gap-4">
          <View className="flex-1 gap-1 flex-row items-center">
            <NativeIcon name="alertTriangle" size={20} color="#fbbf24" style={{ marginRight: spacing.xs }} />
          ...
          <View className="flex-row items-center gap-2">
            ...
            <Pressable onPress={handleDismiss} className="w-7 h-7 rounded-full bg-black/40 items-center justify-center">
              <NativeIcon name="x" size={18} color="rgba(255, 255, 255, 0.6)" />
            </Pressable>
          </View>
        </View>
      </GlassView>
