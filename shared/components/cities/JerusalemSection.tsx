...
interface JerusalemSectionProps {
 ...
 * Jerusalem Section Component
 */
export const JerusalemSection: React.FC<JerusalemSectionProps> = ({
  onItemPress,
  category,
  showTitle = true,
}) => {
  ...
  const fetchContent = async () => {
    try {
      ...
      logger.info('Jerusalem content loaded', { itemCount: result.data?.items?.length || 0 });
    } catch (err) {
      const errorMsg = err instanceof Error ? err.message : 'Unknown error';
      logger.error('Failed to fetch Jerusalem content', { error: errorMsg });
      setError(errorMsg);
    } finally {
      setLoading(false);
    }
...
export default JerusalemSection;
