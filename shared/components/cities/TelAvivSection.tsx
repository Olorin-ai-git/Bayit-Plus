/**
 * Tel Aviv Section Component
 *
 * Displays Tel Aviv-focused content from Israeli news sources.
 * Built from scratch with React Native StyleSheet, Glass components, theme colors, and i18n.
 *
 * Features:
 * - Beaches, nightlife, culture, music, food, tech, events
...
const TELAVIV_COLOR = '#f97316'; // orange-500
const TELAVIV_COLOR_LIGHT = '#fb923c'; // orange-400
const TELAVIV_COLOR_DARK = '#ea580c'; // orange-600

/**
 * Tel Aviv Section Component
 */
export const TelAvivSection: React.FC<TelAvivSectionProps> = ({
  onItemPress,
  category,
  showTitle = true,
}) => {
  ...
  // Get category label
  const getCategoryLabel = (item: TelAvivContentItem): string => {
    const categoryKey = `cities.telAviv.categories.${item.category}`;
          ...
          <View style={[styles.content, isMobilePhone && styles.contentMobile]}>
            ...
            <View style={[styles.emptyContainer, isMobilePhone && styles.emptyContainerMobile]}>
              <Text style={[styles.emptyText, isMobilePhone && styles.emptyTextMobile]}>
                {error ? t('common.error') : t('cities.telAviv.noContent')}
              </Text>
            </View>
          </View>
        </ImageBackground>
...
// Styles (Tel Aviv theme with orange accents)
const styles = StyleSheet.create({
  ...
  badge: {
    backgroundColor: `${TELAVIV_COLOR}4D`,
    borderColor: `${TELAVIV_COLOR}99`,
  },
  badgeMobile: {
  ...
  scoreDotActive: {
    backgroundColor: TELAVIV_COLOR,
  },
});

export default TelAvivSection;
